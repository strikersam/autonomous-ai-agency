"""packages/config/control_registry.py — operator-facing platform controls.

The declarative catalogue of every feature switch and multi-option setting an
operator is expected to change. It exists so those choices can be made from the
dashboard instead of hand-editing ~100 ``true``/``false`` rows in the Render
environment editor and redeploying.
...
environment-only per the repository constitution, and this registry is served to
the dashboard.
