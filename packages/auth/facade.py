...
#
# IMPORTANT: the underlying implementations are `async def`, so the facade
# wrappers here MUST also be `async def` and await the implementation —
# otherwise callers receive a coroutine object instead of the result.
...
# OAuth helpers (re-export from packages/auth/oauth.py)
async def github_exchange_code(code: str):
    from packages.auth.oauth import github_exchange_code as _impl
    return await _impl(code)


async def github_fetch_user(access_token: str):
    from packages.auth.oauth import github_fetch_user as _impl
    return await _impl(access_token)


async def google_exchange_code(code: str, redirect_uri: str | None = None):
    from packages.auth.oauth import google_exchange_code as _impl
    return await _impl(code, redirect_uri)


async def google_fetch_user(access_token: str):
