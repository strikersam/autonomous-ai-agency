...
from agent.user_memory import get_user_memory_store
...
async def _handle_agent_mode(
    ...
    async def _run_agent_job(heartbeat):
        ...
        result = await runner.run(metadata=spec.context.get("metadata", {}), instruction=req.content, history=history, requested_model=req.model, auto_commit=True, max_steps=25, user_id=user.id, department=None, key_id=None, memory_store=get_user_memory_store(), session_id=session_id)
