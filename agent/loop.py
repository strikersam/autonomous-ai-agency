...
class AgentRunner:
    ...
    async def _chat_text(self, model: str, messages: list[dict[str, str]]) -> str:
        ...
        # Emit Langfuse observation + record token spend for budget enforcement
        usage_data = data.get("usage", {})
        pt = int(usage_data.get("prompt_tokens") or 0)
        ct = int(usage_data.get("completion_tokens") or 0)
        if self.email:
            try:
                from langfuse_obs import emit_chat_observation
                await asyncio.to_thread(
