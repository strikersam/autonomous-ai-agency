...
async def _timed_phase(phase: str, **context: Any) -> AsyncIterator[None]:
    ...
    _note_phase("start", phase, label)
    try:
        yield
    finally:
        _note_phase("end", phase, label)
        log.info("phase_end %s %s elapsed_s=%.1f", phase, label, time.perf_counter() - start)


def _note_phase(edge: str, phase: str, label: str) -> None:
    """Forward a phase boundary to the incident tracker. Never raises."""
    try:
        from agent import operational_incidents as _ops

        (_ops.note_phase_start if edge == "start" else _ops.note_phase_end)(phase, label)
    except Exception:  # noqa: BLE001 - instrumentation must never break a run
        log.debug("phase tracking unavailable for %s/%s", phase, edge)
