...
- **Four pre-existing CI-blocking bugs on `master`, unrelated to the above, fixed while making this PR mergeable:** (1) `.github/scripts/implement_agent.py` had 2968 trailing NUL bytes appended after its valid `main()` body, which made `python -m py_compile` fail with `SyntaxError: source code string cannot contain null bytes` in the "Test (Python 3.13)" syntax-check step — the trailing NULs were stripped. (2) `frontend/src/v5/screens/CompanyScreen.jsx` was truncated mid-statement (`exp` instead of `export default CompanyScreen;`), so `V5App.jsx`'s `import CompanyScreen from './screens/CompanyScreen'` resolved to no default export and `npm run build` (and the Docker-based Playwright E2E build) failed — completed the statement. (3) `proxy.py`'s `/v1/models` alias entries used `"owned_by": "llm-relay-alias"`, a leftover from the pre-rename project name, while `tests/test_daily_automation_2026_05_14.py::TestModelsEndpointAliases::test_list_models_includes_alias_entries` asserts `"owned_by": "autonomous-ai-agency-alias"` — updated the literal to match the project's current name and the test's documented contract. (4) `tests/test_brain_priority_scanner.py::test_scanner_imports_cleanly` did `sys.modules.pop("services.scanner", None)` then re-imported it, creating a second `WebsiteScanner` class object distinct from the one `services/onboarding.py` captured via `from services.scanner import WebsiteScanner` at its own import time. Any later test's `monkeypatch.setattr(scanner_mod.WebsiteScanner, "scan_website", ...)` then patched the *new* class while `OnboardingService._scan_website` kept calling the *old* (real, network-hitting) one — causing all 9 tests in `tests/test_onboarding_provisioning.py` to silently fall back to "no systems detected" when run after `test_brain_priority_scanner.py` in a full suite. Removed the `sys.modules.pop`/re-import.
