...
### Added
- **README: 24x7 autonomous operations section.** New dedicated section explains how agents stay alive post-onboarding — crash-recovery reconciler, Wake All Runtimes, AI runner watchdog, provider failover. Mermaid diagram updated with the continuous loop. Changelog removed from README body; all history consolidated in `docs/changelog.md`. Sections reorganised: context, features, 24x7 ops, setup, architecture, configuration, security, development, roadmap.
- **README: Phase 9 — AI onboarding, Quick Notes, skill registry.** Documents AI-tailored onboarding questions endpoint, Quick Note processor (iPhone URL-initiated tasks), skill registry ETag caching + semaphore rate limiting, Doctor per-check Fix buttons, and agent task stats wiring. All existing screenshots preserved.
