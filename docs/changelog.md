...
- **FreeBuff rate-limit exemption (`proxy.is_rate_limit_exempt`).** Phone-driven free-NVIDIA agent runs can be exempted from the per-key RPM limiter via `FREEBUFF_RATELIMIT_EXEMPT_KEY_IDS` (comma-separated key_ids). Opt-in and narrowly scoped — default empty (no key exempt), legacy keys (no `key_id`) never exempt, so paid/general endpoints stay protected.
