...
- **B1 Nemotron Reward Model Scoring (`services/reward_scorer.py`).** RewardScorer class evaluates agent step outputs via NVIDIA NIM's Nemotron-4-340B-Reward model — a cheaper/faster alternative to the LLM verifier. Scores on 0.0–1.0 scale; high-confidence scores skip the LLM verifier entirely. Falls back gracefully when NVIDIA_API_KEY is not configured. Integrated into agent/loop.py verifier phase with NEMOTRON_REWARD_ENABLED toggle.
- **C2 Function Calling / Tool Use — OpenAI-compatible (`chat_handlers.py`).** _parse_tool_calls_from_response parses tool calls from JSON arrays, markdown fences, and function-call format. _normalize_tool_choice translates OpenAI tool_choice (none/auto/required/specific) into system-prompt instructions for Ollama/local models. _inject_tool_results_as_messages supports multi-turn tool execution by injecting assistant+tool messages into the conversation.
- **A3 Agent Capability Registry + Dynamic Tool Discovery (`agent/capability_registry.py`).** ToolRegistry with @agent_tool decorator for auto-registration, JSON Schema advertising, capability-based discovery (find_by_capability, negotiate), hot-reload, and OpenAI tool export. Six built-in tools (read_file, write_file, search_code, list_files, apply_diff, finish) auto-registered. Registry-based dispatch integrated into AgentRunner._dispatch_tool as a fast-path before the legacy hardcoded chain.
- **P0 roadmap test suite (`tests/test_p0_roadmap_b1_c2_a3.py`).** 35+ tests: RewardScore validation, RewardScorer availability/parsing, tool call parsing (JSON/fence/function formats), tool_choice normalization (5 modes), tool result injection, ToolDef export, ToolRegistry CRUD + search + capabilities + negotiation, @agent_tool decorator, builtin tools singleton. All syntax-valid + importable (full execution requires pymongo/bson environment).
- **★3 Reasoning Token Budget + Toggle (`agent/loop.py`, `chat_handlers.py`).** AGENT_REASONING_BUDGET env var (low=512, medium=2048, high=8192, max=unbounded) injects thinking_token_budget into LLM payloads for DeepSeek-R1, Qwen3-Coder, and Nemotron NIM models. <think> blocks are extracted as reasoning_content in chat_handlers.py before think-tag stripping. vLLM backend registered in router/registry.py as a first-class target alongside Ollama.
- **A2 Multi-Hop ReAct Loop (`agent/react_loop.py`).** ReactScratchpad accumulates thought→action→observation traces across tool calls within a step. Scratchpad context is injected into the executor's tool prompt so the model can see its own reasoning chain. build_react_prompt() and parse_react_response() exported for WorkflowOrchestrator agents that prefer ReAct format over JSON tool calls.
- **P0 roadmap test suite (`tests/test_p0_roadmap_implementation.py`).** 24 tests covering SubAgentConfig validation, reasoning budget mapping, ReactScratchpad recording/truncation/serialisation, react response parsing, prompt building, and vLLM backend registration. All tests pass; zero regressions in existing agent test suite (26 tests).
...
### Added
- **Killer TODO Roadmap — 33-item implementation backlog from 6 leading OSS projects (`docs/roadmap-killer-todos.md`).** Comprehensive roadmap derived from deep analysis of hermes-agent, agentic-os, Nemotron, OpenMythos, codebuff, and companyhelm — cross-referenced against the current codebase. 33 items organized across 8 sections (A-H) plus 7 highest-ROI TOP items. Each item scoped to be implementable without architectural rewrites, with priority (P0-P3), source project reference, and specific files to create/modify. The TOP 7 highest-ROI items are: 3-Phase Context-Pruner Middleware, Specialized Sub-Agents, Reasoning Token Budget, Skill/Procedural Memory, Sandboxed Agent Execution, Cost Analytics + FTS5 Memory, and Adaptive Loop Halting.
- **Dynamic Session Planning Workflow — mandatory 10-step process in AGENTS.md.** Every non-trivial agentic session across all AI tools (Claude Code, Codex, Cursor, Aider, etc.) must follow READ→ORIENT→SKILLS→RESEARCH→PLAN→TRACK→PR→CONFIRM→EXECUTE→CLOSE before writing code. Backed by `.claude/state/active-tasks.md` (living task tracker with TODO/IN_PROGRESS/DONE/BLOCKED/BUG_FOUND/BUG_FIXED statuses) that all agents update continuously.
- **Active task tracker (`.claude/state/active-tasks.md`).** Living document updated by every agent session across all tools. Tracks current sprint tasks, bug log, roadmap items from `docs/roadmap-killer-todos.md`, and session activity log. Rules: mark IN_PROGRESS when starting, DONE when verified, BUG_FOUND/BUG_FIXED for bugs. Never delete rows — append new rows for re-attempts.

### Changed
- **CI workflows optimized to prevent duplicate runs.** `push:` triggers on `browser-e2e.yml`, `ci.yml`, and `e2e.yml` restricted from `["**"]` to `["main", "master"]` so feature-branch pushes don't trigger duplicate CI runs (the `pull_request` trigger already covers those). Added `if: github.event.pull_request.draft == false` guards on the `e2e-mongodb` and `live-scanner-verification` jobs in `e2e.yml` to skip heavy E2E runs on draft PRs.
...
### Fixed
- **FreeBuff Telegram bot crash-looped with `UnboundLocalError: TELEGRAM_BOT_TOKEN`.** A later whitespace-strip reassigns `TELEGRAM_BOT_TOKEN` inside `run_bot()`, which (without a `global` declaration) made the name function-local, so the earlier read raised `UnboundLocalError` and the bot restarted every 30s. Added `TELEGRAM_BOT_TOKEN` to the `global` statement in `run_bot()`. Regression test `test_run_bot_no_unbound_local_for_token`.
