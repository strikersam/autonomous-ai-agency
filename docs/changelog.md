...
- **Reorg: move 18 root modules to packages/ with backward-compat shims** (2026-06-29). Moved key_store + secrets_store → packages/auth/, app_settings + activation_api + activation → packages/config/, langfuse_obs + log_watcher + trend_analysis → packages/telemetry/, admin_gui + output_filter → packages/ui/, commercial_equivalent + infra_cost + tokens + version + audit → packages/shared/, chat_handlers + service_manager + task_runner → packages/orchestration/. All old paths have backward-compat shims. Updated all production + test imports + string-based monkeypatch paths. 284 tests pass.

- **Shim removal: delete all 21 backward-compat shims** (2026-06-29). All 21 backward-compat shim files deleted from root. Every caller now imports directly from packages/. Updated all production imports, test imports, string-based monkeypatch paths, importlib.reload targets, and loop registry source paths. Root .py count dropped from 29 to 8 (all operational scripts: proxy.py, worker_main.py, generate_api_key.py, run-claude-code.py, service_daemon.py, setup_local_models.py, setup_ngrok.py, start_tunnel_simple.py). 352 tests pass.

- **V2.1 Intelligence & Agent Platform Enhancement — Phase 1** (2026-06-29). Created four new platform packages: (1) **packages/tools/** — Unified Tool Platform with abstract Tool interface, ToolRegistry with capability-based discovery, OpenAI function-calling schema export. (2) **packages/intelligence/** — Intelligence Layer with Planner (task decomposition + step-by-step execution plans), Reflector (self-evaluation + quality scoring + retry suggestions). (3) **packages/resilience/** — Provider Resilience with CheckpointManager (save task state mid-execution, resume on different provider without losing work, DB-persisted for cross-process recovery). (4) **packages/knowledge/** — Knowledge Platform with KnowledgeStore (short-term in-process memory + long-term DB-persisted memory, keyword search with relevance ranking, quality scoring + automatic pruning, shared across all agents). Added 4 ADRs (008-011) documenting the architectural decisions. Added 20 tests covering all new capabilities. V2.1 architecture document created at V2.1_INTELLIGENCE_PLATFORM.md.

- **V2.1 Phase 2: Complete remaining intelligence platform capabilities** (2026-06-29). Added: (1) **BrowserTool** (packages/tools/browser.py) — Playwright-based browser automation with navigation, extraction, interaction, screenshot, session reuse. (2) **GitHubTool** (packages/tools/github_tool.py) — GitHub API operations: list repos, read files, create branches/PRs, list issues. (3) **ShellTool** (packages/tools/shell.py) — Safe shell execution with command allowlisting (blocks rm -rf, sudo, etc.), timeout, output capture. (4) **Verifier** (packages/intelligence/verifier.py) — Output verification against requirements: non-empty check, goal relevance, error indicator detection, requirement compliance. (5) **ContextOptimizer** (packages/intelligence/context.py) — Context window management with 3 strategies: truncate (keep system + recent), summarize (replace old with summary), relevance filter (drop irrelevant messages). (6) **SemanticSearch** (packages/knowledge/search.py) — TF-IDF-based search with stop word removal, IDF scoring, no external embeddings needed. (7) **SelfImprover** (packages/intelligence/self_improve.py) — Controlled self-improvement: reviews completed work, suggests prompt/workflow/knowledge/config improvements, all require explicit approval. (8) **OAuth speed fix** — GitHub callback uses find_one_and_delete (1 DB round-trip vs 2), skips /user/emails API call when user has public email, log_activity is fire-and-forget (asyncio.create_task). 40 tests pass. Also fixed root package.json invalid JSON (was breaking CSS Minifier + Render deploy).

- **Fix OAuth + login: revert find_one_and_delete (SQLite doesn't support it) + add find_one_and_delete to SQLiteStore** (2026-06-30). The GitHub OAuth callback used `find_one_and_delete()` which is a MongoDB method not implemented on SQLiteStore. On Render (STORAGE_BACKEND=sqlite), this raised AttributeError → crashed the callback → social login failed. Reverted to separate `find_one` + `delete_one` calls. Also added `find_one_and_delete` to SQLiteStore for future compatibility. Admin login was also affected because Render was in a crash loop from the failed OAuth callback.

- **Root cleanup: move all scripts, docs, and config files to proper directories** (2026-06-30). Moved 40+ root files to their proper locations: Python scripts → scripts/ (generate_api_key, setup_local_models, setup_ngrok, start_tunnel_simple, service_daemon, run_claude_code), shell scripts → scripts/ (run_ollama, run_proxy, run_tunnel, start, start_server, stop_server, stop_proxy, get_tunnel_url, install, launch-claude-code, setup-claude-code, setup_autostart_macos), Windows scripts → scripts/windows/ (all .bat/.ps1/.cmd files), docs → docs/ (AGENTS.md, ARCHITECTURE.md, ENGINEERING_STANDARDS.md, REWRITE_PLAN.md, SECURITY.md, TOOLS.md, V2.1_INTELLIGENCE_PLATFORM.md, CONTRIBUTING.md, design_guidelines.json, pr_body.md, github-pages-*.html), docker-compose files → deploy/ (docker-compose.yml, docker-compose.prod.yml, docker-compose.e2e.yml), static frontend files → frontend/ (index.html, style.css, script.js). Updated all references in README.md, CLAUDE.md, docs/, .github/workflows/, and internal script cross-references. Root now contains only: Dockerfiles, Makefile, package.json, pytest.ini, render.yaml, requirements.txt, wrangler.jsonc, vercel.json, netlify.toml, proxy.py, worker_main.py, and the 3 root .db files.

- **Fix login 502: Cloudflare Worker timeout + OAuth callback routing** (2026-06-30). Root cause: (1) Cloudflare Worker's default fetch timeout is ~30s but Render free tier cold-starts take 50+s → Worker times out → Cloudflare returns 502. (2) OAUTH_REDIRECT_BASE was not set → OAuth callbacks went directly to Render (bypassing the Worker) → Render cold start → timeout → 502. Fix: (1) Worker now uses AbortController with 90s timeout + returns 503 with Retry-After header (instead of generic 502) when Render is cold-starting. (2) Set OAUTH_REDIRECT_BASE to the Cloudflare Worker URL so OAuth callbacks go through the Worker (which has the extended timeout). (3) Frontend now retries on 503 up to 3 times with 3s delay before showing an error.

- **Fix login + Telegram + Render warm-up: 3 root causes** (2026-06-30). (1) **Login 502/timeout**: axios had NO timeout set (default 10s) — Render cold starts take 50+s → axios aborts → "Network Error". Fixed: set 120s timeout. Social login `<a href>` used `backendUrl` (absolute URL to Render) instead of relative path — browser navigated directly to Render (30s timeout → 502). Fixed: use relative paths so navigation goes through the Cloudflare Worker (90s timeout). (2) **Telegram notifications not working**: web service has `TELEGRAM_POLLER_DISABLED=true` (correct — avoids double-poll), but the worker service (`worker_main.py`) never started the Telegram bot — it only started background services. Nobody was polling Telegram. Fixed: added `_bot_supervisor()` to `worker_main.py` that starts + monitors the bot. Also added `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `FREEBUFF_EMBEDDED` env vars to the worker service in render.yaml. (3) **Render cold start on every login**: Worker cron only pinged `/api/scheduler/tick` (heavy endpoint) — didn't keep Render warm. Fixed: Worker cron now pings `/api/health` (lightweight) every minute to prevent Render from sleeping. Also fixed stale repo URL (`strikersam/local-llm-server` → `strikersam/autonomous-ai-agency`) in `FREEBUFF_REPO_URL` and `backend/server.py`.

- **Fix login permanently: OAUTH_REDIRECT_BASE must point to Render, not Worker** (2026-06-30). Root cause: PR #911 set OAUTH_REDIRECT_BASE to the Cloudflare Worker URL. But Cloudflare Workers have a hard 30-second wall-clock limit. Render free tier cold starts take 50+ seconds. So OAuth callbacks went through the Worker → 30s timeout → 502. Before PR #911, OAUTH_REDIRECT_BASE was unset → callbacks went directly to Render (no Worker timeout). Fix: set OAUTH_REDIRECT_BASE to Render's direct URL (https://local-llm-server.onrender.com) so OAuth callbacks bypass the Worker entirely. Also fixed: `import asyncio` inside github_callback was redundant (asyncio already imported as `_asyncio` at module level) — changed to `_asyncio.create_task()`.

- **Fix login permanently: remove OAUTH_REDIRECT_BASE entirely, use request.url_for()** (2026-06-30). Root cause: OAUTH_REDIRECT_BASE was introduced in PR #911 to route OAuth callbacks through the Cloudflare Worker. But: (1) the Worker has a 30s wall-clock limit (Render cold starts take 50+s → 502), and (2) the Worker URL was NOT registered in the GitHub/Google OAuth apps → "redirect_uri not associated with this application". Before PR #911, OAUTH_REDIRECT_BASE was never set — the code used `request.url_for("github_callback")` which generates the Render direct URL (`https://local-llm-server.onrender.com/api/auth/github/callback`) — this IS registered in the OAuth apps and was working perfectly. Fix: removed ALL OAUTH_REDIRECT_BASE usage from github_login, google_login, and google_callback handlers. They now always use `request.url_for()`. Also removed OAUTH_REDIRECT_BASE from render.yaml entirely. The user should also remove it from the Render dashboard environment variables.

- **Fix login: retry on network errors during Render cold start** (2026-06-30). Root cause: Cloudflare Workers have a hard 30-second wall-clock limit. When Render is cold-starting (50+ seconds), the Worker's fetch() is killed by Cloudflare at 30s → the browser gets NO response → axios sees error.response=undefined → fmtErr(null) = "Something went wrong." The AbortController at 90s doesn't help because Cloudflare kills the Worker before it fires. Fix: the frontend axios interceptor now retries on network errors (no response) up to 4 times with increasing delays (3s, 6s, 9s, 12s = 30s total retry window). This gives Render enough time to wake up. The retry covers both cases: (1) Worker returns 503 (our explicit cold-start response), and (2) Worker connection is dropped entirely (Cloudflare kills it at 30s). Also removed OAUTH_REDIRECT_BASE from render.yaml + all backend/server.py OAuth handlers now use request.url_for() (same as before V2.1).

- **Fix login 502: Worker retry strategy (3x10s fetch attempts instead of 1x90s)** (2026-06-30). Root cause: Cloudflare Workers have a hard 30s wall-clock CPU limit. A single fetch() with 90s AbortController doesn't help — Cloudflare kills the Worker at 30s before the abort fires. Fix: Worker now makes 3 fetch attempts with 10s timeout each + 2s delay between attempts (total ~34s). If Render is cold, the first attempt wakes it up, the second or third gets through. All 3 failures → returns 503 (not 502) which the frontend retries. The Playwright test confirmed: login POST returns 502 (not 503) — meaning the old single-fetch-at-90s was being killed by Cloudflare before our catch block ran. The new 3x10s approach stays within the 30s wall-clock limit per attempt.

- **Fix Worker crash (error 1101): clearTimeout scope bug + AbortSignal.timeout incompatibility** (2026-06-30). The Worker was crashing on ALL API requests (error 1101 = Worker threw exception). Root cause: `clearTimeout(timeoutId)` in the catch block referenced a `const` declared in the try block — out of scope → ReferenceError → Worker crash. Also: `AbortSignal.timeout()` in the scheduled function may not exist in the Cloudflare Workers runtime. Fix: rewrote the Worker to be dead simple — single `fetch()` with no AbortController. If fetch fails → return 503. The frontend's axios interceptor (4 retries with 3s/6s/9s/12s delays) handles the cold-start retry.

- **Revert Worker + frontend to PR #896 state — remove ALL retry/timeout changes** (2026-06-30). Reverted worker/index.js to the exact code that was working in PR #896: simple fetch + pass through, no AbortController, no retry loop, no try/catch. Reverted frontend/src/api.js: removed the 120s timeout, removed all cold-start retry logic. The retry logic I introduced in PRs #911-918 caused more problems than it solved (Worker crashes from scope bugs, axios hanging for 120s on dead connections, Playwright showing 0 auth responses). The original simple code worked because the Cloudflare cron pings Render every minute to keep it warm.
