...
### Security
- **CodeQL configuration improved to reduce noise from false positives (`.codeql/codeql-config.yml`, `.github/workflows/codeql.yml`).** Added workspace-level CodeQL configuration that: (1) uses `paths-ignore` to exclude test files, CI scripts, and generated code from scanning; (2) excludes known false-positive queries like `py/hardcoded-sensitive-string` and `js/logger`; (3) references the config file in the CodeQL workflow for consistent application. This addresses the alert volume spike caused by the more thorough `security-extended,security-and-quality` query suites catching patterns in new code.
