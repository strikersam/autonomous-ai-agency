...
### Fixed
- **`GET /api/mcp/servers` 500 in the default (sqlite) config — Providers → MCP Servers tab was broken.** Found in an end-to-end UI test pass. The MCP-server endpoints (`list/create/update/delete`) access `get_db().mcp_servers`, but `mcp_servers` was missing from the `SQLiteStore` collection whitelist (`db/sqlite_store.py::_COLLECTIONS`), so `get_db().mcp_servers` raised `AttributeError: refusing to bind to non-whitelisted table` → HTTP 500 whenever the MCP tab loaded (`STORAGE_BACKEND=sqlite` is the default in `render.yaml`, so this hit production too). An audit of all `get_db().<collection>` accesses surfaced three more unwhitelisted collections read for company tech-stack/workflow inference (`website_scans`, `repo_scans`, `workflows`) which were silently degrading (their `AttributeError` was caught). All four are now whitelisted and indexed, so the MCP tab works and company stack/workflow inference functions in sqlite mode. Regression tests in `tests/test_sqlite_store.py`.
- **Autonomy-paths robustness audit fixes (self-heal + trend-watcher).** Two latent bugs found in an adversarial audit of the merged autonomy code: (1) `agent/self_healing.py::_regress()` re-dispatched a regressed heal as a fire-and-forget task with **no exception handling** while zeroing `_verify_deadline`, so if the re-dispatch raised, the heal was silently stranded in `REGRESSED` forever (never retried, never escalated, never swept) — it now escalates to a human (`AWAITING_HUMAN`) on re-dispatch failure and attaches a done-callback that surfaces task exceptions, closing the G2 self-heal loop. (2) `agent/trend_watcher.py::dispatch_high_relevance_to_hermes()` called `asyncio.run()` from inside the already-running fetch loop (always `RuntimeError`) and leaked an unclosed `AsyncClient` via mangled indentation; it's now an `async` method using `async with httpx.AsyncClient()`, gated behind an explicit `TREND_HERMES_DISPATCH_ENABLED` flag (default off) with HTTP-status validation, and intentionally left un-wired from `fetch()` (auto-creating issues/PRs from trends is an outward-facing action that must flow through the Gate Matrix, not fire automatically). Regression tests in `tests/test_autonomy_hardening_audit.py`.
...
### Changed
- **Runtime scope narrowed to `internal_agent` + Hermes (`render.yaml`).** Goose and Aider are registered "on by default" by `runtimes/manager._build_default_manager()` (they self-report unavailable when their CLI/sidecar is absent, but still incur health-poll probes and clutter the `/runtimes` list). Both Render services now set `RUNTIME_EXTERNAL_DISABLED=true` (stops every default-on/opt-in external runtime from registering) + `RUNTIME_HERMES_ENABLED=true` (re-enables only Hermes), leaving exactly `internal_agent` (the free-NVIDIA workhorse) and Hermes registered. Hermes stays inert until `HERMES_BASE_URL` points at a running sidecar; once it does it can take specialized/heavy task slices via the `RUNTIME_<TASK_TYPE>` overrides. **`RUNTIME_CODE_GENERATION=hermes`** is now wired on both services so a decomposed task's code slice runs on Hermes (and falls back to `internal_agent` when the Hermes sidecar is down) — the multi-agent "many hands" is the CEO decomposition + parallel specialists, with Hermes as an optional specialized engine for the code-gen slice. Tests in `tests/test_runtime_registration_scope.py`.
- **Extracted `NVIDIA_CANDIDATE_MODELS` to shared `.github/scripts/nvidia_models.py`** — single source of truth for implement_agent.py, review_agent.py, and apply_review.py. Uses sys.path injection for standalone CLI script compatibility. Exports both `NVIDIA_CANDIDATE_MODELS` (tuple list with labels) and `NVIDIA_MODEL_IDS` (plain string list).
- **Replaced all remaining references to dead `nemotron-3-super-120b-a12b`** with live `llama-3.3-nemotron-super-49b-v1` across 26 files: router/model_router.py, agent/loop.py, agents/profiles.py, provider_router.py, direct_chat.py, setup/api.py, handlers/v3_models.py, agents/harness_adapter.py, runtimes/adapters/internal_agent.py, router/harness_routing.py, setup_local_models.py, services/cost_attribution.py, services/nim_pool.py, telegram_bot.py, scripts/test_nim_models.py, .github/scripts/generate_context.py, backend/server.py, and all test fixtures.
- **Hardened `_call_review_llm()` fallback in `review_agent.py`** to match `implement_agent.py`: 429 rate-limit triggers exponential backoff retry (3 attempts, jittered) on same model before advancing; timeout advances immediately; 404/422 drops model from rotation; non-429 errors on retry break immediately.
- **NVIDIA NIM model list curated from live endpoint testing.** Tested 10 candidate models against https://integrate.api.nvidia.com/v1 — only 3 returned OK (Nemotron Super 49B tool_calls=True 3.7s, Llama 4 Maverick 1.3s, Llama 3.3 70B tool_calls=True 6.0s); 7 returned 404/APIStatusError/BadRequest. Updated NVIDIA_CANDIDATE_MODELS in implement_agent.py, apply_review.py, and review_agent.py to the 3 live models, removed dead entries. Updated _default_agent_role_models() and _get_nim_provider_record() in backend/server.py to reference live Nemotron Super 49B. Hardened 429 rate-limit fallback with exponential backoff + jitter, timeout detection, and 404/422 model dropout.
...
### Added
- **Render deployment + Telegram + keep-alive runbooks.** New `docs/runbooks/apply-task2-render-env.md` documents the manual Render dashboard brain-flip path that aligns the live deploy with the fast free model already pinned in `render.yaml` (and the Blueprint-sync alternative), plus the rollback + verification curls. New `docs/runbooks/verify-task3-5-telegram-and-keepalive.md` covers the Telegram human-in-the-loop approval-gate flow (Auth / execute / poll / Approve / Reject) and the keep-alive workflow + external 5-min monitor recipe, closing with the six-item DoD PASS/FAIL grid from the autonomous-agency runbook.
- **Public autonomy-readiness probe `GET /api/autonomy/status` (no auth).** Answers the one operational question that otherwise fails *silently* — "is this deploy actually autonomous right now?". Reports whether a free brain is configured (`NVIDIA_API_KEY` present) and which model it resolves to, which of the four autonomous loops are live (`log_monitor` / `self_healing` / `improvement_loop` / `trend_watcher`), and which critical secrets are missing — with an overall `status` of `no_brain` / `idle` / `partial` / `autonomous`. This makes a misconfigured live deploy (e.g. an unset `NVIDIA_API_KEY`, which leaves every agent task with no brain) visible at a glance instead of manifesting only as "nothing happens / everything is slow". Defensive throughout (every probe is independent and never raises). Tests in `tests/test_autonomy_status.py`.
- **`decide_merge()` (G5) wired into the orchestrator land step (`services/workflow_orchestrator.py`).** The repo DeliveryPolicy is now consulted at the ApprovalGate: after PLAN the orchestrator calls `_resolve_merge_decision(run)` → `services.repo_connection.decide_merge(company.repo_connection)` and records it on `run.merge_decision`. The **first unattended merge on a newly connected repo now always pauses for Telegram approval — even under `auto_approve`** (charter Gate Matrix); the gate message's risk reason states how the change will land (`open_pr` / `direct_push` / `telegram_gate` / `awaiting_repo_connection`). On approval, `_record_first_merge_consent(run)` persists consent onto the Company's `RepoConnection` so later merges follow the detected policy instead of re-gating. URL-only companies surface `awaiting_repo_connection`. Best-effort throughout (a lookup failure never blocks the gate). Tests in `tests/test_orchestrator_merge_decision.py`.
...
### Changed
- **Default free brain switched to `nvidia/nemotron-3-super-120b-a12b` (faster, still reasoning).** The production deploy ran the 550B-a55b "ultra", which activates ~55B params/token and makes the multi-call agent loop (plan→execute→verify→judge) slow enough to hit the task timeout — a primary cause of the "dead slow / Execution timed out after 150s" reports. The Nemotron-3 *super* 120B-a12b is a same-generation MoE that activates only **~12B params/token**, so it is several times faster per call while remaining a reasoning model — and runs on the free, no-rate-limit NIM endpoint. It briefly 404'd around issue #656 (which is why the ultra was pinned then); it is confirmed live again at https://build.nvidia.com/nvidia/nemotron-3-super-120b-a12b . Changed the canonical default `brain_policy.DEFAULT_FREE_NVIDIA_MODEL`, the inline free-brain fallback in `agent/loop.py`, the production `render.yaml` (`NVIDIA_DEFAULT_MODEL` + the four `AGENT_*_MODEL` roles), and the `.env.example` guidance (120B now the documented default; the dense 49B is kept as a documented fast fallback). The runtime still honours any operator `NVIDIA_DEFAULT_MODEL` override and the free-only policy (no silent paid Anthropic) is unchanged.
...
### Fixed
- **Scheduler leaked un-awaited persistence coroutines, and durable rehydration silently failed on async startup (#505 follow-up).** `AgentScheduler.create/delete/toggle/_fire` used a `try: asyncio.create_task(self._persist(...)) / except RuntimeError` pattern that *constructed* the coroutine before scheduling it, so when there was no running loop (sync callers, tests, the APScheduler background thread) the coroutine was orphaned and emitted `RuntimeWarning: coroutine '...' was never awaited`. They now check for a running loop *before* building the coroutine. More importantly, `attach_persistence()` → `rehydrate()` called `asyncio.run(self.hydrate())`, which raises inside a running loop — `services/background.py` invokes it from the async FastAPI lifespan, so the `RuntimeError` was swallowed and persisted schedules were **never rehydrated on real startup**, defeating #505. Added `attach_persistence_async()` (awaited directly from the lifespan) and hardened the sync `rehydrate()` to schedule hydration as a task instead of leaking a coroutine when a loop is live. Regression tests in `tests/test_scheduler.py` and `tests/test_schedule_persistence.py`.
