...
### Added
- **Portfolio screen in the v5 dashboard (`frontend/src/v5/screens/PortfolioScreen.jsx`).** New **Portfolio** nav entry (AGENCY section) that visualises the agentic-portfolio data: a metrics strip, a **Now/Next/Later roadmap board**, a **WSJF priority table** (BV/TC/RR → Cost of Delay → Job Size → WSJF score bars), a backlog overflow row, and **sprint-health cards** (health pill, burndown %, scope-creep) rolled up from agentic-agile. Backed by a new read API `agents/portfolio_api.py` (`GET /api/portfolio/board`, `POST /api/portfolio/initiatives`, `DELETE /api/portfolio/initiatives/{id}`, `POST /api/portfolio/seed`) wired into `backend/server.py` and seeded with illustrative demo data so it renders immediately after deploy. Client helpers in `frontend/src/api.js`. 9 tests in `tests/test_portfolio_api.py`.
