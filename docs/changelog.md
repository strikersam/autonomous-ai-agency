...
### Fixed
- **v4 Dashboard API: all endpoints now exist and proxy through Cloudflare Worker** (2026-06-27). The Continuous Improvement Dashboard at `autonomous-ai-agency.strikersam.workers.dev` called `/v4/*` endpoints that didn't exist (404 → rejected Promise → empty panels). Created `backend/v4_api.py` with 9 endpoints bridging ImprovementLoop, SelfHealingAgent, TaskStore, and AgentScheduler. Added `/v4` to Cloudflare Worker proxy list and `wrangler.jsonc`. Added Agent Tasks panel to the dashboard with auto-refresh. PR #844.
