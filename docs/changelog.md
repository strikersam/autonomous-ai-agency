...
### Added
- **★2 Specialized Sub-Agents with Per-Role Models (`agent/models.py`, `agent/loop.py`).** SubAgentConfig Pydantic model (role, model, tool_names, instruction, max_steps) with per-role model routing in AgentRunner._execute_step and _spawn_subagent. Enables File Picker → Planner → Editor → Reviewer pattern where each role uses the cheapest capable model. configure_sub_agents() registers configs; _spawn_subagent inherits per-role configs for delegated work.
