...
- **Flaky `test_admin_policy_brain_returns_resolution_and_paid_state` CI failure: sqlite mirror pollution** (2026-06-28). The test expected `body["resolution"]["role"] == "ollama_local"` but got `"brain_config"` on CI (with MongoDB). Root cause: `tests/test_brain_config_store.py::test_sqlite_mirror_round_trips_through_set` and `tests/test_brain_resolution.py::test_invalidate_then_refresh_picks_up_db_change` called `set_brain_config()` without isolating the sqlite mirror, writing a config with non-empty `updated_at` to `.data/agency.db_brain.db`. When `test_admin_policy_brain_returns_resolution_and_paid_state` later called `resolve_active_brain()`, `_load_unlocked()` read the polluted mirror doc → `cfg.updated_at` was truthy → step 2 short-circuited to `role="brain_config"` instead of falling through to `ollama_local`. Fix: (1) both polluting tests now point `SQLITE_DB_PATH` at a `tmp_path` and reset `_store` so they cannot touch the production mirror; (2) `test_admin_policy_brain_returns_resolution_and_paid_state` now resets `_store` + monkeypatches `BrainConfigStore._load_unlocked` to return `recommended_brain_config()` directly (bypassing Mongo + sqlite mirror), and calls `invalidate_brain_cache()` for defence in depth — making the test hermetic regardless of prior test state. Files: `tests/test_webui_provider_priority.py`, `tests/test_brain_config_store.py`, `tests/test_brain_resolution.py`.
