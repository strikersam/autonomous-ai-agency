...
> **Recovery status (2026-07-12):** this report was written on a branch that never got a PR and
> sat unmerged for two days. F1 (stale CLAUDE.md/AGENTS.md refs), F2 (15 skills missing
> frontmatter descriptions), and F5 (graphify hook nag) were independently fixed and merged via
> PR #1002 before this report itself landed — those three sections below are historical record,
> not open work. **F3 (68 direct `os.environ` reads in `packages/ai/router.py`), F4 (`print()` in
> production modules — re-verified at 56 files, higher than the ~30 originally cited), and F6
> (god files — `backend/server.py` re-verified at 9,765 lines, up from 9,667) are still valid and
> still open** as of this recovery. Re-run each finding's evidence command before acting on P3/P4/P6.
