...
| Policies & Governance | `policies_governance` | **stable** | ✅ | — | GOVERNANCE_ENABLED, GOVERNANCE_MODE, GOVERNANCE_APPROVAL_TTL_S | **Promoted to stable (2026-08-22)** — the policy engine now enforces on every execution path: `AgentRunner._dispatch_tool` guards each tool (commands, file writes, `execute_skill`, `spawn_subagent`), and runtime dispatch passes through the same policy + session budget before any adapter runs, so work handed to a runtime cannot route around a restriction. Every allow/block is audited under the acting identity; six session budget ceilings each fire independently (tool calls, spend, tokens, retries, depth, duration); approvals fail closed (TTL-expired denies, a restart expires pending). Ships in `observe` by default — nothing blocks until `GOVERNANCE_MODE=enforce`. Policy authoring is a separate beta row below. |
| Policy Authoring UI | `policy_authoring_ui` | beta | ✅ | Policies & Governance | — | The dashboard shows posture, approvals, the audit trail and a read-only simulator, but cannot edit policy — rules live in `config/agent_policy.yaml` and change through git review only. The enforcement engine is stable; in-product authoring is the beta part, tracked here rather than hidden as a caveat. |
| Sidecar Runtimes (Hermes/OpenCode/Goose) | `sidecar_runtimes` | **stable** | ✅ | Sidecar process running, `RuntimeManager.wake_all_sleeping_runtimes()` | CEO_WAKE_COOLDOWN_SEC | **Promoted to stable (2026-08-15)** — every CEO delegation wakes + health-checks sidecars before dispatch (real guarantee, not a guess) and routes around any still sleeping; covered by the runtime test suites. Hermes ships deployed by default (`agency-hermes` on Render); OpenCode/Goose remain opt-in. |
| Prime Agent Runtime | `prime_agent_runtime` | experimental | ⛔ | `prime-agent`/`pi` binary on PATH, `agency` provider extension loaded | RUNTIME_PRIME_AGENT_ENABLED, PRIME_AGENT_BIN, PRIME_AGENT_EXTENSION, PRIME_AGENT_TRUST_WORKSPACE | Opt-in, default off. Not baked into the default image (`--build-arg INSTALL_PRIME_AGENT=true`). Declared on the worker service only — it runs model-generated shell with container permissions and is not a sandbox. See [prime-agent.md](runbooks/prime-agent.md). |
| Multi-Agent / Swarm | `multi_agent_swarm` | **stable** | ✅ | CEO dispatcher (`services/ceo_dispatcher.py`) | CEO_FANOUT_COMPLEXITY, CEO_MAX_CONCURRENT, QN_ATOMIC_CLAIM | **Promoted to stable (2026-08-15)** — wired into the golden path via `CEODispatcher.delegate`; the `WorkflowOrchestrator` EXECUTE phase fans out across N specialists for medium/high-complexity tasks, with per-subtask cross-verification and a durable CEO ledger for supervision/retry; covered by the CEO/swarm test suites. |
