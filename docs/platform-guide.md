...
| Multi-agent orchestration — deep swarm (CEO fans out across N specialists) | **Stable** | Promoted from beta 2026-08-15 — wired into the golden path via `services/ceo_dispatcher.py:CEODispatcher.delegate`; the `WorkflowOrchestrator` EXECUTE phase calls the CEO for medium/high-complexity tasks, which routes each specialist through `RuntimeManager` to its preferred runtime, with per-subtask cross-verification and a durable CEO ledger for supervision/retry. `CEO_FANOUT_COMPLEXITY=high` restricts fan-out to the hardest requests; fallback stats via `get_ceo_fallback_stats()`. Covered by the CEO/swarm test suites. |
| Hermes runtime (default code-generation sidecar / external agent runtime) | **Stable** | Promoted from beta 2026-08-15 — deployed by default (`agency-hermes` on Render) and set as the default runtime for `code_generation` tasks. `RuntimeManager.wake_all_sleeping_runtimes()` gives every CEO delegation a real, rate-limited health check (not a guess) before dispatch, with automatic fallback to the Internal Agent if Hermes is down. Covered by the runtime test suites. |
...
- **Extended 1-hour prompt cache TTL** — Set `ANTHROPIC_CACHE_TTL=1h` to use Anthropic's extended cache duration for system prompts, tool lists, and conversation history breakpoints. The standard 5-minute TTL (the default) expires between turns in long agent sessions; 1-hour keeps the cache warm across a full work session. The tradeoff: 1-hour writes cost ~60% more than 5-minute writes, but reads within the hour are free, so the extended TTL pays off for any session longer than a few minutes. The `extended-cache-ttl-2025-04-11` beta header is sent automatically when `ANTHROPIC_CACHE_TTL=1h` is set.
