...
| `TELEGRAM_WEBHOOK_ENABLED` | `false` | Switch the bot's inbound transport from getUpdates long-polling to a webhook. When true **and** `TELEGRAM_WEBHOOK_SECRET` is set, startup registers `setWebhook` at `${RENDER_EXTERNAL_URL}/api/telegram/webhook` and does **not** start the poller (a set webhook and getUpdates are mutually exclusive). Recommended on Render: it removes the failure mode where inline buttons go dead because the background poll task stopped while cards (sent by independent direct calls) keep arriving. |
| `TELEGRAM_WEBHOOK_SECRET` | (none) | Shared secret Telegram echoes in the `X-Telegram-Bot-Api-Secret-Token` header on every webhook POST; it is the `/api/telegram/webhook` endpoint's authentication (a mismatch or unset secret is a flat 403). Mandatory for webhook mode. Must be 1–256 chars of `[A-Za-z0-9_-]`; use a long random value. |
...
| `AGENCY_GATE_OUTWARD_FACING` | `true` | Promote outward-facing autonomous tasks — those that leave the repo with no merge in between (`deploy`, `release`, `runtime_change`, or tagged `deploy`/`release`/`external-write`/`irreversible`/`risk:*`) — to `requires_approval` at dispatch, so they park and send a Telegram Approve/Reject prompt before running. Work that only commits to an agent branch and opens a PR (bug fixes, features, issue intake) is not gated here: agents cannot merge or push to a protected branch, so the human merge is its gate. Set `false` to stop pre-run prompts entirely. |
| `AGENCY_AUTO_TRIAGE` | `true` | CEO triage on the dispatcher's cadence: approve parked tasks that were auto-gated but no longer need a human, reject parked duplicates of already-open tasks, and queue fix tasks for open error alerts on the dashboard bell. Deliberately gated tasks (deploys, auth/secrets, 🔴 trend items) are never touched. Set `false` to disable. |
| `AGENCY_AUTO_TRIAGE_EVERY_POLLS` | `60` | Dispatcher polls between CEO triage passes (~5 min at the default 5 s poll). `0` disables. |
