...
description: >
  ...
  Integrates with Claude Code and Codex via repo hooks that auto-refresh on session start and after turns.
triggers:
  - "/graphify"
...
- [ ] `python -m pip install -r requirements.txt` or `python -m pip install graphifyy && graphify install` completed on this machine
- [ ] `.claude/hooks/graphify-refresh --session` produced `graphify-out/graph.json` and refreshed `graphify-out/GRAPH_REPORT.md`
- [ ] `graphify-out/GRAPH_REPORT.md` is committed; `graphify-out/graph.json` remains gitignored because node IDs are checkout-path-specific
- [ ] `graph.html` and `cache/` are in `.gitignore`
- [ ] `.claude/settings.json` `SessionStart` and `Stop` hooks point at `.claude/hooks/graphify-refresh`
