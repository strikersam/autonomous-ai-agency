...
Tests use ``monkeypatch.setattr(setup_api, "_WIZARD_STATE_DIR", tmp_path)``
so the module-level directory override auto-restores between tests and
does not leak into other test modules that load wizard state later in
the session.
"""
...
def test_wizard_state_roundtrip_embeds_valid_checksum(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(setup_api, "_WIZARD_STATE_DIR", tmp_path)
...
def test_wizard_state_checksum_mismatch_returns_fresh(tmp_path, monkeypatch) -> None:
...
def test_wizard_state_legacy_file_without_checksum_is_loaded(tmp_path, monkeypatch) -> None:
