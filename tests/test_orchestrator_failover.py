...
class TestProviderRoleTags:
    ...
    The Providers screen surfaces a 'BRAIN' badge per provider based on
    services.workflow_orchestrator.get_provider_role_tags(). The classifier
    must agree with the live _resolve_brain_provider() so the UI never
    disagrees with the runtime.
    """

    async def test_one_provider_is_brain(self, monkeypatch):
        """When a free cloud provider is configured, it is the brain."""
        ...
        async def _list_records():
            return records
        monkeypatch.setattr(
            "backend.server._list_configured_provider_records", _list_records
        )

        # Force env override to be empty so the brain resolver picks from records.
        ...
        tags = await _wo.get_provider_role_tags()
        assert tags["nvidia-nim"]["is_brain"] is True
        assert tags["nvidia-nim"]["role"] == "brain"
        assert tags["anthropic"]["is_brain"] is False
        assert tags["anthropic"]["role"] == "fallback"  # paid → fallback tag

    async def test_unconfigured_provider_tagged(self, monkeypatch):
        """A provider with no api_key gets role=unconfigured."""
        import services.workflow_orchestrator as _wo
        ...
        assert "missing" in tags["no-key"]["reason"].lower() or "api key" in tags["no-key"]["reason"].lower()

    async def test_env_override_only_records(self, monkeypatch):
        """When AGENT_LLM_BASE_URL points at a URL not in any record, tagging still returns (no brain is flagged)."""
        import services.workflow_orchestrator as _wo
        ...
        # Env override points somewhere none of the records are configured for.
        monkeypatch.setenv("AGENT_LLM_BASE_URL", "https://override.example.com/v1")

        tags = await _wo.get_provider_role_tags()
        # No record matches the override URL — none of them is_brain.
