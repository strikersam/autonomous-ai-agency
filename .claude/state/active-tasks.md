...
| 35 | Frontend UX redesign: 20→6 IA, retire legacy UI, unify tokens | `IN_REVIEW` | `claude/agency-ui-ux-redesign-yjvb85` (PR #1325) | Design canvas (Artifact) + implemented: 6 goal-based hubs w/ back-compat alias map for old /v5/* deep links; legacy /legacy dashboard + 21 pages + 6 dead components removed (~23k LOC); tokens unified to index.css (Tailwind Outfit→Manrope fix); new StatusPill/Spinner/HubTabs primitives. Fixed test_frontend_deployment_guards OAuth-origin guard (repointed from deleted SettingsPage.js → api.js). Master merged in (governance landed first); full local suite 6172 passed. Awaiting final CI green → squash-merge. | 2026-08-22 |
