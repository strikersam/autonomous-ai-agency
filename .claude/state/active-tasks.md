...
| 34 | Unblock the autonomous loop: failing-test parser + hermetic test storage | `IN_PROGRESS` | `claude/render-bottleneck-alternatives-sv8aj1` | Root-caused the stalled backlog (issues #1352/#1354, 7 never-landing `docs: investigate failing tests` draft PRs). (a) `agency-cycle.yml`'s `grep -E '^(FAILED\|ERROR) '` matched pytest captured log records, emitting bogus IDs like `qwen-proxy:app_settings.py:70`; `continuous-improvement.yml`'s `grep '^FAILED '` dropped ERROR lines. Both now use new unit-tested `scripts/parse_pytest_failures.py`. (b) conftest never pinned `STORAGE_BACKEND`, so tests dialled real Mongo at localhost:27017 → 1 permanent CI error. (c) Fixing (b) unmasked a split admin identity: 6 test modules did `setdefault("ADMIN_EMAIL","admin@test.local")` at import while `backend/server.py` had already captured `admin@llmrelay.local`, so seed→200 then login→401; conftest now pins it and the 6 dead assignments are gone. (d) `voice/memory_kernel.py` binds `MEMORY_KERNEL_DIR` at import, so facts persisted into `.data/memory` and a 2nd local `pytest` went red; conftest now pins a per-session temp dir. 30 new tests | 2026-08-25 |
