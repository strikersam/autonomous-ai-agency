...
| 35 | Frontend UX redesign: 20→6 IA, retire legacy UI, unify tokens | `DONE` | `claude/agency-ui-ux-redesign-yjvb85` | Design canvas (Artifact) + implemented: 6 goal-based hubs w/ back-compat alias map for old /v5/* deep links; legacy /legacy dashboard + 21 pages + 6 dead components removed (~23k LOC); tokens unified to index.css (Tailwind Outfit→Manrope fix); new StatusPill/Spinner/HubTabs primitives; 22 suites/152 tests green, build green. Provider cards kept (load-bearing, folded behind disclosure) — broad pill migration deferred | 2026-08-22 |
| 36 | Governance enforcement GA: policy+budget on every execution path, matrix STABLE | `DONE` | `feat/governance-enforcement-ga` (PR #1324) | GovernanceGate.charge feeds tokens/cost/depth; runtime dispatch runs the session budget; Surface.AGENT for spawn depth; all six ceilings fire independently; approvals fail-closed (TTL+restart); policies_governance→STABLE + policy_authoring_ui BETA; matrix-guard test; 295 tests green | 2026-08-22 |
