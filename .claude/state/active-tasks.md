...
| 2026-07-25 | Claude Code | claude/aerolink-probe-runtime-errors-mjeowt | Provider reliability series (#1120, #1121, #1123 merged): shared failover client for CEO + agent loop, Anthropic native Messages API, bounded failover fan-out, billing-refusal handling, per-provider on/off switch. Follow-up in this branch: operator provider state (kill switch + `allow_paid`) now persists through Mongo so it survives a Render deploy, with the SQLite file kept as a warm mirror; `state_durable` surfaced in `GET /api/brain/providers` and the Providers screen. Also fixed `tests/test_brain_failover.py` reading a developer's real `.data/agency.db`. 41 new tests (Python + Jest). |
...
| 2026-07-25 | Claude Code | claude/ceo-agent-delegation-4xae6g | CEO micro-management: tier ladder (intern→junior→midlevel→senior) with bounded escalation, seven-section subtask briefs, anti-slop quality gate, durable goal/subtask/attempt ledger (`services/ceo_ledger.py`), and a 24x7 supervisor that closes / re-drives / abandons open goals (`services/ceo_supervisor.py`, wired into background services + `loops/registry.yaml`). Fixed two live bugs found while surveying: `CEODispatcher.delegate` did not accept the `goal` / `max_steps` kwargs the orchestrator has always passed, so **every** medium/high-complexity EXECUTE phase died with TypeError (the existing test used an AsyncMock, which hid it); and CEO-executed work always reported an empty changed-file list despite internal_agent/e2b populating it. New `/api/ceo/*` routes. 84 new tests. |
