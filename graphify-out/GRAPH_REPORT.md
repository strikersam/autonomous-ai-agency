# Graph Report - autonomous-ai-agency  (2026-07-12)

## Corpus Check
- 1179 files · ~1,576,250 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 21654 nodes · 43098 edges · 1021 communities (892 shown, 129 thin omitted)
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 5941 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d255589c`
...
## Communities (1021 total, 129 thin omitted)

### Community 0 - "server.py"
Cohesion: 0.02
Nodes (234): _is_admin(), Check if a user has admin role.      Works for both social_auth users (role in S, admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), auth_me(), auto_recommend_skills() (+226 more)

### Community 1 - "AgentScheduler"
Cohesion: 0.02
Nodes (134): _acquire_provider_probe(), clear_cooldowns(), _dead_model_key(), _exponential_backoff_cooldown(), extract_openai_text(), get_dead_models(), is_commercial_provider(), _is_model_dead() (+126 more)

### Community 2 - "CompanyGraphStore"
Cohesion: 0.05
Nodes (142): AccountLifecycleResponse, _DoctorCheck, _DoctorReport, get_public_doctor_report(), OnboardingAnswersRequest, OnboardingProgressResponse, OnboardingQuestionsRequest, BaseModel (+134 more)

### Community 3 - "Agency"
Cohesion: 0.10
Nodes (19): AgencyCycleResult, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _fetch_github_quick_notes(), _gh_repo(), _gh_token() (+11 more)

### Community 4 - "PortfolioManager"
Cohesion: 0.03
Nodes (84): AgentRole, str, BackgroundAgent, _now(), Any, agent/background.py — Background Agent  An always-on worker thread that proces, Enqueue *task* for processing. Returns the task (with task_id set)., Convenience: create a task and submit it in one call. (+76 more)
...
Cohesion: 0.02
Nodes (137): Any, Return recent commits with agent attribution trailers parsed out., get_skill_registry_safe(), Return the global SkillRegistry if set, else None.     Used by onboarding and ot, admin_login(), admin_logout(), agency_status(), agent_chat() (+129 more)

### Community 7 - "TaskResult"
Cohesion: 0.02
Nodes (148): PreflightReport, AgentJob, WorkspaceEscapeError, WorkspaceIDError, FeatureMaturity, str, Feature maturity classification., kimi_bridge_runtime_config() (+140 more)
...
Cohesion: 0.02
Nodes (151): _brain_provider_status(), get_brain_policy_route(), Return per-provider metadata for the GET endpoint.      Iterates every provider, Return the active brain config + per-provider key-present flags.      The respon, BrainConfig, BrainConfigStore, default_brain_config(), get_brain_config() (+143 more)

### Community 11 - "AgentSessionStore"
Cohesion: 0.08
Nodes (38): AgentPhaseError, Exception, Raised when a named agent phase (planning, verification, etc.) fails., _make_runner(), Path, Reproduce the exact error from the screenshot: slices present, goal absent., End-to-end: model returns CRISPY-style slices, plan should still execute., spawn_subagent creates a child AgentRunner and returns a condensed result. (+30 more)

### Community 12 - "AgentRunner"
Cohesion: 0.05
Nodes (42): AgentRunner, _nvidia_api_key(), Any, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it.          Asks the planner model to writ, Send chat messages and return the assistant's text output.          If an Anthro, Call-time resolver for an agent role model id.      Delegates to ``services.brai, Lazy accessor for the SteerLM steering injector (B2). (+34 more)

### Community 13 - "RuntimeDependency"
Cohesion: 0.09
Nodes (21): make_isolated_workspace(), _now(), Any, Path, Run a job using the provided runner and update the job's lifecycle, progress, re, Create an isolated workspace directory under *root*.      This is the legacy p, Serialize the AgentJob to a JSON-serializable dictionary for external clients., runner() (+13 more)

### Community 14 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (49): Called by APScheduler when a cron fires. Dispatches to the orchestrator., Execute work through the 11-phase golden path., Return orchestrator queue depth, active runs, and supervisor state (#522)., List recent workflow orchestrator runs.      Non-admin users see only their own, _scheduler_on_fire(), workflow_orchestrator_execute(), workflow_orchestrator_list_runs(), workflow_orchestrator_status() (+41 more)
...
Cohesion: 0.03
Nodes (102): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), get_ceo_dispatcher(), _merge_changed_files(), Any, services/ceo_dispatcher.py — Real CEO delegation layer.  The CEO splits a comple (+94 more)
...
Cohesion: 0.03
Nodes (34): Delete a company and all its associated data., Delete a company and all associated data from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON         blob, Count total companies in the store., Create a new website in SQLite. The full model is stored in ``data``         so, Get a website by ID from SQLite., Update a website in SQLite. When ``company_id`` is omitted the existing (+26 more)
...
Cohesion: 0.04
Nodes (98): PreflightIssue, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), agent/job_manager.py — Async agent job lifecycle manager.  Manages agent jobs, _workspace_component() (+90 more)

### Community 25 - "workflow_orchestrator.py"
Cohesion: 0.05
Nodes (141): Agency, get_agency(), CEO-coordinated multi-agent agency for continuous codebase management.      The, set_agency(), AgentJobRequest, AgentJobSnapshot, BaseModel, Complete point-in-time view of a job, safe to serialise as API response. (+133 more)
...
Cohesion: 0.04
Nodes (65): get_catalog_models_route(), Return the mirrored model catalog (advisory-only).      Flag-gated by ``FREELLM_, Force a catalog mirror rebuild + persist (admin only).      Rebuilds the catalog, sync_catalog_route(), CatalogActiveBrain, CatalogMirror, CatalogProviderEntry, get_catalog() (+57 more)
...
Cohesion: 0.05
Nodes (74): activation_required(), ActivationResult, activate_instance(), ActivateRequest, ActivateResponse, activation_audit_log(), activation_status(), ActivationStatusResponse (+66 more)

### Community 32 - "Specialist"
Cohesion: 0.03
Nodes (93): account_lifecycle(), auto_recommend_skills(), cancel_onboarding(), delete_company_endpoint(), generate_onboarding_questions(), get_company(), get_company_access(), get_company_graph() (+85 more)
...
Cohesion: 0.04
Nodes (46): Evidence, Evidence supporting a system detection., Inferred technology stack from website/repo analysis., StackInference, _is_blocked_host(), Cheap (no-DNS) SSRF check for headless-browser subrequests.      A rendered page, _ds(), _models() (+38 more)
...
Cohesion: 0.05
Nodes (45): AIToolMetrics, build_report(), EngagementMetrics, PerformanceAnalytics, datetime, Enum, str, AI-Assisted Engineering Insights — track AI tool usage, engagement, and performa (+37 more)
...
Cohesion: 0.05
Nodes (51): ApprovalCheckpoint, Human approval gate in a task's execution., _is_brain_connection_error(), Any, BaseException, Task, TaskSpec, TaskStatus (+43 more)

### Community 47 - "Task"
Cohesion: 0.04
Nodes (49): _coerce_ts(), ExecutionLogEntry, Any, Full task/issue document., Update the updated_at timestamp., Coerce ISO-8601 datetime strings (from DB) to float timestamps., Single entry in a task's execution log., Dict-compatible accessor.          Log entries are serialized to/from dicts in m (+41 more)
...
Cohesion: 0.06
Nodes (38): get_improvement_loop(), Any, Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., get_self_healing_agent(), HealingEvent, _now(), Any (+30 more)
...
Cohesion: 0.13
Nodes (80): BackgroundTask, BrowserSession, Async Playwright browser session.      Usage::          session = BrowserSes, True if Playwright is installed and ready to use., CommitAttribution, CommitTracker, Path, agent/commit_tracker.py — AI Commit Attribution  Tags git commits with metadat (+72 more)
...
Cohesion: 0.05
Nodes (46): Probe a ``(provider, model)`` pair without persisting.      Powers the UI "Test", test_brain_model_route(), _describe_http_status(), probe_model_liveness(), _probe_ollama(), _probe_openai_compat(), ProbeResult, BaseModel (+38 more)
...
Cohesion: 0.08
Nodes (63): BoundContext, ClassifyOutput, emit_deprecation(), ExecutionResult, JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput (+55 more)
...
Cohesion: 0.04
Nodes (64): create_access_token(), create_refresh_token(), github_callback(), github_login(), github_repo_access(), github_repo_callback(), github_webhook(), google_callback() (+56 more)
...
Cohesion: 0.05
Nodes (46): generate_backlog_retro(), generate_standup(), Derive a retrospective from the task tracker when no sprint is active.      DONE, Build a :class:`StandupReport` from ``.claude/state/active-tasks.md``.      Read, Initiative, _bug_scores(), _clean(), _default_repo() (+38 more)
...
Cohesion: 0.02
Nodes (98): RoutingDecision, AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., GooseAdapter, Any, Adapter for Goose — TIER 2 general-purpose local runtime., HermesAdapter (+90 more)
...
Cohesion: 0.06
Nodes (64): _fake_user_auth(), Path, test_admin_can_create_anthropic_provider_via_webui_admin_api(), test_admin_can_create_provider_via_webui_admin_api(), test_ui_providers_and_workspaces_use_app_state(), _bootstrap(), Path, ProviderManager (+56 more)
...
Cohesion: 0.06
Nodes (35): [5.0.0] — 2026-05-24, Added, Added, Added, Added, Added, Added, Added (+27 more)
...
Cohesion: 0.03
Nodes (91): agents/store.py — Persistent store for user-defined agent configurations.  Store, Set the global agent store instance (e.g., with MongoDB on startup)., set_agent_store(), quick_notes_submit(), Submit a quick-note URL or instruction from the dashboard FAB., Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system.  Provides a lightweight task/issue tracker (+83 more)
...
Cohesion: 0.09
Nodes (25): ContextManager, Any, True when the history is long enough to warrant compaction., Replace the old portion of *history* with a single compaction note.          T, True when the harness should use head_file instead of read_file.          When, Trim a step result so sub-agent outputs stay within ~1-2k tokens.          The, Manages context window state for a single agent run.      The Brain (LLM) stay, Return a copy of *observations* with old tool outputs truncated.          JetB (+17 more)
...
Cohesion: 0.11
Nodes (18): ABC, CerebrasProvider, Provider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NVIDIA NIM provider adapter — wraps the existing provider_router logic.  This is, Ollama provider adapter — local LLM inference., packages.ai — provider abstraction, model registry, and failover manager. (+10 more)
...
Cohesion: 0.04
Nodes (69): Report whether the SAM realtime voice (LiveKit) transport is configured., Mint a LiveKit room token so the dashboard can talk to SAM live., sam_livekit_status_backend(), sam_livekit_token_backend(), auth_headers(), livekit_env(), no_livekit_env(), _normalize_dockerfile() (+61 more)
...
Cohesion: 0.08
Nodes (25): Added, Added, Added, Added, Added, Added, Added, Changed (+17 more)
...
Cohesion: 0.08
Nodes (14): Workflow, Detect the Git provider from a repository URL.                  Args:, Start the onboarding process for a company.                  Args:             c, Schedule a fire-and-forget background task, keeping a strong reference., Force-refresh the dynamic skill registry and log tech-stack recommendations., Get the current onboarding progress for a company.                  Args:, Resume onboarding from where it left off.                  Args:             com, Pause onboarding for a company (sets status to paused).          Args: (+6 more)
...
Cohesion: 0.07
Nodes (25): CompanyAgencyService, get_company_agency_service(), _is_runtime_available_sync(), _pick_available_runtime(), Any, SpecialistFamily, services/company_agency.py — Company Agency Orchestration Service  After onboard, Orchestrates specialist activation, runtime startup, and 24x7 scheduling     for (+17 more)
...
Cohesion: 0.15
Nodes (10): get_enrichment(), HarnessEnrichment, invalidate_enrichment_cache(), Any, agent/harness_enrichment.py — Automatic Harness Enrichment for Agent Prompts  Ev, Build a compact catalog of available runtime skills.          Discovers from Ski, Build the complete enrichment block (tools + skills).          Returns empty str, Inject enrichment blocks into a system prompt string.          Appends blocks af (+2 more)
...
Cohesion: 0.07
Nodes (39): get_public_doctor(), legacy_scheduler_create(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_toggle(), legacy_scheduler_trigger(), Force-dedup and clean stale schedules from the durable store.      Admin-only en, Public Doctor endpoint — no authentication required.      Returns system-level d (+31 more)
...
Cohesion: 0.22
Nodes (13): _check_extra_kwargs(), _enforce_signature(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic classe, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only need, Raise TypeError if fn's signature drifts from the locked contract (Pydantic extr, build_compaction_prompt(), build_execution_prompt() (+5 more)
...
Cohesion: 0.08
Nodes (9): FakeResp, Tests for agents/portfolio_intelligence.py — autonomous signal → initiative.  Lo, DEFAULT_REPO was hardcoded to the stale pre-rename repo name     ('strikersam/lo, fetch_github_signals must degrade gracefully (log + return empty     lists) on a, Even with a 200, a malformed/rate-limited body that isn't a         list must no, TestDefaultRepoFollowUpFix, TestGithubAndResearch, TestGithubSignalHardening (+1 more)
...
### Community 412 - "DailyDigestAggregatorTests"
Cohesion: 0.24
...
### Community 417 - "TestChatHistoryStore"
Cohesion: 0.07
...
### Community 511 - "telegram_inbound_handlers.py"
Cohesion: 0.31
...
### Community 514 - "test_doctor_service_token_check.py"
Cohesion: 0.25
Nodes (8): _openclaw_instructions(), openclaw_reverse_proxy(), openclaw_status(), Return the OpenClaw Gateway integration status + pairing QR data.      The gatew, Build the instructions string (separate function to avoid .format() brace issues, Catch-all for /openclaw/* — returns gateway info (the WS endpoint is at /opencla, is_gateway_alive(), Check if a device is currently connected to the gateway.
...
Cohesion: 0.12
Nodes (5): OllamaProvider, Provider, RateLimit, Provider rate limit info., Return the provider's rate limits.
...
### Community 655 - "TestParseToolCalls"
Cohesion: 0.48
...
### Community 712 - "test_agent_tools.py"
Cohesion: 0.33
...
### Community 749 - "Workspace Issues"
Cohesion: 0.15
...
### Community 750 - "Model and Response Issues"
Cohesion: 0.23
Nodes (13): create_company(), _is_ephemeral_user(), True when this user's companies should be temporary (reaped after TTL).      Eph, Best-effort auth provider for a user dict.      social_auth users carry an expli, Create a new company.          This is the entry point for the Company Graph., _resolve_provider(), Tests for company lifecycle gating — who gets ephemeral vs persistent agencies., test_admin_local_is_persistent() (+5 more)
...
Cohesion: 0.08
Nodes (30): AgentDirective, Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentSession, AgentSessionMessage, AgentStep, Any (+22 more)
...
### Community 803 - "test_runtime_registration_scope.py"
Cohesion: 0.20
...
### Community 805 - "commands.py"
Cohesion: 0.17
Nodes (10): _best_cloud_primary_base(), _nvidia_provider_chain(), TaskSpec, Execute a TaskSpec using the internal AgentRunner and convert the agent's outcom, Create an isolated execution context for a single task.          Tries ``git wor, Clean up the worktree or temp copy created by ``_create_worktree``., Build Nvidia NIM provider config from env.  Empty list when key is absent., Return the highest-priority available cloud LLM base URL.      Tries free cloud (+2 more)
...
### Community 853 - ".delete_company_graph"
Cohesion: 0.50
...
### Community 863 - "test_skills_route_order.py"
Cohesion: 0.20
...
### Community 865 - "7. Scheduler Architecture"
Cohesion: 0.50
Nodes (3): Any, Resolve the default executor model via the catalog (UNIT 7).      Was hardcoded, _resolve_default_executor_model()

### Community 893 - "commercial_equivalent.py"
Cohesion: 0.50
...
### Community 929 - ".__init__"
Cohesion: 0.15
Nodes (8): PR throughput per cohort over the last `days` days., A single AI tool interaction., Record a usage event., UsageEvent, A spread of events from 3 users across 3 tools over a week., sample_events(), Backdates the older row via raw SQLite UPDATE so it falls outside         the cu, timedelta
...
### Community 999 - "6. Agent Architecture"
Cohesion: 0.33
...
### Community 1008 - "test_activate_agency_runs_in_background_without_blocking"
Cohesion: 0.67
...
## Knowledge Gaps
- **3141 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `aider_config.sh script`, `OPENAI_API_BASE` (+3136 more)
...
- **Why does `AgentRunner` connect `AgentRunner` to `openclaw_websocket_endpoint`, `server.py`, `MCPClient`, `PortfolioManager`, `ContextPruner`, `proxy.py`, `TaskResult`, `test_agent_free_brain.py`, `AgentSessionStore`, `RuntimeDependency`, `test_ceo_dispatcher.py`, `direct_chat.py`, `workflow_orchestrator.py`, `diagnostics.py`, `TestMCPServer`, `E2BSandboxSession`, `MultiAgentSwarm`, `commands.py`, `ExecutionRequest`, `SamConversation`, `TokenBudget`, `GitHubTools`, `TaskSpec`, `ContextManager`, `StuckDetector`, `_resolve_brain_provider`, `ReactScratchpad`, `TestHarnessAdapter`, `_resolve_push_token`, `test_schedule_persistence.py`, `TestSchedulerStore`, `hermes_server.py`, `LocalWorkspace`, `WorkspaceTools`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `WorkspaceTools` connect `WorkspaceTools` to `openclaw_websocket_endpoint`, `TestMCPServer`, `MCPClient`, `test_unit7_catalog_propagation.py`, `PortfolioManager`, `ToolRegistry`, `AgentSessionStore`, `AgentRunner`, `hermes_server.py`, `.__init__`, `RepowiseIntelligence`, `TestHarnessAdapter`, `workflow_orchestrator.py`, `TaskSpec`, `ContextManager`, `TestExtendedThinkingRouting`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `TaskSpec` connect `TaskResult` to `server.py`, `AgentScheduler`, `test_phase4_runtime_resilience.py`, `ClaudeCodeAdapter`, `TaskWorkflowService`, `ProviderManager`, `direct_chat.py`, `workflow_orchestrator.py`, `llm_providers.py`, `api.py`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Are the 151 inferred relationships involving `AgentRunner` (e.g. with `AgentCoordinator` and `AgentSpec`) actually correct?**
  _`AgentRunner` has 151 INFERRED edges - model-reasoned connections that need verification._
