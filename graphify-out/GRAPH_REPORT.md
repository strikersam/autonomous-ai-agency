# Graph Report - local-llm-server  (2026-08-10)

## Corpus Check
- 1425 files · ~2,044,146 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28374 nodes · 57945 edges · 1164 communities (1057 shown, 107 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 6298 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f02c40f6`
...
## Communities (1164 total, 107 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.01
Nodes (413): get_harness_adapter(), backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, admin_seed(), _agent_timeout_fallback_response() (+405 more)

### Community 1 - "ProviderConfig"
Cohesion: 0.02
Nodes (182): get_tracker(), Return the process-singleton RateLimitTracker., _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), extract_openai_text(), _get_director(), is_anthropic_base_url() (+174 more)

### Community 2 - "AgentScheduler"
Cohesion: 0.04
Nodes (153): Agency, get_agency(), CEO-coordinated multi-agent agency for continuous codebase management. The CEO…, set_agency(), AgentJobRequest, AgentJobSnapshot, Complete point-in-time view of a job, safe to serialise as API response., Convenience: the canonical text response, whether success or failure. (+145 more)

### Community 3 - "AgentRunner"
Cohesion: 0.02
Nodes (134): AgentRunner, _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), _nvidia_api_key(), Any, Path (+126 more)

### Community 4 - "Task"
Cohesion: 0.02
Nodes (124): Set the global agent store instance (e.g., with MongoDB on startup)., set_agent_store(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), ApprovalCheckpoint, Full task/issue document., Human approval gate in a task's execution., Task (+116 more)

### Community 5 - "KnowledgeGraph"
Cohesion: 0.02
Nodes (101): Status of a user story within a sprint., StoryStatus, HarnessAdapter, HarnessSpec, Any, agents/harness_adapter.py — ECC Cross-Harness Adapter Normalises API…, Adapt harness-native requests to the local-llm-server internal format. Each…, Detect which harness sent this request from headers. Check order: explicit… (+93 more)

### Community 6 - "company_api.py"
Cohesion: 0.04
Nodes (137): AccountLifecycleResponse, _DoctorCheck, _DoctorReport, _get_current_user_thunk(), _get_optional_user_thunk(), get_public_doctor_report(), OnboardingAnswersRequest, OnboardingProgressResponse (+129 more)

### Community 7 - "proxy.py"
Cohesion: 0.02
Nodes (161): Any, Return recent commits with agent attribution trailers parsed out., Start a daemon thread that processes one queued note every *interval_hours*.…, start_processor(), get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other… (+153 more)

### Community 8 - "_fixture"
Cohesion: 0.01
Nodes (124): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+116 more)

### Community 9 - "MCPUnavailableError"
Cohesion: 0.03
Nodes (111): MCPUnavailableError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Raised when the MCP server is unreachable or the circuit is open., TaskResult, TaskSpec, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, E2BSandboxSession (+103 more)

### Community 10 - "workflow_orchestrator.py"
Cohesion: 0.04
...
Cohesion: 0.03
Nodes (114): CEODispatcher, _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, Real CEO delegation: wake runtimes, fan out, and merge results., Attempt, _backend(), CEOLedger, GoalRecord (+106 more)

### Community 12 - "BackgroundAgent"
Cohesion: 0.02
Nodes (105): BackgroundAgent, _now(), Any, agent/background.py — Background Agent An always-on worker thread that…, Enqueue *task* for processing. Returns the task (with task_id set)., Convenience: create a task and submit it in one call., Real handler — dispatches through AgentRunner when available. HARDENED (PR…, Always-on worker that drains a task queue on a daemon thread. GATE: Golden Path… (+97 more)

### Community 13 - "LLMProvider"
Cohesion: 0.04
Nodes (73): openai_body_from_response(), Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on…, Render an ``LLMResponse`` as an OpenAI chat-completions body. Used by the…, should_use_router(), AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic… (+65 more)
...
Cohesion: 0.02
Nodes (115): all_provider_ids(), _provider_ids_from_literal(), Resolve the model id for a component's role on a provider. Parameters…, Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, resolve_component_model(), CatalogActiveBrain, CatalogMirror (+107 more)

### Community 16 - "WorkspaceManager"
Cohesion: 0.04
...
Cohesion: 0.03
Nodes (97): _load(), main(), int, get_cache(), The process-wide cache manager., Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Translate an OpenAI-shaped chat payload into an ``LLMRequest``. Unknown keys… (+89 more)

### Community 20 - "test_operational_incidents.py"
Cohesion: 0.03
Nodes (112): _diagnose_and_file(), _file_incident(), _format_incident(), gather_render_evidence(), _iso_from_monotonic(), normalise(), note_phase_end(), note_phase_start() (+104 more)

### Community 21 - "brain_config.py"
Cohesion: 0.03
Nodes (104): BrainConfig, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), provider_base_url(), _provider_env_value() (+96 more)

### Community 22 - "TaskDispatcher"
Cohesion: 0.03
Nodes (71): AgencyCycleResult, AgentDirective, AgentRole, Any, str, Capture the FastAPI main event loop so the CEO thread can dispatch run_cycle()…, Generate directives for each onboarded company. The CEO examines each company's…, Call the configured provider stack to perform CEO strategic assessment. (+63 more)

### Community 23 - "AgentSessionStore"
Cohesion: 0.04
Nodes (81): Agent subsystem — planner / executor / verifier loop., AgentPhaseError, Exception, Raised when a named agent phase (planning, verification, etc.) fails., AgentEvent, AgentPlan, AgentSession, AgentSessionMessage (+73 more)
...
### Community 25 - "NotificationDispatcher"
Cohesion: 0.03
Nodes (78): ChatSendRequest, Send chat message request., NotificationDispatcher, Any, Path, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess). (+70 more)
...
### Community 27 - "TaskStore"
Cohesion: 0.04
Nodes (93): agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, tasks/models.py — Pydantic models for the task/issue system. (+85 more)

### Community 28 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (91): _load(), Import the governance layer once, recording why if it is absent., BudgetTracker, classify(), evaluate_call(), get_gate(), GovernanceGate, guard_tool_call() (+83 more)

### Community 29 - "TaskHarnessAdapter"
Cohesion: 0.04
...
### Community 30 - "WebsiteScanner"
Cohesion: 0.03
Nodes (82): main(), Returns (url, ok, summary)., _scan_one(), _content_contains_domain(), _hostname_contains(), _hostname_matches(), _is_safe_url(), _looks_like_bot_challenge() (+74 more)

### Community 31 - "LLMRequest"
Cohesion: 0.04
...
### Community 32 - "WorkflowOrchestrator"
Cohesion: 0.04
Nodes (93): CEOResult, _decompose_into_subtasks(), Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, reset_ceo_dispatcher(), ClassifyOutput, get_ceo_fallback_stats() (+85 more)
...
Cohesion: 0.03
Nodes (84): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, BudgetExceeded, governance_enabled(), Exception, Raised internally when a session exhausts a limit. Never escapes guard()., True when the governance layer should run at all. A single global off switch… (+76 more)

### Community 35 - "telegram_bot.py"
Cohesion: 0.04
Nodes (99): Convenience: resolve all four role models for a component. Returns a dict with…, resolve_component_role_models(), _configure(), _default(), main(), Set an env var only when the operator hasn't already provided one., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview() (+91 more)
...
### Community 40 - "FreeBuffAgent"
Cohesion: 0.12
Nodes (87): BackgroundTask, BrowserSession, Async Playwright browser session. Usage:: session = BrowserSession() await…, True if Playwright is installed and ready to use., CommitAttribution, CommitTracker, Path, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata… (+79 more)

### Community 41 - "AgentSwarm"
Cohesion: 0.04
Nodes (62): agents/__init__.py — CRISPY multi-agent coding system., AgentProfile, _catalog_defaults(), _catalog_provider(), _get_defaults(), load_all_profiles(), make_architect_profile(), make_coder_profile() (+54 more)

### Community 42 - "SelfHealingAgent"
Cohesion: 0.04
Nodes (67): heal_signature(), HealingEvent, _now(), Any, agent/self_healing.py — Self-Healing Agent (closed-loop, Autonomy Charter G2)…, Translate external failure signals into improvement tasks and verify the fix…, Launch the background sweeper that resolves quiet verifying heals., Called when a CI workflow fails. (+59 more)

### Community 43 - "AgileSprint"
Cohesion: 0.03
Nodes (52): _bullets(), generate_backlog_retro(), generate_sprint_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive retro notes for ``sprint`` from its current metrics. Records… (+44 more)
...
### Community 49 - "resolve_active_brain"
Cohesion: 0.04
Nodes (86): BrainResolution, provider_api_key(), Return the live API key for *provider* (env-only — never persisted)., get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache() (+78 more)

### Community 50 - "TrendWatcher"
Cohesion: 0.06
Nodes (41): Any, AsyncClient, Path, agent/trend_watcher.py — Internet-connected AI trend intelligence. Fetches from…, Fetches AI trend signals from many public sources and surfaces relevant ones., Fetch all sources in parallel; return new alerts sorted by relevance., Fan trends out to onboarded companies whose stack matches (G4). For each…, Dispatch high-relevance alerts to the Hermes sidecar for action. Only… (+33 more)
...
Cohesion: 0.04
Nodes (71): Synchronize the Company Graph. Re-scans all websites and repositories, updates…, sync_company_graph(), autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, CompanyGraphSnapshot, A point-in-time snapshot of a Company Graph for history and rollback., Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap() (+63 more)

### Community 53 - "test_phase6_workflow.py"
Cohesion: 0.04
...
### Community 55 - "RenderDeploy"
Cohesion: 0.05
Nodes (44): list_specs(), MCPServerSpec, _not_dialable(), Any, Report a server the backend structurally cannot reach. These servers are real…, Measure one server. Never raises — a probe failure is a status, not an error., Measure every declared server concurrently and return dashboard rows. Probes…, Return the catalogue without probing. Cheap; used by tests and docs. (+36 more)

### Community 56 - "test_user_research_skill.py"
Cohesion: 0.06
Nodes (45): analyze_qualitative(), analyze_quantitative(), auto_register(), _classify_sentiment(), _extract_keywords(), Any, BaseModel, field_validator (+37 more)
...
Cohesion: 0.05
Nodes (58): AgileManager, Manages multiple agile sprints with velocity tracking., Predict next sprint velocity from historical data., Number of managed sprints., add_initiative(), AllocationOut, BoardOut, get_board() (+50 more)

### Community 60 - "SecretRecord"
Cohesion: 0.06
Nodes (57): _can_read(), _can_write(), create_secret(), _decrypt(), delete_secret(), _encrypt(), _get_master_key(), get_secret_metadata() (+49 more)

### Community 61 - "getBackendUrl"
Cohesion: 0.04
Nodes (49): completeSetup(), createSecret(), detectHardwareForSetup(), detectModelsForSetup(), getApiUrl(), getBackendUrl(), getDefaultBackendUrl(), getMe() (+41 more)

### Community 62 - "test_loop_registry.py"
Cohesion: 0.05
Nodes (64): audit_drift(), _cmd_audit(), DriftReport, _grade(), load_registry(), load_registry_sync(), loop_readiness(), LoopRegistry (+56 more)

### Community 63 - "WorkflowRun"
Cohesion: 0.04
Nodes (41): _NoopStore, Any, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs…, Push a Telegram approval-gate notification when a run pauses (Charter G1).…, G5: resolve how a run should land from the company's DeliveryPolicy. Returns…, Persist first-merge consent once an operator approves a ``telegram_gate``. No-… (+33 more)

### Community 64 - "get_runtime_manager"
Cohesion: 0.05
...
### Community 68 - "ControlPlanePage.js"
Cohesion: 0.05
Nodes (52): createAgent(), deleteAgent(), getActivity(), getCostAttribution(), getDecisionLog(), getDueSoonTasks(), getSavings(), getStats() (+44 more)

### Community 69 - "CEOSupervisor"
Cohesion: 0.05
Nodes (52): brain_availability_summary(), Non-secret answer to "can the brain answer a request right now?". Three callers…, get_ceo_dispatcher(), Return the shared CEODispatcher singleton., CEOSupervisor, _env_flag(), _env_int(), Any (+44 more)
...
### Community 72 - "PortfolioManager"
Cohesion: 0.04
Nodes (35): CapacityAllocation, PortfolioManager, Result of fitting initiatives into a fixed capacity by WSJF priority., Total job size of initiatives that fit within capacity., Unused capacity after committing the selected initiatives., Fraction of capacity consumed (0.0–1.0)., Manages a portfolio of initiatives with WSJF prioritisation and roadmapping., Create and register a new initiative, returning it. (+27 more)

### Community 73 - "get_company_access"
Cohesion: 0.05
Nodes (69): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), generate_onboarding_questions() (+61 more)
...
### Community 75 - "ChatPage.js"
Cohesion: 0.06
Nodes (59): cancelAgentChatJob(), chatSend(), deleteSession(), getAccessToken(), getAgentChatJob(), getAuthHeaders(), getSession(), listProviderModels() (+51 more)

### Community 76 - "test_runtime_governance.py"
Cohesion: 0.07
...
### Community 77 - "brain_failover.py"
Cohesion: 0.05
...
Cohesion: 0.07
Nodes (60): ResumeRequest, AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState, FailedJob, BaseModel, Enum (+52 more)

### Community 79 - "SeoAuditRequest"
Cohesion: 0.04
...
Cohesion: 0.05
Nodes (55): get_improvement_loop(), Any, Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., get_trend_watcher(), _await_hermes_ready(), BackgroundServices, _env_float() (+47 more)

### Community 87 - "agent/workspace.py"
Cohesion: 0.06
...
### Community 89 - "PromptCacheManager"
Cohesion: 0.05
Nodes (34): FailureCategory, E2: Classify a failure from its description text. Order matters:…, Classified failure types for targeted self-healing (E2)., CostAttributor, Tracks and attributes LLM costs per model, phase, and provider. Usage:: attr =…, Clear all usage records., get_current_trace_id(), langfuse_metadata_with_trace() (+26 more)
...
Cohesion: 0.06
Nodes (32): get_tool_registry(), Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed., Look up a tool by name., Return all registered tools. (+24 more)
...
### Community 96 - "render_ops.py"
Cohesion: 0.04
Nodes (48): IssueSeverity, get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), _playwright_configured(), _probe_http(), _probe_internal(), _probe_render() (+40 more)

### Community 97 - "clear_cooldowns"
Cohesion: 0.06
Nodes (44): clear_cooldowns(), get_dead_models(), is_provider_on_cooldown(), mark_provider_failed(), Snapshot of active dead-model entries {provider_id/model: expiry_ts}. Also…, Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown., Clear all cooldown entries (useful for testing). Delegates to… (+36 more)
...
### Community 99 - "LLMRouter"
Cohesion: 0.06
Nodes (35): Attempt, payload_key(), Exact-match cache key over the fields that change the answer. Routing…, text_key(), Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any (+27 more)
...
### Community 102 - "test_self_heal_v2.py"
Cohesion: 0.05
...
### Community 104 - "get_self_healing_agent"
Cohesion: 0.06
Nodes (50): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+42 more)

### Community 105 - "Artifact"
Cohesion: 0.07
Nodes (40): AgentRole, agents/swarm.py — AgentSwarm: routes workflow phases to the correct agent. The…, When coder == reviewer model, swarm should log a warning., TestCheckRun, TestSlice, workflow/artifact_store.py — Durable artifact persistence. Artifacts are stored…, _extract_slices_from_plan(), workflow/engine.py — WorkflowEngine: CRISPY phase sequencer. The engine is the… (+32 more)

### Community 106 - "FinancialMetrics"
Cohesion: 0.06
...
### Community 108 - "test_governance_api.py"
Cohesion: 0.06
Nodes (57): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), get_approval_store(), get_audit_log() (+49 more)

### Community 109 - "BrainWatchdog"
Cohesion: 0.05
Nodes (42): _brain_provider_status(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., _served_models(), provider_key_present(), True when the env var for *provider*'s key is set (or it's Ollama)., Any, One-shot self-healing pass. 1. Checks if the active brain provider is in a… (+34 more)

### Community 110 - "TasksPage.js"
Cohesion: 0.05
Nodes (48): addTaskComment(), approveTaskCheckpoint(), approveTaskExecution(), clarifyTask(), createQuickNote(), createSprint(), createTask(), escalateTask() (+40 more)
...
### Community 112 - "api_keys_for"
Cohesion: 0.05
Nodes (34): provider_api_keys(), Every API key configured for *provider*, primary first. Reads ``base_env`` then…, _build_request(), _provider_keys(), Every configured key for *provider*, primary first. Returns an empty list when…, Return ``(url, headers, is_anthropic_native)`` for *provider*. Anthropic's…, api_keys_for(), Per-provider API key rotation — the one lever that adds capacity. Every other… (+26 more)

### Community 113 - "AuditLog"
Cohesion: 0.05
Nodes (49): Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, Any, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., One governed action, fully described. Field order follows the… (+41 more)

### Community 114 - "FeatureMaturity"
Cohesion: 0.08
Nodes (33): PreflightReport, AgentJob, make_isolated_workspace(), Path, Create an isolated workspace directory under *root*. This is the legacy path…, _workspace_component(), Create, open, and lifecycle-manage per-job isolated workspaces. All workspaces…, Resolve *relative* within *ws*.source and reject traversal/symlink escapes.… (+25 more)

### Community 115 - "WebReach"
Cohesion: 0.07
...
### Community 117 - "test_knowledge_sync.py"
Cohesion: 0.08
...
Cohesion: 0.05
Nodes (40): createProvider(), deleteProvider(), getBrainConfig(), getLocalBrainState(), getProviderPolicy(), getRenderHealth(), getRenderOpsStatus(), patchBrainConfig() (+32 more)

### Community 119 - "DetectedSystem"
Cohesion: 0.05
...
### Community 123 - "MCPClient"
Cohesion: 0.06
Nodes (25): MCPClient, Any, RuntimeError, Thin async MCP client with open/close circuit breaker. Thread-safe only within…, Full URL of the JSON-RPC endpoint this client posts to., Build the request headers shared by ``_rpc`` and ``notify``. ``Accept`` lists…, Propagate the calling agent's identity across the process boundary. The MCP…, Attach the agent identity whose actions this client executes. (+17 more)
...
### Community 126 - "get_feature_matrix"
Cohesion: 0.05
...
### Community 127 - "V5App.jsx"
Cohesion: 0.04
Nodes (33): API, getLoops(), ActivationGate(), activityToAlert(), AlertsBell(), priorityConfig, _relativeTime(), typeIcon (+25 more)

### Community 128 - "probe_model_liveness"
Cohesion: 0.05
...
### Community 134 - "ceo_dispatcher.py"
Cohesion: 0.06
Nodes (41): _complexity_rank(), _merge_changed_files(), _offload(), Any, Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask. (+33 more)
...
### Community 136 - "Initiative"
Cohesion: 0.06
Nodes (41): Initiative, _bug_scores(), _clean(), _default_repo(), _env_github_token(), estimate_job_size(), fetch_github_signals(), fetch_research_alerts() (+33 more)

### Community 137 - "InferenceCache"
Cohesion: 0.06
...
### Community 139 - "RepowiseIntelligence"
Cohesion: 0.06
...
### Community 147 - "Platform Guide — the full tour"
Cohesion: 0.04
...
### Community 148 - "StreamingDeltaReconstructor"
Cohesion: 0.07
...
Cohesion: 0.05
Nodes (44): Await one warm-up step, deferring it to the background if it overruns.…, _warmup_step(), defer_to_background(), Any, services/warmup.py — bounded startup warm-up steps. Extracted from…, Let an overrunning step finish out of band without leaking or warning twice., Await ``coro`` until ``deadline`` (a loop timestamp); defer it past that.…, warmup_step() (+36 more)

### Community 156 - "persist_plan_spec"
Cohesion: 0.07
...
### Community 158 - "ContextWindowManager"
Cohesion: 0.08
...
### Community 160 - "_resolve_brain_provider"
Cohesion: 0.06
...
### Community 167 - "test_e2b_task_wiring.py"
Cohesion: 0.08
...
Cohesion: 0.04
Nodes (45): [5.0.0], Added, Added, Added, Added, Added, Added, Added (+37 more)

### Community 170 - "[Unreleased]"
Cohesion: 0.04
Nodes (45): [5.0.0], Added, Added, Added, Added, Added, Added, Added (+37 more)

### Community 171 - "OnboardingScreen.jsx"
Cohesion: 0.05
...
### Community 172 - "test_agent_free_brain.py"
Cohesion: 0.05
...
### Community 175 - "test_telegram_freebuff.py"
Cohesion: 0.07
Nodes (41): _parse_callback(), _parse_user_ids(), _process_callback(), Extract numeric Telegram user IDs from a raw env value, tolerantly. Accepts…, Resolve the ALLOWED/ADMIN Telegram user-ID sets. ``TELEGRAM_CHAT_ID`` is the…, Parse callback_data of the form ``fb:<action>[:<arg>]`` (FreeBuff) or…, Accept / reject keyboard shown after a FreeBuff plan is generated., Handle an inline-button press for the FreeBuff accept/reject/model flow. (+33 more)
...
### Community 178 - "test_agent_tool_governance.py"
Cohesion: 0.08
...
### Community 179 - "test_platform_controls.py"
Cohesion: 0.06
...
### Community 182 - "test_video_transcript.py"
Cohesion: 0.05
...
### Community 184 - "test_quick_note.py"
Cohesion: 0.09
...
### Community 185 - "Part A — CodeRabbit review fixes for this PR (do first, small)"
Cohesion: 0.05
...
### Community 186 - "Docker Agent Runtimes Setup"
Cohesion: 0.05
...
### Community 190 - "Persistent Memory System"
Cohesion: 0.05
...
### Community 192 - "_build_default_manager"
Cohesion: 0.07
Nodes (37): _build_default_manager(), _env_flag(), Construct the default RuntimeManager. Production runtime (InternalAgentAdapter)…, Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., _make_task(), asyncio, Task, TaskStatus (+29 more)
...
### Community 194 - "test_portfolio_intake.py"
Cohesion: 0.09
...
### Community 195 - "compare_runtimes.py"
Cohesion: 0.07
...
### Community 196 - "test_crispy_workflow.py"
Cohesion: 0.07
...
### Community 205 - "control_overrides.py"
Cohesion: 0.08
...
### Community 207 - "configuration-reference.md"
Cohesion: 0.07
...
### Community 208 - "TestEstimateTokensForMessages"
Cohesion: 0.06
...
### Community 212 - "cache.py"
Cohesion: 0.09
Nodes (21): Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, Any, T (+13 more)

### Community 213 - "ServiceDaemon"
Cohesion: 0.09
...
### Community 217 - "test_backend_server_features.py"
Cohesion: 0.06
...
### Community 219 - "test_control_plane_api.py"
Cohesion: 0.07
...
### Community 224 - "REWRITE_PLAN.md — Phased Migration Strategy"
Cohesion: 0.06
...
### Community 226 - "test_all_providers_discovery.py"
Cohesion: 0.16
...
### Community 227 - "test_persistent_memory.py"
Cohesion: 0.06
...
Cohesion: 0.12
Nodes (32): IssueCategory, Enum, str, cluster_friction(), clusters_to_issues(), collect_friction_events(), FrictionCluster, FrictionEvent (+24 more)
...
### Community 240 - "local_controller.py"
Cohesion: 0.12
...
### Community 241 - "test_live_server.py"
Cohesion: 0.22
...
### Community 242 - "ContextCompressor"
Cohesion: 0.11
...
### Community 245 - "ResourceWatchdog"
Cohesion: 0.10
...
### Community 249 - "test_daily_2026_06_04.py"
Cohesion: 0.10
...
### Community 251 - "RateLimitTracker"
Cohesion: 0.10
...
### Community 252 - "control_registry.py"
Cohesion: 0.11
...
### Community 253 - "test_freebuff.py"
Cohesion: 0.09
...
### Community 254 - "test_microagents.py"
Cohesion: 0.15
...
### Community 255 - "Security Analysis — local-llm-server"
Cohesion: 0.06
...
### Community 256 - "Langfuse Observability Guide"
Cohesion: 0.06
...
### Community 258 - "OrchestratorQueue"
Cohesion: 0.08
...
### Community 261 - "High-Agency Frontend Skill"
Cohesion: 0.06
Nodes (30): 10. FINAL PRE-FLIGHT CHECK, 1. ACTIVE BASELINE CONFIGURATION, 2. DEFAULT ARCHITECTURE & CONVENTIONS, 3. DESIGN ENGINEERING DIRECTIVES (Bias Correction), 4. CREATIVE PROACTIVITY (Anti-Slop Implementation), 5. PERFORMANCE GUARDRAILS, 6. TECHNICAL REFERENCE (Dial Definitions), 7. AI TELLS (Forbidden Patterns) (+22 more)

### Community 262 - "Quick-Note GitHub Issues Processing - Session Summary"
Cohesion: 0.06
...
### Community 267 - "TestRecordUsageAndStats"
Cohesion: 0.06
...
### Community 271 - "tests/test_browser.py"
Cohesion: 0.10
...
### Community 273 - "test_colibri_provider.py"
Cohesion: 0.12
...
### Community 274 - "RuntimeHealthService"
Cohesion: 0.09
...
### Community 279 - "TestPlanResearch"
Cohesion: 0.10
...
### Community 280 - "Configuration Reference"
Cohesion: 0.07
...
### Community 283 - "parse_event_stream"
Cohesion: 0.09
...
### Community 285 - "test_scheduler_hydration_bounded.py"
Cohesion: 0.09
...
### Community 286 - "test_internal_agent_did_work.py"
Cohesion: 0.12
...
### Community 287 - "Python Dependencies (`requirements.txt`)"
Cohesion: 0.07
...
### Community 288 - "Technical Debt Register — local-llm-server"
Cohesion: 0.07
...
### Community 289 - "get_skill_bindings"
Cohesion: 0.12
...
### Community 291 - "BrainFailoverManager"
Cohesion: 0.10
...
### Community 292 - "test_crispy_burn_in.py"
Cohesion: 0.07
...
### Community 293 - "TestBrainFailoverModelUpdates"
Cohesion: 0.12
...
### Community 295 - "test_skill_registry_boot_refresh.py"
Cohesion: 0.12
...
### Community 297 - "test_scaffolding.py"
Cohesion: 0.11
...
### Community 298 - "TestAgentRunnerExecution"
Cohesion: 0.15
Nodes (24): complete_sprint(), create_sprint(), _get_mgr(), get_velocity(), list_sprints(), Any, BaseModel, get (+16 more)
...
### Community 303 - "generate_context.py"
Cohesion: 0.12
...
### Community 304 - "test_render_mcp.py"
Cohesion: 0.13
...
### Community 306 - "NIMConnectionPool"
Cohesion: 0.10
...
### Community 307 - "isolated_telegram_config"
Cohesion: 0.11
...
### Community 309 - "webui/frontend/package.json"
Cohesion: 0.07
...
### Community 311 - "test_terminal.py"
Cohesion: 0.10
...
### Community 313 - "Performance Analysis — local-llm-server"
Cohesion: 0.08
...
### Community 314 - "test_ceo_router.py"
Cohesion: 0.15
Nodes (25): build_ceo_router(), Any, APIRouter, ledger(), _make_app(), FastAPI, tests/test_ceo_router.py — auth and behaviour for /api/ceo/*. Follows the…, The happy path — the branch that actually spends provider budget. (+17 more)

### Community 315 - "LLM Router — troubleshooting"
Cohesion: 0.08
...
### Community 316 - "test_phase5_doctor.py"
Cohesion: 0.08
...
### Community 330 - "SeoCheckDefinition"
Cohesion: 0.10
...
### Community 331 - "traffic_director.py"
Cohesion: 0.11
...
### Community 332 - "ClaudeCodeAdapter"
Cohesion: 0.14
...
### Community 335 - "test_telegram_approval_e2e.py"
Cohesion: 0.13
...
### Community 336 - "test_daily_automation_2026_08_03.py"
Cohesion: 0.11
...
### Community 338 - "_FakePersistence"
Cohesion: 0.14
...
### Community 339 - "test_service_token.py"
Cohesion: 0.08
...
### Community 340 - "TestStreamableHTTPTransport"
Cohesion: 0.13
...
### Community 356 - "TestHarnessAdapter"
Cohesion: 0.08
...
### Community 358 - "test_telegram_mutating_commands.py"
Cohesion: 0.08
...
### Community 361 - "monitor_colibri.py"
Cohesion: 0.15
...
### Community 362 - "Agent Governance Guide"
Cohesion: 0.09
...
### Community 363 - "The fifteen strategies"
Cohesion: 0.09
...
### Community 366 - "provider_max_rpm"
Cohesion: 0.13
...
### Community 368 - "test_all_features.py"
Cohesion: 0.09
...
### Community 370 - "test_monitor_lib.py"
Cohesion: 0.11
...
### Community 372 - "test_mostly_failed_steps.py"
Cohesion: 0.12
...
### Community 374 - "classify_direct_chat_intent"
Cohesion: 0.13
...
### Community 378 - "PriorityTaskQueue"
Cohesion: 0.13
...
### Community 382 - "openclaw_gateway.py"
Cohesion: 0.15
...
### Community 383 - "_resolve_user_github_token"
Cohesion: 0.18
...
### Community 387 - "test_chat_mode_regressions.py"
Cohesion: 0.18
...
### Community 388 - "system_instruction"
Cohesion: 0.16
...
### Community 390 - "test_issue_triage.py"
Cohesion: 0.17
...
### Community 394 - "test_local_controller.py"
Cohesion: 0.17
...
### Community 395 - "run_trend_analysis"
Cohesion: 0.19
...
### Community 396 - "test_unit5_ui_provider_surface.py"
Cohesion: 0.10
...
### Community 400 - "AGENTS.md — Source of Truth for All AI Agents"
Cohesion: 0.10
...
### Community 411 - "Dynamic Model Routing"
Cohesion: 0.10
...
### Community 413 - "ControlsScreen.jsx"
Cohesion: 0.15
...
### Community 415 - "output_filter.py"
Cohesion: 0.10
...
### Community 418 - "DecisionsStoreTests"
Cohesion: 0.11
...
### Community 421 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.11
...
### Community 422 - "test_llm_router_tpm.py"
Cohesion: 0.15
...
### Community 423 - "test_tasks_cache_ttl_env.py"
Cohesion: 0.21
...
### Community 424 - "test_voice_pipeline.py"
Cohesion: 0.14
...
### Community 427 - "test_contract_enforcement.py"
Cohesion: 0.15
...
### Community 429 - "_extract_tech_relevance"
Cohesion: 0.17
...
### Community 430 - "Skill: fabric-patterns"
Cohesion: 0.11
...
### Community 431 - "Analysis & Synthesis Instructions"
Cohesion: 0.11
Nodes (18): 1. Define the Atmosphere, 2. Map the Color Palette, 3. Establish Typography Rules, 4. Define the Hero Section, 5. Describe Component Stylings, 6. Define Layout Principles, 7. Define Responsive Rules, 8. Encode Motion Philosophy (+10 more)

### Community 432 - "Production Readiness Assessment — local-llm-server"
Cohesion: 0.11
...
### Community 433 - "test_dashboard_cache.py"
Cohesion: 0.18
...
### Community 434 - "TestNormalizeResponseFormat"
Cohesion: 0.11
...
### Community 435 - "Skill: fabric-patterns"
Cohesion: 0.11
Nodes (18): 1. Ensure Pattern Directory Exists, 2. List Available Patterns, 3. Retrieve a Pattern, 4. Apply a Pattern with Variables, 5. Stitch Patterns Together, 6. Create New Patterns, Acceptance Checks, Directory Structure (+10 more)

### Community 436 - "db/__init__.py"
Cohesion: 0.15
...
### Community 437 - "Admin Dashboard Guide"
Cohesion: 0.11
...
### Community 438 - "Implementation Plan"
Cohesion: 0.11
...
### Community 442 - "build_workflow.py"
Cohesion: 0.33
Nodes (18): _c(), _get(), _header(), main(), _make_headers(), _phase_icon(), _post(), _print_phases() (+10 more)

### Community 443 - ".record_usage"
Cohesion: 0.11
Nodes (12): CostReport, get_cost_attributor(), Any, Record a single LLM call's usage., Batch record multiple usage entries. Returns number recorded., Estimate USD cost for a given model and token count. Looks up the per-model…, Generate a full per-model cost attribution report., Return usage records from the last N seconds. (+4 more)
...
### Community 446 - "TestSelfHealingInfrastructureClassification"
Cohesion: 0.19
...
### Community 447 - "test_fabric_patterns.py"
Cohesion: 0.11
...
### Community 448 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
### Community 449 - "test_schedule_backlog_drain.py"
Cohesion: 0.14
...
### Community 450 - "validate_session_id"
Cohesion: 0.16
...
### Community 451 - "agency.py"
Cohesion: 0.16
Nodes (15): _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _fetch_github_quick_notes(), _gh_repo(), _gh_token(), _now_str() (+7 more)

### Community 452 - "ErrorInterceptorMiddleware"
Cohesion: 0.18
...
### Community 456 - "Comprehensive Skill Index (By Category)"
Cohesion: 0.11
...
### Community 457 - "Agent Skill: Principal UI/UX Architect & Motion Choreographer (Awwwards-Tier)"
Cohesion: 0.11
Nodes (17): 1. Meta Information & Core Directive, 2. THE "ABSOLUTE ZERO" DIRECTIVE (STRICT ANTI-PATTERNS), 3. THE CREATIVE VARIANCE ENGINE, 4. HAPTIC MICRO-AESTHETICS (COMPONENT MASTERY), 5. MOTION CHOREOGRAPHY (FLUID DYNAMICS), 6. PERFORMANCE GUARDRAILS, 7. EXECUTION PROTOCOL, 8. PRE-OUTPUT CHECKLIST (+9 more)

### Community 458 - "Component Map"
Cohesion: 0.11
...
### Community 459 - "TestRenderRouter"
Cohesion: 0.14
...
### Community 464 - "Part A — Health Report"
Cohesion: 0.11
...
### Community 465 - "KnowledgeScreen.jsx"
Cohesion: 0.14
Nodes (13): getCompanyGraph(), actorColors, AddSourceForm(), buildGraphElements(), CompanyGraphPanel(), GRAPH_CATEGORY_ORDER, GRAPH_NODE_COLORS, KnowledgeScreen() (+5 more)
...
### Community 468 - "Delegation Plan (agent-ready work packages)"
Cohesion: 0.11
...
### Community 471 - "sync_readme_gallery.py"
Cohesion: 0.22
...
### Community 472 - "test_skill_executors_live.py"
Cohesion: 0.16
...
### Community 474 - "test_company_api.py"
Cohesion: 0.11
...
### Community 475 - "test_langfuse_agency_wide.py"
Cohesion: 0.11
...
### Community 477 - "TestStopSlopChecker"
Cohesion: 0.11
...
### Community 478 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 479 - "handle_workflow_ide_chat"
Cohesion: 0.18
...
### Community 480 - "test_workspace_ref_path_preflight.py"
Cohesion: 0.17
Nodes (14): PreflightIssue, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), _fake_user(), _make_fake_doctor() (+6 more)

### Community 481 - "harness_spec.py"
Cohesion: 0.19
...
### Community 483 - "._fetch_flat_skill_file"
Cohesion: 0.21
...
### Community 484 - "SKILL: Industrial Brutalism & Tactical Telemetry UI"
Cohesion: 0.12
...
### Community 485 - "Skill: data-quality-audit"
Cohesion: 0.12
...
### Community 486 - "What "Slop" Looks Like"
Cohesion: 0.12
...
### Community 487 - "test_admin_local_brain_router.py"
Cohesion: 0.18
...
### Community 488 - "local_brain_router.py"
Cohesion: 0.19
...
### Community 489 - "Section-by-Section Acceptance Criteria"
Cohesion: 0.12
...
### Community 490 - "redact_connection_url"
Cohesion: 0.17
...
### Community 492 - "agent_readiness_audit.py"
Cohesion: 0.21
...
### Community 496 - "test_shared_state.py"
Cohesion: 0.17
...
### Community 498 - "test_activation_api.py"
Cohesion: 0.21
...
### Community 499 - "test_health_endpoints.py"
Cohesion: 0.17
...
### Community 500 - "test_keepalive.py"
Cohesion: 0.18
...
### Community 501 - "test_openclaw_endpoints.py"
Cohesion: 0.12
...
### Community 508 - "admin_digest_router.py"
Cohesion: 0.20
...
### Community 509 - "_valid_login_state"
Cohesion: 0.24
...
### Community 510 - "Skill: repowise-intelligence"
Cohesion: 0.12
Nodes (15): 1. Graph Intelligence (Dependency Graph), 2. Git Intelligence, 3. Documentation Intelligence, 4. Decision Intelligence, Acceptance Checks, Directory Structure, Example Usage, Implementation Approach (+7 more)

### Community 511 - "The 10-Step Workflow"
Cohesion: 0.12
...
### Community 512 - "Contributing to local-llm-server"
Cohesion: 0.12
...
### Community 513 - "CEO Micro-Management"
Cohesion: 0.12
...
### Community 514 - "467 Brutal Audit — File-by-File Status"
Cohesion: 0.12
...
### Community 516 - "Runbook — Apply the Fast Free NVIDIA Brain to Render (TASK 2)"
Cohesion: 0.12
...
### Community 517 - "implement_agent.py"
Cohesion: 0.16
...
### Community 522 - "test_telegram_auto_approve.py"
Cohesion: 0.21
...
### Community 523 - "ManagedAgentDreams"
Cohesion: 0.22
...
### Community 525 - "_process_task_callback"
Cohesion: 0.28
...
### Community 527 - "test_dockerfile_ships_root_modules.py"
Cohesion: 0.17
...
### Community 528 - "test_frontend_deployment_guards.py"
Cohesion: 0.20
...
### Community 529 - "test_glm52_brain.py"
Cohesion: 0.12
...
### Community 530 - "test_local_brain_state.py"
Cohesion: 0.12
...
### Community 534 - "test_telegram_diag_endpoint.py"
Cohesion: 0.12
...
### Community 536 - "CollaborationContext"
Cohesion: 0.19
...
### Community 537 - "Skill: agent-harness"
Cohesion: 0.13
...
### Community 538 - "Skill: checkpoint-strategy"
Cohesion: 0.13
...
### Community 540 - "Skill: local-ai-query"
Cohesion: 0.13
...
### Community 541 - "Skill: parallel-agents"
Cohesion: 0.13
...
### Community 542 - "Skill: parallel-worktrees"
Cohesion: 0.13
...
### Community 543 - "Design System: Taste Standard"
Cohesion: 0.13
...
### Community 546 - "Skill: user-research"
Cohesion: 0.13
...
### Community 547 - "Agency Core — Progress & Resume Log"
Cohesion: 0.13
...
### Community 548 - "Attention Mechanisms Internals"
Cohesion: 0.13
...
### Community 550 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 551 - "_push_down_where"
Cohesion: 0.14
...
### Community 552 - "router/health.py"
Cohesion: 0.20
...
### Community 554 - "AdminDigestRouterAuthTests"
Cohesion: 0.23
...
### Community 555 - "TestDecisionsBotLinks"
Cohesion: 0.17
...
### Community 558 - "clear_wizard_state_cache"
Cohesion: 0.21
...
### Community 559 - "e2e/test_browser.py"
Cohesion: 0.23
...
### Community 560 - "test_regression.py"
Cohesion: 0.13
...
### Community 562 - "test_dockerfile_ships_config_dir.py"
Cohesion: 0.14
...
### Community 568 - "Skill: lr-schedule-advisor"
Cohesion: 0.14
...
### Community 572 - "Checks Performed"
Cohesion: 0.14
...
### Community 573 - "Skill: training-stability-monitor"
Cohesion: 0.14
...
### Community 574 - "Skill: branch-cleanup"
Cohesion: 0.14
...
### Community 575 - "Skill: perplexity — Web Research via Perplexity API"
Cohesion: 0.14
...
### Community 578 - "Quick-Note Issues Processing Summary"
Cohesion: 0.14
...
### Community 579 - "Implementation Plan — DB-persisted, UI-switchable Brain (no redeploy)"
Cohesion: 0.14
...
### Community 581 - "Render MCP — autonomous platform debugging and environment monitoring"
Cohesion: 0.14
...
### Community 582 - "Runbook: Auto-Resume After Cooldown / Interruption"
Cohesion: 0.14
...
### Community 583 - "SEO / GEO / AIO Audit Engine"
Cohesion: 0.14
...
### Community 587 - "SchedulerStore"
Cohesion: 0.20
Nodes (6): Any, Delete a persisted job., Durable store for AgentScheduler jobs. Delegates to the shared DB (MongoDB or…, Persist a scheduled job (create or update)., Load all persisted jobs (rehydrate on boot)., SchedulerStore

### Community 588 - "CacheStats"
Cohesion: 0.14
Nodes (7): CacheEntry, CacheStats, get_prompt_cache(), Clear all cache entries. Returns count cleared., Return the module-level PromptCacheManager singleton., A single cached prefix entry., Prompt cache statistics.

### Community 589 - "_RedisBackend"
Cohesion: 0.22
...
### Community 590 - "test_critical_flows.py"
Cohesion: 0.29
...
### Community 592 - "test_openclaw_gateway.py"
Cohesion: 0.14
...
### Community 597 - "Skill: Brain Dump"
Cohesion: 0.15
...
### Community 600 - "Skill: duplicate-thread"
Cohesion: 0.15
...
### Community 601 - "Skill: Email Triage"
Cohesion: 0.15
...
### Community 604 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
...
### Community 605 - "Skill: prompt-library"
Cohesion: 0.15
...
### Community 606 - "Skill: prompt-transparency"
Cohesion: 0.15
...
### Community 608 - "Skill: scope-guard"
Cohesion: 0.15
...
### Community 612 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
Nodes (12): Acceptance Checks, Claude's query protocol (use this instead of Read tool for exploration):, Graph Artifacts — What to Commit, How to Use the Graph (Token Savings Protocol), Installation (one-time per machine), Instead of reading raw files:, Key commands:, Relationship to repowise-intelligence Skill (+4 more)

### Community 613 - "Skill: platform-setup — Autonomous Agency Bootstrap"
Cohesion: 0.15
...
### Community 614 - "Device compatibility and model picks"
Cohesion: 0.15
...
### Community 615 - "Autonomy Uplift — Living Roadmap & Detailed Implementation Specs"
Cohesion: 0.15
...
### Community 616 - "OpenClaw — iOS Control of the Agency (Single-Service Free-Tier Deploy)"
Cohesion: 0.15
...
### Community 620 - "Agent Transparency Report"
Cohesion: 0.15
...
### Community 621 - "update_provider_policy"
Cohesion: 0.19
...
### Community 623 - "_InMemoryBackend"
Cohesion: 0.18
...
### Community 624 - "TestModelCostTableUpdates"
Cohesion: 0.26
...
### Community 630 - "WorkspaceManifest"
Cohesion: 0.17
...
### Community 632 - "skill_registry.py"
Cohesion: 0.17
...
### Community 638 - "Skill: system-prompt-audit"
Cohesion: 0.17
...
### Community 639 - "Skill: task-alive-updates"
Cohesion: 0.17
...
### Community 643 - "14. Standing Instructions — Universal Agent Discipline"
Cohesion: 0.17
...
### Community 658 - "check_container_posture.py"
Cohesion: 0.26
...
### Community 659 - "Kimi Web-Bridge Service"
Cohesion: 0.17
...
### Community 660 - "_resolve_push_token"
Cohesion: 0.24
...
### Community 661 - "test_agile_api.py"
Cohesion: 0.17
...
### Community 662 - "test_app_settings.py"
Cohesion: 0.21
...
### Community 665 - "test_skill_registry.py"
Cohesion: 0.20
...
### Community 666 - "TestAnthropicPayloadStructuredOutput"
Cohesion: 0.29
...
### Community 667 - "test_task_clarification.py"
Cohesion: 0.17
...
### Community 668 - "TestDiagCommand"
Cohesion: 0.27
Nodes (3): Drive _process_update with a /diag message and return the response. Restores…, ``/diag`` behaviour under admin + non-admin + empty-allowlist states., TestDiagCommand

### Community 669 - "test_telegram_inbound.py"
Cohesion: 0.17
Nodes (7): _patch_send_message(), tests/test_telegram_inbound.py Pytest coverage for the Step 1 inbound-routing…, ``_route_plain_text`` classifies and dispatches per the documented map., Replace ``_send_message`` and ``_send_message_with_id`` on the bot module.…, Lower-level smoke tests for inbound_router plumbed through tih calls. These…, TestClassifySimpleRouting, TestPlainTextRouting

### Community 670 - "validate_job_id"
Cohesion: 0.18
Nodes (4): parametrize, TestPathTraversalPrevention, Validate and return a job ID, or raise InvalidJobIdError., validate_job_id()

### Community 671 - "EvalHarness"
Cohesion: 0.24
...
### Community 673 - "_extractive_compress"
Cohesion: 0.18
...
### Community 675 - "cowork_session.py"
Cohesion: 0.24
...
### Community 677 - "Skill: pro-workflow"
Cohesion: 0.18
...
### Community 680 - "Skill: resource-panel"
Cohesion: 0.18
...
### Community 681 - "Skill: sandboxed-exec"
Cohesion: 0.18
...
### Community 683 - "ECC Harness Patterns Skill"
Cohesion: 0.18
...
### Community 686 - "Stop-Slop Quality Skill"
Cohesion: 0.18
...
### Community 688 - "Agency Core — Ruthless Architecture Audit & Migration Plan"
Cohesion: 0.18
...
### Community 689 - "AUTONOMY_CHARTER.md"
Cohesion: 0.18
...
### Community 690 - "Tailored Onboarding, Editable Companies & Dynamic Roles"
Cohesion: 0.18
...
### Community 691 - "Issue #467 — Section 1: Pulled State + PR Inventory"
Cohesion: 0.18
...
### Community 693 - "Context: Agentic Agile + Portfolio Management"
Cohesion: 0.18
...
### Community 694 - "Deploy to Google Cloud Run"
Cohesion: 0.18
...
### Community 696 - "Sampling Strategies Internals"
Cohesion: 0.18
...
### Community 697 - "LLM Router — architecture"
Cohesion: 0.18
...
### Community 698 - "Killer TODO Roadmap — local-llm-server"
Cohesion: 0.18
...
### Community 699 - "NVIDIA NIM — Free Tier Setup"
Cohesion: 0.18
...
### Community 700 - "What to clean up"
Cohesion: 0.18
...
### Community 701 - "Worker Service — Operations Runbook"
Cohesion: 0.18
...
### Community 708 - "test_agent_readiness_audit.py"
Cohesion: 0.18
...
### Community 709 - "test_empirical_verify.py"
Cohesion: 0.49
...
### Community 713 - "Protocol: Premium Utilitarian Minimalism UI Architect"
Cohesion: 0.20
...
### Community 714 - "The 5-Step Wrap-Up Ritual"
Cohesion: 0.20
...
### Community 715 - "CLAUDE.md — Master Architect Operating Manual"
Cohesion: 0.20
...
### Community 716 - "Agent: Reviewer (Verifier)"
Cohesion: 0.20
...
### Community 717 - "Skill: Agentic Agile"
Cohesion: 0.20
...
### Community 718 - "Skill: browserbase-ui-test — Adversarial UI Testing"
Cohesion: 0.20
...
### Community 719 - "Skill: financial-analyst (Agentic CFO)"
Cohesion: 0.20
...
### Community 720 - "Graphiti Temporal Context Skill"
Cohesion: 0.20
...
### Community 721 - "Skill: seo-audit-report"
Cohesion: 0.20
...
### Community 722 - "ADR-008: LLMRouter — the single multi-provider routing gateway"
Cohesion: 0.20
...
### Community 727 - "CI Troubleshooting Runbook"
Cohesion: 0.20
...
### Community 729 - "capture_screens.py"
Cohesion: 0.29
...
### Community 730 - "build_tech_db.py"
Cohesion: 0.40
...
### Community 733 - "_GlobalsRestorer"
Cohesion: 0.22
Nodes (4): TestCase, _GlobalsRestorer, Snapshot/restore tb globals + TELEGRAM_POLLER_DISABLED env var., Replace ``_send_message`` for the next ``_process_update`` call.

### Community 734 - "test_bootstrap_source_id_index.py"
Cohesion: 0.27
...
### Community 735 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 736 - "TestZeroAttemptDiagnostics"
Cohesion: 0.29
...
### Community 737 - "TestSessionMemory"
Cohesion: 0.20
...
### Community 740 - "test_quick_note_engine.py"
Cohesion: 0.22
...
### Community 741 - "test_telegram_diag.py"
Cohesion: 0.24
Nodes (5): tests/test_telegram_diag.py Regression test for the new ``/diag`` (admin)…, The Operator Charter §"Telegram bot" silent-drop path MUST surface a…, Once we've warned once, subsequent silent drops must NOT spam the log., _run(), TestSilentDropRemediation

### Community 743 - "synthesize"
Cohesion: 0.36
...
### Community 745 - "_extract_workflow_relevance"
Cohesion: 0.33
...
### Community 749 - "Skill: learn-rule"
Cohesion: 0.22
...
### Community 751 - "._from_doc"
Cohesion: 0.22
Nodes (4): Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents., Return all agents in the system (admin use only).
...
### Community 764 - "Platform Controls"
Cohesion: 0.22
...
### Community 765 - "Release Procedure"
Cohesion: 0.22
...
### Community 766 - "V2.0 Modernization — Runbook"
Cohesion: 0.22
...
### Community 777 - "test_backend_requirements_cover_runtime_imports.py"
Cohesion: 0.25
...
### Community 778 - "test_changelog_parity_guard.py"
Cohesion: 0.22
...
### Community 780 - "TestGithubSignalHardening"
Cohesion: 0.22
...
### Community 785 - "navigation_metrics.py"
Cohesion: 0.32
...
### Community 790 - "ContributorState"
Cohesion: 0.25
...
### Community 791 - "quality_checker.py"
Cohesion: 0.32
...
### Community 803 - "Advisor Strategy — Local Proxy Handling"
Cohesion: 0.25
...
### Community 804 - "ceo-micromanagement.md"
Cohesion: 0.25
...
### Community 808 - "Free NVIDIA brain + UI-controlled provider policy + no silent spend"
Cohesion: 0.25
...
### Community 809 - "Issue #362: Nvidia repo setup"
Cohesion: 0.25
...
### Community 810 - "Issue #364: quick-note:https://www.marktechpost.com/2026/06/01/meet-memory-os-a-6-layer-open-source-memory-stack-built-on-top-of-hermes-agent/"
Cohesion: 0.25
...
### Community 811 - "Issue #378: quick-note:https://www.marktechpost.com/2026/06/02/tinyfish-launches-bigset-an-open-source-multi-agent-system-that-builds-structured-live-datasets-from-plain-english-descriptions/"
Cohesion: 0.25
...
### Community 812 - "Issue #379: quick-note:https://searchengineland.com/schema-markup-optimize-agentic-web-479080"
Cohesion: 0.25
...
### Community 813 - "Issue #380: quick-note:https://cursor.com/blog/cloud-agent-lessons"
Cohesion: 0.25
...
### Community 814 - "Issue #381: quick-note:https://www.xda-developers.com/claude-code-with-opus-48-is-expensive-but-i-made-it-efficient-with-my-local-ai-workflow/"
Cohesion: 0.25
...
### Community 815 - "Issue #382: quick-note:https://claude.com/blog/how-coderabbit-used-claude-to-build-an-agent-orchestration-system"
Cohesion: 0.25
...
### Community 816 - "Issue #383: quick-note:https://www.marktechpost.com/2026/05/29/hexo-labs-open-sources-sia-a-self-improving-agent-that-updates-both-the-harness-and-the-model-weights/"
Cohesion: 0.25
...
### Community 817 - "Issue #416: feat: Self-hosted Codebuff (freebuff) on free NVIDIA models + Telegram bot phone control"
Cohesion: 0.25
...
### Community 818 - "Issue #485: [Trend Digest] Week of 2026-06-08"
Cohesion: 0.25
...
### Community 819 - "Issue #488: quick-note:https://github.com/cookiy-ai/user-research-skill"
Cohesion: 0.25
...
### Community 820 - "Issue #491: Implement whatever is necessary from https://github.com/BehiSecc/awesome-claude-skills"
Cohesion: 0.25
...
### Community 821 - "Issue #493: Use the https://github.com/mvanhorn/last30days-skill skill to get the trend updated"
Cohesion: 0.25
...
### Community 822 - "Issue #495: Read https://www.anthropic.com/news/claude-fable-5-mythos-5 and understand if mythos or fable can be added to the repo"
Cohesion: 0.25
...
### Community 823 - "Issue #504: EPIC: Autonomy hardening — live-verified defects 2026-06-10"
Cohesion: 0.25
...
### Community 824 - "Issue #581: Sprint tracker: pending work after brand rename + mobile-first pass"
Cohesion: 0.25
...
### Community 826 - "Issue #657: quick-note:https://github.com/earendil-works/pi"
Cohesion: 0.25
...
### Community 827 - "Issue #659: quick-note:https://github.com/nex-agi/Nex-N2"
Cohesion: 0.25
...
### Community 828 - "Issue #660: quick-note:https://github.com/getsentry/sentry-for-ai"
Cohesion: 0.25
...
### Community 829 - "Issue #661: quick-note:https://github.com/XiaomiMiMo/MiMo-Code"
Cohesion: 0.25
...
### Community 830 - "Issue #664: quick-note:https://github.com/Grominet95/jarvis-OS"
Cohesion: 0.25
...
### Community 831 - "Issue #666: quick-note:https://github.com/porokka/jarvis-os"
Cohesion: 0.25
...
### Community 832 - "Issue #670: quick-note:https://github.com/perplexityai/bumblebee"
Cohesion: 0.25
...
### Community 833 - "Issue #672: quick-note:https://github.com/Chachamaru127/claude-code-harness"
Cohesion: 0.25
...
### Community 834 - "Issue #676: quick-note:https://github.com/WeiboAI/VibeThinker"
Cohesion: 0.25
...
### Community 835 - "Issue #820: quick-note:https://github.com/cobusgreyling/loop-engineering"
Cohesion: 0.25
...
### Community 836 - "Positional Encoding Internals"
Cohesion: 0.25
...
### Community 837 - "TOP 6 — Highest-ROI Items (Validated by Opus Research Agent)"
Cohesion: 0.25
...
### Community 838 - "SECTION A — Agent Efficiency (Hermes / AOS / MYT)"
Cohesion: 0.25
...
### Community 839 - "SECTION C — Direct Chat Improvements (CBF / HRM)"
Cohesion: 0.25
...
### Community 840 - "Runbook — Instance Activation"
Cohesion: 0.25
...
### Community 841 - "Continual Harness (`agent/harness_spec.py`)"
Cohesion: 0.25
...
### Community 842 - "Prime Agent Runtime"
Cohesion: 0.25
...
### Community 846 - "scheduler/store.py"
Cohesion: 0.25
Nodes (4): get_scheduler_store(), _MemDeleteResult, services/scheduler_store.py — Durable scheduler persistence. Issue #505:…, _MemDeleteResult supports explicit count for batch deletes.

### Community 847 - "test_scheduler_store.py"
Cohesion: 0.25
Nodes (3): _MemDB, tests/test_scheduler_store.py — Tests for durable scheduler store (#505)., AgentScheduler.hydrate() loads jobs from the store.

### Community 848 - "Prompt Library"
Cohesion: 0.25
Nodes (8): Agents, Commands, How This Library Is Maintained, Philosophy, Prompt Library, Skills, Transparency, What Is This?

### Community 849 - "CLAUDE.md — router/"
Cohesion: 0.25
...
### Community 850 - "crispy_burn_in.py"
Cohesion: 0.36
...
### Community 851 - "run_patched_colibri.py"
Cohesion: 0.36
...
### Community 853 - "test_compose_and_coordinate_api.py"
Cohesion: 0.36
...
### Community 854 - "test_local_brain_router_smoke.py"
Cohesion: 0.25
...
### Community 857 - "test_provider_models_db_outage.py"
Cohesion: 0.25
...
### Community 858 - "test_runtimes_health_endpoint.py"
Cohesion: 0.25
...
### Community 860 - "test_serve_spa_prefixes.py"
Cohesion: 0.36
...
### Community 861 - "test_task_store_fails_loud_in_production.py"
Cohesion: 0.25
...
### Community 863 - "TOOLS.md — Available Tools for AI Agents"
Cohesion: 0.25
...
### Community 868 - "platform_controls_router.py"
Cohesion: 0.29
...
### Community 870 - "4. Current Architecture (As-Is)"
Cohesion: 0.29
...
### Community 871 - "AI Engineering Insights Skill"
Cohesion: 0.29
...
### Community 881 - "ADR-006: Strangler Fig migration with backward-compat shims"
Cohesion: 0.29
Nodes (6): ADR-006: Strangler Fig migration with backward-compat shims, Consequences, Context, Decision, Examples, Migration path

### Community 882 - "claude-mem Plugin — Persistent Memory for All Sessions"
Cohesion: 0.29
...
### Community 883 - "Implementation plan + TO-DO (check off as you go)"
Cohesion: 0.29
...
### Community 888 - "Workspace Issues"
Cohesion: 0.29
...
### Community 889 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 897 - "check_changelog_parity.py"
Cohesion: 0.43
...
### Community 900 - "test_daily_2026_06_14.py"
Cohesion: 0.38
...
### Community 901 - "TestSupportMatrixDocsSync"
Cohesion: 0.29
...
### Community 902 - "TestGithubTokenSQLiteRegression"
Cohesion: 0.38
...
### Community 923 - "5. The five autonomous loops"
Cohesion: 0.33
...
### Community 924 - "Master Goal Prompt — Autonomous Agency CEO"
Cohesion: 0.33
...
### Community 925 - "Agency Core — Operational Knowledge (verified live, 2026-06-10/11)"
Cohesion: 0.33
...
### Community 926 - "Pre-Mortem Analysis: Agency Core autonomy story (Cloudflare deployment)"
Cohesion: 0.33
...
### Community 927 - "SECTION B — NVIDIA / Cloud Model Integration (Nemotron / NVD)"
Cohesion: 0.33
...
### Community 928 - "SECTION D — Deployment & Infrastructure (CHM / NVD)"
Cohesion: 0.33
...
### Community 931 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 968 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 969 - "Agent API Issues"
Cohesion: 0.40
...
### Community 970 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 971 - "knowledgeGraphTab.test.js"
Cohesion: 0.40
Nodes (4): { describe, test, expect }, fs, path, src

### Community 972 - "loginFlowNoTimeout.test.js"
Cohesion: 0.40
Nodes (4): apiSource, { describe, test, expect }, fs, path

### Community 973 - "test_company_stale_id_recovery.test.js"
Cohesion: 0.40
Nodes (4): { describe, test, expect }, fs, path, src

### Community 974 - "worker_no_cache.test.js"
Cohesion: 0.40
Nodes (4): { describe, test, expect }, fs, path, workerSource

### Community 975 - "scripts/agile_ceremonies.py"
Cohesion: 0.70
Nodes (4): _load(), main(), ModuleType, _write_summary()

### Community 976 - "governance/__init__.py"
Cohesion: 0.40
Nodes (3): __getattr__(), Any, packages/governance — agent identity, policy, approvals, audit, sandboxes. The…

### Community 977 - "Prompt Library Changelog"
Cohesion: 0.40
Nodes (4): Added, Format, Prompt Library Changelog, [Unreleased]

### Community 978 - "_add_colibri_shim_changelog_entry.py"
Cohesion: 0.50
...
### Community 979 - "build_llama_cpp.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 980 - "download_glm52_weights.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 981 - "download_glm52_weights.sh script"
Cohesion: 0.70
Nodes (4): download_glm52_weights.sh script, fail(), ok(), warn()

### Community 982 - "_fetch_pytest_failures.py"
Cohesion: 0.50
...
### Community 983 - "setup_colibri.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 984 - "setup_colibri.sh script"
Cohesion: 0.70
Nodes (4): setup_colibri.sh script, fail(), ok(), warn()

### Community 985 - "status_colibri_server.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 986 - "TestDeepMerge"
Cohesion: 0.50
Nodes (3): _deep_merge(), Deep merge two dicts. Override values take precedence., TestDeepMerge

### Community 987 - "CircuitBreakerOpenError"
Cohesion: 0.50
...
### Community 992 - "test_agent_runtime_wrapper.py"
Cohesion: 0.70
...
### Community 1009 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1026 - "test_skills_route_order.py"
Cohesion: 0.67
...
### Community 1060 - "test_direct_adapter_does_not_bypass"
Cohesion: 0.67
...
- **Why does `_fixture()` connect `_fixture` to `ProviderConfig`, `Task`, `MCPUnavailableError`, `test_telegram_auto_approve.py`, `test_ceo_supervision.py`, `BackgroundAgent`, `LLMProvider`, `test_autonomy_status.py`, `test_unit8_model_catalog.py`, `test_llm_router_strategies.py`, `llm/router.py`, `test_local_brain_state.py`, `brain_config.py`, `test_operational_incidents.py`, `AgentSessionStore`, `ExecutionRequest`, `TestChatHistoryStore`, `test_provider_state_durability.py`, `TaskStore`, `test_governance_enforcement.py`, `NotificationDispatcher`, `test_telegram_diag_endpoint.py`, `PolicyEngine`, `telegram_bot.py`, `cost_tracker.py`, `AgentSwarm`, `SelfHealingAgent`, `test_llm_router_resilience.py`, `HermesAdapter`, `e2e/test_browser.py`, `test_regression.py`, `ImprovementLoop`, `resolve_active_brain`, `TrendWatcher`, `config.py`, `failover_chat_completion`, `test_model_router.py`, `SecretRecord`, `WorkflowRun`, `PrimeAgentAdapter`, `test_sam_livekit.py`, `CEOSupervisor`, `brain_failover.py`, `SeoAuditRequest`, `test_openclaw_gateway.py`, `test_ai_insights.py`, `ResearchTask`, `services/background.py`, `TestClient`, `_enable_filter`, `ProceduralMemoryStore`, `test_new_features_e2e.py`, `resolve_e2b_config`, `clear_cooldowns`, `test_self_heal_v2.py`, `get_self_healing_agent`, `_Budget`, `BrainWatchdog`, `api_keys_for`, `AuditLog`, `test_doctor_service_token_check.py`, `test_task_brain_preflight.py`, `test_knowledge_sync.py`, `DetectedSystem`, `KeyStore`, `HealState`, `get_feature_matrix`, `SeoFixer`, `test_context_rulebook.py`, `ArtifactStore`, `test_issue_intake.py`, `test_kimi_bridge_server.py`, `_resolve_push_token`, `test_agile_api.py`, `test_app_settings.py`, `test_direct_chat_evolution.py`, `ChatHistoryStore`, `E2BAdapter`, `get_registry`, `test_startup_warmup.py`, `persist_plan_spec`, `test_task_clarification.py`, `test_sam_voice.py`, `test_e2b_task_wiring.py`, `test_agent_free_brain.py`, `OllamaCircuitBreaker`, `test_harness_spec.py`, `test_telegram_freebuff.py`, `test_agent_tool_governance.py`, `test_platform_controls.py`, `test_features_api.py`, `test_video_transcript.py`, `test_portfolio_intake.py`, `test_crispy_workflow.py`, `TestEstimateTokensForMessages`, `TestClient`, `cache.py`, `ContextPruner`, `capture_screens.py`, `test_control_plane_api.py`, `test_persistent_memory.py`, `TestMongoGate`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `IssueCategory`, `OrchestratorQueue`, `TestSwarmRoleRouting`, `test_rate_limiter.py`, `test_mcp_governance.py`, `test_purge_backlog.py`, `parse_event_stream`, `test_scheduler_hydration_bounded.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `timedelta`, `test_ceo_router.py`, `test_phase5_doctor.py`, `analyze_page`, `SeoCheckDefinition`, `traffic_director.py`, `ClaudeCodeAdapter`, `test_workspace_repowise.py`, `test_telegram_approval_e2e.py`, `test_scheduler_store.py`, `test_service_token.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `test_agency_fix.py`, `TestHarnessAdapter`, `test_monitor_lib.py`, `test_v4_api.py`, `_resolve_user_github_token`, `allow_paid`, `test_llm_router_tpm.py`, `test_voice_pipeline.py`, `test_dashboard_cache.py`, `TestNormalizeResponseFormat`, `model_discovery.py`, `test_brain_patch_service_token.py`, `test_company_api.py`, `client`, `TestWorkspace`, `SQLiteStore`, `test_shared_state.py`, `test_openclaw_endpoints.py`?**
  _High betweenness centrality (0.096) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `AgentScheduler`, `StuckDetector`, `ceo_dispatcher.py`, `proxy.py`, `MCPUnavailableError`, `workflow_orchestrator.py`, `test_ceo_supervision.py`, `BackgroundAgent`, `_StubManager`, `TaskSpec`, `ReactScratchpad`, `TestMCPServer`, `TaskDispatcher`, `AgentSessionStore`, `ExecutionRequest`, `NotificationDispatcher`, `E2BAdapter`, `WorkflowOrchestrator`, `WorkspaceTools`, `FreeBuffAgent`, `.execute`, `TestAgentRunnerExecution`, `test_agent_free_brain.py`, `test_agent_tool_governance.py`, `LocalWorkspace`, `failover_chat_completion`, `WorkflowRun`, `test_empirical_verify.py`, `PortfolioManager`, `direct_chat.py`, `GitHubTools`, `TestAgentLoopMCPIntegration`, `ContextPruner`, `test_backend_server_features.py`, `TestWorkspace`, `AutonomyTracker`, `TokenBudget`, `TestZeroAttemptDiagnostics`, `test_self_heal_v2.py`, `AdaptiveHalter`, `test_governance_api.py`, `api_keys_for`, `ContextManager`, `MCPClient`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `TaskStore` connect `TaskStore` to `backend/server.py`, `_build_default_manager`, `test_portfolio_intake.py`, `Task`, `test_self_heal_v2.py`, `_fixture`, `tasks/api.py`, `test_issue_intake.py`, `_ts_to_float`, `test_trend_scoping.py`, `test_task_brain_preflight.py`, `services/background.py`, `TaskDispatcher`, `test_task_source_id_race.py`, `test_sqlite_store.py`?**
