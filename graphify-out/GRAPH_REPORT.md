...
## Corpus Check
- 1518 files · ~2,198,739 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 31980 nodes · 59045 edges · 1332 communities (1154 shown, 178 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3741 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `18b0d800`
...
## Communities (1332 total, 178 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.01
Nodes (425): get_harness_adapter(), get_optional_company_access(), _is_admin(), list_companies(), Extract a consistent user_id from the user dict. Works across all auth methods:…, Check if a user has admin role. Works for both social_auth users (role in…, Verify user has access to a company (if authenticated). Admins bypass ownership…, List companies. - Admin users see ALL companies across the platform. - Regular… (+417 more)

### Community 1 - "TaskWorkflowService"
Cohesion: 0.05
Nodes (43): Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, ApprovalCheckpoint, Human approval gate in a task's execution., Record the human decision on a task's **pre-execution** approval gate. A…, Re-run a task. Permissive across every non-active state so the dashboard Retry… (+35 more)

### Community 2 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (124): AlertHandler, BudgetTracker, Counter, _Dimensions, _month(), Any, Register a callback fired when a spend threshold is crossed., Fold a new key into "other" once the dimension is at its cap. (+116 more)
...
### Community 4 - "Agency"
Cohesion: 0.04
Nodes (75): Agency, AgencyCycleResult, AgentDirective, AgentRole, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context() (+67 more)

### Community 5 - "models/company_graph.py"
Cohesion: 0.03
Nodes (95): ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraphResponse, CompanyGraphSnapshot, CompanyResponse, CompanyUpdateRequest, Connector (+87 more)

### Community 6 - "_fixture"
Cohesion: 0.02
Nodes (102): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+94 more)
...
### Community 8 - "WorkflowRun"
Cohesion: 0.06
Nodes (32): _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test(), _tracking_run_single(), TestPhaseSequence (+24 more)
...
Cohesion: 0.02
Nodes (139): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+131 more)
...
Cohesion: 0.03
Nodes (119): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), _migrate_brain_to_safe_default(), _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config() (+111 more)

### Community 13 - "LLMRequest"
Cohesion: 0.02
Nodes (125): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, LLMProvider, OpenAICompatible, ABC, Any, AsyncClient, Every provider adapter implements exactly this. (+117 more)
...
Cohesion: 0.02
Nodes (200): int, Catalog entries for router-configured providers missing from the YAML. Never…, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager. (+192 more)

### Community 16 - "PolicyEngine"
Cohesion: 0.04
Nodes (85): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _action_matches(), _as_list(), Decision, GroupPolicy, _host_matches() (+77 more)
...
Cohesion: 0.03
Nodes (92): livenim, allow_paid_brain(), BrainResolution, get_brain_config(), Convenience wrapper used by the agent loop + brain resolver., get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible() (+84 more)
...
### Community 24 - "Task"
Cohesion: 0.03
Nodes (80): Full task/issue document., Task, _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing(), Any, BaseException, One-line human explanation of why this task needs human approval. Reads risk… (+72 more)

### Community 25 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (122): CEOResult, _decompose_into_subtasks(), _merge_changed_files(), Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, Collect changed_files across all specialists into a single de-duped list., reset_ceo_dispatcher() (+114 more)

### Community 26 - "test_agent_runner.py"
Cohesion: 0.03
Nodes (78): AgentPhaseError, _note_phase_end(), _note_phase_start(), Exception, Raised when a named agent phase (planning, verification, etc.) fails., Log start/elapsed for one phase of the plan/execute/verify loop. A task that…, Open a tracked phase, returning the token that closes it. Never raises., Close the tracked phase *token* opened above. Never raises. (+70 more)

### Community 27 - "test_runtimes.py"
Cohesion: 0.05
Nodes (23): CircuitState, runtimes/health.py — RuntimeHealthService. Periodically polls all registered…, Reduce probe frequency for runtimes that have never come online., runtimes/registry.py — RuntimeCapabilityRegistry. Maps task types and required…, Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry (+15 more)

### Community 28 - "E2BSandboxSession"
Cohesion: 0.04
Nodes (68): MCPUnavailableError, RuntimeError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any (+60 more)

### Community 29 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (82): BudgetTracker, classify(), GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., Return the ``(surface, action)`` a tool call really represents. The *action* is…, AgentIdentity, _coerce_text() (+74 more)
...
### Community 35 - "SQLiteStore"
Cohesion: 0.02
Nodes (97): KnowledgeItem, Structured knowledge about the company, systems, or processes., MongoDBStore, Any, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., Create a new company in SQLite. (+89 more)
...
### Community 38 - "rbac.py"
Cohesion: 0.06
Nodes (36): compute_savings(), compute_time_series(), get_savings(), get_usage(), get_user_savings(), _period_start(), Any, BaseModel (+28 more)

### Community 39 - "test_direct_chat_async.py"
Cohesion: 0.07
Nodes (26): make_isolated_workspace(), Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Create an isolated workspace directory under *root*. This is the legacy path…, _workspace_component(), _fake_user(), _FakeChatResult, _FakeResponse (+18 more)

### Community 40 - "test_unit8_model_catalog.py"
Cohesion: 0.02
Nodes (116): _default_agent_role_models(), Resolve default per-role agent models via the catalog (UNIT 6). Was a hardcoded…, all_provider_ids(), _provider_ids_from_literal(), provider_key_present(), Resolve the model id for a component's role on a provider. Parameters…, Convenience: resolve all four role models for a component. Returns a dict with…, Return every provider id recognised by the brain config system. Iterates the… (+108 more)

### Community 41 - "test_llm_router_resilience.py"
Cohesion: 0.03
Nodes (68): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), get_ring(), KeyRing (+60 more)
...
Cohesion: 0.05
Nodes (32): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, One deploy, normalised from whatever shape the tool returned., RenderDeploy, Polls Render for platform-level failures and files them as issues., RenderOpsMonitor, _client() (+24 more)

### Community 44 - "WorkflowRun"
Cohesion: 0.03
Nodes (56): MergeDecision, _NoopStore, Any, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs…, Push a Telegram approval-gate notification when a run pauses (Charter G1).…, G5: resolve how a run should land from the company's DeliveryPolicy. Returns… (+48 more)

### Community 45 - "AgentDefinition"
Cohesion: 0.04
Nodes (82): _apply_activity_status(), create_agent(), delete_agent(), get_agent(), _get_user(), list_agents(), list_runtime_agents(), Any (+74 more)
...
### Community 47 - "ModelConfig"
Cohesion: 0.04
Nodes (36): ModelConfig, Capability and pricing metadata for one model id., Re-read configuration from disk. Used by the admin API and by tests., reload_config(), config_reload(), ModelRegistry, Add or replace a model (used by runtime discovery)., Record models found by a provider's ``/models`` endpoint. Discovered models… (+28 more)
...
### Community 49 - "loop.py"
Cohesion: 0.05
Nodes (49): loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, simulate_policy(), _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Write the audit row for a completed (or blocked) MCP tool call., Import the governance layer once, recording why if it is absent. (+41 more)

### Community 50 - "AgentProfile"
Cohesion: 0.12
Nodes (22): agents/__init__.py — CRISPY multi-agent coding system., AgentProfile, _get_defaults(), load_all_profiles(), make_architect_profile(), make_coder_profile(), make_reviewer_profile(), make_scout_profile() (+14 more)
...
Cohesion: 0.02
Nodes (95): BusinessCategory, CompanyGraph, OnboardingProgress, Get the index of a phase in the workflow., The complete Company Graph - canonical core model for the Autonomous AI Agency.…, Find a website by its URL., Find a repository by its URL., Find a workflow by its name. (+87 more)
...
Cohesion: 0.03
Nodes (98): CEODispatcher, _harvest_changed_files(), _offload(), Any, Semaphore, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask. (+90 more)
...
Cohesion: 0.06
Nodes (37): Attempt, _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One delegation of one subtask to one tier. (+29 more)
...
### Community 83 - "test_render_mcp.py"
Cohesion: 0.04
...
### Community 84 - "test_north_mini_code.py"
Cohesion: 0.11
Nodes (9): tests/test_north_mini_code.py — North Mini Code 1.0 integration. Covers the…, The switch defaults ON so North is the default post-install., The agency/Hermes execution path defaults to North via the resolver., North is cost_tier=2, so it must not become the ``best_model_for`` pick for the…, test_flag_default_is_on(), test_internal_agent_consults_coding_resolver(), test_north_does_not_displace_default_coder(), test_north_mini_code_model_for_maps_only_serving_providers() (+1 more)
...
Cohesion: 0.05
Nodes (44): _get_director(), is_commercial_provider(), _normalized_provider_type(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Any (+36 more)

### Community 92 - "test_runtime_governance.py"
Cohesion: 0.07
Nodes (57): _audit_dispatch(), _blocked_result(), _governance_check(), Any, runtimes/routing.py — RuntimeRoutingPolicyEngine. Implements the 8-step routing…, Build the failed TaskResult returned when policy denies a dispatch. A result…, Evaluate a runtime dispatch against policy; audit it either way. Returns…, Full record of a routing decision — audit-trail entry. (+49 more)
...
Cohesion: 0.04
Nodes (41): Await one warm-up step, deferring it to the background if it overruns.…, _warmup_step(), Any, Await ``coro`` until ``deadline`` (a loop timestamp); defer it past that.…, warmup_step(), _isolate_warmup_overflow(), asyncio, NoReturn (+33 more)
...
Cohesion: 0.07
Nodes (30): clear_cooldowns(), is_provider_on_cooldown(), mark_provider_failed(), Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown., Clear all cooldown entries (useful for testing). Delegates to…, reset_cooldowns(), anyio (+22 more)
...
Cohesion: 0.05
Nodes (70): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState, FailedJob (+62 more)
...
Cohesion: 0.06
Nodes (24): AgentJob, AgentJobManager, heartbeat(), _now(), Any, Run a job using the provided runner and update the job's lifecycle, progress,…, Serialize the AgentJob to a JSON-serializable dictionary for external clients.…, get_agent_job_manager() (+16 more)
...
### Community 120 - "test_brain_failover.py"
Cohesion: 0.10
...
Cohesion: 0.07
Nodes (40): Backward-compatibility shim — use scripts/generate_api_key.py instead. This…, _check_rate_limit(), default_keys_path(), issue_new_api_key(), KeyRecord, KeyStore, load_key_store(), Exception (+32 more)
...
Cohesion: 0.05
Nodes (40): _check_auth(), health(), Any, BaseModel, get, post, services/hermes_server.py — the agency's OWN Hermes runtime server. This is…, Execute a task synchronously via the InternalAgentAdapter (our brain). Response… (+32 more)

### Community 129 - "CacheManager"
Cohesion: 0.09
...
### Community 133 - "AgentPlan"
Cohesion: 0.05
Nodes (42): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is… (+34 more)

### Community 134 - "UserRole"
Cohesion: 0.09
Nodes (38): UserRole, _can_read(), _can_write(), _decrypt(), _encrypt(), _get_master_key(), BaseModel, Enum (+30 more)
...
Cohesion: 0.06
Nodes (45): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion() (+37 more)

### Community 142 - "AgentScheduler"
Cohesion: 0.07
Nodes (25): AgentScheduler, Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Remove a job. Returns *True* if it existed., Capture the FastAPI main event loop so APScheduler's background thread can…, Drop a job from in-memory state and APScheduler (mirrors ``delete()``). Popping…, Durably remove one job, then drop its in-memory + APScheduler mirrors. The…, Durably remove one **unfired** run-once row and its in-memory mirror. Prefers…, Delete EVERY schedule from the durable store and in-memory state. Operator… (+17 more)
...
Cohesion: 0.06
Nodes (48): _bullets(), generate_backlog_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive a retrospective from the task tracker when no sprint is active. DONE /…, The result of allocating portfolio capacity into a new sprint. (+40 more)

### Community 149 - "test_governance_api.py"
Cohesion: 0.04
Nodes (60): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), get_approval_store(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer… (+52 more)
...
### Community 155 - "test_llm_router_gateway.py"
Cohesion: 0.07
Nodes (16): health(), toggle_provider(), get_tracker(), The process-wide health tracker., client(), The routing layer's REST surface (``/api/llm/*``). Two properties matter most…, Exposing the platform's keys to other applications must be deliberate., A disabled router degrades the page, never breaks it. (+8 more)
...
### Community 157 - "test_ceo_cross_verify.py"
Cohesion: 0.20
...
Cohesion: 0.05
Nodes (38): _run(), _latest_metric_value(), _note_recurrence(), _parse_timestamp(), Any, datetime, services/render_ops.py — autonomous Render debugging + environment monitoring.…, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``. (+30 more)

### Community 159 - "claim"
Cohesion: 0.04
Nodes (39): claim(), clear_all_locks(), cooldown_clear(), cooldown_get(), cooldown_set(), _get_backend(), incr_window(), _InMemoryBackend (+31 more)

### Community 160 - "test_p0_roadmap_b1_c2_a3.py"
Cohesion: 0.06
...
### Community 169 - "test_agent_tool_governance.py"
Cohesion: 0.04
...
### Community 188 - "KnowledgeScreen.jsx"
Cohesion: 0.13
...
Cohesion: 0.05
Nodes (70): get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers(), _backend_get(), _check_rate_limit() (+62 more)
...
Cohesion: 0.07
Nodes (22): InternalAgentAdapter, Any, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Create an isolated execution context for a single task. Tries ``git worktree…, Clean up the worktree or temp copy created by ``_create_worktree``., InternalAgentAdapter should construct without error. (+14 more)
...
### Community 194 - "RuntimeManager"
Cohesion: 0.08
Nodes (13): Any, Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking)., Return all registered runtimes with their cached health status., Return the active routing policy as a plain dict. (+5 more)
...
Cohesion: 0.09
Nodes (21): AgentRole, When coder == reviewer model, swarm should log a warning., TestSlice, TestWorkflowRun, Artifact, A durable, phase-produced document (markdown or JSON)., An independently deliverable unit of work within the execute phase. Each slice…, Slice (+13 more)
...
### Community 213 - "WorkspaceManager"
Cohesion: 0.18
...
Cohesion: 0.07
Nodes (30): engine(), Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory (+22 more)
...
### Community 217 - "analyze_quantitative"
Cohesion: 0.13
...
Cohesion: 0.06
Nodes (21): Enum, SuperClaude Workflow Engine — Workflow, Task, and topological DAG execution.…, Return tasks whose dependencies are all satisfied., Number of tasks in the workflow., Number of completed tasks., Number of failed tasks., Register a workflow with the engine., Execute a workflow topologically, returning the task results. Tasks are… (+13 more)
...
### Community 237 - "TestRuntimeControl"
Cohesion: 0.04
Nodes (30): Test iteration 6 features: - POST /api/tasks/ auto-assigns an available agent…, POST /api/tasks/ without agent_id should attempt auto-assignment if agents exist, Test runtime start/stop endpoints return informational payloads in remote…, Get authentication token for admin user, GET /runtimes/ should return list of runtimes, POST /runtimes/{id}/start should return non-blocking informational payload in…, POST /runtimes/stop-all should return non-blocking informational payload, PUT /runtimes/policy should work with valid auth (+22 more)
...
### Community 244 - "skill_bindings.py"
Cohesion: 0.05
Nodes (55): ConsolidationPhase, MemoryKind, Enum, str, Dream Memory Consolidation — pattern consolidation across AI sessions. Inspired…, What kind of memory artifact this is., Current phase of the consolidation lifecycle., Executes workflows using topological ordering. (+47 more)

### Community 245 - "TestAdapterMetadata"
Cohesion: 0.05
Nodes (25): Resolve the base URL of the agency's own Hermes server. Precedence:…, resolve_hermes_base_url(), _env_float(), HermesAdapter, Any, AsyncClient, Read a float env var, falling back to *default* on unset/garbage., Adapter for Hermes Agent — FIRST CLASS autonomous runtime. (+17 more)
...
### Community 247 - "model_router.py"
Cohesion: 0.05
Nodes (45): _catalog_defaults(), _catalog_provider(), Pick the catalog provider based on which API key is configured., Resolve per-role defaults via the catalog. Returns None on import error., _build_builtin_model_map(), _default_model(), _default_reasoning_model(), _nvidia_key_present() (+37 more)
...
### Community 280 - "test_brain_availability_doctor.py"
Cohesion: 0.14
...
### Community 294 - "get_secrets_store"
Cohesion: 0.15
...
### Community 299 - "test_executive_advisory_api.py"
Cohesion: 0.11
...
Cohesion: 0.06
Nodes (42): BrainFailoverManager, _disabled_from_mongo(), _disabled_from_sqlite(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _mongo_db(), _mongo_enabled() (+34 more)
...
### Community 310 - "test_backend_server_features.py"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (50): _get_current_user_thunk(), _get_optional_user_thunk(), Request, create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., Item (+42 more)
...
### Community 314 - "executive_advisory_api.py"
Cohesion: 0.11
...
### Community 318 - "_mock_provider_records"
Cohesion: 0.13
...
### Community 320 - "test_colibri_provider.py"
Cohesion: 0.13
...
### Community 357 - "schedules/api.py"
Cohesion: 0.13
...
### Community 360 - "TaskDispatcher"
Cohesion: 0.06
Nodes (40): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Re-queue BLOCKED tasks that have cooled down and are ready for retry., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, _make_task(), asyncio (+32 more)
...
### Community 367 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.09
...
### Community 372 - "get_store"
Cohesion: 0.10
Nodes (33): Admin: update the global onboarding gate default and/or ephemeral TTL. Turning…, update_onboarding_settings(), all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours(), ephemeral_ttl_hours_cached(), get_setting() (+25 more)
...
Cohesion: 0.06
Nodes (15): fake_mongo(), _FakeCollection, _FakeDb, _live_mongo_url(), Operator provider state must survive a redeploy. The per-provider kill switch…, Tests must never mutate a shared operational store., The storage layer's localhost default is a placeholder, not config. Treating it…, This is the document the UI toggle writes via _set_provider_policy. (+7 more)
...
Cohesion: 0.08
Nodes (19): get_nim_pool(), NIMConnectionPool, Any, AsyncClient, Persistent httpx.AsyncClient pool with circuit breaker and retry logic. Manages…, Get or create the shared httpx.AsyncClient., Context manager for a pooled client session., Close the connection pool. (+11 more)
...
### Community 404 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.15
...
### Community 427 - "_autonomy_bg_cycle"
Cohesion: 0.08
Nodes (34): get_agency(), set_agency(), _autonomy_bg_cycle(), autonomy_tick(), Background CEO cycle + task dispatch. Runs fire-and-forget., Execute ONE pending task synchronously. Called by the cron workflow every 2…, Return the caller's GitHub token from EITHER place it can be stored. A token…, _resolve_user_github_token() (+26 more)
...
### Community 445 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 447 - "RuntimeHealthService"
Cohesion: 0.15
...
### Community 455 - "test_agent_tools.py"
Cohesion: 0.19
...
### Community 461 - "TestOrchestratorQueue"
Cohesion: 0.13
...
Cohesion: 0.09
Nodes (23): PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, test_specialized_runtime_execution(), check_all(), execute(), readiness_check() (+15 more)

### Community 463 - "test_schedule_persistence.py"
Cohesion: 0.16
...
### Community 467 - "AgentJobSnapshot"
Cohesion: 0.20
...
### Community 498 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.09
...
### Community 513 - "persist_plan_spec"
Cohesion: 0.06
...
### Community 534 - "redact_connection_url"
Cohesion: 0.17
...
### Community 536 - "test_portfolio_intelligence.py"
Cohesion: 0.14
...
### Community 537 - "test_agent_coordinate.py"
Cohesion: 0.20
...
### Community 581 - "ApplyReviewAgent"
Cohesion: 0.22
...
### Community 593 - "test_brain_failover_413.py"
Cohesion: 0.13
...
### Community 594 - "agent/workspace.py"
Cohesion: 0.21
...
### Community 620 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.33
...
### Community 621 - "flesch_reading_ease"
Cohesion: 0.20
...
Cohesion: 0.04
Nodes (35): CerebrasProvider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NvidiaProvider, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., OllamaProvider, Ollama provider adapter — local LLM inference. (+27 more)

### Community 648 - "test_scanner_live.py"
Cohesion: 0.23
...
Cohesion: 0.04
Nodes (56): main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), _is_safe_url(), _looks_like_bot_challenge(), BeautifulSoup, True if the HTML is an anti-bot/CAPTCHA interstitial rather than content. Used… (+48 more)
...
### Community 681 - "Separate hosted dashboard backend (`backend/server.py`)"
Cohesion: 0.13
...
### Community 688 - "test_swarm_concurrency.py"
Cohesion: 0.15
...
### Community 693 - "test_telegram_observe.py"
Cohesion: 0.29
...
### Community 732 - "traffic_director.py"
Cohesion: 0.13
...
### Community 739 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 809 - "TestDeprecationWarnings"
Cohesion: 0.17
...
### Community 818 - "audit"
Cohesion: 0.07
Nodes (21): audit(), get_audit_log(), get_user_role(), is_admin(), is_power_user_or_above(), Any, Extract role from a user object (dict or Pydantic model)., Return a short human-readable badge label for display in the UI. (+13 more)
...
### Community 824 - "TestAnthropicWorkspaceIdCapture"
Cohesion: 0.33
...
### Community 859 - "WorkspaceManager"
Cohesion: 0.19
...
### Community 897 - "test_runtimes_health_endpoint.py"
Cohesion: 0.25
...
### Community 901 - "TestHarnessRegistry"
Cohesion: 0.20
...
### Community 903 - "ENGINEERING_STANDARDS.md — Patterns & Reference"
Cohesion: 0.22
...
### Community 909 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.22
...
### Community 917 - "get_output_filter"
Cohesion: 0.32
...
### Community 991 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1020 - "_parse_tool_calls_from_response"
Cohesion: 0.39
...
### Community 1023 - "_build_curl_cffi_fetcher"
Cohesion: 0.36
...
### Community 1034 - "_answer_callback"
Cohesion: 0.29
...
### Community 1035 - "Workspace Issues"
Cohesion: 0.29
...
### Community 1036 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 1067 - "DirectChatDoctor"
Cohesion: 0.47
...
### Community 1068 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 1081 - "TestAgentRunnerExecution"
Cohesion: 0.33
...
### Community 1090 - "TestCompanyGraphServices"
Cohesion: 0.33
...
### Community 1141 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 1156 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 1157 - "CircuitBreakerOpenError"
Cohesion: 0.50
...
### Community 1158 - "TestDirectChatNonBlocking"
Cohesion: 0.40
...
### Community 1178 - "services/background.py"
Cohesion: 0.04
...
### Community 1202 - "test_activity_logs.py"
Cohesion: 0.67
...
### Community 1205 - "_InMemoryErrorLogHandler"
Cohesion: 0.50
Nodes (3): _ensure_error_log_capture(), _InMemoryErrorLogHandler, LogRecord

### Community 1238 - "Authentication Issues"
Cohesion: 0.50
...
### Community 1304 - "Feature Maturity Issues"
Cohesion: 0.50
...
### Community 1305 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1326 - "ControlUpdateBody"
Cohesion: 0.67
Nodes (3): ControlUpdateBody, BaseModel, One or more control changes, as ``{"updates": {"KEY": value}}``.

## Knowledge Gaps
- **3519 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3514 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14336 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **178 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `persist_plan_spec`, `TestNormalizeResponseFormat`, `test_llm_router_queue_cache.py`, `Agency`, `models/company_graph.py`, `TaskWorkflowService`, `client`, `test_llm_router_strategies.py`, `ImprovementLoop`, `llm/router.py`, `PolicyEngine`, `test_brain_patch_service_token.py`, `TaskSpec`, `test_operational_incidents.py`, `_Response`, `SelfHealingAgent`, `Task`, `E2BSandboxSession`, `MCPClient`, `test_provider_router.py`, `test_failover_client_shared.py`, `test_unit8_model_catalog.py`, `test_llm_router_resilience.py`, `test_company_api.py`, `test_model_router.py`, `WorkflowRun`, `test_langfuse_agency_wide.py`, `AgentDefinition`, `_redact_for_notification`, `test_telegram_webhook.py`, `AgentProfile`, `TokenBudget`, `BrainWatchdog`, `services/seo_audit.py`, `ChatHistoryStore`, `Specialist`, `TaskStatus`, `AgentRunner`, `build_governance_router`, `GoalRecord`, `config.py`, `TestClient`, `test_brain_failover_413.py`, `test_openclaw_endpoints.py`, `test_sam_livekit.py`, `test_procedural_memory.py`, `test_sam_voice.py`, `ExecutionRequest`, `AgentSwarm`, `ProviderConfig`, `test_startup_warmup.py`, `PrimeAgentAdapter`, `test_llm_router_e2e.py`, `SQLiteStore`, `clear_cooldowns`, `_resolve_push_token`, `e2e/test_browser.py`, `test_autonomy_status.py`, `direct_chat.py`, `test_knowledge_sync.py`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `test_task_service_failed_comment.py`, `test_brain_failover.py`, `KeyStore`, `test_context_rulebook.py`, `ArtifactStore`, `run_task`, `CacheManager`, `UserRole`, `test_issue_intake.py`, `seo_api.py`, `test_task_store_fails_loud_in_production.py`, `_step`, `captured`, `UsageEvent`, `test_trend_watcher.py`, `services/background.py`, `test_llm_router_gateway.py`, `claim`, `test_new_features_e2e.py`, `FeatureMaturity`, `TestEstimateTokensForMessages`, `test_trend_scoping.py`, `TestChatHistoryStore`, `test_agent_tool_governance.py`, `router_factory`, `TestSelfHealingInfrastructureClassification`, `quick_note.py`, `KeyPool`, `TestHarnessAdapter`, `test_changelog_check_workflow.py`, `test_e2b_data_flow.py`, `test_openclaw_gateway.py`, `_Recorder`, `OllamaCircuitBreaker`, `_job_text`, `test_ceo_router.py`, `test_features_api.py`, `test_video_transcript.py`, `TrendWatcher`, `test_portfolio_intake.py`, `SchedulerStore`, `ResearchOrchestrator`, `_Budget`, `WorkflowBuildRequest`, `test_telegram_approval_gate.py`, `traffic_director.py`, `nvidia_models.py`, `NotificationDispatcher`, `routed`, `test_doctor_service_token_check.py`, `test_task_brain_preflight.py`, `TestClient`, `TestRuntimeControl`, `test_mcp_protocol_version.py`, `skill_bindings.py`, `ContextPruner`, `test_control_plane_api.py`, `test_agile_api.py`, `test_app_settings.py`, `test_persistent_memory.py`, `SecurityScanner`, `test_task_clarification.py`, `OutputFilter`, `test_platform_controls.py`, `controls_app`, `test_rate_limiter.py`, `test_colibri_brain_shim.py`, `test_executive_advisory_api.py`, `test_nvidia_model_discovery.py`, `isolated_telegram_config`, `_mock_provider_records`, `cost_tracker.py`, `analyze_page`, `model_discovery.py`, `parse_event_stream`, `test_scheduler_hydration_bounded.py`, `ai/router.py`, `test_catalogue_probe.py`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_telegram_auto_approve.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `test_dashboard_cache.py`, `ModelRouter`, `_start_ceo_agency`, `test_runtimes_health_endpoint.py`, `_undeclared`, `TestHarnessRegistry`, `test_process_quick_note_workflow.py`, `test_provider_state_durability.py`, `test_service_token.py`, `test_harness_spec.py`, `test_freebuff_bot.py`, `test_agency_fix.py`, `_Recorder`, `test_webui_provider_priority.py`, `_autonomy_bg_cycle`, `OrchestratorCheckpointStore`, `_get`, `test_monitor_lib.py`, `test_v4_api.py`, `test_kimi_bridge_server.py`, `test_telegram_diag_endpoint.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_ping.py`, `test_voice_pipeline.py`, `TestCountTokensEndpoint`?**
  _High betweenness centrality (0.103) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `proxy.py`, `AgentPlan`, `AdaptiveHalter`, `E2BAdapter`, `PolicyEngine`, `ReactScratchpad`, `TaskSpec`, `test_autonomous_agency_e2e.py`, `resolve_active_brain`, `test_ceo_micromanager.py`, `test_governance_api.py`, `test_ceo_dispatcher.py`, `test_agent_runner.py`, `E2BSandboxSession`, `ContextManager`, `test_ceo_cross_verify.py`, `test_p0_roadmap_b1_c2_a3.py`, `MCPClient`, `AgentSessionStore`, `test_failover_client_shared.py`, `test_daily_automation_2026_07_11.py`, `test_agent_tool_governance.py`, `TestDeprecationWarnings`, `WorkflowRun`, `loop.py`, `UserMemoryStore`, `TokenBudget`, `test_empirical_verify.py`, `WorkspaceTools`, `coordinator.py`, `test_ceo_supervision.py`, `TestAgentRunnerExecution`, `MultiAgentSwarm`, `InternalAgentAdapter`, `StuckDetector`, `test_brain_failover_413.py`, `TestAgentLoopMCPIntegration`, `LocalWorkspace`, `test_agent_chat_integration.py`, `TaskDispatcher`, `direct_chat.py`, `GitHubTools`, `ContextPruner`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `backend/server.py`, `settings.py`, `TestAnthropicPayloadStructuredOutput`, `failover_client.py`, `test_all_providers_discovery.py`, `_Response`, `TestLegacyRouterServerFallback`, `test_chat_mode_regressions.py`, `test_provider_router.py`, `test_colibri_brain_shim.py`, `test_bedrock_live.py`, `ProviderCircuit`, `TestRouterIntegration`, `test_kimi_bridge_provider.py`, `test_colibri_provider.py`, `TestLegacyRouterCacheTTL`, `TrafficDirector`, `is_anthropic_base_url`, `ai/router.py`, `system_instruction`, `ProviderConfig`, `_payload`, `clear_cooldowns`, `direct_chat.py`, `test_anthropic_refusal_fallback.py`, `implement_agent.py`?**
