...
- TestLoopAdaptiveHalterIntegration
- TestAwaitReady
...
## Communities (1331 total, 175 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.01
Nodes (358): admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, _append_agent_session_message() (+350 more)

### Community 1 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (103): BoundContext, ClassifyOutput, ExecutionResult, _get_ceo_dispatcher(), get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput, _orchestrator_bypass() (+95 more)

### Community 2 - "test_provider_router.py"
Cohesion: 0.05
Nodes (45): _normalize_nvidia_base_url(), _openai_url(), Normalize NVIDIA base URLs to avoid double /v1 when openai_compat_url appends…, anyio, A 429 (e.g. 40 rpm hit) must NOT retry the same provider — it fails over to the…, When a provider call fails (and failover succeeds), the brain watchdog's…, Even if the brain watchdog import itself fails, the request MUST still succeed…, 419 on first model → try second model on same provider. 429 on first → skip… (+37 more)

### Community 3 - "company_api.py"
Cohesion: 0.02
Nodes (196): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint() (+188 more)

### Community 4 - "brain_config.py"
Cohesion: 0.02
Nodes (133): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), patch_brain_policy_route(), Apply a partial brain config update. Hard constraint: every changed model id is…, _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore (+125 more)

### Community 5 - "llm/router.py"
Cohesion: 0.03
Nodes (115): get_budget(), The process-wide budget tracker., get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager., text_key(), failover_chat_completion_via_router(), openai_body_from_response() (+107 more)
...
### Community 7 - "OnboardingService"
Cohesion: 0.03
Nodes (59): Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., Inferred technology stack from website/repo analysis., StackInference, WebsiteScanResult, OnboardingService, Detect the Git provider from a repository URL. Args: repo_url: Repository URL… (+51 more)

### Community 8 - "telegram_bot.py"
Cohesion: 0.06
Nodes (62): get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control() (+54 more)

### Community 9 - "test_ceo_supervision.py"
Cohesion: 0.06
Nodes (64): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, A subtask's full history: what it is, and every attempt at it., SubtaskRecord, _goal(), asyncio, Tests for the CEO ledger, the supervised escalation loop, and the 24x7 sweeper.…, A storage outage must never take the agency down. (+56 more)

### Community 10 - "TaskSpec"
Cohesion: 0.04
Nodes (77): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor., json_safe(), runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).… (+69 more)

### Community 11 - "proxy.py"
Cohesion: 0.02
Nodes (184): middleware, AdminIdentity, admin_control(), admin_create_key(), admin_create_user(), admin_delete_user(), admin_list_users(), admin_login() (+176 more)
...
Cohesion: 0.03
Nodes (53): AgentJob, AgentJobManager, heartbeat(), make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs… (+45 more)
...
### Community 15 - "TaskStatus"
Cohesion: 0.03
Nodes (152): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, _coerce_ts(), Enum, str (+144 more)

### Community 16 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (95): int, cosine_similarity(), payload_key(), Exact-match cache key over the fields that change the answer. Routing…, Cosine similarity between two vectors, 0.0 when either is degenerate., build_summariser(), summarise(), chunk_document() (+87 more)
...
Cohesion: 0.04
Nodes (89): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), FailoverResult, _is_billing_refusal(), _is_ollama(), _key_pool() (+81 more)

### Community 20 - "types.py"
Cohesion: 0.03
Nodes (83): _load(), main(), ModuleType, Scheduled agile ceremonies digest. Run by `.github/workflows/agile-…, _write_summary(), _load(), main(), Scheduled portfolio intelligence sweep. Run by `.github/workflows/portfolio-… (+75 more)

### Community 21 - "test_platform_controls.py"
Cohesion: 0.05
Nodes (38): apply_overrides(), Write *overrides* into ``os.environ`` and refresh dependent caches. Keys that…, Re-read every ``settings`` attribute from the updated environment. Re-runs…, _refresh_settings_singleton(), all_controls(), Every control in the catalogue, in display order., clean_overrides(), controls_app() (+30 more)

### Community 22 - "AgentSessionStore"
Cohesion: 0.05
Nodes (63): AgentEvent, AgentSession, AgentSessionMessage, A single entry in the session's append-only event log. Inspired by Anthropic's…, AgentSessionStore, _now(), Connection, Path (+55 more)
...
### Community 24 - "ImprovementLoop"
Cohesion: 0.02
Nodes (152): DetectedIssue, ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity, _now(), Any, Enum (+144 more)

### Community 25 - "LLMRouter"
Cohesion: 0.06
Nodes (35): Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, run(), Any, AsyncClient, BaseException, One shared connection pool. Reconnects if it was closed. (+27 more)

### Community 26 - "SQLiteStore"
Cohesion: 0.03
Nodes (68): DetectedSystem, A business system detected on a company's website or in their stack., Get the most confident evidence description., Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., Create a new company in SQLite., Get a company by ID from SQLite., Update a company in SQLite. (+60 more)

### Community 27 - "test_ceo_micromanager.py"
Cohesion: 0.05
Nodes (102): build_subtask_brief(), _coerce_subtasks(), decompose(), _env_flag(), _env_int(), _extract_json_object(), fallback_decomposition(), get_config() (+94 more)
...
### Community 29 - "_fixture"
Cohesion: 0.01
Nodes (117): client(), TestClient, Function-scoped TestClient for backend.server — used by backend-specific tests.…, TestClient with ``raise_server_exceptions=False`` for integration tests. Tests…, wiki_client(), base_url(), mobile_page(), proxy_url() (+109 more)
...
Cohesion: 0.04
Nodes (69): FailureCategory, heal_signature(), HealingEvent, HealState, _now(), Any, Enum, str (+61 more)

### Community 32 - "Specialist"
Cohesion: 0.04
Nodes (43): SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, Create a new specialist., Get a specialist by ID. (+35 more)

### Community 33 - "test_runtimes.py"
Cohesion: 0.04
Nodes (31): Any, Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking)., Return all registered runtimes with their cached health status., Return the active routing policy as a plain dict. (+23 more)

### Community 34 - "test_unit8_model_catalog.py"
Cohesion: 0.02
Nodes (125): _brain_provider_status(), get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, Force a catalog mirror rebuild + persist (admin only). Rebuilds the catalog…, Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., _served_models(), sync_catalog_route() (+117 more)
...
### Community 40 - "Task"
Cohesion: 0.03
Nodes (78): Full task/issue document., Task, _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing(), Any, BaseException, One-line human explanation of why this task needs human approval. Reads risk… (+70 more)
...
### Community 44 - "services/background.py"
Cohesion: 0.06
...
Cohesion: 0.05
Nodes (49): build_ceo_router(), ceo_status(), get_goal(), list_goals(), redrive_goal(), run_sweep(), Any, APIRouter (+41 more)

### Community 46 - "resolve_identity"
Cohesion: 0.03
Nodes (108): simulate_policy(), _load(), Import the governance layer once, recording why if it is absent., AgentIdentity, _coerce_text(), new_session_id(), Any, packages/governance/identity.py — who is acting, and under what mandate. Docker… (+100 more)
...
### Community 52 - "TestAgentRunnerSafety"
Cohesion: 0.24
...
### Community 54 - "test_unit7_catalog_propagation.py"
Cohesion: 0.06
...
### Community 55 - "ProbeResult"
Cohesion: 0.04
Nodes (58): provider_api_key(), Return the live API key for *provider* (env-only — never persisted)., _describe_http_status(), probe_model_liveness(), _probe_ollama(), _probe_openai_compat(), ProbeResult, BaseModel (+50 more)
...
Cohesion: 0.05
Nodes (39): validate_policy(), backend/governance_router.py — read and operate the governance layer. Mounted…, _audit_proposal(), _baseline_loosened(), _baseline_patterns(), diff_policy(), _GitHub, parse_policy_text() (+31 more)

### Community 59 - "CompanyGraphService"
Cohesion: 0.02
Nodes (53): BusinessCategory, Find a website by its URL., Find a repository by its URL., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo (+45 more)
...
Cohesion: 0.02
Nodes (141): Agent subsystem — planner / executor / verifier loop., AgentPhaseError, AgentRunner, _run(), Any, Exception, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in. (+133 more)
...
Cohesion: 0.03
Nodes (110): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, list_all_approvals(), list_pending(), ApprovalStatus, ApprovalStore, get_approval_store(), Enum, str (+102 more)
...
Cohesion: 0.04
Nodes (89): _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy(), PolicyUpdateBody, Any (+81 more)
...
### Community 74 - "test_a_failed_filing_releases_the_cooldown_so_the_next_recurrence_retries"
Cohesion: 0.40
...
Cohesion: 0.05
Nodes (48): field_validator, A single occurrence of a check firing on a specific URL., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoSiteFindings, _count_syllables() (+40 more)
...
Cohesion: 0.07
Nodes (44): _bullets(), generate_backlog_retro(), generate_standup(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive a retrospective from the task tracker when no sprint is active. DONE /…, Render the sprint plan as markdown., Render a list of strings as markdown bullets (or a placeholder). (+36 more)
...
Cohesion: 0.05
Nodes (65): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover() (+57 more)
...
### Community 91 - "test_backend_server_features.py"
Cohesion: 0.05
...
Cohesion: 0.11
Nodes (23): AgentRole, When coder == reviewer model, swarm should log a warning., _fake_artifact(), TestSlice, Artifact, ModelRoutingConfig, A durable, phase-produced document (markdown or JSON)., An independently deliverable unit of work within the execute phase. Each slice… (+15 more)

### Community 93 - "SeoFixer"
Cohesion: 0.07
Nodes (27): BaseModel, Request to remediate auto-fixable findings in a local code repository., One concrete remediation performed (or proposed) by the fixer., Result of a fixer run., SeoFixAction, SeoFixRequest, SeoFixResult, _humanize_filename() (+19 more)

### Community 94 - "test_runtime_governance.py"
Cohesion: 0.07
Nodes (57): _blocked_result(), _governance_check(), _governance_identity(), Any, runtimes/routing.py — RuntimeRoutingPolicyEngine. Implements the 8-step routing…, Build the failed TaskResult returned when policy denies a dispatch. A result…, Evaluate a runtime dispatch against policy; audit it either way. Returns…, Full record of a routing decision — audit-trail entry. (+49 more)
...
### Community 102 - "ensure_self_company"
Cohesion: 0.06
Nodes (37): autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, _count_specialists(), _create_company_directly(), ensure_self_company(), _find_self_company(), _find_stale_self_companies(), _list_companies_safe() (+29 more)

### Community 103 - "get_registry"
Cohesion: 0.03
Nodes (39): get_registry(), Return model registry, extended with ROUTER_EXTRA_MODELS env entries.…, claude-opus-5 must be in the built-in alias map so routing resolves it., TestClaude5RegistryEntries, test_bedrock_haiku_4_5_in_registry(), test_bedrock_opus_48_in_registry(), test_bedrock_opus_4_6_v1_in_registry(), test_bedrock_opus_4_7_in_registry() (+31 more)
...
Cohesion: 0.03
Nodes (107): PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AgentStep (+99 more)
...
Cohesion: 0.05
Nodes (32): ProviderRouter, Any, Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either…, Translate an OpenAI chat/completions payload to Bedrock Converse format., Translate a Bedrock Converse API response to OpenAI chat.completion format., Return *payload* with ``reasoning_effort`` added for Ollama thinking models.…, Priority-ordered LLM provider fallback with health checks and retries., with_ollama_reasoning_effort() (+24 more)
...
### Community 125 - "BrainFailoverManager"
Cohesion: 0.10
...
### Community 127 - "AnthropicProvider"
Cohesion: 0.04
...
Cohesion: 0.12
Nodes (7): AdaptiveHalter, ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, Tracks step-level progress and signals when a run should halt early. The halter…, Ratio of applied steps to steps attempted (0.0–1.0). Returns 1.0 when no steps…, Record one step outcome; return a halt reason or None to continue. ``status``…, Unit tests for agent.adaptive_halting.AdaptiveHalter., TestAdaptiveHalter
...
### Community 138 - "seo_api.py"
Cohesion: 0.05
Nodes (55): _get_current_user_thunk(), _get_optional_user_thunk(), Request, build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit() (+47 more)
...
Cohesion: 0.05
Nodes (67): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), _domain_list_reason(), _domain_matches(), _egress_policy_reason(), get_web_reach(), _parse_domain_list(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives… (+59 more)
...
Cohesion: 0.09
Nodes (30): Backward-compatibility shim — use scripts/generate_api_key.py instead. This…, _check_rate_limit(), default_keys_path(), issue_new_api_key(), KeyRecord, KeyStore, load_key_store(), Exception (+22 more)
...
Cohesion: 0.06
Nodes (38): BackgroundAgent, heartbeat(), BackgroundTask, _now(), Any, agent/background.py — Background Agent An always-on worker thread that…, Enqueue *task* for processing. Returns the task (with task_id set)., Convenience: create a task and submit it in one call. (+30 more)
...
Cohesion: 0.08
Nodes (51): _run(), Issue title: the failure mode plus how hard it is recurring., _company_attr(), company_stack_tags(), extract_stack_tags(), fan_out_trend(), fan_out_trends(), is_code_change_trend() (+43 more)

### Community 168 - "test_user_research_skill.py"
Cohesion: 0.03
Nodes (66): analyze_qualitative(), analyze_quantitative(), auto_register(), _classify_sentiment(), _extract_keywords(), plan_research(), Any, BaseModel (+58 more)
...
Cohesion: 0.09
Nodes (37): Aggregated report row - Screaming Frog CSV compatible., An agent-delegable remediation work package derived from the findings. Findings…, Complete result of one audit run., SeoAuditReport, SeoDelegationTask, SeoIssueReportRow, Paragraph, compute_pressure() (+29 more)

### Community 172 - "Settings"
Cohesion: 0.04
...
Cohesion: 0.07
Nodes (30): Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory, do_approve() (+22 more)
...
### Community 182 - "test_agent_api.py"
Cohesion: 0.06
Nodes (18): AdminAuthManager, AdminSession, AdminSessionStore, _is_truthy(), admin_auth.py — auto-generated module docstring (user-research skill scan)., WindowsCredentialAuthenticator, patch, TestWindowsAuth (+10 more)
...
### Community 187 - "test_portfolio_intelligence.py"
Cohesion: 0.05
...
### Community 191 - "test_failover_silent_exhaustion.py"
Cohesion: 0.12
...
Cohesion: 0.08
Nodes (39): Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient, Tests for the governance HTTP surface and the AgentRunner integration. The…, The tool that makes turning on enforcement safe., Minimal stand-in exposing only what the governance seam touches. (+31 more)
...
Cohesion: 0.04
Nodes (76): livenim, allow_paid_brain(), BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), is_anthropic_model(), _norm() (+68 more)

### Community 196 - "get_feature_matrix"
Cohesion: 0.08
...
### Community 201 - "WorkspaceManager"
Cohesion: 0.18
...
Cohesion: 0.14
Nodes (8): KeyPool, Round-robin key selection with per-key rate-limit cooldowns., Clear all state (tests only)., The default configuration must be untouched by rotation., With no sibling to fall back to, withholding the key would convert a provider…, The capacity property: one spent key does not stop the provider., TestRotation, TestSnapshotLeaksNothing
...
Cohesion: 0.03
Nodes (43): MCPClient, Any, Thin async MCP client with open/close circuit breaker. Thread-safe only within…, Full URL of the JSON-RPC endpoint this client posts to., Build the request headers shared by ``_rpc`` and ``notify``. ``Accept`` lists…, Propagate the calling agent's identity across the process boundary. The MCP…, Attach the agent identity whose actions this client executes., Remember a server-issued ``Mcp-Session-Id`` for subsequent requests. (+35 more)

### Community 212 - "loop.py"
Cohesion: 0.07
Nodes (40): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only… (+32 more)
...
Cohesion: 0.09
Nodes (37): _apply_chat_defaults(), _apply_reasoning_budget(), _emit_safely(), _extract_exact_output(), _filter_fragment(), _filter_openai_sse_line(), handle_ollama_native_chat(), handle_openai_chat_completions() (+29 more)

### Community 219 - "settings.py"
Cohesion: 0.03
Nodes (52): CerebrasProvider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NvidiaProvider, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., OllamaProvider (+44 more)

### Community 220 - "ApprovalRequest"
Cohesion: 0.09
...
### Community 221 - "NotificationDispatcher"
Cohesion: 0.05
Nodes (44): NotificationDispatcher, _send(), _send(), Any, Path, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess). (+36 more)
...
### Community 228 - "verify_service_token"
Cohesion: 0.15
...
### Community 231 - "resolve_provider_for"
Cohesion: 0.06
...
Cohesion: 0.07
Nodes (35): engine(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test(), _tracking_run_single() (+27 more)
...
### Community 238 - "TaskDispatcher"
Cohesion: 0.09
Nodes (14): Re-queue BLOCKED tasks that have cooled down and are ready for retry., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, tests/test_fixes_reliability.py — Regression tests for the batch of fixes.…, Dispatcher should track first_seen times for pending tasks., Executing a task removes it from _first_seen and logs pickup time. Uses async…, DashboardHome.js must use Promise.allSettled() not Promise.all(). Promise.all()… (+6 more)
...
### Community 242 - "test_runtimes_health_endpoint.py"
Cohesion: 0.25
...
### Community 244 - "control_overrides.py"
Cohesion: 0.07
Nodes (46): _actor(), build_platform_controls_router(), _admin(), list_controls(), reset_control(), update_controls(), ControlUpdateBody, APIRouter (+38 more)
...
### Community 257 - "report_to_markdown"
Cohesion: 0.08
...
### Community 262 - "AgentProfile"
Cohesion: 0.13
Nodes (21): agents/__init__.py — CRISPY multi-agent coding system., AgentProfile, _get_defaults(), load_all_profiles(), make_architect_profile(), make_coder_profile(), make_reviewer_profile(), make_scout_profile() (+13 more)
...
Cohesion: 0.06
Nodes (25): _best_cloud_primary_base(), InternalAgentAdapter, _nvidia_provider_chain(), Any, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Execute a TaskSpec using the internal AgentRunner and convert the agent's… (+17 more)
...
Cohesion: 0.04
Nodes (58): Any, packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, Background tick that runs self_heal_brain_and_unblock_tasks(). Called from the…, One-shot self-healing pass. 1. Checks if the active brain provider is in a…, self_heal_brain_and_unblock_tasks(), _self_heal_tick(), BrainWatchdog, _classify_failure_cause() (+50 more)

### Community 273 - "test_ephemeral_reaper.py"
Cohesion: 0.11
Nodes (27): _as_aware_utc(), _company_alive(), _company_id_for_agent(), datetime, services/ephemeral_reaper.py — destroy expired ephemeral companies. The…, Whether *company_id* still exists, cached per sweep. A lookup error returns…, Delete registered specialist agents whose owning company no longer exists.…, Treat naive datetimes as UTC so comparisons never raise. (+19 more)
...
Cohesion: 0.06
Nodes (30): build_connectors_router(), list_connectors(), webhook_send(), Any, APIRouter, BaseModel, backend/connectors_api.py — the connector catalogue API (`/api/connectors/*`).…, Reject anyone who is not the agency admin. (+22 more)

### Community 279 - "_is_denied_path"
Cohesion: 0.16
...
### Community 287 - "ai/router.py"
Cohesion: 0.06
Nodes (35): _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog(), _ollama_reasoning_effort() (+27 more)
...
### Community 293 - "register_webui"
Cohesion: 0.11
Nodes (26): get_brain_preference(), Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, _admin_out(), register_webui(), _admin_app_index(), _admin_app_spa(), admin_bootstrap(), admin_create_provider() (+18 more)
...
### Community 295 - "test_kimi_bridge_server.py"
Cohesion: 0.05
...
### Community 296 - "test_colibri_brain_shim.py"
Cohesion: 0.07
...
Cohesion: 0.13
Nodes (19): Tests for workspace isolation model (Area A). Covers: - Unique workspace path…, TestJobIdValidation, Security-oriented tests for workspace isolation (Area C4). Covers: - No path…, InvalidJobIdError, InvalidSessionIdError, Exception, workspace/errors.py — Structured, actionable workspace errors. Every error…, Base class for all workspace errors. (+11 more)
...
### Community 318 - "LogMonitor"
Cohesion: 0.07
Nodes (34): _dispatch_async(), _ErrorCaptureHandler, LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log…, Register the error capture handler on the root logger. (+26 more)
...
### Community 336 - "build_governance_router"
Cohesion: 0.16
...
### Community 338 - "test_colibri_provider.py"
Cohesion: 0.13
...
### Community 359 - "test_brain_availability_doctor.py"
Cohesion: 0.10
...
### Community 369 - "CompanyAgencyService"
Cohesion: 0.09
...
### Community 402 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.09
...
### Community 416 - "control_registry.py"
Cohesion: 0.10
...
### Community 421 - "CacheManager"
Cohesion: 0.09
Nodes (15): CacheManager, _Entry, LRUCache, Any, T, Live (unexpired) entries — used by the semantic layer's scan., Owns every cache layer and the policy for what may enter them., Whether a response to ``request`` may be stored. Deliberately conservative — a… (+7 more)
...
Cohesion: 0.09
Nodes (32): DirectChatDoctor, _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Consolidated system health report: preflight checks + runtime health. Returns a…, Public Doctor endpoint — no authentication required. Returns system-level… (+24 more)
...
### Community 450 - "telegram_service.py"
Cohesion: 0.12
...
### Community 453 - "_mock_provider_records"
Cohesion: 0.13
...
### Community 456 - "openclaw_gateway.py"
Cohesion: 0.15
...
### Community 458 - "models/seo_audit.py"
Cohesion: 0.15
Nodes (15): models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, Snapshot of one crawled page with the on-page facts the checks used., Lightweight listing entry for past audits., Static definition of a single audit check (catalog entry)., SeoAuditSummary, SeoCheckDefinition, SeoPageAudit, list_reports() (+7 more)

### Community 459 - "mcp_client.py"
Cohesion: 0.11
...
Cohesion: 0.05
Nodes (27): Resolve the base URL of the agency's own Hermes server. Precedence:…, resolve_hermes_base_url(), _env_float(), HermesAdapter, Any, AsyncClient, Submit task to Hermes via its /tasks endpoint., Read a float env var, falling back to *default* on unset/garbage. (+19 more)
...
### Community 465 - "get_skill_bindings"
Cohesion: 0.12
Nodes (21): list_skills(), List all available skills with optional filtering. Returns the skill catalog…, build_matrix(), _families(), main(), Any, scripts/generate_specialist_skill_matrix.py — Specialist × Skill matrix.…, Map family -> sorted list of test files that mention it (quoted token). (+13 more)
...
### Community 485 - "_execute_skill_impl"
Cohesion: 0.10
...
Cohesion: 0.10
Nodes (23): _cached(), cost_attribution_stats(), _fast_count(), get_stats(), legacy_scheduler_list(), _produce_scheduler_jobs(), _produce_stats(), Single-flight TTL cache. Concurrent callers wait for the first producer. (+15 more)
...
### Community 513 - "parse_event_stream"
Cohesion: 0.09
...
### Community 524 - "test_one_model_catalogue.py"
Cohesion: 0.07
...
### Community 529 - "test_ceo_router.py"
Cohesion: 0.05
Nodes (48): Attempt, _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One delegation of one subtask to one tier. (+40 more)
...
### Community 546 - "test_daily_automation_2026_07_11.py"
Cohesion: 0.14
...
### Community 550 - "ExecutionRequest"
Cohesion: 0.04
Nodes (54): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), OrchestratorSupervisor, Any, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor(), ExecutionRequest (+46 more)
...
### Community 569 - "test_task_brain_preflight.py"
Cohesion: 0.21
...
### Community 591 - "test_kimi_bridge_provider.py"
Cohesion: 0.20
...
### Community 598 - "reset_failover_manager"
Cohesion: 0.08
Nodes (19): Reset the singleton (for tests)., Drop the cached Mongo client, backoff, and read caches (tests)., reset_failover_manager(), reset_kv_state(), _clean_env(), _FakeAsyncClient, _FakeResponse, Regression test: HTTP 413 (Payload Too Large) must fail over to the next… (+11 more)
...
### Community 609 - "test_telegram_task_callback.py"
Cohesion: 0.12
...
### Community 611 - "get_failover_manager"
Cohesion: 0.16
Nodes (11): get_failover_manager(), Return the singleton BrainFailoverManager., test_singleton(), Regression guard: a rate-limit cooldown must survive, on every route.…, The anti-wedge valve must not fire for an ordinary 429 backoff — otherwise it…, The threshold must clear the widest backoff ANY registered provider can earn.…, A corrupted/absurd cooldown must still be recoverable., The honest reset: probe permitted, failure history kept. (+3 more)
...
### Community 616 - ".test_all_cooling_returns_none_instead_of_resetting"
Cohesion: 0.10
...
### Community 682 - "test_autonomy_pipeline_regressions.py"
Cohesion: 0.11
...
### Community 712 - "_InMemoryBackend"
Cohesion: 0.18
...
### Community 717 - "tests/conftest.py"
Cohesion: 0.05
...
### Community 722 - "test_schedule_persistence.py"
Cohesion: 0.15
...
### Community 755 - "TestModelRegistryUpdates"
Cohesion: 0.21
Nodes (5): Test hook — rebuild the registry on next access., reset(), tests/test_daily_automation_2026_07_10.py — Daily automation tests…, Verify new models are in the packages/ai/registry., TestModelRegistryUpdates
...
Cohesion: 0.15
Nodes (12): Unique identifier (e.g. 'nvidia', 'cerebras')., provider_id_of(), Any, Extract a provider id from a ProviderConfig dataclass or a plain dict., Return *providers* reordered according to the active strategy. ``group_key``…, Reorder one interchangeable group of providers., Ascending sort by *score* with a random tie-break. The tie-break is the point:…, Weighted random permutation — heavier providers tend to come first. Weight… (+4 more)
...
Cohesion: 0.16
Nodes (12): generate_sprint_retro(), Derive retro notes for ``sprint`` from its current metrics. Records…, Enum, Agentic Agile — Sprint management with velocity tracking and burndown. Issue:…, Sprint retrospective notes and follow-up action items., Whether the retrospective has any recorded content., Status of a user story within a sprint., Qualitative health signal derived from sprint metrics. (+4 more)
...
### Community 796 - "test_telegram_auto_approve.py"
Cohesion: 0.21
...
### Community 798 - "test_hermes_server.py"
Cohesion: 0.18
...
### Community 799 - "build_digest"
Cohesion: 0.16
Nodes (11): build_digest(), _count_open_auto_prs(), Send the digest text via NotificationDispatcher (Telegram)., Count open PRs with the 'automated' or 'auto-pr' label via git log heuristic., Build the weekly readiness digest as Markdown text., send_digest(), object, patch (+3 more)
...
### Community 802 - "clear_wizard_state_cache"
Cohesion: 0.19
...
Cohesion: 0.16
Nodes (14): _build_cost_table(), clear_stats(), cost_for_tokens(), get_stats(), _load_env_overrides(), Any, Per-model cost attribution for the LLM provider router. Maintains in-memory…, Parse MODEL_COST_INPUT / MODEL_COST_OUTPUT env overrides. Format:… (+6 more)
...
### Community 823 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 828 - "_start_ceo_agency"
Cohesion: 0.27
...
### Community 837 - "TestRevenuePortfolio"
Cohesion: 0.22
...
Cohesion: 0.06
Nodes (46): get_agency(), set_agency(), _autonomy_bg_cycle(), autonomy_tick(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_trigger(), Background CEO cycle + task dispatch. Runs fire-and-forget. (+38 more)

### Community 839 - "TestAnthropicPayloadStructuredOutput"
Cohesion: 0.29
...
Cohesion: 0.06
Nodes (17): AgentSwarm, Any, Path, Return the agent role responsible for *phase*., Return the AgentProfile for the agent driving *phase*., Return a JSON-serialisable summary of all agent profiles., Run a pre-gate or report phase through the correct agent. Enforces permission…, Execute a slice via the Coder agent (write-permitted). (+9 more)

### Community 852 - "openclaw_reverse_proxy"
Cohesion: 0.22
...
### Community 868 - "PortfolioIntelligence"
Cohesion: 0.17
...
### Community 938 - ".mark_rate_limited"
Cohesion: 0.18
...
### Community 959 - "_looks_unknown_model"
Cohesion: 0.20
...
### Community 997 - "TestProvidersScreen"
Cohesion: 0.43
...
### Community 1028 - "TestAgentRunnerPublicPlanMethod"
Cohesion: 0.20
...
Cohesion: 0.04
Nodes (72): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), get_ceo_dispatcher(), _merge_changed_files(), _offload(), Any (+64 more)
...
### Community 1067 - "Feature Support Matrix"
Cohesion: 0.33
Nodes (6): Admin API, Config Overrides, Feature Matrix, Feature Support Matrix, Gating Behavior, Maturity Tiers

### Community 1068 - "test_key_pool.py"
Cohesion: 0.25
Nodes (7): provider_api_keys(), Every API key configured for *provider*, primary first. Reads ``base_env`` then…, Per-provider API key rotation — the one lever that adds capacity. Every other…, Clear pool state (tests only)., reset(), _clean_pool(), Tests for packages/ai/key_pool.py — per-provider API key rotation. Rotation is…
...
### Community 1107 - "CapacityAllocation"
Cohesion: 0.25
...
### Community 1139 - "test_skills_route_order.py"
Cohesion: 0.67
...
### Community 1156 - "_answer_callback"
Cohesion: 0.29
...
### Community 1207 - "TaskHarnessAdapter"
Cohesion: 0.15
Nodes (7): Any, Adapter for a compatible external task harness., TaskHarnessAdapter, TestTaskHarnessAdapterExecution, When task-harness is required but not present, preflight should return a…, Every preflight report must have runtime_id, ready, summary., TestRuntimePreflight
...
### Community 1277 - "TestExhaustedPoolDoesNotReusePrimary"
Cohesion: 0.29
...
### Community 1279 - "InitiativeProgress"
Cohesion: 0.33
...
### Community 1284 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 1287 - "TestLoopAdaptiveHalterIntegration"
Cohesion: 0.33
Nodes (3): Verify loop.py imports and wires AdaptiveHalter correctly., AgentRunner must expose _adaptive_halter on construction., TestLoopAdaptiveHalterIntegration

### Community 1289 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 1298 - "Who is this for?"
Cohesion: 0.40
...
### Community 1312 - "Privacy, security, and cost"
Cohesion: 0.50
Nodes (4): Privacy, security, and cost, Security posture, What it costs to run, Your data never leaves your server

### Community 1314 - "test_a_scheduled_diagnosis_is_held_until_it_completes"
Cohesion: 0.67
...
### Community 1319 - "Configuration reference"
Cohesion: 0.67
...
### Community 1322 - "test_mint_token_rejects_missing_args"
Cohesion: 0.67
...
- **3548 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3543 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14447 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **175 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `ClaudeCodeAdapter`, `parse_event_stream`, `test_autonomy_status.py`, `brain_config.py`, `test_brain_patch_service_token.py`, `llm/router.py`, `OnboardingService`, `test_portfolio_intake.py`, `E2BAdapter`, `test_freebuff_bot.py`, `TaskStatus`, `test_llm_router_strategies.py`, `test_harness_spec.py`, `failover_client.py`, `test_catalogue_probe.py`, `types.py`, `test_platform_controls.py`, `AgentSessionStore`, `ImprovementLoop`, `Agency`, `SelfHealingAgent`, `Specialist`, `test_company_api.py`, `test_unit8_model_catalog.py`, `test_langfuse_agency_wide.py`, `test_phase5_doctor.py`, `TaskWorkflowService`, `test_failover_client_shared.py`, `Task`, `E2BSandboxSession`, `UserRole`, `test_model_router.py`, `services/background.py`, `CEOSupervisor`, `resolve_identity`, `test_key_pool.py`, `test_llm_router_resilience.py`, `test_agent_tool_governance.py`, `claim`, `e2e/test_browser.py`, `test_task_brain_preflight.py`, `AuditLog`, `test_openclaw_endpoints.py`, `AgentRunner`, `PrimeAgentAdapter`, `client`, `test_governance_enforcement.py`, `test_mcp_governance.py`, `nvidia_models.py`, `services/seo_audit.py`, `get_task_store`, `reset_failover_manager`, `quick_note.py`, `test_local_brain_state.py`, `model_router.py`, `SeoFixer`, `WorkflowRun`, `config.py`, `TestChatHistoryStore`, `test_refresh_agent_built_proof.py`, `test_telegram_task_callback.py`, `TestClient`, `test_procedural_memory.py`, `resolve_e2b_config`, `test_knowledge_sync.py`, `test_sam_voice.py`, `test_provider_state_durability.py`, `ArtifactStore`, `test_new_features_e2e.py`, `direct_chat.py`, `test_control_plane_api.py`, `test_sam_livekit.py`, `TokenBudget`, `test_voice_pipeline.py`, `test_startup_warmup.py`, `diagnostics.py`, `seo_api.py`, `ChatHistoryStore`, `persist_plan_spec`, `test_context_rulebook.py`, `test_llm_router_e2e.py`, `test_trend_watcher.py`, `KeyStore`, `Usage`, `_step`, `_is_exempt`, `_run_analyze`, `test_openclaw_gateway.py`, `TestEstimateTokensForMessages`, `ResearchOrchestrator`, `run_task`, `test_issue_intake.py`, `WorkflowBuildRequest`, `test_e2b_data_flow.py`, `test_e2b_task_wiring.py`, `resolve_active_brain`, `get_feature_matrix`, `test_telegram_freebuff.py`, `tests/conftest.py`, `OllamaCircuitBreaker`, `_job_text`, `test_video_transcript.py`, `test_daily_2026_07_27.py`, `MCPClient`, `SchedulerStore`, `NotificationDispatcher`, `test_features_api.py`, `TrendWatcher`, `WorkflowRun`, `test_agile_api.py`, `test_app_settings.py`, `test_runtimes_health_endpoint.py`, `api_keys_for`, `test_monitor_lib.py`, `test_spa_trailing_slash_redirect.py`, `test_telegram_webhook.py`, `._order_group`, `ContextPruner`, `test_purge_backlog.py`, `AgentProfile`, `test_rate_limiter.py`, `test_persistent_memory.py`, `test_provider_enable_disable.py`, `SecurityScanner`, `BrainWatchdog`, `test_ephemeral_reaper.py`, `TestClient`, `test_nvidia_model_discovery.py`, `OutputFilter`, `reset_cache`, `test_telegram_auto_approve.py`, `ai/router.py`, `analyze_page`, `test_kimi_bridge_server.py`, `test_colibri_brain_shim.py`, `clear_cooldowns`, `cost_tracker.py`, `_Budget`, `_start_ceo_agency`, `LogMonitor`, `test_agency_fix.py`, `TestTheWorkflowIsSafeAndReadOnly`, `_resolve_push_token`, `test_skill_registry_boot_refresh.py`, `TestMongoGate`, `OrchestratorCheckpointStore`, `_redact_for_notification`, `AgentSwarm`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_brain_availability_doctor.py`, `test_crispy_burn_in.py`, `test_implement_agent_routing.py`, `test_executive_advisory_api.py`, `test_process_quick_note_workflow.py`, `isolated_telegram_config`, `_Recorder`, `test_scheduler_hydration_bounded.py`, `_undeclared`, `test_service_token.py`, `test_telegram_diag_endpoint.py`, `CacheManager`, `DirectChatDoctor`, `test_v4_api.py`, `captured`, `UsageEvent`, `test_gh_brain_failover.py`, `_mock_provider_records`, `SQLiteStore`, `TestHarnessAdapter`, `TestCountTokensEndpoint`, `test_webui_provider_priority.py`, `test_dashboard_cache.py`, `TestNormalizeResponseFormat`?**
  _High betweenness centrality (0.097) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `test_ceo_dispatcher.py`, `GitHubTools`, `TestAgentRunnerPublicPlanMethod`, `AdaptiveHalter`, `diagnostics.py`, `TaskSpec`, `ContextManager`, `proxy.py`, `E2BAdapter`, `InternalAgentAdapter`, `CEODispatcher`, `AgentSessionStore`, `test_empirical_verify.py`, `ReactScratchpad`, `test_daily_automation_2026_07_11.py`, `WorkspaceTools`, `test_autonomous_agency_e2e.py`, `LocalWorkspace`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `E2BSandboxSession`, `.__init__`, `resolve_identity`, `test_agent_loop_delegates_to_the_shared_client`, `TestAgentRunnerSafety`, `StuckDetector`, `test_agent_tool_governance.py`, `test_governance_api.py`, `resolve_active_brain`, `test_governance_enforcement.py`, `UserMemoryStore`, `MCPClient`, `loop.py`, `reset_failover_manager`, `WorkflowRun`, `TestAgentLoopMCPIntegration`, `RewardScorer`, `FreeBuffAgent`, `TaskDispatcher`, `direct_chat.py`, `TokenBudget`, `ContextPruner`, `test_agent_chat_integration.py`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `backend/server.py`, `test_provider_router.py`, `failover_client.py`, `test_bedrock_live.py`, `TestLegacyRouterServerFallback`, `ai/router.py`, `test_colibri_brain_shim.py`, `implement_agent.py`, `TestOpenAiToBedrockConverse`, `clear_cooldowns`, `TestLegacyRouterCacheTTL`, `model_discovery.py`, `TestAnthropicPayloadStructuredOutput`, `TrafficDirector`, `test_kimi_bridge_provider.py`, `test_colibri_provider.py`, `settings.py`, `test_all_providers_discovery.py`, `get_failover_manager`, `TestRouterIntegration`, `_payload`, `test_implement_agent_routing.py`, `ProviderConfig`, `direct_chat.py`, `test_anthropic_refusal_fallback.py`?**
