# Graph Report - local-llm-server  (2026-09-19)

## Corpus Check
- 1528 files · ~2,221,760 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 32268 nodes · 59544 edges · 1365 communities (1184 shown, 181 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3752 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f12118e5`
...
- `_run()` --indirect_call--> `root()`  [INFERRED]
  agent/loop.py → launcher.py
...
- `test_engagement_dau_counts_unique_users()` --calls--> `EngagementMetrics`  [INFERRED]
  tests/test_ai_insights.py → agents/ai_insights.py

## Import Cycles
- None detected.

## Communities (1365 total, 181 thin omitted)

### Community 0 - "TaskSpec"
Cohesion: 0.04
Nodes (91): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor., json_safe(), runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).… (+83 more)

### Community 1 - "post"
Cohesion: 0.03
Nodes (83): BrainTestRequest, clear_response_cache(), create_mcp_server(), _get_backend_sam(), get_catalog_models_route(), github_oauth_start(), GitHubFileWrite, GitHubPRCreate (+75 more)

### Community 2 - "LLMRequest"
Cohesion: 0.03
Nodes (106): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, OpenAICompatible, Adapter for any endpoint speaking the OpenAI chat-completions shape. Adding a…, build_provider(), Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the…, resolve_kind() (+98 more)

### Community 3 - "build_llm_router"
Cohesion: 0.05
Nodes (54): get_budget(), The process-wide budget tracker., get_cache(), The process-wide cache manager., gateway_enabled(), Whether the OpenAI-compatible passthrough routes accept traffic. Off by…, _available(), build_llm_router() (+46 more)

### Community 4 - "_is_ephemeral_user"
Cohesion: 0.18
Nodes (17): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), create_company(), _is_ephemeral_user(), True when this user's companies should be temporary (reaped after TTL).…, Best-effort auth provider for a user dict. social_auth users carry an explicit…, Create a new company. This is the entry point for the Company Graph. Creates a… (+9 more)
...
Cohesion: 0.03
Nodes (163): middleware, admin_control(), admin_create_key(), admin_create_user(), admin_delete_user(), admin_login(), admin_logout(), admin_status() (+155 more)

### Community 7 - "api.js"
Cohesion: 0.01
Nodes (46): approveGovernanceRequest(), autoRecommendCompanySkills(), createMcpServer(), createQuickNote(), createTask(), delegateSeoFindings(), deleteCompany(), deleteMcpServer() (+38 more)

### Community 8 - "test_governance_sandbox.py"
Cohesion: 0.03
Nodes (83): Write the audit row for a completed (or blocked) MCP tool call., record(), events_from_identity(), Extract the identity-derived audit fields from an AgentIdentity. Kept here…, build_docker_run_argv(), detect_backend(), DockerBackend, E2BBackend (+75 more)

### Community 9 - "test_portfolio_intake.py"
Cohesion: 0.07
Nodes (37): Issue title: the failure mode plus how hard it is recurring., fetch_research_alerts(), Best-effort fetch of trend alerts. Returns [] if offline or unavailable., Weighted Shortest Job First score — higher schedules sooner., map_initiative_to_task(), materialize_committed(), _wsjf(), _portfolio_materialize_enabled() (+29 more)
...
### Community 11 - "TaskStore"
Cohesion: 0.04
Nodes (79): tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, TaskPriority, _priority_rank(), Any, tasks/store.py — MongoDB-backed task store. Falls back to in-memory storage…, Fail loud in production if no persistent storage is configured. Telegram… (+71 more)

### Community 12 - "AgentDefinition"
Cohesion: 0.05
Nodes (50): AgentDefinition, AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+42 more)

### Community 13 - "company_api.py"
Cohesion: 0.02
Nodes (141): AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), delete_company_endpoint(), _DoctorCheck, _DoctorReport, generate_onboarding_questions(), get_company() (+133 more)

### Community 14 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (84): CEOResult, _merge_changed_files(), Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Collect changed_files across all specialists into a single de-duped list., reset_ceo_dispatcher(), get_ceo_fallback_stats(), Increment the named counter (silently ignores unknown reasons). (+76 more)

### Community 15 - "test_llm_router_strategies.py"
Cohesion: 0.03
Nodes (111): count, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, HealthTracker, _Outcome, ProviderHealth (+103 more)

### Community 16 - "ExecutionRequest"
Cohesion: 0.05
Nodes (35): ExecutionRequest, _get_ceo_dispatcher(), Exception, Determine domain and task type from the request text., Generate a structured execution plan. Uses the LLM if available; falls back to…, Select the best specialist(s) for the classified domain., Resolve and bind skills, memory, and Company Graph context., Execute the plan via the selected specialist(s). (+27 more)

### Community 17 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (124): _diagnose_and_file(), _format_incident(), gather_render_evidence(), get_operational_incident_tracker(), _iso_from_monotonic(), normalise(), note_phase_end(), note_phase_start() (+116 more)

### Community 18 - "test_platform_controls.py"
Cohesion: 0.09
...
### Community 19 - "LLMRouter"
Cohesion: 0.07
Nodes (26): text_key(), Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, run(), Any, AsyncClient, One shared connection pool. Reconnects if it was closed. (+18 more)

### Community 20 - "RequestQueue"
Cohesion: 0.04
Nodes (33): Bulkhead, Deduplicator, Any, Semaphore, T, Collapse concurrent identical calls onto a single execution., Execute ``factory``, or await an identical call already running. Returns…, One job waiting for a concurrency slot, ordered by (priority, arrival). (+25 more)

### Community 21 - "_fixture"
Cohesion: 0.02
Nodes (108): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., setup_database_moks() (+100 more)
...
Cohesion: 0.03
Nodes (95): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), BaseModel (+87 more)

### Community 24 - "telegram_bot.py"
Cohesion: 0.07
Nodes (55): get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control() (+47 more)

### Community 25 - "get_workflow_orchestrator"
Cohesion: 0.04
Nodes (43): get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, start_orchestrator_supervisor() (+35 more)

### Community 26 - "test_agent_tool_governance.py"
Cohesion: 0.04
Nodes (66): Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer…, Store and emit *event*. Never raises. (+58 more)

### Community 27 - "test_brain_failover.py"
Cohesion: 0.05
Nodes (49): get_failover_manager(), ProviderHealth, Return the singleton BrainFailoverManager., Reset the singleton (for tests)., Circuit-breaker state for a provider., reset_failover_manager(), _clean_env(), _FakeAsyncClient (+41 more)

### Community 28 - "Agency"
Cohesion: 0.04
Nodes (76): Agency, AgencyCycleResult, AgentDirective, AgentRole, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context() (+68 more)

### Community 29 - "build_governance_router"
Cohesion: 0.04
Nodes (73): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget() (+65 more)

### Community 30 - "Task"
Cohesion: 0.03
Nodes (82): Full task/issue document., Update the updated_at timestamp., Task, _is_brain_connection_error(), _is_outward_facing(), Any, BaseException, One-line human explanation of why this task needs human approval. Reads risk… (+74 more)

### Community 31 - "WorkspaceTools"
Cohesion: 0.08
Nodes (34): Delegate to RepowiseIntelligence for a natural-language codebase question., Delegate to RepowiseIntelligence for a semantic/text codebase search., Delegate to RepowiseIntelligence to list architectural decision nodes., Return a high-level repository map and hotspot summary., Return a formatted context block for the given file paths., Return a risk summary for the workspace or a subset of files., Return git-blame / decision rationale for a file., WorkspaceTools (+26 more)

### Community 32 - "direct_chat.py"
Cohesion: 0.07
...
Cohesion: 0.04
Nodes (68): AgentConfig, build_agent_specs(), build_swarm(), build_task_specs(), coordinate_v2(), CoordinateRequestV2, CoordinateResponse, Any (+60 more)

### Community 34 - "loop.py"
Cohesion: 0.05
Nodes (42): Agent subsystem — planner / executor / verifier loop., _check_extra_kwargs(), _enforce_signature(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, AgentEvent (+34 more)

### Community 35 - "test_unit8_model_catalog.py"
Cohesion: 0.04
Nodes (70): all_provider_ids(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the…, True when the env var for *provider*'s key is set (or it's Ollama)., CatalogActiveBrain, CatalogMirror, CatalogProviderEntry, get_model_catalog_store() (+62 more)

### Community 36 - "WebsiteScanner"
Cohesion: 0.03
Nodes (71): main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), _is_safe_url(), _looks_like_bot_challenge(), BeautifulSoup, True if the HTML is an anti-bot/CAPTCHA interstitial rather than content. Used… (+63 more)

### Community 37 - "PolicyEngine"
Cohesion: 0.04
Nodes (81): _action_matches(), _as_list(), Decision, GroupPolicy, _host_matches(), Mode, _normalise_path(), _path_matches() (+73 more)

### Community 38 - "E2BSandboxSession"
Cohesion: 0.04
Nodes (72): MCPUnavailableError, RuntimeError, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any, Create the sandbox. Raises :class:`MCPUnavailableError` on failure. (+64 more)

### Community 39 - "test_telegram_freebuff.py"
Cohesion: 0.07
Nodes (44): cmd_freebuff(), _model_keyboard(), _parse_callback(), _parse_user_ids(), _process_callback(), Accept / reject keyboard shown after a FreeBuff plan is generated., Start a FreeBuff flow: fetch free models and present a picker keyboard., Handle an inline-button press for the FreeBuff accept/reject/model flow. (+36 more)

### Community 40 - "ImprovementLoop"
Cohesion: 0.05
Nodes (61): DetectedIssue, get_improvement_loop(), ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity, _now(), Any (+53 more)

### Community 41 - "get_db"
Cohesion: 0.03
Nodes (115): ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills(), brain_failover_status(), create_api_key(), create_github_pr(), create_provider() (+107 more)

### Community 42 - "get_user_role"
Cohesion: 0.04
Nodes (53): compute_savings(), compute_time_series(), get_savings(), get_usage(), get_user_savings(), _period_start(), Any, BaseModel (+45 more)

### Community 43 - "test_failover_client_shared.py"
Cohesion: 0.05
Nodes (89): failover_chat_completion(), FailoverResult, _log_recovery(), A successful completion plus the accounting its callers need., Report a successful failover at INFO — recovery is not an incident. Everything…, Run one chat completion across the brain-failover chain. Tries each healthy…, mark_dead(), Hold *model_id* out of *provider_id*'s rotation for *ttl_sec* seconds. Called… (+81 more)

### Community 44 - "resolve_active_brain"
Cohesion: 0.05
Nodes (59): BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), _norm(), _pick_from_records(), _build(), _prio() (+51 more)
...
### Community 46 - "failover_client.py"
Cohesion: 0.06
Nodes (49): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion() (+41 more)

### Community 47 - "test_repo_connection.py"
Cohesion: 0.06
...
Cohesion: 0.04
Nodes (50): MongoDBStore, Any, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., MongoDB implementation of the Company Graph store. Uses Motor (async MongoDB…, Get or create the MongoDB database connection., Convert string ID to ObjectId. (+42 more)
...
### Community 50 - "FreeBuffAgent"
Cohesion: 0.07
Nodes (37): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is… (+29 more)

### Community 51 - "test_web_reach.py"
Cohesion: 0.05
Nodes (64): _domain_list_reason(), _domain_matches(), _egress_policy_reason(), get_web_reach(), _parse_domain_list(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return True if *host* is *pattern* or a subdomain of it., Split a comma-separated env var into a normalised domain list. (+56 more)

### Community 52 - "test_governance_enforcement.py"
Cohesion: 0.02
Nodes (125): ApprovalStatus, ApprovalStore, get_approval_store(), Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`…, Block until *approval_id* is resolved or its TTL expires. Combines an event… (+117 more)
...
### Community 57 - "test_startup_warmup.py"
Cohesion: 0.05
...
### Community 61 - "test_model_router.py"
Cohesion: 0.04
Nodes (96): classify_task(), _extract_recent_text(), Any, Task classification from request context. Classifies an incoming request into a…, Concatenate plain text from the last *last_n* messages., Return the most likely task category for this request. Args: messages: OpenAI-…, _enabled(), get_available_models() (+88 more)

### Community 62 - "test_ceo_supervision.py"
Cohesion: 0.06
Nodes (66): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, A subtask's full history: what it is, and every attempt at it., SubtaskRecord, _goal(), asyncio, Tests for the CEO ledger, the supervised escalation loop, and the 24x7 sweeper.…, A storage outage must never take the agency down. (+58 more)

### Community 63 - "agent/workspace.py"
Cohesion: 0.05
Nodes (48): _get_workspace_lock(), get_workspace_manager(), _hash_component(), _iso_now(), _iso_offset_hours(), _load_workspace(), _parse_iso(), Any (+40 more)

### Community 64 - "ProviderConfig"
Cohesion: 0.05
Nodes (58): _acquire_provider_probe(), _dead_model_key(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _ollama_reasoning_effort(), provider_access_tier() (+50 more)

### Community 65 - "BrainWatchdog"
Cohesion: 0.05
Nodes (47): _migrate_brain_to_safe_default(), get_brain_config_store(), Return the process-wide ``BrainConfigStore`` singleton., Force a cache refresh (used by tests + the GET /admin/api/policy/brain…, refresh_brain_config_cache(), Any, packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, Background tick that runs self_heal_brain_and_unblock_tasks(). Called from the… (+39 more)

### Community 66 - "services/seo_audit.py"
Cohesion: 0.04
Nodes (58): field_validator, A single occurrence of a check firing on a specific URL., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoIssueReportRow (+50 more)

### Community 67 - "unsafe_target_reason"
Cohesion: 0.12
Nodes (15): _load_script_module(), Any, ModuleType, Dynamically load a pure-stdlib helper module from .github/scripts/. Returns…, Read a web page as plain text, following the same fallback chain (direct ->…, GET *url*, re-validating every redirect hop against ``unsafe_target_reason`` —…, httpx-only fallback used when the .github/scripts helpers can't be loaded (e.g.…, Free-text web search with no API key, via DuckDuckGo's HTML-only endpoint.… (+7 more)

### Community 68 - "nvidia_models.py"
Cohesion: 0.05
Nodes (44): ApplyReviewAgent, build_review_context(), _gh(), main(), _openai_tools_to_anthropic(), Convert OpenAI function-calling tool schemas to Anthropic tool schemas., Return (result_text, should_stop)., Run using NVIDIA NIM (OpenAI-compatible). Called as fallback. (+36 more)

### Community 69 - "RenderOpsMonitor"
Cohesion: 0.05
Nodes (33): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, One deploy, normalised from whatever shape the tool returned., RenderDeploy, Polls Render for platform-level failures and files them as issues., Map service ID → human name, degrading to the ID when unavailable., RenderOpsMonitor (+25 more)

### Community 70 - "SelfHealingAgent"
Cohesion: 0.03
Nodes (78): FailureCategory, heal_signature(), HealingEvent, HealState, _now(), Any, Enum, str (+70 more)

### Community 71 - "ensure_self_company"
Cohesion: 0.08
Nodes (26): autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, _create_company_directly(), ensure_self_company(), _find_self_company(), _find_stale_self_companies(), _list_companies_safe(), List companies, skipping any that fail to deserialize. A previous deploy wrote… (+18 more)

### Community 72 - "test_runtime_governance.py"
Cohesion: 0.11
Nodes (44): _governance_check(), Evaluate a runtime dispatch against policy; audit it either way. Returns…, _decision(), _engine(), Any, Governance on runtime dispatch — the last coverage gap.…, `route_and_execute` callers handle failure; they do not expect a new exception…, So callers can branch without string-matching the output. Regression: an… (+36 more)

### Community 73 - "Specialist"
Cohesion: 0.03
Nodes (71): A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, build_matrix(), _families(), main(), Any, scripts/generate_specialist_skill_matrix.py — Specialist × Skill matrix.… (+63 more)
...
### Community 78 - "services/background.py"
Cohesion: 0.04
Nodes (88): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+80 more)

### Community 79 - "generate_context.py"
Cohesion: 0.12
...
### Community 81 - "WorkflowRun"
Cohesion: 0.06
Nodes (31): _NoopStore, Any, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs…, Push a Telegram approval-gate notification when a run pauses (Charter G1).…, Persist first-merge consent once an operator approves a ``telegram_gate``. No-…, List recent runs. When ``owner_id`` is provided, only runs stamped with that…, No-op checkpoint store when the real one is unavailable. (+23 more)

### Community 82 - "test_sam_livekit.py"
Cohesion: 0.06
Nodes (34): auth_headers(), livekit_env(), no_livekit_env(), tests/test_sam_livekit.py — SAM realtime voice (LiveKit) integration. Covers: -…, SAM_LLM_* env vars must override the NVIDIA defaults (Hermes/proxy routing)., Under TESTING the in-process worker must never be eligible to start., OPT-IN: defaulting to on OOM-killed the 512MB Render instance at boot…, start_in_process must be a safe no-op in the test environment (conftest sets… (+26 more)

### Community 83 - "activation_api.py"
Cohesion: 0.09
...
Cohesion: 0.04
Nodes (69): _get_current_user_thunk(), _get_optional_user_thunk(), Request, create_refresh_token(), get_current_user(), get_optional_user(), github_callback(), github_login() (+61 more)
...
### Community 86 - "portfolio_intelligence.py"
Cohesion: 0.07
Nodes (43): generate_standup(), Build a :class:`StandupReport` from ``.claude/state/active-tasks.md``. Reads…, InitiativeStatus, _bug_scores(), _clean(), _default_repo(), _env_github_token(), estimate_job_size() (+35 more)

### Community 87 - "test_provider_router.py"
Cohesion: 0.05
Nodes (48): extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), Normalize NVIDIA base URLs to avoid double /v1 when openai_compat_url appends…, Registry base URLs must produce routes the provider actually serves., brain_failover must not drift from the catalog's base URLs., TestFailoverRegistryBaseUrls, _clear_probe_locks() (+40 more)

### Community 88 - "test_bedrock_provider.py"
Cohesion: 0.08
...
### Community 89 - "test_mcp_registry.py"
Cohesion: 0.05
Nodes (51): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable(), _playwright_configured(), _playwright_spec() (+43 more)

### Community 90 - "claim"
Cohesion: 0.05
Nodes (35): claim(), clear_all_locks(), cooldown_clear(), cooldown_get(), cooldown_set(), _get_backend(), incr_window(), Shared-state abstraction — in-memory (default) and Redis backends. Provides… (+27 more)

### Community 91 - "useSafeData"
Cohesion: 0.06
...
### Community 92 - "test_model_catalog.py"
Cohesion: 0.04
...
### Community 93 - "sync/service.py"
Cohesion: 0.09
Nodes (34): FastAPI dependency: require Power User or Admin role. Raises 403 otherwise., require_power_user(), sync/ — Syncthing-style workspace synchronisation service., add_peer(), get_folder_index(), get_sync_file(), get_sync_service(), list_conflicts() (+26 more)

### Community 94 - "AgentScheduler"
Cohesion: 0.08
Nodes (28): AgentScheduler, _now(), Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Register a new job. Returns the created :class:`ScheduledJob`.…, Fire a job immediately (webhook / manual trigger)., Remove a job. Returns *True* if it existed., Update the display name of a job., Enable or disable a job without deleting it. (+20 more)

### Community 95 - "TestClient"
Cohesion: 0.08
Nodes (46): _auth_headers(), _build_agent_http_mock(), mock_get(), mock_post(), mock_put(), _exec(), _fake_request(), _mcp_tool_response() (+38 more)

### Community 96 - "ContextManager"
Cohesion: 0.09
Nodes (25): ContextManager, Any, True when the history is long enough to warrant compaction., Replace the old portion of *history* with a single compaction note. The…, True when the harness should use head_file instead of read_file. When a file is…, Trim a step result so sub-agent outputs stay within ~1-2k tokens. The Anthropic…, Manages context window state for a single agent run. The Brain (LLM) stays…, Return a copy of *observations* with old tool outputs truncated. JetBrains… (+17 more)

### Community 97 - "SetupWizardPage.js"
Cohesion: 0.13
...
### Community 98 - "WorkflowRun"
Cohesion: 0.07
Nodes (30): engine(), do_approve(), _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure (+22 more)

### Community 99 - "TestClient"
Cohesion: 0.10
Nodes (29): bare_repo(), _call(), _data(), git_config_env(), _is_error(), mcp_workspace_root(), Path, skipif (+21 more)

### Community 100 - "test_procedural_memory.py"
Cohesion: 0.05
...
### Community 101 - "seo_portfolio_bridge.py"
Cohesion: 0.06
Nodes (42): Enum, Agentic Portfolio Management — initiative prioritisation, capacity allocation,…, Roadmap placement for an initiative (Now/Next/Later planning)., RoadmapHorizon, build_seo_roadmap(), delegation_plan_to_initiatives(), delegation_task_to_initiative(), plan_seo_sprint() (+34 more)

### Community 102 - "CEOSupervisor"
Cohesion: 0.05
Nodes (47): build_ceo_router(), ceo_status(), get_goal(), list_goals(), redrive_goal(), run_sweep(), Any, APIRouter (+39 more)

### Community 103 - "ModelRouter"
Cohesion: 0.09
Nodes (15): ModelRouter, Any, Central model router. Create one instance (use ``get_router()``). ``route()``…, Decide which Ollama model to use for this request. Args: requested_model: Model…, Return *model* if it is available in Ollama, else the first available fallback.…, Full record of a routing decision — both what was chosen and why. Fields:…, Build an ordered list of alternative models to try if *primary* fails., Flat dict suitable for Langfuse observation metadata. (+7 more)

### Community 104 - "V5App.jsx"
Cohesion: 0.05
Nodes (36): API, getAccountLifecycle(), getDefaultBackendUrl(), getMe(), login(), logout(), App(), AppRoutes() (+28 more)

### Community 105 - "test_knowledge_sync.py"
Cohesion: 0.07
...
### Community 106 - "PersistentMemoryStore"
Cohesion: 0.09
Nodes (30): PersistentMemoryStore, Any, Connection, Path, Recall a specific memory entry., Auto-load relevant memories based on context. Returns memories prioritized by:…, Get all memories in a specific category., Delete a memory entry. (+22 more)

### Community 107 - "CompanyGraph"
Cohesion: 0.04
Nodes (34): BusinessCategory, detect_company_id(), DirectChatSession, handle_chat_message_with_context(), Direct chat session with Company Graph context binding., Bind a company to this chat session and load its Company Graph., Bind a repository to this chat session., Get the bound Company Graph. (+26 more)

### Community 108 - "run_task"
Cohesion: 0.05
Nodes (42): Resolve the model id to force for a code-execution run, or ``None``. Returns…, resolve_coding_model_preference(), _check_auth(), health(), Any, BaseModel, get, post (+34 more)

### Community 109 - "test_provider_state_durability.py"
Cohesion: 0.15
...
### Community 110 - "get_task_store"
Cohesion: 0.06
Nodes (64): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover() (+56 more)

### Community 111 - "gsap.min.js"
Cohesion: 0.06
Nodes (15): ia(), ie(), ja(), Mc(), Nd(), oa(), Oc(), Od() (+7 more)

### Community 112 - "LogWatcher"
Cohesion: 0.05
...
### Community 113 - "CEODispatcher"
Cohesion: 0.06
Nodes (38): CEODispatcher, _complexity_rank(), _decompose_into_subtasks(), _offload(), Any, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default… (+30 more)
...
### Community 118 - "test_colibri_provider.py"
Cohesion: 0.13
...
### Community 119 - "test_integration_c4_c5_c6_d3.py"
Cohesion: 0.06
Nodes (38): get_current_trace_id(), get_tracer(), langfuse_metadata_with_trace(), _NoOpSpan, _NoOpTracer, otel_middleware_factory(), otel_status_error(), otel_status_ok() (+30 more)

### Community 120 - "BrowserSession"
Cohesion: 0.05
Nodes (29): BrowserAction, BrowserSession, _not_started(), PageState, Any, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, True if browser automation is enabled and Playwright is importable., Which backend a start() would use: 'browserbase', 'local', or 'stub'. (+21 more)

### Community 121 - "tasks/api.py"
Cohesion: 0.16
...
### Community 124 - "WorkflowEngine"
Cohesion: 0.07
Nodes (54): approve(), build(), cancel(), _engine(), get_agent_team(), get_artifact_content(), get_events(), get_run() (+46 more)

### Community 125 - "TokenBudget"
Cohesion: 0.05
...
### Community 128 - "OnboardingScreen.jsx"
Cohesion: 0.04
Nodes (45): createCompany(), getCompany(), getOnboardingProgress(), listSpecialists(), scanRepo(), scanWebsite(), setGithubToken(), startOnboarding() (+37 more)

### Community 129 - "get_runtime_manager"
Cohesion: 0.05
...
### Community 131 - "E2BAdapter"
Cohesion: 0.03
Nodes (100): E2BAdapter, Any, runtimes/adapters/e2b.py — E2B Firecracker micro-VM runtime adapter. Routes…, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, Available iff config resolves AND the SDK is importable. Never raises — a… (+92 more)
...
### Community 133 - "test_autonomous_agency_e2e.py"
Cohesion: 0.05
Nodes (27): AutonomyCounter, AutonomySnapshot, AutonomyTracker, get_tracker(), Any, agent/kpi.py — Autonomy KPIs: evidence capture and metrics tracking. Tracks key…, Return a point-in-time snapshot of all KPIs., Reset all counters (test helper). (+19 more)
...
### Community 135 - "persist_plan_spec"
Cohesion: 0.06
...
Cohesion: 0.03
Nodes (41): Company, The core company entity - root of the Company Graph., Find a repository by its URL., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, Get a company by ID. Args: company_id: Company ID Returns: Company instance or… (+33 more)

### Community 137 - "Killer TODO Roadmap — local-llm-server"
Cohesion: 0.18
...
### Community 138 - "test_context_rulebook.py"
Cohesion: 0.06
...
### Community 140 - "test_llm_router_e2e.py"
Cohesion: 0.10
Nodes (50): _ok(), parametrize, End-to-end routing against mock providers (ADR-008). These are the tests that…, A router wired to three mock providers, with all singletons isolated., Two keys on alpha means a 429 costs a key, not the provider., The NVIDIA 410 incident, as a regression test (CLAUDE.md §7)., A 422 is the request's fault — trying five providers just adds latency., A 413 is one provider's context window, not a fact about the request. The… (+42 more)

### Community 141 - "run"
Cohesion: 0.17
Nodes (18): CreateIssue, build_body(), _failures(), find_tracking_issue(), _gh_ops(), create_issue(), list_issues(), update_issue() (+10 more)

### Community 142 - "KeyStore"
Cohesion: 0.08
Nodes (32): Backward-compatibility shim — use scripts/generate_api_key.py instead. This…, _check_rate_limit(), default_keys_path(), issue_new_api_key(), KeyRecord, KeyStore, load_key_store(), Exception (+24 more)
...
### Community 145 - "test_trend_watcher.py"
Cohesion: 0.06
...
### Community 146 - "WorkspaceManager"
Cohesion: 0.06
...
### Community 148 - "BackgroundAgent"
Cohesion: 0.07
Nodes (33): BackgroundAgent, heartbeat(), BackgroundTask, _now(), Any, agent/background.py — Background Agent An always-on worker thread that…, Enqueue *task* for processing. Returns the task (with task_id set)., Convenience: create a task and submit it in one call. (+25 more)

### Community 149 - "test_trend_scoping.py"
Cohesion: 0.10
Nodes (44): _company_attr(), company_stack_tags(), extract_stack_tags(), fan_out_trend(), fan_out_trends(), is_code_change_trend(), map_trend_to_company_task(), Any (+36 more)

### Community 150 - "scrub"
Cohesion: 0.07
Nodes (26): _luhn_ok(), Any, packages/governance/audit.py — the evidence trail for every governed action.…, True when ``digits`` (bare, no separators) satisfies the Luhn checksum., Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Recursively strip secrets from an arbitrary argument structure. Fails…, Redact secret-shaped substrings, then truncate., Return the most recent events, newest first, optionally filtered. (+18 more)

### Community 151 - "test_brain_priority_scanner.py"
Cohesion: 0.09
...
Cohesion: 0.06
Nodes (42): AgentRole, agents/swarm.py — AgentSwarm: routes workflow phases to the correct agent. The…, Run a pre-gate or report phase through the correct agent. Enforces permission…, Execute a slice via the Coder agent (write-permitted)., Review a slice via the Reviewer agent (different model from Coder). This is the…, Run verification commands via the Verifier agent. The Verifier is execution-…, Return a copy of *routing* with the profile's model injected for its role., When coder == reviewer model, swarm should log a warning. (+34 more)

### Community 154 - "TestHarnessAdapter"
Cohesion: 0.05
...
### Community 155 - "ProvidersScreen.jsx"
Cohesion: 0.06
Nodes (32): createProvider(), deleteProvider(), getBrainConfig(), getBrainProviders(), getLocalBrainState(), getProviderPolicy(), patchBrainConfig(), postLocalBrainToggle() (+24 more)

### Community 156 - "diagnostics.py"
Cohesion: 0.06
...
### Community 159 - "Security Analysis — local-llm-server"
Cohesion: 0.06
...
### Community 161 - "KnowledgeItem"
Cohesion: 0.07
Nodes (13): KnowledgeItem, Any, field_validator, Structured knowledge about the company, systems, or processes., Coerce unrecognised system_type values to 'custom' so the model never crashes…, Add a knowledge item to a company. Args: company_id: Company ID title:…, Get a knowledge item by ID., Search knowledge items. (+5 more)

### Community 162 - "test_response_cache.py"
Cohesion: 0.12
...
### Community 164 - "RenderFinding"
Cohesion: 0.08
Nodes (26): _run(), _latest_metric_value(), _note_recurrence(), _parse_timestamp(), datetime, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Pull the most recent numeric sample out of a ``get_metrics`` payload. Render…, Run one full scan and return every finding, filed or not. Never raises: a… (+18 more)

### Community 165 - "test_issue_intake.py"
Cohesion: 0.08
Nodes (46): _capability_tags(), create_task_from_oldest_open_issue(), intake_issue(), _issue_labels(), issue_source_id(), map_issue_to_task(), Any, tasks/issue_intake.py — Auto issue → Task intake (Autonomy Charter G3) Turns… (+38 more)
...
### Community 167 - "QuickNoteQueue"
Cohesion: 0.05
Nodes (41): _fetch_text(), __init__(), _now(), process_note(), Any, Path, QuickNote, QuickNoteQueue (+33 more)

### Community 168 - "ProbeResult"
Cohesion: 0.05
Nodes (54): provider_api_key(), Return the live API key for *provider* (env-only — never persisted)., _describe_http_status(), probe_model_liveness(), _probe_ollama(), _probe_openai_compat(), ProbeResult, BaseModel (+46 more)

### Community 169 - "AgentRunner"
Cohesion: 0.02
Nodes (111): AgentPhaseError, AgentRunner, _run(), Any, Exception, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a… (+103 more)

### Community 170 - "PortfolioManager"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (26): PostProcessHook, get_chat_history(), Return the module-level ChatHistoryStore singleton., create_streaming_reconstructor(), DeltaChunk, Any, Register a post-processing hook (runs before re-streaming)., Remove a post-processing hook. (+18 more)

### Community 174 - "ADR-008: LLMRouter — the single multi-provider routing gateway"
Cohesion: 0.20
...
### Community 175 - "Implementation Plan"
Cohesion: 0.11
...
### Community 176 - "provider_max_rpm"
Cohesion: 0.11
...
Cohesion: 0.06
Nodes (21): AdminAuthManager, AdminIdentity, AdminSession, AdminSessionStore, _is_truthy(), admin_auth.py — auto-generated module docstring (user-research skill scan)., WindowsCredentialAuthenticator, admin_update_user() (+13 more)

### Community 178 - "ContextWindowManager"
Cohesion: 0.08
...
### Community 179 - "WorkflowBuildRequest"
Cohesion: 0.09
...
### Community 181 - "test_e2b_data_flow.py"
Cohesion: 0.06
...
### Community 182 - "tasks/service.py"
Cohesion: 0.04
Nodes (68): Background dispatcher for task execution., ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest, ExecutionApprovalRequest, ExecutionLogEntry (+60 more)

### Community 183 - "FakeResp"
Cohesion: 0.13
...
### Community 184 - "RepowiseIntelligence"
Cohesion: 0.06
Nodes (28): Any, Path, Build symbol-level dependency graph for Python files., Build git intelligence: hotspots, ownership, co-change pairs., Run a git command and return stdout as string., Compute cyclomatic complexity for Python files. Returns 0 for non-Python files…, Extract docstrings and store as documentation., Get the latest commit hash. (+20 more)

### Community 185 - "test_verification_strategies.py"
Cohesion: 0.09
...
### Community 186 - "seo_api.py"
Cohesion: 0.07
Nodes (50): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), list_seo_audits(), plan_seo_sprint(), BaseModel (+42 more)

### Community 187 - "test_pr923_fixes.py"
Cohesion: 0.07
...
### Community 190 - "TestPoliciesGovernanceStableClaim"
Cohesion: 0.20
...
### Community 193 - "TaskBoardScreen.jsx"
Cohesion: 0.06
Nodes (32): addTaskComment(), approveTaskCheckpoint(), approveTaskExecution(), clarifyTask(), createSprint(), escalateTask(), fetchSprints(), fetchVelocity() (+24 more)

### Community 194 - "FeatureMaturity"
Cohesion: 0.07
...
### Community 195 - "BrainFailoverExhausted"
Cohesion: 0.06
...
### Community 200 - "get_store"
Cohesion: 0.08
Nodes (34): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled(), onboarding_gate_enabled_cached() (+26 more)

### Community 201 - "InternalAgentAdapter"
Cohesion: 0.06
Nodes (28): _best_cloud_primary_base(), InternalAgentAdapter, _nvidia_provider_chain(), Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Execute a TaskSpec using the internal AgentRunner and convert the agent's…, Create an isolated execution context for a single task. Tries ``git worktree… (+20 more)
...
### Community 203 - "report_to_markdown"
Cohesion: 0.07
Nodes (33): _build_curl_cffi_fetcher(), get(), get_text(), head(), _sess(), _build_pdf(), findings_block(), hr() (+25 more)

### Community 204 - "PromptCacheManager"
Cohesion: 0.06
...
### Community 206 - "LLMResponse"
Cohesion: 0.05
Nodes (44): packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, ABC, Any, AsyncClient, packages/llm/providers/base.py — the provider contract and the generic adapter.…, Parse ``Retry-After`` (and common vendor variants) into seconds. (+36 more)

### Community 207 - "test_video_transcript.py"
Cohesion: 0.05
...
### Community 208 - "Agent: Reviewer (Verifier)"
Cohesion: 0.20
...
### Community 209 - "mcp_client.py"
Cohesion: 0.06
Nodes (23): filter_safe_tools(), get_tool_annotations(), agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Typed representation of MCP tool annotations (spec 2025-11-05 §5.6.1). All…, Return True only when the tool is definitively read-only and non-destructive.…, Extract ``ToolAnnotations`` for a named tool from a ``list_tools()`` result.…, Return tools where ``readOnlyHint`` is True and ``destructiveHint`` is not…, ToolAnnotations (+15 more)
...
### Community 213 - "compare_runtimes.py"
Cohesion: 0.06
...
### Community 214 - "test_all_providers_discovery.py"
Cohesion: 0.17
...
### Community 215 - "TaskStatus"
Cohesion: 0.04
Nodes (69): Helpers that turn scheduler and playbook activity into real tasks., Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, TaskStatus, Record the human decision on a task's **pre-execution** approval gate. A…, Re-run a task. Permissive across every non-active state so the dashboard Retry…, Add a follow-up instruction to a task and re-queue it for execution. Unlike…, Owns lifecycle transitions, comment semantics, and approval rules. (+61 more)

### Community 216 - "settings.py"
Cohesion: 0.05
Nodes (30): CerebrasProvider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, OllamaProvider, Ollama provider adapter — local LLM inference., packages.ai — provider abstraction, model registry, and failover manager. (+22 more)

### Community 217 - "ApprovalRequest"
Cohesion: 0.09
Nodes (16): Event, publish(), packages/events/bus.py — In-process event bus. Loosely couples components via…, An event published on the bus., Subscribe to an event type., Publish an event to all subscribers., subscribe(), ApprovalRequest (+8 more)

### Community 218 - "test_features_api.py"
Cohesion: 0.05
...
### Community 220 - "2. Pending ⬜ — detailed implementation specs"
Cohesion: 0.17
...
### Community 221 - "anthropic_compat.py"
Cohesion: 0.09
...
### Community 222 - "GoalRecord"
Cohesion: 0.07
Nodes (29): Attempt, _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One delegation of one subtask to one tier. (+21 more)

### Community 223 - "AnthropicProvider"
Cohesion: 0.04
Nodes (44): AnthropicProvider, Any, AsyncClient, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Return the cache_control block appropriate for this provider's TTL setting.…, Add a rolling cache_control breakpoint to the most-recent stable message. In a… (+36 more)

### Community 224 - "test_schedule_backlog_drain.py"
Cohesion: 0.09
...
### Community 225 - "test_user_research_skill.py"
Cohesion: 0.04
...
### Community 226 - "models/seo_audit.py"
Cohesion: 0.09
Nodes (24): Run the repo-aware auto-fixer against this company's workspace checkout.…, run_seo_fixes(), BaseModel, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, Snapshot of one crawled page with the on-page facts the checks used., Request to remediate auto-fixable findings in a local code repository., One concrete remediation performed (or proposed) by the fixer., Result of a fixer run. (+16 more)

### Community 227 - "skill_bindings.py"
Cohesion: 0.09
...
### Community 228 - "Persistent Memory System"
Cohesion: 0.05
...
### Community 231 - "chat_handlers.py"
Cohesion: 0.11
Nodes (36): _apply_chat_defaults(), _emit_safely(), _extract_exact_output(), _filter_fragment(), _filter_openai_sse_line(), handle_ollama_native_chat(), handle_openai_chat_completions(), _single_exact_stream() (+28 more)

### Community 232 - "test_colibri_brain_shim.py"
Cohesion: 0.07
Nodes (34): get_brain_preference(), Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, clean_env(), parametrize, Regression test for the colibri env-shim in brain_policy.resolve_active_brain.…, `rstrip('/')` + re-append `/v1` must be idempotent across URL variants., Default to glm-5.2 when COLIBRI_MODEL/AGENT_LLM_MODEL both unset., AGENT_LLM_MODEL override wins when COLIBRI_MODEL is unset. (+26 more)

### Community 233 - "467 Brutal Audit — File-by-File Status"
Cohesion: 0.12
...
### Community 234 - "Platform Guide — the full tour"
Cohesion: 0.11
...
### Community 241 - "llm/router.py"
Cohesion: 0.02
Nodes (123): _load(), main(), Scheduled portfolio intelligence sweep. Run by `.github/workflows/portfolio-…, int, openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on… (+115 more)

### Community 242 - "TestRecordUsageAndStats"
Cohesion: 0.05
...
### Community 243 - "test_schedule_growth_invariants.py"
Cohesion: 0.04
...
### Community 244 - "test_direct_chat_async.py"
Cohesion: 0.09
...
### Community 245 - "LlmProviderConfig"
Cohesion: 0.13
...
### Community 246 - "Kept Rules — the 44 that survive the audit"
Cohesion: 0.05
...
### Community 247 - "test_slop_gate.py"
Cohesion: 0.07
Nodes (44): _extract_mentioned_paths(), Autonomous agent: fetch oldest open issue, generate implementation, create PR.…, Extract plausible file paths from issue text., Read existing files for codebase context (max 8000 chars total)., _read_grounding_files(), diff_is_sloppy(), is_destructive_overwrite(), is_doc_only_boilerplate() (+36 more)

### Community 248 - "TestEstimateTokensForMessages"
Cohesion: 0.05
...
### Community 252 - "test_local_controller.py"
Cohesion: 0.14
Nodes (14): _env_defaults(), _fake_subprocess_run(), _import_controller(), tests/test_local_controller.py — unit tests for the local GLM-5.2 daemon. These…, The diag output must surface binary/model errors clearly., Regression: the machine-id file default was a bare Windows literal…, Yield a fake subprocess.run that returns the next side_effect., Pin daemon env so test reproducibility doesn't fight the operator. (+6 more)
...
### Community 255 - "PatternConsolidation"
Cohesion: 0.10
...
### Community 257 - "_autonomy_bg_cycle"
Cohesion: 0.09
Nodes (27): _autonomy_bg_cycle(), Background CEO cycle + task dispatch. Runs fire-and-forget., Return the caller's GitHub token from EITHER place it can be stored. A token…, _resolve_user_github_token(), _FakeCollection, _FakeDB, patch_db(), _apply() (+19 more)

### Community 258 - ".on_task_complete"
Cohesion: 0.11
Nodes (11): _send(), _send(), Any, Callback for BackgroundAgent.on_task_complete. Dispatches task result…, Send notification to configured Telegram chat IDs., Proactively push a Telegram approval-gate message with inline buttons. Sent…, Send a Telegram message with an inline keyboard to all configured chats., Dispatch the daily review digest to every authorized chat_id. Mirrors… (+3 more)

### Community 259 - "clear_cooldowns"
Cohesion: 0.05
Nodes (47): clear_cooldowns(), get_dead_models(), _is_model_dead(), is_provider_on_cooldown(), mark_provider_failed(), Snapshot of active dead-model entries {provider_id/model: expiry_ts}. Also…, Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown. (+39 more)

### Community 260 - "system_instruction"
Cohesion: 0.09
...
### Community 262 - "TestCatalogFable51"
Cohesion: 0.09
...
### Community 265 - "AgentSwarm"
Cohesion: 0.07
Nodes (33): agents/__init__.py — CRISPY multi-agent coding system., AgentProfile, _catalog_defaults(), _catalog_provider(), _get_defaults(), load_all_profiles(), make_architect_profile(), make_coder_profile() (+25 more)

### Community 266 - "test_backend_server_features.py"
Cohesion: 0.05
Nodes (27): _agent_provider_failure_response(), _append_agent_session_message(), _run_agent_job(), _is_index_options_conflict(), _mask_observations(), _persist_agent_chat_response(), _persist_chat_session(), Exception (+19 more)

### Community 267 - "test_provider_render_env.py"
Cohesion: 0.08
...
### Community 270 - "test_governance_api.py"
Cohesion: 0.08
...
### Community 271 - "test_llm_router_disabled.py"
Cohesion: 0.09
Nodes (41): auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable., Persist a provider as disabled, through the store that already owns it. (+33 more)

### Community 272 - "RuntimeCapabilityRegistry"
Cohesion: 0.04
Nodes (27): Any, Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking)., Return all registered runtimes with their cached health status., Return the active routing policy as a plain dict. (+19 more)

### Community 273 - "test_implement_agent_routing.py"
Cohesion: 0.07
...
### Community 274 - "emit_chat_observation"
Cohesion: 0.09
...
### Community 280 - "NotificationDispatcher"
Cohesion: 0.07
Nodes (32): NotificationDispatcher, Path, service_manager.py — Telegram & Notification Integration Extension Extends the…, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess)., Run the bot with stop-event awareness., Routes background task results to configured notification channels. Currently… (+24 more)

### Community 281 - "test_control_plane_api.py"
Cohesion: 0.06
...
Cohesion: 0.05
Nodes (49): Memory middleware for automatic context injection into AI tool requests. This…, MemoryCategory, MemoryEntry, MemoryScope, Enum, Row, str, Enhanced persistent memory system with auto-loading across AI coding tools.… (+41 more)

### Community 283 - "test_executive_advisory.py"
Cohesion: 0.10
Nodes (29): _compose_context(), ExecutiveAdvisory, _format_company_context(), Grounding, Evidence gathered for a question before the executives answer., Routes a business question to the right executives and synthesises them., Assemble the evidence block handed to each executive., Coerce supplied company context to a compact string, or empty. (+21 more)

### Community 284 - "SecurityScanner"
Cohesion: 0.11
...
### Community 286 - "test_mcp_governance.py"
Cohesion: 0.09
...
### Community 287 - "test_brain_availability_doctor.py"
Cohesion: 0.10
...
### Community 288 - "test_ceo_router.py"
Cohesion: 0.13
...
### Community 290 - "test_nvidia_model_discovery.py"
Cohesion: 0.06
...
### Community 291 - "test_purge_backlog.py"
Cohesion: 0.07
...
### Community 292 - "resolve_provider_for"
Cohesion: 0.06
...
### Community 293 - "test_connector_registry.py"
Cohesion: 0.09
...
### Community 294 - "Runbook: Auto-Resume After Cooldown / Interruption"
Cohesion: 0.14
...
### Community 297 - "NIMConnectionPool"
Cohesion: 0.09
Nodes (16): NIMConnectionPool, Any, AsyncClient, Persistent httpx.AsyncClient pool with circuit breaker and retry logic. Manages…, Get or create the shared httpx.AsyncClient., Context manager for a pooled client session., Close the connection pool., Make an HTTP request through the pool with circuit breaker protection. Args:… (+8 more)
...
### Community 300 - "test_provider_enable_disable.py"
Cohesion: 0.07
...
### Community 301 - "test_render_mcp.py"
Cohesion: 0.08
Nodes (24): _as_list(), _coerce_payload(), RuntimeError, packages/integrations/render_mcp.py — Render platform access over MCP. The…, Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return…, Drop the cached singleton. Used by tests and after a config change., Raised when a mutating Render tool is called with writes disabled. (+16 more)

### Community 302 - "test_sam_voice.py"
Cohesion: 0.06
Nodes (34): get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, asyncio, tests/test_sam_voice.py — Integration tests for SAM voice agent. Tests the SAM…, Same session_id must return the same session., SAM's system prompt must address the user as Commander., SAM's system prompt must instruct concise responses., _build_context must return a dict with expected keys. (+26 more)

### Community 303 - "ContextCompressor"
Cohesion: 0.11
...
### Community 305 - "ResourceWatchdog"
Cohesion: 0.10
...
### Community 306 - "test_daily_2026_06_04.py"
Cohesion: 0.10
...
### Community 308 - "REWRITE_PLAN.md — Phased Migration Strategy"
Cohesion: 0.06
...
### Community 309 - "test_kimi_bridge_server.py"
Cohesion: 0.05
...
### Community 311 - "agents/agile_ceremonies.py"
Cohesion: 0.08
Nodes (27): _bullets(), generate_backlog_retro(), generate_sprint_retro(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive retro notes for ``sprint`` from its current metrics. Records…, Derive a retrospective from the task tracker when no sprint is active. DONE /… (+19 more)

### Community 312 - "openclaw_gateway.py"
Cohesion: 0.09
...
### Community 313 - "Langfuse Observability Guide"
Cohesion: 0.06
...
### Community 317 - "test_telegram_mutating_commands.py"
Cohesion: 0.09
...
### Community 318 - "test_workspace_isolation.py"
Cohesion: 0.16
...
### Community 319 - "executive_advisory.py"
Cohesion: 0.18
Nodes (9): AdvisoryMemory, _keywords(), _mem_key(), agent/executive_advisory.py — an intelligent C-suite business-advisory layer.…, The C-suite's durable memory of past recommendations. A thin adapter over…, Distinct significant words (len ≥ 5), order-preserved, for LIKE recall., test_advisory_memory_is_fail_soft_on_broken_store(), test_advisory_memory_roundtrips_via_store() (+1 more)

### Community 320 - "UserMemoryStore"
Cohesion: 0.10
Nodes (12): Return a previously saved memory value, or an empty string if absent., Persist a key/value pair to the user's profile store., Connection, Path, Per-user key/value memory store backed by SQLite. Allows agents to persist and…, Return all stored key/value pairs for *user_id*., Delete a memory entry. Returns ``True`` if a row was removed., Persistent key/value store scoped per user. Thread-safe; uses a single SQLite… (+4 more)

### Community 321 - "Render MCP — autonomous platform debugging and environment monitoring"
Cohesion: 0.14
...
### Community 324 - "High-Agency Frontend Skill"
Cohesion: 0.06
Nodes (30): 10. FINAL PRE-FLIGHT CHECK, 1. ACTIVE BASELINE CONFIGURATION, 2. DEFAULT ARCHITECTURE & CONVENTIONS, 3. DESIGN ENGINEERING DIRECTIVES (Bias Correction), 4. CREATIVE PROACTIVITY (Anti-Slop Implementation), 5. PERFORMANCE GUARDRAILS, 6. TECHNICAL REFERENCE (Dial Definitions), 7. AI TELLS (Forbidden Patterns) (+22 more)

### Community 325 - "model_discovery.py"
Cohesion: 0.09
Nodes (25): cached_models(), dead_models(), discover_models(), _fresh_entry(), _models_url(), _parse_ids(), Any, Ask a provider which models the configured API key may actually use.… (+17 more)

### Community 326 - "Quick-Note GitHub Issues Processing - Session Summary"
Cohesion: 0.06
...
### Community 331 - "session_retro.py"
Cohesion: 0.13
...
### Community 332 - "test_agency_fix.py"
Cohesion: 0.06
...
### Community 335 - "CommitTracker"
Cohesion: 0.14
Nodes (22): CommitAttribution, CommitTracker, Any, Path, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata…, Return recent commits with agent attribution trailers parsed out., Create git commits enriched with agent-session attribution trailers. Usage::…, Return ``--trailer`` arguments ready to append to a ``git commit`` call. (+14 more)

### Community 336 - "test_phase6_workflow.py"
Cohesion: 0.13
...
### Community 338 - "build_render_router"
Cohesion: 0.13
...
### Community 342 - "ProviderRouter"
Cohesion: 0.05
Nodes (32): _exponential_backoff_cooldown(), _notify_watchdog(), ProviderRouter, Any, Try all models for one provider. Returns ProviderResult on success, None on…, Notify the brain watchdog of a provider call outcome (fire-and-forget).…, Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either…, Translate an OpenAI chat/completions payload to Bedrock Converse format. (+24 more)

### Community 343 - "local_controller.py"
Cohesion: 0.11
...
### Community 346 - "OrchestratorCheckpointStore"
Cohesion: 0.13
...
### Community 347 - "test_force_cleanup_conditional_delete.py"
Cohesion: 0.10
...
### Community 350 - "test_executive_advisory_api.py"
Cohesion: 0.11
...
### Community 351 - "executive_advisory_api.py"
Cohesion: 0.14
Nodes (21): _company_advisory_context(), Ask the C-suite advisory layer a business question. This is the CEO's bridge…, Pull a compact company profile for the C-suite advisory, or None. Best-effort:…, get_executive_advisory(), Return the shared :class:`ExecutiveAdvisory`, built on first use., AdvisoryConsultRequest, AdvisoryResponse, build_executive_advisory_router() (+13 more)

### Community 352 - "test_lessons.py"
Cohesion: 0.23
...
### Community 354 - "test_rag_context.py"
Cohesion: 0.12
...
### Community 355 - "ProjectScaffolder"
Cohesion: 0.13
...
### Community 356 - "README.md"
Cohesion: 0.09
Nodes (10): Architecture and operations, Documentation map, Repo hygiene, Screenshots and README sync, Start here, Specialist × Skill Matrix, A sample of what the agents shipped (all merged, all real), The numbers (verifiable via the GitHub API) (+2 more)

### Community 357 - "Initiative"
Cohesion: 0.09
...
### Community 358 - "Part A — CodeRabbit review fixes for this PR (do first, small)"
Cohesion: 0.05
...
### Community 360 - "SteeringInjector"
Cohesion: 0.11
...
### Community 362 - "test_claude_setup_audit.py"
Cohesion: 0.16
...
### Community 365 - "test_internal_agent_did_work.py"
Cohesion: 0.12
...
### Community 366 - "_captured_request_headers"
Cohesion: 0.12
...
### Community 367 - "TestDiagCommand"
Cohesion: 0.08
Nodes (13): TestCase, _GlobalsRestorer, tests/test_telegram_diag.py Regression test for the new ``/diag`` (admin)…, Drive _process_update with a /diag message and return the response. Restores…, The Operator Charter §"Telegram bot" silent-drop path MUST surface a…, Once we've warned once, subsequent silent drops must NOT spam the log., Snapshot/restore tb globals + TELEGRAM_POLLER_DISABLED env var., ``/diag`` behaviour under admin + non-admin + empty-allowlist states. (+5 more)

### Community 368 - "test_llm_router_queue_cache.py"
Cohesion: 0.03
Nodes (104): Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any (+96 more)

### Community 369 - "ScheduleStore"
Cohesion: 0.12
...
### Community 371 - "Python Dependencies (`requirements.txt`)"
Cohesion: 0.07
...
### Community 373 - "KV Cache Internals"
Cohesion: 0.22
...
### Community 376 - "ProviderConsole.jsx"
Cohesion: 0.08
...
### Community 380 - "test_regression.py"
Cohesion: 0.10
...
### Community 381 - "test_agent_scripts_share_one_model_list.py"
Cohesion: 0.10
...
### Community 382 - "test_crispy_burn_in.py"
Cohesion: 0.07
...
Cohesion: 0.18
Nodes (21): _bootstrap(), Path, tests/test_webui_provider_priority.py — Priority + reorder + brain-policy…, The /policy/brain endpoint must return the resolved brain + the paid policy…, The /providers/role-tags endpoint surfaces brain/sub/fallback roles consistent…, Reset the brain_config + brain_policy singletons before each test. V2.0 Phase 2…, _reset_brain_singletons(), test_admin_policy_brain_returns_resolution_and_paid_state() (+13 more)

### Community 385 - "CompanyAgencyService"
Cohesion: 0.09
...
### Community 387 - "workflow_orchestrator.py"
Cohesion: 0.08
Nodes (47): BoundContext, ClassifyOutput, ExecutionResult, JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput, Phase (+39 more)
...
### Community 389 - "get_scheduler"
Cohesion: 0.11
Nodes (28): autonomy_tick(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_trigger(), Execute ONE pending task synchronously. Called by the cron workflow every 2…, get_scheduler(), create_schedule(), delete_schedule() (+20 more)

### Community 390 - "Claude Code + Qwen Local Setup"
Cohesion: 0.07
...
### Community 391 - "Configuration Reference"
Cohesion: 0.06
...
### Community 392 - "Docker Agent Runtimes Setup"
Cohesion: 0.05
...
### Community 394 - "isolated_telegram_config"
Cohesion: 0.10
...
### Community 397 - "register_webui"
Cohesion: 0.05
Nodes (55): allow_paid_brain(), invalidate_brain_cache(), Clear the cached brain so the next read re-resolves. Called from…, True only when the operator explicitly opted into a paid (Anthropic) brain.…, When BrainConfig.updated_at is empty (never applied), the resolver falls…, test_resolve_active_brain_falls_through_when_db_unset(), Any, Path (+47 more)

### Community 398 - "VoiceCommandInterface"
Cohesion: 0.14
...
### Community 399 - "Performance Analysis — local-llm-server"
Cohesion: 0.08
...
### Community 400 - "Technical Debt Register — local-llm-server"
Cohesion: 0.07
...
### Community 401 - "check_changelog_parity.py"
Cohesion: 0.43
...
### Community 402 - "test_agent_chat_integration.py"
Cohesion: 0.13
...
### Community 404 - "_execute_skill_impl"
Cohesion: 0.10
Nodes (24): _execute_skill_impl(), _get_portfolio_manager(), Any, Live Graphify executor — queries the codebase knowledge graph. Order of…, Live council reviewer — deterministic, rules-based multi-perspective review…, Recommend skills based on detected systems and provisioned specialists. With no…, Execute a skill by ID and update its execution counters., Lazily build and cache the shared PortfolioManager. (+16 more)

### Community 405 - "TemporalContextGraph"
Cohesion: 0.10
Nodes (15): demo_agent_tracking(), datetime, Temporal context graph inspired by Graphiti…, Get history of an entity between two times, Get current state of an entity (most recent fact), Query facts with pattern matching, Get source (provenance) of a specific fact, A fact at a specific point in time (+7 more)

### Community 406 - "test_daily_automation_2026_08_03.py"
Cohesion: 0.11
...
### Community 409 - "get_tool_registry"
Cohesion: 0.13
Nodes (13): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, auto_register(), Register the four user-research tools with a ``ToolRegistry``. Each tool wraps…, Register the user research tools into the module-level singleton registry.…, register_user_research_tools(), user_research_plan_tool(), user_research_qual_tool() (+5 more)

### Community 410 - "tests/conftest.py"
Cohesion: 0.06
Nodes (30): Item, app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client() (+22 more)

### Community 411 - "model_router.py"
Cohesion: 0.06
...
### Community 412 - "Skill: user-research"
Cohesion: 0.13
...
### Community 413 - "test_keepalive.py"
Cohesion: 0.18
...
### Community 415 - "r"
Cohesion: 0.17
Nodes (19): _a(), cb(), dc(), Fo(), ga(), gb(), hb(), ib() (+11 more)

### Community 416 - "db/__init__.py"
Cohesion: 0.14
...
### Community 417 - "Session Handoff — 2026-06-15"
Cohesion: 0.08
...
### Community 418 - "Telegram Bot Setup"
Cohesion: 0.08
...
### Community 419 - "autonomous_fix.py"
Cohesion: 0.08
...
### Community 420 - "video_transcript.py"
Cohesion: 0.11
...
### Community 423 - "test_memory_guard.py"
Cohesion: 0.12
...
### Community 424 - "test_telegram_approval_e2e.py"
Cohesion: 0.12
...
### Community 425 - "test_activation_api.py"
Cohesion: 0.13
...
### Community 426 - "test_agent_free_brain.py"
Cohesion: 0.05
Nodes (35): livenim, is_anthropic_model(), True when *model* names a paid Anthropic/Bedrock-Claude model. Covers native…, Resolve the free NVIDIA NIM brain from env, or ``None`` if unconfigured.…, resolve_free_nvidia_brain(), _FakeAsyncClient, _FakeResponse, _free_env() (+27 more)

### Community 427 - "TestClassifyPlainText"
Cohesion: 0.08
...
### Community 431 - "TestStreamableHTTPTransport"
Cohesion: 0.12
...
### Community 432 - "register_admin_gui"
Cohesion: 0.18
Nodes (19): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), dashboard(), diag_langfuse(), _guest_redirect(), login_page(), login_submit() (+11 more)

### Community 433 - "github_tools.py"
Cohesion: 0.20
Nodes (23): get_repo(), _get_token(), _get_user(), init_workspace(), list_branches(), list_prs(), list_repos(), BaseModel (+15 more)

### Community 434 - "._prune"
Cohesion: 0.22
...
### Community 435 - "test_dashboard_cache.py"
Cohesion: 0.11
Nodes (22): _cached(), cost_attribution_stats(), _fast_count(), get_stats(), legacy_scheduler_list(), _produce_scheduler_jobs(), _produce_stats(), Single-flight TTL cache. Concurrent callers wait for the first producer. (+14 more)

### Community 436 - "AdminScreen.jsx"
Cohesion: 0.11
...
### Community 437 - "Local AI Stack with Docker"
Cohesion: 0.08
...
### Community 438 - "TASK 4 — End-to-end approval-gate test"
Cohesion: 0.08
...
### Community 441 - "WindowsServiceManager"
Cohesion: 0.17
...
### Community 442 - "test_all_features.py"
Cohesion: 0.08
...
### Community 449 - "Implementation Prompt: Rich TaskBoard + Agile Sprint Integration"
Cohesion: 0.08
...
### Community 450 - "LLM Router — troubleshooting"
Cohesion: 0.08
...
### Community 452 - "implement_agent.py"
Cohesion: 0.12
...
### Community 453 - "PrioritizedTask"
Cohesion: 0.10
Nodes (12): IntEnum, Queue, PrioritizedTask, Priority, Any, Submit a task to the queue. Returns True if accepted, False if rejected due to…, Return queue introspection data for status endpoints., Subscribe to progress events for a specific task. Returns an asyncio.Queue that… (+4 more)

### Community 454 - "test_phase4_runtime_resilience.py"
Cohesion: 0.14
...
### Community 455 - "PreflightReport"
Cohesion: 0.08
Nodes (26): PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, clean_store(), _clear_overrides(), _fake_user(), Tests for direct-chat evolution: intent routing, sticky context, humanized… (+18 more)

### Community 456 - "daily_digest.py"
Cohesion: 0.22
...
### Community 460 - "test_cerebras_catalogue.py"
Cohesion: 0.13
...
### Community 461 - "test_mostly_failed_steps.py"
Cohesion: 0.12
...
### Community 463 - "AdaptivePermissions"
Cohesion: 0.18
...
### Community 469 - "test_schedule_persistence.py"
Cohesion: 0.15
...
### Community 471 - "test_p0_roadmap_b3_b4_b5.py"
Cohesion: 0.15
Nodes (11): _deep_merge(), get_guardrails(), Deep merge two dicts. Override values take precedence., Return the module-level GuardrailEngine singleton., CircuitBreakerOpenError, RuntimeError, Raised when a request is blocked by an open circuit breaker., get_synthetic_pipeline() (+3 more)

### Community 472 - "PriorityTaskQueue"
Cohesion: 0.14
Nodes (7): PriorityTaskQueue, Start the worker pool., Stop the worker pool gracefully., Asyncio-based priority queue with backpressure and worker pool. Features: -…, Higher-priority tasks should be processed before lower-priority ones., TestPriorityTaskQueue, handler()

### Community 473 - "test_task_source_id_race.py"
Cohesion: 0.12
...
### Community 474 - "TestLegacyRouterCacheTTL"
Cohesion: 0.13
...
### Community 475 - "test_anthropic_refusal_fallback.py"
Cohesion: 0.19
...
### Community 476 - "_redact_for_notification"
Cohesion: 0.11
...
### Community 478 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
### Community 480 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.09
...
### Community 482 - "The fifteen strategies"
Cohesion: 0.09
...
### Community 483 - "asyncio"
Cohesion: 0.09
Nodes (16): requires_db, asyncio, Test Company Graph services., Test that storage service can be initialized., Test company CRUD operations - skipped as requires specific config., Test specialist service., Test that specialist service can be initialized., Test onboarding service. (+8 more)

### Community 484 - "SeoCheckDefinition"
Cohesion: 0.12
...
### Community 486 - "DecisionsStoreTests"
Cohesion: 0.13
...
### Community 488 - "backend/server.py"
Cohesion: 0.02
Nodes (165): _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, _bootstrap_within_budget() (+157 more)

### Community 489 - "_process_task_callback"
Cohesion: 0.17
...
### Community 490 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.15
...
### Community 491 - "test_brain_migration_writes_a_live_model.py"
Cohesion: 0.10
...
### Community 492 - "test_openclaw_str_e_fix.py"
Cohesion: 0.10
...
### Community 494 - "run_trend_analysis"
Cohesion: 0.18
...
### Community 495 - "test_unit5_ui_provider_surface.py"
Cohesion: 0.10
...
### Community 496 - "brain_failover.py"
Cohesion: 0.05
Nodes (43): BrainFailoverManager, _disabled_from_mongo(), _disabled_from_sqlite(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _mongo_db(), _mongo_enabled() (+35 more)

### Community 497 - "Trajectory"
Cohesion: 0.20
...
### Community 498 - "HarnessEnrichment"
Cohesion: 0.14
...
### Community 499 - "[Unreleased]"
Cohesion: 0.05
Nodes (41): Added, Added, Added, Added, Added, Added, Added, Added (+33 more)

### Community 500 - "_extract_tech_relevance"
Cohesion: 0.17
...
### Community 501 - "Skill: modularity-review"
Cohesion: 0.10
...
### Community 502 - "Design Audit"
Cohesion: 0.10
Nodes (19): Code Quality, Color and Surfaces, Component Patterns, Content, Design Audit, Fix Priority, How This Works, Iconography (+11 more)

### Community 503 - "Findings"
Cohesion: 0.10
...
### Community 507 - "Dynamic Model Routing"
Cohesion: 0.10
...
### Community 508 - "SettingsHub.jsx"
Cohesion: 0.04
Nodes (45): deleteGithubToken(), getLoops(), getPlatformControls(), githubStatus(), listGithubRepos(), resetPlatformControl(), setPlatformControls(), { getPlatformControls, setPlatformControls, resetPlatformControl } (+37 more)

### Community 509 - "PortfolioScreen.jsx"
Cohesion: 0.11
Nodes (12): getPortfolioBoard(), refreshPortfolio(), btnStyle, HEALTH, HORIZONS, PortfolioScreen(), SOURCE, STATUS_COLOR (+4 more)

### Community 510 - "infra_cost.py"
Cohesion: 0.15
...
Cohesion: 0.14
Nodes (10): get_steering_injector(), Return recommended steering labels for a given task category. Used by the model…, Return the module-level SteeringInjector singleton., steering_for_task(), get_agent_bus(), Return the module-level AgentMessageBus singleton., get_task_queue(), Return the module-level PriorityTaskQueue singleton. (+2 more)

### Community 513 - "ClaudeCodeAdapter"
Cohesion: 0.21
...
### Community 519 - "test_langfuse_agency_wide.py"
Cohesion: 0.10
...
### Community 524 - "test_prompt_injection_boundary.py"
Cohesion: 0.16
...
### Community 526 - "Skill: fabric-patterns"
Cohesion: 0.11
...
### Community 527 - "Analysis & Synthesis Instructions"
Cohesion: 0.11
Nodes (18): 1. Define the Atmosphere, 2. Map the Color Palette, 3. Establish Typography Rules, 4. Define the Hero Section, 5. Describe Component Stylings, 6. Define Layout Principles, 7. Define Responsive Rules, 8. Encode Motion Philosophy (+10 more)

### Community 528 - "Production Readiness Assessment — local-llm-server"
Cohesion: 0.11
...
### Community 530 - "TestNormalizeResponseFormat"
Cohesion: 0.07
Nodes (17): _normalize_response_format(), Translate OpenAI ``response_format`` into Ollama's ``format`` field. For…, list_models_openai(), List available models — union of live Ollama models, router registry, and…, _get_model_map(), Merge built-in defaults with MODEL_MAP env overrides (lazy, cached)., Daily automation tests — 2026-05-14 Covers three features implemented in this…, Payload without 'model' field should apply normalization (no '/' → local). (+9 more)

### Community 531 - "Skill: fabric-patterns"
Cohesion: 0.11
Nodes (18): 1. Ensure Pattern Directory Exists, 2. List Available Patterns, 3. Retrieve a Pattern, 4. Apply a Pattern with Variables, 5. Stitch Patterns Together, 6. Create New Patterns, Acceptance Checks, Directory Structure (+10 more)

### Community 532 - "Admin Dashboard Guide"
Cohesion: 0.11
...
### Community 533 - "fmtErr"
Cohesion: 0.06
Nodes (24): approveWorkflow(), buildWorkflow(), cancelWorkflow(), consultExecutives(), fmtErr(), getWorkflowRun(), getWorkflowRuns(), listExecutives() (+16 more)

### Community 534 - "scripts/doctor.py"
Cohesion: 0.27
Nodes (17): NamedTuple, Check, check_core_deps(), check_env(), check_git(), check_mongo(), check_node(), check_ollama() (+9 more)

### Community 535 - "control_registry.py"
Cohesion: 0.12
Nodes (23): packages/config/control_catalogue.py — the 109 operator-facing controls. The…, coerce(), _coerce_choice(), _coerce_number(), _coerce_toggle(), Any, packages/config/control_registry.py — the platform-control API. The public…, Normalise *value* into the env string this control stores. Raises… (+15 more)

### Community 536 - "ai/registry.py"
Cohesion: 0.13
Nodes (18): Any, Send a chat request with automatic failover. Retry policy: 1. If ``model`` is…, all_models(), best_model_for(), get(), ModelInfo, models_by_provider(), packages/ai/registry.py — Model Registry. Centralized registry of all models… (+10 more)

### Community 537 - "Delegation Plan (agent-ready work packages)"
Cohesion: 0.11
...
### Community 538 - "RuntimeHealthService"
Cohesion: 0.10
...
### Community 540 - "classify_direct_chat_intent"
Cohesion: 0.13
...
### Community 543 - "test_autonomy_status.py"
Cohesion: 0.11
...
### Community 544 - "test_brain_patch_service_token.py"
Cohesion: 0.18
...
### Community 545 - "test_freebuff_bot.py"
Cohesion: 0.09
...
### Community 546 - "TestSelfHealingInfrastructureClassification"
Cohesion: 0.19
...
### Community 547 - "test_one_model_catalogue.py"
Cohesion: 0.08
...
### Community 549 - "validate_session_id"
Cohesion: 0.16
...
### Community 550 - "handle_workflow_ide_chat"
Cohesion: 0.17
...
### Community 551 - "_register_builtin_tools"
Cohesion: 0.09
Nodes (14): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, Register the built-in agent tools that are always available., Register the interactive-browser capability (agent/browser.py). Unlike…, Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_browser_tools(), _browse_page_tool(), _register_builtin_tools() (+6 more)

### Community 552 - "ErrorInterceptorMiddleware"
Cohesion: 0.18
Nodes (11): _dispatch_async(), _run(), ErrorInterceptorMiddleware, Any, BaseHTTPMiddleware, Exception, Request, agent/error_interceptor.py — HTTP Error Interceptor Middleware… (+3 more)

### Community 553 - "SEO / GEO / AIO Audit Engine"
Cohesion: 0.14
...
### Community 555 - "Comprehensive Skill Index (By Category)"
Cohesion: 0.11
...
### Community 556 - "Agent Skill: Principal UI/UX Architect & Motion Choreographer (Awwwards-Tier)"
Cohesion: 0.11
Nodes (17): 1. Meta Information & Core Directive, 2. THE "ABSOLUTE ZERO" DIRECTIVE (STRICT ANTI-PATTERNS), 3. THE CREATIVE VARIANCE ENGINE, 4. HAPTIC MICRO-AESTHETICS (COMPONENT MASTERY), 5. MOTION CHOREOGRAPHY (FLUID DYNAMICS), 6. PERFORMANCE GUARDRAILS, 7. EXECUTION PROTOCOL, 8. PRE-OUTPUT CHECKLIST (+9 more)

### Community 557 - "Architecture Overview — local-llm-server"
Cohesion: 0.11
...
### Community 560 - "Part A — Health Report"
Cohesion: 0.11
...
### Community 562 - "sync_readme_gallery.py"
Cohesion: 0.20
...
### Community 565 - "test_autonomy_pipeline_regressions.py"
Cohesion: 0.11
...
### Community 567 - "test_company_api.py"
Cohesion: 0.11
...
### Community 569 - "test_phase5_doctor.py"
Cohesion: 0.07
...
### Community 570 - "TestStopSlopChecker"
Cohesion: 0.11
...
### Community 574 - "SKILL: Industrial Brutalism & Tactical Telemetry UI"
Cohesion: 0.12
...
### Community 575 - "Skill: data-quality-audit"
Cohesion: 0.12
...
### Community 576 - "What "Slop" Looks Like"
Cohesion: 0.12
...
### Community 578 - "local_brain_router.py"
Cohesion: 0.19
...
### Community 579 - "Section-by-Section Acceptance Criteria"
Cohesion: 0.12
...
### Community 580 - "sam_livekit_worker.py"
Cohesion: 0.12
Nodes (23): LiveKitConfig, Resolved LiveKit + speech-provider configuration., True when the LiveKit room transport itself is usable., _build_llm(), _build_stt(), _build_tts(), entrypoint(), main() (+15 more)

### Community 581 - "_Budget"
Cohesion: 0.15
...
### Community 582 - "redact_connection_url"
Cohesion: 0.17
...
### Community 583 - "Native operations"
Cohesion: 0.09
...
### Community 584 - "agent_readiness_audit.py"
Cohesion: 0.21
...
### Community 585 - "sync_ngrok.py"
Cohesion: 0.24
Nodes (15): detect_ngrok_url(), dim(), fail(), header(), info(), main(), ok(), patch_platform_brain_via_switch_brain() (+7 more)

### Community 586 - "test_ci.sh"
Cohesion: 0.15
Nodes (16): ADMIN_EMAIL, ADMIN_PASSWORD, API_KEYS, cleanup(), DB_NAME, fail(), LANGFUSE_HOST, LANGFUSE_PUBLIC_KEY (+8 more)

### Community 587 - "build_platform_controls_router"
Cohesion: 0.15
Nodes (23): _actor(), build_platform_controls_router(), _admin(), list_controls(), reset_control(), update_controls(), APIRouter, backend/platform_controls_router.py — admin API for the platform controls.… (+15 more)

### Community 588 - "e2e/test_browser.py"
Cohesion: 0.19
...
### Community 589 - "test_frontend_deployment_guards.py"
Cohesion: 0.18
...
### Community 590 - "test_health_endpoints.py"
Cohesion: 0.15
...
### Community 591 - "test_openclaw_endpoints.py"
Cohesion: 0.12
...
### Community 592 - "_status_snapshot"
Cohesion: 0.15
...
### Community 593 - "test_skill_registry.py"
Cohesion: 0.13
...
### Community 594 - "Agent Governance Guide"
Cohesion: 0.09
...
### Community 595 - "TestRouterIntegration"
Cohesion: 0.21
...
### Community 596 - "test_north_mini_code.py"
Cohesion: 0.10
Nodes (11): north_mini_code_model_for(), Return the North Mini Code model id served by *provider*, else ``None``.…, tests/test_north_mini_code.py — North Mini Code 1.0 integration. Covers the…, The switch defaults ON so North is the default post-install., The agency/Hermes execution path defaults to North via the resolver., Only high/medium/low are honoured; anything else means 'unset'., test_flag_default_is_on(), test_internal_agent_consults_coding_resolver() (+3 more)

### Community 597 - "hermes_prompt.py"
Cohesion: 0.19
...
### Community 598 - "MemoryMiddleware"
Cohesion: 0.17
...
### Community 600 - "test_ai_insights.py"
Cohesion: 0.15
Nodes (13): AIToolMetrics, Per-tool quality metrics: which AI tools actually deliver value? Tracks…, Average response latency in ms for the given tool., Output tokens per input token (higher = more verbose responses). Useful for…, Event counts grouped by ToolKind., Tests for agents.ai_insights — AI-Assisted Engineering metrics., test_engagement_dau_counts_unique_users(), test_tool_metrics_acceptance_rate() (+5 more)

### Community 601 - "PerformanceAnalytics"
Cohesion: 0.14
...
### Community 604 - "ARCHITECTURE.md — Target Architecture"
Cohesion: 0.12
...
### Community 605 - "_valid_login_state"
Cohesion: 0.24
...
### Community 606 - "Attention Mechanisms Internals"
Cohesion: 0.13
...
### Community 607 - "Skill: repowise-intelligence"
Cohesion: 0.12
...
### Community 608 - "The 10-Step Workflow"
Cohesion: 0.12
...
### Community 609 - "Contributing to local-llm-server"
Cohesion: 0.12
...
### Community 610 - "refresh_agent_built_proof.py"
Cohesion: 0.20
...
### Community 611 - "CEO Micro-Management"
Cohesion: 0.12
...
### Community 612 - "Deploy: FreeBuff Telegram bot (24×7)"
Cohesion: 0.07
...
### Community 615 - "test_kimi_bridge_provider.py"
Cohesion: 0.20
...
### Community 616 - "TestMCPToolsListCache"
Cohesion: 0.15
...
### Community 619 - "AdminDigestRouterAuthTests"
Cohesion: 0.20
...
### Community 621 - "ManagedAgentDreams"
Cohesion: 0.22
...
### Community 622 - "_resolve_push_token"
Cohesion: 0.31
...
### Community 623 - "clear_wizard_state_cache"
Cohesion: 0.19
...
### Community 624 - "test_minimax_bug_fixes.py"
Cohesion: 0.13
...
### Community 625 - "test_agency_workflows_carry_the_failover_chain.py"
Cohesion: 0.18
...
### Community 626 - "test_voice_pipeline.py"
Cohesion: 0.13
...
### Community 627 - "DailyDigestAggregatorTests"
Cohesion: 0.19
...
### Community 628 - "test_dockerfile_ships_root_modules.py"
Cohesion: 0.17
...
### Community 630 - "test_migrate_local_brain_env.py"
Cohesion: 0.38
...
### Community 631 - "test_nvidia_default_model.py"
Cohesion: 0.17
...
### Community 632 - "Docker AI Governance Audit — Final Report"
Cohesion: 0.10
...
### Community 634 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
### Community 635 - "test_refresh_agent_built_proof.py"
Cohesion: 0.12
...
### Community 636 - "test_telegram_task_callback.py"
Cohesion: 0.12
Nodes (14): _Captured, Load telegram_bot fresh in each test, with env vars reset., telegram_bot(), _parked_task(), asyncio, Telegram inline-button callbacks for the task pre-execution gate. Regression:…, A non-admin tapping Approve must get a modal (show_alert), not silence.…, _Recorder (+6 more)

### Community 637 - "_hash_component"
Cohesion: 0.16
...
### Community 640 - "EngagementMetrics"
Cohesion: 0.15
Nodes (12): build_report(), EngagementMetrics, How many distinct tools each user has touched., Assemble the executive-summary report., Track adoption and engagement across the engineering org. DX report key…, Count distinct sessions per user. A session ends when there's a gap of more…, test_build_report_assembles_all_sections(), test_engagement_dau_zero_when_no_events() (+4 more)

### Community 641 - "CollaborationContext"
Cohesion: 0.17
...
### Community 642 - "1. Capability-by-capability"
Cohesion: 0.10
...
### Community 644 - "Skill: agent-harness"
Cohesion: 0.13
...
### Community 645 - "Skill: checkpoint-strategy"
Cohesion: 0.13
...
### Community 647 - "Skill: local-ai-query"
Cohesion: 0.13
...
### Community 648 - "Skill: parallel-agents"
Cohesion: 0.13
...
### Community 649 - "Skill: parallel-worktrees"
Cohesion: 0.13
...
### Community 650 - "Skill: repowise-intelligence"
Cohesion: 0.12
Nodes (15): 1. Graph Intelligence (Dependency Graph), 2. Git Intelligence, 3. Documentation Intelligence, 4. Decision Intelligence, Acceptance Checks, Directory Structure, Example Usage, Implementation Approach (+7 more)

### Community 651 - "Design System: Taste Standard"
Cohesion: 0.13
...
### Community 654 - "OpenClaw — iOS Control of the Agency (Single-Service Free-Tier Deploy)"
Cohesion: 0.15
...
### Community 655 - "ChatScreen.jsx"
Cohesion: 0.10
Nodes (14): chatSend(), getAgentChatJob(), getSession(), listProviderModels(), listProviders(), listSessions(), AVAILABLE_AGENTS, CHAT_PHASES (+6 more)

### Community 656 - "test_daily_automation_2026_07_11.py"
Cohesion: 0.14
...
### Community 658 - "RuntimeReadinessReport"
Cohesion: 0.05
Nodes (31): make_isolated_workspace(), Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Create an isolated workspace directory under *root*. This is the legacy path…, _workspace_component(), Fail preflight when the provider extension is configured but missing. Without…, Any, Structured, actionable preflight validation issue. (+23 more)

### Community 659 - "check_model_catalog_consistency.py"
Cohesion: 0.22
Nodes (14): _check_cross_catalogue(), _check_prefer_models(), _check_presets(), _llm_declared(), _load(), main(), Any, Path (+6 more)

### Community 660 - "test_critical_flows.py"
Cohesion: 0.26
...
### Community 663 - "TestBrainConfigUpdates"
Cohesion: 0.13
Nodes (6): tests/test_daily_automation_2026_07_10.py — Daily automation tests…, Verify Llama 4 and Claude Sonnet 5 cross-provider aliases are registered., Verify brain_config.py changes: Google provider, updated presets., The durable property, not the id of the week. This assertion has been amended…, TestBrainConfigUpdates, TestBrainFailoverModelAliases

### Community 664 - "test_dockerfile_ships_config_dir.py"
Cohesion: 0.14
...
### Community 665 - "test_fabric_patterns.py"
Cohesion: 0.11
...
### Community 666 - "compilerOptions"
Cohesion: 0.13
Nodes (14): compilerOptions, isolatedModules, jsx, lib, module, moduleResolution, noEmit, resolveJsonModule (+6 more)

### Community 667 - "EvalHarness"
Cohesion: 0.22
Nodes (7): EvalHarness, _bounded(), Runs agent functions against Tasks, records Trajectories and produces…, Execute the agent on a single task and return an EvalResult., Delegate to the agent callable (sync or async)., Run multiple tasks and aggregate into a BenchmarkReport. Set concurrency > 1 to…, AgentFn

### Community 668 - "4. Threats"
Cohesion: 0.10
...
### Community 670 - "Agent Orchestration Design"
Cohesion: 0.17
...
### Community 671 - "KnowledgeScreen.jsx"
Cohesion: 0.13
...
### Community 672 - "DeterministicEngine"
Cohesion: 0.21
...
### Community 676 - "Skill: lr-schedule-advisor"
Cohesion: 0.14
...
### Community 680 - "Checks Performed"
Cohesion: 0.14
...
### Community 681 - "Skill: training-stability-monitor"
Cohesion: 0.14
...
### Community 682 - "admin_digest_router.py"
Cohesion: 0.22
...
### Community 683 - "Skill: branch-cleanup"
Cohesion: 0.14
...
### Community 684 - "Skill: dev-browser — Browser Automation via Sandboxed JS"
Cohesion: 0.17
...
### Community 685 - "Skill: perplexity — Web Research via Perplexity API"
Cohesion: 0.14
...
### Community 686 - "Instructions"
Cohesion: 0.14
Nodes (13): 1 — Tests green, 2 — Changelog updated, 3 — Determine the version bump, 4 — Update changelog, 5 — Commit the changelog update, 6 — Tag the release, 7 — Verify CI on the tag, 8 — Post-release (+5 more)

### Community 687 - "Quick-Note Issues Processing Summary"
Cohesion: 0.14
...
### Community 688 - "Separate hosted dashboard backend (`backend/server.py`)"
Cohesion: 0.13
...
### Community 691 - "extract_failures"
Cohesion: 0.21
...
### Community 692 - "context_plan_gate.py"
Cohesion: 0.14
...
### Community 693 - "test_telegram_auto_approve.py"
Cohesion: 0.21
...
### Community 694 - "test_wrapper_falls_back_to_installed_model"
Cohesion: 0.19
...
### Community 695 - "TestAnthropicToolListCaching"
Cohesion: 0.34
...
### Community 697 - "_StubManager"
Cohesion: 0.14
Nodes (3): _apply(), Minimal stand-in for BrainFailoverManager., _StubManager

### Community 698 - ".test_set_github_token_sqlite_string_id_does_not_500"
Cohesion: 0.16
...
### Community 699 - "control_overrides.py"
Cohesion: 0.14
...
### Community 700 - "test_openclaw_gateway.py"
Cohesion: 0.14
...
### Community 701 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 703 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 704 - "._log_event"
Cohesion: 0.11
Nodes (18): PhaseType, Append an event to the workflow event log., Create a new WorkflowRun and begin pre-gate phase execution. The run is…, Return current WorkflowRun snapshot or None., Approve the plan — lifts the ApprovalGate and resumes execution. Raises…, Reject the plan — marks run as failed., Cancel a non-terminal run., Resume a paused or failed run from its last completed phase. (+10 more)

### Community 705 - "test_monitor_lib.py"
Cohesion: 0.15
Nodes (7): _isolate_env(), MonkeyPatch, tests/test_monitor_lib.py — unit tests for scripts/monitor_lib.py. Covers the…, Pin all env-overridable paths to tmp_path for hermetic tests., TestIsProcessAlive, TestSuperviseLoopGiveUp, TestSupervisorTick

### Community 706 - "AGENTS.md — Codebase Map & Operations Reference"
Cohesion: 0.15
...
### Community 708 - "ResearchOrchestrator"
Cohesion: 0.16
Nodes (7): Coordinates a multi-agent research workflow. Workflow: 1. plan(question) → list…, Round-robin pick within a role (least-loaded first)., Execute the DAG until all tasks resolve or no progress is possible., Return the final synthesized answer, or a status report if blocked., Status counts across all tasks., ResearchOrchestrator, orchestrator()
...
### Community 710 - "Skill: Brain Dump"
Cohesion: 0.15
...
### Community 713 - "Skill: duplicate-thread"
Cohesion: 0.15
...
### Community 714 - "Skill: Email Triage"
Cohesion: 0.15
...
### Community 717 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
...
### Community 718 - "Skill: prompt-library"
Cohesion: 0.15
...
### Community 719 - "Skill: prompt-transparency"
Cohesion: 0.15
...
### Community 721 - "Skill: scope-guard"
Cohesion: 0.15
...
### Community 722 - "admin_update_task_router.py"
Cohesion: 0.22
...
### Community 725 - "Skill: platform-setup — Autonomous Agency Bootstrap"
Cohesion: 0.15
...
### Community 726 - "Agency Core — Progress & Resume Log"
Cohesion: 0.13
...
### Community 727 - "Workspace Isolation Architecture"
Cohesion: 0.15
...
### Community 728 - "Device compatibility and model picks"
Cohesion: 0.15
...
### Community 731 - "cost_tracker.py"
Cohesion: 0.11
Nodes (20): clear_cost_attribution(), cost_table(), Return the active per-model cost table (USD per million tokens). Useful for…, Reset in-memory cost attribution counters. Admin-only., _build_cost_table(), clear_stats(), cost_for_tokens(), get_cost_table() (+12 more)

### Community 732 - "_is_bedrock_model_id"
Cohesion: 0.27
...
### Community 733 - "test_portfolio_intelligence.py"
Cohesion: 0.14
...
### Community 734 - "test_task_brain_preflight.py"
Cohesion: 0.21
...
### Community 735 - "_InMemoryBackend"
Cohesion: 0.18
...
### Community 736 - "cmd_autonomy"
Cohesion: 0.18
Nodes (14): _backend_get(), cmd_autonomy(), cmd_loops(), _grade_icon(), GET an un-gated backend read endpoint (/api/autonomy/status, /api/loops). These…, Snapshot of the agency's autonomy: active brain, loop readiness, dispatch., Loop Engineering fleet readiness + the costliest loops, from /api/loops., tests/test_telegram_observe.py Tests for the read-only "observe from Telegram"… (+6 more)

### Community 737 - "safe_agency.py"
Cohesion: 0.22
...
### Community 738 - "test_compose_and_coordinate_api.py"
Cohesion: 0.19
...
### Community 739 - "TestModelCostTableUpdates"
Cohesion: 0.26
...
### Community 740 - "TestDecisionsBotLinks"
Cohesion: 0.21
...
### Community 741 - "test_deploy_trigger_covers_image.py"
Cohesion: 0.21
...
### Community 742 - "FeatureUnavailableError"
Cohesion: 0.16
Nodes (13): check_feature(), get_feature(), list_features(), Any, get, post, features/api.py — Admin API for the feature support matrix. Exposes: GET…, Return the full support matrix with summary. (+5 more)

### Community 743 - "test_providers_live_e2e.py"
Cohesion: 0.24
...
### Community 744 - "test_quick_note_engine.py"
Cohesion: 0.17
...
### Community 746 - "skill_registry.py"
Cohesion: 0.17
...
### Community 749 - "Skill: hybrid-reasoning (Hybrid AI)"
Cohesion: 0.29
...
### Community 754 - "Skill: system-prompt-audit"
Cohesion: 0.17
...
### Community 755 - "Skill: task-alive-updates"
Cohesion: 0.17
...
### Community 759 - "connectors_api.py"
Cohesion: 0.21
Nodes (11): build_connectors_router(), list_connectors(), webhook_send(), Any, APIRouter, BaseModel, backend/connectors_api.py — the connector catalogue API (`/api/connectors/*`).…, Reject anyone who is not the agency admin. (+3 more)

### Community 760 - "Skill: agent-browser — Real Chrome Browser Automation"
Cohesion: 0.17
...
### Community 764 - "Quantization Internals"
Cohesion: 0.17
...
### Community 765 - "Autonomy Uplift — Living Roadmap & Detailed Implementation Specs"
Cohesion: 0.15
...
### Community 766 - "Implementation Plan — DB-persisted, UI-switchable Brain (no redeploy)"
Cohesion: 0.14
...
### Community 767 - "Traffic Distribution Across Providers"
Cohesion: 0.14
...
### Community 770 - "install-agents.sh"
Cohesion: 0.39
...
### Community 771 - "check_container_posture.py"
Cohesion: 0.26
...
### Community 772 - "test_pytest_failure_parser.py"
Cohesion: 0.23
...
### Community 773 - "Kimi Web-Bridge Service"
Cohesion: 0.17
...
### Community 776 - "test_agile_api.py"
Cohesion: 0.17
...
### Community 777 - "test_app_settings.py"
Cohesion: 0.21
...
### Community 778 - "test_brain_default_consistency.py"
Cohesion: 0.24
...
### Community 780 - "TestModelRegistryUpdates"
Cohesion: 0.20
Nodes (6): Test hook — rebuild the registry on next access., reset(), Verify new models are in the packages/ai/registry., TestModelRegistryUpdates, Migration must not make a model disappear from routing., test_legacy_registry_models_are_visible_to_the_new_registry()

### Community 781 - "._resolve_path"
Cohesion: 0.13
...
### Community 784 - "_extractive_compress"
Cohesion: 0.18
...
### Community 785 - "ai_insights.py"
Cohesion: 0.40
Nodes (5): Enum, str, AI-Assisted Engineering Insights — track AI tool usage, engagement, and…, Categories of AI engineering tools tracked., ToolKind

### Community 786 - "AI Engineering Insights Skill"
Cohesion: 0.29
...
### Community 787 - "cowork_session.py"
Cohesion: 0.15
...
Cohesion: 0.31
Nodes (10): A specialized agent that executes tasks of a specific role., ResearchAgent, _echo_handler(), Test handler that echoes the task question + context keys., test_agent_can_handle_matching_role(), test_agent_cannot_handle_other_roles(), test_agent_execute_success_increments_counter(), test_agent_execute_wrong_role_marks_failed() (+2 more)
...
### Community 796 - "ECC Harness Patterns Skill"
Cohesion: 0.18
...
### Community 799 - "Stop-Slop Quality Skill"
Cohesion: 0.18
...
### Community 800 - "Universality: case-coverage matrix"
Cohesion: 0.17
...
### Community 801 - "2. Critical Bugs & Exact Detection Signatures"
Cohesion: 0.18
...
### Community 802 - "Issue #467 — Section 1: Pulled State + PR Inventory"
Cohesion: 0.18
...
### Community 803 - "Deploy to Google Cloud Run"
Cohesion: 0.18
...
### Community 805 - "Sampling Strategies Internals"
Cohesion: 0.18
...
### Community 806 - "Runbook — Apply the Fast Free NVIDIA Brain to Render (TASK 2)"
Cohesion: 0.12
...
### Community 807 - "CI Troubleshooting Runbook"
Cohesion: 0.18
...
### Community 808 - "NVIDIA NIM — Free Tier Setup"
Cohesion: 0.18
...
### Community 809 - "Worker Service — Operations Runbook"
Cohesion: 0.18
...
### Community 810 - "test_bedrock_live.py"
Cohesion: 0.25
...
### Community 811 - "build_tech_db.py"
Cohesion: 0.35
...
### Community 812 - "migrate_local_brain_env.py"
Cohesion: 0.25
...
### Community 815 - "test_conftest_hermetic_env.py"
Cohesion: 0.18
...
### Community 816 - "test_local_brain_state.py"
Cohesion: 0.12
...
### Community 817 - "TestMCPClientStructuredOutput"
Cohesion: 0.27
...
### Community 818 - "test_empirical_verify.py"
Cohesion: 0.49
...
### Community 819 - "test_event_log.py"
Cohesion: 0.45
...
### Community 821 - "brain_config.py"
Cohesion: 0.05
Nodes (37): _active_primary_provider(), is_north_mini_code_default(), provider_base_url(), _provider_env_value(), _provider_ids_from_literal(), provider_key_rotation_enabled(), services/brain_config_store.py — DB-persisted, UI-switchable "brain" config.…, True when the ``NORTH_MINI_CODE_DEFAULT`` flag is on (default ON). Reads the… (+29 more)

### Community 822 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.22
...
### Community 827 - "Protocol: Premium Utilitarian Minimalism UI Architect"
Cohesion: 0.20
...
### Community 828 - "The 5-Step Wrap-Up Ritual"
Cohesion: 0.20
...
### Community 829 - "Brag Plan: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 831 - "Hyperframes Composition Brief: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 832 - "_normalize_tool_choice"
Cohesion: 0.31
...
### Community 833 - "Skill: Agentic Agile"
Cohesion: 0.20
...
### Community 834 - "Skill: browserbase-ui-test — Adversarial UI Testing"
Cohesion: 0.20
...
### Community 835 - "Skill: financial-analyst (Agentic CFO)"
Cohesion: 0.20
...
### Community 836 - "Graphiti Temporal Context Skill"
Cohesion: 0.20
...
### Community 837 - "Skill: seo-audit-report"
Cohesion: 0.20
...
### Community 838 - "Agent Readiness Report"
Cohesion: 0.20
...
### Community 839 - "467 Golden Path — Locked Implementation Order"
Cohesion: 0.20
...
### Community 840 - "PR #634 Implementation Tracker"
Cohesion: 0.22
...
### Community 841 - "One command (recommended)"
Cohesion: 0.20
...
### Community 842 - "What to clean up"
Cohesion: 0.18
...
### Community 845 - "capture_screens.py"
Cohesion: 0.29
...
### Community 847 - "enrich_quick_note_issues.py"
Cohesion: 0.36
...
### Community 848 - "_start_ceo_agency"
Cohesion: 0.27
...
### Community 850 - "_get_provider_policy"
Cohesion: 0.19
...
### Community 851 - "TestDisabledProvidersAreNotFalselyReportedUnreachable"
Cohesion: 0.22
...
### Community 852 - "TestTheWorkflowIsSafeAndReadOnly"
Cohesion: 0.20
...
### Community 853 - "TestSessionMemory"
Cohesion: 0.20
...
### Community 854 - "test_model_catalog_guard.py"
Cohesion: 0.29
...
### Community 857 - "test_task_clarification.py"
Cohesion: 0.17
...
### Community 858 - "test_workflow_api_mount.py"
Cohesion: 0.20
...
### Community 860 - "WorkspaceMetrics"
Cohesion: 0.20
...
### Community 863 - "_extract_workflow_relevance"
Cohesion: 0.33
...
### Community 864 - "test_daemon_restart_reprobes_http_port_only"
Cohesion: 0.13
Nodes (5): _fake_http_sequence(), fake_urlopen(), Pins the v3 fix: after the multi-port preamble probe finds colibri serving a…, Yield a list of (status, body) tuples the daemon will see in order when it…, test_daemon_restart_reprobes_http_port_only()

### Community 865 - "Skill: changelog-enforcer"
Cohesion: 0.22
...
### Community 866 - "Skill: learn-rule"
Cohesion: 0.22
...
### Community 869 - "Skill: changelog-enforcer"
Cohesion: 0.22
Nodes (8): Acceptance Checks, Changelog Location, Entry Format, Examples, Hook Behaviour, Instructions, Skill: changelog-enforcer, When to Use

### Community 870 - "Skill: video-context — read a video without watching it"
Cohesion: 0.22
...
### Community 871 - "ADR 003: Multi-Agent Orchestration with Plan-Execute-Verify Loop"
Cohesion: 0.22
...
### Community 872 - "The 8-Step Golden Path"
Cohesion: 0.22
...
### Community 873 - "Issue #1356: quick-note:https://searchengineland.com/turn-seo-backlog-into-roadmap-485713"
Cohesion: 0.22
...
### Community 874 - "LLM Router — provider guide"
Cohesion: 0.20
...
### Community 879 - ".test_recurrences_are_reported_even_inside_cooldown"
Cohesion: 0.17
Nodes (3): A fix that did not hold must be detectable, or it looks healed forever., TestRenderFindingsReachTheHealer, latest_deploy()

### Community 880 - "e2e_smoke.py"
Cohesion: 0.39
Nodes (6): _chat(), check(), _health(), _models(), Real-API end-to-end smoke test for a *running* LLM Relay instance. Unlike the…, _req()

### Community 881 - "run_patched_colibri.py"
Cohesion: 0.31
...
### Community 882 - "models/company_graph.py"
Cohesion: 0.03
Nodes (101): ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraphResponse, CompanyGraphSnapshot, CompanyResponse, CompanyUpdateRequest, Connector (+93 more)

### Community 883 - "Instructions"
Cohesion: 0.14
Nodes (13): Acceptance Checks, `admin_auth.py` checklist, `agent/tools.py` checklist, Escalation, Instructions, `key_store.py` checklist, `proxy.py` auth middleware checklist, Risky Modules in This Repo (+5 more)

### Community 884 - "TestDashboard"
Cohesion: 0.22
...
### Community 885 - "test_backend_requirements_cover_runtime_imports.py"
Cohesion: 0.25
...
### Community 888 - ".test_every_request_carries_a_real_user_agent"
Cohesion: 0.22
...
### Community 889 - "Tween"
Cohesion: 0.21
Nodes (13): _assertThisInitialized(), Hc(), Ic(), jc(), t(), ta(), Timeline(), Tween() (+5 more)

### Community 890 - "test_changelog_parity_guard.py"
Cohesion: 0.22
...
### Community 891 - "traffic_director.py"
Cohesion: 0.13
...
### Community 892 - "test_process_quick_note_workflow.py"
Cohesion: 0.25
...
### Community 893 - "TestEveryFullSuiteJobHasMongo"
Cohesion: 0.22
...
### Community 894 - "test_scanner_deps_parity.py"
Cohesion: 0.31
...
### Community 895 - "test_serve_spa_prefixes.py"
Cohesion: 0.31
...
### Community 903 - "hybrid_reasoning.py"
Cohesion: 0.36
...
### Community 904 - "quality_checker.py"
Cohesion: 0.32
...
### Community 905 - "research_coordinator.py"
Cohesion: 0.36
...
### Community 908 - "Agent: Judge (Release / QA Gate)"
Cohesion: 0.25
Nodes (7): Activation, Agent: Judge (Release / QA Gate), Enforcement, Output, Responsibilities, Role, Verdict Meanings

### Community 909 - "Skill: browserbase-browser — Real Browser Automation"
Cohesion: 0.25
...
### Community 914 - "GitHub Branch Protection Settings"
Cohesion: 0.25
...
### Community 921 - "Issue #362: Nvidia repo setup"
Cohesion: 0.25
...
### Community 922 - "Issue #364: quick-note:https://www.marktechpost.com/2026/06/01/meet-memory-os-a-6-layer-open-source-memory-stack-built-on-top-of-hermes-agent/"
Cohesion: 0.25
...
### Community 923 - "Issue #378: quick-note:https://www.marktechpost.com/2026/06/02/tinyfish-launches-bigset-an-open-source-multi-agent-system-that-builds-structured-live-datasets-from-plain-english-descriptions/"
Cohesion: 0.25
...
### Community 924 - "Issue #379: quick-note:https://searchengineland.com/schema-markup-optimize-agentic-web-479080"
Cohesion: 0.25
...
### Community 925 - "Issue #380: quick-note:https://cursor.com/blog/cloud-agent-lessons"
Cohesion: 0.25
...
### Community 926 - "Issue #381: quick-note:https://www.xda-developers.com/claude-code-with-opus-48-is-expensive-but-i-made-it-efficient-with-my-local-ai-workflow/"
Cohesion: 0.25
...
### Community 927 - "Issue #382: quick-note:https://claude.com/blog/how-coderabbit-used-claude-to-build-an-agent-orchestration-system"
Cohesion: 0.25
...
### Community 928 - "Issue #383: quick-note:https://www.marktechpost.com/2026/05/29/hexo-labs-open-sources-sia-a-self-improving-agent-that-updates-both-the-harness-and-the-model-weights/"
Cohesion: 0.25
...
### Community 929 - "Issue #416: feat: Self-hosted Codebuff (freebuff) on free NVIDIA models + Telegram bot phone control"
Cohesion: 0.25
...
### Community 930 - "Issue #485: [Trend Digest] Week of 2026-06-08"
Cohesion: 0.25
...
### Community 931 - "Issue #488: quick-note:https://github.com/cookiy-ai/user-research-skill"
Cohesion: 0.25
...
### Community 932 - "Issue #491: Implement whatever is necessary from https://github.com/BehiSecc/awesome-claude-skills"
Cohesion: 0.25
...
### Community 933 - "Issue #493: Use the https://github.com/mvanhorn/last30days-skill skill to get the trend updated"
Cohesion: 0.25
...
### Community 934 - "Issue #495: Read https://www.anthropic.com/news/claude-fable-5-mythos-5 and understand if mythos or fable can be added to the repo"
Cohesion: 0.25
...
### Community 935 - "Issue #581: Sprint tracker: pending work after brand rename + mobile-first pass"
Cohesion: 0.25
...
### Community 946 - "build_admin_local_brain_router"
Cohesion: 0.24
Nodes (11): build_admin_local_brain_router(), get_admin_local_brain_state(), post_admin_local_brain_toggle(), Any, APIRouter, BaseModel, backend/admin_local_brain_router.py — admin-session proxy for the local-brain…, Construct a ready-to-mount APIRouter with the auth dependency baked in. The… (+3 more)

### Community 947 - "Implementation — DB-persisted, UI-switchable Brain (PR #824 follow-up)"
Cohesion: 0.17
...
### Community 948 - "Continual Harness (`agent/harness_spec.py`)"
Cohesion: 0.25
...
### Community 949 - "Prime Agent Runtime"
Cohesion: 0.25
...
### Community 953 - "467 Public Site Truth Spec"
Cohesion: 0.17
...
### Community 954 - "TestAgentRunnerSafety"
Cohesion: 0.24
...
### Community 961 - "TestItIsNotBuiltForOneVendor"
Cohesion: 0.25
...
### Community 962 - "_self_heal_ready"
Cohesion: 0.20
Nodes (5): Any, Snapshot for ``/api/render/ops/status`` and the dashboard. ``self_heal_ready``…, True when a filed finding will actually be scheduled as a fix. Detection and…, _self_heal_ready(), A loop with no on_task registers issues but schedules no repair —…
...
### Community 965 - "apply_overrides"
Cohesion: 0.17
Nodes (12): apply_overrides(), Write *overrides* into ``os.environ`` and refresh dependent caches. Keys that…, Re-read every ``settings`` attribute from the updated environment. Re-runs…, _refresh_settings_singleton(), is_controllable(), True when *key* may be overridden from the dashboard. The allow-list is what…, Re-running Settings.__init__ mints a new random secret when SECRET_KEY is…, test_apply_overrides_refreshes_the_settings_singleton() (+4 more)

### Community 966 - "test_gh_brain_failover.py"
Cohesion: 0.17
Nodes (6): gh_brain_failover(), Regression test for the auto-PR scripts' brain failover. On 2026-09-06 the…, Guards the actual regression: the crash-prone single-call site is gone., An empty string is falsy — this used to be returned as a live NVIDIA candidate…, TestAutonomousAgentUsesTheSharedFailover, TestBrainCandidates

### Community 967 - "_FakeInner"
Cohesion: 0.22
...
### Community 969 - "RuntimeHealth"
Cohesion: 0.07
Nodes (18): Check whether Docker is available and report the adapter's runtime health.…, Health status snapshot for a runtime., Return a health snapshot. Must not raise; return available=False instead., RuntimeHealth, Some runtimes awake, some sleeping — both reported correctly., test_wake_all_sleeping_runtimes_mixed_state(), _poll(), hermes_only_manager() (+10 more)

### Community 970 - "test_nuclear_cleanup_targets_the_schedule_collection"
Cohesion: 0.25
...
### Community 972 - "Agency Core — Ruthless Architecture Audit & Migration Plan"
Cohesion: 0.18
...
### Community 973 - "AUTONOMY_CHARTER.md"
Cohesion: 0.18
...
### Community 975 - "_check_storage_health"
Cohesion: 0.13
Nodes (9): _check_storage_health(), health(), Check if the storage backend is reachable. Works with BOTH MongoDB and SQLite:…, Authenticated system status summary for the Doctor screen., system_status(), Guard the version single-source-of-truth: every place that hardcodes the…, deployed_commit(), Single source of truth for the application version and brand. Bump with… (+1 more)

### Community 976 - "Tailored Onboarding, Editable Companies & Dynamic Roles"
Cohesion: 0.18
...
### Community 978 - "Autonomy Charter — Telegram-Gated Self-Running Agency"
Cohesion: 0.18
...
### Community 980 - "Context: Agentic Agile + Portfolio Management"
Cohesion: 0.18
...
### Community 995 - "Pending Activities — Implementation Playbook"
Cohesion: 0.11
...
### Community 996 - "Platform Controls"
Cohesion: 0.25
...
### Community 997 - "LLM Router — architecture"
Cohesion: 0.18
...
### Community 998 - "Cloudflare = the real working app"
Cohesion: 0.29
...
### Community 1004 - "Agent Transparency Report"
Cohesion: 0.15
...
### Community 1010 - "test_daily_2026_06_14.py"
Cohesion: 0.38
...
### Community 1013 - "Competitor Analysis — Autonomous AI Agency"
Cohesion: 0.20
...
### Community 1014 - "TestReasonsAreActionable"
Cohesion: 0.29
...
### Community 1015 - "Issue #1427: quick-note:https://github.com/bingreeky/JIT"
Cohesion: 0.20
...
### Community 1016 - "LLM Router — configuration guide"
Cohesion: 0.20
...
### Community 1017 - "TestMongoService"
Cohesion: 0.29
...
### Community 1024 - "Music Cues: happy-beats-business-moves-vol-1-by-ende-dot-app"
Cohesion: 0.33
...
### Community 1039 - ".advise"
Cohesion: 0.12
Nodes (10): default_executives(), Executive, Any, Return the roles whose keywords match *question*. Falls back to a sensible…, Consult the relevant executives and return a unified recommendation. With…, One C-suite persona: its identity and the domain it answers from., Return the built-in C-suite personas., LLMFn (+2 more)

### Community 1040 - "Active Task Tracker"
Cohesion: 0.22
...
### Community 1044 - "Autonomous SDLC Loop (Agency Core, repo-agnostic)"
Cohesion: 0.22
...
### Community 1045 - "apply_phase1_changes.py"
Cohesion: 0.33
...
### Community 1058 - "test_harness_spec.py"
Cohesion: 0.12
...
### Community 1060 - "TestTheAgentRunsCurrentCode"
Cohesion: 0.33
...
### Community 1066 - "test_daemon_picks_colibri_8081_when_only_it_listening"
Cohesion: 0.22
Nodes (3): Pins the multi-port fix end-to-end: when ONLY colibri's :8081 is up serving…, test_daemon_picks_colibri_8081_when_only_it_listening(), fake_probe_v1_models()

### Community 1067 - ".update_status"
Cohesion: 0.33
...
### Community 1068 - "harness_spec.py"
Cohesion: 0.19
...
### Community 1069 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 1099 - "_add_colibri_shim_changelog_entry.py"
Cohesion: 0.50
...
### Community 1100 - "build_llama_cpp.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 1101 - "download_glm52_weights.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 1102 - "download_glm52_weights.sh script"
Cohesion: 0.70
Nodes (4): download_glm52_weights.sh script, fail(), ok(), warn()

### Community 1103 - "_fetch_pytest_failures.py"
Cohesion: 0.50
...
### Community 1104 - "setup_colibri.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 1105 - "setup_colibri.sh script"
Cohesion: 0.70
Nodes (4): setup_colibri.sh script, fail(), ok(), warn()

### Community 1106 - "status_colibri_server.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 1107 - "start_tunnel.py"
Cohesion: 0.50
...
### Community 1111 - "nvidia_live_test.py"
Cohesion: 0.40
...
### Community 1113 - "Advisor Strategy — Local Proxy Handling"
Cohesion: 0.25
...
### Community 1115 - "Feature Maturity / Support Matrix"
Cohesion: 0.25
...
### Community 1117 - "TestGhIsNotReAuthenticated"
Cohesion: 0.40
...
### Community 1118 - "TestModelRoleSeparation"
Cohesion: 0.40
...
### Community 1119 - "Free NVIDIA brain + UI-controlled provider policy + no silent spend"
Cohesion: 0.25
...
### Community 1121 - "recovery.py"
Cohesion: 0.67
Nodes (3): detect_secrets(), main(), Recover CHANGELOG.md from a Git merge conflict in its [Unreleased] block. Pre-…

### Community 1122 - "Issue #504: EPIC: Autonomy hardening — live-verified defects 2026-06-10"
Cohesion: 0.25
...
### Community 1123 - "codebase-explorer.md"
Cohesion: 0.50
Nodes (3): Hard constraints, Method, Output

### Community 1124 - "docs-auditor.md"
Cohesion: 0.50
Nodes (3): Hard constraints, Output, What to check

### Community 1125 - "Issue #656: Bugs"
Cohesion: 0.25
...
### Community 1132 - "TOP 6 — Highest-ROI Items (Validated by Opus Research Agent)"
Cohesion: 0.25
...
### Community 1133 - "SECTION A — Agent Efficiency (Hermes / AOS / MYT)"
Cohesion: 0.25
...
### Community 1134 - "SECTION C — Direct Chat Improvements (CBF / HRM)"
Cohesion: 0.25
...
### Community 1135 - "Runbook — Instance Activation"
Cohesion: 0.25
...
### Community 1154 - "test_skills_route_order.py"
Cohesion: 0.67
...
### Community 1216 - "get_output_filter"
Cohesion: 0.32
...
### Community 1281 - "[5.0.0]"
Cohesion: 0.10
Nodes (20): [5.0.0], Added, Added, Added, Changed, Changed, Changed, Changelog (+12 more)

### Community 1282 - "CapacityAllocation"
Cohesion: 0.25
...
### Community 1284 - "Implementation plan + TO-DO (check off as you go)"
Cohesion: 0.29
...
### Community 1287 - "Cost-aware routing evaluation"
Cohesion: 0.29
Nodes (6): Caveats, CLI, Cost-aware routing evaluation, How the metric resists gaming, Method, What's here

### Community 1288 - "_answer_callback"
Cohesion: 0.29
Nodes (8): _answer_callback(), _edit_message(), _process_pr_callback(), _process_wfo_callback(), Handle an inline Approve/Reject button on a WorkflowOrchestrator approval-gate…, Handle the green-PR approval card buttons (pr_merge / pr_reject). ``pr_merge``…, Edit an existing message's text and (optionally) its inline keyboard. Best-…, Acknowledge a callback query so Telegram stops the button's loading spinner.…

### Community 1289 - "InitiativeProgress"
Cohesion: 0.33
...
### Community 1292 - "Master Goal Prompt — Autonomous Agency CEO"
Cohesion: 0.33
...
### Community 1293 - "Agency Core — Operational Knowledge (verified live, 2026-06-10/11)"
Cohesion: 0.33
...
### Community 1294 - "SECTION B — NVIDIA / Cloud Model Integration (Nemotron / NVD)"
Cohesion: 0.33
...
### Community 1295 - "SECTION D — Deployment & Infrastructure (CHM / NVD)"
Cohesion: 0.33
...
### Community 1296 - "TestDirectChatAgentExecution"
Cohesion: 0.33
...
### Community 1298 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 1300 - ".daily_active_users"
Cohesion: 0.40
...
### Community 1306 - "SECTION E — Autonomy & Self-Healing (AOS / MYT / ECC)"
Cohesion: 0.40
...
### Community 1307 - "SECTION F — Developer Experience (CBF / ECC)"
Cohesion: 0.40
...
### Community 1309 - "._resolve_merge_decision"
Cohesion: 0.40
...
### Community 1310 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 1311 - "test_mcp_protocol_version.py"
Cohesion: 0.25
...
### Community 1332 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 1333 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.29
...
### Community 1335 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1336 - "TestProvidersScreen"
Cohesion: 0.43
...
### Community 1339 - "_brain_provider_status"
Cohesion: 0.33
...
### Community 1340 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 1341 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 1343 - "_governance_identity"
Cohesion: 0.33
Nodes (3): _governance_identity(), Any, Resolve the acting identity for a runtime dispatch. Identity comes from…

### Community 1344 - "TestAgentRunnerExecution"
Cohesion: 0.33
...
### Community 1345 - "TestWorkflowIntegration"
Cohesion: 0.33
...
### Community 1346 - "Who is this for?"
Cohesion: 0.40
...
### Community 1347 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 1348 - "Agent API Issues"
Cohesion: 0.40
...
### Community 1349 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 1350 - "check_rate_limit"
Cohesion: 0.60
Nodes (4): check_rate_limit(), asyncio, test_rate_limiter_concurrency(), call_limit()

### Community 1351 - "_scheduler_on_fire"
Cohesion: 0.50
Nodes (4): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), The old sync callback called the deprecated ``asyncio.get_event_loop()`` from…, test_scheduler_on_fire_is_a_coroutine()

### Community 1352 - "Issue → Context → Draft PR automation"
Cohesion: 0.50
Nodes (4): Backfilling existing issues, Free-first model routing, Issue → Context → Draft PR automation, The workflows

### Community 1353 - "The 24x7 agency — your agents never go idle"
Cohesion: 0.50
...
### Community 1357 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1361 - "Configuration reference"
Cohesion: 0.67
Nodes (3): Configuration reference, Provider priority chain, Running the brain on local Ollama (via a tunnel)

## Knowledge Gaps
- **3548 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3543 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14421 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **181 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `ClaudeCodeAdapter`, `build_llm_router`, `test_langfuse_agency_wide.py`, `test_portfolio_intake.py`, `TaskStore`, `AgentDefinition`, `._check_permissions`, `test_llm_router_strategies.py`, `ExecutionRequest`, `test_operational_incidents.py`, `TestNormalizeResponseFormat`, `test_platform_controls.py`, `BrainConfig`, `get_workflow_orchestrator`, `test_agent_tool_governance.py`, `test_brain_failover.py`, `Agency`, `build_governance_router`, `Task`, `test_autonomy_status.py`, `test_brain_patch_service_token.py`, `test_catalogue_probe.py`, `test_freebuff_bot.py`, `test_harness_spec.py`, `test_unit8_model_catalog.py`, `PolicyEngine`, `E2BSandboxSession`, `test_telegram_freebuff.py`, `ImprovementLoop`, `test_failover_client_shared.py`, `resolve_active_brain`, `config.py`, `UserRole`, `PrimeAgentAdapter`, `test_company_api.py`, `test_phase5_doctor.py`, `test_startup_warmup.py`, `KeyPool`, `test_model_router.py`, `BrainWatchdog`, `services/seo_audit.py`, `nvidia_models.py`, `_Budget`, `SelfHealingAgent`, `Specialist`, `e2e/test_browser.py`, `services/background.py`, `test_openclaw_endpoints.py`, `WorkflowRun`, `test_sam_livekit.py`, `facade.py`, `test_provider_router.py`, `claim`, `WorkflowRun`, `TestClient`, `test_procedural_memory.py`, `SQLiteStore`, `test_knowledge_sync.py`, `run_task`, `test_provider_state_durability.py`, `_resolve_push_token`, `test_voice_pipeline.py`, `TestChatHistoryStore`, `test_refresh_agent_built_proof.py`, `test_telegram_task_callback.py`, `TokenBudget`, `ChatHistoryStore`, `E2BAdapter`, `persist_plan_spec`, `test_context_rulebook.py`, `ArtifactStore`, `test_llm_router_e2e.py`, `test_new_features_e2e.py`, `KeyStore`, `test_trend_watcher.py`, `TestHarnessAdapter`, `diagnostics.py`, `get_registry`, `test_issue_intake.py`, `_step`, `QuickNoteQueue`, `AgentRunner`, `WorkflowBuildRequest`, `test_e2b_data_flow.py`, `test_telegram_auto_approve.py`, `_is_exempt`, `test_openclaw_gateway.py`, `test_monitor_lib.py`, `FeatureMaturity`, `ResearchOrchestrator`, `TestSwarmRoleRouting`, `OllamaCircuitBreaker`, `_job_text`, `test_video_transcript.py`, `mcp_client.py`, `TaskStatus`, `test_features_api.py`, `cost_tracker.py`, `test_task_brain_preflight.py`, `models/seo_audit.py`, `TrendWatcher`, `test_colibri_brain_shim.py`, `MCPClient`, `llm/router.py`, `TestEstimateTokensForMessages`, `test_telegram_webhook.py`, `_autonomy_bg_cycle`, `.on_task_complete`, `clear_cooldowns`, `SchedulerStore`, `test_agile_api.py`, `test_app_settings.py`, `AgentSwarm`, `ContextPruner`, `test_llm_router_disabled.py`, `test_implement_agent_routing.py`, `test_rate_limiter.py`, `NotificationDispatcher`, `test_control_plane_api.py`, `test_persistent_memory.py`, `SecurityScanner`, `test_mcp_governance.py`, `test_brain_availability_doctor.py`, `test_ceo_router.py`, `TestClient`, `test_mcp_protocol_version.py`, `test_nvidia_model_discovery.py`, `controls_app`, `test_purge_backlog.py`, `test_provider_enable_disable.py`, `test_sam_voice.py`, `test_local_brain_state.py`, `test_kimi_bridge_server.py`, `OutputFilter`, `model_discovery.py`, `analyze_page`, `session_retro.py`, `test_agency_fix.py`, `test_skill_registry_boot_refresh.py`, `_run_analyze`, `_start_ceo_agency`, `TestTheWorkflowIsSafeAndReadOnly`, `test_scheduler_hydration_bounded.py`, `test_task_clarification.py`, `test_executive_advisory_api.py`, `test_llm_router_queue_cache.py`, `models/company_graph.py`, `traffic_director.py`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_process_quick_note_workflow.py`, `test_webui_provider_priority.py`, `isolated_telegram_config`, `_Recorder`, `_undeclared`, `tests/conftest.py`, `test_agent_free_brain.py`, `test_service_token.py`, `test_telegram_diag_endpoint.py`, `test_dashboard_cache.py`, `test_gh_brain_failover.py`, `PreflightReport`, `RuntimeHealth`, `_get`, `captured`, `test_v4_api.py`, `UsageEvent`, `_redact_for_notification`, `client`, `TestMongoGate`?**
  _High betweenness centrality (0.106) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `TaskSpec`, `StuckDetector`, `E2BAdapter`, `workflow_orchestrator.py`, `proxy.py`, `test_autonomous_agency_e2e.py`, `ContextPruner`, `test_backend_server_features.py`, `test_governance_api.py`, `ExecutionRequest`, `test_daily_automation_2026_07_11.py`, `test_agent_chat_integration.py`, `GitHubTools`, `test_ceo_micromanager.py`, `test_agent_tool_governance.py`, `test_brain_failover.py`, `diagnostics.py`, `build_governance_router`, `Agency`, `WorkspaceTools`, `ReactScratchpad`, `MultiAgentSwarm`, `loop.py`, `direct_chat.py`, `PolicyEngine`, `E2BSandboxSession`, `test_agent_free_brain.py`, `test_failover_client_shared.py`, `FreeBuffAgent`, `test_empirical_verify.py`, `TestAgentRunnerSafety`, `LocalWorkspace`, `UserMemoryStore`, `TestAgentRunnerExecution`, `BrainFailoverExhausted`, `AdaptiveHalter`, `InternalAgentAdapter`, `ContextManager`, `AgentSessionStore`, `backend/server.py`, `test_p0_roadmap_b1_c2_a3.py`, `MCPClient`, `CEODispatcher`, `TestAgentLoopMCPIntegration`, `TokenBudget`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `get_workflow_orchestrator()` connect `get_workflow_orchestrator` to `post`, `workflow_orchestrator.py`, `_scheduler_on_fire`, `backend/server.py`, `get_db`, `admin_digest_router.py`, `_answer_callback`, `OrchestratorQueue`, `ExecutionRequest`, `admin_update_task_router.py`, `telegram_bot.py`, `OrchestratorCheckpointStore`?**
...
- **Are the 61 inferred relationships involving `AgentRunner` (e.g. with `MultiAgentSwarm` and `AdaptiveHalter`) actually correct?**
  _`AgentRunner` has 61 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 67 inferred relationships involving `ProviderRouter` (e.g. with `_build_provider_router()` and `NvidiaProvider`) actually correct?**
  _`ProviderRouter` has 67 INFERRED edges - model-reasoned connections that need verification._