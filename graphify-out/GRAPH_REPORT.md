...
## Corpus Check
- 1420 files · ~2,035,316 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28305 nodes · 57805 edges · 1177 communities (1045 shown, 132 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 6264 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8ae0c60e`
...
## Communities (1177 total, 132 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.01
Nodes (456): get_agency(), _gh_token(), set_agency(), get_harness_adapter(), admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message() (+448 more)

### Community 1 - "ai/router.py"
Cohesion: 0.05
Nodes (51): _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _is_model_dead(), _mark_model_dead(), _normalized_provider_type() (+43 more)

### Community 2 - "company_api.py"
Cohesion: 0.06
Nodes (131): AccountLifecycleResponse, _DoctorCheck, _DoctorReport, OnboardingAnswersRequest, OnboardingProgressResponse, OnboardingQuestionsRequest, BaseModel, Company Graph API Router Provides all endpoints for managing companies, their… (+123 more)

### Community 3 - "proxy.py"
Cohesion: 0.02
Nodes (359): BackgroundAgent, BackgroundTask, _now(), Any, agent/background.py — Background Agent An always-on worker thread that…, Enqueue *task* for processing. Returns the task (with task_id set)., Convenience: create a task and submit it in one call., Real handler — dispatches through AgentRunner when available. HARDENED (PR… (+351 more)

### Community 4 - "ExecutionRequest"
Cohesion: 0.01
Nodes (239): Called by APScheduler when a cron fires. Dispatches to the orchestrator., _scheduler_on_fire(), get_orchestrator_checkpoint_store(), _NoopDB, OrchestratorCheckpointStore, Any, services/orchestrator_checkpoint.py — Durable step-level checkpointing Issue…, Restore in-flight runs at startup. Called during backend bootstrap. Returns a… (+231 more)
...
Cohesion: 0.02
Nodes (238): AgentJobRequest, AgentJobSnapshot, BaseModel, Complete point-in-time view of a job, safe to serialise as API response., Convenience: the canonical text response, whether success or failure., Validated input for creating a new agent job. Passed from the API handler into…, DirectChatDoctor, AgentJobManager (+230 more)

### Community 9 - "gateway.py"
Cohesion: 0.03
Nodes (94): AlertHandler, BudgetTracker, Counter, _Dimensions, get_budget(), _month(), Any, packages/llm/budget.py — token and cost accounting with spend alerts. Tracks… (+86 more)
...
### Community 17 - "get_company_graph_store"
Cohesion: 0.03
...
### Community 19 - "api.js"
Cohesion: 0.02
Nodes (49): approveGovernanceRequest(), approveTaskCheckpoint(), approveTaskExecution(), autoRecommendCompanySkills(), createMcpServer(), createSprint(), delegateSeoFindings(), deleteCompany() (+41 more)
...
Cohesion: 0.02
Nodes (52): Delete a company and all its associated data., Delete a company and all associated data from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON blob (which…, Create a new website in SQLite. The full model is stored in ``data`` so scan…, Get a website by ID from SQLite., Update a website in SQLite. When ``company_id`` is omitted the existing company…, Count total companies in the store. (+44 more)
...
Cohesion: 0.04
Nodes (76): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, all_provider_ids(), _provider_ids_from_literal(), Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, CatalogActiveBrain, CatalogMirror (+68 more)

### Community 31 - "test_llm_router_resilience.py"
Cohesion: 0.03
Nodes (61): Backoff policy for retryable failures., RetryConfig, _digest(), KeyRing, KeyState, Per-provider round-robin key selection with cooldowns., Return the next healthy key and its index. Returns ``(None, -1)`` when every…, Cool a key down and return the cooldown length in seconds. Consecutive limits… (+53 more)
...
### Community 33 - "Task"
Cohesion: 0.04
Nodes (52): ApprovalCheckpoint, _coerce_ts(), ExecutionLogEntry, Any, field_validator, Full task/issue document., Update the updated_at timestamp., Coerce ISO-8601 datetime strings (from DB) to float timestamps. (+44 more)
...
Cohesion: 0.03
Nodes (115): _brain_provider_status(), get_brain_policy_route(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the active brain config + per-provider key-present flags. The response…, BrainConfig, BrainConfigStore, default_brain_config(), get_brain_config() (+107 more)
...
Cohesion: 0.09
Nodes (50): _bootstrap(), Path, ProviderManager, WorkspaceManager, tests/test_webui_provider_priority.py — Priority + reorder + brain-policy…, The /policy/brain endpoint must return the resolved brain + the paid policy…, The /providers/role-tags endpoint surfaces brain/sub/fallback roles consistent…, Reset the brain_config + brain_policy singletons before each test. V2.0 Phase 2… (+42 more)

### Community 40 - "diagnostics.py"
Cohesion: 0.09
Nodes (36): _check_background_liveness(), _check_ci_parity(), _check_company_graph(), _check_disk(), _check_event_log_integrity(), _check_feature_matrix(), _check_github_readiness(), _check_ollama() (+28 more)

### Community 41 - "test_telegram_approval_gate.py"
Cohesion: 0.08
Nodes (22): Path, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess)., Run the bot with stop-event awareness., Send a Telegram message with an inline keyboard to all configured chats., Manages the Telegram bot as a managed service alongside ollama/proxy/tunnel.…, TelegramBotManager (+14 more)
...
Cohesion: 0.01
Nodes (217): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, get_enrichment(), agent/harness_enrichment.py — Automatic Harness Enrichment for Agent Prompts…, Return the enrichment instance for a workspace. Keyed by workspace root rather…, Agent subsystem — planner / executor / verifier loop., AgentPhaseError, AgentRunner (+209 more)
...
Cohesion: 0.13
Nodes (26): _api_key(), _auth_headers(), create_wiki_page(), fetch_and_store(), AsyncClient, Create (or silently skip if duplicate title) a Wiki page., Push trend alerts into Sources + create a Wiki digest page. If *alerts* is…, Ingest a URL into the Sources database so it becomes RAG-searchable. Returns… (+18 more)

### Community 50 - "CEOSupervisor"
Cohesion: 0.06
Nodes (35): get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., reset_ceo_dispatcher(), get_ceo_ledger(), Return the shared :class:`CEOLedger`, constructing it on first use., CEOSupervisor, _env_flag() (+27 more)
...
Cohesion: 0.04
Nodes (74): get_self_healing_agent(), heal_signature(), HealingEvent, _now(), Any, agent/self_healing.py — Self-Healing Agent (closed-loop, Autonomy Charter G2)…, Translate external failure signals into improvement tasks and verify the fix…, Launch the background sweeper that resolves quiet verifying heals. (+66 more)
...
Cohesion: 0.05
Nodes (48): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Re-queue BLOCKED tasks that have cooled down and are ready for retry., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, _make_task(), asyncio (+40 more)

### Community 56 - "direct_chat.py"
Cohesion: 0.02
Nodes (139): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), PreflightIssue, PreflightReport, Any, BaseModel (+131 more)

### Community 57 - "fmtErr"
Cohesion: 0.04
Nodes (68): createAgent(), deleteAgent(), fmtErr(), getActivity(), getCostAttribution(), getDecisionLog(), getDueSoonTasks(), getSavings() (+60 more)
...
### Community 65 - "llm/router.py"
Cohesion: 0.05
Nodes (73): auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable., Persist a provider as disabled, through the store that already owns it. (+65 more)
...
### Community 69 - "test_crispy_workflow.py"
Cohesion: 0.07
...
Cohesion: 0.09
Nodes (29): get_workspace_manager(), _hash_component(), _iso_now(), _iso_offset_hours(), _load_workspace(), _parse_iso(), BaseModel, Enum (+21 more)
...
Cohesion: 0.05
Nodes (26): PortfolioManager, PortfolioMetrics, Aggregate metrics across the whole portfolio., Manages a portfolio of initiatives with WSJF prioritisation and roadmapping., Create and register a new initiative, returning it., Add a pre-built Initiative (e.g. from the intelligence layer)., Remove an initiative from the portfolio., Look up an initiative by ID. (+18 more)
...
Cohesion: 0.07
Nodes (35): Initiative, _bug_scores(), _clean(), _default_repo(), _env_github_token(), estimate_job_size(), fetch_github_signals(), initiatives_from_bug_log() (+27 more)

### Community 80 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (84): ApprovalStatus, str, BudgetExceeded, BudgetTracker, classify(), evaluate_call(), get_gate(), GovernanceGate (+76 more)

### Community 81 - "ChatPage.js"
Cohesion: 0.06
Nodes (51): cancelAgentChatJob(), chatSend(), createQuickNote(), createSchedule(), createTask(), deleteSession(), getAgentChatJob(), getGithubStatus (+43 more)

### Community 82 - "test_render_mcp.py"
Cohesion: 0.05
Nodes (40): agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, _as_list(), _coerce_payload(), Any, RuntimeError, packages/integrations/render_mcp.py — Render platform access over MCP. The…, Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return… (+32 more)
...
Cohesion: 0.04
Nodes (68): get_store(), Return the active store singleton, creating it on first call., apply_from_db(), Load the stored overrides and apply them. Called once at startup., Reset the singleton (for tests)., reset_failover_manager(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task() (+60 more)
...
Cohesion: 0.05
Nodes (34): API, changeUserRole(), createApiKey(), deleteApiKey(), pauseSchedule(), resumeSchedule(), setUserOnboarding(), triggerSchedule() (+26 more)
...
### Community 96 - "DashboardLayout.js"
Cohesion: 0.04
...
Cohesion: 0.05
Nodes (57): get_improvement_loop(), Any, Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., get_trend_watcher(), agent/trend_watcher.py — Internet-connected AI trend intelligence. Fetches from…, set_trend_watcher(), backend/ceo_router.py — observability and manual control for the CEO. Surfaces… (+49 more)
...
Cohesion: 0.04
Nodes (60): Status of a user story within a sprint., StoryStatus, MemoryKind, What kind of memory artifact this is., Enum, SuperClaude Workflow Engine — Workflow, Task, and topological DAG execution.…, Return tasks whose dependencies are all satisfied., Number of tasks in the workflow. (+52 more)
...
Cohesion: 0.04
Nodes (73): _is_brain_connection_error(), Any, BaseException, Task, TaskResult, TaskSpec, TaskStatus, Best-effort label of who will run this task once approved. Returns… (+65 more)
...
Cohesion: 0.07
Nodes (51): BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), _norm(), _pick_from_records(), Any (+43 more)
...
Cohesion: 0.07
Nodes (36): build_spec_router(), Any, APIRouter, backend/spec_router.py — review/approve persisted plan specifications. Surfaces…, await_spec_approval(), _db(), _flag(), get_spec() (+28 more)
...
Cohesion: 0.08
Nodes (26): _models_to_try(), Order the models to attempt on *provider*, correcting a stale catalogue. Cache-…, disabled(), _mock_get(), _ok(), asyncio, A stale model catalogue must not be mistaken for a dead account.…, None means "could not find out" and must not read as "no models". (+18 more)
...
Cohesion: 0.09
Nodes (29): _Circuit, _enabled(), _failure_threshold(), OllamaCircuitBreaker, Record a successful response; close the circuit., Record a 5xx error; open the circuit after threshold is reached., Reset circuit state (used in tests and admin operations)., Return the current state string (CLOSED/OPEN/HALF_OPEN). (+21 more)

### Community 152 - "telegram_bot.py"
Cohesion: 0.08
Nodes (51): Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost(), cmd_keylist() (+43 more)

### Community 153 - "_fixture"
Cohesion: 0.01
Nodes (98): parse_event_stream(), Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.… (+90 more)
...
Cohesion: 0.08
Nodes (10): FeatureMatrix, Central support matrix — single source of truth. Loads the canonical feature…, Render the matrix as a Markdown table for docs., TestConfigOverrides, TestEnforcement, TestRegistryLoads, All features come from _CANONICAL_FEATURES., TestAdminVisibility (+2 more)

### Community 165 - "RuntimesPage.js"
Cohesion: 0.13
...
Cohesion: 0.05
Nodes (52): _build_anthropic_response(), _emit_safely(), _finish_reason_to_stop_reason(), get_local_model(), handle_anthropic_messages(), _messages_to_openai(), _openai_choice_to_anthropic_content(), _post_anthropic_with_fallback() (+44 more)
...
Cohesion: 0.10
Nodes (13): _MemCollection, _MemCursor, _MemDB, _MemDeleteResult, Any, services/scheduler_store.py — Durable scheduler persistence. Issue #505:…, Delete a persisted job., Return the total number of persisted jobs. (+5 more)
...
### Community 198 - "mcp_registry.py"
Cohesion: 0.07
Nodes (41): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _playwright_configured(), _probe_http(), _probe_internal() (+33 more)
...
### Community 201 - "TestRouterIntegration"
Cohesion: 0.31
...
### Community 227 - "scheduler.py"
Cohesion: 0.09
Nodes (20): _age_seconds(), _now(), agent/scheduler.py — Scheduled Agent Jobs Cron-based job scheduler. Each job…, Register a new job. Returns the created :class:`ScheduledJob`.…, Fire a job immediately (webhook / manual trigger)., Remove a job. Returns *True* if it existed., Update the display name of a job., Enable or disable a job without deleting it. (+12 more)

### Community 228 - "GoalRecord"
Cohesion: 0.08
Nodes (24): _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One piece of work the CEO has taken ownership of. ``state`` is the supervisor's…, Return ``(done, total)`` subtask counts. (+16 more)

### Community 229 - "TrainingSample"
Cohesion: 0.10
...
### Community 240 - "test_watchdog.py"
Cohesion: 0.09
...
Cohesion: 0.04
Nodes (32): _env_int(), _get_settings(), Read an int env var, falling back to *default* on a missing/bad value. Never…, When True, the governance layer evaluates and audits agent actions. This is…, Typed configuration loaded from environment variables., When True, approval-gated actions self-approve. Local dev only., ``RENDER_SERVICE_IDS`` split into a clean list (empty when unset)., True when there is both an API key and an endpoint to reach. (+24 more)
...
### Community 259 - "test_skill_executors_live.py"
Cohesion: 0.16
...
### Community 267 - "openclaw_gateway.py"
Cohesion: 0.15
...
Cohesion: 0.09
Nodes (30): free_nvidia_models(), Return the curated list of free NVIDIA NIM models FreeBuff may use., True when *model* is in the curated free NVIDIA NIM set., _enforce_rate_limit(), _is_freebuff_unlimited(), is_rate_limit_exempt(), True when this key is on the FreeBuff rate-limit exemption allowlist. Only…, True when this request targets a FreeBuff route and should skip rate limiting.… (+22 more)

### Community 272 - "telegram_service.py"
Cohesion: 0.18
...
Cohesion: 0.14
Nodes (25): authorizeGithubRepos(), createGithubPR(), deleteGithubToken(), getGithubTree(), getPlatformInfo(), githubStatus(), healthCheck(), listGithubBranches() (+17 more)
...
### Community 327 - "portfolio_api.py"
Cohesion: 0.09
...
### Community 339 - "Artifact"
Cohesion: 0.06
Nodes (40): AgentRole, Run a pre-gate or report phase through the correct agent. Enforces permission…, Execute a slice via the Coder agent (write-permitted)., Review a slice via the Reviewer agent (different model from Coder). This is the…, Run verification commands via the Verifier agent. The Verifier is execution-…, Return a copy of *routing* with the profile's model injected for its role., When coder == reviewer model, swarm should log a warning., tests/test_workflow_models.py — Unit tests for workflow/models.py. (+32 more)
...
### Community 351 - "SyntheticDataPipeline"
Cohesion: 0.17
...
Cohesion: 0.06
Nodes (60): Decision, str, A control surface — the kind of thing being governed. Mirrors Docker AI…, The verdict for one governed action., Surface, _blocked_result(), _governance_check(), _governance_identity() (+52 more)
...
### Community 378 - "weekly_digest.py"
Cohesion: 0.13
...
### Community 397 - "self_heal_brain_and_unblock_tasks"
Cohesion: 0.10
...
### Community 454 - "test_agency.py"
Cohesion: 0.14
Nodes (32): _parse_ceo_directives(), set_improvement_loop(), agency(), asyncio, Path, Tests for agent/agency.py, With no issues, CEO should report nominal., CEO should parse valid JSON directive list from LLM response. (+24 more)
...
### Community 458 - "AnthropicProvider"
Cohesion: 0.12
...
### Community 472 - "traffic_director.py"
Cohesion: 0.13
...
### Community 476 - "ai/self_heal.py"
Cohesion: 0.10
Nodes (18): packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, tests/test_langfuse_agency_wide.py — tests for PR #961 agency-wide Langfuse.…, langfuse_obs.py must define emit_agency_observation., emit_agency_observation must be a no-op when Langfuse is not configured., tasks/service.py must call emit_agency_observation for task execution., agent/agency.py must call emit_agency_observation for CEO directives., agent/sam.py must call emit_agency_observation for voice commands., backend/server.py scheduler_tick must call emit_agency_observation. (+10 more)
...
Cohesion: 0.10
Nodes (17): List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., _fb_models(), _fb_run(), Return the free model list (embedded or via proxy)., Execute a FreeBuff task (embedded or via proxy). Shape: {result: {...}}., Tests for the always-on FreeBuff Telegram bot: embedded vs HTTP dispatch., In orchestrator mode the embedded run must set the bypass so the agent runs., Snapshot/restore os.environ — the launcher writes env vars directly. (+9 more)
...
### Community 482 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
### Community 536 - "WorkspaceError"
Cohesion: 0.08
Nodes (14): _get_workspace_lock(), Any, Exception, Lock, Open a workspace for resumption. Only READY or PAUSED workspaces may be…, Raise :exc:`WorkspaceAccessDeniedError` if *requesting_session_id* doesn't own…, Acquire the exclusive async lock for *ws*. Raises :exc:`WorkspaceLockError` if…, Return the process-wide asyncio.Lock for *root*, creating it if needed. (+6 more)
...
### Community 549 - "TestModelsEndpointAliases"
Cohesion: 0.31
...
### Community 550 - "test_harness_spec.py"
Cohesion: 0.15
Nodes (12): _flag(), propose_entries(), Turn qualifying lessons into candidate entries. A lesson qualifies only when it…, Promote repeated lessons into the spec. Returns the entries added. No-op unless…, refine(), _lesson(), tests/test_harness_spec.py — the Continual Harness spec. Covers the property…, Treat every citation in the spec file as recorded. These tests cover block… (+4 more)
...
### Community 574 - "test_ceo_router.py"
Cohesion: 0.16
...
### Community 585 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.40
...
### Community 587 - "_mock_provider_records"
Cohesion: 0.11
...
### Community 589 - "_status_snapshot"
Cohesion: 0.31
...
### Community 626 - "._run_git_command"
Cohesion: 0.20
...
### Community 716 - "._scan_github_repo"
Cohesion: 0.10
...
### Community 722 - "test_telegram_task_callback.py"
Cohesion: 0.13
...
### Community 728 - "knowledge_sync.py"
Cohesion: 0.16
Nodes (13): get_knowledge_sync(), KnowledgeSync, Any, agent/knowledge_sync.py — Wiki + Sources knowledge ingestion pipeline. Bridges…, Cron entry-point: fetch fresh trends then sync everything to Wiki+Sources., Lightweight handle used as a singleton so proxy.py can reference it., run_weekly_sync(), set_knowledge_sync() (+5 more)
...
### Community 733 - "TestSupportMatrixDocsSync"
Cohesion: 0.29
...
### Community 738 - "sam_livekit_worker.py"
Cohesion: 0.17
...
### Community 764 - "model_discovery.py"
Cohesion: 0.09
Nodes (26): Return the discovered model list for *provider_id*, or ``[]`` if unknown., _served_models(), cached_models(), discover_models(), _fresh_entry(), _models_url(), _parse_ids(), Any (+18 more)
...
### Community 766 - "test_agent_tools.py"
Cohesion: 0.43
...
### Community 804 - "_FakeSQLiteStore"
Cohesion: 0.18
...
### Community 855 - "test_direct_adapter_does_not_bypass"
Cohesion: 0.67
...
### Community 857 - "get_optional_user"
Cohesion: 0.19
...
### Community 858 - "test_feature_stores_are_wired_on_demand_not_eagerly"
Cohesion: 0.50
...
### Community 862 - "test_docs_consistency.py"
Cohesion: 0.17
...
### Community 865 - "get_livekit_config"
Cohesion: 0.18
...
### Community 873 - "._check_permissions"
Cohesion: 0.29
...
### Community 902 - "FakeScheduleCollection"
Cohesion: 0.25
...
### Community 906 - "TestParseToolCalls"
Cohesion: 0.39
...
### Community 935 - "start_in_process"
Cohesion: 0.25
...
### Community 937 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 971 - "_build_digest_markdown"
Cohesion: 0.33
Nodes (7): _build_digest_markdown(), _now_iso(), Render a markdown digest from a list of TrendAlert dicts., _make_alert(), test_build_digest_includes_high_relevance(), test_build_digest_includes_low_relevance_section(), test_build_digest_no_alerts()
...
### Community 992 - "security_fix_agent.py"
Cohesion: 0.57
...
Cohesion: 0.05
Nodes (50): detect_secrets(), main(), Recover CHANGELOG.md from a Git merge conflict in its [Unreleased] block. Pre-…, Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, Any (+42 more)
...
### Community 999 - "TestCitationBinding"
Cohesion: 0.47
...
### Community 1006 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 1023 - "test_p0_roadmap_b3_b4_b5.py"
Cohesion: 0.18
...
### Community 1041 - "test_mint_token_rejects_missing_args"
Cohesion: 0.67
...
### Community 1062 - "_resolve_default_executor_model"
Cohesion: 0.50
Nodes (3): Any, Resolve the default executor model via the catalog (UNIT 7). Was hardcoded to…, _resolve_default_executor_model()
...
- **Why does `_fixture()` connect `_fixture` to `ai/router.py`, `proxy.py`, `ExecutionRequest`, `cost_tracker.py`, `test_scheduler_hydration_bounded.py`, `gateway.py`, `AgentScheduler`, `test_llm_router_strategies.py`, `test_colibri_brain_shim.py`, `test_llm_router_tpm.py`, `SeoAuditRequest`, `OrchestratorQueue`, `clear_cooldowns`, `config.py`, `test_unit8_model_catalog.py`, `test_llm_router_resilience.py`, `Task`, `test_openclaw_gateway.py`, `brain_config.py`, `CompanyGraphService`, `test_harness_spec.py`, `KeyPool`, `webui/router.py`, `test_telegram_approval_gate.py`, `failover_chat_completion`, `SecretRecord`, `AgentRunner`, `test_sam_livekit.py`, `test_model_router.py`, `test_new_features_e2e.py`, `MCPUnavailableError`, `test_knowledge_sync.py`, `CEOSupervisor`, `test_operational_incidents.py`, `SelfHealingAgent`, `FailureCategory`, `direct_chat.py`, `MCPClient`, `test_ai_insights.py`, `ResearchTask`, `llm/router.py`, `test_crispy_workflow.py`, `TestClient`, `test_task_clarification.py`, `resolve_e2b_config`, `test_governance_enforcement.py`, `test_sam_voice.py`, `get_task_store`, `SeoFixer`, `ImprovementLoop`, `TaskIn`, `test_context_rulebook.py`, `test_agile_api.py`, `test_app_settings.py`, `ArtifactStore`, `services/background.py`, `analyze_page`, `KeyStore`, `test_kimi_bridge_server.py`, `BrainWatchdog`, `ChatHistoryStore`, `TaskStore`, `test_telegram_freebuff.py`, `SQLiteStore`, `ModelRouter`, `resolve_active_brain`, `_start_ceo_agency`, `PrimeAgentAdapter`, `tests/conftest.py`, `persist_plan_spec`, `_StubProvider`, `test_e2b_data_flow.py`, `test_llm_router_queue_cache.py`, `E2BAdapter`, `TrendWatcher`, `test_features_api.py`, `test_video_transcript.py`, `test_portfolio_intake.py`, `anthropic_compat.py`, `resolve_identity`, `ProceduralMemoryStore`, `test_agent_free_brain.py`, `_resolve_push_token`, `SchedulerStore`, `TestClient`, `ContextPruner`, `AgentSwarm`, `TestEstimateTokensForMessages`, `test_trend_watcher.py`, `test_telegram_task_callback.py`, `test_persistent_memory.py`, `SecurityScanner`, `test_provider_state_durability.py`, `test_startup_warmup.py`, `test_control_plane_api.py`, `test_llm_router_e2e.py`, `GoalRecord`, `WorkflowBuildRequest`, `model_discovery.py`, `client`, `IssueCategory`, `test_purge_backlog.py`, `test_ping.py`, `TestMongoGate`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `test_rate_limiter.py`, `test_platform_controls.py`, `isolated_telegram_config`, `test_e2b_task_wiring.py`, `test_telegram_auto_approve.py`, `_resolve_user_github_token`, `test_telegram_approval_e2e.py`, `test_service_token.py`, `timedelta`, `test_agency_fix.py`, `get_optional_user`, `ClaudeCodeAdapter`, `test_monitor_lib.py`, `test_output_filter.py`, `test_v4_api.py`, `test_dashboard_cache.py`, `._check_permissions`, `allow_paid`, `seo_api.py`, `test_voice_pipeline.py`, `TestNormalizeResponseFormat`, `test_brain_patch_service_token.py`, `test_company_api.py`, `_runner`, `claim`, `NotificationDispatcher`, `test_agency.py`, `test_task_brain_preflight.py`, `test_openclaw_endpoints.py`, `test_task_service_failed_comment.py`, `traffic_director.py`, `test_doctor_service_token_check.py`, `test_freebuff_bot.py`, `test_autonomy_status.py`, `AuditLog`, `test_local_brain_state.py`, `TestFeaturesAPI`, `TestChatHistoryStore`, `test_phase5_doctor.py`, `test_telegram_diag_endpoint.py`, `test_mcp_governance.py`, `e2e/test_browser.py`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `proxy.py`, `ExecutionRequest`, `_StubManager`, `StuckDetector`, `AgentScheduler`, `_build_request`, `TestMCPServer`, `CEODispatcher`, `TaskSpec`, `Agency`, `AutonomyTracker`, `.execute`, `TestWorkspace`, `AdaptiveHalter`, `E2BAdapter`, `failover_chat_completion`, `MCPUnavailableError`, `LocalWorkspace`, `direct_chat.py`, `test_agent_free_brain.py`, `MCPClient`, `test_empirical_verify.py`, `_runner`, `ContextPruner`, `ContextManager`, `GitHubTools`, `get_task_store`, `MultiAgentSwarm`, `TestZeroAttemptDiagnostics`, `test_governance_api.py`, `TestAgentLoopMCPIntegration`, `TokenBudget`, `WorkspaceTools`, `ReactScratchpad`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `TaskStore` connect `TaskStore` to `backend/server.py`, `AgentScheduler`, `test_trend_scoping.py`, `tasks/service.py`, `Agency`, `_fixture`, `test_phase6_workflow.py`, `Task`, `test_portfolio_intake.py`, `test_task_source_id_race.py`, `TaskDispatcher`, `_ts_to_float`, `test_task_brain_preflight.py`, `test_tasks_awaiting_approval_api.py`, `test_issue_intake.py`, `test_telegram_task_callback.py`, `get_task_store`, `tasks/api.py`, `ai/self_heal.py`, `test_sqlite_store.py`, `services/background.py`?**
