...
## Corpus Check
- 1441 files · ~2,088,179 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 29285 nodes · 54333 edges · 1241 communities (1124 shown, 117 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2672 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b80588c3`
...
## Communities (1241 total, 117 thin omitted)

### Community 0 - "MongoDBStore"
Cohesion: 0.02
Nodes (97): CompanyGraphSnapshot, KnowledgeItem, A point-in-time snapshot of a Company Graph for history and rollback., Structured knowledge about the company, systems, or processes., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo (+89 more)

### Community 1 - "proxy.py"
Cohesion: 0.03
Nodes (161): get_sam(), middleware, admin_control(), admin_delete_user(), admin_login(), admin_logout(), admin_rotate_user(), admin_status() (+153 more)
...
Cohesion: 0.03
Nodes (73): _apply_activity_status(), create_agent(), delete_agent(), get_agent(), _get_user(), list_agents(), list_runtime_agents(), Any (+65 more)

### Community 8 - "Usage"
Cohesion: 0.05
Nodes (53): packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC, Any, AsyncClient, ProviderConfig (+45 more)

### Community 9 - "TaskSpec"
Cohesion: 0.04
Nodes (84): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns…, runtimes/adapters/e2b.py — E2B Firecracker micro-VM runtime adapter. Routes…, runtimes/adapters/goose.py — Goose adapter (TIER 2). Goose…, runtimes/adapters/hermes.py — Hermes Agent adapter (FIRST CLASS). Hermes Agent…, Internal runtime adapter that executes tasks via the built-in AgentRunner.… (+76 more)
...
Cohesion: 0.03
Nodes (104): BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), provider_base_url() (+96 more)

### Community 12 - "LLMRequest"
Cohesion: 0.04
Nodes (101): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, build_provider(), ProviderConfig, Instantiate the adapter for one provider config. Unknown kinds fall back to the…, LLMRequest, Cheap character-based token estimate (~4 chars/token). Deliberately dependency-…, A provider-neutral chat request. ``model`` is a *hint*: the router may… (+93 more)
...
Cohesion: 0.05
Nodes (32): _audit_proposal(), diff_policy(), _GitHub, parse_policy_text(), _pr_body(), _proposal_branch_name(), propose_policy_change(), Any (+24 more)
...
Cohesion: 0.03
Nodes (122): _load(), main(), int, FailoverResult, A successful completion plus the accounting its callers need., get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker. (+114 more)
...
Cohesion: 0.04
Nodes (84): _merge_changed_files(), Collect changed_files across all specialists into a single de-duped list., ClassifyOutput, get_ceo_fallback_stats(), PlanOutput, BaseModel, CLASSIFY phase: domain and task type determination., PLAN phase: structured execution plan. (+76 more)
...
### Community 29 - "_load_local_metrics_since"
Cohesion: 0.43
...
Cohesion: 0.07
Nodes (59): _audit_dispatch(), _governance_check(), _governance_identity(), Any, TaskResult, TaskSpec, Evaluate a runtime dispatch against policy; audit it either way. Returns…, Full record of a routing decision — audit-trail entry. (+51 more)

### Community 31 - "company_api.py"
Cohesion: 0.02
Nodes (164): cancel_onboarding(), create_company(), generate_onboarding_questions(), get_company_access(), get_company_graph(), _get_current_user_thunk(), _get_fallback_questions(), get_onboarding_progress() (+156 more)

### Community 32 - "AgentRunner"
Cohesion: 0.02
Nodes (129): AgentPhaseError, AgentRunner, _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), Any, Exception (+121 more)

### Community 33 - "CompanyGraphStore"
Cohesion: 0.03
Nodes (29): requires_db, Initialize the service. Args: store: Optional CompanyGraphStore instance. If…, CompanyGraphStore, Delete a company and all its associated data., List companies with optional filtering., Count total companies in the store., Delete a company graph., Create a snapshot of a company graph. (+21 more)
...
### Community 39 - "Task"
Cohesion: 0.04
Nodes (79): Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), Task, Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, ApprovalCheckpoint, Full task/issue document., Human approval gate in a task's execution. (+71 more)
...
Cohesion: 0.06
Nodes (45): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), ExecutionRequest, ExecutionResult, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY… (+37 more)
...
Cohesion: 0.04
Nodes (74): MCPUnavailableError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Raised when the MCP server is unreachable or the circuit is open., packages/integrations/render_mcp.py — Render platform access over MCP. The…, Drop the cached singleton. Used by tests and after a config change., reset_render_mcp(), E2BSandboxSession, _inject_token() (+66 more)
...
Cohesion: 0.04
Nodes (38): A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, SystemType, Add a specialist to a company. Args: company_id: Company ID name: Specialist…, Get specialists that can handle a task with given capabilities. Args:…, Any, SpecialistFamily (+30 more)

### Community 65 - "ProviderConfig"
Cohesion: 0.05
Nodes (64): _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog(), _ollama_reasoning_effort() (+56 more)
...
Cohesion: 0.05
Nodes (36): _as_list(), _coerce_payload(), Any, RuntimeError, Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return…, Unwrap a nested envelope such as ``{"service": {...}}`` when present., Typed facade over the Render MCP server's tools. Every method returns plain… (+28 more)
...
Cohesion: 0.08
Nodes (54): Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _answer_callback(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost() (+46 more)
...
Cohesion: 0.09
Nodes (16): build_react_prompt(), parse_react_response(), Any, Parse a ReAct-format response into structured components. Intended caller:…, Structured scratchpad that accumulates across tool calls within a step. Each…, Record a reasoning step before taking action., Record a tool call action., Record the result of a tool call. (+8 more)
...
### Community 149 - "call_llm"
Cohesion: 0.03
Nodes (63): _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), _build_auto_skill_guidance(), _build_direct_chat_schedule_suggestion(), _build_direct_chat_tags(), _build_direct_chat_task_suggestion(), _build_provider_router() (+55 more)
...
Cohesion: 0.10
Nodes (36): generate_standup(), Build a :class:`StandupReport` from ``.claude/state/active-tasks.md``. Reads…, InitiativeStatus, _bug_scores(), _clean(), _default_repo(), estimate_job_size(), fetch_github_signals() (+28 more)
...
Cohesion: 0.04
Nodes (67): ApprovalRequest, ApprovalStore, Any, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`…, Register a pending approval and return it. Arguments are scrubbed on the way…, Drop the oldest resolved requests once over capacity. Resolved requests go…, Approve or deny a pending request. Returns ``None`` for an unknown id. A…, Block until *approval_id* is resolved or its TTL expires. Combines an event… (+59 more)
...
Cohesion: 0.08
Nodes (30): _disable_unless_key_serves_other_models(), _models_to_try(), Order the models to attempt on *provider*, correcting a stale catalogue. Cache-…, Auto-disable *provider*, unless its key demonstrably serves other models. "No…, Drop every cached list and attempt record. For tests, and for an explicit…, reset_cache(), _clear_cache(), _mock_get() (+22 more)
...
Cohesion: 0.09
Nodes (30): PreflightReport, AgentPlan, AgentStep, _fake_user(), _FakeChatResult, _FakeResponse, Path, test_agent_mode_github_preflight_missing_token() (+22 more)

### Community 180 - "test_governance_api.py"
Cohesion: 0.07
Nodes (48): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), ApprovalStatus, get_approval_store() (+40 more)
...
Cohesion: 0.05
Nodes (66): brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, auto_disable(), _billing_signals(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable. (+58 more)
...
### Community 201 - "runtimes/manager.py"
Cohesion: 0.05
...
### Community 204 - "agent/models.py"
Cohesion: 0.10
Nodes (23): Agent subsystem — planner / executor / verifier loop., AgentRunRequest, AgentSession, AgentSessionCreateRequest, _known_tool_names(), Any, BaseModel, field_validator (+15 more)
...
### Community 207 - "test_colibri_brain_shim.py"
Cohesion: 0.08
...
Cohesion: 0.11
Nodes (32): delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), BaseModel, get, post, SEO / GEO / AIO Audit API Router Endpoints for the world-class SEO audit engine… (+24 more)
...
Cohesion: 0.04
Nodes (53): filter_output(), FilterResult, get_savings_summary(), OutputFilter, Any, agent/output_filter.py — LLM Output Compression & Token Savings Inspired by…, Filter and compress command outputs to reduce LLM token consumption. Provides…, Compact git status output — keep only changed file paths. (+45 more)
...
Cohesion: 0.07
Nodes (35): auto_recommend_skills(), get_company(), get_skill(), get_specialist_bound_skills(), list_companies(), list_skills(), list_specialists(), get (+27 more)
...
### Community 289 - "TestTypedContracts"
Cohesion: 0.09
Nodes (24): BoundContext, JudgeVerdict, MonitorOutput, PersistOutput, PreflightReport, Run doctor checks before execution., Resolve and bind skills, memory, and Company Graph context., Log KPIs for autonomous operation tracking. (+16 more)

### Community 290 - "test_sam_voice.py"
Cohesion: 0.04
Nodes (52): Any, SAM voice agent — the voice-controlled interface to the agency., Process a voice command and return SAM's spoken response. Args: text: The…, Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., Call the NVIDIA NIM LLM (free tier) for SAM's response., Rule-based fallback when the LLM is unavailable., A single voice conversation session with SAM. (+44 more)
...
Cohesion: 0.04
Nodes (88): Recursively strip secrets from an arbitrary argument structure. Fails…, scrub(), BudgetTracker, GovernanceGate, Per-session consumption counters and their ceilings. Ceilings come from the…, Return the name of the first exhausted session-wide limit, or None. Only covers…, Return a reason string if *tool* has hit its per-tool session cap. Checks only…, Holds live session budgets, bounded so it cannot leak. Sessions end without… (+80 more)
...
Cohesion: 0.05
Nodes (39): get_scheduler(), agent/scheduler.py — Scheduled Agent Jobs Cron-based job scheduler. Each job…, set_scheduler(), create_schedule(), delete_schedule(), get_schedule(), get_schedule_runs(), list_schedules() (+31 more)
...
### Community 345 - "_execute_skill_impl"
Cohesion: 0.10
...
Cohesion: 0.04
Nodes (40): ChatResponse, CerebrasProvider, Provider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Provider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is… (+32 more)
...
### Community 359 - "._notify_execution_gate"
Cohesion: 0.11
Nodes (21): One-line human explanation of why this task needs human approval. Reads risk…, Best-effort label of who will run this task once approved. Returns…, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, captured(), Readability of the Telegram pre-execution approval-gate message. The original…, Issue #1072: description was truncated to 280 chars. Now it shows the full…, Description > 1500 chars: shown truncated with a 'view on dashboard' pointer. (+13 more)
...
Cohesion: 0.05
Nodes (54): get_active_brain_sync(), get_provider_role_tags(), invalidate_brain_cache(), _norm(), Any, Clear the cached brain so the next read re-resolves. Called from…, Return the cached brain read by sync callers. None if never resolved., Single source of truth for the active brain LLM. Resolution order (matches the… (+46 more)
...
Cohesion: 0.08
Nodes (14): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, TaskResult, TaskSpec, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor. (+6 more)
...
Cohesion: 0.04
Nodes (67): delete_company_endpoint(), delete, patch, Update a company's metadata. Accepts partial updates for company fields…, Delete a company and all associated data. Access control is handled by…, update_company(), autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy… (+59 more)
...
### Community 437 - "self_heal_brain_and_unblock_tasks"
Cohesion: 0.10
...
### Community 439 - "TestBrainFailoverModelUpdates"
Cohesion: 0.18
...
### Community 493 - "test_llm_router_tpm.py"
Cohesion: 0.17
...
Cohesion: 0.04
Nodes (23): fake_mongo(), _FakeCollection, _FakeDb, isolated_state(), _live_mongo_url(), Operator provider state must survive a redeploy. The per-provider kill switch…, Tests must never mutate a shared operational store., The storage layer's localhost default is a placeholder, not config. Treating it… (+15 more)
...
### Community 568 - "mint_access_token"
Cohesion: 0.14
...
### Community 577 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.37
...
### Community 604 - "make_isolated_workspace"
Cohesion: 0.33
...
### Community 605 - "test_scanner_ssl_cert.py"
Cohesion: 0.25
...
Cohesion: 0.02
Nodes (64): BusinessCategory, detect_company_id(), DirectChatSession, handle_chat_message_with_context(), Direct chat session with Company Graph context binding., Bind a company to this chat session and load its Company Graph., Bind a repository to this chat session., Get the bound Company Graph. (+56 more)
...
Cohesion: 0.08
Nodes (23): discover_models(), _fresh_entry(), _models_url(), _parse_ids(), Any, Ask a provider which models the configured API key may actually use.…, Return the model ids *provider*'s key may use, or ``None`` if unknown. ``None``…, Cache *models* under *provider_id* and return it unchanged. A successful… (+15 more)
...
### Community 641 - "flesch_reading_ease"
Cohesion: 0.22
...
Cohesion: 0.11
Nodes (36): _fake_user_auth(), Path, test_admin_can_create_anthropic_provider_via_webui_admin_api(), test_admin_can_create_provider_via_webui_admin_api(), test_ui_providers_and_workspaces_use_app_state(), _bootstrap(), Path, ProviderManager (+28 more)
...
### Community 652 - "get_orchestrator_queue"
Cohesion: 0.27
...
Cohesion: 0.09
Nodes (24): _is_safe_url(), _looks_like_bot_challenge(), BeautifulSoup, True if the HTML is an anti-bot/CAPTCHA interstitial rather than content. Used…, Render a page with a real headless browser (Playwright/Chromium) and return…, Block SSRF: reject loopback, link-local, private, and non-HTTP schemes., Fetch a builtwith.com page, defeating its bot protection as far as is possible…, Last-resort fallback for sites we can't fingerprint live (JS-rendered +… (+16 more)
...
### Community 695 - "test_scanner_e2e.py"
Cohesion: 0.27
...
### Community 723 - "Feature Support Matrix"
Cohesion: 0.33
...
### Community 786 - "workflow_orchestrator.py"
Cohesion: 0.07
Nodes (28): get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., reset_ceo_dispatcher(), _get_ceo_dispatcher(), MergeDecision, _orchestrator_bypass(), Phase (+20 more)
...
### Community 907 - "_brain_provider_status"
Cohesion: 0.33
...
### Community 997 - "brain.py"
Cohesion: 0.04
Nodes (46): livenim, allow_paid_brain(), BrainResolution, _host_is_openai_compatible(), is_anthropic_model(), _pick_from_records(), Shared free-brain policy helpers (issue #656). Single source of truth for "is…, Resolved brain LLM for the current agency configuration. ``provider_id`` is the… (+38 more)
...
Cohesion: 0.08
Nodes (30): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), AccountLifecycleResponse, _DoctorCheck, _DoctorReport, get_public_doctor_report(), _is_ephemeral_user() (+22 more)
...
### Community 1222 - "_provider_env_value"
Cohesion: 0.50
...
- **Why does `_fixture()` connect `_fixture` to `ProviderRouter`, `services/background.py`, `test_llm_router_queue_cache.py`, `TaskExecutionCoordinator`, `test_company_api.py`, `validate_policy_text`, `PolicyEngine`, `SelfHealingAgent`, `llm/router.py`, `Agency`, `PrimeAgentAdapter`, `test_provider_state_durability.py`, `CEOLedger`, `clear_cooldowns`, `company_api.py`, `TestAuthAndTaskOwnership`, `test_monitor_lib.py`, `test_openclaw_endpoints.py`, `test_unit8_model_catalog.py`, `Task`, `ImprovementLoop`, `test_model_router.py`, `test_llm_router_strategies.py`, `test_failover_client_shared.py`, `test_implement_agent_routing.py`, `test_ceo_router.py`, `_mock_provider_records`, `TaskStore`, `test_process_quick_note_workflow.py`, `TestRuntimeControl`, `AgentJobManager`, `SQLiteStore`, `ExecutionRequest`, `ChatHistoryStore`, `E2BSandboxSession`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `get_task_store`, `test_local_brain_state.py`, `NotificationDispatcher`, `test_operational_incidents.py`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `services/seo_audit.py`, `test_telegram_diag_endpoint.py`, `config.py`, `ResearchTask`, `TestClient`, `resolve_e2b_config`, `test_procedural_memory.py`, `e2e/test_browser.py`, `TokenBudget`, `provider_max_rpm`, `UserRole`, `_provider`, `test_knowledge_sync.py`, `SchedulerStore`, `test_startup_warmup.py`, `SeoFixRequest`, `KeyStore`, `claim`, `test_context_rulebook.py`, `ArtifactStore`, `run_task`, `test_webui_provider_priority.py`, `test_issue_intake.py`, `test_openclaw_gateway.py`, `test_kimi_bridge_server.py`, `test_trend_scoping.py`, `BackgroundAgent`, `TestHarnessAdapter`, `test_e2b_data_flow.py`, `MCPClient`, `test_new_features_e2e.py`, `test_agent_tool_governance.py`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `_Budget`, `_StubProvider`, `KeyPool`, `.on_task_complete`, `AgentPlan`, `OllamaCircuitBreaker`, `analyze_page`, `_resolve_user_github_token`, `test_doctor_service_token_check.py`, `brain_failover.py`, `test_features_api.py`, `test_video_transcript.py`, `test_task_brain_preflight.py`, `TrendWatcher`, `test_colibri_brain_shim.py`, `test_agile_api.py`, `test_app_settings.py`, `test_agent_scripts_share_one_model_list.py`, `test_portfolio_intake.py`, `test_trend_watcher.py`, `AgentSwarm`, `test_nvidia_model_discovery.py`, `TestClient`, `UsageEvent`, `test_brain_failover_413.py`, `test_doctor_coding_brain.py`, `ContextPruner`, `BrainWatchdog`, `test_llm_router_e2e.py`, `session_retro.py`, `WorkflowRun`, `test_telegram_service_webhook.py`, `test_daily_2026_07_27.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `OutputFilter`, `test_platform_controls.py`, `workflow_orchestrator.py`, `test_scheduler_hydration_bounded.py`, `model_router.py`, `test_rate_limiter.py`, `test_brain_failover.py`, `test_sam_voice.py`, `OrchestratorQueue`, `test_purge_backlog.py`, `cost_tracker.py`, `test_task_clarification.py`, `get_scheduler`, `test_task_service_failed_comment.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `._notify_execution_gate`, `api_keys_for`, `timedelta`, `resolve_active_brain`, `TestFeaturesAPI`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_brain_patch_service_token.py`, `test_harness_spec.py`, `test_ping.py`, `test_dashboard_cache.py`, `TestSwarmRoleRouting`, `test_regression.py`, `test_agency_fix.py`, `test_telegram_mutating_commands.py`, `test_llm_router_resilience.py`, `_get`, `test_v4_api.py`, `apply_review.py`, `client`, `test_voice_pipeline.py`, `_job_text`, `brain.py`, `TestNormalizeResponseFormat`, `test_task_store_fails_loud_in_production.py`, `test_runtimes_health_endpoint.py`, `test_llm_router_tpm.py`, `_Recorder`?**
  _High betweenness centrality (0.088) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `proxy.py`, `TaskSpec`, `LocalWorkspace`, `ReactScratchpad`, `enforcement.py`, `test_governance_sandbox.py`, `PolicyEngine`, `CEODispatcher`, `.execute`, `workflow_orchestrator.py`, `call_llm`, `BackgroundAgent`, `ContextManager`, `Agency`, `test_agent_chat_integration.py`, `MCPClient`, `test_agent_tool_governance.py`, `test_e2b_adapter.py`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `StuckDetector`, `test_daily_automation_2026_07_11.py`, `WorkspaceTools`, `AgentPlan`, `test_governance_api.py`, `AgentJobManager`, `ExecutionRequest`, `E2BSandboxSession`, `test_empirical_verify.py`, `UserMemoryStore`, `agent/models.py`, `TestAgentRunnerSafety`, `TokenBudget`, `TestAgentLoopMCPIntegration`, `brain.py`, `FreeBuffAgent`, `backend/server.py`, `InternalAgentAdapter`, `test_brain_failover_413.py`, `AdaptiveHalter`, `ContextPruner`, `direct_chat.py`, `AgentSessionStore`, `TestAgentRunnerExecution`, `WorkflowRun`, `GitHubTools`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `TaskStore` connect `TaskStore` to `test_tasks_awaiting_approval_api.py`, `test_task_brain_preflight.py`, `services/background.py`, `get_task_store`, `test_issue_intake.py`, `backend/server.py`, `Task`, `test_startup_warmup.py`, `TaskExecutionCoordinator`, `runtimes/manager.py`, `test_phase6_workflow.py`, `test_sqlite_store.py`, `test_trend_scoping.py`, `TaskDispatcher`, `test_portfolio_intake.py`, `Agency`, `test_task_source_id_race.py`, `tasks/api.py`?**
