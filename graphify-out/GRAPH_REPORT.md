...
## Corpus Check
- 1429 files · ~2,048,523 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28406 nodes · 57946 edges · 1203 communities (1076 shown, 127 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 6298 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `858cb168`
...
## Communities (1203 total, 127 thin omitted)

### Community 0 - "ExecutionRequest"
Cohesion: 0.03
Nodes (121): get_orchestrator_checkpoint_store(), _NoopDB, OrchestratorCheckpointStore, services/orchestrator_checkpoint.py — Durable step-level checkpointing Issue…, Fallback in-memory store when no DB is available., Persist orchestrator runs so they survive restarts., BoundContext, ClassifyOutput (+113 more)

### Community 1 - "AgentScheduler"
Cohesion: 0.02
Nodes (285): Agency, CEO-coordinated multi-agent agency for continuous codebase management. The CEO…, AgentJobRequest, AgentJobSnapshot, BaseModel, agent/contract.py — Typed public contract for the agent job lifecycle. Phase 1…, Complete point-in-time view of a job, safe to serialise as API response., Convenience: the canonical text response, whether success or failure. (+277 more)

### Community 2 - "backend/server.py"
Cohesion: 0.01
Nodes (553): _close_github_issue(), _fetch_github_quick_notes(), _gh_repo(), _gh_token(), agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered) Runs the…, Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, set_agency() (+545 more)
...
Cohesion: 0.02
Nodes (127): AgentRunner, _enforce_signature(), _note_phase_end(), _nvidia_api_key(), Any, Path, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin… (+119 more)

### Community 5 - "kimi_bridge_provider_config"
Cohesion: 0.10
...
Cohesion: 0.06
Nodes (131): AccountLifecycleResponse, _DoctorCheck, _DoctorReport, get_onboarding_progress(), get_public_doctor_report(), OnboardingAnswersRequest, OnboardingProgressResponse, OnboardingQuestionsRequest (+123 more)
...
### Community 10 - "get_specialist_service"
Cohesion: 0.09
Nodes (23): cancel_onboarding(), match_specialists(), pause_onboarding(), provision_specialist(), post, SystemType, Provision a new specialist for a company. Creates and provisions a specialist…, Match specialists to a task. Returns the best specialists for the given task… (+15 more)

### Community 11 - "llm/router.py"
Cohesion: 0.03
Nodes (101): _load(), main(), int, get_cache(), The process-wide cache manager., failover_chat_completion_via_router(), openai_body_from_response(), Any (+93 more)

### Community 12 - "direct_chat.py"
Cohesion: 0.04
Nodes (108): AgentRole, Enum, str, Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), ResumeRequest, AcceptedJob (+100 more)

### Community 13 - "skill_bindings.py"
Cohesion: 0.03
Nodes (85): ConsolidationPhase, MemoryKind, Enum, str, Dream Memory Consolidation — pattern consolidation across AI sessions. Inspired…, What kind of memory artifact this is., Current phase of the consolidation lifecycle., Enum (+77 more)

### Community 14 - "_fixture"
Cohesion: 0.02
Nodes (81): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., regression_base_url() (+73 more)

### Community 15 - "BrainWatchdog"
Cohesion: 0.11
Nodes (13): BrainWatchdog, Any, services/brain_watchdog.py — Brain health watchdog. Monitors the active brain…, Persist the new provider in the brain config store (fire-and-forget). Uses the…, Send a Telegram notification about the failover., Reset the singleton (test helper)., Monitors provider health and triggers failover on consecutive failures., Reset failure counter for a provider after a successful call. (+5 more)

### Community 16 - "TaskSpec"
Cohesion: 0.03
Nodes (106): runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns…, E2BAdapter, Any, runtimes/adapters/e2b.py — E2B Firecracker micro-VM runtime adapter. Routes…, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:… (+98 more)
...
Cohesion: 0.06
Nodes (41): BrainConfig, BrainConfigStore, default_brain_config(), BaseModel, Build a ``BrainConfig`` from a Mongo doc, dropping Mongo's ``_id``., Resolve the Ollama base URL the UI controls — DB value wins over env.…, The agency's active brain — provider + per-role models. Stored as a single…, Return the safe-default brain (used on first boot + store errors). (+33 more)
...
Cohesion: 0.06
Nodes (57): ApprovalStatus, Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Recursively strip secrets from an arbitrary argument structure. Fails…, scrub(), BudgetTracker, GovernanceGate (+49 more)

### Community 27 - "_make_task"
Cohesion: 0.19
...
### Community 29 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (65): requires_db, Initialize the service. Args: store: Optional CompanyGraphStore instance. If…, CompanyGraphStore, Delete a company and all its associated data., Delete a company and all associated data from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON blob (which…, Create a new website in SQLite. The full model is stored in ``data`` so scan… (+57 more)

### Community 30 - "TestRuntimeControl"
Cohesion: 0.04
...
### Community 36 - "test_mcp_registry.py"
Cohesion: 0.05
...
### Community 37 - "test_brain_config_store.py"
Cohesion: 0.06
Nodes (45): invalidate_brain_config_cache(), Convenience wrapper used by the admin API endpoints., Clear the singleton's cache (used by tests + brain_policy invalidation)., Synchronous call-time resolver for an agent role model id. Precedence (highest…, Async variant — refreshes the cache if stale before resolving. Used by code…, Clear the in-process cache. Called by the admin API after a successful PATCH…, resolve_role_model(), resolve_role_model_sync() (+37 more)
...
### Community 41 - "activation_api.py"
Cohesion: 0.10
...
Cohesion: 0.03
Nodes (76): all_provider_ids(), Return every provider id recognised by the brain config system. Iterates the…, CatalogActiveBrain, CatalogMirror, CatalogProviderEntry, get_model_catalog_store(), _include_router_providers(), invalidate_catalog_cache() (+68 more)
...
Cohesion: 0.06
Nodes (13): FeatureMatrix, Central support matrix — single source of truth. Loads the canonical feature…, Render the matrix as a Markdown table for docs., tests/test_feature_matrix.py — Feature maturity / support matrix tests. Covers:…, Integration test: admin endpoint returns feature matrix JSON., TestAdminVisibility, TestConfigOverrides, TestEnforcement (+5 more)
...
### Community 54 - "_FakeCommandResult"
Cohesion: 0.22
Nodes (3): _FakeCommandResult, _FakeCommands, _FakeFiles

### Community 55 - "get_runtime_manager"
Cohesion: 0.03
Nodes (127): Available iff config resolves AND the SDK is importable. Never raises — a…, e2b_status(), _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy() (+119 more)
...
Cohesion: 0.07
Nodes (57): Convenience: resolve all four role models for a component. Returns a dict with…, resolve_component_role_models(), get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _answer_callback() (+49 more)
...
### Community 59 - "App.js"
Cohesion: 0.05
Nodes (40): completeSetup(), createSecret(), detectHardwareForSetup(), detectModelsForSetup(), getDefaultBackendUrl(), getMe(), getPublicPath(), getSetupState() (+32 more)

### Community 60 - "CompanyGraph"
Cohesion: 0.03
Nodes (48): BusinessCategory, Get the bound Company Graph., CompanyGraph, SystemType, The complete Company Graph - canonical core model for the Autonomous AI Agency.…, Find a website by its URL., Find a repository by its URL., Find all systems of a specific type. (+40 more)
...
### Community 71 - "test_self_heal_v2.py"
Cohesion: 0.08
...
### Community 77 - "GoalRecord"
Cohesion: 0.08
Nodes (22): _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One piece of work the CEO has taken ownership of. ``state`` is the supervisor's…, Return ``(done, total)`` subtask counts. (+14 more)
...
### Community 83 - "Company"
Cohesion: 0.03
Nodes (80): Company, CompanyGraphSnapshot, DeliveryPolicy, The core company entity - root of the Company Graph., A point-in-time snapshot of a Company Graph for history and rollback., How code lands on a repo's default branch (detected, GitHub-only for now). The…, A company's connection to a code repository (GitHub-only this pass). URL-only…, RepoConnection (+72 more)
...
### Community 87 - "TaskExecutionCoordinator"
Cohesion: 0.08
Nodes (19): _is_brain_connection_error(), Any, BaseException, Task, TaskResult, TaskSpec, TaskStatus, Best-effort label of who will run this task once approved. Returns… (+11 more)

### Community 88 - "LLMProvider"
Cohesion: 0.04
Nodes (60): AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Anthropic's extended-thinking constraints on ``budget_tokens``: at least 1024,… (+52 more)

### Community 89 - "failover_client.py"
Cohesion: 0.05
Nodes (51): _auto_disable(), _Budget, _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool() (+43 more)
...
### Community 91 - "test_e2b_data_flow.py"
Cohesion: 0.08
...
### Community 92 - "Kept Rules — the 44 that survive the audit"
Cohesion: 0.05
...
### Community 93 - "agents/agile_ceremonies.py"
Cohesion: 0.08
...
Cohesion: 0.15
Nodes (18): _check_rate_limit(), issue_new_api_key(), Exception, RateLimitError, Persistent API key store: each key maps to email + department (seat) and a…, Generate a new plaintext API key, persist hash + metadata, return (plain_key,…, Raised when an IP exceeds the failed-key-lookup rate limit., Raise RateLimitError if *ip* has too many recent failed lookups. (+10 more)
...
Cohesion: 0.04
Nodes (66): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent., BudgetExceeded, classify(), evaluate_call() (+58 more)
...
### Community 106 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (125): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), get_ceo_dispatcher(), _merge_changed_files(), _offload(), Any (+117 more)
...
### Community 111 - "test_colibri_brain_shim.py"
Cohesion: 0.05
...
### Community 115 - "tests/conftest.py"
Cohesion: 0.08
...
### Community 124 - "validate_outbound_url"
Cohesion: 0.14
...
### Community 128 - "KnowledgeScreen.jsx"
Cohesion: 0.06
Nodes (35): getCompanyGraph(), useSafeData(), AgentCard(), AgentsScreen(), BUILTIN_AGENT_DEFS, CEOCyclePanel(), mapBackendAgent(), NewAgentForm() (+27 more)
...
Cohesion: 0.06
Nodes (70): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, A subtask's full history: what it is, and every attempt at it., SubtaskRecord, What one sweep observed and did. Returned for tests and diagnostics., SweepReport, _goal(), asyncio (+62 more)
...
Cohesion: 0.07
Nodes (30): Attempt, payload_key(), Exact-match cache key over the fields that change the answer. Routing…, text_key(), describe(), The operator-facing reason string for an unfixable status., Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys() (+22 more)

### Community 133 - "ImprovementLoop"
Cohesion: 0.08
Nodes (29): DetectedIssue, ImprovementLoop, ImprovementLoopState, _now(), Any, Path, agent/improvement_loop.py — Continuous Improvement Engine Background scanner…, Background scanner and task dispatcher for continuous codebase improvement.… (+21 more)
...
Cohesion: 0.05
Nodes (28): API, getLoops(), ActivationGate(), activityToAlert(), AlertsBell(), priorityConfig, _relativeTime(), typeIcon (+20 more)
...
### Community 152 - "test_webui_provider_priority.py"
Cohesion: 0.21
...
### Community 170 - "TasksPage.js"
Cohesion: 0.05
Nodes (44): addTaskComment(), clarifyTask(), createQuickNote(), createTask(), escalateTask(), fetchVelocity(), followUpTask(), getTask() (+36 more)
...
Cohesion: 0.04
Nodes (86): build_ceo_router(), Any, APIRouter, backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Drop the singleton (test helper)., reset_ceo_ledger(), CEOSupervisor, _env_flag() (+78 more)
...
### Community 186 - "CompanyAgencyService"
Cohesion: 0.11
...
Cohesion: 0.06
Nodes (43): _get_current_user_thunk(), _get_optional_user_thunk(), Request, create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., create_access_token() (+35 more)
...
### Community 194 - "open_phase_report"
Cohesion: 0.13
...
### Community 204 - "KeyStore"
Cohesion: 0.17
Nodes (9): default_keys_path(), KeyRecord, KeyStore, load_key_store(), Path, Look up a plaintext key by its SHA-256 hash. When *client_ip* is provided,…, Record a failed key lookup for *ip*., Thread-safe JSON-backed key store. (+1 more)

### Community 205 - "fmtErr"
Cohesion: 0.06
Nodes (57): authorizeGithubRepos(), createAgent(), createGithubPR(), createWikiPage(), deleteAgent(), deleteGithubToken(), deleteSource(), deleteWikiPage() (+49 more)
...
Cohesion: 0.10
Nodes (14): get_scheduler_store(), _MemCollection, _MemCursor, _MemDB, _MemDeleteResult, Any, services/scheduler_store.py — Durable scheduler persistence. Issue #505:…, Delete a persisted job. (+6 more)
...
### Community 210 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 215 - "resolve_identity"
Cohesion: 0.04
...
Cohesion: 0.06
Nodes (55): delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), list_seo_audits(), BaseModel, get, post (+47 more)
...
### Community 251 - "agent/context.py"
Cohesion: 0.16
...
### Community 257 - "weekly_digest.py"
Cohesion: 0.13
...
Cohesion: 0.05
Nodes (71): brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, auto_disable(), _billing_signals(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable. (+63 more)
...
### Community 272 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.11
...
### Community 275 - "test_schedule_growth_invariants.py"
Cohesion: 0.09
...
Cohesion: 0.12
Nodes (20): build_block(), Path, Absolute path of the harness spec for a workspace., Parse existing entries. Never raises — a broken file yields no entries. With…, Rewrite the spec file, preserving any non-entry (hand-written) lines., Compact prompt block of standing instructions, or '' when there are none.…, One standing instruction plus the evidence that earned it., read_entries() (+12 more)
...
Cohesion: 0.02
Nodes (189): get_agency(), get_tracker(), Return the shared AutonomyTracker singleton (lazy-init)., free_nvidia_models(), Return the curated list of free NVIDIA NIM models FreeBuff may use., List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., list_available_fixes() (+181 more)

### Community 288 - "terminal.py"
Cohesion: 0.17
...
### Community 296 - "parse_event_stream"
Cohesion: 0.15
...
### Community 297 - "test_scanner_security.py"
Cohesion: 0.15
...
### Community 299 - "scripts/doctor.py"
Cohesion: 0.29
...
### Community 305 - "TaskWorkflowService"
Cohesion: 0.05
...
### Community 306 - "._scan_website_impl"
Cohesion: 0.14
Nodes (9): _looks_like_bot_challenge(), BeautifulSoup, True if the HTML is an anti-bot/CAPTCHA interstitial rather than content. Used…, Run a full scan under a wall-clock budget. Delegates to…, Render a page with a real headless browser (Playwright/Chromium) and return…, Fetch a builtwith.com page, defeating its bot protection as far as is possible…, Last-resort fallback for sites we can't fingerprint live (JS-rendered +…, `_looks_like_bot_challenge` is the gate that stops us parsing a CAPTCHA /… (+1 more)
...
Cohesion: 0.13
Nodes (21): Test hook — restart the round-robin counters., reset(), _big_request(), _ok(), A per-minute token budget is an input ceiling, not just a sort key. Reproduces…, A conversation comfortably past the metered provider's per-minute budget.…, A 131k window behind a 4k budget is a 4k ceiling., Routing around the low ceiling beats compressing to fit it. (+13 more)
...
### Community 322 - "test_shared_state.py"
Cohesion: 0.17
...
### Community 325 - "AgentSessionStore"
Cohesion: 0.02
Nodes (106): AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _collect_recent_git_context(), _now_str(), Any, Capture the FastAPI main event loop so the CEO thread can dispatch run_cycle()… (+98 more)
...
### Community 360 - "services/background.py"
Cohesion: 0.03
Nodes (79): get_skill_registry_safe(), agent/skill_registry.py — Dynamic Skill Registry & Recommender Fetches skill…, Return the global SkillRegistry if set, else None. Used by onboarding and other…, set_skill_registry(), delete_company_endpoint(), get_company(), get_company_graph(), delete (+71 more)
...
### Community 376 - "ApprovalCheckpoint"
Cohesion: 0.14
...
### Community 429 - "test_telegram_service_webhook.py"
Cohesion: 0.18
...
### Community 448 - "OperationalIncidentTracker"
Cohesion: 0.13
...
Cohesion: 0.07
Nodes (34): Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log. (+26 more)
...
### Community 462 - "test_agency.py"
Cohesion: 0.13
Nodes (33): _parse_ceo_directives(), set_improvement_loop(), agency(), asyncio, Path, Tests for agent/agency.py, With no issues, CEO should report nominal., CEO should parse valid JSON directive list from LLM response. (+25 more)
...
Cohesion: 0.08
Nodes (32): Create an isolated execution context for a single task. Tries ``git worktree…, Clean up the worktree or temp copy created by ``_create_worktree``., _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., _make_task(), asyncio, Task, TaskStatus (+24 more)
...
### Community 496 - "AdminPortalPage.js"
Cohesion: 0.38
...
Cohesion: 0.03
Nodes (96): agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, tasks/models.py — Pydantic models for the task/issue system., Comment or reply on a task. (+88 more)
...
### Community 512 - "analyze_qualitative"
Cohesion: 0.12
...
### Community 523 - "test_orchestrator_merge_decision.py"
Cohesion: 0.24
...
### Community 545 - "operational_incidents.py"
Cohesion: 0.13
...
### Community 592 - "get_tool_registry"
Cohesion: 0.14
...
### Community 596 - "telegram_service.py"
Cohesion: 0.18
...
### Community 599 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 638 - "TestPreCompiledPatterns"
Cohesion: 0.22
...
### Community 651 - "AdminSessionStore"
Cohesion: 0.18
...
### Community 683 - "_is_bedrock_model_id"
Cohesion: 0.27
...
### Community 719 - "OperationalIncident"
Cohesion: 0.24
...
### Community 736 - "AgentJobManager"
Cohesion: 0.03
Nodes (101): PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, AgentJob, AgentJobManager, make_isolated_workspace(), _now() (+93 more)

### Community 737 - "_is_ephemeral_user"
Cohesion: 0.27
...
### Community 749 - "test_workflow_models.py"
Cohesion: 0.09
...
### Community 756 - "admin_update_task_router.py"
Cohesion: 0.22
...
Cohesion: 0.09
Nodes (16): _bedrock_api_response(), _bedrock_provider(), _mock_boto3(), Any, ProviderConfig, Tests for AWS Bedrock provider support in ProviderRouter., Inject a mock boto3 module into sys.modules for the duration of the block., Verify that Bedrock model IDs bypass non-Bedrock providers. (+8 more)
...
### Community 780 - "rate_limiter.py"
Cohesion: 0.18
Nodes (8): _ProviderQuota, Response, Proactive rate-limit throttling for LLM providers — two complementary layers.…, Parse x-ratelimit-* headers and update per-provider quota state. Safe to call…, Reset all state (primarily for tests)., Clear all token-bucket state (tests only). Does not touch the header tracker's…, reset(), _reset_buckets()
...
### Community 792 - "local_brain_provider_config"
Cohesion: 0.29
...
### Community 796 - "_hostname_matches"
Cohesion: 0.22
...
### Community 848 - "_FakePersistence"
Cohesion: 0.20
...
### Community 856 - "test_providers_live_e2e.py"
Cohesion: 0.27
...
### Community 869 - "_status_snapshot"
Cohesion: 0.31
...
### Community 891 - "test_scanner_ssl_cert.py"
Cohesion: 0.25
...
### Community 894 - "test_agent_readiness_audit.py"
Cohesion: 0.18
...
Cohesion: 0.05
Nodes (39): get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, start_orchestrator_supervisor() (+31 more)
...
### Community 910 - "test_scanner_e2e.py"
Cohesion: 0.27
...
### Community 913 - "TestGithubSignalHardening"
Cohesion: 0.22
...
### Community 914 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 982 - "test_agent_tools.py"
Cohesion: 0.43
...
### Community 1013 - "4. Current Architecture (As-Is)"
Cohesion: 0.29
...
### Community 1023 - "PhaseSequenceError"
Cohesion: 0.33
...
### Community 1035 - "FeatureUnavailableError"
Cohesion: 0.10
Nodes (7): FeatureUnavailableError, Exception, Raised when code attempts to use a feature that is disabled or unavailable., TestAdminVisibility, TestFeaturesAPI, TestMatrixSerialization, TestWorkspaceSecurityRegressions
...
### Community 1048 - "CircuitBreakerOpenError"
Cohesion: 0.50
...
### Community 1050 - "gather_render_evidence"
Cohesion: 0.50
...
### Community 1051 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 1052 - "TestReviewRegressions"
Cohesion: 0.33
...
### Community 1083 - "The 24x7 agency — your agents never go idle"
Cohesion: 0.50
...
### Community 1131 - "test_feature_stores_are_wired_on_demand_not_eagerly"
Cohesion: 0.50
...
- **Why does `_fixture()` connect `_fixture` to `ExecutionRequest`, `AgentScheduler`, `backend/server.py`, `AgentRunner`, `company_api.py`, `Task`, `FeatureUnavailableError`, `llm/router.py`, `test_orchestrator_merge_decision.py`, `BrainWatchdog`, `TokenBudget`, `test_llm_router_strategies.py`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `BrainConfig`, `KeyPool`, `test_local_brain_state.py`, `get_improvement_loop`, `test_phase5_doctor.py`, `TestRuntimeControl`, `test_model_router.py`, `test_telegram_diag_endpoint.py`, `SelfHealingAgent`, `operational_incidents.py`, `test_llm_router_resilience.py`, `AgentSwarm`, `test_unit8_model_catalog.py`, `failover_chat_completion`, `PrimeAgentAdapter`, `e2e/test_browser.py`, `test_knowledge_sync.py`, `test_agent_free_brain.py`, `test_sam_livekit.py`, `SecretRecord`, `test_self_heal_v2.py`, `SeoAuditRequest`, `ResearchTask`, `GoalRecord`, `TestClient`, `TestHarnessAdapter`, `test_openclaw_gateway.py`, `ProceduralMemoryStore`, `failover_client.py`, `test_e2b_data_flow.py`, `issue_new_api_key`, `test_agent_tool_governance.py`, `Surface`, `test_new_features_e2e.py`, `cache.py`, `test_colibri_brain_shim.py`, `tests/conftest.py`, `SeoFixer`, `HermesAdapter`, `test_context_rulebook.py`, `ArtifactStore`, `ImprovementLoop`, `test_trend_scoping.py`, `test_issue_intake.py`, `test_startup_warmup.py`, `config.py`, `ChatHistoryStore`, `persist_plan_spec`, `test_webui_provider_priority.py`, `test_agile_api.py`, `test_app_settings.py`, `test_e2b_task_wiring.py`, `test_task_clarification.py`, `OllamaCircuitBreaker`, `TrendWatcher`, `test_ceo_router.py`, `test_provider_enable_disable.py`, `TestChatHistoryStore`, `test_features_api.py`, `test_video_transcript.py`, `facade.py`, `emit_chat_observation`, `test_portfolio_intake.py`, `OrchestratorQueue`, `test_task_brain_preflight.py`, `TestEstimateTokensForMessages`, `SchedulerStore`, `TestClient`, `test_kimi_bridge_server.py`, `timedelta`, `resolve_identity`, `ContextPruner`, `IssueCategory`, `AgentJobManager`, `test_trend_watcher.py`, `test_provider_state_durability.py`, `test_platform_controls.py`, `_resolve_push_token`, `test_tasks_awaiting_approval_api.py`, `WorkflowBuildRequest`, `test_persistent_memory.py`, `SecurityScanner`, `test_llm_router_e2e.py`, `test_control_plane_api.py`, `test_telegram_mutating_commands.py`, `MCPUnavailableError`, `test_brain_failover.py`, `brain_failover.py`, `rate_limiter.py`, `TestRecordUsageAndStats`, `test_purge_backlog.py`, `test_dashboard_cache.py`, `_resolve_brain_provider`, `parse_event_stream`, `TaskWorkflowService`, `NotificationDispatcher`, `test_crispy_burn_in.py`, `test_llm_router_tpm.py`, `test_skill_registry_boot_refresh.py`, `test_shared_state.py`, `AgentSessionStore`, `resolve_active_brain`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `analyze_page`, `capture_screens.py`, `test_sam_voice.py`, `test_ping.py`, `ClaudeCodeAdapter`, `test_task_store_fails_loud_in_production.py`, `test_telegram_approval_e2e.py`, `_get`, `test_service_token.py`, `services/background.py`, `test_agency_fix.py`, `test_output_filter.py`, `model_discovery.py`, `get_workflow_orchestrator`, `test_monitor_lib.py`, `test_v4_api.py`, `_resolve_user_github_token`, `allow_paid`, `TestSwarmRoleRouting`, `test_telegram_service_webhook.py`, `test_langfuse_agency_wide.py`, `test_voice_pipeline.py`, `TestNormalizeResponseFormat`, `OperationalIncidentTracker`, `test_brain_patch_service_token.py`, `test_brain_failover_413.py`, `AuditLog`, `TestMongoGate`, `test_agency.py`, `TrafficDirector`, `TestHarnessRegistry`, `test_company_api.py`, `client`, `test_task_service_failed_comment.py`, `SQLiteStore`, `TaskStore`, `test_openclaw_endpoints.py`, `test_workspace_repowise.py`, `test_ai_insights.py`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `ExecutionRequest`, `AgentScheduler`, `backend/server.py`, `direct_chat.py`, `TokenBudget`, `TaskSpec`, `_build_request`, `StuckDetector`, `AutonomyTracker`, `proxy.py`, `MultiAgentSwarm`, `failover_chat_completion`, `_StubManager`, `MCPClient`, `test_agent_free_brain.py`, `WorkspaceTools`, `AgentSessionStore`, `test_brain_failover_413.py`, `LocalWorkspace`, `TestWorkspace`, `test_empirical_verify.py`, `GitHubTools`, `ContextPruner`, `.execute`, `AgentJobManager`, `test_agent_tool_governance.py`, `AdaptiveHalter`, `TestZeroAttemptDiagnostics`, `test_ceo_dispatcher.py`, `TestAgentLoopMCPIntegration`, `ReactScratchpad`, `ContextManager`, `.execute`, `MCPUnavailableError`, `test_governance_api.py`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `AgentScheduler` to `backend/server.py`, `kimi_bridge_provider_config`, `test_bedrock_provider.py`, `direct_chat.py`, `TestOpenAiToBedrockConverse`, `system_instruction`, `TestActiveStrategy`, `NvidiaProvider`, `CircuitState`, `TestAnthropicPayloadStructuredOutput`, `settings.py`, `_is_bedrock_model_id`, `failover_chat_completion`, `TestAnthropicToOpenAICacheUsage`, `test_bedrock_live.py`, `TrafficDirector`, `failover_client.py`, `AgentJobManager`, `test_all_providers_discovery.py`, `test_anthropic_router.py`, `test_colibri_brain_shim.py`, `TestRouterIntegration`?**
