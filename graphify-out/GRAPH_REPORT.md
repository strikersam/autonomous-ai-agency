...
## Corpus Check
- 1482 files · ~2,160,820 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30184 nodes · 55908 edges · 1291 communities (1119 shown, 117 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2731 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `02425f56`
...
## Communities (1291 total, 117 thin omitted)

### Community 0 - "CEOLedger"
Cohesion: 0.07
...
Cohesion: 0.04
Nodes (52): MergeDecision, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs…, G5: resolve how a run should land from the company's DeliveryPolicy. Returns…, Persist first-merge consent once an operator approves a ``telegram_gate``. No-…, Determine domain and task type from the request text., Generate a structured execution plan. Uses the LLM if available; falls back to… (+44 more)

### Community 3 - "ProviderConfig"
Cohesion: 0.07
Nodes (35): is_commercial_provider(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, The pieces the failover loop assembles for an Anthropic-native provider., TestAnthropicRequestShape, Tests that verify the exact provider priority ordering. (+27 more)

### Community 4 - "backend/server.py"
Cohesion: 0.02
Nodes (292): Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry (+284 more)

### Community 5 - "proxy.py"
Cohesion: 0.03
Nodes (163): middleware, admin_control(), admin_create_user(), admin_delete_user(), admin_login(), admin_logout(), admin_rotate_user(), admin_status() (+155 more)

### Community 6 - "AgentPlan"
Cohesion: 0.05
Nodes (59): PreflightReport, Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage (+51 more)
...
Cohesion: 0.03
Nodes (97): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, AgentIdentity, _coerce_text(), new_session_id(), Any, packages/governance/identity.py — who is acting, and under what mandate. Docker… (+89 more)
...
Cohesion: 0.03
Nodes (118): BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), provider_base_url() (+110 more)

### Community 14 - "TaskWorkflowService"
Cohesion: 0.04
Nodes (61): AgentDefinition, Any, field_validator, A user-defined agent configuration stored in the database., Accept both ISO-8601 datetime strings (from DB) and float timestamps., Keep legacy backend fields and Control Plane UI fields aligned., _gate_outward_facing_enabled(), _is_outward_facing() (+53 more)
...
### Community 17 - "_check_storage_health"
Cohesion: 0.12
...
Cohesion: 0.04
Nodes (106): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, build_provider(), ProviderConfig, Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the…, resolve_kind(), OllamaProvider (+98 more)

### Community 19 - "_fixture"
Cohesion: 0.02
Nodes (92): parse_event_stream(), ParsedRun, Structured view of one ``--mode json`` event stream., Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, app_client(), _clear_discovered_models(), _clear_response_cache(), client() (+84 more)
...
Cohesion: 0.03
Nodes (114): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+106 more)

### Community 23 - "Task"
Cohesion: 0.04
Nodes (68): Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), Full task/issue document., Task, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, asyncio, A reject + human Retry starts a NEW approval cycle → fresh notification. (+60 more)

### Community 24 - "SQLiteStore"
Cohesion: 0.03
Nodes (63): DetectedSystem, A business system detected on a company's website or in their stack., Get the most confident evidence description., Company, Create a new company in SQLite., Get a company by ID from SQLite., Update a company in SQLite., Delete a company and all associated data from SQLite. (+55 more)

### Community 25 - "CompanyGraphStore"
Cohesion: 0.03
Nodes (39): Find a repository by its URL., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, requires_db, Add a repository to a company. Args: company_id: Company ID url: Repository URL…, Initialize the service. Args: store: Optional CompanyGraphStore instance. If… (+31 more)
...
Cohesion: 0.03
Nodes (85): _brain_provider_status(), get_brain_policy_route(), get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., Return the active brain config + per-provider key-present flags. The response…, _served_models() (+77 more)

### Community 30 - "test_governance_enforcement.py"
Cohesion: 0.05
Nodes (72): BudgetTracker, classify(), GovernanceGate, _host_of(), Extract a hostname from a URL, or return a bare host unchanged., Per-session consumption counters and their ceilings. Ceilings come from the…, Return the name of the first exhausted session-wide limit, or None. Only covers…, Return a reason string if *tool* has hit its per-tool session cap. Checks only… (+64 more)

### Community 31 - "TestAuthAndTaskOwnership"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (58): Run a full SEO/GEO/AIO audit against a website and persist the evidence., run_seo_audit(), BaseModel, field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Aggregated report row - Screaming Frog CSV compatible. (+50 more)

### Community 39 - "WebsiteScanner"
Cohesion: 0.04
Nodes (66): Result of a website scan with detected systems and stack inference., WebsiteScanResult, main(), Returns (url, ok, summary)., _scan_one(), _content_contains_domain(), _hostname_contains(), _hostname_matches() (+58 more)

### Community 40 - "test_render_mcp.py"
Cohesion: 0.04
...
Cohesion: 0.05
Nodes (53): Feed LLM token/cost spend into the governance session budget. Kept separate…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Write the audit row for a completed (or blocked) MCP tool call., Import the governance layer once, recording why if it is absent., record(), events_from_identity() (+45 more)
...
Cohesion: 0.03
Nodes (97): allow_paid_brain(), BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), is_anthropic_model() (+89 more)

### Community 45 - "Artifact"
Cohesion: 0.07
Nodes (37): AgentRole, agents/swarm.py — AgentSwarm: routes workflow phases to the correct agent. The…, When coder == reviewer model, swarm should log a warning., tests/test_workflow_models.py — Unit tests for workflow/models.py., TestApprovalGate, TestCheckRun, TestSlice, workflow/artifact_store.py — Durable artifact persistence. Artifacts are stored… (+29 more)
...
Cohesion: 0.06
Nodes (37): build_ceo_router(), Any, APIRouter, backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, get_ceo_dispatcher(), Return the shared CEODispatcher singleton., get_ceo_ledger(), Return the shared :class:`CEOLedger`, constructing it on first use. (+29 more)
...
### Community 52 - "OnboardingService"
Cohesion: 0.06
Nodes (39): Evidence, Evidence supporting a system detection., Inferred technology stack from website/repo analysis., StackInference, OnboardingService, Workflow, Detect the Git provider from a repository URL. Args: repo_url: Repository URL…, Start the onboarding process for a company. Args: company_id: Company ID… (+31 more)
...
Cohesion: 0.05
Nodes (68): DirectChatDoctor, PreflightIssue, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob (+60 more)
...
Cohesion: 0.05
Nodes (38): _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), _build_auto_skill_guidance(), _build_provider_router(), _builtin_provider_records(), call_llm(), _chat_provider_policy() (+30 more)

### Community 63 - "loop.py"
Cohesion: 0.05
Nodes (46): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only… (+38 more)
...
### Community 66 - "get_task_store"
Cohesion: 0.05
Nodes (43): _autonomy_bg_cycle(), autonomy_tick(), purge_backlog(), _purge_backlog_core(), PurgeBacklogRequest, quick_notes_submit(), _QuickNoteBody, Shared purge implementation for the admin endpoint and the boot hook. (+35 more)
...
Cohesion: 0.07
Nodes (61): A subtask's full history: what it is, and every attempt at it., SubtaskRecord, str, Implementer tiers, ordered cheapest-and-narrowest to most-capable. A subtask…, Tier, _goal(), asyncio, Tests for the CEO ledger, the supervised escalation loop, and the 24x7 sweeper.… (+53 more)
...
### Community 78 - "test_self_heal_v2.py"
Cohesion: 0.06
...
Cohesion: 0.03
Nodes (90): BusinessCategory, ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraph, CompanyGraphSnapshot, CompanyResponse, CompanyUpdateRequest (+82 more)
...
### Community 84 - "test_ai_insights.py"
Cohesion: 0.05
...
### Community 93 - "resolve_free_nvidia_brain"
Cohesion: 0.12
...
Cohesion: 0.06
Nodes (26): BudgetUsage, Any, Reset usage counters for *session_id* (cap is preserved)., Reset token counters for all sessions (caps preserved). Called at the start of…, Reset all budgets if the UTC calendar day has changed since last reset. Safe to…, Generate a token savings analytics report. Returns per-session statistics and…, Track and enforce per-session or per-agent token budgets. Usage:: budget =…, Set (or update) the token cap for *session_id*. (+18 more)
...
Cohesion: 0.04
Nodes (79): List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _answer_callback(), _api_headers(), _check_rate_limit(), cmd_control() (+71 more)
...
Cohesion: 0.08
Nodes (31): get_tracker(), Return the shared AutonomyTracker singleton (lazy-init)., Any, Connection, Path, PhaseType, WorkflowRun, CRISPY workflow engine — phase sequencer + gate controller. GATE: Golden Path… (+23 more)
...
Cohesion: 0.08
Nodes (41): get_approval_store(), Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient, Tests for the governance HTTP surface and the AgentRunner integration. The…, Policy is a git-reviewed file. An HTTP mutation route would make "who changed… (+33 more)
...
### Community 146 - "BrainFailoverManager"
Cohesion: 0.09
...
Cohesion: 0.08
Nodes (23): _get_director(), _normalized_provider_type(), ProviderAttempt, ProviderRouter, Any, Response, Try all models for one provider. Returns ProviderResult on success, None on…, Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either… (+15 more)
...
### Community 217 - "_ts_to_float"
Cohesion: 0.07
Nodes (30): Task, Create a task. Deduplicates by source_id if set (Charter G3). If a task with…, Fetch a task by ID. If owner_id is set, enforces ownership., Return the task previously created for an external ``source_id`` (e.g.…, Return BLOCKED tasks that are candidates for auto-retry., Return tasks due within the next N hours (any user)., Return tasks parked at the pre-execution approval gate (any user). A task is…, Reset tasks that are stuck IN_PROGRESS but no longer actively executing. A task… (+22 more)
...
### Community 219 - "OperationalIncident"
Cohesion: 0.10
...
### Community 231 - "get_scheduler"
Cohesion: 0.04
Nodes (69): _get_cached_tasks(), _get_tasks_cache_lock(), _load_improvement_state(), Any, BaseModel, get, Lock, post (+61 more)
...
### Community 235 - "test_implement_agent_routing.py"
Cohesion: 0.07
...
### Community 287 - "test_webui_provider_priority.py"
Cohesion: 0.08
...
### Community 298 - "ensure_self_company"
Cohesion: 0.10
Nodes (32): Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap(), _count_specialists(), _create_company_directly(), ensure_self_company(), _find_self_company(), _find_stale_self_companies(), _list_companies_safe() (+24 more)
...
### Community 302 - "test_phase6_workflow.py"
Cohesion: 0.05
Nodes (77): add_pr_comment(), _find_existing_pr(), get_branch_sha(), get_default_branch(), _headers(), Any, agent/safe_agency.py — Safe GitHub operations for the workflow engine. All…, Create a pull request. Returns the PR object dict. If a PR already exists for… (+69 more)
...
### Community 307 - "test_colibri_provider.py"
Cohesion: 0.12
...
### Community 312 - "_execute_skill_impl"
Cohesion: 0.09
...
### Community 359 - "_build_default_manager"
Cohesion: 0.05
...
### Community 363 - "open_phase_report"
Cohesion: 0.13
...
Cohesion: 0.06
Nodes (51): _get_current_user_thunk(), get_optional_company_access(), _get_optional_user_thunk(), Request, Extract a consistent user_id from the user dict. Works across all auth methods:…, Verify user has access to a company (if authenticated). Admins bypass ownership…, _resolve_user_id(), build_seo_roadmap() (+43 more)
...
### Community 403 - "CompanyAgencyService"
Cohesion: 0.09
...
### Community 419 - "HarnessRegistry"
Cohesion: 0.07
Nodes (23): get_harness_adapter(), harness_active(), harness_catalog(), harness_session_close(), harness_session_start(), HarnessSessionBody, HarnessSessionCloseBody, Return the full ECC harness catalog with capabilities. Public — no auth… (+15 more)
...
Cohesion: 0.12
Nodes (22): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is…, Tests for FreeBuff Telegram agent. (+14 more)
...
### Community 432 - "sam_livekit_worker.py"
Cohesion: 0.13
...
### Community 437 - "reset_router"
Cohesion: 0.12
Nodes (14): Reset the singleton and clear the cached model map (test helper)., reset_router(), has_image_content(), Return True if any message contains an image_url content part., _opus_model() with ANTHROPIC_API_KEY + explicit paid opt-in should return…, _opus_model() with Bedrock keys + explicit paid opt-in should return the…, test_opus_model_prefers_4_6_bedrock_arn(), test_opus_model_prefers_4_6_for_direct_api() (+6 more)
...
Cohesion: 0.05
Nodes (51): AgentCreateRequest, AgentStore, AgentUpdateRequest, BaseModel, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+43 more)
...
### Community 446 - "_start_ceo_agency"
Cohesion: 0.31
...
Cohesion: 0.07
Nodes (28): _diagnose_and_file(), gather_render_evidence(), get_operational_incident_tracker(), _iso_from_monotonic(), OperationalIncidentTracker, agent/operational_incidents.py — recurring operational failures, auto-…, Count operational failures; diagnose and file the ones that persist. Every…, Start with no tracked signatures and no filing history. (+20 more)
...
Cohesion: 0.04
Nodes (41): provider_api_keys(), Every API key configured for *provider*, primary first. Reads ``base_env`` then…, api_keys_for(), _digest(), KeyPool, _KeyState, _PoolState, Per-provider API key rotation — the one lever that adds capacity. Every other… (+33 more)
...
### Community 459 - "test_memory_guard.py"
Cohesion: 0.13
...
### Community 522 - "test_e2b_sandbox.py"
Cohesion: 0.05
Nodes (54): _inject_token(), Best-effort scrub of ``token`` from ``text``., Return ``(authed_url, clean_url)`` for a GitHub repo URL., _scrub_token(), _clean_e2b_env(), fake_sandbox(), _FakeAsyncSandboxClass, _FakeCommandResult (+46 more)
...
### Community 544 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 545 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 556 - "test_autonomous_agency_e2e.py"
Cohesion: 0.08
...
### Community 630 - "ApprovalCheckpoint"
Cohesion: 0.14
...
### Community 636 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.16
...
### Community 648 - "get_skill_bindings"
Cohesion: 0.14
...
### Community 664 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 671 - "FeatureUnavailableError"
Cohesion: 0.13
...
### Community 674 - "test_loop_registry.py"
Cohesion: 0.21
...
### Community 677 - "test_llm_router_tpm.py"
Cohesion: 0.17
...
### Community 679 - "test_cost_attribution.py"
Cohesion: 0.07
...
### Community 742 - "TestGithubSignalHardening"
Cohesion: 0.22
...
### Community 757 - "test_daily_automation_2026_07_09.py"
Cohesion: 0.16
...
### Community 831 - "flesch_reading_ease"
Cohesion: 0.20
...
### Community 965 - "TestRevenuePortfolio"
Cohesion: 0.20
...
### Community 1009 - "_harvest_changed_files"
Cohesion: 0.29
...
### Community 1023 - "._notify_approval_gate"
Cohesion: 0.22
...
### Community 1046 - "_FakeAsyncClient"
Cohesion: 0.20
...
### Community 1113 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 1163 - "tests/conftest.py"
Cohesion: 0.29
Nodes (7): Item, Config, pytest_collection_modifyitems(), pytest_configure(), pytest_sessionfinish(), Pytest configuration for the backend test suite. Mongo availability…, Report threads that will stop the interpreter from exiting. A suite can pass…
...
### Community 1266 - "TestRunCoroSync"
Cohesion: 0.29
...
### Community 1267 - "_load_local_metrics_since"
Cohesion: 0.43
...
### Community 1269 - "_clean_director"
Cohesion: 0.50
...
### Community 1277 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 1278 - "llm/router.py"
Cohesion: 0.03
Nodes (108): _load(), main(), int, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., Test hook — clear all usage accounting., reset() (+100 more)

### Community 1281 - "loops_overview"
Cohesion: 0.50
...
### Community 1283 - "brain_providers"
Cohesion: 0.50
...
## Knowledge Gaps
- **3475 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3470 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13299 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **117 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `test_brain_patch_service_token.py`, `CEOLedger`, `WorkflowRun`, `AgentPlan`, `PolicyEngine`, `test_e2b_sandbox.py`, `TaskStore`, `TaskWorkflowService`, `test_llm_router_strategies.py`, `SelfHealingAgent`, `Task`, `AgentJobManager`, `test_company_api.py`, `test_unit8_model_catalog.py`, `TestAuthAndTaskOwnership`, `ImprovementLoop`, `test_kimi_bridge_server.py`, `test_failover_client_shared.py`, `E2BSandboxSession`, `test_catalogue_probe.py`, `services/seo_audit.py`, `routing.py`, `PrimeAgentAdapter`, `SeoFixer`, `ExecutionRequest`, `OnboardingService`, `test_openclaw_endpoints.py`, `TestAgentRunnerSafety`, `test_task_service_failed_comment.py`, `clear_cooldowns`, `validate_policy_text`, `ai/router.py`, `ChatHistoryStore`, `ArtifactStore`, `SQLiteStore`, `test_self_heal_v2.py`, `test_telegram_auto_approve.py`, `ResearchTask`, `TestClient`, `test_ai_insights.py`, `test_autonomy_status.py`, `test_procedural_memory.py`, `test_langfuse_agency_wide.py`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_startup_warmup.py`, `test_refresh_agent_built_proof.py`, `test_telegram_diag_endpoint.py`, `resolve_e2b_config`, `run_task`, `cost_tracker.py`, `BackgroundAgent`, `UserRole`, `test_context_rulebook.py`, `e2e/test_browser.py`, `test_provider_router.py`, `BrainWatchdog`, `KeyStore`, `test_issue_intake.py`, `test_llm_router_queue_cache.py`, `_step`, `test_trend_scoping.py`, `_clean_phases`, `_StubProvider`, `claim`, `MCPClient`, `persist_plan_spec`, `config.py`, `test_daily_2026_07_27.py`, `test_openclaw_gateway.py`, `clear_override`, `test_llm_router_tpm.py`, `NotificationDispatcher`, `OllamaCircuitBreaker`, `test_telegram_webhook.py`, `_job_text`, `test_features_api.py`, `test_new_features_e2e.py`, `test_task_brain_preflight.py`, `test_video_transcript.py`, `test_llm_router_resilience.py`, `test_process_quick_note_workflow.py`, `_Budget`, `_resolve_push_token`, `nvidia_models.py`, `TrendWatcher`, `TestRuntimeControl`, `test_portfolio_intake.py`, `test_trend_watcher.py`, `TestClient`, `AgentSwarm`, `test_agent_tool_governance.py`, `ContextPruner`, `TestEstimateTokensForMessages`, `get_scheduler`, `AgentProfile`, `test_agile_api.py`, `test_app_settings.py`, `WorkflowBuildRequest`, `test_daily_automation_2026_07_09.py`, `TestHarnessAdapter`, `test_persistent_memory.py`, `SecurityScanner`, `test_task_clarification.py`, `_clean_director`, `test_llm_router_e2e.py`, `llm/router.py`, `OutputFilter`, `test_platform_controls.py`, `test_rate_limiter.py`, `test_brain_failover.py`, `test_sam_voice.py`, `Agency`, `OrchestratorQueue`, `test_nvidia_model_discovery.py`, `brain_failover.py`, `session_retro.py`, `test_mcp_governance.py`, `test_purge_backlog.py`, `test_doctor_service_token_check.py`, `test_provider_enable_disable.py`, `analyze_page`, `_resolve_user_github_token`, `capture_screens.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_skill_registry_boot_refresh.py`, `test_ceo_router.py`, `_mock_provider_records`, `FeatureMaturity`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `test_dashboard_cache.py`, `test_telegram_mutating_commands.py`, `_undeclared`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_harness_spec.py`, `test_brain_failover_413.py`, `test_regression.py`, `test_agency_fix.py`, `test_sam_livekit.py`, `HarnessRegistry`, `_get`, `test_monitor_lib.py`, `test_v4_api.py`, `reset_router`, `AgentStore`, `SchedulerStore`, `_start_ceo_agency`, `test_knowledge_sync.py`, `operational_incidents.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `TestNormalizeResponseFormat`, `KeyPool`, `test_task_store_fails_loud_in_production.py`, `test_provider_state_durability.py`, `_Recorder`, `test_voice_pipeline.py`, `client`, `TestTheWorkflowIsSafeAndReadOnly`, `test_doctor_coding_brain.py`, `services/background.py`?**
  _High betweenness centrality (0.083) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `WorkflowRun`, `ReactScratchpad`, `backend/server.py`, `proxy.py`, `AgentPlan`, `ContextManager`, `PolicyEngine`, `TaskSpec`, `CEODispatcher`, `test_governance_api.py`, `MCPClient`, `test_brain_failover_413.py`, `AgentJobManager`, `test_failover_client_shared.py`, `E2BSandboxSession`, `test_e2b_adapter.py`, `MultiAgentSwarm`, `test_empirical_verify.py`, `routing.py`, `FreeBuffAgent`, `resolve_active_brain`, `LocalWorkspace`, `test_autonomous_agency_e2e.py`, `AgentSessionStore`, `WorkspaceTools`, `TestAgentRunnerSafety`, `direct_chat.py`, `call_llm`, `loop.py`, `TestAgentLoopMCPIntegration`, `_resolve_push_token`, `workflow_orchestrator.py`, `UserMemoryStore`, `TestAgentRunnerExecution`, `StuckDetector`, `test_agent_tool_governance.py`, `AdaptiveHalter`, `ContextPruner`, `TokenBudget`, `InternalAgentAdapter`, `GitHubTools`, `BackgroundAgent`, `test_daily_automation_2026_07_09.py`, `test_daily_automation_2026_07_11.py`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `ProviderConfig`, `backend/server.py`, `TestLegacyRouterServerFallback`, `test_bedrock_provider.py`, `test_bedrock_live.py`, `TestRouterIntegration`, `settings.py`, `resolve_active_brain`, `kimi_bridge_provider_config`, `test_colibri_provider.py`, `TestLegacyRouterCacheTTL`, `direct_chat.py`, `call_llm`, `clear_cooldowns`, `ai/router.py`, `test_anthropic_refusal_fallback.py`, `failover_client.py`, `TrafficDirector`, `NvidiaProvider`, `test_implement_agent_routing.py`, `TestOpenAiToBedrockConverse`, `TestAnthropicPayloadStructuredOutput`, `test_all_providers_discovery.py`, `test_provider_router.py`, `model_discovery.py`, `_payload`, `implement_agent.py`?**
