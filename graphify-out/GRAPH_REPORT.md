...
## Communities (1331 total, 171 thin omitted)

### Community 0 - "TaskSpec"
Cohesion: 0.04
Nodes (107): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor., json_safe(), runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).… (+99 more)

### Community 1 - "backend/server.py"
Cohesion: 0.02
Nodes (172): admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, _brain_provider_status(), BrainTestRequest (+164 more)

### Community 2 - "LLMRequest"
Cohesion: 0.03
Nodes (105): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, OpenAICompatible, Adapter for any endpoint speaking the OpenAI chat-completions shape. Adding a…, build_provider(), Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the…, resolve_kind() (+97 more)

### Community 3 - "build_llm_router"
Cohesion: 0.03
Nodes (86): get_budget(), The process-wide budget tracker., get_cache(), The process-wide cache manager., openai_body_from_response(), Any, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on…, Translate an OpenAI-shaped chat payload into an ``LLMRequest``. Unknown keys… (+78 more)

### Community 4 - "get_company_access"
Cohesion: 0.04
Nodes (79): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), delete_company_endpoint(), _DoctorCheck, _DoctorReport, generate_onboarding_questions() (+71 more)

### Community 5 - "test_llm_router_queue_cache.py"
Cohesion: 0.03
Nodes (82): AlertHandler, BudgetTracker, Counter, _Dimensions, _month(), Any, packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, Register a callback fired when a spend threshold is crossed. (+74 more)
...
Cohesion: 0.07
Nodes (38): fetch_research_alerts(), Run an async coroutine from sync code, safe even inside a running loop.…, Best-effort fetch of trend alerts. Returns [] if offline or unavailable., _run_coro_sync(), Weighted Shortest Job First score — higher schedules sooner., map_initiative_to_task(), materialize_committed(), _wsjf() (+30 more)

### Community 10 - "AgentJobManager"
Cohesion: 0.04
Nodes (40): AgentJob, AgentJobManager, heartbeat(), _now(), Any, Run a job using the provided runner and update the job's lifecycle, progress,…, Serialize the AgentJob to a JSON-serializable dictionary for external clients.…, AgentJobManager method signature enforcement. (+32 more)

### Community 11 - "TaskStore"
Cohesion: 0.03
Nodes (79): _priority_rank(), Any, Fail loud in production if no persistent storage is configured. Telegram…, Create a task. Deduplicates by source_id if set (Charter G3). If a task with…, Fetch a task by ID. If owner_id is set, enforces ownership., Return the task previously created for an external ``source_id`` (e.g.…, List tasks for a specific user with optional filters. ``include_system``…, Admin-only: list all tasks across all users. ``include_log`` mirrors… (+71 more)
...
Cohesion: 0.02
Nodes (156): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., create_company(), get_company(), get_company_graph(), provision_specialist(), patch, Company Graph API Router Provides all endpoints for managing companies, their… (+148 more)

### Community 14 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (72): _merge_changed_files(), Collect changed_files across all specialists into a single de-duped list., get_ceo_fallback_stats(), PlanOutput, PLAN phase: structured execution plan., SELECT_SPECIALIST phase: which specialist(s) handle this work., Increment the named counter (silently ignores unknown reasons)., Return a snapshot of the CEO fallback counters. (+64 more)

### Community 15 - "test_llm_router_strategies.py"
Cohesion: 0.03
Nodes (104): count, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, HealthTracker, _Outcome, ProviderHealth (+96 more)

### Community 16 - "PolicyDecision"
Cohesion: 0.07
Nodes (30): classify(), evaluate_call(), guard_tool_call(), GuardResult, _host_of(), _is_write_tool(), Any, Extract a hostname from a URL, or return a bare host unchanged. (+22 more)

### Community 17 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (122): _diagnose_and_file(), _file_incident(), _format_incident(), gather_render_evidence(), get_operational_incident_tracker(), _iso_from_monotonic(), normalise(), note_phase_end() (+114 more)
...
### Community 19 - "test_agent_runner.py"
Cohesion: 0.04
...
### Community 20 - "llm/router.py"
Cohesion: 0.02
Nodes (89): _load(), main(), Scheduled portfolio intelligence sweep. Run by `.github/workflows/portfolio-…, int, FailoverResult, A successful completion plus the accounting its callers need., text_key(), failover_chat_completion_via_router() (+81 more)

### Community 21 - "_fixture"
Cohesion: 0.01
Nodes (118): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., setup_database_moks() (+110 more)
...
### Community 23 - "brain_config.py"
Cohesion: 0.04
Nodes (77): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, is_north_mini_code_default(), provider_api_keys() (+69 more)
...
### Community 25 - "ExecutionRequest"
Cohesion: 0.05
Nodes (37): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), ExecutionRequest, Select the best specialist(s) for the classified domain., Issue final pass/fail verdict., Log KPIs for autonomous operation tracking., Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY… (+29 more)
...
Cohesion: 0.04
Nodes (68): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget() (+60 more)

### Community 30 - "Task"
Cohesion: 0.03
Nodes (84): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., Full task/issue document., Task, _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing(), Any (+76 more)
...
### Community 34 - "AgentSessionStore"
Cohesion: 0.04
Nodes (57): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep (+49 more)

### Community 35 - "test_unit8_model_catalog.py"
Cohesion: 0.04
Nodes (70): Resolve the model id for a component's role on a provider. Parameters…, resolve_component_model(), CatalogActiveBrain, CatalogMirror, CatalogProviderEntry, get_catalog(), get_model_catalog_store(), _include_router_providers() (+62 more)

### Community 36 - "WebsiteScanner"
Cohesion: 0.03
Nodes (75): main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), _is_safe_url(), _looks_like_bot_challenge(), BeautifulSoup, True if the HTML is an anti-bot/CAPTCHA interstitial rather than content. Used… (+67 more)

### Community 37 - "loop.py"
Cohesion: 0.03
Nodes (101): loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, Feed LLM token/cost spend into the governance session budget. Kept separate…, _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, cost_for_tokens(), Return the USD cost for (prompt_tokens, completion_tokens) on *model*. Returns… (+93 more)
...
Cohesion: 0.05
Nodes (57): DetectedIssue, ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity, _now(), Any, Enum (+49 more)

### Community 41 - "get_db"
Cohesion: 0.03
Nodes (114): auth_me(), auto_recommend_skills(), brain_failover_status(), create_github_pr(), delete_api_key(), delete_github_token(), _delete_limited_chat_session(), delete_mcp_server() (+106 more)
...
Cohesion: 0.06
Nodes (60): _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir(), _env_key_names(), expand_env(), _fields_of() (+52 more)

### Community 46 - "failover_client.py"
Cohesion: 0.05
Nodes (54): _auto_disable(), _build_request(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool() (+46 more)
...
Cohesion: 0.04
Nodes (44): MongoDBStore, Any, ObjectId, MongoDB implementation of the Company Graph store. Uses Motor (async MongoDB…, Get or create the MongoDB database connection., Convert string ID to ObjectId., Convert ObjectId to string., Prepare a Pydantic model for MongoDB storage. (+36 more)

### Community 49 - "UserRole"
Cohesion: 0.06
Nodes (60): _get_github_token_for_user(), Fetch GitHub token for user from secrets store or environment., UserRole, _can_read(), _can_write(), create_secret(), _decrypt(), delete_secret() (+52 more)
...
Cohesion: 0.06
Nodes (57): _domain_list_reason(), _domain_matches(), _parse_domain_list(), Return True if *host* is *pattern* or a subdomain of it., Split a comma-separated env var into a normalised domain list., Return why the env-var domain policy blocks *host*, or None. Checks…, Zero-key internet access: pages, YouTube transcripts, search, RSS. Every public…, WebReach (+49 more)

### Community 52 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (75): BudgetTracker, GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., AgentIdentity, _coerce_text(), new_session_id(), Any (+67 more)
...
Cohesion: 0.06
Nodes (35): _as_list(), _coerce_payload(), Any, RuntimeError, packages/integrations/render_mcp.py — Render platform access over MCP. The…, Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return…, Unwrap a nested envelope such as ``{"service": {...}}`` when present. (+27 more)

### Community 55 - "SQLiteStore"
Cohesion: 0.03
Nodes (60): DetectedSystem, A business system detected on a company's website or in their stack., Get the most confident evidence description., Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., Create a new company in SQLite., Get a company by ID from SQLite., Update a company in SQLite. (+52 more)

### Community 56 - "test_unit6_resolve_component_model.py"
Cohesion: 0.04
Nodes (51): invalidate_brain_config_cache(), Clear the singleton's cache (used by tests + brain_policy invalidation)., Convenience: resolve all four role models for a component. Returns a dict with…, resolve_component_role_models(), A PATCH persists + the next get returns the applied config., invalidate() clears the cache so the next get re-reads from storage., set_brain_config writes to both Mongo AND the sqlite mirror., With no cache and no requested, the env var wins. (+43 more)
...
Cohesion: 0.05
Nodes (80): classify_task(), _extract_recent_text(), Any, Task classification from request context. Classifies an incoming request into a…, Concatenate plain text from the last *last_n* messages., Return the most likely task category for this request. Args: messages: OpenAI-…, Reset the singleton and clear the cached model map (test helper)., reset_router() (+72 more)

### Community 62 - "test_ceo_supervision.py"
Cohesion: 0.05
Nodes (75): CEOResult, _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, Aggregated output from a multi-specialist execution., A subtask's full history: what it is, and every attempt at it., SubtaskRecord, CEOResult serializes every field used by callers., CEOResult.verdict defaults to 'OK' (the delegate() method computes the real… (+67 more)
...
Cohesion: 0.06
Nodes (46): _get_director(), is_commercial_provider(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Return the process TrafficDirector, or None if it is unavailable., Infer an access tier from *hostname*, longest matching suffix winning. Returns… (+38 more)

### Community 65 - "BrainWatchdog"
Cohesion: 0.03
Nodes (66): _migrate_brain_to_safe_default(), get_brain_config(), get_brain_config_store(), provider_key_present(), Return the process-wide ``BrainConfigStore`` singleton., Convenience wrapper used by the agent loop + brain resolver., True when the env var for *provider*'s key is set (or it's Ollama)., Any (+58 more)
...
Cohesion: 0.04
Nodes (70): FailureCategory, heal_signature(), HealingEvent, HealState, _now(), Any, Enum, str (+62 more)

### Community 71 - "get_company_graph_store"
Cohesion: 0.03
Nodes (71): autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, CompanyAgencyService, get_company_agency_service(), _is_runtime_available_sync(), _pick_available_runtime(), Any, SpecialistFamily (+63 more)

### Community 72 - "test_runtime_governance.py"
Cohesion: 0.11
Nodes (40): Replace the process-wide audit log. Tests only., reset_audit_log(), Replace the process-wide engine. Tests only., reset_policy_engine(), isolated_governance(), Give every test its own engine, audit log, and approval store., isolated_governance(), _decision() (+32 more)

### Community 73 - "Specialist"
Cohesion: 0.03
Nodes (52): SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, SpecialistFamily, SystemType (+44 more)
...
Cohesion: 0.03
Nodes (101): get_improvement_loop(), _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord (+93 more)
...
Cohesion: 0.05
Nodes (61): create_access_token(), create_refresh_token(), github_callback(), github_login(), github_repo_access(), github_repo_callback(), google_callback(), google_login() (+53 more)
...
Cohesion: 0.08
Nodes (35): generate_standup(), Build a :class:`StandupReport` from ``.claude/state/active-tasks.md``. Reads…, _bug_scores(), _clean(), _default_repo(), _env_github_token(), estimate_job_size(), fetch_github_signals() (+27 more)

### Community 87 - "test_provider_router.py"
Cohesion: 0.05
Nodes (49): _acquire_provider_probe(), extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), Try to acquire a distributed probe lock for *provider_id*. Returns True if this…, Release the probe lock for *provider_id*., Normalize NVIDIA base URLs to avoid double /v1 when openai_compat_url appends…, _release_provider_probe() (+41 more)
...
Cohesion: 0.04
Nodes (55): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable(), _playwright_configured(), _playwright_spec() (+47 more)
...
Cohesion: 0.04
Nodes (81): all_provider_ids(), _build_base_url_env_from_yaml(), _build_candidates_from_yaml(), _build_default_base_url_from_yaml(), _build_display_names_from_yaml(), _build_key_env_from_yaml(), _build_presets_from_yaml(), _build_tier_from_yaml() (+73 more)
...
Cohesion: 0.06
Nodes (39): _age_seconds(), AgentScheduler, _now(), agent/scheduler.py — Scheduled Agent Jobs Cron-based job scheduler. Each job…, Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Register a new job. Returns the created :class:`ScheduledJob`.…, Fire a job immediately (webhook / manual trigger)., Remove a job. Returns *True* if it existed. (+31 more)
...
Cohesion: 0.07
Nodes (34): engine(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test(), _tracking_run_single() (+26 more)
...
### Community 107 - "DirectChatSession"
Cohesion: 0.14
...
### Community 108 - "HermesAdapter"
Cohesion: 0.03
Nodes (53): Resolve the model id to force for a code-execution run, or ``None``. Returns…, resolve_coding_model_preference(), _env_float(), HermesAdapter, Any, AsyncClient, Read a float env var, falling back to *default* on unset/garbage., Adapter for Hermes Agent — FIRST CLASS autonomous runtime. (+45 more)

### Community 109 - "test_provider_state_durability.py"
Cohesion: 0.09
Nodes (15): Drop the cached Mongo client, backoff, and read caches (tests)., reset_kv_state(), one_configured_provider(), A registry with exactly two known providers and isolated state., fake_mongo(), _FakeCollection, _FakeDb, isolated_state() (+7 more)

### Community 110 - "get_task_store"
Cohesion: 0.05
Nodes (67): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., apply_from_db(), Load the stored overrides and apply them. Called once at startup., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec() (+59 more)
...
Cohesion: 0.05
Nodes (49): CEODispatcher, _runner_factory(), _complexity_rank(), _decompose_into_subtasks(), _offload(), Any, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is… (+41 more)
...
Cohesion: 0.05
Nodes (36): get_tool_registry(), _infer_parameters_from_func(), Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed., Look up a tool by name. (+28 more)
...
### Community 118 - "test_colibri_brain_shim.py"
Cohesion: 0.06
Nodes (52): colibri_enabled(), colibri_provider_config(), colibri_status(), providers/colibri.py — Free local GLM-5.2 brain served by JustVugg/colibri.…, Return True iff the operator opted in via ``COLIBRI_ENABLED=true``., Cheap status snapshot for tests + admin UI., Return the ``ProviderConfig`` for the local colibri server, or ``None`` when…, _safe_priority() (+44 more)
...
Cohesion: 0.07
Nodes (74): BackgroundTasks, add_comment(), approve_checkpoint(), approve_execution(), clarify_task(), create_task(), _current_user(), delete_task() (+66 more)
...
Cohesion: 0.03
Nodes (103): _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy(), PolicyUpdateBody, Any (+95 more)
...
### Community 131 - "resolve_e2b_config"
Cohesion: 0.05
...
### Community 136 - "Company"
Cohesion: 0.05
Nodes (22): BusinessCategory, Company, Any, field_validator, The core company entity - root of the Company Graph., Coerce unrecognised system_type values to 'custom' so the model never crashes…, Get a company by ID. Args: company_id: Company ID Returns: Company instance or…, Update a company. Args: company_id: Company ID **kwargs: Fields to update… (+14 more)
...
Cohesion: 0.06
Nodes (27): TestTeamSummary, Path, tests/test_artifact_store.py — Unit tests for workflow/artifact_store.py., Verify artifacts that are stored as JSON (e.g., CheckRun results)., Writing the same (run_id, name) twice should update, not duplicate., store(), TestArtifactStoreDeletion, TestArtifactStoreJSONArtifact (+19 more)
...
Cohesion: 0.07
Nodes (25): AgentRole, _fake_artifact(), TestCheckRun, TestSlice, Artifact, CheckRun, Any, A durable, phase-produced document (markdown or JSON). (+17 more)
...
### Community 161 - "TestAnthropicWorkspaceIdCapture"
Cohesion: 0.33
...
Cohesion: 0.03
Nodes (50): north_mini_code_model_for(), Return the North Mini Code model id served by *provider*, else ``None``.…, best_model_for(), best_vision_model(), get_registry(), ModelCapability, Model capability registry. Defines the known local models, their strengths, and…, # NOTE: suspended under US export-control directive as of 2026-06-12. (+42 more)

### Community 164 - "RenderFinding"
Cohesion: 0.05
Nodes (33): _run(), _latest_metric_value(), _note_recurrence(), _parse_timestamp(), Any, datetime, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Pull the most recent numeric sample out of a ``get_metrics`` payload. Render… (+25 more)

### Community 165 - "TaskPriority"
Cohesion: 0.08
...
Cohesion: 0.05
Nodes (56): provider_api_key(), provider_base_url(), Return the OpenAI-compatible base URL for *provider* (env- and UI-aware)., Return the live API key for *provider* (env-only — never persisted)., _describe_http_status(), probe_model_liveness(), _probe_ollama(), _probe_openai_compat() (+48 more)

### Community 169 - "AgentRunner"
Cohesion: 0.03
Nodes (76): AgentRunner, _run(), _check_extra_kwargs(), _enforce_signature(), Any, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin… (+68 more)

### Community 170 - "PortfolioManager"
Cohesion: 0.05
Nodes (28): PortfolioManager, PortfolioMetrics, Aggregate metrics across the whole portfolio., Manages a portfolio of initiatives with WSJF prioritisation and roadmapping., Remove an initiative from the portfolio., Number of initiatives in the portfolio., Return initiatives sorted by WSJF (highest first). Cancelled initiatives are…, Greedily fill ``capacity`` (in job-size units) by WSJF priority. Walks the… (+20 more)
...
Cohesion: 0.06
Nodes (19): AdminAuthManager, AdminIdentity, AdminSession, AdminSessionStore, _is_truthy(), admin_auth.py — auto-generated module docstring (user-research skill scan)., WindowsCredentialAuthenticator, patch (+11 more)
...
Cohesion: 0.11
Nodes (24): BrainFailoverExhausted, RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, Paid-tier providers admitted to the chain and not yet attempted. Empty when the…, _untried_paid(), _FM, _P, Regression tests for a chain that fails silently. From a real incident:… (+16 more)
...
Cohesion: 0.06
Nodes (19): AdaptiveHalter, Any, ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, Return current halter state for logging / telemetry., Tracks step-level progress and signals when a run should halt early. The halter…, Ratio of applied steps to steps attempted (0.0–1.0). Returns 1.0 when no steps…, Record one step outcome; return a halt reason or None to continue. ``status``…, MCPToolResult (+11 more)

### Community 200 - "lifespan"
Cohesion: 0.04
Nodes (48): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled(), onboarding_gate_enabled_cached() (+40 more)

### Community 201 - "InternalAgentAdapter"
Cohesion: 0.07
Nodes (25): _best_cloud_primary_base(), InternalAgentAdapter, _nvidia_provider_chain(), Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Execute a TaskSpec using the internal AgentRunner and convert the agent's…, Create an isolated execution context for a single task. Tries ``git worktree… (+17 more)
...
Cohesion: 0.05
Nodes (52): packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, ABC, Any, AsyncClient, packages/llm/providers/base.py — the provider contract and the generic adapter.…, Parse ``Retry-After`` (and common vendor variants) into seconds. (+44 more)
...
### Community 209 - "test_daily_2026_07_27.py"
Cohesion: 0.07
...
### Community 213 - "test_compare_runtimes.py"
Cohesion: 0.10
Nodes (8): _immediate(), asyncio, tests/test_compare_runtimes.py — the runtime comparison harness. The point of…, Adapter success only means the CLI ran, not that the answer was right., A healthy runtime whose every task raised must not exit 0. `execute()` can…, TestExecution, TestExitCodes, TestReviewRegressions
...
Cohesion: 0.05
Nodes (62): Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, TaskStatus, Record the human decision on a task's **pre-execution** approval gate. A…, Re-run a task. Permissive across every non-active state so the dashboard Retry…, Add a follow-up instruction to a task and re-queue it for execution. Unlike…, Owns lifecycle transitions, comment semantics, and approval rules., TaskWorkflowService (+54 more)

### Community 216 - "Provider"
Cohesion: 0.09
Nodes (20): Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., Ollama provider adapter — local LLM inference., packages.ai — provider abstraction, model registry, and failover manager., ProviderManager, packages/ai/manager.py — ProviderManager. Single entry point for all LLM calls.…, Coordinates provider selection, failover, and health., Return providers sorted by priority (lowest = highest priority)., Check health of all configured providers. (+12 more)

### Community 217 - "ApprovalStore"
Cohesion: 0.05
Nodes (38): Event, publish(), packages/events/bus.py — In-process event bus. Loosely couples components via…, An event published on the bus., Subscribe to an event type., Publish an event to all subscribers., subscribe(), ApprovalRequest (+30 more)
...
Cohesion: 0.06
Nodes (39): Attempt, _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One delegation of one subtask to one tier. (+31 more)
...
Cohesion: 0.05
Nodes (45): analyze_qualitative(), analyze_quantitative(), _classify_sentiment(), _extract_keywords(), plan_research(), Any, BaseModel, field_validator (+37 more)
...
### Community 229 - "_FakeCommandResult"
Cohesion: 0.20
...
### Community 232 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 241 - "test_llm_router_resilience.py"
Cohesion: 0.03
Nodes (70): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), get_ring(), KeyRing (+62 more)
...
### Community 244 - "get_skill_bindings"
Cohesion: 0.11
Nodes (22): build_matrix(), _families(), main(), Any, scripts/generate_specialist_skill_matrix.py — Specialist × Skill matrix.…, Map family -> sorted list of test files that mention it (quoted token)., Return one row per family, derived entirely from code., render_markdown() (+14 more)
...
Cohesion: 0.05
Nodes (63): _extract_mentioned_paths(), Autonomous agent: fetch oldest open issue, generate implementation, create PR.…, Extract plausible file paths from issue text., Read existing files for codebase context (max 8000 chars total)., _read_grounding_files(), _decline(), _fetch_failure_context(), _list_target_prs() (+55 more)
...
### Community 257 - "_resolve_user_github_token"
Cohesion: 0.11
...
### Community 259 - "ai/router.py"
Cohesion: 0.05
Nodes (49): clear_cooldowns(), CommercialFallbackRequiredError, _dead_model_key(), _exponential_backoff_cooldown(), is_provider_on_cooldown(), _mark_model_dead(), mark_provider_failed(), _notify_watchdog() (+41 more)
...
Cohesion: 0.06
Nodes (18): get_scheduler_store(), _MemCollection, _MemCursor, _MemDB, _MemDeleteResult, Any, services/scheduler_store.py — Durable scheduler persistence. Issue #505:…, Delete a persisted job. (+10 more)
...
Cohesion: 0.06
Nodes (41): agents/__init__.py — CRISPY multi-agent coding system., AgentProfile, _catalog_defaults(), _catalog_provider(), _get_defaults(), load_all_profiles(), make_architect_profile(), make_coder_profile() (+33 more)

### Community 266 - "test_backend_server_features.py"
Cohesion: 0.07
Nodes (17): _append_agent_session_message(), _mask_observations(), Path, Truncate tool/observation content in older messages to prevent context bloat., _run_agent_loop(), anyio, Tests for backend/server.py cloud model catalog, multi-agent loop, and context…, Planner and Verifier should be assigned to reasoning (DeepSeek/QwQ) models. (+9 more)
...
Cohesion: 0.11
Nodes (21): ProviderResult, _auth_headers(), test_agent_status_endpoint_reports_live_progress_and_tool_calls(), test_agent_stream_endpoint_emits_server_sent_events(), test_chat_send_emits_langfuse_observation_for_direct_chat(), test_chat_send_keeps_complex_prompt_on_direct_path_when_agent_mode_is_off(), test_chat_send_keeps_explanatory_github_pr_guidance_on_direct_path(), test_chat_send_keeps_general_docker_explanation_on_direct_path_when_no_repo_action_is_requested() (+13 more)

### Community 270 - "test_governance_api.py"
Cohesion: 0.07
Nodes (43): Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient, Tests for the governance HTTP surface and the AgentRunner integration. The…, Policy is a git-reviewed file. An HTTP mutation route would make "who changed…, The tool that makes turning on enforcement safe. (+35 more)

### Community 271 - "test_llm_router_disabled.py"
Cohesion: 0.08
Nodes (41): auto_disable(), _billing_signals(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable., Persist a provider as disabled, through the store that already owns it., The one list of "you have no money" phrases, borrowed not copied. Lazy because… (+33 more)

### Community 272 - "test_runtimes.py"
Cohesion: 0.05
Nodes (32): Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry, Admin-configurable routing policy. Defaults are conservative (local-first, no…, Routes a TaskSpec to the best available runtime following the configured…, Selects an available runtime for the given task type, preferring a specified…, Return the last *limit* routing decisions (newest first). (+24 more)
...
Cohesion: 0.08
Nodes (37): observability_diag_public(), PUBLIC diagnostic endpoint for Langfuse — no auth required. Returns exactly…, CommercialEquivalent, estimate_commercial_equivalent_usd(), get_prices(), _load_from_env(), _parse_mapping(), Any (+29 more)
...
Cohesion: 0.05
Nodes (51): filter_output(), FilterResult, get_output_filter(), get_savings_summary(), OutputFilter, agent/output_filter.py — LLM Output Compression & Token Savings Inspired by…, Filter and compress command outputs to reduce LLM token consumption. Provides…, Compact git status output — keep only changed file paths. (+43 more)
...
Cohesion: 0.08
Nodes (22): AdvisoryMemory, _compose_context(), _format_company_context(), Grounding, _keywords(), _mem_key(), Any, agent/executive_advisory.py — an intelligent C-suite business-advisory layer.… (+14 more)
...
Cohesion: 0.08
Nodes (28): cached_models(), dead_models(), discover_models(), _fresh_entry(), mark_dead(), _models_url(), _parse_ids(), Any (+20 more)
...
### Community 339 - "_ensure_tasks_source_id_unique_index"
Cohesion: 0.10
Nodes (23): _agent_provider_failure_response(), _ensure_tasks_source_id_unique_index(), _build(), _is_index_options_conflict(), Exception, True when *exc* is Mongo refusing to redefine an existing index. Mongo raises…, Add a unique **partial** index on tasks.source_id — isolated from the main…, Fall back to a direct LLM call when the agent loop cannot reach any provider.… (+15 more)
...
Cohesion: 0.05
Nodes (30): build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, _normalized_provider_type(), _ollama_reasoning_effort(), ProviderRouter, Any, Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either…, Translate an OpenAI chat/completions payload to Bedrock Converse format. (+22 more)
...
### Community 368 - "test_llm_router_tpm.py"
Cohesion: 0.05
Nodes (56): build_summariser(), summarise(), ContextManager, estimate_tokens(), FitResult, message_tokens(), prune(), Any (+48 more)

### Community 369 - "ScheduleStore"
Cohesion: 0.10
Nodes (18): _backend(), _json_default(), Any, agent/schedule_store.py — durable persistence for scheduled agent jobs. Fixes…, Return all persisted schedule docs (for boot rehydration)., Persist (insert or update) a single schedule by job_id., Delete a persisted schedule., Atomically delete a run-once row only while it is still unfired. Closes the… (+10 more)
...
Cohesion: 0.17
Nodes (23): _bootstrap(), Path, tests/test_webui_provider_priority.py — Priority + reorder + brain-policy…, The /policy/brain endpoint must return the resolved brain + the paid policy…, The /providers/role-tags endpoint surfaces brain/sub/fallback roles consistent…, Reset the brain_config + brain_policy singletons before each test. V2.0 Phase 2…, _reset_brain_singletons(), test_admin_policy_brain_returns_resolution_and_paid_state() (+15 more)

### Community 385 - "webui/router.py"
Cohesion: 0.13
Nodes (20): AdminCommandBody, _anthropic_chat_payload(), _anthropic_text(), BrainPolicyUpdate, _provider_headers(), _provider_kind(), ProviderReorderBody, Any (+12 more)
...
Cohesion: 0.07
Nodes (43): BoundContext, ClassifyOutput, ExecutionResult, get_workflow_orchestrator(), JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput (+35 more)
...
Cohesion: 0.06
Nodes (41): _autonomy_bg_cycle(), autonomy_tick(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_trigger(), Background CEO cycle + task dispatch. Runs fire-and-forget., Execute ONE pending task synchronously. Called by the cron workflow every 2…, Called by Cloudflare Cron every minute. Protected by CRON_SECRET header. (+33 more)
...
### Community 396 - "validate_outbound_url"
Cohesion: 0.14
...
Cohesion: 0.08
Nodes (36): allow_paid_brain(), get_brain_preference(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, parametrize, Operator can set BRAIN_PREFERENCE=colibri without it being rejected., Defensive default: unknown values coerce to nvidia (cloud default)., Regression guard: full allowlist stays intact across edits. (+28 more)
...
### Community 409 - "register_user_research_tools"
Cohesion: 0.19
...
### Community 419 - "_is_denied_path"
Cohesion: 0.16
Nodes (7): _extract_pytest_failure(), _is_denied_path(), Return a rejection reason, or "" if *path* is allowed., Pull the real pytest failure out of a raw Actions job log. Full CI job logs…, tests/test_autonomous_fix.py — the autonomous CI-fix bot's safety gates.…, TestExtractPytestFailure, TestIsDeniedPath
...
### Community 431 - "WorkspaceManager"
Cohesion: 0.18
...
Cohesion: 0.12
Nodes (14): Analysis, analyze(), _failure_table(), main(), Return the exhaustion :class:`Analysis` for a run's captured *output*., scripts/analyze_exhaustion.py Decide whether a run that ran out of retries has…, The verdict plus the human-readable justification behind it., Tests for ``scripts/analyze_exhaustion.py``. The case that matters most is the… (+6 more)
...
Cohesion: 0.07
Nodes (30): PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), check_all() (+22 more)
...
Cohesion: 0.12
Nodes (24): _fake_user_auth(), Path, test_admin_can_create_anthropic_provider_via_webui_admin_api(), test_admin_can_create_provider_via_webui_admin_api(), test_ui_providers_and_workspaces_use_app_state(), _atomic_write_json(), default_store_paths(), get_data_dir() (+16 more)
...
### Community 480 - "test_brain_config_store.py"
Cohesion: 0.07
Nodes (32): default_brain_config(), Return ``(max_output_tokens, request_timeout_sec)`` for an agent LLM call. Same…, Return the safe-default brain (used on first boot + store errors)., Return the recommended default brain based on which provider keys are present.…, recommended_brain_config(), resolve_agent_generation_sync(), _isolated_store(), tests/test_brain_config_store.py — BrainConfigStore unit tests. Pins the… (+24 more)
...
### Community 483 - "ApplyReviewAgent"
Cohesion: 0.22
...
### Community 488 - "call_llm"
Cohesion: 0.07
Nodes (35): _agent_timeout_fallback_response(), _build_provider_router(), _builtin_provider_records(), call_llm(), _chat_error_detail_text(), _chat_provider_policy(), chat_send(), _run_agent_job() (+27 more)
...
### Community 493 - "TestAdapterMetadata"
Cohesion: 0.08
Nodes (9): Any, Adapter for a compatible external task harness., TaskHarnessAdapter, Verify all adapters have required metadata without needing a running runtime., TestAdapterMetadata, TestTaskHarnessAdapterExecution, When task-harness is required but not present, preflight should return a…, Every preflight report must have runtime_id, ready, summary. (+1 more)
...
### Community 496 - "BrainFailoverManager"
Cohesion: 0.09
...
### Community 540 - "parse_event_stream"
Cohesion: 0.09
...
Cohesion: 0.14
Nodes (7): E2: Classify a failure from its description text. Order matters:…, _classify(), Classify *text* with the repo's own classifier. Imported lazily:…, _classify_failure correctly identifies infrastructure errors., MongoDB timeout is an infra error, not a generic timeout., MongoDB 'connection refused' is infra, not generic network., TestSelfHealingInfrastructureClassification

### Community 547 - "test_one_model_catalogue.py"
Cohesion: 0.10
Nodes (15): _hardcoded_candidates(), Model ids belong in a catalogue, not in Python. Four files in this repo each…, A frozen record of the divergence found on 2026-08-28. Reconciling these eleven…, These two are the ones this work reconciled, and each was reconciled because…, A shrinking divergence is progress, but the list must be trimmed to match or it…, ``config/llm/models.yaml`` drives capability filtering. An entry for a retired…, A model served by more than one provider is declared once. ``openai/gpt-…, Groq's rotation must contain a real, tool-capable model — the three ids it used… (+7 more)
...
Cohesion: 0.09
Nodes (12): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, Register the built-in agent tools that are always available., Register the interactive-browser capability (agent/browser.py). Unlike…, Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_browser_tools(), _browse_page_tool(), _register_builtin_tools() (+4 more)
...
### Community 564 - "OrchestratorSupervisor"
Cohesion: 0.10
Nodes (11): OrchestratorSupervisor, Any, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor(), SupervisorState, tests/test_orchestrator_supervisor.py — Tests for deterministic supervisor…, Supervisor startup, stall detection, and alert emission. (+3 more)
...
### Community 568 - "brain_failover.py"
Cohesion: 0.12
...
### Community 596 - "test_provider_410_per_model.py"
Cohesion: 0.15
...
### Community 599 - "compare_runtimes.py"
Cohesion: 0.16
...
### Community 616 - "router/health.py"
Cohesion: 0.16
...
### Community 622 - "get_ceo_dispatcher"
Cohesion: 0.11
...
### Community 624 - ".test_all_cooling_returns_none_instead_of_resetting"
Cohesion: 0.10
...
### Community 656 - "resolve_free_nvidia_brain"
Cohesion: 0.12
...
### Community 657 - "NvidiaProvider"
Cohesion: 0.13
Nodes (6): NvidiaProvider, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., RateLimit, Provider rate limit info., Return the provider's rate limits.

### Community 658 - "RuntimeReadinessReport"
Cohesion: 0.04
Nodes (33): make_isolated_workspace(), Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Create an isolated workspace directory under *root*. This is the legacy path…, _workspace_component(), Fail preflight when the provider extension is configured but missing. Without…, Any, Structured, actionable preflight validation issue. (+25 more)
...
### Community 703 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.33
...
### Community 704 - "WorkflowEngine"
Cohesion: 0.09
Nodes (26): Any, Connection, Path, PhaseType, CRISPY workflow engine — phase sequencer + gate controller. GATE: Golden Path…, Return the AgentSwarm singleton if available, else None., Append an event to the workflow event log., Create a new WorkflowRun and begin pre-gate phase execution. The run is… (+18 more)
...
Cohesion: 0.12
Nodes (18): clear_cost_attribution(), cost_table(), Return the active per-model cost table (USD per million tokens). Useful for…, Reset in-memory cost attribution counters. Admin-only., _build_cost_table(), clear_stats(), get_cost_table(), get_stats() (+10 more)
...
### Community 733 - "TestOrchestratorQueue"
Cohesion: 0.13
...
### Community 734 - "scan_repo_with_user_research.py"
Cohesion: 0.22
...
### Community 737 - "TestFailoverRegistryBaseUrls"
Cohesion: 0.40
...
### Community 742 - "TestTheGatewayRegistryOffersOnlyLiveModels"
Cohesion: 0.22
...
### Community 821 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 825 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 846 - "TestModelsEndpointAliases"
Cohesion: 0.31
...
### Community 850 - "get_failover_manager"
Cohesion: 0.09
...
### Community 856 - "TestAnthropicPayloadStructuredOutput"
Cohesion: 0.29
...
### Community 879 - "ChatResponse"
Cohesion: 0.10
Nodes (7): CerebrasProvider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., OllamaProvider, Any, Send a chat request with automatic failover. Retry policy: 1. If ``model`` is…, ChatResponse, Standard response from a provider chat call.
...
### Community 882 - "scanner.py"
Cohesion: 0.06
Nodes (47): Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., Inferred technology stack from website/repo analysis., StackInference, WebsiteScanResult, _content_contains_domain(), _hostname_contains() (+39 more)
...
### Community 887 - "test_probe_base_url_override_hits_the_typed_url"
Cohesion: 0.22
...
### Community 1000 - "test_hermes_server.py"
Cohesion: 0.18
...
Cohesion: 0.15
Nodes (11): brain_providers(), openclaw_mobile_ui(), Every configured provider with its health AND its on/off state. Powers the…, Mobile web UI for iOS control of the agency. Open this on your iPhone, tap…, describe_disabled_reason(), True when operator state is stored somewhere that survives a redeploy. False…, Render a stored disable reason for display. Returns ``code`` (the HTTP status…, state_is_durable() (+3 more)
...
### Community 1039 - "default_executives"
Cohesion: 0.25
...
### Community 1061 - "_get_current_user"
Cohesion: 0.25
...
### Community 1065 - "telegram_service.py"
Cohesion: 0.12
...
### Community 1138 - "_clean_director"
Cohesion: 0.50
...
### Community 1281 - "docs/changelog.md"
Cohesion: 0.29
Nodes (6): Added, Changed, Changelog, Fixed, Maintenance, [v4.1.0]

### Community 1282 - "[5.0.0]"
Cohesion: 0.29
Nodes (7): [5.0.0], Added, Changed, Fixed, Maintenance, Removed, Security

### Community 1283 - "Changelog"
Cohesion: 0.29
Nodes (7): Added, Changed, Changelog, Fixed, Maintenance, Removed, Security
...
- **3548 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3543 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14417 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **171 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `ClaudeCodeAdapter`, `build_llm_router`, `test_llm_router_queue_cache.py`, `test_langfuse_agency_wide.py`, `test_portfolio_intake.py`, `TaskStore`, `AgentDefinition`, `test_llm_router_strategies.py`, `test_operational_incidents.py`, `TestNormalizeResponseFormat`, `test_platform_controls.py`, `brain_config.py`, `test_agent_tool_governance.py`, `Agency`, `build_governance_router`, `parse_event_stream`, `test_autonomy_status.py`, `test_brain_patch_service_token.py`, `test_catalogue_probe.py`, `test_freebuff_bot.py`, `test_harness_spec.py`, `Task`, `loop.py`, `E2BSandboxSession`, `test_telegram_freebuff.py`, `ImprovementLoop`, `test_unit8_model_catalog.py`, `test_failover_client_shared.py`, `resolve_active_brain`, `config.py`, `UserRole`, `PrimeAgentAdapter`, `test_company_api.py`, `test_phase5_doctor.py`, `test_startup_warmup.py`, `KeyPool`, `test_model_router.py`, `BrainWatchdog`, `services/seo_audit.py`, `nvidia_models.py`, `_Budget`, `SelfHealingAgent`, `test_runtime_governance.py`, `Specialist`, `e2e/test_browser.py`, `services/background.py`, `test_openclaw_endpoints.py`, `WorkflowRun`, `test_sam_livekit.py`, `test_provider_410_per_model.py`, `test_provider_router.py`, `claim`, `WorkflowRun`, `TestClient`, `test_procedural_memory.py`, `CEOSupervisor`, `SQLiteStore`, `test_knowledge_sync.py`, `HermesAdapter`, `test_provider_state_durability.py`, `get_ceo_dispatcher`, `_clean_director`, `test_voice_pipeline.py`, `test_colibri_brain_shim.py`, `TestChatHistoryStore`, `tasks/api.py`, `test_refresh_agent_built_proof.py`, `test_telegram_task_callback.py`, `TokenBudget`, `ChatHistoryStore`, `resolve_e2b_config`, `persist_plan_spec`, `test_context_rulebook.py`, `ArtifactStore`, `test_llm_router_e2e.py`, `test_new_features_e2e.py`, `KeyStore`, `test_trend_watcher.py`, `_resolve_brain_provider`, `TestHarnessAdapter`, `diagnostics.py`, `_step`, `quick_note.py`, `AgentRunner`, `test_e2b_data_flow.py`, `test_e2b_task_wiring.py`, `test_telegram_auto_approve.py`, `_is_exempt`, `test_openclaw_gateway.py`, `test_monitor_lib.py`, `ResearchOrchestrator`, `TestSwarmRoleRouting`, `OllamaCircuitBreaker`, `_job_text`, `test_video_transcript.py`, `test_daily_2026_07_27.py`, `TaskStatus`, `ApprovalStore`, `test_features_api.py`, `cost_tracker.py`, `GoalRecord`, `models/seo_audit.py`, `TrendWatcher`, `MCPClient`, `test_llm_router_resilience.py`, `get_skill_bindings`, `TestEstimateTokensForMessages`, `test_telegram_webhook.py`, `_resolve_user_github_token`, `.on_task_complete`, `ai/router.py`, `SchedulerStore`, `test_agile_api.py`, `test_app_settings.py`, `AgentSwarm`, `ContextPruner`, `TestFeaturesAPI`, `test_llm_router_disabled.py`, `emit_chat_observation`, `test_rate_limiter.py`, `NotificationDispatcher`, `test_control_plane_api.py`, `test_persistent_memory.py`, `SecurityScanner`, `test_mcp_governance.py`, `controls_app`, `TestClient`, `test_nvidia_model_discovery.py`, `test_purge_backlog.py`, `patched_agent_runner`, `test_provider_enable_disable.py`, `test_sam_voice.py`, `test_local_brain_state.py`, `test_kimi_bridge_server.py`, `OutputFilter`, `model_discovery.py`, `analyze_page`, `session_retro.py`, `test_agency_fix.py`, `test_skill_registry_boot_refresh.py`, `_start_ceo_agency`, `get_failover_manager`, `TestTheWorkflowIsSafeAndReadOnly`, `test_scheduler_hydration_bounded.py`, `test_task_clarification.py`, `test_executive_advisory_api.py`, `test_llm_router_tpm.py`, `scanner.py`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_process_quick_note_workflow.py`, `test_webui_provider_priority.py`, `isolated_telegram_config`, `_Recorder`, `_undeclared`, `tests/conftest.py`, `test_agent_free_brain.py`, `test_service_token.py`, `test_telegram_diag_endpoint.py`, `test_dashboard_cache.py`, `test_gh_brain_failover.py`, `PreflightReport`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `captured`, `test_v4_api.py`, `UsageEvent`, `_redact_for_notification`, `test_brain_config_store.py`, `client`, `TestMongoGate`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `TaskSpec`, `backend/server.py`, `StuckDetector`, `workflow_orchestrator.py`, `proxy.py`, `ContextPruner`, `test_backend_server_features.py`, `test_governance_api.py`, `test_agent_chat_integration.py`, `GitHubTools`, `test_agent_runner.py`, `test_agent_tool_governance.py`, `Agency`, `build_governance_router`, `diagnostics.py`, `WorkspaceTools`, `ReactScratchpad`, `MultiAgentSwarm`, `AgentSessionStore`, `direct_chat.py`, `loop.py`, `E2BSandboxSession`, `test_agent_free_brain.py`, `test_failover_client_shared.py`, `FreeBuffAgent`, `test_empirical_verify.py`, `LocalWorkspace`, `UserMemoryStore`, `BrainFailoverExhausted`, `AdaptiveHalter`, `InternalAgentAdapter`, `WorkflowRun`, `get_failover_manager`, `ContextManager`, `RewardScorer`, `MCPClient`, `get_ceo_dispatcher`, `CEODispatcher`, `TokenBudget`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `get_workflow_orchestrator()` connect `workflow_orchestrator.py` to `backend/server.py`, `_resolve_user_github_token`, `get_db`, `admin_digest_router.py`, `WorkflowRun`, `admin_update_task_router.py`, `OrchestratorSupervisor`, `telegram_bot.py`, `ExecutionRequest`, `TestHarnessAdapter`?**
