...
## Corpus Check
- 1528 files · ~2,222,850 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 32410 nodes · 66704 edges · 1298 communities (1122 shown, 176 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 6578 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `15feb2ff`
...
- test_daily_automation_2026_08_21.py
- JsonConfigStore
...
## Communities (1298 total, 176 thin omitted)

### Community 0 - "llm/router.py"
Cohesion: 0.02
Nodes (202): int, itertools, packages_llm, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., get_cache(), The process-wide cache manager. (+194 more)

### Community 1 - "backend/server.py"
Cohesion: 0.01
Nodes (437): set_skill_registry(), get_harness_adapter(), admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry (+429 more)

### Community 2 - "_fixture"
Cohesion: 0.02
Nodes (109): _clear_discovered_models(), _clear_response_cache(), _isolate_operator_provider_state(), MonkeyPatch, Path, Default all tests to legacy workflow mode (Phase 2 compatibility). Only patches…, Keep the suite out of the developer's real operator-state database.…, Drop the per-provider model listings between tests.… (+101 more)

### Community 3 - "proxy.py"
Cohesion: 0.03
Nodes (164): Describes a single worker's task and constraints., WorkerSpec, set_quick_note_queue(), get_sam(), fastapi_middleware_cors, middleware, admin_control(), admin_create_user() (+156 more)

### Community 4 - "TaskSpec"
Cohesion: 0.05
Nodes (85): abc, N2. Surface Hermes (and all runtimes) status in the Doctor/Runtimes UI ⬜  (size: S, risk: low), kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor. (+77 more)

### Community 5 - "CacheManager"
Cohesion: 0.07
...
Cohesion: 0.01
Nodes (286): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+278 more)
...
### Community 9 - "api.js"
Cohesion: 0.01
Nodes (51): approveGovernanceRequest(), autoRecommendCompanySkills(), chatSend(), createMcpServer(), createQuickNote(), createTask(), deleteGithubToken(), deleteMcpServer() (+43 more)

### Community 10 - "brain_config.py"
Cohesion: 0.02
Nodes (131): _migrate_brain_to_safe_default(), Architecture (per plan §3), Files touched, Hard constraints (from the plan) — all met, Implementation — DB-persisted, UI-switchable Brain (PR #824 follow-up), New files, Resolution precedence, Risks & mitigations (per plan §6) (+123 more)

### Community 11 - "ModelConfig"
Cohesion: 0.04
Nodes (45): LLMConfig, _merge_env_defaults(), ModelConfig, Capability and pricing metadata for one model id., The fully-resolved routing configuration., Add providers that are configured in the environment but absent from YAML. This…, Give every provider's default model a registry entry if YAML omitted it., Re-read configuration from disk. Used by the admin API and by tests. (+37 more)
...
### Community 14 - "Task"
Cohesion: 0.04
Nodes (57): ApprovalCheckpoint, ExecutionLogEntry, Any, field_validator, Full task/issue document., Update the updated_at timestamp., Single entry in a task's execution log., Human approval gate in a task's execution. (+49 more)
...
Cohesion: 0.02
Nodes (126): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, fnmatch, ipaddress, guard(), identity_from_headers(), _load() (+118 more)
...
Cohesion: 0.05
Nodes (96): Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, build_subtask_brief(), _coerce_subtasks(), decompose(), _extract_json_object(), fallback_decomposition(), MicroManagerConfig, next_tier() (+88 more)

### Community 22 - "test_governance_enforcement.py"
Cohesion: 0.03
Nodes (102): ApprovalRequest, ApprovalStatus, ApprovalStore, Any, Enum, str, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`…, Register a pending approval and return it. Arguments are scrubbed on the way… (+94 more)
...
Cohesion: 0.05
Nodes (75): get_improvement_loop(), _dispatch_async(), _run(), get_log_monitor(), LogMonitor, _note_recurrence(), Best-effort: hand an operational failure to the incident tracker. Import is…, Best-effort, synchronous: tell the healer this signature was seen again. (+67 more)

### Community 29 - "Agency"
Cohesion: 0.05
Nodes (46): Agency, AgencyCycleResult, AgentDirective, AgentRole, _build_ceo_prompt(), _collect_recent_git_context(), get_agency(), _now_str() (+38 more)

### Community 30 - "Added"
Cohesion: 0.02
Nodes (131): Build the complete enrichment block (tools + skills). Returns empty string when…, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Inject available tools + skills catalog into the system message.…, Cooldown + hourly cap. Caller must hold the lock. Reserves the slot on success.…, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Added, Changed, Added (+123 more)
...
Cohesion: 0.02
Nodes (111): A0. Fix live scanner crashes on real-world sites (`services/scanner.py`) — do first, P1 — Close the remaining product gaps, Task 6 — Deeper website/repo scanning (more than "2 systems"), Task 7 — Knowledge base auto-reflect (ECC / Obsidian / graphify-style), Task 8 — Vector retrieval for the knowledge base (self-learn), DetectedSystem, Evidence, Result of a website scan with detected systems and stack inference. (+103 more)

### Community 33 - "MongoDBStore"
Cohesion: 0.02
Nodes (97): Security, Security, MongoDBStore, Any, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., Create a new company in SQLite. (+89 more)
...
Cohesion: 0.03
Nodes (76): Fixed, count, Fixed, Preferring local, Writing a custom adapter, Choosing one, provider_health, Costs are higher than expected (+68 more)
...
Cohesion: 0.03
Nodes (86): CEOResult, _decompose_into_subtasks(), get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, reset_ceo_dispatcher() (+78 more)
...
Cohesion: 0.02
Nodes (135): AgentPhaseError, AgentRunner, _check_extra_kwargs(), _enforce_signature(), Any, Exception, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic… (+127 more)
...
Cohesion: 0.04
Nodes (43): API, getActivity(), V5App, HubTabs(), ActivationGate(), activityToAlert(), AlertsBell(), priorityConfig (+35 more)

### Community 47 - ".add_specialist"
Cohesion: 0.14
...
Cohesion: 0.07
Nodes (23): field_validator, Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoSiteFindings, Run an audit from synchronous code, loop-safe. Used by the skill-bindings…, Crawls a site and produces a complete SeoAuditReport., Pick the fetch backend for this run (http / browser / auto). (+15 more)
...
Cohesion: 0.08
Nodes (46): audit_drift(), _cmd_audit(), DriftReport, _grade(), load_registry(), load_registry_sync(), loop_readiness(), LoopRegistry (+38 more)
...
Cohesion: 0.04
Nodes (83): _build_quick_note_instruction(), _close_github_issue(), _fetch_github_quick_notes(), _gh_repo(), _gh_token(), Close exhausted quick-note issues; dispatch Dev directives for open ones., Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).… (+75 more)

### Community 56 - "WorkflowOrchestrator"
Cohesion: 0.03
Nodes (46): Changed, Beta, Config Overrides, Disabled (demoted per issue #467 Section I), Enforcement, Experimental, Feature Maturity / Support Matrix, Maturity Tiers (+38 more)

### Community 57 - "get_task_store"
Cohesion: 0.03
Nodes (90): _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody, Shared purge implementation for the admin endpoint and the boot hook., Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec() (+82 more)

### Community 58 - "RenderOpsMonitor"
Cohesion: 0.06
Nodes (27): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, One deploy, normalised from whatever shape the tool returned., Fetch one deploy by ID., Build a ``RenderDeploy`` from an upstream deploy object., RenderDeploy, _to_deploy() (+19 more)
...
Cohesion: 0.06
Nodes (59): 1. Render (production backend + worker), 2. Cloudflare Worker (frontend), 3. Local development machines, 4. GitHub secrets, 5. MongoDB collections, Post-Merge Environment Cleanup Guide, Post-merge verification checklist, Rollback (+51 more)

### Community 61 - "seo_portfolio_bridge.py"
Cohesion: 0.05
Nodes (54): CapacityAllocation, InitiativeStatus, Enum, Agentic Portfolio Management — initiative prioritisation, capacity allocation,…, Result of fitting initiatives into a fixed capacity by WSJF priority., Total job size of initiatives that fit within capacity., Unused capacity after committing the selected initiatives., Fraction of capacity consumed (0.0–1.0). (+46 more)
...
Cohesion: 0.04
Nodes (62): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget() (+54 more)

### Community 69 - "Fixed"
Cohesion: 0.02
Nodes (112): _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, Fixed, Bug Log, Next Action, A failed drive does not abandon the goal, Fixed, getCompanyGraph() (+104 more)
...
Cohesion: 0.04
Nodes (68): analyze_qualitative(), analyze_quantitative(), _classify_sentiment(), _extract_keywords(), plan_research(), Any, BaseModel, field_validator (+60 more)
...
Cohesion: 0.05
Nodes (42): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), stop_orchestrator_supervisor(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY… (+34 more)
...
Cohesion: 0.08
Nodes (50): evals_cost_aware_routing, Cost-aware routing evaluation harness. Measures the cost-aware subagent routing…, main(), CLI for the cost-aware routing evaluation. python -m evals.cost_aware_routing…, ModelPrice, Model pricing for the cost-aware routing evaluation. Rates are first-party…, Per-token price for one model, in USD per 1,000,000 tokens., Override the price for a tier (e.g. a discounted or partner rate). (+42 more)

### Community 79 - "test_model_catalog.py"
Cohesion: 0.04
Nodes (77): packages_ai, _build_base_url_env_from_yaml(), _build_candidates_from_yaml(), _build_default_base_url_from_yaml(), _build_display_names_from_yaml(), _build_key_env_from_yaml(), _build_presets_from_yaml(), _build_tier_from_yaml() (+69 more)
...
Cohesion: 0.05
Nodes (25): get_procedural_memory(), _overlap_score(), ProceduralMemoryStore, ProceduralRecord, Any, Store a successful step pattern and return its record id. Duplicate step…, Return up to *limit* stored patterns relevant to *query*. Relevance is scored…, Return all stored records (for admin/debug). (+17 more)

### Community 90 - "failover_client.py"
Cohesion: 0.05
Nodes (55): Parse an LLM tool-call response, tolerating common malformations. Handles: 1.…, Spawn a child AgentRunner for a delegated sub-task. When sub-agent configs are…, Fixed, Fixed, _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), FailoverResult (+47 more)
...
Cohesion: 0.04
Nodes (65): build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, Record token usage for *model* (fire-and-forget, never raises). ``tag`` is a…, record_usage(), _exponential_backoff_cooldown(), is_commercial_provider(), _normalized_provider_type(), _ollama_reasoning_effort() (+57 more)
...
Cohesion: 0.21
Nodes (4): Last-resort fallback that asks builtwith.com what it already knows about a…, Replace curl_cffi's AsyncSession.get with a canned response., _scanner(), TestBuiltWithFallback
...
Cohesion: 0.05
Nodes (39): functools, get_current_trace_id(), get_tracer(), langfuse_metadata_with_trace(), _NoOpSpan, _NoOpTracer, otel_middleware_factory(), otel_status_error() (+31 more)

### Community 117 - "activation_api.py"
Cohesion: 0.09
...
Cohesion: 0.04
Nodes (42): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, _infer_parameters_from_func(), Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed. (+34 more)
...
### Community 124 - "os"
Cohesion: 0.02
Nodes (205): Browser admin UI for login, service control, key management, and diagnostics., ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered) Runs the…, agent/background.py — Background Agent An always-on worker thread that…, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, Cached LLM Client wrapper. Drop-in wrapper around any LLM API call that…, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata…, context_pruner.py — auto-generated module docstring (user-research skill scan). (+197 more)
...
Cohesion: 0.14
Nodes (18): heal_signature(), HealState, Enum, str, Lifecycle of a heal (Autonomy Charter G2 closed loop)., Stable signature for an error/heal, used to dedup and detect recurrence.…, If the re-dispatch coroutine raises, the heal escalates instead of stranding., test_regress_redispatch_failure_escalates() (+10 more)
...
Cohesion: 0.05
Nodes (50): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+42 more)
...
Cohesion: 0.10
Nodes (26): Any, Path, run_command(), _admin_out(), register_webui(), _admin_app_index(), _admin_app_spa(), admin_bootstrap() (+18 more)
...
### Community 136 - "test_hermes_warm_and_severity.py"
Cohesion: 0.11
...
Cohesion: 0.07
Nodes (51): contextvars, BoundContext, ClassifyOutput, ExecutionResult, JudgeVerdict, MergeDecision, MonitorOutput, _orchestrator_bypass() (+43 more)
...
Cohesion: 0.07
Nodes (31): _clean_e2b_env(), _FakeAsyncSandboxClass, _FakeCommandResult, _FakeCommands, _FakeFiles, _FakeSandbox, patched_async_sandbox(), Any (+23 more)
...
Cohesion: 0.06
Nodes (45): cosine_similarity(), Cosine similarity between two vectors, 0.0 when either is degenerate., chunk_document(), ContextManager, estimate_tokens(), FitResult, message_tokens(), prune() (+37 more)
...
### Community 151 - "app.py"
Cohesion: 0.10
Nodes (24): chat_completions(), ChatCompletionRequest, _content_to_str(), _ContentPart, health(), lifespan(), list_models(), _Message (+16 more)

### Community 152 - "TaskPriority"
Cohesion: 0.05
Nodes (60): _capability_tags(), create_task_from_oldest_open_issue(), intake_issue(), _issue_labels(), issue_source_id(), map_issue_to_task(), Any, tasks/issue_intake.py — Auto issue → Task intake (Autonomy Charter G3) Turns… (+52 more)
...
### Community 160 - "Added"
Cohesion: 0.09
Nodes (24): Any, Prefer structured data; fall back to text when unavailable., Force the next ``list_tools()`` call to fetch a fresh tool list from the…, Added, Added, What It Does, Specialist × Skill Matrix, BarChart() (+16 more)
...
Cohesion: 0.03
Nodes (118): Workflow Orchestrator Approval Gate (🔴 Telegram gate), get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., _admin_headers(), _answer_callback(), _api_headers(), _check_rate_limit(), cmd_control() (+110 more)
...
### Community 176 - "AdminIdentity"
Cohesion: 0.04
Nodes (60): `admin_auth.py` checklist, `agent/tools.py` checklist, `key_store.py` checklist, `proxy.py` auth middleware checklist, Step 2 — Checklist by module, 1. Authentication & Authorization, Admin Authentication, API Key Authentication (+52 more)
...
### Community 178 - "TestAuthAndTaskOwnership"
Cohesion: 0.05
...
Cohesion: 0.03
Nodes (102): onboarding_gate_enabled(), Async read of the gate default straight from the DB., loops_overview(), Full Loop Engineering fleet view for the UI: the catalogued loops plus the…, _get_cached_tasks(), _get_tasks_cache_lock(), Lock, Return cached task dicts; fetches + caches from TaskStore when stale. Uses… (+94 more)
...
Cohesion: 0.19
Nodes (7): main(), Cross-platform Claude Code launcher for local models. Auto-detects OS and runs…, Execute platform-specific scripts with error handling., Build script arguments., Launch Claude Code, auto-running setup if needed., Show system information and setup status., ScriptRunner

### Community 194 - "test_runtime_governance.py"
Cohesion: 0.11
Nodes (43): _blocked_result(), _governance_check(), Build the failed TaskResult returned when policy denies a dispatch. A result…, Evaluate a runtime dispatch against policy; audit it either way. Returns…, _decision(), _engine(), Any, Governance on runtime dispatch — the last coverage gap.… (+35 more)

### Community 195 - "NotificationDispatcher"
Cohesion: 0.05
Nodes (42): NotificationDispatcher, _send(), _send(), Any, Path, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess). (+34 more)
...
Cohesion: 0.06
Nodes (31): PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, test_specialized_runtime_execution(), check_all(), execute(), readiness_check() (+23 more)

### Community 202 - "WorkspaceTools"
Cohesion: 0.04
Nodes (55): Register the built-in agent tools that are always available., _register_builtin_tools(), CLAUDE.md — agent/, Skills worth invoking here, Testing, What this package does, Path, Return the first *lines* lines of a file. Just-in-time retrieval: the executor… (+47 more)
...
Cohesion: 0.07
Nodes (38): _build_anthropic_response(), _content_block_to_text(), _emit_safely(), _finish_reason_to_stop_reason(), get_local_model(), handle_anthropic_messages(), _messages_to_openai(), _openai_choice_to_anthropic_content() (+30 more)
...
### Community 210 - "commercial_equivalent.py"
Cohesion: 0.50
...
### Community 212 - "AgentScheduler"
Cohesion: 0.03
Nodes (70): gc, _age_seconds(), AgentScheduler, _now(), Any, Reconstruct a ScheduledJob from its as_dict() output., Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Register a new job. Returns the created :class:`ScheduledJob`.… (+62 more)

### Community 213 - "TaskStatus"
Cohesion: 0.04
Nodes (90): Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, TaskStatus, Record the human decision on a task's **pre-execution** approval gate. A…, Re-run a task. Permissive across every non-active state so the dashboard Retry…, Add a follow-up instruction to a task and re-queue it for execution. Unlike…, Owns lifecycle transitions, comment semantics, and approval rules., TaskWorkflowService (+82 more)

### Community 214 - "test_agent_tool_governance.py"
Cohesion: 0.03
Nodes (79): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log. (+71 more)

### Community 215 - "ProviderRouter"
Cohesion: 0.06
Nodes (26): ProviderRouter, Priority-ordered LLM provider fallback with health checks and retries., main(), probe_one(), Live probe — makes a tiny real API call to every configured provider. Usage (in…, _bedrock_api_response(), _bedrock_provider(), _mock_boto3() (+18 more)
...
Cohesion: 0.09
Nodes (32): Weighted Shortest Job First score — higher schedules sooner., map_initiative_to_task(), materialize_committed(), _wsjf(), _portfolio_materialize_enabled(), portfolio_source_id(), Any, Content-derived stable id for a portfolio initiative. Initiative UUIDs… (+24 more)
...
Cohesion: 0.10
Nodes (6): Tests for packages/ai/cost_tracker.py — per-model cost attribution. Covers: -…, Verify all Opus models referenced by brain_config are priced., TestClaudeOpusModelCoverage, TestClearStats, TestEnvOverrides, TestGetCostTable

### Community 222 - "test_schedule_backlog_drain.py"
Cohesion: 0.07
...
### Community 223 - "generate_onboarding_questions"
Cohesion: 0.07
Nodes (30): Short label for TUI display: {NAME}:{model}, generate_onboarding_questions(), _get_fallback_questions(), Generate AI-tailored onboarding questions based on detected domain and…, Return hardcoded fallback questions when AI generation fails., (1) & partly (4): "Something went wrong" masks the real error everywhere, (2) & (3): Company creation flow / non-admin gate placement, (5): Tailored questions are hardcoded today (+22 more)

### Community 224 - "test_verification_strategies.py"
Cohesion: 0.18
Nodes (15): agent, cross_verify(), Have an independent agent re-check a completed task's changed files. Returns…, _agent_result(), Tests for agent/verification_strategies.py (cross_verify + race).…, Shape a fixture the way the real AgentRunner.run() actually returns., The review instruction asks the agent not to modify files, but nothing in…, StubRunner (+7 more)
...
### Community 227 - "allow_paid_brain"
Cohesion: 0.09
...
Cohesion: 0.10
Nodes (21): Run doctor checks before execution., Issue final pass/fail verdict., In-flight state for a single golden-path execution., WorkflowRun, tests/test_orchestrator_checkpoint.py — Tests for durable checkpointing (#522)., Save, load, list, delete, and restore operations., TestOrchestratorCheckpointStore, _company() (+13 more)
...
### Community 245 - "test_backend_server_features.py"
Cohesion: 0.05
...
### Community 250 - "pydantic"
Cohesion: 0.03
Nodes (77): _company_advisory_context(), Pull a compact company profile for the C-suite advisory, or None. Best-effort:…, BaseModel, backend/admin_local_brain_router.py — admin-session proxy for the local-brain…, ToggleBody, _expected_admin_secret(), _extract_admin_token(), BaseModel (+69 more)
...
Cohesion: 0.05
Nodes (38): Removed, Removed, getMe(), login(), logout(), App(), AppRoutes(), frontend_src_app_default (+30 more)

### Community 260 - "Autonomy Charter — Telegram-Gated Self-Running Agency"
Cohesion: 0.14
...
Cohesion: 0.05
Nodes (53): colibri_enabled(), colibri_provider_config(), colibri_status(), providers/colibri.py — Free local GLM-5.2 brain served by JustVugg/colibri.…, Return True iff the operator opted in via ``COLIBRI_ENABLED=true``., Cheap status snapshot for tests + admin UI., Return the ``ProviderConfig`` for the local colibri server, or ``None`` when…, _safe_priority() (+45 more)
...
### Community 269 - "AnthropicProvider"
Cohesion: 0.04
Nodes (68): Added, Fixed, collections_abc, Added, Fixed, Modules, AnthropicProvider, Any (+60 more)

### Community 270 - "pr_approval_gate.py"
Cohesion: 0.18
Nodes (21): _card_keyboard(), _card_text(), _dedupe_key(), default_run_sweep(), _gh_get(), _gh_token(), interval_sec(), load_notified() (+13 more)
...
### Community 280 - "RenderEnvError"
Cohesion: 0.15
Nodes (12): RuntimeError, Raised when Render rejects or cannot serve an env-var write., Set one environment variable on a Render service via the REST API. Updates a…, RenderEnvError, update_service_env_var(), _FakeClient, _FakeResp, asyncio (+4 more)
...
### Community 305 - "Added"
Cohesion: 0.03
Nodes (133): True if browser automation is enabled and Playwright is importable., Send chat messages and return the assistant's text output. If an…, Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., Atomically delete a run-once row only while it is still unfired. Closes the…, BudgetExceededError (+125 more)
...
### Community 313 - "CompanyAgencyService"
Cohesion: 0.08
...
### Community 314 - "test_key_pool.py"
Cohesion: 0.08
...
Cohesion: 0.10
Nodes (34): Fixed, Active Task Tracker, Roadmap Items (from `docs/roadmap-killer-todos.md`), Status Key, Fixed, _auth_headers(), _chat_targets(), _dump_matching() (+26 more)
...
Cohesion: 0.06
Nodes (42): DirectChatDoctor, get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor() (+34 more)
...
Cohesion: 0.03
Nodes (58): Strip API keys, tokens, emails, IPs, and private keys from a log line., _redact_sensitive(), pytest, Regression test for /api/auth/me — verifies the critical endpoint on both the…, _StubRuntimeManager, _StubRuntimeRegistry, _StubTask, _StubTaskDispatcher (+50 more)
...
Cohesion: 0.16
Nodes (21): livekit_agents, get_livekit_config(), LiveKitConfig, Resolved LiveKit + speech-provider configuration., Resolve LiveKit configuration from the environment (read fresh each call)., _build_llm(), _build_stt(), _build_tts() (+13 more)
...
### Community 369 - "test_bootstrap_source_id_index.py"
Cohesion: 0.14
...
### Community 380 - "socket"
Cohesion: 0.09
Nodes (19): _is_dns_failure(), _probe_failure_reason(), BaseException, Turn a probe exception into an operator-actionable one-line reason. A dead…, True when *exc* (or anything it wraps) is a name-resolution failure., socket, asyncio, Exception (+11 more)
...
Cohesion: 0.08
Nodes (35): _note_phase_end(), _note_phase_start(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, Log start/elapsed for one phase of the plan/execute/verify loop. A task that…, Open a tracked phase, returning the token that closes it. Never raises., Close the tracked phase *token* opened above. Never raises., _timed_phase() (+27 more)
...
### Community 387 - "Fixed"
Cohesion: 0.11
Nodes (15): FailureCategory, E2: Classify a failure from its description text. Order matters:…, Classified failure types for targeted self-healing (E2)., ma(), Fixed, Fixed, _build_request(), Return ``(url, headers, is_anthropic_native)`` for *provider*. Anthropic's… (+7 more)
...
Cohesion: 0.12
Nodes (22): `db/sqlite_store.py`, fastapi_staticfiles, AdminCommandBody, _anthropic_chat_payload(), _anthropic_text(), BrainPolicyUpdate, _provider_headers(), _provider_kind() (+14 more)
...
Cohesion: 0.08
Nodes (35): Adding a new tool, build_tool_prompt(), _load_script_module(), Any, ModuleType, Dynamically load a pure-stdlib helper module from .github/scripts/. Returns…, Read a web page as plain text, following the same fallback chain (direct ->…, GET *url*, re-validating every redirect hop against ``unsafe_target_reason`` —… (+27 more)
...
### Community 409 - "monitor_colibri.py"
Cohesion: 0.08
...
Cohesion: 0.03
Nodes (72): ast, Example: Access your home PC models from any machine. Install: pip install…, build_review_context(), _gh(), main(), Aggregate all review feedback for the PR into a single context string., main(), _nvidia_candidates() (+64 more)
...
Cohesion: 0.09
Nodes (22): Propagate the calling agent's identity across the process boundary. The MCP…, 1. What makes this system different from a normal web app, 2. Assets, 3. Trust boundaries, 4. Threats, 5. Why the engine fails open but approvals fail closed, 6. Honest limits, 7. Priority follow-ups (+14 more)
...
Cohesion: 0.06
Nodes (27): 1.11 Multi-Agent Governance (10 / 100 / 1000 agents), Failure behaviour, detect_backend(), E2BBackend, LocalBackend, Any, RuntimeError, Raised when no backend can provide the requested sandbox. Callers treat this… (+19 more)
...
Cohesion: 0.13
Nodes (20): TestClient, tests/test_v4_api.py — Tests for the v4 dashboard API endpoints., Return the test client — reuses conftest client which has bootstrap., GET /v4/status returns 200 with improvement_loop and self_healing keys., GET /v4/improvements returns 200 with active and resolved lists., GET /v4/tasks returns 200 with tasks array., GET /v4/scheduler/jobs returns 200 with jobs array., GET /v4/quick-notes returns 200 with notes array. (+12 more)
...
### Community 476 - "services/seo_audit.py"
Cohesion: 0.05
Nodes (38): _run(), Run real validation (byte-compile + scoped pytest) on this step's changes. Opt-…, bs4, csv, get_status(), get, Serve the launcher UI., Get current service status. (+30 more)
...
### Community 491 - "LLM Router — troubleshooting"
Cohesion: 0.09
...
Cohesion: 0.13
Nodes (19): CEODispatcher, _complexity_rank(), services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Real CEO delegation: wake runtimes, fan out, and merge results., _runtime_id_for_role(), _should_fan_out(), _base_result(), Tests for CEODispatcher._maybe_cross_verify (opt-in risky-module re-check). (+11 more)

### Community 493 - "ClaudeCodeAdapter"
Cohesion: 0.18
Nodes (17): ClaudeCodeAdapter, json_safe(), Any, Adapter for Claude Code CLI — FIRST CLASS autonomous coding runtime., adapter(), asyncio, Tests for runtimes/adapters/claude_code.py, test_adapter_metadata() (+9 more)

### Community 494 - "Fixed"
Cohesion: 0.11
Nodes (20): Normalise a raw planner JSON response into a consistent plan dict. Handles: -…, Fixed, Current Sprint Tasks, Fixed, call_brain_with_failover(), POST *messages* to each candidate in order; return the first success. Returns…, The first tool call, for callers that only ever expect one., Deactivate a company's AI agency. Stops all company-specific schedules and… (+12 more)
...
### Community 500 - "test_voice_pipeline.py"
Cohesion: 0.14
...
### Community 547 - "test_catalogue_probe.py"
Cohesion: 0.05
Nodes (46): date, Popen, _capture(), _login(), main(), Capture README screenshots of every v5 screen from a locally-running server.…, Launch the local uvicorn server (activated, sqlite, loops off) for capture., _start_server() (+38 more)
...
### Community 549 - "_seed_requires_approval_task"
Cohesion: 0.10
...
### Community 558 - "emit_agency_observation"
Cohesion: 0.10
Nodes (23): _base_url(), _department_trace_tags(), emit_agency_observation(), _emit_langfuse_http_sync(), _emit_sdk(), _env_val(), get_langfuse_client(), _langfuse_enabled() (+15 more)
...
### Community 576 - "get_tool_registry"
Cohesion: 0.11
...
### Community 600 - "4. Troubleshooting"
Cohesion: 0.10
...
### Community 602 - "test_iteration_6_features.py"
Cohesion: 0.05
...
### Community 604 - "test_social_login_browser.py"
Cohesion: 0.12
Nodes (17): playwright_sync_api, do_login(), fail(), ok(), Page, Navigate to a page and verify it loads without errors., Browser-based E2E tests — runs with Playwright against a live LLM Relay…, Verify server responds to health check before running browser tests. (+9 more)
...
### Community 606 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 616 - "AUTONOMY_CHARTER.md"
Cohesion: 0.11
...
### Community 624 - "test_brain_default_model.py"
Cohesion: 0.12
...
Cohesion: 0.07
Nodes (26): CEO Micro-Management, Configuration reference, Escalation, and why it terminates, Five bounds, Operator surface, Tests, The 24x7 supervisor, The anti-slop gate (+18 more)
...
### Community 629 - "parse_event_stream"
Cohesion: 0.15
...
### Community 633 - "StreamingDeltaReconstructor"
Cohesion: 0.26
...
Cohesion: 0.20
Nodes (4): tests/test_probe_report.py — the catalogue-probe drift-report step.…, TestBuildBody, TestFindTrackingIssue, TestIsRetired
...
### Community 646 - "sys"
Cohesion: 0.02
Nodes (143): detect_secrets(), main(), Recover CHANGELOG.md from a Git merge conflict in its [Unreleased] block. Pre-…, argparse, db — storage abstraction layer (V2.0 Phase 5: real code moved to…, # IMPORTANT: keep these imports LAZY (inside __getattr__) so that a Mongo-only, Backward-compatibility shim — use scripts/generate_api_key.py instead. This…, _load() (+135 more)
...
Cohesion: 0.18
Nodes (15): classify_direct_chat_intent(), _contains_keyword(), detect_intent(), intent.py — Intent classification for direct chat (answer_only, execute_now,…, Return True if content contains any execution or analysis keyword., Detect the user's intent from message content., Map lower-level intents into conversation-driven action categories. Returns one…, classify_plain_text() (+7 more)
...
### Community 692 - "admin_digest_router.py"
Cohesion: 0.22
...
Cohesion: 0.17
Nodes (12): A worked example, Adding capacity: multi-key rotation, Attribution, Configuration, Failure behaviour, Observability, Pre-call budget checks, Provider ids contain dashes (+4 more)
...
### Community 706 - "HealthStatus"
Cohesion: 0.13
Nodes (7): NvidiaProvider, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., Check health of all configured providers., HealthStatus, Provider health check result., Check provider health.

### Community 707 - "Request"
Cohesion: 0.13
...
### Community 715 - "fastapi_testclient"
Cohesion: 0.13
Nodes (10): fastapi_testclient, MonkeyPatch, TestClient, Regression test for PUT/DELETE /api/github/token returning 500 for SQLite-…, TestGithubTokenSQLiteRegression, _auth_headers(), TestClient, test_agent_profile_api_preserves_ui_fields() (+2 more)

### Community 716 - "test_nightly_regression_workflow.py"
Cohesion: 0.24
...
### Community 717 - "test_company_api.py"
Cohesion: 0.11
...
Cohesion: 0.16
Nodes (8): MCPToolResult, Result from ``call_tool_structured()``. ``structured`` is populated when the…, tests/test_daily_automation_2026_07_13.py — Daily automation tests…, Unit tests for agent.mcp_client.MCPToolResult., Verify loop.py imports and wires AdaptiveHalter correctly., AgentRunner must expose _adaptive_halter on construction., TestLoopAdaptiveHalterIntegration, TestMCPToolResult
...
### Community 737 - "test_phase5_doctor.py"
Cohesion: 0.11
...
### Community 748 - "OnboardingScreen.jsx"
Cohesion: 0.15
Nodes (13): Phase 4 — Onboarding fixes (#593, #619, PR #623) ⏳, createCompany(), getOnboardingProgress(), getSetupState(), OnboardingScreen, detectBusinessCategory(), detectSiteType(), DiscoveryStep() (+5 more)
...
### Community 750 - "Native operations"
Cohesion: 0.12
...
Cohesion: 0.05
Nodes (34): Direct Chat Evolution: Seamless Assistant Architecture, Failure Recovery, Overview, Runtime Selection Policy, 3b. Hermes — **our own** Hermes server (in-repo), UI-wired ✅  (size: M), Resolve the base URL of the agency's own Hermes server. Precedence:…, resolve_hermes_base_url(), Any (+26 more)
...
### Community 767 - "test_kimi_bridge_provider.py"
Cohesion: 0.20
...
### Community 777 - "_self_heal_ready"
Cohesion: 0.13
...
### Community 778 - "test_refresh_agent_built_proof.py"
Cohesion: 0.12
...
### Community 788 - "test_new_features_e2e.py"
Cohesion: 0.26
...
### Community 790 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 796 - "test_task_service_failed_comment.py"
Cohesion: 0.21
...
### Community 805 - "467 Brutal Audit — File-by-File Status"
Cohesion: 0.14
...
### Community 823 - "Free NVIDIA brain + UI-controlled provider policy + no silent spend"
Cohesion: 0.14
...
### Community 827 - "RenderFinding"
Cohesion: 0.06
Nodes (25): _note_recurrence(), _parse_timestamp(), datetime, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Run one full scan and return every finding, filed or not. Never raises: a…, Scan, then file everything that is past its cooldown. The cooldown is claimed…, Map service ID → human name, degrading to the ID when unavailable., Flag a failed latest deploy, or one that has been building too long. (+17 more)

### Community 828 - "test_provider_render_env.py"
Cohesion: 0.20
...
### Community 836 - "GitHubPolicyProbe"
Cohesion: 0.19
...
### Community 839 - "test_daily_automation_2026_08_21.py"
Cohesion: 0.26
Nodes (6): asyncio, tests/test_daily_automation_2026_08_21.py — Daily automation tests…, Verify the workspace-id header is captured from Anthropic API responses., _parse must work without passing workspace_id (backwards compat)., chat() must read anthrophic-workspace-id from the response headers., TestAnthropicWorkspaceIdCapture

### Community 840 - "JsonConfigStore"
Cohesion: 0.19
...
### Community 841 - "test_openclaw_gateway.py"
Cohesion: 0.14
...
### Community 846 - "race"
Cohesion: 0.17
Nodes (9): Any, race(), Run *n* independent attempts at *instruction* concurrently; return the winner.…, _score_result(), The eight gaps and what closed them, RunnerFactory, get_reward_scorer(), Return the module-level RewardScorer singleton. (+1 more)
...
### Community 853 - "test_scanner_headless.py"
Cohesion: 0.18
...
### Community 870 - "TestGemini3xCostTracker"
Cohesion: 0.27
...
### Community 889 - "test_deploy_trigger_covers_image.py"
Cohesion: 0.21
...
Cohesion: 0.15
Nodes (12): _env(), tests/test_render_mcp.py — Render MCP integration. Covers: - MCPClient…, The Go binary hardcodes :10000 and ignores $PORT, so Render must be told., In HTTP mode the token comes from the caller's header, so nothing here is…, The mux serves /mcp only; a health check there opens an SSE stream that never…, pkg/oauth fails the process at boot on a partial OAuth config., The backend must use the private-network address, not the public URL., RenderOps polling is deliberately throttled to cut outbound egress on the free… (+4 more)

### Community 893 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 911 - "test_app_settings.py"
Cohesion: 0.21
...
### Community 952 - "TestGeminiCostTrackerEntries"
Cohesion: 0.26
...
### Community 983 - "test_server_autonomy_and_index_fixes.py"
Cohesion: 0.20
...
### Community 996 - "_extract_pytest_failure"
Cohesion: 0.31
...
### Community 998 - "ServiceManager"
Cohesion: 0.24
Nodes (3): Start the FastAPI proxy server., ServiceManager, admin_status()

### Community 999 - "ChatResponse"
Cohesion: 0.20
Nodes (7): Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., Ollama provider adapter — local LLM inference., Any, Send a chat request with automatic failover. Retry policy: 1. If ``model`` is…, ChatResponse, Standard response from a provider chat call., packages_config
...
### Community 1003 - "test_brain_failover_413.py"
Cohesion: 0.20
...
### Community 1012 - "_is_denied_path"
Cohesion: 0.33
Nodes (3): _is_denied_path(), Return a rejection reason, or "" if *path* is allowed., TestIsDeniedPath

### Community 1013 - "mask_dict"
Cohesion: 0.24
Nodes (4): mask_dict(), _mask(), Return a copy of *data* with secret values masked. Common secret key names are…, TestMaskSecret
...
### Community 1032 - "GovernanceScreen.jsx"
Cohesion: 0.25
...
### Community 1034 - "_parse_user_ids"
Cohesion: 0.18
Nodes (13): Changed, Fixed, [v4.1.0], Changed, Fixed, [v4.1.0], RoleBadge(), _parse_user_ids() (+5 more)

### Community 1035 - "TestTheProducersAndTheParserCannotDrift"
Cohesion: 0.25
...
### Community 1054 - "PR #634 Implementation Tracker"
Cohesion: 0.25
...
### Community 1056 - "TestItIsNotBuiltForOneVendor"
Cohesion: 0.25
...
### Community 1061 - "test_doctor_coding_brain.py"
Cohesion: 0.32
...
### Community 1066 - "test_mcp_protocol_version.py"
Cohesion: 0.25
...
### Community 1071 - "_ErrorCaptureHandler"
Cohesion: 0.25
...
### Community 1076 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 1077 - "SamConversation"
Cohesion: 0.29
...
### Community 1080 - "_score_attempt"
Cohesion: 0.29
Nodes (7): Heuristic fallback score when the reward model is unavailable.…, _score_attempt(), Regression: _score_attempt used to branch on a top-level "status" key that…, The "error" sentinel is set by race()'s own _attempt() wrapper when the runner…, test_score_attempt_error_sentinel_scores_zero(), test_score_attempt_heuristic_no_steps_is_neutral_not_status_dependent(), test_score_attempt_heuristic_perfect_run()

### Community 1081 - "._maybe_cross_verify"
Cohesion: 0.29
Nodes (6): True if any path matches the repo's risky-module trigger list., touches_risky_module(), _runner_factory(), Post-hoc independent re-check when a completed task touched a risky module.…, test_touches_risky_module_false_for_unrelated_files(), test_touches_risky_module_matches_known_files()
...
### Community 1097 - "Cost-aware routing evaluation"
Cohesion: 0.29
Nodes (5): Caveats, CLI, Cost-aware routing evaluation, How the metric resists gaming, Method

### Community 1099 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1109 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 1119 - ".test_missing_binary_produces_structured_validation_issue"
Cohesion: 0.33
Nodes (4): Preflight must return RuntimeReadinessReport with issues, not raise., execute(), health_check(), required_dependencies()

### Community 1120 - "agile_sprints.py"
Cohesion: 0.50
Nodes (4): Enum, Agentic Agile — Sprint management with velocity tracking and burndown. Issue:…, Status of a user story within a sprint., StoryStatus

### Community 1121 - "Writing policy"
Cohesion: 0.40
...
### Community 1123 - "TestAnthropicDefaultModel"
Cohesion: 0.40
...
### Community 1129 - "test_activity_logs.py"
Cohesion: 0.67
Nodes (3): clear_error_log_buffer(), _auth_headers(), test_activity_endpoint_includes_recent_error_logs()

### Community 1130 - "_InMemoryErrorLogHandler"
Cohesion: 0.50
Nodes (3): _ensure_error_log_capture(), _InMemoryErrorLogHandler, LogRecord

### Community 1131 - "_get_hashed_token"
Cohesion: 0.50
Nodes (4): _get_hashed_token(), _hash_token(), SHA-256 hash the plaintext token. We compare hashes (not plaintext) so the in-…, Return the hashed service token, or None if SERVICE_TOKEN is unset. Caches the…
...
### Community 1136 - "test_skills_route_order.py"
Cohesion: 0.67
...
### Community 1150 - "test_the_reserve_is_bounded_when_read_from_the_environment"
Cohesion: 0.67
...
### Community 1151 - "test_mint_token_rejects_missing_args"
Cohesion: 0.67
...
### Community 1190 - "test_v4_reliability_regression.py"
Cohesion: 0.08
...
- **Why does `AgentRunner` connect `AgentRunner` to `ContextPruner`, `AgentPlan`, `UserMemoryStore`, `get_skill_bindings`, `backend/server.py`, `company_api.py`, `proxy.py`, `MultiAgentSwarm`, `GitHubTools`, `TaskSpec`, `workflow_orchestrator.py`, `PolicyEngine`, `Universality: case-coverage matrix`, `test_contracts_agency.py`, `unsafe_target_reason`, `AdaptiveHalter`, `test_agent_chat_integration.py`, `test_governance_api.py`, `Agency Core — Progress & Resume Log`, `Issue #656: Bugs`, `ReactScratchpad`, `Added`, `failover_chat_completion`, `TestAgentLoopMCPIntegration`, `test_fabric_patterns.py`, `ContextManager`, `Added`, `AgentJobRequest`, `Changed`, `Agent: Implementer (Executor)`, `._maybe_cross_verify`, `WorkflowOrchestrator`, `Killer TODO Roadmap — local-llm-server`, `build_governance_router`, `[Unreleased]`, `StuckDetector`, `FreeBuffAgent`, `Fixed`, `Agent Autonomy Roadmap`, `WorkspaceTools`, `race`, `test_empirical_verify.py`, `Section-by-Section Acceptance Criteria`, `LocalWorkspace`, `HarnessEnrichment`, `Issue #504: EPIC: Autonomy hardening — live-verified defects 2026-06-10`, `test_agent_tool_governance.py`, `test_repo_connection.py`, `failover_client.py`, `AdminScreen.jsx`, `services/seo_audit.py`, `.apply_diff`, `_resolve_push_token`, `TokenBudget`, `allow_paid_brain`, `pytest`, `E2BSandboxSession`, `AgentSessionStore`, `direct_chat.py`, `test_brain_failover_413.py`, `CEODispatcher`, `Fixed`, `InternalAgentAdapter`, `os`, `loop.py`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `ma()` connect `Fixed` to `la`, `HarnessEnrichment`, `de`, `gsap.min.js`?**
...
- **Should `llm/router.py` be split into smaller, more focused modules?**
  _Cohesion score 0.0164638511095204 - nodes in this community are weakly interconnected._
- **Should `backend/server.py` be split into smaller, more focused modules?**
  _Cohesion score 0.009535378074703917 - nodes in this community are weakly interconnected._
- **Should `_fixture` be split into smaller, more focused modules?**
  _Cohesion score 0.017178075006556517 - nodes in this community are weakly interconnected._