...
## Corpus Check
- 1444 files · ~2,078,940 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28714 nodes · 57822 edges · 1192 communities (1064 shown, 128 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 6347 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `223d9282`
...
- github
- test_chat_agent_run_budget_default_is_600
...
## Communities (1192 total, 128 thin omitted)

### Community 0 - "ExecutionRequest"
Cohesion: 0.01
Nodes (248): Execute work through the 11-phase golden path., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_execute(), allow_paid_brain(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, get_orchestrator_checkpoint_store(), _NoopDB (+240 more)
...
Cohesion: 0.03
Nodes (211): Agency, get_agency(), CEO-coordinated multi-agent agency for continuous codebase management. The CEO…, set_agency(), AgentJobRequest, AgentJobSnapshot, Complete point-in-time view of a job, safe to serialise as API response., Convenience: the canonical text response, whether success or failure. (+203 more)

### Community 3 - "backend/server.py"
Cohesion: 0.01
Nodes (458): get_harness_adapter(), admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), auth_me(), auto_recommend_skills(), _bootstrap_within_budget() (+450 more)

### Community 4 - "company_api.py"
Cohesion: 0.05
Nodes (131): AccountLifecycleResponse, _DoctorCheck, _DoctorReport, get_public_doctor_report(), OnboardingAnswersRequest, OnboardingProgressResponse, OnboardingQuestionsRequest, BaseModel (+123 more)

### Community 5 - "SQLiteStore"
Cohesion: 0.03
Nodes (82): MongoDBStore, Any, Company, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., Create a new company in SQLite., Get a company by ID from SQLite. (+74 more)

### Community 6 - "LLMRouter"
Cohesion: 0.03
Nodes (89): Attempt, count, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any, AsyncClient, BaseException (+81 more)

### Community 7 - "AgentRunner"
Cohesion: 0.02
Nodes (159): AgentRunner, _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), _nvidia_api_key(), Any, Path (+151 more)

### Community 8 - "proxy.py"
Cohesion: 0.02
Nodes (360): AgentRole, str, BackgroundAgent, BackgroundTask, _now(), Any, agent/background.py — Background Agent An always-on worker thread that…, Enqueue *task* for processing. Returns the task (with task_id set). (+352 more)
...
Cohesion: 0.04
Nodes (64): _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing(), Any, BaseException, Task, TaskResult, TaskSpec (+56 more)

### Community 12 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (125): IssueCategory, IssueSeverity, Enum, str, agent/improvement_loop.py — Continuous Improvement Engine Background scanner…, _diagnose_and_file(), _file_incident(), _format_incident() (+117 more)

### Community 13 - "direct_chat.py"
Cohesion: 0.03
Nodes (141): PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), Agent subsystem — planner / executor / verifier loop. (+133 more)

### Community 14 - "TaskSpec"
Cohesion: 0.04
Nodes (71): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns… (+63 more)
...
Cohesion: 0.03
Nodes (64): AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., GooseAdapter, Any, Adapter for Goose — TIER 2 general-purpose local runtime., HermesAdapter, Adapter for Hermes Agent — FIRST CLASS autonomous runtime. (+56 more)

### Community 17 - "TestRuntimeControl"
Cohesion: 0.04
...
Cohesion: 0.03
Nodes (105): BrainConfig, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), provider_base_url(), BaseModel (+97 more)
...
Cohesion: 0.07
Nodes (53): governance_enabled(), True when the governance layer should run at all. A single global off switch…, _audit_dispatch(), _blocked_result(), _governance_check(), _governance_identity(), Any, runtimes/routing.py — RuntimeRoutingPolicyEngine. Implements the 8-step routing… (+45 more)

### Community 33 - "test_unit8_model_catalog.py"
Cohesion: 0.03
Nodes (86): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, all_provider_ids(), _provider_ids_from_literal(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the…, True when the env var for *provider*'s key is set (or it's Ollama)., Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the… (+78 more)
...
Cohesion: 0.05
Nodes (21): Typed configuration loaded from environment variables., When True, the governance layer evaluates and audits agent actions. This is…, When True, approval-gated actions self-approve. Local dev only., ``RENDER_SERVICE_IDS`` split into a clean list (empty when unset)., True when there is both an API key and an endpoint to reach., When True, the Render ops loop runs. On by default. Also requires…, When True, mutating Render MCP tools may be called. Default False., When True, agents may drive a real browser (agent/browser.py). (+13 more)
...
Cohesion: 0.04
Nodes (44): api_keys_for(), _digest(), KeyPool, _KeyState, _PoolState, Per-provider API key rotation — the one lever that adds capacity. Every other…, Round-robin key selection with per-key rate-limit cooldowns., Return the next usable key, or None when every key is cooling. With one key… (+36 more)

### Community 47 - "_fixture"
Cohesion: 0.03
...
Cohesion: 0.06
Nodes (59): HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, BreakerState, HealthTracker, _Outcome, ProviderHealth (+51 more)

### Community 51 - "SecretRecord"
Cohesion: 0.05
Nodes (59): _get_github_token_for_user(), Fetch GitHub token for user from secrets store or environment., _can_read(), _can_write(), create_secret(), _decrypt(), delete_secret(), _encrypt() (+51 more)
...
### Community 53 - "AutonomyTracker"
Cohesion: 0.04
Nodes (38): AutonomyCounter, AutonomySnapshot, AutonomyTracker, get_tracker(), Any, agent/kpi.py — Autonomy KPIs: evidence capture and metrics tracking. Tracks key…, Return a point-in-time snapshot of all KPIs., Reset all counters (test helper). (+30 more)
...
### Community 56 - "Surface"
Cohesion: 0.04
Nodes (69): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent., BudgetExceeded, classify(), evaluate_call() (+61 more)
...
### Community 68 - "Task"
Cohesion: 0.05
Nodes (61): Full task/issue document., Task, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, Tests for task follow-up / rerun with conversation carry-over. Before this,…, The runtime reads context['conversation']; prior comments must be carried as…, test_build_spec_exposes_conversation_for_runtime(), test_follow_up_empty_message_rejected() (+53 more)

### Community 69 - "test_mcp_registry.py"
Cohesion: 0.04
Nodes (56): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable(), _playwright_configured(), _playwright_spec() (+48 more)
...
Cohesion: 0.05
Nodes (29): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), AuditEvent, AuditLog, Any, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer… (+21 more)

### Community 76 - "test_webui_provider_priority.py"
Cohesion: 0.19
Nodes (21): _bootstrap(), Path, ProviderManager, WorkspaceManager, tests/test_webui_provider_priority.py — Priority + reorder + brain-policy…, The /policy/brain endpoint must return the resolved brain + the paid policy…, The /providers/role-tags endpoint surfaces brain/sub/fallback roles consistent…, Reset the brain_config + brain_policy singletons before each test. V2.0 Phase 2… (+13 more)
...
Cohesion: 0.05
Nodes (47): _escape_md_v1(), NotificationDispatcher, Path, service_manager.py — Telegram & Notification Integration Extension Extends the…, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess)., Run the bot with stop-event awareness. (+39 more)
...
Cohesion: 0.06
Nodes (62): Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost(), cmd_keylist() (+54 more)

### Community 105 - "AgileSprint"
Cohesion: 0.04
Nodes (50): _bullets(), generate_backlog_retro(), generate_sprint_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive retro notes for ``sprint`` from its current metrics. Records… (+42 more)
...
Cohesion: 0.04
Nodes (43): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, _infer_parameters_from_func(), Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed. (+35 more)
...
### Community 142 - "TestAuthAndTaskOwnership"
Cohesion: 0.05
...
### Community 153 - "test_provider_router.py"
Cohesion: 0.08
Nodes (27): is_anthropic_base_url(), _normalize_nvidia_base_url(), _openai_url(), True when ``base_url`` points at Anthropic's native Messages API. Anthropic's…, Normalize NVIDIA base URLs to avoid double /v1 when openai_compat_url appends…, parametrize, Regression tests for the agent loop's multi-provider failover wire format.…, Registry base URLs must produce routes the provider actually serves. (+19 more)
...
Cohesion: 0.07
Nodes (46): Replace the process-wide store. Tests only., reset_approval_store(), get_audit_log(), Return the process-wide audit log, created on first use., Replace the process-wide audit log. Tests only., reset_audit_log(), Replace the process-wide engine. Tests only., reset_policy_engine() (+38 more)
...
Cohesion: 0.04
Nodes (90): Set the global agent store instance (e.g., with MongoDB on startup)., set_agent_store(), Point the feature stores at the shared database connection. Deliberately…, _wire_feature_stores(), Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum (+82 more)
...
Cohesion: 0.05
Nodes (65): quick_notes_submit(), Submit a quick-note URL or instruction from the dashboard FAB., Reset the singleton (for tests)., reset_failover_manager(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog() (+57 more)
...
Cohesion: 0.09
Nodes (20): filter_output(), FilterResult, OutputFilter, Any, agent/output_filter.py — LLM Output Compression & Token Savings Inspired by…, Filter and compress command outputs to reduce LLM token consumption. Provides…, Compact git status output — keep only changed file paths., Compact git log — one line per commit. (+12 more)

### Community 181 - ".compress"
Cohesion: 0.18
Nodes (9): ContextStats, _estimate_tokens(), Strategy, Drop the oldest non-system messages until under the token threshold., Remove exact-duplicate and near-empty messages., Return a (possibly shorter) copy of *messages* using *strategy*. ``inspect``…, Return token usage stats without modifying *messages*., Return *True* when estimated token count exceeds the threshold. (+1 more)
...
### Community 186 - "test_watchdog.py"
Cohesion: 0.09
...
Cohesion: 0.04
Nodes (25): FeatureEntry, FeatureMatrix, BaseModel, One entry in the support matrix., Central support matrix — single source of truth. Loads the canonical feature…, Return the feature entry if available, or raise FeatureUnavailableError., Return True if the feature is enabled and not disabled., Return a warning string for beta/experimental features, or None. (+17 more)
...
### Community 236 - "get_skill_bindings"
Cohesion: 0.11
...
### Community 248 - "provider_access_tier"
Cohesion: 0.10
Nodes (24): provider_access_tier(), _provider_field(), provider_sort_key(), Infer an access tier from *hostname*, longest matching suffix winning. Returns…, _tier_from_hostname(), Tests that verify the exact provider priority ordering., When INCLUDE_LOCAL_FALLBACK=true and no cloud keys are set, local Ollama must…, ollama-local must NOT appear when NVIDIA_API_KEY is set and… (+16 more)
...
Cohesion: 0.07
Nodes (48): delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), list_seo_audits(), BaseModel, get, post, SEO / GEO / AIO Audit API Router Endpoints for the world-class SEO audit engine… (+40 more)
...
Cohesion: 0.10
Nodes (34): get_failover_manager(), Return the singleton BrainFailoverManager., _clean_env(), _make_manager(), tests/test_brain_failover.py — Universal multi-provider brain failover tests.…, Strip all provider API keys and isolate the operator-state store.…, Status snapshot doesn't leak API keys., Make a fresh manager (bypasses the singleton for isolation). (+26 more)
...
Cohesion: 0.05
Nodes (70): brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The… (+62 more)
...
Cohesion: 0.07
Nodes (27): BrowserAction, _not_started(), PageState, Any, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, Evaluate a JavaScript expression in the page context., Return a summary of the current page state., Screen a URL for SSRF. Fails closed — if the guard cannot be imported the URL… (+19 more)
...
Cohesion: 0.10
Nodes (25): nuclear_cleanup(), Directly delete ALL stale jobs from the DB collection. More aggressive than…, FakeDB, asyncio, tests/test_pr923_fixes.py — regression tests for PR #923 (5 production issues).…, nuclear_cleanup should keep newest job per name, delete duplicates., nuclear_cleanup should gracefully handle a DB without a schedules collection., reconcile_stranded_tasks source code must include a FAILED-task re-queue pass.… (+17 more)
...
Cohesion: 0.02
Nodes (131): AlertHandler, _load(), main(), ModuleType, _write_summary(), _load(), main(), int (+123 more)
...
### Community 317 - "ApprovalCheckpoint"
Cohesion: 0.14
...
### Community 345 - "providers.py"
Cohesion: 0.22
Nodes (9): invalidate_brain_cache(), _normalize_base_url(), _now(), ProviderRecord, ProviderSecret, Any, Reorder the provider priority list. ``provider_ids`` is the desired ordering…, Ensure at least one provider exists, seeded from env when empty. (+1 more)
...
### Community 380 - "test_scheduler.py"
Cohesion: 0.11
...
### Community 411 - "sam_livekit_worker.py"
Cohesion: 0.17
...
### Community 445 - "test_task_service_failed_comment.py"
Cohesion: 0.18
...
### Community 461 - "facade.py"
Cohesion: 0.05
Nodes (53): _get_current_user_thunk(), _get_optional_user_thunk(), Request, create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., Item (+45 more)
...
### Community 501 - "Agent Runtime Setup"
Cohesion: 0.12
...
### Community 507 - "issue_new_api_key"
Cohesion: 0.26
Nodes (11): issue_new_api_key(), Generate a new plaintext API key, persist hash + metadata, return (plain_key,…, main(), _make_store(), Security regression tests for key_store: hashing, constant-time compare, rate…, _reset_rate_state(), test_keys_stored_as_hash_not_plaintext(), test_lookup_success_and_miss() (+3 more)
...
### Community 520 - "test_doctor_coding_brain.py"
Cohesion: 0.38
...
### Community 570 - "Workspace Isolation Architecture"
Cohesion: 0.17
...
Cohesion: 0.07
Nodes (30): AgentJob, make_isolated_workspace(), Path, Create an isolated workspace directory under *root*. This is the legacy path…, _workspace_component(), Create, open, and lifecycle-manage per-job isolated workspaces. All workspaces…, Resolve *relative* within *ws*.source and reject traversal/symlink escapes.…, Remove expired workspaces that are cleanup_eligible and past cleanup_after.… (+22 more)
...
### Community 613 - "get_livekit_config"
Cohesion: 0.18
...
### Community 683 - "_extract_pytest_failure"
Cohesion: 0.42
...
### Community 733 - "start_in_process"
Cohesion: 0.25
...
### Community 748 - "get_registry"
Cohesion: 0.06
Nodes (27): get_registry(), Return model registry, extended with ROUTER_EXTRA_MODELS env entries.…, claude-opus-5 must be in the built-in alias map so routing resolves it., TestClaude5RegistryEntries, test_bedrock_haiku_4_5_in_registry(), test_bedrock_opus_48_in_registry(), test_bedrock_opus_4_6_v1_in_registry(), test_bedrock_opus_4_7_in_registry() (+19 more)

### Community 749 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 750 - "test_task_store_fails_loud_in_production.py"
Cohesion: 0.25
...
### Community 764 - "Workspace Issues"
Cohesion: 0.29
...
Cohesion: 0.04
Nodes (42): ChatResponse, CerebrasProvider, Provider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Provider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is… (+34 more)
...
### Community 792 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 794 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 808 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 883 - "provider_api_keys"
Cohesion: 0.33
...
### Community 886 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 915 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 938 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 939 - "Agent API Issues"
Cohesion: 0.40
...
### Community 940 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 983 - "Issue → Context → Draft PR automation"
Cohesion: 0.50
...
### Community 1010 - "The 24x7 agency — your agents never go idle"
Cohesion: 0.50
...
### Community 1032 - "Feature Maturity Issues"
Cohesion: 0.50
...
### Community 1033 - "Claude Code Specific Issues"
Cohesion: 0.50
...
Cohesion: 0.07
Nodes (35): _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog() (+27 more)
...
### Community 1048 - "Configuration reference"
Cohesion: 0.67
...
### Community 1061 - "test_mint_token_rejects_missing_args"
Cohesion: 0.67
...
- **Why does `_fixture()` connect `_fixture` to `ExecutionRequest`, `test_openclaw_endpoints.py`, `backend/server.py`, `LLMRouter`, `test_doctor_coding_brain.py`, `MCPUnavailableError`, `test_llm_router_queue_cache.py`, `TaskWorkflowService`, `test_operational_incidents.py`, `direct_chat.py`, `TestRuntimeControl`, `SelfHealingAgent`, `CompanyGraphStore`, `test_autonomy_status.py`, `config.py`, `GoalRecord`, `CEOSupervisor`, `test_local_brain_state.py`, `test_unit8_model_catalog.py`, `TestChatHistoryStore`, `test_phase5_doctor.py`, `test_task_brain_preflight.py`, `test_brain_failover_413.py`, `test_llm_router_resilience.py`, `ImprovementLoop`, `test_telegram_diag_endpoint.py`, `ModelRouter`, `KeyPool`, `SecretRecord`, `AutonomyTracker`, `allow_paid`, `test_failover_client_shared.py`, `test_model_router.py`, `clear_cooldowns`, `test_knowledge_sync.py`, `e2e/test_browser.py`, `PrimeAgentAdapter`, `resolve_identity`, `Task`, `AgentSwarm`, `test_webui_provider_priority.py`, `test_e2b_task_wiring.py`, `ResearchTask`, `ToolAnnotations`, `TestClient`, `test_repo_connection.py`, `NotificationDispatcher`, `ProceduralMemoryStore`, `TestHarnessAdapter`, `test_startup_warmup.py`, `test_openclaw_gateway.py`, `_start_ceo_agency`, `OllamaCircuitBreaker`, `test_llm_router_e2e.py`, `SeoFixer`, `test_new_features_e2e.py`, `test_context_rulebook.py`, `ArtifactStore`, `BrainWatchdog`, `_Budget`, `SeoAuditRequest`, `test_provider_state_durability.py`, `test_agent_free_brain.py`, `ChatHistoryStore`, `TestAuthAndTaskOwnership`, `persist_plan_spec`, `test_agent_tool_governance.py`, `test_issue_intake.py`, `test_provider_router.py`, `session_retro.py`, `test_mcp_governance.py`, `test_agile_api.py`, `test_app_settings.py`, `test_features_api.py`, `test_video_transcript.py`, `TaskStore`, `test_task_clarification.py`, `test_platform_controls.py`, `get_task_store`, `test_trend_watcher.py`, `test_portfolio_intake.py`, `TaskIn`, `TrendWatcher`, `OrchestratorQueue`, `test_control_plane_api.py`, `TestEstimateTokensForMessages`, `TestClient`, `TestSwarmRoleRouting`, `ContextPruner`, `test_persistent_memory.py`, `SpecEntry`, `SecurityScanner`, `test_task_store_fails_loud_in_production.py`, `test_doctor_service_token_check.py`, `test_provider_enable_disable.py`, `test_rate_limiter.py`, `test_brain_failover.py`, `services/background.py`, `reap_expired_companies`, `brain_failover.py`, `ClaudeCodeAdapter`, `test_purge_backlog.py`, `test_telegram_mutating_commands.py`, `test_agency.py`, `traffic_director.py`, `test_skill_registry_boot_refresh.py`, `llm/router.py`, `analyze_page`, `test_crispy_burn_in.py`, `test_kimi_bridge_server.py`, `isolated_telegram_config`, `WorkflowBuildRequest`, `test_scheduler_hydration_bounded.py`, `test_sam_livekit.py`, `test_telegram_auto_approve.py`, `_resolve_user_github_token`, `test_service_token.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_regression.py`, `test_agency_fix.py`, `test_output_filter.py`, `test_monitor_lib.py`, `test_v4_api.py`, `SchedulerStore`, `parse_event_stream`, `client`, `test_dashboard_cache.py`, `TestNormalizeResponseFormat`, `test_langfuse_agency_wide.py`, `test_voice_pipeline.py`, `test_task_service_failed_comment.py`, `model_discovery.py`, `claim`, `facade.py`, `FeatureMaturity`, `test_ai_insights.py`, `sam_with_mocks`, `test_brain_patch_service_token.py`, `_resolve_push_token`, `test_company_api.py`, `test_refresh_agent_built_proof.py`, `SQLiteStore`, `issue_new_api_key`, `cost_tracker.py`, `_get`?**
  _High betweenness centrality (0.100) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `ExecutionRequest`, `AgentSessionStore`, `backend/server.py`, `test_empirical_verify.py`, `LocalWorkspace`, `proxy.py`, `MCPUnavailableError`, `test_agent_free_brain.py`, `direct_chat.py`, `TaskSpec`, `Workspace`, `MultiAgentSwarm`, `test_agent_tool_governance.py`, `CEODispatcher`, `TestMCPServer`, `_StubManager`, `E2BAdapter`, `MCPClient`, `test_brain_failover_413.py`, `AutonomyTracker`, `test_failover_client_shared.py`, `test_governance_api.py`, `WorkspaceTools`, `TokenBudget`, `ContextPruner`, `GitHubTools`, `AdaptiveHalter`, `.execute`, `ContextManager`, `StuckDetector`, `TestZeroAttemptDiagnostics`, `ReactScratchpad`, `failover_client.py`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `MCPUnavailableError` connect `MCPUnavailableError` to `TestMCPServer`, `AdaptiveHalter`, `MCPClient`, `test_mcp_registry.py`, `RenderOpsMonitor`, `AgentRunner`, `proxy.py`, `RenderFinding`, `Settings`, `direct_chat.py`, `RenderMCPClient`, `test_governance_sandbox.py`, `resolve_e2b_config`, `Workspace`, `TestRenderRouter`?**
