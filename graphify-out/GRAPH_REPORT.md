...
## Summary
- 30409 nodes · 56320 edges · 1284 communities (1113 shown, 116 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2734 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `62f9b76c`
...
## Communities (1284 total, 116 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.02
Nodes (199): _agent_provider_failure_response(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, _append_agent_session_message(), auth_me(), auto_recommend_skills() (+191 more)
...
Cohesion: 0.03
Nodes (168): Describes a single worker's task and constraints., WorkerSpec, get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, middleware, admin_control(), admin_delete_user(), admin_login() (+160 more)
...
### Community 7 - "test_agent_runner.py"
Cohesion: 0.04
...
Cohesion: 0.03
Nodes (95): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., _is_duplicate_key_error(), Any, Exception, Task, TaskStatus (+87 more)

### Community 13 - "test_unit8_model_catalog.py"
Cohesion: 0.04
Nodes (77): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, Force a catalog mirror rebuild + persist (admin only). Rebuilds the catalog…, sync_catalog_route(), all_provider_ids(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the…, True when the env var for *provider*'s key is set (or it's Ollama). (+69 more)
...
Cohesion: 0.05
Nodes (44): AgentRunner, _check_extra_kwargs(), _enforce_signature(), Any, Path, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Append an event to the durable session log if a store is wired in. (+36 more)
...
Cohesion: 0.01
Nodes (111): parse_event_stream(), Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, Reset the singleton (for tests)., reset_failover_manager(), base_url(), mobile_page(), proxy_url(), Config (+103 more)
...
Cohesion: 0.04
Nodes (42): ExecutionRequest, PlanOutput, Exception, Inject additional instructions into an in-flight run (no state change). Backs…, Determine domain and task type from the request text., Generate a structured execution plan. Uses the LLM if available; falls back to…, Select the best specialist(s) for the classified domain., Run doctor checks before execution. (+34 more)

### Community 23 - "E2BSandboxSession"
Cohesion: 0.03
Nodes (100): MCPUnavailableError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Raised when the MCP server is unreachable or the circuit is open., TaskResult, TaskSpec, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, E2BSandboxSession (+92 more)
...
Cohesion: 0.03
Nodes (95): BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), invalidate_brain_config_cache(), provider_base_url(), BaseModel (+87 more)
...
Cohesion: 0.03
Nodes (91): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, AgentIdentity, _coerce_text(), new_session_id(), Any, packages/governance/identity.py — who is acting, and under what mandate. Docker… (+83 more)

### Community 29 - "loop.py"
Cohesion: 0.04
Nodes (58): loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Condense a tool result down to what belongs in an audit row. A tool result can…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, Feed LLM token/cost spend into the governance session budget. Kept separate…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _summarise_tool_result(), _load() (+50 more)
...
Cohesion: 0.04
Nodes (94): BrainResolution, provider_api_key(), Return the live API key for *provider* (env-only — never persisted)., get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache() (+86 more)

### Community 35 - "Task"
Cohesion: 0.04
Nodes (70): Full task/issue document., Task, One-line human explanation of why this task needs human approval. Reads risk…, Best-effort label of who will run this task once approved. Returns…, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, asyncio, A reject + human Retry starts a NEW approval cycle → fresh notification. (+62 more)

### Community 36 - "Specialist"
Cohesion: 0.03
Nodes (50): SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, Get a specialist by ID., Create a new specialist. (+42 more)

### Community 37 - "TaskWorkflowService"
Cohesion: 0.04
Nodes (71): Task, Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, ApprovalCheckpoint, Human approval gate in a task's execution., _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing() (+63 more)
...
Cohesion: 0.04
Nodes (80): BudgetTracker, classify(), GovernanceGate, _host_of(), Extract a hostname from a URL, or return a bare host unchanged., Per-session consumption counters and their ceilings. Ceilings come from the…, Return the name of the first exhausted session-wide limit, or None. Only covers…, Return a reason string if *tool* has hit its per-tool session cap. Checks only… (+72 more)
...
Cohesion: 0.07
Nodes (64): A subtask's full history: what it is, and every attempt at it., SubtaskRecord, str, Implementer tiers, ordered cheapest-and-narrowest to most-capable. A subtask…, Tier, Bounds for the unattended supervision loop., SupervisorConfig, _goal() (+56 more)
...
Cohesion: 0.03
Nodes (82): Resolve the model id for a component's role on a provider. Parameters…, resolve_component_model(), _build_builtin_model_map(), _default_model(), _default_reasoning_model(), _get_model_map(), _nvidia_key_present(), Dynamic model router. Central routing logic for all chat and agent requests.… (+74 more)
...
Cohesion: 0.04
Nodes (83): _active_primary_provider(), _build_base_url_env_from_yaml(), _build_candidates_from_yaml(), _build_default_base_url_from_yaml(), _build_display_names_from_yaml(), _build_key_env_from_yaml(), _build_presets_from_yaml(), _build_tier_from_yaml() (+75 more)

### Community 66 - "ModelRouter"
Cohesion: 0.08
Nodes (27): _enabled(), get_available_models(), invalidate_cache(), is_model_available(), Ollama model availability check with TTL cache. Keeps a short-lived cache of…, Force the next call to re-probe Ollama (useful in tests)., Return True if *model* is in the Ollama tag list (or health checks off).…, Return the set of model names currently present in Ollama. Returns an empty set… (+19 more)
...
Cohesion: 0.04
Nodes (40): _keepalive_self_ping(), lifespan(), _maybe_boot_purge(), purge_backlog(), _purge_backlog_core(), _purge_summary_clean(), PurgeBacklogRequest, Run the FreeBuff Telegram bot, restarting it on unexpected exit. (+32 more)
...
Cohesion: 0.08
Nodes (42): BoundContext, ClassifyOutput, ExecutionResult, JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput, Phase (+34 more)
...
Cohesion: 0.05
Nodes (36): _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestPhaseSequenceError (+28 more)
...
Cohesion: 0.05
Nodes (74): Convenience: resolve all four role models for a component. Returns a dict with…, resolve_component_role_models(), get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers() (+66 more)
...
Cohesion: 0.05
Nodes (43): ApiKeyCreate, AuthorizeReposBody, BrainTestRequest, create_mcp_server(), GitHubFileWrite, GitHubPRCreate, GitHubTokenBody, LoginBody (+35 more)

### Community 97 - "direct_chat.py"
Cohesion: 0.07
Nodes (57): Any, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState (+49 more)
...
Cohesion: 0.04
Nodes (86): get_improvement_loop(), _dispatch_async(), get_log_monitor(), _note_recurrence(), agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log…, Best-effort: hand an operational failure to the incident tracker. Import is…, Best-effort, synchronous: tell the healer this signature was seen again., Fire-and-forget: schedule a self-healing task from any thread. (+78 more)
...
Cohesion: 0.08
Nodes (19): _get_director(), _normalized_provider_type(), ProviderAttempt, ProviderRouter, Any, Try all models for one provider. Returns ProviderResult on success, None on…, Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either…, Return the process TrafficDirector, or None if it is unavailable. (+11 more)
...
Cohesion: 0.06
Nodes (61): get_failover_manager(), Return the singleton BrainFailoverManager., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog() (+53 more)
...
Cohesion: 0.04
Nodes (83): agents/store.py — Persistent store for user-defined agent configurations.…, Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, _capability_tags(), create_task_from_oldest_open_issue(), intake_issue(), _issue_labels() (+75 more)
...
Cohesion: 0.03
Nodes (151): AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), delete_company_endpoint(), _DoctorCheck, _DoctorReport, generate_onboarding_questions(), get_company() (+143 more)
...
Cohesion: 0.04
Nodes (59): _escape_md_v1(), NotificationDispatcher, Any, Path, service_manager.py — Telegram & Notification Integration Extension Extends the…, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess). (+51 more)

### Community 176 - "TaskDispatcher"
Cohesion: 0.08
...
### Community 184 - "OrchestratorSupervisor"
Cohesion: 0.09
...
### Community 212 - "test_claude_code_adapter.py"
Cohesion: 0.14
...
Cohesion: 0.08
Nodes (20): _is_bedrock_model_id(), Return True if model_id is an AWS Bedrock model or inference profile ID., _bedrock_api_response(), _bedrock_provider(), _mock_boto3(), Any, asyncio, ProviderConfig (+12 more)
...
Cohesion: 0.04
Nodes (42): Delete a company and all associated data from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON blob (which…, Create a new website in SQLite. The full model is stored in ``data`` so scan…, Get a website by ID from SQLite., Update a website in SQLite. When ``company_id`` is omitted the existing company…, Delete a website from SQLite., List websites from SQLite. (+34 more)
...
### Community 230 - "AgentDefinition"
Cohesion: 0.04
Nodes (55): AgentDefinition, AgentStore, Any, field_validator, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents. (+47 more)
...
### Community 238 - "test_colibri_provider.py"
Cohesion: 0.12
...
Cohesion: 0.08
Nodes (43): build_ceo_router(), Any, APIRouter, backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), _env_flag(), _env_int() (+35 more)
...
### Community 266 - "sam_livekit_worker.py"
Cohesion: 0.13
...
### Community 294 - "test_self_heal.py"
Cohesion: 0.11
...
Cohesion: 0.07
Nodes (13): _normalize_response_format(), Translate OpenAI ``response_format`` into Ollama's ``format`` field. For…, list_models_openai(), List available models — union of live Ollama models, router registry, and…, Daily automation tests — 2026-05-14 Covers three features implemented in this…, Payload without 'model' field should apply normalization (no '/' → local)., _normalize_response_format must not mutate the input dict., list_models_openai returns object:list with data array. (+5 more)
...
### Community 356 - "models/seo_audit.py"
Cohesion: 0.09
...
### Community 397 - "TestWorkflowIntegration"
Cohesion: 0.33
...
### Community 427 - "test_fixes_reliability.py"
Cohesion: 0.11
...
### Community 439 - "test_webui_provider_priority.py"
Cohesion: 0.21
...
### Community 447 - "test_schedule_persistence.py"
Cohesion: 0.15
...
### Community 448 - "platform_controls_router.py"
Cohesion: 0.10
...
### Community 463 - "test_agent_chat_integration.py"
Cohesion: 0.23
...
### Community 468 - "state.py"
Cohesion: 0.11
Nodes (27): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+19 more)
...
### Community 501 - "TestPoliciesGovernanceStableClaim"
Cohesion: 0.20
...
### Community 519 - "traffic_director.py"
Cohesion: 0.13
...
### Community 525 - "SeoCheckDefinition"
Cohesion: 0.09
...
### Community 545 - "llm/router.py"
Cohesion: 0.03
Nodes (120): get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., get_cache(), The process-wide cache manager., failover_chat_completion_via_router(), openai_body_from_response(), Any (+112 more)
...
### Community 560 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
Cohesion: 0.05
Nodes (56): Record token usage for *model* (fire-and-forget, never raises). ``tag`` is a…, record_usage(), _dead_model_key(), _exponential_backoff_cooldown(), get_dead_models(), _is_model_dead(), _mark_model_dead(), _notify_watchdog() (+48 more)
...
### Community 573 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 574 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 592 - "HealthStatus"
Cohesion: 0.07
Nodes (15): GroqProvider, Provider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NvidiaProvider, Provider, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., OllamaProvider (+7 more)
...
### Community 594 - "test_runtimes.py"
Cohesion: 0.03
Nodes (47): AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., GooseAdapter, Any, Adapter for Goose — TIER 2 general-purpose local runtime., HermesAdapter, Any (+39 more)
...
### Community 635 - "PortfolioIntelligence"
Cohesion: 0.17
...
### Community 638 - "DockerAgentAdapter"
Cohesion: 0.17
...
### Community 646 - "test_provider_state_durability.py"
Cohesion: 0.17
...
### Community 670 - "has_image_content"
Cohesion: 0.31
...
### Community 673 - "test_event_log.py"
Cohesion: 0.45
...
### Community 702 - "_harvest_changed_files"
Cohesion: 0.29
...
Cohesion: 0.03
Nodes (121): _get_current_user_thunk(), _get_optional_user_thunk(), Request, admin_seed(), clear_cost_attribution(), clear_response_cache(), create_access_token(), create_api_key() (+113 more)
...
### Community 727 - "_start_ceo_agency"
Cohesion: 0.31
...
### Community 728 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 746 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.22
...
### Community 762 - "TestGithubSignalHardening"
Cohesion: 0.22
...
### Community 763 - "tests/conftest.py"
Cohesion: 0.08
...
### Community 772 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 773 - "services/seo_audit.py"
Cohesion: 0.11
...
### Community 807 - "get_workflow_orchestrator"
Cohesion: 0.06
Nodes (33): Execute work through the 11-phase golden path., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_execute(), get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper)., reset_orchestrator() (+25 more)

### Community 808 - "CapacityAllocation"
Cohesion: 0.25
...
Cohesion: 0.04
Nodes (52): legacy_scheduler_create(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_toggle(), LegacyScheduleJobRequest, LegacyScheduleToggleRequest, get_store(), Return the active store singleton, creating it on first call. (+44 more)
...
### Community 842 - "Workspace Issues"
Cohesion: 0.29
...
### Community 845 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 849 - "BackgroundServices"
Cohesion: 0.38
...
### Community 857 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 889 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 891 - "Agent API Issues"
Cohesion: 0.40
...
### Community 970 - "WorkflowRun"
Cohesion: 0.07
Nodes (29): _NoopStore, Any, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Push a Telegram approval-gate notification when a run pauses (Charter G1).…, Persist first-merge consent once an operator approves a ``telegram_gate``. No-…, List recent runs. When ``owner_id`` is provided, only runs stamped with that…, No-op checkpoint store when the real one is unavailable., In-flight state for a single golden-path execution. (+21 more)
...
### Community 978 - "._resolve_merge_decision"
Cohesion: 0.40
...
### Community 983 - "seo_api.py"
Cohesion: 0.07
Nodes (50): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), list_seo_audits(), plan_seo_sprint(), BaseModel, get (+42 more)

### Community 984 - "agency.py"
Cohesion: 0.06
...
### Community 1017 - "test_feature_maturity.py"
Cohesion: 0.14
...
### Community 1027 - "Authentication Issues"
Cohesion: 0.50
...
### Community 1032 - "ProviderManager"
Cohesion: 0.17
...
### Community 1037 - "Feature Maturity Issues"
Cohesion: 0.50
...
### Community 1061 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1077 - "Runtime & Onboarding Issues"
Cohesion: 0.50
...
### Community 1134 - "test_daily_automation_2026_07_09.py"
Cohesion: 0.16
...
- **3505 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3500 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13387 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **116 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `TestTheWorkflowIsSafeAndReadOnly`, `test_brain_patch_service_token.py`, `traffic_director.py`, `test_llm_router_queue_cache.py`, `test_llm_router_strategies.py`, `test_doctor_service_token_check.py`, `test_operational_incidents.py`, `TaskStore`, `test_model_router.py`, `SeoCheckDefinition`, `config.py`, `test_unit8_model_catalog.py`, `SelfHealingAgent`, `ExecutionRequest`, `E2BSandboxSession`, `BrainConfig`, `test_llm_router_resilience.py`, `PolicyEngine`, `loop.py`, `WebsiteScanner`, `resolve_active_brain`, `test_company_api.py`, `test_langfuse_agency_wide.py`, `TaskWorkflowService`, `test_failover_client_shared.py`, `Task`, `SeoAuditEngine`, `UserRole`, `test_agent_tool_governance.py`, `validate_policy_text`, `ai/router.py`, `test_catalogue_probe.py`, `BrainWatchdog`, `test_knowledge_sync.py`, `AgentJobManager`, `PrimeAgentAdapter`, `ChatHistoryStore`, `ImprovementLoop`, `test_openclaw_endpoints.py`, `AgentSwarm`, `ResearchTask`, `test_provider_router.py`, `TestClient`, `test_procedural_memory.py`, `SQLiteStore`, `workflow/models.py`, `test_startup_warmup.py`, `test_llm_router_tpm.py`, `test_telegram_auto_approve.py`, `telegram_bot.py`, `_resolve_push_token`, `test_autonomy_status.py`, `test_gh_brain_failover.py`, `test_local_brain_state.py`, `resolve_e2b_config`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `TestHarnessAdapter`, `services/background.py`, `test_e2b_adapter.py`, `test_daily_automation_2026_07_09.py`, `cost_tracker.py`, `test_sam_voice.py`, `test_context_rulebook.py`, `ArtifactStore`, `test_trend_scoping.py`, `KeyStore`, `run_task`, `e2e/test_browser.py`, `test_provider_state_durability.py`, `test_kimi_bridge_server.py`, `_step`, `brain_failover.py`, `BackgroundAgent`, `test_trend_watcher.py`, `clear_cooldowns`, `claim`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `test_response_cache.py`, `MCPClient`, `test_daily_2026_07_27.py`, `test_openclaw_gateway.py`, `KeyPool`, `test_telegram_webhook.py`, `test_telegram_freebuff.py`, `NotificationDispatcher`, `OllamaCircuitBreaker`, `_job_text`, `test_new_features_e2e.py`, `test_features_api.py`, `test_video_transcript.py`, `TrendWatcher`, `test_agent_free_brain.py`, `_Budget`, `TestMongoGate`, `test_process_quick_note_workflow.py`, `test_portfolio_intake.py`, `TestAgentRunnerSafety`, `test_claude_code_adapter.py`, `_start_ceo_agency`, `test_skill_registry_boot_refresh.py`, `test_brain_availability_doctor.py`, `TestSwarmRoleRouting`, `TestClient`, `AgentDefinition`, `test_e2b_task_wiring.py`, `ContextPruner`, `test_ceo_router.py`, `tests/conftest.py`, `test_agile_api.py`, `test_app_settings.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `UsageEvent`, `test_task_clarification.py`, `OutputFilter`, `LogMonitor`, `test_platform_controls.py`, `test_rate_limiter.py`, `OrchestratorQueue`, `TestNormalizeResponseFormat`, `session_retro.py`, `test_purge_backlog.py`, `analyze_page`, `get_scheduler`, `_resolve_user_github_token`, `test_mcp_governance.py`, `test_telegram_task_callback.py`, `get_registry`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_implement_agent_routing.py`, `_FakeSandbox`, `test_nvidia_model_discovery.py`, `isolated_telegram_config`, `models/seo_audit.py`, `test_scheduler_hydration_bounded.py`, `test_telegram_approval_e2e.py`, `test_provider_enable_disable.py`, `_undeclared`, `api_keys_for`, `test_sam_livekit.py`, `test_service_token.py`, `test_harness_spec.py`, `model_discovery.py`, `test_regression.py`, `test_agency_fix.py`, `test_dashboard_cache.py`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `test_webui_provider_priority.py`, `platform_controls_router.py`, `test_telegram_diag_endpoint.py`, `_Recorder`, `test_ping.py`, `ScheduledJob`, `test_runtimes_health_endpoint.py`, `test_task_service_failed_comment.py`, `test_voice_pipeline.py`, `test_feature_maturity.py`, `client`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `proxy.py`, `test_agent_runner.py`, `ReactScratchpad`, `TaskSpec`, `BackgroundAgent`, `FreeBuffAgent`, `_fixture`, `ContextManager`, `ExecutionRequest`, `E2BSandboxSession`, `PolicyEngine`, `loop.py`, `MCPClient`, `CEODispatcher`, `test_failover_client_shared.py`, `test_fixes_reliability.py`, `StuckDetector`, `test_agent_tool_governance.py`, `AgentSessionStore`, `InternalAgentAdapter`, `test_empirical_verify.py`, `LocalWorkspace`, `MultiAgentSwarm`, `AgentJobManager`, `test_daily_automation_2026_07_11.py`, `WorkspaceTools`, `ImprovementLoop`, `test_agent_free_brain.py`, `workflow_orchestrator.py`, `test_agent_chat_integration.py`, `TestAgentRunnerSafety`, `state.py`, `TokenBudget`, `UserMemoryStore`, `_resolve_push_token`, `TestAgentRunnerExecution`, `test_governance_api.py`, `direct_chat.py`, `test_e2b_adapter.py`, `test_daily_automation_2026_07_09.py`, `AdaptiveHalter`, `ContextPruner`, `GitHubTools`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `backend/server.py`, `test_all_providers_discovery.py`, `ProviderConfig`, `_payload`, `implement_agent.py`, `clear_cooldowns`, `resolve_active_brain`, `failover_client.py`, `test_bedrock_live.py`, `TestOpenAiToBedrockConverse`, `TestFromEnvBedrock`, `TestRouterIntegration`, `ai/router.py`, `TestLegacyRouterServerFallback`, `TestLegacyRouterCacheTTL`, `is_anthropic_base_url`, `test_anthropic_refusal_fallback.py`, `test_provider_router.py`, `HealthStatus`, `test_bedrock_provider.py`, `test_implement_agent_routing.py`, `TestAnthropicPayloadStructuredOutput`, `direct_chat.py`, `test_colibri_provider.py`, `TrafficDirector`?**
