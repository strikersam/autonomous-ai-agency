...
## Corpus Check
- 1501 files · ~2,174,667 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30421 nodes · 56833 edges · 1272 communities (1115 shown, 109 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3331 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `abbc2867`
...
## Communities (1272 total, 109 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.02
Nodes (215): _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills() (+207 more)
...
Cohesion: 0.02
Nodes (122): BusinessSystem, CompanyGraph, KnowledgeItem, SpecialistFamily, SystemType, Get the index of a phase in the workflow., The complete Company Graph - canonical core model for the Autonomous AI Agency.…, Find all systems of a specific type. (+114 more)
...
### Community 6 - "Task"
Cohesion: 0.03
Nodes (92): AgentDefinition, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, A user-defined agent configuration stored in the database., Accept both ISO-8601 datetime strings (from DB) and float timestamps., Keep legacy backend fields and Control Plane UI fields aligned., Task (+84 more)
...
Cohesion: 0.03
Nodes (97): _acquire_provider_probe(), CommercialFallbackRequiredError, extract_openai_text(), _get_director(), is_commercial_provider(), _normalize_nvidia_base_url(), _normalized_provider_type(), _openai_url() (+89 more)

### Community 9 - "LLMRequest"
Cohesion: 0.03
Nodes (111): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, OpenAICompatible, Any, AsyncClient, Cheap liveness probe. Never raises — returns a status dict., Discover model ids. Providers that cannot enumerate return []., Build auth headers according to the configured auth style. (+103 more)
...
Cohesion: 0.03
Nodes (114): autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config() (+106 more)

### Community 12 - "test_llm_router_queue_cache.py"
Cohesion: 0.03
Nodes (90): Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any (+82 more)

### Community 13 - "RuntimeAdapter"
Cohesion: 0.03
Nodes (58): Abstract base class every runtime adapter must implement. Subclasses must set…, Execute a task and return the result. Must handle its own timeout…, Stream output tokens/lines. Default implementation runs execute() and yields…, Called once when the runtime is registered. Override to start a subprocess,…, Called when the runtime is being unregistered or the server is shutting down.…, Clean up any runtime-owned workspace resources after execution., Return True if this runtime supports the given capability., RuntimeAdapter (+50 more)

### Community 14 - "HTTPException"
Cohesion: 0.03
Nodes (123): admin_seed(), call_llm(), clear_cost_attribution(), clear_response_cache(), create_access_token(), create_api_key(), create_github_pr(), create_provider() (+115 more)

### Community 15 - "TaskStatus"
Cohesion: 0.04
Nodes (90): Set the global agent store instance (e.g., with MongoDB on startup)., set_agent_store(), autonomy_tick(), Point the feature stores at the shared database connection. Deliberately…, Execute ONE pending task synchronously. Called by the cron workflow every 2…, _wire_feature_stores(), Helpers that turn scheduler and playbook activity into real tasks., Creates tasks from scheduler and playbook runs using one workflow. (+82 more)

### Community 16 - "llm/router.py"
Cohesion: 0.03
Nodes (94): _load(), main(), Scheduled portfolio intelligence sweep. Run by `.github/workflows/portfolio-…, int, FailoverResult, A successful completion plus the accounting its callers need., get_cache(), The process-wide cache manager. (+86 more)

### Community 17 - "AnthropicProvider"
Cohesion: 0.04
Nodes (59): AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Return the cache_control block appropriate for this provider's TTL setting.… (+51 more)

### Community 18 - "_fixture"
Cohesion: 0.02
Nodes (94): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., setup_database_moks() (+86 more)
...
Cohesion: 0.04
Nodes (89): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, governance_enabled(), True when the governance layer should run at all. A single global off switch…, _action_matches(), _as_list(), Decision (+81 more)

### Community 22 - "AgentRunner"
Cohesion: 0.04
Nodes (65): AgentRunner, Any, Path, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a…, Send chat messages and return the assistant's text output. If an…, Call-time resolver for an agent role model id. Delegates to… (+57 more)
...
Cohesion: 0.03
Nodes (49): AgentJob, AgentJobManager, _now(), Any, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,…, Serialize the AgentJob to a JSON-serializable dictionary for external clients.…, Any (+41 more)
...
Cohesion: 0.05
Nodes (94): BrainFailoverExhausted, failover_chat_completion(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, Run one chat completion across the brain-failover chain. Tries each healthy…, mark_dead(), Hold *model_id* out of *provider_id*'s rotation for *ttl_sec* seconds. Called…, Drop every cached list, attempt record, and dead-model mark. For tests, and for… (+86 more)
...
Cohesion: 0.03
Nodes (79): _auto_disable(), _build_request(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool() (+71 more)

### Community 32 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (45): Find a website by its URL., Find a repository by its URL., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, Website (+37 more)
...
### Community 35 - "test_render_mcp.py"
Cohesion: 0.04
...
Cohesion: 0.04
Nodes (50): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), _load() (+42 more)
...
Cohesion: 0.04
Nodes (43): field_validator, A single occurrence of a check firing on a specific URL., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoIssueReportRow (+35 more)
...
Cohesion: 0.05
Nodes (74): PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob (+66 more)
...
### Community 48 - "test_mcp_registry.py"
Cohesion: 0.05
...
### Community 49 - "ProviderRouter"
Cohesion: 0.04
Nodes (36): build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, ProviderRouter, Priority-ordered LLM provider fallback with health checks and retries., main(), probe_one(), Live probe — makes a tiny real API call to every configured provider. Usage (in…, _bedrock_api_response() (+28 more)

### Community 50 - "test_governance_enforcement.py"
Cohesion: 0.05
Nodes (69): get_approval_store(), BudgetTracker, GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., AgentIdentity, _coerce_text(), new_session_id() (+61 more)

### Community 51 - "AgentSessionStore"
Cohesion: 0.05
Nodes (47): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+39 more)
...
### Community 53 - "_start_autonomy_loops"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (72): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), list_seo_audits(), plan_seo_sprint(), BaseModel, get (+64 more)
...
Cohesion: 0.05
Nodes (74): _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody, The task store the background services should use, wiring it if needed.…, Shared purge implementation for the admin endpoint and the boot hook., Submit a quick-note URL or instruction from the dashboard FAB., _task_store_for_background(), get_failover_manager() (+66 more)
...
Cohesion: 0.04
Nodes (68): AgentPhaseError, Exception, Raised when a named agent phase (planning, verification, etc.) fails., AgentPlan, _make_runner(), MonkeyPatch, Path, Regression for #1422 item 3: when the model never returns a JSON object, the… (+60 more)
...
Cohesion: 0.05
Nodes (54): main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), _is_safe_url(), _looks_like_bot_challenge(), BeautifulSoup, True if the HTML is an anti-bot/CAPTCHA interstitial rather than content. Used… (+46 more)
...
### Community 82 - "ModelConfig"
Cohesion: 0.05
Nodes (31): AgentPolicy, get_config(), LLMConfig, ModelConfig, Capability and pricing metadata for one model id., Per-agent routing overrides (ADR-008 §"named profiles")., The fully-resolved routing configuration., Return the process-wide configuration, loading it on first use. (+23 more)

### Community 83 - "test_llm_router_tpm.py"
Cohesion: 0.05
Nodes (42): _Counter, _escape(), _Gauge, _Histogram, _labels(), MetricsRegistry, Any, packages/llm/metrics.py — Prometheus metrics without the client library. The… (+34 more)

### Community 84 - "tasks/models.py"
Cohesion: 0.05
Nodes (50): tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest, ExecutionApprovalRequest, ExecutionLogEntry (+42 more)
...
Cohesion: 0.06
Nodes (53): get_tracker(), The process-wide health tracker., get_ring(), The process-wide key ring., get_metrics(), The process-wide metrics registry., _ok(), parametrize (+45 more)
...
### Community 100 - "ai/router.py"
Cohesion: 0.05
Nodes (41): _dead_model_key(), _exponential_backoff_cooldown(), _mark_model_dead(), _notify_watchdog(), provider_router.py — auto-generated module docstring (user-research skill scan)., Notify the brain watchdog of a provider call outcome (fire-and-forget).…, # NOTE: 429 and 410 are handled specially in _try_one_provider (immediate, Compute exponential backoff cooldown for repeated 429 failures. Each… (+33 more)
...
### Community 105 - "ExecutionRequest"
Cohesion: 0.07
...
### Community 106 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.06
...
Cohesion: 0.07
Nodes (50): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), get_web_reach(), _load_script_module(), Any, ModuleType, Dynamically load a pure-stdlib helper module from .github/scripts/. Returns…, Zero-key internet access: pages, YouTube transcripts, search, RSS. Every public… (+42 more)
...
Cohesion: 0.06
Nodes (60): all_provider_ids(), _provider_ids_from_literal(), Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers() (+52 more)
...
Cohesion: 0.05
Nodes (43): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _await_hermes_ready(), _env_on(), Any, Path, services/background.py — shared background-service startup. Both the FastAPI…, Await the refresh and log the outcome; swallow every failure. (+35 more)
...
Cohesion: 0.07
Nodes (31): _looks_unknown_model(), _models_to_try(), True when the provider rejected the model id itself, not the request. Some…, Order the models to attempt on *provider*, correcting a stale catalogue. Cache-…, _mock_get(), _ok(), asyncio, parametrize (+23 more)
...
### Community 139 - "AgentStore"
Cohesion: 0.06
Nodes (40): AgentStore, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents., Return all agents in the system (admin use only)., main(), Register agent runtimes into the local agent store. Usage: python… (+32 more)
...
### Community 143 - "TestAdapterMetadata"
Cohesion: 0.05
...
Cohesion: 0.06
Nodes (40): free_nvidia_models(), FreeBuffAgent, Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is…, is_rate_limit_exempt() (+32 more)
...
Cohesion: 0.07
Nodes (26): Attempt, text_key(), Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), Execute ``factory``, or await an identical call already running. Returns…, LLMRouter, Any, AsyncClient (+18 more)
...
### Community 163 - "test_p0_roadmap_b1_c2_a3.py"
Cohesion: 0.06
...
Cohesion: 0.06
Nodes (39): events_from_identity(), Extract the identity-derived audit fields from an AgentIdentity. Kept here…, BudgetExceeded, classify(), guard_tool_call(), GuardResult, _host_of(), _is_write_tool() (+31 more)
...
Cohesion: 0.08
Nodes (33): get_brain_preference(), Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, Operator can set BRAIN_PREFERENCE=colibri without it being rejected., test_get_brain_preference_accepts_colibri(), _admin_out(), AdminCommandBody, _anthropic_chat_payload(), _anthropic_text() (+25 more)
...
Cohesion: 0.07
Nodes (23): CerebrasProvider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NvidiaProvider, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., Ollama provider adapter — local LLM inference., packages/ai/manager.py — ProviderManager. Single entry point for all LLM calls.… (+15 more)
...
### Community 199 - "workflow_orchestrator.py"
Cohesion: 0.09
...
### Community 202 - "provider_max_rpm"
Cohesion: 0.07
...
### Community 229 - "test_iteration_6_features.py"
Cohesion: 0.05
...
### Community 230 - "test_unit6_resolve_component_model.py"
Cohesion: 0.05
...
Cohesion: 0.08
Nodes (34): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), _nvidia_api_key(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic… (+26 more)

### Community 242 - "get_current_user"
Cohesion: 0.08
...
Cohesion: 0.11
Nodes (28): _get_github_token_for_user(), Fetch GitHub token for user from secrets store or environment., get_user_role(), Extract role from a user object (dict or Pydantic model)., create_secret(), delete_secret(), get_secret_metadata(), get_secrets_store() (+20 more)
...
### Community 264 - "skill_bindings.py"
Cohesion: 0.09
...
Cohesion: 0.07
Nodes (21): _append_agent_session_message(), _in_container(), _mask_observations(), Path, Truncate tool/observation content in older messages to prevent context bloat., Return True when running inside a container runtime. ``/.dockerenv`` alone…, Swap localhost → `ollama` when running inside a container. Inside a container,…, _resolve_ollama_url() (+13 more)
...
### Community 280 - "_ts_to_float"
Cohesion: 0.08
Nodes (27): Return tasks queued for agent execution, highest-priority first. Ordered by…, Return BLOCKED tasks that are candidates for auto-retry., Return tasks due within the next N hours (any user)., Return tasks parked at the pre-execution approval gate (any user). A task is…, Reset tasks that are stuck IN_PROGRESS but no longer actively executing. A task…, Normalise a timestamp value to a float Unix epoch. Handles: - float / int…, _ts_to_float(), asyncio (+19 more)
...
### Community 328 - "RuntimeHealthService"
Cohesion: 0.10
...
### Community 351 - "analyze_qualitative"
Cohesion: 0.12
...
### Community 373 - "telegram_inbound_handlers.py"
Cohesion: 0.13
...
### Community 376 - "TestMCPServer"
Cohesion: 0.13
Nodes (5): Any, tests/test_mcp_server.py — Unit tests for the MCP server and client. Tests: -…, Ensure _dispatch_tool routes MCP-only tools correctly., TestAgentLoopMCPIntegration, TestMCPServer
...
### Community 379 - "test_direct_chat_async.py"
Cohesion: 0.15
...
### Community 381 - "test_autonomous_agency_e2e.py"
Cohesion: 0.10
...
### Community 408 - "seo_report_pdf.py"
Cohesion: 0.24
...
Cohesion: 0.13
Nodes (10): get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, start_orchestrator_supervisor() (+2 more)
...
### Community 459 - "get_tool_registry"
Cohesion: 0.12
Nodes (15): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, Register the built-in agent tools that are always available., Register the interactive-browser capability (agent/browser.py). Unlike…, _register_browser_tools(), _register_builtin_tools(), auto_register(), Register the four user-research tools with a ``ToolRegistry``. Each tool wraps… (+7 more)

### Community 460 - "WorkflowEngine"
Cohesion: 0.13
...
### Community 463 - "_ensure_tasks_source_id_unique_index"
Cohesion: 0.14
...
### Community 479 - "render_router.py"
Cohesion: 0.13
...
### Community 487 - "FeatureUnavailableError"
Cohesion: 0.13
...
### Community 495 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 519 - "resolve_free_nvidia_brain"
Cohesion: 0.12
...
### Community 522 - "parse_event_stream"
Cohesion: 0.15
...
### Community 528 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
### Community 548 - "test_task_pre_execution_gate.py"
Cohesion: 0.24
...
### Community 564 - "kimi_bridge_provider_config"
Cohesion: 0.18
...
### Community 597 - "AdminDigestRouterAuthTests"
Cohesion: 0.20
...
### Community 612 - "test_tasks_reconciler_todo_requeue.py"
Cohesion: 0.20
...
### Community 629 - "RateLimit"
Cohesion: 0.14
Nodes (4): GroqProvider, RateLimit, Provider rate limit info., Return the provider's rate limits.
...
### Community 642 - "test_task_service_failed_comment.py"
Cohesion: 0.21
...
### Community 671 - "DockerAgentAdapter"
Cohesion: 0.19
Nodes (9): DockerAgentAdapter, Any, TaskResult, Adapter that runs agent tasks inside isolated Docker containers., Check whether Docker is available and report the adapter's runtime health.…, asyncio, test_docker_binary_missing(), asyncio (+1 more)
...
### Community 674 - "TestSeoApiSurface"
Cohesion: 0.16
...
### Community 677 - "test_brain_failover_413.py"
Cohesion: 0.15
...
### Community 708 - ".mark_rate_limited"
Cohesion: 0.18
...
### Community 712 - "verify_service_token"
Cohesion: 0.22
...
### Community 723 - "TestRuntimeControl"
Cohesion: 0.15
...
### Community 738 - "_is_ephemeral_user"
Cohesion: 0.27
...
### Community 799 - "test_key_pool.py"
Cohesion: 0.20
Nodes (9): provider_api_keys(), Every API key configured for *provider*, primary first. Reads ``base_env`` then…, get_pool(), Per-provider API key rotation — the one lever that adds capacity. Every other…, Return the process-singleton KeyPool., Clear pool state (tests only)., reset(), _clean_pool() (+1 more)

### Community 800 - "ProviderManager"
Cohesion: 0.18
...
### Community 808 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 825 - "Agent Readiness Report"
Cohesion: 0.20
...
### Community 875 - "cmd_loops"
Cohesion: 0.36
Nodes (8): cmd_loops(), Loop Engineering fleet readiness + the costliest loops, from /api/loops., tests/test_telegram_observe.py Tests for the read-only "observe from Telegram"…, _run(), test_cmd_autonomy_degrades_gracefully(), test_cmd_autonomy_formats_brain_and_readiness(), test_cmd_loops_formats_readiness_and_costliest(), test_cmd_loops_reports_registry_error()
...
### Community 881 - "TestGithubSignalHardening"
Cohesion: 0.22
...
### Community 896 - "_parse_tool_calls_from_response"
Cohesion: 0.39
...
### Community 906 - "Agent Autonomy Roadmap"
Cohesion: 0.25
...
### Community 1059 - "TestDirectChatAgentExecution"
Cohesion: 0.33
...
### Community 1060 - "TestWorkflowIntegration"
Cohesion: 0.33
...
### Community 1063 - "TestExhaustedPoolDoesNotReusePrimary"
Cohesion: 0.33
...
### Community 1102 - "._resolve_merge_decision"
Cohesion: 0.40
...
### Community 1115 - "verify_pr_exists"
Cohesion: 0.50
...
### Community 1129 - "_provider_env_value"
Cohesion: 0.50
...
### Community 1130 - "_clean_director"
Cohesion: 0.50
...
### Community 1146 - "test_skills_route_order.py"
Cohesion: 0.67
...
- **3521 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3516 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13437 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **109 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `clear_override`, `cost_tracker.py`, `TestNormalizeResponseFormat`, `company_api.py`, `TaskSpec`, `Task`, `test_llm_router_strategies.py`, `ProviderConfig`, `test_operational_incidents.py`, `parse_event_stream`, `test_llm_router_queue_cache.py`, `test_gh_brain_failover.py`, `llm/router.py`, `TaskStatus`, `E2BSandboxSession`, `PolicyEngine`, `SelfHealingAgent`, `AgentJobManager`, `test_failover_client_shared.py`, `Agency`, `test_llm_router_resilience.py`, `test_company_api.py`, `validate_policy_text`, `ImprovementLoop`, `test_task_pre_execution_gate.py`, `SeoAuditEngine`, `test_model_router.py`, `test_telegram_webhook.py`, `WorkflowRun`, `direct_chat.py`, `test_unit8_model_catalog.py`, `BrainWatchdog`, `test_knowledge_sync.py`, `_start_autonomy_loops`, `AgentSwarm`, `test_openclaw_endpoints.py`, `ChatHistoryStore`, `get_registry`, `TokenBudget`, `test_ai_insights.py`, `ResearchTask`, `test_llm_router_tpm.py`, `SQLiteStore`, `TestClient`, `tasks/models.py`, `config.py`, `test_llm_router_e2e.py`, `e2e/test_browser.py`, `_resolve_push_token`, `ArtifactStore`, `test_autonomy_status.py`, `PrimeAgentAdapter`, `CEOSupervisor`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_procedural_memory.py`, `test_refresh_agent_built_proof.py`, `SeoFixer`, `BackgroundAgent`, `ai/router.py`, `test_tasks_reconciler_todo_requeue.py`, `_clean_director`, `OutputFilter`, `test_sam_voice.py`, `test_colibri_brain_shim.py`, `test_startup_warmup.py`, `clear_cooldowns`, `KeyStore`, `test_task_service_failed_comment.py`, `NotificationDispatcher`, `test_context_rulebook.py`, `test_trend_scoping.py`, `run_task`, `UserRole`, `test_new_features_e2e.py`, `FeatureMaturity`, `test_issue_intake.py`, `_step`, `test_trend_watcher.py`, `test_brain_failover_413.py`, `TestHarnessAdapter`, `test_openclaw_gateway.py`, `test_agent_tool_governance.py`, `CEOLedger`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `test_daily_2026_07_27.py`, `OllamaCircuitBreaker`, `_job_text`, `_Budget`, `analyze_page`, `test_features_api.py`, `TestAgentRunnerSafety`, `TrendWatcher`, `test_video_transcript.py`, `test_portfolio_intake.py`, `TestRuntimeControl`, `SchedulerStore`, `WorkflowBuildRequest`, `test_process_quick_note_workflow.py`, `nvidia_models.py`, `test_iteration_6_features.py`, `TestClient`, `get_current_user`, `test_agile_api.py`, `test_app_settings.py`, `ContextPruner`, `_FakeFiles`, `test_control_plane_api.py`, `test_task_clarification.py`, `test_brain_failover.py`, `test_persistent_memory.py`, `test_telegram_mutating_commands.py`, `SecurityScanner`, `test_platform_controls.py`, `test_key_pool.py`, `test_llm_router_disabled.py`, `OrchestratorCheckpointStore`, `OrchestratorQueue`, `test_nvidia_model_discovery.py`, `tests/conftest.py`, `test_rate_limiter.py`, `session_retro.py`, `test_catalogue_probe.py`, `TestMongoGate`, `test_scheduler_hydration_bounded.py`, `test_provider_state_durability.py`, `test_telegram_service_webhook.py`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `_start_ceo_agency`, `TestSwarmRoleRouting`, `telegram_inbound_handlers.py`, `_undeclared`, `test_service_token.py`, `test_harness_spec.py`, `test_agent_free_brain.py`, `test_agency_fix.py`, `_Recorder`, `test_webui_provider_priority.py`, `_resolve_user_github_token`, `api_keys_for`, `test_monitor_lib.py`, `test_v4_api.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_telegram_diag_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `test_dashboard_cache.py`, `_FakeSandbox`, `test_kimi_bridge_server.py`, `client`, `test_langfuse_agency_wide.py`, `test_voice_pipeline.py`, `test_doctor_coding_brain.py`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `test_governance_api.py`, `proxy.py`, `TaskSpec`, `test_backend_server_features.py`, `E2BSandboxSession`, `test_agent_free_brain.py`, `PolicyEngine`, `AgentJobManager`, `test_failover_client_shared.py`, `ContextManager`, `FreeBuffAgent`, `ReactScratchpad`, `E2BAdapter`, `Agency`, `MultiAgentSwarm`, `TestAgentRunnerExecution`, `validate_policy_text`, `test_brain_failover_413.py`, `test_empirical_verify.py`, `InternalAgentAdapter`, `test_agent_tool_governance.py`, `WorkflowRun`, `direct_chat.py`, `CEODispatcher`, `StuckDetector`, `UserMemoryStore`, `AgentSessionStore`, `test_agent_runner.py`, `workflow_orchestrator.py`, `TokenBudget`, `TestAgentRunnerSafety`, `test_agent_chat_integration.py`, `LocalWorkspace`, `_resolve_push_token`, `WorkspaceTools`, `BackgroundAgent`, `GitHubTools`, `loop.py`, `TestMCPServer`, `AdaptiveHalter`, `ContextPruner`, `test_autonomous_agency_e2e.py`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `Task` connect `Task` to `backend/server.py`, `company_api.py`, `CompanyGraphService`, `test_task_service_failed_comment.py`, `test_sam_livekit.py`, `test_trend_scoping.py`, `TaskStatus`, `WorkflowPhase`, `_ts_to_float`, `test_issue_intake.py`, `tasks/api.py`, `test_task_pre_execution_gate.py`, `test_tasks_awaiting_approval_api.py`, `BrainWatchdog`, `test_task_source_id_race.py`, `seo_api.py`, `get_task_store`, `test_e2b_task_wiring.py`, `test_phase4_runtime_resilience.py`, `test_purge_backlog.py`, `test_portfolio_intake.py`, `tasks/models.py`, `v4_api.py`, `test_tasks_reconciler_todo_requeue.py`, `test_phase6_workflow.py`, `test_schedule_growth_invariants.py`, `services/background.py`?**
