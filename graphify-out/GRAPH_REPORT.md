...
## Corpus Check
- 1439 files · ~2,066,275 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28598 nodes · 53418 edges · 1204 communities (1089 shown, 115 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 3673 edges (avg confidence: 0.57)
...
## Communities (1204 total, 115 thin omitted)

### Community 0 - "ExecutionRequest"
Cohesion: 0.03
Nodes (85): get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, start_orchestrator_supervisor() (+77 more)

### Community 1 - "TestRuntimeControl"
Cohesion: 0.05
Nodes (22): Test iteration 6 features: - POST /api/tasks/ auto-assigns an available agent…, POST /api/tasks/ without agent_id should attempt auto-assignment if agents exist, Test runtime start/stop endpoints return informational payloads in remote…, Get authentication token for admin user, GET /runtimes/ should return list of runtimes, POST /runtimes/{id}/start should return non-blocking informational payload in…, POST /runtimes/stop-all should return non-blocking informational payload, PUT /runtimes/policy should work with valid auth (+14 more)
...
Cohesion: 0.08
Nodes (27): CommercialFallbackRequiredError, _get_director(), is_commercial_provider(), _normalized_provider_type(), ProviderAttempt, ProviderFallbackError, ProviderRouter, Any (+19 more)

### Community 5 - "company_api.py"
Cohesion: 0.02
Nodes (161): AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), delete_company_endpoint(), _DoctorCheck, _DoctorReport, generate_onboarding_questions(), get_company() (+153 more)
...
Cohesion: 0.03
Nodes (76): AgentDefinition, AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+68 more)
...
Cohesion: 0.04
Nodes (89): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., ClaudeCodeAdapter, json_safe() (+81 more)
...
Cohesion: 0.03
Nodes (53): Raised when a runtime fails readiness validation before execution starts., RuntimePreflightError, runtimes/health.py — RuntimeHealthService. Periodically polls all registered…, Any, RoutingDecision, TaskResult, TaskSpec, runtimes/manager.py — RuntimeManager. (+45 more)
...
Cohesion: 0.03
Nodes (57): API, changeUserRole(), createApiKey(), createQuickNote(), deleteApiKey(), listQuickNotes(), setUserOnboarding(), V5App (+49 more)
...
### Community 25 - "Task"
Cohesion: 0.04
Nodes (66): Full task/issue document., Task, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, _escape_md_v1(), Escape Telegram Markdown-v1 reserved chars in free-text fields. Markdown-v1…, asyncio, A reject + human Retry starts a NEW approval cycle → fresh notification., A gated task parked while IN_PROGRESS must not be treated as stranded. (+58 more)
...
Cohesion: 0.03
Nodes (67): KnowledgeItem, Find knowledge items matching any of the given tags., Structured knowledge about the company, systems, or processes., Add a knowledge item to a company. Args: company_id: Company ID title:…, Get a knowledge item by ID., Search knowledge items., Get knowledge items by tags for a company. Args: company_id: Company ID tags:…, MongoDBStore (+59 more)

### Community 28 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (110): BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput (+102 more)
...
### Community 32 - "test_e2b_sandbox.py"
Cohesion: 0.06
Nodes (48): _inject_token(), Return ``(authed_url, clean_url)`` for a GitHub repo URL., fake_sandbox(), _FakeAsyncSandboxClass, _FakeCommandResult, _FakeCommands, _FakeFiles, _FakeRunner (+40 more)

### Community 33 - "rbac.py"
Cohesion: 0.05
...
Cohesion: 0.06
Nodes (89): BrainFailoverExhausted, failover_chat_completion(), FailoverResult, RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, A successful completion plus the accounting its callers need., Run one chat completion across the brain-failover chain. Tries each healthy…, _free_tier() (+81 more)

### Community 35 - "test_unit8_model_catalog.py"
Cohesion: 0.03
Nodes (101): _brain_provider_status(), get_brain_policy_route(), get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, Force a catalog mirror rebuild + persist (admin only). Rebuilds the catalog…, Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., Return the active brain config + per-provider key-present flags. The response… (+93 more)
...
Cohesion: 0.05
Nodes (59): BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), _norm(), _pick_from_records(), Any (+51 more)
...
Cohesion: 0.04
Nodes (80): Event, publish(), packages/events/bus.py — In-process event bus. Loosely couples components via…, An event published on the bus., Subscribe to an event type., Publish an event to all subscribers., subscribe(), ApprovalRequest (+72 more)
...
Cohesion: 0.07
Nodes (20): _keepalive_self_ping(), lifespan(), _maybe_boot_purge(), _purge_summary_clean(), Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free…, Start the Telegram bot (and keep-alive) inside the web process when enabled.…, True when a purge summary contains no error markers. _purge_backlog_core… (+12 more)
...
Cohesion: 0.07
Nodes (35): addTaskComment(), createAgent(), createTask(), deleteAgent(), escalateTask(), getTask(), listAgents(), retryTask() (+27 more)

### Community 72 - "test_mcp_registry.py"
Cohesion: 0.06
Nodes (43): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable(), _playwright_configured(), _probe_http() (+35 more)
...
### Community 85 - "Company"
Cohesion: 0.04
Nodes (61): Company, DeliveryPolicy, The core company entity - root of the Company Graph., How code lands on a repo's default branch (detected, GitHub-only for now). The…, A company's connection to a code repository (GitHub-only this pass). URL-only…, RepoConnection, attach_repo_connection(), build_repo_connection() (+53 more)
...
### Community 91 - "AuditLog"
Cohesion: 0.05
Nodes (30): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), _scheduler_on_fire(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log. (+22 more)
...
Cohesion: 0.08
Nodes (24): _best_cloud_primary_base(), InternalAgentAdapter, _nvidia_provider_chain(), ProviderConfig, TaskResult, TaskSpec, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:… (+16 more)
...
Cohesion: 0.06
Nodes (55): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), get_approval_store(), get_gate() (+47 more)

### Community 148 - "failover_client.py"
Cohesion: 0.06
Nodes (46): _auto_disable(), _describe_registry(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion(), _log_recovery() (+38 more)
...
Cohesion: 0.04
Nodes (47): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,… (+39 more)
...
Cohesion: 0.11
Nodes (41): governance_enabled(), True when the governance layer should run at all. A single global off switch…, _governance_check(), Evaluate a runtime dispatch against policy; audit it either way. Returns…, _decision(), _engine(), Any, RoutingDecision (+33 more)
...
Cohesion: 0.07
Nodes (39): _autonomy_bg_cycle(), autonomy_status(), autonomy_tick(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_trigger(), Background CEO cycle + task dispatch. Runs fire-and-forget., Public autonomy readiness probe — no authentication required. A live deploy… (+31 more)
...
Cohesion: 0.04
Nodes (59): MCPUnavailableError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Raised when the MCP server is unreachable or the circuit is open., TaskResult, TaskSpec, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, is_e2b_sdk_importable() (+51 more)
...
### Community 203 - "HTTPException"
Cohesion: 0.04
Nodes (89): clear_cost_attribution(), clear_response_cache(), create_access_token(), create_github_pr(), create_provider(), create_refresh_token(), delete_api_key(), delete_mcp_server() (+81 more)
...
Cohesion: 0.04
Nodes (76): delegate_seo_findings(), Create real agent tasks from the audit's delegation plan. Each work package…, quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., apply_from_db(), Load the stored overrides and apply them. Called once at startup., Path (+68 more)

### Community 208 - "test_brain_availability_doctor.py"
Cohesion: 0.05
Nodes (51): _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One piece of work the CEO has taken ownership of. ``state`` is the supervisor's…, Return ``(done, total)`` subtask counts. (+43 more)
...
Cohesion: 0.04
Nodes (69): packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., Recursively strip secrets from an arbitrary argument structure. Fails…, scrub(), _scrub_inner(), _scrub_text(), _truncate(), BudgetTracker (+61 more)
...
Cohesion: 0.09
Nodes (32): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Public Doctor endpoint — no authentication required. Returns system-level… (+24 more)
...
### Community 243 - "ai/router.py"
Cohesion: 0.10
Nodes (24): _dead_model_key(), _exponential_backoff_cooldown(), _mark_model_dead(), _notify_watchdog(), _ollama_reasoning_effort(), provider_router.py — auto-generated module docstring (user-research skill scan)., Notify the brain watchdog of a provider call outcome (fire-and-forget).…, # NOTE: 429 and 410 are handled specially in _try_one_provider (immediate (+16 more)
...
Cohesion: 0.04
Nodes (38): approveTaskCheckpoint(), approveTaskExecution(), clarifyTask(), createSprint(), fetchSprints(), fetchVelocity(), followUpTask(), BarChart() (+30 more)
...
Cohesion: 0.03
Nodes (67): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., setup_database_moks() (+59 more)
...
Cohesion: 0.05
Nodes (48): When coder == reviewer model, swarm should log a warning., _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestPhaseSequenceError (+40 more)
...
Cohesion: 0.03
Nodes (92): auth_me(), auto_recommend_skills(), _check_storage_health(), delete_github_token(), _delete_limited_chat_session(), delete_session(), delete_source(), discover_remote_skills() (+84 more)
...
### Community 598 - "_FakeAsyncClient"
Cohesion: 0.20
...
### Community 630 - ".on_task_complete"
Cohesion: 0.16
...
Cohesion: 0.04
Nodes (58): brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, auto_disable_provider(), brain_availability_summary(), describe_disabled_reason(), _disabled_from_mongo(), _disabled_from_sqlite(), disabled_providers() (+50 more)
...
### Community 641 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 744 - "_fake_fetch_module"
Cohesion: 0.36
...
### Community 748 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 809 - "TestGithubSignalHardening"
Cohesion: 0.22
...
### Community 817 - "_resolve_user_id"
Cohesion: 0.12
...
Cohesion: 0.02
Nodes (156): admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, BrainTestRequest (+148 more)
...
### Community 937 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
Cohesion: 0.05
Nodes (72): _acquire_provider_probe(), extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig (+64 more)
...
### Community 1073 - "_provider_env_value"
Cohesion: 0.50
...
- **Why does `_fixture()` connect `_fixture` to `ExecutionRequest`, `TestRuntimeControl`, `test_llm_router_queue_cache.py`, `ProviderConfig`, `company_api.py`, `TaskStore`, `test_operational_incidents.py`, `TaskWorkflowService`, `test_llm_router_tpm.py`, `TaskSpec`, `SelfHealingAgent`, `test_openclaw_endpoints.py`, `test_llm_router_resilience.py`, `test_orchestrator_merge_decision.py`, `config.py`, `Task`, `resolve_identity`, `Agency`, `services/background.py`, `test_e2b_sandbox.py`, `test_failover_client_shared.py`, `test_unit8_model_catalog.py`, `services/seo_audit.py`, `ImprovementLoop`, `UserRole`, `test_agent_tool_governance.py`, `SQLiteStore`, `NotificationDispatcher`, `test_telegram_auto_approve.py`, `test_model_router.py`, `test_autonomy_status.py`, `PrimeAgentAdapter`, `test_local_brain_state.py`, `test_telegram_diag_endpoint.py`, `ChatHistoryStore`, `TokenBudget`, `test_autonomous_agency_e2e.py`, `ResearchTask`, `llm/router.py`, `TestClient`, `e2e/test_browser.py`, `test_control_plane_api.py`, `test_procedural_memory.py`, `claim`, `ArtifactStore`, `test_startup_warmup.py`, `KeyPool`, `KeyStore`, `TestHarnessAdapter`, `parse_event_stream`, `BrainWatchdog`, `test_trend_scoping.py`, `.on_task_complete`, `test_context_rulebook.py`, `run_task`, `test_openclaw_gateway.py`, `brain_failover.py`, `reset_failover_manager`, `test_doctor_service_token_check.py`, `test_issue_intake.py`, `model_router.py`, `test_new_features_e2e.py`, `AgentJobManager`, `MCPClient`, `_Budget`, `test_provider_enable_disable.py`, `persist_plan_spec`, `clear_cooldowns`, `test_daily_2026_07_27.py`, `_clear_cache`, `_Recorder`, `OllamaCircuitBreaker`, `test_ceo_router.py`, `FeatureMaturity`, `sam`, `test_sam_livekit.py`, `AgentPlan`, `test_features_api.py`, `test_video_transcript.py`, `test_agile_api.py`, `test_app_settings.py`, `test_llm_router_disabled.py`, `test_task_clarification.py`, `E2BSandboxSession`, `test_brain_availability_doctor.py`, `TrendWatcher`, `test_trend_watcher.py`, `test_brain_patch_service_token.py`, `test_portfolio_intake.py`, `TestEstimateTokensForMessages`, `test_agent_free_brain.py`, `TestClient`, `ContextPruner`, `get_store`, `TestMongoGate`, `OrchestratorQueue`, `test_sam_voice.py`, `SchedulerStore`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `OutputFilter`, `test_platform_controls.py`, `test_purge_backlog.py`, `_start_ceo_agency`, `AgentSwarm`, `analyze_page`, `TestAuthAndTaskOwnership`, `test_telegram_service_webhook.py`, `test_crispy_burn_in.py`, `test_webui_provider_priority.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `reap_expired_companies`, `test_rate_limiter.py`, `test_phase5_doctor.py`, `seo_api.py`, `api_keys_for`, `test_rate_limit_backoff_survives.py`, `test_service_token.py`, `test_harness_spec.py`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `test_regression.py`, `test_agency_fix.py`, `test_telegram_mutating_commands.py`, `test_dashboard_cache.py`, `clear_override`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `test_ai_insights.py`, `_resolve_push_token`, `test_knowledge_sync.py`, `client`, `allow_paid`, `test_ping.py`, `test_voice_pipeline.py`, `provider_max_rpm`, `test_task_service_failed_comment.py`, `cost_tracker.py`, `TestNormalizeResponseFormat`, `tests/conftest.py`, `test_task_brain_preflight.py`, `test_kimi_bridge_server.py`, `test_company_api.py`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `ExecutionRequest`, `test_backend_server_features.py`, `LocalWorkspace`, `proxy.py`, `TestAgentRunnerExecution`, `reset_failover_manager`, `direct_chat.py`, `ReactScratchpad`, `TestAgentRunnerSafety`, `ContextManager`, `TaskSpec`, `test_e2b_adapter.py`, `test_daily_automation_2026_07_11.py`, `test_governance_api.py`, `AgentJobManager`, `MCPClient`, `test_ceo_dispatcher.py`, `StuckDetector`, `test_failover_client_shared.py`, `CEODispatcher`, `MultiAgentSwarm`, `test_agent_tool_governance.py`, `WorkspaceTools`, `AgentPlan`, `backend/server.py`, `TokenBudget`, `test_autonomous_agency_e2e.py`, `E2BSandboxSession`, `UserMemoryStore`, `GitHubTools`, `AgentSessionStore`, `InternalAgentAdapter`, `test_agent_free_brain.py`, `test_empirical_verify.py`, `ContextPruner`, `TestAgentLoopMCPIntegration`, `AdaptiveHalter`, `FreeBuffAgent`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `AgentScheduler` connect `AgentScheduler` to `test_schedule_growth_invariants.py`, `proxy.py`, `ScheduleStore`, `test_purge_backlog.py`, `TaskWorkflowService`, `test_force_cleanup_conditional_delete.py`, `get_task_store`, `ScheduledJob`, `test_control_plane_api.py`, `SchedulerStore`, `test_schedule_persistence.py`, `test_schedule_backlog_drain.py`, `backend/server.py`, `services/background.py`?**
...
- **Should `ExecutionRequest` be split into smaller, more focused modules?**
  _Cohesion score 0.02645971914264597 - nodes in this community are weakly interconnected._