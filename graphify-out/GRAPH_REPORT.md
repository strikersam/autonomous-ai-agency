...
## Corpus Check
- 1445 files · ~2,094,430 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 29389 nodes · 54561 edges · 1243 communities (1131 shown, 112 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2693 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d8ac0396`
...
## Communities (1243 total, 112 thin omitted)
...
Cohesion: 0.03
Nodes (160): middleware, admin_control(), admin_delete_user(), admin_login(), admin_logout(), admin_rotate_user(), admin_status(), AdminControlBody (+152 more)
...
Cohesion: 0.05
Nodes (74): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+66 more)

### Community 5 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (112): int, cosine_similarity(), payload_key(), Exact-match cache key over the fields that change the answer. Routing…, Cosine similarity between two vectors, 0.0 when either is degenerate., CacheLayerConfig, chunk_document(), ContextManager (+104 more)
...
### Community 7 - "TaskExecutionCoordinator"
Cohesion: 0.03
Nodes (65): AgentDefinition, AgentStore, Any, field_validator, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents. (+57 more)

### Community 8 - "HTTPException"
Cohesion: 0.04
Nodes (121): _get_current_user_thunk(), _get_optional_user_thunk(), Request, admin_seed(), call_llm(), cancel_chat_agent_job(), clear_cost_attribution(), clear_response_cache() (+113 more)

### Community 9 - "TaskSpec"
Cohesion: 0.04
Nodes (75): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns…, runtimes/adapters/e2b.py — E2B Firecracker micro-VM runtime adapter. Routes…, runtimes/adapters/goose.py — Goose adapter (TIER 2). Goose…, runtimes/adapters/hermes.py — Hermes Agent adapter (FIRST CLASS). Hermes Agent…, Internal runtime adapter that executes tasks via the built-in AgentRunner.… (+67 more)
...
Cohesion: 0.02
Nodes (131): _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache() (+123 more)

### Community 12 - "LLMRequest"
Cohesion: 0.03
Nodes (144): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC, Any (+136 more)

### Community 13 - "enforcement.py"
Cohesion: 0.08
Nodes (25): BudgetExceeded, get_gate(), guard_tool_call(), GuardResult, _host_of(), _is_write_tool(), _more_restrictive(), Any (+17 more)
...
Cohesion: 0.04
Nodes (81): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent., _action_matches(), _as_list(), Decision (+73 more)
...
Cohesion: 0.03
Nodes (82): FailureCategory, heal_signature(), HealingEvent, HealState, _now(), Any, Enum, str (+74 more)

### Community 21 - "llm/router.py"
Cohesion: 0.03
Nodes (100): _load(), main(), get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., Test hook — clear all usage accounting., reset(), CacheManager (+92 more)
...
Cohesion: 0.04
Nodes (87): _merge_changed_files(), Reset the singleton (test helper)., Collect changed_files across all specialists into a single de-duped list., reset_ceo_dispatcher(), ClassifyOutput, get_ceo_fallback_stats(), PlanOutput, CLASSIFY phase: domain and task type determination. (+79 more)
...
Cohesion: 0.06
Nodes (40): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Call-time resolver for an agent role model id. Delegates to… (+32 more)

### Community 30 - "test_runtime_governance.py"
Cohesion: 0.10
Nodes (47): _governance_check(), _governance_identity(), Evaluate a runtime dispatch against policy; audit it either way. Returns…, Resolve the acting identity for a runtime dispatch. Identity comes from…, _decision(), _engine(), Any, RoutingDecision (+39 more)

### Community 31 - "models/company_graph.py"
Cohesion: 0.03
Nodes (91): ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraphResponse, CompanyGraphSnapshot, CompanyUpdateRequest, Connector, DetectedSystem (+83 more)
...
Cohesion: 0.06
Nodes (34): Attempt, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any, AsyncClient, BaseException, ProviderConfig (+26 more)

### Community 36 - "test_vision_routing.py"
Cohesion: 0.12
...
Cohesion: 0.04
Nodes (77): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, all_provider_ids(), _provider_ids_from_literal(), Resolve the model id for a component's role on a provider. Parameters…, Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, resolve_component_model() (+69 more)
...
### Community 39 - "Task"
Cohesion: 0.04
Nodes (85): ApprovalCheckpoint, Full task/issue document., Human approval gate in a task's execution., Task, Owns lifecycle transitions, comment semantics, and approval rules., TaskWorkflowService, asyncio, The reconciler must not re-queue tasks parked at the pre-execution gate.… (+77 more)
...
Cohesion: 0.07
Nodes (16): Company, Any, field_validator, The core company entity - root of the Company Graph., Coerce unrecognised system_type values to 'custom' so the model never crashes…, Test Company Graph Pydantic models., Test that all Company Graph models can be imported., Test Company model creation. (+8 more)
...
Cohesion: 0.12
Nodes (25): loop_readiness(), LoopRegistry, LoopSpec, BaseModel, field_validator, loop-cost: approximate tokens this loop spends over 30 days., The full fleet catalogue., loop-cost: total fleet token spend over 30 days. (+17 more)
...
Cohesion: 0.05
Nodes (47): Register the built-in agent tools that are always available., _register_builtin_tools(), Path, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Return a previously saved memory value, or an empty string if absent., Persist a key/value pair to the user's profile store., Return the first *lines* lines of a file. Just-in-time retrieval: the executor…, Return a lightweight index of files with line counts and sizes. This is the… (+39 more)

### Community 50 - "TaskStore"
Cohesion: 0.03
Nodes (115): agents/store.py — Persistent store for user-defined agent configurations.…, Set the global agent store instance (e.g., with MongoDB on startup)., set_agent_store(), Point the feature stores at the shared database connection. Deliberately…, _wire_feature_stores(), Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker… (+107 more)
...
Cohesion: 0.02
Nodes (108): parse_event_stream(), Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, Drop the cached Mongo client, backoff, and read caches (tests)., reset_kv_state(), base_url(), mobile_page(), proxy_url(), Config (+100 more)
...
Cohesion: 0.05
Nodes (43): PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, AgentJob, AgentJobManager, _now(), Any (+35 more)

### Community 56 - "RuntimeCapabilityRegistry"
Cohesion: 0.09
Nodes (12): Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry, TaskResult, TaskSpec, StubAdapter, TaskHarnessStub (+4 more)

### Community 57 - "ExecutionRequest"
Cohesion: 0.06
Nodes (40): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY…, reset_orchestrator() (+32 more)
...
Cohesion: 0.09
Nodes (17): get, Return the full SEO/GEO/AIO check catalog. Static metadata only, but gated…, seo_check_catalog(), Static definition of a single audit check (catalog entry)., SeoCheckDefinition, auto_fixable_checks(), _c(), get_check() (+9 more)

### Community 69 - "get_task_store"
Cohesion: 0.04
Nodes (76): quick_notes_submit(), _QuickNoteBody, The task store the background services should use, wiring it if needed.…, Submit a quick-note URL or instruction from the dashboard FAB., _task_store_for_background(), apply_from_db(), Load the stored overrides and apply them. Called once at startup., Path (+68 more)
...
Cohesion: 0.04
Nodes (58): Run a full SEO/GEO/AIO audit against a website and persist the evidence., run_seo_audit(), field_validator, A single occurrence of a check firing on a specific URL., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest (+50 more)
...
Cohesion: 0.06
Nodes (21): MockTransport, BrowserFetcher, FetchResult, HttpxFetcher, looks_blocked(), Response, services/seo_fetch.py - pluggable page-fetch backends for the SEO audit engine.…, Browser backend: renders pages with a real Chromium via Playwright. This is the… (+13 more)
...
### Community 90 - "test_unit6_resolve_component_model.py"
Cohesion: 0.05
Nodes (39): Convenience: resolve all four role models for a component. Returns a dict with…, resolve_component_role_models(), tests/test_unit6_resolve_component_model.py — UNIT 6 regression tests. Verifies…, When the DB cache is fresh AND provider matches the active primary, the DB-…, When the DB primary differs from the requested provider, the catalog preset for…, When `provider` is None, the DB primary's saved model wins., With no DB cache and no provider, the env var is consulted., An unknown role returns the safe default rather than raising. (+31 more)
...
Cohesion: 0.11
Nodes (60): ApprovalRequest, BackgroundTasks, add_comment(), approve_checkpoint(), approve_execution(), clarify_task(), create_task(), _current_user() (+52 more)
...
Cohesion: 0.05
Nodes (72): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), classify_direct_chat_intent(), _contains_keyword(), detect_intent(), intent.py — Intent classification for direct chat (answer_only, execute_now,…, Return True if content contains any execution or analysis keyword. (+64 more)
...
Cohesion: 0.08
Nodes (26): AgentSessionStore, _now(), Connection, Path, Row, Safe getter for sqlite3.Row — Row supports index access but not .get()., Create a session with a caller-supplied session_id (useful for tests and…, SQLite-backed session store. Sessions and their message history are persisted… (+18 more)
...
Cohesion: 0.11
Nodes (25): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is… (+17 more)
...
Cohesion: 0.04
Nodes (40): get_tool_registry(), _infer_parameters_from_func(), Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed., Look up a tool by name. (+32 more)
...
Cohesion: 0.08
Nodes (45): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), get_web_reach(), _load_script_module(), Any, ModuleType, Response, Dynamically load a pure-stdlib helper module from .github/scripts/. Returns… (+37 more)
...
Cohesion: 0.04
Nodes (44): _build_default_manager(), Any, RoutingDecision, TaskResult, TaskSpec, runtimes/manager.py — RuntimeManager., Return health status for all registered runtimes., Return routing decision audit log (newest first). (+36 more)
...
Cohesion: 0.07
Nodes (36): E2BAdapter, Any, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, _FakeAsyncSandboxClass, _FakeCommandResult, _FakeCommands, _FakeFiles (+28 more)
...
Cohesion: 0.06
Nodes (31): Added, Added, Added, Added, Added, Added, Added, Added (+23 more)
...
### Community 177 - "audit_drift"
Cohesion: 0.12
Nodes (24): audit_drift(), _cmd_audit(), DriftReport, _grade(), load_registry(), load_registry_sync(), main(), Namespace (+16 more)
...
Cohesion: 0.10
Nodes (29): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep (+21 more)
...
Cohesion: 0.10
Nodes (30): _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), _in_container(), Path, Public Doctor endpoint — no authentication required. Returns system-level… (+22 more)
...
### Community 189 - "make_fetcher"
Cohesion: 0.08
Nodes (32): export_seo_audit(), Export a stored audit. - ``csv`` aggregated findings, Screaming Frog…, _build_curl_cffi_fetcher(), _build_pdf(), main(), _parse_args(), Namespace, Path (+24 more)
...
Cohesion: 0.09
Nodes (14): tools.py — WorkspaceTools: read/write/search and diff application (risky…, Connection, Path, Per-user key/value memory store backed by SQLite. Allows agents to persist and…, Return all stored key/value pairs for *user_id*., Delete a memory entry. Returns ``True`` if a row was removed., Persistent key/value store scoped per user. Thread-safe; uses a single SQLite…, Upsert a memory entry for *user_id*. (+6 more)
...
### Community 204 - "DirectChatSession"
Cohesion: 0.14
...
Cohesion: 0.09
Nodes (33): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), observability_diag_public(), PUBLIC diagnostic endpoint for Langfuse — no auth required. Returns exactly…, _base_url(), _department_trace_tags() (+25 more)
...
### Community 207 - "brain_failover.py"
Cohesion: 0.12
...
Cohesion: 0.03
Nodes (80): main(), Returns (url, ok, summary)., _scan_one(), _is_blocked_host(), _is_safe_url(), _looks_like_bot_challenge(), Any, BeautifulSoup (+72 more)
...
### Community 225 - "skill_bindings.py"
Cohesion: 0.04
Nodes (50): Enum, SuperClaude Workflow Engine — Workflow, Task, and topological DAG execution.…, Return tasks whose dependencies are all satisfied., Number of tasks in the workflow., Number of completed tasks., Number of failed tasks., Executes workflows using topological ordering., Register a workflow with the engine. (+42 more)

### Community 226 - "CEOSupervisor"
Cohesion: 0.10
Nodes (16): get_ceo_dispatcher(), Return the shared CEODispatcher singleton., CEOSupervisor, Any, Sweeps the CEO ledger and drives open goals to closure., Sweep on the configured cadence until cancelled. A failing sweep is logged and…, Inter-sweep delay, behind a method so tests can shorten it. Patching…, True when this process already has a re-drive in flight for *goal_id*. (+8 more)
...
Cohesion: 0.05
Nodes (34): _best_cloud_primary_base(), InternalAgentAdapter, TaskResult, TaskSpec, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Execute a TaskSpec using the internal AgentRunner and convert the agent's… (+26 more)
...
### Community 240 - "delegate_seo_findings"
Cohesion: 0.07
Nodes (40): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), plan_seo_sprint(), BaseModel, post, Fetch a complete stored audit report. Side-effect: an audit whose stub is still… (+32 more)
...
Cohesion: 0.09
Nodes (33): _build_caller_chain(), _build_context_doc(), _build_grounding_block(), _build_pr_description(), _build_todos_md(), _build_user_message(), _call_claude(), _call_mistral() (+25 more)
...
Cohesion: 0.07
Nodes (49): _digest(), get_ring(), KeyState, packages/llm/keys.py — multi-key rotation with per-key health. A provider may…, Return the next healthy key and its index. Returns ``(None, -1)`` when every…, Cool a key down and return the cooldown length in seconds. Consecutive limits…, True when every key for the provider is in cooldown., The process-wide key ring. (+41 more)
...
### Community 254 - "plan_next_sprint"
Cohesion: 0.17
...
### Community 276 - "company_api.py"
Cohesion: 0.04
Nodes (106): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+98 more)
...
Cohesion: 0.08
Nodes (26): Dynamic model router package. Public API:: from router import get_router,…, _build_builtin_model_map(), _default_model(), _default_reasoning_model(), get_router(), ModelRouter, _nvidia_key_present(), Any (+18 more)
...
Cohesion: 0.09
Nodes (37): get_failover_manager(), ProviderHealth, Return the singleton BrainFailoverManager., Circuit-breaker state for a provider., _make_manager(), tests/test_brain_failover.py — Universal multi-provider brain failover tests.…, Guards the three tests below from pinning a retired id again: if the fixture…, Status snapshot doesn't leak API keys. (+29 more)

### Community 289 - "workflow_orchestrator.py"
Cohesion: 0.08
Nodes (36): _NoopDB, Fallback in-memory store when no DB is available., BoundContext, ExecutionResult, JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput (+28 more)
...
Cohesion: 0.10
Nodes (14): Atomically delete a run-once row only while it is still unfired. Closes the…, _age_seconds(), Any, agent/scheduler.py — Scheduled Agent Jobs Cron-based job scheduler. Each job…, Capture the FastAPI main event loop so APScheduler's background thread can…, The one retention policy for unfired one-shots, read from its owner.…, Drop a job from in-memory state and APScheduler (mirrors ``delete()``). Popping…, Durably remove one job, then drop its in-memory + APScheduler mirrors. The… (+6 more)
...
Cohesion: 0.06
Nodes (43): Enum, Agentic Agile — Sprint management with velocity tracking and burndown. Issue:…, Status of a user story within a sprint., Qualitative health signal derived from sprint metrics., SprintHealth, StoryStatus, CapacityAllocation, Enum (+35 more)
...
Cohesion: 0.04
Nodes (79): BudgetTracker, classify(), GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., Return the ``(surface, action)`` a tool call really represents. The *action* is…, AgentIdentity, _coerce_text() (+71 more)
...
### Community 322 - "test_phase6_workflow.py"
Cohesion: 0.16
...
### Community 345 - "get_skill_bindings"
Cohesion: 0.10
Nodes (25): build_matrix(), _families(), main(), Any, Map family -> sorted list of test files that mention it (quoted token)., Return one row per family, derived entirely from code., render_markdown(), _test_evidence_index() (+17 more)
...
Cohesion: 0.15
Nodes (10): Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., Ollama provider adapter — local LLM inference., packages/ai/manager.py — ProviderManager. Single entry point for all LLM calls.…, Provider, ABC, packages/ai/provider.py — Provider abstraction interface. Every LLM provider…, Base interface every provider must implement. Implementations live in…, Lower = higher priority in the fallback chain. (+2 more)

### Community 354 - "OrchestratorSupervisor"
Cohesion: 0.11
Nodes (14): get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, start_orchestrator_supervisor() (+6 more)
...
Cohesion: 0.08
Nodes (22): CommercialFallbackRequiredError, _get_director(), _normalized_provider_type(), ProviderAttempt, ProviderRouter, Any, Response, RuntimeError (+14 more)
...
Cohesion: 0.09
Nodes (17): _hardcoded_candidates(), parametrize, Model ids belong in a catalogue, not in Python. Four files in this repo each…, A frozen record of the divergence found on 2026-08-28. Reconciling these eleven…, NVIDIA is the one this work reconciled. It must not regress — that divergence…, A shrinking divergence is progress, but the list must be trimmed to match or it…, ``config/llm/models.yaml`` drives capability filtering. An entry for a retired…, A sixth model source, found on 2026-08-28 while verifying a fix.… (+9 more)

### Community 392 - "AiderAdapter"
Cohesion: 0.07
Nodes (17): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, TaskResult, TaskSpec, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor. (+9 more)
...
Cohesion: 0.13
Nodes (18): _cached(), _fast_count(), legacy_scheduler_list(), _produce_scheduler_jobs(), Single-flight TTL cache. Concurrent callers wait for the first producer., Count without materialising rows — prefers estimated_document_count., _clear_cache(), _CollWithEstimate (+10 more)
...
Cohesion: 0.08
Nodes (17): requires_db, asyncio, Test Company Graph services., Test that storage service can be initialized., Test company CRUD operations - skipped as requires specific config., Test specialist service., Test that specialist service can be initialized., Regression for the create-company 500. ``MongoDBStore.create_company_graph``… (+9 more)
...
Cohesion: 0.04
Nodes (62): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, packages/llm/health.py — provider health tracking and circuit breaking. Two…, Test hook — clear all health state., reset() (+54 more)
...
### Community 437 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 483 - "test_workflow_models.py"
Cohesion: 0.13
...
### Community 497 - "test_minimax_bug_fixes.py"
Cohesion: 0.13
...
### Community 509 - "Nothing is blocked on an agent. Four things need a human."
Cohesion: 0.20
Nodes (9): 0. NVIDIA models — resolved, with one measurement still missing, 1. Render is suspended for billing — this is the big one, 2. PR #1336 needs a human decision, 3. The loop implemented an issue a previous PR had rejected as out of scope, 4. The queue is not ordered the way the loop's purpose implies, If you pick this up next, Next Action, Nothing is blocked on an agent. Four things need a human. (+1 more)
...
### Community 513 - "test_ai_insights.py"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (46): Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, Any, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., One governed action, fully described. Field order follows the… (+38 more)
...
### Community 537 - "test_agent_chat_integration.py"
Cohesion: 0.23
...
Cohesion: 0.05
Nodes (56): clear_cooldowns(), _dead_model_key(), _exponential_backoff_cooldown(), get_dead_models(), _is_model_dead(), is_provider_on_cooldown(), _mark_model_dead(), mark_provider_failed() (+48 more)
...
### Community 560 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 604 - "test_direct_chat_async.py"
Cohesion: 0.12
...
### Community 605 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
Cohesion: 0.02
Nodes (94): BusinessCategory, CompanyGraph, Get the index of a phase in the workflow., The complete Company Graph - canonical core model for the Autonomous AI Agency.…, Find a website by its URL., Find a repository by its URL., Find a workflow by its name., Find knowledge items matching any of the given tags. (+86 more)
...
Cohesion: 0.11
Nodes (9): generate_sprint_retro(), Derive retro notes for ``sprint`` from its current metrics. Records…, Sprint retrospective notes and follow-up action items., Whether the retrospective has any recorded content., Retrospective, Tests for agents/agile_ceremonies.py — autonomous agile ceremonies. Loads…, TestGenerateBacklogRetro, TestGenerateSprintRetro (+1 more)
...
Cohesion: 0.13
Nodes (3): DecisionsStoreTests, _fresh_store(), Smoke: create() returns a fresh dec_<hex8> per call (no error surfaces from…
...
### Community 631 - "is_anthropic_base_url"
Cohesion: 0.18
...
### Community 639 - "_redact_for_notification"
Cohesion: 0.24
...
### Community 640 - "tests/conftest.py"
Cohesion: 0.08
...
### Community 653 - "TestKillSwitchDurability"
Cohesion: 0.15
...
### Community 705 - "TestRequireAdmin"
Cohesion: 0.29
...
### Community 763 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 766 - "._check_permissions"
Cohesion: 0.29
...
Cohesion: 0.08
Nodes (25): AnthropicProvider, Any, AsyncClient, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Anthropic's extended-thinking constraints on ``budget_tokens``: at least 1024,…, Return a system-prompt instruction that enforces JSON output. Anthropic's… (+17 more)

### Community 786 - "_resolve_push_token"
Cohesion: 0.17
...
### Community 788 - "test_server_autonomy_and_index_fixes.py"
Cohesion: 0.29
...
### Community 826 - "BackgroundServices"
Cohesion: 0.38
...
### Community 940 - "nvidia_models.py"
Cohesion: 0.15
Nodes (19): _fetch_models_json(), _is_chat_model(), live_model_ids(), _rank_key(), rank_models(), Shared NVIDIA NIM model source for the autonomous agent scripts. This module…, The ids a caller should try, in order. Memoised: the agent loop asks once per…, Drop the memoised result. For tests. (+11 more)
...
### Community 986 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
Cohesion: 0.05
Nodes (37): livenim, allow_paid_brain(), is_anthropic_model(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, True when *model* names a paid Anthropic/Bedrock-Claude model. Covers native…, Resolve the free NVIDIA NIM brain from env, or ``None`` if unconfigured.…, resolve_free_nvidia_brain(), _FakeAsyncClient (+29 more)

### Community 998 - "TestAdapterMetadata"
Cohesion: 0.05
Nodes (25): HermesAdapter, Any, AsyncClient, Response, TaskResult, TaskSpec, Submit task to Hermes via its /tasks endpoint., Adapter for Hermes Agent — FIRST CLASS autonomous runtime. (+17 more)

### Community 999 - "backend/server.py"
Cohesion: 0.02
Nodes (208): _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills() (+200 more)
...
Cohesion: 0.05
Nodes (44): AgentRole, AgentSwarm, Any, agents/swarm.py — AgentSwarm: routes workflow phases to the correct agent. The…, Return the agent role responsible for *phase*., Return the AgentProfile for the agent driving *phase*., Return a JSON-serialisable summary of all agent profiles., Run a pre-gate or report phase through the correct agent. Enforces permission… (+36 more)
...
### Community 1202 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 1207 - "TestDirectChatNonBlocking"
Cohesion: 0.40
...
### Community 1218 - "_extract_pytest_failure"
Cohesion: 0.42
...
### Community 1223 - "commercial_equivalent.py"
Cohesion: 0.50
...
### Community 1225 - "classify_domain"
Cohesion: 0.29
...
### Community 1226 - "JWTUserStateMiddleware"
Cohesion: 0.67
...
### Community 1235 - "TestWorkflowIntegration"
Cohesion: 0.33
...
### Community 1238 - "verify_pr_exists"
Cohesion: 0.50
...
### Community 1239 - "_clean_director"
Cohesion: 0.50
...
### Community 1248 - "_fake_fetch_module"
Cohesion: 0.36
...
## Knowledge Gaps
- **3416 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3411 more)
...
- **Why does `_fixture()` connect `_fixture` to `test_ai_insights.py`, `test_provider_router.py`, `services/background.py`, `test_llm_router_queue_cache.py`, `workflow/api.py`, `TaskExecutionCoordinator`, `test_company_api.py`, `brain_config.py`, `AuditLog`, `settings.py`, `PolicyEngine`, `SelfHealingAgent`, `llm/router.py`, `Agency`, `PrimeAgentAdapter`, `test_provider_state_durability.py`, `CEOLedger`, `ai/router.py`, `models/company_graph.py`, `test_monitor_lib.py`, `test_openclaw_endpoints.py`, `test_unit8_model_catalog.py`, `test_vision_routing.py`, `Task`, `ImprovementLoop`, `test_model_router.py`, `test_llm_router_strategies.py`, `test_failover_client_shared.py`, `test_implement_agent_routing.py`, `test_ceo_router.py`, `TaskStore`, `test_process_quick_note_workflow.py`, `AgentJobManager`, `SQLiteStore`, `ChatHistoryStore`, `E2BSandboxSession`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `SeoCheckDefinition`, `test_local_brain_state.py`, `NotificationDispatcher`, `test_phase5_doctor.py`, `test_operational_incidents.py`, `test_refresh_agent_built_proof.py`, `services/seo_audit.py`, `test_telegram_diag_endpoint.py`, `config.py`, `ResearchTask`, `TestClient`, `resolve_e2b_config`, `test_procedural_memory.py`, `TestMongoGate`, `test_direct_chat_async.py`, `e2e/test_browser.py`, `TokenBudget`, `UserRole`, `_provider`, `test_knowledge_sync.py`, `test_startup_warmup.py`, `seo_api.py`, `KeyStore`, `claim`, `FeatureMaturity`, `test_context_rulebook.py`, `tests/conftest.py`, `ArtifactStore`, `run_task`, `test_webui_provider_priority.py`, `test_issue_intake.py`, `test_openclaw_gateway.py`, `test_response_cache.py`, `test_kimi_bridge_server.py`, `test_trend_scoping.py`, `ScheduledJob`, `BackgroundAgent`, `test_telegram_freebuff.py`, `TestHarnessAdapter`, `test_e2b_data_flow.py`, `MCPClient`, `test_new_features_e2e.py`, `test_agent_tool_governance.py`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `test_e2b_adapter.py`, `_Budget`, `KeyPool`, `.on_task_complete`, `test_e2b_task_wiring.py`, `OllamaCircuitBreaker`, `analyze_page`, `_resolve_user_github_token`, `test_doctor_service_token_check.py`, `test_llm_router_disabled.py`, `test_features_api.py`, `test_video_transcript.py`, `TrendWatcher`, `_undeclared`, `test_agile_api.py`, `test_app_settings.py`, `test_agent_scripts_share_one_model_list.py`, `test_portfolio_intake.py`, `OrchestratorCheckpointStore`, `_clean_director`, `test_trend_watcher.py`, `test_unit7_catalog_propagation.py`, `test_nvidia_model_discovery.py`, `TestClient`, `test_brain_failover_413.py`, `ContextPruner`, `BrainWatchdog`, `test_llm_router_e2e.py`, `session_retro.py`, `WorkflowRun`, `test_telegram_service_webhook.py`, `._check_permissions`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `OutputFilter`, `test_platform_controls.py`, `_resolve_push_token`, `test_scheduler_hydration_bounded.py`, `test_rate_limiter.py`, `test_mcp_governance.py`, `test_sam_voice.py`, `OrchestratorQueue`, `test_purge_backlog.py`, `cost_tracker.py`, `test_task_clarification.py`, `_resolve_brain_provider`, `test_control_plane_api.py`, `test_task_service_failed_comment.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `reap_expired_companies`, `resolve_active_brain`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_brain_patch_service_token.py`, `test_harness_spec.py`, `test_ping.py`, `test_dashboard_cache.py`, `model_discovery.py`, `TestSwarmRoleRouting`, `test_regression.py`, `test_agency_fix.py`, `test_v4_api.py`, `test_freebuff_bot.py`, `client`, `platform_controls_router.py`, `test_voice_pipeline.py`, `_job_text`, `test_agent_free_brain.py`, `TestNormalizeResponseFormat`, `test_task_store_fails_loud_in_production.py`, `test_runtimes_health_endpoint.py`, `test_sam_livekit.py`?**
  _High betweenness centrality (0.119) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `proxy.py`, `TaskSpec`, `ReactScratchpad`, `PolicyEngine`, `CEODispatcher`, `.execute`, `_resolve_push_token`, `BackgroundAgent`, `ContextManager`, `Agency`, `test_agent_chat_integration.py`, `MCPClient`, `test_agent_tool_governance.py`, `loop.py`, `test_e2b_adapter.py`, `workflow_orchestrator.py`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `StuckDetector`, `test_daily_automation_2026_07_11.py`, `WorkspaceTools`, `AgentPlan`, `test_governance_api.py`, `E2BSandboxSession`, `test_empirical_verify.py`, `UserMemoryStore`, `TestGetCurrentTimeTool`, `LocalWorkspace`, `test_direct_chat_async.py`, `TokenBudget`, `TestAgentLoopMCPIntegration`, `test_agent_free_brain.py`, `backend/server.py`, `InternalAgentAdapter`, `test_brain_failover_413.py`, `GitHubTools`, `AdaptiveHalter`, `ContextPruner`, `direct_chat.py`, `AgentSessionStore`, `TestAgentRunnerExecution`, `WorkflowRun`, `FreeBuffAgent`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `test_all_providers_discovery.py`, `test_provider_router.py`, `test_bedrock_provider.py`, `test_anthropic_router.py`, `test_colibri_provider.py`, `implement_agent.py`, `ai/router.py`, `TestAnthropicPayloadStructuredOutput`, `test_brain_failover.py`, `TrafficDirector`, `NvidiaProvider`, `test_implement_agent_routing.py`, `_get_provider_policy`, `TestRouterIntegration`, `ProviderConfig`, `TestOpenAiToBedrockConverse`, `failover_client.py`, `kimi_bridge_provider_config`, `backend/server.py`, `system_instruction`, `direct_chat.py`, `resolve_active_brain`, `test_bedrock_live.py`, `is_anthropic_base_url`?**
