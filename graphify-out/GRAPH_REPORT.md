...
## Corpus Check
- 1505 files · ~2,182,379 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30502 nodes · 56948 edges · 1278 communities (1116 shown, 114 thin omitted)
...
## Communities (1278 total, 114 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.02
Nodes (289): admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills() (+281 more)
...
Cohesion: 0.03
Nodes (172): check_ollama_async(), Async Ollama reachability check — non-blocking for FastAPI handlers., middleware, AdminIdentity, admin_control(), admin_create_key(), admin_create_user(), admin_delete_user() (+164 more)

### Community 3 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (153): DetectedIssue, get_improvement_loop(), ImprovementLoopState, IssueCategory, IssueSeverity, Enum, str, agent/improvement_loop.py — Continuous Improvement Engine Background scanner… (+145 more)

### Community 4 - "TaskStatus"
Cohesion: 0.03
Nodes (131): Task, Helpers that turn scheduler and playbook activity into real tasks., Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, TaskStatus, Record the human decision on a task's **pre-execution** approval gate. A…, Re-run a task. Permissive across every non-active state so the dashboard Retry…, Add a follow-up instruction to a task and re-queue it for execution. Unlike… (+123 more)

### Community 5 - "Task"
Cohesion: 0.03
Nodes (110): emit_agency_observation(), Record an agency platform operation in Langfuse. Unlike…, Background dispatcher for task execution., Full task/issue document., Task, _brain_is_configured(), _gate_outward_facing_enabled(), _is_brain_connection_error() (+102 more)

### Community 6 - "AgentRunner"
Cohesion: 0.02
Nodes (119): AgentPhaseError, AgentRunner, Any, Exception, Path, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a… (+111 more)
...
Cohesion: 0.02
Nodes (115): CacheManager, cosine_similarity(), payload_key(), Any, Exact-match cache key over the fields that change the answer. Routing…, Cosine similarity between two vectors, 0.0 when either is degenerate., Owns every cache layer and the policy for what may enter them., Whether a response to ``request`` may be stored. Deliberately conservative — a… (+107 more)
...
Cohesion: 0.03
Nodes (98): _Entry, get_cache(), LRUCache, T, packages/llm/cache.py — layered caching. Five independent layers, each with its…, Live (unexpired) entries — used by the semantic layer's scan., The process-wide cache manager., Bounded TTL cache with LRU eviction. Thread-safe, dependency-free. (+90 more)
...
Cohesion: 0.02
Nodes (76): BusinessCategory, BusinessSystem, CompanyGraph, OnboardingProgress, SystemType, Get the index of a phase in the workflow., The complete Company Graph - canonical core model for the Autonomous AI Agency.…, Find a website by its URL. (+68 more)

### Community 13 - "test_brain_config_store.py"
Cohesion: 0.03
Nodes (99): BrainConfigPatch, get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), provider_key_present(), Return the process-wide ``BrainConfigStore`` singleton., Convenience wrapper used by the agent loop + brain resolver., Convenience wrapper used by the admin API endpoints. (+91 more)

### Community 14 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (104): BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput (+96 more)
...
Cohesion: 0.03
Nodes (112): Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint() (+104 more)
...
Cohesion: 0.03
Nodes (58): PreflightReport, AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs… (+50 more)

### Community 19 - "ExecutionRequest"
Cohesion: 0.03
Nodes (62): ExecutionRequest, MergeDecision, _NoopStore, Any, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs…, Push a Telegram approval-gate notification when a run pauses (Charter G1).… (+54 more)

### Community 20 - "LLMRequest"
Cohesion: 0.04
Nodes (102): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, build_provider(), Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the…, resolve_kind(), The models to consider on one provider., LLMRequest (+94 more)
...
### Community 26 - "resolve_component_model"
Cohesion: 0.03
Nodes (73): Resolve the model id for a component's role on a provider. Parameters…, resolve_component_model(), Dynamic model router package. Public API:: from router import get_router,…, _build_builtin_model_map(), _default_model(), _default_reasoning_model(), ModelRouter, _nvidia_key_present() (+65 more)

### Community 27 - "MongoDBStore"
Cohesion: 0.04
Nodes (55): KnowledgeItem, Structured knowledge about the company, systems, or processes., MongoDBStore, Any, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., Get a company by ID from SQLite. (+47 more)
...
### Community 29 - "Usage"
Cohesion: 0.05
Nodes (50): packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC, Any, AsyncClient, packages/llm/providers/base.py — the provider contract and the generic adapter.… (+42 more)

### Community 30 - "direct_chat.py"
Cohesion: 0.04
Nodes (86): DirectChatDoctor, PreflightIssue, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob (+78 more)
...
Cohesion: 0.04
Nodes (84): _load(), main(), Scheduled portfolio intelligence sweep. Run by `.github/workflows/portfolio-…, int, failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.… (+76 more)
...
Cohesion: 0.03
Nodes (56): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), AuditEvent, AuditLog, _luhn_ok(), Any, packages/governance/audit.py — the evidence trail for every governed action.…, True when ``digits`` (bare, no separators) satisfies the Luhn checksum. (+48 more)
...
Cohesion: 0.04
Nodes (35): Any, Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking)., Return all registered runtimes with their cached health status., Return the active routing policy as a plain dict. (+27 more)

### Community 38 - "CEODispatcher"
Cohesion: 0.04
Nodes (75): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), get_ceo_dispatcher(), _merge_changed_files(), _offload(), Any (+67 more)

### Community 39 - "SeoAuditEngine"
Cohesion: 0.04
Nodes (44): Run a full SEO/GEO/AIO audit against a website and persist the evidence., run_seo_audit(), field_validator, A single occurrence of a check firing on a specific URL., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest (+36 more)
...
Cohesion: 0.05
Nodes (57): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+49 more)

### Community 42 - "get_workflow_orchestrator"
Cohesion: 0.04
Nodes (44): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), OrchestratorSupervisor, Any, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor(), get_workflow_orchestrator() (+36 more)
...
### Community 47 - "brain_config.py"
Cohesion: 0.04
Nodes (73): BrainConfig, BrainConfigStore, default_brain_config(), provider_api_keys(), provider_base_url(), _provider_env_value(), provider_key_rotation_enabled(), BaseModel (+65 more)

### Community 48 - "tasks/api.py"
Cohesion: 0.07
Nodes (75): BackgroundTasks, add_comment(), approve_checkpoint(), approve_execution(), clarify_task(), create_task(), _current_user(), delete_task() (+67 more)
...
Cohesion: 0.05
Nodes (68): _action_matches(), _as_list(), Decision, GroupPolicy, _host_matches(), Mode, _normalise_path(), _path_matches() (+60 more)

### Community 51 - "models/company_graph.py"
Cohesion: 0.05
Nodes (74): ApprovalPolicy, CompanyCreateRequest, CompanyGraphSnapshot, CompanyResponse, CompanyUpdateRequest, Connector, DetectedSystem, Evidence (+66 more)

### Community 52 - "services/background.py"
Cohesion: 0.04
Nodes (78): get_agency(), set_agency(), Fan trends out to onboarded companies whose stack matches (G4). For each…, _autonomy_bg_cycle(), autonomy_status(), autonomy_tick(), Background CEO cycle + task dispatch. Runs fire-and-forget., Public autonomy readiness probe — no authentication required. A live deploy… (+70 more)
...
Cohesion: 0.04
Nodes (41): agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, _as_list(), _coerce_payload(), Any, RuntimeError, packages/integrations/render_mcp.py — Render platform access over MCP. The…, Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return… (+33 more)

### Community 55 - "ProviderConfig"
Cohesion: 0.05
Nodes (58): _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog() (+50 more)

### Community 56 - "_start_autonomy_loops"
Cohesion: 0.05
Nodes (66): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+58 more)

### Community 57 - "test_ceo_router.py"
Cohesion: 0.05
Nodes (60): build_ceo_router(), Any, APIRouter, backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, CEOSupervisor, _env_flag(), _env_int(), get_ceo_supervisor() (+52 more)
...
Cohesion: 0.04
Nodes (57): Feed LLM token/cost spend into the governance session budget. Kept separate…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Import the governance layer once, recording why if it is absent., Convenience: build and record an event in one call., record_event(), BudgetExceeded (+49 more)
...
Cohesion: 0.04
Nodes (52): BreakerState, Enum, str, _digest(), get_ring(), KeyRing, KeyState, packages/llm/keys.py — multi-key rotation with per-key health. A provider may… (+44 more)
...
### Community 67 - "test_brain_availability_doctor.py"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (55): main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), _is_safe_url(), _looks_like_bot_challenge(), Any, BeautifulSoup (+47 more)
...
Cohesion: 0.03
Nodes (33): Find a repository by its URL., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, Add a repository to a company. Args: company_id: Company ID url: Repository URL…, Initialize the service. Args: store: Optional CompanyGraphStore instance. If…, Get a repository by ID. (+25 more)
...
Cohesion: 0.05
Nodes (61): TaskResult, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, Available iff config resolves AND the SDK is importable. Never raises — a…, e2b_status(), Return the E2B sandbox integration status for the ProvidersScreen badge. Does…, e2b_enabled(), E2BConfig (+53 more)
...
Cohesion: 0.05
Nodes (35): ProviderRouter, Priority-ordered LLM provider fallback with health checks and retries., main(), probe_one(), Live probe — makes a tiny real API call to every configured provider. Usage (in…, _bedrock_api_response(), _bedrock_provider(), _mock_boto3() (+27 more)

### Community 82 - "test_agent_tool_governance.py"
Cohesion: 0.05
Nodes (65): Replace the process-wide store. Tests only., reset_approval_store(), Replace the process-wide audit log. Tests only., reset_audit_log(), Load a policy document from YAML, falling back to the default. Never raises:…, Replace the process-wide engine. Tests only., reset_policy_engine(), _drive() (+57 more)

### Community 83 - "WorkflowRun"
Cohesion: 0.06
Nodes (40): _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestPhaseSequenceError, TestWorkspaceIsolation, _extract_slices_from_plan() (+32 more)
...
Cohesion: 0.06
Nodes (55): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), _egress_policy_reason(), get_web_reach(), _load_script_module(), Any, ModuleType, agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives… (+47 more)

### Community 86 - "lifespan"
Cohesion: 0.04
Nodes (53): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled() (+45 more)

### Community 87 - "get_task_store"
Cohesion: 0.06
Nodes (65): _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody, Shared purge implementation for the admin endpoint and the boot hook., Submit a quick-note URL or instruction from the dashboard FAB., _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover() (+57 more)
...
### Community 90 - "test_model_catalog.py"
Cohesion: 0.05
...
Cohesion: 0.04
Nodes (53): _auto_disable(), _build_request(), _describe_registry(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion() (+45 more)
...
Cohesion: 0.05
Nodes (36): browse_page(), BrowserAction, BrowserSession, _not_started(), PageState, Any, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, True if browser automation is enabled and Playwright is importable. (+28 more)

### Community 109 - "loop.py"
Cohesion: 0.04
Nodes (51): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only… (+43 more)
...
Cohesion: 0.06
Nodes (59): BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), _norm(), _pick_from_records(), Any (+51 more)
...
Cohesion: 0.06
Nodes (32): AgentRole, When coder == reviewer model, swarm should log a warning., _fake_artifact(), tests/test_workflow_models.py — Unit tests for workflow/models.py., TestCheckRun, TestSlice, Execute, review, and verify a single slice. Returns the updated Slice. Raises…, Trigger verification for all applied-but-unverified slices. (+24 more)
...
Cohesion: 0.08
Nodes (50): _blocked_result(), _governance_check(), TaskResult, Build the failed TaskResult returned when policy denies a dispatch. A result…, Evaluate a runtime dispatch against policy; audit it either way. Returns…, Full record of a routing decision — audit-trail entry., Selects a runtime for the given task, executes the task (including readiness…, Execute the given task spec on the primary runtime, attempt configured fallback… (+42 more)
...
### Community 140 - "TestAdapterMetadata"
Cohesion: 0.05
...
### Community 147 - "seo_api.py"
Cohesion: 0.08
Nodes (33): list_seo_audits(), SEO / GEO / AIO Audit API Router Endpoints for the world-class SEO audit engine…, List stored audits for this company (most recent first)., Root directory under which repo fixes are allowed to operate., Run the repo-aware auto-fixer against this company's workspace checkout.…, run_seo_fixes(), _workspace_root(), FsPath (+25 more)
...
### Community 187 - "test_e2b_task_wiring.py"
Cohesion: 0.08
...
### Community 188 - "TestAuthAndTaskOwnership"
Cohesion: 0.05
...
### Community 192 - "Company"
Cohesion: 0.07
Nodes (30): Company, The core company entity - root of the Company Graph., Get a company by ID. Args: company_id: Company ID Returns: Company instance or…, Update a company. Args: company_id: Company ID **kwargs: Fields to update…, List companies with optional filtering. Args: owner_id: Filter by owner ID…, List companies with optional filtering., Create a new company., Test Company Graph Pydantic models. (+22 more)
...
Cohesion: 0.06
Nodes (31): _cached(), _check_storage_health(), cost_attribution_stats(), doctor_health(), _fast_count(), get_active_provider(), get_stats(), health() (+23 more)
...
Cohesion: 0.07
Nodes (22): CircuitBreakerOpenError, get_nim_pool(), NIMConnectionPool, Any, AsyncClient, RuntimeError, Persistent httpx.AsyncClient pool with circuit breaker and retry logic. Manages…, Get or create the shared httpx.AsyncClient. (+14 more)
...
Cohesion: 0.13
Nodes (33): _expire_stale_pending_report(), get_seo_audit(), Fetch a complete stored audit report. Side-effect: an audit whose stub is still…, Auto-fail a pending SEO audit stub that is older than…, Complete result of one audit run., SeoAuditReport, Paragraph, Store a report in the bounded in-memory registry. (+25 more)
...
Cohesion: 0.09
Nodes (39): get_approval_store(), Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient, Tests for the governance HTTP surface and the AgentRunner integration. The…, Policy is a git-reviewed file. An HTTP mutation route would make "who changed… (+31 more)
...
### Community 245 - "test_control_plane_api.py"
Cohesion: 0.06
...
### Community 247 - "get_tool_annotations"
Cohesion: 0.08
...
Cohesion: 0.09
Nodes (31): _extract_mentioned_paths(), Autonomous agent: fetch oldest open issue, generate implementation, create PR.…, Extract plausible file paths from issue text., Read existing files for codebase context (max 8000 chars total)., _read_grounding_files(), _decline(), _fetch_failure_context(), _list_target_prs() (+23 more)
...
### Community 271 - "test_unit7_catalog_propagation.py"
Cohesion: 0.06
...
### Community 309 - "agile_sprints.py"
Cohesion: 0.08
...
Cohesion: 0.14
Nodes (21): Path, agent/security_scanner.py — Security & Vulnerability Scanner Runs static…, Run all available scanners and aggregate results., Return True if *name* is on PATH., Run security scans and return structured findings. Usage:: scanner =…, SecurityFinding, SecurityScanner, _tool_available() (+13 more)
...
### Community 321 - "RuntimeHealthService"
Cohesion: 0.10
...
### Community 341 - "analyze_qualitative"
Cohesion: 0.12
...
### Community 343 - "delegate_seo_findings"
Cohesion: 0.10
Nodes (27): build_seo_roadmap(), delegate_seo_findings(), plan_seo_sprint(), BaseModel, post, Options for turning an audit's delegation plan into real agent tasks., Create real agent tasks from the audit's delegation plan. Each work package…, Options for building a Now/Next/Later roadmap from SEO findings. (+19 more)
...
### Community 351 - "reset_failover_manager"
Cohesion: 0.08
...
### Community 366 - "brain_failover.py"
Cohesion: 0.12
...
### Community 384 - "test_feature_matrix.py"
Cohesion: 0.08
Nodes (11): tests/test_feature_matrix.py — Feature maturity / support matrix tests. Covers:…, `policies_governance` may be STABLE only while enforcement is real. The same…, Each documented ceiling must fire from its own counter alone., LLM cost/tokens and spawn depth must be chargeable onto the budget — the wiring…, The runtime-dispatch seam must consult the same gate budget, so work handed to…, `policy_authoring_ui` may be STABLE only while safe in-product authoring is…, The load-bearing safety check: dropping a baseline guardrail is refused., The router must expose the propose route the STABLE claim depends on. (+3 more)
...
### Community 391 - "TestStreamableHTTPTransport"
Cohesion: 0.13
...
Cohesion: 0.13
Nodes (23): openclaw_websocket(), websocket, WebSocket endpoint for iOS / mobile web UI pairing + command routing., get_failover_manager(), Return the singleton BrainFailoverManager., _cmd_chat(), _cmd_freebuff(), _cmd_list_files() (+15 more)
...
### Community 404 - "schedules/api.py"
Cohesion: 0.13
...
### Community 420 - "TestModelRegistryUpdates"
Cohesion: 0.14
...
### Community 434 - "classify_direct_chat_intent"
Cohesion: 0.13
...
### Community 442 - "test_phase4_runtime_resilience.py"
Cohesion: 0.15
...
### Community 443 - "services/seo_audit.py"
Cohesion: 0.12
Nodes (15): _count_syllables(), estimate_pixel_width(), flesch_reading_ease(), _host_key(), is_internal(), normalize_url(), services/seo_audit.py - World-Class SEO / GEO / AIO Audit Engine Screaming…, Approximate SERP rendering width of ``text`` in pixels. (+7 more)
...
### Community 449 - "create_wiki_page"
Cohesion: 0.14
...
### Community 452 - "render_router.py"
Cohesion: 0.12
...
### Community 478 - "is_anthropic_base_url"
Cohesion: 0.12
...
### Community 479 - "ClaudeCodeAdapter"
Cohesion: 0.19
...
### Community 483 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 510 - "FeatureUnavailableError"
Cohesion: 0.13
...
### Community 516 - "test_issue_triage.py"
Cohesion: 0.16
...
### Community 545 - "knowledge_sync.py"
Cohesion: 0.21
...
### Community 585 - "RepoScanner"
Cohesion: 0.17
Nodes (10): Result of a repository scan with detected stack and systems., RepoScanResult, Scanner for detecting technology stack from Git repositories. Capabilities: -…, Initialize the repository scanner. Args: company_id: Optional company ID for…, Scan a Git repository and detect its technology stack. Args: repo_url: URL of…, Detect the Git provider from a repository URL. Args: repo_url: Repository URL…, Scan a GitHub repository. Args: repo_url: GitHub repository URL scan_id: Scan…, Infer stack from repository URL (basic detection). Args: repo_url: Repository… (+2 more)
...
### Community 590 - "test_telegram_auto_approve.py"
Cohesion: 0.21
...
### Community 635 - "test_task_service_failed_comment.py"
Cohesion: 0.21
...
### Community 638 - "test_ai_insights.py"
Cohesion: 0.20
...
### Community 667 - "TestMCPToolsListCache"
Cohesion: 0.19
...
### Community 699 - "verify_service_token"
Cohesion: 0.22
...
### Community 708 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 709 - "TestRuntimeControl"
Cohesion: 0.15
...
### Community 710 - "test_omniroute_provider.py"
Cohesion: 0.15
...
### Community 732 - "Workspace Isolation Architecture"
Cohesion: 0.17
...
### Community 750 - "TestAuthAndTaskCreation"
Cohesion: 0.17
...
Cohesion: 0.17
Nodes (4): Tests for agents/portfolio_intelligence.py — autonomous signal → initiative.…, DEFAULT_REPO was hardcoded to the stale pre-rename repo name…, TestDefaultRepoFollowUpFix, TestGithubAndResearch
...
Cohesion: 0.20
Nodes (8): get_persistent_queue(), _LocalBucket, packages/llm/distributed.py — cross-instance coordination. Two facilities that…, The process-wide persistent queue., Test hook — drop both singletons., In-process token bucket — the fallback when Redis is absent., _redis_url(), reset()

### Community 790 - "DistributedRateLimiter"
Cohesion: 0.22
Nodes (4): DistributedRateLimiter, True once a Redis connection has actually been established., Consume capacity, waiting up to ``max_wait_sec``. Returns False when capacity…, Token bucket shared across instances when Redis is available.
...
### Community 793 - "migrate_local_brain_env.py"
Cohesion: 0.25
...
### Community 795 - "test_daily_digest.py"
Cohesion: 0.24
...
### Community 796 - "test_agent_readiness_audit.py"
Cohesion: 0.18
...
### Community 799 - "_FakeCommandResult"
Cohesion: 0.22
...
### Community 817 - "Agent Readiness Report"
Cohesion: 0.20
...
### Community 828 - "local_brain_provider_config"
Cohesion: 0.29
...
### Community 840 - "_FakePersistence"
Cohesion: 0.20
...
### Community 851 - "openclaw_reverse_proxy"
Cohesion: 0.22
...
### Community 869 - "_get_current_user"
Cohesion: 0.25
...
### Community 891 - "_brain_provider_status"
Cohesion: 0.25
...
### Community 941 - "Continual Harness (`agent/harness_spec.py`)"
Cohesion: 0.25
...
### Community 1015 - "TestSupportMatrixDocsSync"
Cohesion: 0.29
...
### Community 1016 - "test_doctor_coding_brain.py"
Cohesion: 0.38
...
### Community 1018 - "test_hermes_server.py"
Cohesion: 0.29
Nodes (3): tests/test_hermes_server.py — the agency's OWN Hermes runtime server.…, Regression: the Hermes runtime was 100% broken in orchestrator mode. The…, test_tasks_sets_orchestrator_bypass_across_http_hop()
...
### Community 1023 - ".run_harness_audit"
Cohesion: 0.33
Nodes (4): _now(), Any, Run a cross-harness security audit. Checks that the agent harness configuration…, Return current UTC timestamp as ISO string.
...
### Community 1025 - "brain_providers"
Cohesion: 0.33
...
### Community 1066 - "TestNoHardcodedModels"
Cohesion: 0.33
...
### Community 1084 - "Who is this for?"
Cohesion: 0.40
...
### Community 1115 - "TestDirectChatNonBlocking"
Cohesion: 0.40
...
### Community 1131 - "Privacy, security, and cost"
Cohesion: 0.50
...
### Community 1135 - "pytest_collection_modifyitems"
Cohesion: 0.50
...
### Community 1157 - "Configuration reference"
Cohesion: 0.67
...
- **3530 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3525 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13468 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **114 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `parse_event_stream`, `test_operational_incidents.py`, `TaskStatus`, `Task`, `test_brain_patch_service_token.py`, `test_llm_router_queue_cache.py`, `test_gh_brain_failover.py`, `config.py`, `test_llm_router_strategies.py`, `test_brain_config_store.py`, `test_model_router.py`, `AgentJobManager`, `SelfHealingAgent`, `TrafficDirector`, `AgentDefinition`, `resolve_component_model`, `test_company_api.py`, `Agency`, `test_telegram_webhook.py`, `direct_chat.py`, `E2BSandboxSession`, `SeoAuditEngine`, `get_workflow_orchestrator`, `test_failover_client_shared.py`, `NotificationDispatcher`, `brain_config.py`, `tasks/api.py`, `test_unit8_model_catalog.py`, `PolicyEngine`, `models/company_graph.py`, `services/background.py`, `PrimeAgentAdapter`, `_start_autonomy_loops`, `test_ceo_router.py`, `test_openclaw_endpoints.py`, `enforcement.py`, `test_sam_livekit.py`, `test_llm_router_resilience.py`, `validate_policy_text`, `test_brain_availability_doctor.py`, `ChatHistoryStore`, `SQLiteStore`, `test_telegram_auto_approve.py`, `TokenBudget`, `e2e/test_browser.py`, `ProviderRouter`, `test_agent_tool_governance.py`, `test_autonomy_status.py`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `ResearchTask`, `test_tasks_reconciler_todo_requeue.py`, `test_procedural_memory.py`, `tools.py`, `test_llm_router_e2e.py`, `claim`, `KeyStore`, `loop.py`, `OutputFilter`, `clear_cooldowns`, `KeyPool`, `UserRole`, `sam`, `test_task_service_failed_comment.py`, `ArtifactStore`, `BackgroundAgent`, `test_new_features_e2e.py`, `TaskPriority`, `test_context_rulebook.py`, `test_trend_scoping.py`, `test_provider_router.py`, `seo_api.py`, `_step`, `run_task`, `test_changelog_check_workflow.py`, `test_trend_watcher.py`, `TestHarnessAdapter`, `BudgetTracker`, `test_openclaw_gateway.py`, `MCPClient`, `HarnessRegistry`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `FeatureMaturity`, `test_e2b_data_flow.py`, `test_process_quick_note_workflow.py`, `AgentSwarm`, `OllamaCircuitBreaker`, `_Budget`, `_job_text`, `TestAuthAndTaskOwnership`, `Company`, `SchedulerStore`, `test_doctor_service_token_check.py`, `TestRuntimeControl`, `test_features_api.py`, `test_video_transcript.py`, `test_dashboard_cache.py`, `nvidia_models.py`, `TrendWatcher`, `AgentProfile`, `test_portfolio_intake.py`, `test_llm_router_disabled.py`, `test_agile_api.py`, `test_app_settings.py`, `TestClient`, `TestAuthAndTaskCreation`, `test_task_clarification.py`, `ContextPruner`, `test_control_plane_api.py`, `test_persistent_memory.py`, `_resolve_user_github_token`, `test_platform_controls.py`, `test_sam_voice.py`, `test_nvidia_model_discovery.py`, `session_retro.py`, `analyze_page`, `rate_limiter.py`, `SecurityScanner`, `_resolve_push_token`, `test_catalogue_probe.py`, `OrchestratorQueue`, `TestMongoGate`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_skill_registry_boot_refresh.py`, `reset_failover_manager`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `FailoverResult`, `test_provider_enable_disable.py`, `test_webui_provider_priority.py`, `_undeclared`, `test_service_token.py`, `test_harness_spec.py`, `test_agency_fix.py`, `_Recorder`, `api_keys_for`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `model_discovery.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_telegram_diag_endpoint.py`, `test_knowledge_sync.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `test_provider_state_durability.py`, `UsageEvent`, `ClaudeCodeAdapter`, `test_kimi_bridge_server.py`, `TestNormalizeResponseFormat`, `test_voice_pipeline.py`, `client`, `clear_override`, `cost_tracker.py`, `test_doctor_coding_brain.py`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `TaskSpec`, `proxy.py`, `BackgroundAgent`, `FreeBuffAgent`, `test_ceo_dispatcher.py`, `UserMemoryStore`, `AgentJobManager`, `ContextManager`, `ExecutionRequest`, `ReactScratchpad`, `Agency`, `direct_chat.py`, `E2BSandboxSession`, `TestAgentRunnerSafety`, `test_empirical_verify.py`, `MCPClient`, `StuckDetector`, `CEODispatcher`, `MultiAgentSwarm`, `AgentSessionStore`, `TestAgentRunnerExecution`, `test_failover_client_shared.py`, `PolicyEngine`, `test_daily_automation_2026_07_11.py`, `WorkspaceTools`, `enforcement.py`, `TestAgentLoopMCPIntegration`, `LocalWorkspace`, `TokenBudget`, `call_llm`, `resolve_e2b_config`, `test_agent_tool_governance.py`, `reset_failover_manager`, `test_governance_api.py`, `InternalAgentAdapter`, `loop.py`, `GitHubTools`, `AdaptiveHalter`, `ContextPruner`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `backend/server.py`, `test_all_providers_discovery.py`, `test_provider_router.py`, `implement_agent.py`, `test_bedrock_live.py`, `TrafficDirector`, `settings.py`, `direct_chat.py`, `kimi_bridge_provider_config`, `ProviderConfig`, `TestFromEnvBedrock`, `TestLegacyRouterCacheTTL`, `test_anthropic_refusal_fallback.py`, `test_colibri_brain_shim.py`, `TestRouterIntegration`, `call_llm`, `_payload`, `TestLegacyRouterServerFallback`, `test_chat_mode_regressions.py`, `is_anthropic_base_url`, `failover_client.py`, `TestAnthropicPayloadStructuredOutput`, `clear_cooldowns`?**
