...
## Corpus Check
- 1500 files · ~2,172,822 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30350 nodes · 56222 edges · 1281 communities (1111 shown, 115 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2734 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5369901f`
...
## Communities (1281 total, 115 thin omitted)

### Community 1 - "LLMRequest"
Cohesion: 0.02
Nodes (162): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic… (+154 more)

### Community 2 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (101): BusinessCategory, CompanyGraph, SystemType, Get the index of a phase in the workflow., The complete Company Graph - canonical core model for the Autonomous AI Agency.…, Find a website by its URL., Find a repository by its URL., Find all systems of a specific type. (+93 more)
...
Cohesion: 0.02
Nodes (184): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, check_ollama_async(), Async Ollama reachability check — non-blocking for FastAPI handlers., middleware, admin_control(), admin_create_key(), admin_create_user() (+176 more)

### Community 5 - "SQLiteStore"
Cohesion: 0.02
Nodes (93): CompanyGraphSnapshot, KnowledgeItem, A point-in-time snapshot of a Company Graph for history and rollback., Structured knowledge about the company, systems, or processes., MongoDBStore, Any, Company, ObjectId (+85 more)
...
Cohesion: 0.04
Nodes (89): AgentPhaseError, AgentRunner, Exception, Send chat messages and return the assistant's text output. If an…, Lazy accessor for the SteerLM steering injector (B2)., Return True when no file is touched by more than one step., Raised when a named agent phase (planning, verification, etc.) fails., GATE: Golden Path steps #7-12 — the primary agent execution loop. This is the… (+81 more)

### Community 8 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (102): CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any, T, Live (unexpired) entries — used by the semantic layer's scan. (+94 more)

### Community 9 - "TaskSpec"
Cohesion: 0.03
Nodes (103): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag. (+95 more)
...
Cohesion: 0.04
Nodes (85): AgentRole, _parse_ceo_directives(), str, DetectedIssue, get_improvement_loop(), ImprovementLoop, ImprovementLoopState, IssueCategory (+77 more)

### Community 12 - "TaskStore"
Cohesion: 0.02
Nodes (140): AgentDefinition, AgentStore, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents., Return all agents in the system (admin use only). (+132 more)

### Community 13 - "test_unit8_model_catalog.py"
Cohesion: 0.03
Nodes (79): _brain_provider_status(), get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, Return per-provider metadata for the GET endpoint. Iterates every provider in…, all_provider_ids(), _provider_ids_from_literal(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the… (+71 more)
...
Cohesion: 0.06
Nodes (16): Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry, Any, Admin-configurable routing policy. Defaults are conservative (local-first, no…, RoutingPolicy, TaskResult (+8 more)
...
Cohesion: 0.05
Nodes (65): MCPUnavailableError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Raised when the MCP server is unreachable or the circuit is open., packages/integrations/render_mcp.py — Render platform access over MCP. The…, Drop the cached singleton. Used by tests and after a config change., reset_render_mcp(), E2BSandboxSession, _inject_token() (+57 more)
...
Cohesion: 0.03
Nodes (89): _merge_changed_files(), Collect changed_files across all specialists into a single de-duped list., get_ceo_fallback_stats(), PlanOutput, PLAN phase: structured execution plan., SELECT_SPECIALIST phase: which specialist(s) handle this work., Increment the named counter (silently ignores unknown reasons)., Return a snapshot of the CEO fallback counters. (+81 more)

### Community 26 - "brain_config.py"
Cohesion: 0.03
Nodes (108): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store() (+100 more)
...
Cohesion: 0.03
Nodes (91): loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Condense a tool result down to what belongs in an audit row. A tool result can…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, Feed LLM token/cost spend into the governance session budget. Kept separate…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _summarise_tool_result(), _egress_policy_reason() (+83 more)

### Community 29 - "enforcement.py"
Cohesion: 0.06
Nodes (40): BudgetExceeded, classify(), evaluate_call(), guard_tool_call(), GuardResult, _host_of(), _is_write_tool(), _more_restrictive() (+32 more)

### Community 30 - "Usage"
Cohesion: 0.06
Nodes (38): AlertHandler, BudgetTracker, Counter, _Dimensions, get_budget(), _month(), Any, packages/llm/budget.py — token and cost accounting with spend alerts. Tracks… (+30 more)
...
### Community 35 - "Task"
Cohesion: 0.05
Nodes (61): Full task/issue document., Task, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, _coordinator(), asyncio, BaseException, Brain-availability preflight tests (graceful degradation). When NO LLM brain is… (+53 more)

### Community 36 - "Specialist"
Cohesion: 0.03
...
### Community 37 - "_ts_to_float"
Cohesion: 0.05
Nodes (46): _priority_rank(), Any, Task, TaskStatus, Fail loud in production if no persistent storage is configured. Telegram…, Create a task. Deduplicates by source_id if set (Charter G3). If a task with…, Fetch a task by ID. If owner_id is set, enforces ownership., Return the task previously created for an external ``source_id`` (e.g.… (+38 more)

### Community 38 - "test_failover_client_shared.py"
Cohesion: 0.06
Nodes (89): BrainFailoverExhausted, failover_chat_completion(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, Run one chat completion across the brain-failover chain. Tries each healthy…, _free_tier(), _hit_ids(), _many_providers() (+81 more)

### Community 39 - "LLMRouter"
Cohesion: 0.06
Nodes (33): Attempt, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any, AsyncClient, BaseException, ProviderConfig (+25 more)

### Community 40 - "services/seo_audit.py"
Cohesion: 0.05
Nodes (56): list_seo_audits(), List stored audits for this company (most recent first)., BaseModel, field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Aggregated report row - Screaming Frog CSV compatible. (+48 more)

### Community 41 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (83): BudgetTracker, GovernanceGate, Per-session consumption counters and their ceilings. Ceilings come from the…, Return the name of the first exhausted session-wide limit, or None. Only covers…, Return a reason string if *tool* has hit its per-tool session cap. Checks only…, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., SessionBudget (+75 more)

### Community 42 - "rbac.py"
Cohesion: 0.05
...
Cohesion: 0.04
Nodes (81): One tool selection parsed from the executor model's JSON response., ToolCall, Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, Any, packages/governance/audit.py — the evidence trail for every governed action.… (+73 more)
...
Cohesion: 0.06
Nodes (41): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+33 more)

### Community 48 - "validate_policy_text"
Cohesion: 0.05
Nodes (38): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), _baseline_loosened(), _baseline_patterns() (+30 more)
...
Cohesion: 0.03
Nodes (73): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,… (+65 more)
...
Cohesion: 0.05
Nodes (43): _build_builtin_model_map(), _default_model(), _default_reasoning_model(), _get_model_map(), _nvidia_key_present(), Dynamic model router. Central routing logic for all chat and agent requests.…, Build the built-in alias table — Nvidia NIM models when key is set, local…, Merge built-in defaults with MODEL_MAP env overrides (lazy, cached). (+35 more)
...
Cohesion: 0.05
Nodes (56): _bullets(), generate_backlog_retro(), generate_standup(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive a retrospective from the task tracker when no sprint is active. DONE /…, Render the sprint plan as markdown., Render a list of strings as markdown bullets (or a placeholder). (+48 more)
...
Cohesion: 0.06
Nodes (26): _keepalive_self_ping(), lifespan(), _maybe_boot_purge(), _purge_summary_clean(), Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free…, Start the Telegram bot (and keep-alive) inside the web process when enabled.…, #522 + #505: Reliability startup — schedule hydration, orchestrator restore,… (+18 more)

### Community 71 - "ProviderConfig"
Cohesion: 0.04
Nodes (81): _acquire_provider_probe(), extract_openai_text(), is_commercial_provider(), _normalize_nvidia_base_url(), _openai_url(), provider_access_tier(), _provider_field(), provider_sort_key() (+73 more)
...
### Community 96 - "test_e2b_data_flow.py"
Cohesion: 0.07
...
### Community 101 - "types.py"
Cohesion: 0.05
Nodes (63): _load(), main(), ModuleType, _write_summary(), _load(), main(), int, failover_chat_completion_via_router() (+55 more)

### Community 102 - "_start_autonomy_loops"
Cohesion: 0.06
Nodes (58): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+50 more)
...
Cohesion: 0.07
Nodes (24): CommercialFallbackRequiredError, _get_director(), _normalized_provider_type(), ProviderRouter, Any, RuntimeError, Try all models for one provider. Returns ProviderResult on success, None on…, Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either… (+16 more)
...
Cohesion: 0.05
Nodes (69): quick_notes_submit(), _QuickNoteBody, The task store the background services should use, wiring it if needed.…, Submit a quick-note URL or instruction from the dashboard FAB., _task_store_for_background(), Reset the singleton (for tests)., reset_failover_manager(), Hand the 'connect & verify the repo' work to the agency's own agents. The task… (+61 more)
...
Cohesion: 0.07
Nodes (22): _append_agent_session_message(), _in_container(), _mask_observations(), Path, ProviderConfig, Truncate tool/observation content in older messages to prevent context bloat., Return True when running inside a container runtime. ``/.dockerenv`` alone…, Swap localhost → `ollama` when running inside a container. Inside a container,… (+14 more)
...
Cohesion: 0.05
Nodes (52): clear_cooldowns(), get_dead_models(), _is_model_dead(), is_provider_on_cooldown(), mark_provider_failed(), ProviderFallbackError, Snapshot of active dead-model entries {provider_id/model: expiry_ts}. Also…, Put provider_id on cooldown for *cooldown_seconds* (default:… (+44 more)
...
Cohesion: 0.05
Nodes (57): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), plan_seo_sprint(), BaseModel, get, post (+49 more)
...
Cohesion: 0.06
Nodes (59): _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestWorkspaceIsolation, approve(), build() (+51 more)
...
Cohesion: 0.05
Nodes (33): __init__.py — Feature flag/matrix package., FeatureMaturity, get_feature_matrix(), Enum, str, features/matrix.py — Feature maturity tiers and support matrix. Single source…, Feature maturity classification., Return the global FeatureMatrix singleton. (+25 more)
...
Cohesion: 0.03
Nodes (63): _escape_md_v1(), NotificationDispatcher, Any, Path, service_manager.py — Telegram & Notification Integration Extension Extends the…, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess). (+55 more)
...
Cohesion: 0.04
Nodes (53): Register the built-in agent tools that are always available., _register_builtin_tools(), repowise.py — RepowiseIntelligence: context packing and dependency analysis., Path, tools.py — WorkspaceTools: read/write/search and diff application (risky…, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Return the first *lines* lines of a file. Just-in-time retrieval: the executor…, Return a lightweight index of files with line counts and sizes. This is the… (+45 more)
...
Cohesion: 0.06
Nodes (46): add_initiative(), AllocationOut, BoardOut, get_board(), get_service(), InitiativeIn, InitiativeOut, _materialize_and_log() (+38 more)
...
### Community 210 - "test_autonomous_agency_e2e.py"
Cohesion: 0.17
Nodes (9): FakeTask, End-to-end tests for the autonomous AI agency system (issue #467). These tests…, Tests for Telegram notification dispatch., Verify NotificationDispatcher.on_task_complete dispatches notifications., Tests for CEO-driven agency system., Verify CEO agent system is implemented., Verify agency cycle GitHub Actions workflow exists., TestCEOAgencySystem (+1 more)

### Community 211 - "CEOLedger"
Cohesion: 0.07
...
Cohesion: 0.06
Nodes (54): CircuitState, Return the last-known health for *runtime_id* (may be stale)., Return True if the runtime is available (not circuit-open)., Return health snapshots for all known runtimes., Force an immediate health check of all runtimes and return results., Attempt to start a dead runtime subprocess before re-probing. Uses the local…, Reduce probe frequency for runtimes that have never come online., Async health polling service for all registered runtimes. (+46 more)
...
Cohesion: 0.07
Nodes (35): AgentRole, Run a pre-gate or report phase through the correct agent. Enforces permission…, Execute a slice via the Coder agent (write-permitted)., Review a slice via the Reviewer agent (different model from Coder). This is the…, Run verification commands via the Verifier agent. The Verifier is execution-…, Return a copy of *routing* with the profile's model injected for its role., When coder == reviewer model, swarm should log a warning., _fake_artifact() (+27 more)
...
### Community 239 - "tasks/models.py"
Cohesion: 0.06
Nodes (55): tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, Enum, str, tasks/models.py — Pydantic models for the task/issue system., Comment or reply on a task., Human approval gate in a task's execution., TaskComment (+47 more)
...
### Community 243 - "operational_incidents.py"
Cohesion: 0.11
...
Cohesion: 0.06
Nodes (53): get_tracker(), The process-wide health tracker., get_metrics(), The process-wide metrics registry., _ok(), parametrize, End-to-end routing against mock providers (ADR-008). These are the tests that…, A router wired to three mock providers, with all singletons isolated. (+45 more)
...
### Community 266 - "open_phase_report"
Cohesion: 0.10
...
### Community 273 - "TestStreamableHTTPTransport"
Cohesion: 0.13
...
### Community 294 - "self_heal_brain_and_unblock_tasks"
Cohesion: 0.09
...
### Community 306 - "test_session_retro.py"
Cohesion: 0.16
Nodes (20): cluster_friction(), collect_friction_events(), FrictionCluster, FrictionEvent, judge_cluster(), Return a human-readable description of the cluster. Uses *judge_fn* (an LLM-as-…, Scan the most recent *lookback* sessions for friction-signal events., Group friction events by signature, most frequent first. (+12 more)
...
### Community 321 - "get_store"
Cohesion: 0.09
Nodes (32): _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Consolidated system health report: preflight checks + runtime health. Returns a…, Public Doctor endpoint — no authentication required. Returns system-level…, Authenticated Doctor endpoint — full diagnostics. Returns all system-level… (+24 more)
...
### Community 326 - "_build_default_manager"
Cohesion: 0.08
...
Cohesion: 0.08
Nodes (25): _decline(), _extract_pytest_failure(), _fetch_failure_context(), _is_denied_path(), _list_target_prs(), main(), _post_comment(), _pr_head() (+17 more)

### Community 335 - "provider_max_rpm"
Cohesion: 0.07
...
### Community 355 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.09
...
### Community 357 - "services/background.py"
Cohesion: 0.05
Nodes (43): _await_hermes_ready(), BackgroundServices, _env_on(), _hydrate_scheduler_bounded(), Any, Path, services/background.py — shared background-service startup. Both the FastAPI…, Await the refresh and log the outcome; swallow every failure. (+35 more)
...
### Community 376 - "PerformanceAnalytics"
Cohesion: 0.12
...
### Community 384 - "record_event"
Cohesion: 0.08
Nodes (35): guard(), identity_from_headers(), _load(), Any, mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Build an AgentIdentity from the caller's ``X-Agent-*`` headers. Absent headers…, Evaluate *tool* before it runs. Returns ``(allowed, message, decision)``.…, Write the audit row for a completed (or blocked) MCP tool call. (+27 more)
...
### Community 397 - "_mock_provider_records"
Cohesion: 0.14
...
### Community 410 - "cowork_session.py"
Cohesion: 0.15
...
### Community 427 - "DailyDigestAggregatorTests"
Cohesion: 0.19
...
### Community 444 - "test_ai_insights.py"
Cohesion: 0.20
...
### Community 448 - "_self_heal_ready"
Cohesion: 0.15
...
### Community 459 - "TestMCPToolsListCache"
Cohesion: 0.19
...
### Community 468 - "TestAgentRunnerPublicPlanMethod"
Cohesion: 0.22
...
### Community 523 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 525 - "_is_ephemeral_user"
Cohesion: 0.27
...
### Community 530 - "test_brain_priority_scanner.py"
Cohesion: 0.10
...
### Community 560 - "redact_connection_url"
Cohesion: 0.17
...
### Community 561 - "ai/router.py"
Cohesion: 0.06
Nodes (41): Record token usage for *model* (fire-and-forget, never raises). ``tag`` is a…, record_usage(), _dead_model_key(), _exponential_backoff_cooldown(), _mark_model_dead(), _notify_watchdog(), provider_router.py — auto-generated module docstring (user-research skill scan)., Notify the brain watchdog of a provider call outcome (fire-and-forget).… (+33 more)
...
### Community 592 - "TestAuthAndTaskCreation"
Cohesion: 0.17
...
Cohesion: 0.04
Nodes (36): _env_float(), HermesAdapter, Any, AsyncClient, TaskResult, TaskSpec, Submit task to Hermes via its /tasks endpoint., Read a float env var, falling back to *default* on unset/garbage. (+28 more)
...
### Community 627 - "DirectChatSession"
Cohesion: 0.12
...
### Community 639 - "run"
Cohesion: 0.19
Nodes (14): CreateIssue, build_body(), _failures(), find_tracking_issue(), _gh_ops(), _is_retired(), main(), Return the existing open tracking issue, if any (marker match wins). (+6 more)

### Community 640 - "ModelConfig"
Cohesion: 0.06
Nodes (26): AgentPolicy, LLMConfig, ModelConfig, Capability and pricing metadata for one model id., Per-agent routing overrides (ADR-008 §"named profiles")., The fully-resolved routing configuration., get_registry(), ModelRegistry (+18 more)
...
### Community 646 - "test_event_log.py"
Cohesion: 0.45
...
### Community 670 - "test_tasks_reconciler_todo_requeue.py"
Cohesion: 0.18
...
### Community 673 - "mint_access_token"
Cohesion: 0.18
...
### Community 681 - "test_webui_provider_priority.py"
Cohesion: 0.12
Nodes (34): AdminIdentity, _bootstrap(), Path, ProviderManager, WorkspaceManager, tests/test_webui_provider_priority.py — Priority + reorder + brain-policy…, The /policy/brain endpoint must return the resolved brain + the paid policy…, The /providers/role-tags endpoint surfaces brain/sub/fallback roles consistent… (+26 more)
...
Cohesion: 0.02
Nodes (237): _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills() (+229 more)
...
### Community 728 - "test_provider_state_durability.py"
Cohesion: 0.05
...
Cohesion: 0.02
Nodes (178): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+170 more)
...
### Community 763 - "HTTPException"
Cohesion: 0.03
Nodes (116): _get_current_user_thunk(), _get_optional_user_thunk(), Request, admin_seed(), clear_cost_attribution(), clear_response_cache(), create_access_token(), create_github_pr() (+108 more)
...
### Community 773 - "flesch_reading_ease"
Cohesion: 0.20
...
### Community 807 - "ExecutionRequest"
Cohesion: 0.06
...
### Community 810 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 813 - "agent/output_filter.py"
Cohesion: 0.33
...
### Community 820 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 828 - "test_control_plane_api.py"
Cohesion: 0.06
...
### Community 857 - "TestWorkflowIntegration"
Cohesion: 0.33
...
### Community 889 - "TestNoHardcodedModels"
Cohesion: 0.33
...
### Community 893 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 907 - "Agent API Issues"
Cohesion: 0.40
...
### Community 920 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 1017 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1026 - "Langfuse Issues"
Cohesion: 0.50
...
### Community 1033 - "_clean_director"
Cohesion: 0.50
...
### Community 1035 - "test_skills_route_order.py"
Cohesion: 0.67
...
- **3505 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3500 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13374 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **115 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `TestTheWorkflowIsSafeAndReadOnly`, `test_doctor_coding_brain.py`, `test_brain_patch_service_token.py`, `test_llm_router_queue_cache.py`, `_clean_director`, `test_llm_router_strategies.py`, `ImprovementLoop`, `test_doctor_service_token_check.py`, `TaskStore`, `test_model_router.py`, `test_unit8_model_catalog.py`, `config.py`, `SelfHealingAgent`, `WorkflowRun`, `test_llm_router_resilience.py`, `loop.py`, `enforcement.py`, `Usage`, `test_company_api.py`, `test_langfuse_agency_wide.py`, `Task`, `test_failover_client_shared.py`, `services/seo_audit.py`, `UserRole`, `test_agent_tool_governance.py`, `validate_policy_text`, `test_knowledge_sync.py`, `test_catalogue_probe.py`, `BrainWatchdog`, `AgentJobManager`, `PrimeAgentAdapter`, `ChatHistoryStore`, `Agency`, `test_openclaw_endpoints.py`, `AgentSwarm`, `test_task_service_failed_comment.py`, `ResearchTask`, `ProviderConfig`, `TestClient`, `test_procedural_memory.py`, `TestAuthAndTaskCreation`, `SQLiteStore`, `test_startup_warmup.py`, `test_telegram_auto_approve.py`, `_resolve_push_token`, `test_autonomy_status.py`, `test_brain_failover_413.py`, `test_gh_brain_failover.py`, `SeoFixer`, `test_local_brain_state.py`, `resolve_e2b_config`, `test_e2b_data_flow.py`, `direct_chat.py`, `test_phase5_doctor.py`, `TestHarnessAdapter`, `test_refresh_agent_built_proof.py`, `_start_autonomy_loops`, `cost_tracker.py`, `test_sam_voice.py`, `test_context_rulebook.py`, `ArtifactStore`, `KeyStore`, `run_task`, `e2e/test_browser.py`, `_step`, `llm/router.py`, `BackgroundAgent`, `FreeBuffAgent`, `test_issue_intake.py`, `test_trend_watcher.py`, `emit_chat_observation`, `clear_cooldowns`, `claim`, `TestAuthAndTaskOwnership`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `test_tasks_reconciler_todo_requeue.py`, `MCPClient`, `test_daily_2026_07_27.py`, `FeatureMaturity`, `._check_permissions`, `test_openclaw_gateway.py`, `KeyPool`, `test_telegram_webhook.py`, `NotificationDispatcher`, `TestSwarmRoleRouting`, `OllamaCircuitBreaker`, `_job_text`, `TestRuntimeControl`, `WorkspaceTools`, `test_new_features_e2e.py`, `test_features_api.py`, `test_video_transcript.py`, `TrendWatcher`, `test_llm_router_tpm.py`, `_Budget`, `.record`, `test_process_quick_note_workflow.py`, `test_portfolio_intake.py`, `CEOLedger`, `test_provider_state_durability.py`, `test_skill_registry_boot_refresh.py`, `TestClient`, `timedelta`, `company_api.py`, `tasks/models.py`, `ContextPruner`, `test_ceo_router.py`, `test_brain_failover.py`, `parse_event_stream`, `test_agile_api.py`, `test_app_settings.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `UsageEvent`, `test_task_clarification.py`, `OutputFilter`, `test_platform_controls.py`, `test_rate_limiter.py`, `OrchestratorQueue`, `TestNormalizeResponseFormat`, `test_session_retro.py`, `test_purge_backlog.py`, `test_colibri_brain_shim.py`, `analyze_page`, `test_control_plane_api.py`, `get_store`, `TestMongoGate`, `test_telegram_task_callback.py`, `capture_screens.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_nvidia_model_discovery.py`, `_FakeSandbox`, `isolated_telegram_config`, `services/background.py`, `test_telegram_approval_e2e.py`, `test_provider_enable_disable.py`, `_undeclared`, `test_kimi_bridge_server.py`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_harness_spec.py`, `model_discovery.py`, `_mock_provider_records`, `test_regression.py`, `test_agency_fix.py`, `get_skill_bindings`, `test_dashboard_cache.py`, `_get`, `test_monitor_lib.py`, `test_v4_api.py`, `test_telegram_diag_endpoint.py`, `_Recorder`, `_clean_phases`, `test_ping.py`, `nvidia_models.py`, `test_runtimes_health_endpoint.py`, `SchedulerStore`, `_Recorder`, `test_task_store_fails_loud_in_production.py`, `test_voice_pipeline.py`, `client`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `proxy.py`, `ReactScratchpad`, `TaskSpec`, `BackgroundAgent`, `FreeBuffAgent`, `Any`, `ContextManager`, `test_backend_server_features.py`, `WorkflowRun`, `E2BSandboxSession`, `loop.py`, `MCPClient`, `CEODispatcher`, `test_failover_client_shared.py`, `test_agent_tool_governance.py`, `StuckDetector`, `AgentSessionStore`, `test_empirical_verify.py`, `LocalWorkspace`, `MultiAgentSwarm`, `AgentJobManager`, `test_daily_automation_2026_07_11.py`, `WorkspaceTools`, `Agency`, `test_agent_free_brain.py`, `backend/server.py`, `workflow_orchestrator.py`, `test_agent_chat_integration.py`, `test_autonomous_agency_e2e.py`, `TestAgentRunnerSafety`, `TestAgentRunnerPublicPlanMethod`, `TestAgentLoopMCPIntegration`, `TokenBudget`, `UserMemoryStore`, `.execute`, `_resolve_push_token`, `TestAgentRunnerExecution`, `test_brain_failover_413.py`, `test_governance_api.py`, `direct_chat.py`, `test_e2b_adapter.py`, `TestGetCurrentTimeTool`, `AdaptiveHalter`, `ContextPruner`, `GitHubTools`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `test_all_providers_discovery.py`, `_payload`, `implement_agent.py`, `test_chat_mode_regressions.py`, `clear_cooldowns`, `failover_client.py`, `test_bedrock_live.py`, `TestOpenAiToBedrockConverse`, `TestRouterIntegration`, `settings.py`, `ai/router.py`, `test_colibri_brain_shim.py`, `TestLegacyRouterCacheTTL`, `is_anthropic_base_url`, `test_anthropic_refusal_fallback.py`, `ProviderConfig`, `backend/server.py`, `test_bedrock_provider.py`, `TestAnthropicPayloadStructuredOutput`, `direct_chat.py`, `system_instruction`, `TrafficDirector`?**
