...
## Summary
- 30500 nodes · 56946 edges · 1268 communities (1108 shown, 112 thin omitted)
...
## Communities (1268 total, 112 thin omitted)
...
### Community 2 - "Task"
Cohesion: 0.02
Nodes (142): classify_domain(), Return the best-matching domain for a task title+description., AgentDefinition, A user-defined agent configuration stored in the database., autonomy_tick(), Execute ONE pending task synchronously. Called by the cron workflow every 2…, Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task() (+134 more)
...
Cohesion: 0.01
Nodes (117): parse_event_stream(), Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, Reset the singleton (for tests)., reset_failover_manager(), base_url(), mobile_page(), proxy_url(), Config (+109 more)

### Community 6 - "AgentRunner"
Cohesion: 0.02
Nodes (124): AgentPhaseError, AgentRunner, Any, Exception, Path, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a…, Send chat messages and return the assistant's text output. If an… (+116 more)
...
Cohesion: 0.02
Nodes (112): CacheManager, cosine_similarity(), payload_key(), Any, Exact-match cache key over the fields that change the answer. Routing…, Cosine similarity between two vectors, 0.0 when either is degenerate., Owns every cache layer and the policy for what may enter them., Whether a response to ``request`` may be stored. Deliberately conservative — a… (+104 more)
...
Cohesion: 0.03
Nodes (103): ApprovalRequest, ApprovalStatus, ApprovalStore, Any, Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`… (+95 more)

### Community 12 - "ImprovementLoop"
Cohesion: 0.04
Nodes (94): Agency, AgentRole, get_agency(), _parse_ceo_directives(), str, CEO-coordinated multi-agent agency for continuous codebase management. The CEO…, set_agency(), DetectedIssue (+86 more)
...
Cohesion: 0.02
Nodes (73): CompanyGraph, DetectedSystem, SystemType, The complete Company Graph - canonical core model for the Autonomous AI Agency.…, Find a website by its URL., Find all systems of a specific type., Find a workflow by its name., Find knowledge items matching any of the given tags. (+65 more)

### Community 15 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (112): _merge_changed_files(), Collect changed_files across all specialists into a single de-duped list., BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput (+104 more)

### Community 16 - "config.py"
Cohesion: 0.03
Nodes (93): Test hook — clear all usage accounting., reset(), _Entry, LRUCache, T, packages/llm/cache.py — layered caching. Five independent layers, each with its…, Live (unexpired) entries — used by the semantic layer's scan., Test hook — drop every cache layer. (+85 more)

### Community 17 - "llm/router.py"
Cohesion: 0.03
Nodes (98): int, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., get_cache(), The process-wide cache manager., failover_chat_completion_via_router(), openai_body_from_response() (+90 more)
...
Cohesion: 0.04
Nodes (84): agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Point the feature stores at the shared database connection. Deliberately…, _wire_feature_stores(), tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, TaskPriority, _priority_rank() (+76 more)

### Community 24 - "get_db"
Cohesion: 0.03
Nodes (106): auth_me(), auto_recommend_skills(), brain_failover_status(), brain_providers(), _check_storage_health(), cost_attribution_stats(), discover_remote_skills(), doctor_health() (+98 more)

### Community 25 - "AgentJobManager"
Cohesion: 0.04
Nodes (59): PreflightReport, AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs… (+51 more)
...
Cohesion: 0.04
Nodes (89): classify_task(), _extract_recent_text(), Any, Task classification from request context. Classifies an incoming request into a…, Concatenate plain text from the last *last_n* messages., Return the most likely task category for this request. Args: messages: OpenAI-…, Dynamic model router package. Public API:: from router import get_router,…, get_router() (+81 more)
...
Cohesion: 0.03
Nodes (48): BusinessCategory, Find a repository by its URL., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, requires_db, CompanyGraphService (+40 more)

### Community 34 - "services/seo_audit.py"
Cohesion: 0.05
Nodes (52): BaseModel, field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website. (+44 more)
...
Cohesion: 0.04
Nodes (67): Result of a website scan with detected systems and stack inference., WebsiteScanResult, main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), _content_contains_domain(), _hostname_contains() (+59 more)

### Community 38 - "WorkflowRun"
Cohesion: 0.04
Nodes (49): _NoopStore, Phase, Any, Enum, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs…, Push a Telegram approval-gate notification when a run pauses (Charter G1).… (+41 more)
...
Cohesion: 0.05
Nodes (37): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, RuntimeError, One deploy, normalised from whatever shape the tool returned., Raised when a mutating Render tool is called with writes disabled., RenderDeploy, RenderWriteBlockedError (+29 more)
...
Cohesion: 0.05
Nodes (63): get_brain_preference(), Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, Operator can set BRAIN_PREFERENCE=colibri without it being rejected., test_get_brain_preference_accepts_colibri(), test_git_ref_rejects_empty(), test_git_ref_rejects_flag_injection(), test_git_ref_rejects_shell_metacharacters(), test_git_ref_rejects_traversal() (+55 more)
...
Cohesion: 0.05
Nodes (70): extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Normalize NVIDIA base URLs to avoid double /v1 when openai_compat_url appends… (+62 more)
...
Cohesion: 0.03
Nodes (71): _bootstrap_within_budget(), _create_bootstrap_indexes(), ensure_bootstrap(), _keepalive_self_ping(), lifespan(), Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free…, Start the Telegram bot (and keep-alive) inside the web process when enabled.… (+63 more)
...
### Community 51 - "tasks/models.py"
Cohesion: 0.05
Nodes (62): ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest, ExecutionLogEntry, FollowUpRequest, Any (+54 more)
...
### Community 54 - "ExecutionRequest"
Cohesion: 0.05
Nodes (45): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY…, reset_orchestrator() (+37 more)

### Community 55 - "OnboardingService"
Cohesion: 0.04
Nodes (52): Evidence, OnboardingProgress, Evidence supporting a system detection., Tracks the onboarding progress for a company., Result of a repository scan with detected stack and systems., Inferred technology stack from website/repo analysis., RepoScanResult, StackInference (+44 more)

### Community 56 - "AgentStore"
Cohesion: 0.04
Nodes (52): AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents. (+44 more)
...
Cohesion: 0.04
Nodes (42): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), AuditEvent, _luhn_ok(), packages/governance/audit.py — the evidence trail for every governed action.…, True when ``digits`` (bare, no separators) satisfies the Luhn checksum., Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Recursively strip secrets from an arbitrary argument structure. Fails… (+34 more)
...
Cohesion: 0.05
Nodes (66): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., get_failover_manager(), Return the singleton BrainFailoverManager., _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover() (+58 more)
...
Cohesion: 0.04
Nodes (55): _brain_provider_status(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., _served_models(), provider_base_url(), Return the OpenAI-compatible base URL for *provider* (env- and UI-aware)., _describe_http_status(), probe_model_liveness() (+47 more)
...
Cohesion: 0.06
Nodes (22): Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry, _blocked_result(), Any, TaskResult, Build the failed TaskResult returned when policy denies a dispatch. A result… (+14 more)

### Community 102 - "loop.py"
Cohesion: 0.04
Nodes (50): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Condense a tool result down to what belongs in an audit row. A tool result can… (+42 more)
...
Cohesion: 0.06
Nodes (39): Path, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Return the first *lines* lines of a file. Just-in-time retrieval: the executor…, Return a lightweight index of files with line counts and sizes. This is the…, Delegate to RepowiseIntelligence for a natural-language codebase question., Delegate to RepowiseIntelligence for a semantic/text codebase search., Delegate to RepowiseIntelligence to list architectural decision nodes., Return a high-level repository map and hotspot summary. (+31 more)
...
### Community 129 - "test_bedrock_provider.py"
Cohesion: 0.06
...
### Community 133 - "WorkflowRun"
Cohesion: 0.06
Nodes (27): _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestPhaseSequenceError (+19 more)
...
Cohesion: 0.06
Nodes (42): Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, BudgetExceeded, classify(), evaluate_call(), get_gate(), guard_tool_call(), GuardResult, _host_of() (+34 more)
...
### Community 147 - "test_kimi_bridge_server.py"
Cohesion: 0.05
...
### Community 151 - "ai/router.py"
Cohesion: 0.06
Nodes (42): _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog() (+34 more)
...
### Community 165 - "E2BAdapter"
Cohesion: 0.07
Nodes (36): E2BAdapter, Any, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, _FakeAsyncSandboxClass, _FakeCommandResult, _FakeCommands, _FakeFiles (+28 more)
...
### Community 183 - "services/background.py"
Cohesion: 0.06
Nodes (38): _await_hermes_ready(), _env_on(), _hydrate_scheduler_bounded(), Any, Path, services/background.py — shared background-service startup. Both the FastAPI…, Await the refresh and log the outcome; swallow every failure., Block until uvicorn is genuinely accepting connections. Without this the… (+30 more)
...
### Community 219 - "MCPUnavailableError"
Cohesion: 0.10
...
Cohesion: 0.06
Nodes (40): _build_builtin_model_map(), _default_model(), _default_reasoning_model(), _nvidia_key_present(), Build the built-in alias table — Nvidia NIM models when key is set, local…, Resolve the default executor model via the catalog (UNIT 7). Was a hardcoded…, Resolve the default planner (reasoning) model via the catalog (UNIT 7)., Resolve the NVIDIA default model via the catalog (UNIT 7). Was hardcoded to… (+32 more)
...
### Community 230 - "test_brain_availability_doctor.py"
Cohesion: 0.09
...
Cohesion: 0.07
Nodes (27): _latest_metric_value(), _note_recurrence(), _parse_timestamp(), Any, datetime, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Pull the most recent numeric sample out of a ``get_metrics`` payload. Render…, Run one full scan and return every finding, filed or not. Never raises: a… (+19 more)
...
### Community 264 - "ensure_self_company"
Cohesion: 0.10
...
### Community 282 - "get_scheduler"
Cohesion: 0.09
Nodes (31): get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, _get_backend_sam(), legacy_scheduler_delete(), legacy_scheduler_get(), Lazy-init the SAM agent for the backend deployment., Get SAM's current status., sam_status_backend() (+23 more)
...
### Community 289 - "test_render_mcp.py"
Cohesion: 0.10
...
### Community 299 - "CEOSupervisor"
Cohesion: 0.09
Nodes (18): CEOSupervisor, get_ceo_supervisor(), Any, What one sweep observed and did. Returned for tests and diagnostics., Sweeps the CEO ledger and drives open goals to closure., Sweep on the configured cadence until cancelled. A failing sweep is logged and…, Inter-sweep delay, behind a method so tests can shorten it. Patching…, True when this process already has a re-drive in flight for *goal_id*. (+10 more)
...
### Community 304 - "agents/agile_ceremonies.py"
Cohesion: 0.09
...
### Community 311 - "OrchestratorSupervisor"
Cohesion: 0.10
Nodes (14): get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, start_orchestrator_supervisor(), stop_orchestrator_supervisor() (+6 more)
...
### Community 319 - "test_ceo_router.py"
Cohesion: 0.12
...
### Community 330 - "openclaw_gateway.py"
Cohesion: 0.09
...
### Community 365 - "provider_max_rpm"
Cohesion: 0.11
...
### Community 373 - "test_phase6_workflow.py"
Cohesion: 0.13
...
### Community 449 - "get_ceo_dispatcher"
Cohesion: 0.11
...
### Community 460 - "traffic_director.py"
Cohesion: 0.13
...
### Community 465 - "weekly_digest.py"
Cohesion: 0.13
...
### Community 494 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.11
...
### Community 542 - "test_task_pre_execution_gate.py"
Cohesion: 0.24
...
Cohesion: 0.12
Nodes (8): repowise.py — RepowiseIntelligence: context packing and dependency analysis., tools.py — WorkspaceTools: read/write/search and diff application (risky…, Per-user key/value memory store backed by SQLite. Allows agents to persist and…, tools(), Test that the new tools are exposed and return strings., Test that WorkspaceTools initializes correctly., test_tools_expose_new_methods(), test_workspace_tools_initialization()
...
Cohesion: 0.12
Nodes (14): get_provider_tier(), Return the tier (``free`` / ``paid`` / ``local``) for *provider*. Returns…, test_get_provider_tier_known(), test_get_provider_tier_unknown_returns_unknown(), tests/test_omniroute_provider.py — OmniRoute self-hosted gateway wiring.…, Without an explicit base URL it points at the local default, never blank., One provider, one model — OmniRoute does its own internal fan-out., Promoted to the preferred free provider, but NVIDIA stays the floor. OmniRoute… (+6 more)
...
Cohesion: 0.14
Nodes (12): _best_cloud_primary_base(), TaskResult, Execute a TaskSpec using the internal AgentRunner and convert the agent's…, Create an isolated execution context for a single task. Tries ``git worktree…, Clean up the worktree or temp copy created by ``_create_worktree``., Return the highest-priority available cloud LLM base URL. Tries free cloud…, Resolve the LLM endpoint for a named surface (task/chat/ceo/sdlc/…). Honours…, resolve_provider_for() (+4 more)
...
### Community 676 - ".on_task_complete"
Cohesion: 0.16
...
### Community 730 - "_is_ephemeral_user"
Cohesion: 0.27
...
### Community 744 - "TestRequireAdmin"
Cohesion: 0.29
...
### Community 792 - "has_image_content"
Cohesion: 0.31
...
### Community 802 - "register_user_research_tools"
Cohesion: 0.29
...
### Community 828 - "_redact_for_notification"
Cohesion: 0.24
...
### Community 830 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 847 - "_autonomy_bg_cycle"
Cohesion: 0.28
...
### Community 865 - "TestTheProducersAndTheParserCannotDrift"
Cohesion: 0.25
...
### Community 963 - ".discover_directory"
Cohesion: 0.29
...
### Community 1042 - "_decompose_into_subtasks"
Cohesion: 0.33
...
### Community 1051 - "TestCEOAgencySystem"
Cohesion: 0.33
...
### Community 1093 - "._resolve_merge_decision"
Cohesion: 0.40
...
### Community 1102 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 1105 - "TestInternalAgentAdapterProviderChain"
Cohesion: 0.40
...
### Community 1170 - "test_direct_adapter_does_not_bypass"
Cohesion: 0.67
...
- **3528 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3523 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13466 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **112 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `Task`, `test_brain_patch_service_token.py`, `test_llm_router_strategies.py`, `test_llm_router_queue_cache.py`, `ImprovementLoop`, `brain_config.py`, `test_gh_brain_failover.py`, `config.py`, `llm/router.py`, `test_operational_incidents.py`, `PolicyEngine`, `SelfHealingAgent`, `TaskStore`, `AgentJobManager`, `test_company_api.py`, `test_llm_router_resilience.py`, `test_langfuse_agency_wide.py`, `test_model_router.py`, `Specialist`, `test_task_pre_execution_gate.py`, `test_telegram_service_webhook.py`, `services/seo_audit.py`, `test_failover_client_shared.py`, `test_telegram_webhook.py`, `tools.py`, `WorkflowRun`, `validate_policy_text`, `PrimeAgentAdapter`, `cost_tracker.py`, `_start_autonomy_loops`, `UserRole`, `test_unit8_model_catalog.py`, `E2BSandboxSession`, `test_startup_warmup.py`, `ExecutionRequest`, `OnboardingService`, `AgentStore`, `reset_cache`, `test_sam_livekit.py`, `ChatHistoryStore`, `test_openclaw_endpoints.py`, `test_autonomous_agency_e2e.py`, `test_task_service_failed_comment.py`, `KeyStore`, `test_webui_provider_priority.py`, `TokenBudget`, `ResearchTask`, `AgentSwarm`, `routed`, `_resolve_brain_provider`, `TestClient`, `test_ai_insights.py`, `SQLiteStore`, `test_runtime_governance.py`, `ArtifactStore`, `WorkflowEngine`, `test_procedural_memory.py`, `e2e/test_browser.py`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `resolve_e2b_config`, `test_local_brain_state.py`, `ProviderRouter`, `test_refresh_agent_built_proof.py`, `test_tasks_reconciler_todo_requeue.py`, `loop.py`, `test_colibri_brain_shim.py`, `test_knowledge_sync.py`, `SeoFixer`, `ScheduledJob`, `test_context_rulebook.py`, `run_task`, `WorkflowRun`, `test_issue_intake.py`, `test_new_features_e2e.py`, `TestHarnessAdapter`, `test_kimi_bridge_server.py`, `ai/router.py`, `_step`, `test_telegram_freebuff.py`, `test_trend_watcher.py`, `tests/conftest.py`, `CEOLedger`, `claim`, `persist_plan_spec`, `.on_task_complete`, `E2BAdapter`, `test_changelog_check_workflow.py`, `MCPClient`, `test_openclaw_gateway.py`, `clear_cooldowns`, `test_daily_2026_07_27.py`, `resolve_active_brain`, `services/background.py`, `test_process_quick_note_workflow.py`, `OllamaCircuitBreaker`, `_job_text`, `emit_chat_observation`, `_Budget`, `test_features_api.py`, `test_video_transcript.py`, `TrendWatcher`, `test_platform_controls.py`, `test_e2b_data_flow.py`, `nvidia_models.py`, `AgentProfile`, `test_portfolio_intake.py`, `NotificationDispatcher`, `test_agile_api.py`, `test_app_settings.py`, `TestEstimateTokensForMessages`, `test_llm_router_disabled.py`, `test_task_clarification.py`, `TestClient`, `ContextPruner`, `api_keys_for`, `test_control_plane_api.py`, `test_agent_tool_governance.py`, `get_store`, `test_persistent_memory.py`, `SecurityScanner`, `test_llm_router_e2e.py`, `capture_screens.py`, `OutputFilter`, `test_rate_limiter.py`, `test_mcp_governance.py`, `OrchestratorQueue`, `test_sam_voice.py`, `OrchestratorCheckpointStore`, `test_nvidia_model_discovery.py`, `test_catalogue_probe.py`, `test_ceo_router.py`, `TestNormalizeResponseFormat`, `session_retro.py`, `TestMongoGate`, `test_purge_backlog.py`, `analyze_page`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `ai/watchdog.py`, `_start_ceo_agency`, `TestSwarmRoleRouting`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `test_dashboard_cache.py`, `isolated_telegram_config`, `test_phase5_doctor.py`, `_undeclared`, `test_provider_state_durability.py`, `test_service_token.py`, `test_harness_spec.py`, `test_agency_fix.py`, `_Recorder`, `test_freebuff_bot.py`, `test_monitor_lib.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_v4_api.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `get_ceo_dispatcher`, `test_telegram_task_callback.py`, `test_telegram_diag_endpoint.py`, `traffic_director.py`, `ClaudeCodeAdapter`, `client`, `TestFeaturesAPI`, `test_voice_pipeline.py`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `TaskSpec`, `backend/server.py`, `test_agent_tool_governance.py`, `_fixture`, `proxy.py`, `enforcement.py`, `call_llm`, `test_ceo_dispatcher.py`, `PolicyEngine`, `ReactScratchpad`, `AgentJobManager`, `AgentSessionStore`, `BrainFailoverExhausted`, `test_empirical_verify.py`, `ContextManager`, `MultiAgentSwarm`, `E2BAdapter`, `WorkflowRun`, `test_failover_client_shared.py`, `MCPClient`, `direct_chat.py`, `test_daily_automation_2026_07_11.py`, `StuckDetector`, `agency.py`, `get_ceo_dispatcher`, `test_autonomous_agency_e2e.py`, `TokenBudget`, `UserMemoryStore`, `TestInternalAgentAdapterProviderChain`, `LocalWorkspace`, `MCPUnavailableError`, `CEODispatcher`, `loop.py`, `WorkspaceTools`, `test_governance_api.py`, `GitHubTools`, `.execute`, `FreeBuffAgent`, `AdaptiveHalter`, `ContextPruner`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `backend/server.py`, `test_bedrock_provider.py`, `implement_agent.py`, `call_llm`, `test_bedrock_live.py`, `test_all_providers_discovery.py`, `ai/router.py`, `settings.py`, `clear_cooldowns`, `ProviderConfig`, `direct_chat.py`, `kimi_bridge_provider_config`, `TestFromEnvBedrock`, `TestLegacyRouterCacheTTL`, `test_anthropic_refusal_fallback.py`, `failover_client.py`, `TestLegacyRouterServerFallback`, `_payload`, `TrafficDirector`, `is_anthropic_base_url`, `test_colibri_brain_shim.py`, `TestAnthropicPayloadStructuredOutput`?**
