...
## Corpus Check
- 1487 files · ~2,162,207 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30155 nodes · 56340 edges · 1273 communities (1110 shown, 108 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3327 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `65861705`
...
## Communities (1273 total, 108 thin omitted)
...
Cohesion: 0.02
Nodes (271): admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills() (+263 more)

### Community 2 - "TaskStore"
Cohesion: 0.03
Nodes (112): classify_domain(), agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Return the best-matching domain for a task title+description., Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, tasks/models.py — Pydantic models for the task/issue system. (+104 more)
...
Cohesion: 0.06
Nodes (42): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY…, reset_orchestrator() (+34 more)
...
### Community 10 - "Task"
Cohesion: 0.03
Nodes (81): Full task/issue document., Task, _brain_is_configured(), _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing(), Any, BaseException (+73 more)

### Community 11 - "AnthropicProvider"
Cohesion: 0.04
Nodes (51): AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Return the cache_control block appropriate for this provider's TTL setting.… (+43 more)
...
Cohesion: 0.03
Nodes (112): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, OpenAICompatible, AsyncClient, Perform one completion. Raises ``PermanentError``/``TransientError``., Yield normalised chunks for one streaming completion., Build auth headers according to the configured auth style., Join *path* onto the configured base, inserting ``/v1`` when the base is a bare… (+104 more)
...
Cohesion: 0.03
Nodes (109): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store() (+101 more)
...
Cohesion: 0.10
Nodes (6): AutonomyTracker, Reset all counters (test helper)., Singleton tracker for autonomy KPIs. Usage:: from agent.kpi import get_tracker…, Contract: KPI tracker correctly counts events and produces snapshots., Contract: Two snapshots can differ (non-blocking counters)., TestAutonomyKPIs
...
Cohesion: 0.03
Nodes (58): CircuitState, runtimes/health.py — RuntimeHealthService. Periodically polls all registered…, Reduce probe frequency for runtimes that have never come online., _build_default_manager(), Any, runtimes/manager.py — RuntimeManager., Return health status for all registered runtimes., Return routing decision audit log (newest first). (+50 more)
...
Cohesion: 0.03
Nodes (79): _brain_provider_status(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, all_provider_ids(), _provider_ids_from_literal(), provider_key_present(), Resolve the model id for a component's role on a provider. Parameters…, Return every provider id recognised by the brain config system. Iterates the…, True when the env var for *provider*'s key is set (or it's Ollama). (+71 more)
...
Cohesion: 0.09
Nodes (32): AgentCoordinator, AgentSpec, CoordinatorResult, MultiAgentSwarm, Any, Enum, str, agent/coordinator.py — Multi-Agent Coordinator One coordinator agent breaks a… (+24 more)
...
Cohesion: 0.04
Nodes (51): Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Condense a tool result down to what belongs in an audit row. A tool result can…, Feed LLM token/cost spend into the governance session budget. Kept separate…, _summarise_tool_result(), cost_for_tokens(), Return the USD cost for (prompt_tokens, completion_tokens) on *model*. Returns…, BudgetExceeded, classify() (+43 more)
...
### Community 46 - "test_ceo_router.py"
Cohesion: 0.06
...
### Community 50 - "_load_local_metrics_since"
Cohesion: 0.43
...
Cohesion: 0.05
Nodes (62): get_improvement_loop(), get_trend_watcher(), agent/trend_watcher.py — Internet-connected AI trend intelligence. Fetches from…, set_trend_watcher(), _await_hermes_ready(), _env_float(), _env_on(), Any (+54 more)

### Community 55 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.06
...
Cohesion: 0.03
Nodes (66): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,… (+58 more)
...
Cohesion: 0.05
Nodes (65): _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody, Shared purge implementation for the admin endpoint and the boot hook., Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec() (+57 more)
...
Cohesion: 0.03
Nodes (59): _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), _build_auto_skill_guidance(), _build_direct_chat_schedule_suggestion(), _build_direct_chat_tags(), _build_direct_chat_task_suggestion(), _build_provider_router() (+51 more)
...
### Community 87 - "test_agent_coordinate.py"
Cohesion: 0.11
...
Cohesion: 0.02
Nodes (138): Attempt, _load(), main(), int, get_budget(), The process-wide budget tracker., get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its… (+130 more)
...
Cohesion: 0.12
Nodes (31): ProviderHealth, Enum, Circuit-breaker state for a provider., _make_manager(), tests/test_brain_failover.py — Universal multi-provider brain failover tests.…, Status snapshot doesn't leak API keys., Make a fresh manager (bypasses the singleton for isolation)., No API keys set → no providers in the registry. (+23 more)
...
Cohesion: 0.05
Nodes (22): _env_float(), HermesAdapter, Any, AsyncClient, Response, TaskResult, Submit task to Hermes via its /tasks endpoint., Read a float env var, falling back to *default* on unset/garbage. (+14 more)
...
Cohesion: 0.07
Nodes (37): get_brain_preference(), invalidate_brain_cache(), Clear the cached brain so the next read re-resolves. Called from…, Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, A DB-stored BrainConfig wins over provider records + env (except…, When BrainConfig.updated_at is empty (never applied), the resolver falls…, AGENT_LLM_BASE_URL (the operator kill-switch) wins over a DB-stored config., test_env_override_still_wins_over_db_brain_config() (+29 more)
...
### Community 162 - "test_crispy_run_history.py"
Cohesion: 0.17
...
Cohesion: 0.06
Nodes (47): cmd_freebuff(), _model_keyboard(), _parse_callback(), _parse_user_ids(), _process_callback(), Accept / reject keyboard shown after a FreeBuff plan is generated., Start a FreeBuff flow: fetch free models and present a picker keyboard., Handle an inline-button press for the FreeBuff accept/reject/model flow. (+39 more)
...
Cohesion: 0.02
Nodes (101): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., store() (+93 more)
...
Cohesion: 0.09
Nodes (21): _get_director(), _normalized_provider_type(), ProviderAttempt, ProviderRouter, Any, Response, Try all models for one provider. Returns ProviderResult on success, None on…, Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either… (+13 more)
...
### Community 204 - "parse_event_stream"
Cohesion: 0.09
...
Cohesion: 0.09
Nodes (19): _latest_metric_value(), _note_recurrence(), Any, Pull the most recent numeric sample out of a ``get_metrics`` payload. Render…, Run one full scan and return every finding, filed or not. Never raises: a…, Scan, then file everything that is past its cooldown. The cooldown is claimed…, Map service ID → human name, degrading to the ID when unavailable., Return True when this finding is outside its cooldown. Does not claim. (+11 more)
...
Cohesion: 0.07
Nodes (38): CEODispatcher, CEOResult, _offload(), Any, Semaphore, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask., Aggregated output from a multi-specialist execution. (+30 more)
...
Cohesion: 0.05
Nodes (41): legacy_scheduler_create(), LegacyScheduleJobRequest, get_scheduler(), agent/scheduler.py — Scheduled Agent Jobs Cron-based job scheduler. Each job…, set_scheduler(), create_schedule(), delete_schedule(), get_schedule() (+33 more)
...
Cohesion: 0.05
Nodes (42): ApprovalRequest, ApprovalStatus, ApprovalStore, Any, Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`… (+34 more)
...
Cohesion: 0.06
Nodes (34): _cached(), _check_storage_health(), cost_attribution_stats(), doctor_health(), _fast_count(), get_active_provider(), get_stats(), health() (+26 more)
...
Cohesion: 0.08
Nodes (40): brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, auto_disable_provider(), describe_disabled_reason(), _disabled_from_mongo(), _disabled_from_sqlite(), disabled_providers(), _is_paid_allowed_db() (+32 more)
...
Cohesion: 0.06
Nodes (24): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer…, Store and emit *event*. Never raises. (+16 more)
...
### Community 545 - "get_tracker"
Cohesion: 0.10
Nodes (17): AutonomyCounter, AutonomySnapshot, get_tracker(), Any, agent/kpi.py — Autonomy KPIs: evidence capture and metrics tracking. Tracks key…, Return a point-in-time snapshot of all KPIs., Return the shared AutonomyTracker singleton (lazy-init)., Reset the singleton (test helper). (+9 more)
...
### Community 587 - "OrchestratorSupervisor"
Cohesion: 0.13
...
### Community 596 - "reset_failover_manager"
Cohesion: 0.10
...
### Community 626 - "weekly_digest.py"
Cohesion: 0.13
...
### Community 688 - "ApprovalCheckpoint"
Cohesion: 0.14
...
### Community 757 - "get_current_user"
Cohesion: 0.15
...
### Community 785 - "RuntimeHealthService"
Cohesion: 0.15
...
### Community 872 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 1039 - "test_autonomous_agency_e2e.py"
Cohesion: 0.12
Nodes (14): is_legacy_mode(), True when parallel execution paths are allowed (with warnings)., FakeTask, End-to-end tests for the autonomous AI agency system (issue #467). These tests…, Verify AgentRunner has _execute_step for ReAct execution loop., Verify _BYPASS context var is used for internal agent execution., Tests for Telegram notification dispatch., Verify NotificationDispatcher.on_task_complete dispatches notifications. (+6 more)
...
### Community 1144 - "TestTheProducersAndTheParserCannotDrift"
Cohesion: 0.25
...
### Community 1249 - "test_server_autonomy_and_index_fixes.py"
Cohesion: 0.29
...
### Community 1250 - "test_task_store_fails_loud_in_production.py"
Cohesion: 0.25
...
### Community 1252 - "test_doctor_coding_brain.py"
Cohesion: 0.38
...
### Community 1260 - "agent/output_filter.py"
Cohesion: 0.33
...
### Community 1262 - "provider_api_keys"
Cohesion: 0.33
...
### Community 1265 - "._resolve_merge_decision"
Cohesion: 0.40
...
### Community 1268 - "get_failover_manager"
Cohesion: 0.50
Nodes (4): get_failover_manager(), Return the singleton BrainFailoverManager., test_singleton(), test_singleton_reset()

### Community 1269 - "JWTUserStateMiddleware"
Cohesion: 0.67
...
- **3492 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3487 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13291 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **108 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `_Recorder`, `TaskStore`, `test_llm_router_queue_cache.py`, `test_ai_insights.py`, `TaskSpec`, `ExecutionRequest`, `Task`, `AuditLog`, `ImprovementLoop`, `TaskStatus`, `test_llm_router_strategies.py`, `get_skill_bindings`, `tests/conftest.py`, `E2BSandboxSession`, `OrchestratorCheckpointStore`, `brain_config.py`, `SelfHealingAgent`, `test_company_api.py`, `loop.py`, `config.py`, `test_telegram_service_webhook.py`, `test_telegram_webhook.py`, `Agency`, `test_unit8_model_catalog.py`, `services/seo_audit.py`, `test_failover_client_shared.py`, `cost_tracker.py`, `test_llm_router_resilience.py`, `test_model_router.py`, `AgentSwarm`, `PrimeAgentAdapter`, `test_openclaw_endpoints.py`, `WorkflowRun`, `services/background.py`, `AgentJobManager`, `test_knowledge_sync.py`, `TokenBudget`, `test_agent_tool_governance.py`, `test_sam_livekit.py`, `direct_chat.py`, `CEOSupervisor`, `ChatHistoryStore`, `test_vision_routing.py`, `_resolve_brain_provider`, `_clean_director`, `models/company_graph.py`, `test_telegram_auto_approve.py`, `ResearchTask`, `test_autonomy_status.py`, `reset_failover_manager`, `test_local_brain_state.py`, `TestClient`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `test_procedural_memory.py`, `resolve_e2b_config`, `OutputFilter`, `llm/router.py`, `test_startup_warmup.py`, `get_self_healing_agent`, `test_sam_voice.py`, `.record`, `test_colibri_brain_shim.py`, `test_provider_router.py`, `ClaudeCodeAdapter`, `e2e/test_browser.py`, `SeoFixer`, `ai/watchdog.py`, `test_context_rulebook.py`, `_start_ceo_agency`, `ArtifactStore`, `test_issue_intake.py`, `run_task`, `UserRole`, `test_kimi_bridge_server.py`, `_step`, `WorkflowRun`, `clear_cooldowns`, `.on_task_complete`, `test_trend_watcher.py`, `test_openclaw_gateway.py`, `TestHarnessAdapter`, `claim`, `persist_plan_spec`, `test_tasks_reconciler_todo_requeue.py`, `test_crispy_run_history.py`, `Workspace`, `KeyPool`, `test_new_features_e2e.py`, `test_telegram_freebuff.py`, `KeyStore`, `OllamaCircuitBreaker`, `_job_text`, `Usage`, `_Budget`, `resolve_active_brain`, `test_features_api.py`, `test_video_transcript.py`, `test_brain_availability_doctor.py`, `test_provider_state_durability.py`, `TrendWatcher`, `test_freebuff_bot.py`, `parse_event_stream`, `test_process_quick_note_workflow.py`, `test_portfolio_intake.py`, `NotificationDispatcher`, `platform_controls_router.py`, `ai/router.py`, `TestEstimateTokensForMessages`, `test_task_store_fails_loud_in_production.py`, `TestClient`, `test_doctor_coding_brain.py`, `test_ephemeral_reaper.py`, `test_agile_api.py`, `test_app_settings.py`, `test_task_clarification.py`, `test_e2b_task_wiring.py`, `_clean_phases`, `ContextPruner`, `WorkflowBuildRequest`, `get_current_user`, `get_scheduler`, `test_llm_router_tpm.py`, `_Collection`, `test_implement_agent_routing.py`, `test_daily_2026_07_27.py`, `nvidia_models.py`, `test_persistent_memory.py`, `SecurityScanner`, `api_keys_for`, `BackgroundAgent`, `test_llm_router_e2e.py`, `test_platform_controls.py`, `ScheduledJob`, `get_skill_registry_safe`, `test_rate_limiter.py`, `test_nvidia_model_discovery.py`, `capture_screens.py`, `_resolve_push_token`, `test_catalogue_probe.py`, `TestNormalizeResponseFormat`, `TestMongoGate`, `analyze_page`, `test_scheduler_hydration_bounded.py`, `OrchestratorQueue`, `test_mcp_governance.py`, `test_purge_backlog.py`, `model_discovery.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_e2b_data_flow.py`, `test_skill_registry_boot_refresh.py`, `TestSwarmRoleRouting`, `get_feature_matrix`, `isolated_telegram_config`, `test_agent_free_brain.py`, `test_dashboard_cache.py`, `test_provider_enable_disable.py`, `_undeclared`, `test_service_token.py`, `test_harness_spec.py`, `test_regression.py`, `test_agency_fix.py`, `test_webui_provider_priority.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_monitor_lib.py`, `test_v4_api.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_telegram_diag_endpoint.py`, `session_retro.py`, `client`, `test_voice_pipeline.py`, `test_brain_patch_service_token.py`?**
  _High betweenness centrality (0.091) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `proxy.py`, `AdaptiveHalter`, `FreeBuffAgent`, `TaskSpec`, `Any`, `test_autonomous_agency_e2e.py`, `ReactScratchpad`, `E2BSandboxSession`, `test_ceo_micromanager.py`, `test_empirical_verify.py`, `ContextManager`, `loop.py`, `Agency`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `enforcement.py`, `WorkspaceTools`, `Workspace`, `AgentSessionStore`, `StuckDetector`, `_resolve_push_token`, `test_daily_automation_2026_07_11.py`, `WorkflowRun`, `test_agent_chat_integration.py`, `AgentJobManager`, `TokenBudget`, `test_agent_tool_governance.py`, `InternalAgentAdapter`, `test_governance_api.py`, `direct_chat.py`, `TestAgentRunnerSafety`, `LocalWorkspace`, `workflow_orchestrator.py`, `call_llm`, `reset_failover_manager`, `UserMemoryStore`, `GitHubTools`, `test_agent_free_brain.py`, `CEODispatcher`, `ContextPruner`, `WorkflowBuildRequest`, `TestAgentLoopMCPIntegration`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `SelfHealingAgent` connect `SelfHealingAgent` to `analyze`, `get_self_healing_agent`, `ImprovementLoop`, `test_integration_c4_c5_c6_d3.py`, `NotificationDispatcher`, `services/background.py`, `test_daily_2026_07_27.py`, `TestSelfHealingInfrastructureClassification`?**
