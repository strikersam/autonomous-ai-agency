...
## Corpus Check
- 1449 files · ~2,105,374 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 29551 nodes · 54810 edges · 1263 communities (1145 shown, 118 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2696 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `374dd39e`
...
## Communities (1263 total, 118 thin omitted)

### Community 0 - "MongoDBStore"
Cohesion: 0.02
Nodes (100): KnowledgeItem, Find a website by its URL., Find a repository by its URL., Structured knowledge about the company, systems, or processes., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name. (+92 more)
...
Cohesion: 0.02
Nodes (105): CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any, T, Live (unexpired) entries — used by the semantic layer's scan. (+97 more)

### Community 6 - "engine.py"
Cohesion: 0.08
Nodes (26): _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestPhaseSequenceError, TestWorkspaceIsolation (+18 more)
...
Cohesion: 0.04
Nodes (88): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).… (+80 more)
...
Cohesion: 0.03
Nodes (108): _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache() (+100 more)

### Community 12 - "LLMRequest"
Cohesion: 0.02
Nodes (160): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic… (+152 more)

### Community 13 - "PolicyDecision"
Cohesion: 0.06
Nodes (35): Feed LLM token/cost spend into the governance session budget. Kept separate…, classify(), evaluate_call(), guard_tool_call(), GuardResult, _host_of(), _is_write_tool(), Any (+27 more)
...
Cohesion: 0.05
Nodes (46): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), Convenience: build and record an event in one call. (+38 more)

### Community 16 - "test_governance_sandbox.py"
Cohesion: 0.04
Nodes (77): events_from_identity(), Extract the identity-derived audit fields from an AgentIdentity. Kept here…, build_docker_run_argv(), detect_backend(), DockerBackend, E2BBackend, load_profiles(), LocalBackend (+69 more)

### Community 17 - "PolicyEngine"
Cohesion: 0.04
Nodes (85): _egress_policy_reason(), Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent., BudgetExceeded, governance_enabled(), _more_restrictive(), Exception (+77 more)

### Community 18 - "CEODispatcher"
Cohesion: 0.07
Nodes (38): CEODispatcher, _complexity_rank(), _offload(), Any, Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask. (+30 more)
...
Cohesion: 0.03
Nodes (103): _load(), main(), int, get_budget(), The process-wide budget tracker., get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager. (+95 more)
...
Cohesion: 0.04
Nodes (92): CEOResult, _decompose_into_subtasks(), Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, reset_ceo_dispatcher(), ClassifyOutput, ExecutionResult (+84 more)
...
Cohesion: 0.04
Nodes (51): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), Any, Path, loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic… (+43 more)
...
Cohesion: 0.02
Nodes (112): DetectedSystem, Evidence, Evidence supporting a system detection., A business system detected on a company's website or in their stack., Get the most confident evidence description., Inferred technology stack from website/repo analysis., StackInference, main() (+104 more)

### Community 32 - "AgentRunner"
Cohesion: 0.04
Nodes (92): AgentPhaseError, AgentRunner, Exception, Return True when no file is touched by more than one step., Raised when a named agent phase (planning, verification, etc.) fails., GATE: Golden Path steps #7-12 — the primary agent execution loop. This is the…, Set a per-session token spend cap (★3 rollout token budget). When *cap* > 0 the…, Record token spend for *session_id* and enforce the budget cap. Logs a warning… (+84 more)
...
Cohesion: 0.03
Nodes (93): AgentDefinition, AgentStore, Any, field_validator, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents. (+85 more)
...
### Community 39 - "Task"
Cohesion: 0.04
Nodes (61): Full task/issue document., Task, One-line human explanation of why this task needs human approval. Reads risk…, Best-effort label of who will run this task once approved. Returns…, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, asyncio, The reconciler must not re-queue tasks parked at the pre-execution gate.… (+53 more)

### Community 40 - "ImprovementLoop"
Cohesion: 0.04
Nodes (97): Agency, AgencyCycleResult, AgentDirective, AgentRole, _build_ceo_prompt(), _build_quick_note_instruction(), _collect_recent_git_context(), get_agency() (+89 more)
...
Cohesion: 0.04
Nodes (50): Register the built-in agent tools that are always available., _register_builtin_tools(), repowise.py — RepowiseIntelligence: context packing and dependency analysis., Path, tools.py — WorkspaceTools: read/write/search and diff application (risky…, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Return a previously saved memory value, or an empty string if absent., Persist a key/value pair to the user's profile store. (+42 more)

### Community 50 - "TaskStore"
Cohesion: 0.03
Nodes (91): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Any, Task, TaskStatus, Fail loud in production if no persistent storage is configured. Telegram…, Create a task. Deduplicates by source_id if set (Charter G3). If a task with…, Fetch a task by ID. If owner_id is set, enforces ownership. (+83 more)
...
Cohesion: 0.04
Nodes (79): _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy(), PolicyUpdateBody, Any (+71 more)
...
Cohesion: 0.05
Nodes (63): MCPUnavailableError, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), Any, Create the sandbox. Raises :class:`MCPUnavailableError` on failure., Kill the sandbox. Idempotent; never raises (best-effort cleanup)., Route an MCP-style tool call into the E2B sandbox. Implements: ``write_file``,… (+55 more)

### Community 62 - "RenderOpsMonitor"
Cohesion: 0.07
Nodes (26): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, Polls Render for platform-level failures and files them as issues., Snapshot for ``/api/render/ops/status`` and the dashboard. ``self_heal_ready``…, True when a filed finding will actually be scheduled as a fix. Detection and…, RenderOpsMonitor, _self_heal_ready() (+18 more)
...
Cohesion: 0.05
Nodes (57): _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog(), provider_access_tier() (+49 more)
...
Cohesion: 0.06
Nodes (63): apply_from_db(), Load the stored overrides and apply them. Called once at startup., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog() (+55 more)
...
### Community 74 - "seo_api.py"
Cohesion: 0.02
Nodes (164): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), list_seo_audits(), plan_seo_sprint(), BaseModel (+156 more)

### Community 75 - "test_render_mcp.py"
Cohesion: 0.05
Nodes (35): _as_list(), _coerce_payload(), RuntimeError, packages/integrations/render_mcp.py — Render platform access over MCP. The…, One deploy, normalised from whatever shape the tool returned., Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return…, Unwrap a nested envelope such as ``{"service": {...}}`` when present. (+27 more)
...
Cohesion: 0.03
Nodes (93): agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Available iff config resolves AND the SDK is importable. Never raises — a…, e2b_status(), Return the E2B sandbox integration status for the ProvidersScreen badge. Does…, e2b_enabled(), E2BConfig, _env_falsy(), _env_truthy() (+85 more)
...
Cohesion: 0.07
Nodes (20): Any, Typed facade over the Render MCP server's tools. Every method returns plain…, True when there is both an API key and an endpoint to reach., Run the MCP handshake once per client instance. Streamable-HTTP servers create…, Call a Render MCP tool and return its decoded payload. Raises…, Return the tool descriptors the connected Render MCP server offers., Report reachability without raising. Used by the ``/api/render/health``…, List the Render workspaces this API key can see. (+12 more)
...
Cohesion: 0.09
Nodes (35): UserRole, _can_read(), _can_write(), _decrypt(), _encrypt(), _get_master_key(), BaseModel, str (+27 more)

### Community 103 - "_provider"
Cohesion: 0.05
Nodes (27): probe(), _provider(), SimpleNamespace, The provider catalogue probe: must not leak, must not lie, must not be…, Rule 6: secrets are never logged, not even partially., Error paths print exception text — that must not carry the key., Listing is not proof. A retired id can still appear in a catalogue., A ${{ }} expansion inside `run:` is pasted in as shell source, so a crafted… (+19 more)
...
Cohesion: 0.07
Nodes (27): Root directory under which repo fixes are allowed to operate., Run the repo-aware auto-fixer against this company's workspace checkout.…, run_seo_fixes(), _workspace_root(), FsPath, Request to remediate auto-fixable findings in a local code repository., One concrete remediation performed (or proposed) by the fixer., SeoFixAction (+19 more)
...
Cohesion: 0.06
Nodes (35): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is…, End-to-end tests for the autonomous AI agency system (issue #467). These tests… (+27 more)
...
Cohesion: 0.07
Nodes (56): Convenience: resolve all four role models for a component. Returns a dict with…, resolve_component_role_models(), get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _answer_callback() (+48 more)
...
Cohesion: 0.05
Nodes (36): build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, ProviderRouter, Priority-ordered LLM provider fallback with health checks and retries., main(), probe_one(), ProviderRouter, _bedrock_api_response() (+28 more)

### Community 132 - "test_issue_intake.py"
Cohesion: 0.11
Nodes (34): _capability_tags(), create_task_from_oldest_open_issue(), intake_issue(), _issue_labels(), issue_source_id(), map_issue_to_task(), Any, Task (+26 more)
...
Cohesion: 0.05
Nodes (28): Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed., Look up a tool by name., Return all registered tools., Find tools that advertise a specific capability. (+20 more)
...
Cohesion: 0.07
Nodes (52): Register the interactive-browser capability (agent/browser.py). Unlike…, Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_browser_tools(), _register_web_reach_tools(), get_web_reach(), _load_script_module(), Any, ModuleType (+44 more)
...
Cohesion: 0.08
Nodes (44): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, _drive(), _enforce(), governance_on(), _observations(), Any, MonkeyPatch (+36 more)
...
Cohesion: 0.08
Nodes (34): _Circuit, _enabled(), _failure_threshold(), OllamaCircuitBreaker, Per-model circuit breaker for Ollama backend health. Tracks consecutive failure…, Record a successful response; close the circuit., Record a 5xx error; open the circuit after threshold is reached., Reset circuit state (used in tests and admin operations). (+26 more)
...
Cohesion: 0.08
Nodes (17): analyze_page(), _count_syllables(), estimate_pixel_width(), flesch_reading_ease(), BeautifulSoup, Approximate SERP rendering width of ``text`` in pixels., Flesch reading-ease score; None when there is too little text to score., Run every page-scoped check against one HTML document (no network). Returns a… (+9 more)
...
### Community 189 - "test_colibri_brain_shim.py"
Cohesion: 0.08
...
### Community 201 - "operational_incidents.py"
Cohesion: 0.11
...
Cohesion: 0.07
Nodes (41): Issue title: the failure mode plus how hard it is recurring., Stable id for a trend alert (matches ``TrendWatcher._sig`` shape)., trend_id(), map_initiative_to_task(), materialize_committed(), _portfolio_materialize_enabled(), portfolio_source_id(), Any (+33 more)

### Community 214 - "test_workflow_orchestrator.py"
Cohesion: 0.06
Nodes (38): get_orchestrator_checkpoint_store(), _NoopDB, OrchestratorCheckpointStore, Any, services/orchestrator_checkpoint.py — Durable step-level checkpointing Issue…, Restore in-flight runs at startup. Called during backend bootstrap. Returns a…, Fallback in-memory store when no DB is available., Persist orchestrator runs so they survive restarts. (+30 more)
...
### Community 232 - "_FakeAsyncClient"
Cohesion: 0.20
...
Cohesion: 0.06
Nodes (20): Initiative, PortfolioMetrics, Aggregate metrics across the whole portfolio., Create and register a new initiative, returning it., Add a pre-built Initiative (e.g. from the intelligence layer)., Look up an initiative by ID., Return initiatives sorted by WSJF (highest first). Cancelled initiatives are…, Greedily fill ``capacity`` (in job-size units) by WSJF priority. Walks the… (+12 more)
...
Cohesion: 0.05
Nodes (42): get_circuit_breaker(), _enabled(), get_available_models(), invalidate_cache(), is_model_available(), Ollama model availability check with TTL cache. Keeps a short-lived cache of…, Force the next call to re-probe Ollama (useful in tests)., Return True if *model* is in the Ollama tag list (or health checks off).… (+34 more)
...
Cohesion: 0.04
Nodes (46): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), AuditEvent, AuditLog, Any, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., One governed action, fully described. Field order follows the… (+38 more)
...
Cohesion: 0.08
Nodes (27): _merge_changed_files(), Collect changed_files across all specialists into a single de-duped list., _get_ceo_dispatcher(), _orchestrator_bypass(), Phase, Enum, services/workflow_orchestrator.py — Canonical Execution Backbone THE single…, Execute the plan via the selected specialist(s). (+19 more)
...
### Community 304 - "platform_controls_router.py"
Cohesion: 0.10
...
Cohesion: 0.05
Nodes (75): ApprovalStatus, ApprovalStore, get_approval_store(), Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`…, Block until *approval_id* is resolved or its TTL expires. Combines an event… (+67 more)
...
Cohesion: 0.13
Nodes (11): Return orchestrator queue depth, active runs, and supervisor state (#522)., workflow_orchestrator_status(), get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log. (+3 more)
...
### Community 356 - "TestTelegramNotifications"
Cohesion: 0.40
...
Cohesion: 0.05
Nodes (63): BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), _norm(), _pick_from_records(), Any (+55 more)
...
### Community 407 - "test_provider_410_per_model.py"
Cohesion: 0.18
...
Cohesion: 0.03
Nodes (77): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), KeyRing, KeyState (+69 more)
...
### Community 499 - "get"
Cohesion: 0.02
Nodes (89): auth_me(), auto_recommend_skills(), _builtin_provider_records(), _check_storage_health(), cost_attribution_stats(), discover_remote_skills(), doctor_health(), _DoctorCheck (+81 more)
...
Cohesion: 0.07
Nodes (33): _file_issue(), _latest_metric_value(), _note_recurrence(), _parse_timestamp(), Any, datetime, services/render_ops.py — autonomous Render debugging + environment monitoring.…, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``. (+25 more)
...
### Community 524 - "test_agent_chat_integration.py"
Cohesion: 0.23
...
Cohesion: 0.05
Nodes (59): agents/store.py — Persistent store for user-defined agent configurations.…, Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, tasks/models.py — Pydantic models for the task/issue system., Comment or reply on a task. (+51 more)
...
Cohesion: 0.10
Nodes (22): List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., _embedded(), _embedded_run(), _fb_models(), _fb_plan(), _fb_run(), _freebuff_max_steps(), Return the free model list (embedded or via proxy). (+14 more)
...
Cohesion: 0.06
Nodes (38): _append_agent_session_message(), ProviderConfig, _run_agent_loop(), clear_cooldowns(), CommercialFallbackRequiredError, is_provider_on_cooldown(), mark_provider_failed(), ProviderFallbackError (+30 more)
...
### Community 605 - "nvidia_models.py"
Cohesion: 0.20
...
Cohesion: 0.03
Nodes (33): requires_db, CompanyGraphStore, Delete a company and all its associated data., List companies with optional filtering., Count total companies in the store., Create a new company graph., Get the complete company graph for a company., Update a company graph. (+25 more)
...
### Community 639 - "reset_failover_manager"
Cohesion: 0.17
Nodes (12): Reset the singleton (for tests)., reset_failover_manager(), _clean_env(), Regression test: HTTP 413 (Payload Too Large) must fail over to the next…, Groq (will 413) + Cerebras (will succeed) both configured, NVIDIA deliberately…, test_413_fails_over_to_next_provider_after_one_attempt(), _two_provider_env(), _clean_env() (+4 more)
...
### Community 648 - "TestTheGatewayRegistryOffersOnlyLiveModels"
Cohesion: 0.22
...
### Community 685 - "TestAnthropicWorkspaceIdCapture"
Cohesion: 0.33
...
Cohesion: 0.06
Nodes (42): Execute work through the 11-phase golden path., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_execute(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper). (+34 more)

### Community 762 - "_enforce_rate_limit"
Cohesion: 0.13
...
### Community 785 - "test_llm_router_tpm.py"
Cohesion: 0.15
...
Cohesion: 0.02
Nodes (231): admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, BrainTestRequest (+223 more)
...
### Community 1195 - "_start_ceo_agency"
Cohesion: 0.31
...
### Community 1200 - "FeatureUnavailableError"
Cohesion: 0.13
...
### Community 1211 - "agent/output_filter.py"
Cohesion: 0.32
...
### Community 1242 - "TestCEOAgencySystem"
Cohesion: 0.33
...
## Knowledge Gaps
- **3425 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3420 more)
...
- **Why does `_fixture()` connect `_fixture` to `reap_expired_companies`, `test_provider_router.py`, `test_llm_router_queue_cache.py`, `CEOLedger`, `test_company_api.py`, `brain_config.py`, `PolicyDecision`, `validate_policy_text`, `PolicyEngine`, `SelfHealingAgent`, `llm/router.py`, `test_provider_state_durability.py`, `PrimeAgentAdapter`, `clear_cooldowns`, `WebsiteScanner`, `test_monitor_lib.py`, `test_openclaw_endpoints.py`, `TaskWorkflowService`, `test_unit8_model_catalog.py`, `Task`, `ImprovementLoop`, `test_model_router.py`, `test_llm_router_strategies.py`, `AgentSwarm`, `test_failover_client_shared.py`, `Usage`, `TaskStore`, `test_process_quick_note_workflow.py`, `TestRuntimeControl`, `AgentJobManager`, `SQLiteStore`, `ChatHistoryStore`, `E2BSandboxSession`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `get_task_store`, `test_local_brain_state.py`, `NotificationDispatcher`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `seo_api.py`, `test_telegram_diag_endpoint.py`, `config.py`, `ResearchTask`, `TestClient`, `resolve_e2b_config`, `test_procedural_memory.py`, `TestAuthAndTaskOwnership`, `e2e/test_browser.py`, `TrafficDirector`, `UserRole`, `_provider`, `test_knowledge_sync.py`, `SchedulerStore`, `test_startup_warmup.py`, `SeoFixer`, `KeyStore`, `claim`, `FeatureMaturity`, `test_context_rulebook.py`, `reset_failover_manager`, `ArtifactStore`, `run_task`, `test_issue_intake.py`, `test_openclaw_gateway.py`, `TestAgentRunnerSafety`, `test_trend_scoping.py`, `BackgroundAgent`, `TestHarnessAdapter`, `MCPClient`, `_step`, `test_new_features_e2e.py`, `test_agent_tool_governance.py`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `test_e2b_adapter.py`, `_Budget`, `_start_ceo_agency`, `TestSwarmRoleRouting`, `test_telegram_service_webhook.py`, `OllamaCircuitBreaker`, `analyze_page`, `_resolve_user_github_token`, `TestMongoGate`, `brain_failover.py`, `test_features_api.py`, `test_task_service_failed_comment.py`, `test_video_transcript.py`, `TrendWatcher`, `WorkflowOrchestrator`, `emit_chat_observation`, `UsageEvent`, `_undeclared`, `test_agile_api.py`, `test_app_settings.py`, `test_agent_scripts_share_one_model_list.py`, `test_portfolio_intake.py`, `test_trend_watcher.py`, `test_unit7_catalog_propagation.py`, `test_nvidia_model_discovery.py`, `_clean_phases`, `test_ceo_router.py`, `TestClient`, `services/background.py`, `sam`, `ContextPruner`, `BrainWatchdog`, `test_llm_router_e2e.py`, `session_retro.py`, `test_daily_2026_07_27.py`, `rate_limiter.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `OutputFilter`, `test_platform_controls.py`, `test_llm_router_tpm.py`, `test_scheduler_hydration_bounded.py`, `AuditLog`, `workflow_orchestrator.py`, `test_sam_voice.py`, `WorkflowRun`, `OrchestratorQueue`, `platform_controls_router.py`, `test_purge_backlog.py`, `cost_tracker.py`, `test_governance_enforcement.py`, `test_task_clarification.py`, `test_control_plane_api.py`, `test_kimi_bridge_server.py`, `.record`, `tests/conftest.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_brain_patch_service_token.py`, `test_harness_spec.py`, `test_ping.py`, `capture_screens.py`, `test_dashboard_cache.py`, `model_discovery.py`, `test_regression.py`, `test_agency_fix.py`, `test_provider_410_per_model.py`, `test_telegram_mutating_commands.py`, `test_llm_router_resilience.py`, `test_v4_api.py`, `allow_paid`, `client`, `parse_event_stream`, `api_keys_for`, `test_voice_pipeline.py`, `_job_text`, `test_doctor_coding_brain.py`, `TestNormalizeResponseFormat`, `test_task_store_fails_loud_in_production.py`, `test_runtimes_health_endpoint.py`, `test_sam_livekit.py`, `test_tasks_awaiting_approval_api.py`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `proxy.py`, `UserMemoryStore`, `TaskSpec`, `TestAgentRunnerSafety`, `ReactScratchpad`, `PolicyDecision`, `test_agent_chat_integration.py`, `validate_policy_text`, `PolicyEngine`, `CEODispatcher`, `ContextManager`, `MCPClient`, `clear_cooldowns`, `loop.py`, `test_agent_tool_governance.py`, `test_e2b_adapter.py`, `workflow_orchestrator.py`, `MultiAgentSwarm`, `ImprovementLoop`, `test_failover_client_shared.py`, `StuckDetector`, `WorkspaceTools`, `test_governance_api.py`, `AgentJobManager`, `E2BSandboxSession`, `test_empirical_verify.py`, `WorkflowOrchestrator`, `TestAgentLoopMCPIntegration`, `LocalWorkspace`, `test_workflow_orchestrator.py`, `.execute`, `TokenBudget`, `backend/server.py`, `InternalAgentAdapter`, `GitHubTools`, `AdaptiveHalter`, `ContextPruner`, `diagnostics.py`, `direct_chat.py`, `test_agent_free_brain.py`, `AgentSessionStore`, `FreeBuffAgent`, `reset_failover_manager`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `test_all_providers_discovery.py`, `test_provider_router.py`, `test_chat_mode_regressions.py`, `test_anthropic_router.py`, `is_anthropic_base_url`, `test_colibri_provider.py`, `test_provider_410_per_model.py`, `implement_agent.py`, `clear_cooldowns`, `TestAnthropicPayloadStructuredOutput`, `test_colibri_brain_shim.py`, `ProviderConfig`, `NvidiaProvider`, `TestFromEnvBedrock`, `failover_client.py`, `kimi_bridge_provider_config`, `settings.py`, `TrafficDirector`, `backend/server.py`, `system_instruction`, `direct_chat.py`, `test_bedrock_live.py`, `TestRouterIntegration`?**
