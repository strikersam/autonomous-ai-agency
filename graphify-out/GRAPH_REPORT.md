...
## Corpus Check
- 1514 files · ~2,194,396 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 31891 nodes · 58829 edges · 1325 communities (1154 shown, 171 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3723 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5a4fec25`
...
## Communities (1325 total, 171 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.02
Nodes (188): admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, BrainTestRequest (+180 more)

### Community 1 - "Task"
Cohesion: 0.03
Nodes (92): Helpers that turn scheduler and playbook activity into real tasks., Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, Full task/issue document., Task, TaskStatus, Record the human decision on a task's **pre-execution** approval gate. A…, Re-run a task. Permissive across every non-active state so the dashboard Retry… (+84 more)

### Community 2 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (107): CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any, T, Live (unexpired) entries — used by the semantic layer's scan. (+99 more)

### Community 3 - "proxy.py"
Cohesion: 0.03
Nodes (145): admin_control(), admin_login(), admin_status(), AdminControlBody, AdminLoginBody, AdminUpdateKeyBody, agency_status(), agent_chat() (+137 more)
...
### Community 5 - "models/company_graph.py"
Cohesion: 0.03
Nodes (110): ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraphResponse, CompanyUpdateRequest, Connector, DetectedSystem, Evidence (+102 more)

### Community 6 - "TestRuntimeControl"
Cohesion: 0.04
...
Cohesion: 0.03
Nodes (54): Find a website by its URL., A company website with detected systems and stack inference., Website, Add a website to a company. Args: company_id: Company ID url: Website URL…, Create a new company in SQLite., Get a company by ID from SQLite., Update a company in SQLite., Delete a company and all associated data from SQLite. (+46 more)
...
Cohesion: 0.04
Nodes (98): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+90 more)

### Community 11 - "test_llm_router_strategies.py"
Cohesion: 0.03
Nodes (95): count, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, HealthTracker, _Outcome, ProviderHealth (+87 more)

### Community 12 - "brain_config.py"
Cohesion: 0.03
Nodes (116): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), _migrate_brain_to_safe_default(), _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config() (+108 more)

### Community 13 - "test_llm_router_providers.py"
Cohesion: 0.03
Nodes (97): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, build_provider(), Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the…, resolve_kind(), _anthropic_provider(), _client() (+89 more)
...
Cohesion: 0.03
Nodes (134): int, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., Test hook — clear all usage accounting., reset(), get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its… (+126 more)

### Community 16 - "PolicyEngine"
Cohesion: 0.05
Nodes (69): _action_matches(), _as_list(), Decision, GroupPolicy, _host_matches(), Mode, _normalise_path(), _path_matches() (+61 more)

### Community 17 - "get_db"
Cohesion: 0.03
Nodes (102): auth_me(), auto_recommend_skills(), _builtin_provider_records(), _check_storage_health(), cost_attribution_stats(), create_access_token(), discover_remote_skills(), doctor_health() (+94 more)

### Community 18 - "TaskSpec"
Cohesion: 0.04
Nodes (92): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor., json_safe(), runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).… (+84 more)

### Community 19 - "LLMRequest"
Cohesion: 0.03
Nodes (75): AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Return the cache_control block appropriate for this provider's TTL setting.… (+67 more)

### Community 20 - "resolve_active_brain"
Cohesion: 0.06
Nodes (44): get_active_brain_sync(), Return the cached brain read by sync callers. None if never resolved., Single source of truth for the active brain LLM. Resolution order (matches the…, resolve_active_brain(), Resolve the LLM endpoint for agent execution (module-level, #522 failover).…, _resolve_brain_provider(), tests/test_brain_resolver.py — Tests for the single brain resolver. The agency…, When a free provider (NVIDIA NIM) is configured, the brain must NOT auto-pick… (+36 more)
...
Cohesion: 0.04
Nodes (81): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+73 more)

### Community 23 - "test_governance_sandbox.py"
Cohesion: 0.03
Nodes (85): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, events_from_identity(), Any, Convenience: build and record an event in one call., Extract the identity-derived audit fields from an AgentIdentity. Kept here…, record_event(), build_docker_run_argv(), detect_backend() (+77 more)

### Community 24 - "TaskExecutionCoordinator"
Cohesion: 0.03
Nodes (69): AgentDefinition, AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+61 more)

### Community 25 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (92): CEOResult, _merge_changed_files(), Aggregated output from a multi-specialist execution., Collect changed_files across all specialists into a single de-duped list., ClassifyOutput, get_ceo_fallback_stats(), PlanOutput, CLASSIFY phase: domain and task type determination. (+84 more)
...
Cohesion: 0.03
Nodes (52): JCodeAdapter, Path, Write .jcode/mcp.json in the workspace, pointing at our proxy's MCP endpoint.…, Adapter for jcode — TIER 2 high-performance Rust coding agent., runtimes/health.py — RuntimeHealthService. Periodically polls all registered…, _build_default_manager(), Any, runtimes/manager.py — RuntimeManager. (+44 more)

### Community 28 - "E2BSandboxSession"
Cohesion: 0.04
Nodes (70): MCPUnavailableError, RuntimeError, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any, Create the sandbox. Raises :class:`MCPUnavailableError` on failure. (+62 more)
...
### Community 31 - "_start_autonomy_loops"
Cohesion: 0.13
Nodes (21): get_trend_watcher(), set_trend_watcher(), _env_float(), Bootstrap the autonomous loops that turn signals into work (the heart of the…, Parse a float env var, falling back to *default* on a bad value (never raises)., Periodically fetch trends (which fans out per-company scoped tasks, G4). Sleeps…, _start_autonomy_loops(), _trend_watch_loop() (+13 more)

### Community 32 - "MCPClient"
Cohesion: 0.03
Nodes (41): MCPClient, Any, Thin async MCP client with open/close circuit breaker. Thread-safe only within…, Full URL of the JSON-RPC endpoint this client posts to., Build the request headers shared by ``_rpc`` and ``notify``. ``Accept`` lists…, Propagate the calling agent's identity across the process boundary. The MCP…, Attach the agent identity whose actions this client executes., Remember a server-issued ``Mcp-Session-Id`` for subsequent requests. (+33 more)

### Community 33 - "test_provider_router.py"
Cohesion: 0.05
Nodes (51): _acquire_provider_probe(), CommercialFallbackRequiredError, extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), RuntimeError, Try to acquire a distributed probe lock for *provider_id*. Returns True if this…, Release the probe lock for *provider_id*. (+43 more)

### Community 34 - "loop.py"
Cohesion: 0.04
Nodes (53): Agent subsystem — planner / executor / verifier loop., loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, AgentEvent, AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest (+45 more)
...
### Community 39 - "get_company_graph_store"
Cohesion: 0.03
Nodes (82): Synchronize the Company Graph. Re-scans all websites and repositories, updates…, Scan a repository for technology stack and systems., scan_repo_endpoint(), sync_company_graph(), autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap() (+74 more)

### Community 40 - "Settings"
Cohesion: 0.02
Nodes (102): _brain_provider_status(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, all_provider_ids(), _provider_ids_from_literal(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the…, True when the env var for *provider*'s key is set (or it's Ollama)., Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the… (+94 more)

### Community 41 - "test_llm_router_resilience.py"
Cohesion: 0.04
Nodes (64): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), KeyRing, KeyState (+56 more)
...
### Community 45 - "services/background.py"
Cohesion: 0.05
Nodes (50): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _await_hermes_ready(), _env_on(), Any, Path, services/background.py — shared background-service startup. Both the FastAPI…, Await the refresh and log the outcome; swallow every failure. (+42 more)

### Community 46 - "test_mcp_registry.py"
Cohesion: 0.04
Nodes (52): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable(), _playwright_configured(), _playwright_spec() (+44 more)
...
Cohesion: 0.06
Nodes (68): is_user_onboarding_allowed(), Return True if this user may run the onboarding wizard. Resolution order: 1. If…, audit(), Append an audit log entry. Never logs raw secrets — only secret IDs / masked…, complete_wizard(), _delete_wizard_state(), detect_configured_providers(), detect_hardware_for_wizard() (+60 more)

### Community 49 - "enforcement.py"
Cohesion: 0.05
Nodes (45): BudgetExceeded, classify(), evaluate_call(), guard_tool_call(), GuardResult, _host_of(), _is_write_tool(), _more_restrictive() (+37 more)
...
### Community 52 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (78): BusinessCategory, DirectChatSession, Direct chat session with Company Graph context binding., Bind a company to this chat session and load its Company Graph., Bind a repository to this chat session., Get the bound Company Graph., Get enriched context including Company Graph data., Company (+70 more)

### Community 53 - "BrainWatchdog"
Cohesion: 0.04
Nodes (55): Any, One-shot self-healing pass. 1. Checks if the active brain provider is in a…, self_heal_brain_and_unblock_tasks(), BrainWatchdog, _classify_failure_cause(), get_watchdog(), _is_provider_actually_available(), Any (+47 more)
...
Cohesion: 0.04
Nodes (51): field_validator, A single occurrence of a check firing on a specific URL., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoSiteFindings, _count_syllables() (+43 more)
...
Cohesion: 0.05
Nodes (42): analyze_quantitative(), _classify_sentiment(), _extract_keywords(), plan_research(), Any, BaseModel, field_validator, QualAnalysis (+34 more)

### Community 69 - "get_task_store"
Cohesion: 0.06
Nodes (65): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., Reset the singleton (for tests)., reset_failover_manager(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover() (+57 more)
...
Cohesion: 0.04
Nodes (63): build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget(), get_metrics() (+55 more)

### Community 78 - "GoalRecord"
Cohesion: 0.07
...
Cohesion: 0.05
Nodes (64): _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir(), _env_key_names(), expand_env(), _fields_of() (+56 more)
...
Cohesion: 0.06
Nodes (48): DeliveryPolicy, How code lands on a repo's default branch (detected, GitHub-only for now). The…, A company's connection to a code repository (GitHub-only this pass). URL-only…, RepoConnection, attach_repo_connection(), build_repo_connection(), decide_merge(), detect_delivery_policy() (+40 more)
...
### Community 91 - "ProviderRouter"
Cohesion: 0.04
Nodes (58): build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, _get_director(), is_commercial_provider(), _normalized_provider_type(), provider_access_tier(), _provider_field(), provider_sort_key() (+50 more)
...
Cohesion: 0.04
Nodes (56): _bootstrap_within_budget(), _create_bootstrap_indexes(), ensure_bootstrap(), Await one warm-up step, deferring it to the background if it overruns.…, The task store the background services should use, wiring it if needed.…, Create every boot index concurrently rather than one round-trip at a time.…, Idempotent bootstrap for indexes + seeded admin/providers. FastAPI startup…, Auto-detect the best available Ollama model and update ollama-local in the DB.… (+48 more)
...
Cohesion: 0.03
Nodes (70): _describe_http_status(), _is_dns_failure(), _probe_failure_reason(), probe_model_liveness(), _probe_ollama(), _probe_openai_compat(), ProbeResult, BaseException (+62 more)
...
Cohesion: 0.06
Nodes (63): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), ResumeRequest, AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState (+55 more)
...
Cohesion: 0.02
Nodes (69): AgentJob, AgentJobManager, heartbeat(), make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs… (+61 more)

### Community 115 - "_fixture"
Cohesion: 0.02
Nodes (90): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+82 more)
...
Cohesion: 0.05
Nodes (64): _domain_list_reason(), _domain_matches(), _egress_policy_reason(), _parse_domain_list(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return True if *host* is *pattern* or a subdomain of it., Split a comma-separated env var into a normalised domain list., Return why the env-var domain policy blocks *host*, or None. Checks… (+56 more)
...
### Community 128 - "HermesAdapter"
Cohesion: 0.03
Nodes (55): Resolve the model id to force for a code-execution run, or ``None``. Returns…, resolve_coding_model_preference(), _env_float(), HermesAdapter, Any, AsyncClient, Submit task to Hermes via its /tasks endpoint., Read a float env var, falling back to *default* on unset/garbage. (+47 more)

### Community 129 - "TestAdapterMetadata"
Cohesion: 0.07
Nodes (10): Any, Adapter for a compatible external task harness., TaskHarnessAdapter, Verify all adapters have required metadata without needing a running runtime., Tests for _start_local_runtime safety guards in runtimes/control.py., task_harness (no _base_url) must not spawn agent_runtime.py., If agent_runtime.py exits immediately, _base_url must NOT be set., TestAdapterMetadata (+2 more)
...
### Community 135 - "_FakeCommandResult"
Cohesion: 0.13
Nodes (7): _FakeAsyncSandboxClass, _FakeCommandResult, _FakeCommands, _FakeFiles, _FakeSandbox, Any, counting_run()

### Community 136 - "TaskPriority"
Cohesion: 0.08
...
Cohesion: 0.05
Nodes (22): get_harness_adapter(), harness_active(), harness_catalog(), harness_session_close(), HarnessSessionCloseBody, Return the full ECC harness catalog with capabilities. Public — no auth…, Return the currently active ECC harnesses with session metrics. Authenticated —…, Close a harness session and aggregate its metrics. (+14 more)

### Community 139 - "models/seo_audit.py"
Cohesion: 0.10
Nodes (23): Run the repo-aware auto-fixer against this company's workspace checkout.…, run_seo_fixes(), BaseModel, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, Snapshot of one crawled page with the on-page facts the checks used., Lightweight listing entry for past audits., Request to remediate auto-fixable findings in a local code repository., One concrete remediation performed (or proposed) by the fixer. (+15 more)
...
Cohesion: 0.06
Nodes (49): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), FailoverResult, _is_billing_refusal(), _is_ollama(), _key_pool() (+41 more)
...
### Community 151 - "test_bedrock_provider.py"
Cohesion: 0.06
...
### Community 157 - "CEODispatcher"
Cohesion: 0.06
Nodes (41): CEODispatcher, _complexity_rank(), _decompose_into_subtasks(), _offload(), Any, Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is… (+33 more)

### Community 158 - "RenderFinding"
Cohesion: 0.05
...
Cohesion: 0.09
Nodes (48): _run(), Issue title: the failure mode plus how hard it is recurring., _company_attr(), company_stack_tags(), extract_stack_tags(), fan_out_trend(), fan_out_trends(), is_code_change_trend() (+40 more)
...
### Community 166 - "workflow_orchestrator.py"
Cohesion: 0.10
Nodes (33): BoundContext, ExecutionResult, JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput, Phase, PreflightReport (+25 more)
...
Cohesion: 0.05
Nodes (28): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer…, Store and emit *event*. Never raises. (+20 more)
...
Cohesion: 0.04
Nodes (64): auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable., Persist a provider as disabled, through the store that already owns it. (+56 more)
...
Cohesion: 0.04
Nodes (68): Replace the process-wide store. Tests only., reset_approval_store(), Replace the process-wide audit log. Tests only., reset_audit_log(), Load a policy document from YAML, falling back to the default. Never raises:…, Replace the process-wide engine. Tests only., reset_policy_engine(), _drive() (+60 more)

### Community 175 - "get_tool_annotations"
Cohesion: 0.08
...
Cohesion: 0.07
Nodes (52): get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control() (+44 more)
...
Cohesion: 0.06
Nodes (27): _best_cloud_primary_base(), InternalAgentAdapter, _nvidia_provider_chain(), Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Execute a TaskSpec using the internal AgentRunner and convert the agent's…, Create an isolated execution context for a single task. Tries ``git worktree… (+19 more)
...
Cohesion: 0.08
Nodes (40): activation_required(), ActivationResult, _b64url_decode(), _b64url_encode(), _decode_jwt_unverified(), _generate_token_for_owner(), get_activation(), get_or_create_instance_id() (+32 more)
...
Cohesion: 0.07
Nodes (32): AgentRole, _fake_artifact(), tests/test_workflow_models.py — Unit tests for workflow/models.py., TestCheckRun, TestSlice, workflow/artifact_store.py — Durable artifact persistence. Artifacts are stored…, _extract_slices_from_plan(), workflow/engine.py — WorkflowEngine: CRISPY phase sequencer. The engine is the… (+24 more)

### Community 202 - "emit_chat_observation"
Cohesion: 0.10
Nodes (29): Browser admin UI for login, service control, key management, and diagnostics., observability_diag_public(), PUBLIC diagnostic endpoint for Langfuse — no auth required. Returns exactly…, _base_url(), _department_trace_tags(), emit_agency_observation(), emit_chat_observation(), _emit_langfuse_http_sync() (+21 more)
...
Cohesion: 0.07
Nodes (25): Path, service_manager.py — Telegram & Notification Integration Extension Extends the…, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess)., Run the bot with stop-event awareness., # NOTE: do NOT re-escape here — services.daily_digest.format_digest_markdown, Manages the Telegram bot as a managed service alongside ollama/proxy/tunnel.… (+17 more)
...
Cohesion: 0.07
Nodes (26): engine(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test(), _tracking_run_single(), TestPhaseSequence (+18 more)
...
Cohesion: 0.09
Nodes (35): Aggregated report row - Screaming Frog CSV compatible., Complete result of one audit run., SeoAuditReport, SeoIssueReportRow, Paragraph, compute_pressure(), loss_share_from_pressure(), Map aggregate issue pressure to an at-risk revenue share via the diminishing-… (+27 more)
...
Cohesion: 0.08
Nodes (34): cross_verify(), Any, race(), agent/verification_strategies.py — opt-in parallel patterns for high-stakes…, Heuristic fallback score when the reward model is unavailable.…, Run *n* independent attempts at *instruction* concurrently; return the winner.…, True if any path matches the repo's risky-module trigger list., Have an independent agent re-check a completed task's changed files. Returns… (+26 more)

### Community 240 - "get_failover_manager"
Cohesion: 0.07
...
### Community 241 - "self_healing.py"
Cohesion: 0.06
Nodes (35): FailureCategory, heal_signature(), HealState, Enum, str, agent/self_healing.py — Self-Healing Agent (closed-loop, Autonomy Charter G2)…, E2: Return a corrective hint for each failure category., Classified failure types for targeted self-healing (E2). (+27 more)
...
Cohesion: 0.08
Nodes (35): AdviceResult, default_executives(), ExecOpinion, Executive, ExecutiveAdvisory, _format_company_context(), get_executive_advisory(), Any (+27 more)
...
### Community 251 - "scrub"
Cohesion: 0.05
Nodes (30): _luhn_ok(), packages/governance/audit.py — the evidence trail for every governed action.…, True when ``digits`` (bare, no separators) satisfies the Luhn checksum., Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Recursively strip secrets from an arbitrary argument structure. Fails…, Redact secret-shaped substrings, then truncate., _redact_pii(), _card() (+22 more)
...
Cohesion: 0.08
Nodes (42): _apply_chat_defaults(), _apply_reasoning_budget(), _emit_safely(), _extract_exact_output(), _filter_fragment(), _filter_openai_sse_line(), handle_ollama_native_chat(), handle_openai_chat_completions() (+34 more)
...
Cohesion: 0.08
Nodes (29): export_seo_audit(), Export a stored audit. - ``csv`` aggregated findings, Screaming Frog…, _build_pdf(), findings_block(), hr(), S(), section(), main() (+21 more)

### Community 266 - "OrchestratorSupervisor"
Cohesion: 0.10
...
### Community 272 - "test_output_filter.py"
Cohesion: 0.06
...
### Community 273 - "agile_sprints.py"
Cohesion: 0.14
...
Cohesion: 0.08
Nodes (27): all_controls(), controls_by_group(), Every control in the catalogue, in display order., The catalogue grouped for the dashboard, groups in display order., clean_overrides(), _python_sources(), Tests for the dashboard platform-controls surface. Covers the three things that…, The call-volume throttle is present, numeric, and defaults to the calmer free-… (+19 more)
...
### Community 299 - "test_brain_availability_doctor.py"
Cohesion: 0.10
...
### Community 300 - "BrainFailoverManager"
Cohesion: 0.09
...
Cohesion: 0.09
Nodes (30): allow_paid_brain(), get_brain_preference(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, Operator can set BRAIN_PREFERENCE=colibri without it being rejected., test_get_brain_preference_accepts_colibri(), _admin_out(), register_webui() (+22 more)
...
### Community 335 - "mcp_client.py"
Cohesion: 0.11
Nodes (12): MCPToolResult, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Result from ``call_tool_structured()``. ``structured`` is populated when the…, Prefer structured data; fall back to text when unavailable., asyncio, tests/test_daily_automation_2026_07_13.py — Daily automation tests…, Unit tests for agent.mcp_client.MCPToolResult., Tests for MCPClient.call_tool_structured() using an async mock. (+4 more)
...
Cohesion: 0.10
Nodes (11): Static definition of a single audit check (catalog entry)., SeoCheckDefinition, auto_fixable_checks(), _c(), get_check(), list_checks(), Return the catalog definition for a check code (raises KeyError if unknown)., Return all check definitions. (+3 more)
...
### Community 343 - "brain_failover.py"
Cohesion: 0.11
...
### Community 349 - "test_failover_silent_exhaustion.py"
Cohesion: 0.06
...
### Community 353 - "TestAuthAndTaskOwnership"
Cohesion: 0.07
Nodes (18): skip, Test iteration 7 features: - POST /api/tasks/ auto-assigns an available agent…, Test runtime start/stop endpoints return informational payloads in remote…, POST /runtimes/{id}/start should return 200 with informational payload (not 500), POST /runtimes/stop-all should return 200 with informational payload, Test authentication and task creation with owner assignment, Test provider configuration including anthropic-universal, GET /api/providers should include anthropic-universal provider (+10 more)
...
Cohesion: 0.07
Nodes (46): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), list_seo_audits(), plan_seo_sprint(), BaseModel, get (+38 more)

### Community 356 - "test_dashboard_cache.py"
Cohesion: 0.13
Nodes (17): _cached(), _fast_count(), legacy_scheduler_list(), _produce_scheduler_jobs(), Single-flight TTL cache. Concurrent callers wait for the first producer., Count without materialising rows — prefers estimated_document_count., _clear_cache(), _CollWithEstimate (+9 more)

### Community 357 - "get_scheduler"
Cohesion: 0.09
Nodes (34): autonomy_tick(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_trigger(), Execute ONE pending task synchronously. Called by the cron workflow every 2…, Called by Cloudflare Cron every minute. Protected by CRON_SECRET header., Force-dedup and clean stale schedules from the durable store. Admin-only…, scheduler_force_cleanup() (+26 more)
...
### Community 375 - "test_brain_priority_scanner.py"
Cohesion: 0.09
...
Cohesion: 0.11
Nodes (25): packages/config/control_catalogue.py — the 109 operator-facing controls. The…, coerce(), _coerce_choice(), _coerce_number(), _coerce_toggle(), get_control(), Any, packages/config/control_registry.py — the platform-control API. The public… (+17 more)
...
Cohesion: 0.09
Nodes (29): apply_from_db(), apply_overrides(), _as_int(), load_overrides(), _policy_updates(), packages/config/control_overrides.py — DB-persisted overrides for platform…, Read the stored overrides. Returns ``{}`` on any storage failure. The fail-open…, Write *overrides* into ``os.environ`` and refresh dependent caches. Keys that… (+21 more)
...
### Community 433 - "TestBrainFailoverModelUpdates"
Cohesion: 0.08
Nodes (10): Test hook — rebuild the registry on next access., reset(), tests/test_daily_automation_2026_07_10.py — Daily automation tests…, Verify Llama 4 and Claude Sonnet 5 cross-provider aliases are registered., Verify the provider registry in brain_failover contains the 2026 model set., Verify new models are in the packages/ai/registry., The rotation must equal the catalogue, which the probe keeps honest., TestBrainFailoverModelAliases (+2 more)
...
### Community 455 - "test_phase4_runtime_resilience.py"
Cohesion: 0.11
...
### Community 456 - "test_memory_guard.py"
Cohesion: 0.12
...
Cohesion: 0.08
Nodes (28): DirectChatDoctor, PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, asyncio, When git is missing and no GitHub token is present, the doctor should report…, test_missing_git_and_token() (+20 more)
...
### Community 467 - "test_north_mini_code.py"
Cohesion: 0.10
...
Cohesion: 0.18
Nodes (17): build_execution_prompt(), build_planning_prompt(), build_tool_prompt(), build_verification_prompt(), Any, prompts.py — Prompt builders for planner, executor, and verifier roles., tests/test_prompt_injection_boundary.py — untrusted-content trust boundary.…, The Executor is told observations are DATA, not instructions. (+9 more)
...
Cohesion: 0.06
Nodes (24): _keepalive_self_ping(), lifespan(), _deferred_startup_cleanup(), Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free…, Start the Telegram bot (and keep-alive) inside the web process when enabled.…, _start_in_web_bot_tasks(), _register() (+16 more)

### Community 499 - "register_admin_gui"
Cohesion: 0.19
Nodes (20): Update or append a KEY=value line in the .env file., register_admin_gui(), dashboard(), diag_langfuse(), _guest_redirect(), login_page(), login_submit(), _redirect() (+12 more)
...
### Community 502 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 520 - "resolve_free_nvidia_brain"
Cohesion: 0.12
...
Cohesion: 0.13
Nodes (19): build_matrix(), _families(), main(), Any, scripts/generate_specialist_skill_matrix.py — Specialist × Skill matrix.…, Map family -> sorted list of test files that mention it (quoted token)., Return one row per family, derived entirely from code., render_markdown() (+11 more)
...
### Community 528 - "TaskStore"
Cohesion: 0.05
Nodes (57): _is_duplicate_key_error(), Exception, Create a task. Deduplicates by source_id if set (Charter G3). If a task with…, Fetch a task by ID. If owner_id is set, enforces ownership., Return the task previously created for an external ``source_id`` (e.g.…, Return BLOCKED tasks that are candidates for auto-retry., Return task counts keyed by ``agent_id`` for the requested statuses., Return counts per status for a user's tasks. Mirrors ``list_for_user``: by… (+49 more)
...
### Community 533 - "test_gh_brain_failover.py"
Cohesion: 0.10
...
### Community 534 - "_mock_provider_records"
Cohesion: 0.12
...
Cohesion: 0.11
Nodes (7): Register the built-in agent tools that are always available., Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_builtin_tools(), _register_web_reach_tools(), get_web_reach(), Module-level singleton, mirroring `agent.capability_registry.get_tool_registry`., test_capability_registry_exposes_web_reach_tools()
...
### Community 574 - "test_agent_coordinate.py"
Cohesion: 0.20
...
### Community 582 - "tasks/models.py"
Cohesion: 0.04
Nodes (62): Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest, ExecutionApprovalRequest (+54 more)
...
Cohesion: 0.03
Nodes (79): AgentRunner, _run(), _check_extra_kwargs(), _enforce_signature(), Any, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin… (+71 more)
...
### Community 594 - "test_source_id_index_is_partial_not_sparse"
Cohesion: 0.40
Nodes (3): asyncio, The unique index must be built with partialFilterExpression, never sparse. A…, test_source_id_index_is_partial_not_sparse()
...
### Community 616 - "get_ceo_dispatcher"
Cohesion: 0.11
...
### Community 620 - "test_brain_failover_413.py"
Cohesion: 0.15
Nodes (6): _clean_env(), _FakeAsyncClient, _FakeResponse, Regression test: HTTP 413 (Payload Too Large) must fail over to the next…, Returns 413 for every call to a Groq URL, success for anything else., test_413_fails_over_to_next_provider_after_one_attempt()

### Community 621 - "brain.py"
Cohesion: 0.19
Nodes (15): BrainResolution, get_provider_role_tags(), _host_is_openai_compatible(), _norm(), _pick_from_records(), _build(), _prio(), Any (+7 more)
...
### Community 628 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
### Community 648 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 650 - "Request"
Cohesion: 0.07
Nodes (33): middleware, admin_logout(), check_rate_limit(), _enforce_rate_limit(), _get_admin_identity_from_request(), _is_freebuff_unlimited(), is_rate_limit_exempt(), ollama_api() (+25 more)
...
### Community 658 - "test_telegram_notification_suppression.py"
Cohesion: 0.14
Nodes (10): True when outbound Telegram sends must be suppressed. Tests must never page a…, _telegram_sends_suppressed(), test_send_guard_holds_under_pytest(), test_send_guard_opt_in_override(), Regression: tests must never page a human via Telegram. The self-heal…, PYTEST_CURRENT_TEST is set during every test, so sends are suppressed., The actual escalation path must not page a human when run under pytest., test_opt_in_override() (+2 more)
...
Cohesion: 0.03
Nodes (70): main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), _is_safe_url(), _looks_like_bot_challenge(), BeautifulSoup, True if the HTML is an anti-bot/CAPTCHA interstitial rather than content. Used… (+62 more)
...
Cohesion: 0.06
Nodes (53): build_ceo_router(), ceo_status(), get_goal(), list_goals(), redrive_goal(), run_sweep(), Any, APIRouter (+45 more)
...
### Community 724 - "ApplyReviewAgent"
Cohesion: 0.22
...
### Community 728 - "verify_service_token"
Cohesion: 0.15
...
### Community 732 - "analyze_qualitative"
Cohesion: 0.12
...
### Community 734 - "NotificationDispatcher"
Cohesion: 0.10
Nodes (18): NotificationDispatcher, _send(), _send(), Any, Routes background task results to configured notification channels. Currently…, Callback for BackgroundAgent.on_task_complete. Dispatches task result…, Send notification to configured Telegram chat IDs., Proactively push a Telegram approval-gate message with inline buttons. Sent… (+10 more)
...
### Community 736 - "test_llm_router_compat.py"
Cohesion: 0.18
Nodes (12): Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on…, should_use_router(), Backwards compatibility of the routing layer (ADR-008 §8, Golden Rule). The…, With the flag off, the shim must not even import the new router., Tool calling is the compatibility case most likely to break silently., routed(), test_flag_is_read_per_call_so_rollback_needs_no_restart(), test_legacy_path_runs_when_the_flag_is_off() (+4 more)
...
### Community 739 - "test_omniroute_provider.py"
Cohesion: 0.15
...
### Community 777 - "test_swarm_concurrency.py"
Cohesion: 0.15
...
### Community 778 - "TestAnthropicPayloadStructuredOutput"
Cohesion: 0.29
...
### Community 818 - "test_agent_readiness_audit.py"
Cohesion: 0.18
...
### Community 822 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 831 - "_require_admin"
Cohesion: 0.05
Nodes (38): clear_cost_attribution(), clear_response_cache(), get_brain_policy_route(), get_catalog_models_route(), _maybe_boot_purge(), patch_brain_policy_route(), purge_backlog(), _purge_backlog_core() (+30 more)
...
### Community 856 - "test_agent_free_brain.py"
Cohesion: 0.09
...
### Community 894 - "TestDeprecationWarnings"
Cohesion: 0.17
...
### Community 897 - "TestAnthropicWorkspaceIdCapture"
Cohesion: 0.33
...
Cohesion: 0.26
Nodes (10): _parked_task(), asyncio, Telegram inline-button callbacks for the task pre-execution gate. Regression:…, A non-admin tapping Approve must get a modal (show_alert), not silence.…, store(), test_approve_button_approves_and_requeues(), test_callback_parser_routes_task_prefix(), test_non_admin_tap_gets_visible_alert_not_silent_toast() (+2 more)
...
Cohesion: 0.06
Nodes (22): Check whether Docker is available and report the adapter's runtime health.…, Available iff config resolves AND the SDK is importable. Never raises — a…, GooseAdapter, Any, Adapter for Goose — TIER 2 general-purpose local runtime., Health status snapshot for a runtime., RuntimeHealth, Return the last-known health for *runtime_id* (may be stale). (+14 more)
...
### Community 1020 - "rate_limiter.py"
Cohesion: 0.18
...
### Community 1023 - "test_hermes_server.py"
Cohesion: 0.18
...
### Community 1035 - "test_doctor_coding_brain.py"
Cohesion: 0.32
...
### Community 1134 - "test_autonomous_agency_e2e.py"
Cohesion: 0.12
...
### Community 1141 - "commercial_equivalent.py"
Cohesion: 0.50
...
### Community 1178 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1238 - "_wfo_owned_run_or_404"
Cohesion: 0.33
...
### Community 1304 - "effective_value"
Cohesion: 0.40
...
### Community 1305 - "._resolve_merge_decision"
Cohesion: 0.40
...
### Community 1312 - "_provider_env_value"
Cohesion: 0.50
Nodes (4): _provider_env_value(), provider_key_rotation_enabled(), Read a per-provider env var, tolerating dashes in the provider id. Provider ids…, True when the operator has explicitly opted *this provider* into using more…
...
- **3517 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3512 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14307 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **171 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `Task`, `TestNormalizeResponseFormat`, `.run`, `Agency`, `models/company_graph.py`, `TestRuntimeControl`, `client`, `test_llm_router_queue_cache.py`, `test_doctor_coding_brain.py`, `test_llm_router_strategies.py`, `ImprovementLoop`, `llm/router.py`, `PolicyEngine`, `test_brain_patch_service_token.py`, `test_operational_incidents.py`, `TaskStore`, `resolve_active_brain`, `test_gh_brain_failover.py`, `SelfHealingAgent`, `_mock_provider_records`, `router_factory`, `TaskExecutionCoordinator`, `ClaudeCodeAdapter`, `E2BSandboxSession`, `_start_autonomy_loops`, `MCPClient`, `test_provider_router.py`, `test_failover_client_shared.py`, `Settings`, `test_company_api.py`, `test_model_router.py`, `WorkflowRun`, `test_langfuse_agency_wide.py`, `_Recorder`, `_redact_for_notification`, `test_telegram_webhook.py`, `AgentSwarm`, `TokenBudget`, `BrainWatchdog`, `cost_tracker.py`, `services/seo_audit.py`, `ChatHistoryStore`, `get_task_store`, `tasks/models.py`, `AgentRunner`, `ai/router.py`, `build_governance_router`, `test_openclaw_endpoints.py`, `config.py`, `TestClient`, `test_task_pre_execution_gate.py`, `get_registry`, `test_sam_livekit.py`, `test_procedural_memory.py`, `test_sam_voice.py`, `test_startup_warmup.py`, `PrimeAgentAdapter`, `test_llm_router_e2e.py`, `SQLiteStore`, `clear_cooldowns`, `get_ceo_dispatcher`, `e2e/test_browser.py`, `test_autonomy_status.py`, `test_brain_failover_413.py`, `test_knowledge_sync.py`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `test_task_service_failed_comment.py`, `test_tasks_reconciler_todo_requeue.py`, `KeyStore`, `test_context_rulebook.py`, `ArtifactStore`, `HermesAdapter`, `UserRole`, `TaskPriority`, `TestHarnessAdapter`, `models/seo_audit.py`, `test_task_store_fails_loud_in_production.py`, `_step`, `captured`, `UsageEvent`, `test_trend_watcher.py`, `tests/conftest.py`, `claim`, `test_new_features_e2e.py`, `get_feature_matrix`, `TestEstimateTokensForMessages`, `test_response_cache.py`, `test_trend_scoping.py`, `TestChatHistoryStore`, `LLMRouter`, `test_agent_tool_governance.py`, `test_changelog_check_workflow.py`, `test_e2b_data_flow.py`, `test_openclaw_gateway.py`, `OllamaCircuitBreaker`, `_job_text`, `test_ceo_router.py`, `test_features_api.py`, `test_video_transcript.py`, `TrendWatcher`, `test_portfolio_intake.py`, `SchedulerStore`, `ResearchOrchestrator`, `_Budget`, `WorkflowBuildRequest`, `test_telegram_approval_gate.py`, `WorkflowRun`, `NotificationDispatcher`, `test_llm_router_compat.py`, `test_task_brain_preflight.py`, `TestClient`, `self_healing.py`, `ContextPruner`, `api_keys_for`, `test_control_plane_api.py`, `test_agile_api.py`, `test_app_settings.py`, `test_persistent_memory.py`, `SecurityScanner`, `test_task_clarification.py`, `test_output_filter.py`, `test_platform_controls.py`, `test_brain_failover.py`, `controls_app`, `test_colibri_brain_shim.py`, `test_brain_availability_doctor.py`, `OrchestratorQueue`, `test_nvidia_model_discovery.py`, `session_retro.py`, `isolated_telegram_config`, `analyze_page`, `model_discovery.py`, `parse_event_stream`, `test_scheduler_hydration_bounded.py`, `TrafficDirector`, `test_agent_free_brain.py`, `test_catalogue_probe.py`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_telegram_auto_approve.py`, `test_provider_enable_disable.py`, `TestMongoGate`, `TestAuthAndTaskOwnership`, `test_skill_registry_boot_refresh.py`, `test_dashboard_cache.py`, `_start_ceo_agency`, `_undeclared`, `test_process_quick_note_workflow.py`, `test_provider_state_durability.py`, `test_service_token.py`, `test_telegram_task_callback.py`, `test_harness_spec.py`, `test_freebuff_bot.py`, `test_agency_fix.py`, `_Recorder`, `test_webui_provider_priority.py`, `get_store`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `test_telegram_diag_endpoint.py`, `PreflightReport`, `TestTheWorkflowIsSafeAndReadOnly`, `test_ping.py`, `RuntimeHealth`, `test_voice_pipeline.py`, `test_kimi_bridge_server.py`, `rate_limiter.py`, `TestCountTokensEndpoint`?**
  _High betweenness centrality (0.119) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `.run`, `proxy.py`, `Agency`, `FreeBuffAgent`, `AdaptiveHalter`, `test_agent_runner.py`, `PolicyEngine`, `ReactScratchpad`, `TaskSpec`, `BackgroundAgent`, `test_governance_api.py`, `test_governance_sandbox.py`, `E2BSandboxSession`, `ContextManager`, `CEODispatcher`, `MCPClient`, `loop.py`, `test_failover_client_shared.py`, `workflow_orchestrator.py`, `WorkflowRun`, `test_agent_tool_governance.py`, `UserMemoryStore`, `TokenBudget`, `test_empirical_verify.py`, `WorkspaceTools`, `test_backend_server_features.py`, `InternalAgentAdapter`, `StuckDetector`, `coordinator.py`, `MultiAgentSwarm`, `TestAgentLoopMCPIntegration`, `test_agent_free_brain.py`, `LocalWorkspace`, `test_agent_chat_integration.py`, `get_ceo_dispatcher`, `direct_chat.py`, `test_brain_failover_413.py`, `GitHubTools`, `test_verification_strategies.py`, `test_autonomous_agency_e2e.py`, `ContextPruner`, `TestDeprecationWarnings`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `backend/server.py`, `settings.py`, `TestAnthropicPayloadStructuredOutput`, `failover_client.py`, `test_all_providers_discovery.py`, `test_bedrock_provider.py`, `TestLegacyRouterServerFallback`, `test_chat_mode_regressions.py`, `test_provider_router.py`, `test_colibri_brain_shim.py`, `test_bedrock_live.py`, `NvidiaProvider`, `TestRouterIntegration`, `test_kimi_bridge_provider.py`, `TestLegacyRouterCacheTTL`, `ai/router.py`, `is_anthropic_base_url`, `TrafficDirector`, `test_implement_agent_routing.py`, `_payload`, `clear_cooldowns`, `direct_chat.py`, `test_anthropic_refusal_fallback.py`, `implement_agent.py`?**
