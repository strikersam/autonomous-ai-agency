...
## Corpus Check
- 1482 files · ~2,158,474 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30101 nodes · 56252 edges · 1271 communities (1097 shown, 118 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3326 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `26c46d66`
...
## Communities (1271 total, 118 thin omitted)
...
Cohesion: 0.02
Nodes (229): admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, BrainTestRequest (+221 more)

### Community 2 - "TaskStore"
Cohesion: 0.03
Nodes (100): classify_domain(), agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Return the best-matching domain for a task title+description., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, _coerce_ts(), Enum, str (+92 more)

### Community 3 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (114): CacheManager, cosine_similarity(), _Entry, LRUCache, Any, T, Live (unexpired) entries — used by the semantic layer's scan., Cosine similarity between two vectors, 0.0 when either is degenerate. (+106 more)
...
Cohesion: 0.02
Nodes (92): KnowledgeItem, Find a website by its URL., Find a repository by its URL., Find knowledge items matching any of the given tags., Structured knowledge about the company, systems, or processes., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name. (+84 more)

### Community 6 - "TaskSpec"
Cohesion: 0.03
Nodes (123): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, TaskResult, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor. (+115 more)

### Community 7 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (93): _complexity_rank(), _merge_changed_files(), Reset the singleton (test helper)., Collect changed_files across all specialists into a single de-duped list., reset_ceo_dispatcher(), _should_fan_out(), ClassifyOutput, get_ceo_fallback_stats() (+85 more)

### Community 8 - "ExecutionRequest"
Cohesion: 0.02
Nodes (132): Execute work through the 11-phase golden path., Return orchestrator queue depth, active runs, and supervisor state (#522)., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_execute(), workflow_orchestrator_status(), get_orchestrator_checkpoint_store(), get_orchestrator_queue() (+124 more)
...
### Community 10 - "Task"
Cohesion: 0.03
Nodes (86): Task, ApprovalCheckpoint, Any, field_validator, Full task/issue document., Update the updated_at timestamp., Human approval gate in a task's execution., Task (+78 more)

### Community 11 - "Usage"
Cohesion: 0.04
Nodes (56): packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC, Any, AsyncClient, Response (+48 more)

### Community 12 - "ImprovementLoop"
Cohesion: 0.04
Nodes (89): DetectedIssue, ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity, _now(), Any, Enum (+81 more)
...
Cohesion: 0.06
Nodes (69): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, A subtask's full history: what it is, and every attempt at it., SubtaskRecord, Enum, str, Implementer tiers, ordered cheapest-and-narrowest to most-capable. A subtask…, Tier (+61 more)

### Community 17 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (122): _note_phase_end(), _note_phase_start(), Open a tracked phase, returning the token that closes it. Never raises., Close the tracked phase *token* opened above. Never raises., _diagnose_and_file(), _file_incident(), _format_incident(), gather_render_evidence() (+114 more)
...
Cohesion: 0.04
Nodes (104): _decompose_into_subtasks(), services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, Build a single-task sub-task list (low-complexity fast path)., Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, _runtime_id_for_role(), _single_specialist_task(), build_subtask_brief() (+96 more)

### Community 22 - "brain_config.py"
Cohesion: 0.03
Nodes (118): _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), invalidate_brain_config_cache(), is_north_mini_code_default() (+110 more)
...
Cohesion: 0.04
Nodes (71): build_docker_run_argv(), detect_backend(), DockerBackend, E2BBackend, load_profiles(), LocalBackend, Any, Path (+63 more)
...
Cohesion: 0.07
Nodes (56): _apply_env_overrides(), _apply_key_env(), _build(), CacheConfig, _coerce(), config_dir(), _env_key_names(), expand_env() (+48 more)
...
### Community 31 - "Agency"
Cohesion: 0.06
Nodes (42): Agency, AgencyCycleResult, AgentDirective, AgentRole, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context() (+34 more)
...
Cohesion: 0.08
Nodes (74): BrainFailoverExhausted, failover_chat_completion(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, Run one chat completion across the brain-failover chain. Tries each healthy…, _free_tier(), _hit_ids(), _many_providers() (+66 more)
...
Cohesion: 0.05
Nodes (53): loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Condense a tool result down to what belongs in an audit row. A tool result can…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _summarise_tool_result(), _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes… (+45 more)
...
Cohesion: 0.03
Nodes (75): ApprovalPolicy, BusinessSystem, Company, CompanyCreateRequest, CompanyGraph, CompanyGraphResponse, CompanyGraphSnapshot, CompanyUpdateRequest (+67 more)
...
Cohesion: 0.04
Nodes (69): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep (+61 more)
...
Cohesion: 0.04
Nodes (45): _accumulate_usage(), _assistant_messages(), _child_env(), _iter_events(), _message_text(), parse_event_stream(), ParsedRun, PrimeAgentAdapter (+37 more)

### Community 50 - "get"
Cohesion: 0.03
Nodes (84): auth_me(), auto_recommend_skills(), cost_attribution_stats(), cost_table(), discover_remote_skills(), get_brain_policy_route(), _get_github_token(), get_github_tree() (+76 more)
...
Cohesion: 0.05
Nodes (67): AgentPhaseError, Exception, Raised when a named agent phase (planning, verification, etc.) fails., Log start/elapsed for one phase of the plan/execute/verify loop. A task that…, _timed_phase(), _make_runner(), MonkeyPatch, Path (+59 more)
...
### Community 75 - "test_brain_priority_scanner.py"
Cohesion: 0.10
...
Cohesion: 0.04
Nodes (47): E2BAdapter, Any, TaskResult, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, _kill_and_reap() (+39 more)
...
### Community 79 - "test_llm_router_disabled.py"
Cohesion: 0.09
Nodes (39): _disabled_ids(), The operator kill-switch set, or empty when it cannot be read., auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The… (+31 more)
...
Cohesion: 0.04
Nodes (74): _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody, Shared purge implementation for the admin endpoint and the boot hook., Submit a quick-note URL or instruction from the dashboard FAB., Reset the singleton (for tests)., reset_failover_manager(), _blocked_retire_age_sec() (+66 more)
...
Cohesion: 0.05
Nodes (60): quote_command(), packages/governance/sandbox.py — governed, disposable execution environments.…, Shell-quote *parts* into a single command string. Used when a caller has an…, Available iff config resolves AND the SDK is importable. Never raises — a…, e2b_status(), Return the E2B sandbox integration status for the ProvidersScreen badge. Does…, e2b_enabled(), E2BConfig (+52 more)
...
Cohesion: 0.02
Nodes (135): _load(), main(), int, _describe_registry(), FailoverResult, _log_exhaustion(), _log_recovery(), _paid_reserve() (+127 more)

### Community 98 - "telegram_bot.py"
Cohesion: 0.06
Nodes (64): project_session_cost(), Project costs for a typical coding session. Args: avg_session_latency_ms: Total…, Convenience: resolve all four role models for a component. Returns a dict with…, resolve_component_role_models(), get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview() (+56 more)
...
### Community 103 - "get_improvement_loop"
Cohesion: 0.05
Nodes (70): get_improvement_loop(), _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord (+62 more)
...
### Community 117 - "_try_provider"
Cohesion: 0.07
Nodes (32): _auto_disable(), _build_request(), _disable_unless_key_serves_other_models(), _is_billing_refusal(), _is_ollama(), _key_pool(), _parse_success(), _provider_keys() (+24 more)
...
Cohesion: 0.12
Nodes (24): get_brain_preference(), Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, Operator can set BRAIN_PREFERENCE=colibri without it being rejected., test_get_brain_preference_accepts_colibri(), _admin_out(), AdminCommandBody, _anthropic_chat_payload(), _anthropic_text() (+16 more)
...
Cohesion: 0.02
Nodes (115): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., _clean_env() (+107 more)
...
Cohesion: 0.05
Nodes (65): allow_paid_brain(), BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), is_anthropic_model(), _norm(), _pick_from_records() (+57 more)
...
Cohesion: 0.09
Nodes (21): CommercialFallbackRequiredError, _get_director(), ProviderAttempt, ProviderFallbackError, ProviderRouter, Any, Response, RuntimeError (+13 more)

### Community 192 - "InternalAgentAdapter"
Cohesion: 0.06
Nodes (30): _best_cloud_primary_base(), InternalAgentAdapter, TaskResult, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Execute a TaskSpec using the internal AgentRunner and convert the agent's…, Create an isolated execution context for a single task. Tries ``git worktree… (+22 more)
...
### Community 206 - "test_cost_attribution.py"
Cohesion: 0.07
...
Cohesion: 0.06
Nodes (40): _dead_model_key(), _exponential_backoff_cooldown(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog(), _ollama_reasoning_effort(), provider_router.py — auto-generated module docstring (user-research skill scan)., Notify the brain watchdog of a provider call outcome (fire-and-forget).… (+32 more)
...
### Community 220 - "RenderFinding"
Cohesion: 0.07
Nodes (22): _latest_metric_value(), _note_recurrence(), Any, Pull the most recent numeric sample out of a ``get_metrics`` payload. Render…, Snapshot for ``/api/render/ops/status`` and the dashboard. ``self_heal_ready``…, Run one full scan and return every finding, filed or not. Never raises: a…, Scan, then file everything that is past its cooldown. The cooldown is claimed…, Map service ID → human name, degrading to the ID when unavailable. (+14 more)
...
Cohesion: 0.06
Nodes (43): CEODispatcher, CEOResult, _offload(), Any, Semaphore, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask., Aggregated output from a multi-specialist execution. (+35 more)
...
Cohesion: 0.07
Nodes (38): AgentRole, Review a slice via the Reviewer agent (different model from Coder). This is the…, When coder == reviewer model, swarm should log a warning., tests/test_workflow_models.py — Unit tests for workflow/models.py., TestApprovalGate, TestCheckRun, TestSlice, workflow/artifact_store.py — Durable artifact persistence. Artifacts are stored… (+30 more)
...
### Community 247 - "get_scheduler"
Cohesion: 0.05
Nodes (40): legacy_scheduler_get(), get_scheduler(), agent/scheduler.py — Scheduled Agent Jobs Cron-based job scheduler. Each job…, set_scheduler(), create_schedule(), delete_schedule(), get_schedule(), get_schedule_runs() (+32 more)
...
### Community 278 - "_execute_skill_impl"
Cohesion: 0.10
...
### Community 285 - "_resolve_user_github_token"
Cohesion: 0.08
Nodes (36): _builtin_provider_records(), _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), _in_container(), list_providers() (+28 more)
...
Cohesion: 0.06
Nodes (33): build_render_router(), Any, APIRouter, BaseModel, Exception, backend/render_router.py — Render platform view for operators and agents.…, Reject anyone who is not the agency admin., Map an MCP transport failure onto 503 rather than a 500. The distinction… (+25 more)
...
Cohesion: 0.08
Nodes (40): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), list_seo_audits(), plan_seo_sprint(), BaseModel, get (+32 more)
...
Cohesion: 0.09
Nodes (26): Paid-tier providers admitted to the chain and not yet attempted. Empty when the…, Pick the next provider, honouring the paid reserve. Once free/local providers…, _select_provider(), _untried_paid(), True when the provider can accept traffic right now., _FM, _P, Regression tests for a chain that fails silently. From a real incident:… (+18 more)
...
### Community 324 - "services/background.py"
Cohesion: 0.04
Nodes (61): get_agency(), set_agency(), _autonomy_bg_cycle(), autonomy_tick(), Background CEO cycle + task dispatch. Runs fire-and-forget., Execute ONE pending task synchronously. Called by the cron workflow every 2…, _await_hermes_ready(), _env_on() (+53 more)
...
Cohesion: 0.09
Nodes (16): requires_db, asyncio, Test Company Graph services., Test that storage service can be initialized., Test company CRUD operations - skipped as requires specific config., Test scanner service., Test that scanner service can be initialized., Test specialist service. (+8 more)
...
### Community 355 - "skill_bindings.py"
Cohesion: 0.09
...
### Community 418 - "TestBrainFailoverModelUpdates"
Cohesion: 0.08
...
### Community 478 - "test_session_retro.py"
Cohesion: 0.13
...
### Community 484 - "WorkspaceManager"
Cohesion: 0.19
...
### Community 490 - "TestStreamableHTTPTransport"
Cohesion: 0.13
...
### Community 503 - "test_agent_free_brain.py"
Cohesion: 0.07
Nodes (25): livenim, Resolve the free NVIDIA NIM brain from env, or ``None`` if unconfigured.…, resolve_free_nvidia_brain(), _FakeAsyncClient, _FakeResponse, _free_env(), Free-brain policy regression tests for the agent runtime (issue #656).…, Captures the POST url/json/headers so tests can assert the endpoint. (+17 more)
...
### Community 518 - "test_ai_insights.py"
Cohesion: 0.05
...
Cohesion: 0.04
Nodes (49): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate. (+41 more)
...
### Community 620 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
### Community 625 - "test_agent_tools.py"
Cohesion: 0.19
...
### Community 661 - "DailyDigestAggregatorTests"
Cohesion: 0.19
...
### Community 688 - "TestMCPToolsListCache"
Cohesion: 0.19
...
### Community 750 - "TestCatalogClaude5Models"
Cohesion: 0.10
...
### Community 756 - "register_user_research_tools"
Cohesion: 0.21
...
### Community 792 - "test_providers_live_e2e.py"
Cohesion: 0.27
...
### Community 824 - "_looks_unknown_model"
Cohesion: 0.20
...
### Community 867 - "DirectChatSession"
Cohesion: 0.20
...
### Community 996 - "cleanup_stale_jobs"
Cohesion: 0.31
...
### Community 1144 - "What's New"
Cohesion: 0.29
Nodes (7): 2026-06-16, 2026-06-25, 2026-06-26, 2026-07-04, 2026-07-05, 2026-07-09, What's New

### Community 1248 - ".create_company"
Cohesion: 0.33
Nodes (4): BusinessCategory, Company, Create an initial company graph for a new company. Args: company_id: Company ID…, Create a new company (idempotent — returns existing if domain matches). Checks…

### Community 1249 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 1252 - "Who is this for?"
Cohesion: 0.40
...
### Community 1264 - "Configuration reference"
Cohesion: 0.67
Nodes (3): Configuration reference, Provider priority chain, Running the brain on local Ollama (via a tunnel)

## Knowledge Gaps
- **3488 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3483 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13270 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **118 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `_Recorder`, `TaskStore`, `test_llm_router_queue_cache.py`, `test_ai_insights.py`, `TaskSpec`, `ExecutionRequest`, `Task`, `ImprovementLoop`, `AuditLog`, `TaskStatus`, `test_llm_router_strategies.py`, `test_operational_incidents.py`, `tests/conftest.py`, `E2BSandboxSession`, `OrchestratorCheckpointStore`, `brain_config.py`, `SelfHealingAgent`, `test_company_api.py`, `PolicyEngine`, `config.py`, `test_telegram_service_webhook.py`, `test_telegram_webhook.py`, `test_unit8_model_catalog.py`, `MCPClient`, `services/seo_audit.py`, `test_failover_client_shared.py`, `cost_tracker.py`, `test_model_router.py`, `test_brain_availability_doctor.py`, `AgentSwarm`, `PrimeAgentAdapter`, `BrainWatchdog`, `test_openclaw_endpoints.py`, `test_orchestrator_merge_decision.py`, `test_task_service_failed_comment.py`, `AgentJobManager`, `test_knowledge_sync.py`, `test_agent_tool_governance.py`, `test_sam_livekit.py`, `test_ceo_router.py`, `ChatHistoryStore`, `test_vision_routing.py`, `_FakeSandbox`, `SQLiteStore`, `E2BAdapter`, `WebsiteScanner`, `test_llm_router_disabled.py`, `test_telegram_auto_approve.py`, `ResearchTask`, `get_task_store`, `test_autonomy_status.py`, `validate_policy_text`, `test_local_brain_state.py`, `TestClient`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `test_procedural_memory.py`, `resolve_e2b_config`, `OutputFilter`, `llm/router.py`, `test_startup_warmup.py`, `get_improvement_loop`, `test_sam_voice.py`, `test_provider_router.py`, `test_colibri_brain_shim.py`, `e2e/test_browser.py`, `test_daily_automation_2026_07_09.py`, `SeoFixer`, `test_context_rulebook.py`, `ArtifactStore`, `test_issue_intake.py`, `run_task`, `UserRole`, `FreeBuffAgent`, `FeatureMaturity`, `test_kimi_bridge_server.py`, `_step`, `WorkflowRun`, `clear_cooldowns`, `.on_task_complete`, `test_trend_watcher.py`, `test_openclaw_gateway.py`, `TestHarnessAdapter`, `claim`, `persist_plan_spec`, `test_tasks_reconciler_todo_requeue.py`, `WorkflowBuildRequest`, `test_new_features_e2e.py`, `test_telegram_freebuff.py`, `KeyStore`, `OllamaCircuitBreaker`, `_job_text`, `_Budget`, `resolve_active_brain`, `SchedulerStore`, `TestAgentRunnerSafety`, `test_features_api.py`, `test_video_transcript.py`, `test_provider_state_durability.py`, `TrendWatcher`, `test_process_quick_note_workflow.py`, `test_portfolio_intake.py`, `NotificationDispatcher`, `platform_controls_router.py`, `TestEstimateTokensForMessages`, `TestClient`, `test_ephemeral_reaper.py`, `test_agile_api.py`, `test_app_settings.py`, `_clear_cache`, `test_task_clarification.py`, `test_e2b_task_wiring.py`, `ContextPruner`, `get_scheduler`, `test_implement_agent_routing.py`, `nvidia_models.py`, `test_persistent_memory.py`, `SecurityScanner`, `api_keys_for`, `BackgroundAgent`, `test_llm_router_e2e.py`, `test_platform_controls.py`, `_resolve_user_github_token`, `test_rate_limiter.py`, `test_nvidia_model_discovery.py`, `_resolve_push_token`, `test_catalogue_probe.py`, `TestNormalizeResponseFormat`, `TestMongoGate`, `analyze_page`, `services/background.py`, `OrchestratorQueue`, `test_mcp_governance.py`, `test_purge_backlog.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_e2b_data_flow.py`, `test_skill_registry_boot_refresh.py`, `TestSwarmRoleRouting`, `isolated_telegram_config`, `SeoCheckDefinition`, `test_dashboard_cache.py`, `test_provider_enable_disable.py`, `_undeclared`, `test_service_token.py`, `test_harness_spec.py`, `test_regression.py`, `test_agency_fix.py`, `_clean_director`, `test_webui_provider_priority.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_monitor_lib.py`, `test_v4_api.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_rate_limit_backoff_survives.py`, `test_telegram_diag_endpoint.py`, `test_session_retro.py`, `client`, `test_voice_pipeline.py`, `test_agent_free_brain.py`, `test_brain_patch_service_token.py`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `proxy.py`, `AdaptiveHalter`, `FreeBuffAgent`, `TaskSpec`, `ExecutionRequest`, `BackgroundAgent`, `TestAgentRunnerExecution`, `ReactScratchpad`, `E2BSandboxSession`, `test_ceo_micromanager.py`, `test_empirical_verify.py`, `ContextManager`, `PolicyEngine`, `Agency`, `MCPClient`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `loop.py`, `WorkspaceTools`, `AgentSessionStore`, `StuckDetector`, `test_daily_automation_2026_07_11.py`, `AgentJobManager`, `test_agent_runner.py`, `TokenBudget`, `test_agent_tool_governance.py`, `InternalAgentAdapter`, `test_governance_api.py`, `direct_chat.py`, `TestAgentRunnerSafety`, `LocalWorkspace`, `E2BAdapter`, `get_task_store`, `test_backend_server_features.py`, `UserMemoryStore`, `GitHubTools`, `CEODispatcher`, `ContextPruner`, `test_daily_automation_2026_07_09.py`, `test_agent_loop_delegates_to_the_shared_client`, `test_agent_free_brain.py`, `TestAgentLoopMCPIntegration`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `LLMRequest` connect `LLMRequest` to `llm/router.py`, `test_llm_router_queue_cache.py`, `Usage`, `test_llm_router_e2e.py`, `TestCatalogClaude5Models`, `test_llm_router_strategies.py`, `test_llm_router_disabled.py`, `TestCatalogFable51`, `AnthropicProvider`, `LLMRouter`, `TestAnthropicToolListCaching`, `test_anthropic_refusal_fallback.py`?**
