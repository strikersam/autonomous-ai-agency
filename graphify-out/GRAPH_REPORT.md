# Graph Report - autonomous-ai-agency  (2026-08-15)

## Corpus Check
- 1434 files · ~2,053,311 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28454 nodes · 53145 edges · 1191 communities (1076 shown, 115 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 3663 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `dc12b93b`
...
2. `AgentRunner` - 185 edges
3. `Task` - 153 edges
4. `TaskStore` - 122 edges
...
- `test_steps_are_independent_empty_files()` --uses--> `AgentRunner`  [INFERRED]
  tests/test_agent_runner.py → agent/loop.py
- `test_steps_are_independent_no_overlap()` --uses--> `AgentRunner`  [INFERRED]
  tests/test_agent_runner.py → agent/loop.py
- `test_steps_are_independent_with_overlap()` --uses--> `AgentRunner`  [INFERRED]
  tests/test_agent_runner.py → agent/loop.py
...
## Communities (1191 total, 115 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.02
Nodes (293): admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills() (+285 more)

### Community 1 - "_fixture"
Cohesion: 0.01
Nodes (125): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+117 more)

### Community 2 - "llm/router.py"
Cohesion: 0.02
Nodes (121): int, get_budget(), The process-wide budget tracker., CacheManager, cosine_similarity(), _Entry, get_cache(), LRUCache (+113 more)

### Community 3 - "proxy.py"
Cohesion: 0.03
Nodes (160): get_sam(), middleware, admin_control(), admin_login(), admin_logout(), admin_rotate_user(), admin_status(), AdminControlBody (+152 more)

### Community 4 - "ProviderRouter"
Cohesion: 0.03
Nodes (124): _acquire_provider_probe(), CommercialFallbackRequiredError, _dead_model_key(), _exponential_backoff_cooldown(), extract_openai_text(), _get_director(), is_commercial_provider(), _mark_model_dead() (+116 more)

### Community 5 - "company_api.py"
Cohesion: 0.03
Nodes (144): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+136 more)
...
Cohesion: 0.03
Nodes (123): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config() (+115 more)

### Community 9 - "WebsiteScanner"
Cohesion: 0.03
Nodes (99): DetectedSystem, Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., Result of a repository scan with detected stack and systems., A business system detected on a company's website or in their stack., Get the most confident evidence description., Inferred technology stack from website/repo analysis. (+91 more)
...
Cohesion: 0.03
Nodes (76): AgentDefinition, AgentStore, Any, field_validator, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents. (+68 more)

### Community 13 - "LLMRequest"
Cohesion: 0.03
Nodes (110): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, Return a system-prompt instruction that enforces JSON output. Anthropic's…, build_provider(), ProviderConfig, Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the…, resolve_kind() (+102 more)

### Community 14 - "test_llm_router_strategies.py"
Cohesion: 0.04
Nodes (100): count, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, HealthTracker, _Outcome, packages/llm/health.py — provider health tracking and circuit breaking. Two… (+92 more)

### Community 15 - "types.py"
Cohesion: 0.04
Nodes (69): _load(), main(), packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC (+61 more)

### Community 16 - "TaskSpec"
Cohesion: 0.04
Nodes (81): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., ClaudeCodeAdapter, json_safe() (+73 more)
...
Cohesion: 0.03
Nodes (84): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), get_ring(), KeyRing (+76 more)
...
### Community 25 - "Task"
Cohesion: 0.04
Nodes (75): Full task/issue document., Task, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, _escape_md_v1(), Escape Telegram Markdown-v1 reserved chars in free-text fields. Markdown-v1…, asyncio, A reject + human Retry starts a NEW approval cycle → fresh notification. (+67 more)

### Community 26 - "AgentSwarm"
Cohesion: 0.04
...
Cohesion: 0.03
Nodes (47): check_kwargs(), Any, agent/contract_enforcement.py — Runtime signature locking (J) Provides…, # NOTE: limit has a default so it is accepted; owner_id is keyword-only., Raise TypeError on unknown kwarg (runtime extra='forbid'). Args: kwargs: The…, # NOTE: limit is NOT locked — it is a legitimate optional param that does not, MergeDecision, _NoopStore (+39 more)

### Community 29 - "resolve_identity"
Cohesion: 0.04
Nodes (84): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, guard(), _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Evaluate *tool* before it runs. Returns ``(allowed, message, decision)``.…, Import the governance layer once, recording why if it is absent. (+76 more)
...
Cohesion: 0.04
Nodes (76): all_provider_ids(), _provider_ids_from_literal(), Resolve the model id for a component's role on a provider. Parameters…, Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, resolve_component_model(), CatalogActiveBrain, CatalogMirror (+68 more)

### Community 36 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (75): AuditEvent, AuditLog, Any, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer… (+67 more)
...
Cohesion: 0.04
Nodes (51): field_validator, A single occurrence of a check firing on a specific URL., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoIssueReportRow (+43 more)

### Community 41 - "resolve_active_brain"
Cohesion: 0.04
Nodes (86): allow_paid_brain(), BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), is_anthropic_model() (+78 more)

### Community 42 - "AgentRunner"
Cohesion: 0.05
Nodes (44): AgentRunner, Any, Path, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a…, Send chat messages and return the assistant's text output. If an…, Parse an LLM tool-call response, tolerating common malformations. Handles: 1.… (+36 more)
...
Cohesion: 0.04
Nodes (75): Replace the process-wide store. Tests only., reset_approval_store(), Replace the process-wide audit log. Tests only., reset_audit_log(), PolicyEngine, Path, Evaluates governed actions against a layered policy document. Thread-safe for…, Load a policy document from YAML, falling back to the default. Never raises:… (+67 more)
...
Cohesion: 0.04
Nodes (53): NotificationDispatcher, Any, Path, service_manager.py — Telegram & Notification Integration Extension Extends the…, Signal the bot to stop and wait for graceful shutdown., Routes background task results to configured notification channels. Currently…, Callback for BackgroundAgent.on_task_complete. Dispatches task result…, Send notification to configured Telegram chat IDs. (+45 more)

### Community 49 - "CompanyGraphStore"
Cohesion: 0.03
Nodes (37): Find a website by its URL., A company website with detected systems and stack inference., Website, Add a website to a company. Args: company_id: Company ID url: Website URL…, Initialize the service. Args: store: Optional CompanyGraphStore instance. If…, CompanyGraphStore, Delete a company and all its associated data., List companies with optional filtering. (+29 more)
...
Cohesion: 0.05
Nodes (70): Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _answer_callback(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost() (+62 more)
...
### Community 60 - "TestAdapterMetadata"
Cohesion: 0.04
...
### Community 63 - "get_workflow_orchestrator"
Cohesion: 0.05
Nodes (35): OrchestratorSupervisor, Any, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor(), get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper). (+27 more)
...
### Community 68 - "start_background_services"
Cohesion: 0.04
Nodes (57): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled() (+49 more)
...
### Community 72 - "test_mcp_registry.py"
Cohesion: 0.06
Nodes (44): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable(), _playwright_configured(), _probe_http() (+36 more)
...
Cohesion: 0.05
Nodes (53): failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Router-backed implementation of ``packages.ai.failover_client``'s entry point.…, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on…, Translate an OpenAI-shaped chat payload into an ``LLMRequest``. Unknown keys…, Render an ``LLMResponse`` as an OpenAI chat-completions body. Used by the… (+45 more)
...
### Community 81 - "ControlPlanePage.js"
Cohesion: 0.06
...
### Community 83 - "test_ai_insights.py"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (42): legacy_scheduler_delete(), get_scheduler(), agent/scheduler.py — Scheduled Agent Jobs Cron-based job scheduler. Each job…, set_scheduler(), create_schedule(), delete_schedule(), get_schedule(), get_schedule_runs() (+34 more)
...
### Community 91 - "seo_api.py"
Cohesion: 0.06
Nodes (53): _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), list_seo_audits(), BaseModel, get, post, SEO / GEO / AIO Audit API Router Endpoints for the world-class SEO audit engine… (+45 more)
...
Cohesion: 0.04
Nodes (51): _best_cloud_primary_base(), InternalAgentAdapter, _nvidia_provider_chain(), ProviderConfig, TaskResult, TaskSpec, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:… (+43 more)
...
### Community 101 - "test_brain_failover.py"
Cohesion: 0.06
Nodes (48): BrainFailoverManager, ProviderHealth, ProviderInfo, Any, Permit one probe call without claiming the provider succeeded. This is the…, Seconds until the soonest cooling provider is probeable again. ``None`` when no…, Record a provider failure — opens the circuit breaker on threshold., Map a requested model to the provider's equivalent. If the requested model is… (+40 more)

### Community 102 - "KeyStore"
Cohesion: 0.06
Nodes (40): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), _check_rate_limit(), default_keys_path(), issue_new_api_key(), KeyRecord (+32 more)
...
### Community 112 - "test_user_research_skill.py"
Cohesion: 0.07
...
Cohesion: 0.08
Nodes (32): _disable_unless_key_serves_other_models(), _models_to_try(), Order the models to attempt on *provider*, correcting a stale catalogue. Cache-…, Auto-disable *provider*, unless its key demonstrably serves other models. "No…, attempted(), Return every model of *provider_id* tried so far by a failed round., Drop every cached list and attempt record. For tests, and for an explicit…, reset_cache() (+24 more)
...
### Community 126 - "test_agent_runner.py"
Cohesion: 0.06
Nodes (53): AgentPhaseError, Exception, Raised when a named agent phase (planning, verification, etc.) fails., _make_runner(), MonkeyPatch, Path, Regression: a planning timeout must not surface as an empty "planning: ".…, Reproduce the exact error from the screenshot: slices present, goal absent. (+45 more)
...
### Community 129 - "call_llm"
Cohesion: 0.05
Nodes (40): _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), _build_auto_skill_guidance(), _build_provider_router(), _builtin_provider_records(), call_llm(), _chat_provider_policy() (+32 more)

### Community 130 - "CEODispatcher"
Cohesion: 0.07
...
### Community 133 - "get_task_store"
Cohesion: 0.06
Nodes (49): get_agency(), set_agency(), _autonomy_bg_cycle(), autonomy_tick(), _ensure_tasks_source_id_unique_index(), _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody (+41 more)
...
### Community 139 - "test_render_mcp.py"
Cohesion: 0.06
...
Cohesion: 0.07
Nodes (44): generate_backlog_retro(), generate_standup(), Derive a retrospective from the task tracker when no sprint is active. DONE /…, Build a :class:`StandupReport` from ``.claude/state/active-tasks.md``. Reads…, InitiativeStatus, _bug_scores(), _clean(), _default_repo() (+36 more)
...
Cohesion: 0.07
Nodes (46): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), packages.config — single source of truth for all configuration. Every module…, packages/config/settings.py — typed configuration. This is the ONLY module in… (+38 more)

### Community 148 - "failover_client.py"
Cohesion: 0.06
Nodes (44): _auto_disable(), _describe_registry(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion(), _looks_unknown_model() (+36 more)
...
### Community 150 - "activation_api.py"
Cohesion: 0.09
...
### Community 155 - "brain_failover.py"
Cohesion: 0.06
Nodes (34): _disabled_from_mongo(), _disabled_from_sqlite(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _mongo_db(), _mongo_enabled(), _mongo_unavailable() (+26 more)
...
### Community 163 - "test_daily_2026_07_27.py"
Cohesion: 0.07
...
Cohesion: 0.06
Nodes (44): backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), brain_failover_status(), brain_providers(), _openclaw_instructions(), openclaw_reverse_proxy(), openclaw_status() (+36 more)
...
### Community 180 - "test_ceo_router.py"
Cohesion: 0.08
...
### Community 181 - "models/seo_audit.py"
Cohesion: 0.09
Nodes (26): Run the repo-aware auto-fixer against this company's workspace checkout.…, Root directory under which repo fixes are allowed to operate., run_seo_fixes(), _workspace_root(), FsPath, BaseModel, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, Snapshot of one crawled page with the on-page facts the checks used. (+18 more)

### Community 182 - "emit_chat_observation"
Cohesion: 0.09
Nodes (36): CommercialEquivalent, estimate_commercial_equivalent_usd(), get_prices(), _load_from_env(), _parse_mapping(), Any, Map each local Ollama model name to a commercial API reference price (USD per…, compute_request_cost() (+28 more)
...
### Community 184 - "test_runtime_governance.py"
Cohesion: 0.11
Nodes (41): _governance_check(), Evaluate a runtime dispatch against policy; audit it either way. Returns…, _decision(), _engine(), Any, RoutingDecision, TaskResult, TaskSpec (+33 more)

### Community 185 - "test_brain_config_api.py"
Cohesion: 0.06
...
Cohesion: 0.08
Nodes (32): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep (+24 more)

### Community 188 - "test_v4_reliability_regression.py"
Cohesion: 0.06
Nodes (22): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Any, Structured, actionable preflight validation issue., Preflight result returned before a runtime task starts., Ensure the requested workspace exists and is writable., Validate task-specific prerequisites beyond runtime dependencies., Run runtime preflight validation before execution starts., Serialise adapter metadata (no secrets). (+14 more)
...
### Community 199 - "test_e2b_data_flow.py"
Cohesion: 0.07
...
### Community 206 - "_build_default_manager"
Cohesion: 0.07
...
### Community 207 - "test_self_heal_v2.py"
Cohesion: 0.08
Nodes (40): Reset the singleton (for tests)., reset_failover_manager(), _heal_brain_failover(), _heal_stuck_tasks(), _heal_telegram(), _heal_timestamps(), Any, services/self_heal.py — Autonomous self-healing for the agency. Runs periodic… (+32 more)
...
### Community 213 - "test_llm_router_disabled.py"
Cohesion: 0.09
Nodes (39): auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable., Persist a provider as disabled, through the store that already owns it. (+31 more)
...
### Community 218 - "tests/conftest.py"
Cohesion: 0.08
Nodes (37): _get_current_user_thunk(), _get_optional_user_thunk(), Request, get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., Item, Config (+29 more)

### Community 219 - "ExecutionRequest"
Cohesion: 0.08
Nodes (26): Called by APScheduler when a cron fires. Dispatches to the orchestrator., _scheduler_on_fire(), ExecutionRequest, PlanOutput, Generate a structured execution plan. Uses the LLM if available; falls back to…, Canonical request to execute work through the golden path. This is the ONLY…, PLAN phase: structured execution plan., Low complexity bypasses CEO and goes straight to AgentRunner. (+18 more)
...
### Community 222 - "ApprovalStore"
Cohesion: 0.07
Nodes (25): Event, publish(), packages/events/bus.py — In-process event bus. Loosely couples components via…, An event published on the bus., Subscribe to an event type., Publish an event to all subscribers., subscribe(), ApprovalRequest (+17 more)
...
Cohesion: 0.07
Nodes (26): best_vision_model(), get_registry(), ModelCapability, Return model registry, extended with ROUTER_EXTRA_MODELS env entries.…, Return the name of the best registered vision-capable model, or None. Prefers…, test_bedrock_haiku_4_5_in_registry(), test_bedrock_opus_48_in_registry(), test_bedrock_opus_4_6_v1_in_registry() (+18 more)
...
### Community 241 - "DirectChatDoctor"
Cohesion: 0.09
Nodes (31): DirectChatDoctor, get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor() (+23 more)
...
### Community 256 - "test_brain_availability_doctor.py"
Cohesion: 0.11
...
### Community 262 - "agents/agile_ceremonies.py"
Cohesion: 0.08
...
Cohesion: 0.12
Nodes (12): AnthropicProvider, Any, AsyncClient, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Anthropic's extended-thinking constraints on ``budget_tokens``: at least 1024,…, Accept both OpenAI and native Anthropic tool declarations. (+4 more)
...
### Community 286 - "get_skill_bindings"
Cohesion: 0.09
...
### Community 293 - "TestDetectedSystemPersistence"
Cohesion: 0.07
...
### Community 295 - "session_retro.py"
Cohesion: 0.13
...
### Community 304 - "test_direct_chat_async.py"
Cohesion: 0.12
...
Cohesion: 0.18
Nodes (23): An agent-delegable remediation work package derived from the findings. Findings…, SeoDelegationTask, Paragraph, _build_pdf(), Path, Render an executive-level PDF from the audit report dict., _appendix_full_findings(), _appendix_worst_pages() (+15 more)
...
### Community 322 - "test_provider_enable_disable.py"
Cohesion: 0.09
...
### Community 324 - "test_direct_chat_evolution.py"
Cohesion: 0.12
...
### Community 329 - "SeoCheckDefinition"
Cohesion: 0.09
...
### Community 334 - "test_phase4_runtime_resilience.py"
Cohesion: 0.11
...
### Community 346 - "autonomous_agent.py"
Cohesion: 0.09
...
### Community 368 - "test_rate_limit_backoff_survives.py"
Cohesion: 0.12
...
### Community 374 - "sam_livekit_worker.py"
Cohesion: 0.13
...
### Community 378 - "TestStreamableHTTPTransport"
Cohesion: 0.13
...
### Community 386 - "Any"
Cohesion: 0.12
Nodes (12): _as_list(), GroupPolicy, Any, Coerce a YAML scalar-or-list field into a list of strings. Operators write…, The resolved rules for one policy group, after ``extends`` flattening., Flatten ``extends`` chains and index rules for O(1) surface lookup., Flatten one group's ``extends`` chain into concrete rules. A cycle (``a extends…, Extract ``{surface: {effect: [patterns]}}`` from a policy section. A *declared… (+4 more)
...
### Community 392 - "test_webui_provider_priority.py"
Cohesion: 0.21
...
### Community 415 - "test_slop_gate.py"
Cohesion: 0.14
...
### Community 418 - "get_ceo_dispatcher"
Cohesion: 0.11
...
### Community 419 - "TestResolveBrainProvider"
Cohesion: 0.09
...
### Community 422 - "create_wiki_page"
Cohesion: 0.14
...
### Community 425 - "portfolio.py"
Cohesion: 0.10
Nodes (14): CapacityAllocation, InitiativeProgress, Enum, Agentic Portfolio Management — initiative prioritisation, capacity allocation,…, Percentage of linked sprint points completed., Result of fitting initiatives into a fixed capacity by WSJF priority., Total job size of initiatives that fit within capacity., Unused capacity after committing the selected initiatives. (+6 more)
...
### Community 430 - "self_heal_brain_and_unblock_tasks"
Cohesion: 0.10
...
### Community 462 - "test_workflow_models.py"
Cohesion: 0.13
...
### Community 474 - "ai/__init__.py"
Cohesion: 0.13
Nodes (12): ChatResponse, packages.ai — provider abstraction, model registry, and failover manager., ProviderManager, Any, Provider, packages/ai/manager.py — ProviderManager. Single entry point for all LLM calls.…, Coordinates provider selection, failover, and health., Return providers sorted by priority (lowest = highest priority). (+4 more)
...
### Community 488 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
### Community 510 - "test_agent_tools.py"
Cohesion: 0.19
...
### Community 515 - "knowledge_sync.py"
Cohesion: 0.21
...
### Community 523 - "FeatureUnavailableError"
Cohesion: 0.13
...
### Community 534 - "test_orchestrator_merge_decision.py"
Cohesion: 0.24
...
### Community 546 - "_valid_login_state"
Cohesion: 0.24
...
### Community 555 - "test_agent_chat_integration.py"
Cohesion: 0.23
...
### Community 582 - "_check_storage_health"
Cohesion: 0.13
...
### Community 587 - "ChatResponse"
Cohesion: 0.15
Nodes (5): CerebrasProvider, Provider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., ChatResponse, Standard response from a provider chat call.

### Community 588 - "HealthStatus"
Cohesion: 0.15
Nodes (6): OllamaProvider, Provider, Ollama provider adapter — local LLM inference., HealthStatus, Provider health check result., Check provider health.

### Community 589 - "Provider"
Cohesion: 0.16
Nodes (9): Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, Provider, ABC, packages/ai/provider.py — Provider abstraction interface. Every LLM provider…, Base interface every provider must implement. Implementations live in…, Lower = higher priority in the fallback chain., Calculate cost in USD for the given token counts. (+1 more)
...
### Community 598 - "test_brain_failover_413.py"
Cohesion: 0.14
...
### Community 604 - "analyze_qualitative"
Cohesion: 0.25
...
### Community 630 - "test_autonomy_pipeline_regressions.py"
Cohesion: 0.15
...
### Community 635 - "test_provider_state_durability.py"
Cohesion: 0.15
...
### Community 671 - "TestAgentRunnerSafety"
Cohesion: 0.19
...
### Community 707 - "test_providers_live_e2e.py"
Cohesion: 0.27
...
### Community 708 - "TestAnthropicPayloadStructuredOutput"
Cohesion: 0.29
...
### Community 741 - "response_cache.py"
Cohesion: 0.20
...
### Community 744 - "test_scanner_ssl_cert.py"
Cohesion: 0.25
...
### Community 748 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 771 - "TestModelsEndpointAliases"
Cohesion: 0.31
...
### Community 774 - "_execute_skill_impl"
Cohesion: 0.24
...
### Community 775 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 817 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 820 - "_seed"
Cohesion: 0.42
Nodes (9): _client(), asyncio, Task, TestClient, _seed(), test_admin_sees_system_owned_gated_tasks(), test_approving_removes_task_from_awaiting_list(), test_non_admin_sees_only_own_gated_tasks() (+1 more)
...
### Community 824 - "agent/output_filter.py"
Cohesion: 0.32
...
### Community 830 - "_build_direct_chat_schedule_suggestion"
Cohesion: 0.32
...
### Community 879 - "Continual Harness (`agent/harness_spec.py`)"
Cohesion: 0.25
...
### Community 895 - "test_runtimes_health_endpoint.py"
Cohesion: 0.25
...
### Community 908 - "platform_controls_router.py"
Cohesion: 0.29
...
### Community 909 - "_load_local_metrics_since"
Cohesion: 0.43
...
### Community 928 - "security_fix_agent.py"
Cohesion: 0.57
...
### Community 932 - "RateLimit"
Cohesion: 0.29
Nodes (3): RateLimit, Provider rate limit info., Return the provider's rate limits.

### Community 933 - "get_control"
Cohesion: 0.29
...
### Community 937 - "BackgroundServices"
Cohesion: 0.38
...
### Community 945 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 950 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 983 - "TestAgentRunnerExecution"
Cohesion: 0.33
...
### Community 987 - "start_in_process"
Cohesion: 0.33
...
### Community 1027 - "TestFailoverRegistryBaseUrls"
Cohesion: 0.40
...
### Community 1029 - "TestDirectChatNonBlocking"
Cohesion: 0.40
...
### Community 1034 - "_brain_provider_status"
Cohesion: 0.50
...
### Community 1062 - "JWTUserStateMiddleware"
Cohesion: 0.67
...
## Knowledge Gaps
- **3446 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3441 more)
...
- **Why does `_fixture()` connect `_fixture` to `llm/router.py`, `ProviderRouter`, `TaskStore`, `brain_config.py`, `WebsiteScanner`, `test_operational_incidents.py`, `TaskWorkflowService`, `test_llm_router_strategies.py`, `TaskSpec`, `SelfHealingAgent`, `test_openclaw_endpoints.py`, `test_llm_router_resilience.py`, `test_orchestrator_merge_decision.py`, `config.py`, `reset_kv_state`, `AgentSwarm`, `Task`, `TrafficDirector`, `resolve_identity`, `Agency`, `services/background.py`, `E2BSandboxSession`, `clear_override`, `test_failover_client_shared.py`, `test_unit8_model_catalog.py`, `services/seo_audit.py`, `resolve_active_brain`, `ImprovementLoop`, `UserRole`, `PolicyEngine`, `SQLiteStore`, `NotificationDispatcher`, `test_telegram_auto_approve.py`, `test_model_router.py`, `test_autonomy_status.py`, `workflow/api.py`, `telegram_bot.py`, `PrimeAgentAdapter`, `test_local_brain_state.py`, `test_telegram_diag_endpoint.py`, `ChatHistoryStore`, `get_workflow_orchestrator`, `TokenBudget`, `test_autonomous_agency_e2e.py`, `ResearchTask`, `gateway.py`, `TestClient`, `test_ai_insights.py`, `e2e/test_browser.py`, `get_scheduler`, `test_brain_failover_413.py`, `test_procedural_memory.py`, `claim`, `resolve_e2b_config`, `ArtifactStore`, `test_startup_warmup.py`, `KeyPool`, `test_colibri_brain_shim.py`, `KeyStore`, `TestHarnessAdapter`, `BrainWatchdog`, `test_context_rulebook.py`, `_StubProvider`, `run_task`, `test_openclaw_gateway.py`, `test_provider_state_durability.py`, `test_issue_intake.py`, `get_task_store`, `test_kimi_bridge_server.py`, `test_telegram_freebuff.py`, `test_new_features_e2e.py`, `test_e2b_adapter.py`, `BudgetTracker`, `_Budget`, `MCPClient`, `persist_plan_spec`, `TestAgentRunnerSafety`, `clear_cooldowns`, `test_daily_2026_07_27.py`, `test_e2b_task_wiring.py`, `OllamaCircuitBreaker`, `test_ceo_router.py`, `models/seo_audit.py`, `emit_chat_observation`, `test_sam_livekit.py`, `test_features_api.py`, `test_video_transcript.py`, `test_agile_api.py`, `test_app_settings.py`, `SchedulerStore`, `test_task_clarification.py`, `test_e2b_data_flow.py`, `test_self_heal_v2.py`, `TrendWatcher`, `test_llm_router_disabled.py`, `test_trend_watcher.py`, `ExecutionRequest`, `test_portfolio_intake.py`, `TestEstimateTokensForMessages`, `test_agent_free_brain.py`, `TestClient`, `rate_limiter.py`, `ContextPruner`, `DirectChatDoctor`, `OrchestratorQueue`, `test_sam_voice.py`, `test_persistent_memory.py`, `SecurityScanner`, `test_brain_availability_doctor.py`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `OutputFilter`, `test_platform_controls.py`, `TestMongoGate`, `test_mcp_governance.py`, `session_retro.py`, `test_purge_backlog.py`, `TestSwarmRoleRouting`, `analyze_page`, `test_direct_chat_async.py`, `parse_event_stream`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `test_direct_chat_evolution.py`, `SeoCheckDefinition`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `timedelta`, `test_phase5_doctor.py`, `api_keys_for`, `test_rate_limit_backoff_survives.py`, `test_service_token.py`, `test_harness_spec.py`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `test_regression.py`, `test_agency_fix.py`, `test_webui_provider_priority.py`, `test_dashboard_cache.py`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `get_ceo_dispatcher`, `test_knowledge_sync.py`, `client`, `allow_paid`, `TestFeaturesAPI`, `_FakeSandbox`, `test_langfuse_agency_wide.py`, `test_voice_pipeline.py`, `cost_tracker.py`, `TestNormalizeResponseFormat`, `test_company_api.py`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `call_llm`, `CEODispatcher`, `proxy.py`, `direct_chat.py`, `ReactScratchpad`, `ContextManager`, `TaskSpec`, `test_e2b_adapter.py`, `test_daily_automation_2026_07_11.py`, `test_governance_api.py`, `MCPClient`, `WorkflowRun`, `workflow_orchestrator.py`, `TestAgentRunnerSafety`, `E2BSandboxSession`, `StuckDetector`, `test_failover_client_shared.py`, `get_ceo_dispatcher`, `MultiAgentSwarm`, `test_agent_chat_integration.py`, `PolicyEngine`, `WorkspaceTools`, `test_direct_chat_async.py`, `AgentPlan`, `TokenBudget`, `UserMemoryStore`, `test_autonomous_agency_e2e.py`, `LocalWorkspace`, `test_brain_failover_413.py`, `TestAgentRunnerExecution`, `GitHubTools`, `AgentSessionStore`, `InternalAgentAdapter`, `test_agent_free_brain.py`, `test_empirical_verify.py`, `ContextPruner`, `loop.py`, `TestAgentLoopMCPIntegration`, `AdaptiveHalter`, `test_agent_runner.py`, `FreeBuffAgent`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `WorkspaceManager` connect `WorkspaceManager` to `test_workspace_isolation.py`, `.test_concurrent_create_same_session`, `validate_job_id`, `dry_clone_repo`, `WorkspaceManifest`, `.get_workspace`, `WorkspaceMetrics`, `validate_session_id`, `diagnostics.py`, `_safe_resolve`, `AgentJobManager`, `test_v4_reliability_regression.py`, `_hash_component`?**
...
- **Are the 104 inferred relationships involving `AgentRunner` (e.g. with `MultiAgentSwarm` and `AdaptiveHalter`) actually correct?**
  _`AgentRunner` has 104 INFERRED edges - model-reasoned connections that need verification._
...
- **Should `backend/server.py` be split into smaller, more focused modules?**
  _Cohesion score 0.016376546037562986 - nodes in this community are weakly interconnected._