# Graph Report - local-llm-server  (2026-09-23)

## Corpus Check
- 1532 files · ~2,233,734 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 32550 nodes · 66963 edges · 1302 communities (1115 shown, 187 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 6620 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5128a1ff`
...
- TestTheCopiesMayNotDriftFurther
- test_memory.py
- UserMemoryStore
- PreflightReport
- test_keepalive.py
...
1. `_fixture()` - 314 edges
2. `AgentRunner` - 233 edges
3. `Task` - 223 edges
4. `TaskStatus` - 143 edges
5. `ProviderRouter` - 140 edges
...
## Communities (1302 total, 187 thin omitted)

### Community 0 - "os"
Cohesion: 0.01
Nodes (256): Browser admin UI for login, service control, key management, and diagnostics., ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered) Runs the…, agent/background.py — Background Agent An always-on worker thread that…, Cached LLM Client wrapper. Drop-in wrapper around any LLM API call that…, Durable agent checkpointing for crash-recovery and resumption. Provides…, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata…, context_pruner.py — auto-generated module docstring (user-research skill scan). (+248 more)

### Community 1 - "TaskSpec"
Cohesion: 0.01
Nodes (304): abc, 3b. Hermes — **our own** Hermes server (in-repo), UI-wired ✅  (size: M), N2. Surface Hermes (and all runtimes) status in the Doctor/Runtimes UI ⬜  (size: S, risk: low), FastAPI dependency: require any authenticated user., require_authenticated(), kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter (+296 more)

### Community 2 - "backend/server.py"
Cohesion: 0.01
Nodes (421): set_skill_registry(), get_harness_adapter(), _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), AgentStatusEntry (+413 more)

### Community 3 - "build_llm_router"
Cohesion: 0.09
Nodes (34): Any, Translate an OpenAI-shaped chat payload into an ``LLMRequest``. Unknown keys…, request_from_openai_payload(), gateway_enabled(), Whether the OpenAI-compatible passthrough routes accept traffic. Off by…, Whether callers should route through ``LLMRouter`` (ADR-008 §8). Defaults to…, router_enabled(), _available() (+26 more)

### Community 4 - "Added"
Cohesion: 0.01
Nodes (253): get_agency(), set_agency(), get_sam(), _first_paragraph(), Return the first non-empty, non-heading line. Skips YAML frontmatter (--- ...…, BudgetExceededError, Exception, Reset token counters for all sessions (caps preserved). Called at the start of… (+245 more)

### Community 5 - "ImprovementLoop"
Cohesion: 0.04
Nodes (80): AgentRole, _parse_ceo_directives(), Enum, str, DetectedIssue, ImprovementLoop, ImprovementLoopState, IssueCategory (+72 more)

### Community 6 - "brain_config.py"
Cohesion: 0.02
Nodes (193): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), Architecture (per plan §3), Files touched, Hard constraints (from the plan) — all met, Implementation — DB-persisted, UI-switchable Brain (PR #824 follow-up), New files, Resolution precedence (+185 more)

### Community 7 - "company_api.py"
Cohesion: 0.02
Nodes (144): Short label for TUI display: {NAME}:{model}, account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck (+136 more)

### Community 8 - "Changed"
Cohesion: 0.04
Nodes (102): _fetch_github_quick_notes(), _gh_repo(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, Real handler — dispatches through AgentRunner when available. HARDENED (PR…, Public wrapper around _generate_plan for callers that want plan-only. Returns…, _fast_count(), Count without materialising rows — prefers estimated_document_count. (+94 more)

### Community 9 - "proxy.py"
Cohesion: 0.03
Nodes (170): Describes a single worker's task and constraints., WorkerSpec, set_quick_note_queue(), fastapi_middleware_cors, middleware, admin_control(), admin_create_key(), admin_create_user() (+162 more)
...
### Community 11 - "_fixture"
Cohesion: 0.01
Nodes (153): Drop the cached config (test hook)., reset(), Test hook — clear all key statistics., reset(), Drop the cached Mongo client, backoff, and read caches (tests)., reset_kv_state(), _clear_discovered_models(), _clear_response_cache() (+145 more)

### Community 12 - "services/background.py"
Cohesion: 0.06
Nodes (64): get_improvement_loop(), get_log_monitor(), set_log_monitor(), get_self_healing_agent(), set_self_healing_agent(), get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, get_trend_watcher() (+56 more)

### Community 13 - "TaskStatus"
Cohesion: 0.04
Nodes (79): Submit a quick note instruction. Creates a Task with task_type='quick_note'., v4_quick_notes_submit(), Helpers that turn scheduler and playbook activity into real tasks., Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, TaskStatus, Record the human decision on a task's **pre-execution** approval gate. A…, Re-run a task. Permissive across every non-active state so the dashboard Retry… (+71 more)

### Community 14 - "test_platform_controls.py"
Cohesion: 0.06
Nodes (44): apply_overrides(), _as_int(), _policy_updates(), packages/config/control_overrides.py — DB-persisted overrides for platform…, Write *overrides* into ``os.environ`` and refresh dependent caches. Keys that…, Re-read every ``settings`` attribute from the updated environment. Re-runs…, Push the *changed* routing keys into the live runtime manager. Only the keys…, Build the ``update_policy`` kwargs for exactly the *changed* keys. (+36 more)

### Community 15 - "test_agent_runner.py"
Cohesion: 0.04
Nodes (73): AgentPhaseError, _note_phase_start(), Exception, Raised when a named agent phase (planning, verification, etc.) fails., Log start/elapsed for one phase of the plan/execute/verify loop. A task that…, Open a tracked phase, returning the token that closes it. Never raises., _timed_phase(), _make_runner() (+65 more)

### Community 16 - "test_ceo_micromanager.py"
Cohesion: 0.08
Nodes (53): Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, build_subtask_brief(), Return the :class:`TierProfile` for *tier*, defaulting to MIDLEVEL., Render the self-contained brief handed to an executing agent. Every delegated…, Execution envelope for one tier. ``runtimes`` is a preference list handed to…, tier_profile(), TierProfile, assess_quality() (+45 more)

### Community 17 - "llm/context.py"
Cohesion: 0.09
Nodes (30): build_summariser(), summarise(), ContextManager, estimate_tokens(), FitResult, message_tokens(), prune(), Any (+22 more)

### Community 18 - "LLMRequest"
Cohesion: 0.03
Nodes (111): _merge_env_defaults(), ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, Add providers that are configured in the environment but absent from YAML. This…, Give every provider's default model a registry entry if YAML omitted it., _seed_model_defaults(), packages_llm_providers, build_provider() (+103 more)

### Community 19 - "tasks/store.py"
Cohesion: 0.06
Nodes (39): _priority_rank(), Any, tasks/store.py — MongoDB-backed task store. Falls back to in-memory storage…, Fail loud in production if no persistent storage is configured. Telegram…, List tasks for a specific user with optional filters. ``include_system``…, Return tasks queued for agent execution, highest-priority first. Ordered by…, Return BLOCKED tasks that are candidates for auto-retry., Return a shallow copy of ``doc`` without the heavy list fields. The Mongo path… (+31 more)

### Community 20 - "test_schedule_backlog_drain.py"
Cohesion: 0.09
...
### Community 21 - "Tier"
Cohesion: 0.05
Nodes (63): _coerce_subtasks(), decompose(), _env_flag(), _env_int(), _extract_json_object(), fallback_decomposition(), MicroManagerConfig, next_tier() (+55 more)

### Community 22 - "SeoAuditEngine"
Cohesion: 0.06
Nodes (38): BaseModel, field_validator, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance (+30 more)

### Community 23 - "test_governance_api.py"
Cohesion: 0.07
Nodes (45): What changed on merge, Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient, Tests for the governance HTTP surface and the AgentRunner integration. The…, Policy is a git-reviewed file. An HTTP mutation route would make "who changed… (+37 more)

### Community 24 - "looks_like_secret_file"
Cohesion: 0.03
Nodes (145): audit_drift(), _cmd_audit(), DriftReport, _grade(), load_registry(), load_registry_sync(), loop_readiness(), LoopRegistry (+137 more)

### Community 25 - "failover_chat_completion"
Cohesion: 0.04
Nodes (100): BrainFailoverExhausted, _Budget, failover_chat_completion(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, Shared attempt + wall-clock budget for one logical completion. Bounds the whole…, True when free/local providers have used everything but the reserve. Only the…, Run one chat completion across the brain-failover chain. Tries each healthy… (+92 more)

### Community 26 - "CEODispatcher"
Cohesion: 0.05
Nodes (54): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), Any, Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask. (+46 more)

### Community 27 - "webui/router.py"
Cohesion: 0.03
Nodes (68): get_local_model(), Return the local Ollama model name for a given Anthropic model name. ..…, livenim, allow_paid_brain(), is_anthropic_model(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, True when *model* names a paid Anthropic/Bedrock-Claude model. Covers native…, Resolve the free NVIDIA NIM brain from env, or ``None`` if unconfigured.… (+60 more)

### Community 28 - "test_scanner_headless.py"
Cohesion: 0.04
Nodes (26): 5. Input Validation, _is_blocked_host(), Cheap (no-DNS) SSRF check for headless-browser subrequests. A rendered page's…, _guard(), Tests for the scanner's headless-render fallback (JS-rendered / bot-protected…, The scan flow must invoke the render fallback when static detection is empty…, BuiltWith-style off-site identification: a CNAME chain that points at a known…, A scan must never hang past its wall-clock budget — a slow/blocked domain has… (+18 more)

### Community 29 - "Task"
Cohesion: 0.03
Nodes (86): Background dispatcher for task execution., Full task/issue document., Task, _is_outward_facing(), Any, Domain services for task workflow and runtime-backed execution., One-line human explanation of why this task needs human approval. Reads risk…, Best-effort label of who will run this task once approved. Returns… (+78 more)

### Community 30 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (103): contextvars, BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MergeDecision, MonitorOutput (+95 more)

### Community 31 - "test_repo_connection.py"
Cohesion: 0.11
...
Cohesion: 0.03
Nodes (98): simulate_policy(), R3 — Reach a verdict, and let that verdict be "reject" **[gate]**, fnmatch, guard(), _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Evaluate *tool* before it runs. Returns ``(allowed, message, decision)``.…, Import the governance layer once, recording why if it is absent. (+90 more)

### Community 33 - "WorkflowPhase"
Cohesion: 0.11
Nodes (18): str, Drives a Task through the execution state machine with persistence. Usage::…, Advance task through the full phase sequence. Resumes from…, Execute a single phase and advance to the next., Run the logic for ``phase`` and return the next phase. Handles both sync and…, Record a plan stub (concrete planning happens inside the agent loop)., Route to the best specialist agent by domain and capability., Run doctor checks before execution; block task if critical issues found. (+10 more)

### Community 34 - "MongoDBStore"
Cohesion: 0.04
Nodes (43): Security, Security, MongoDBStore, Any, ObjectId, MongoDB implementation of the Company Graph store. Uses Motor (async MongoDB…, Get or create the MongoDB database connection., Convert string ID to ObjectId. (+35 more)

### Community 35 - "failover_client.py"
Cohesion: 0.02
Nodes (128): Spawn a child AgentRunner for a delegated sub-task. When sub-agent configs are…, FailureCategory, E2: Classify a failure from its description text. Order matters:…, Classified failure types for targeted self-healing (E2)., ma(), Fixed, Fixed, Fixed (+120 more)

### Community 36 - "build_governance_router"
Cohesion: 0.04
Nodes (62): build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget(), get_metrics() (+54 more)

### Community 37 - "test_ceo_supervision.py"
Cohesion: 0.07
Nodes (52): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, A subtask's full history: what it is, and every attempt at it., SubtaskRecord, _goal(), asyncio, Tests for the CEO ledger, the supervised escalation loop, and the 24x7 sweeper.…, A storage outage must never take the agency down. (+44 more)

### Community 38 - "OnboardingService"
Cohesion: 0.06
Nodes (36): Result of a website scan with detected systems and stack inference., Inferred technology stack from website/repo analysis., StackInference, WebsiteScanResult, OnboardingService, Detect the Git provider from a repository URL. Args: repo_url: Repository URL…, Schedule a fire-and-forget background task, keeping a strong reference.…, Force-refresh the dynamic skill registry and log tech-stack recommendations.… (+28 more)

### Community 39 - "AgileSprint"
Cohesion: 0.06
...
### Community 40 - "test_bedrock_provider.py"
Cohesion: 0.07
...
Cohesion: 0.03
Nodes (71): AgentRunner, _run(), _check_extra_kwargs(), _enforce_signature(), Any, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Append an event to the durable session log if a store is wired in. (+63 more)

### Community 42 - "Added"
Cohesion: 0.04
Nodes (54): Build the complete enrichment block (tools + skills). Returns empty string when…, Inject available tools + skills catalog into the system message.…, note_phase_start(), Record that *phase* began; returns the token that ends it. Called from…, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Tool Parameters, Added, Tool Parameters (+46 more)

### Community 43 - "test_mcp_registry.py"
Cohesion: 0.08
...
### Community 44 - "ProviderConfig"
Cohesion: 0.05
Nodes (43): provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, _nvidia_provider_chain(), Build Nvidia NIM provider config from env. Empty list when key is absent., Regression tests for the agent loop's multi-provider failover wire format.…, Registry base URLs must produce routes the provider actually serves. (+35 more)

### Community 45 - "test_llm_router_resilience.py"
Cohesion: 0.04
Nodes (63): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), KeyRing, KeyState (+55 more)

### Community 46 - "Artifact"
Cohesion: 0.07
Nodes (25): AgentRole, When coder == reviewer model, swarm should log a warning., _fake_artifact(), TestSlice, _extract_slices_from_plan(), Execute, review, and verify a single slice. Returns the updated Slice. Raises…, Extract slice definitions from a plan.md artifact. Looks for sections matching:…, Artifact (+17 more)

### Community 47 - "MCPClient"
Cohesion: 0.08
Nodes (13): MCPClient, Any, Thin async MCP client with open/close circuit breaker. Thread-safe only within…, Full URL of the JSON-RPC endpoint this client posts to., Build the request headers shared by ``_rpc`` and ``notify``. ``Accept`` lists…, Attach the agent identity whose actions this client executes., Send a JSON-RPC notification (no ``id``, no response body expected). Used for…, Return the list of tools available on the MCP server. Implements tools/list TTL… (+5 more)
...
### Community 50 - "WorkflowOrchestrator"
Cohesion: 0.04
Nodes (48): Changed, Changed, Feature maturity — what's stable vs. beta, Actively wake every sleeping/circuit-open runtime. The default health service…, get_orchestrator_checkpoint_store(), Phase, Enum, Exception (+40 more)

### Community 51 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (78): BudgetTracker, classify(), GovernanceGate, _host_of(), Extract a hostname from a URL, or return a bare host unchanged., Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., Return the ``(surface, action)`` a tool call really represents. The *action* is… (+70 more)

### Community 52 - "ExecutionRequest"
Cohesion: 0.05
Nodes (39): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY…, reset_orchestrator() (+31 more)

### Community 53 - "SchedulerStore"
Cohesion: 0.06
...
### Community 54 - "Evidence"
Cohesion: 0.06
Nodes (15): Evidence, Evidence supporting a system detection., Replicates builtwith.builtwith() data-driven logic natively using the…, Detect systems from GitHub repository data. Args: repo_data: Repository data…, add_sys(), End-to-end resilience for bot-protected sites — the regression guard the user…, The critical guard: when both tiers hit a Cloudflare wall, the fallback returns…, Tier 1 hits the Cloudflare wall; the headless browser clears the automatic JS… (+7 more)

### Community 55 - "test_provider_router.py"
Cohesion: 0.04
Nodes (57): _acquire_provider_probe(), CommercialFallbackRequiredError, extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), RuntimeError, Try to acquire a distributed probe lock for *provider_id*. Returns True if this…, Release the probe lock for *provider_id*. (+49 more)

### Community 56 - "strategies.py"
Cohesion: 0.03
Nodes (101): count, Preferring local, Choosing one, Latency got worse, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig (+93 more)

### Community 57 - "test_model_catalog.py"
Cohesion: 0.04
Nodes (72): _build_base_url_env_from_yaml(), _build_default_base_url_from_yaml(), _build_display_names_from_yaml(), _build_key_env_from_yaml(), _build_tier_from_yaml(), get_provider_candidates(), get_provider_display_name(), get_provider_tier() (+64 more)

### Community 58 - "WebsiteScanner"
Cohesion: 0.04
Nodes (54): main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), Scan a website and save the results. Args: company_id: Company ID url: Website…, _is_safe_url(), _looks_like_bot_challenge(), Any (+46 more)

### Community 59 - "settings.py"
Cohesion: 0.05
Nodes (35): CerebrasProvider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NvidiaProvider, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., Ollama provider adapter — local LLM inference. (+27 more)

### Community 60 - "PrimeAgentAdapter"
Cohesion: 0.07
...
### Community 61 - "SQLiteStore"
Cohesion: 0.03
Nodes (61): DetectedSystem, Find a website by its URL., A business system detected on a company's website or in their stack., Get the most confident evidence description., A company website with detected systems and stack inference., Website, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model. (+53 more)

### Community 62 - "skill_bindings.py"
Cohesion: 0.08
Nodes (28): Enum, Status of a user story within a sprint., StoryStatus, 3. Dynamic, expandable roles (open registry, not a closed enum), BaseModel, Enum, str, services/skill_bindings.py — Runtime Skill Bindings for Specialist Agents Wires… (+20 more)

### Community 63 - "test_model_router.py"
Cohesion: 0.05
...
### Community 64 - "test_self_heal_v2.py"
Cohesion: 0.06
Nodes (54): Reset the singleton (for tests)., reset_failover_manager(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog(), _heal_stuck_tasks(), _heal_telegram() (+46 more)

### Community 65 - "models/company_graph.py"
Cohesion: 0.03
Nodes (96): bson, What this maps to (real code, not a generic scaffold), What already exists (don't rebuild), ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraph, CompanyGraphSnapshot (+88 more)

### Community 66 - "PortfolioManager"
Cohesion: 0.05
...
### Community 67 - "mcp_registry.py"
Cohesion: 0.09
Nodes (27): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, Playwright, The MCP registry — one catalogue, measured status, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable() (+19 more)

### Community 68 - "seo_portfolio_bridge.py"
Cohesion: 0.06
Nodes (44): plan_next_sprint(), The result of allocating portfolio capacity into a new sprint., Allocate ``capacity`` of WSJF-ranked initiatives into a new sprint. Creates one…, SprintPlan, Roadmap placement for an initiative (Now/Next/Later planning)., RoadmapHorizon, build_seo_roadmap(), delegation_plan_to_initiatives() (+36 more)

### Community 69 - "load_config"
Cohesion: 0.08
Nodes (43): _apply_key_env(), _build(), _coerce(), config_dir(), expand_env(), _fields_of(), _load_cache(), load_config() (+35 more)
...
### Community 75 - "test_all_features.py"
Cohesion: 0.04
...
### Community 77 - "test_backend_server_features.py"
Cohesion: 0.05
Nodes (53): _anthropic_headers(), _anthropic_payload(), _anthropic_response_text(), _auth_headers(), chat_completion_text(), _do(), list_openai_models(), LlmProviderConfig (+45 more)

### Community 78 - "FeatureMatrix"
Cohesion: 0.05
...
### Community 79 - "_StubProvider"
Cohesion: 0.08
Nodes (22): _mock_get(), _ok(), _handler(), asyncio, None means "could not find out" and must not read as "no models"., Otherwise every failed call re-asks a provider that is already down., ``cached_models`` is sync by contract — a chat call must not wait., A provider that lists nothing must not leave the chain with no model. (+14 more)

### Community 80 - "build_tool_prompt"
Cohesion: 0.09
Nodes (28): build_compaction_prompt(), build_execution_prompt(), build_planning_prompt(), build_tool_prompt(), build_verification_prompt(), Any, prompts.py — Prompt builders for planner, executor, and verifier roles., Context Plan — Issue #1427: quick-note:https://github.com/bingreeky/JIT (+20 more)

### Community 81 - "test_governance_sandbox.py"
Cohesion: 0.03
Nodes (85): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, 1.11 Multi-Agent Governance (10 / 100 / 1000 agents), Failure behaviour, P1 — Close the remaining product gaps, Task 6 — Deeper website/repo scanning (more than "2 systems"), Task 7 — Knowledge base auto-reflect (ECC / Obsidian / graphify-style), Task 8 — Vector retrieval for the knowledge base (self-learn), events_from_identity() (+77 more)

### Community 82 - "clear_cooldowns"
Cohesion: 0.07
Nodes (34): clear_cooldowns(), is_provider_on_cooldown(), mark_provider_failed(), ProviderFallbackError, Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown., Clear all cooldown entries (useful for testing). Delegates to…, clear_all_locks() (+26 more)

### Community 83 - "test_direct_chat_evolution.py"
Cohesion: 0.06
Nodes (57): AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState, FailedJob, BaseModel, Enum, str (+49 more)

### Community 84 - "setup/api.py"
Cohesion: 0.07
Nodes (66): is_user_onboarding_allowed(), Return True if this user may run the onboarding wizard. Resolution order: 1. If…, motor_motor_asyncio, complete_wizard(), _delete_wizard_state(), detect_configured_providers(), detect_hardware_for_wizard(), detect_models_for_wizard() (+58 more)

### Community 85 - "TaskDispatcher"
Cohesion: 0.13
Nodes (16): Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, _make_task(), asyncio, tests/test_dispatcher_iso8601.py — regression test for the TaskDispatcher auto-…, A BLOCKED task with a recent ISO 8601 updated_at should be skipped (cooldown…, Build a task with a specific updated_at value (bypassing the validator). (+8 more)

### Community 86 - "sync/service.py"
Cohesion: 0.10
Nodes (34): FastAPI dependency: require Power User or Admin role. Raises 403 otherwise., Return a FastAPI dependency that checks for a specific permission., require_permission(), _dep(), require_power_user(), add_peer(), get_folder_index(), get_sync_file() (+26 more)

### Community 87 - "test_e2e_agent_chat.py"
Cohesion: 0.08
Nodes (46): _auth_headers(), _build_agent_http_mock(), mock_get(), mock_post(), mock_put(), _exec(), _fake_request(), _mcp_tool_response() (+38 more)

### Community 88 - "QuickNoteQueue"
Cohesion: 0.06
Nodes (39): _fetch_text(), __init__(), _now(), process_note(), Any, Path, QuickNote, QuickNoteQueue (+31 more)

### Community 89 - "test_sqlite_store.py"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (37): browse_page(), BrowserAction, BrowserSession, _not_started(), PageState, Any, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, Which backend a start() would use: 'browserbase', 'local', or 'stub'. (+29 more)

### Community 93 - "test_sam_livekit.py"
Cohesion: 0.03
Nodes (60): Any, SAM voice agent — the voice-controlled interface to the agency., Process a voice command and return SAM's spoken response. Args: text: The…, Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., Call the NVIDIA NIM LLM (free tier) for SAM's response., Rule-based fallback when the LLM is unavailable., A single voice conversation session with SAM. (+52 more)

### Community 94 - "SeoFixRequest"
Cohesion: 0.14
Nodes (12): Request to remediate auto-fixable findings in a local code repository., Result of a fixer run., SeoFixRequest, SeoFixResult, services/seo_fixer.py - Repo-Aware SEO Auto-Fixer When a company has a code…, Convenience wrapper used by the API layer and skill executor., run_fixes(), Path (+4 more)

### Community 95 - "ApprovalStore"
Cohesion: 0.08
...
### Community 97 - "resolve_active_brain"
Cohesion: 0.10
Nodes (40): BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _norm(), _pick_from_records(), _build(), _prio(), Any (+32 more)

### Community 98 - "test_shared_state.py"
Cohesion: 0.06
...
### Community 101 - "monitor_lib.py"
Cohesion: 0.03
Nodes (100): ArgumentParser, date, .github/scripts/provider_policy.py — Read the durable provider policy from the…, Reset the cached policy (test helper)., reset_cache(), Popen, build_prompt(), call_llm() (+92 more)

### Community 102 - "test_colibri_provider.py"
Cohesion: 0.13
...
### Community 103 - "test_force_cleanup_conditional_delete.py"
Cohesion: 0.10
...
### Community 104 - "SelfHealingAgent"
Cohesion: 0.05
...
### Community 105 - "test_web_reach.py"
Cohesion: 0.07
Nodes (48): _domain_list_reason(), Return why the env-var domain policy blocks *host*, or None. Checks…, Zero-key internet access: pages, YouTube transcripts, search, RSS. Every public…, WebReach, _fake_fetch_module(), _fake_video_module(), ModuleType, MonkeyPatch (+40 more)

### Community 106 - "gsap.min.js"
Cohesion: 0.05
...
### Community 109 - "AgentJobManager"
Cohesion: 0.05
Nodes (38): AgentJob, AgentJobManager, heartbeat(), _now(), Any, Run a job using the provided runner and update the job's lifecycle, progress,…, Serialize the AgentJob to a JSON-serializable dictionary for external clients.…, get_agent_job_manager() (+30 more)

### Community 110 - "V5App.jsx"
Cohesion: 0.05
Nodes (33): API, V5App, HubTabs(), Spinner(), ActivationGate(), AssistantHub(), SamVoiceScreen, AgentsScreen (+25 more)
...
### Community 117 - "frontend/package.json"
Cohesion: 0.04
Nodes (55): browserslist, development, production, dependencies, axios, fast-uri, livekit-client, lucide-react (+47 more)

### Community 118 - "test_refresh_agent_built_proof.py"
Cohesion: 0.12
...
### Community 119 - "resolve_e2b_config"
Cohesion: 0.05
Nodes (55): io, e2b_status(), Return the E2B sandbox integration status for the ProvidersScreen badge. Does…, e2b_enabled(), E2BConfig, _env_falsy(), _env_truthy(), services/e2b_config.py — Single reader of E2B sandbox environment config.… (+47 more)

### Community 120 - "_llm_catalog"
Cohesion: 0.06
...
### Community 121 - "portfolio_intelligence.py"
Cohesion: 0.06
Nodes (51): _bullets(), generate_backlog_retro(), generate_standup(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive a retrospective from the task tracker when no sprint is active. DONE /…, Render the sprint plan as markdown., Render a list of strings as markdown bullets (or a placeholder). (+43 more)

### Community 122 - "PatternConsolidation"
Cohesion: 0.08
...
### Community 123 - "test_spec_store.py"
Cohesion: 0.06
Nodes (39): build_spec_router(), approve_spec(), _decide(), get_spec_artifact(), list_spec_artifacts(), reject_spec(), Any, APIRouter (+31 more)

### Community 124 - "E2BSandboxSession"
Cohesion: 0.04
Nodes (73): MCPUnavailableError, RuntimeError, Raised when the MCP server is unreachable or the circuit is open., ★5 — Sandboxed Agent Execution (E2B / Docker micro-VM) [P1] [CHM] ✅ Delivered 2026-07-04, E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any (+65 more)

### Community 125 - "LogWatcher"
Cohesion: 0.05
...
### Community 127 - "RepoConnection"
Cohesion: 0.11
Nodes (30): Phase 0 — `RepoConnection` plumbing + delivery-policy detection, Phase 1 — Plan-PR → Implementation  *(highest leverage; closes the live gap)*, Phase 2 — Review-comment resolution (Codex / CodeRabbit), Phase 3 — Quality gate + policy-conformant landing, Phase 4 — Monitor & regression guard, Phases, 6. Integration gaps to wire (follow-up implementation), Loop 3 — Agentic SDLC (the golden path) (+22 more)

### Community 128 - "ref_react"
Cohesion: 0.04
Nodes (47): Removed, Removed, completeSetup(), createSecret(), detectHardwareForSetup(), detectModelsForSetup(), getMe(), getPublicPath() (+39 more)

### Community 129 - "SeoFixer"
Cohesion: 0.13
...
### Community 130 - "OnboardingScreen.jsx"
Cohesion: 0.04
Nodes (35): Phase 1 — Stop the bleeding + paid kill switch ✅, Phase 2 — Per-surface assignment in the UI 🔄, Phase 3 — Persistence hardening (#537, #524) ⏳, Phase 4 — Onboarding fixes (#593, #619, PR #623) ⏳, Phase 5 — Reliability (#522) ⏳, Phase 6 — Green tests + housekeeping ⏳, PR #634 Implementation Tracker, Verification checklist (final) (+27 more)

### Community 131 - "NotificationDispatcher"
Cohesion: 0.06
Nodes (34): NotificationDispatcher, Path, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess)., Run the bot with stop-event awareness., Routes background task results to configured notification channels. Currently…, Manages the Telegram bot as a managed service alongside ollama/proxy/tunnel.… (+26 more)

### Community 132 - "ai_runner.py"
Cohesion: 0.07
...
### Community 133 - "test_llm_router_e2e.py"
Cohesion: 0.09
...
### Community 134 - "Fixed"
Cohesion: 0.07
Nodes (37): Normalise a raw planner JSON response into a consistent plan dict. Handles: -…, Fixed, CreateIssue, Fixed, call_brain_with_failover(), POST *messages* to each candidate in order; return the first success. Returns…, build_body(), _failures() (+29 more)

### Community 135 - "Any"
Cohesion: 0.15
Nodes (3): Any, field_validator, Coerce unrecognised system_type values to 'custom' so the model never crashes…

### Community 136 - "CompanyGraphStore"
Cohesion: 0.03
Nodes (44): Company, The core company entity - root of the Company Graph., Find a repository by its URL., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, Add a repository to a company. Args: company_id: Company ID url: Repository URL… (+36 more)
...
Cohesion: 0.08
Nodes (49): activate_instance(), ActivateRequest, ActivateResponse, activation_audit_log(), activation_status(), ActivationStatusResponse, _append_audit(), AuditLogEntry (+41 more)

### Community 143 - "get_tracker"
Cohesion: 0.06
Nodes (31): get_tracker(), Return the shared AutonomyTracker singleton (lazy-init)., Reset the singleton (test helper)., reset_tracker(), Contract: get_tracker() returns the same instance., TestClient, tests/test_public_kpi.py — public read-only autonomy KPI endpoint. The autonomy…, The public strip must expose only aggregate counts — no user/company/repo or… (+23 more)

### Community 144 - "ensure_self_company"
Cohesion: 0.06
Nodes (36): _count_specialists(), _create_company_directly(), ensure_self_company(), _find_self_company(), _find_stale_self_companies(), _list_companies_safe(), Self-onboarding bootstrap. On startup the platform registers *itself* as a…, List companies, skipping any that fail to deserialize. A previous deploy wrote… (+28 more)

### Community 145 - "WorkflowRun"
Cohesion: 0.09
Nodes (23): engine(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test(), _tracking_run_single() (+15 more)

### Community 146 - "BackgroundAgent"
Cohesion: 0.07
...
### Community 147 - "test_phase4_runtime_resilience.py"
Cohesion: 0.09
...
### Community 150 - "TrafficDirector"
Cohesion: 0.10
Nodes (11): Failure behaviour, In-process traffic distribution and budget accounting for providers., Record the outcome of a request started with ``record_start``. Must be called…, Count a request that was routed away from *provider_id* (diagnostics)., EWMA latency in ms; never-sampled providers sort first. Returning -1.0 for an…, Clear all counters (tests only)., TrafficDirector, `int(0.5)` is 0, and a cap of 0 makes `in_flight >= cap` true at zero in-flight… (+3 more)

### Community 151 - "WorkspaceTools"
Cohesion: 0.03
Nodes (66): Register the built-in agent tools that are always available., _register_builtin_tools(), Adding a new tool, CLAUDE.md — agent/, Skills worth invoking here, Testing, What this package does, Delegate to RepowiseIntelligence for a natural-language codebase question. (+58 more)

### Community 152 - "test_verification_strategies.py"
Cohesion: 0.07
...
### Community 153 - "test_brain_config_api.py"
Cohesion: 0.07
Nodes (31): ProbeResult, BaseModel, Outcome of a single (provider, model) liveness probe., _catalogue_safe_default(), tests/test_brain_config_api.py — admin endpoint contract tests. Pins the…, A model that 410s must be rejected with 422 + a probe report., A model that probes 200 OK must be persisted., If the chosen provider's key is missing, the probe short-circuits → 422. (+23 more)

### Community 154 - "_cfg"
Cohesion: 0.12
Nodes (6): _cfg(), 3.8 Flash should have higher priority (lower number) than 3.7 Flash., Kimi K2 and QwQ-32B must have catalog entries so the router includes them., Gemini 3.x models must be declared with correct capability flags., TestGemini3xCatalog, TestGroqNewModelsCatalog

### Community 155 - "test_e2b_task_wiring.py"
Cohesion: 0.08
...
Cohesion: 0.06
Nodes (18): Command, CommandDispatcher, Parse and execute a slash command from raw text. Args: text: Raw command text,…, Return all enabled commands in a given category., Return all registered commands., Number of registered commands., A single slash command with metadata, handler, and lifecycle hooks., Execute the command, running pre/post hooks if configured. (+10 more)

### Community 157 - "KeyStore"
Cohesion: 0.08
...
Cohesion: 0.15
Nodes (9): get_pool(), KeyPool, Round-robin key selection with per-key rate-limit cooldowns., Clear all state (tests only)., Return the process-singleton KeyPool., The default configuration must be untouched by rotation., With no sibling to fall back to, withholding the key would convert a provider…, The capacity property: one spent key does not stop the provider. (+1 more)

### Community 159 - "Settings"
Cohesion: 0.05
...
### Community 160 - "test_runtime_governance.py"
Cohesion: 0.07
Nodes (49): Return True if the runtime is available (not circuit-open)., Return health snapshots for all known runtimes., Force an immediate health check of all runtimes and return results., Attempt to start a dead runtime subprocess before re-probing. Uses the local…, Async health polling service for all registered runtimes., Start the background polling loop with an immediate initial check., RuntimeHealthService, _governance_check() (+41 more)

### Community 161 - "test_brain_availability_doctor.py"
Cohesion: 0.04
Nodes (65): Configuration reference, brain_availability_summary(), Non-secret answer to "can the brain answer a request right now?". Three callers…, get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Drop the singleton (test helper)., reset_ceo_ledger(), CEOSupervisor (+57 more)

### Community 162 - "AdminScreen.jsx"
Cohesion: 0.08
Nodes (22): Docs / changelog, Frontend changes, Goal, Implementation Plan — Onboarding-Gate Admin Setting + Ephemeral Companies, Requirements → design, Tests, changeUserRole(), createApiKey() (+14 more)

### Community 163 - "WorkflowEngine"
Cohesion: 0.09
...
Cohesion: 0.08
Nodes (44): activation_required(), ActivationResult, _b64url_decode(), _b64url_encode(), _decode_jwt_unverified(), _generate_token_for_owner(), get_activation(), get_or_create_instance_id() (+36 more)

### Community 165 - "scrub"
Cohesion: 0.06
Nodes (26): Security, _luhn_ok(), Any, True when ``digits`` (bare, no separators) satisfies the Luhn checksum., Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Recursively strip secrets from an arbitrary argument structure. Fails…, Redact secret-shaped substrings, then truncate., Return the most recent events, newest first, optionally filtered. (+18 more)

### Community 166 - "_untried_paid"
Cohesion: 0.06
...
### Community 167 - "ContextWindowManager"
Cohesion: 0.08
...
### Community 168 - "test_dependabot_sweep_workflow.py"
Cohesion: 0.05
...
### Community 169 - "test_video_transcript.py"
Cohesion: 0.05
...
### Community 171 - "chat_handlers.py"
Cohesion: 0.11
Nodes (38): Layer 2 — Chat Handlers (`chat_handlers.py`, 710 lines), Fixed, _apply_chat_defaults(), _apply_reasoning_budget(), _emit_safely(), _extract_exact_output(), _filter_fragment(), _filter_openai_sse_line() (+30 more)

### Community 172 - "LLM Router — architecture"
Cohesion: 0.03
Nodes (55): 1. `LLMRouter` is the only gateway, 2. Providers are data, not code, 3. Secrets stay in the environment, 4. Three independent failure scopes, 5. Bulkhead isolation, 6. Context is managed losslessly, 7. Configuration is six committed YAML files, 8. Backwards compatibility by shim, not by rewrite (+47 more)

### Community 173 - "v3_auth.py"
Cohesion: 0.07
Nodes (42): _get_admin_email(), _get_admin_name(), _get_admin_secret(), login(), LoginRequest, LoginResponse, BaseModel, V3 API authentication endpoints (/api/auth/*). (+34 more)

### Community 174 - "BudgetTracker"
Cohesion: 0.07
...
### Community 176 - "ManagedAgentDreams"
Cohesion: 0.06
...
### Community 177 - "PromptCacheManager"
Cohesion: 0.06
...
### Community 178 - "render_ops.py"
Cohesion: 0.05
Nodes (36): _latest_metric_value(), _note_recurrence(), _parse_timestamp(), Any, datetime, services/render_ops.py — autonomous Render debugging + environment monitoring.…, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Pull the most recent numeric sample out of a ``get_metrics`` payload. Render… (+28 more)

### Community 179 - "analyze_page"
Cohesion: 0.13
Nodes (11): analyze_page(), _host_key(), BeautifulSoup, Host normalized for internal/external comparison (www. stripped)., Run every page-scoped check against one HTML document (no network). Returns a…, _visible_text(), codes(), TestCleanPage (+3 more)

### Community 180 - "test_phase6_workflow.py"
Cohesion: 0.14
Nodes (23): BaseModel, WorkflowTransition, _make_store(), _make_task(), tests/test_phase6_workflow.py — Phase 6: workflow engine, safe_agency, Task…, Create a minimal Task for testing., Infinite loops: engine must stop at max_phases., Return a MagicMock TaskStore with async get/update. (+15 more)

### Community 181 - "AdminAuthManager"
Cohesion: 0.12
Nodes (6): Admin Authentication, AdminAuthManager, AdminSession, AdminSessionStore, _is_truthy(), WindowsCredentialAuthenticator

### Community 182 - "CompanyAgencyService"
Cohesion: 0.12
...
### Community 184 - "get_registry"
Cohesion: 0.05
Nodes (33): best_vision_model(), get_registry(), ModelCapability, Return model registry, extended with ROUTER_EXTRA_MODELS env entries.…, Return the name of the best registered vision-capable model, or None. Prefers…, claude-opus-5 must be in the built-in alias map so routing resolves it., TestClaude5RegistryEntries, test_bedrock_haiku_4_5_in_registry() (+25 more)

### Community 185 - "SyntheticDataPipeline"
Cohesion: 0.07
...
### Community 186 - "test_features_api.py"
Cohesion: 0.05
...
### Community 188 - "WorkspaceManifest"
Cohesion: 0.08
...
### Community 189 - "portfolio_api.py"
Cohesion: 0.08
...
### Community 191 - "resolve_provider_for"
Cohesion: 0.06
Nodes (40): _get_provider_policy(), ProviderPolicyUpdate, Read the durable provider policy from DB, falling back to a safe default.…, Editable subset of the durable provider policy (paid-provider kill switch)., Resolve the LLM endpoint for a named surface (task/chat/ceo/sdlc/…). Honours…, _prio(), resolve_provider_for(), asyncio (+32 more)
...
### Community 193 - "AnthropicProvider"
Cohesion: 0.05
...
### Community 195 - "test_telegram_service_webhook.py"
Cohesion: 0.14
...
Cohesion: 0.05
Nodes (53): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is… (+45 more)

### Community 197 - "test_llm_router_queue_cache.py"
Cohesion: 0.06
Nodes (39): CacheManager, cosine_similarity(), payload_key(), Any, Exact-match cache key over the fields that change the answer. Routing…, Cosine similarity between two vectors, 0.0 when either is degenerate., Owns every cache layer and the policy for what may enter them., Whether a response to ``request`` may be stored. Deliberately conservative — a… (+31 more)

### Community 198 - "NIMConnectionPool"
Cohesion: 0.08
Nodes (17): NIMConnectionPool, Any, AsyncClient, Persistent httpx.AsyncClient pool with circuit breaker and retry logic. Manages…, Get or create the shared httpx.AsyncClient., Context manager for a pooled client session., Close the connection pool., Make an HTTP request through the pool with circuit breaker protection. Args:… (+9 more)

### Community 199 - "test_daily_automation_2026_09_21.py"
Cohesion: 0.09
...
### Community 200 - "Persistent Memory System"
Cohesion: 0.05
...
### Community 201 - "FeatureMaturity"
Cohesion: 0.05
Nodes (36): 18. Feature Maturity & Support Matrix, check_feature(), get_feature(), list_features(), Any, get, post, Return the full support matrix with summary. (+28 more)
...
### Community 203 - "_step"
Cohesion: 0.06
Nodes (21): _job(), parametrize, Path, quick_note(), `--missing-ok` is correct for an unplanned issue and wrong otherwise., `continue-on-error: true` means a crash must be its own state., Waiting is not reviewing., They have their own handler step; excluding them would strand them. (+13 more)

### Community 204 - "_llm_catalog"
Cohesion: 0.07
...
### Community 205 - "test_connector_registry.py"
Cohesion: 0.12
Nodes (8): packages_integrations, Execute the ``webhook`` connector: POST *payload* as JSON to *url*. Fails soft…, send_webhook(), tests/test_connector_registry.py — the connector catalogue + webhook action.…, test_send_webhook_does_not_follow_a_redirect(), test_send_webhook_posts_json_on_a_safe_target(), test_send_webhook_rejects_loopback_before_any_request(), test_send_webhook_rejects_non_http_scheme()

### Community 206 - "Feature Maturity / Support Matrix"
Cohesion: 0.29
...
### Community 207 - "TaskBoardScreen.jsx"
Cohesion: 0.06
Nodes (33): Key files, addTaskComment(), approveTaskCheckpoint(), approveTaskExecution(), clarifyTask(), createSprint(), escalateTask(), fetchSprints() (+25 more)

### Community 208 - "ProvidersScreen.jsx"
Cohesion: 0.07
Nodes (32): UI-first — an API is not "done", Phase 2 — Per-surface assignment in the UI (the "one place"), Modified files, createProvider(), deleteProvider(), getBrainConfig(), getLocalBrainState(), getProviderPolicy() (+24 more)

### Community 209 - "CEOLedger"
Cohesion: 0.08
...
### Community 211 - "engine.py"
Cohesion: 0.08
Nodes (26): tests/test_contracts_agency.py — Contract tests for agency core autonomy…, Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory (+18 more)

### Community 212 - "AutonomyTracker"
Cohesion: 0.06
...
### Community 213 - "AgentSessionStore"
Cohesion: 0.05
Nodes (42): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, _known_tool_names(), Any (+34 more)
...
### Community 215 - "test_daily_digest.py"
Cohesion: 0.06
...
### Community 217 - "SeoAuditReport"
Cohesion: 0.13
Nodes (27): html, Complete result of one audit run., SeoAuditReport, Paragraph, reportlab_lib, reportlab_lib_pagesizes, reportlab_lib_styles, reportlab_lib_units (+19 more)

### Community 218 - "ScheduledJob"
Cohesion: 0.06
Nodes (23): _age_seconds(), Any, Reconstruct a ScheduledJob from its as_dict() output., Fire a job immediately (webhook / manual trigger)., Attach a durable store and immediately rehydrate from it (#505). Called at…, Async variant of :meth:`attach_persistence` for callers already on an event…, Sync entry-point for attach_persistence(); delegates to hydrate(). With no…, #505: Rehydrate persisted schedules on boot. Skips stale run-once jobs (already… (+15 more)

### Community 219 - "AuditLog"
Cohesion: 0.05
Nodes (43): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log. (+35 more)

### Community 220 - "TestRecordUsageAndStats"
Cohesion: 0.04
...
### Community 221 - "SpecEntry"
Cohesion: 0.11
Nodes (22): build_block(), _int_env(), Path, Absolute path of the harness spec for a workspace., Parse existing entries. Never raises — a broken file yields no entries. With…, Rewrite the spec file, preserving any non-entry (hand-written) lines., Merge qualifying lessons into the spec. Caller holds _REFINE_LOCK., Compact prompt block of standing instructions, or '' when there are none.… (+14 more)

### Community 222 - "AgileManager"
Cohesion: 0.07
...
### Community 224 - "test_background_services.py"
Cohesion: 0.07
...
### Community 226 - "test_local_controller.py"
Cohesion: 0.06
...
### Community 228 - "test_telegram_webhook.py"
Cohesion: 0.06
...
### Community 229 - "user_research_skill.py"
Cohesion: 0.08
...
### Community 232 - "facade.py"
Cohesion: 0.07
Nodes (35): create_refresh_token(), N5 dual-auth dependency: accept EITHER a user session OR a service token. Used…, _user_or_service_token(), 4. Architecture reference, Auth flows, Deployment topology, Providers, Scheduler (+27 more)

### Community 233 - "probe_catalogues.py"
Cohesion: 0.10
...
### Community 237 - "REWRITE_PLAN.md — Phased Migration Strategy"
Cohesion: 0.06
...
### Community 239 - "_cfg"
Cohesion: 0.13
Nodes (10): packages_llm, _skip_without_pydantic, _cfg(), tests/test_daily_automation_2026_09_11.py — Daily automation tests…, Pro should be tried after Flash (lower priority number = higher preference)., The router must not exclude gemini-2.5-pro from tool-calling requests., Every model in the 'google' PROVIDER_PRESET must be declared in models.yaml., gemini-2.5-pro must be declared in models.yaml with tool support. (+2 more)

### Community 240 - "_Recorder"
Cohesion: 0.09
...
### Community 246 - "agent_runtime.py"
Cohesion: 0.09
...
### Community 247 - "useSafeData"
Cohesion: 0.05
Nodes (34): createSchedule(), createWikiPage(), deleteSource(), ingestSource(), pauseSchedule(), resumeSchedule(), triggerSchedule(), useSafeData() (+26 more)

### Community 248 - "analyze"
Cohesion: 0.11
...
### Community 249 - "test_issue_intake.py"
Cohesion: 0.07
...
### Community 250 - "test_all_providers_discovery.py"
Cohesion: 0.17
...
### Community 252 - "test_purge_backlog.py"
Cohesion: 0.07
...
### Community 255 - "_resolve_user_github_token"
Cohesion: 0.10
...
### Community 256 - "Platform Guide — the full tour"
Cohesion: 0.06
...
### Community 257 - "context_rules.py"
Cohesion: 0.11
...
### Community 258 - "platform_controls_router.py"
Cohesion: 0.12
Nodes (27): _actor(), build_platform_controls_router(), _admin(), list_controls(), reset_control(), update_controls(), ControlUpdateBody, APIRouter (+19 more)

### Community 259 - "Free NVIDIA brain + UI-controlled provider policy + no silent spend"
Cohesion: 0.09
Nodes (19): Cadence & stop conditions, First-run bootstrap, Hard constraints, Master Goal Prompt — Autonomous Agency CEO, Mission, The gate contract (Telegram human-in-the-loop), Decisions (locked with the owner), Free NVIDIA brain + UI-controlled provider policy + no silent spend (+11 more)

### Community 260 - "test_live_server.py"
Cohesion: 0.22
...
### Community 261 - "_routing_candidates"
Cohesion: 0.09
...
### Community 262 - "test_provider_enable_disable.py"
Cohesion: 0.07
...
### Community 263 - "test_render_mcp.py"
Cohesion: 0.09
...
### Community 264 - "PersistentMemoryStore"
Cohesion: 0.04
Nodes (77): MemoryCategory, MemoryEntry, MemoryScope, PersistentMemoryStore, Any, Connection, Enum, Path (+69 more)
...
### Community 266 - "test_openclaw_gateway.py"
Cohesion: 0.14
...
Cohesion: 0.11
Nodes (17): aiosqlite, _apply_update(), _Collection, _DeleteResult, _InsertResult, _match(), _new_id(), _now_iso() (+9 more)

### Community 268 - "get_ceo_ledger"
Cohesion: 0.16
Nodes (21): build_ceo_router(), ceo_status(), get_goal(), list_goals(), redrive_goal(), run_sweep(), Any, APIRouter (+13 more)
...
### Community 270 - "test_kimi_bridge_server.py"
Cohesion: 0.06
Nodes (20): lifespan(), FastAPI, KimiBrowserDriver, _messages_to_prompt(), Flatten an OpenAI messages list into a single string for the web UI., Poll the page until the streaming response is complete, then return its text., Manages a single persistent Chromium context pointing at kimi.com., Launch a persistent Chromium context (headless by default). (+12 more)

### Community 271 - "Universality: case-coverage matrix"
Cohesion: 0.10
...
### Community 274 - "classify_direct_chat_intent"
Cohesion: 0.23
...
### Community 278 - "Langfuse Observability Guide"
Cohesion: 0.06
...
### Community 279 - "v3_models.py"
Cohesion: 0.09
Nodes (41): _get_current_user, _get_bearer_token(), _get_current_user(), logout(), Depends, get, post, Extract and validate current user from token. (+33 more)

### Community 280 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.15
...
### Community 281 - "BrainWatchdog"
Cohesion: 0.04
Nodes (57): _migrate_brain_to_safe_default(), N1. Activate the reliability spine — wire the watchdog, schedule the digest ⬜  (size: M, risk: low), emit_agency_observation(), Record an agency platform operation in Langfuse. Unlike…, BrainConfigPatch, get_brain_config_store(), BaseModel, Return the process-wide ``BrainConfigStore`` singleton. (+49 more)

### Community 282 - "ModelRouter"
Cohesion: 0.15
Nodes (10): ModelRouter, Any, Central model router. Create one instance (use ``get_router()``). ``route()``…, Decide which Ollama model to use for this request. Args: requested_model: Model…, Return *model* if it is available in Ollama, else the first available fallback.…, Build an ordered list of alternative models to try if *primary* fails., Flat dict suitable for Langfuse observation metadata., ModelRouter.route() signature enforcement. (+2 more)

### Community 283 - "test_unit7_catalog_propagation.py"
Cohesion: 0.06
...
### Community 284 - "extract_failures"
Cohesion: 0.21
...
### Community 285 - "test_workspace_isolation.py"
Cohesion: 0.16
...
### Community 286 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (125): agent, _note_phase_end(), Close the tracked phase *token* opened above. Never raises., _diagnose_and_file(), _format_incident(), gather_render_evidence(), get_operational_incident_tracker(), _iso_from_monotonic() (+117 more)

### Community 287 - "ContextCompressor"
Cohesion: 0.11
...
### Community 289 - "AgentJobRequest"
Cohesion: 0.08
Nodes (28): AgentJobRequest, Validated input for creating a new agent job. Passed from the API handler into…, Path, [5.0.0], Added, Changed, Removed, Acceptance check (+20 more)
...
### Community 292 - "ResourceWatchdog"
Cohesion: 0.10
...
### Community 293 - "test_provider_410_per_model.py"
Cohesion: 0.16
Nodes (16): _dead_model_key(), _is_model_dead(), Return True if (provider, model) is on the dead list and not yet expired., _nvidia(), anyio, Regression tests: 410 Gone is a per-model condition, not per-provider.…, Once _DEAD_MODEL_COOLDOWN_SECONDS elapses, the model is re-probed., When every candidate 410s, the provider is cooled and failover proceeds. (+8 more)

### Community 294 - "High-Agency Frontend Skill"
Cohesion: 0.06
Nodes (30): 10. FINAL PRE-FLIGHT CHECK, 1. ACTIVE BASELINE CONFIGURATION, 2. DEFAULT ARCHITECTURE & CONVENTIONS, 3. DESIGN ENGINEERING DIRECTIVES (Bias Correction), 4. CREATIVE PROACTIVITY (Anti-Slop Implementation), 5. PERFORMANCE GUARDRAILS, 6. TECHNICAL REFERENCE (Dial Definitions), 7. AI TELLS (Forbidden Patterns) (+22 more)

### Community 295 - "Quick-Note GitHub Issues Processing - Session Summary"
Cohesion: 0.06
...
### Community 296 - "DashboardScreen.jsx"
Cohesion: 0.08
Nodes (9): Charts, Donut(), ExecutionTimeline(), Sparkline(), ErrorBoundary, DashboardScreen(), fmtTokens(), relTime() (+1 more)

### Community 297 - "test_provider_render_env.py"
Cohesion: 0.10
Nodes (21): provider_env_names(), RuntimeError, Raised when Render rejects or cannot serve an env-var write., Return ``(key_env, base_url_env)`` for a provider id, or ``None``. Sourced from…, Set one environment variable on a Render service via the REST API. Updates a…, RenderEnvError, update_service_env_var(), _enable_render() (+13 more)

### Community 298 - "DistributedRateLimiter"
Cohesion: 0.09
...
### Community 300 - "local_controller.py"
Cohesion: 0.10
...
### Community 302 - "test_hermes_in_process.py"
Cohesion: 0.06
Nodes (30): _check_auth(), health(), Any, BaseModel, get, post, Execute a task synchronously via the InternalAgentAdapter (our brain). Response…, Body for POST /tasks — mirrors the payload HermesAdapter.execute sends. (+22 more)

### Community 303 - "TestClient"
Cohesion: 0.09
...
### Community 304 - "test_chat_mode_regressions.py"
Cohesion: 0.09
...
### Community 305 - "test_skill_registry_boot_refresh.py"
Cohesion: 0.11
...
### Community 306 - "test_autonomy_gate.py"
Cohesion: 0.12
...
### Community 307 - "test_executive_advisory.py"
Cohesion: 0.04
Nodes (76): AdviceResult, AdvisoryMemory, _compose_context(), default_executives(), ExecOpinion, Executive, ExecutiveAdvisory, _format_company_context() (+68 more)

### Community 308 - "output_filter.py"
Cohesion: 0.07
...
### Community 309 - "AgentDefinition"
Cohesion: 0.03
Nodes (78): _apply_activity_status(), create_agent(), delete_agent(), get_agent(), _get_user(), list_agents(), list_runtime_agents(), Any (+70 more)

### Community 310 - "AgentScheduler"
Cohesion: 0.08
Nodes (32): gc, AgentScheduler, Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Delete EVERY schedule from the durable store and in-memory state. Operator…, #505: Remove a job from durable storage., test_scheduler_purge_all_wipes_store_and_memory(), _FakePersistence, tests/test_schedule_persistence.py — #505 schedules survive restart. Regression… (+24 more)

### Community 311 - "test_rag_context.py"
Cohesion: 0.12
...
### Community 314 - "context_plan_gate.py"
Cohesion: 0.14
...
### Community 315 - "Kept Rules — the 44 that survive the audit"
Cohesion: 0.05
...
### Community 317 - "AppShell.jsx"
Cohesion: 0.14
Nodes (11): getAccountLifecycle(), MOBILE_PRIMARY, NAV_ITEMS, EphemeralBanner(), formatRemaining(), ALIASES, frontend_src_v5_v5app_default, APP_LABEL (+3 more)
...
### Community 319 - "_emit_langfuse_http_sync"
Cohesion: 0.11
Nodes (17): Carry-over items (from prior sessions), Next Action, Routine to pick up next session, _department_trace_tags(), _emit_langfuse_http_sync(), _emit_sdk(), Any, Avoid oversized payloads that make Langfuse reject the event. (+9 more)
...
### Community 321 - "RequestQueue"
Cohesion: 0.08
Nodes (18): T, One job waiting for a concurrency slot, ordered by (priority, arrival)., Bounded priority queue with concurrency limits and backpressure. Admission is…, Run ``job`` under the queue's concurrency limit. Raises ``QueueFull``…, Wait for a slot, yielding to higher-priority work already queued., Give the slot to the best waiting job, or return it to the pool., RequestQueue, _Waiter (+10 more)

### Community 322 - "test_claude_setup_audit.py"
Cohesion: 0.16
...
### Community 323 - "test_brain_failover.py"
Cohesion: 0.07
Nodes (40): ProviderHealth, Circuit-breaker state for a provider., _make_manager(), tests/test_brain_failover.py — Universal multi-provider brain failover tests.…, Guards the three tests below from pinning a retired id again: if the fixture…, Status snapshot doesn't leak API keys., Make a fresh manager (bypasses the singleton for isolation)., No API keys set → no providers in the registry. (+32 more)

### Community 324 - "test_ceo_router.py"
Cohesion: 0.16
Nodes (22): ledger(), _make_app(), FastAPI, tests/test_ceo_router.py — auth and behaviour for /api/ceo/*. Follows the…, The happy path — the branch that actually spends provider budget., Filtering after a LIMIT silently under-reported matching goals., The manual route must not be a way around the intervention cap., _seed() (+14 more)

### Community 325 - "pr_approval_gate.py"
Cohesion: 0.14
Nodes (27): _card_keyboard(), _card_text(), _dedupe_key(), default_run_sweep(), _gh_get(), _gh_token(), interval_sec(), load_notified() (+19 more)

### Community 326 - "test_session_retro.py"
Cohesion: 0.13
...
### Community 327 - "test_internal_agent_did_work.py"
Cohesion: 0.12
...
### Community 328 - "_captured_request_headers"
Cohesion: 0.12
...
### Community 329 - ".update_intelligence"
Cohesion: 0.08
...
### Community 330 - "test_harness_spec.py"
Cohesion: 0.14
Nodes (14): _flag(), propose_entries(), Turn qualifying lessons into candidate entries. A lesson qualifies only when it…, Promote repeated lessons into the spec. Returns the entries added. No-op unless…, refine(), _lesson(), tests/test_harness_spec.py — the Continual Harness spec. Covers the property…, Regressions for defects found in review of this module. (+6 more)

### Community 331 - "test_agent_api.py"
Cohesion: 0.14
...
### Community 334 - "Python Dependencies (`requirements.txt`)"
Cohesion: 0.07
...
### Community 336 - "KV Cache Internals"
Cohesion: 0.07
...
### Community 337 - "provider_max_rpm"
Cohesion: 0.11
Nodes (20): Pre-call budget checks, provider_max_parallel(), provider_max_rpm(), Return the operator-configured requests/min cap for *provider*, or None if…, Return the operator-configured in-flight request cap for *provider*. Reads…, _ProviderUsage, Sliding-window usage counters for one provider., Drop request/token events that have fallen out of the window. (+12 more)

### Community 338 - "ProviderConsole.jsx"
Cohesion: 0.08
...
### Community 344 - "control_registry.py"
Cohesion: 0.12
Nodes (24): Adding a control, packages/config/control_catalogue.py — the 109 operator-facing controls. The…, coerce(), _coerce_choice(), _coerce_number(), _coerce_toggle(), Any, packages/config/control_registry.py — the platform-control API. The public… (+16 more)

### Community 345 - "ProjectScaffolder"
Cohesion: 0.14
...
### Community 347 - "Fixed"
Cohesion: 0.29
Nodes (10): Fixed, difflib, Fixed, DetailsStep(), _blocks(), main(), normalize_text(), scripts/check_changelog_parity.py CI guard for the changelog mirror. Closes the… (+2 more)
...
### Community 349 - "Docker Agent Runtimes Setup"
Cohesion: 0.07
...
### Community 350 - "models/seo_audit.py"
Cohesion: 0.11
Nodes (12): models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, Static definition of a single audit check (catalog entry)., SeoCheckDefinition, auto_fixable_checks(), _c(), get_check(), list_checks(), services/seo_checks.py - SEO / GEO / AIO Check Catalog The authoritative… (+4 more)

### Community 351 - "generate_context.py"
Cohesion: 0.13
Nodes (26): _build_caller_chain(), _build_context_doc(), _build_pr_description(), _build_todos_md(), _build_user_message(), _call_cerebras(), _call_claude(), _call_groq() (+18 more)

### Community 352 - "llm/router.py"
Cohesion: 0.03
Nodes (77): get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager., text_key(), AgentPolicy (+69 more)

### Community 353 - "decide"
Cohesion: 0.12
...
### Community 355 - "TestCatalogFable51"
Cohesion: 0.09
...
Cohesion: 0.08
Nodes (32): ipaddress, test_git_ref_rejects_empty(), test_git_ref_rejects_flag_injection(), test_git_ref_rejects_shell_metacharacters(), test_git_ref_rejects_traversal(), test_git_ref_valid(), test_git_scheme_allows_ssh(), test_http_scheme_rejects_ssh() (+24 more)
...
### Community 360 - "Changelog"
Cohesion: 0.19
Nodes (13): serve_spa(), Changelog, Changelog, Changelog, Changelog, Get the details of skills bound to a specialist. Args: specialist_id:…, TestCase, isolated_telegram() (+5 more)

### Community 361 - "unsafe_target_reason"
Cohesion: 0.08
Nodes (25): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), _load_script_module(), Any, ModuleType, Dynamically load a pure-stdlib helper module from .github/scripts/. Returns…, Read a web page as plain text, following the same fallback chain (direct ->…, GET *url*, re-validating every redirect hop against ``unsafe_target_reason`` —… (+17 more)

### Community 362 - "_extract_tags"
Cohesion: 0.22
...
### Community 363 - "test_skill_registry.py"
Cohesion: 0.09
...
### Community 364 - "Technical Debt Register — local-llm-server"
Cohesion: 0.08
...
### Community 365 - "_ensure_tasks_source_id_unique_index"
Cohesion: 0.11
Nodes (20): _ensure_tasks_source_id_unique_index(), _build(), _is_index_options_conflict(), Exception, True when *exc* is Mongo refusing to redefine an existing index. Mongo raises…, Add a unique **partial** index on tasks.source_id — isolated from the main…, asyncio, The proactive dedup pass must run before the index-build attempt, so a first-… (+12 more)

### Community 366 - "test_daily_automation_2026_08_03.py"
Cohesion: 0.11
...
### Community 368 - "test_workflow_shell_vars_are_declared.py"
Cohesion: 0.10
...
### Community 369 - "TestTheCopiesMayNotDriftFurther"
Cohesion: 0.15
Nodes (10): _hardcoded_candidates(), A frozen record of the divergence found on 2026-08-28. Reconciling these eleven…, These two are the ones this work reconciled, and each was reconciled because…, A shrinking divergence is progress, but the list must be trimmed to match or it…, ``PROVIDER_CANDIDATES`` as written in the module, before YAML overrides. Read…, The YAML wins at import. A fix applied only to the Python copy is a fix that…, Checked against the catalogue that wins, not the copy that loses., TestTheCatalogueIsWhatActuallyRuns (+2 more)
...
### Community 372 - "PreflightReport"
Cohesion: 0.21
Nodes (10): DirectChatDoctor, PreflightIssue, PreflightReport, BaseModel, asyncio, When git is missing and no GitHub token is present, the doctor should report…, test_missing_git_and_token(), _make_fake_doctor() (+2 more)

### Community 373 - "test_keepalive.py"
Cohesion: 0.23
...
### Community 375 - "AgentsScreen.jsx"
Cohesion: 0.19
...
### Community 376 - "Session Handoff — 2026-06-15"
Cohesion: 0.08
...
### Community 377 - "Agency"
Cohesion: 0.05
Nodes (36): Agency, AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _gh_token() (+28 more)

### Community 378 - "_build_anthropic_response"
Cohesion: 0.09
...
### Community 380 - "test_north_mini_code.py"
Cohesion: 0.09
...
### Community 381 - "test_activation_api.py"
Cohesion: 0.13
...
### Community 383 - "test_service_token.py"
Cohesion: 0.08
...
### Community 384 - "test_commit_tracker.py"
Cohesion: 0.20
...
### Community 385 - "MultiAgentSwarm"
Cohesion: 0.22
Nodes (10): AgentSpec, MultiAgentSwarm, execute(), Dependency-aware multi-agent task runner with capability routing. DEPRECATED:…, TaskSpec, test_multi_agent_swarm_blocks_missing_dependency(), test_multi_agent_swarm_respects_dependencies(), Tests for MultiAgentSwarm using the coordinate module helpers. (+2 more)

### Community 386 - "github_tools.py"
Cohesion: 0.18
Nodes (24): get_repo(), _get_token(), _get_user(), init_workspace(), list_branches(), list_prs(), list_repos(), BaseModel (+16 more)

### Community 387 - "FilterResult"
Cohesion: 0.13
...
### Community 388 - "VoiceCommandInterface"
Cohesion: 0.15
...
### Community 390 - "reset_store"
Cohesion: 0.05
Nodes (58): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled_cached() (+50 more)

### Community 391 - "build_render_router"
Cohesion: 0.13
...
### Community 392 - "LLMRouter"
Cohesion: 0.02
Nodes (112): Fixed, Fixed, Modules, Writing a custom adapter, int, openai_body_from_response(), packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on… (+104 more)

### Community 393 - "Local AI Stack with Docker"
Cohesion: 0.08
...
### Community 394 - "Implementation Prompt: Rich TaskBoard + Agile Sprint Integration"
Cohesion: 0.08
...
### Community 395 - "TASK 4 — End-to-end approval-gate test"
Cohesion: 0.08
...
### Community 396 - "CompanyGraphService"
Cohesion: 0.03
Nodes (50): BusinessCategory, requires_db, CompanyGraphService, SpecialistFamily, SystemType, Get a company by ID. Args: company_id: Company ID Returns: Company instance or…, Update a company. Args: company_id: Company ID **kwargs: Fields to update…, Stop schedules and remove the registered agents for a company. Best-effort: a… (+42 more)

### Community 397 - "test_agent_chat_integration.py"
Cohesion: 0.13
...
### Community 398 - "TemporalContextGraph"
Cohesion: 0.10
...
### Community 400 - "_mock_provider_records"
Cohesion: 0.12
...
### Community 401 - "TaskInput"
Cohesion: 0.15
Nodes (10): build_task_specs(), Convert TaskInput list to TaskSpec list for MultiAgentSwarm., Task definition for /agent/coordinate requests., TaskInput, test_build_task_specs(), test_build_task_specs_with_type_alias(), test_task_input_aliases(), test_task_input_defaults() (+2 more)

### Community 402 - "register_admin_gui"
Cohesion: 0.16
...
### Community 403 - "executive_advisory_api.py"
Cohesion: 0.19
...
### Community 405 - "record_step_failures"
Cohesion: 0.14
...
### Community 406 - "Testing Analysis — local-llm-server"
Cohesion: 0.13
...
### Community 407 - "openclaw_str_e_fix.py"
Cohesion: 0.15
...
### Community 408 - "autonomous_fix.py"
Cohesion: 0.10
Nodes (22): _decline(), _extract_pytest_failure(), _fetch_failure_context(), _is_denied_path(), _list_target_prs(), main(), _post_comment(), _pr_head() (+14 more)

### Community 409 - "Workspace Isolation Architecture"
Cohesion: 0.09
...
### Community 410 - "Telegram Bot Setup"
Cohesion: 0.09
...
### Community 417 - "isolated_telegram_config"
Cohesion: 0.13
...
### Community 418 - "_FakeAsyncClient"
Cohesion: 0.20
...
### Community 420 - "TestCapabilitiesAreNotClaimedWithoutEvidence"
Cohesion: 0.24
...
### Community 421 - "test_mostly_failed_steps.py"
Cohesion: 0.12
...
### Community 422 - "HarnessEnrichment"
Cohesion: 0.12
Nodes (11): get_enrichment(), HarnessEnrichment, Any, Build a compact catalog of available runtime skills. Discovers from…, Standing instructions from the Continual Harness spec. Deliberately uncached:…, Inject enrichment blocks into a system prompt string. Appends blocks after the…, Return the enrichment instance for a workspace. Keyed by workspace root rather…, Auto-discovers skills and tools for agent prompt injection. Usage:: enrichment… (+3 more)

### Community 423 - "AdaptivePermissions"
Cohesion: 0.19
...
### Community 427 - "Fixed"
Cohesion: 0.04
Nodes (61): True if browser automation is enabled and Playwright is importable., Send chat messages and return the assistant's text output. If an…, Atomically delete a run-once row only while it is still unfired. Closes the…, _in_container(), observability_diag_public(), PUBLIC diagnostic endpoint for Langfuse — no auth required. Returns exactly…, Return True when running inside a container runtime. ``/.dockerenv`` alone…, Added (+53 more)

### Community 428 - "test_fabric_patterns.py"
Cohesion: 0.10
...
### Community 429 - "LiveKitConfig"
Cohesion: 0.28
Nodes (9): LiveKitConfig, Resolved LiveKit + speech-provider configuration., _build_llm(), _build_stt(), _build_tts(), Any, Pick a TTS provider: ElevenLabs → Groq PlayAI (free)., Pick an STT provider: Deepgram → Groq Whisper (free). (+1 more)

### Community 430 - "compare_runtimes.py"
Cohesion: 0.16
...
### Community 431 - "_execute_skill_impl"
Cohesion: 0.11
...
### Community 432 - "_resolve_push_token"
Cohesion: 0.17
...
### Community 433 - "tasks/models.py"
Cohesion: 0.09
Nodes (26): tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest, ExecutionApprovalRequest, ExecutionLogEntry (+18 more)

### Community 434 - "test_task_source_id_race.py"
Cohesion: 0.14
...
### Community 435 - "TestLegacyRouterCacheTTL"
Cohesion: 0.13
...
### Community 436 - "test_schedule_growth_invariants.py"
Cohesion: 0.05
...
### Community 442 - "_handle_command"
Cohesion: 0.13
Nodes (19): openclaw_websocket(), websocket, WebSocket endpoint for iOS / mobile web UI pairing + command routing., _cmd_chat(), _cmd_freebuff(), _cmd_list_files(), _cmd_read_file(), _cmd_status() (+11 more)

### Community 443 - "TestNormalizeResponseFormat"
Cohesion: 0.08
...
### Community 444 - "is_model_available"
Cohesion: 0.11
Nodes (21): Layer 3 — Model Router (`router/`), F. Router invariants (`router/`), F. Router invariants (`router/`), Health check and availability filtering, fallback_chain(), Follow the explicit chain in ``routing.yaml``, then everything else. The chain…, _enabled(), get_available_models() (+13 more)

### Community 445 - "V3 API Migration Plan — LLM Relay Platform"
Cohesion: 0.10
...
### Community 446 - "WindowsServiceManager"
Cohesion: 0.20
Nodes (6): Tunnel — Permanent Static URL (ngrok), _creationflags(), CompletedProcess, Spawn a new proxy process on Linux/Mac using the current Python interpreter., ServiceState, WindowsServiceManager

### Community 447 - "The fifteen strategies"
Cohesion: 0.09
Nodes (21): adaptive *(default)*, automatic_failover, Candidate selection, context_length_optimized, cost_optimized, fallback_chain, highest_success_rate, least_loaded (+13 more)

### Community 448 - "LLM Router — troubleshooting"
Cohesion: 0.08
...
### Community 449 - "system_instruction"
Cohesion: 0.13
Nodes (7): Structured output normalization across LLM providers. Translates the OpenAI…, Return a plain-English JSON instruction for a ``response_format`` dict. Returns…, system_instruction(), system_instruction() uses stronger language when strict: true., TestSystemInstructionStrictMode, Tests for packages/ai/structured_output.py and its integration with the…, TestSystemInstruction

### Community 450 - "PriorityTaskQueue"
Cohesion: 0.14
...
### Community 451 - "test_agent_coordinate.py"
Cohesion: 0.23
Nodes (13): AgentConfig, build_agent_specs(), build_swarm(), Create a MultiAgentSwarm and agent specs from request configs., Agent definition for /agent/coordinate requests., Convert AgentConfig list to AgentSpec list for MultiAgentSwarm., Tests for agent/coordinate.py models, helpers, and v2 router., test_agent_config_defaults() (+5 more)

### Community 452 - "test_memory_guard.py"
Cohesion: 0.12
...
### Community 453 - "web_reach.py"
Cohesion: 0.12
Nodes (20): Ask the C-suite advisory layer a business question. This is the CEO's bridge…, _domain_matches(), _egress_policy_reason(), _parse_domain_list(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return True if *host* is *pattern* or a subdomain of it., Split a comma-separated env var into a normalised domain list., Return why governance policy blocks *host*, or None. Runs strictly *after* the… (+12 more)

### Community 454 - "test_openclaw_str_e_fix.py"
Cohesion: 0.10
...
### Community 456 - "_make_mock_response"
Cohesion: 0.12
...
### Community 457 - "trend_analysis.py"
Cohesion: 0.18
...
### Community 458 - "test_unit5_ui_provider_surface.py"
Cohesion: 0.12
Nodes (11): tests/test_unit5_ui_provider_surface.py — UNIT 5 regression tests. Verifies…, The dropdown shows a [free]/[paid]/[local] tier tag so the operator can tell…, The <option> tag uses providerLabel(p), not PROVIDER_LABELS[]., The operator must be able to see what a key really serves. ``candidates`` is…, Providers that were filtered out before UNIT 5 are now present. ``mistral``,…, A known paid provider is reported as tier=paid (was filtered before)., test_brain_card_jsx_dropdown_renders_provider_label(), test_brain_card_jsx_renders_tier_badge_in_dropdown() (+3 more)

### Community 459 - "Workspace"
Cohesion: 0.18
...
### Community 460 - "test_webui_provider_priority.py"
Cohesion: 0.07
Nodes (47): AdminIdentity, _fake_user_auth(), Path, test_admin_can_create_anthropic_provider_via_webui_admin_api(), test_admin_can_create_provider_via_webui_admin_api(), test_ui_providers_and_workspaces_use_app_state(), _bootstrap(), Path (+39 more)

### Community 461 - "safe_agency.py"
Cohesion: 0.22
...
### Community 462 - "LessonStore"
Cohesion: 0.14
Nodes (11): LessonStore, Any, Connection, Path, SQLite-backed store of failure lessons. Thread-safe, zero deps., refine() cannot cite what the store does not return., A workspace is often a third-party repo — its spec file is untrusted. Without…, A known signature must not license arbitrary text. (+3 more)

### Community 463 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.18
Nodes (14): _expire_stale_pending_report(), Auto-fail a pending SEO audit stub that is older than…, Store a report in the bounded in-memory registry., Run an audit from synchronous code, loop-safe. Used by the skill-bindings…, run_audit_sync(), _run(), save_report(), _pending_started() (+6 more)

### Community 464 - "_extract_tech_relevance"
Cohesion: 0.23
...
### Community 465 - "_hydrate_scheduler_bounded"
Cohesion: 0.10
Nodes (16): _hydrate_scheduler_bounded(), Attach durable persistence and rehydrate (#505), bounded by a budget. Without…, _BrokenScheduler, _FastScheduler, _HangingScheduler, asyncio, NoReturn, A fast hydration must not be deferred or warned about. (+8 more)
...
### Community 467 - "Design Audit"
Cohesion: 0.10
Nodes (19): Code Quality, Color and Surfaces, Component Patterns, Content, Design Audit, Fix Priority, How This Works, Iconography (+11 more)

### Community 468 - "Findings"
Cohesion: 0.10
...
Cohesion: 0.10
Nodes (17): _accumulate_usage(), _assistant_messages(), _message_text(), parse_event_stream(), ParsedRun, Any, Structured view of one ``--mode json`` event stream., Return assistant messages carried by an ``agent_end`` event. (+9 more)

### Community 470 - "Skill: modularity-review"
Cohesion: 0.10
Nodes (19): Acceptance Checks, Applying to This Repo, Further Reading, Modularity Findings Template, Part A: Reviewing Existing Code for Modularity Problems, Part B: Designing New Modular Boundaries, Skill: modularity-review, Step 1 — Map the dependency graph (+11 more)

### Community 471 - "crispy_client.py"
Cohesion: 0.14
Nodes (18): cmd_approve(), cmd_artifacts(), cmd_build(), cmd_events(), cmd_reject(), cmd_status(), cmd_watch(), _get() (+10 more)

### Community 472 - "._dispatch_tool"
Cohesion: 0.02
Nodes (115): Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Propagate the calling agent's identity across the process boundary. The MCP…, Changed, Changed, 1. Which sandbox backend applies where, 2. Container hardening, 3. Supply chain, 4. Troubleshooting (+107 more)

### Community 473 - "PortfolioScreen.jsx"
Cohesion: 0.11
Nodes (12): getPortfolioBoard(), refreshPortfolio(), btnStyle, HEALTH, HORIZONS, PortfolioScreen(), SOURCE, STATUS_COLOR (+4 more)

### Community 474 - "infra_cost.py"
Cohesion: 0.15
...
### Community 475 - "generate_specialist_skill_matrix.py"
Cohesion: 0.17
...
### Community 479 - "TestMCPToolsListCache"
Cohesion: 0.15
...
### Community 480 - "service_daemon.py"
Cohesion: 0.14
...
### Community 483 - "harness_registry.py"
Cohesion: 0.16
...
### Community 484 - "video_transcript.py"
Cohesion: 0.11
...
### Community 485 - "test_integration_c4_c5_c6_d3.py"
Cohesion: 0.16
...
### Community 486 - "test_anthropic_refusal_fallback.py"
Cohesion: 0.15
...
### Community 487 - "TestBrainFailoverModelUpdates"
Cohesion: 0.18
...
### Community 489 - "test_dashboard_cache.py"
Cohesion: 0.18
...
### Community 491 - "test_research_coordinator.py"
Cohesion: 0.11
...
### Community 492 - "test_tasks_cache_ttl_env.py"
Cohesion: 0.21
...
### Community 493 - "PhaseSequenceError"
Cohesion: 0.14
...
### Community 495 - "WorkspaceManager"
Cohesion: 0.12
...
### Community 496 - "PerformanceAnalytics"
Cohesion: 0.12
...
### Community 497 - "Skill: fabric-patterns"
Cohesion: 0.11
...
### Community 498 - "Analysis & Synthesis Instructions"
Cohesion: 0.11
Nodes (18): 1. Define the Atmosphere, 2. Map the Color Palette, 3. Establish Typography Rules, 4. Define the Hero Section, 5. Describe Component Stylings, 6. Define Layout Principles, 7. Define Responsive Rules, 8. Encode Motion Philosophy (+10 more)

### Community 499 - "Performance Analysis — local-llm-server"
Cohesion: 0.08
...
### Community 500 - "Production Readiness Assessment — local-llm-server"
Cohesion: 0.11
...
### Community 501 - "Skill: fabric-patterns"
Cohesion: 0.11
Nodes (18): 1. Ensure Pattern Directory Exists, 2. List Available Patterns, 3. Retrieve a Pattern, 4. Apply a Pattern with Variables, 5. Stitch Patterns Together, 6. Create New Patterns, Acceptance Checks, Directory Structure (+10 more)

### Community 502 - "Admin Dashboard Guide"
Cohesion: 0.11
...
### Community 503 - "Dynamic Model Routing"
Cohesion: 0.11
...
### Community 504 - "ControlsScreen.jsx"
Cohesion: 0.15
...
### Community 507 - "Delegation Plan (agent-ready work packages)"
Cohesion: 0.11
...
### Community 508 - "test_crispy_run_history.py"
Cohesion: 0.18
...
### Community 509 - "harness_routing.py"
Cohesion: 0.14
...
### Community 513 - "test_brain_patch_service_token.py"
Cohesion: 0.18
...
### Community 514 - "start_runtime"
Cohesion: 0.08
Nodes (32): (1) & partly (4): "Something went wrong" masks the real error everywhere, (2) & (3): Company creation flow / non-admin gate placement, A0. Fix live scanner crashes on real-world sites (`services/scanner.py`) — do first, Agent Prompt (paste this to start the implementation session), B. Make runtime activation non-blocking (`runtimes/control.py`,, C. Defer company persistence to a final "Confirm" step + relocate the email gate, Context — bugs reported during manual QA, E. General bug sweep (+24 more)

### Community 515 - "telegram_bot.py"
Cohesion: 0.03
Nodes (105): _configure(), _default(), main(), Entry point for the always-on FreeBuff Telegram bot (Render worker / Docker).…, Set an env var only when the operator hasn't already provided one., _admin_headers(), _answer_callback(), _api_headers() (+97 more)

### Community 516 - "test_pr923_fixes.py"
Cohesion: 0.07
...
### Community 517 - "test_voice_pipeline.py"
Cohesion: 0.14
...
### Community 518 - "validate_session_id"
Cohesion: 0.16
...
### Community 519 - "SEO / GEO / AIO Audit Engine"
Cohesion: 0.12
...
### Community 520 - "test_quick_note_to_task.py"
Cohesion: 0.52
...
### Community 522 - "Comprehensive Skill Index (By Category)"
Cohesion: 0.11
...
### Community 523 - "Agent Skill: Principal UI/UX Architect & Motion Choreographer (Awwwards-Tier)"
Cohesion: 0.11
Nodes (17): 1. Meta Information & Core Directive, 2. THE "ABSOLUTE ZERO" DIRECTIVE (STRICT ANTI-PATTERNS), 3. THE CREATIVE VARIANCE ENGINE, 4. HAPTIC MICRO-AESTHETICS (COMPONENT MASTERY), 5. MOTION CHOREOGRAPHY (FLUID DYNAMICS), 6. PERFORMANCE GUARDRAILS, 7. EXECUTION PROTOCOL, 8. PRE-OUTPUT CHECKLIST (+9 more)

### Community 524 - "OrchestratorSupervisor"
Cohesion: 0.10
...
### Community 529 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 530 - "Part A — Health Report"
Cohesion: 0.11
...
### Community 531 - "ApplyReviewAgent"
Cohesion: 0.14
...
### Community 539 - "TestStopSlopChecker"
Cohesion: 0.11
...
### Community 540 - "test_delegate_propagates_runtime_errors"
Cohesion: 0.25
...
### Community 541 - "test_startup_warmup.py"
Cohesion: 0.03
Nodes (58): backend, _maybe_boot_purge(), _purge_backlog_core(), _purge_summary_clean(), Await one warm-up step, deferring it to the background if it overruns.…, Shared purge implementation for the admin endpoint and the boot hook., True when a purge summary contains no error markers. _purge_backlog_core…, One-shot boot purge keyed to the PURGE_BACKLOG_ON_BOOT nonce. Lets the operator… (+50 more)

### Community 542 - "test_issue_triage.py"
Cohesion: 0.13
Nodes (18): _match_family(), Any, Classify a single GitHub issue payload and return the routing decision. Pure…, Fetch unlabeled open issues, triage each, and route them. Returns a summary…, run_triage_cycle(), _severity_for(), triage_enabled(), triage_one() (+10 more)

### Community 543 - "TestUpdateTask"
Cohesion: 0.18
...
### Community 545 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.33
...
### Community 546 - "ProceduralMemoryStore"
Cohesion: 0.17
Nodes (9): ProceduralMemoryStore, ProceduralRecord, Any, Store a successful step pattern and return its record id. Duplicate step…, Return up to *limit* stored patterns relevant to *query*. Relevance is scored…, Return all stored records (for admin/debug)., Remove all in-memory records. Returns the count removed., One stored skill pattern. (+1 more)
...
### Community 548 - "test_tasks_reconciler_todo_requeue.py"
Cohesion: 0.22
...
### Community 551 - "DeterministicEngine"
Cohesion: 0.21
...
### Community 552 - "SKILL: Industrial Brutalism & Tactical Telemetry UI"
Cohesion: 0.12
...
### Community 553 - "Skill: data-quality-audit"
Cohesion: 0.12
...
### Community 554 - "What "Slop" Looks Like"
Cohesion: 0.12
...
### Community 556 - "local_brain_router.py"
Cohesion: 0.19
...
### Community 557 - "Section-by-Section Acceptance Criteria"
Cohesion: 0.12
...
### Community 558 - "The rules"
Cohesion: 0.12
Nodes (16): Changing these rules, How the gate behaves, Quick-Note Context Rulebook, R10 — Use the repository's real identity **[gate]**, R11 — Name a real integration point **[gate]**, R12 — Mark epistemic status at the claim **[review]**, R1 — Ground the plan in the source before planning anything **[gate]**, R2 — Say what the artifact actually is **[gate]** (+8 more)

### Community 559 - "analyze_quantitative"
Cohesion: 0.15
Nodes (8): analyze_quantitative(), Compute descriptive statistics for a numeric series. Args: source: Where the…, Combine qualitative + quantitative findings into a decision-ready brief. If…, synthesize_research(), Combine quant + qual into a decision-ready brief., synthesize_brief(), TestAnalyzeQuantitative, TestSynthesizeResearch

### Community 560 - "HttpxFetcher"
Cohesion: 0.05
...
### Community 562 - "redact_connection_url"
Cohesion: 0.17
...
### Community 563 - "audit"
Cohesion: 0.05
Nodes (37): audit(), Append an audit log entry. Never logs raw secrets — only secret IDs / masked…, Go deeper, Quick start, Routes, Sol Advisor, Updating, What happens automatically (+29 more)

### Community 564 - "agent_readiness_audit.py"
Cohesion: 0.21
...
### Community 569 - "Current Sprint Tasks"
Cohesion: 0.05
Nodes (43): Perform MCP handshake. Optional — tools/call works without it., Fixed, Active Task Tracker, Bug Log, Current Sprint Tasks, Roadmap Items (from `docs/roadmap-killer-todos.md`), Fixed, getCompany() (+35 more)

### Community 571 - "_migration_block"
Cohesion: 0.15
...
### Community 572 - "test_frontend_deployment_guards.py"
Cohesion: 0.18
...
### Community 573 - "_process_task_callback"
Cohesion: 0.08
Nodes (30): Stable Core, Feature Matrix (spec §I — demotions needed), _process_task_callback(), Handle Approve/Reject inline-button presses for task execution gates. Callback…, _Captured, _make_fake_task(), _patch_workflow(), Robustness tests for the Telegram inline-button callback flow. (+22 more)

### Community 574 - "brain_failover.py"
Cohesion: 0.05
Nodes (69): Any, Force the next ``list_tools()`` call to fetch a fresh tool list from the…, Added, Added, getBrainProviders(), setBrainProviderEnabled(), BarChart(), ProviderHealthToggleCard() (+61 more)

### Community 575 - "_build_execution_request"
Cohesion: 0.24
...
### Community 578 - "Skill: repowise-intelligence"
Cohesion: 0.13
Nodes (14): 1. Graph Intelligence (Dependency Graph), 2. Git Intelligence, 3. Documentation Intelligence, 4. Decision Intelligence, Acceptance Checks, Directory Structure, Example Usage, Implementation Approach (+6 more)

### Community 579 - "handle_workflow_ide_chat"
Cohesion: 0.15
...
### Community 581 - "ErrorInterceptorMiddleware"
Cohesion: 0.19
...
### Community 586 - "Skill: branch-cleanup"
Cohesion: 0.14
...
### Community 589 - "ARCHITECTURE.md — Target Architecture"
Cohesion: 0.12
...
### Community 590 - "Security Analysis — local-llm-server"
Cohesion: 0.12
...
### Community 591 - "test_social_login_oauth.py"
Cohesion: 0.24
...
### Community 593 - ".apply_diff"
Cohesion: 0.03
Nodes (57): Security surface, Path, Return the first *lines* lines of a file. Just-in-time retrieval: the executor…, Return a lightweight index of files with line counts and sizes. This is the…, Resolve filepath relative to workspace_root, rejecting traversal attempts. Uses…, Security, 1. The Rules, A. Prime directive (+49 more)

### Community 594 - "The 10-Step Workflow"
Cohesion: 0.12
...
### Community 595 - "Contributing to local-llm-server"
Cohesion: 0.12
...
### Community 596 - "DirectChatSession"
Cohesion: 0.13
...
### Community 597 - "Agent Runtime Setup"
Cohesion: 0.12
...
### Community 599 - "nvidia_models.py"
Cohesion: 0.13
Nodes (21): _fetch_models_json(), _is_chat_model(), live_model_ids(), _rank_key(), rank_models(), Shared NVIDIA NIM model source for the autonomous agent scripts. This module…, The ids a caller should try, in order. Memoised: the agent loop asks once per…, ``(model_id, label)`` pairs, for callers that log a label. (+13 more)

### Community 600 - "PrioritizedTask"
Cohesion: 0.14
Nodes (10): IntEnum, PrioritizedTask, Priority, Any, Start the worker pool., Submit a task to the queue. Returns True if accepted, False if rejected due to…, Return queue introspection data for status endpoints., Task priority levels. Lower number = higher priority (executed first). (+2 more)

### Community 601 - "Instructions"
Cohesion: 0.14
Nodes (13): Acceptance Checks, `admin_auth.py` checklist, `agent/tools.py` checklist, Escalation, Instructions, `key_store.py` checklist, `proxy.py` auth middleware checklist, Risky Modules in This Repo (+5 more)

### Community 602 - "ai/router.py"
Cohesion: 0.08
Nodes (32): _exponential_backoff_cooldown(), _ollama_reasoning_effort(), provider_router.py — auto-generated module docstring (user-research skill scan)., # NOTE: 429 and 410 are handled specially in _try_one_provider (immediate, Return the configured ``OLLAMA_REASONING_EFFORT`` (``high``/``medium``/…, Compute exponential backoff cooldown for repeated 429 failures. Each…, Infer an access tier from *hostname*, longest matching suffix winning. Returns…, _record_429_failure() (+24 more)

### Community 603 - "fabric_cli.py"
Cohesion: 0.29
Nodes (15): cmd_apply(), cmd_list(), cmd_new(), cmd_save(), cmd_show(), cmd_stitch(), _ensure_patterns_dir(), main() (+7 more)

### Community 604 - ".publish"
Cohesion: 0.14
...
### Community 605 - "GitHubPolicyProbe"
Cohesion: 0.19
...
### Community 607 - "TestCreateCompanyValidation"
Cohesion: 0.33
...
### Community 609 - "DecisionsStoreTests"
Cohesion: 0.12
...
### Community 611 - "test_langfuse_agency_wide.py"
Cohesion: 0.12
...
### Community 613 - "test_migrate_local_brain_env.py"
Cohesion: 0.38
...
### Community 616 - "_redact_for_notification"
Cohesion: 0.18
...
### Community 618 - "test_sam_voice.py"
Cohesion: 0.12
Nodes (15): tests/test_sam_voice.py — Integration tests for SAM voice agent. Tests the SAM…, Same session_id must return the same session., SAM's system prompt must address the user as Commander., SAM agent with all external dependencies mocked., get_status() must return uptime and session count., Empty input must return a prompt to repeat., Whitespace-only input must be treated as empty., Fallback generic response must mention being on fallback mode. (+7 more)

### Community 619 - "TestKillSwitchDurability"
Cohesion: 0.15
...
### Community 621 - "AgentJobSnapshot"
Cohesion: 0.24
Nodes (9): AgentJobSnapshot, Complete point-in-time view of a job, safe to serialise as API response., Build a snapshot from an ``AgentJob`` dataclass instance., cancel_chat_agent_job(), get_chat_agent_job(), Resume a paused agent job for *session_id*. When the agent loop reaches a…, resume_agent_chat_job(), Build a minimal mock AgentJob-like object. (+1 more)
...
### Community 624 - "test_ai_insights.py"
Cohesion: 0.18
...
### Community 635 - "TestGemini3xCostTracker"
Cohesion: 0.16
Nodes (6): _cost_table(), tests/test_daily_automation_2026_09_13.py — Daily automation tests…, Gemini 3.x models must have pricing entries in the cost table., CLAUDE.md must no longer reference the deprecated deepseek-r1-70b as the Groq…, TestClaudeMdGroqReference, TestGemini3xCostTracker

### Community 636 - "_build_payload_or_500"
Cohesion: 0.18
...
### Community 638 - "CEO Micro-Management"
Cohesion: 0.13
...
### Community 640 - "OpenClaw — iOS Control of the Agency (Single-Service Free-Tier Deploy)"
Cohesion: 0.13
...
### Community 641 - "seo_api.py"
Cohesion: 0.05
Nodes (64): build_seo_roadmap(), delegate_seo_findings(), export_seo_audit(), get_seo_audit(), list_seo_audits(), plan_seo_sprint(), BaseModel, get (+56 more)

### Community 642 - "ChatScreen.jsx"
Cohesion: 0.14
...
### Community 644 - "test_pytest_failure_parser.py"
Cohesion: 0.23
...
### Community 645 - "OrchestratorQueue"
Cohesion: 0.17
...
### Community 646 - "StreamingDeltaReconstructor"
Cohesion: 0.26
...
### Community 648 - "test_phase5_doctor.py"
Cohesion: 0.12
...
### Community 649 - "TestBrainConfigUpdates"
Cohesion: 0.22
...
### Community 650 - "test_dockerfile_ships_config_dir.py"
Cohesion: 0.14
...
### Community 651 - "TestGeminiCostTrackerEntries"
Cohesion: 0.26
Nodes (5): _cost_table(), Gemini models must have entries in the cost table., Flash is on the free tier in this deployment — tracked as $0., 2.0 Flash fallback is on the free tier., TestGeminiCostTrackerEntries

### Community 652 - "test_task_service_failed_comment.py"
Cohesion: 0.18
...
### Community 656 - "Agent Orchestration Design"
Cohesion: 0.14
...
### Community 657 - "get_tool_registry"
Cohesion: 0.10
Nodes (18): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, auto_register(), Any, field_validator, Register the four user-research tools with a ``ToolRegistry``. Each tool wraps…, Register the user research tools into the module-level singleton registry.…, register_user_research_tools() (+10 more)

### Community 658 - "coordinator.py"
Cohesion: 0.20
Nodes (8): AgentCoordinator, CoordinatorResult, Any, Enum, str, agent/coordinator.py — Multi-Agent Coordinator One coordinator agent breaks a…, Run N worker AgentRunners in parallel under a single coordinator. Usage::…, TaskStatus

### Community 659 - "BudgetOptimizer"
Cohesion: 0.15
...
### Community 662 - "ResearchOrchestrator"
Cohesion: 0.18
...
### Community 664 - "Skill: lr-schedule-advisor"
Cohesion: 0.14
...
### Community 668 - "Checks Performed"
Cohesion: 0.14
...
### Community 669 - "Skill: training-stability-monitor"
Cohesion: 0.14
...
### Community 670 - "Skill: perplexity — Web Research via Perplexity API"
Cohesion: 0.14
...
### Community 672 - "Deduplicator"
Cohesion: 0.20
Nodes (5): Deduplicator, Collapse concurrent identical calls onto a single execution., Execute ``factory``, or await an identical call already running. Returns…, test_dedupe_failure_propagates_to_every_waiter(), test_identical_concurrent_requests_collapse_to_one_call()

### Community 673 - "Quick-Note Issues Processing Summary"
Cohesion: 0.14
...
### Community 674 - "Separate hosted dashboard backend (`backend/server.py`)"
Cohesion: 0.14
...
### Community 675 - "allow_paid"
Cohesion: 0.09
Nodes (25): 1. Mission & operating principles, 2. Brain policy (free cloud LLMs), 3. The Gate Matrix (core artifact), 4. Telegram gate protocol, 5. The five autonomous loops, 7. Definition of "fully autonomous" — acceptance criteria, 8. Safety invariants (carried from `agent/CLAUDE.md`), 🟢 Autonomous — run, then notify-only (+17 more)

### Community 676 - "_InMemoryBackend"
Cohesion: 0.18
...
### Community 684 - "workflow.py"
Cohesion: 0.17
Nodes (10): _append_transition(), classify_domain(), Enum, agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Classify domain from title+description; store on task_type., Add a workflow transition to the task's history list., Return the best-matching domain for a task title+description., parametrize (+2 more)

### Community 685 - "ui_chat"
Cohesion: 0.21
Nodes (10): `db/sqlite_store.py`, _anthropic_chat_payload(), _anthropic_text(), _provider_headers(), _provider_kind(), Any, Return True when two URLs share the same scheme, host, and port., provider_models() (+2 more)

### Community 686 - "test_wrapper_falls_back_to_installed_model"
Cohesion: 0.19
...
### Community 688 - "TestAnthropicToolListCaching"
Cohesion: 0.34
...
### Community 689 - "_StubManager"
Cohesion: 0.14
Nodes (3): _apply(), Minimal stand-in for BrainFailoverManager., _StubManager

### Community 690 - "TestHarnessAdapter"
Cohesion: 0.08
Nodes (5): tests/test_harness_adapter.py — Tests for ECC cross-harness adapter., Harness detection, normalization, and registration., Harness session tracking and metrics., TestHarnessAdapter, TestHarnessRegistry

### Community 691 - "TestDeprecationWarnings"
Cohesion: 0.17
...
### Community 692 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 693 - "test_task_clarification.py"
Cohesion: 0.17
...
### Community 695 - "ProviderRouter"
Cohesion: 0.07
Nodes (27): Added, Added, build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, _get_director(), is_commercial_provider(), _normalized_provider_type(), ProviderRouter (+19 more)

### Community 696 - "RegistrySkill"
Cohesion: 0.15
...
### Community 697 - "agent/workspace.py"
Cohesion: 0.29
...
### Community 698 - "AGENTS.md — Codebase Map & Operations Reference"
Cohesion: 0.15
...
### Community 700 - "CollaborationContext"
Cohesion: 0.21
...
### Community 703 - "Skill: Brain Dump"
Cohesion: 0.15
...
### Community 706 - "Skill: duplicate-thread"
Cohesion: 0.15
...
### Community 707 - "Skill: Email Triage"
Cohesion: 0.15
...
### Community 710 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
...
### Community 711 - "Skill: prompt-library"
Cohesion: 0.15
...
### Community 712 - "Skill: prompt-transparency"
Cohesion: 0.15
...
### Community 713 - "Deploy: FreeBuff Telegram bot (24×7)"
Cohesion: 0.09
...
### Community 715 - "Skill: scope-guard"
Cohesion: 0.15
...
### Community 716 - "admin_update_task_router.py"
Cohesion: 0.22
...
### Community 718 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
Nodes (12): Acceptance Checks, Claude's query protocol (use this instead of Read tool for exploration):, Graph Artifacts — What to Commit, How to Use the Graph (Token Savings Protocol), Installation (one-time per machine), Instead of reading raw files:, Key commands:, Relationship to repowise-intelligence Skill (+4 more)

### Community 719 - "Skill: platform-setup — Autonomous Agency Bootstrap"
Cohesion: 0.15
...
### Community 720 - "Device compatibility and model picks"
Cohesion: 0.15
...
### Community 721 - "Implementation Plan — DB-persisted, UI-switchable Brain (no redeploy)"
Cohesion: 0.14
...
### Community 724 - "TestEstimateTokensForMessages"
Cohesion: 0.15
...
### Community 725 - "_is_bedrock_model_id"
Cohesion: 0.27
...
### Community 726 - "test_task_brain_preflight.py"
Cohesion: 0.21
Nodes (12): _coordinator(), has_brain(), no_brain(), asyncio, BaseException, Brain-availability preflight tests (graceful degradation). When NO LLM brain is…, _RecordingRuntimeManager, store() (+4 more)

### Community 727 - "_get_provider_policy"
Cohesion: 0.19
...
### Community 728 - "test_providers_live_e2e.py"
Cohesion: 0.24
...
### Community 729 - "TestDisabledProvidersAreNotFalselyReportedUnreachable"
Cohesion: 0.17
...
### Community 730 - "test_compose_and_coordinate_api.py"
Cohesion: 0.19
...
### Community 731 - "TestModelCostTableUpdates"
Cohesion: 0.26
...
### Community 732 - "test_deploy_trigger_covers_image.py"
Cohesion: 0.21
...
### Community 733 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 734 - "TestRevenuePortfolio"
Cohesion: 0.20
...
### Community 735 - "467 Public Site Truth Spec"
Cohesion: 0.17
...
### Community 736 - "test_daily_2026_07_24.py"
Cohesion: 0.13
Nodes (10): extract_refusal(), is_strict(), Any, Extract the ``refusal`` string from an OpenAI-format response body. Returns the…, Return True when the caller has requested strict schema enforcement. Strict…, Daily automation tests — 2026-07-24. Covers three features added in this…, is_strict() detects strict: true inside json_schema., extract_refusal() surfaces model refusals from provider response bodies. (+2 more)

### Community 737 - "test_quick_note_engine.py"
Cohesion: 0.17
...
### Community 738 - "record_usage_endpoint"
Cohesion: 0.22
...
### Community 739 - "test_setup_api.py"
Cohesion: 0.19
...
### Community 741 - "test_seo_audit.py"
Cohesion: 0.08
Nodes (17): Aggregated report row - Screaming Frog CSV compatible., SeoIssueReportRow, compute_pressure(), _count_syllables(), estimate_pixel_width(), flesch_reading_ease(), loss_share_from_pressure(), Map aggregate issue pressure to an at-risk revenue share via the diminishing-… (+9 more)
...
### Community 750 - "Skill: agent-browser — Real Chrome Browser Automation"
Cohesion: 0.17
...
### Community 755 - "Quantization Internals"
Cohesion: 0.17
...
### Community 756 - "Autonomy Uplift — Living Roadmap & Detailed Implementation Specs"
Cohesion: 0.17
...
### Community 757 - "Traffic Distribution Across Providers"
Cohesion: 0.17
...
### Community 761 - "check_container_posture.py"
Cohesion: 0.26
...
### Community 763 - "test_probe_base_url_override_hits_the_typed_url"
Cohesion: 0.22
...
### Community 772 - ".test_set_github_token_sqlite_string_id_does_not_500"
Cohesion: 0.20
Nodes (3): MonkeyPatch, TestClient, TestGithubTokenSQLiteRegression

### Community 773 - "TestAuthAndTaskCreation"
Cohesion: 0.17
...
### Community 774 - "LogMonitor"
Cohesion: 0.07
Nodes (32): _dispatch_async(), _run(), _ErrorCaptureHandler, LogMonitor, _note_recurrence(), LogRecord, Register the error capture handler on the root logger., Best-effort: hand an operational failure to the incident tracker. Import is… (+24 more)

### Community 775 - "TestRunCoroSync"
Cohesion: 0.18
...
### Community 776 - "memory_consolidation.py"
Cohesion: 0.36
...
### Community 777 - "CapacityAllocation"
Cohesion: 0.25
...
### Community 780 - "test_colibri_brain_shim.py"
Cohesion: 0.07
...
### Community 782 - "InferenceCache"
Cohesion: 0.10
Nodes (14): CacheEntry, InferenceCache, Any, Look up a cached response for the given model + messages. Returns the cached…, Store a response in the cache., Remove a specific entry from the cache., Pre-populate the cache with known prompt→response pairs. Useful for seeding…, Build a messages list optimized for KV/prefix caching. Best practice from… (+6 more)

### Community 783 - "Tailored Onboarding, Editable Companies & Dynamic Roles"
Cohesion: 0.25
...
### Community 784 - "test_procedural_memory.py"
Cohesion: 0.22
Nodes (6): get_procedural_memory(), Return the process-wide ``ProceduralMemoryStore``, creating it if needed., tests/test_procedural_memory.py — Unit tests for agent/procedural_memory.py…, store(), TestClear, TestGetProceduralMemory

### Community 785 - "_keyword_search"
Cohesion: 0.20
...
### Community 786 - "_extractive_compress"
Cohesion: 0.18
...
### Community 791 - "Skill: pro-workflow"
Cohesion: 0.18
...
### Community 794 - "Skill: resource-panel"
Cohesion: 0.18
...
### Community 795 - "Skill: sandboxed-exec"
Cohesion: 0.18
...
### Community 796 - "Skill: dev-browser — Browser Automation via Sandboxed JS"
Cohesion: 0.18
...
### Community 797 - "ECC Harness Patterns Skill"
Cohesion: 0.18
...
### Community 800 - "Stop-Slop Quality Skill"
Cohesion: 0.18
...
### Community 801 - "Skill: user-research"
Cohesion: 0.18
Nodes (10): Architecture, Auto-Registration, Files, Purpose, Sample-Size Math, See Also, Sentiment + Theme Heuristics, Skill: user-research (+2 more)

### Community 802 - "tts.py"
Cohesion: 0.31
...
### Community 803 - "2. Critical Bugs & Exact Detection Signatures"
Cohesion: 0.18
...
### Community 804 - "Issue #467 — Section 1: Pulled State + PR Inventory"
Cohesion: 0.18
...
### Community 805 - "Deploy to Google Cloud Run"
Cohesion: 0.18
...
### Community 807 - "Sampling Strategies Internals"
Cohesion: 0.18
...
### Community 808 - "TestTheGatewayRegistryOffersOnlyLiveModels"
Cohesion: 0.27
Nodes (5): parametrize, A sixth model source, found on 2026-08-28 while verifying a fix.…, The platform default must survive ``require_tools`` filtering. Undeclared…, Every ``fallback_model`` must name a model this registry knows. Removing the…, TestTheGatewayRegistryOffersOnlyLiveModels

### Community 809 - "Killer TODO Roadmap — local-llm-server"
Cohesion: 0.18
...
### Community 810 - "CI Troubleshooting Runbook"
Cohesion: 0.18
...
### Community 811 - "NVIDIA NIM — Free Tier Setup"
Cohesion: 0.18
...
### Community 812 - "Worker Service — Operations Runbook"
Cohesion: 0.18
...
### Community 813 - "test_bedrock_live.py"
Cohesion: 0.25
...
### Community 814 - "_push_down_where"
Cohesion: 0.18
...
### Community 815 - "analyze_qualitative"
Cohesion: 0.25
...
### Community 816 - "migrate_local_brain_env.py"
Cohesion: 0.25
...
### Community 818 - ".mark_rate_limited"
Cohesion: 0.20
...
### Community 819 - "CircuitBreakerOpenError"
Cohesion: 0.50
...
### Community 820 - "TestTheSharedListFitsBothCallers"
Cohesion: 0.18
...
### Community 825 - "TestMCPClientStructuredOutput"
Cohesion: 0.38
...
### Community 827 - "TestDecisionsBotLinks"
Cohesion: 0.25
...
### Community 828 - "test_empirical_verify.py"
Cohesion: 0.49
...
### Community 829 - "test_event_log.py"
Cohesion: 0.45
...
### Community 830 - "TestZeroAttemptDiagnostics"
Cohesion: 0.25
...
### Community 831 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 832 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.22
...
### Community 834 - "test_hermes_server.py"
Cohesion: 0.18
...
### Community 836 - "financial_analyst.py"
Cohesion: 0.22
...
### Community 839 - "Protocol: Premium Utilitarian Minimalism UI Architect"
Cohesion: 0.20
...
### Community 840 - "Skill: repowise-intelligence"
Cohesion: 0.13
Nodes (14): 1. Graph Intelligence (Dependency Graph), 2. Git Intelligence, 3. Documentation Intelligence, 4. Decision Intelligence, Acceptance Checks, Directory Structure, Example Usage, Implementation Approach (+6 more)

### Community 841 - "The 5-Step Wrap-Up Ritual"
Cohesion: 0.20
...
### Community 842 - "._run_git_command"
Cohesion: 0.20
...
### Community 843 - "Brag Plan: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 845 - "Hyperframes Composition Brief: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 847 - "test_p0_roadmap_b1_c2_a3.py"
Cohesion: 0.14
Nodes (12): _infer_parameters_from_func(), Infer a basic JSON Schema from a function's signature., _inject_tool_results_as_messages(), Inject tool call results as follow-up messages for multi-turn execution. When…, BaseModel, Result of a single reward model scoring operation., RewardScore, Score must be between 0.0 and 1.0. (+4 more)

### Community 848 - "KnowledgeItem"
Cohesion: 0.06
Nodes (21): Acceptance checks, Fill these in, Skill: client-onboarding, Step 1 — Create the company and kick off onboarding, Step 2 — Poll progress, Step 3 — Verify specialists were provisioned, Step 4 — Confirm the 24x7 agency runtime is live, Step 5 — Note real gaps instead of pretending they're solved (+13 more)

### Community 849 - "Skill: Agentic Agile"
Cohesion: 0.20
...
### Community 850 - "wiki_client"
Cohesion: 0.22
Nodes (10): Changed, Fixed, [v4.1.0], Changed, Fixed, [v4.1.0], RoleBadge(), TestClient (+2 more)

### Community 851 - "Skill: browserbase-ui-test — Adversarial UI Testing"
Cohesion: 0.20
...
### Community 852 - "Skill: financial-analyst (Agentic CFO)"
Cohesion: 0.20
...
### Community 853 - "Graphiti Temporal Context Skill"
Cohesion: 0.20
...
### Community 855 - "Skill: seo-audit-report"
Cohesion: 0.20
...
### Community 856 - "Agent Readiness Report"
Cohesion: 0.20
...
### Community 857 - "467 Golden Path — Locked Implementation Order"
Cohesion: 0.20
...
### Community 858 - "_start_ceo_agency"
Cohesion: 0.27
...
### Community 859 - "LLM Router — provider guide"
Cohesion: 0.22
...
### Community 860 - "One command (recommended)"
Cohesion: 0.20
...
### Community 861 - "What to clean up"
Cohesion: 0.20
...
### Community 862 - "test_spa_trailing_slash_redirect.py"
Cohesion: 0.18
Nodes (3): Regression test: SPA catch-all no longer preempts trailing-slash redirects.…, _run_child(), spa_results()

### Community 863 - "ENGINEERING_STANDARDS.md — Patterns & Reference"
Cohesion: 0.20
...
### Community 867 - "Competitor Analysis — Autonomous AI Agency"
Cohesion: 0.20
...
### Community 868 - "_parse_reset_epoch"
Cohesion: 0.33
...
### Community 870 - "run_patched_colibri.py"
Cohesion: 0.27
...
### Community 873 - "test_backend_lifespan_skips_bg_when_flag_false"
Cohesion: 0.24
...
### Community 877 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 878 - "test_model_catalog_guard.py"
Cohesion: 0.29
...
### Community 880 - "test_key_pool.py"
Cohesion: 0.22
...
### Community 881 - "test_process_command_does_not_hang_when_context_stalls"
Cohesion: 0.20
...
### Community 883 - "test_workflow_api_mount.py"
Cohesion: 0.20
...
### Community 886 - "test_hosted_control_plane_regressions.py"
Cohesion: 0.73
...
### Community 891 - "_extract_workflow_relevance"
Cohesion: 0.33
...
### Community 894 - "Skill: changelog-enforcer"
Cohesion: 0.22
...
### Community 895 - "Skill: learn-rule"
Cohesion: 0.22
...
### Community 899 - "Skill: changelog-enforcer"
Cohesion: 0.22
Nodes (8): Acceptance Checks, Changelog Location, Entry Format, Examples, Hook Behaviour, Instructions, Skill: changelog-enforcer, When to Use

### Community 900 - "Skill: cowork-session (Claude Cowork)"
Cohesion: 0.22
Nodes (8): Branch, Components, Purpose, Quick Start, Session Roles, Skill: cowork-session (Claude Cowork), Testing, When to Use

### Community 901 - "Skill: video-context — read a video without watching it"
Cohesion: 0.22
...
### Community 902 - "ADR 003: Multi-Agent Orchestration with Plan-Execute-Verify Loop"
Cohesion: 0.22
...
### Community 903 - "The 8-Step Golden Path"
Cohesion: 0.22
...
### Community 904 - "Issue #1356: quick-note:https://searchengineland.com/turn-seo-backlog-into-roadmap-485713"
Cohesion: 0.22
...
### Community 905 - "Release Procedure"
Cohesion: 0.22
...
### Community 906 - "V2.0 Modernization — Runbook"
Cohesion: 0.22
...
### Community 907 - "_normalize_tool_choice"
Cohesion: 0.31
...
### Community 908 - "GovernanceScreen.jsx"
Cohesion: 0.25
Nodes (5): AuditTable(), BACKEND_META, DECISION_COLOR, relTime(), GovernanceScreen

### Community 909 - "test_scanner_deps_parity.py"
Cohesion: 0.31
...
### Community 910 - "implement_agent.py"
Cohesion: 0.13
...
### Community 914 - "TestAnthropicDefaultModel"
Cohesion: 0.40
Nodes (3): Verify the Anthropic provider default model was promoted to claude-sonnet-5., packages/llm/config.py env-provider fallback must also use sonnet-5., TestAnthropicDefaultModel

### Community 915 - "_NoopStore"
Cohesion: 0.25
...
### Community 917 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 921 - ".test_every_request_carries_a_real_user_agent"
Cohesion: 0.22
...
### Community 924 - "parse_react_response"
Cohesion: 0.47
...
### Community 926 - "test_process_quick_note_workflow.py"
Cohesion: 0.25
...
### Community 927 - "TestEveryFullSuiteJobHasMongo"
Cohesion: 0.22
...
### Community 928 - "CLAUDE.md — Operating Manual"
Cohesion: 0.29
...
### Community 931 - "get_control"
Cohesion: 0.18
Nodes (11): effective_value(), The value *spec* currently resolves to, following override → env → default., Where the effective value came from: ``override``, ``environment``, or…, value_source(), get_control(), The spec for *key*, or ``None`` when it is not operator-controllable., The call-volume throttle is present, numeric, and defaults to the calmer free-…, A runtime an operator cannot pick from the dropdown is unreachable. (+3 more)

### Community 932 - "stt.py"
Cohesion: 0.36
...
### Community 934 - "_is_trivial_message"
Cohesion: 0.29
...
### Community 941 - "hybrid_reasoning.py"
Cohesion: 0.36
...
### Community 942 - "quality_checker.py"
Cohesion: 0.32
...
### Community 944 - "_start_in_web_bot_tasks"
Cohesion: 0.29
...
### Community 945 - "prompts/README.md"
Cohesion: 0.03
...
### Community 946 - "Skill: browserbase-browser — Real Browser Automation"
Cohesion: 0.25
...
### Community 950 - "TestSupportMatrixDocsSync"
Cohesion: 0.29
...
### Community 951 - "GitHub Branch Protection Settings"
Cohesion: 0.25
...
### Community 954 - "AGENTS.md — AI Agent Configuration for local-llm-server"
Cohesion: 0.25
...
### Community 958 - "Issue #362: Nvidia repo setup"
Cohesion: 0.25
...
### Community 959 - "Issue #364: quick-note:https://www.marktechpost.com/2026/06/01/meet-memory-os-a-6-layer-open-source-memory-stack-built-on-top-of-hermes-agent/"
Cohesion: 0.25
...
### Community 960 - "Issue #378: quick-note:https://www.marktechpost.com/2026/06/02/tinyfish-launches-bigset-an-open-source-multi-agent-system-that-builds-structured-live-datasets-from-plain-english-descriptions/"
Cohesion: 0.25
...
### Community 961 - "Issue #379: quick-note:https://searchengineland.com/schema-markup-optimize-agentic-web-479080"
Cohesion: 0.25
...
### Community 962 - "Issue #380: quick-note:https://cursor.com/blog/cloud-agent-lessons"
Cohesion: 0.25
...
### Community 963 - "Issue #381: quick-note:https://www.xda-developers.com/claude-code-with-opus-48-is-expensive-but-i-made-it-efficient-with-my-local-ai-workflow/"
Cohesion: 0.25
...
### Community 964 - "Issue #382: quick-note:https://claude.com/blog/how-coderabbit-used-claude-to-build-an-agent-orchestration-system"
Cohesion: 0.25
...
### Community 965 - "Issue #383: quick-note:https://www.marktechpost.com/2026/05/29/hexo-labs-open-sources-sia-a-self-improving-agent-that-updates-both-the-harness-and-the-model-weights/"
Cohesion: 0.25
...
### Community 966 - "Issue #416: feat: Self-hosted Codebuff (freebuff) on free NVIDIA models + Telegram bot phone control"
Cohesion: 0.25
...
### Community 967 - "Issue #485: [Trend Digest] Week of 2026-06-08"
Cohesion: 0.25
...
### Community 968 - "Issue #488: quick-note:https://github.com/cookiy-ai/user-research-skill"
Cohesion: 0.25
...
### Community 969 - "Issue #491: Implement whatever is necessary from https://github.com/BehiSecc/awesome-claude-skills"
Cohesion: 0.25
...
### Community 970 - "Issue #493: Use the https://github.com/mvanhorn/last30days-skill skill to get the trend updated"
Cohesion: 0.25
...
### Community 971 - "Issue #495: Read https://www.anthropic.com/news/claude-fable-5-mythos-5 and understand if mythos or fable can be added to the repo"
Cohesion: 0.25
...
### Community 972 - "_build_curl_cffi_fetcher"
Cohesion: 0.36
Nodes (6): _build_curl_cffi_fetcher(), get(), get_text(), head(), _sess(), Return a PageFetcher that uses curl_cffi for all requests.

### Community 973 - "Issue #581: Sprint tracker: pending work after brand rename + mobile-first pass"
Cohesion: 0.25
...
### Community 974 - "TestExhaustedPoolDoesNotReusePrimary"
Cohesion: 0.29
...
### Community 975 - "Issue #657: quick-note:https://github.com/earendil-works/pi"
Cohesion: 0.25
...
### Community 976 - "Issue #659: quick-note:https://github.com/nex-agi/Nex-N2"
Cohesion: 0.25
...
### Community 977 - "Issue #660: quick-note:https://github.com/getsentry/sentry-for-ai"
Cohesion: 0.25
...
### Community 978 - "Issue #661: quick-note:https://github.com/XiaomiMiMo/MiMo-Code"
Cohesion: 0.25
...
### Community 979 - "Issue #664: quick-note:https://github.com/Grominet95/jarvis-OS"
Cohesion: 0.25
...
### Community 980 - "Issue #666: quick-note:https://github.com/porokka/jarvis-os"
Cohesion: 0.25
...
### Community 981 - "Issue #670: quick-note:https://github.com/perplexityai/bumblebee"
Cohesion: 0.25
...
### Community 982 - "Issue #672: quick-note:https://github.com/Chachamaru127/claude-code-harness"
Cohesion: 0.25
...
### Community 983 - "Issue #676: quick-note:https://github.com/WeiboAI/VibeThinker"
Cohesion: 0.25
...
### Community 984 - "Issue #820: quick-note:https://github.com/cobusgreyling/loop-engineering"
Cohesion: 0.25
...
### Community 987 - "SECTION C — Direct Chat Improvements (CBF / HRM)"
Cohesion: 0.25
...
### Community 988 - "Runbook — Instance Activation"
Cohesion: 0.25
...
### Community 989 - "TestAllCooling"
Cohesion: 0.29
Nodes (3): So the caller falls straight through to the provider-level cooldown — byte-for-…, A hostile or malformed Retry-After must not park a key indefinitely., TestAllCooling

### Community 990 - "test_job_failure_structures_runtime_preflight"
Cohesion: 0.29
...
### Community 991 - "_parse_tool_calls_from_response"
Cohesion: 0.39
...
### Community 992 - "PULL_REQUEST_TEMPLATE.md"
Cohesion: 0.25
Nodes (7): Changelog, Changes, Council Review (for larger PRs), Related, Risky Module Review, Summary, Testing

### Community 993 - "test_iteration_6_features.py"
Cohesion: 0.07
Nodes (30): codeql_count(), dependabot_count(), main(), Any, OpenClaw security fix helper. Lightweight CLI used by CI to check/fix…, _repo_parts(), _request(), requests (+22 more)

### Community 994 - "ADR 002: Dynamic Model Routing with Task Classification"
Cohesion: 0.25
Nodes (7): ADR 002: Dynamic Model Routing with Task Classification, Consequences, Context, Decision, Negative, Neutral, Positive

### Community 995 - "SQLiteStore"
Cohesion: 0.17
...
### Community 997 - "TestOneIdCanBelongToSeveralProviders"
Cohesion: 0.39
...
### Community 998 - "Prime Agent Runtime"
Cohesion: 0.25
...
### Community 999 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.29
...
### Community 1002 - "TestItIsNotBuiltForOneVendor"
Cohesion: 0.25
...
### Community 1003 - "test_doctor_coding_brain.py"
Cohesion: 0.32
...
### Community 1004 - "TestPoliciesGovernanceStableClaim"
Cohesion: 0.20
...
### Community 1006 - "test_nuclear_cleanup_targets_the_schedule_collection"
Cohesion: 0.25
...
### Community 1008 - "test_local_brain_router_smoke.py"
Cohesion: 0.25
...
### Community 1016 - "Troubleshooting"
Cohesion: 0.03
...
### Community 1023 - "build_connectors_router"
Cohesion: 0.38
...
### Community 1036 - "Platform Controls"
Cohesion: 0.29
...
### Community 1037 - "What's New"
Cohesion: 0.29
Nodes (7): 2026-06-16, 2026-06-25, 2026-06-26, 2026-07-04, 2026-07-05, 2026-07-09, What's New

### Community 1038 - "TOP 6 — Highest-ROI Items (Validated by Opus Research Agent)"
Cohesion: 0.29
...
### Community 1039 - "SECTION F — Developer Experience (CBF / ECC)"
Cohesion: 0.29
...
### Community 1047 - "WorkspaceEscapeError"
Cohesion: 0.40
...
### Community 1054 - "apply_phase1_changes.py"
Cohesion: 0.33
...
### Community 1057 - "TestReasonsAreActionable"
Cohesion: 0.29
...
### Community 1058 - "TestProvidersScreen"
Cohesion: 0.43
...
### Community 1061 - "TestMongoService"
Cohesion: 0.29
...
### Community 1062 - "test_provider_state_durability.py"
Cohesion: 0.15
...
### Community 1069 - "CachedLLMClient"
Cohesion: 0.22
Nodes (6): CachedLLMClient, Any, Return performance metrics for this client instance., Try to extract token count from various response formats., Wraps an LLM call function with inference caching. Usage: from agent.cached_llm…, Execute an LLM completion, using cache when available. Args: model: Model…

### Community 1071 - "Runtime troubleshooting"
Cohesion: 0.33
Nodes (4): Agent mode timeout, Missing binary / task harness, Runtime troubleshooting, Workspace validation failures

### Community 1072 - "AgentRole"
Cohesion: 0.40
Nodes (6): AgentRole, Enum, str, Lifecycle states for a research task., Specialized agent roles in the research team., TaskStatus

### Community 1073 - "Music Cues: happy-beats-business-moves-vol-1-by-ende-dot-app"
Cohesion: 0.33
...
### Community 1089 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 1090 - "Pre-Mortem Analysis: Agency Core autonomy story (Cloudflare deployment)"
Cohesion: 0.33
...
### Community 1091 - "SECTION B — NVIDIA / Cloud Model Integration (Nemotron / NVD)"
Cohesion: 0.33
...
### Community 1092 - "SECTION D — Deployment & Infrastructure (CHM / NVD)"
Cohesion: 0.33
...
### Community 1098 - "Feature Support Matrix"
Cohesion: 0.40
Nodes (5): Admin API, Config Overrides, Feature Support Matrix, Gating Behavior, Maturity Tiers

### Community 1099 - "get_stats"
Cohesion: 0.67
Nodes (3): get_stats(), Any, Return a JSON-serialisable snapshot of per-model cost attribution.

### Community 1100 - "test_p0_roadmap_a4_a5_b2.py"
Cohesion: 0.15
...
### Community 1103 - "nvidia_live_test.py"
Cohesion: 0.40
...
### Community 1121 - "TestTheAgentRunsCurrentCode"
Cohesion: 0.33
...
### Community 1123 - "TestInternalAgentAdapterProviderChain"
Cohesion: 0.40
...
### Community 1125 - "test_new_features_e2e.py"
Cohesion: 0.29
Nodes (11): APIRequestContext, do_login(), fail(), ok(), Page, Browser-based E2E tests for the 33 new roadmap features. Covers: - Dashboard UI…, Result, run_tests() (+3 more)

### Community 1126 - "_known_entry_texts"
Cohesion: 0.40
Nodes (5): _known_entry_texts(), _one_line(), Any, Recorded lessons as ``{signature: {acceptable text, ...}}``. The citation binds…, Collapse a value to a single trimmed line. Entries are one Markdown list item…
...
### Community 1129 - "make_isolated_workspace"
Cohesion: 0.08
Nodes (18): make_isolated_workspace(), Path, Create an isolated workspace directory under *root*. This is the legacy path…, _workspace_component(), _fake_user(), _FakeChatResult, _FakeResponse, Path (+10 more)

### Community 1130 - ".daily_active_users"
Cohesion: 0.40
...
### Community 1131 - "agents/commands.py"
Cohesion: 0.50
Nodes (4): CommandCategory, Enum, SuperClaude Slash Commands — CommandDispatcher with registration, role gating,…, Category grouping for discoverability and permission scoping.

### Community 1134 - "feature-implementer.md"
Cohesion: 0.40
Nodes (4): Before you edit, Prove it, Report back, While you edit

### Community 1135 - "/devops-check — DevOps Agent"
Cohesion: 0.40
Nodes (4): Deployment Checklist, /devops-check — DevOps Agent, Steps, When to use

### Community 1136 - "/docs-update — Documentation Agent"
Cohesion: 0.40
Nodes (4): /docs-update — Documentation Agent, Documentation Standards, Steps, When to use

### Community 1137 - "/qa-check — QA Agent"
Cohesion: 0.40
Nodes (4): /qa-check — QA Agent, Steps, What NOT to do, When to use

### Community 1140 - "/security-audit — Security Agent"
Cohesion: 0.40
Nodes (4): Escalation, /security-audit — Security Agent, Steps, When to use

### Community 1141 - "pre-push"
Cohesion: 0.70
Nodes (4): pre-push script, _error(), _head(), _info()

### Community 1142 - "Skill: browserbase-search — Structured Web Search"
Cohesion: 0.40
Nodes (4): Best practice: search → fetch → browse, Python snippet, Setup, Skill: browserbase-search — Structured Web Search

### Community 1144 - "Issue #230 — DUPLICATE"
Cohesion: 0.40
Nodes (4): Actions Taken, Issue #230 — DUPLICATE, References, Resolution

### Community 1145 - "AgentPlan"
Cohesion: 0.04
Nodes (39): AgentPlan, AgentStep, Activation, Agent: Planner (Architect), Failure Behaviour, Handoff, Output Format, Preferred Model (+31 more)
...
### Community 1160 - "TestWorkflowIntegration"
Cohesion: 0.50
Nodes (3): Tests for workflow orchestrator integration., Verify InternalAgentAdapter provides agent execution for workflows., TestWorkflowIntegration

### Community 1161 - "build_llama_cpp.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 1163 - "download_glm52_weights.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 1164 - "download_glm52_weights.sh script"
Cohesion: 0.70
Nodes (4): download_glm52_weights.sh script, fail(), ok(), warn()

### Community 1165 - "setup_colibri.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 1166 - "setup_colibri.sh script"
Cohesion: 0.70
Nodes (4): setup_colibri.sh script, fail(), ok(), warn()

### Community 1167 - "status_colibri_server.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 1170 - "TestMobileNavigation"
Cohesion: 0.40
Nodes (3): Mobile-specific: hamburger menu, responsive layout., Verify key pages load in mobile viewport., TestMobileNavigation

### Community 1173 - "reset_ceo_dispatcher"
Cohesion: 0.50
...
### Community 1177 - "TestHermesKeepAlive"
Cohesion: 0.40
...
### Community 1185 - "TestGhIsNotReAuthenticated"
Cohesion: 0.40
...
### Community 1189 - "TestModelRoleSeparation"
Cohesion: 0.40
...
### Community 1194 - "test_motor_event_loop_isolation.py"
Cohesion: 0.50
...
### Community 1215 - "pr_body.md"
Cohesion: 0.50
Nodes (3): Checklist, Rollout notes, Test plan

### Community 1217 - "test-anthropic.js"
Cohesion: 0.50
Nodes (3): ref_anthropic_ai_sdk, { Anthropic }, client

### Community 1219 - "stop_colibri_server.ps1"
Cohesion: 0.83
Nodes (3): Fail(), Ok(), W()

### Community 1221 - "submit_simple_task"
Cohesion: 0.50
...
### Community 1237 - "github"
Cohesion: 0.50
Nodes (3): github, enabled, silent

### Community 1242 - "WebhookSendRequest"
Cohesion: 0.67
Nodes (3): BaseModel, Body for ``POST /api/connectors/webhook/send`` (rule 11 — no raw dict in)., WebhookSendRequest

## Knowledge Gaps
- **3067 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3062 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14007 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **187 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AgentRunner` connect `AgentRunner` to `os`, `MultiAgentSwarm`, `backend/server.py`, `TaskSpec`, `Added`, `Fixed`, `Changed`, `proxy.py`, `TokenBudget`, `test_agent_chat_integration.py`, `Universality: case-coverage matrix`, `test_agent_runner.py`, `ContextManager`, `coordinator.py`, `LocalWorkspace`, `WorkspaceTools`, `test_verification_strategies.py`, `failover_chat_completion`, `CEODispatcher`, `webui/router.py`, `test_governance_api.py`, `TestAgentLoopMCPIntegration`, `test_ceo_dispatcher.py`, `PolicyEngine`, `AgentJobRequest`, `failover_client.py`, `HarnessEnrichment`, `Added`, `Fixed`, `test_fabric_patterns.py`, `Section-by-Section Acceptance Criteria`, `_resolve_push_token`, `prompts/README.md`, `WorkflowOrchestrator`, `TestDeprecationWarnings`, `pytest`, `StuckDetector`, `test_empirical_verify.py`, `FreeBuffAgent`, `_runner`, `ReactScratchpad`, `test_backend_server_features.py`, `build_tool_prompt`, `test_governance_sandbox.py`, `.apply_diff`, `test_direct_chat_evolution.py`, `engine.py`, `AgentSessionStore`, `._dispatch_tool`, `SECTION A — Agent Efficiency (Hermes / AOS / MYT)`, `Competitor Analysis — Autonomous AI Agency`, `TestInternalAgentAdapterProviderChain`, `ContextPruner`, `unsafe_target_reason`, `GitHubTools`, `AdaptiveHalter`, `UserMemoryStore`, `AgentPlan`, `E2BSandboxSession`, `Agency Core — Progress & Resume Log`, `RepoConnection`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `WorkspaceManager` connect `WorkspaceManager` to `TaskSpec`, `validate_job_id`, `validate_session_id`, `_hash_component`, `AgentJobManager`, `.test_concurrent_create_same_session`, `diagnostics.py`, `dry_clone_repo`, `Workspace Isolation Architecture`, `_safe_resolve`, `WorkspaceManifest`, `test_workspace_isolation.py`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `StopSlopChecker` connect `StopSlopChecker` to `.test_cleans_removes_double_spaces`, `.test_detects_multiple_throat_clearing`, `.test_detects_wh_starters`, `.test_cleans_emphasis_crutches`, `AITellIssue`, `quality_checker.py`, `TestStopSlopChecker`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
...
- **Should `os` be split into smaller, more focused modules?**
  _Cohesion score 0.013517665130568356 - nodes in this community are weakly interconnected._
- **Should `TaskSpec` be split into smaller, more focused modules?**
  _Cohesion score 0.010141739980449658 - nodes in this community are weakly interconnected._
- **Should `backend/server.py` be split into smaller, more focused modules?**
  _Cohesion score 0.009983397653681652 - nodes in this community are weakly interconnected._