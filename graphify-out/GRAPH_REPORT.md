...
## Corpus Check
- 1475 files · ~2,153,955 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30051 nodes · 56200 edges · 1248 communities (1081 shown, 112 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3323 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `20f6848f`
...
Cohesion: 0.01
Nodes (243): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+235 more)

### Community 1 - "backend/server.py"
Cohesion: 0.02
Nodes (218): Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry (+210 more)

### Community 2 - "TaskStatus"
Cohesion: 0.02
Nodes (177): classify_domain(), agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Return the best-matching domain for a task title+description., _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker… (+169 more)

### Community 3 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (131): int, Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key() (+123 more)
...
Cohesion: 0.02
Nodes (92): BusinessCategory, Company, CompanyGraph, CompanyGraphSnapshot, KnowledgeItem, SystemType, The core company entity - root of the Company Graph., The complete Company Graph - canonical core model for the Autonomous AI Agency.… (+84 more)

### Community 6 - "TaskSpec"
Cohesion: 0.04
Nodes (94): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, TaskResult, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag. (+86 more)

### Community 7 - "test_ceo_dispatcher.py"
Cohesion: 0.02
Nodes (125): CEOResult, _complexity_rank(), _decompose_into_subtasks(), _merge_changed_files(), Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, Build a single-task sub-task list (low-complexity fast path). (+117 more)

### Community 8 - "ExecutionRequest"
Cohesion: 0.03
Nodes (93): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), _NoopDB, Fallback in-memory store when no DB is available., stop_orchestrator_supervisor(), BoundContext, ExecutionRequest, ExecutionResult (+85 more)
...
### Community 10 - "Task"
Cohesion: 0.03
Nodes (87): Task, ApprovalCheckpoint, _coerce_ts(), Any, field_validator, Full task/issue document., Update the updated_at timestamp., Coerce ISO-8601 datetime strings (from DB) to float timestamps. (+79 more)

### Community 11 - "llm/router.py"
Cohesion: 0.03
Nodes (90): _load(), main(), failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Router-backed implementation of ``packages.ai.failover_client``'s entry point.…, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on… (+82 more)
...
Cohesion: 0.03
Nodes (95): AgentPhaseError, AgentRunner, _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), Any, Exception (+87 more)
...
Cohesion: 0.02
Nodes (111): _diagnose_and_file(), _format_incident(), gather_render_evidence(), _iso_from_monotonic(), normalise(), note_phase_end(), note_phase_start(), _now() (+103 more)

### Community 18 - "_fixture"
Cohesion: 0.02
Nodes (98): parse_event_stream(), ParsedRun, Structured view of one ``--mode json`` event stream., Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, app_client(), _clear_discovered_models(), _clear_response_cache(), client() (+90 more)
...
Cohesion: 0.04
Nodes (82): MCPUnavailableError, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any, Create the sandbox. Raises :class:`MCPUnavailableError` on failure., Kill the sandbox. Idempotent; never raises (best-effort cleanup). (+74 more)
...
Cohesion: 0.03
Nodes (101): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store() (+93 more)
...
Cohesion: 0.04
Nodes (93): ApprovalStatus, ApprovalStore, str, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`…, Block until *approval_id* is resolved or its TTL expires. Combines an event…, Recursively strip secrets from an arbitrary argument structure. Fails…, scrub(), BudgetTracker (+85 more)

### Community 27 - "loop.py"
Cohesion: 0.04
Nodes (80): loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _action_matches(), _as_list(), Decision (+72 more)
...
Cohesion: 0.03
Nodes (39): Any, Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking)., Return all registered runtimes with their cached health status., Return the active routing policy as a plain dict. (+31 more)
...
Cohesion: 0.04
Nodes (54): Register the built-in agent tools that are always available., _register_builtin_tools(), repowise.py — RepowiseIntelligence: context packing and dependency analysis., Path, tools.py — WorkspaceTools: read/write/search and diff application (risky…, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Return a previously saved memory value, or an empty string if absent., Persist a key/value pair to the user's profile store. (+46 more)

### Community 42 - "models/company_graph.py"
Cohesion: 0.05
Nodes (75): ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyUpdateRequest, Connector, DetectedSystem, Evidence, BaseModel (+67 more)

### Community 43 - "test_llm_router_resilience.py"
Cohesion: 0.04
Nodes (61): Backoff policy for retryable failures., RetryConfig, BreakerState, str, _digest(), get_ring(), KeyRing, KeyState (+53 more)

### Community 44 - "RenderOpsMonitor"
Cohesion: 0.05
Nodes (36): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, One deploy, normalised from whatever shape the tool returned., RenderDeploy, Polls Render for platform-level failures and files them as issues., Snapshot for ``/api/render/ops/status`` and the dashboard. ``self_heal_ready``…, Map service ID → human name, degrading to the ID when unavailable. (+28 more)
...
### Community 46 - "test_brain_availability_doctor.py"
Cohesion: 0.04
Nodes (60): brain_availability_summary(), Non-secret answer to "can the brain answer a request right now?". Three callers…, Attempt, _backend(), CEOLedger, get_ceo_ledger(), GoalRecord, _now() (+52 more)
...
### Community 50 - "get"
Cohesion: 0.03
Nodes (81): auth_me(), auto_recommend_skills(), discover_remote_skills(), _get_backend_sam(), _get_github_token(), get_github_tree(), get_provider_policy_route(), get_public_kpis() (+73 more)
...
Cohesion: 0.04
Nodes (70): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, get_trend_watcher(), set_trend_watcher(), build_ceo_router(), Any, APIRouter, backend/ceo_router.py — observability and manual control for the CEO. Surfaces… (+62 more)
...
Cohesion: 0.04
Nodes (39): Create a new company in SQLite., Update a company in SQLite., Delete a company and all associated data from SQLite., List companies from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON blob (which…, Create a new website in SQLite. The full model is stored in ``data`` so scan…, Get a website by ID from SQLite. (+31 more)
...
Cohesion: 0.05
Nodes (65): Replace the process-wide store. Tests only., reset_approval_store(), AuditLog, Bounded in-memory ring buffer plus a structured log stream. The ring buffer…, Store and emit *event*. Never raises., Return the most recent events, newest first, optionally filtered., Aggregate view for the dashboard and the metrics endpoint. ``would_block`` is…, Replace the process-wide audit log. Tests only. (+57 more)
...
Cohesion: 0.05
Nodes (65): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), get_approval_store(), Enum (+57 more)
...
Cohesion: 0.05
Nodes (54): get_ceo_dispatcher(), Return the shared CEODispatcher singleton., CEOSupervisor, _env_flag(), _env_int(), Any, services/ceo_supervisor.py — the CEO's 24x7 babysitting loop. Delegation alone…, What one sweep observed and did. Returned for tests and diagnostics. (+46 more)
...
Cohesion: 0.04
Nodes (52): _get_provider_policy(), Read the durable provider policy from DB, falling back to a safe default.…, allow_paid_brain(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, Resolve the LLM endpoint for agent execution (module-level, #522 failover).…, Resolve the LLM endpoint for a named surface (task/chat/ceo/sdlc/…). Honours…, _resolve_brain_provider(), resolve_provider_for() (+44 more)
...
### Community 77 - "runtimes/manager.py"
Cohesion: 0.04
Nodes (52): E2BAdapter, Any, TaskResult, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, _build_default_manager() (+44 more)
...
Cohesion: 0.04
Nodes (52): _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), _build_auto_skill_guidance(), _build_provider_router(), _builtin_provider_records(), call_llm(), _chat_error_detail_text() (+44 more)
...
Cohesion: 0.06
Nodes (34): Attempt, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any, AsyncClient, BaseException, One shared connection pool. Reconnects if it was closed. (+26 more)
...
### Community 97 - "gateway.py"
Cohesion: 0.05
Nodes (49): get_budget(), The process-wide budget tracker., get_cache(), The process-wide cache manager., gateway_enabled(), Whether the OpenAI-compatible passthrough routes accept traffic. Off by…, Whether callers should route through ``LLMRouter`` (ADR-008 §8). Defaults to…, router_enabled() (+41 more)
...
### Community 109 - "test_colibri_brain_shim.py"
Cohesion: 0.06
...
Cohesion: 0.06
Nodes (39): is_commercial_provider(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Regression tests for the agent loop's multi-provider failover wire format.…, The pieces the failover loop assembles for an Anthropic-native provider., TestAnthropicRequestShape (+31 more)
...
Cohesion: 0.06
Nodes (32): Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, Ollama provider adapter — local LLM inference., packages.ai — provider abstraction, model registry, and failover manager., ProviderManager, Any, packages/ai/manager.py — ProviderManager. Single entry point for all LLM calls.… (+24 more)
...
### Community 185 - "TestAuthAndTaskOwnership"
Cohesion: 0.05
...
### Community 204 - "test_v4_reliability_regression.py"
Cohesion: 0.07
...
### Community 233 - "test_provider_policy.py"
Cohesion: 0.07
Nodes (36): ProviderPolicyUpdate, Editable subset of the durable provider policy (paid-provider kill switch)., seed_default_providers(), asyncio, tests/test_provider_policy.py — Unit tests for the paid-provider kill switch.…, ProviderPolicyUpdate defaults allow_paid to False., ProviderPolicyUpdate accepts allow_paid=True., ProviderPolicyUpdate converts truthy strings to True (Pydantic v2 coercion).… (+28 more)
...
### Community 243 - "TestRuntimeControl"
Cohesion: 0.05
...
Cohesion: 0.15
Nodes (29): Complete result of one audit run., SeoAuditReport, Paragraph, Store a report in the bounded in-memory registry., save_report(), _appendix_full_findings(), _appendix_worst_pages(), _appendix_wsjf_roadmap() (+21 more)
...
Cohesion: 0.07
Nodes (7): CerebrasProvider, GroqProvider, NvidiaProvider, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., RateLimit, Provider rate limit info., Return the provider's rate limits.
...
Cohesion: 0.10
Nodes (16): list_seo_audits(), get, Return the full SEO/GEO/AIO check catalog. Static metadata only, but gated…, List stored audits for this company (most recent first)., seo_check_catalog(), Static definition of a single audit check (catalog entry)., SeoCheckDefinition, auto_fixable_checks() (+8 more)
...
Cohesion: 0.15
Nodes (24): Path, Start RuntimeManager, TaskDispatcher, SCHEDULER, and self-bootstrap. Returns a…, start_background_services(), Set the global task store instance (e.g., during app startup with MongoDB)., set_task_store(), _inmem_store(), _client(), asyncio (+16 more)
...
### Community 441 - "test_rate_limit_backoff_survives.py"
Cohesion: 0.15
...
### Community 443 - "test_agent_free_brain.py"
Cohesion: 0.10
...
### Community 456 - "weekly_digest.py"
Cohesion: 0.13
...
### Community 490 - "test_loop_registry.py"
Cohesion: 0.21
...
### Community 503 - "resolve_free_nvidia_brain"
Cohesion: 0.12
...
### Community 523 - "render_router.py"
Cohesion: 0.14
...
### Community 539 - "test_telegram_service_webhook.py"
Cohesion: 0.18
...
### Community 542 - "test_contract_enforcement.py"
Cohesion: 0.17
...
### Community 564 - "test_orchestrator_merge_decision.py"
Cohesion: 0.22
...
### Community 615 - "_check_storage_health"
Cohesion: 0.13
...
### Community 620 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 624 - "AdminDigestRouterAuthTests"
Cohesion: 0.22
...
### Community 625 - "test_freebuff_bot.py"
Cohesion: 0.14
...
### Community 626 - "telegram_service.py"
Cohesion: 0.16
...
### Community 664 - ".on_task_complete"
Cohesion: 0.16
...
### Community 688 - "get_orchestrator_queue"
Cohesion: 0.21
...
### Community 749 - "TestCompanyGraphModels"
Cohesion: 0.17
...
### Community 751 - "TestAnthropicPayloadStructuredOutput"
Cohesion: 0.29
...
### Community 815 - "flesch_reading_ease"
Cohesion: 0.24
...
### Community 817 - "_redact_for_notification"
Cohesion: 0.24
...
### Community 826 - "_FakePersistence"
Cohesion: 0.20
...
### Community 860 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.22
...
### Community 862 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 875 - "_build_direct_chat_schedule_suggestion"
Cohesion: 0.32
...
### Community 941 - "TestOneIdCanBelongToSeveralProviders"
Cohesion: 0.39
...
### Community 950 - "test_server_autonomy_and_index_fixes.py"
Cohesion: 0.29
...
### Community 1105 - "_isolate_operator_provider_state"
Cohesion: 0.50
...
- **3463 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3458 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13237 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **112 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `_Recorder`, `TaskStatus`, `test_llm_router_queue_cache.py`, `ExecutionRequest`, `Task`, `ImprovementLoop`, `TaskWorkflowService`, `test_llm_router_strategies.py`, `test_operational_incidents.py`, `E2BSandboxSession`, `OrchestratorCheckpointStore`, `SelfHealingAgent`, `test_company_api.py`, `test_autonomous_agency_e2e.py`, `loop.py`, `config.py`, `test_telegram_service_webhook.py`, `test_telegram_webhook.py`, `Agency`, `test_unit8_model_catalog.py`, `services/seo_audit.py`, `test_failover_client_shared.py`, `cost_tracker.py`, `WorkspaceTools`, `models/company_graph.py`, `test_llm_router_resilience.py`, `test_model_router.py`, `test_brain_availability_doctor.py`, `AgentSwarm`, `PrimeAgentAdapter`, `BrainWatchdog`, `test_openclaw_endpoints.py`, `test_orchestrator_merge_decision.py`, `services/background.py`, `test_task_service_failed_comment.py`, `AgentJobManager`, `test_knowledge_sync.py`, `test_agent_tool_governance.py`, `test_sam_livekit.py`, `test_ceo_router.py`, `ChatHistoryStore`, `get_registry`, `_resolve_brain_provider`, `SQLiteStore`, `runtimes/manager.py`, `_clean_director`, `brain_failover.py`, `test_telegram_auto_approve.py`, `_isolate_operator_provider_state`, `ResearchTask`, `test_autonomy_status.py`, `test_brain_failover_413.py`, `validate_policy_text`, `test_local_brain_state.py`, `TestClient`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `test_procedural_memory.py`, `OutputFilter`, `gateway.py`, `test_startup_warmup.py`, `get_self_healing_agent`, `test_sam_voice.py`, `test_provider_router.py`, `e2e/test_browser.py`, `test_daily_automation_2026_07_09.py`, `SeoFixer`, `test_context_rulebook.py`, `ArtifactStore`, `test_issue_intake.py`, `facade.py`, `run_task`, `test_response_cache.py`, `UserRole`, `FeatureMaturity`, `test_brain_failover.py`, `test_kimi_bridge_server.py`, `_step`, `clear_cooldowns`, `.on_task_complete`, `test_trend_watcher.py`, `test_openclaw_gateway.py`, `TestHarnessAdapter`, `test_trend_scoping.py`, `claim`, `persist_plan_spec`, `WorkflowBuildRequest`, `test_new_features_e2e.py`, `KeyStore`, `OllamaCircuitBreaker`, `_job_text`, `TestAuthAndTaskOwnership`, `_Budget`, `SchedulerStore`, `TestAgentRunnerSafety`, `test_features_api.py`, `test_video_transcript.py`, `test_provider_state_durability.py`, `TrendWatcher`, `test_task_brain_preflight.py`, `test_process_quick_note_workflow.py`, `test_portfolio_intake.py`, `NotificationDispatcher`, `platform_controls_router.py`, `TestEstimateTokensForMessages`, `TestClient`, `timedelta`, `test_agile_api.py`, `test_app_settings.py`, `test_task_clarification.py`, `TestRuntimeControl`, `ContextPruner`, `test_control_plane_api.py`, `test_implement_agent_routing.py`, `test_persistent_memory.py`, `SecurityScanner`, `api_keys_for`, `test_llm_router_e2e.py`, `test_platform_controls.py`, `get_store`, `test_rate_limiter.py`, `test_nvidia_model_discovery.py`, `capture_screens.py`, `test_direct_chat_evolution.py`, `_resolve_push_token`, `test_catalogue_probe.py`, `TestNormalizeResponseFormat`, `TestMongoGate`, `analyze_page`, `_Recorder`, `test_scheduler_hydration_bounded.py`, `OrchestratorQueue`, `model_discovery.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_e2b_data_flow.py`, `test_skill_registry_boot_refresh.py`, `TestSwarmRoleRouting`, `isolated_telegram_config`, `test_dashboard_cache.py`, `test_tasks_awaiting_approval_api.py`, `test_provider_enable_disable.py`, `_undeclared`, `_get`, `test_service_token.py`, `test_harness_spec.py`, `test_regression.py`, `test_agency_fix.py`, `test_webui_provider_priority.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_monitor_lib.py`, `test_v4_api.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `test_rate_limit_backoff_survives.py`, `UsageEvent`, `test_agent_free_brain.py`, `test_telegram_diag_endpoint.py`, `ClaudeCodeAdapter`, `client`, `test_doctor_coding_brain.py`, `test_voice_pipeline.py`, `test_brain_patch_service_token.py`?**
  _High betweenness centrality (0.096) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `proxy.py`, `AdaptiveHalter`, `FreeBuffAgent`, `TaskSpec`, `ExecutionRequest`, `test_ceo_dispatcher.py`, `TestAgentRunnerExecution`, `ReactScratchpad`, `test_ceo_supervision.py`, `E2BSandboxSession`, `test_ceo_micromanager.py`, `test_empirical_verify.py`, `test_autonomous_agency_e2e.py`, `ContextManager`, `loop.py`, `Agency`, `MCPClient`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `enforcement.py`, `WorkspaceTools`, `AgentSessionStore`, `StuckDetector`, `test_daily_automation_2026_07_11.py`, `AgentJobManager`, `TokenBudget`, `test_agent_free_brain.py`, `test_agent_tool_governance.py`, `InternalAgentAdapter`, `test_governance_api.py`, `direct_chat.py`, `TestAgentRunnerSafety`, `test_agent_chat_integration.py`, `LocalWorkspace`, `runtimes/manager.py`, `call_llm`, `test_brain_failover_413.py`, `UserMemoryStore`, `GitHubTools`, `ContextPruner`, `test_daily_automation_2026_07_09.py`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `implement_agent.py`, `backend/server.py`, `ProviderConfig`, `settings.py`, `test_all_providers_discovery.py`, `test_bedrock_live.py`, `clear_cooldowns`, `NvidiaProvider`, `test_rate_limit_backoff_survives.py`, `TestLegacyRouterServerFallback`, `TestLegacyRouterCacheTTL`, `test_anthropic_refusal_fallback.py`, `direct_chat.py`, `_payload`, `TrafficDirector`, `call_llm`, `test_bedrock_provider.py`, `TestOpenAiToBedrockConverse`, `ai/router.py`, `test_provider_router.py`, `test_colibri_brain_shim.py`, `TestAnthropicPayloadStructuredOutput`, `failover_client.py`, `test_implement_agent_routing.py`?**
