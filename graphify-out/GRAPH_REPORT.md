...
## Summary
- 30367 nodes · 56734 edges · 1280 communities (1113 shown, 119 thin omitted)
...
## Communities (1280 total, 119 thin omitted)

### Community 0 - "company_api.py"
Cohesion: 0.02
Nodes (188): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+180 more)

### Community 1 - "TaskSpec"
Cohesion: 0.03
Nodes (115): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., ClaudeCodeAdapter, json_safe(), Any (+107 more)

### Community 2 - "LLMRequest"
Cohesion: 0.03
Nodes (101): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, Normalise a configured ``kind`` to an adapter key., resolve_kind(), LLMRequest, Cheap character-based token estimate (~4 chars/token). Deliberately dependency-…, A provider-neutral chat request. ``model`` is a *hint*: the router may…, tests/test_anthropic_tool_list_caching.py — C6 extension: tool-list prompt… (+93 more)

### Community 3 - "post"
Cohesion: 0.03
Nodes (74): admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, BrainTestRequest, _build_agent_status_snapshot(), _build_agent_stream_event(), clear_response_cache() (+66 more)

### Community 4 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (83): Reset the singleton (test helper)., reset_ceo_dispatcher(), get_ceo_fallback_stats(), Increment the named counter (silently ignores unknown reasons)., Return a snapshot of the CEO fallback counters., Zero all CEO fallback counters (useful for testing)., _record_ceo_fallback(), reset_ceo_fallback_stats() (+75 more)
...
Cohesion: 0.03
Nodes (174): Describes a single worker's task and constraints., WorkerSpec, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), HTTPException, middleware, AdminIdentity, admin_control() (+166 more)
...
Cohesion: 0.03
Nodes (102): int, build_summariser(), chunk_document(), ContextManager, estimate_tokens(), FitResult, message_tokens(), prune() (+94 more)
...
Cohesion: 0.04
Nodes (84): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, governance_enabled(), True when the governance layer should run at all. A single global off switch…, _action_matches(), _as_list(), Decision (+76 more)
...
Cohesion: 0.07
Nodes (36): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, BaseModel (+28 more)
...
Cohesion: 0.05
Nodes (62): AgentPolicy, _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir(), _env_key_names(), expand_env() (+54 more)

### Community 22 - "gateway.py"
Cohesion: 0.03
Nodes (89): FailoverResult, A successful completion plus the accounting its callers need., get_cache(), The process-wide cache manager., failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.… (+81 more)
...
### Community 24 - "Task"
Cohesion: 0.03
Nodes (92): Task, ApprovalCheckpoint, _coerce_ts(), ExecutionLogEntry, Any, field_validator, Full task/issue document., Update the updated_at timestamp. (+84 more)
...
Cohesion: 0.04
Nodes (75): all_provider_ids(), _provider_ids_from_literal(), Resolve the model id for a component's role on a provider. Parameters…, Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, resolve_component_model(), CatalogActiveBrain, CatalogMirror (+67 more)
...
Cohesion: 0.06
Nodes (61): _get_github_token_for_user(), Fetch GitHub token for user from secrets store or environment., str, UserRole, _can_read(), _can_write(), create_secret(), _decrypt() (+53 more)
...
Cohesion: 0.02
Nodes (215): _agent_timeout_fallback_response(), ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills(), _build_direct_chat_schedule_suggestion(), _build_direct_chat_tags(), _build_direct_chat_task_suggestion() (+207 more)
...
Cohesion: 0.10
Nodes (22): Measure one server. Never raises — a probe failure is a status, not an error., Measure every declared server concurrently and return dashboard rows. Probes…, status_all(), _status_for(), asyncio, tests/test_mcp_registry.py — the single MCP catalogue and the screen it backs.…, A probe exception must not break the Providers page., Never claim connected without measuring it. (+14 more)

### Community 39 - "brain_config.py"
Cohesion: 0.03
Nodes (85): _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config_store(), is_north_mini_code_default(), provider_api_keys() (+77 more)
...
Cohesion: 0.05
Nodes (47): Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Feed LLM token/cost spend into the governance session budget. Kept separate…, _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Import the governance layer once, recording why if it is absent., BudgetExceeded, classify(), evaluate_call() (+39 more)
...
Cohesion: 0.03
Nodes (121): autonomy_tick(), Execute ONE pending task synchronously. Called by the cron workflow every 2…, Helpers that turn scheduler and playbook activity into real tasks., Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum (+113 more)
...
Cohesion: 0.05
Nodes (32): _keepalive_self_ping(), lifespan(), Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free…, Start the Telegram bot (and keep-alive) inside the web process when enabled.…, _start_in_web_bot_tasks(), _telegram_bot_supervisor(), get_store() (+24 more)

### Community 53 - "LLMRouter"
Cohesion: 0.04
Nodes (59): Attempt, auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable. (+51 more)

### Community 54 - "settings.py"
Cohesion: 0.04
Nodes (44): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, packages.config — single source of truth for all configuration. Every module…, packages/config/settings.py — typed configuration. This is the ONLY module in…, _audit_proposal(), _baseline_loosened() (+36 more)
...
Cohesion: 0.04
Nodes (61): AgentRunner, _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), Any, Path, Raise TypeError if fn's signature drifts from the locked contract (Pydantic… (+53 more)
...
Cohesion: 0.03
Nodes (68): get_skill(), list_skills(), provision_specialist(), Provision a new specialist for a company. Creates and provisions a specialist…, List all available skills with optional filtering. Returns the skill catalog…, Get a single skill by its ID., SpecialistFamily, Find all specialists of a specific family. (+60 more)
...
Cohesion: 0.04
Nodes (40): AgentJob, AgentJobManager, _now(), Any, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,…, Serialize the AgentJob to a JSON-serializable dictionary for external clients.…, Fail preflight when the provider extension is configured but missing. Without… (+32 more)
...
Cohesion: 0.04
Nodes (63): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is… (+55 more)
...
Cohesion: 0.04
Nodes (45): AnthropicProvider, Any, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Return the cache_control block appropriate for this provider's TTL setting.…, Add a rolling cache_control breakpoint to the most-recent stable message. In a…, Anthropic's extended-thinking constraints on ``budget_tokens``: at least 1024,… (+37 more)
...
### Community 104 - "test_colibri_brain_shim.py"
Cohesion: 0.06
...
Cohesion: 0.02
Nodes (149): _acquire_provider_probe(), CommercialFallbackRequiredError, _dead_model_key(), _exponential_backoff_cooldown(), extract_openai_text(), _get_director(), is_commercial_provider(), _mark_model_dead() (+141 more)

### Community 114 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (42): Find a repository by its URL., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, requires_db, Add a repository to a company. Args: company_id: Company ID url: Repository URL…, Initialize the service. Args: store: Optional CompanyGraphStore instance. If… (+34 more)
...
Cohesion: 0.11
Nodes (44): _governance_check(), _governance_identity(), Evaluate a runtime dispatch against policy; audit it either way. Returns…, Resolve the acting identity for a runtime dispatch. Identity comes from…, _decision(), _engine(), Any, RoutingDecision (+36 more)

### Community 117 - "get_task_store"
Cohesion: 0.06
Nodes (63): _purge_backlog_core(), Shared purge implementation for the admin endpoint and the boot hook., Reset the singleton (for tests)., reset_failover_manager(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog() (+55 more)
...
### Community 128 - "llm/router.py"
Cohesion: 0.03
Nodes (81): packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC, Any (+73 more)
...
Cohesion: 0.04
Nodes (75): One tool selection parsed from the executor model's JSON response., ToolCall, Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log. (+67 more)
...
Cohesion: 0.04
Nodes (64): _auto_disable(), _build_request(), _describe_registry(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion(), _paid_reserve() (+56 more)

### Community 146 - "TestAdapterMetadata"
Cohesion: 0.05
Nodes (24): AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., _env_float(), HermesAdapter, Any, AsyncClient, TaskResult (+16 more)
...
Cohesion: 0.05
Nodes (41): _escape_md_v1(), NotificationDispatcher, Any, Path, service_manager.py — Telegram & Notification Integration Extension Extends the…, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess). (+33 more)
...
Cohesion: 0.05
Nodes (81): Issue title: the failure mode plus how hard it is recurring., _company_attr(), company_stack_tags(), extract_stack_tags(), fan_out_trend(), fan_out_trends(), is_code_change_trend(), map_trend_to_company_task() (+73 more)

### Community 158 - "spec_store.py"
Cohesion: 0.20
...
### Community 169 - "test_unit6_resolve_component_model.py"
Cohesion: 0.05
...
Cohesion: 0.04
Nodes (78): add_initiative(), AllocationOut, BoardOut, get_board(), get_service(), InitiativeIn, InitiativeOut, _materialize_and_log() (+70 more)
...
Cohesion: 0.04
Nodes (52): repowise.py — RepowiseIntelligence: context packing and dependency analysis., Path, tools.py — WorkspaceTools: read/write/search and diff application (risky…, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Return a previously saved memory value, or an empty string if absent., Persist a key/value pair to the user's profile store., Return the first *lines* lines of a file. Just-in-time retrieval: the executor…, Return a lightweight index of files with line counts and sizes. This is the… (+44 more)
...
Cohesion: 0.07
Nodes (42): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), get_output_filter(), Get or create the singleton OutputFilter instance., observability_diag_public(), PUBLIC diagnostic endpoint for Langfuse — no auth required. Returns exactly… (+34 more)
...
Cohesion: 0.05
Nodes (57): _disabled_ids(), Reset every circuit breaker when they have all tripped, and re-select. Without…, The operator kill-switch set, or empty when it cannot be read., _recover_all_unhealthy(), BrainFailoverManager, ProviderHealth, ProviderInfo, Any (+49 more)
...
Cohesion: 0.10
Nodes (30): legacy_scheduler_create(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_trigger(), LegacyScheduleJobRequest, Force-dedup and clean stale schedules from the durable store. Admin-only…, scheduler_force_cleanup(), get_scheduler() (+22 more)
...
Cohesion: 0.03
Nodes (92): BrainResolution, get_brain_config(), invalidate_brain_config_cache(), provider_api_key(), Convenience wrapper used by the agent loop + brain resolver., Convenience wrapper used by the admin API endpoints., Clear the singleton's cache (used by tests + brain_policy invalidation)., Return the live API key for *provider* (env-only — never persisted). (+84 more)
...
### Community 307 - "get_skill_registry_safe"
Cohesion: 0.10
Nodes (29): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Consolidated system health report: preflight checks + runtime health. Returns a… (+21 more)
...
Cohesion: 0.07
Nodes (14): fake_mongo(), _FakeCollection, _FakeDb, isolated_state(), _live_mongo_url(), Operator provider state must survive a redeploy. The per-provider kill switch…, This is the document the UI toggle writes via _set_provider_policy., Temp SQLite mirror + clean caches, so no test sees another's state. (+6 more)
...
### Community 336 - "get_company_graph_store"
Cohesion: 0.04
...
Cohesion: 0.03
Nodes (57): KnowledgeItem, Find knowledge items matching any of the given tags., Structured knowledge about the company, systems, or processes., Create a new company in SQLite., Get a company by ID from SQLite., Update a company in SQLite., Delete a company and all associated data from SQLite., List companies from SQLite. (+49 more)
...
### Community 381 - "test_direct_chat_async.py"
Cohesion: 0.14
...
Cohesion: 0.06
Nodes (17): Company, Any, field_validator, The core company entity - root of the Company Graph., Coerce unrecognised system_type values to 'custom' so the model never crashes…, Get a company by ID. Args: company_id: Company ID Returns: Company instance or…, Update a company. Args: company_id: Company ID **kwargs: Fields to update…, Test Company Graph Pydantic models. (+9 more)
...
### Community 451 - "TestRenderMCPClient"
Cohesion: 0.12
...
Cohesion: 0.10
Nodes (13): get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor(), SupervisorState (+5 more)
...
### Community 517 - "get_current_user"
Cohesion: 0.07
...
### Community 549 - "test_fixes_reliability.py"
Cohesion: 0.11
...
### Community 564 - "test_minimax_bug_fixes.py"
Cohesion: 0.13
...
### Community 578 - "persist_plan_spec"
Cohesion: 0.16
...
### Community 632 - "TaskHarnessAdapter"
Cohesion: 0.15
Nodes (8): Any, TaskResult, Adapter for a compatible external task harness., TaskHarnessAdapter, TestTaskHarnessAdapterExecution, When task-harness is required but not present, preflight should return a…, Every preflight report must have runtime_id, ready, summary., TestRuntimePreflight
...
### Community 644 - "FeatureUnavailableError"
Cohesion: 0.14
...
### Community 667 - "test_brain_failover_413.py"
Cohesion: 0.13
...
### Community 706 - "TestTheGatewayRegistryOffersOnlyLiveModels"
Cohesion: 0.22
...
### Community 766 - "Request"
Cohesion: 0.07
Nodes (35): github_login(), github_repo_access(), github_webhook(), google_login(), JWTUserStateMiddleware, merge_pr_route(), openclaw_command(), PRMergeRequest (+27 more)
...
### Community 839 - "test_key_pool.py"
Cohesion: 0.22
...
### Community 879 - "._expire_run_once_orphan"
Cohesion: 0.29
...
### Community 886 - "Agent Autonomy Roadmap"
Cohesion: 0.25
...
### Community 890 - "Feature Maturity / Support Matrix"
Cohesion: 0.25
...
### Community 977 - "_known_tool_names"
Cohesion: 0.29
...
### Community 1008 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1025 - "Feature Support Matrix"
Cohesion: 0.33
...
### Community 1068 - "Who is this for?"
Cohesion: 0.40
...
### Community 1071 - "._resolve_merge_decision"
Cohesion: 0.40
...
### Community 1095 - "The 24x7 agency — your agents never go idle"
Cohesion: 0.50
...
### Community 1100 - "Privacy, security, and cost"
Cohesion: 0.50
...
### Community 1126 - "Configuration reference"
Cohesion: 0.67
...
### Community 1139 - "test_direct_adapter_does_not_bypass"
Cohesion: 0.67
...
- **3527 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3522 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13429 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **119 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `company_api.py`, `TaskSpec`, `parse_event_stream`, `get_current_user`, `test_llm_router_strategies.py`, `_Recorder`, `_FakeSandbox`, `test_llm_router_queue_cache.py`, `PolicyEngine`, `test_operational_incidents.py`, `test_kimi_bridge_server.py`, `ImprovementLoop`, `test_ai_insights.py`, `test_llm_router_resilience.py`, `config.py`, `gateway.py`, `Task`, `SelfHealingAgent`, `test_failover_client_shared.py`, `services/background.py`, `test_company_api.py`, `test_telegram_service_webhook.py`, `UserRole`, `test_telegram_webhook.py`, `test_unit8_model_catalog.py`, `test_llm_router_e2e.py`, `test_model_router.py`, `brain_config.py`, `ExecutionRequest`, `SeoAuditEngine`, `test_ceo_router.py`, `test_gh_brain_failover.py`, `enforcement.py`, `_mock_provider_records`, `AgentDefinition`, `TaskStatus`, `LLMRouter`, `settings.py`, `CEOLedger`, `ChatHistoryStore`, `test_openclaw_endpoints.py`, `PrimeAgentAdapter`, `AgentJobManager`, `TokenBudget`, `Agency`, `ResearchTask`, `KeyStore`, `TestClient`, `SQLiteStore`, `test_startup_warmup.py`, `test_procedural_memory.py`, `test_telegram_auto_approve.py`, `e2e/test_browser.py`, `resolve_e2b_config`, `test_autonomy_status.py`, `WorkflowBuildRequest`, `TestHarnessAdapter`, `test_langfuse_agency_wide.py`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_knowledge_sync.py`, `test_refresh_agent_built_proof.py`, `cache.py`, `test_sam_livekit.py`, `ProviderRouter`, `cost_tracker.py`, `TestSchedulerStore`, `test_context_rulebook.py`, `run_task`, `TestRuntimeControl`, `test_issue_intake.py`, `test_agent_tool_governance.py`, `test_new_features_e2e.py`, `_step`, `BackgroundAgent`, `ArtifactStore`, `test_trend_watcher.py`, `claim`, `test_brain_failover_413.py`, `NotificationDispatcher`, `test_trend_scoping.py`, `MCPClient`, `test_openclaw_gateway.py`, `TestSwarmRoleRouting`, `test_task_service_failed_comment.py`, `_StubProvider`, `test_tasks_awaiting_approval_api.py`, `test_tasks_reconciler_todo_requeue.py`, `test_daily_2026_07_27.py`, `clear_cooldowns`, `OllamaCircuitBreaker`, `_job_text`, `_Budget`, `test_task_brain_preflight.py`, `test_features_api.py`, `test_video_transcript.py`, `_start_ceo_agency`, `test_direct_chat_evolution.py`, `TrendWatcher`, `test_process_quick_note_workflow.py`, `FakeCollection`, `nvidia_models.py`, `AgentSwarm`, `TestAuthAndTaskOwnership`, `WorkflowRun`, `TestEstimateTokensForMessages`, `TestClient`, `tests/conftest.py`, `ContextPruner`, `test_agile_api.py`, `test_app_settings.py`, `api_keys_for`, `TrafficDirector`, `test_control_plane_api.py`, `test_brain_failover.py`, `test_persistent_memory.py`, `SecurityScanner`, `OutputFilter`, `timedelta`, `test_platform_controls.py`, `test_task_clarification.py`, `test_sam_voice.py`, `rate_limiter.py`, `test_nvidia_model_discovery.py`, `get_skill_registry_safe`, `reset_cache`, `TestNormalizeResponseFormat`, `test_provider_state_durability.py`, `OrchestratorCheckpointStore`, `session_retro.py`, `_resolve_push_token`, `test_catalogue_probe.py`, `analyze_page`, `test_key_pool.py`, `TestMongoGate`, `seo_api.py`, `OrchestratorQueue`, `test_e2b_data_flow.py`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_skill_registry_boot_refresh.py`, `ai/watchdog.py`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `test_provider_enable_disable.py`, `_undeclared`, `test_dashboard_cache.py`, `test_service_token.py`, `test_harness_spec.py`, `test_agency_fix.py`, `_get`, `test_monitor_lib.py`, `test_v4_api.py`, `test_webui_provider_priority.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_telegram_diag_endpoint.py`, `platform_controls_router.py`, `test_task_store_fails_loud_in_production.py`, `test_vision_routing.py`, `test_voice_pipeline.py`, `client`, `TestFeaturesAPI`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `TaskSpec`, `proxy.py`, `test_agent_runner.py`, `test_agent_loop_delegates_to_the_shared_client`, `loop.py`, `ReactScratchpad`, `test_agent_tool_governance.py`, `PolicyEngine`, `BackgroundAgent`, `AgentSessionStore`, `ContextManager`, `test_governance_sandbox.py`, `test_governance_api.py`, `test_brain_failover_413.py`, `test_failover_client_shared.py`, `TestAgentRunnerExecution`, `TestAgentRunnerSafety`, `CEODispatcher`, `test_autonomous_agency_e2e.py`, `MultiAgentSwarm`, `E2BAdapter`, `backend/server.py`, `test_empirical_verify.py`, `test_fixes_reliability.py`, `workflow_orchestrator.py`, `MCPClient`, `TestAgentLoopMCPIntegration`, `direct_chat.py`, `enforcement.py`, `StuckDetector`, `test_daily_automation_2026_07_11.py`, `WorkspaceTools`, `test_backend_server_features.py`, `_resolve_push_token`, `E2BSandboxSession`, `AgentJobManager`, `TokenBudget`, `test_agent_chat_integration.py`, `test_agent_free_brain.py`, `LocalWorkspace`, `FreeBuffAgent`, `.execute`, `WorkflowRun`, `UserMemoryStore`, `GitHubTools`, `AdaptiveHalter`, `ContextPruner`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `test_all_providers_discovery.py`, `failover_client.py`, `implement_agent.py`, `test_bedrock_live.py`, `backend/server.py`, `direct_chat.py`, `kimi_bridge_provider_config`, `clear_cooldowns`, `test_backend_server_features.py`, `TestLegacyRouterCacheTTL`, `TestAnthropicPayloadStructuredOutput`, `_payload`, `NvidiaProvider`, `TestRouterIntegration`, `test_bedrock_provider.py`, `TestLegacyRouterServerFallback`, `test_implement_agent_routing.py`, `test_colibri_brain_shim.py`, `system_instruction`, `test_anthropic_refusal_fallback.py`, `TrafficDirector`?**
