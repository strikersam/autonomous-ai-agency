...
## Corpus Check
- 1435 files · ~2,075,134 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 29094 nodes · 54055 edges · 1240 communities (1120 shown, 120 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2670 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `57d80791`
...
## Communities (1240 total, 120 thin omitted)
...
Cohesion: 0.03
Nodes (99): int, cosine_similarity(), payload_key(), Exact-match cache key over the fields that change the answer. Routing…, Cosine similarity between two vectors, 0.0 when either is degenerate., failover_chat_completion_via_router(), Router-backed implementation of ``packages.ai.failover_client``'s entry point.…, build_summariser() (+91 more)
...
Cohesion: 0.04
Nodes (85): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).… (+77 more)
...
Cohesion: 0.04
Nodes (92): BrainConfig, BrainConfigPatch, BrainConfigStore, get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), BaseModel, Persist *cfg* to Mongo (primary) and sqlite (mirror). Either backend failing is… (+84 more)
...
Cohesion: 0.03
Nodes (104): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent., AgentIdentity, _coerce_text(), new_session_id() (+96 more)

### Community 18 - "CEODispatcher"
Cohesion: 0.05
Nodes (63): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), _offload(), Any, Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a… (+55 more)
...
Cohesion: 0.03
Nodes (102): AgentPhaseError, AgentRunner, Exception, Return True when no file is touched by more than one step., Raised when a named agent phase (planning, verification, etc.) fails., GATE: Golden Path steps #7-12 — the primary agent execution loop. This is the…, Set a per-session token spend cap (★3 rollout token budget). When *cap* > 0 the…, Record token spend for *session_id* and enforce the budget cap. Logs a warning… (+94 more)
...
Cohesion: 0.03
Nodes (97): AgentCreateRequest, AgentDefinition, AgentStore, AgentUpdateRequest, Any, BaseModel, field_validator, agents/store.py — Persistent store for user-defined agent configurations.… (+89 more)

### Community 24 - "Agency"
Cohesion: 0.05
Nodes (57): Agency, AgencyCycleResult, AgentDirective, AgentRole, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context() (+49 more)
...
Cohesion: 0.03
Nodes (123): get_ceo_dispatcher(), _merge_changed_files(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., Collect changed_files across all specialists into a single de-duped list., reset_ceo_dispatcher(), BoundContext, ClassifyOutput (+115 more)

### Community 28 - "test_brain_availability_doctor.py"
Cohesion: 0.05
...
Cohesion: 0.03
Nodes (101): DetectedSystem, Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., A business system detected on a company's website or in their stack., Get the most confident evidence description., Inferred technology stack from website/repo analysis., StackInference (+93 more)

### Community 30 - "test_runtime_governance.py"
Cohesion: 0.09
Nodes (48): _blocked_result(), _governance_check(), _governance_identity(), Any, Build the failed TaskResult returned when policy denies a dispatch. A result…, Evaluate a runtime dispatch against policy; audit it either way. Returns…, Resolve the acting identity for a runtime dispatch. Identity comes from…, _decision() (+40 more)

### Community 31 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (42): Find a repository by its URL., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, requires_db, Add a repository to a company. Args: company_id: Company ID url: Repository URL…, Get a repository by ID. (+34 more)
...
Cohesion: 0.08
Nodes (8): GroqProvider, Provider, NvidiaProvider, Provider, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., RateLimit, Provider rate limit info., Return the provider's rate limits.

### Community 35 - "services/background.py"
Cohesion: 0.04
Nodes (76): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+68 more)
...
Cohesion: 0.04
Nodes (97): count, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, HealthTracker, Health and circuit-breaker state for every provider., Whether the breaker will admit a request right now. Transitions OPEN →… (+89 more)
...
Cohesion: 0.07
Nodes (83): BrainFailoverExhausted, failover_chat_completion(), FailoverResult, RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, A successful completion plus the accounting its callers need., Run one chat completion across the brain-failover chain. Tries each healthy…, _free_tier() (+75 more)
...
### Community 48 - "Any"
Cohesion: 0.04
Nodes (48): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), Any, Path, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic… (+40 more)
...
Cohesion: 0.03
Nodes (87): ApprovalRequest, ApprovalStatus, ApprovalStore, get_approval_store(), Any, Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The… (+79 more)
...
### Community 53 - "Path"
Cohesion: 0.12
Nodes (16): _get_workspace_lock(), get_workspace_manager(), _hash_component(), _load_workspace(), Any, BaseModel, Lock, Path (+8 more)
...
Cohesion: 0.05
Nodes (37): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, Run a job using the provided runner and update the job's lifecycle, progress,…, Create an isolated workspace directory under *root*. This is the legacy path… (+29 more)
...
Cohesion: 0.04
Nodes (71): MCPUnavailableError, Raised when the MCP server is unreachable or the circuit is open., TaskResult, TaskSpec, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, E2BSandboxSession, _inject_token() (+63 more)
...
Cohesion: 0.02
Nodes (118): _diagnose_and_file(), _format_incident(), gather_render_evidence(), get_operational_incident_tracker(), _iso_from_monotonic(), normalise(), note_phase_end(), note_phase_start() (+110 more)
...
Cohesion: 0.05
Nodes (60): Run a full SEO/GEO/AIO audit against a website and persist the evidence., run_seo_audit(), BaseModel, field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Aggregated report row - Screaming Frog CSV compatible. (+52 more)

### Community 75 - "loop.py"
Cohesion: 0.04
Nodes (57): Agent subsystem — planner / executor / verifier loop., loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, AgentEvent, AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest (+49 more)
...
Cohesion: 0.03
Nodes (89): AgentPolicy, _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir(), _env_key_names(), expand_env() (+81 more)
...
Cohesion: 0.06
Nodes (50): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion() (+42 more)
...
Cohesion: 0.06
Nodes (55): e2b_status(), Return the E2B sandbox integration status for the ProvidersScreen badge. Does…, e2b_enabled(), E2BConfig, _env_falsy(), _env_truthy(), is_e2b_sdk_importable(), services/e2b_config.py — Single reader of E2B sandbox environment config.… (+47 more)

### Community 86 - "test_procedural_memory.py"
Cohesion: 0.06
Nodes (25): get_procedural_memory(), _overlap_score(), ProceduralMemoryStore, ProceduralRecord, Any, agent/procedural_memory.py — Skill/Procedural Memory for the agent loop (★4).…, Store a successful step pattern and return its record id. Duplicate step…, Return up to *limit* stored patterns relevant to *query*. Relevance is scored… (+17 more)
...
Cohesion: 0.06
Nodes (37): _check_rate_limit(), default_keys_path(), issue_new_api_key(), KeyRecord, KeyStore, load_key_store(), Exception, Path (+29 more)
...
Cohesion: 0.05
Nodes (79): DirectChatDoctor, PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational() (+71 more)
...
### Community 119 - "call_llm"
Cohesion: 0.07
Nodes (34): get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, _agent_timeout_fallback_response(), _build_provider_router(), _builtin_provider_records(), call_llm(), _chat_error_detail_text(), _chat_provider_policy() (+26 more)
...
### Community 123 - "test_colibri_brain_shim.py"
Cohesion: 0.08
...
Cohesion: 0.10
Nodes (22): Path, PhaseType, WorkflowRun, Append an event to the workflow event log., Create a new WorkflowRun and begin pre-gate phase execution. The run is…, Return current WorkflowRun snapshot or None., Return a paginated list of runs, newest first., Approve the plan — lifts the ApprovalGate and resumes execution. Raises… (+14 more)
...
Cohesion: 0.04
Nodes (67): _bullets(), generate_backlog_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive a retrospective from the task tracker when no sprint is active. DONE /…, The result of allocating portfolio capacity into a new sprint. (+59 more)
...
Cohesion: 0.07
Nodes (42): _dead_model_key(), _exponential_backoff_cooldown(), get_dead_models(), _is_model_dead(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog(), _ollama_reasoning_effort() (+34 more)
...
### Community 177 - "engine.py"
Cohesion: 0.05
Nodes (46): When coder == reviewer model, swarm should log a warning., _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestPhaseSequenceError, TestWorkspaceIsolation (+38 more)

### Community 178 - "_get_provider_policy"
Cohesion: 0.07
Nodes (42): _get_provider_policy(), get_provider_policy_route(), ProviderPolicyUpdate, Read the durable provider policy from DB, falling back to a safe default.…, Editable subset of the durable provider policy (paid-provider kill switch)., Persist the durable provider policy and return the new state. Also writes to…, Return the provider policy (paid-provider kill switch state)., seed_default_providers() (+34 more)

### Community 179 - "test_e2b_data_flow.py"
Cohesion: 0.07
Nodes (32): fake_sandbox(), _FakeAsyncSandboxClass, _FakeCmdResult, _FakeCommands, _FakeFiles, _FakeSandbox, Any, asyncio (+24 more)

### Community 180 - "test_governance_api.py"
Cohesion: 0.07
Nodes (45): Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient, Tests for the governance HTTP surface and the AgentRunner integration. The…, Policy is a git-reviewed file. An HTTP mutation route would make "who changed…, The tool that makes turning on enforcement safe. (+37 more)
...
Cohesion: 0.05
Nodes (55): BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), _norm(), _pick_from_records(), Any, Shared free-brain policy helpers (issue #656). Single source of truth for "is… (+47 more)
...
### Community 207 - "TaskHarnessAdapter"
Cohesion: 0.09
...
Cohesion: 0.06
Nodes (26): Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry, TaskResult, TaskSpec, Admin-configurable routing policy. Defaults are conservative (local-first, no…, Full record of a routing decision — audit-trail entry. (+18 more)
...
Cohesion: 0.10
Nodes (17): _assigned_names(), models(), parametrize, The three autonomous agent scripts must share one NVIDIA model list.…, One candidate is what a turn-1 exhaustion looks like., apply_review.py listed the same model twice, wasting a retry., Source-text assertions cannot catch a NameError. Every test above reads the…, Runs only where `openai` is installed. It is not in requirements.txt —… (+9 more)
...
Cohesion: 0.04
Nodes (35): InitiativeProgress, PortfolioManager, PortfolioMetrics, Percentage of linked sprint points completed., Aggregate metrics across the whole portfolio., Manages a portfolio of initiatives with WSJF prioritisation and roadmapping., Create and register a new initiative, returning it., Add a pre-built Initiative (e.g. from the intelligence layer). (+27 more)
...
Cohesion: 0.08
Nodes (21): CommercialFallbackRequiredError, _get_director(), ProviderAttempt, ProviderRouter, Any, Response, RuntimeError, Try all models for one provider. Returns ProviderResult on success, None on… (+13 more)
...
### Community 231 - "Task"
Cohesion: 0.05
Nodes (57): Full task/issue document., Update the updated_at timestamp., Task, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, _coordinator(), asyncio, BaseException (+49 more)
...
### Community 240 - "get_skill_bindings"
Cohesion: 0.14
...
Cohesion: 0.04
Nodes (61): _best_cloud_primary_base(), InternalAgentAdapter, TaskResult, TaskSpec, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Execute a TaskSpec using the internal AgentRunner and convert the agent's… (+53 more)
...
Cohesion: 0.03
Nodes (92): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap(), CompanyAgencyService, get_company_agency_service() (+84 more)
...
### Community 276 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.11
...
Cohesion: 0.07
Nodes (30): _latest_metric_value(), _note_recurrence(), _parse_timestamp(), Any, datetime, services/render_ops.py — autonomous Render debugging + environment monitoring.…, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Pull the most recent numeric sample out of a ``get_metrics`` payload. Render… (+22 more)
...
Cohesion: 0.04
Nodes (57): Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Feed LLM token/cost spend into the governance session budget. Kept separate…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, events_from_identity(), Any, Return the most recent events, newest first, optionally filtered., Aggregate view for the dashboard and the metrics endpoint. ``would_block`` is…, Convenience: build and record an event in one call. (+49 more)
...
### Community 324 - "test_orchestrator_merge_decision.py"
Cohesion: 0.24
...
### Community 352 - "Agent Runtime Setup"
Cohesion: 0.12
...
Cohesion: 0.06
Nodes (41): delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), list_seo_audits(), BaseModel, get, post, SEO / GEO / AIO Audit API Router Endpoints for the world-class SEO audit engine… (+33 more)
...
Cohesion: 0.05
Nodes (56): AdminAuthManager, AdminIdentity, AdminSession, AdminSessionStore, _is_truthy(), admin_auth.py — auto-generated module docstring (user-research skill scan)., WindowsCredentialAuthenticator, _fake_user_auth() (+48 more)
...
### Community 383 - "verify_service_token"
Cohesion: 0.19
...
Cohesion: 0.04
Nodes (59): Backoff policy for retryable failures., RetryConfig, BreakerState, str, _digest(), KeyRing, KeyState, Per-provider round-robin key selection with cooldowns. (+51 more)
...
Cohesion: 0.09
Nodes (15): _iso_now(), _iso_offset_hours(), _parse_iso(), Create, open, and lifecycle-manage per-job isolated workspaces. All workspaces…, Open a workspace for resumption. Only READY or PAUSED workspaces may be…, Raise :exc:`WorkspaceAccessDeniedError` if *requesting_session_id* doesn't own…, Update *ws* status and persist the manifest., Update last_heartbeat timestamp and persist. (+7 more)
...
Cohesion: 0.04
Nodes (72): Any, Task, TaskStatus, Fail loud in production if no persistent storage is configured. Telegram…, Create a task. Deduplicates by source_id if set (Charter G3). If a task with…, Fetch a task by ID. If owner_id is set, enforces ownership., Return the task previously created for an external ``source_id`` (e.g.…, List tasks for a specific user with optional filters. ``include_system``… (+64 more)

### Community 514 - "backend/server.py"
Cohesion: 0.02
Nodes (193): backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, _actor(), backend/platform_controls_router.py — admin API for the platform controls.…, A stable identifier for the audit trail on each write. (+185 more)
...
### Community 524 - "test_providers_live_e2e.py"
Cohesion: 0.27
...
### Community 544 - "test_iteration_6_features.py"
Cohesion: 0.06
...
Cohesion: 0.03
Nodes (87): BusinessCategory, ApprovalPolicy, BusinessSystem, Company, CompanyCreateRequest, CompanyGraph, CompanyGraphSnapshot, CompanyUpdateRequest (+79 more)
...
### Community 631 - "_brain_provider_status"
Cohesion: 0.20
Nodes (10): _brain_provider_status(), get_brain_policy_route(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., Return the active brain config + per-provider key-present flags. The response…, _served_models(), Force a cache refresh (used by tests + the GET /admin/api/policy/brain…, refresh_brain_config_cache() (+2 more)
...
Cohesion: 0.13
Nodes (16): Enum, Exception, str, agent/workspace.py — Isolated workspace lifecycle management. Every agent…, Open an existing workspace from disk. Raises :exc:`WorkspaceNotFoundError` if…, Base class for all workspace errors., _read_manifest(), _validate_id() (+8 more)
...
### Community 643 - "ControlUpdateBody"
Cohesion: 0.67
...
### Community 687 - "_get_current_user"
Cohesion: 0.25
...
### Community 705 - "brain_config.py"
Cohesion: 0.04
Nodes (52): _active_primary_provider(), _build_base_url_env_from_yaml(), default_brain_config(), is_north_mini_code_default(), provider_api_key(), provider_base_url(), _provider_env_value(), _provider_ids_from_literal() (+44 more)
...
Cohesion: 0.02
Nodes (94): get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, start_orchestrator_supervisor() (+86 more)
...
### Community 795 - "_start_ceo_agency"
Cohesion: 0.31
...
### Community 829 - "tasks/models.py"
Cohesion: 0.07
Nodes (39): tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest, ExecutionApprovalRequest, ExecutionLogEntry (+31 more)
...
Cohesion: 0.13
Nodes (18): build_review_context(), _gh(), main(), Aggregate all review feedback for the PR into a single context string., Shared NVIDIA NIM model list — single source of truth for all autonomous agent…, allow_paid(), _fetch_policy(), .github/scripts/provider_policy.py — Read the durable provider policy from the… (+10 more)
...
Cohesion: 0.04
Nodes (69): packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC, Any, AsyncClient (+61 more)
...
### Community 999 - "get_db"
Cohesion: 0.03
Nodes (108): auth_me(), auto_recommend_skills(), _check_storage_health(), cost_attribution_stats(), create_access_token(), delete_github_token(), delete_source(), discover_remote_skills() (+100 more)
...
### Community 1001 - "_load_local_metrics_since"
Cohesion: 0.43
...
Cohesion: 0.03
Nodes (104): get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager., text_key(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on… (+96 more)
...
### Community 1212 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 1214 - "TestInternalAgentAdapterProviderChain"
Cohesion: 0.40
...
### Community 1215 - "TestDirectChatNonBlocking"
Cohesion: 0.40
...
### Community 1224 - "test_budget_is_operator_tunable"
Cohesion: 0.33
Nodes (4): call_llm must reach the env chain when DB providers are exhausted., Breadth vs quota is the operator's call, so both budgets are env vars., test_budget_is_operator_tunable(), test_call_llm_falls_through_to_the_shared_client()
...
- **Why does `_fixture()` connect `_fixture` to `TaskStore`, `test_provider_router.py`, `test_llm_router_queue_cache.py`, `test_company_api.py`, `test_langfuse_agency_wide.py`, `test_llm_router_tpm.py`, `validate_policy_text`, `resolve_identity`, `SelfHealingAgent`, `TaskWorkflowService`, `Agency`, `reset_failover_manager`, `test_provider_state_durability.py`, `PrimeAgentAdapter`, `test_brain_availability_doctor.py`, `clear_cooldowns`, `WebsiteScanner`, `test_iteration_6_features.py`, `NotificationDispatcher`, `services/background.py`, `test_agent_tool_governance.py`, `test_monitor_lib.py`, `test_openclaw_endpoints.py`, `test_unit8_model_catalog.py`, `ImprovementLoop`, `test_model_router.py`, `test_llm_router_strategies.py`, `_resolve_push_token`, `test_failover_client_shared.py`, `test_ceo_router.py`, `TestAgentRunnerSafety`, `test_process_quick_note_workflow.py`, `AgentJobManager`, `SQLiteStore`, `ChatHistoryStore`, `E2BSandboxSession`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `get_task_store`, `test_local_brain_state.py`, `test_operational_incidents.py`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `services/seo_audit.py`, `test_telegram_diag_endpoint.py`, `config.py`, `ResearchTask`, `TestClient`, `test_procedural_memory.py`, `TestFeaturesAPI`, `e2e/test_browser.py`, `TokenBudget`, `UserRole`, `parse_event_stream`, `test_knowledge_sync.py`, `test_startup_warmup.py`, `.on_task_complete`, `SeoFixer`, `KeyStore`, `direct_chat.py`, `claim`, `test_task_store_fails_loud_in_production.py`, `test_context_rulebook.py`, `model_router.py`, `ArtifactStore`, `run_task`, `test_issue_intake.py`, `test_openclaw_gateway.py`, `AgentSwarm`, `test_response_cache.py`, `test_tasks_reconciler_todo_requeue.py`, `test_doctor_service_token_check.py`, `BackgroundAgent`, `TestHarnessAdapter`, `MCPClient`, `BudgetTracker`, `test_new_features_e2e.py`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `_Budget`, `ai/router.py`, `llm/router.py`, `test_e2b_task_wiring.py`, `test_doctor_coding_brain.py`, `test_e2b_data_flow.py`, `OllamaCircuitBreaker`, `analyze_page`, `TestRuntimeControl`, `test_features_api.py`, `brain_config.py`, `test_video_transcript.py`, `TrendWatcher`, `_clear_cache`, `CacheManager`, `test_agile_api.py`, `test_app_settings.py`, `test_agent_scripts_share_one_model_list.py`, `test_portfolio_intake.py`, `OrchestratorCheckpointStore`, `test_trend_watcher.py`, `api_keys_for`, `clear_override`, `TestClient`, `Task`, `ContextPruner`, `BrainWatchdog`, `ExecutionRequest`, `reap_expired_companies`, `test_daily_2026_07_27.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `OutputFilter`, `test_platform_controls.py`, `capture_screens.py`, `test_scheduler_hydration_bounded.py`, `_resolve_user_github_token`, `_start_ceo_agency`, `test_rate_limiter.py`, `TestAuthAndTaskOwnership`, `test_sam_voice.py`, `OrchestratorQueue`, `test_purge_backlog.py`, `tasks/models.py`, `test_orchestrator_merge_decision.py`, `test_kimi_bridge_server.py`, `session_retro.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `test_control_plane_api.py`, `test_webui_provider_priority.py`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_brain_patch_service_token.py`, `test_harness_spec.py`, `test_ping.py`, `test_telegram_service_webhook.py`, `test_runtimes_health_endpoint.py`, `test_dashboard_cache.py`, `test_regression.py`, `test_agency_fix.py`, `test_telegram_mutating_commands.py`, `test_v4_api.py`, `client`, `SchedulerStore`, `TestMongoGate`, `_clean_director`, `test_voice_pipeline.py`, `_job_text`, `test_task_clarification.py`, `cost_tracker.py`, `TestNormalizeResponseFormat`, `test_task_service_failed_comment.py`, `test_ai_insights.py`, `test_sam_livekit.py`?**
  _High betweenness centrality (0.102) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `test_agent_free_brain.py`, `proxy.py`, `backend/server.py`, `TestAgentLoopMCPIntegration`, `TaskSpec`, `ReactScratchpad`, `resolve_identity`, `CEODispatcher`, `ContextManager`, `Agency`, `reset_failover_manager`, `MCPClient`, `test_ceo_dispatcher.py`, `test_e2b_adapter.py`, `test_agent_tool_governance.py`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `StuckDetector`, `test_backend_server_features.py`, `Any`, `WorkspaceTools`, `TestAgentRunnerSafety`, `LocalWorkspace`, `test_governance_api.py`, `AgentJobManager`, `E2BSandboxSession`, `enforcement.py`, `test_empirical_verify.py`, `test_agent_chat_integration.py`, `TestInternalAgentAdapterProviderChain`, `loop.py`, `test_agent_loop_delegates_to_the_shared_client`, `UserMemoryStore`, `TokenBudget`, `GitHubTools`, `AdaptiveHalter`, `ContextPruner`, `diagnostics.py`, `direct_chat.py`, `InternalAgentAdapter`, `ExecutionRequest`, `FreeBuffAgent`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `test_all_providers_discovery.py`, `backend/server.py`, `test_provider_router.py`, `test_bedrock_provider.py`, `test_anthropic_router.py`, `kimi_bridge_provider_config`, `test_colibri_provider.py`, `settings.py`, `implement_agent.py`, `clear_cooldowns`, `TestAnthropicPayloadStructuredOutput`, `NvidiaProvider`, `TrafficDirector`, `ai/router.py`, `TestRouterIntegration`, `ProviderConfig`, `TestOpenAiToBedrockConverse`, `model_discovery.py`, `failover_client.py`, `direct_chat.py`, `test_bedrock_live.py`, `call_llm`, `test_colibri_brain_shim.py`?**
