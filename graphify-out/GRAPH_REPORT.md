...
## Corpus Check
- 1597 files · ~2,314,342 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 55 file(s) not represented in the graph (top: (none) 17, .bat 5, .css 5)

## Summary
- 33920 nodes · 68956 edges · 1349 communities (1161 shown, 188 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 6749 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a640b1e4`
...
- Skill: Research
- Skill: scope-guard
- test_dependent_tasks_are_not_concurrent
- reset_store
- Instructions
- Skill: graphify — Knowledge Graph Token Optimization
...
1. `_fixture()` - 338 edges
2. `Task` - 237 edges
3. `AgentRunner` - 235 edges
4. `TaskStatus` - 151 edges
5. `TaskStore` - 147 edges
...
## Communities (1349 total, 188 thin omitted)

### Community 0 - "typing"
Cohesion: 0.01
Nodes (337): Browser admin UI for login, service control, key management, and diagnostics., ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered) Runs the…, agent/background.py — Background Agent An always-on worker thread that…, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, Cached LLM Client wrapper. Drop-in wrapper around any LLM API call that…, agent/code_graph.py — structural code queries for agents (opt-in). Wraps the…, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata… (+329 more)

### Community 1 - "build_llm_router"
Cohesion: 0.05
Nodes (56): get_cache(), The process-wide cache manager., gateway_enabled(), get_config(), Return the process-wide configuration, loading it on first use., Whether the OpenAI-compatible passthrough routes accept traffic. Off by…, Whether callers should route through ``LLMRouter`` (ADR-008 §8). Defaults to…, router_enabled() (+48 more)

### Community 2 - "LLMRequest"
Cohesion: 0.03
Nodes (112): Added, Added, ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, packages_llm_providers, build_provider(), Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the… (+104 more)

### Community 3 - "TaskSpec"
Cohesion: 0.01
Nodes (214): abc, WorkspaceIDError, N2. Surface Hermes (and all runtimes) status in the Doctor/Runtimes UI ⬜  (size: S, risk: low), kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag. (+206 more)

### Community 4 - "TaskStatus"
Cohesion: 0.03
Nodes (116): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., Submit a quick note instruction. Creates a Task with task_type='quick_note'., v4_quick_notes_submit(), Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, TaskStatus (+108 more)

### Community 5 - "get_db"
Cohesion: 0.03
Nodes (122): ApiKeyCreate, auth_me(), AuthorizeReposBody, auto_recommend_skills(), create_api_key(), create_github_pr(), delete_api_key(), delete_github_token() (+114 more)

### Community 6 - ".session_id"
Cohesion: 0.10
Nodes (22): _do_run_agent_job(), _run_agent_job(), wait_for_resume(), 1.1 Agent Identity, guard_tool_call(), GuardResult, _more_restrictive(), Any (+14 more)

### Community 7 - "_fixture"
Cohesion: 0.01
Nodes (145): parse_event_stream(), ParsedRun, Structured view of one ``--mode json`` event stream., Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, _clear_discovered_models(), _clear_response_cache(), _isolate_operator_provider_state(), MonkeyPatch (+137 more)

### Community 8 - "proxy.py"
Cohesion: 0.03
Nodes (166): set_quick_note_queue(), fastapi_middleware_cors, middleware, admin_control(), admin_create_key(), admin_create_user(), admin_delete_user(), admin_list_users() (+158 more)

### Community 9 - "Added"
Cohesion: 0.02
Nodes (241): get_agency(), set_agency(), Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., _first_paragraph(), Return the first non-empty, non-heading line. Skips YAML frontmatter (--- ...…, Reset token counters for all sessions (caps preserved). Called at the start of…, Reset all budgets if the UTC calendar day has changed since last reset. Safe to… (+233 more)

### Community 10 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (112): Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any (+104 more)

### Community 11 - "BrainConfig"
Cohesion: 0.02
Nodes (174): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), patch_brain_policy_route(), Apply a partial brain config update. Hard constraint: every changed model id is…, Design: one UI-controlled Provider Policy (single source of truth), 4. Tests (required — risky surfaces), Architecture (per plan §3), Files touched (+166 more)

### Community 12 - "TaskDispatcher"
Cohesion: 0.11
Nodes (18): Approve/reject parked tasks and queue fixes for open error alerts., Re-queue BLOCKED tasks that have cooled down and are ready for retry., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, _make_task(), asyncio, tests/test_dispatcher_iso8601.py — regression test for the TaskDispatcher auto-… (+10 more)

### Community 13 - "api.js"
Cohesion: 0.02
Nodes (51): approveGovernanceRequest(), autoRecommendCompanySkills(), createMcpServer(), createQuickNote(), createTask(), delegateSeoFindings(), deleteGithubToken(), deleteMcpServer() (+43 more)

### Community 14 - "test_ceo_micromanager.py"
Cohesion: 0.04
Nodes (105): Semaphore, Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, Run one sub-task, judge the result, and escalate it if it is slop. The loop is…, build_subtask_brief(), _coerce_subtasks(), decompose(), _env_flag(), _env_int() (+97 more)

### Community 15 - "SQLiteStore"
Cohesion: 0.03
Nodes (73): P1 — Close the remaining product gaps, Task 6 — Deeper website/repo scanning (more than "2 systems"), Task 7 — Knowledge base auto-reflect (ECC / Obsidian / graphify-style), Task 8 — Vector retrieval for the knowledge base (self-learn), DetectedSystem, KnowledgeItem, Find knowledge items matching any of the given tags., Structured knowledge about the company, systems, or processes. (+65 more)

### Community 16 - "Changed"
Cohesion: 0.02
Nodes (249): _close_github_issue(), _fetch_github_quick_notes(), _gh_repo(), _gh_token(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, Real handler — dispatches through AgentRunner when available. HARDENED (PR…, Public wrapper around _generate_plan for callers that want plan-only. Returns… (+241 more)

### Community 17 - "Task"
Cohesion: 0.03
Nodes (82): Full task/issue document., Update the updated_at timestamp., Task, Any, One-line human explanation of why this task needs human approval. Reads risk…, Best-effort label of who will run this task once approved. Returns…, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline… (+74 more)

### Community 18 - "PolicyEngine"
Cohesion: 0.04
Nodes (79): R3 — Reach a verdict, and let that verdict be "reject" **[gate]**, fnmatch, _action_matches(), _as_list(), Decision, GroupPolicy, _host_matches(), Mode (+71 more)

### Community 19 - "workflow_orchestrator.py"
Cohesion: 0.06
Nodes (66): contextvars, BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MergeDecision, MonitorOutput (+58 more)

### Community 20 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (63): _complexity_rank(), _decompose_into_subtasks(), get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, Build a single-task sub-task list (low-complexity fast path)., reset_ceo_dispatcher() (+55 more)

### Community 21 - "SelfHealingAgent"
Cohesion: 0.03
Nodes (76): FailureCategory, heal_signature(), HealingEvent, HealState, _now(), Any, Enum, str (+68 more)

### Community 22 - "AgentDefinition"
Cohesion: 0.04
Nodes (56): AgentDefinition, AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+48 more)
...
### Community 24 - "E2BSandboxSession"
Cohesion: 0.04
Nodes (71): MCPUnavailableError, RuntimeError, Raised when the MCP server is unreachable or the circuit is open., ★5 — Sandboxed Agent Execution (E2B / Docker micro-VM) [P1] [CHM] ✅ Delivered 2026-07-04, E2BSandboxSession, _inject_token(), Any, Create the sandbox. Raises :class:`MCPUnavailableError` on failure. (+63 more)
...
### Community 27 - "DashboardScreen.jsx"
Cohesion: 0.04
...
Cohesion: 0.05
Nodes (73): BudgetTracker, classify(), GovernanceGate, _host_of(), Extract a hostname from a URL, or return a bare host unchanged., Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., Return the ``(surface, action)`` a tool call really represents. The *action* is… (+65 more)

### Community 29 - "Agency"
Cohesion: 0.05
Nodes (41): Agency, AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _collect_recent_git_context(), _now_str(), Any (+33 more)

### Community 30 - "test_governance_api.py"
Cohesion: 0.08
Nodes (41): Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient, Tests for the governance HTTP surface and the AgentRunner integration. The…, Policy is a git-reviewed file. An HTTP mutation route would make "who changed…, The tool that makes turning on enforcement safe. (+33 more)

### Community 31 - "TrafficDirector"
Cohesion: 0.13
...
### Community 32 - "AgentRunner"
Cohesion: 0.01
Nodes (202): Adding a new tool, CLAUDE.md — agent/, Security surface, Skills worth invoking here, Testing, What this package does, AgentRunner, _run() (+194 more)

### Community 33 - "AgentJobSnapshot"
Cohesion: 0.22
...
### Community 34 - "get_improvement_loop"
Cohesion: 0.06
Nodes (56): get_improvement_loop(), _dispatch_async(), _run(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), LogRecord (+48 more)

### Community 35 - "Fixed"
Cohesion: 0.03
Nodes (82): Perform MCP handshake. Optional — tools/call works without it., Added, Fixed, Fixed, Active Task Tracker, Bug Log, Current Sprint Tasks, Added (+74 more)

### Community 36 - "sam_actions.py"
Cohesion: 0.11
...
### Community 38 - "build_governance_router"
Cohesion: 0.05
Nodes (61): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget() (+53 more)

### Community 39 - "test_scanner_security.py"
Cohesion: 0.11
...
### Community 40 - "test_telegram_freebuff.py"
Cohesion: 0.07
Nodes (41): Workflow Orchestrator Approval Gate (🔴 Telegram gate), _model_keyboard(), _parse_callback(), _parse_user_ids(), _process_callback(), Accept / reject keyboard shown after a FreeBuff plan is generated., Handle an inline-button press for the FreeBuff accept/reject/model flow., Extract numeric Telegram user IDs from a raw env value, tolerantly. Accepts… (+33 more)

### Community 41 - "backend/server.py"
Cohesion: 0.03
Nodes (130): set_skill_registry(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, _append_agent_session_message(), _build_agent_status_snapshot() (+122 more)

### Community 42 - "session_retro.py"
Cohesion: 0.13
...
Cohesion: 0.04
Nodes (30): _get_settings(), Typed configuration loaded from environment variables., When True, the governance layer evaluates and audits agent actions. This is…, When True, approval-gated actions self-approve. Local dev only., ``RENDER_SERVICE_IDS`` split into a clean list (empty when unset)., When True, mutating Render MCP tools may be called. Default False., When True, agents may drive a real browser (agent/browser.py)., When True, agents get code_trace/code_search/code_impact tools. (+22 more)

### Community 45 - "SpecEntry"
Cohesion: 0.06
Nodes (47): build_block(), _flag(), _int_env(), _known_entry_texts(), _one_line(), propose_entries(), Any, Path (+39 more)

### Community 46 - "mcp_registry.py"
Cohesion: 0.04
Nodes (58): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, Playwright, The MCP registry — one catalogue, measured status, _code_graph_configured(), _code_graph_spec(), _internal_configured(), list_specs() (+50 more)

### Community 47 - "test_render_mcp.py"
Cohesion: 0.04
...
### Community 48 - "services/seo_audit.py"
Cohesion: 0.06
Nodes (44): bs4, csv, io, lxml, BaseModel, field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, Snapshot of one crawled page with the on-page facts the checks used. (+36 more)

### Community 49 - "secrets_store.py"
Cohesion: 0.05
Nodes (65): Finding A — `list_for_user` Mongo query diverges from the `_can_read` policy, Finding B — `/api/secrets` router is mounted with no authentication dependency, Minor, non-security, Part 1 — The audit, Suggested fixes (not applied here), The chain, What was checked and is sound, UserRole (+57 more)

### Community 50 - "runtimes/api.py"
Cohesion: 0.02
Nodes (126): Available iff config resolves AND the SDK is importable. Never raises — a…, e2b_status(), _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy() (+118 more)

### Community 51 - "MongoDBStore"
Cohesion: 0.04
Nodes (44): Security, Security, MongoDBStore, Any, ObjectId, MongoDB implementation of the Company Graph store. Uses Motor (async MongoDB…, Get or create the MongoDB database connection., Convert string ID to ObjectId. (+36 more)

### Community 52 - "failover_chat_completion"
Cohesion: 0.05
Nodes (97): BrainFailoverExhausted, failover_chat_completion(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, Run one chat completion across the brain-failover chain. Tries each healthy…, mark_dead(), Hold *model_id* out of *provider_id*'s rotation for *ttl_sec* seconds. Called…, Drop every cached list, attempt record, and dead-model mark. For tests, and for… (+89 more)

### Community 53 - "failover_client.py"
Cohesion: 0.03
Nodes (103): Return the discovered model list for *provider_id*, or ``[]`` if unknown., _served_models(), Fixed, Fixed, _auto_disable(), _build_request(), _describe_registry(), _disable_unless_key_serves_other_models() (+95 more)

### Community 54 - "test_model_router.py"
Cohesion: 0.05
Nodes (75): classify_task(), _extract_recent_text(), Any, Concatenate plain text from the last *last_n* messages., Return the most likely task category for this request. Args: messages: OpenAI-…, Tests for the dynamic model router., _router(), test_agent_execute_classifies_as_code_generation() (+67 more)

### Community 55 - "test_ceo_supervision.py"
Cohesion: 0.03
Nodes (99): Configuration reference, CEODispatcher, CEOResult, _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, Aggregated output from a multi-specialist execution., Real CEO delegation: wake runtimes, fan out, and merge results., Record the goal and its sub-tasks in the ledger before any work runs. Writing… (+91 more)

### Community 56 - "Evidence"
Cohesion: 0.06
Nodes (40): Evidence, Inferred technology stack from website/repo analysis., Evidence supporting a system detection., StackInference, _hostname_contains(), _hostname_matches(), Any, datetime (+32 more)

### Community 57 - "looks_like_secret_file"
Cohesion: 0.03
Nodes (125): onboarding_gate_enabled(), Async read of the gate default straight from the DB., loops_overview(), Full Loop Engineering fleet view for the UI: the catalogued loops plus the…, seed_default_providers(), _get_cached_tasks(), _get_tasks_cache_lock(), Lock (+117 more)

### Community 58 - "MCPClient"
Cohesion: 0.04
Nodes (40): MCPClient, Any, Thin async MCP client with open/close circuit breaker. Thread-safe only within…, Full URL of the JSON-RPC endpoint this client posts to., Build the request headers shared by ``_rpc`` and ``notify``. ``Accept`` lists…, Propagate the calling agent's identity across the process boundary. The MCP…, Attach the agent identity whose actions this client executes., Decode a JSON-RPC response body from either JSON or an SSE stream. Streamable-… (+32 more)

### Community 59 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (73): BusinessCategory, Find a website by its URL., Find a repository by its URL., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo (+65 more)

### Community 60 - "HttpxFetcher"
Cohesion: 0.05
Nodes (30): MockTransport, browser_backend_available(), BrowserFetcher, FetchResult, HttpxFetcher, looks_blocked(), make_fetcher(), AsyncBaseTransport (+22 more)

### Community 61 - "test_e2b_adapter.py"
Cohesion: 0.07
Nodes (29): _FakeAsyncSandboxClass, _FakeCommandResult, _FakeCommands, _FakeFiles, _FakeSandbox, Any, asyncio, tests/test_e2b_adapter.py — E2BAdapter behaviour, mocked at the SDK seam.… (+21 more)

### Community 62 - "_mock_provider_records"
Cohesion: 0.06
Nodes (24): _mock_provider_records(), tests/test_orchestrator_failover.py — Provider failover, brain resolution,…, Non-Ollama providers without an API key are skipped., Ollama-type providers don't need an API key., When _list_configured_provider_records raises, falls back to OLLAMA_BASE., When all configured providers are excluded, falls back to Ollama., End-to-end provider failover through the WorkflowOrchestrator., First provider fails → retry picks next → llm_provenance tracks the winner. (+16 more)

### Community 63 - "RenderOpsMonitor"
Cohesion: 0.05
...
### Community 64 - "test_llm_router_resilience.py"
Cohesion: 0.03
...
### Community 67 - "test_self_heal_v2.py"
Cohesion: 0.05
Nodes (59): Reset the singleton (for tests)., reset_failover_manager(), _blocked_retire_age_sec(), _heal_brain_failover(), _heal_purge_backlog(), _heal_stuck_tasks(), _heal_telegram(), _heal_timestamps() (+51 more)

### Community 68 - "test_startup_warmup.py"
Cohesion: 0.05
Nodes (30): _isolate_warmup_overflow(), asyncio, RuntimeError, Regression tests for the bounded startup warm-up and login bootstrap. Uvicorn…, A deferred step that fails must not leak a reference or an exception., The deferred-bootstrap path: nothing registered, so wire it now., Whatever triggers it, wiring must register the pair., A hanging bootstrap must not hold /api/auth/login open. (+22 more)

### Community 69 - "allow_paid"
Cohesion: 0.04
Nodes (56): 1. Mission & operating principles, 2. Brain policy (free cloud LLMs), 3. The Gate Matrix (core artifact), 5. The five autonomous loops, 7. Definition of "fully autonomous" — acceptance criteria, 8. Safety invariants (carried from `agent/CLAUDE.md`), 🟢 Autonomous — run, then notify-only, Autonomy Charter — Telegram-Gated Self-Running Agency (+48 more)

### Community 70 - "AnthropicProvider"
Cohesion: 0.10
Nodes (25): Added, Fixed, Added, Fixed, AnthropicProvider, Any, AsyncClient, Translate OpenAI-shaped messages into Anthropic's system/turn split. (+17 more)

### Community 71 - "get_company_graph_store"
Cohesion: 0.03
Nodes (70): Added, What already exists (don't rebuild), Added, buildGraphElements(), _ProviderQuota, Response, Parse x-ratelimit-* headers and update per-provider quota state. Safe to call…, get_company_agency_service() (+62 more)

### Community 72 - "ProviderRouter"
Cohesion: 0.04
Nodes (57): _get_director(), is_commercial_provider(), _normalized_provider_type(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, ProviderRouter (+49 more)

### Community 73 - "scrub"
Cohesion: 0.06
Nodes (27): _luhn_ok(), Any, True when ``digits`` (bare, no separators) satisfies the Luhn checksum., Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Recursively strip secrets from an arbitrary argument structure. Fails…, Return the most recent events, newest first, optionally filtered., Aggregate view for the dashboard and the metrics endpoint. ``would_block`` is…, _redact_pii() (+19 more)

### Community 74 - "test_schedule_backlog_drain.py"
Cohesion: 0.15
...
### Community 75 - "agency_fix.py"
Cohesion: 0.04
Nodes (60): date, Popen, build_prompt(), call_llm(), collect_context(), collect_source_files(), decline_cleanly(), extract_failing_tests() (+52 more)

### Community 76 - "PrimeAgentAdapter"
Cohesion: 0.05
Nodes (32): _accumulate_usage(), _assistant_messages(), _child_env(), _message_text(), PrimeAgentAdapter, Any, Build the allowlisted environment for the CLI subprocess., Return assistant messages carried by an ``agent_end`` event. (+24 more)

### Community 77 - "test_web_reach.py"
Cohesion: 0.05
Nodes (60): _domain_list_reason(), _domain_matches(), Return True if *host* is *pattern* or a subdomain of it., Return why the env-var domain policy blocks *host*, or None. Checks…, Zero-key internet access: pages, YouTube transcripts, search, RSS. Every public…, WebReach, _fake_fetch_module(), _fake_video_module() (+52 more)

### Community 78 - "ExecutionRequest"
Cohesion: 0.04
Nodes (49): get_orchestrator_supervisor(), OrchestratorSupervisor, Any, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor(), ExecutionRequest, get_workflow_orchestrator() (+41 more)
...
### Community 81 - "chat_handlers.py"
Cohesion: 0.04
Nodes (71): Architecture Audit — local-llm-server, Architecture Diagram, Component Map, Layer 10 — WebUI (`webui/`), Layer 11 — Infrastructure, Layer 1 — API Proxy (`proxy.py`, 1719 lines), Layer 2 — Chat Handlers (`chat_handlers.py`, 710 lines), Layer 4 — Agent System (`agent/`) (+63 more)

### Community 82 - "FeatureMatrix"
Cohesion: 0.04
Nodes (21): FeatureMatrix, Central support matrix — single source of truth. Loads the canonical feature…, Return True if the feature is enabled and not disabled., Return a warning string for beta/experimental features, or None., Render the matrix as a Markdown table for docs., The feature matrix can produce a markdown table for docs., Every config flag referenced in the matrix should be documented., The matrix should cover the key areas from the spec. (+13 more)
...
### Community 87 - "SeoFixRequest"
Cohesion: 0.14
Nodes (11): Request to remediate auto-fixable findings in a local code repository., SeoFixRequest, Convenience wrapper used by the API layer and skill executor., run_fixes(), Path, tests/test_seo_fixer.py - repo-aware SEO auto-fixer tests (issue #533)., The fixer's output must satisfy the audit engine's fixable checks., repo() (+3 more)

### Community 88 - "resolve_component_model"
Cohesion: 0.03
Nodes (82): _catalog_defaults(), Resolve per-role defaults via the catalog. Returns None on import error., invalidate_brain_config_cache(), Clear the singleton's cache (used by tests + brain_policy invalidation)., Resolve the model id for a component's role on a provider. Parameters…, resolve_component_model(), UNIT 7 — Catalog propagation to all remaining call sites ✅, _default_model() (+74 more)

### Community 89 - "ref_react"
Cohesion: 0.07
Nodes (28): Removed, Removed, getMe(), login(), logout(), App(), AppRoutes(), frontend_src_app_default (+20 more)

### Community 90 - "test_provider_router.py"
Cohesion: 0.04
Nodes (52): _acquire_provider_probe(), _normalize_nvidia_base_url(), _openai_url(), Try to acquire a distributed probe lock for *provider_id*. Returns True if this…, Release the probe lock for *provider_id*., Normalize NVIDIA base URLs to avoid double /v1 when openai_compat_url appends…, _release_provider_probe(), _best_cloud_primary_base() (+44 more)
...
### Community 92 - "direct_chat.py"
Cohesion: 0.02
Nodes (167): PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), classify_direct_chat_intent() (+159 more)

### Community 93 - "test_cost_aware_routing_eval.py"
Cohesion: 0.06
...
Cohesion: 0.05
Nodes (32): createWikiPage(), deleteSource(), ingestSource(), updateCompany(), AgentsScreen, CompanyScreen, ExecutiveAdvisoryScreen, KnowledgeScreen (+24 more)

### Community 96 - "test_agency_fix.py"
Cohesion: 0.06
...
Cohesion: 0.05
Nodes (64): _push_down_where(), Build a SQL ``WHERE`` suffix from the subset of *query* conditions that map…, asyncio, tests/test_sqlite_store.py — Unit tests for the SQLite storage adapter. These…, The exact query shape backend/server.py's provider "Set default" uses: clear…, Unfiltered count uses the SELECT COUNT(*) fast path and must match the number…, estimated_document_count mirrors an unfiltered count_documents., db['tasks'] must work like db.tasks (motor exposes both). (+56 more)

### Community 98 - "test_webui_provider_priority.py"
Cohesion: 0.27
...
### Community 99 - "test_daily_digest.py"
Cohesion: 0.06
...
### Community 100 - "test_e2e_agent_chat.py"
Cohesion: 0.08
...
Cohesion: 0.08
Nodes (20): BrainWatchdog, Any, Reset failure counter for a provider after a successful call., Send a Telegram notification about the failover., Monitors provider health and triggers failover on consecutive failures., object, TestBrainWatchdog, tests/test_watchdog_name_normalization.py — PR #963 Verifies the watchdog… (+12 more)

### Community 102 - "test_all_features.py"
Cohesion: 0.03
...
### Community 103 - "TestOrchestratorQueue"
Cohesion: 0.13
...
### Community 104 - "BudgetTracker"
Cohesion: 0.08
Nodes (22): AlertHandler, BudgetTracker, _Dimensions, _month(), Any, Register a callback fired when a spend threshold is crossed., Fold a new key into "other" once the dimension is at its cap., Drop period buckets older than the retention window. (+14 more)

### Community 105 - "importlib_util"
Cohesion: 0.04
Nodes (29): _load(), main(), ModuleType, Scheduled agile ceremonies digest. Run by `.github/workflows/agile-…, _write_summary(), _load(), main(), Scheduled portfolio intelligence sweep. Run by `.github/workflows/portfolio-… (+21 more)

### Community 106 - "TestClient"
Cohesion: 0.10
Nodes (29): bare_repo(), _call(), _data(), git_config_env(), _is_error(), mcp_workspace_root(), Path, skipif (+21 more)

### Community 107 - "FreeBuffAgent"
Cohesion: 0.04
...
### Community 109 - "SECTION F — Developer Experience (CBF / ECC)"
Cohesion: 0.50
...
### Community 110 - "ProvidersScreen.jsx"
Cohesion: 0.06
Nodes (33): UI-first — an API is not "done", Phase 2 — Per-surface assignment in the UI (the "one place"), Modified files, createProvider(), deleteProvider(), getBrainConfig(), getBrainProviders(), getProviderPolicy() (+25 more)

### Community 111 - "AgentJobRequest"
Cohesion: 0.05
Nodes (33): AgentJobError, AgentJobRequest, AgentJobResult, Any, BaseModel, field_validator, agent/contract.py — Typed public contract for the agent job lifecycle. Phase 1…, Structured error payload attached to a failed job. (+25 more)
...
### Community 113 - "run_task"
Cohesion: 0.11
Nodes (17): _check_auth(), health(), Any, BaseModel, get, post, Execute a task synchronously via the InternalAgentAdapter (our brain). Response…, Body for POST /tasks — mirrors the payload HermesAdapter.execute sends. (+9 more)

### Community 114 - "workflow/api.py"
Cohesion: 0.10
Nodes (39): approve(), build(), cancel(), _engine(), get_agent_team(), get_artifact_content(), get_events(), get_run() (+31 more)

### Community 115 - "test_loop_registry.py"
Cohesion: 0.06
Nodes (53): audit_drift(), _cmd_audit(), DriftReport, _grade(), load_registry(), load_registry_sync(), loop_readiness(), LoopRegistry (+45 more)

### Community 116 - "BrowserSession"
Cohesion: 0.05
Nodes (37): browse_page(), BrowserAction, BrowserSession, _not_started(), PageState, Any, True if browser automation is enabled and Playwright is importable., Which backend a start() would use: 'browserbase', 'local', or 'stub'. (+29 more)

### Community 117 - "gsap.min.js"
Cohesion: 0.05
Nodes (33): Context(), Db(), Eb(), fb(), Hc(), ia(), Ic(), ie() (+25 more)

### Community 118 - "brain_failover.py"
Cohesion: 0.05
Nodes (64): Any, Force the next ``list_tools()`` call to fetch a fresh tool list from the…, brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, Added, Added, Feature Flags, Feature Maturity Overrides (+56 more)

### Community 119 - "config.py"
Cohesion: 0.03
Nodes (81): AgentPolicy, _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir(), _env_key_names(), expand_env() (+73 more)

### Community 120 - "activation.py"
Cohesion: 0.07
Nodes (48): activation_required(), ActivationResult, activation_status(), Public endpoint — returns instanceId and whether the instance is activated.…, _b64url_decode(), _b64url_encode(), _decode_jwt_unverified(), _generate_token_for_owner() (+40 more)

### Community 121 - "build_tool_calling_router"
Cohesion: 0.24
...
### Community 122 - "brain_config.py"
Cohesion: 0.03
Nodes (93): _build_base_url_env_from_yaml(), _build_candidates_from_yaml(), _build_default_base_url_from_yaml(), _build_display_names_from_yaml(), _build_key_env_from_yaml(), _build_presets_from_yaml(), _build_tier_from_yaml(), get_provider_candidates() (+85 more)

### Community 123 - "control_registry.py"
Cohesion: 0.11
Nodes (27): Adding a control, packages/config/control_catalogue.py — the 109 operator-facing controls. The…, coerce(), _coerce_choice(), _coerce_number(), _coerce_toggle(), controls_by_group(), Any (+19 more)

### Community 124 - "Added"
Cohesion: 0.07
Nodes (27): Ask the C-suite advisory layer a business question. This is the CEO's bridge…, _egress_policy_reason(), _parse_domain_list(), Split a comma-separated env var into a normalised domain list., Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _clean_heading(), distill(), _heading_key() (+19 more)

### Community 125 - "RepowiseIntelligence"
Cohesion: 0.04
...
### Community 126 - "context_plan_gate.py"
Cohesion: 0.13
...
### Community 132 - "Initiative"
Cohesion: 0.04
Nodes (64): _bullets(), generate_backlog_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive a retrospective from the task tracker when no sprint is active. DONE /…, The result of allocating portfolio capacity into a new sprint. (+56 more)

### Community 133 - "LogWatcher"
Cohesion: 0.05
...
### Community 134 - "TestExtendedCacheTTL"
Cohesion: 0.08
...
### Community 135 - "test_provider_state_durability.py"
Cohesion: 0.11
Nodes (11): fake_mongo(), _FakeCollection, _FakeDb, isolated_state(), _live_mongo_url(), Operator provider state must survive a redeploy. The per-provider kill switch…, Temp SQLite mirror + clean caches, so no test sees another's state., Return a reachable MONGO_URL, or None so the test skips. (+3 more)

### Community 136 - "AgileSprint"
Cohesion: 0.06
...
Cohesion: 0.07
Nodes (18): KnowledgeGraph, KnowledgeNode, Obsidian Knowledge Graph — KnowledgeNode and KnowledgeGraph with typed edges.…, Find all connected components (treating edges as undirected)., Find all nodes with a given tag., Export all edges as (source, target, edge_type) tuples., Number of nodes in the graph., Number of edges in the graph. (+10 more)
...
### Community 139 - "Added"
Cohesion: 0.04
Nodes (52): get_enrichment(), Return the enrichment instance for a workspace. Keyed by workspace root rather…, Inject available tools + skills catalog into the system message.…, Cooldown + hourly cap. Caller must hold the lock. Reserves the slot on success.…, Added, Added, 1. Executive summary, 3. Risk assessment (+44 more)

### Community 140 - "test_autonomy_status.py"
Cohesion: 0.11
...
### Community 145 - "get_user_role"
Cohesion: 0.06
Nodes (25): get_user_role(), has_permission(), is_admin(), is_power_user_or_above(), mask_dict(), _mask(), mask_secret(), Permission (+17 more)

### Community 146 - "test_llm_router_e2e.py"
Cohesion: 0.09
...
### Community 147 - "Configuration Reference"
Cohesion: 0.06
Nodes (30): Any, Return recent commits with agent attribution trailers parsed out., Agent governance — identity, policy, approvals, audit, sandboxes, Agent Models, Anthropic API Compatibility / Claude Code, Authentication and Keys, Claude Code setup, Configuration Reference (+22 more)
...
### Community 150 - "AgentScheduler"
Cohesion: 0.02
Nodes (127): Atomically delete a run-once row only while it is still unfired. Closes the…, _in_container(), Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, Return True when running inside a container runtime. ``/.dockerenv`` alone…, _scheduler_on_fire(), Fixed, 4. Architecture reference, Auth flows (+119 more)
...
### Community 152 - "activation_api.py"
Cohesion: 0.09
...
### Community 155 - "register_webui"
Cohesion: 0.09
Nodes (28): Any, Path, run_command(), _admin_out(), FastAPI, register_webui(), _admin_app_index(), _admin_app_spa() (+20 more)

### Community 156 - "sync/service.py"
Cohesion: 0.14
...
### Community 157 - "test_issue_intake.py"
Cohesion: 0.07
Nodes (47): _capability_tags(), intake_issue(), _issue_labels(), issue_source_id(), map_issue_to_task(), Any, tasks/issue_intake.py — Auto issue → Task intake (Autonomy Charter G3) Turns…, Map issue labels to coarse capability tags for routing/triage. (+39 more)
...
### Community 159 - "ENGINEERING_STANDARDS.md — Patterns & Reference"
Cohesion: 0.08
...
### Community 160 - "checkpoint.py"
Cohesion: 0.15
Nodes (15): Checkpoint, checkpoint_agent_state(), _checkpointing_enabled(), cleanup_checkpoints(), _get_checkpoint_store(), Any, Durable agent checkpointing for crash-recovery and resumption. Provides…, Return the latest checkpoint for a session, or None. (+7 more)
...
### Community 162 - "AgentSessionStore"
Cohesion: 0.10
...
### Community 164 - "ChatHistoryStore"
Cohesion: 0.07
...
### Community 165 - "WorkflowRun"
Cohesion: 0.07
Nodes (28): _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test(), _tracking_run_single(), TestPhaseSequence (+20 more)

### Community 166 - "WorkflowEngine"
Cohesion: 0.08
...
### Community 167 - "AdminIdentity"
Cohesion: 0.05
Nodes (30): Admin Authentication, Acceptance Checks, `admin_auth.py` checklist, `agent/tools.py` checklist, Escalation, Instructions, `key_store.py` checklist, `proxy.py` auth middleware checklist (+22 more)

### Community 168 - "isolated_telegram_config"
Cohesion: 0.13
...
### Community 169 - "llm/router.py"
Cohesion: 0.03
Nodes (125): collections_abc, Modules, Writing a custom adapter, int, Counter, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker. (+117 more)

### Community 170 - "audit"
Cohesion: 0.06
Nodes (29): audit(), get_audit_log(), Any, Request, FastAPI dependency: require any authenticated user., Append an audit log entry. Never logs raw secrets — only secret IDs / masked…, Return recent audit log entries, newest first. Supports filtering by user_id,…, require_authenticated() (+21 more)
...
### Community 173 - "test_daily_automation_2026_09_24.py"
Cohesion: 0.06
...
### Community 175 - "services/background.py"
Cohesion: 0.04
Nodes (56): Register structural code queries (agent/code_graph.py) — opt-in. Registered…, _register_code_graph_tools(), _code_impact_tool(), _code_search_tool(), _code_trace_tool(), graph_for(), code_graph_enabled(), get_code_graph() (+48 more)

### Community 176 - "seo_portfolio_bridge.py"
Cohesion: 0.05
Nodes (50): Agentic Agile — Sprint management with velocity tracking and burndown. Issue:…, CapacityAllocation, InitiativeStatus, Enum, Agentic Portfolio Management — initiative prioritisation, capacity allocation,…, Result of fitting initiatives into a fixed capacity by WSJF priority., Total job size of initiatives that fit within capacity., Unused capacity after committing the selected initiatives. (+42 more)

### Community 177 - "PortfolioManager"
Cohesion: 0.03
Nodes (65): add_initiative(), AllocationOut, BoardOut, get_board(), get_service(), InitiativeIn, InitiativeOut, _materialize_and_log() (+57 more)

### Community 178 - "test_shared_state.py"
Cohesion: 0.06
...
### Community 179 - "Fixed"
Cohesion: 0.12
Nodes (22): Fixed, Fixed, Fixed, Fixed, Fixed, Fixed, DetailsStep(), _blocks() (+14 more)

### Community 180 - "test_trend_scoping.py"
Cohesion: 0.06
Nodes (60): _dispatch_async(), _run(), ErrorInterceptorMiddleware, Any, BaseHTTPMiddleware, Exception, Request, Response (+52 more)

### Community 181 - "WorkspaceTools"
Cohesion: 0.05
Nodes (41): Register the built-in agent tools that are always available., _register_builtin_tools(), Delegate to RepowiseIntelligence for a natural-language codebase question., Delegate to RepowiseIntelligence for a semantic/text codebase search., Delegate to RepowiseIntelligence to list architectural decision nodes., Return a high-level repository map and hotspot summary., Return a formatted context block for the given file paths., Return a risk summary for the workspace or a subset of files. (+33 more)

### Community 182 - "test_verification_strategies.py"
Cohesion: 0.09
...
### Community 185 - "validate_outbound_url"
Cohesion: 0.13
...
### Community 186 - "is_provider_on_cooldown"
Cohesion: 0.09
Nodes (25): _dead_model_key(), _is_model_dead(), is_provider_on_cooldown(), mark_provider_failed(), Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown., Return True if (provider, model) is on the dead list and not yet expired., _nvidia() (+17 more)

### Community 187 - "GoalRecord"
Cohesion: 0.11
Nodes (14): GoalRecord, _now(), Any, One piece of work the CEO has taken ownership of. ``state`` is the supervisor's…, Return ``(done, total)`` subtask counts., Persist *goal*, stamping ``updated_at``., Return one goal by id, or ``None``., Return the most recently updated goals regardless of state. (+6 more)

### Community 188 - "_runner"
Cohesion: 0.08
...
### Community 189 - "_cfg"
Cohesion: 0.11
Nodes (7): _cfg(), tests/test_daily_automation_2026_09_13.py — Daily automation tests…, 3.8 Flash should have higher priority (lower number) than 3.7 Flash., Kimi K2 and QwQ-32B must have catalog entries so the router includes them., Gemini 3.x models must be declared with correct capability flags., TestGemini3xCatalog, TestGroqNewModelsCatalog
...
### Community 191 - "_build_anthropic_response"
Cohesion: 0.09
...
### Community 192 - "test_persistent_memory.py"
Cohesion: 0.06
...
### Community 193 - "KeyPool"
Cohesion: 0.04
Nodes (39): api_keys_for(), _digest(), KeyPool, _KeyState, _PoolState, Round-robin key selection with per-key rate-limit cooldowns., Cool a single key after a 429 from it. Honours the provider's own ``Retry-…, True when every key in the pool is resting. This is the signal that the… (+31 more)

### Community 194 - "tasks/service.py"
Cohesion: 0.03
Nodes (104): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest (+96 more)

### Community 195 - "agent/models.py"
Cohesion: 0.11
Nodes (23): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSessionCreateRequest, AgentSessionMessage, _known_tool_names(), Any, BaseModel (+15 more)

### Community 196 - "test_pr923_fixes.py"
Cohesion: 0.07
...
### Community 197 - "_process_task_callback"
Cohesion: 0.07
...
### Community 198 - "test_response_cache.py"
Cohesion: 0.12
...
### Community 199 - "test_bedrock_provider.py"
Cohesion: 0.07
...
Cohesion: 0.06
Nodes (29): fake_sandbox(), _FakeAsyncSandboxClass, _FakeCmdResult, _FakeCommands, _FakeFiles, _FakeSandbox, Any, asyncio (+21 more)

### Community 201 - "test_schedule_growth_invariants.py"
Cohesion: 0.05
...
### Community 202 - "Skill: Agentic Portfolio Management"
Cohesion: 0.09
...
### Community 203 - "SamAgent"
Cohesion: 0.11
Nodes (16): Any, SAM voice agent — the voice-controlled interface to the agency., Process a voice command and return SAM's spoken response. Args: text: The…, Call the NVIDIA NIM LLM (free tier) for SAM's response., Rule-based fallback when the LLM is unavailable., SamAgent, parametrize, tests/test_sam_actions.py — SAM acts on the alerts bell instead of deflecting.… (+8 more)

### Community 204 - "test_sam_livekit.py"
Cohesion: 0.05
Nodes (34): auth_headers(), livekit_env(), no_livekit_env(), _normalize_dockerfile(), parametrize, tests/test_sam_livekit.py — SAM realtime voice (LiveKit) integration. Covers: -…, Empty key/secret/identity/room must raise ValueError., SAM_LLM_* env vars must override the NVIDIA defaults (Hermes/proxy routing). (+26 more)
...
### Community 206 - "test_video_transcript.py"
Cohesion: 0.05
...
### Community 207 - "test_daily_2026_07_27.py"
Cohesion: 0.07
...
### Community 208 - "KeyStore"
Cohesion: 0.07
Nodes (33): 1. Authentication & Authorization, API Key Authentication, JWT / Token Auth, Category 2 — API Key Naming Confusion, TD-005 [MEDIUM] — Production Keys Have `test-key-` Prefix, Backward-compatibility shim — use scripts/generate_api_key.py instead. This…, _check_rate_limit(), default_keys_path() (+25 more)

### Community 209 - "test_backend_server_features.py"
Cohesion: 0.07
...
### Community 210 - "ChatScreen.jsx"
Cohesion: 0.07
Nodes (23): Fixed, approveWorkflow(), buildWorkflow(), cancelWorkflow(), getWorkflowRun(), getWorkflowRuns(), listProviderModels(), listProviders() (+15 more)

### Community 211 - "SetupChecker"
Cohesion: 0.06
...
### Community 212 - "test_classify_dependabot_update.py"
Cohesion: 0.07
...
### Community 213 - "ManagedAgentDreams"
Cohesion: 0.06
...
### Community 214 - "PromptCacheManager"
Cohesion: 0.06
...
### Community 219 - "OllamaCircuitBreaker"
Cohesion: 0.08
...
Cohesion: 0.12
Nodes (37): ArgumentParser, build_parser(), cmd_autostart_install(), cmd_status(), cmd_supervise(), cmd_wait(), Namespace, Block until download completes + colibri answers /v1/models. (+29 more)
...
### Community 227 - "test_provider_policy.py"
Cohesion: 0.14
...
### Community 228 - "AuditLog"
Cohesion: 0.05
Nodes (41): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log. (+33 more)

### Community 229 - "NotificationDispatcher"
Cohesion: 0.05
Nodes (43): Acceptance criteria, Design, Objective, Part B — G2: Closed-loop self-heal feedback, Tech stack / touch points, Tests, To-dos (checklist), NotificationDispatcher (+35 more)
...
### Community 231 - "SandboxManager"
Cohesion: 0.13
Nodes (14): detect_backend(), Any, Pick a backend by capability probe. Order: explicit setting → Docker (strongest…, Creates, tracks, reaps, and audits sandboxes. Lifecycle guarantees: * Every…, Return a profile by name, falling back to ``development``. An unknown profile…, Provision a sandbox for *identity* under *profile_name*., Run *command* inside a sandbox, auditing the call. The command is audited but…, Copy artifacts out **before** teardown. Ordering is the whole point: a… (+6 more)

### Community 232 - "test_runtime_control.py"
Cohesion: 0.33
...
### Community 233 - "Persistent Memory System"
Cohesion: 0.05
...
### Community 234 - "TestRefreshTokenSQLiteFallback"
Cohesion: 0.17
...
### Community 239 - "Universality: case-coverage matrix"
Cohesion: 0.08
...
### Community 240 - "test_kimi_bridge_provider.py"
Cohesion: 0.20
...
### Community 241 - "_execute_skill_impl"
Cohesion: 0.11
...
### Community 243 - "self_audit_local.py"
Cohesion: 0.08
Nodes (27): export_seo_audit(), Export a stored audit. - ``csv`` aggregated findings, Screaming Frog…, _build_curl_cffi_fetcher(), get(), get_text(), head(), _sess(), main() (+19 more)
...
### Community 246 - "SeoAuditReport"
Cohesion: 0.10
Nodes (33): html, Aggregated report row - Screaming Frog CSV compatible., Complete result of one audit run., SeoAuditReport, SeoIssueReportRow, Paragraph, reportlab_lib, reportlab_lib_pagesizes (+25 more)

### Community 247 - "CheckRun"
Cohesion: 0.12
Nodes (14): Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory, do_approve(), tests/test_workflow_models.py — Unit tests for workflow/models.py. (+6 more)

### Community 248 - "test_brain_availability_doctor.py"
Cohesion: 0.11
...
### Community 249 - "RateLimitTracker"
Cohesion: 0.11
Nodes (10): RateLimitTracker, Snapshot of all tracked provider quotas. Safe to call from any context., Reset all state (primarily for tests)., In-memory tracker for per-provider rate-limit state., asyncio, _response(), TestClear, TestGetStats (+2 more)
...
Cohesion: 0.07
Nodes (33): Changed, Changed, API, changeUserRole(), chatSend(), consultExecutives(), createApiKey(), deleteApiKey() (+25 more)

### Community 253 - "MetricsRegistry"
Cohesion: 0.08
...
### Community 254 - "test_background_services.py"
Cohesion: 0.07
...
### Community 256 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.33
...
### Community 258 - "test_telegram_webhook.py"
Cohesion: 0.06
...
### Community 261 - "test_daily_2026_06_04.py"
Cohesion: 0.07
Nodes (43): _content_block_to_text(), Convert a single Anthropic content block to a plain text string., Adding a model, Adding a task category, CLAUDE.md — router/, Environment variables, Testing, What this package does (+35 more)

### Community 262 - "test_control_plane_api.py"
Cohesion: 0.04
Nodes (44): legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_toggle(), legacy_scheduler_trigger(), LegacyScheduleToggleRequest, patch, Update status/tools/name on an MCP server., Force-dedup and clean stale schedules from the durable store. Admin-only… (+36 more)

### Community 263 - "test_repo_connection.py"
Cohesion: 0.07
...
### Community 264 - "test_local_controller.py"
Cohesion: 0.07
...
### Community 266 - "McpCard.jsx"
Cohesion: 0.14
Nodes (9): getRenderHealth(), getRenderOpsStatus(), runRenderOpsScan(), api, BTN, McpCard(), NOTE(), relTime() (+1 more)

### Community 267 - "pytest"
Cohesion: 0.02
Nodes (114): fastapi_testclient, jwt, pytest, client(), _Cursor, TestClient, GET /api/activation/users must list registered users, not just toggled ones. It…, proxy_client() (+106 more)

### Community 268 - "SyncService"
Cohesion: 0.10
...
### Community 269 - "test_purge_backlog.py"
Cohesion: 0.07
...
### Community 272 - "REWRITE_PLAN.md — Phased Migration Strategy"
Cohesion: 0.06
...
### Community 274 - "_cfg"
Cohesion: 0.15
Nodes (8): _skip_without_pydantic, _cfg(), Pro should be tried after Flash (lower priority number = higher preference)., The router must not exclude gemini-2.5-pro from tool-calling requests., Every model in the 'google' PROVIDER_PRESET must be declared in models.yaml., gemini-2.5-pro must be declared in models.yaml with tool support., TestGemini25ProCatalog, TestGoogleProviderPresetConsistency

### Community 275 - "_client"
Cohesion: 0.13
...
### Community 276 - "test_mcp_governance.py"
Cohesion: 0.09
...
### Community 277 - "PersistentMemoryStore"
Cohesion: 0.07
...
### Community 278 - "test_provider_enable_disable.py"
Cohesion: 0.07
...
### Community 280 - "test_hermes_in_process.py"
Cohesion: 0.12
Nodes (15): _free_port(), hermes_enabled(), _NullDispatcher, _NullRuntimeManager, asyncio, Hermes must actually run on a single-service deployment.…, Web lifespan and worker_main can both call into one process., Hermes is an optimisation. It must never take the backend down with it. (+7 more)
...
### Community 282 - "agent_runtime.py"
Cohesion: 0.06
Nodes (53): _active_cloud_provider(), _candidate_ollama_bases(), _chat(), chat_completions(), _chat_with_ollama(), _chat_with_openai_compat(), ChatRequest, ChatResponse (+45 more)

### Community 283 - "Part A — CodeRabbit review fixes for this PR (do first, small)"
Cohesion: 0.07
...
### Community 286 - "test_live_server.py"
Cohesion: 0.21
...
### Community 288 - "test_nvidia_model_discovery.py"
Cohesion: 0.06
...
### Community 289 - "test_output_filter.py"
Cohesion: 0.06
...
### Community 291 - "TestHarnessAdapter"
Cohesion: 0.06
Nodes (17): get_harness_adapter(), harness_active(), harness_catalog(), harness_session_close(), harness_session_start(), HarnessSessionBody, HarnessSessionCloseBody, Return the full ECC harness catalog with capabilities. Public — no auth… (+9 more)

### Community 292 - "⚙️ Operations Manager Agent"
Cohesion: 0.06
...
### Community 293 - "emit_chat_observation"
Cohesion: 0.08
Nodes (31): _anon_tick_allowed(), _cron_secret_ok(), True when CRON_SECRET is set and the request carries it (constant time)., Admit one un-authenticated tick of *kind* per minimum interval., Called by Cloudflare Cron every minute. Protected by CRON_SECRET header., scheduler_tick(), _base_url(), _department_trace_tags() (+23 more)
...
### Community 295 - "Platform Guide — the full tour"
Cohesion: 0.05
...
### Community 296 - "context_rules.py"
Cohesion: 0.11
...
### Community 297 - "_routing_candidates"
Cohesion: 0.09
...
### Community 299 - "ContextCompressor"
Cohesion: 0.11
...
### Community 300 - "LessonStore"
Cohesion: 0.09
Nodes (27): _get_store(), LessonStore, Any, Connection, Path, Formatted prompt block of recent lessons, or '' when none exist., SQLite-backed store of failure lessons. Thread-safe, zero deps., Persist a lesson for every failed step in a run. Never raises. (+19 more)

### Community 301 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (135): _note_phase_end(), _note_phase_start(), Log start/elapsed for one phase of the plan/execute/verify loop. A task that…, Open a tracked phase, returning the token that closes it. Never raises., Close the tracked phase *token* opened above. Never raises., _timed_phase(), _diagnose_and_file(), _file_incident() (+127 more)

### Community 302 - "SecurityScanner"
Cohesion: 0.11
...
### Community 303 - "_resolve_user_github_token"
Cohesion: 0.08
Nodes (34): _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Consolidated system health report: preflight checks + runtime health. Returns a…, Public Doctor endpoint — no authentication required. Returns system-level…, Authenticated Doctor endpoint — full diagnostics. Returns all system-level… (+26 more)

### Community 304 - "FeatureEntry"
Cohesion: 0.12
...
Cohesion: 0.08
Nodes (19): Fixed, _connectors(), ConnectorSpec, get_connector(), list_connectors(), Any, Execute the ``webhook`` connector: POST *payload* as JSON to *url*. Fails soft…, A declared connector, independent of whether it is currently usable.… (+11 more)

### Community 306 - "app_settings.py"
Cohesion: 0.14
Nodes (22): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled_cached(), Any (+14 more)

### Community 307 - "ContextManager"
Cohesion: 0.10
...
### Community 308 - "test_microagents.py"
Cohesion: 0.15
...
### Community 309 - "Langfuse Observability Guide"
Cohesion: 0.06
...
### Community 311 - "StreamingDeltaReconstructor"
Cohesion: 0.09
...
### Community 312 - "test_telegram_mutating_commands.py"
Cohesion: 0.09
...
### Community 313 - "._call"
Cohesion: 0.16
Nodes (6): _apply_reasoning_budget(), Map the non-standard ``reasoning_budget`` field to ``thinking_token_budget``.…, Any, A message list that exceeds the pruner's threshold should be trimmed., TestApplyReasoningBudget, TestPruneChatMessages

### Community 314 - "test_executive_advisory.py"
Cohesion: 0.05
Nodes (59): AdviceResult, AdvisoryMemory, _compose_context(), default_executives(), ExecOpinion, Executive, ExecutiveAdvisory, _format_company_context() (+51 more)

### Community 315 - "SparkProvider"
Cohesion: 0.07
...
### Community 316 - "ResourceWatchdog"
Cohesion: 0.10
...
### Community 317 - "Platform Engineer Agent"
Cohesion: 0.06
...
### Community 318 - "Marketing SEO Specialist"
Cohesion: 0.06
...
### Community 321 - "ensure_bootstrap"
Cohesion: 0.05
Nodes (46): admin_seed(), _bootstrap_within_budget(), _create_bootstrap_indexes(), ensure_bootstrap(), _ensure_tasks_source_id_unique_index(), _build(), get_activity(), _get_activity_impl() (+38 more)

### Community 322 - "post"
Cohesion: 0.03
Nodes (73): BrainTestRequest, clear_cost_attribution(), clear_response_cache(), create_mcp_server(), create_provider(), _get_backend_sam(), get_brain_policy_route(), github_oauth_start() (+65 more)

### Community 323 - "TestNormalizeResponseFormat"
Cohesion: 0.11
...
### Community 324 - "Quick-Note GitHub Issues Processing - Session Summary"
Cohesion: 0.06
...
### Community 325 - "TaskBoardScreen.jsx"
Cohesion: 0.06
...
### Community 327 - "DistributedRateLimiter"
Cohesion: 0.09
...
### Community 328 - "switch_brain.py"
Cohesion: 0.15
Nodes (30): detect_ollama_models(), dim(), fail(), get_auth_headers(), get_brain_config(), get_ngrok_tunnel_url(), header(), info() (+22 more)

### Community 329 - "probe_model_liveness"
Cohesion: 0.05
Nodes (46): _describe_http_status(), probe_model_liveness(), ProbeResult, BaseModel, services/brain_liveness.py — Provider liveness prober for the brain switcher.…, Turn an HTTP status into a short, human-readable reason. The plan specifically…, Outcome of a single (provider, model) liveness probe., Probe ``(provider, model)`` for liveness. Never raises. Returns a… (+38 more)

### Community 330 - "harness_routing.py"
Cohesion: 0.14
...
### Community 334 - "test_autonomy_gate.py"
Cohesion: 0.12
...
### Community 335 - "test_phase6_workflow.py"
Cohesion: 0.05
Nodes (62): Return True if the PR exists and is open or merged; False if 404., verify_pr_exists(), _append_transition(), classify_domain(), BaseModel, Enum, str, agent/workflow.py — Persisted workflow state machine. Implements the Autonomous… (+54 more)

### Community 336 - "test_runtime_governance.py"
Cohesion: 0.06
Nodes (56): F2 — MCP Server Exposing Proxy Capabilities [P1] [CBF / ECC], CircuitState, Return the last-known health for *runtime_id* (may be stale)., Return True if the runtime is available (not circuit-open)., Return health snapshots for all known runtimes., Force an immediate health check of all runtimes and return results., Attempt to start a dead runtime subprocess before re-probing. Uses the local…, Reduce probe frequency for runtimes that have never come online. (+48 more)
...
### Community 338 - "Application Security Engineer"
Cohesion: 0.07
...
### Community 339 - "output_filter.py"
Cohesion: 0.07
...
### Community 340 - "local_controller.py"
Cohesion: 0.13
...
### Community 341 - "decide"
Cohesion: 0.11
Nodes (16): decide(), Decision, issue_number_from_branch(), main(), Return the issue number a context branch was generated for, if any., Decide how to recover the *issue* behind an orphaned draft PR. *issue* is the…, _issue(), parametrize (+8 more)

### Community 342 - "test_ceo_router.py"
Cohesion: 0.05
Nodes (50): Drop the singleton (test helper)., reset_ceo_ledger(), CEOSupervisor, Any, What one sweep observed and did. Returned for tests and diagnostics., Sweeps the CEO ledger and drives open goals to closure., Sweep on the configured cadence until cancelled. A failing sweep is logged and…, Inter-sweep delay, behind a method so tests can shorten it. Patching… (+42 more)

### Community 343 - "MultiAgentSwarm"
Cohesion: 0.10
Nodes (24): AgentCoordinator, AgentSpec, CoordinatorResult, MultiAgentSwarm, execute(), Any, Enum, str (+16 more)

### Community 344 - "_redact_for_notification"
Cohesion: 0.11
Nodes (19): 4. Telegram gate protocol, Best-effort secret/email/IP redaction for outbound Telegram/webhook messages., _redact_for_notification(), _FakeResponse, _make_task(), SimpleNamespace, Regression tests for telegram_service.NotificationDispatcher._notify_webhook.…, Regression: _notify_webhook used to define _send() but never call it. (+11 more)

### Community 345 - "ScheduleStore"
Cohesion: 0.04
Nodes (39): _backend(), _json_default(), Any, Return all persisted schedule docs (for boot rehydration)., Persist (insert or update) a single schedule by job_id., Delete a persisted schedule., Fallback JSON encoder for schedule docs (datetimes, sets, etc.)., Durable schedule persistence. Backend is chosen from ``STORAGE_BACKEND``: *… (+31 more)

### Community 346 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.15
...
### Community 348 - "test_rag_context.py"
Cohesion: 0.12
...
### Community 351 - "Agent Governance Guide"
Cohesion: 0.12
Nodes (17): A tool call is judged twice, Agent Governance Guide, `[]` and absent mean opposite things, API, Approvals, Audit trail, Backends, Configuration (+9 more)

### Community 352 - "test_ai_insights.py"
Cohesion: 0.18
...
### Community 353 - "Data Engineer Agent"
Cohesion: 0.07
...
### Community 354 - "analyze_quantitative"
Cohesion: 0.10
...
### Community 355 - "get_failover_manager"
Cohesion: 0.07
Nodes (32): brain_failover_status(), openclaw_command(), openclaw_websocket(), ProviderEnabledBody, websocket, Return the brain failover health snapshot. Shows every configured provider, its…, Body for the provider on/off switch., Turn a provider on or off. Manual re-enable clears an auto-disable. Auto-… (+24 more)

### Community 356 - "Conflicts and Stale Facts"
Cohesion: 0.07
...
### Community 358 - "generate_context.py"
Cohesion: 0.12
...
### Community 360 - "test_rate_limit_backoff_survives.py"
Cohesion: 0.06
Nodes (23): _FM, _P, Regression tests for a chain that fails silently. From a real incident:…, Reserve logic must never break the chain it is meant to protect., The incident case: providers ran, none reported a reason., The genuinely-empty chain keeps its original, correct wording., Each cause must read differently, or diagnosis is guesswork., A reserve held for an unreachable provider starves the free tier.… (+15 more)

### Community 361 - "test_colibri_brain_shim.py"
Cohesion: 0.05
...
### Community 362 - "SteeringInjector"
Cohesion: 0.11
...
### Community 363 - "test_claude_setup_audit.py"
Cohesion: 0.16
...
### Community 364 - "WorkflowOrchestrator"
Cohesion: 0.04
Nodes (44): get_orchestrator_checkpoint_store(), _NoopStore, Any, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, List recent runs. When ``owner_id`` is provided, only runs stamped with that…, Determine domain and task type from the request text., Generate a structured execution plan. Uses the LLM if available; falls back to… (+36 more)
...
### Community 367 - "SeoFixer"
Cohesion: 0.12
Nodes (15): One concrete remediation performed (or proposed) by the fixer., Result of a fixer run., SeoFixAction, SeoFixResult, _humanize_filename(), BeautifulSoup, Path, Derive readable alt text from an image path: 'img/hero-banner_2.jpg' -> 'Hero… (+7 more)

### Community 368 - "test_internal_agent_did_work.py"
Cohesion: 0.12
...
### Community 370 - "_SlowLoginPage"
Cohesion: 0.05
Nodes (10): _ExpectResponse, _load(), _Loc, Exception, MonkeyPatch, A SPA whose login round-trip takes longer than any fixed sleep. The URL only…, _Response, _SlowLoginPage (+2 more)

### Community 371 - "test_commit_tracker.py"
Cohesion: 0.18
...
### Community 374 - "UX Researcher Agent Personality"
Cohesion: 0.07
...
### Community 375 - "DevOps Automator Agent Personality"
Cohesion: 0.07
...
### Community 376 - "Mobile App Builder Agent Personality"
Cohesion: 0.07
...
### Community 377 - "Developer Agent Personality"
Cohesion: 0.07
...
### Community 378 - "Analytics Reporter Agent Personality"
Cohesion: 0.07
...
### Community 379 - "Support Responder Agent Personality"
Cohesion: 0.07
...
### Community 381 - "Sampling Strategies Internals"
Cohesion: 0.06
Nodes (27): KV Cache Internals, KV Cache with Grouped Query Attention, Memory Layout, Paged Attention (vLLM), Prefill vs Decode Phase, Quantization of KV Cache, Speculative Decoding, The Problem: Redundant Computation (+19 more)

### Community 382 - "ProviderConsole.jsx"
Cohesion: 0.08
...
### Community 386 - "_is_dns_failure"
Cohesion: 0.19
Nodes (9): _is_dns_failure(), BaseException, True when *exc* (or anything it wraps) is a name-resolution failure., Exception, parametrize, httpx wraps the resolver error — the cause chain must be walked., Some stacks lose the cause and keep only the resolver text., A cyclic __cause__ must not hang the probe. (+1 more)

### Community 387 - "mcp_server/server.py"
Cohesion: 0.10
...
### Community 388 - "test_regression.py"
Cohesion: 0.10
...
### Community 391 - "test_brain_priority_scanner.py"
Cohesion: 0.09
Nodes (26): ProviderUpdate, update_provider(), Regression tests for: brain-skip-paid, provider-priority persistence, scanner…, Critical failover-safety test: if every free provider's base URL is excluded…, When the ONLY configured provider is a paid one (e.g. operator set…, When only Anthropic is configured AND allow_paid=False (default), the resolver…, The PUT /api/providers/{id} endpoint did not persist priority edits because the…, Priority must be an int (or None for unset) and within a sane range so a typo… (+18 more)

### Community 392 - "Next Action"
Cohesion: 0.08
Nodes (24): Roadmap Items (from `docs/roadmap-killer-todos.md`), #1565 nightly regression (2026-09-25, row 80), Current state (2026-09-24), Daily automation 2026-09-25 — DONE (row 80), Daily automation 2026-09-25 (row 81), Daily automation 2026-09-26 — DONE (row 82), Next Action, Next daily run (2026-09-27) (+16 more)
...
### Community 394 - "UI Designer Agent Personality"
Cohesion: 0.07
...
### Community 395 - "Product Sprint Prioritizer Agent"
Cohesion: 0.07
...
### Community 396 - "company_api.py"
Cohesion: 0.03
Nodes (129): Response, Public SSRF-guarded GET (rule 14) for callers outside this module. Raises…, GET *url*, re-validating every redirect hop against ``unsafe_target_reason`` —…, ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), AccountLifecycleResponse, cancel_onboarding() (+121 more)

### Community 397 - "test_integration_c4_c5_c6_d3.py"
Cohesion: 0.16
...
### Community 400 - "test_daily_automation_2026_05_15.py"
Cohesion: 0.10
...
### Community 402 - "pr_approval_gate.py"
Cohesion: 0.11
Nodes (32): Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Added, Added, getTaskCounts(), listTasks(), _card_keyboard(), _card_text(), _dedupe_key() (+24 more)

### Community 403 - "Changelog"
Cohesion: 0.11
Nodes (23): Restore the latest checkpoint for a session. Returns a dict with the…, restore_agent_state(), serve_spa(), [5.0.0], Changelog, Changelog, Removed, Security (+15 more)

### Community 404 - "test_persona_library.py"
Cohesion: 0.08
...
### Community 407 - "_extract_tags"
Cohesion: 0.22
Nodes (5): _extract_tags(), Pull hashtags and bold words from markdown as tags., Tests for module-level helper functions., Regression: frontmatter (--- ... ---) must not surface as '---'., TestHelpers

### Community 408 - "test_skill_registry.py"
Cohesion: 0.09
...
### Community 410 - "VoiceCommandInterface"
Cohesion: 0.14
...
### Community 411 - "Frontend Developer Agent Personality"
Cohesion: 0.08
...
### Community 412 - "Performance Analysis — local-llm-server"
Cohesion: 0.11
...
### Community 414 - "test_agent_chat_integration.py"
Cohesion: 0.13
...
### Community 415 - "test_platform_controls.py"
Cohesion: 0.03
Nodes (87): _actor(), build_platform_controls_router(), _admin(), list_controls(), reset_control(), update_controls(), ControlUpdateBody, APIRouter (+79 more)

### Community 416 - "analyze"
Cohesion: 0.14
Nodes (12): analyze(), _failure_table(), main(), Return the exhaustion :class:`Analysis` for a run's captured *output*., scripts/analyze_exhaustion.py Decide whether a run that ran out of retries has…, Tests for ``scripts/analyze_exhaustion.py``. The case that matters most is the…, A run with no collectable test must never be blamed on the agent., An unreachable service is not a failure the agent could have fixed. (+4 more)

### Community 417 - "seo_api.py"
Cohesion: 0.04
Nodes (82): auto_recommend_skills(), delete_company_endpoint(), get_company(), get_company_access(), _get_current_user_thunk(), get_optional_company_access(), _get_optional_user_thunk(), get_skill() (+74 more)

### Community 418 - "SyntheticDataPipeline"
Cohesion: 0.12
...
### Community 419 - "test_daily_automation_2026_08_03.py"
Cohesion: 0.11
...
### Community 420 - "test_repairs_when_complete_but_zero_specialists"
Cohesion: 0.09
...
### Community 421 - "test_langfuse_agency_wide.py"
Cohesion: 0.10
...
### Community 423 - "test_executive_advisory_api.py"
Cohesion: 0.09
Nodes (30): _company_advisory_context(), Pull a compact company profile for the C-suite advisory, or None. Best-effort:…, AdvisoryConsultRequest, AdvisoryResponse, build_executive_advisory_router(), consult(), list_executives(), ExecutiveListResponse (+22 more)
...
### Community 426 - "PR #634 Implementation Tracker"
Cohesion: 0.20
...
### Community 433 - "Competitor Analysis — Autonomous AI Agency"
Cohesion: 0.20
...
### Community 435 - "video_transcript.py"
Cohesion: 0.11
...
### Community 439 - "scripts/doctor.py"
Cohesion: 0.27
...
### Community 442 - "test_service_token.py"
Cohesion: 0.08
...
### Community 446 - "AI Engineer Agent"
Cohesion: 0.08
...
### Community 447 - "Research Synthesist Agent Personality"
Cohesion: 0.08
...
### Community 448 - "test_dashboard_cache.py"
Cohesion: 0.15
...
### Community 449 - "Local AI Stack with Docker"
Cohesion: 0.08
...
### Community 450 - "strategies.py"
Cohesion: 0.03
Nodes (108): Return True only when the tool is definitively read-only and non-destructive.…, Remember a server-issued ``Mcp-Session-Id`` for subsequent requests., Added, count, Added, Preferring local, Choosing one, Latency got worse (+100 more)

### Community 451 - "TASK 4 — End-to-end approval-gate test"
Cohesion: 0.08
...
### Community 452 - "enrich_quick_note_issues.py"
Cohesion: 0.11
...
### Community 453 - "OrchestratorCheckpointStore"
Cohesion: 0.16
...
### Community 454 - "test_cost_attribution.py"
Cohesion: 0.08
Nodes (10): _build_cost_table(), _load_env_overrides(), Parse MODEL_COST_INPUT / MODEL_COST_OUTPUT env overrides. Format:…, Tests for packages/ai/cost_tracker.py — per-model cost attribution. Covers: -…, Verify all Opus models referenced by brain_config are priced., _reset(), TestClaudeOpusModelCoverage, TestClearStats (+2 more)

### Community 455 - "CollectionLike"
Cohesion: 0.12
...
### Community 459 - "test_openclaw_endpoints.py"
Cohesion: 0.09
...
### Community 460 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.09
...
### Community 461 - "test_scheduler_hydration_bounded.py"
Cohesion: 0.09
...
### Community 463 - "Testing Analysis — local-llm-server"
Cohesion: 0.13
...
### Community 465 - "openclaw_str_e_fix.py"
Cohesion: 0.15
...
### Community 466 - "Deploy: FreeBuff Telegram bot (24×7)"
Cohesion: 0.09
...
### Community 467 - "test_provider_failover_integration.py"
Cohesion: 0.15
...
### Community 468 - "CEO Micro-Management"
Cohesion: 0.13
...
### Community 470 - "Implementation Prompt: Rich TaskBoard + Agile Sprint Integration"
Cohesion: 0.09
...
### Community 471 - "Telegram Bot Setup"
Cohesion: 0.09
...
### Community 472 - "knowledgeGraphTab.test.js"
Cohesion: 0.11
Nodes (19): { describe, test, expect }, fs, path, src, apiSource, { describe, test, expect }, fs, path (+11 more)

### Community 473 - "FeatureUnavailableError"
Cohesion: 0.12
Nodes (6): FeatureUnavailableError, Exception, Load canonical features and apply per-feature then bulk env overrides., Apply a config override string like 'stable', 'beta', 'disabled', 'enabled',…, Raised when code attempts to use a feature that is disabled or unavailable., TestEnforcement
...
### Community 475 - "test_cerebras_catalogue.py"
Cohesion: 0.13
...
### Community 476 - "test_mostly_failed_steps.py"
Cohesion: 0.12
...
### Community 477 - "test_task_source_id_race.py"
Cohesion: 0.14
...
### Community 478 - "agile_api.py"
Cohesion: 0.19
Nodes (21): complete_sprint(), create_sprint(), _get_mgr(), get_velocity(), list_sprints(), Any, BaseModel, get (+13 more)

### Community 479 - "CoworkSession"
Cohesion: 0.16
Nodes (3): CoworkSession, A shared AI coding session with multiple human contributors. Manages turn-…, TestCoworkSession

### Community 480 - "FinancialMetrics"
Cohesion: 0.13
...
### Community 481 - "sync_readme_gallery.py"
Cohesion: 0.20
...
### Community 482 - "test_telegram_diag_endpoint.py"
Cohesion: 0.08
...
### Community 483 - "compare_runtimes.py"
Cohesion: 0.11
...
### Community 485 - "TestLegacyRouterCacheTTL"
Cohesion: 0.13
...
### Community 487 - "analyze_qualitative"
Cohesion: 0.25
...
### Community 489 - "🧭 Product Manager Agent"
Cohesion: 0.10
...
### Community 490 - "test_admin_local_brain_router.py"
Cohesion: 0.15
...
### Community 491 - "V3 API Migration Plan — LLM Relay Platform"
Cohesion: 0.10
...
### Community 492 - "Agent Orchestration Design"
Cohesion: 0.14
...
### Community 493 - "ModelRouter"
Cohesion: 0.15
Nodes (12): ADR 002: Dynamic Model Routing with Task Classification, Consequences, Context, Decision, Negative, Neutral, Positive, ModelRouter (+4 more)

### Community 494 - "HarnessRegistry"
Cohesion: 0.17
...
### Community 495 - "system_instruction"
Cohesion: 0.16
...
### Community 496 - "LLMRouter"
Cohesion: 0.04
Nodes (67): Fixed, 1. `LLMRouter` is the only gateway, 2. Providers are data, not code, 3. Secrets stay in the environment, 4. Three independent failure scopes, 5. Bulkhead isolation, 6. Context is managed losslessly, 7. Configuration is six committed YAML files (+59 more)

### Community 497 - "LLM Router — troubleshooting"
Cohesion: 0.08
...
### Community 498 - "PriorityTaskQueue"
Cohesion: 0.14
...
### Community 499 - "_test_e2e_telegram_approval"
Cohesion: 0.11
...
### Community 500 - "test_daily_automation_2026_09_25.py"
Cohesion: 0.10
...
### Community 501 - "test_openclaw_str_e_fix.py"
Cohesion: 0.10
...
### Community 502 - "provider_max_rpm"
Cohesion: 0.11
Nodes (23): Pre-call budget checks, provider_max_parallel(), provider_max_rpm(), provider_max_tpm(), _provider_positive_float(), provider_weight(), Shared parse/validate for the numeric per-provider traffic budgets. Returns…, Return the operator-configured requests/min cap for *provider*, or None if… (+15 more)

### Community 503 - "trend_analysis.py"
Cohesion: 0.18
...
### Community 504 - "sam_livekit_worker.py"
Cohesion: 0.14
...
### Community 506 - "AdaptivePermissions"
Cohesion: 0.19
...
### Community 507 - "test_agent_free_brain.py"
Cohesion: 0.05
Nodes (33): livenim, Resolve the free NVIDIA NIM brain from env, or ``None`` if unconfigured.…, resolve_free_nvidia_brain(), _FakeAsyncClient, _FakeResponse, _free_env(), Free-brain policy regression tests for the agent runtime (issue #656).…, The core #656 regression: an Anthropic-shaped model must NOT hit… (+25 more)

### Community 508 - "_extract_tech_relevance"
Cohesion: 0.23
Nodes (4): _extract_tech_relevance(), Dynamic extraction: finds any tech keyword mentioned in the skill content,…, Tests for _extract_tech_relevance() word-boundary matching., TestExtractTechRelevance

### Community 509 - "Skill: modularity-review"
Cohesion: 0.10
...
### Community 510 - "Design Audit"
Cohesion: 0.10
Nodes (19): Code Quality, Color and Surfaces, Component Patterns, Content, Design Audit, Fix Priority, How This Works, Iconography (+11 more)

### Community 511 - "Findings"
Cohesion: 0.10
...
### Community 516 - "Dynamic Model Routing"
Cohesion: 0.11
...
### Community 517 - "The rules"
Cohesion: 0.11
Nodes (18): Changing these rules, How the gate behaves, Quick-Note Context Rulebook, R10 — Use the repository's real identity **[gate]**, R11 — Name a real integration point **[gate]**, R12 — Mark epistemic status at the claim **[review]**, R1 — Ground the plan in the source before planning anything **[gate]**, R2 — Say what the artifact actually is **[gate]** (+10 more)

### Community 518 - "ControlsScreen.jsx"
Cohesion: 0.15
...
### Community 523 - "Autonomous AI Agency"
Cohesion: 0.10
...
### Community 526 - "_resolve_push_token"
Cohesion: 0.17
...
### Community 529 - "test_anthropic_refusal_fallback.py"
Cohesion: 0.15
...
### Community 530 - "TestBrainFailoverModelUpdates"
Cohesion: 0.18
...
### Community 533 - "test_research_coordinator.py"
Cohesion: 0.11
...
### Community 537 - "test_voice_pipeline.py"
Cohesion: 0.14
...
### Community 540 - "Analysis & Synthesis Instructions"
Cohesion: 0.11
Nodes (18): 1. Define the Atmosphere, 2. Map the Color Palette, 3. Establish Typography Rules, 4. Define the Hero Section, 5. Describe Component Stylings, 6. Define Layout Principles, 7. Define Responsive Rules, 8. Encode Motion Philosophy (+10 more)

### Community 541 - "Production Readiness Assessment — local-llm-server"
Cohesion: 0.11
...
### Community 542 - "Skill: fabric-patterns"
Cohesion: 0.11
Nodes (18): 1. Ensure Pattern Directory Exists, 2. List Available Patterns, 3. Retrieve a Pattern, 4. Apply a Pattern with Variables, 5. Stitch Patterns Together, 6. Create New Patterns, Acceptance Checks, Directory Structure (+10 more)

### Community 543 - "Admin Dashboard Guide"
Cohesion: 0.11
...
### Community 544 - "DirectChatSession"
Cohesion: 0.12
...
### Community 545 - "test_health_endpoints.py"
Cohesion: 0.15
...
### Community 546 - "test_north_mini_code.py"
Cohesion: 0.08
Nodes (15): north_mini_code_model_for(), Return the North Mini Code model id served by *provider*, else ``None``.…, best_model_for(), Model capability registry. Defines the known local models, their strengths, and…, # NOTE: suspended under US export-control directive as of 2026-06-12., Return the name of the best model for a given task category. Falls back to…, tests/test_north_mini_code.py — North Mini Code 1.0 integration. Covers the…, The switch defaults ON so North is the default post-install. (+7 more)

### Community 547 - "TestRouterIntegration"
Cohesion: 0.21
...
### Community 548 - "_InMemoryBackend"
Cohesion: 0.18
...
### Community 549 - "Delegation Plan (agent-ready work packages)"
Cohesion: 0.11
...
### Community 550 - "1. Capability-by-capability"
Cohesion: 0.12
...
### Community 553 - "WorkflowBuildRequest"
Cohesion: 0.10
Nodes (23): engine(), Contract: WorkflowEngine cannot skip the gate state machine., _make_engine(), _make_run(), tests/test_crispy_run_history.py — N4 acceptance:…, Phase-level outcomes (complete/failed counts per phase_type) come from the…, Only the 5 most recent failure reasons are kept — keeps the response payload…, window_days is the age of the oldest run in days — used to gate the burn-in… (+15 more)

### Community 554 - "TestSelfHealingInfrastructureClassification"
Cohesion: 0.19
...
### Community 555 - "validate_session_id"
Cohesion: 0.16
...
### Community 556 - "HarnessEnrichment"
Cohesion: 0.15
Nodes (8): HarnessEnrichment, Any, Build a compact catalog of available runtime skills. Discovers from…, Standing instructions from the Continual Harness spec. Deliberately uncached:…, Build the complete enrichment block (tools + skills). Returns empty string when…, Inject enrichment blocks into a system prompt string. Appends blocks after the…, Auto-discovers skills and tools for agent prompt injection. Usage:: enrichment…, Build a compact, token-efficient catalog of available agent tools. Discovers…

### Community 557 - "Implementation Plan — DB-persisted, UI-switchable Brain (no redeploy)"
Cohesion: 0.15
...
### Community 563 - "Test Automation Engineer"
Cohesion: 0.11
...
### Community 564 - "Comprehensive Skill Index (By Category)"
Cohesion: 0.11
...
### Community 565 - "Agent Skill: Principal UI/UX Architect & Motion Choreographer (Awwwards-Tier)"
Cohesion: 0.11
Nodes (17): 1. Meta Information & Core Directive, 2. THE "ABSOLUTE ZERO" DIRECTIVE (STRICT ANTI-PATTERNS), 3. THE CREATIVE VARIANCE ENGINE, 4. HAPTIC MICRO-AESTHETICS (COMPONENT MASTERY), 5. MOTION CHOREOGRAPHY (FLUID DYNAMICS), 6. PERFORMANCE GUARDRAILS, 7. EXECUTION PROTOCOL, 8. PRE-OUTPUT CHECKLIST (+9 more)

### Community 566 - "Architecture Overview — local-llm-server"
Cohesion: 0.11
...
### Community 568 - "Part A — Health Report"
Cohesion: 0.11
...
### Community 570 - "test_force_cleanup_conditional_delete.py"
Cohesion: 0.18
...
### Community 574 - "WindowsServiceManager"
Cohesion: 0.19
...
### Community 577 - "TestStopSlopChecker"
Cohesion: 0.11
...
### Community 579 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
### Community 581 - "Task"
Cohesion: 0.17
Nodes (10): Enum, Path, str, Score the agent's final answer. Returns (success, score)., Returns (success: bool, score: float ∈ [0, 1]). Raises NotImplementedError for…, A fully-specified evaluation task. Fields mirror the OpenHarness task schema so…, SuccessCriterion, SuccessCriterionType (+2 more)
...
### Community 583 - "register_user_research_tools"
Cohesion: 0.18
Nodes (9): Any, field_validator, Register the four user-research tools with a ``ToolRegistry``. Each tool wraps…, register_user_research_tools(), user_research_plan_tool(), user_research_qual_tool(), user_research_quant_tool(), user_research_synthesize_tool() (+1 more)
...
### Community 587 - "Salesforce Architect"
Cohesion: 0.12
...
### Community 588 - "SKILL: Industrial Brutalism & Tactical Telemetry UI"
Cohesion: 0.12
...
### Community 589 - "Skill: data-quality-audit"
Cohesion: 0.12
...
### Community 590 - "What "Slop" Looks Like"
Cohesion: 0.12
...
### Community 591 - "test_refresh_agent_built_proof.py"
Cohesion: 0.12
...
### Community 595 - "Section-by-Section Acceptance Criteria"
Cohesion: 0.12
...
### Community 596 - "test_rate_limiter.py"
Cohesion: 0.09
...
### Community 597 - "test_workspace_isolation.py"
Cohesion: 0.16
...
### Community 598 - "redact_connection_url"
Cohesion: 0.17
...
### Community 604 - "_migration_block"
Cohesion: 0.15
...
### Community 605 - "TestCatalogFable51"
Cohesion: 0.09
...
### Community 606 - "test_frontend_deployment_guards.py"
Cohesion: 0.18
...
### Community 609 - "verify_service_token"
Cohesion: 0.14
...
### Community 610 - "Skill: parallel-agents"
Cohesion: 0.13
...
### Community 611 - "hermes_prompt.py"
Cohesion: 0.19
...
### Community 612 - "MemoryMiddleware"
Cohesion: 0.17
...
### Community 615 - "PerformanceAnalytics"
Cohesion: 0.12
...
### Community 617 - "Native operations"
Cohesion: 0.12
...
### Community 619 - "Skill: repowise-intelligence"
Cohesion: 0.12
...
### Community 620 - "ARCHITECTURE.md — Target Architecture"
Cohesion: 0.12
...
### Community 621 - "Security Analysis — local-llm-server"
Cohesion: 0.12
...
### Community 622 - "test_social_login_oauth.py"
Cohesion: 0.24
...
### Community 623 - "github_source.py"
Cohesion: 0.20
...
### Community 624 - "Attention Mechanisms Internals"
Cohesion: 0.12
...
### Community 626 - "The 10-Step Workflow"
Cohesion: 0.12
...
### Community 627 - "Contributing to local-llm-server"
Cohesion: 0.12
...
### Community 628 - "Agent Runtime Setup"
Cohesion: 0.12
...
### Community 629 - "steering_for_task"
Cohesion: 0.47
...
### Community 630 - "test_unit5_ui_provider_surface.py"
Cohesion: 0.10
...
### Community 636 - "get_skill_bindings"
Cohesion: 0.07
Nodes (25): build_matrix(), _families(), main(), Any, scripts/generate_specialist_skill_matrix.py — Specialist × Skill matrix.…, Map family -> sorted list of test files that mention it (quoted token)., Return one row per family, derived entirely from code., render_markdown() (+17 more)
...
### Community 641 - "test_migrate_local_brain_env.py"
Cohesion: 0.38
...
### Community 649 - "RateLimit"
Cohesion: 0.13
Nodes (4): OllamaProvider, RateLimit, Provider rate limit info., Return the provider's rate limits.

### Community 650 - "Skill: agent-harness"
Cohesion: 0.13
...
### Community 651 - "Skill: checkpoint-strategy"
Cohesion: 0.13
...
### Community 653 - "Skill: local-ai-query"
Cohesion: 0.13
...
### Community 654 - "socket"
Cohesion: 0.16
Nodes (10): _probe_failure_reason(), Turn a probe exception into an operator-actionable one-line reason. A dead…, socket, asyncio, A dead provider hostname must be reported as a DNS failure, not a raw errno.…, Documents the actual defect: the shipped Aerolink host does not resolve. This…, The reason must survive the real probe entry point, not just the helper., test_configured_aerolink_host_is_the_one_that_fails() (+2 more)

### Community 655 - "Skill: parallel-worktrees"
Cohesion: 0.13
...
### Community 656 - "Design System: Taste Standard"
Cohesion: 0.13
...
### Community 658 - "test_new_features_e2e.py"
Cohesion: 0.26
...
### Community 659 - "_register_web_reach_tools"
Cohesion: 0.22
Nodes (5): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), get_web_reach(), Module-level singleton, mirroring `agent.capability_registry.get_tool_registry`., test_capability_registry_exposes_web_reach_tools()

### Community 660 - "Agency Core — Progress & Resume Log"
Cohesion: 0.13
...
### Community 661 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 662 - "Killer TODO Roadmap — local-llm-server"
Cohesion: 0.25
...
### Community 663 - "test_traffic_director.py"
Cohesion: 0.10
Nodes (15): Return the configured traffic-distribution strategy (lower-cased). Reads…, routing_strategy(), active_strategy(), get_director(), Adaptive traffic distribution across LLM providers. The router…, Return the configured strategy name, falling back to ``priority``. An…, Return the process-singleton TrafficDirector., Clear director state and the cached strategy warnings (tests only). (+7 more)

### Community 665 - "fetch_url.py"
Cohesion: 0.19
...
### Community 666 - "coordinate.py"
Cohesion: 0.10
Nodes (29): AgentConfig, build_agent_specs(), build_swarm(), build_task_specs(), CoordinateResponse, BaseModel, agent/coordinate.py — Multi-Agent Coordination models, helpers, and optional…, Convert TaskInput list to TaskSpec list for MultiAgentSwarm. (+21 more)

### Community 667 - "test_v4_api.py"
Cohesion: 0.10
...
### Community 670 - "OrchestratorQueue"
Cohesion: 0.17
...
### Community 671 - "test_critical_flows.py"
Cohesion: 0.26
...
### Community 673 - "The 8-Step Golden Path"
Cohesion: 0.22
...
### Community 674 - "TestBrainConfigUpdates"
Cohesion: 0.22
...
### Community 685 - "DeterministicEngine"
Cohesion: 0.21
...
### Community 686 - "Project Manager Agent Personality"
Cohesion: 0.14
...
### Community 689 - "Skill: lr-schedule-advisor"
Cohesion: 0.14
...
### Community 692 - "Checks Performed"
Cohesion: 0.14
...
### Community 693 - "Skill: training-stability-monitor"
Cohesion: 0.14
...
### Community 695 - "get_tool_registry"
Cohesion: 0.11
...
### Community 696 - "Skill: branch-cleanup"
Cohesion: 0.14
...
### Community 697 - "Skill: dev-browser — Browser Automation via Sandboxed JS"
Cohesion: 0.14
...
### Community 698 - "Skill: perplexity — Web Research via Perplexity API"
Cohesion: 0.14
...
### Community 700 - "Quick-Note Issues Processing Summary"
Cohesion: 0.14
...
### Community 701 - "Specialist"
Cohesion: 0.04
Nodes (28): match_specialists(), SystemType, Match specialists to a task. Returns the best specialists for the given task…, SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities. (+20 more)

### Community 702 - "Separate hosted dashboard backend (`backend/server.py`)"
Cohesion: 0.14
...
### Community 703 - "._resolve_path"
Cohesion: 0.06
Nodes (22): Path, Return the first *lines* lines of a file. Just-in-time retrieval: the executor…, Return a lightweight index of files with line counts and sizes. This is the…, Resolve filepath relative to workspace_root, rejecting traversal attempts. Uses…, /arch-review — Architecture Agent, Key Architectural Principles, Steps, When to use (+14 more)

### Community 705 - "Runbook: Auto-Resume After Cooldown / Interruption"
Cohesion: 0.14
...
### Community 706 - "Traffic Distribution Across Providers"
Cohesion: 0.17
...
### Community 707 - "_build_payload_or_500"
Cohesion: 0.18
...
### Community 708 - "webui/router.py"
Cohesion: 0.08
Nodes (34): fastapi_responses, allow_paid_brain(), is_anthropic_model(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, True when *model* names a paid Anthropic/Bedrock-Claude model. Covers native…, _opus_model(), Return an Opus model ID iff the operator explicitly opted into a paid brain.…, chat_completions() (+26 more)

### Community 709 - "KimiBrowserDriver"
Cohesion: 0.17
...
### Community 710 - "test_daily_automation_2026_09_11.py"
Cohesion: 0.20
Nodes (7): packages_llm, _cost_table(), tests/test_daily_automation_2026_09_11.py — Daily automation tests…, Gemini models must have entries in the cost table., Flash is on the free tier in this deployment — tracked as $0., 2.0 Flash fallback is on the free tier., TestGeminiCostTrackerEntries

### Community 711 - "GitHubPolicyProbe"
Cohesion: 0.19
...
### Community 714 - "test_setup_api.py"
Cohesion: 0.19
...
### Community 716 - "The fifteen strategies"
Cohesion: 0.10
...
### Community 717 - "TestAnthropicToolListCaching"
Cohesion: 0.34
...
### Community 719 - "test_providers_live_e2e.py"
Cohesion: 0.24
...
### Community 723 - "test_openclaw_gateway.py"
Cohesion: 0.14
...
### Community 724 - "ceo-micromanagement.md"
Cohesion: 0.25
Nodes (4): P0 behavior change, Readiness contract, Runtime model, Runtime types

### Community 725 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 726 - "test_quick_note_engine.py"
Cohesion: 0.17
...
### Community 727 - "SeoAuditEngine"
Cohesion: 0.09
Nodes (24): A single occurrence of a check firing on a specific URL., SeoIssueInstance, is_internal(), normalize_url(), PageFindings, AsyncBaseTransport, datetime, HEAD a bounded set of internal images and flag heavyweight ones. (+16 more)

### Community 728 - "Screens"
Cohesion: 0.11
...
### Community 730 - "generate_onboarding_questions"
Cohesion: 0.08
Nodes (31): _DoctorCheck, _DoctorReport, generate_onboarding_questions(), _get_fallback_questions(), get_public_doctor_report(), OnboardingAnswersRequest, OnboardingQuestionsRequest, BaseModel (+23 more)

### Community 731 - "RegistrySkill"
Cohesion: 0.15
...
### Community 732 - "AGENTS.md — Codebase Map & Operations Reference"
Cohesion: 0.15
...
### Community 733 - "CollaborationContext"
Cohesion: 0.21
...
### Community 734 - "Software Architect Agent"
Cohesion: 0.15
...
### Community 737 - "Skill: Brain Dump"
Cohesion: 0.15
...
### Community 740 - "Skill: duplicate-thread"
Cohesion: 0.15
...
### Community 741 - "Skill: Email Triage"
Cohesion: 0.15
...
### Community 744 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
...
### Community 745 - "Skill: prompt-library"
Cohesion: 0.15
...
### Community 746 - "Skill: prompt-transparency"
Cohesion: 0.15
...
### Community 748 - "Skill: scope-guard"
Cohesion: 0.15
...
### Community 749 - "test_dependent_tasks_are_not_concurrent"
Cohesion: 0.15
Nodes (7): anyio, run() should return completed/failed counts in CoordinatorResult., Two independent tasks should complete faster than running sequentially., Dependent tasks must run sequentially respecting dependency order., test_dependent_tasks_are_not_concurrent(), test_independent_tasks_run_concurrently(), test_swarm_result_aggregation()

### Community 750 - "reset_store"
Cohesion: 0.05
Nodes (43): 1. The Rules, A. Prime directive, B. Wiring — where things must go, C. Security invariants, D. Risky modules, G. Conventions that are not inferable from the code, H. Tests, I. Ship gates (+35 more)

### Community 751 - "Instructions"
Cohesion: 0.15
Nodes (12): Acceptance Checks, Instructions, Role 1: Security Reviewer, Role 2: Correctness Reviewer, Role 3: Performance Reviewer, Role 4: Maintainability Reviewer, Skill: council-review, Step 1 — Gather the diff (+4 more)

### Community 752 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
Nodes (12): Acceptance Checks, Claude's query protocol (use this instead of Read tool for exploration):, Graph Artifacts — What to Commit, How to Use the Graph (Token Savings Protocol), Installation (one-time per machine), Instead of reading raw files:, Key commands:, Relationship to repowise-intelligence Skill (+4 more)

### Community 753 - "Skill: platform-setup — Autonomous Agency Bootstrap"
Cohesion: 0.15
...
### Community 754 - "Device compatibility and model picks"
Cohesion: 0.15
...
### Community 756 - "ApplyReviewAgent"
Cohesion: 0.22
...
### Community 757 - "TestEstimateTokensForMessages"
Cohesion: 0.15
...
### Community 758 - "prompts/README.md"
Cohesion: 0.05
...
### Community 759 - "_is_bedrock_model_id"
Cohesion: 0.27
...
### Community 760 - "Agent Transparency Report"
Cohesion: 0.15
...
### Community 761 - "_get_provider_policy"
Cohesion: 0.19
...
### Community 762 - "test_wrapper_falls_back_to_installed_model"
Cohesion: 0.19
...
### Community 765 - "test_compose_and_coordinate_api.py"
Cohesion: 0.19
...
### Community 766 - "TestModelCostTableUpdates"
Cohesion: 0.26
...
### Community 767 - "test_deploy_trigger_covers_image.py"
Cohesion: 0.21
...
### Community 769 - "TestKillSwitchDurability"
Cohesion: 0.15
...
### Community 771 - "_is_ephemeral_user"
Cohesion: 0.27
...
### Community 773 - "467 Public Site Truth Spec"
Cohesion: 0.17
...
### Community 780 - "Skill: system-prompt-audit"
Cohesion: 0.17
...
### Community 781 - "Skill: task-alive-updates"
Cohesion: 0.17
...
### Community 784 - "Skill: agent-browser — Real Chrome Browser Automation"
Cohesion: 0.17
...
### Community 788 - "Quantization Internals"
Cohesion: 0.17
...
### Community 789 - "Autonomy Uplift — Living Roadmap & Detailed Implementation Specs"
Cohesion: 0.15
...
### Community 792 - "TestModelsEndpointAliases"
Cohesion: 0.24
Nodes (6): _build_builtin_model_map(), _get_model_map(), Build the built-in alias table — Nvidia NIM models when key is set, local…, Merge built-in defaults with MODEL_MAP env overrides (lazy, cached)., Tests that /v1/models exposes Claude/Anthropic alias entries., TestModelsEndpointAliases

### Community 793 - "test_task_clarification.py"
Cohesion: 0.17
...
### Community 795 - "review_agent.py"
Cohesion: 0.21
Nodes (7): Example: Access your home PC models from any machine. Install: pip install…, get_pr_diff(), get_pr_files(), load_council_skill(), main(), openai, textwrap

### Community 796 - "test_brain_patch_service_token.py"
Cohesion: 0.09
Nodes (33): get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., clean_store(), _clear_overrides(), _make_client_with_user(), tests/test_brain_patch_service_token.py — N5 acceptance: PATCH…, N5 acceptance: no service token + no user session → 401 (not 200). (+25 more)
...
### Community 799 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 802 - ".test_set_github_token_sqlite_string_id_does_not_500"
Cohesion: 0.20
Nodes (3): MonkeyPatch, TestClient, TestGithubTokenSQLiteRegression

### Community 803 - "TestRunCoroSync"
Cohesion: 0.18
...
### Community 806 - "test_phase4_runtime_resilience.py"
Cohesion: 0.09
...
### Community 808 - "migrate_local_brain_env.py"
Cohesion: 0.25
...
### Community 809 - "._prune"
Cohesion: 0.20
Nodes (7): Failure behaviour, _ProviderUsage, Sliding-window usage counters for one provider., Drop request/token events that have fallen out of the window., Count a request that is about to be sent to *provider_id*. Called immediately…, Record the outcome of a request started with ``record_start``. Must be called…, Count a request that was routed away from *provider_id* (diagnostics).

### Community 810 - "_keyword_search"
Cohesion: 0.20
...
### Community 812 - "AI Engineering Insights Skill"
Cohesion: 0.18
...
### Community 816 - "Skill: pro-workflow"
Cohesion: 0.18
...
### Community 819 - "Skill: resource-panel"
Cohesion: 0.18
...
### Community 820 - "Skill: sandboxed-exec"
Cohesion: 0.18
...
### Community 821 - "admin_update_task_router.py"
Cohesion: 0.22
...
### Community 822 - "ECC Harness Patterns Skill"
Cohesion: 0.18
...
### Community 823 - "Instructions"
Cohesion: 0.18
Nodes (10): Acceptance Checks, Failure / Retry Behaviour, Instructions, Skill: implementation-planner, Step 1 — Understand the current state, Step 2 — Write the plan, Step 3 — Get implicit approval before coding, Step 4 — Implement (+2 more)

### Community 824 - "Instructions"
Cohesion: 0.18
...
### Community 825 - "Stop-Slop Quality Skill"
Cohesion: 0.18
...
### Community 827 - "test_vision_routing.py"
Cohesion: 0.13
Nodes (9): best_vision_model(), has_image_content(), ModelCapability, Return the name of the best registered vision-capable model, or None. Prefers…, Return True if any message contains an image_url content part., Tests for vision request routing and session ID propagation. Covers: -…, _reset_router(), TestBestVisionModel (+1 more)

### Community 828 - "check_container_posture.py"
Cohesion: 0.26
...
### Community 829 - "2. Critical Bugs & Exact Detection Signatures"
Cohesion: 0.18
...
### Community 830 - "Issue #467 — Section 1: Pulled State + PR Inventory"
Cohesion: 0.18
...
### Community 831 - "Deploy to Google Cloud Run"
Cohesion: 0.18
...
### Community 834 - "Runbook — Apply the Fast Free NVIDIA Brain to Render (TASK 2)"
Cohesion: 0.18
...
### Community 835 - "CI Troubleshooting Runbook"
Cohesion: 0.18
...
### Community 836 - "test_probe_report.py"
Cohesion: 0.17
...
### Community 837 - "CheckpointStore"
Cohesion: 0.16
Nodes (10): CheckpointStore, Path, Persist a checkpoint to disk. Returns the file path., Return all checkpoint files for a session, sorted by step index., Remove all checkpoints for a session., File-backed checkpoint persistence. Each session gets a directory under…, asyncio, Path (+2 more)
...
### Community 839 - "is_model_available"
Cohesion: 0.08
Nodes (27): Layer 3 — Model Router (`router/`), F. Router invariants (`router/`), F. Router invariants (`router/`), fallback_chain, Health check and availability filtering, fallback_chain(), Follow the explicit chain in ``routing.yaml``, then everything else. The chain…, _enabled() (+19 more)

### Community 840 - "Security Policy"
Cohesion: 0.18
...
### Community 843 - "_request"
Cohesion: 0.20
Nodes (3): `--json PATH` writes a machine-readable summary the scheduled drift-report step…, _request(), TestTheJsonSummary

### Community 844 - "test_scanner_ssl_cert.py"
Cohesion: 0.21
Nodes (6): _make_self_signed_der(), _RaisingCtx, Offline regression tests for the SSL/TLS certificate analysis fix. Background:…, End-to-end (no network): force the unverified DER path and assert the…, test_analyze_ssl_cert_maps_decoded_cert_to_systems(), test_decode_der_cert_extracts_issuer_and_sans()

### Community 845 - "_record_id"
Cohesion: 0.25
...
### Community 848 - "TestDecisionsBotLinks"
Cohesion: 0.25
...
### Community 849 - "test_empirical_verify.py"
Cohesion: 0.49
...
### Community 850 - "NVIDIA NIM — Free Tier Setup"
Cohesion: 0.18
...
### Community 851 - "TestZeroAttemptDiagnostics"
Cohesion: 0.25
...
### Community 852 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 853 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.22
...
### Community 855 - "test_scanner_e2e.py"
Cohesion: 0.24
...
### Community 857 - "TestGemini3xCostTracker"
Cohesion: 0.27
Nodes (3): _cost_table(), Gemini 3.x models must have pricing entries in the cost table., TestGemini3xCostTracker

### Community 858 - "TestRevenuePortfolio"
Cohesion: 0.22
...
### Community 860 - "financial_analyst.py"
Cohesion: 0.22
...
### Community 861 - "build_tech_db.py"
Cohesion: 0.35
...
### Community 863 - "Protocol: Premium Utilitarian Minimalism UI Architect"
Cohesion: 0.20
...
### Community 864 - "The 5-Step Wrap-Up Ritual"
Cohesion: 0.20
...
### Community 865 - "Brag Plan: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 866 - "de"
Cohesion: 0.17
Nodes (15): ae(), Be(), $d(), de(), fe(), ha(), ka(), ma() (+7 more)

### Community 867 - "Hyperframes Composition Brief: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 868 - "test_conftest_hermetic_env.py"
Cohesion: 0.18
...
### Community 869 - "_normalize_tool_choice"
Cohesion: 0.31
...
### Community 870 - "Skill: Agentic Agile"
Cohesion: 0.20
...
### Community 871 - "Skill: browserbase-ui-test — Adversarial UI Testing"
Cohesion: 0.20
...
### Community 872 - "Skill: financial-analyst (Agentic CFO)"
Cohesion: 0.20
...
### Community 873 - "Graphiti Temporal Context Skill"
Cohesion: 0.20
...
### Community 874 - "Skill: seo-audit-report"
Cohesion: 0.20
...
### Community 875 - "Agent Readiness Report"
Cohesion: 0.20
...
### Community 876 - "467 Golden Path — Locked Implementation Order"
Cohesion: 0.20
...
### Community 877 - "sam.py"
Cohesion: 0.20
Nodes (8): agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, A single voice conversation session with SAM., SamConversation, add_turn must append to history and increment command_count., History must be capped at 20 entries (10 turns)., test_conversation_add_turn(), test_conversation_history_capped(), get_memory_kernel()

### Community 878 - "One command (recommended)"
Cohesion: 0.20
...
### Community 881 - "ADR-008: LLMRouter — the single multi-provider routing gateway"
Cohesion: 0.20
...
### Community 884 - "_parse_reset_epoch"
Cohesion: 0.33
...
### Community 885 - "run_patched_colibri.py"
Cohesion: 0.27
...
### Community 886 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 889 - "is_node_id"
Cohesion: 0.24
Nodes (7): is_node_id(), Return True if *candidate* has the shape of a pytest node ID. A node ID is…, parametrize, The naive greps must not come back — that is the whole bug., The shape check that separates real node IDs from log locators., TestIsNodeId, TestWorkflowsUseTheParser

### Community 890 - "TestTheWorkflowIsSafeAndReadOnly"
Cohesion: 0.20
...
### Community 891 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 893 - "TestConflictedPullRequestsGetUnstuck"
Cohesion: 0.20
...
### Community 894 - "TestPoliciesGovernanceStableClaim"
Cohesion: 0.20
...
### Community 895 - "test_model_catalog_guard.py"
Cohesion: 0.29
...
### Community 899 - "test_workflow_api_mount.py"
Cohesion: 0.20
...
### Community 901 - "_extract_workflow_relevance"
Cohesion: 0.33
...
### Community 902 - "Skill: changelog-enforcer"
Cohesion: 0.22
...
### Community 903 - "Skill: learn-rule"
Cohesion: 0.22
...
### Community 908 - "Skill: changelog-enforcer"
Cohesion: 0.22
Nodes (8): Acceptance Checks, Changelog Location, Entry Format, Examples, Hook Behaviour, Instructions, Skill: changelog-enforcer, When to Use

### Community 909 - "Skill: cowork-session (Claude Cowork)"
Cohesion: 0.22
Nodes (8): Branch, Components, Purpose, Quick Start, Session Roles, Skill: cowork-session (Claude Cowork), Testing, When to Use

### Community 910 - "Skill: video-context — read a video without watching it"
Cohesion: 0.22
...
### Community 911 - "Runtime troubleshooting"
Cohesion: 0.33
...
### Community 913 - "record_usage_endpoint"
Cohesion: 0.22
...
### Community 914 - "Issue #1356: quick-note:https://searchengineland.com/turn-seo-backlog-into-roadmap-485713"
Cohesion: 0.22
...
### Community 915 - "Render MCP — autonomous platform debugging and environment monitoring"
Cohesion: 0.22
...
### Community 916 - "Release Procedure"
Cohesion: 0.22
...
### Community 917 - "V2.0 Modernization — Runbook"
Cohesion: 0.22
...
### Community 918 - "TestTheProducersAndTheParserCannotDrift"
Cohesion: 0.25
...
### Community 920 - "ImprovementLoop"
Cohesion: 0.04
Nodes (93): AgentRole, _parse_ceo_directives(), Enum, str, DetectedIssue, ImprovementLoop, ImprovementLoopState, IssueCategory (+85 more)

### Community 921 - "GovernanceScreen.jsx"
Cohesion: 0.25
Nodes (5): AuditTable(), BACKEND_META, DECISION_COLOR, relTime(), GovernanceScreen

### Community 922 - "_extract_pytest_failure"
Cohesion: 0.42
...
### Community 923 - "_get_current_user"
Cohesion: 0.25
...
### Community 927 - "test_scanner_headless.py"
Cohesion: 0.28
Nodes (6): _is_blocked_host(), Cheap (no-DNS) SSRF check for headless-browser subrequests. A rendered page's…, _guard(), Tests for the scanner's headless-render fallback (JS-rendered / bot-protected…, A URL with no hostname (file://, malformed) must be blocked, and…, TestSubrequestSsrfGuard

### Community 929 - "SubAgentConfig"
Cohesion: 0.39
...
### Community 930 - ".test_every_request_carries_a_real_user_agent"
Cohesion: 0.22
...
### Community 932 - "Prime Agent Runtime"
Cohesion: 0.25
...
### Community 940 - "test_backend_requirements_cover_runtime_imports.py"
Cohesion: 0.25
...
### Community 942 - "test_changelog_parity_guard.py"
Cohesion: 0.22
...
### Community 943 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.22
...
### Community 944 - "test_procedural_memory.py"
Cohesion: 0.24
...
### Community 945 - "_FlakyPersistence"
Cohesion: 0.22
...
### Community 951 - "hybrid_reasoning.py"
Cohesion: 0.36
...
### Community 954 - "research_coordinator.py"
Cohesion: 0.36
...
### Community 956 - "_start_in_web_bot_tasks"
Cohesion: 0.29
...
### Community 957 - "test_p0_roadmap_b1_c2_a3.py"
Cohesion: 0.11
...
### Community 958 - "commercial_equivalent.py"
Cohesion: 0.50
...
### Community 959 - "Skill: browserbase-browser — Real Browser Automation"
Cohesion: 0.25
...
### Community 961 - "Skill: memory-consolidation (Dream Memory)"
Cohesion: 0.25
...
### Community 964 - "GitHub Branch Protection Settings"
Cohesion: 0.25
...
### Community 967 - "AGENTS.md — AI Agent Configuration for local-llm-server"
Cohesion: 0.25
...
### Community 968 - "Issue #504: EPIC: Autonomy hardening — live-verified defects 2026-06-10"
Cohesion: 0.25
...
### Community 972 - "Issue #362: Nvidia repo setup"
Cohesion: 0.25
...
### Community 973 - "Issue #364: quick-note:https://www.marktechpost.com/2026/06/01/meet-memory-os-a-6-layer-open-source-memory-stack-built-on-top-of-hermes-agent/"
Cohesion: 0.25
...
### Community 974 - "Issue #378: quick-note:https://www.marktechpost.com/2026/06/02/tinyfish-launches-bigset-an-open-source-multi-agent-system-that-builds-structured-live-datasets-from-plain-english-descriptions/"
Cohesion: 0.25
...
### Community 975 - "Issue #379: quick-note:https://searchengineland.com/schema-markup-optimize-agentic-web-479080"
Cohesion: 0.25
...
### Community 976 - "Issue #380: quick-note:https://cursor.com/blog/cloud-agent-lessons"
Cohesion: 0.25
...
### Community 977 - "Issue #381: quick-note:https://www.xda-developers.com/claude-code-with-opus-48-is-expensive-but-i-made-it-efficient-with-my-local-ai-workflow/"
Cohesion: 0.25
...
### Community 978 - "Issue #382: quick-note:https://claude.com/blog/how-coderabbit-used-claude-to-build-an-agent-orchestration-system"
Cohesion: 0.25
...
### Community 979 - "Issue #383: quick-note:https://www.marktechpost.com/2026/05/29/hexo-labs-open-sources-sia-a-self-improving-agent-that-updates-both-the-harness-and-the-model-weights/"
Cohesion: 0.25
...
### Community 980 - "Issue #416: feat: Self-hosted Codebuff (freebuff) on free NVIDIA models + Telegram bot phone control"
Cohesion: 0.25
...
### Community 981 - "Issue #485: [Trend Digest] Week of 2026-06-08"
Cohesion: 0.25
...
### Community 982 - "Issue #488: quick-note:https://github.com/cookiy-ai/user-research-skill"
Cohesion: 0.25
...
### Community 983 - "Issue #491: Implement whatever is necessary from https://github.com/BehiSecc/awesome-claude-skills"
Cohesion: 0.25
...
### Community 984 - "Issue #493: Use the https://github.com/mvanhorn/last30days-skill skill to get the trend updated"
Cohesion: 0.25
...
### Community 985 - "Issue #495: Read https://www.anthropic.com/news/claude-fable-5-mythos-5 and understand if mythos or fable can be added to the repo"
Cohesion: 0.25
...
### Community 986 - "SECTION A — Agent Efficiency (Hermes / AOS / MYT)"
Cohesion: 0.25
...
### Community 987 - "Issue #581: Sprint tracker: pending work after brand rename + mobile-first pass"
Cohesion: 0.25
...
### Community 989 - "Issue #657: quick-note:https://github.com/earendil-works/pi"
Cohesion: 0.25
...
### Community 990 - "Issue #659: quick-note:https://github.com/nex-agi/Nex-N2"
Cohesion: 0.25
...
### Community 991 - "Issue #660: quick-note:https://github.com/getsentry/sentry-for-ai"
Cohesion: 0.25
...
### Community 992 - "Issue #661: quick-note:https://github.com/XiaomiMiMo/MiMo-Code"
Cohesion: 0.25
...
### Community 993 - "Issue #664: quick-note:https://github.com/Grominet95/jarvis-OS"
Cohesion: 0.25
...
### Community 994 - "Issue #666: quick-note:https://github.com/porokka/jarvis-os"
Cohesion: 0.25
...
### Community 995 - "Issue #670: quick-note:https://github.com/perplexityai/bumblebee"
Cohesion: 0.25
...
### Community 996 - "Issue #672: quick-note:https://github.com/Chachamaru127/claude-code-harness"
Cohesion: 0.25
...
### Community 997 - "Issue #676: quick-note:https://github.com/WeiboAI/VibeThinker"
Cohesion: 0.25
...
### Community 998 - "Issue #820: quick-note:https://github.com/cobusgreyling/loop-engineering"
Cohesion: 0.25
...
### Community 1000 - "Worker Service — Operations Runbook"
Cohesion: 0.18
...
### Community 1001 - "Positional Encoding Internals"
Cohesion: 0.25
...
### Community 1005 - "test_auto_merge_workflow.py"
Cohesion: 0.25
...
### Community 1006 - "Backend changes"
Cohesion: 0.14
Nodes (15): `activation_api.py`, Backend changes, `backend/company_api.py`, `db/sqlite_store.py`, `models/company_graph.py`, `services/ephemeral_reaper.py` (new) + `services/background.py`, _anthropic_chat_payload(), _anthropic_text() (+7 more)

### Community 1007 - "TestItIsNotBuiltForOneVendor"
Cohesion: 0.25
...
### Community 1011 - "test_provider_models_db_outage.py"
Cohesion: 0.25
...
### Community 1014 - "test_dependabot_sweep_workflow.py"
Cohesion: 0.25
...
### Community 1016 - "test_nuclear_cleanup_targets_the_schedule_collection"
Cohesion: 0.25
...
### Community 1020 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 1021 - "test_setup_detect_models_ssrf.py"
Cohesion: 0.29
...
### Community 1024 - "test_weekly_trend_digest_workflow.py"
Cohesion: 0.50
...
### Community 1025 - "is_legacy_mode"
Cohesion: 0.25
Nodes (6): is_legacy_mode(), True when parallel execution paths are allowed (with warnings)., Tests for AgentRunner execution path., Verify AgentRunner has _execute_step for ReAct execution loop., Verify _BYPASS context var is used for internal agent execution., TestAgentRunnerExecution

### Community 1026 - "UsageEvent"
Cohesion: 0.29
...
### Community 1028 - "Marketing Growth Hacker Agent"
Cohesion: 0.29
...
### Community 1032 - "build_connectors_router"
Cohesion: 0.38
...
### Community 1034 - "What to clean up"
Cohesion: 0.20
...
### Community 1045 - "Platform Controls"
Cohesion: 0.29
...
### Community 1046 - "test_runtimes_health_endpoint.py"
Cohesion: 0.25
...
### Community 1047 - "CLAUDE.md — Operating Manual"
Cohesion: 0.29
...
### Community 1048 - "Cloudflare = the real working app"
Cohesion: 0.29
...
### Community 1049 - "Advisor Strategy — Local Proxy Handling"
Cohesion: 0.29
...
### Community 1054 - "Feature Maturity / Support Matrix"
Cohesion: 0.29
...
### Community 1060 - "test_daily_2026_06_14.py"
Cohesion: 0.38
...
### Community 1061 - "check_doc_images.py"
Cohesion: 0.48
...
### Community 1062 - "TestBacklogActuallyDrains"
Cohesion: 0.29
...
### Community 1063 - "TestLockfileRepair"
Cohesion: 0.38
...
### Community 1069 - "TestProvidersScreen"
Cohesion: 0.43
...
### Community 1070 - "TOP 6 — Highest-ROI Items (Validated by Opus Research Agent)"
Cohesion: 0.29
...
### Community 1072 - "TestMongoService"
Cohesion: 0.29
...
### Community 1074 - "extract_failures"
Cohesion: 0.16
Nodes (10): extract_failures(), Return the lines at or after pytest's first summary banner. Falls back to the…, Return up to *max_results* unique failing node IDs from pytest *output*. Order…, _summary_region(), Regression tests for ``scripts/parse_pytest_failures.py``. Guards the two…, Without a banner we scan everything, but the shape check still holds., End-to-end extraction over realistic pytest output., Issue #1354: log records must never be reported as failing tests. (+2 more)

### Community 1081 - "TestCli"
Cohesion: 0.57
Nodes (3): Path, The workflows call this as a subprocess, so the CLI contract matters., TestCli

### Community 1083 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 1084 - "test_skills_route_order.py"
Cohesion: 0.67
...
### Community 1085 - "Music Cues: happy-beats-business-moves-vol-1-by-ende-dot-app"
Cohesion: 0.33
...
### Community 1086 - "/fix-bug — Bug Fix Agent"
Cohesion: 0.33
Nodes (5): Escalation, /fix-bug — Bug Fix Agent, Process, Rules, Usage

### Community 1087 - "_safe_resolve"
Cohesion: 0.25
...
### Community 1088 - "pre-commit"
Cohesion: 0.60
Nodes (5): pre-commit script, _error(), _head(), _info(), _warn()

### Community 1089 - "Skill: browserbase-fetch — Lightweight Web Fetch"
Cohesion: 0.33
...
### Community 1102 - "DirectChatDoctor"
Cohesion: 0.32
Nodes (5): DirectChatDoctor, Run doctor checks before execution., asyncio, When git is missing and no GitHub token is present, the doctor should report…, test_missing_git_and_token()

### Community 1103 - "test_serve_spa_prefixes.py"
Cohesion: 0.31
...
### Community 1104 - "coordinate_v2"
Cohesion: 0.33
Nodes (6): coordinate_v2(), CoordinateRequestV2, Any, post, V2 coordinate endpoint — accepts AgentConfig + TaskInput lists. The primary…, Extended coordinate request supporting both agents+tasks and legacy workers.

### Community 1105 - "StatusPill.jsx"
Cohesion: 0.47
Nodes (5): C, STATUS_META, StatusPill(), TONE_BG(), TONE_BORDER()

### Community 1106 - "get_output_filter"
Cohesion: 0.33
...
### Community 1107 - "parse_react_response"
Cohesion: 0.47
...
### Community 1111 - "SECTION D — Deployment & Infrastructure (CHM / NVD)"
Cohesion: 0.33
...
### Community 1112 - "gen_v4_screenshots.py"
Cohesion: 0.60
Nodes (5): build_screens(), page(), Generate v4 UI screenshots for the README using HTML mockups + system…, shot(), sidebar()

### Community 1113 - "TestCli"
Cohesion: 0.25
Nodes (6): parametrize, Path, The label must not be reachable without the analysis that justifies it., The workflow branches on the verdict file, so that contract matters., TestCli, TestWorkflowUsesTheGate

### Community 1114 - "TestCEOAgencySystem"
Cohesion: 0.33
...
### Community 1121 - "test_hosted_control_plane_regressions.py"
Cohesion: 0.73
...
### Community 1123 - "apply_phase1_changes.py"
Cohesion: 0.33
...
### Community 1129 - "test_doctor_anonymous_no_pat.py"
Cohesion: 0.50
...
### Community 1137 - "TestTheAgentRunsCurrentCode"
Cohesion: 0.33
...
### Community 1139 - "Runbook — Instance Activation"
Cohesion: 0.25
...
### Community 1146 - "feature-implementer.md"
Cohesion: 0.40
Nodes (4): Before you edit, Prove it, Report back, While you edit

### Community 1147 - "/devops-check — DevOps Agent"
Cohesion: 0.40
Nodes (4): Deployment Checklist, /devops-check — DevOps Agent, Steps, When to use

### Community 1148 - "/docs-update — Documentation Agent"
Cohesion: 0.40
Nodes (4): /docs-update — Documentation Agent, Documentation Standards, Steps, When to use

### Community 1149 - "/qa-check — QA Agent"
Cohesion: 0.40
Nodes (4): /qa-check — QA Agent, Steps, What NOT to do, When to use

### Community 1151 - "/security-audit — Security Agent"
Cohesion: 0.40
Nodes (4): Escalation, /security-audit — Security Agent, Steps, When to use

### Community 1152 - "pre-push"
Cohesion: 0.70
Nodes (4): pre-push script, _error(), _head(), _info()

### Community 1153 - "Skill: browserbase-search — Structured Web Search"
Cohesion: 0.40
Nodes (4): Best practice: search → fetch → browse, Python snippet, Setup, Skill: browserbase-search — Structured Web Search

### Community 1154 - "Issue #230 — DUPLICATE"
Cohesion: 0.40
Nodes (4): Actions Taken, Issue #230 — DUPLICATE, References, Resolution

### Community 1155 - "test_task_store_fails_loud_in_production.py"
Cohesion: 0.33
Nodes (5): Regression: prevent silent TaskStore in-memory fallback in production. The…, With TESTING unset (production), TaskStore(db=None) MUST raise., With TESTING=true (CI), TaskStore(db=None) MUST allow in-memory fallback., test_task_store_allows_inmemory_when_testing(), test_task_store_raises_in_production()

### Community 1156 - "Docker (local or any container host)"
Cohesion: 0.40
...
### Community 1158 - "V5App.jsx"
Cohesion: 0.04
Nodes (40): Changed, Changed, frontend_src_api_default, getActivity(), V5App, HubTabs(), Spinner(), activityToAlert() (+32 more)

### Community 1159 - "Core Pillars"
Cohesion: 0.20
...
### Community 1161 - ".__init__"
Cohesion: 0.50
Nodes (3): Any, Resolve the default executor model via the catalog (UNIT 7). Was hardcoded to…, _resolve_default_executor_model()

### Community 1162 - "submit_simple_task"
Cohesion: 0.50
Nodes (4): Submit a task to the agent planner., Submit a simple task via the tasks API., submit_simple_task(), submit_task()

### Community 1163 - "Agency Core — Operational Knowledge (verified live, 2026-06-10/11)"
Cohesion: 0.40
...
### Community 1167 - "SECTION E — Autonomy & Self-Healing (AOS / MYT / ECC)"
Cohesion: 0.40
...
### Community 1186 - "TestFailoverRegistryBaseUrls"
Cohesion: 0.40
...
### Community 1188 - "TestHermesKeepAlive"
Cohesion: 0.40
...
### Community 1190 - "_openai_tools_to_anthropic"
Cohesion: 0.50
...
### Community 1193 - "TestGhIsNotReAuthenticated"
Cohesion: 0.40
...
### Community 1194 - "TestModelRoleSeparation"
Cohesion: 0.40
...
### Community 1216 - "stop_colibri_server.ps1"
Cohesion: 0.83
Nodes (3): Fail(), Ok(), W()

### Community 1217 - "get_store"
Cohesion: 0.19
Nodes (9): get_store(), Return the active store singleton, creating it on first call., ADR-007: Storage backend duck-typing over formal ABC, Consequences, Context, Decision, Rationale, MongoStore (+1 more)

### Community 1218 - "test_no_exception_detail_leaks.py"
Cohesion: 0.50
...
### Community 1222 - "test_feature_stores_are_wired_on_demand_not_eagerly"
Cohesion: 0.50
...
### Community 1224 - "SECTION H — Vision / Multimodal (NVD)"
Cohesion: 0.67
...
### Community 1226 - "BudgetExceeded"
Cohesion: 0.67
Nodes (3): BudgetExceeded, Exception, Raised internally when a session exhausts a limit. Never escapes guard().

### Community 1227 - "_is_pushable_scalar"
Cohesion: 0.67
Nodes (3): _is_pushable_scalar(), Any, Scalar values whose `str()` form matches how they were stored in the indexed…

### Community 1230 - "Karpathy Guidelines Skill"
Cohesion: 0.29
...
### Community 1388 - "TestInternalAgentAdapterProviderChain"
Cohesion: 0.40
...
## Knowledge Gaps
- **3553 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3548 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14812 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **188 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AgentRunner` connect `AgentRunner` to `typing`, `is_legacy_mode`, `GitHubTools`, `Issue #656: Bugs`, `TaskSpec`, `.session_id`, `test_repo_connection.py`, `proxy.py`, `Added`, `Added`, `Agent: Implementer (Executor)`, `pytest`, `TokenBudget`, `_resolve_push_token`, `Changed`, `PolicyEngine`, `workflow_orchestrator.py`, `Agency Core — Progress & Resume Log`, `ContextPruner`, `E2BSandboxSession`, `test_agent_chat_integration.py`, `test_governance_api.py`, `AgentSessionStore`, `Fixed`, `test_agent_runner.py`, `build_governance_router`, `TestAgentLoopMCPIntegration`, `backend/server.py`, `UserMemoryStore`, `Competitor Analysis — Autonomous AI Agency`, `ContextManager`, `failover_chat_completion`, `WorkspaceTools`, `test_ceo_supervision.py`, `TestRunnerWorkspace`, `_runner`, `agent/models.py`, `test_self_heal_v2.py`, `get_company_graph_store`, `Issue #504: EPIC: Autonomy hardening — live-verified defects 2026-06-10`, `AgentPlan`, `test_empirical_verify.py`, `AgentPhaseError`, `Section-by-Section Acceptance Criteria`, `MultiAgentSwarm`, `AdaptiveHalter`, `SECTION A — Agent Efficiency (Hermes / AOS / MYT)`, `diagnostics.py`, `direct_chat.py`, `StuckDetector`, `LocalWorkspace`, `FreeBuffAgent`, `Agent Autonomy Roadmap`, `WorkflowOrchestrator`, `TestInternalAgentAdapterProviderChain`, `AgentJobRequest`, `Universality: case-coverage matrix`, `ReactScratchpad`, `test_agent_free_brain.py`, `fmtErr`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `_fixture()` connect `_fixture` to `typing`, `build_llm_router`, `UsageEvent`, `TaskSpec`, `TaskStatus`, `Added`, `test_llm_router_queue_cache.py`, `_resolve_push_token`, `test_kimi_bridge_server.py`, `Task`, `PolicyEngine`, `router_factory`, `Changed`, `SelfHealingAgent`, `AgentDefinition`, `test_runtimes_health_endpoint.py`, `E2BSandboxSession`, `test_sam_voice.py`, `Agency`, `AgentRunner`, `get_improvement_loop`, `Fixed`, `WorkflowBuildRequest`, `session_retro.py`, `SpecEntry`, `TestSwarmRoleRouting`, `services/seo_audit.py`, `secrets_store.py`, `TestTeamSummary`, `failover_chat_completion`, `TestFeaturesAPI`, `Evidence`, `TestChatHistoryStore`, `_mock_provider_records`, `test_llm_router_resilience.py`, `test_self_heal_v2.py`, `test_startup_warmup.py`, `allow_paid`, `get_company_graph_store`, `ProviderRouter`, `PrimeAgentAdapter`, `ExecutionRequest`, `test_refresh_agent_built_proof.py`, `test_rate_limiter.py`, `ArtifactStore`, `SeoFixRequest`, `test_provider_router.py`, `e2e/test_browser.py`, `diagnostics.py`, `direct_chat.py`, `test_agency_fix.py`, `test_webui_provider_priority.py`, `test_all_features.py`, `TestClient`, `FreeBuffAgent`, `AgentSwarm`, `test_knowledge_sync.py`, `brain_failover.py`, `config.py`, `get_skill_bindings`, `_step`, `test_provider_state_durability.py`, `Added`, `test_autonomy_status.py`, `persist_plan_spec`, `test_context_rulebook.py`, `test_new_features_e2e.py`, `test_llm_router_e2e.py`, `test_trend_watcher.py`, `AgentScheduler`, `test_traffic_director.py`, `test_v4_api.py`, `test_issue_intake.py`, `test_agile_api.py`, `ChatHistoryStore`, `WorkflowRun`, `llm/router.py`, `services/background.py`, `test_shared_state.py`, `process_note`, `is_provider_on_cooldown`, `_runner`, `test_persistent_memory.py`, `KeyPool`, `_process_task_callback`, `asyncio`, `test_sam_livekit.py`, `test_video_transcript.py`, `test_daily_2026_07_27.py`, `KeyStore`, `test_openclaw_gateway.py`, `TestSourceGate`, `OllamaCircuitBreaker`, `test_features_api.py`, `TrendWatcher`, `AuditLog`, `NotificationDispatcher`, `test_autonomy_triage.py`, `TestEstimateTokensForMessages`, `Path`, `test_telegram_webhook.py`, `test_agents.py`, `test_daily_2026_06_04.py`, `test_control_plane_api.py`, `pytest`, `test_purge_backlog.py`, `LocalBrainStore`, `_client`, `test_mcp_governance.py`, `test_provider_enable_disable.py`, `ContextPruner`, `test_hermes_in_process.py`, `test_task_clarification.py`, `test_brain_patch_service_token.py`, `test_app_settings.py`, `test_nvidia_model_discovery.py`, `test_output_filter.py`, `TestHarnessAdapter`, `test_spa_trailing_slash_redirect.py`, `test_operational_incidents.py`, `SecurityScanner`, `_resolve_user_github_token`, `test_vision_routing.py`, `ensure_bootstrap`, `TestNormalizeResponseFormat`, `test_probe_report.py`, `test_skill_registry_boot_refresh.py`, `test_ceo_router.py`, `FakeRunner`, `_redact_for_notification`, `test_colibri_brain_shim.py`, `WorkflowOrchestrator`, `analyze_page`, `wiki_client`, `_SlowLoginPage`, `TestTheWorkflowIsSafeAndReadOnly`, `TestMongoGate`, `test_regression.py`, `test_daily_automation_2026_05_15.py`, `pr_approval_gate.py`, `Changelog`, `test_persona_library.py`, `ImprovementLoop`, `test_platform_controls.py`, `test_langfuse_agency_wide.py`, `test_workflow_shell_vars_are_declared.py`, `test_executive_advisory_api.py`, `test_process_quick_note_workflow.py`, `test_procedural_memory.py`, `SQLiteStore`, `test_service_token.py`, `test_dashboard_cache.py`, `strategies.py`, `test_cost_attribution.py`, `test_openclaw_endpoints.py`, `test_scheduler_hydration_bounded.py`, `test_provider_failover_integration.py`, `test_telegram_diag_endpoint.py`, `test_auto_merge_workflow.py`, `LLMRouter`, `test_daily_automation_2026_09_25.py`, `test_dependabot_sweep_workflow.py`, `orchestrator`, `test_agent_free_brain.py`, `test_setup_detect_models_ssrf.py`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `Company` connect `company_api.py` to `seo_api.py`, `AppShell.jsx`, `Any`, `get_company_graph_store`, `ScheduleStore`, `test_repo_connection.py`, `WorkflowOrchestrator`, `Backend changes`, `SQLiteStore`, `Changed`, `MongoDBStore`, `test_persona_library.py`, `AgentDefinition`, `looks_like_secret_file`, `generate_onboarding_questions`, `CompanyGraphStore`, `Specialist`?**
...
- **Should `typing` be split into smaller, more focused modules?**
  _Cohesion score 0.01023171456268176 - nodes in this community are weakly interconnected._
- **Should `build_llm_router` be split into smaller, more focused modules?**
  _Cohesion score 0.045548654244306416 - nodes in this community are weakly interconnected._
- **Should `LLMRequest` be split into smaller, more focused modules?**
