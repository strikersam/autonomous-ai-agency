...
## Corpus Check
- 1437 files · ~2,063,449 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28492 nodes · 57404 edges · 1189 communities (1061 shown, 128 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 6314 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7d945c2e`
...
## Communities (1189 total, 128 thin omitted)

### Community 0 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (121): _merge_changed_files(), Collect changed_files across all specialists into a single de-duped list., BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput (+113 more)
...
Cohesion: 0.04
Nodes (50): CompanyGraphSnapshot, A point-in-time snapshot of a Company Graph for history and rollback., MongoDBStore, Any, Company, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model. (+42 more)

### Community 6 - "test_llm_router_tpm.py"
Cohesion: 0.15
...
Cohesion: 0.03
Nodes (117): AgentRunner, Any, Path, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a…, Send chat messages and return the assistant's text output. If an…, Parse an LLM tool-call response, tolerating common malformations. Handles: 1.… (+109 more)

### Community 8 - "FreeBuffAgent"
Cohesion: 0.09
Nodes (101): BackgroundTask, BrowserSession, Async Playwright browser session. Usage:: session = BrowserSession() await…, True if Playwright is installed and ready to use., CommitAttribution, CommitTracker, Path, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata… (+93 more)

### Community 9 - "MCPUnavailableError"
Cohesion: 0.03
Nodes (103): MCPUnavailableError, Raised when the MCP server is unreachable or the circuit is open., TaskResult, TaskSpec, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, E2BSandboxSession, _inject_token() (+95 more)

### Community 10 - "LLMRouter"
Cohesion: 0.06
Nodes (32): Attempt, payload_key(), Exact-match cache key over the fields that change the answer. Routing…, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any, AsyncClient (+24 more)

### Community 11 - "llm/router.py"
Cohesion: 0.03
Nodes (102): FailoverResult, A successful completion plus the accounting its callers need., get_budget(), The process-wide budget tracker., CacheManager, cosine_similarity(), _Entry, get_cache() (+94 more)

### Community 12 - "operational_incidents.py"
Cohesion: 0.06
Nodes (55): _diagnose_and_file(), _file_incident(), _format_incident(), gather_render_evidence(), get_operational_incident_tracker(), _iso_from_monotonic(), note_phase_end(), note_phase_start() (+47 more)

### Community 13 - "loop.py"
Cohesion: 0.04
Nodes (67): Agent subsystem — planner / executor / verifier loop., AgentPhaseError, _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), _nvidia_api_key(), Exception (+59 more)

### Community 14 - "TaskSpec"
Cohesion: 0.03
Nodes (104): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns…, E2BAdapter, Any, runtimes/adapters/e2b.py — E2B Firecracker micro-VM runtime adapter. Routes…, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``… (+96 more)

### Community 15 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (108): _load(), main(), int, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, build_summariser(), chunk_document(), ContextManager, estimate_tokens() (+100 more)

### Community 16 - "TaskHarnessAdapter"
Cohesion: 0.03
Nodes (47): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, TaskResult, TaskSpec, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor. (+39 more)
...
Cohesion: 0.03
Nodes (101): events_from_identity(), Any, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., Return the most recent events, newest first, optionally filtered., Aggregate view for the dashboard and the metrics endpoint. ``would_block`` is…, Convenience: build and record an event in one call., Extract the identity-derived audit fields from an AgentIdentity. Kept here… (+93 more)

### Community 19 - "api.js"
Cohesion: 0.02
Nodes (44): approveGovernanceRequest(), autoRecommendCompanySkills(), createMcpServer(), createQuickNote(), delegateSeoFindings(), deleteCompany(), deleteMcpServer(), denyGovernanceRequest() (+36 more)
...
Cohesion: 0.03
Nodes (126): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), _active_primary_provider(), BrainConfig, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store() (+118 more)
...
### Community 29 - "Task"
Cohesion: 0.01
Nodes (288): agents/store.py — Persistent store for user-defined agent configurations.…, Set the global agent store instance (e.g., with MongoDB on startup)., set_agent_store(), _get_cached_tasks(), _get_tasks_cache_lock(), _load_improvement_state(), Any, BaseModel (+280 more)
...
Cohesion: 0.03
Nodes (91): _brain_provider_status(), get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., _served_models(), all_provider_ids(), _provider_ids_from_literal() (+83 more)
...
Cohesion: 0.05
Nodes (54): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, analyze_qualitative(), analyze_quantitative(), auto_register(), _classify_sentiment(), _extract_keywords(), plan_research() (+46 more)
...
Cohesion: 0.05
Nodes (38): field_validator, A single occurrence of a check firing on a specific URL., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoIssueReportRow (+30 more)
...
Cohesion: 0.11
Nodes (34): get_provider_role_tags(), _norm(), Any, Single source of truth for the active brain LLM. Resolution order (matches the…, Map provider_id -> ``{is_brain, role, reason}``. Mirrors the surface area…, Read provider records from the live store. Returns ``(records, fetch_failed)``.…, _read_provider_records(), resolve_active_brain() (+26 more)

### Community 45 - "enforcement.py"
Cohesion: 0.05
Nodes (46): _load(), Import the governance layer once, recording why if it is absent., BudgetExceeded, evaluate_call(), get_gate(), guard_tool_call(), GuardResult, _is_write_tool() (+38 more)
...
### Community 47 - "test_governance_enforcement.py"
Cohesion: 0.05
Nodes (63): ApprovalStatus, ApprovalStore, Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`…, Block until *approval_id* is resolved or its TTL expires. Combines an event…, BudgetTracker (+55 more)
...
Cohesion: 0.04
Nodes (52): Register the built-in agent tools that are always available., _register_builtin_tools(), repowise.py — RepowiseIntelligence: context packing and dependency analysis., Path, tools.py — WorkspaceTools: read/write/search and diff application (risky…, Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, Return a previously saved memory value, or an empty string if absent., Persist a key/value pair to the user's profile store. (+44 more)

### Community 54 - "_scanner"
Cohesion: 0.06
...
Cohesion: 0.06
Nodes (59): delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), list_seo_audits(), BaseModel, get, post (+51 more)
...
### Community 68 - "TaskExecutionCoordinator"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (49): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable(), _playwright_configured(), _probe_http() (+41 more)

### Community 70 - "test_ceo_supervision.py"
Cohesion: 0.07
Nodes (59): A subtask's full history: what it is, and every attempt at it., SubtaskRecord, _goal(), asyncio, Tests for the CEO ledger, the supervised escalation loop, and the 24x7 sweeper.…, A storage outage must never take the agency down., Two sweeping processes must not both re-drive the same goal.…, A sweeper that loses the claim reports in-flight, not redriven. (+51 more)
...
### Community 74 - "test_telegram_service_webhook.py"
Cohesion: 0.11
...
### Community 84 - "IssueCategory"
Cohesion: 0.06
Nodes (34): IssueCategory, Enum, str, filter_safe_tools(), get_tool_annotations(), Typed representation of MCP tool annotations (spec 2025-11-05 §5.6.1). All…, Return True only when the tool is definitively read-only and non-destructive.…, Extract ``ToolAnnotations`` for a named tool from a ``list_tools()`` result.… (+26 more)
...
Cohesion: 0.06
Nodes (37): Initiative, _bug_scores(), _clean(), _default_repo(), _env_github_token(), estimate_job_size(), fetch_github_signals(), initiatives_from_bug_log() (+29 more)
...
Cohesion: 0.06
Nodes (63): Decision, Enum, str, A control surface — the kind of thing being governed. Mirrors Docker AI…, The verdict for one governed action., Surface, _blocked_result(), _governance_check() (+55 more)
...
Cohesion: 0.04
Nodes (38): _check_rate_limit(), default_keys_path(), issue_new_api_key(), KeyRecord, KeyStore, load_key_store(), Exception, Path (+30 more)
...
Cohesion: 0.05
Nodes (44): addTaskComment(), approveTaskCheckpoint(), approveTaskExecution(), clarifyTask(), createSprint(), createTask(), escalateTask(), fetchSprints() (+36 more)
...
Cohesion: 0.03
Nodes (53): _bullets(), generate_backlog_retro(), generate_sprint_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive retro notes for ``sprint`` from its current metrics. Records… (+45 more)
...
Cohesion: 0.03
Nodes (36): requires_db, Initialize the service. Args: store: Optional CompanyGraphStore instance. If…, CompanyGraphStore, Delete a company and all its associated data., List companies with optional filtering., Count total companies in the store., Create a new company graph., Get the complete company graph for a company. (+28 more)
...
Cohesion: 0.04
Nodes (43): createProvider(), deleteProvider(), getBrainConfig(), getBrainProviders(), getLocalBrainState(), getProviderPolicy(), getRenderHealth(), getRenderOpsStatus() (+35 more)
...
Cohesion: 0.04
Nodes (58): emit_agency_observation(), Record an agency platform operation in Langfuse. Unlike…, Any, packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, One-shot self-healing pass. 1. Checks if the active brain provider is in a…, self_heal_brain_and_unblock_tasks(), BrainWatchdog, _classify_failure_cause() (+50 more)
...
### Community 137 - "test_telegram_approval_gate.py"
Cohesion: 0.08
...
Cohesion: 0.05
Nodes (14): Agent jobs created with workspace integration should have a workspace_path…, If a symlink inside the workspace points outside, resolve_path blocks it., Only expired workspaces (past retention TTL) are cleaned up., Two threads creating the same session/job should not corrupt state., TestWorkspaceLifecycle, TestWorkspaceManifest, TestWorkspaceResume, Any (+6 more)
...
Cohesion: 0.04
Nodes (44): agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, _as_list(), _coerce_payload(), Any, RuntimeError, packages/integrations/render_mcp.py — Render platform access over MCP. The…, Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return… (+36 more)
...
Cohesion: 0.03
Nodes (58): Atomically delete a run-once row only while it is still unfired. Closes the…, _age_seconds(), _now(), Any, Reconstruct a ScheduledJob from its as_dict() output., Register a new job. Returns the created :class:`ScheduledJob`.…, Fire a job immediately (webhook / manual trigger)., Remove a job. Returns *True* if it existed. (+50 more)
...
Cohesion: 0.06
Nodes (62): get_agency(), set_agency(), _autonomy_bg_cycle(), autonomy_tick(), Background CEO cycle + task dispatch. Runs fire-and-forget., Execute ONE pending task synchronously. Called by the cron workflow every 2…, _capability_tags(), create_task_from_oldest_open_issue() (+54 more)

### Community 153 - "TestAuthAndTaskOwnership"
Cohesion: 0.07
Nodes (17): skip, Test iteration 7 features: - POST /api/tasks/ auto-assigns an available agent…, Test that routing policy defaults allow paid fallback only with approval, GET /runtimes/policy should show never_use_paid_providers=false and…, Test authentication and task creation with owner assignment, Test provider configuration including anthropic-universal, GET /api/providers should include anthropic-universal provider, Get authentication token for admin user (+9 more)
...
### Community 155 - "report_to_markdown"
Cohesion: 0.11
...
### Community 159 - "session_retro.py"
Cohesion: 0.13
...
Cohesion: 0.06
Nodes (17): When True, the governance layer evaluates and audits agent actions. This is…, Typed configuration loaded from environment variables., When True, approval-gated actions self-approve. Local dev only., ``RENDER_SERVICE_IDS`` split into a clean list (empty when unset)., True when there is both an API key and an endpoint to reach., When True, the Render ops loop runs. On by default. Also requires…, When True, mutating Render MCP tools may be called. Default False., Validated `reasoning_effort` for Ollama thinking models, or ``""``. Returns one… (+9 more)
...
### Community 168 - "TestRenderRouter"
Cohesion: 0.14
...
### Community 182 - "openclaw_gateway.py"
Cohesion: 0.09
...
Cohesion: 0.10
Nodes (32): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), CommercialEquivalent, estimate_commercial_equivalent_usd(), get_prices(), _load_from_env() (+24 more)
...
### Community 188 - "test_trend_watcher.py"
Cohesion: 0.08
...
### Community 194 - "HermesAdapter"
Cohesion: 0.04
Nodes (53): _env_float(), HermesAdapter, Any, AsyncClient, Response, TaskResult, TaskSpec, Submit task to Hermes via its /tasks endpoint. (+45 more)
...
Cohesion: 0.12
Nodes (15): tests/test_sam_voice.py — Integration tests for SAM voice agent. Tests the SAM…, SAM's system prompt must address the user as Commander., get_sam() must return the same instance., get_status() must return uptime and session count., Empty input must return a prompt to repeat., Whitespace-only input must be treated as empty., Fallback must return operational status when LLM is down., Fallback must acknowledge task requests. (+7 more)

### Community 197 - "CEOSupervisor"
Cohesion: 0.06
Nodes (36): get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., reset_ceo_dispatcher(), CEOSupervisor, _env_flag(), _env_int(), Any (+28 more)
...
### Community 213 - "CompanyAgencyService"
Cohesion: 0.09
...
Cohesion: 0.04
Nodes (28): FeatureEntry, FeatureMatrix, BaseModel, One entry in the support matrix., Central support matrix — single source of truth. Loads the canonical feature…, Load canonical features and apply per-feature then bulk env overrides., Apply a config override string like 'stable', 'beta', 'disabled', 'enabled',…, Return the feature entry if available, or raise FeatureUnavailableError. (+20 more)
...
Cohesion: 0.13
Nodes (17): createSchedule(), pauseSchedule(), resumeSchedule(), triggerSchedule(), C, FREQ_OPTS, FREQ_TO_CRON, NewScheduleForm() (+9 more)
...
### Community 228 - "OrchestratorCheckpointStore"
Cohesion: 0.10
...
### Community 230 - "company_api.py"
Cohesion: 0.03
Nodes (129): account_lifecycle(), auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), generate_onboarding_questions(), get_company(), get_company_access() (+121 more)
...
### Community 240 - "test_task_source_id_race.py"
Cohesion: 0.17
...
### Community 250 - "test_scaffolding.py"
Cohesion: 0.10
...
### Community 254 - "agent/workspace.py"
Cohesion: 0.06
...
### Community 258 - "test_watchdog.py"
Cohesion: 0.09
...
### Community 272 - ".start_onboarding"
Cohesion: 0.08
...
Cohesion: 0.03
Nodes (98): AgentRole, str, Lifecycle status of a sprint., SprintStatus, add_initiative(), AllocationOut, BoardOut, get_board() (+90 more)
...
### Community 278 - "test_agent_readiness_audit.py"
Cohesion: 0.18
...
### Community 281 - "DockerAgentAdapter"
Cohesion: 0.17
...
### Community 290 - "get_secrets_store"
Cohesion: 0.13
...
### Community 291 - "test_crispy_workflow.py"
Cohesion: 0.08
...
### Community 308 - "TestSelfHealingInfrastructureClassification"
Cohesion: 0.19
...
### Community 310 - "test_integration_c4_c5_c6_d3.py"
Cohesion: 0.07
...
### Community 320 - "telegram_service.py"
Cohesion: 0.13
...
Cohesion: 0.07
Nodes (21): CircuitBreakerOpenError, NIMConnectionPool, Any, AsyncClient, Response, RuntimeError, Persistent httpx.AsyncClient pool with circuit breaker and retry logic. Manages…, Get or create the shared httpx.AsyncClient. (+13 more)
...
### Community 340 - "brain.py"
Cohesion: 0.10
Nodes (22): allow_paid_brain(), BrainResolution, get_active_brain_sync(), _host_is_openai_compatible(), _pick_from_records(), Shared free-brain policy helpers (issue #656). Single source of truth for "is…, Resolved brain LLM for the current agency configuration. ``provider_id`` is the…, Return the cached brain read by sync callers. None if never resolved. (+14 more)
...
### Community 345 - "TestActiveStrategy"
Cohesion: 0.29
...
Cohesion: 0.13
Nodes (22): get_doctor_diagnostics(), _in_container(), Path, Authenticated Doctor endpoint — full diagnostics. Returns all system-level…, Return True when running inside a container runtime. ``/.dockerenv`` alone…, Return the caller's GitHub token from EITHER place it can be stored. A token…, _resolve_user_github_token(), _FakeCollection (+14 more)
...
Cohesion: 0.05
Nodes (49): _build_provider_router(), _builtin_provider_records(), _chat_provider_policy(), _fallback_local_provider_record(), _get_provider_policy(), _list_configured_provider_records(), _nvidia_nim_provider_record(), ProviderRouter (+41 more)
...
### Community 366 - "AgentsScreen.jsx"
Cohesion: 0.11
Nodes (24): createAgent(), deleteAgent(), listAgents(), updateAgent(), AgentCard(), AgentForm(), AgentsPage(), cls() (+16 more)
...
### Community 382 - "test_phase4_runtime_resilience.py"
Cohesion: 0.06
Nodes (40): _best_cloud_primary_base(), _nvidia_provider_chain(), ProviderConfig, TaskResult, TaskSpec, Execute a TaskSpec using the internal AgentRunner and convert the agent's…, Create an isolated execution context for a single task. Tries ``git worktree…, Build Nvidia NIM provider config from env. Empty list when key is absent. (+32 more)
...
### Community 384 - "TestBrainConfigUpdates"
Cohesion: 0.14
...
### Community 400 - "test_minimax_bug_fixes.py"
Cohesion: 0.12
...
### Community 414 - "test_scanner_live.py"
Cohesion: 0.23
...
Cohesion: 0.11
Nodes (6): DecisionsStore, Any, Connection, DecisionsStoreTests, _fresh_store(), Smoke: create() returns a fresh dec_<hex8> per call (no error surfaces from…
...
### Community 449 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 454 - "._scan_github_repo"
Cohesion: 0.10
...
### Community 464 - "test_brain_availability_doctor.py"
Cohesion: 0.16
...
### Community 467 - "test_onboarding_provisioning.py"
Cohesion: 0.18
Nodes (17): _ds(), _models(), _profiles(), asyncio, parametrize, End-to-end test: onboarding across all domain types provisions specialists…, Step 8 (activate_agency) must not block start_onboarding's response. Regression…, A detected system's type must show up as context on at least one agent. (+9 more)
...
Cohesion: 0.14
Nodes (10): _extract_tags(), _first_paragraph(), _fmt_name(), Path, Fetch a flat .md file and convert it to a RegistrySkill., Return the first non-empty, non-heading line. Skips YAML frontmatter (--- ...…, Pull hashtags and bold words from markdown as tags., Tests for module-level helper functions. (+2 more)
...
### Community 492 - "CollaborationContext"
Cohesion: 0.09
Nodes (12): CollaborationContext, ContributorState, Enum, str, Claude Cowork — shared AI coding sessions with real-time sync. Enables multiple…, Role within a cowork session., Current phase of a cowork session., State of a single contributor within a session. (+4 more)
...
### Community 502 - "SQLiteStore"
Cohesion: 0.03
Nodes (49): DetectedSystem, A business system detected on a company's website or in their stack., Get the most confident evidence description., Create a new company in SQLite., Update a company in SQLite., Delete a company and all associated data from SQLite., List companies from SQLite., Count total companies in SQLite. (+41 more)
...
Cohesion: 0.03
Nodes (148): Any, Return recent commits with agent attribution trailers parsed out., get_sam(), get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, middleware, admin_control(), admin_create_key() (+140 more)
...
### Community 539 - "OperationalIncidentTracker"
Cohesion: 0.12
...
### Community 540 - "test_agent_chat_integration.py"
Cohesion: 0.23
...
Cohesion: 0.21
Nodes (9): _deep_merge(), get_guardrails(), Deep merge two dicts. Override values take precedence., Return the module-level GuardrailEngine singleton., CircuitState, get_nim_pool(), Enum, Return the module-level NIMConnectionPool singleton. (+1 more)
...
### Community 550 - "TestRouterIntegration"
Cohesion: 0.31
...
### Community 567 - "test_brain_failover_413.py"
Cohesion: 0.13
...
### Community 581 - "scan_repo_with_user_research.py"
Cohesion: 0.22
...
### Community 604 - ".on_task_complete"
Cohesion: 0.18
...
### Community 606 - "Workspace Isolation Architecture"
Cohesion: 0.17
...
### Community 607 - "test_direct_chat_async.py"
Cohesion: 0.29
...
### Community 647 - "TestRevenuePortfolio"
Cohesion: 0.20
...
### Community 648 - "resolve_free_nvidia_brain"
Cohesion: 0.13
...
### Community 665 - "test_providers_live_e2e.py"
Cohesion: 0.27
...
### Community 692 - "get_brain_preference"
Cohesion: 0.20
...
### Community 728 - "._check_permissions"
Cohesion: 0.29
...
### Community 756 - "_FakePersistence"
Cohesion: 0.20
...
### Community 761 - "GovernanceScreen.jsx"
Cohesion: 0.25
...
### Community 763 - "cleanup_stale_jobs"
Cohesion: 0.31
...
### Community 764 - "Agent Autonomy Roadmap"
Cohesion: 0.25
...
### Community 769 - "Feature Maturity / Support Matrix"
Cohesion: 0.25
...
### Community 793 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 803 - "test_hermes_server.py"
Cohesion: 0.25
...
### Community 828 - "services/background.py"
Cohesion: 0.06
Nodes (51): get_improvement_loop(), get_trend_watcher(), agent/trend_watcher.py — Internet-connected AI trend intelligence. Fetches from…, set_trend_watcher(), _await_hermes_ready(), BackgroundServices, _env_float(), _env_on() (+43 more)
...
### Community 867 - "scheduler/store.py"
Cohesion: 0.29
Nodes (3): get_scheduler_store(), _MemCursor, services/scheduler_store.py — Durable scheduler persistence. Issue #505:…
...
### Community 876 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 882 - "TestAdapterMetadata"
Cohesion: 0.08
...
### Community 914 - "TestProvidersScreen"
Cohesion: 0.43
...
### Community 935 - "test_schedule_store_create_index_options.py"
Cohesion: 0.48
...
### Community 936 - "TestDirectChatAgentModeSplit"
Cohesion: 0.29
Nodes (4): When agent_mode is False, no AgentJob should be created at the job manager…, When runtime preflight fails, the error should be structured with code +…, When agent_mode is True, an AgentJob should be created in the job manager. This…, TestDirectChatAgentModeSplit

### Community 938 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 939 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 940 - "Configuration reference"
Cohesion: 0.67
...
### Community 983 - "test_activity_logs.py"
Cohesion: 0.67
...
### Community 1003 - "test_direct_chat_doctor.py"
Cohesion: 0.67
...
- **Why does `_fixture()` connect `_fixture` to `ai/router.py`, `TestSwarmRoleRouting`, `test_openclaw_endpoints.py`, `test_task_service_failed_comment.py`, `test_llm_router_tpm.py`, `test_shared_state.py`, `MCPUnavailableError`, `llm/router.py`, `loop.py`, `_enable_filter`, `brain_config.py`, `SelfHealingAgent`, `test_autonomy_status.py`, `BackgroundAgent`, `config.py`, `Task`, `OperationalIncidentTracker`, `test_local_brain_state.py`, `test_unit8_model_catalog.py`, `TestChatHistoryStore`, `test_phase5_doctor.py`, `test_provider_state_durability.py`, `sam_with_mocks`, `test_llm_router_resilience.py`, `ImprovementLoop`, `ModelRouter`, `SeoAuditRequest`, `test_telegram_diag_endpoint.py`, `resolve_active_brain`, `enforcement.py`, `KeyPool`, `test_llm_router_strategies.py`, `SecretRecord`, `GoalRecord`, `WorkspaceTools`, `allow_paid`, `test_brain_failover_413.py`, `test_failover_client_shared.py`, `test_knowledge_sync.py`, `e2e/test_browser.py`, `PrimeAgentAdapter`, `resolve_identity`, `TestHarnessAdapter`, `TaskExecutionCoordinator`, `AgentProfile`, `test_telegram_service_webhook.py`, `ResearchTask`, `IssueCategory`, `test_agent_tool_governance.py`, `ProceduralMemoryStore`, `test_direct_chat_async.py`, `KeyStore`, `test_startup_warmup.py`, `test_openclaw_gateway.py`, `test_colibri_brain_shim.py`, `OllamaCircuitBreaker`, `test_llm_router_e2e.py`, `SeoFixer`, `test_new_features_e2e.py`, `get_feature_matrix`, `test_context_rulebook.py`, `ArtifactStore`, `BrainWatchdog`, `_Budget`, `test_telegram_approval_gate.py`, `ChatHistoryStore`, `persist_plan_spec`, `TestAuthAndTaskOwnership`, `session_retro.py`, `test_agile_api.py`, `test_app_settings.py`, `Usage`, `test_features_api.py`, `test_video_transcript.py`, `test_task_clarification.py`, `test_platform_controls.py`, `TestRuntimeControl`, `test_trend_watcher.py`, `lifespan`, `test_portfolio_intake.py`, `HermesAdapter`, `TrendWatcher`, `CEOSupervisor`, `OrchestratorQueue`, `test_control_plane_api.py`, `TestEstimateTokensForMessages`, `TestClient`, `_start_ceo_agency`, `ContextPruner`, `._check_permissions`, `test_persistent_memory.py`, `SecurityScanner`, `OrchestratorCheckpointStore`, `TestMongoGate`, `test_provider_enable_disable.py`, `test_rate_limiter.py`, `test_brain_failover.py`, `get_self_healing_agent`, `reap_expired_companies`, `TestNormalizeResponseFormat`, `brain_failover.py`, `test_purge_backlog.py`, `test_telegram_mutating_commands.py`, `parse_event_stream`, `test_skill_registry_boot_refresh.py`, `services/seo_audit.py`, `test_crispy_burn_in.py`, `traffic_director.py`, `isolated_telegram_config`, `WorkflowBuildRequest`, `test_scheduler_hydration_bounded.py`, `test_telegram_auto_approve.py`, `brain.py`, `_resolve_user_github_token`, `capture_screens.py`, `test_service_token.py`, `test_ping.py`, `TestTeamSummary`, `ClaudeCodeAdapter`, `test_runtimes_health_endpoint.py`, `test_regression.py`, `test_agency_fix.py`, `test_task_store_fails_loud_in_production.py`, `test_monitor_lib.py`, `test_v4_api.py`, `client`, `test_doctor_coding_brain.py`, `test_dashboard_cache.py`, `_MemDB`, `test_kimi_bridge_server.py`, `test_voice_pipeline.py`, `test_doctor_service_token_check.py`, `model_discovery.py`, `test_brain_patch_service_token.py`, `test_onboarding_provisioning.py`, `test_ai_insights.py`, `_clean_phases`, `WorkflowRun`, `_resolve_push_token`, `test_company_api.py`, `sam`, `SQLiteStore`, `cost_tracker.py`, `_get`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `test_ceo_dispatcher.py`, `AgentSessionStore`, `backend/server.py`, `test_empirical_verify.py`, `LocalWorkspace`, `FreeBuffAgent`, `MCPUnavailableError`, `AutonomyTracker`, `StuckDetector`, `loop.py`, `proxy.py`, `TaskSpec`, `Workspace`, `PortfolioManager`, `CEODispatcher`, `BackgroundAgent`, `test_agent_chat_integration.py`, `TestMCPServer`, `_StubManager`, `MCPClient`, `TestAgentLoopMCPIntegration`, `test_backend_server_features.py`, `WorkspaceTools`, `test_brain_failover_413.py`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `direct_chat.py`, `_build_request`, `test_governance_api.py`, `ExecutionRequest`, `RewardScorer`, `brain.py`, `TokenBudget`, `test_agent_tool_governance.py`, `ContextPruner`, `GitHubTools`, `WorkflowRun`, `test_direct_chat_async.py`, `AdaptiveHalter`, `_resolve_push_token`, `_FakeAsyncClient`, `ContextManager`, `TestZeroAttemptDiagnostics`, `ReactScratchpad`, `test_phase4_runtime_resilience.py`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `AgentSessionStore` to `ai/router.py`, `backend/server.py`, `llm/router.py`, `_P`, `system_instruction`, `settings.py`, `TestRouterIntegration`, `test_bedrock_provider.py`, `TestAnthropicPayloadStructuredOutput`, `direct_chat.py`, `test_bedrock_live.py`, `TrafficDirector`, `TestActiveStrategy`, `TestBrainFailoverBackoff`, `test_all_providers_discovery.py`, `test_colibri_brain_shim.py`, `test_anthropic_router.py`, `failover_client.py`, `_Budget`?**
