...
## Corpus Check
- 1475 files · ~2,154,693 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30056 nodes · 56212 edges · 1280 communities (1109 shown, 116 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3326 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9cec9070`
...
## Communities (1280 total, 116 thin omitted)
...
Cohesion: 0.02
Nodes (200): admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, autonomy_tick(), BrainTestRequest (+192 more)

### Community 2 - "TaskStatus"
Cohesion: 0.03
Nodes (124): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, CommentAddRequest, ExecutionApprovalRequest (+116 more)

### Community 3 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (106): int, CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any, T (+98 more)

### Community 4 - "proxy.py"
Cohesion: 0.03
Nodes (159): Describes a single worker's task and constraints., WorkerSpec, middleware, admin_control(), admin_create_key(), admin_create_user(), admin_delete_user(), admin_list_users() (+151 more)

### Community 5 - "CompanyGraphStore"
Cohesion: 0.03
Nodes (32): requires_db, Initialize the service. Args: store: Optional CompanyGraphStore instance. If…, CompanyGraphStore, Delete a company and all its associated data., List companies with optional filtering., Count total companies in the store., Delete a company graph., Create a snapshot of a company graph. (+24 more)

### Community 6 - "RuntimeHealth"
Cohesion: 0.04
Nodes (65): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Adapter for Aider — TIER 3 specialized git-aware code editor., runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns… (+57 more)

### Community 7 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (98): BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput, _orchestrator_bypass(), PersistOutput (+90 more)

### Community 8 - "ExecutionRequest"
Cohesion: 0.05
Nodes (46): Execute work through the 11-phase golden path., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_execute(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper). (+38 more)
...
### Community 10 - "Task"
Cohesion: 0.04
Nodes (69): Task, Full task/issue document., Update the updated_at timestamp., Task, _brain_is_configured(), _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing() (+61 more)
...
Cohesion: 0.01
Nodes (140): parse_event_stream(), ParsedRun, Structured view of one ``--mode json`` event stream., Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, _clear_discovered_models(), _clear_response_cache(), _isolate_brain_data_layer(), _isolate_operator_provider_state() (+132 more)

### Community 19 - "LLMRequest"
Cohesion: 0.03
Nodes (136): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, LLMProvider, OpenAICompatible, ABC, Any, AsyncClient (+128 more)
...
Cohesion: 0.03
Nodes (95): Recursively strip secrets from an arbitrary argument structure. Fails…, scrub(), BudgetExceeded, BudgetTracker, classify(), evaluate_call(), GovernanceGate, guard_tool_call() (+87 more)

### Community 27 - "loop.py"
Cohesion: 0.03
Nodes (116): loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, Feed LLM token/cost spend into the governance session budget. Kept separate…, _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent. (+108 more)

### Community 28 - "load_config"
Cohesion: 0.06
Nodes (51): _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir(), expand_env(), _fields_of(), _load_cache() (+43 more)
...
### Community 30 - "runtimes/manager.py"
Cohesion: 0.03
Nodes (49): _build_default_manager(), Any, runtimes/manager.py — RuntimeManager., Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Construct the default RuntimeManager. Production runtime (InternalAgentAdapter)… (+41 more)

### Community 31 - "agency.py"
Cohesion: 0.06
...
Cohesion: 0.04
Nodes (73): all_provider_ids(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the…, True when the env var for *provider*'s key is set (or it's Ollama)., CatalogActiveBrain, CatalogMirror, CatalogProviderEntry, get_catalog() (+65 more)

### Community 33 - "MongoDBStore"
Cohesion: 0.04
Nodes (50): CompanyGraphSnapshot, A point-in-time snapshot of a Company Graph for history and rollback., MongoDBStore, Any, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., MongoDB implementation of the Company Graph store. Uses Motor (async MongoDB… (+42 more)
...
Cohesion: 0.04
Nodes (57): field_validator, A single occurrence of a check firing on a specific URL., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoIssueReportRow (+49 more)
...
### Community 39 - "._dispatch_tool"
Cohesion: 0.50
Nodes (3): Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Condense a tool result down to what belongs in an audit row. A tool result can…, _summarise_tool_result()
...
Cohesion: 0.05
Nodes (52): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep (+44 more)

### Community 48 - "AgentSwarm"
Cohesion: 0.06
...
### Community 50 - "get_db"
Cohesion: 0.02
Nodes (119): _agent_timeout_fallback_response(), auth_me(), auto_recommend_skills(), _build_provider_router(), _builtin_provider_records(), call_llm(), _chat_error_detail_text(), _chat_provider_policy() (+111 more)

### Community 51 - "BrainWatchdog"
Cohesion: 0.05
Nodes (43): get_brain_config_store(), Return the process-wide ``BrainConfigStore`` singleton., Force a cache refresh (used by tests + the GET /admin/api/policy/brain…, refresh_brain_config_cache(), Any, packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, One-shot self-healing pass. 1. Checks if the active brain provider is in a…, self_heal_brain_and_unblock_tasks() (+35 more)

### Community 52 - "brain_config.py"
Cohesion: 0.04
Nodes (88): _active_primary_provider(), _build_base_url_env_from_yaml(), _build_candidates_from_yaml(), _build_default_base_url_from_yaml(), _build_display_names_from_yaml(), _build_key_env_from_yaml(), _build_presets_from_yaml(), _build_tier_from_yaml() (+80 more)
...
Cohesion: 0.04
Nodes (72): get_improvement_loop(), get_trend_watcher(), agent/trend_watcher.py — Internet-connected AI trend intelligence. Fetches from…, set_trend_watcher(), _await_hermes_ready(), BackgroundServices, _env_float(), _env_on() (+64 more)
...
Cohesion: 0.04
Nodes (51): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,… (+43 more)
...
Cohesion: 0.02
Nodes (77): KnowledgeItem, Structured knowledge about the company, systems, or processes., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, Website (+69 more)
...
Cohesion: 0.04
Nodes (71): Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, Any, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer… (+63 more)
...
Cohesion: 0.05
Nodes (64): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), packages.config — single source of truth for all configuration. Every module…, packages/config/settings.py — typed configuration. This is the ONLY module in… (+56 more)

### Community 66 - "direct_chat.py"
Cohesion: 0.08
Nodes (55): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), ResumeRequest, AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState (+47 more)
...
### Community 68 - "test_ceo_router.py"
Cohesion: 0.07
...
Cohesion: 0.04
Nodes (53): A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, build_matrix(), _families(), main(), Any, Map family -> sorted list of test files that mention it (quoted token). (+45 more)
...
### Community 77 - "E2BAdapter"
Cohesion: 0.07
Nodes (32): E2BAdapter, Any, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, _FakeAsyncSandboxClass, _FakeCommandResult, _FakeCommands, _FakeFiles (+24 more)

### Community 78 - "WebsiteScanner"
Cohesion: 0.03
Nodes (98): DetectedSystem, Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., A business system detected on a company's website or in their stack., Get the most confident evidence description., Inferred technology stack from website/repo analysis., StackInference (+90 more)
...
Cohesion: 0.05
Nodes (71): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover() (+63 more)

### Community 83 - "validate_policy_text"
Cohesion: 0.05
Nodes (34): _audit_proposal(), _baseline_loosened(), _baseline_patterns(), _GitHub, parse_policy_text(), _pr_body(), _proposal_branch_name(), propose_policy_change() (+26 more)

### Community 84 - "test_backend_server_features.py"
Cohesion: 0.08
...
Cohesion: 0.06
Nodes (34): Attempt, describe(), The operator-facing reason string for an unfixable status., Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any, AsyncClient (+26 more)
...
Cohesion: 0.02
Nodes (156): FailoverResult, A successful completion plus the accounting its callers need., get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., Test hook — clear all usage accounting., reset(), get_cache() (+148 more)
...
### Community 103 - "get_self_healing_agent"
Cohesion: 0.06
...
### Community 108 - "ProviderRouter"
Cohesion: 0.02
Nodes (145): _append_agent_session_message(), _run_agent_loop(), build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, _acquire_provider_probe(), CommercialFallbackRequiredError, _dead_model_key(), _exponential_backoff_cooldown() (+137 more)
...
### Community 129 - "is_anthropic_base_url"
Cohesion: 0.12
...
Cohesion: 0.06
Nodes (53): _check_background_liveness(), _check_ci_parity(), _check_company_graph(), _check_disk(), _check_event_log_integrity(), _check_feature_matrix(), _check_github_readiness(), _check_ollama() (+45 more)
...
Cohesion: 0.06
Nodes (46): ProviderHealth, Reset the singleton (for tests)., Circuit-breaker state for a provider., reset_failover_manager(), _clean_env(), _FakeAsyncClient, _FakeResponse, Regression test: HTTP 413 (Payload Too Large) must fail over to the next… (+38 more)
...
### Community 147 - "TestAdapterMetadata"
Cohesion: 0.05
Nodes (19): _env_float(), HermesAdapter, Any, Read a float env var, falling back to *default* on unset/garbage., Adapter for Hermes Agent — FIRST CLASS autonomous runtime., parametrize, Hermes must survive free-tier cold starts, and fallback must not log errors.…, A handled fallback is a warning; only an unrecoverable state is an error. (+11 more)
...
### Community 158 - "WorkspaceManager"
Cohesion: 0.19
...
Cohesion: 0.05
Nodes (31): _keepalive_self_ping(), lifespan(), _maybe_boot_purge(), _purge_backlog_core(), _purge_summary_clean(), Return orchestrator queue depth, active runs, and supervisor state (#522)., Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free… (+23 more)
...
### Community 177 - "test_runtime_governance.py"
Cohesion: 0.12
...
### Community 185 - "TaskSpec"
Cohesion: 0.04
Nodes (58): TaskResult, Run aider non-interactively via `--message` flag., TaskResult, TaskResult, TaskResult, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, TaskResult, AsyncClient (+50 more)
...
### Community 188 - "resolve_active_brain"
Cohesion: 0.04
Nodes (80): BrainResolution, provider_api_key(), Return the live API key for *provider* (env-only — never persisted)., get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache() (+72 more)

### Community 189 - "ScheduledJob"
Cohesion: 0.05
...
### Community 203 - "test_unit7_catalog_propagation.py"
Cohesion: 0.06
...
Cohesion: 0.04
Nodes (54): NotificationDispatcher, Any, Path, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess)., Run the bot with stop-event awareness., Routes background task results to configured notification channels. Currently… (+46 more)
...
### Community 218 - "kimi_bridge_provider_config"
Cohesion: 0.11
...
### Community 227 - "test_p0_roadmap_b1_c2_a3.py"
Cohesion: 0.06
...
### Community 230 - "CompanyGraph"
Cohesion: 0.04
Nodes (32): BusinessCategory, DirectChatSession, Direct chat session with Company Graph context binding., Bind a company to this chat session and load its Company Graph., Bind a repository to this chat session., Get the bound Company Graph., Get enriched context including Company Graph data., CompanyGraph (+24 more)
...
Cohesion: 0.06
Nodes (47): _get_provider_policy(), get_provider_policy_route(), _in_container(), ProviderPolicyUpdate, Path, Read the durable provider policy from DB, falling back to a safe default.…, Editable subset of the durable provider policy (paid-provider kill switch)., Persist the durable provider policy and return the new state. Also writes to… (+39 more)
...
### Community 242 - "_build_coordinator"
Cohesion: 0.09
...
### Community 245 - "Artifact"
Cohesion: 0.07
Nodes (37): AgentRole, When coder == reviewer model, swarm should log a warning., tests/test_workflow_models.py — Unit tests for workflow/models.py., TestApprovalGate, TestCheckRun, TestSlice, workflow/artifact_store.py — Durable artifact persistence. Artifacts are stored…, _extract_slices_from_plan() (+29 more)
...
### Community 285 - "DirectChatDoctor"
Cohesion: 0.08
...
Cohesion: 0.04
Nodes (46): get_tracker(), pace(), _parse_reset_epoch(), _ProviderQuota, Response, RateLimitTracker, Proactive rate-limit throttling for LLM providers — two complementary layers.…, Parse x-ratelimit-* headers and update per-provider quota state. Safe to call… (+38 more)

### Community 288 - "TaskDispatcher"
Cohesion: 0.08
...
### Community 293 - "OnboardingScreen.jsx"
Cohesion: 0.07
...
### Community 302 - "test_direct_chat_evolution.py"
Cohesion: 0.12
...
Cohesion: 0.04
Nodes (60): provider_max_parallel(), provider_max_rpm(), provider_max_tpm(), Return the operator-configured requests/min cap for *provider*, or None if…, Return the operator-configured tokens/min cap for *provider*. Reads…, Return the operator-configured in-flight request cap for *provider*. Reads…, Return the configured traffic-distribution strategy (lower-cased). Reads…, routing_strategy() (+52 more)
...
### Community 350 - "provider_base_url"
Cohesion: 0.09
...
### Community 351 - "tests/conftest.py"
Cohesion: 0.07
...
### Community 375 - "disabled_providers"
Cohesion: 0.06
Nodes (26): auto_disable(), _billing_signals(), disabled_provider_ids(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable., Persist a provider as disabled, through the store that already owns it., The one list of "you have no money" phrases, borrowed not copied. Lazy because…, disabled_providers() (+18 more)
...
### Community 406 - "schedules/api.py"
Cohesion: 0.13
...
### Community 423 - "test_task_source_id_race.py"
Cohesion: 0.13
...
### Community 478 - "ClaudeCodeAdapter"
Cohesion: 0.18
...
### Community 483 - "test_brain_availability_doctor.py"
Cohesion: 0.13
...
### Community 484 - "TestBrainFailoverModelUpdates"
Cohesion: 0.18
...
### Community 513 - "session_retro.py"
Cohesion: 0.13
...
### Community 523 - "CompanyAgencyService"
Cohesion: 0.09
...
### Community 545 - "cowork_session.py"
Cohesion: 0.15
...
### Community 567 - "test_task_service_failed_comment.py"
Cohesion: 0.18
...
### Community 586 - "test_agent_chat_integration.py"
Cohesion: 0.23
...
### Community 596 - "open_phase_report"
Cohesion: 0.10
...
### Community 600 - "test_nvidia_default_model.py"
Cohesion: 0.17
...
### Community 615 - "test_version_consistency.py"
Cohesion: 0.18
...
### Community 620 - "TestStreamableHTTPTransport"
Cohesion: 0.13
...
### Community 622 - "router/health.py"
Cohesion: 0.20
...
### Community 625 - "operational_incidents.py"
Cohesion: 0.12
...
### Community 628 - "test_daily_automation_2026_07_09.py"
Cohesion: 0.16
...
### Community 633 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 635 - "test_ai_insights.py"
Cohesion: 0.20
...
### Community 658 - "test_phase4_runtime_resilience.py"
Cohesion: 0.15
...
### Community 661 - "OrchestratorSupervisor"
Cohesion: 0.11
...
### Community 664 - "test_issue_triage.py"
Cohesion: 0.17
...
### Community 668 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 688 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
### Community 704 - "test_setup_api.py"
Cohesion: 0.23
...
### Community 707 - "TestModelCostTableUpdates"
Cohesion: 0.26
...
### Community 710 - "test_provider_state_durability.py"
Cohesion: 0.05
Nodes (25): True when operator state is stored somewhere that survives a redeploy. False…, Drop the cached Mongo client, backoff, and read caches (tests)., reset_kv_state(), state_is_durable(), one_configured_provider(), A registry with exactly two known providers and isolated state., fake_mongo(), _FakeCollection (+17 more)
...
### Community 713 - "test_task_brain_preflight.py"
Cohesion: 0.31
...
### Community 784 - "TestModelsEndpointAliases"
Cohesion: 0.16
...
### Community 792 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.22
...
### Community 793 - "test_agent_tools.py"
Cohesion: 0.19
...
### Community 815 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 817 - "test_task_pre_execution_gate.py"
Cohesion: 0.24
...
### Community 825 - "DailyDigestAggregatorTests"
Cohesion: 0.19
...
### Community 826 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
### Community 827 - "test_telegram_task_callback.py"
Cohesion: 0.12
...
### Community 857 - "get_tool_registry"
Cohesion: 0.18
...
### Community 861 - "analyze_quantitative"
Cohesion: 0.22
...
### Community 862 - "test_autonomy_pipeline_regressions.py"
Cohesion: 0.15
...
### Community 867 - "TestMCPToolsListCache"
Cohesion: 0.19
...
### Community 876 - "_parse_tool_calls_from_response"
Cohesion: 0.39
...
### Community 889 - "TestTheGatewayRegistryOffersOnlyLiveModels"
Cohesion: 0.22
...
### Community 941 - "_is_ephemeral_user"
Cohesion: 0.27
...
### Community 946 - "test_runtimes_health_endpoint.py"
Cohesion: 0.25
...
### Community 996 - "test_doctor_coding_brain.py"
Cohesion: 0.38
...
### Community 1026 - "_enforce_rate_limit"
Cohesion: 0.22
...
### Community 1100 - "ENGINEERING_STANDARDS.md — Patterns & Reference"
Cohesion: 0.22
...
### Community 1105 - "service_token.py"
Cohesion: 0.25
...
### Community 1113 - "_start_ceo_agency"
Cohesion: 0.31
...
### Community 1141 - "DockerAgentAdapter"
Cohesion: 0.23
...
### Community 1142 - "Continual Harness (`agent/harness_spec.py`)"
Cohesion: 0.25
...
### Community 1248 - "._run_phase_with_timeout"
Cohesion: 0.25
...
### Community 1250 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 1251 - "_load_local_metrics_since"
Cohesion: 0.43
...
### Community 1252 - "Workspace Issues"
Cohesion: 0.29
...
### Community 1253 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 1257 - "_brain_provider_status"
Cohesion: 0.33
...
### Community 1258 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 1260 - "TestDirectChatAgentExecution"
Cohesion: 0.33
...
### Community 1261 - "TestWorkflowIntegration"
Cohesion: 0.33
...
### Community 1262 - "TestCEOAgencySystem"
Cohesion: 0.33
...
### Community 1265 - "Agent API Issues"
Cohesion: 0.40
...
### Community 1266 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 1268 - "parse_pytest_failures.py"
Cohesion: 0.50
...
### Community 1271 - "verify_pr_exists"
Cohesion: 0.50
...
### Community 1276 - "Claude Code Specific Issues"
Cohesion: 0.50
...
- **3463 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3458 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13238 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **116 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `_Recorder`, `session_retro.py`, `test_llm_router_queue_cache.py`, `ImprovementLoop`, `TaskWorkflowService`, `test_llm_router_strategies.py`, `OrchestratorCheckpointStore`, `SelfHealingAgent`, `test_company_api.py`, `test_governance_enforcement.py`, `loop.py`, `load_config`, `test_telegram_webhook.py`, `test_unit8_model_catalog.py`, `MCPClient`, `services/seo_audit.py`, `test_failover_client_shared.py`, `cost_tracker.py`, `test_model_router.py`, `CEOLedger`, `AgentSwarm`, `PrimeAgentAdapter`, `BrainWatchdog`, `test_openclaw_endpoints.py`, `WorkflowRun`, `test_task_service_failed_comment.py`, `AgentJobManager`, `test_knowledge_sync.py`, `test_agent_tool_governance.py`, `test_sam_livekit.py`, `test_ceo_router.py`, `ChatHistoryStore`, `_resolve_brain_provider`, `SQLiteStore`, `WebsiteScanner`, `test_telegram_auto_approve.py`, `ResearchTask`, `get_task_store`, `test_autonomy_status.py`, `validate_policy_text`, `test_local_brain_state.py`, `TestClient`, `_start_ceo_agency`, `test_phase5_doctor.py`, `test_procedural_memory.py`, `test_refresh_agent_built_proof.py`, `OutputFilter`, `llm/router.py`, `test_startup_warmup.py`, `get_self_healing_agent`, `test_sam_voice.py`, `ProviderRouter`, `e2e/test_browser.py`, `test_daily_automation_2026_07_09.py`, `models/seo_audit.py`, `test_context_rulebook.py`, `ArtifactStore`, `test_issue_intake.py`, `run_task`, `UserRole`, `test_brain_failover.py`, `test_kimi_bridge_server.py`, `.record`, `_step`, `clear_cooldowns`, `test_trend_watcher.py`, `test_openclaw_gateway.py`, `TestHarnessAdapter`, `_FakeSandbox`, `claim`, `persist_plan_spec`, `KeyPool`, `test_new_features_e2e.py`, `KeyStore`, `OllamaCircuitBreaker`, `_job_text`, `_Budget`, `ScheduledJob`, `TestAgentRunnerSafety`, `test_features_api.py`, `test_video_transcript.py`, `test_provider_state_durability.py`, `TrendWatcher`, `test_task_brain_preflight.py`, `test_process_quick_note_workflow.py`, `test_portfolio_intake.py`, `NotificationDispatcher`, `platform_controls_router.py`, `TestEstimateTokensForMessages`, `TestClient`, `TestTeamSummary`, `TestFeaturesAPI`, `test_agile_api.py`, `test_app_settings.py`, `test_task_clarification.py`, `BackgroundAgent`, `ContextPruner`, `_clean_phases`, `test_control_plane_api.py`, `test_daily_2026_07_27.py`, `nvidia_models.py`, `test_persistent_memory.py`, `SecurityScanner`, `api_keys_for`, `test_llm_router_e2e.py`, `test_platform_controls.py`, `DirectChatDoctor`, `test_rate_limiter.py`, `test_nvidia_model_discovery.py`, `capture_screens.py`, `test_direct_chat_evolution.py`, `_resolve_push_token`, `test_task_pre_execution_gate.py`, `test_catalogue_probe.py`, `TestNormalizeResponseFormat`, `analyze_page`, `test_telegram_task_callback.py`, `test_scheduler_hydration_bounded.py`, `OrchestratorQueue`, `test_purge_backlog.py`, `model_discovery.py`, `TrafficDirector`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_e2b_data_flow.py`, `test_skill_registry_boot_refresh.py`, `TestSwarmRoleRouting`, `tools.py`, `tests/conftest.py`, `model_router.py`, `isolated_telegram_config`, `test_dashboard_cache.py`, `disabled_providers`, `_undeclared`, `test_service_token.py`, `test_harness_spec.py`, `test_regression.py`, `test_agency_fix.py`, `test_webui_provider_priority.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_monitor_lib.py`, `test_v4_api.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `UsageEvent`, `test_telegram_diag_endpoint.py`, `ClaudeCodeAdapter`, `client`, `test_doctor_coding_brain.py`, `test_voice_pipeline.py`, `test_brain_patch_service_token.py`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `diagnostics.py`, `proxy.py`, `AdaptiveHalter`, `FreeBuffAgent`, `RuntimeHealth`, `test_ceo_dispatcher.py`, `test_brain_failover.py`, `TestAgentRunnerExecution`, `ReactScratchpad`, `E2BSandboxSession`, `test_empirical_verify.py`, `test_governance_sandbox.py`, `ContextManager`, `loop.py`, `agency.py`, `MCPClient`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `._dispatch_tool`, `WorkspaceTools`, `AgentSessionStore`, `_resolve_push_token`, `test_daily_automation_2026_07_11.py`, `WorkflowRun`, `AgentJobManager`, `TaskSpec`, `TokenBudget`, `test_agent_tool_governance.py`, `CEODispatcher`, `InternalAgentAdapter`, `test_governance_api.py`, `direct_chat.py`, `TestAgentRunnerSafety`, `Any`, `test_agent_chat_integration.py`, `LocalWorkspace`, `E2BAdapter`, `NotificationDispatcher`, `UserMemoryStore`, `TestAgentLoopMCPIntegration`, `GitHubTools`, `ProviderRouter`, `ContextPruner`, `test_daily_automation_2026_07_09.py`, `webui/router.py`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `implement_agent.py`, `backend/server.py`, `is_anthropic_base_url`, `test_all_providers_discovery.py`, `test_bedrock_live.py`, `clear_cooldowns`, `NvidiaProvider`, `get_db`, `TestRouterIntegration`, `TestLegacyRouterServerFallback`, `resolve_active_brain`, `TestLegacyRouterCacheTTL`, `test_anthropic_refusal_fallback.py`, `direct_chat.py`, `system_instruction`, `_payload`, `TrafficDirector`, `test_bedrock_provider.py`, `kimi_bridge_provider_config`, `test_colibri_provider.py`, `TestAnthropicPayloadStructuredOutput`, `failover_client.py`?**
