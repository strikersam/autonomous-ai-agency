...
## Corpus Check
- 1442 files · ~2,073,758 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28688 nodes · 53606 edges · 1189 communities (1079 shown, 110 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 3686 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ed23199e`
...
## Communities (1189 total, 110 thin omitted)

### Community 0 - "ExecutionRequest"
Cohesion: 0.03
Nodes (67): Return orchestrator queue depth, active runs, and supervisor state (#522)., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, #522 + #505: Reliability startup — schedule hydration, orchestrator restore,…, _scheduler_on_fire(), _startup_reliability_hooks(), workflow_orchestrator_status(), get_orchestrator_supervisor(), OrchestratorSupervisor (+59 more)

### Community 1 - "test_agent_runner.py"
Cohesion: 0.06
...
Cohesion: 0.02
Nodes (155): int, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity() (+147 more)
...
Cohesion: 0.08
Nodes (28): _dead_model_key(), _exponential_backoff_cooldown(), _mark_model_dead(), _notify_watchdog(), _ollama_reasoning_effort(), provider_router.py — auto-generated module docstring (user-research skill scan)., Try all models for one provider. Returns ProviderResult on success, None on…, Notify the brain watchdog of a provider call outcome (fire-and-forget).… (+20 more)

### Community 5 - "services/background.py"
Cohesion: 0.05
Nodes (51): _await_hermes_ready(), BackgroundServices, _env_on(), Any, Path, services/background.py — shared background-service startup. Both the FastAPI…, Await the refresh and log the outcome; swallow every failure., Block until uvicorn is genuinely accepting connections. Without this the… (+43 more)

### Community 6 - "HTTPException"
Cohesion: 0.03
Nodes (117): _get_current_user_thunk(), _get_optional_user_thunk(), Request, admin_seed(), clear_cost_attribution(), clear_response_cache(), create_access_token(), create_api_key() (+109 more)

### Community 7 - "TaskStore"
Cohesion: 0.03
Nodes (112): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, tasks/models.py — Pydantic models for the task/issue system., Comment or reply on a task., TaskComment (+104 more)

### Community 8 - "brain_config.py"
Cohesion: 0.03
Nodes (111): _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache() (+103 more)

### Community 9 - "WebsiteScanner"
Cohesion: 0.02
Nodes (114): DetectedSystem, Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., A business system detected on a company's website or in their stack., Get the most confident evidence description., Inferred technology stack from website/repo analysis., StackInference (+106 more)

### Community 10 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (122): _diagnose_and_file(), _format_incident(), gather_render_evidence(), _iso_from_monotonic(), normalise(), note_phase_end(), note_phase_start(), _now() (+114 more)
...
Cohesion: 0.05
Nodes (69): Normalise a configured ``kind`` to an adapter key., resolve_kind(), LLMRequest, Cheap character-based token estimate (~4 chars/token). Deliberately dependency-…, A provider-neutral chat request. ``model`` is a *hint*: the router may…, _anthropic_provider(), Provider adapters: payload translation, response parsing, streaming. Tool calls…, request.extra must not be able to defeat the enforced temperature=1. (+61 more)
...
### Community 18 - "runtimes/manager.py"
Cohesion: 0.03
Nodes (43): Any, RoutingDecision, TaskResult, TaskSpec, runtimes/manager.py — RuntimeManager., Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service… (+35 more)
...
### Community 21 - "test_scheduler_hydration_bounded.py"
Cohesion: 0.09
...
Cohesion: 0.03
Nodes (76): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, packages/llm/health.py — provider health tracking and circuit breaking. Two…, Test hook — clear all health state., reset() (+68 more)

### Community 23 - "test_ceo_micromanager.py"
Cohesion: 0.04
Nodes (117): _decompose_into_subtasks(), Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, Build a single-task sub-task list (low-complexity fast path)., Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, Run one sub-task, judge the result, and escalate it if it is slop. The loop is… (+109 more)
...
### Community 25 - "Task"
Cohesion: 0.04
Nodes (70): Full task/issue document., Update the updated_at timestamp., Task, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, asyncio, A reject + human Retry starts a NEW approval cycle → fresh notification., A gated task parked while IN_PROGRESS must not be treated as stranded. (+62 more)
...
Cohesion: 0.03
Nodes (118): _complexity_rank(), _should_fan_out(), BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput (+110 more)

### Community 29 - "PolicyEngine"
Cohesion: 0.04
Nodes (88): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent., governance_enabled(), True when the governance layer should run at all. A single global off switch…, _action_matches() (+80 more)
...
### Community 33 - "audit"
Cohesion: 0.03
Nodes (60): compute_savings(), compute_time_series(), get_savings(), get_usage(), get_user_savings(), _period_start(), Any, BaseModel (+52 more)
...
Cohesion: 0.07
Nodes (36): CEODispatcher, CEOResult, _merge_changed_files(), _offload(), Any, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Aggregated output from a multi-specialist execution., Real CEO delegation: wake runtimes, fan out, and merge results. (+28 more)
...
Cohesion: 0.05
Nodes (40): A single occurrence of a check firing on a specific URL., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., SeoIssueInstance, SeoIssueReportRow, SeoSiteFindings, compute_pressure(), _count_syllables() (+32 more)

### Community 41 - "autonomous_fix.py"
Cohesion: 0.25
...
Cohesion: 0.04
Nodes (84): DetectedIssue, get_improvement_loop(), ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity, _now(), Any (+76 more)
...
Cohesion: 0.04
Nodes (71): ApprovalRequest, ApprovalStore, Any, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`…, Register a pending approval and return it. Arguments are scrubbed on the way…, Drop the oldest resolved requests once over capacity. Resolved requests go…, Approve or deny a pending request. Returns ``None`` for an unknown id. A… (+63 more)
...
Cohesion: 0.03
Nodes (162): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+154 more)
...
Cohesion: 0.08
Nodes (53): all_provider_ids(), _provider_ids_from_literal(), Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview() (+45 more)
...
Cohesion: 0.04
Nodes (29): AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., OpenCodeAdapter, Any, TaskResult, TaskSpec, Execute via OpenCode CLI: `opencode run --json <instruction>`. (+21 more)
...
Cohesion: 0.02
Nodes (108): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+100 more)
...
### Community 88 - "test_webui_provider_priority.py"
Cohesion: 0.11
...
### Community 91 - "CompanyGraphService"
Cohesion: 0.03
Nodes (39): BusinessCategory, Find a website by its URL., A company website with detected systems and stack inference., Website, CompanyGraphService, Company, SpecialistFamily, SystemType (+31 more)
...
### Community 95 - "test_llm_router_tpm.py"
Cohesion: 0.06
...
### Community 119 - "TestRuntimeControl"
Cohesion: 0.04
...
### Community 135 - "TestSchedulerStore"
Cohesion: 0.07
...
### Community 145 - "ProviderRouter"
Cohesion: 0.09
Nodes (23): ProviderRouter, Priority-ordered LLM provider fallback with health checks and retries., main(), probe_one(), ProviderRouter, _bedrock_api_response(), _bedrock_provider(), _mock_boto3() (+15 more)

### Community 146 - "resolve_component_model"
Cohesion: 0.06
...
Cohesion: 0.08
Nodes (43): build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), get_approval_store(), Replace the process-wide gate. Tests only. (+35 more)

### Community 148 - "failover_client.py"
Cohesion: 0.06
Nodes (44): _auto_disable(), _build_request(), _describe_registry(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion() (+36 more)
...
Cohesion: 0.04
Nodes (50): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,… (+42 more)
...
Cohesion: 0.05
Nodes (61): ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), ExecutionApprovalRequest, ExecutionLogEntry, FollowUpRequest, Any (+53 more)
...
Cohesion: 0.10
Nodes (22): AgentRole, _fake_artifact(), workflow/artifact_store.py — Durable artifact persistence. Artifacts are stored…, workflow/engine.py — WorkflowEngine: CRISPY phase sequencer. The engine is the…, Reset the singleton (test helper)., reset_engine(), Artifact, A durable, phase-produced document (markdown or JSON). (+14 more)

### Community 178 - "LLMRouter"
Cohesion: 0.07
Nodes (25): Attempt, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any, AsyncClient, BaseException, One shared connection pool. Reconnects if it was closed. (+17 more)
...
Cohesion: 0.05
Nodes (59): authorizeGithubRepos(), createGithubPR(), createWikiPage(), deleteGithubToken(), deleteSource(), deleteWikiPage(), fmtErr(), getCompanyGraph() (+51 more)

### Community 206 - "_build_default_manager"
Cohesion: 0.05
Nodes (45): JCodeAdapter, Any, Path, TaskResult, TaskSpec, Write .jcode/mcp.json in the workspace, pointing at our proxy's MCP endpoint.…, Resolve the default executor model via the catalog (UNIT 7). Was hardcoded to…, Adapter for jcode — TIER 2 high-performance Rust coding agent. (+37 more)

### Community 207 - "get_task_store"
Cohesion: 0.06
Nodes (62): apply_from_db(), Load the stored overrides and apply them. Called once at startup., Reset the singleton (for tests)., reset_failover_manager(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog() (+54 more)

### Community 208 - "test_ceo_router.py"
Cohesion: 0.05
Nodes (47): build_ceo_router(), Any, APIRouter, Attempt, _backend(), CEOLedger, get_ceo_ledger(), GoalRecord (+39 more)
...
Cohesion: 0.10
Nodes (29): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Public Doctor endpoint — no authentication required. Returns system-level… (+21 more)
...
### Community 267 - "types.py"
Cohesion: 0.03
Nodes (107): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, Translate OpenAI-shaped messages into Anthropic's system/turn split. (+99 more)
...
Cohesion: 0.04
Nodes (26): cost_table(), Return the active per-model cost table (USD per million tokens). Useful for…, _build_cost_table(), clear_stats(), cost_for_tokens(), get_cost_table(), get_stats(), _load_env_overrides() (+18 more)
...
### Community 321 - "BrainFailoverManager"
Cohesion: 0.09
...
### Community 325 - "Slice"
Cohesion: 0.07
Nodes (16): WorkflowRun, tests/test_workflow_models.py — Unit tests for workflow/models.py., TestApprovalGate, TestCheckRun, TestSlice, TestWorkflowRun, _extract_slices_from_plan(), Extract slice definitions from a plan.md artifact. Looks for sections matching:… (+8 more)

### Community 326 - "WorkspaceManager"
Cohesion: 0.06
Nodes (46): test_git_ref_rejects_empty(), test_git_ref_rejects_flag_injection(), test_git_ref_rejects_shell_metacharacters(), test_git_ref_rejects_traversal(), test_git_ref_valid(), test_git_scheme_allows_ssh(), test_http_scheme_rejects_ssh(), test_https_public_host_allowed() (+38 more)

### Community 327 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 329 - "seo_api.py"
Cohesion: 0.05
Nodes (53): delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), list_seo_audits(), BaseModel, get, post, SEO / GEO / AIO Audit API Router Endpoints for the world-class SEO audit engine… (+45 more)
...
### Community 335 - "lifespan"
Cohesion: 0.09
Nodes (14): lifespan(), The task store the background services should use, wiring it if needed.…, Point the feature stores at the shared database connection. Deliberately…, _task_store_for_background(), _wire_feature_stores(), anyio, Web lifespan delegates to start_background_services when…, RUN_BACKGROUND_IN_WEB=false: lifespan starts but background services are NOT… (+6 more)
...
### Community 337 - "test_brain_priority_scanner.py"
Cohesion: 0.10
...
### Community 344 - "resolve_active_brain"
Cohesion: 0.03
...
### Community 366 - "CompanyAgencyService"
Cohesion: 0.11
...
### Community 367 - "Agent Runtime Setup"
Cohesion: 0.12
...
### Community 378 - "InternalAgentAdapter"
Cohesion: 0.04
Nodes (52): _best_cloud_primary_base(), InternalAgentAdapter, _nvidia_provider_chain(), ProviderConfig, TaskResult, TaskSpec, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:… (+44 more)
...
### Community 408 - "TestStreamableHTTPTransport"
Cohesion: 0.13
...
### Community 422 - "SchedulesPage.js"
Cohesion: 0.12
...
### Community 452 - "test_unit5_ui_provider_surface.py"
Cohesion: 0.10
...
### Community 472 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 515 - "register_user_research_tools"
Cohesion: 0.21
...
### Community 516 - "skill_registry.py"
Cohesion: 0.13
...
### Community 528 - "test_task_service_failed_comment.py"
Cohesion: 0.18
...
### Community 588 - "load_registry_sync"
Cohesion: 0.15
...
### Community 589 - "test_providers_live_e2e.py"
Cohesion: 0.27
...
### Community 632 - "test_daily_automation_2026_07_09.py"
Cohesion: 0.16
...
Cohesion: 0.08
Nodes (41): brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, auto_disable_provider(), brain_availability_summary(), describe_disabled_reason(), _disabled_from_mongo(), _disabled_from_sqlite(), disabled_providers() (+33 more)
...
### Community 637 - "TestMCPToolsListCache"
Cohesion: 0.19
...
### Community 639 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 678 - "TestPreCompiledPatterns"
Cohesion: 0.22
...
### Community 738 - "_redact_for_notification"
Cohesion: 0.24
...
### Community 778 - "verify_service_token"
Cohesion: 0.22
...
### Community 779 - "TestMCPClientStructuredOutput"
Cohesion: 0.31
...
### Community 781 - "model_catalog.py"
Cohesion: 0.21
...
### Community 808 - "TestAuthAndTaskOwnership"
Cohesion: 0.17
...
### Community 809 - "ProviderConfig"
Cohesion: 0.07
Nodes (38): _get_director(), is_commercial_provider(), _normalized_provider_type(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Any (+30 more)
...
Cohesion: 0.09
Nodes (28): get_agency(), set_agency(), _autonomy_bg_cycle(), autonomy_status(), autonomy_tick(), Background CEO cycle + task dispatch. Runs fire-and-forget., Public autonomy readiness probe — no authentication required. A live deploy…, Execute ONE pending task synchronously. Called by the cron workflow every 2… (+20 more)
...
### Community 813 - "test_task_store_fails_loud_in_production.py"
Cohesion: 0.25
...
### Community 818 - "_extract_pytest_failure"
Cohesion: 0.31
...
### Community 820 - "response_cache.py"
Cohesion: 0.20
...
Cohesion: 0.02
Nodes (169): get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, _agent_provider_failure_response(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate (+161 more)
...
### Community 882 - "has_image_content"
Cohesion: 0.31
...
### Community 884 - "TestAgentRunnerSafety"
Cohesion: 0.24
...
### Community 899 - "agent/output_filter.py"
Cohesion: 0.32
...
### Community 903 - "_brain_provider_status"
Cohesion: 0.25
...
### Community 937 - "test_doctor_coding_brain.py"
Cohesion: 0.38
...
### Community 945 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 988 - "TestReasonsAreActionable"
Cohesion: 0.40
...
### Community 1029 - "test_seo_audit.py"
Cohesion: 0.06
Nodes (16): field_validator, Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, mock_report(), _mock_site_handler(), Request, Response, tests/test_seo_audit.py - SEO / GEO / AIO audit engine tests (issue #533).… (+8 more)
...
### Community 1034 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 1199 - "get"
Cohesion: 0.03
Nodes (74): auth_me(), auto_recommend_skills(), discover_remote_skills(), _get_github_token(), get_github_tree(), get_public_kpis(), get_skill(), _gh_headers() (+66 more)
...
- **Why does `_fixture()` connect `_fixture` to `ExecutionRequest`, `llm/router.py`, `test_provider_router.py`, `test_seo_audit.py`, `TaskStore`, `WebsiteScanner`, `test_operational_incidents.py`, `TaskWorkflowService`, `test_llm_router_strategies.py`, `test_task_service_failed_comment.py`, `SelfHealingAgent`, `test_openclaw_endpoints.py`, `test_llm_router_resilience.py`, `WorkflowRun`, `config.py`, `test_scheduler_hydration_bounded.py`, `AgentSwarm`, `Task`, `PolicyEngine`, `Agency`, `get_self_healing_agent`, `test_claude_code_adapter.py`, `test_failover_client_shared.py`, `test_unit8_model_catalog.py`, `ImprovementLoop`, `UserRole`, `test_agent_tool_governance.py`, `SQLiteStore`, `NotificationDispatcher`, `SchedulerStore`, `test_telegram_auto_approve.py`, `test_model_router.py`, `test_autonomy_status.py`, `PrimeAgentAdapter`, `test_local_brain_state.py`, `test_telegram_diag_endpoint.py`, `ChatHistoryStore`, `test_brain_availability_doctor.py`, `test_autonomous_agency_e2e.py`, `ResearchTask`, `e2e/test_browser.py`, `test_control_plane_api.py`, `test_procedural_memory.py`, `test_webui_provider_priority.py`, `claim`, `ArtifactStore`, `test_llm_router_tpm.py`, `test_startup_warmup.py`, `api_keys_for`, `KeyStore`, `TestHarnessAdapter`, `parse_event_stream`, `BrainWatchdog`, `test_trend_scoping.py`, `TestRuntimeControl`, `test_daily_automation_2026_07_09.py`, `test_context_rulebook.py`, `_StubProvider`, `run_task`, `test_openclaw_gateway.py`, `FreeBuffAgent`, `test_brain_failover_413.py`, `test_issue_intake.py`, `direct_chat.py`, `test_e2b_data_flow.py`, `test_new_features_e2e.py`, `SeoFixRequest`, `AgentJobManager`, `MCPClient`, `_Budget`, `test_provider_enable_disable.py`, `persist_plan_spec`, `clear_cooldowns`, `test_daily_2026_07_27.py`, `OllamaCircuitBreaker`, `sam`, `CEOSupervisor`, `emit_chat_observation`, `FeatureMaturity`, `test_sam_livekit.py`, `test_brain_failover.py`, `test_features_api.py`, `test_video_transcript.py`, `test_agile_api.py`, `test_app_settings.py`, `test_llm_router_disabled.py`, `test_task_clarification.py`, `TrendWatcher`, `test_trend_watcher.py`, `test_brain_patch_service_token.py`, `session_retro.py`, `test_portfolio_intake.py`, `TestEstimateTokensForMessages`, `TestSwarmRoleRouting`, `TestClient`, `ContextPruner`, `get_skill_registry_safe`, `OrchestratorQueue`, `test_sam_voice.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `OutputFilter`, `test_platform_controls.py`, `KeyPool`, `TestRecordUsageAndStats`, `TestAuthAndTaskOwnership`, `test_purge_backlog.py`, `_autonomy_bg_cycle`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `analyze_page`, `test_telegram_service_webhook.py`, `test_crispy_burn_in.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `clear_override`, `timedelta`, `test_rate_limiter.py`, `test_phase5_doctor.py`, `TestRouterIntegration`, `test_service_token.py`, `test_harness_spec.py`, `allow_paid`, `test_regression.py`, `test_agency_fix.py`, `test_telegram_mutating_commands.py`, `test_dashboard_cache.py`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `test_ai_insights.py`, `_resolve_push_token`, `test_knowledge_sync.py`, `test_doctor_coding_brain.py`, `client`, `test_ping.py`, `_FakeSandbox`, `test_voice_pipeline.py`, `provider_max_rpm`, `test_kimi_bridge_server.py`, `TestNormalizeResponseFormat`, `test_provider_state_durability.py`, `test_tasks_awaiting_approval_api.py`, `test_llm_router_compat.py`, `test_company_api.py`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `test_brain_failover_413.py`, `test_agent_runner.py`, `proxy.py`, `direct_chat.py`, `ReactScratchpad`, `ContextManager`, `TaskSpec`, `test_e2b_adapter.py`, `test_daily_automation_2026_07_11.py`, `test_governance_api.py`, `WorkflowRun`, `test_ceo_micromanager.py`, `AgentJobManager`, `MCPClient`, `test_ceo_dispatcher.py`, `AgentPlan`, `Agency`, `PolicyEngine`, `StuckDetector`, `test_failover_client_shared.py`, `TestAgentLoopMCPIntegration`, `CEODispatcher`, `MultiAgentSwarm`, `test_agent_tool_governance.py`, `WorkspaceTools`, `UserMemoryStore`, `backend/server.py`, `TokenBudget`, `test_autonomous_agency_e2e.py`, `E2BSandboxSession`, `LocalWorkspace`, `TestAgentRunnerExecution`, `GitHubTools`, `AgentSessionStore`, `test_agent_free_brain.py`, `test_empirical_verify.py`, `test_daily_automation_2026_07_09.py`, `ContextPruner`, `TestAgentRunnerSafety`, `test_agent_chat_integration.py`, `loop.py`, `InternalAgentAdapter`, `AdaptiveHalter`, `FreeBuffAgent`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `get_skill_bindings()` connect `get_skill_bindings` to `SpecEntry`, `skill_bindings.py`, `Specialist`, `AgentRunner`, `company_api.py`, `WorkflowRun`, `FeatureMaturity`, `loop.py`, `HarnessEnrichment`, `test_ceo_dispatcher.py`, `_execute_skill_impl`?**
