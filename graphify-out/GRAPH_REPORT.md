...
## Corpus Check
- 1440 files · ~2,085,595 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 29257 nodes · 54282 edges · 1254 communities (1137 shown, 117 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2671 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7d142413`
...
Cohesion: 0.04
Nodes (55): requires_db, MongoDBStore, Any, Company, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., List companies with optional filtering. (+47 more)
...
Cohesion: 0.03
Nodes (85): int, Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key() (+77 more)
...
Cohesion: 0.05
Nodes (36): Attempt, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), Whether sleeping ``delay`` still leaves time for the attempt after it., Wall-clock and attempt ceiling shared by every attempt in one request. Without…, RetryBudget, LLMRouter, Any (+28 more)

### Community 9 - "TaskSpec"
Cohesion: 0.04
Nodes (75): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns… (+67 more)
...
Cohesion: 0.03
Nodes (123): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), _brain_provider_status(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore (+115 more)
...
Cohesion: 0.06
Nodes (47): events_from_identity(), Convenience: build and record an event in one call., Extract the identity-derived audit fields from an AgentIdentity. Kept here…, record_event(), BudgetExceeded, classify(), evaluate_call(), get_gate() (+39 more)
...
Cohesion: 0.04
Nodes (79): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent., governance_enabled(), True when the governance layer should run at all. A single global off switch… (+71 more)
...
Cohesion: 0.04
Nodes (57): get_cache(), The process-wide cache manager., failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Router-backed implementation of ``packages.ai.failover_client``'s entry point.…, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on… (+49 more)
...
Cohesion: 0.04
Nodes (48): TaskResult, TaskSpec, Execute via OpenCode CLI: `opencode run --json <instruction>`., _accumulate_usage(), _assistant_messages(), _child_env(), _iter_events(), _kill_and_reap() (+40 more)
...
Cohesion: 0.04
Nodes (56): brain_availability_summary(), Non-secret answer to "can the brain answer a request right now?". Three callers…, _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.… (+48 more)

### Community 29 - "HTTPException"
Cohesion: 0.03
Nodes (145): admin_seed(), auth_me(), auto_recommend_skills(), create_api_key(), create_github_pr(), create_provider(), create_wiki_page(), delete_api_key() (+137 more)
...
Cohesion: 0.02
Nodes (104): Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., Inferred technology stack from website/repo analysis., StackInference, WebsiteScanResult, main(), Returns (url, ok, summary). (+96 more)

### Community 32 - "AgentRunner"
Cohesion: 0.05
Nodes (45): AgentRunner, Any, Path, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a…, Send chat messages and return the assistant's text output. If an…, Parse an LLM tool-call response, tolerating common malformations. Handles: 1.… (+37 more)
...
Cohesion: 0.03
Nodes (92): Backoff policy for retryable failures., RetryConfig, packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC (+84 more)

### Community 36 - "test_agent_tool_governance.py"
Cohesion: 0.07
Nodes (49): Replace the process-wide engine. Tests only., reset_policy_engine(), _drive(), _enforce(), governance_on(), _observations(), Any, MonkeyPatch (+41 more)

### Community 37 - "test_unit8_model_catalog.py"
Cohesion: 0.04
Nodes (73): all_provider_ids(), _provider_ids_from_literal(), Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, CatalogActiveBrain, CatalogMirror, CatalogProviderEntry, get_catalog() (+65 more)
...
Cohesion: 0.03
Nodes (115): count, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, BreakerState, HealthTracker, _Outcome (+107 more)
...
Cohesion: 0.04
Nodes (63): build_ceo_router(), Any, APIRouter, BackgroundServices, Handle returned by ``start_background_services`` — call ``stop()`` on shutdown., Cancel the boot refresh if it is still fetching at shutdown., Shut the in-process Hermes down so port 8100 is released. Uvicorn's own…, CEOSupervisor (+55 more)

### Community 48 - "loop.py"
Cohesion: 0.12
Nodes (20): _check_extra_kwargs(), _enforce_signature(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, build_compaction_prompt(), build_execution_prompt() (+12 more)
...
Cohesion: 0.04
Nodes (79): e2b_status(), _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy(), PolicyUpdateBody (+71 more)

### Community 55 - "AgentJobManager"
Cohesion: 0.05
Nodes (37): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,… (+29 more)

### Community 56 - "test_agent_runner.py"
Cohesion: 0.05
...
Cohesion: 0.02
Nodes (120): Execute work through the 11-phase golden path., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_execute(), get_orchestrator_checkpoint_store(), stop_orchestrator_supervisor(), BoundContext, ClassifyOutput (+112 more)
...
Cohesion: 0.02
Nodes (124): ApprovalPolicy, BusinessSystem, Company, CompanyCreateRequest, CompanyGraph, CompanyGraphSnapshot, CompanyUpdateRequest, Connector (+116 more)

### Community 65 - "ProviderRouter"
Cohesion: 0.06
Nodes (31): build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, _get_director(), _normalized_provider_type(), ProviderAttempt, ProviderRouter, Any, Response (+23 more)
...
Cohesion: 0.05
Nodes (73): quick_notes_submit(), _QuickNoteBody, The task store the background services should use, wiring it if needed.…, Submit a quick-note URL or instruction from the dashboard FAB., _task_store_for_background(), Path, Start RuntimeManager, TaskDispatcher, SCHEDULER, and self-bootstrap. Returns a…, start_background_services() (+65 more)
...
Cohesion: 0.04
Nodes (74): export_seo_audit(), Run a full SEO/GEO/AIO audit against a website and persist the evidence., Export a stored audit. - ``csv`` aggregated findings, Screaming Frog…, run_seo_audit(), BaseModel, field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, A single occurrence of a check firing on a specific URL. (+66 more)

### Community 75 - "test_render_mcp.py"
Cohesion: 0.03
Nodes (55): agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, _as_list(), _coerce_payload(), Any, RuntimeError, packages/integrations/render_mcp.py — Render platform access over MCP. The…, Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return… (+47 more)

### Community 76 - "lifespan"
Cohesion: 0.05
Nodes (47): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled(), onboarding_gate_enabled_cached() (+39 more)
...
Cohesion: 0.14
Nodes (52): ApprovalRequest, BackgroundTasks, add_comment(), approve_checkpoint(), approve_execution(), clarify_task(), create_task(), _current_user() (+44 more)
...
### Community 115 - "claim"
Cohesion: 0.07
Nodes (30): _acquire_provider_probe(), Try to acquire a distributed probe lock for *provider_id*. Returns True if this…, claim(), cooldown_clear(), cooldown_get(), cooldown_set(), _get_backend(), incr_window() (+22 more)
...
Cohesion: 0.04
Nodes (47): Resolve the model id to force for a code-execution run, or ``None``. Returns…, resolve_coding_model_preference(), HermesAdapter, Any, AsyncClient, Response, TaskResult, TaskSpec (+39 more)
...
Cohesion: 0.05
Nodes (24): AutonomyCounter, AutonomySnapshot, AutonomyTracker, Any, agent/kpi.py — Autonomy KPIs: evidence capture and metrics tracking. Tracks key…, Return a point-in-time snapshot of all KPIs., Reset all counters (test helper)., Reset the singleton (test helper). (+16 more)

### Community 136 - "WorkflowEngine"
Cohesion: 0.08
Nodes (30): get_tracker(), Return the shared AutonomyTracker singleton (lazy-init)., Any, Connection, Path, PhaseType, WorkflowRun, CRISPY workflow engine — phase sequencer + gate controller. GATE: Golden Path… (+22 more)
...
Cohesion: 0.07
Nodes (20): _append_agent_session_message(), _in_container(), _mask_observations(), Path, ProviderConfig, Truncate tool/observation content in older messages to prevent context bloat., Return True when running inside a container runtime. ``/.dockerenv`` alone…, _run_agent_loop() (+12 more)

### Community 150 - "RuntimeCapabilityRegistry"
Cohesion: 0.05
Nodes (29): Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry, _governance_identity(), Any, TaskResult, TaskSpec (+21 more)
...
Cohesion: 0.06
Nodes (37): E2BAdapter, Any, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, Available iff config resolves AND the SDK is importable. Never raises — a…, _FakeAsyncSandboxClass, _FakeCommandResult, _FakeCommands (+29 more)
...
Cohesion: 0.07
Nodes (31): ApprovalRequest, ApprovalStatus, ApprovalStore, Any, Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`… (+23 more)
...
Cohesion: 0.05
Nodes (65): ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest, ExecutionLogEntry, FollowUpRequest, Any (+57 more)

### Community 177 - "company_api.py"
Cohesion: 0.03
Nodes (116): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint() (+108 more)
...
### Community 194 - "test_workflow_orchestrator_scoping.py"
Cohesion: 0.18
...
Cohesion: 0.04
Nodes (27): AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., OpenCodeAdapter, Any, Resolve the default executor model via the catalog (UNIT 7). Was hardcoded to…, Adapter for OpenCode — FIRST CLASS coding runtime., _resolve_default_executor_model() (+19 more)
...
Cohesion: 0.06
Nodes (36): _catalog_defaults(), _catalog_provider(), _nvidia_defaults(), Pick the catalog provider based on which API key is configured., Resolve per-role defaults via the catalog. Returns None on import error., Resolve the NVIDIA default model via the catalog (UNIT 7). Was hardcoded to…, _resolve_nvidia_default_model(), _catalog_preset() (+28 more)
...
Cohesion: 0.07
Nodes (37): _dead_model_key(), _exponential_backoff_cooldown(), get_dead_models(), _is_model_dead(), _mark_model_dead(), _notify_watchdog(), provider_router.py — auto-generated module docstring (user-research skill scan)., Snapshot of active dead-model entries {provider_id/model: expiry_ts}. Also… (+29 more)
...
### Community 232 - "brain_failover.py"
Cohesion: 0.05
Nodes (45): auto_disable_provider(), _disabled_from_mongo(), _disabled_from_sqlite(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _mongo_db(), _mongo_enabled() (+37 more)
...
Cohesion: 0.08
Nodes (34): create_access_token(), create_refresh_token(), google_callback(), hash_password(), login(), LoginBody, seed_admin(), verify_password() (+26 more)
...
Cohesion: 0.11
Nodes (29): autonomy_tick(), legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_list(), _produce_scheduler_jobs(), Execute ONE pending task synchronously. Called by the cron workflow every 2…, get_scheduler(), create_schedule() (+21 more)
...
Cohesion: 0.07
Nodes (30): AgentScheduler, Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Capture the FastAPI main event loop so APScheduler's background thread can…, _FakePersistence, tests/test_schedule_persistence.py — #505 schedules survive restart. Regression…, Populate the store directly so hydration tests don't depend on the timing of…, Regression for the production startup path: services/background.py runs inside…, The sync attach_persistence()/rehydrate() must stay safe even if called from… (+22 more)
...
### Community 333 - "CompanyAgencyService"
Cohesion: 0.09
...
Cohesion: 0.13
Nodes (30): Aggregated report row - Screaming Frog CSV compatible., Complete result of one audit run., SeoAuditReport, SeoIssueReportRow, Paragraph, _build_pdf(), Path, Render an executive-level PDF from the audit report dict. (+22 more)
...
Cohesion: 0.03
Nodes (112): allow_paid_brain(), BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), is_anthropic_model() (+104 more)
...
### Community 392 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
Cohesion: 0.10
Nodes (24): _cached(), cost_attribution_stats(), _fast_count(), get_activity(), _get_activity_impl(), get_stats(), _produce_stats(), Any (+16 more)
...
Cohesion: 0.03
Nodes (62): DetectedSystem, A business system detected on a company's website or in their stack., Get the most confident evidence description., Create a new company in SQLite., Get a company by ID from SQLite., Update a company in SQLite., Delete a company and all associated data from SQLite., List companies from SQLite. (+54 more)
...
Cohesion: 0.06
Nodes (46): autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, detect_company_id(), DirectChatSession, handle_chat_message_with_context(), Direct chat session with Company Graph context binding., Bind a company to this chat session and load its Company Graph., Bind a repository to this chat session. (+38 more)
...
### Community 483 - "test_llm_router_tpm.py"
Cohesion: 0.06
...
### Community 493 - ".on_task_complete"
Cohesion: 0.18
...
### Community 499 - "test_telegram_task_callback.py"
Cohesion: 0.13
...
### Community 514 - "self_heal_brain_and_unblock_tasks"
Cohesion: 0.09
...
Cohesion: 0.04
Nodes (43): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), AuditEvent, AuditLog, Any, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., One governed action, fully described. Field order follows the… (+35 more)
...
### Community 537 - "note_phase_start"
Cohesion: 0.12
...
Cohesion: 0.02
Nodes (111): parse_event_stream(), Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.… (+103 more)

### Community 545 - "ProviderHealth"
Cohesion: 0.35
...
### Community 568 - "test_pr_approval_gate.py"
Cohesion: 0.16
Nodes (5): _pr(), tests/test_pr_approval_gate.py — the green-PR Telegram approval sweep. Covers…, TestPrIsGreen, TestRunSweep, TestSelectNotifiable
...
### Community 605 - "test_tasks_reconciler_todo_requeue.py"
Cohesion: 0.18
...
Cohesion: 0.03
Nodes (49): BusinessCategory, Find a website by its URL., Find a repository by its URL., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo (+41 more)
...
### Community 648 - "OperationalIncidentTracker"
Cohesion: 0.12
...
### Community 653 - "reap_expired_companies"
Cohesion: 0.14
...
### Community 690 - "WorkflowRun"
Cohesion: 0.10
Nodes (14): _make_engine(), Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestWorkspaceIsolation, Phase, Any, PhaseType (+6 more)
...
### Community 704 - "OrchestratorSupervisor"
Cohesion: 0.26
...
### Community 726 - "TestCompanyGraphModels"
Cohesion: 0.17
...
### Community 761 - "test_orchestrator_merge_decision.py"
Cohesion: 0.24
...
### Community 798 - "agent/output_filter.py"
Cohesion: 0.33
...
### Community 827 - "_is_admin"
Cohesion: 0.05
Nodes (44): backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, _actor(), ControlUpdateBody, BaseModel (+36 more)
...
### Community 829 - "tasks/store.py"
Cohesion: 0.07
Nodes (31): tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, TaskPriority, TaskStatus, _priority_rank(), tasks/store.py — MongoDB-backed task store. Falls back to in-memory storage…, Numeric rank for a task priority (higher = more urgent). Accepts an enum, its… (+23 more)
...
### Community 835 - "test_autonomous_agency_e2e.py"
Cohesion: 0.10
Nodes (13): End-to-end tests for the autonomous AI agency system (issue #467). These tests…, Verify AgentRunner has _execute_step for ReAct execution loop., Verify _BYPASS context var is used for internal agent execution., Tests for direct chat agent execution beyond planning., Verify ChatSendRequest supports agent mode execution., Verify WorkspaceTools provides filesystem operations for agents., Tests for workflow orchestrator integration., Verify WorkflowOrchestrator correctly bypasses for internal callers. (+5 more)
...
### Community 998 - "_status_snapshot"
Cohesion: 0.31
...
Cohesion: 0.02
Nodes (152): _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, BrainTestRequest, _build_agent_status_snapshot() (+144 more)
...
Cohesion: 0.06
Nodes (42): AgentRole, When coder == reviewer model, swarm should log a warning., _fake_artifact(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., TestPhaseSequenceError, tmp_db(), tests/test_workflow_models.py — Unit tests for workflow/models.py. (+34 more)
...
### Community 1219 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 1222 - "TestCEOAgencySystem"
Cohesion: 0.33
...
### Community 1223 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 1233 - "test_anthropic_failover_endpoint.py"
Cohesion: 0.15
...
### Community 1237 - "Privacy, security, and cost"
Cohesion: 0.50
...
### Community 1249 - "_clean_director"
Cohesion: 0.50
...
### Community 1251 - "Configuration reference"
Cohesion: 0.67
...
- **Why does `_fixture()` connect `_fixture` to `test_ai_insights.py`, `ProviderConfig`, `services/background.py`, `test_llm_router_queue_cache.py`, `TaskStore`, `test_company_api.py`, `brain_config.py`, `AuditLog`, `enforcement.py`, `PolicyEngine`, `SelfHealingAgent`, `gateway.py`, `Agency`, `PrimeAgentAdapter`, `test_provider_state_durability.py`, `test_brain_availability_doctor.py`, `clear_cooldowns`, `WebsiteScanner`, `test_monitor_lib.py`, `test_agent_tool_governance.py`, `test_openclaw_endpoints.py`, `test_unit8_model_catalog.py`, `Task`, `ImprovementLoop`, `test_model_router.py`, `test_llm_router_resilience.py`, `Artifact`, `test_failover_client_shared.py`, `test_ceo_router.py`, `test_process_quick_note_workflow.py`, `AgentJobManager`, `ExecutionRequest`, `SQLiteStore`, `ChatHistoryStore`, `E2BSandboxSession`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `test_local_brain_state.py`, `NotificationDispatcher`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `services/seo_audit.py`, `test_telegram_diag_endpoint.py`, `config.py`, `ResearchTask`, `TestClient`, `resolve_e2b_config`, `test_procedural_memory.py`, `test_tasks_reconciler_todo_requeue.py`, `e2e/test_browser.py`, `TokenBudget`, `UserRole`, `_provider`, `test_knowledge_sync.py`, `SchedulerStore`, `test_startup_warmup.py`, `SeoFixer`, `KeyStore`, `claim`, `model_discovery.py`, `test_context_rulebook.py`, `tests/conftest.py`, `ArtifactStore`, `HermesAdapter`, `test_webui_provider_priority.py`, `test_issue_intake.py`, `TestMongoGate`, `OperationalIncidentTracker`, `test_openclaw_gateway.py`, `test_response_cache.py`, `test_kimi_bridge_server.py`, `reap_expired_companies`, `BackgroundAgent`, `test_telegram_freebuff.py`, `TestHarnessAdapter`, `test_e2b_data_flow.py`, `MCPClient`, `test_new_features_e2e.py`, `get_feature_matrix`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `test_e2b_adapter.py`, `_Budget`, `KeyPool`, `ApprovalStore`, `OllamaCircuitBreaker`, `analyze_page`, `_resolve_user_github_token`, `test_llm_router_disabled.py`, `sam`, `test_features_api.py`, `test_video_transcript.py`, `TrendWatcher`, `_clean_phases`, `test_agile_api.py`, `test_app_settings.py`, `test_agent_scripts_share_one_model_list.py`, `test_portfolio_intake.py`, `OrchestratorCheckpointStore`, `test_trend_watcher.py`, `test_nvidia_model_discovery.py`, `api_keys_for`, `ai/router.py`, `_clean_director`, `TestClient`, `brain_failover.py`, `ContextPruner`, `BrainWatchdog`, `test_llm_router_e2e.py`, `AgentProfile`, `test_orchestrator_merge_decision.py`, `test_telegram_service_webhook.py`, `test_daily_2026_07_27.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `OutputFilter`, `test_platform_controls.py`, `_resolve_push_token`, `test_scheduler_hydration_bounded.py`, `get_skill_bindings`, `model_router.py`, `test_rate_limiter.py`, `test_mcp_governance.py`, `test_brain_failover.py`, `test_sam_voice.py`, `OrchestratorQueue`, `test_purge_backlog.py`, `_is_admin`, `tasks/store.py`, `clear_override`, `test_direct_chat_evolution.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `test_control_plane_api.py`, `resolve_active_brain`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_brain_patch_service_token.py`, `test_harness_spec.py`, `test_ping.py`, `capture_screens.py`, `test_dashboard_cache.py`, `AgentSwarm`, `test_regression.py`, `test_agency_fix.py`, `KeyRing`, `test_v4_api.py`, `test_freebuff_bot.py`, `client`, `test_voice_pipeline.py`, `TestAgentRunnerSafety`, `_job_text`, `test_llm_router_tpm.py`, `test_agent_free_brain.py`, `TestNormalizeResponseFormat`, `test_task_store_fails_loud_in_production.py`, `test_runtimes_health_endpoint.py`, `test_sam_livekit.py`, `test_telegram_task_callback.py`?**
  _High betweenness centrality (0.122) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `proxy.py`, `TaskSpec`, `ReactScratchpad`, `PolicyEngine`, `CEODispatcher`, `test_backend_server_features.py`, `ContextManager`, `Agency`, `MCPClient`, `test_e2b_adapter.py`, `test_agent_tool_governance.py`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `StuckDetector`, `test_agent_chat_integration.py`, `test_daily_automation_2026_07_11.py`, `loop.py`, `WorkspaceTools`, `test_governance_api.py`, `AgentJobManager`, `test_agent_runner.py`, `ExecutionRequest`, `E2BSandboxSession`, `test_empirical_verify.py`, `UserMemoryStore`, `test_autonomous_agency_e2e.py`, `AgentPlan`, `TestAgentRunnerSafety`, `TokenBudget`, `test_agent_free_brain.py`, `backend/server.py`, `InternalAgentAdapter`, `brain_failover.py`, `GitHubTools`, `AdaptiveHalter`, `ContextPruner`, `LocalWorkspace`, `direct_chat.py`, `diagnostics.py`, `AgentSessionStore`, `FreeBuffAgent`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `TaskStore` connect `TaskStore` to `services/background.py`, `test_issue_intake.py`, `brain_config.py`, `test_trend_scoping.py`, `Agency`, `test_tasks_awaiting_approval_api.py`, `_fixture`, `Task`, `tasks/models.py`, `_ts_to_float`, `TaskDispatcher`, `tasks/store.py`, `get_task_store`, `test_phase4_runtime_resilience.py`, `test_portfolio_intake.py`, `WorkflowPhase`, `test_tasks_reconciler_todo_requeue.py`, `tasks/api.py`, `backend/server.py`, `test_sqlite_store.py`, `test_telegram_task_callback.py`, `test_task_source_id_race.py`?**
