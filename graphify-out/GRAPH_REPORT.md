...
## Corpus Check
- 1523 files · ~2,209,942 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30946 nodes · 57778 edges · 1279 communities (1117 shown, 114 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 3356 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `25bdd0d7`
...
Cohesion: 0.02
Nodes (142): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, AnthropicProvider, Any, AsyncClient, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs… (+134 more)
...
Cohesion: 0.02
Nodes (92): Backoff policy for retryable failures., RetryConfig, BreakerState, _Outcome, ProviderHealth, Any, Enum, str (+84 more)
...
Cohesion: 0.03
Nodes (78): classify_domain(), agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Return the best-matching domain for a task title+description., quick_notes_submit(), _QuickNoteBody, The task store the background services should use, wiring it if needed.…, Point the feature stores at the shared database connection. Deliberately…, Submit a quick-note URL or instruction from the dashboard FAB. (+70 more)

### Community 11 - "test_llm_router_strategies.py"
Cohesion: 0.04
Nodes (98): count, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, HealthTracker, Health and circuit-breaker state for every provider., Whether the breaker will admit a request right now. Transitions OPEN →… (+90 more)

### Community 12 - "brain_config.py"
Cohesion: 0.03
Nodes (129): autonomy_status(), _brain_provider_status(), get_brain_policy_route(), patch_brain_policy_route(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., Return the active brain config + per-provider key-present flags. The response…, Apply a partial brain config update. Hard constraint: every changed model id is… (+121 more)

### Community 13 - "CompanyGraphStore"
Cohesion: 0.03
Nodes (38): Company, The core company entity - root of the Company Graph., requires_db, CompanyGraphStore, Delete a company and all its associated data., List companies with optional filtering., Count total companies in the store., Create a new company graph. (+30 more)
...
Cohesion: 0.02
Nodes (109): _format_incident(), gather_render_evidence(), _iso_from_monotonic(), normalise(), note_phase_end(), note_phase_start(), _now(), open_phase_report() (+101 more)

### Community 16 - "PolicyEngine"
Cohesion: 0.03
Nodes (90): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, AgentIdentity, _coerce_text(), new_session_id(), Any, packages/governance/identity.py — who is acting, and under what mandate. Docker… (+82 more)

### Community 17 - "backend/server.py"
Cohesion: 0.01
Nodes (323): admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, auth_me(), AuthorizeReposBody (+315 more)

### Community 18 - "_fixture"
Cohesion: 0.02
Nodes (103): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., setup_database_moks() (+95 more)
...
### Community 20 - "Agency"
Cohesion: 0.04
Nodes (108): Agency, AgentRole, get_agency(), _parse_ceo_directives(), str, CEO-coordinated multi-agent agency for continuous codebase management. The CEO…, set_agency(), DetectedIssue (+100 more)
...
Cohesion: 0.04
Nodes (64): PreflightIssue, PreflightReport, BaseModel, AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any (+56 more)
...
### Community 25 - "models/company_graph.py"
Cohesion: 0.03
Nodes (127): ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraph, CompanyGraphResponse, CompanyGraphSnapshot, CompanyResponse, CompanyUpdateRequest (+119 more)
...
### Community 27 - "agency.py"
Cohesion: 0.08
Nodes (22): AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _company_advisory_context(), _now_str() (+14 more)

### Community 28 - "direct_chat.py"
Cohesion: 0.06
Nodes (60): DirectChatDoctor, Any, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob, AgentJobEnvelope, CompletedJob (+52 more)

### Community 29 - "E2BSandboxSession"
Cohesion: 0.04
Nodes (80): MCPUnavailableError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Raised when the MCP server is unreachable or the circuit is open., packages/integrations/render_mcp.py — Render platform access over MCP. The…, Drop the cached singleton. Used by tests and after a config change., reset_render_mcp(), E2BConfig, Resolved E2B sandbox configuration. Never logged in full. Attributes: api_key:… (+72 more)
...
Cohesion: 0.04
Nodes (81): AgentDefinition, AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+73 more)

### Community 34 - "test_failover_client_shared.py"
Cohesion: 0.05
Nodes (94): BrainFailoverExhausted, failover_chat_completion(), FailoverResult, _log_recovery(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, A successful completion plus the accounting its callers need., Report a successful failover at INFO — recovery is not an incident. Everything… (+86 more)

### Community 35 - "failover_client.py"
Cohesion: 0.04
Nodes (72): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion() (+64 more)

### Community 36 - "SQLiteStore"
Cohesion: 0.02
Nodes (96): KnowledgeItem, Find a repository by its URL., Find knowledge items matching any of the given tags., Structured knowledge about the company, systems, or processes., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo (+88 more)

### Community 37 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (77): BudgetTracker, classify(), GovernanceGate, _host_of(), Extract a hostname from a URL, or return a bare host unchanged., Per-session consumption counters and their ceilings. Ceilings come from the…, Return the name of the first exhausted session-wide limit, or None. Only covers…, Return a reason string if *tool* has hit its per-tool session cap. Checks only… (+69 more)
...
### Community 41 - "timedelta"
Cohesion: 0.06
Nodes (41): _as_aware_utc(), _company_alive(), _company_id_for_agent(), _env_float(), ephemeral_reaper_loop(), datetime, services/ephemeral_reaper.py — destroy expired ephemeral companies. The…, Whether *company_id* still exists, cached per sweep. A lookup error returns… (+33 more)

### Community 42 - "WorkflowRun"
Cohesion: 0.03
...
Cohesion: 0.03
Nodes (90): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, all_provider_ids(), _provider_ids_from_literal(), Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, CatalogActiveBrain, CatalogMirror (+82 more)

### Community 46 - "test_minimax_bug_fixes.py"
Cohesion: 0.12
...
Cohesion: 0.06
Nodes (48): BrainResolution, get_active_brain_sync(), _pick_from_records(), Resolved brain LLM for the current agency configuration. ``provider_id`` is the…, Return the cached brain read by sync callers. None if never resolved., Single source of truth for the active brain LLM. Resolution order (matches the…, Pick the highest-priority free provider, with paid opt-in fallback., resolve_active_brain() (+40 more)
...
Cohesion: 0.04
Nodes (105): Reset the singleton (test helper)., reset_ceo_dispatcher(), BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput (+97 more)
...
Cohesion: 0.05
Nodes (65): Reset the singleton (for tests)., reset_failover_manager(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog() (+57 more)
...
Cohesion: 0.06
Nodes (69): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, Attempt, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One delegation of one subtask to one tier., A subtask's full history: what it is, and every attempt at it., SubtaskRecord, _goal() (+61 more)
...
### Community 71 - "test_agent_runner.py"
Cohesion: 0.04
Nodes (71): AgentPhaseError, _note_phase_end(), _note_phase_start(), Exception, Raised when a named agent phase (planning, verification, etc.) fails., Log start/elapsed for one phase of the plan/execute/verify loop. A task that…, Open a tracked phase, returning the token that closes it. Never raises., Close the tracked phase *token* opened above. Never raises. (+63 more)
...
Cohesion: 0.05
Nodes (35): One deploy, normalised from whatever shape the tool returned., Typed facade over the Render MCP server's tools. Every method returns plain…, True when there is both an API key and an endpoint to reach., List a service's deploy history, newest first as Render returns it., Fetch one deploy by ID., Return the most recent deploy, or ``None`` when there are none., Build a ``RenderDeploy`` from an upstream deploy object., RenderDeploy (+27 more)
...
### Community 76 - "SettingsHub.jsx"
Cohesion: 0.04
Nodes (38): getLoops(), OBSERVE_STATUS, HubTabs(), Spinner(), AgentsScreen, KnowledgeScreen, OnboardingScreen, TABS (+30 more)
...
Cohesion: 0.03
Nodes (121): Attempt, int, get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager., text_key(), failover_chat_completion_via_router(), openai_body_from_response() (+113 more)

### Community 79 - "brain_failover.py"
Cohesion: 0.06
Nodes (47): _disabled_from_mongo(), _disabled_from_sqlite(), disabled_providers(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _mongo_db(), _mongo_enabled() (+39 more)
...
Cohesion: 0.05
Nodes (36): API, getAccountLifecycle(), getMe(), logout(), App(), AppRoutes(), ProtectedRoute(), V5App (+28 more)
...
Cohesion: 0.09
Nodes (18): MemoryEntry, PersistentMemoryStore, Any, Connection, Path, Row, Save a memory entry with categorization and scoping., Recall a specific memory entry. (+10 more)
...
Cohesion: 0.06
Nodes (53): _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost(), cmd_keylist(), cmd_merge(), cmd_models() (+45 more)
...
Cohesion: 0.04
Nodes (40): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, _infer_parameters_from_func(), Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed. (+32 more)
...
Cohesion: 0.07
Nodes (26): _backend(), CEOLedger, GoalRecord, _now(), Any, One piece of work the CEO has taken ownership of. ``state`` is the supervisor's…, Return ``(done, total)`` subtask counts., Durable store for :class:`GoalRecord`s. Every method is best-effort and never… (+18 more)
...
### Community 115 - "HermesAdapter"
Cohesion: 0.03
Nodes (62): _active_primary_provider(), Best-effort read of the active brain's primary provider (or ``None``)., Resolve the model id to force for a code-execution run, or ``None``. Returns…, Resolve the base URL of the agency's own Hermes server. Precedence:…, resolve_coding_model_preference(), resolve_hermes_base_url(), _env_float(), HermesAdapter (+54 more)
...
### Community 120 - "tests/conftest.py"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (54): Feed LLM token/cost spend into the governance session budget. Kept separate…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, build_governance_router(), Any, APIRouter, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin() (+46 more)

### Community 124 - "Usage"
Cohesion: 0.05
Nodes (42): AlertHandler, BudgetTracker, Counter, _Dimensions, get_budget(), _month(), Any, packages/llm/budget.py — token and cost accounting with spend alerts. Tracks… (+34 more)
...
Cohesion: 0.06
Nodes (39): Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory, _make_engine() (+31 more)
...
### Community 133 - "test_ceo_router.py"
Cohesion: 0.04
Nodes (66): build_ceo_router(), Any, APIRouter, backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), get_ceo_dispatcher(), Return the shared CEODispatcher singleton. (+58 more)

### Community 134 - "test_one_model_catalogue.py"
Cohesion: 0.08
...
Cohesion: 0.04
Nodes (35): runtimes/health.py — RuntimeHealthService. Periodically polls all registered…, Any, Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking)., Return all registered runtimes with their cached health status. (+27 more)
...
Cohesion: 0.07
Nodes (38): get_brain_preference(), get_provider_role_tags(), invalidate_brain_cache(), Any, Clear the cached brain so the next read re-resolves. Called from…, Map provider_id -> ``{is_brain, role, reason}``. Mirrors the surface area…, Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, Read provider records from the live store. Returns ``(records, fetch_failed)``.… (+30 more)

### Community 144 - "E2BAdapter"
Cohesion: 0.06
Nodes (39): E2BAdapter, Any, TaskResult, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, _FakeAsyncSandboxClass (+31 more)
...
Cohesion: 0.04
Nodes (36): consultExecutives(), createCompany(), delegateSeoFindings(), getCompany(), getOnboardingProgress(), getSeoAudit(), listExecutives(), listSeoAudits() (+28 more)

### Community 147 - "TaskPriority"
Cohesion: 0.08
...
Cohesion: 0.05
Nodes (52): AuditEvent, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient (+44 more)

### Community 153 - "Task"
Cohesion: 0.03
Nodes (82): Task, Full task/issue document., Update the updated_at timestamp., Task, _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing(), Any (+74 more)
...
Cohesion: 0.04
Nodes (43): MCPClient, Any, RuntimeError, Thin async MCP client with open/close circuit breaker. Thread-safe only within…, Full URL of the JSON-RPC endpoint this client posts to., Build the request headers shared by ``_rpc`` and ``notify``. ``Accept`` lists…, Propagate the calling agent's identity across the process boundary. The MCP…, Attach the agent identity whose actions this client executes. (+35 more)
...
Cohesion: 0.07
Nodes (35): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), Backward-compatibility shim — use scripts/generate_api_key.py instead. This…, _check_rate_limit(), default_keys_path(), issue_new_api_key() (+27 more)
...
Cohesion: 0.07
Nodes (39): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Re-queue BLOCKED tasks that have cooled down and are ready for retry., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, _make_task(), asyncio (+31 more)
...
### Community 182 - "test_llm_router_disabled.py"
Cohesion: 0.11
...
Cohesion: 0.05
Nodes (54): _build_provider_router(), _builtin_provider_records(), _chat_provider_policy(), _fallback_local_provider_record(), _get_provider_policy(), get_provider_policy_route(), _list_configured_provider_records(), _nvidia_nim_provider_record() (+46 more)

### Community 184 - "fmtErr"
Cohesion: 0.07
Nodes (27): chatSend(), createWikiPage(), deleteSource(), fmtErr(), getAgentChatJob(), getCompanyGraph(), getDefaultBackendUrl(), getSession() (+19 more)
...
Cohesion: 0.05
Nodes (65): tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest, ExecutionLogEntry, FollowUpRequest (+57 more)

### Community 189 - "session_retro.py"
Cohesion: 0.13
...
### Community 196 - "test_prompt_injection_boundary.py"
Cohesion: 0.17
...
Cohesion: 0.05
Nodes (36): Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NvidiaProvider, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., OllamaProvider, Ollama provider adapter — local LLM inference., packages.ai — provider abstraction, model registry, and failover manager. (+28 more)

### Community 201 - ".call"
Cohesion: 0.07
Nodes (24): _as_list(), _coerce_payload(), Any, RuntimeError, Return tool output as Python data. MCP tool results arrive either as…, Normalise a tool payload into a list of dicts. Upstream tools variously return…, Unwrap a nested envelope such as ``{"service": {...}}`` when present., Run the MCP handshake once per client instance. Streamable-HTTP servers create… (+16 more)

### Community 202 - "MemoryCategory"
Cohesion: 0.14
Nodes (26): Memory middleware for automatic context injection into AI tool requests. This…, MemoryCategory, MemoryScope, Enum, str, Enhanced persistent memory system with auto-loading across AI coding tools.…, Memory categories for semantic organization., Memory scope determines when memory is auto-loaded. (+18 more)
...
Cohesion: 0.05
Nodes (34): _keepalive_self_ping(), lifespan(), _maybe_boot_purge(), _purge_backlog_core(), _purge_summary_clean(), Return orchestrator queue depth, active runs, and supervisor state (#522)., Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free… (+26 more)
...
### Community 214 - "ExecutiveAdvisory"
Cohesion: 0.08
Nodes (25): _compose_context(), default_executives(), ExecOpinion, Executive, ExecutiveAdvisory, _format_company_context(), Grounding, Any (+17 more)
...
Cohesion: 0.05
Nodes (50): BrainFailoverManager, ProviderHealth, ProviderInfo, Any, Return the next healthy provider, excluding any in the exclude set. Ordering:…, Record a successful call — resets the circuit breaker., Permit one probe call without claiming the provider succeeded. This is the…, Seconds until the soonest cooling provider is probeable again. ``None`` when no… (+42 more)
...
Cohesion: 0.05
Nodes (38): 🛡 Admin — users & access, 🤖 Agents — autonomous team, Architecture, security, license, Autonomous AI Agency, 💬 Chat — unified assistant, 🏢 Company — operating context, Contributing, 📊 Dashboard — system overview (+30 more)
...
### Community 261 - "Artifact"
Cohesion: 0.11
Nodes (23): AgentRole, When coder == reviewer model, swarm should log a warning., workflow/artifact_store.py — Durable artifact persistence. Artifacts are stored…, workflow/engine.py — WorkflowEngine: CRISPY phase sequencer. The engine is the…, Reset the singleton (test helper)., reset_engine(), Artifact, ModelRoutingConfig (+15 more)

### Community 262 - "ExecutionRequest"
Cohesion: 0.03
Nodes (64): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), OrchestratorSupervisor, Any, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor(), ExecutionRequest (+56 more)
...
### Community 265 - "loop.py"
Cohesion: 0.04
Nodes (84): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, Agent subsystem — planner / executor / verifier loop., _check_extra_kwargs(), _enforce_signature(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic… (+76 more)
...
### Community 271 - "FeatureUnavailableError"
Cohesion: 0.10
...
### Community 272 - "_autonomy_bg_cycle"
Cohesion: 0.06
Nodes (42): _fetch_github_quick_notes(), _gh_repo(), _gh_token(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, _autonomy_bg_cycle(), _DoctorCheck, _DoctorReport (+34 more)
...
### Community 286 - "test_backend_server_features.py"
Cohesion: 0.07
...
### Community 292 - "test_kimi_bridge_server.py"
Cohesion: 0.05
...
### Community 314 - "ControlsScreen.jsx"
Cohesion: 0.15
...
### Community 320 - "schedules/api.py"
Cohesion: 0.13
...
### Community 342 - "test_task_pre_execution_gate.py"
Cohesion: 0.24
...
### Community 351 - "get_company_graph_store"
Cohesion: 0.05
Nodes (50): CompanyAgencyService, Any, SpecialistFamily, services/company_agency.py — Company Agency Orchestration Service After…, Orchestrates specialist activation, runtime startup, and 24x7 scheduling for a…, Return the best available runtime for a specialist family. Checks available…, Return the ordered runtime preferences for a specialist family., Activate a company's AI agency after onboarding completes. Steps: 1. Assign… (+42 more)
...
### Community 357 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 361 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
Cohesion: 0.10
Nodes (15): Send a plaintext Markdown-v1 message and return its Telegram message_id.…, Send a message and return the Telegram ``message_id``. Used by…, _send_message(), _send_message_with_id(), isolated_telegram(), isolated_telegram_config(), tests/_telegram_test_utils.py Snapshot/restore helper for ``telegram_bot``…, Pytest fixture alias for ``isolated_telegram_config``. Use this in tests that… (+7 more)
...
Cohesion: 0.13
Nodes (24): AdvisoryMemory, _keywords(), _mem_key(), The C-suite's durable memory of past recommendations. A thin adapter over…, Distinct significant words (len ≥ 5), order-preserved, for LIKE recall., _FakeMemory, asyncio, Tests for the intelligent C-suite advisory layer (agent/executive_advisory.py).… (+16 more)
...
Cohesion: 0.24
Nodes (11): AdvisoryConsultRequest, AdvisoryResponse, ExecutiveListResponse, ExecutiveModel, GroundingModel, OpinionModel, BaseModel, backend/executive_advisory_api.py — REST surface for the C-suite advisory.… (+3 more)
...
### Community 394 - "test_tasks_reconciler_todo_requeue.py"
Cohesion: 0.20
...
### Community 397 - "telegram_service.py"
Cohesion: 0.16
...
### Community 406 - "admin_update_task_router.py"
Cohesion: 0.22
...
### Community 444 - "AuditLog"
Cohesion: 0.05
Nodes (56): Replace the process-wide store. Tests only., reset_approval_store(), AuditLog, get_audit_log(), Bounded in-memory ring buffer plus a structured log stream. The ring buffer…, Store and emit *event*. Never raises., Return the most recent events, newest first, optionally filtered., Aggregate view for the dashboard and the metrics endpoint. ``would_block`` is… (+48 more)
...
### Community 489 - "has_image_content"
Cohesion: 0.31
...
### Community 493 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 496 - "CompanyGraphService"
Cohesion: 0.03
...
### Community 508 - "flesch_reading_ease"
Cohesion: 0.24
...
### Community 525 - "TestBrainFailoverModelUpdates"
Cohesion: 0.06
...
### Community 526 - "TestPoliciesGovernanceStableClaim"
Cohesion: 0.20
...
### Community 566 - "_get_current_user"
Cohesion: 0.25
...
### Community 605 - "telegram_inbound_handlers.py"
Cohesion: 0.12
...
Cohesion: 0.13
Nodes (23): AdviceResult, get_executive_advisory(), The full advisory response: per-executive opinions plus the synthesis., Return the shared :class:`ExecutiveAdvisory`, built on first use., Drop the singleton (test helper)., reset_executive_advisory(), build_executive_advisory_router(), Any (+15 more)
...
### Community 643 - "ProviderRouter"
Cohesion: 0.07
Nodes (23): _append_agent_session_message(), _run_agent_loop(), CommercialFallbackRequiredError, _get_director(), _normalized_provider_type(), ProviderAttempt, ProviderFallbackError, ProviderRouter (+15 more)
...
### Community 646 - "agent/output_filter.py"
Cohesion: 0.32
...
### Community 649 - "_build_direct_chat_schedule_suggestion"
Cohesion: 0.32
...
### Community 682 - "Continual Harness (`agent/harness_spec.py`)"
Cohesion: 0.25
...
### Community 731 - "TestRouterIntegration"
Cohesion: 0.31
...
### Community 766 - "_load_local_metrics_since"
Cohesion: 0.43
...
### Community 767 - "BackgroundServices"
Cohesion: 0.38
...
### Community 771 - "TestSupportMatrixDocsSync"
Cohesion: 0.29
...
### Community 802 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.29
...
### Community 810 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 811 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 834 - "TestAgentRunnerExecution"
Cohesion: 0.33
...
### Community 840 - "brain.py"
Cohesion: 0.05
Nodes (43): livenim, allow_paid_brain(), _host_is_openai_compatible(), is_anthropic_model(), _norm(), Shared free-brain policy helpers (issue #656). Single source of truth for "is…, True when the base URL should get an /v1 prefix appended (openai-compat).…, True only when the operator explicitly opted into a paid (Anthropic) brain.… (+35 more)
...
### Community 850 - "TestModelRoleSeparation"
Cohesion: 0.40
...
### Community 1132 - "InternalAgentAdapter"
Cohesion: 0.04
Nodes (41): InternalAgentAdapter, Any, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Create an isolated execution context for a single task. Tries ``git worktree…, Clean up the worktree or temp copy created by ``_create_worktree``., Fail preflight when the provider extension is configured but missing. Without… (+33 more)
...
## Knowledge Gaps
- **3555 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3550 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13619 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **114 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `test_llm_router_queue_cache.py`, `parse_event_stream`, `test_llm_router_resilience.py`, `test_brain_patch_service_token.py`, `TaskStore`, `test_llm_router_strategies.py`, `brain_config.py`, `test_gh_brain_failover.py`, `NotificationDispatcher`, `test_operational_incidents.py`, `PolicyEngine`, `Agency`, `AgentJobManager`, `test_autonomous_agency_e2e.py`, `models/company_graph.py`, `claim`, `E2BSandboxSession`, `SelfHealingAgent`, `test_provider_router.py`, `TaskStatus`, `test_failover_client_shared.py`, `test_company_api.py`, `test_webui_provider_priority.py`, `services/background.py`, `timedelta`, `WorkflowRun`, `Specialist`, `test_telegram_service_webhook.py`, `test_telegram_webhook.py`, `test_unit8_model_catalog.py`, `test_model_router.py`, `cost_tracker.py`, `UserRole`, `resolve_active_brain`, `BrainWatchdog`, `validate_policy_text`, `test_sam_livekit.py`, `AgentSwarm`, `PrimeAgentAdapter`, `ChatHistoryStore`, `test_openclaw_endpoints.py`, `TokenBudget`, `brain_failover.py`, `reset_cache`, `ResearchTask`, `TestClient`, `FeatureMaturity`, `SeoAuditEngine`, `test_procedural_memory.py`, `SQLiteStore`, `test_llm_router_e2e.py`, `telegram_inbound_handlers.py`, `_resolve_push_token`, `e2e/test_browser.py`, `test_autonomy_status.py`, `config.py`, `telegram_bot.py`, `test_local_brain_state.py`, `KeyPool`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `test_knowledge_sync.py`, `test_executive_advisory_api.py`, `test_startup_warmup.py`, `HermesAdapter`, `tests/conftest.py`, `Usage`, `resolve_e2b_config`, `TestHarnessAdapter`, `test_ceo_router.py`, `test_context_rulebook.py`, `ArtifactStore`, `E2BAdapter`, `Task`, `test_new_features_e2e.py`, `_step`, `test_trend_watcher.py`, `MCPClient`, `persist_plan_spec`, `KeyStore`, `test_brain_failover_413.py`, `test_changelog_check_workflow.py`, `test_daily_2026_07_27.py`, `test_openclaw_gateway.py`, `test_telegram_freebuff.py`, `test_llm_router_disabled.py`, `OllamaCircuitBreaker`, `_job_text`, `session_retro.py`, `ClaudeCodeAdapter`, `test_provider_state_durability.py`, `test_portfolio_intake.py`, `test_features_api.py`, `_Budget`, `test_video_transcript.py`, `TrendWatcher`, `TestAgentRunnerSafety`, `SchedulerStore`, `test_e2b_data_flow.py`, `nvidia_models.py`, `test_task_brain_preflight.py`, `WorkflowRun`, `TestEstimateTokensForMessages`, `TestClient`, `test_runtimes_health_endpoint.py`, `ContextPruner`, `TestNormalizeResponseFormat`, `test_control_plane_api.py`, `.on_task_complete`, `test_agile_api.py`, `test_app_settings.py`, `test_task_clarification.py`, `test_persistent_memory.py`, `SecurityScanner`, `ExecutionRequest`, `loop.py`, `OutputFilter`, `_autonomy_bg_cycle`, `test_platform_controls.py`, `test_rate_limiter.py`, `test_kimi_bridge_server.py`, `_clean_director`, `OrchestratorQueue`, `test_nvidia_model_discovery.py`, `test_sam_voice.py`, `test_colibri_brain_shim.py`, `analyze_page`, `brain.py`, `test_catalogue_probe.py`, `test_scheduler_hydration_bounded.py`, `test_purge_backlog.py`, `SeoFixer`, `get_registry`, `test_task_pre_execution_gate.py`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `sam`, `test_skill_registry_boot_refresh.py`, `test_brain_availability_doctor.py`, `isolated_telegram_config`, `capture_screens.py`, `_start_ceo_agency`, `TestSwarmRoleRouting`, `test_process_quick_note_workflow.py`, `_undeclared`, `test_tasks_reconciler_todo_requeue.py`, `test_service_token.py`, `test_harness_spec.py`, `test_agency_fix.py`, `_Recorder`, `test_monitor_lib.py`, `test_v4_api.py`, `test_telegram_task_callback.py`, `AuditLog`, `OrchestratorCheckpointStore`, `TestTheWorkflowIsSafeAndReadOnly`, `test_telegram_diag_endpoint.py`, `test_ping.py`, `clear_cooldowns`, `test_task_store_fails_loud_in_production.py`, `UsageEvent`, `test_dashboard_cache.py`, `test_voice_pipeline.py`, `platform_controls_router.py`, `client`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `TaskSpec`, `ProviderRouter`, `proxy.py`, `loop.py`, `AgentSessionStore`, `PolicyEngine`, `backend/server.py`, `E2BAdapter`, `ReactScratchpad`, `AgentJobManager`, `test_autonomous_agency_e2e.py`, `test_governance_api.py`, `ContextManager`, `direct_chat.py`, `E2BSandboxSession`, `test_provider_router.py`, `test_failover_client_shared.py`, `MCPClient`, `test_empirical_verify.py`, `WorkflowRun`, `MultiAgentSwarm`, `test_brain_failover_413.py`, `StuckDetector`, `test_ceo_dispatcher.py`, `WorkspaceTools`, `CEODispatcher`, `TestAgentRunnerExecution`, `test_agent_runner.py`, `brain.py`, `TokenBudget`, `TestAgentLoopMCPIntegration`, `TestAgentRunnerSafety`, `LocalWorkspace`, `UserMemoryStore`, `FreeBuffAgent`, `InternalAgentAdapter`, `GitHubTools`, `AdaptiveHalter`, `ContextPruner`, `enforcement.py`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `TestAnthropicPayloadStructuredOutput`, `test_all_providers_discovery.py`, `backend/server.py`, `implement_agent.py`, `direct_chat.py`, `test_provider_router.py`, `failover_client.py`, `test_bedrock_live.py`, `_get_provider_policy`, `ai/router.py`, `test_colibri_brain_shim.py`, `TestLegacyRouterCacheTTL`, `test_anthropic_refusal_fallback.py`, `settings.py`, `clear_cooldowns`, `TestLegacyRouterServerFallback`, `TrafficDirector`, `_payload`, `test_bedrock_provider.py`, `TestRouterIntegration`, `ProviderConfig`?**
