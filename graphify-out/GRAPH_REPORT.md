...
## Corpus Check
- 1529 files · ~2,224,687 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 32420 nodes · 66721 edges · 1354 communities (1151 shown, 203 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 6579 edges (avg confidence: 0.93)
...
- test_dispatcher_honors_concurrency_env
...
- test_get_provider_policy_returns_default_without_db
- Rule
...
## Communities (1354 total, 203 thin omitted)

### Community 0 - "gateway.py"
Cohesion: 0.04
Nodes (85): get_budget(), The process-wide budget tracker., get_cache(), The process-wide cache manager., failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.… (+77 more)

### Community 1 - "backend/server.py"
Cohesion: 0.01
Nodes (384): set_skill_registry(), get_harness_adapter(), admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate (+376 more)
...
Cohesion: 0.03
Nodes (164): Describes a single worker's task and constraints., WorkerSpec, set_quick_note_queue(), get_sam(), fastapi_middleware_cors, middleware, admin_control(), admin_delete_user() (+156 more)

### Community 4 - "os"
Cohesion: 0.01
Nodes (380): abc, Browser admin UI for login, service control, key management, and diagnostics., ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered) Runs the…, agent/background.py — Background Agent An always-on worker thread that…, Cached LLM Client wrapper. Drop-in wrapper around any LLM API call that…, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata…, context_pruner.py — auto-generated module docstring (user-research skill scan). (+372 more)

### Community 5 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (118): heapq, int, Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache (+110 more)

### Community 6 - "company_api.py"
Cohesion: 0.01
Nodes (244): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+236 more)

### Community 7 - "Task"
Cohesion: 0.01
Nodes (481): agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, list_runtime_agents(), agents/api.py — FastAPI router for user-defined agent CRUD. Endpoints: GET…, Return all system-registered runtime agents with live health status. Combines…, AgentCreateRequest, AgentDefinition, AgentStore, AgentUpdateRequest (+473 more)
...
### Community 9 - "api.js"
Cohesion: 0.01
Nodes (52): approveGovernanceRequest(), approveWorkflow(), autoRecommendCompanySkills(), cancelWorkflow(), createMcpServer(), createQuickNote(), delegateSeoFindings(), deleteGithubToken() (+44 more)

### Community 10 - "BrainConfig"
Cohesion: 0.03
Nodes (74): Architecture (per plan §3), Files touched, Hard constraints (from the plan) — all met, Implementation — DB-persisted, UI-switchable Brain (PR #824 follow-up), New files, Resolution precedence, Rollout / verification (per plan §5), Tests (+66 more)

### Community 11 - "config.py"
Cohesion: 0.04
Nodes (67): Added, Added, _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir(), expand_env() (+59 more)

### Community 12 - "LLMRequest"
Cohesion: 0.03
Nodes (105): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, packages_llm_providers, The models to consider on one provider., LLMRequest, Cheap character-based token estimate (~4 chars/token). Deliberately dependency-…, A provider-neutral chat request. ``model`` is a *hint*: the router may…, tests/test_anthropic_tool_list_caching.py — C6 extension: tool-list prompt… (+97 more)

### Community 13 - "test_llm_router_resilience.py"
Cohesion: 0.03
Nodes (64): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), get_ring(), KeyRing (+56 more)

### Community 14 - "SandboxManager"
Cohesion: 0.06
Nodes (25): detect_backend(), E2BBackend, LocalBackend, Any, A live sandbox and everything needed to audit and reap it., Report daemon reachability and whether it is rootless. Rootless is surfaced…, Runs sandboxes as E2B Firecracker micro-VMs. Delegates entirely to…, No isolation. Present so 'ungoverned' is a visible state, not a silent one.… (+17 more)
...
### Community 16 - "resolve_identity"
Cohesion: 0.04
Nodes (95): _egress_policy_reason(), Return why governance policy blocks *host*, or None. Runs strictly *after* the…, fnmatch, _coerce_text(), new_session_id(), Any, packages/governance/identity.py — who is acting, and under what mandate. Docker…, Build an :class:`AgentIdentity` from whatever the caller knows. Total by design… (+87 more)
...
Cohesion: 0.06
Nodes (53): Agent governance — identity, policy, approvals, audit, sandboxes, Sandbox isolation in production, 1.11 Multi-Agent Governance (10 / 100 / 1000 agents), Failure behaviour, build_docker_run_argv(), DockerBackend, load_profiles(), Path (+45 more)

### Community 19 - "test_model_router.py"
Cohesion: 0.04
Nodes (87): classify_task(), _extract_recent_text(), Any, Concatenate plain text from the last *last_n* messages., Return the most likely task category for this request. Args: messages: OpenAI-…, invalidate_cache(), Force the next call to re-probe Ollama (useful in tests)., Reset the singleton and clear the cached model map (test helper). (+79 more)
...
Cohesion: 0.05
Nodes (102): Semaphore, Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, Run one sub-task, judge the result, and escalate it if it is slop. The loop is…, build_subtask_brief(), _coerce_subtasks(), decompose(), _extract_json_object(), fallback_decomposition() (+94 more)

### Community 22 - "test_governance_enforcement.py"
Cohesion: 0.03
Nodes (99): ApprovalRequest, ApprovalStatus, ApprovalStore, Any, Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The…, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`… (+91 more)

### Community 23 - "test_e2b_sandbox.py"
Cohesion: 0.05
Nodes (56): _inject_token(), maybe_attach_e2b(), Best-effort scrub of ``token`` from ``text``., Open an E2B session and attach it as ``runner._mcp``. Single wiring line for…, Return ``(authed_url, clean_url)`` for a GitHub repo URL., _scrub_token(), _clean_e2b_env(), fake_sandbox() (+48 more)

### Community 24 - "RuntimeManager"
Cohesion: 0.04
Nodes (25): Runtime Selection Policy, Overview, Return routing decision audit log (newest first)., True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking)., Return all registered runtimes with their cached health status., RuntimeManager, Return the single best adapter for *task_type*. If *preferred_runtime_id* is… (+17 more)

### Community 25 - "CompanyGraphService"
Cohesion: 0.03
Nodes (61): BusinessCategory, Find a website by its URL., A company website with detected systems and stack inference., Inferred technology stack from website/repo analysis., StackInference, Website, CompanyGraphService, SpecialistFamily (+53 more)

### Community 26 - "ModelConfig"
Cohesion: 0.08
Nodes (18): ModelConfig, Capability and pricing metadata for one model id., ModelRegistry, packages/llm/registry.py — the model registry. One queryable index of every…, Add or replace a model (used by runtime discovery)., The biggest-context model available — the context-overflow escape hatch., The lowest-cost model that still meets the requirements., The fastest model by declared speed tier. (+10 more)

### Community 27 - ".test_thin_static_detection_still_escalates_to_render"
Cohesion: 0.12
Nodes (6): End-to-end resilience for bot-protected sites — the regression guard the user…, gucci.com / workers.dev regression: a bot-walled or JS-rendered site leaks a…, Full flow for an Akamai-walled site: live fetch returns a bot wall, headless…, TestBotProtectionResilience, fake_detect(), _sys()
...
### Community 29 - "Agency"
Cohesion: 0.04
Nodes (59): Agency, AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _gh_token() (+51 more)
...
Cohesion: 0.03
Nodes (78): Completed Task Archive — June to August 2026, pytest_asyncio, main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), Scan a website and save the results. Args: company_id: Company ID url: Website…, _content_contains_domain() (+70 more)

### Community 33 - "E2BSandboxSession"
Cohesion: 0.13
Nodes (16): MCPUnavailableError, RuntimeError, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, Any, Create the sandbox. Raises :class:`MCPUnavailableError` on failure., Kill the sandbox. Idempotent; never raises (best-effort cleanup)., Route an MCP-style tool call into the E2B sandbox. Implements: ``write_file``,… (+8 more)
...
Cohesion: 0.04
Nodes (100): count, Preferring local, Choosing one, Latency got worse, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig (+92 more)

### Community 36 - "MongoDBStore"
Cohesion: 0.02
Nodes (94): Security, Security, MongoDBStore, Any, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., Create a new company in SQLite. (+86 more)

### Community 37 - "rbac.py"
Cohesion: 0.04
...
Cohesion: 0.06
Nodes (35): probe_model_liveness(), ProbeResult, BaseModel, Outcome of a single (provider, model) liveness probe., Probe ``(provider, model)`` for liveness. Never raises. Returns a…, A model that 410s must be rejected with 422 + a probe report., A model that probes 200 OK must be persisted., If the chosen provider's key is missing, the probe short-circuits → 422. (+27 more)

### Community 40 - "brain_config.py"
Cohesion: 0.01
Nodes (187): 3b. Hermes — **our own** Hermes server (in-repo), UI-wired ✅  (size: M), 1. Render (production backend + worker), livenim, packages_ai, _active_primary_provider(), invalidate_brain_config_cache(), is_north_mini_code_default(), provider_api_key() (+179 more)

### Community 41 - "AgentRunner"
Cohesion: 0.02
Nodes (169): AgentPhaseError, AgentRunner, _run(), _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), Any (+161 more)
...
Cohesion: 0.04
Nodes (24): PrimeAgentAdapter, Adapter for the Prime Agent / pi coding CLI., Fail preflight when the provider extension is configured but missing. Without…, tests/test_prime_agent_runtime.py — PrimeAgentAdapter unit tests. The event-…, plain `pi` rejects --cwd; the workspace is set on the subprocess instead., Regressions for defects found in review of this adapter., An agent_end carrying no assistant message produced nothing., health_check() and readiness_check() must agree about the binary. The base… (+16 more)

### Community 44 - "Settings"
Cohesion: 0.03
Nodes (74): CatalogMirror, ModelCatalogStore, Any, The full mirrored catalog document., Dual-storage catalog mirror (Mongo primary, sqlite mirror). All public methods…, Return the mirrored catalog. Resolution order: 1. In-process cache (if fresh)…, Force a rebuild + persist. Returns the synced catalog. Used by the ``POST…, Clear the in-process cache. (+66 more)

### Community 45 - "Added"
Cohesion: 0.04
Nodes (53): Return True only when the tool is definitively read-only and non-destructive.…, Remember a server-issued ``Mcp-Session-Id`` for subsequent requests., Added, Added, Render MCP — platform debugging and environment monitoring, 1. Coding sessions — stdio, via `.mcp.json`, 2. The running agency — Streamable HTTP against a deployed sidecar, Configuration (+45 more)

### Community 46 - "V5App.jsx"
Cohesion: 0.05
Nodes (36): API, V5App, HubTabs(), Spinner(), ActivationGate(), AssistantHub(), SamVoiceScreen, IntelligenceScreen (+28 more)

### Community 47 - "SpecialistService"
Cohesion: 0.03
Nodes (64): cost_table(), Return the active per-model cost table (USD per million tokens). Useful for…, Added, What already exists (don't rebuild), Added, buildGraphElements(), clear_stats(), cost_for_tokens() (+56 more)
...
Cohesion: 0.05
Nodes (58): csv, lxml, BaseModel, field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Aggregated report row - Screaming Frog CSV compatible. (+50 more)

### Community 51 - "get_runtime_manager"
Cohesion: 0.02
Nodes (92): Annotate an agent dict with its runtime's current health status., _with_runtime_health(), _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy() (+84 more)

### Community 52 - "setup/api.py"
Cohesion: 0.06
Nodes (69): motor_motor_asyncio, audit(), get_audit_log(), Append an audit log entry. Never logs raw secrets — only secret IDs / masked…, Return recent audit log entries, newest first. Supports filtering by user_id,…, complete_wizard(), _delete_wizard_state(), detect_configured_providers() (+61 more)
...
Cohesion: 0.02
Nodes (171): _fetch_github_quick_notes(), _gh_repo(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, Real handler — dispatches through AgentRunner when available. HARDENED (PR…, Public wrapper around _generate_plan for callers that want plan-only. Returns…, _cached(), _fast_count() (+163 more)

### Community 56 - "WorkflowOrchestrator"
Cohesion: 0.05
Nodes (42): Changed, Changed, Feature maturity — what's stable vs. beta, Actively wake every sleeping/circuit-open runtime. The default health service…, get_orchestrator_checkpoint_store(), Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Determine domain and task type from the request text. (+34 more)
...
Cohesion: 0.05
Nodes (44): Enum, Agentic Agile — Sprint management with velocity tracking and burndown. Issue:…, Status of a user story within a sprint., Qualitative health signal derived from sprint metrics., SprintHealth, StoryStatus, CapacityAllocation, Enum (+36 more)
...
Cohesion: 0.04
Nodes (77): Ask the C-suite advisory layer a business question. This is the CEO's bridge…, Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), _domain_list_reason(), _domain_matches(), _parse_domain_list(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return True if *host* is *pattern* or a subdomain of it. (+69 more)

### Community 64 - "facade.py"
Cohesion: 0.07
Nodes (39): _get_current_user_thunk(), _get_optional_user_thunk(), Request, create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., N5 dual-auth dependency: accept EITHER a user session OR a service token. Used… (+31 more)

### Community 65 - "BrainWatchdog"
Cohesion: 0.11
Nodes (13): N1. Activate the reliability spine — wire the watchdog, schedule the digest ⬜  (size: M, risk: low), BrainWatchdog, Any, Reset failure counter for a provider after a successful call., Fail over to the next available provider., Send a Telegram notification about the failover., Monitors provider health and triggers failover on consecutive failures., object (+5 more)
...
Cohesion: 0.04
Nodes (67): build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget(), get_metrics() (+59 more)

### Community 69 - "CEODispatcher"
Cohesion: 0.11
Nodes (20): CEODispatcher, CEOResult, Aggregated output from a multi-specialist execution., Real CEO delegation: wake runtimes, fan out, and merge results., Best-effort wake of any sleeping runtimes via RuntimeManager. Returns runtime…, _base_result(), Tests for CEODispatcher._maybe_cross_verify (opt-in risky-module re-check)., The _BYPASS contextvar must not leak true after cross_verify finishes, or every… (+12 more)
...
Cohesion: 0.12
Nodes (29): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, auto_register(), BaseModel, QualAnalysis, QualQuote, QualTheme, QuantAnalysis (+21 more)
...
Cohesion: 0.06
Nodes (38): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), get_orchestrator_supervisor(), stop_orchestrator_supervisor(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper). (+30 more)
...
Cohesion: 0.16
Nodes (19): evals_cost_aware_routing, USD cost of one model call. Raises KeyError on an unknown tier., token_cost(), Attempt, compare(), One model call made while working a task (an initial pass or a redo)., Aggregate a suite of task runs into headline metrics.…, Compare a baseline workflow against the routed one on the same tasks. A… (+11 more)
...
Cohesion: 0.07
Nodes (51): 6. Integration gaps to wire (follow-up implementation), Loop 3 — Agentic SDLC (the golden path), Acceptance criteria, Design, Objective, Part E — G5: `RepoConnection` + `DeliveryPolicy` (GitHub-only scope), Tech stack / touch points, Tests (+43 more)
...
Cohesion: 0.07
Nodes (40): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _is_ollama(), _key_pool(), _log_exhaustion(), _paid_reserve(), _parse_success() (+32 more)

### Community 91 - "ProviderRouter"
Cohesion: 0.06
Nodes (32): Added, Added, CommercialFallbackRequiredError, _get_director(), _normalized_provider_type(), ProviderAttempt, ProviderFallbackError, ProviderResult (+24 more)

### Community 92 - "claim"
Cohesion: 0.08
Nodes (18): claim(), cooldown_get(), cooldown_set(), _get_backend(), incr_window(), Try to acquire a named lock. Returns True if acquired, False if already held., Release a previously acquired lock., Put a key on cooldown for *ttl* seconds. (+10 more)
...
Cohesion: 0.05
Nodes (34): consultExecutives(), createWikiPage(), deleteSource(), ingestSource(), listExecutives(), updateCompany(), CompanyScreen, ExecutiveAdvisoryScreen (+26 more)

### Community 98 - "SeoFixer"
Cohesion: 0.12
Nodes (16): bs4, One concrete remediation performed (or proposed) by the fixer., Result of a fixer run., SeoFixAction, SeoFixResult, _humanize_filename(), BeautifulSoup, Path (+8 more)
...
Cohesion: 0.06
Nodes (41): is_commercial_provider(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, _nvidia_provider_chain(), Build Nvidia NIM provider config from env. Empty list when key is absent., Regression tests for the agent loop's multi-provider failover wire format.… (+33 more)
...
Cohesion: 0.08
Nodes (44): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState, FailedJob (+36 more)
...
Cohesion: 0.12
Nodes (21): functools, Enum, How to truncate messages when over the context limit., TruncationStrategy, get_current_trace_id(), get_tracer(), langfuse_metadata_with_trace(), Portable trace context that can be passed across async boundaries. (+13 more)
...
### Community 126 - "_FakeCommandResult"
Cohesion: 0.12
...
Cohesion: 0.05
Nodes (46): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+38 more)

### Community 131 - "Command"
Cohesion: 0.06
Nodes (20): Command, CommandCategory, CommandDispatcher, Enum, Parse and execute a slash command from raw text. Args: text: Raw command text,…, Return all enabled commands in a given category., Return all registered commands., Category grouping for discoverability and permission scoping. (+12 more)

### Community 132 - "register_webui"
Cohesion: 0.08
Nodes (35): allow_paid_brain(), get_brain_preference(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, test_allow_paid_brain_default_false(), test_allow_paid_brain_opt_in(), Operator can set BRAIN_PREFERENCE=colibri without it being rejected., test_get_brain_preference_accepts_colibri() (+27 more)
...
Cohesion: 0.07
Nodes (40): contextvars, BoundContext, emit_deprecation(), ExecutionResult, JudgeVerdict, MergeDecision, MonitorOutput, _orchestrator_bypass() (+32 more)
...
Cohesion: 0.06
Nodes (33): autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap(), _count_specialists(), _create_company_directly(), ensure_self_company(), _find_self_company() (+25 more)

### Community 148 - "render_ops.py"
Cohesion: 0.05
...
### Community 150 - "test_minimax_bug_fixes.py"
Cohesion: 0.13
...
Cohesion: 0.07
Nodes (46): _check_secret(), _expected_secret(), _expected_admin_secret(), _extract_admin_token(), BaseModel, backend/admin_update_task_router.py Step 1: POST…, Mount the update-task endpoint on ``app``. Idempotent: skips if a path with the…, Body for ``POST /api/workflow/orchestrator/update-task/{run_id}``.… (+38 more)
...
Cohesion: 0.06
Nodes (36): clear_cooldowns(), is_provider_on_cooldown(), mark_provider_failed(), Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown., Clear all cooldown entries (useful for testing). Delegates to…, clear_all_locks(), cooldown_clear() (+28 more)
...
Cohesion: 0.09
Nodes (21): _disabled_from_sqlite(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _paid_allowed_sqlite(), services/brain_failover.py — Universal multi-provider brain failover system.…, Reset the singleton (for tests)., Read the kill-switch set from the local SQLite mirror. Raises on error. (+13 more)

### Community 161 - "._get_conn"
Cohesion: 0.15
...
### Community 166 - "test_telegram_freebuff.py"
Cohesion: 0.07
...
Cohesion: 0.11
Nodes (16): _accumulate_usage(), _assistant_messages(), _message_text(), parse_event_stream(), ParsedRun, Any, Structured view of one ``--mode json`` event stream., Return assistant messages carried by an ``agent_end`` event. (+8 more)

### Community 169 - "LLMRouter"
Cohesion: 0.04
Nodes (71): Fixed, Fixed, packages_llm, AgentPolicy, LLMConfig, Per-agent routing overrides (ADR-008 §"named profiles")., The fully-resolved routing configuration., auto_disable() (+63 more)

### Community 170 - "useSafeData.js"
Cohesion: 0.13
...
### Community 172 - "_build_coordinator"
Cohesion: 0.09
...
Cohesion: 0.07
Nodes (39): 1. Authentication & Authorization, API Key Authentication, JWT / Token Auth, Category 2 — API Key Naming Confusion, TD-005 [MEDIUM] — Production Keys Have `test-key-` Prefix, _check_rate_limit(), default_keys_path(), issue_new_api_key() (+31 more)
...
### Community 178 - "test_governance_api.py"
Cohesion: 0.02
Nodes (151): Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, get_audit_log(), packages/governance/audit.py — the evidence trail for every governed action.…, Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Redact secret-shaped substrings, then truncate. (+143 more)

### Community 179 - "test_dispatcher_honors_concurrency_env"
Cohesion: 0.07
Nodes (12): _FakeMongoStore, _FakeSQLiteStore, asyncio, parametrize, Simulates a Mongo-style store with find/aggregate/replace_one., Simulates a SQLite-style store with load_all/upsert/remove., Duplicate-named rows must collapse to newest on BOTH backends. Regression for…, Stale unfired run-once jobs (run_count==0, old created_at) must be deleted. The… (+4 more)
...
### Community 184 - "self_audit_local.py"
Cohesion: 0.07
Nodes (27): _build_curl_cffi_fetcher(), get(), get_text(), head(), _sess(), main(), _parse_args(), Namespace (+19 more)
...
Cohesion: 0.12
Nodes (12): _creationflags(), CompletedProcess, Path, Spawn a new proxy process on Linux/Mac using the current Python interpreter., ServiceState, WindowsServiceManager, build_digest(), _count_open_auto_prs() (+4 more)
...
### Community 201 - "test_direct_chat_evolution.py"
Cohesion: 0.11
...
Cohesion: 0.02
Nodes (121): Register the built-in agent tools that are always available., _register_builtin_tools(), CLAUDE.md — agent/, Security surface, Skills worth invoking here, Testing, What this package does, Path (+113 more)
...
Cohesion: 0.12
Nodes (20): _emit_safely(), handle_anthropic_messages(), _messages_to_openai(), _openai_choice_to_anthropic_content(), Any, JSONResponse, Request, StreamingResponse (+12 more)
...
Cohesion: 0.04
Nodes (32): Any, Reconstruct a ScheduledJob from its as_dict() output., Fire a job immediately (webhook / manual trigger)., ScheduledJob, _MemCollection, _MemCursor, _MemDB, _MemDeleteResult (+24 more)

### Community 213 - "asyncio"
Cohesion: 0.09
...
Cohesion: 0.09
Nodes (23): Resolve the NVIDIA default model via the catalog (UNIT 7). Was hardcoded to…, _resolve_nvidia_default_model(), tests/test_unit7_catalog_propagation.py — UNIT 7 regression tests. Verifies…, ``_get_defaults()`` must consult the catalog first; the hardcoded…, ``_get_defaults()`` returns the catalog-derived defaults (not the hardcoded…, ``jcode.py`` must NOT have the stale hardcoded ``meta/llama-3.3-70b-instruct``…, ``opencode.py`` must NOT have the stale hardcoded model id inline., ``_NVIDIA_DEFAULT_MODEL`` must equal the first entry in the catalog's nvidia… (+15 more)

### Community 215 - "test_bedrock_provider.py"
Cohesion: 0.06
Nodes (20): _is_bedrock_model_id(), Return True if model_id is an AWS Bedrock model or inference profile ID., _bedrock_api_response(), _bedrock_provider(), _mock_boto3(), Any, asyncio, Tests for AWS Bedrock provider support in ProviderRouter. (+12 more)
...
### Community 221 - "test_cost_attribution.py"
Cohesion: 0.09
...
Cohesion: 0.03
Nodes (82): gc, AgentScheduler, Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Remove a job. Returns *True* if it existed., Enable or disable a job without deleting it., Return the running event loop, or ``None`` when called synchronously. Used so…, Attach a durable store and immediately rehydrate from it (#505). Called at…, Async variant of :meth:`attach_persistence` for callers already on an event… (+74 more)
...
### Community 227 - "_FakeAsyncClient"
Cohesion: 0.20
...
Cohesion: 0.07
Nodes (20): auth_headers(), FakeTaskStore, MonkeyPatch, tests/test_purge_backlog.py — 2026-07-03 crash-loop remediation. Covers: - POST…, Drive _maybe_boot_purge with fakes; return (purged, marker_writes). ``core``…, A failed purge must NOT record the nonce — it retries next boot., A PARTIAL purge (error markers inside the summary) must not record the nonce…, Minimal in-memory stand-in for tasks.store.TaskStore. (+12 more)
...
Cohesion: 0.12
Nodes (11): BaseModel, Validate configured paths., Check if proxy is running., Check if Ollama is running., Start the proxy server., Stop the proxy server., Get current status of all services., Status of all services. (+3 more)
...
### Community 256 - "TestOrchestratorQueue"
Cohesion: 0.13
...
### Community 258 - "WorkspaceManager"
Cohesion: 0.21
Nodes (6): _normalize_path(), _now(), Any, Path, WorkspaceManager, WorkspaceRecord

### Community 259 - "ref_react"
Cohesion: 0.07
Nodes (35): Fixed, Removed, Removed, buildWorkflow(), getMe(), getWorkflowRuns(), login(), logout() (+27 more)
...
Cohesion: 0.11
Nodes (12): _Collection, _DeleteResult, _InsertResult, _match(), _new_id(), _now_iso(), Return True if *doc* satisfies the MongoDB-style *query*. Supports: exact…, Motor-compatible async collection backed by a SQLite table. All I/O uses… (+4 more)
...
### Community 269 - "llm/router.py"
Cohesion: 0.04
Nodes (84): collections_abc, Modules, Writing a custom adapter, packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible (+76 more)
...
### Community 274 - "test_webui_provider_priority.py"
Cohesion: 0.27
...
Cohesion: 0.04
Nodes (50): Any, SAM voice agent — the voice-controlled interface to the agency., Process a voice command and return SAM's spoken response. Args: text: The…, Call the NVIDIA NIM LLM (free tier) for SAM's response., Rule-based fallback when the LLM is unavailable., A single voice conversation session with SAM., SamAgent, SamConversation (+42 more)
...
### Community 284 - "run_once"
Cohesion: 0.10
Nodes (27): _bin_exists(), _default_agency_url(), _default_machine_id_file(), _env_int(), _get_or_create_machine_id(), _http_json(), _log(), _log_path() (+19 more)
...
Cohesion: 0.12
Nodes (28): _check_constitution_echo(), _check_files_exist(), _check_grounding(), _check_hedges(), _check_project_identity(), _check_risk_flags(), _check_source_summary(), _check_todos() (+20 more)
...
### Community 305 - "Added"
Cohesion: 0.02
Nodes (169): Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., _first_paragraph(), Return the first non-empty, non-heading line. Skips YAML frontmatter (--- ...…, BudgetExceededError, Exception, Reset token counters for all sessions (caps preserved). Called at the start of…, Reset all budgets if the UTC calendar day has changed since last reset. Safe to… (+161 more)

### Community 306 - "test_connector_registry.py"
Cohesion: 0.15
Nodes (7): packages_integrations, Execute the ``webhook`` connector: POST *payload* as JSON to *url*. Fails soft…, send_webhook(), tests/test_connector_registry.py — the connector catalogue + webhook action.…, test_send_webhook_does_not_follow_a_redirect(), test_send_webhook_rejects_loopback_before_any_request(), test_send_webhook_rejects_non_http_scheme()

### Community 307 - "test_ephemeral_reaper.py"
Cohesion: 0.14
Nodes (19): _company_id_for_agent(), Delete registered specialist agents whose owning company no longer exists.…, Return the company id an auto-provisioned specialist belongs to, else None.…, reap_orphaned_agents(), _company(), asyncio, Tests for services.ephemeral_reaper — destroy expired ephemeral companies. Runs…, A company-lookup failure is treated as 'still exists' — no deletion. (+11 more)
...
Cohesion: 0.06
Nodes (24): _append_agent_session_message(), _build_auto_skill_guidance(), _run_agent_job(), _in_container(), _mask_observations(), Path, Truncate tool/observation content in older messages to prevent context bloat., Return True when running inside a container runtime. ``/.dockerenv`` alone… (+16 more)

### Community 315 - "openclaw_gateway.py"
Cohesion: 0.11
Nodes (25): openclaw_reverse_proxy(), openclaw_websocket(), api_route, websocket, WebSocket endpoint for iOS / mobile web UI pairing + command routing., Catch-all for /openclaw/* — returns gateway info (the WS endpoint is at…, _cmd_chat(), _cmd_freebuff() (+17 more)
...
Cohesion: 0.07
Nodes (41): delegation_task_to_initiative(), Convert a SeoDelegationTask into a portfolio Initiative. The WSJF components…, Fixed, Active Task Tracker, Roadmap Items (from `docs/roadmap-killer-todos.md`), Status Key, Fixed, _auth_headers() (+33 more)

### Community 320 - "Fixed"
Cohesion: 0.02
Nodes (104): ceo_status(), redrive_goal(), run_sweep(), backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), _is_admin(), Check if a user has admin role. Works for both social_auth users (role in… (+96 more)
...
### Community 326 - "list_agents"
Cohesion: 0.16
Nodes (21): _apply_activity_status(), create_agent(), delete_agent(), get_agent(), _get_user(), list_agents(), Any, delete (+13 more)

### Community 327 - "High-Agency Frontend Skill"
Cohesion: 0.06
Nodes (30): 10. FINAL PRE-FLIGHT CHECK, 1. ACTIVE BASELINE CONFIGURATION, 2. DEFAULT ARCHITECTURE & CONVENTIONS, 3. DESIGN ENGINEERING DIRECTIVES (Bias Correction), 4. CREATIVE PROACTIVITY (Anti-Slop Implementation), 5. PERFORMANCE GUARDRAILS, 6. TECHNICAL REFERENCE (Dial Definitions), 7. AI TELLS (Forbidden Patterns) (+22 more)

### Community 328 - "get_store"
Cohesion: 0.08
Nodes (36): _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Consolidated system health report: preflight checks + runtime health. Returns a…, Public Doctor endpoint — no authentication required. Returns system-level…, Authenticated Doctor endpoint — full diagnostics. Returns all system-level… (+28 more)
...
### Community 330 - "mcp_server/server.py"
Cohesion: 0.11
...
### Community 341 - "SupervisorConfig"
Cohesion: 0.20
Nodes (10): _env_flag(), _env_int(), Read a bounded int, clamping out-of-range values rather than raising., Bounds for the unattended supervision loop., True when the 24x7 supervisor should run in this process. Off under ``TESTING``…, supervisor_enabled(), SupervisorConfig, Unit tests must never start a background sweeper. (+2 more)

### Community 342 - "SeoAuditReport"
Cohesion: 0.07
Nodes (43): Run a full SEO/GEO/AIO audit against a website and persist the evidence., run_seo_audit(), html, io, Complete result of one audit run., SeoAuditReport, Paragraph, reportlab_lib (+35 more)
...
### Community 344 - "_hydrate_scheduler_bounded"
Cohesion: 0.09
...
Cohesion: 0.12
Nodes (29): _doctor(), _P, _patch_providers(), asyncio, Tests for the public brain-availability diagnosis and the supervisor's use of…, Minimal provider stand-in matching what the summary reads., Wrongly pausing every goal is worse than one wasted re-drive., The core fix: an outage must not burn the backlog's intervention budget. (+21 more)

### Community 346 - "test_ceo_router.py"
Cohesion: 0.09
Nodes (37): build_ceo_router(), get_goal(), list_goals(), Any, APIRouter, Drop the singleton (test helper)., reset_ceo_ledger(), Install (or clear) the singleton — used by startup wiring and tests. (+29 more)
...
Cohesion: 0.17
Nodes (11): _clear_cache(), _CollWithEstimate, _CollWithoutEstimate, asyncio, tests/test_dashboard_cache.py — unit tests for the dashboard hot-path helpers…, test_cached_is_single_flight(), test_cached_recomputes_after_expiry(), test_cached_serves_within_ttl() (+3 more)

### Community 354 - "main"
Cohesion: 0.15
Nodes (13): format_violations(), Build the follow-up message asking the model to fix its own output., Render the Quality Gate section appended to a shipped document., repair_instruction(), _build_context_doc(), _build_user_message(), _enforce_rulebook(), _extract_url() (+5 more)
...
### Community 361 - "start_runtime"
Cohesion: 0.05
Nodes (46): (1) & partly (4): "Something went wrong" masks the real error everywhere, (2) & (3): Company creation flow / non-admin gate placement, (4): Agent provisioning "loading forever" — blocking subprocess in async path, A0. Fix live scanner crashes on real-world sites (`services/scanner.py`) — do first, Agent Prompt (paste this to start the implementation session), B. Make runtime activation non-blocking (`runtimes/control.py`,, Context — bugs reported during manual QA, E. General bug sweep (+38 more)
...
### Community 370 - "FeatureUnavailableError"
Cohesion: 0.08
Nodes (17): Added, Added, 18. Feature Maturity & Support Matrix, FeatureEntry, FeatureUnavailableError, Any, BaseModel, Exception (+9 more)
...
Cohesion: 0.01
Nodes (413): DirectChatDoctor, PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, repowise.py — RepowiseIntelligence: context packing and dependency analysis., tools.py — WorkspaceTools: read/write/search and diff application (risky…, detect_secrets() (+405 more)
...
### Community 383 - "build_planning_prompt"
Cohesion: 0.16
Nodes (17): build_compaction_prompt(), build_execution_prompt(), build_planning_prompt(), build_verification_prompt(), Any, prompts.py — Prompt builders for planner, executor, and verifier roles., tests/test_prompt_injection_boundary.py — untrusted-content trust boundary.…, The Executor is told observations are DATA, not instructions. (+9 more)
...
Cohesion: 0.23
Nodes (11): provider_max_rpm(), Return the operator-configured requests/min cap for *provider*, or None if…, Selection weight for *provider_id* (see ``_weighted_shuffle``)., tests/test_provider_max_rpm.py — packages/ai/brain_config.provider_max_rpm().…, inf previously parsed successfully and passed a bare `> 0` check, producing a…, test_infinite_and_nan_return_none(), test_non_numeric_returns_none(), test_provider_id_uppercased_for_lookup() (+3 more)
...
Cohesion: 0.16
Nodes (5): _client(), Regression: the admin routes and the ops loop share this singleton. Without the…, A handshake that raised must be retried, not remembered as done., TestRenderMCPClient, _flaky_initialize()
...
### Community 401 - "note_phase_start"
Cohesion: 0.15
...
Cohesion: 0.02
Nodes (113): True if browser automation is enabled and Playwright is importable., Adding a new tool, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Spawn a child AgentRunner for a delegated sub-task. When sub-agent configs are…, Perform MCP handshake. Optional — tools/call works without it., build_tool_prompt(), Precise string replacement — F1 roadmap item (Codebuff/Claude Code-style edit).…, _load_script_module() (+105 more)
...
Cohesion: 0.08
Nodes (42): ArgumentParser, build_parser(), cmd_autostart_install(), cmd_status(), cmd_supervise(), cmd_wait(), Namespace, Block until download completes + colibri answers /v1/models. (+34 more)
...
### Community 416 - "CLAUDE.md — Operating Manual"
Cohesion: 0.17
...
### Community 417 - "test_motor_event_loop_isolation.py"
Cohesion: 0.50
...
### Community 421 - "urllib_parse"
Cohesion: 0.05
Nodes (44): date, caption_tracks(), extract_player_response(), fetch_transcript(), _get(), is_video_url(), parse_json3(), parse_timedtext_xml() (+36 more)
...
### Community 425 - "_seed_requires_approval_task"
Cohesion: 0.10
Nodes (23): _approve_execution_via_rest(), _delete_task(), _extract_admin_token(), _login_admin(), _looks_like_admin_token(), _open_dashboard(), _poll_task_execution_approved(), Any (+15 more)
...
### Community 433 - "Testing Analysis — local-llm-server"
Cohesion: 0.13
...
### Community 439 - "Native operations"
Cohesion: 0.09
...
### Community 440 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 447 - "cost_aware_routing/__init__.py"
Cohesion: 0.15
Nodes (15): Cost-aware routing evaluation harness. Measures the cost-aware subagent routing…, ModelPrice, Model pricing for the cost-aware routing evaluation. Rates are first-party…, Per-token price for one model, in USD per 1,000,000 tokens., Override the price for a tier (e.g. a discounted or partner rate)., set_price(), Caveats, EvalTask (+7 more)
...
### Community 449 - "control_registry.py"
Cohesion: 0.18
...
Cohesion: 0.12
Nodes (16): adaptive *(default)*, automatic_failover, context_length_optimized, cost_optimized, fallback_chain, highest_success_rate, least_loaded, lowest_latency (+8 more)
...
### Community 454 - "test_provider_410_per_model.py"
Cohesion: 0.18
Nodes (15): _dead_model_key(), _is_model_dead(), Return True if (provider, model) is on the dead list and not yet expired., _nvidia(), anyio, Regression tests: 410 Gone is a per-model condition, not per-provider.…, Once _DEAD_MODEL_COOLDOWN_SECONDS elapses, the model is re-probed., When every candidate 410s, the provider is cooled and failover proceeds. (+7 more)
...
### Community 458 - "services/background.py"
Cohesion: 0.07
Nodes (37): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, ctypes, ctypes_util, _await_hermes_ready(), _env_on(), Any, services/background.py — shared background-service startup. Both the FastAPI… (+29 more)
...
### Community 467 - "register_user_research_tools"
Cohesion: 0.18
...
### Community 470 - "OperationalIncidentTracker"
Cohesion: 0.10
...
Cohesion: 0.26
Nodes (16): aggregate_last_24h(), build_daily_digest(), compute_cutoff(), format_digest_markdown(), _md_escape(), _now_utc(), Any, datetime (+8 more)
...
### Community 482 - "test_anthropic_refusal_fallback.py"
Cohesion: 0.19
...
### Community 500 - "Fixed"
Cohesion: 0.04
Nodes (58): Send chat messages and return the assistant's text output. If an…, Atomically delete a run-once row only while it is still unfired. Closes the…, FailureCategory, E2: Classify a failure from its description text. Order matters:…, Classified failure types for targeted self-healing (E2)., ma(), Fixed, Fixed (+50 more)
...
### Community 507 - "4. Threats"
Cohesion: 0.05
Nodes (41): Propagate the calling agent's identity across the process boundary. The MCP…, 1. Which sandbox backend applies where, 2. Container hardening, 3. Supply chain, 4. Troubleshooting, 5. Scaling, Agents suddenly failing after enabling enforcement, An agent is stuck (+33 more)
...
### Community 509 - "Added"
Cohesion: 0.02
Nodes (119): Build the complete enrichment block (tools + skills). Returns empty string when…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, Tool Parameters, Added, Tool Parameters, Added, 1. Executive summary, 3. Risk assessment (+111 more)

### Community 510 - "TestTheCopiesMayNotDriftFurther"
Cohesion: 0.15
...
### Community 528 - "SchedulesScreen.jsx"
Cohesion: 0.17
...
### Community 538 - "Fixed"
Cohesion: 0.02
Nodes (99): Normalise a raw planner JSON response into a consistent plan dict. Handles: -…, Fixed, CreateIssue, Fixed, Bulkheads, Circuit breaker, Configuration, Context management (+91 more)
...
Cohesion: 0.15
Nodes (16): Short label for TUI display: {NAME}:{model}, generate_onboarding_questions(), _get_fallback_questions(), OnboardingQuestionsRequest, Request to generate AI-tailored onboarding questions., Generate AI-tailored onboarding questions based on detected domain and…, Return hardcoded fallback questions when AI generation fails., _bootstrap_within_budget() (+8 more)
...
### Community 563 - "operational_incidents.py"
Cohesion: 0.06
Nodes (44): Best-effort: hand an operational failure to the incident tracker. Import is…, _record_operational(), _diagnose_and_file(), _file_incident(), _format_incident(), gather_render_evidence(), get_operational_incident_tracker(), OperationalIncident (+36 more)
...
### Community 574 - "webui/router.py"
Cohesion: 0.03
Nodes (84): dotenv, fastapi_responses, fastapi_staticfiles, BaseModel, post, Autonomous AI Agency Launcher - One-button service start with web UI. Run this…, start_proxy(), start_tunnel() (+76 more)
...
### Community 579 - "test_company_api.py"
Cohesion: 0.11
...
### Community 595 - "ephemeral_reaper.py"
Cohesion: 0.20
Nodes (13): _as_aware_utc(), _company_alive(), _env_float(), ephemeral_reaper_loop(), datetime, services/ephemeral_reaper.py — destroy expired ephemeral companies. The…, Whether *company_id* still exists, cached per sweep. A lookup error returns…, Parse a positive, finite float env var (seconds), else the default. Rejects… (+5 more)
...
### Community 598 - "ai/router.py"
Cohesion: 0.08
Nodes (34): Record token usage for *model* (fire-and-forget, never raises). ``tag`` is a…, record_usage(), _exponential_backoff_cooldown(), _ollama_reasoning_effort(), provider_router.py — auto-generated module docstring (user-research skill scan)., # NOTE: 429 and 410 are handled specially in _try_one_provider (immediate, Return the configured ``OLLAMA_REASONING_EFFORT`` (``high``/``medium``/…, Compute exponential backoff cooldown for repeated 429 failures. Each… (+26 more)
...
### Community 607 - "test_log_emits_timestamped_line"
Cohesion: 0.24
Nodes (22): Maintenance, Maintenance, Maintenance, Maintenance, Maintenance, Maintenance, Maintenance, Path (+14 more)

### Community 608 - "_RedisBackend"
Cohesion: 0.22
...
Cohesion: 0.04
Nodes (39): Beta, Config Overrides, Disabled (demoted per issue #467 Section I), Enforcement, Experimental, Feature Maturity / Support Matrix, Maturity Tiers, Stable Core (+31 more)
...
Cohesion: 0.10
Nodes (18): svc(), tests/test_langfuse_agency_wide.py — tests for PR #961 agency-wide Langfuse.…, langfuse_obs.py must define emit_agency_observation., emit_agency_observation must be a no-op when Langfuse is not configured., tasks/service.py must call emit_agency_observation for task execution., agent/agency.py must call emit_agency_observation for CEO directives., backend/server.py scheduler_tick must call emit_agency_observation., packages/ai/self_heal.py must call emit_agency_observation. (+10 more)
...
### Community 673 - "ChatHistoryStore"
Cohesion: 0.22
...
### Community 677 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 683 - "workflow/api.py"
Cohesion: 0.17
Nodes (11): _engine(), workflow/api.py — FastAPI router for all /workflow/* endpoints. All 13…, get_engine(), Return the shared WorkflowEngine singleton (lazy-init)., workflow — CRISPY deterministic workflow engine. Lifecycle: request → context →…, Request body for POST /workflow/{id}/approve., Request body for POST /workflow/{id}/reject., Request body for POST /workflow/{id}/slices/{slice_id}/run. (+3 more)
...
Cohesion: 0.20
Nodes (11): _build_payload_or_500(), preview_digest_endpoint(), Any, get, post, Dry-run: same auth, returns the would-be markdown body but does NOT dispatch to…, Mount this router on a FastAPI app. Idempotent., Returns a DigestPayload (or raises HTTPException(503/500)). (+3 more)
...
### Community 699 - "Implementation Plan — DB-persisted, UI-switchable Brain (no redeploy)"
Cohesion: 0.15
...
### Community 701 - "runsio.py"
Cohesion: 0.24
Nodes (10): _money(), Load recorded runs and render the comparison report. A runs file is JSON with…, Render a `compare()` result as a fixed-width text table., render_report(), _row(), Scoring for the cost-aware routing evaluation. The headline metric is **cost…, Aggregate metrics for one workflow over a suite of task runs., SuiteResult (+2 more)
...
### Community 704 - "Added"
Cohesion: 0.07
Nodes (34): Any, Force the next ``list_tools()`` call to fetch a fresh tool list from the…, Added, Added, A worked example, Adding capacity: multi-key rotation, Attribution, Configuration (+26 more)
...
### Community 709 - "TestTheProducersAndTheParserCannotDrift"
Cohesion: 0.18
Nodes (7): _build_grounding_block(), _build_pr_description(), _build_todos_md(), Render the Source Grounding table — rulebook R1. A reader must be able to tell…, Render TODOs with their done-conditions — rulebook R4., The markers are matched against strings this repo generates elsewhere. A hand-…, TestTheProducersAndTheParserCannotDrift

### Community 710 - "_build_anthropic_response"
Cohesion: 0.32
...
### Community 717 - "test_openclaw_gateway.py"
Cohesion: 0.14
...
### Community 718 - "test_bootstrap_indexes_are_created_concurrently"
Cohesion: 0.29
...
### Community 755 - "._remove_worktree"
Cohesion: 0.33
...
### Community 767 - "make_isolated_workspace"
Cohesion: 0.33
...
### Community 777 - "test_procedural_memory.py"
Cohesion: 0.22
...
### Community 788 - "Next-Pass Roadmap — Detailed Implementation Specs"
Cohesion: 0.33
...
### Community 790 - "GovernanceScreen.jsx"
Cohesion: 0.18
...
### Community 823 - "AUTONOMY_CHARTER.md"
Cohesion: 0.12
...
### Community 827 - "Autonomy Charter — Telegram-Gated Self-Running Agency"
Cohesion: 0.14
...
### Community 828 - "test_hermes_server.py"
Cohesion: 0.18
...
Cohesion: 0.20
Nodes (9): Architecture, Deployment on Render, Environment variables, First-time setup, Graceful shutdown, Local development, Troubleshooting, Verifying the worker is alive (+1 more)
...
### Community 844 - "test_agent_readiness_audit.py"
Cohesion: 0.18
...
### Community 858 - "report_for_file"
Cohesion: 0.22
Nodes (10): main(), load_runs(), _parse_runs(), Path, Parse a runs JSON file into ``{"baseline": [...], "routed": [...]}``., Load a runs file and return its rendered comparison report., report_for_file(), test_example_runs_score_and_routing_is_cheaper() (+2 more)
...
### Community 881 - "_FakePersistence"
Cohesion: 0.20
...
### Community 882 - "TestModelsEndpointAliases"
Cohesion: 0.31
...
### Community 884 - "_extract_keywords"
Cohesion: 0.25
...
### Community 893 - "test_server_autonomy_and_index_fixes.py"
Cohesion: 0.20
...
### Community 916 - "PR #634 Implementation Tracker"
Cohesion: 0.20
...
### Community 923 - "test_scanner_headless.py"
Cohesion: 0.28
...
### Community 946 - "TestHarnessRegistry"
Cohesion: 0.22
...
### Community 966 - "Free NVIDIA brain + UI-controlled provider policy + no silent spend"
Cohesion: 0.15
...
### Community 994 - "research_coordinator.py"
Cohesion: 0.36
...
### Community 999 - "_start_in_web_bot_tasks"
Cohesion: 0.29
...
Cohesion: 0.08
Nodes (49): AgentRole, get_agency(), _parse_ceo_directives(), Enum, str, set_agency(), set_improvement_loop(), Configuration (+41 more)

### Community 1003 - "AlertsBell.jsx"
Cohesion: 0.32
...
### Community 1009 - "test_shared_state.py"
Cohesion: 0.32
...
### Community 1010 - "test_job_failure_structures_runtime_preflight"
Cohesion: 0.29
...
### Community 1014 - "test_generate_context_standing_instructions.py"
Cohesion: 0.32
...
### Community 1019 - "test_runtimes_health_endpoint.py"
Cohesion: 0.33
...
### Community 1027 - "test_p0_roadmap_implementation.py"
Cohesion: 0.43
...
Cohesion: 0.22
Nodes (10): [5.0.0], Changed, Removed, Security, [5.0.0], Changed, Removed, Security (+2 more)
...
### Community 1050 - "reset_decisions_store_singleton"
Cohesion: 0.29
Nodes (6): Test-only: clears the cached singleton so the next get_decisions_store() builds…, reset_decisions_store_singleton(), _fresh_stores(), Any, _tmp_db_path(), _fresh_store()
...
### Community 1056 - "TaskRun"
Cohesion: 0.20
Nodes (8): CLI, Cost-aware routing evaluation, How the metric resists gaming, Method, What's here, One task carried end to end under a given workflow. ``accepted`` is the…, Attempts beyond the first — the redo count., TaskRun
...
### Community 1060 - "model_router.py"
Cohesion: 0.05
Nodes (43): F. Router invariants (`router/`), F. Router invariants (`router/`), get_local_model(), Return the local Ollama model name for a given Anthropic model name. ..…, fallback_chain(), Follow the explicit chain in ``routing.yaml``, then everything else. The chain…, Adding a model, Adding a task category (+35 more)

### Community 1062 - "Fixed"
Cohesion: 0.11
Nodes (24): Fixed, Fixed, Fixed, difflib, Fixed, Fixed, Fixed, DetailsStep() (+16 more)

### Community 1063 - "check_doc_images.py"
Cohesion: 0.48
...
Cohesion: 0.02
Nodes (107): Clear director state and the cached strategy warnings (tests only)., reset(), _clear_discovered_models(), _clear_response_cache(), _isolate_operator_provider_state(), MonkeyPatch, Path, Default all tests to legacy workflow mode (Phase 2 compatibility). Only patches… (+99 more)
...
### Community 1096 - "db/__init__.py"
Cohesion: 0.33
Nodes (3): db — storage abstraction layer (V2.0 Phase 5: real code moved to…, # IMPORTANT: keep these imports LAZY (inside __getattr__) so that a Mongo-only, db/mongo_store.py — MongoDB store backed by Motor (existing implementation).…

### Community 1097 - "orchestrator"
Cohesion: 0.33
...
### Community 1104 - "Feature Support Matrix"
Cohesion: 0.40
...
### Community 1120 - "ChatSendRequest"
Cohesion: 0.40
Nodes (5): ChatSendRequest, Send chat message request., Tests for direct chat agent execution beyond planning., Verify ChatSendRequest supports agent mode execution., TestDirectChatAgentExecution
...
### Community 1133 - "test_runtime_registration_scope.py"
Cohesion: 0.33
...
### Community 1177 - "test_repo_access_preflight_fails_when_git_ls_remote_fails"
Cohesion: 0.40
...
### Community 1180 - "test_v5_screens_smoke.py"
Cohesion: 0.50
...
### Community 1181 - "nvidia_live_test.py"
Cohesion: 0.40
...
### Community 1201 - "test_get_provider_policy_returns_default_without_db"
Cohesion: 0.40
Nodes (5): asyncio, When DB is unreachable, _get_provider_policy must return allow_paid=False., Even when a surface explicitly assigns Anthropic, resolve_provider_for must…, test_get_provider_policy_returns_default_without_db(), test_resolve_provider_for_blocks_paid_when_kill_switch_off()
...
### Community 1217 - "_should_fan_out"
Cohesion: 0.50
...
### Community 1218 - "_single_specialist_task"
Cohesion: 0.50
Nodes (4): Build a single-task sub-task list (low-complexity fast path)., _single_specialist_task(), Single task path is a single SpecialistTask with sensible defaults., test_single_specialist_task_defaults()

### Community 1223 - "webhook_secret"
Cohesion: 0.50
...
### Community 1224 - "test_direct_adapter_does_not_bypass"
Cohesion: 0.50
...
- **3066 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3061 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13965 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **203 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AgentRunner` connect `AgentRunner` to `ContextPruner`, `AgentPlan`, `UserMemoryStore`, `os`, `backend/server.py`, `proxy.py`, `Task`, `MultiAgentSwarm`, `GitHubTools`, `workflow_orchestrator.py`, `[5.0.0]`, `resolve_identity`, `Autonomous SDLC Loop (Agency Core, repo-agnostic)`, `[Unreleased]`, `AdaptiveHalter`, `Path`, `Agency Core — Progress & Resume Log`, `Fixed`, `ReactScratchpad`, `TestDeprecationWarnings`, `failover_chat_completion`, `brain_failover.py`, `E2BSandboxSession`, `ImprovementLoop`, `TestInternalAgentAdapterProviderChain`, `TestAgentLoopMCPIntegration`, `brain_config.py`, `test_fabric_patterns.py`, `ContextManager`, `SpecialistService`, `Added`, `test_governance_api.py`, `AgentJobRequest`, `Changed`, `WorkflowOrchestrator`, `test_backend_server_features.py`, `Agent Autonomy Roadmap`, `Fixed`, `FreeBuffAgent`, `StuckDetector`, `CEODispatcher`, `user_research_skill.py`, `WorkspaceTools`, `test_empirical_verify.py`, `Section-by-Section Acceptance Criteria`, `LocalWorkspace`, `test_repo_connection.py`, `get_ceo_dispatcher`, `TokenBudget`, `test_verification_strategies.py`, `AgentSessionStore`, `SECTION A — Agent Efficiency (Hermes / AOS / MYT)`, `UserInfo`, `Competitor Analysis — Autonomous AI Agency`, `Fixed`, `pytest`, `Added`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `ma()` connect `Fixed` to `la`, `ImprovementLoop`, `de`, `gsap.min.js`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `_fixture()` connect `_fixture` to `gateway.py`, `backend/server.py`, `TestAuthAndTaskOwnership`, `os`, `test_llm_router_queue_cache.py`, `Task`, `wiki_client`, `config.py`, `test_llm_router_resilience.py`, `UsageEvent`, `resolve_identity`, `SelfHealingAgent`, `test_model_router.py`, `test_harness_spec.py`, `test_e2b_sandbox.py`, `TestNormalizeResponseFormat`, `CompanyGraphService`, `Fixed`, `TestTeamSummary`, `_start_autonomy_loops`, `Agency`, `heal_signature`, `failover_chat_completion`, `ImprovementLoop`, `strategies.py`, `test_autonomy_status.py`, `brain_config.py`, `AgentRunner`, `test_freebuff_bot.py`, `router_factory`, `PrimeAgentAdapter`, `Settings`, `secrets_store.py`, `AgentSwarm`, `_clean_phases`, `operational_incidents.py`, `services/seo_audit.py`, `Changed`, `WorkflowOrchestrator`, `process_note`, `SeoFixRequest`, `test_startup_warmup.py`, `test_company_api.py`, `build_governance_router`, `test_sam_livekit.py`, `orchestrator`, `test_provider_router.py`, `analyze_page`, `TestClient`, `ProviderRouter`, `e2e/test_browser.py`, `test_knowledge_sync.py`, `resolve_e2b_config`, `OllamaCircuitBreaker`, `TestHarnessAdapter`, `467 Brutal Audit — File-by-File Status`, `SQLiteStore`, `test_vision_routing.py`, `test_langfuse_agency_wide.py`, `TestChatHistoryStore`, `persist_plan_spec`, `test_refresh_agent_built_proof.py`, `test_context_rulebook.py`, `ArtifactStore`, `test_llm_router_e2e.py`, `test_trend_watcher.py`, `test_new_features_e2e.py`, `TestEstimateTokensForMessages`, `test_kimi_bridge_server.py`, `test_issue_intake.py`, `WorkflowRun`, `clear_cooldowns`, `brain_failover.py`, `ChatHistoryStore`, `test_key_pool.py`, `routed`, `controls_app`, `test_doctor_service_token_check.py`, `FeatureMaturity`, `TestRuntimeControl`, `parse_event_stream`, `LLMRouter`, `KeyStore`, `test_governance_api.py`, `looks_like_secret_file`, `NotificationDispatcher`, `_job_text`, `test_video_transcript.py`, `test_direct_chat_evolution.py`, `_FakeFiles`, `hermes_enabled`, `test_openclaw_gateway.py`, `test_bootstrap_indexes_are_created_concurrently`, `TestAuthAndTaskCreation`, `ScheduledJob`, `test_features_api.py`, `test_cost_attribution.py`, `_reset_brain_singletons`, `test_purge_backlog.py`, `TrendWatcher`, `TestNormalizeAnthropicOutputFormat`, `test_telegram_webhook.py`, `test_control_plane_api.py`, `ContextPruner`, `test_procedural_memory.py`, `test_daily_2026_07_27.py`, `test_webui_provider_priority.py`, `test_persistent_memory.py`, `test_provider_enable_disable.py`, `test_sam_voice.py`, `SecurityScanner`, `test_brain_failover.py`, `test_agile_api.py`, `test_app_settings.py`, `test_nvidia_model_discovery.py`, `OutputFilter`, `test_task_clarification.py`, `test_rate_limiter.py`, `Added`, `test_ephemeral_reaper.py`, `OrchestratorCheckpointStore`, `get_store`, `test_session_retro.py`, `TestCountTokensEndpoint`, `test_skill_registry_boot_refresh.py`, `test_ceo_router.py`, `_make_task`, `test_dashboard_cache.py`, `test_phase5_doctor.py`, `TestTheWorkflowIsSafeAndReadOnly`, `TestMongoGate`, `pytest`, `test_executive_advisory_api.py`, `model_discovery.py`, `_undeclared`, `TestSwarmRoleRouting`, `test_process_quick_note_workflow.py`, `TestChatFallbackAndApproval`, `_build_execution_request`, `_get`, `Path`, `test_task_service_failed_comment.py`, `OperationalIncidentTracker`, `LocalBrainStore`, `get_ceo_dispatcher`, `_mock_provider_records`, `Changelog`, `test_agency.py`, `test_shared_state.py`, `test_doctor_coding_brain.py`, `Fixed`, `TestAgentAutoAssignment`, `test_ping.py`, `Added`, `captured`?**
...
- **Should `gateway.py` be split into smaller, more focused modules?**
  _Cohesion score 0.035247524752475244 - nodes in this community are weakly interconnected._
- **Should `backend/server.py` be split into smaller, more focused modules?**
  _Cohesion score 0.010918107701095706 - nodes in this community are weakly interconnected._
