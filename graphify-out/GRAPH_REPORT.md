...
## Corpus Check
- 1304 files · ~1,807,142 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 24672 nodes · 49884 edges · 1127 communities (985 shown, 142 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 6001 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ff5b39c3`
...
## Communities (1127 total, 142 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.02
Nodes (204): get_harness_adapter(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), auth_me(), auto_recommend_skills(), brain_failover_status(), _build_agent_status_snapshot() (+196 more)

### Community 1 - "ExecutionRequest"
Cohesion: 0.04
Nodes (77): ClassifyOutput, get_ceo_fallback_stats(), PlanOutput, Determine domain and task type from the request text., Generate a structured execution plan.          Uses the LLM if available; falls, Select the best specialist(s) for the classified domain., CLASSIFY phase: domain and task type determination., PLAN phase: structured execution plan. (+69 more)

### Community 2 - "AgentSessionStore"
Cohesion: 0.04
Nodes (178): Agency, CEO-coordinated multi-agent agency for continuous codebase management.      The, AgentJobRequest, AgentJobSnapshot, BaseModel, Complete point-in-time view of a job, safe to serialise as API response., Convenience: the canonical text response, whether success or failure., Validated input for creating a new agent job.      Passed from the API handler (+170 more)

### Community 3 - "AgentRunner"
Cohesion: 0.02
Nodes (116): AgentRunner, _nvidia_api_key(), Any, Path, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it.          Asks the planner model to writ, Send chat messages and return the assistant's text output.          If an Anthro, Call-time resolver for an agent role model id.      Delegates to ``services.brai (+108 more)

### Community 4 - "FreeBuffAgent"
Cohesion: 0.05
Nodes (148): BackgroundAgent, BackgroundTask, _now(), Any, Enqueue *task* for processing. Returns the task (with task_id set)., Convenience: create a task and submit it in one call., Real handler — dispatches through AgentRunner when available.          HARDENE, Always-on worker that drains a task queue on a daemon thread.      GATE: Golde (+140 more)

### Community 5 - "proxy.py"
Cohesion: 0.02
Nodes (189): agent/background.py — Background Agent  An always-on worker thread that proces, Any, Return recent commits with agent attribution trailers parsed out., get_tracker(), Return the shared AutonomyTracker singleton (lazy-init)., free_nvidia_models(), Return the curated list of free NVIDIA NIM models FreeBuff may use., List the free NVIDIA NIM models a user may pick (e.g. via Telegram). (+181 more)
...
Cohesion: 0.06
Nodes (40): AgentPlan, AgentStep, InternalAgentAdapter, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter.                  Returns:, Determine availability of the internal agent runtime.          Checks cloud prov, clean_store(), _clear_overrides() (+32 more)

### Community 8 - "ai/router.py"
Cohesion: 0.02
Nodes (136): _acquire_provider_probe(), clear_cooldowns(), _dead_model_key(), _exponential_backoff_cooldown(), get_dead_models(), is_anthropic_base_url(), is_commercial_provider(), _is_model_dead() (+128 more)
...
Cohesion: 0.04
Nodes (152): AccountLifecycleResponse, _DoctorCheck, _DoctorReport, OnboardingAnswersRequest, OnboardingProgressResponse, OnboardingQuestionsRequest, BaseModel, Request to generate AI-tailored onboarding questions. (+144 more)

### Community 11 - "CompanyGraphStore"
Cohesion: 0.03
Nodes (30): Delete a company and all its associated data., Delete a company and all associated data from SQLite., Count total companies in SQLite., Count total companies in the store., Create a new website in SQLite. The full model is stored in ``data``         so, Update a website in SQLite. When ``company_id`` is omitted the existing, Delete a website from SQLite., Create a new repository in SQLite. (+22 more)
...
Cohesion: 0.03
Nodes (137): account_lifecycle(), auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), generate_onboarding_questions(), get_company(), get_company_access() (+129 more)

### Community 14 - "test_unit8_model_catalog.py"
Cohesion: 0.04
Nodes (70): get_catalog_models_route(), Return the mirrored model catalog (advisory-only).      Flag-gated by ``FREELLM_, all_provider_ids(), _provider_ids_from_literal(), provider_key_present(), Return every provider id recognised by the brain config system.      Iterates th, True when the env var for *provider*'s key is set (or it's Ollama)., Return the provider ids allowed by the ``BrainProvider`` Literal.      Reads the (+62 more)

### Community 15 - "brain_config.py"
Cohesion: 0.04
Nodes (86): BrainConfig, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), BaseModel, services/brain_config_store.py — DB-persisted, UI-switchable "brain" config.  Im (+78 more)
...
Cohesion: 0.05
Nodes (100): build_subtask_brief(), _coerce_subtasks(), decompose(), _env_flag(), _env_int(), _extract_json_object(), fallback_decomposition(), get_config() (+92 more)
...
Cohesion: 0.03
Nodes (60): CompanyGraphSnapshot, A point-in-time snapshot of a Company Graph for history and rollback., MongoDBStore, Any, Company, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model. (+52 more)
...
Cohesion: 0.03
Nodes (86): Status of a user story within a sprint., StoryStatus, HarnessAdapter, HarnessSpec, Any, agents/harness_adapter.py — ECC Cross-Harness Adapter  Normalises API difference, Adapt harness-native requests to the local-llm-server internal format.      Each, Detect which harness sent this request from headers.          Check order: expli (+78 more)
...
Cohesion: 0.04
Nodes (44): Static definition of a single audit check (catalog entry)., SeoCheckDefinition, auto_fixable_checks(), _c(), get_check(), list_checks(), services/seo_checks.py - SEO / GEO / AIO Check Catalog  The authoritative catalo, Return the catalog definition for a check code (raises KeyError if unknown). (+36 more)

### Community 26 - "CEODispatcher"
Cohesion: 0.04
Nodes (66): Semaphore, CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), _merge_changed_files(), _offload(), Any (+58 more)
...
Cohesion: 0.07
Nodes (35): BaseModel, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., SeoIssueInstance, SeoIssueReportRow, SeoPageAudit (+27 more)
...
Cohesion: 0.04
Nodes (91): BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), _norm(), _pick_from_records() (+83 more)
...
Cohesion: 0.04
Nodes (77): MCPUnavailableError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.  T, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any, services/e2b_sandbox.py — E2B Firecracker micro-VM sandbox session.  Implements (+69 more)

### Community 38 - "TaskStore"
Cohesion: 0.03
Nodes (107): agent/workflow.py — Persisted workflow state machine.  Implements the Autonomo, agents/store.py — Persistent store for user-defined agent configurations.  Store, quick_notes_submit(), Submit a quick-note URL or instruction from the dashboard FAB., packages/ai/self_heal.py — automatic brain self-healing.  When the active brain, Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system.  Provides a lightweight task/issue tracker (+99 more)

### Community 39 - "TaskWorkflowService"
Cohesion: 0.04
Nodes (49): _is_brain_connection_error(), Any, BaseException, Task, TaskResult, TaskSpec, TaskStatus, Best-effort label of who will run this task once approved.          Returns ``ta (+41 more)
...
Cohesion: 0.09
Nodes (18): analyze_page(), _count_syllables(), estimate_pixel_width(), flesch_reading_ease(), normalize_url(), Approximate SERP rendering width of ``text`` in pixels., Flesch reading-ease score; None when there is too little text to score., Resolve and normalize a URL: absolute, no fragment, no default port. (+10 more)
...
Cohesion: 0.05
Nodes (74): e2b_status(), _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy(), Any (+66 more)
...
Cohesion: 0.04
Nodes (38): BusinessCategory, requires_db, CompanyGraphService, Company, SpecialistFamily, SystemType, Workflow, Get a company by ID.                  Args:             company_id: Company ID (+30 more)
...
Cohesion: 0.06
Nodes (73): clear_wizard_state_cache(), complete_wizard(), _delete_wizard_state(), detect_configured_providers(), detect_hardware_for_wizard(), detect_models_for_wizard(), _detect_ollama_models(), get_setup_state() (+65 more)
...
Cohesion: 0.05
Nodes (52): Reset the singleton (for tests)., reset_failover_manager(), _heal_brain_failover(), _heal_stuck_tasks(), _heal_task_duplicates(), _heal_telegram(), _heal_timestamps(), Any (+44 more)

### Community 59 - "Task"
Cohesion: 0.05
Nodes (69): Set the global agent store instance (e.g., with MongoDB on startup)., set_agent_store(), Hand the 'connect & verify the repo' work to the agency's own agents.      The t, _seed_connect_task(), ApprovalCheckpoint, Full task/issue document., Human approval gate in a task's execution., Task (+61 more)
...
Cohesion: 0.05
Nodes (67): Called by APScheduler when a cron fires. Dispatches to the orchestrator., _scheduler_on_fire(), BoundContext, emit_deprecation(), ExecutionResult, get_workflow_orchestrator(), JudgeVerdict, MonitorOutput (+59 more)

### Community 83 - "SeoFixer"
Cohesion: 0.08
Nodes (23): Request to remediate auto-fixable findings in a local code repository., One concrete remediation performed (or proposed) by the fixer., SeoFixAction, SeoFixRequest, _humanize_filename(), BeautifulSoup, Path, services/seo_fixer.py - Repo-Aware SEO Auto-Fixer  When a company has a code rep (+15 more)

### Community 84 - "StreamingDeltaReconstructor"
Cohesion: 0.06
Nodes (28): PostProcessHook, get_chat_history(), Return the module-level ChatHistoryStore singleton., get_guardrails(), Return the module-level GuardrailEngine singleton., create_streaming_reconstructor(), DeltaChunk, Any (+20 more)
...
Cohesion: 0.05
Nodes (65): _parse_ceo_directives(), DetectedIssue, ImprovementLoop, ImprovementLoopState, _now(), Any, Path, Background scanner and task dispatcher for continuous codebase improvement. (+57 more)
...
Cohesion: 0.06
Nodes (42): E2BAdapter, Any, Declare ``E2B_API_KEY`` as a required env dependency.          The base ``prefli, Runtime adapter that executes tasks inside an E2B sandbox.      Activation: regi, Available iff config resolves AND the SDK is importable.          Never raises —, PolicyUpdateBody, BaseModel, RunTaskBody (+34 more)
...
Cohesion: 0.05
Nodes (19): harness_session_close(), harness_session_start(), Register a new harness session (called by the orchestrator on execute)., Close a harness session and aggregate its metrics., get_harness_registry(), HarnessMetrics, HarnessRegistry, HarnessSessionRecord (+11 more)
...
Cohesion: 0.05
Nodes (25): AgileManager, Manages multiple agile sprints with velocity tracking., List all active sprints., Predict next sprint velocity from historical data., Number of managed sprints., CapacityAllocation, InitiativeProgress, PortfolioMetrics (+17 more)

### Community 119 - "FeatureMatrix"
Cohesion: 0.02
Nodes (71): check_feature(), get_feature(), list_features(), Any, get, post, Return the full support matrix with summary., Return a single feature entry. (+63 more)

### Community 120 - "failover_client.py"
Cohesion: 0.05
Nodes (40): _auto_disable(), _Budget, _build_request(), _describe_registry(), _disabled_ids(), FailoverResult, _is_billing_refusal(), _is_ollama() (+32 more)

### Community 121 - "ModelRouter"
Cohesion: 0.06
Nodes (35): Dynamic model router package.  Public API::      from router import get_router,, _build_builtin_model_map(), _default_model(), _default_reasoning_model(), ModelRouter, _nvidia_key_present(), Any, Dynamic model router.  Central routing logic for all chat and agent requests.  E (+27 more)

### Community 122 - "tests/conftest.py"
Cohesion: 0.06
Nodes (51): get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., Item, app_client(), _clear_discovered_models(), _clear_response_cache(), client() (+43 more)
...
Cohesion: 0.08
Nodes (52): approve(), build(), cancel(), _engine(), get_agent_team(), get_artifact_content(), get_events(), get_run() (+44 more)
...
Cohesion: 0.08
Nodes (50): activate_instance(), ActivateRequest, ActivateResponse, activation_audit_log(), activation_status(), ActivationStatusResponse, _append_audit(), AuditLogEntry (+42 more)
...
Cohesion: 0.05
Nodes (56): _check_background_liveness(), _check_ci_parity(), _check_company_graph(), _check_disk(), _check_event_log_integrity(), _check_feature_matrix(), _check_github_readiness(), _check_ollama() (+48 more)
...
Cohesion: 0.07
Nodes (42): _close_github_issue(), _fetch_github_quick_notes(), get_agency(), _gh_repo(), _gh_token(), Enum, agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered)  Runs the re, Return the GitHub repo in 'owner/name' format.      Priority:     1. GITHUB_REPO (+34 more)
...
Cohesion: 0.04
Nodes (64): _bullets(), generate_backlog_retro(), generate_sprint_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning).  Where, Render a :class:`Retrospective` as a markdown section., Derive retro notes for ``sprint`` from its current metrics.      Records observa (+56 more)
...
Cohesion: 0.08
Nodes (21): ContextWindowManager, get_context_window_manager(), Any, Enum, Return True if the estimated tokens exceed the model's context limit., Truncate messages to fit within the model's context window.          Args:, Return the context window size for a model.          Looks up the model in the c, Estimate token count for a list of messages.          Uses a character-based heu (+13 more)
...
Cohesion: 0.09
Nodes (23): _FakeClient, asyncio, fixture, Tests for agent/trend_watcher.py, Ensure expanded keyword set covers key new categories., setup_database_moks(), test_fetch_arxiv(), test_fetch_github_trending() (+15 more)

### Community 178 - "_get_provider_policy"
Cohesion: 0.06
Nodes (42): _get_provider_policy(), get_provider_policy_route(), Read the durable provider policy from DB, falling back to a safe default.      R, Persist the durable provider policy and return the new state.      Also writes t, Return the provider policy (paid-provider kill switch state)., seed_default_providers(), _set_provider_policy(), Resolve the LLM endpoint for a named surface (task/chat/ceo/sdlc/…).      Honour (+34 more)
...
Cohesion: 0.06
Nodes (46): get_repo(), _get_token(), _get_user(), GitHubTools, init_workspace(), list_branches(), list_prs(), list_repos() (+38 more)
...
### Community 208 - "audit"
Cohesion: 0.08
Nodes (22): audit(), get_audit_log(), Any, Request, FastAPI dependency: require any authenticated user., Append an audit log entry.      Never logs raw secrets — only secret IDs / maske, Return recent audit log entries, newest first.      Supports filtering by user_i, require_authenticated() (+14 more)
...
Cohesion: 0.08
Nodes (32): Create an isolated execution context for a single task.          Tries ``git wor, Clean up the worktree or temp copy created by ``_create_worktree``., _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., _make_task(), asyncio, Task, TaskStatus (+24 more)
...
Cohesion: 0.04
Nodes (74): get_enrichment(), agent/harness_enrichment.py — Automatic Harness Enrichment for Agent Prompts  Ev, Agent subsystem — planner / executor / verifier loop., AgentPhaseError, _check_extra_kwargs(), _enforce_signature(), Exception, loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures. (+66 more)
...
Cohesion: 0.05
Nodes (25): PortfolioManager, Manages a portfolio of initiatives with WSJF prioritisation and roadmapping., Create and register a new initiative, returning it., Add a pre-built Initiative (e.g. from the intelligence layer)., Remove an initiative from the portfolio., Look up an initiative by ID., Number of initiatives in the portfolio., Return initiatives sorted by WSJF (highest first).          Cancelled initiative (+17 more)
...
Cohesion: 0.06
Nodes (38): backend/ceo_router.py — observability and manual control for the CEO.  Surfaces, Reject non-admin callers for routes that spend provider budget.      Delegates t, _require_admin(), get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., reset_ceo_dispatcher(), get_ceo_ledger() (+30 more)
...
Cohesion: 0.04
Nodes (65): brain_providers(), get_public_doctor(), Every configured provider with its health AND its on/off state.      Powers the, Turn a provider on or off. Manual re-enable clears an auto-disable.      Auto-di, Public Doctor endpoint — no authentication required.      Returns system-level d, set_brain_provider_enabled(), get_store(), Return the active store singleton, creating it on first call. (+57 more)
...
Cohesion: 0.08
Nodes (12): count() returns 0 for an empty store., count() reflects the number of saved jobs., count() decreases after delete., delete_stale() keeps jobs updated recently., delete_stale() removes jobs with old updated_at., delete_stale() reads SCHEDULER_JOB_RETENTION_DAYS from env., delete_stale() returns 0 when all jobs are recent., Explicit retention_days arg takes precedence over env var. (+4 more)
...
Cohesion: 0.10
Nodes (16): codeql_count(), dependabot_count(), main(), Any, _repo_parts(), _request(), OrchestratorSupervisor, Any (+8 more)
...
Cohesion: 0.09
Nodes (36): autonomy_status(), Public autonomy readiness probe — no authentication required.      A live deploy, Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap(), get_company_agency_service(), Get the singleton CompanyAgency service instance., _count_specialists(), _create_company_directly() (+28 more)
...
Cohesion: 0.07
Nodes (12): AgentSwarm, Any, Return the agent role responsible for *phase*., Return a JSON-serialisable summary of all agent profiles., Raise PermissionError if the profile lacks required permissions., Coordinates a team of specialised agents for a WorkflowRun.      Parameters, PermissionError, fixture (+4 more)
...
Cohesion: 0.14
Nodes (23): _as_bool(), _as_int(), ephemeral_ttl_hours(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled(), onboarding_gate_enabled_cached() (+15 more)
...
Cohesion: 0.07
Nodes (24): CompanyAgencyService, _is_runtime_available_sync(), _pick_available_runtime(), Any, SpecialistFamily, services/company_agency.py — Company Agency Orchestration Service  After onboard, Orchestrates specialist activation, runtime startup, and 24x7 scheduling     for, Return the best available runtime for a specialist family.          Checks avail (+16 more)
...
Cohesion: 0.09
Nodes (14): Any, Add a step result. Returns the sample if accepted, None if filtered out., Bulk-add samples from an agent session's step results.          Each step result, Return samples filtered by minimum reward score., Export samples in Alpaca JSONL format.          Returns the path to the exported, Export samples in ShareGPT JSONL format.          Returns the path to the export, Export all samples as a structured JSON array.          Returns the path to the, Return pipeline statistics. (+6 more)
...
Cohesion: 0.13
Nodes (22): get_doctor_report(), Execute work through the 11-phase golden path., Return the caller's GitHub token from EITHER place it can be stored.      A toke, Consolidated system health report: preflight checks + runtime health.      Retur, _resolve_user_github_token(), workflow_orchestrator_execute(), _FakeCollection, _FakeDB (+14 more)
...
Cohesion: 0.11
Nodes (12): get_nim_pool(), NIMConnectionPool, AsyncClient, Persistent httpx.AsyncClient pool with circuit breaker and retry logic.      Man, Get or create the shared httpx.AsyncClient., Context manager for a pooled client session., Close the connection pool., Manually reset a provider's circuit breaker to CLOSED. (+4 more)
...
### Community 317 - "_get"
Cohesion: 0.19
Nodes (6): _get(), Contract tests for the provider on/off endpoints.  ``GET /api/brain/providers``, The operator has to know WHY before deciding to switch it back on.      The raw, The response reaches the browser — a leaked key would be a disclosure., TestDisabledReasonIsReadableNextToTheSwitch, TestListing
...
Cohesion: 0.12
Nodes (21): _cached(), _fast_count(), get_activity(), _get_activity_impl(), Any, Single-flight TTL cache. Concurrent callers wait for the first producer., Count without materialising rows — prefers estimated_document_count., _token_response() (+13 more)
...
Cohesion: 0.09
Nodes (32): IssueCategory, IssueSeverity, Enum, str, agent/improvement_loop.py — Continuous Improvement Engine  Background scanner th, HealState, Enum, str (+24 more)
...
Cohesion: 0.16
Nodes (22): _doctor(), _P, _patch_providers(), Tests for the public brain-availability diagnosis and the supervisor's use of it, Minimal provider stand-in matching what the summary reads., Wrongly pausing every goal is worse than one wasted re-drive., The remedies differ: one needs a human, the other recovers itself., This is served unauthenticated — no keys, no raw provider error text. (+14 more)
...
Cohesion: 0.08
Nodes (25): get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager)  SAM is the voice-contr, tests/test_sam_voice.py — Integration tests for SAM voice agent.  Tests the SAM, Same session_id must return the same session., SAM's system prompt must address the user as Commander., SAM's system prompt must instruct concise responses., get_sam() must return the same instance., get_status() must return uptime and session count. (+17 more)
...
Cohesion: 0.18
Nodes (24): models/seo_audit.py - SEO / GEO / AIO Audit Contracts  Typed Pydantic models for, An agent-delegable remediation work package derived from the findings.      Find, Complete result of one audit run., SeoAuditReport, SeoDelegationTask, Paragraph, _appendix_full_findings(), _appendix_worst_pages() (+16 more)
...
Cohesion: 0.15
Nodes (12): api_client(), _ollama_reachable(), _override_user(), fixture, skipif, tests/test_workflow_orchestrator_scoping.py — Phase 8 multi-tenant isolation.  R, Force backend.server.get_current_user to return ``user``., A non-admin's auto_approve=true is ignored — the run still pauses at HITL. (+4 more)
...
Cohesion: 0.04
Nodes (95): admin_seed(), cancel_chat_agent_job(), clear_response_cache(), create_api_key(), create_github_pr(), create_mcp_server(), create_provider(), create_wiki_page() (+87 more)
...
Cohesion: 0.08
Nodes (22): provider_base_url(), Resolve the Ollama base URL the UI controls — DB value wins over env.      Prece, Return the OpenAI-compatible base URL for *provider* (env- and UI-aware)., resolve_ollama_base_url(), Any, _FakeResp, fixture, tests/test_brain_ollama_base_url.py  The Ollama base URL is UI-configurable and (+14 more)

### Community 646 - "get_livekit_config"
Cohesion: 0.20
Nodes (9): Drop the cached Mongo client, backoff, and read caches (tests)., reset_kv_state(), one_configured_provider(), fixture, A registry with exactly two known providers and isolated state., fake_mongo(), isolated_state(), fixture (+1 more)
...
Cohesion: 0.25
Nodes (3): Silently storing a typo'd id would leave a switch nothing can turn back on., The switch has to reach the dispatcher, not just the listing., TestSwitch
...
Cohesion: 0.33
Nodes (6): _brain_provider_status(), get_brain_policy_route(), Return per-provider metadata for the GET endpoint.      Iterates every provider, Return the discovered model list for *provider_id*, or ``[]`` if unknown., Return the active brain config + per-provider key-present flags.      The respon, _served_models()
...
### Community 993 - ".create_branch_compat"
Cohesion: 0.67
Nodes (3): Configuration reference, Provider priority chain, Running the brain on local Ollama (via a tunnel)

## Knowledge Gaps
- **3219 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `aider_config.sh script`, `OPENAI_API_BASE` (+3214 more)
...
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `ExecutionRequest`, `AgentSessionStore`, `_resolve_push_token`, `FreeBuffAgent`, `proxy.py`, `AdaptiveHalter`, `RuntimeHealth`, `WorkspaceTools`, `ai/router.py`, `diagnostics.py`, `direct_chat.py`, `Initiative`, `CEODispatcher`, `OrchestratorQueue`, `TestZeroAttemptDiagnostics`, `E2BSandboxSession`, `RewardScorer`, `ContextPruner`, `RuntimeReadinessReport`, `MultiAgentSwarm`, `MCPClient`, `test_failover_client_shared.py`, `test_agency.py`, `GitHubTools`, `test_self_heal_v2.py`, `test_tasks_awaiting_approval_api.py`, `TokenBudget`, `AgentStatusPanel.jsx`, `RuntimeCapabilityRegistry`, `.on_task_complete`, `ContextManager`, `MCPUnavailableError`, `_StubManager`, `Workspace`, `DetectedSystem`, `openclaw_mobile_ui`, `._connect`, `StuckDetector`, `PortfolioManager`, `ReactScratchpad`, `failover_client.py`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `AgentSessionStore` to `backend/server.py`, `test_all_providers_discovery.py`, `resolve_active_brain`, `HealthStatus`, `TestAnthropicPayloadStructuredOutput`, `ai/router.py`, `test_anthropic_router.py`, `test_colibri_brain_shim.py`, `direct_chat.py`, `test_bedrock_provider.py`, `failover_client.py`, `system_instruction`, `test_bedrock_live.py`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `AgentScheduler` connect `AgentSessionStore` to `backend/server.py`, `FreeBuffAgent`, `proxy.py`, `test_autonomy_pipeline_regressions.py`, `SchedulerStore`, `_FakePersistence`, `TaskStore`, `.discover_directory`, `get_scheduler`, `ScheduledJob`, `Task`, `TestSupportMatrixDocsSync`, `test_purge_backlog.py`, `get_self_healing_agent`, `Feature Support Matrix`, `clear_wizard_state_cache`, `test_telegram_task_callback.py`, `TestSchedulerStore`, `test_schedule_persistence.py`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
