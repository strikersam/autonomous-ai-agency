...
## Corpus Check
- 1436 files · ~2,058,615 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28501 nodes · 53251 edges · 1210 communities (1098 shown, 112 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 3663 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `625acc52`
...
## Communities (1210 total, 112 thin omitted)

### Community 0 - "WorkflowRun"
Cohesion: 0.04
Nodes (43): MergeDecision, _NoopStore, Phase, Any, Enum, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs… (+35 more)

### Community 1 - "TestRuntimeControl"
Cohesion: 0.04
...
Cohesion: 0.02
Nodes (117): int, Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key() (+109 more)
...
Cohesion: 0.08
Nodes (29): _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog(), ProviderAttempt (+21 more)

### Community 5 - "company_api.py"
Cohesion: 0.02
Nodes (174): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+166 more)

### Community 6 - "TestDetectedSystemPersistence"
Cohesion: 0.08
...
Cohesion: 0.03
Nodes (102): packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, tasks/models.py — Pydantic models for the task/issue system., Comment or reply on a task. (+94 more)

### Community 8 - "brain_config.py"
Cohesion: 0.03
Nodes (99): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store() (+91 more)
...
Cohesion: 0.05
Nodes (80): count, Strategy selection and degradation behaviour., RoutingConfig, Build and order the (provider, model) pairs that can serve ``request``., Cap the breadth of one request's search so a failure is bounded., adaptive(), automatic_failover(), Candidate (+72 more)

### Community 15 - "llm/router.py"
Cohesion: 0.03
Nodes (91): _load(), main(), Backoff policy for retryable failures., RetryConfig, packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, packages/llm/keys.py — multi-key rotation with per-key health. A provider may…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error() (+83 more)
...
Cohesion: 0.04
Nodes (76): build_docker_run_argv(), detect_backend(), DockerBackend, E2BBackend, get_sandbox_manager(), load_profiles(), LocalBackend, Any (+68 more)
...
Cohesion: 0.03
Nodes (64): HealthConfig, Circuit breaker + health tracking thresholds., BreakerState, HealthTracker, _Outcome, ProviderHealth, Any, Enum (+56 more)
...
### Community 25 - "Task"
Cohesion: 0.04
Nodes (71): Full task/issue document., Task, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, asyncio, The reconciler must not re-queue tasks parked at the pre-execution gate.…, A reject + human Retry starts a NEW approval cycle → fresh notification., A gated task parked while IN_PROGRESS must not be treated as stranded. (+63 more)
...
Cohesion: 0.04
Nodes (107): CEOResult, Aggregated output from a multi-specialist execution., BoundContext, ClassifyOutput, ExecutionResult, get_ceo_fallback_stats(), JudgeVerdict, MonitorOutput (+99 more)

### Community 29 - "resolve_identity"
Cohesion: 0.04
Nodes (98): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, backend/governance_router.py — read and operate the governance layer. Mounted…, Reject non-admin callers. Mirrors the RBAC check used elsewhere in this backend…, _require_admin(), guard(), _load() (+90 more)
...
Cohesion: 0.04
Nodes (77): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+69 more)

### Community 32 - "E2BSandboxSession"
Cohesion: 0.03
Nodes (86): MCPUnavailableError, Raised when the MCP server is unreachable or the circuit is open., TaskResult, TaskSpec, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, E2BSandboxSession, _inject_token() (+78 more)
...
Cohesion: 0.04
Nodes (77): all_provider_ids(), _provider_ids_from_literal(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the…, True when the env var for *provider*'s key is set (or it's Ollama)., Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, CatalogActiveBrain, CatalogMirror (+69 more)
...
Cohesion: 0.04
Nodes (86): DetectedIssue, get_improvement_loop(), ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity, _now(), Any (+78 more)
...
### Community 49 - ".start_onboarding"
Cohesion: 0.08
Nodes (13): Workflow, Start the onboarding process for a company. Args: company_id: Company ID…, Schedule a fire-and-forget background task, keeping a strong reference.…, Force-refresh the dynamic skill registry and log tech-stack recommendations.…, Activate a company's 24x7 agency runtimes in the background. Runs after…, Get the current onboarding progress for a company. Args: company_id: Company ID…, Resume onboarding from where it left off. Args: company_id: Company ID Returns:…, Pause onboarding for a company (sets status to paused). Args: company_id:… (+5 more)

### Community 50 - "CompanyGraphStore"
Cohesion: 0.01
Nodes (124): BusinessCategory, BusinessSystem, CompanyGraph, KnowledgeItem, SpecialistFamily, SystemType, Get the index of a phase in the workflow., The complete Company Graph - canonical core model for the Autonomous AI Agency.… (+116 more)
...
Cohesion: 0.09
Nodes (45): Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost(), cmd_keylist() (+37 more)
...
Cohesion: 0.06
Nodes (69): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, A subtask's full history: what it is, and every attempt at it., SubtaskRecord, _goal(), ledger(), asyncio, Tests for the CEO ledger, the supervised escalation loop, and the 24x7 sweeper.… (+61 more)
...
Cohesion: 0.04
Nodes (32): AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., HermesAdapter, Any, AsyncClient, Response, TaskResult (+24 more)

### Community 61 - "SQLiteStore"
Cohesion: 0.04
Nodes (40): Delete a company and all associated data from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON blob (which…, Create a new website in SQLite. The full model is stored in ``data`` so scan…, Get a website by ID from SQLite., Update a website in SQLite. When ``company_id`` is omitted the existing company…, Delete a website from SQLite., List websites from SQLite. (+32 more)
...
Cohesion: 0.06
Nodes (33): ExecutionRequest, Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY…, reset_orchestrator(), ExecutionRequest exposes worktree_path for #504 worktree isolation., worktree_path is internal — must not leak in model_dump()., test_execution_request_has_worktree_path(), test_execution_request_worktree_path_excluded_from_dump() (+25 more)
...
Cohesion: 0.06
Nodes (46): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), list_specs(), MCPServerSpec, _not_dialable(), _playwright_configured(), _probe_http() (+38 more)
...
Cohesion: 0.05
Nodes (54): failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Router-backed implementation of ``packages.ai.failover_client``'s entry point.…, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on…, Translate an OpenAI-shaped chat payload into an ``LLMRequest``. Unknown keys…, Render an ``LLMResponse`` as an OpenAI chat-completions body. Used by the… (+46 more)

### Community 79 - "ToolRegistry"
Cohesion: 0.05
Nodes (29): Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed., Look up a tool by name., Return all registered tools., Find tools that advertise a specific capability. (+21 more)
...
Cohesion: 0.06
Nodes (29): _best_cloud_primary_base(), InternalAgentAdapter, _nvidia_provider_chain(), ProviderConfig, TaskResult, TaskSpec, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:… (+21 more)
...
### Community 100 - "test_colibri_provider.py"
Cohesion: 0.12
...
Cohesion: 0.05
Nodes (52): BrainFailoverManager, ProviderHealth, ProviderInfo, Any, Permit one probe call without claiming the provider succeeded. This is the…, Seconds until the soonest cooling provider is probeable again. ``None`` when no…, True when a provider's cooldown window is wider than any it could legitimately…, Record a provider failure — opens the circuit breaker on threshold. (+44 more)
...
Cohesion: 0.05
Nodes (48): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, analyze_qualitative(), analyze_quantitative(), auto_register(), _classify_sentiment(), _extract_keywords(), Any (+40 more)

### Community 113 - "GovernanceGate"
Cohesion: 0.04
Nodes (53): events_from_identity(), Any, Convenience: build and record an event in one call., Extract the identity-derived audit fields from an AgentIdentity. Kept here…, record_event(), BudgetExceeded, BudgetTracker, evaluate_call() (+45 more)
...
Cohesion: 0.05
Nodes (56): AgentPhaseError, Exception, Inject available tools + skills catalog into the system message.…, Raised when a named agent phase (planning, verification, etc.) fails., Public wrapper around _generate_plan for callers that want plan-only. Returns…, Normalise a raw planner JSON response into a consistent plan dict. Handles: -…, _make_runner(), MonkeyPatch (+48 more)
...
Cohesion: 0.05
Nodes (32): _append_agent_session_message(), _build_auto_skill_guidance(), _build_provider_router(), _builtin_provider_records(), _chat_provider_policy(), _fallback_local_provider_record(), _list_configured_provider_records(), _mask_observations() (+24 more)

### Community 130 - "get_workflow_orchestrator"
Cohesion: 0.08
Nodes (22): Return orchestrator queue depth, active runs, and supervisor state (#522)., workflow_orchestrator_status(), get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log. (+14 more)
...
### Community 139 - "render_router.py"
Cohesion: 0.13
...
Cohesion: 0.06
Nodes (33): AlertHandler, BudgetTracker, Counter, _Dimensions, get_budget(), _month(), Any, packages/llm/budget.py — token and cost accounting with spend alerts. Tracks… (+25 more)

### Community 147 - "test_governance_api.py"
Cohesion: 0.06
Nodes (50): build_governance_router(), Any, APIRouter, Replace the process-wide gate. Tests only., reset_gate(), _client(), parametrize, TestClient (+42 more)

### Community 148 - "failover_client.py"
Cohesion: 0.04
Nodes (61): _auto_disable(), _build_request(), _describe_registry(), _disabled_ids(), FailoverResult, _is_billing_refusal(), _is_ollama(), _key_pool() (+53 more)
...
Cohesion: 0.05
Nodes (38): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Run a job using the provided runner and update the job's lifecycle, progress,… (+30 more)
...
Cohesion: 0.04
Nodes (56): _disabled_from_mongo(), _disabled_from_sqlite(), disabled_providers(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _mongo_db(), _mongo_enabled() (+48 more)
...
### Community 157 - "test_colibri_brain_shim.py"
Cohesion: 0.08
...
Cohesion: 0.08
Nodes (33): clear_cooldowns(), is_provider_on_cooldown(), mark_provider_failed(), ProviderFallbackError, RuntimeError, Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown., Clear all cooldown entries (useful for testing). Delegates to… (+25 more)
...
Cohesion: 0.05
Nodes (49): AgentRole, Run a pre-gate or report phase through the correct agent. Enforces permission…, Execute a slice via the Coder agent (write-permitted)., Review a slice via the Reviewer agent (different model from Coder). This is the…, Run verification commands via the Verifier agent. The Verifier is execution-…, Return a copy of *routing* with the profile's model injected for its role., Return the AgentProfile for *role*, raising KeyError if unknown., When coder == reviewer model, swarm should log a warning. (+41 more)

### Community 178 - "LLMRouter"
Cohesion: 0.04
Nodes (51): Attempt, auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable. (+43 more)
...
Cohesion: 0.07
Nodes (40): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), CommercialEquivalent, estimate_commercial_equivalent_usd(), get_prices(), _load_from_env() (+32 more)
...
Cohesion: 0.10
Nodes (43): _governance_check(), _governance_identity(), Evaluate a runtime dispatch against policy; audit it either way. Returns…, Resolve the acting identity for a runtime dispatch. Identity comes from…, _decision(), _engine(), Any, RoutingDecision (+35 more)
...
Cohesion: 0.10
Nodes (27): Agent subsystem — planner / executor / verifier loop., AgentPlan, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+19 more)

### Community 188 - "operational_incidents.py"
Cohesion: 0.11
...
Cohesion: 0.10
Nodes (17): fake_sandbox(), _FakeAsyncSandboxClass, _FakeCmdResult, _FakeCommands, _FakeFiles, _FakeSandbox, Any, tests/test_e2b_data_flow.py — FS-consistency + write-back + end-to-end tests.… (+9 more)
...
Cohesion: 0.07
Nodes (42): _get_current_user_thunk(), _get_optional_user_thunk(), Request, create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., Item (+34 more)
...
Cohesion: 0.04
Nodes (78): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., apply_from_db(), Load the stored overrides and apply them. Called once at startup., Path, Start RuntimeManager, TaskDispatcher, SCHEDULER, and self-bootstrap. Returns a…, start_background_services() (+70 more)
...
Cohesion: 0.09
Nodes (13): Company, Any, field_validator, The core company entity - root of the Company Graph., Coerce unrecognised system_type values to 'custom' so the model never crashes…, Test Company Graph Pydantic models., Test that all Company Graph models can be imported., Test Company model creation. (+5 more)
...
Cohesion: 0.06
Nodes (36): AgentEvent, A single entry in the session's append-only event log. Inspired by Anthropic's…, AgentSessionStore, _now(), Connection, Path, Row, Safe getter for sqlite3.Row — Row supports index access but not .get(). (+28 more)
...
### Community 220 - "resolve_component_model"
Cohesion: 0.06
...
Cohesion: 0.04
Nodes (73): ApprovalRequest, ApprovalStatus, ApprovalStore, get_approval_store(), Any, Enum, str, packages/governance/approvals.py — human-in-the-loop for high-risk actions. The… (+65 more)
...
### Community 238 - "timedelta"
Cohesion: 0.18
Nodes (21): Count distinct sessions per user. A session ends when there's a gap of more…, aggregate_last_24h(), build_daily_digest(), compute_cutoff(), DigestSummary, format_digest_markdown(), _md_escape(), _now_utc() (+13 more)
...
Cohesion: 0.10
Nodes (29): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Authenticated Doctor endpoint — full diagnostics. Returns all system-level… (+21 more)
...
### Community 243 - "test_brain_availability_doctor.py"
Cohesion: 0.16
...
### Community 258 - "test_crispy_run_history.py"
Cohesion: 0.17
...
### Community 275 - "audit"
Cohesion: 0.07
Nodes (42): _apply_activity_status(), create_agent(), delete_agent(), get_agent(), _get_user(), list_agents(), list_runtime_agents(), Any (+34 more)
...
Cohesion: 0.02
Nodes (95): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+87 more)
...
### Community 310 - "test_claude_code_adapter.py"
Cohesion: 0.14
...
### Community 333 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 338 - "WorkspaceManager"
Cohesion: 0.09
...
### Community 367 - "ensure_self_company"
Cohesion: 0.04
...
Cohesion: 0.15
Nodes (17): discover_models(), _fresh_entry(), _models_url(), _parse_ids(), Any, Ask a provider which models the configured API key may actually use.…, Return the model ids *provider*'s key may use, or ``None`` if unknown. ``None``…, Cache *models* under *provider_id* and return it unchanged. A successful… (+9 more)
...
### Community 419 - "test_provider_410_per_model.py"
Cohesion: 0.19
...
### Community 462 - "test_crispy_workflow.py"
Cohesion: 0.07
...
### Community 465 - "test_task_service_failed_comment.py"
Cohesion: 0.18
...
### Community 488 - "test_kimi_bridge_server.py"
Cohesion: 0.12
...
### Community 504 - "get_skill_bindings"
Cohesion: 0.10
...
### Community 562 - "telegram_inbound_handlers.py"
Cohesion: 0.13
...
### Community 582 - "get_db"
Cohesion: 0.03
Nodes (111): auth_me(), auto_recommend_skills(), _check_storage_health(), cost_attribution_stats(), create_access_token(), delete_source(), discover_remote_skills(), doctor_health() (+103 more)
...
### Community 603 - "open_phase_report"
Cohesion: 0.14
...
### Community 631 - "TestAnthropicToolListCaching"
Cohesion: 0.34
...
Cohesion: 0.04
Nodes (27): Drop the cached Mongo client, backoff, and read caches (tests)., reset_kv_state(), one_configured_provider(), A registry with exactly two known providers and isolated state., fake_mongo(), _FakeCollection, _FakeDb, isolated_state() (+19 more)
...
### Community 638 - "_mock_provider_records"
Cohesion: 0.14
...
### Community 639 - "ApprovalGate"
Cohesion: 0.14
Nodes (10): Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory, TestApprovalGate (+2 more)

### Community 640 - "GeminiProvider"
Cohesion: 0.30
...
### Community 744 - "has_image_content"
Cohesion: 0.31
...
### Community 774 - "WorkflowPhase"
Cohesion: 0.18
Nodes (13): _append_transition(), BaseModel, Enum, str, agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Advance task through the full phase sequence. Resumes from…, Execute a single phase and advance to the next., Ordered phases of a task's execution lifecycle. (+5 more)

### Community 775 - "ENGINEERING_STANDARDS.md — Patterns & Reference"
Cohesion: 0.22
...
Cohesion: 0.07
Nodes (12): AgentSwarm, Any, Return the agent role responsible for *phase*., Return the AgentProfile for the agent driving *phase*., Return a JSON-serialisable summary of all agent profiles., Raise PermissionError if the profile lacks required permissions., Coordinates a team of specialised agents for a WorkflowRun. Parameters…, PermissionError (+4 more)
...
### Community 817 - "_is_ephemeral_user"
Cohesion: 0.27
...
### Community 820 - "TestAuthAndTaskOwnership"
Cohesion: 0.17
...
Cohesion: 0.02
Nodes (193): admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, BrainTestRequest (+185 more)
...
### Community 894 - "Agent Autonomy Roadmap"
Cohesion: 0.25
...
### Community 909 - "Feature Maturity / Support Matrix"
Cohesion: 0.25
...
### Community 937 - "_autonomy_bg_cycle"
Cohesion: 0.10
Nodes (23): _fetch_github_quick_notes(), _gh_repo(), _gh_token(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, _autonomy_bg_cycle(), autonomy_status(), Background CEO cycle + task dispatch. Runs fire-and-forget. (+15 more)
...
### Community 987 - "test_doctor_coding_brain.py"
Cohesion: 0.38
...
### Community 988 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 1027 - "ProviderConfig"
Cohesion: 0.04
Nodes (83): _acquire_provider_probe(), extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig (+75 more)

### Community 1028 - "SpecialistFamily"
Cohesion: 0.18
...
### Community 1034 - "Who is this for?"
Cohesion: 0.40
...
### Community 1036 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.40
...
### Community 1193 - "_brain_provider_status"
Cohesion: 0.25
...
### Community 1195 - "_load_local_metrics_since"
Cohesion: 0.43
...
### Community 1200 - "_merge_changed_files"
Cohesion: 0.33
...
- **Why does `_fixture()` connect `_fixture` to `TestRuntimeControl`, `test_llm_router_queue_cache.py`, `ProviderConfig`, `company_api.py`, `TaskStore`, `brain_config.py`, `TaskWorkflowService`, `llm/router.py`, `SelfHealingAgent`, `test_openclaw_endpoints.py`, `test_llm_router_resilience.py`, `test_orchestrator_merge_decision.py`, `config.py`, `Task`, `resolve_identity`, `Agency`, `services/background.py`, `E2BSandboxSession`, `test_failover_client_shared.py`, `test_unit8_model_catalog.py`, `SeoAuditEngine`, `ImprovementLoop`, `UserRole`, `test_agent_tool_governance.py`, `SQLiteStore`, `NotificationDispatcher`, `telegram_inbound_handlers.py`, `test_model_router.py`, `test_autonomy_status.py`, `PrimeAgentAdapter`, `test_local_brain_state.py`, `test_ceo_supervision.py`, `test_telegram_diag_endpoint.py`, `ChatHistoryStore`, `ExecutionRequest`, `TokenBudget`, `BackgroundAgent`, `ResearchTask`, `gateway.py`, `TestClient`, `e2e/test_browser.py`, `test_control_plane_api.py`, `test_brain_failover_413.py`, `test_procedural_memory.py`, `claim`, `ArtifactStore`, `test_startup_warmup.py`, `KeyPool`, `test_brain_failover.py`, `KeyStore`, `TestHarnessAdapter`, `parse_event_stream`, `BrainWatchdog`, `GovernanceGate`, `test_context_rulebook.py`, `_StubProvider`, `run_task`, `test_provider_state_durability.py`, `test_openclaw_gateway.py`, `_mock_provider_records`, `test_issue_intake.py`, `direct_chat.py`, `test_new_features_e2e.py`, `AgentJobManager`, `MCPClient`, `_Budget`, `brain_failover.py`, `persist_plan_spec`, `clear_cooldowns`, `test_daily_2026_07_27.py`, `OllamaCircuitBreaker`, `test_ceo_router.py`, `_clean_phases`, `emit_chat_observation`, `FeatureMaturity`, `sam`, `test_sam_livekit.py`, `test_features_api.py`, `test_video_transcript.py`, `test_agile_api.py`, `test_app_settings.py`, `test_task_clarification.py`, `test_e2b_data_flow.py`, `facade.py`, `get_task_store`, `CEOLedger`, `TrendWatcher`, `test_trend_watcher.py`, `test_brain_patch_service_token.py`, `test_governance_enforcement.py`, `test_portfolio_intake.py`, `TestEstimateTokensForMessages`, `TestClient`, `ContextPruner`, `get_skill_registry_safe`, `OrchestratorQueue`, `test_sam_voice.py`, `ScheduledJob`, `test_persistent_memory.py`, `SecurityScanner`, `test_crispy_run_history.py`, `test_llm_router_e2e.py`, `OutputFilter`, `test_platform_controls.py`, `TestAgentRunnerSafety`, `.record`, `test_purge_backlog.py`, `_start_ceo_agency`, `AgentSwarm`, `analyze_page`, `TestAuthAndTaskOwnership`, `test_claude_code_adapter.py`, `test_crispy_burn_in.py`, `test_webui_provider_priority.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `reap_expired_companies`, `test_rate_limiter.py`, `test_phase5_doctor.py`, `SeoFixer`, `api_keys_for`, `capture_screens.py`, `test_service_token.py`, `test_harness_spec.py`, `test_runtimes_health_endpoint.py`, `test_task_store_fails_loud_in_production.py`, `test_regression.py`, `test_agency_fix.py`, `test_telegram_mutating_commands.py`, `test_dashboard_cache.py`, `platform_controls_router.py`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `test_ai_insights.py`, `get_ceo_dispatcher`, `test_provider_410_per_model.py`, `test_knowledge_sync.py`, `client`, `allow_paid`, `test_ping.py`, `test_voice_pipeline.py`, `test_crispy_workflow.py`, `provider_max_rpm`, `test_task_service_failed_comment.py`, `cost_tracker.py`, `TestNormalizeResponseFormat`, `test_doctor_coding_brain.py`, `test_kimi_bridge_server.py`, `test_company_api.py`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `WorkflowRun`, `test_backend_server_features.py`, `LocalWorkspace`, `proxy.py`, `direct_chat.py`, `ReactScratchpad`, `TestAgentRunnerSafety`, `ContextManager`, `TaskSpec`, `test_e2b_adapter.py`, `test_daily_automation_2026_07_11.py`, `test_governance_api.py`, `AgentJobManager`, `MCPClient`, `test_ceo_dispatcher.py`, `Agency`, `E2BSandboxSession`, `StuckDetector`, `test_failover_client_shared.py`, `get_ceo_dispatcher`, `CEODispatcher`, `MultiAgentSwarm`, `test_agent_tool_governance.py`, `WorkspaceTools`, `AgentPlan`, `backend/server.py`, `TokenBudget`, `BackgroundAgent`, `UserMemoryStore`, `test_brain_failover_413.py`, `GitHubTools`, `AgentSessionStore`, `InternalAgentAdapter`, `test_agent_free_brain.py`, `test_empirical_verify.py`, `ContextPruner`, `loop.py`, `TestAgentLoopMCPIntegration`, `AdaptiveHalter`, `test_agent_runner.py`, `FreeBuffAgent`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `get_tool_registry()` connect `test_user_research_skill.py` to `SpecEntry`, `loop.py`, `AgentRunner`, `test_agent_tool_governance.py`, `ToolRegistry`, `WorkspaceTools`, `test_p0_roadmap_b1_c2_a3.py`, `HarnessEnrichment`, `AgentPlan`?**
...
- **Should `WorkflowRun` be split into smaller, more focused modules?**
  _Cohesion score 0.036935704514363885 - nodes in this community are weakly interconnected._