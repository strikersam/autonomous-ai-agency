...
## Corpus Check
- 1477 files · ~2,157,087 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30130 nodes · 55820 edges · 1283 communities (1109 shown, 118 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2730 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b27291f5`
...
## Communities (1283 total, 118 thin omitted)
...
Cohesion: 0.06
Nodes (38): build_tool_calling_router(), Return a router limited to providers that pass ``tools`` through intact. The…, is_commercial_provider(), _normalized_provider_type(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig (+30 more)

### Community 4 - "backend/server.py"
Cohesion: 0.02
Nodes (150): _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, BrainTestRequest, _build_agent_status_snapshot() (+142 more)

### Community 5 - "proxy.py"
Cohesion: 0.03
Nodes (157): middleware, admin_control(), admin_delete_user(), admin_login(), admin_logout(), admin_rotate_user(), admin_status(), AdminControlBody (+149 more)
...
Cohesion: 0.03
Nodes (105): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag. (+97 more)

### Community 8 - "PolicyEngine"
Cohesion: 0.03
Nodes (88): _action_matches(), _as_list(), GroupPolicy, _host_matches(), Mode, _normalise_path(), _path_matches(), PolicyEngine (+80 more)
...
### Community 12 - "HTTPException"
Cohesion: 0.03
Nodes (104): Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, admin_seed(), clear_cost_attribution(), clear_response_cache(), create_access_token() (+96 more)
...
Cohesion: 0.03
Nodes (79): AgentDefinition, AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+71 more)
...
### Community 17 - "get_db"
Cohesion: 0.03
Nodes (90): auth_me(), auto_recommend_skills(), _check_storage_health(), cost_attribution_stats(), delete_github_token(), discover_remote_skills(), doctor_health(), get_active_provider() (+82 more)
...
Cohesion: 0.02
Nodes (84): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+76 more)
...
Cohesion: 0.05
Nodes (104): _complexity_rank(), services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, build_subtask_brief(), _coerce_subtasks(), decompose(), _env_flag(), _env_int() (+96 more)

### Community 22 - "get_company_graph_store"
Cohesion: 0.03
Nodes (112): account_lifecycle(), auto_recommend_skills(), cancel_onboarding(), delete_company_endpoint(), generate_onboarding_questions(), get_company(), get_company_access(), get_company_graph() (+104 more)

### Community 23 - "Task"
Cohesion: 0.03
Nodes (78): ApprovalCheckpoint, _coerce_ts(), Any, field_validator, Full task/issue document., Update the updated_at timestamp., Coerce ISO-8601 datetime strings (from DB) to float timestamps., Human approval gate in a task's execution. (+70 more)

### Community 24 - "SQLiteStore"
Cohesion: 0.04
Nodes (41): Delete a company and all associated data from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON blob (which…, Create a new website in SQLite. The full model is stored in ``data`` so scan…, Get a website by ID from SQLite., Update a website in SQLite. When ``company_id`` is omitted the existing company…, Delete a website from SQLite., List websites from SQLite. (+33 more)

### Community 25 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (80): BusinessCategory, Find a website by its URL., Find a repository by its URL., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo (+72 more)
...
Cohesion: 0.03
Nodes (72): DirectChatDoctor, PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, AgentJob, AgentJobManager, make_isolated_workspace() (+64 more)
...
Cohesion: 0.03
Nodes (79): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, Force a catalog mirror rebuild + persist (admin only). Rebuilds the catalog…, sync_catalog_route(), all_provider_ids(), _provider_ids_from_literal(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the… (+71 more)

### Community 30 - "test_governance_enforcement.py"
Cohesion: 0.03
Nodes (113): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, BudgetTracker, classify(), GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass. (+105 more)
...
Cohesion: 0.05
Nodes (80): Agency, AgentRole, get_agency(), _parse_ceo_directives(), str, CEO-coordinated multi-agent agency for continuous codebase management. The CEO…, set_agency(), DetectedIssue (+72 more)

### Community 33 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.06
...
Cohesion: 0.02
Nodes (113): DetectedSystem, Evidence, Evidence supporting a system detection., A business system detected on a company's website or in their stack., Get the most confident evidence description., Inferred technology stack from website/repo analysis., StackInference, main() (+105 more)
...
Cohesion: 0.04
Nodes (72): Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Feed LLM token/cost spend into the governance session budget. Kept separate…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, guard(), identity_from_headers(), _load(), Any, mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes… (+64 more)

### Community 42 - "AgentRunner"
Cohesion: 0.02
Nodes (118): AgentPhaseError, AgentRunner, Any, Exception, Path, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a…, Send chat messages and return the assistant's text output. If an… (+110 more)
...
### Community 45 - "Slice"
Cohesion: 0.08
Nodes (20): AgentRole, TestSlice, _extract_slices_from_plan(), Extract slice definitions from a plan.md artifact. Looks for sections matching:…, Any, An independently deliverable unit of work within the execute phase. Each slice…, Return the Slice with *slice_id*, or None., Slice (+12 more)
...
### Community 48 - "test_brain_availability_doctor.py"
Cohesion: 0.04
Nodes (72): build_ceo_router(), Any, APIRouter, backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, brain_availability_summary(), Non-secret answer to "can the brain answer a request right now?". Three callers…, get_ceo_dispatcher(), Return the shared CEODispatcher singleton. (+64 more)
...
Cohesion: 0.04
Nodes (58): Execute work through the 11-phase golden path., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_execute(), OrchestratorSupervisor, Any, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.… (+50 more)

### Community 52 - "analyze_quantitative"
Cohesion: 0.09
...
Cohesion: 0.06
Nodes (61): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState, FailedJob (+53 more)
...
### Community 62 - "test_backend_server_features.py"
Cohesion: 0.06
Nodes (29): _agent_provider_failure_response(), _append_agent_session_message(), _builtin_provider_records(), _in_container(), _is_index_options_conflict(), _mask_observations(), Exception, Path (+21 more)

### Community 63 - "loop.py"
Cohesion: 0.11
Nodes (28): _check_extra_kwargs(), _enforce_signature(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Condense a tool result down to what belongs in an audit row. A tool result can…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, _summarise_tool_result() (+20 more)
...
### Community 66 - "mint_access_token"
Cohesion: 0.18
...
Cohesion: 0.07
Nodes (32): repowise.py — RepowiseIntelligence: context packing and dependency analysis., Build git intelligence: hotspots, ownership, co-change pairs., Run a git command and return stdout as string., Compute cyclomatic complexity for Python files. Returns 0 for non-Python files…, Get the latest commit hash., Extract architectural decisions from git history and inline comments., Get the last commit hash we processed., Check if we need to update intelligence based on new commits. (+24 more)
...
Cohesion: 0.06
Nodes (45): _dead_model_key(), _exponential_backoff_cooldown(), get_dead_models(), _get_director(), _is_model_dead(), _mark_model_dead(), _notify_watchdog(), provider_router.py — auto-generated module docstring (user-research skill scan). (+37 more)

### Community 70 - "test_ceo_supervision.py"
Cohesion: 0.04
Nodes (96): CEODispatcher, _harvest_changed_files(), Any, Semaphore, Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask., Real CEO delegation: wake runtimes, fan out, and merge results., Record the goal and its sub-tasks in the ledger before any work runs. Writing… (+88 more)
...
Cohesion: 0.04
Nodes (59): _auto_disable(), _build_request(), _describe_registry(), _disabled_ids(), FailoverResult, _is_billing_refusal(), _is_ollama(), _key_pool() (+51 more)
...
Cohesion: 0.05
Nodes (68): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., Reset the singleton (for tests)., reset_failover_manager(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec() (+60 more)
...
Cohesion: 0.04
Nodes (93): AccountLifecycleResponse, create_company(), _DoctorCheck, _DoctorReport, _get_current_user_thunk(), _get_optional_user_thunk(), get_public_doctor_report(), OnboardingAnswersRequest (+85 more)
...
Cohesion: 0.05
Nodes (54): _acquire_provider_probe(), CommercialFallbackRequiredError, extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), ProviderFallbackError, RuntimeError, Try to acquire a distributed probe lock for *provider_id*. Returns True if this… (+46 more)
...
Cohesion: 0.07
Nodes (48): autonomy_tick(), Execute ONE pending task synchronously. Called by the cron workflow every 2…, _capability_tags(), create_task_from_oldest_open_issue(), intake_issue(), _issue_labels(), issue_source_id(), map_issue_to_task() (+40 more)
...
Cohesion: 0.05
Nodes (34): ProviderRouter, Priority-ordered LLM provider fallback with health checks and retries., main(), probe_one(), ProviderRouter, _bedrock_api_response(), _bedrock_provider(), _mock_boto3() (+26 more)
...
Cohesion: 0.06
Nodes (61): _audit_dispatch(), _blocked_result(), _governance_check(), _governance_identity(), Any, TaskResult, TaskSpec, Build the failed TaskResult returned when policy denies a dispatch. A result… (+53 more)
...
Cohesion: 0.09
Nodes (34): _fetch_text(), _now(), process_note(), Any, Path, QuickNote, QuickNoteQueue, agent/quick_note.py — iPhone Quick Note integration. Persistent URL queue +… (+26 more)
...
### Community 219 - "operational_incidents.py"
Cohesion: 0.11
...
### Community 234 - "get_orchestrator_queue"
Cohesion: 0.21
...
Cohesion: 0.06
Nodes (48): ContextManager, Any, True when the history is long enough to warrant compaction., Replace the old portion of *history* with a single compaction note. The…, True when the harness should use head_file instead of read_file. When a file is…, Trim a step result so sub-agent outputs stay within ~1-2k tokens. The Anthropic…, Manages context window state for a single agent run. The Brain (LLM) stays…, Return a copy of *observations* with old tool outputs truncated. JetBrains… (+40 more)
...
### Community 271 - "start_background_services"
Cohesion: 0.05
...
Cohesion: 0.06
Nodes (20): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), get_cooldown_state(), Return a snapshot of active cooldowns {provider_id: expiry_unix_timestamp}., AdminAuthManager, AdminIdentity (+12 more)
...
Cohesion: 0.05
Nodes (49): create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., Item, create_access_token(), create_refresh_token(), get_current_user() (+41 more)
...
### Community 298 - ".update_intelligence"
Cohesion: 0.08
...
### Community 304 - "test_brain_priority_scanner.py"
Cohesion: 0.10
...
### Community 312 - "get_skill_bindings"
Cohesion: 0.09
...
Cohesion: 0.11
Nodes (14): _mock_provider_records(), Non-Ollama providers without an API key are skipped., Ollama-type providers don't need an API key., When _list_configured_provider_records raises, falls back to OLLAMA_BASE., When all configured providers are excluded, falls back to Ollama., Return an AsyncMock that returns the given provider list., Unit tests for the module-level _resolve_brain_provider function., AGENT_LLM_BASE_URL overrides all configured providers. (+6 more)
...
Cohesion: 0.05
Nodes (72): _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence, TestPhaseSequenceError, TestWorkspaceIsolation (+64 more)
...
### Community 384 - "TestModelRegistryUpdates"
Cohesion: 0.14
...
Cohesion: 0.04
Nodes (68): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+60 more)
...
### Community 441 - "test_ephemeral_reaper.py"
Cohesion: 0.10
...
### Community 442 - "seo_report_pdf.py"
Cohesion: 0.24
...
### Community 459 - "test_issue_triage.py"
Cohesion: 0.16
...
### Community 463 - "get_trend_watcher"
Cohesion: 0.16
...
Cohesion: 0.12
Nodes (19): ChatResponse, Any, Send a chat request with automatic failover. Retry policy: 1. If ``model`` is…, all_models(), best_model_for(), get(), ModelInfo, models_by_provider() (+11 more)
...
### Community 541 - "test_agent_tools.py"
Cohesion: 0.17
...
### Community 564 - "parse_event_stream"
Cohesion: 0.15
...
### Community 597 - "AnthropicProvider"
Cohesion: 0.09
...
### Community 603 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.09
...
### Community 609 - "test_contract_enforcement.py"
Cohesion: 0.17
...
### Community 620 - "get_tool_registry"
Cohesion: 0.14
...
### Community 630 - "AdminDigestRouterAuthTests"
Cohesion: 0.22
...
### Community 676 - "DailyDigestAggregatorTests"
Cohesion: 0.19
...
### Community 677 - ".on_task_complete"
Cohesion: 0.16
...
### Community 695 - "ENGINEERING_STANDARDS.md — Patterns & Reference"
Cohesion: 0.22
...
### Community 719 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.37
...
### Community 758 - "PortfolioIntelligence"
Cohesion: 0.18
...
### Community 759 - "_is_ephemeral_user"
Cohesion: 0.27
...
Cohesion: 0.15
Nodes (13): Agent and workflow surfaces, API Surfaces and Route Map, Built-in admin and web UI, Control-plane style routers mounted in the proxy, CRISPY Workflow engine (`/api/workflow/*`, `workflow/api.py`, admin-only), Governance (`/api/governance/*`, admin-only), Main proxy (`proxy.py`), Ollama-compatible (+5 more)
...
### Community 808 - "TestRequireAdmin"
Cohesion: 0.33
...
### Community 832 - "Path"
Cohesion: 0.27
Nodes (4): Path, Returns a structural overview of the repository., Workhorse tool for packing content and metrics of target files., Get dependencies from our built intelligence.
...
Cohesion: 0.06
Nodes (52): BoundContext, ClassifyOutput, ExecutionResult, _get_ceo_dispatcher(), JudgeVerdict, MergeDecision, MonitorOutput, _orchestrator_bypass() (+44 more)
...
Cohesion: 0.03
Nodes (105): CEOResult, _decompose_into_subtasks(), _merge_changed_files(), _offload(), Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default… (+97 more)
...
### Community 871 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 873 - "_execute_skill_impl"
Cohesion: 0.24
...
### Community 876 - "_provider_env_value"
Cohesion: 0.50
...
### Community 882 - "agent/output_filter.py"
Cohesion: 0.32
...
### Community 1009 - "SamConversation"
Cohesion: 0.29
...
Cohesion: 0.04
Nodes (94): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+86 more)
...
### Community 1046 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1063 - "_brain_provider_status"
Cohesion: 0.33
...
### Community 1114 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 1124 - "TestCEOAgencySystem"
Cohesion: 0.33
...
### Community 1132 - "TestNoHardcodedModels"
Cohesion: 0.33
...
### Community 1143 - "Who is this for?"
Cohesion: 0.40
...
### Community 1267 - "Privacy, security, and cost"
Cohesion: 0.50
...
### Community 1277 - "Configuration reference"
Cohesion: 0.67
...
- **3471 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3466 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13278 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **118 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `test_brain_patch_service_token.py`, `test_ceo_router.py`, `WorkflowRun`, `PolicyEngine`, `TaskStore`, `_FakeSandbox`, `TaskWorkflowService`, `test_llm_router_strategies.py`, `AuditLog`, `SelfHealingAgent`, `Task`, `AgentJobManager`, `test_company_api.py`, `test_unit8_model_catalog.py`, `TestAuthAndTaskOwnership`, `ImprovementLoop`, `test_kimi_bridge_server.py`, `test_failover_client_shared.py`, `test_catalogue_probe.py`, `SeoAuditEngine`, `WebsiteScanner`, `PrimeAgentAdapter`, `resolve_active_brain`, `test_brain_availability_doctor.py`, `SeoFixer`, `ExecutionRequest`, `parse_event_stream`, `test_model_router.py`, `test_openclaw_endpoints.py`, `test_task_service_failed_comment.py`, `clear_cooldowns`, `validate_policy_text`, `ai/router.py`, `ChatHistoryStore`, `ArtifactStore`, `SQLiteStore`, `get_task_store`, `test_telegram_auto_approve.py`, `ResearchTask`, `TestClient`, `test_ai_insights.py`, `test_autonomy_status.py`, `test_procedural_memory.py`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_agent_free_brain.py`, `test_refresh_agent_built_proof.py`, `test_startup_warmup.py`, `test_telegram_diag_endpoint.py`, `get_registry`, `TokenBudget`, `reset_kv_state`, `resolve_e2b_config`, `run_task`, `cost_tracker.py`, `BackgroundAgent`, `UserRole`, `test_context_rulebook.py`, `e2e/test_browser.py`, `test_provider_router.py`, `BrainWatchdog`, `KeyStore`, `test_issue_intake.py`, `test_llm_router_queue_cache.py`, `_step`, `test_trend_scoping.py`, `_clean_phases`, `_StubProvider`, `claim`, `MCPClient`, `persist_plan_spec`, `config.py`, `test_openclaw_gateway.py`, `.on_task_complete`, `NotificationDispatcher`, `OllamaCircuitBreaker`, `test_telegram_webhook.py`, `_job_text`, `test_features_api.py`, `test_new_features_e2e.py`, `test_video_transcript.py`, `test_llm_router_resilience.py`, `ScheduledJob`, `test_process_quick_note_workflow.py`, `_Budget`, `nvidia_models.py`, `TrendWatcher`, `TestMongoGate`, `TestRuntimeControl`, `test_portfolio_intake.py`, `test_trend_watcher.py`, `test_e2b_data_flow.py`, `TestClient`, `AgentSwarm`, `test_webui_provider_priority.py`, `ContextPruner`, `TestEstimateTokensForMessages`, `get_scheduler`, `test_agile_api.py`, `test_app_settings.py`, `TestSwarmRoleRouting`, `WorkflowBuildRequest`, `test_persistent_memory.py`, `SecurityScanner`, `_clean_director`, `test_task_clarification.py`, `sam`, `test_llm_router_e2e.py`, `OutputFilter`, `test_platform_controls.py`, `test_rate_limiter.py`, `test_brain_failover.py`, `test_sam_voice.py`, `OrchestratorQueue`, `test_nvidia_model_discovery.py`, `session_retro.py`, `facade.py`, `TestFeaturesAPI`, `test_llm_router_disabled.py`, `test_purge_backlog.py`, `test_provider_enable_disable.py`, `OrchestratorCheckpointStore`, `analyze_page`, `_resolve_user_github_token`, `capture_screens.py`, `workflow_orchestrator.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_skill_registry_boot_refresh.py`, `ApprovalStore`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `test_dashboard_cache.py`, `test_telegram_mutating_commands.py`, `_undeclared`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_harness_spec.py`, `test_brain_failover_413.py`, `test_regression.py`, `test_agency_fix.py`, `TestHarnessAdapter`, `test_monitor_lib.py`, `test_v4_api.py`, `_start_ceo_agency`, `test_knowledge_sync.py`, `.record`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `TestNormalizeResponseFormat`, `KeyPool`, `test_task_store_fails_loud_in_production.py`, `test_provider_state_durability.py`, `_Recorder`, `test_telegram_service_webhook.py`, `test_voice_pipeline.py`, `client`, `TestTheWorkflowIsSafeAndReadOnly`, `test_doctor_coding_brain.py`, `services/background.py`?**
  _High betweenness centrality (0.080) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `WorkflowRun`, `ReactScratchpad`, `backend/server.py`, `proxy.py`, `ContextManager`, `PolicyEngine`, `TaskSpec`, `test_governance_api.py`, `MCPClient`, `test_brain_failover_413.py`, `test_ceo_micromanager.py`, `agency.py`, `AgentJobManager`, `test_governance_enforcement.py`, `test_failover_client_shared.py`, `E2BSandboxSession`, `test_empirical_verify.py`, `MultiAgentSwarm`, `enforcement.py`, `FreeBuffAgent`, `LocalWorkspace`, `AgentSessionStore`, `WorkspaceTools`, `TestAgentRunnerSafety`, `direct_chat.py`, `test_backend_server_features.py`, `loop.py`, `TestAgentLoopMCPIntegration`, `test_ceo_supervision.py`, `workflow_orchestrator.py`, `TestAgentRunnerExecution`, `test_agent_free_brain.py`, `AdaptiveHalter`, `ContextPruner`, `TokenBudget`, `InternalAgentAdapter`, `GitHubTools`, `UserMemoryStore`, `BackgroundAgent`, `test_daily_automation_2026_07_11.py`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `ProviderConfig`, `backend/server.py`, `TestLegacyRouterServerFallback`, `test_bedrock_live.py`, `TestRouterIntegration`, `settings.py`, `resolve_active_brain`, `kimi_bridge_provider_config`, `test_colibri_provider.py`, `TestLegacyRouterCacheTTL`, `direct_chat.py`, `clear_cooldowns`, `ai/router.py`, `test_chat_mode_regressions.py`, `test_anthropic_refusal_fallback.py`, `failover_client.py`, `TrafficDirector`, `TestOpenAiToBedrockConverse`, `TestAnthropicPayloadStructuredOutput`, `test_all_providers_discovery.py`, `test_provider_router.py`, `model_discovery.py`, `_payload`, `implement_agent.py`?**
