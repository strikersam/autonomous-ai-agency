...
## Corpus Check
- 1434 files · ~2,072,493 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 29051 nodes · 53995 edges · 1261 communities (1140 shown, 121 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2670 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `674f47b8`
...
## Communities (1261 total, 121 thin omitted)

### Community 0 - "MongoDBStore"
Cohesion: 0.04
Nodes (51): MongoDBStore, Any, Company, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., Create a new company in SQLite., Get a company by ID from SQLite. (+43 more)
...
Cohesion: 0.04
Nodes (82): _acquire_provider_probe(), extract_openai_text(), _get_director(), is_commercial_provider(), _normalize_nvidia_base_url(), _normalized_provider_type(), _openai_url(), provider_access_tier() (+74 more)
...
Cohesion: 0.02
Nodes (100): CacheManager, cosine_similarity(), _Entry, LRUCache, Any, T, Live (unexpired) entries — used by the semantic layer's scan., Cosine similarity between two vectors, 0.0 when either is degenerate. (+92 more)

### Community 6 - "LLMRouter"
Cohesion: 0.08
Nodes (27): Attempt, payload_key(), Exact-match cache key over the fields that change the answer. Routing…, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, Any, AsyncClient (+19 more)
...
### Community 8 - "Any"
Cohesion: 0.05
Nodes (28): _check_extra_kwargs(), _enforce_signature(), Any, Path, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a… (+20 more)

### Community 9 - "TaskSpec"
Cohesion: 0.03
Nodes (98): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns… (+90 more)
...
Cohesion: 0.03
Nodes (134): Call-time resolver for an agent role model id. Delegates to…, _resolve_role_model(), _brain_provider_status(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore (+126 more)

### Community 12 - "LLMRequest"
Cohesion: 0.04
Nodes (105): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, OpenAICompatible, Adapter for any endpoint speaking the OpenAI chat-completions shape. Adding a…, build_provider(), ProviderConfig, Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the… (+97 more)

### Community 13 - "router_factory"
Cohesion: 0.16
...
Cohesion: 0.03
Nodes (86): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, events_from_identity(), Any, Convenience: build and record an event in one call., Extract the identity-derived audit fields from an AgentIdentity. Kept here…, record_event(), build_docker_run_argv(), detect_backend() (+78 more)

### Community 17 - "PolicyEngine"
Cohesion: 0.04
Nodes (88): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, BudgetExceeded, governance_enabled(), _more_restrictive(), Exception, packages/governance/enforcement.py — where a verdict becomes an outcome. The… (+80 more)

### Community 18 - "CEODispatcher"
Cohesion: 0.06
Nodes (48): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), _merge_changed_files(), _offload(), Any, Semaphore (+40 more)
...
Cohesion: 0.03
Nodes (86): AgentDefinition, AgentStore, Any, field_validator, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents. (+78 more)

### Community 24 - "Agency"
Cohesion: 0.04
Nodes (56): Agency, AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _fetch_github_quick_notes() (+48 more)
...
Cohesion: 0.02
Nodes (122): DetectedSystem, Evidence, OnboardingProgress, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., Tracks the onboarding progress for a company., Result of a repository scan with detected stack and systems., A business system detected on a company's website or in their stack. (+114 more)
...
### Community 33 - "test_unit6_resolve_component_model.py"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (72): AgentRole, _parse_ceo_directives(), str, DetectedIssue, ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity (+64 more)
...
Cohesion: 0.07
Nodes (31): get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., reset_ceo_dispatcher(), get_ceo_ledger(), Return the shared :class:`CEOLedger`, constructing it on first use., CEOSupervisor, _env_flag() (+23 more)
...
Cohesion: 0.05
Nodes (38): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, Run a job using the provided runner and update the job's lifecycle, progress,…, Create an isolated workspace directory under *root*. This is the legacy path… (+30 more)
...
Cohesion: 0.06
Nodes (62): MCPUnavailableError, agent/mcp_client.py — Async MCP client for the mcp-server Docker container.…, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any, services/e2b_sandbox.py — E2B Firecracker micro-VM sandbox session. Implements… (+54 more)

### Community 62 - "RenderOpsMonitor"
Cohesion: 0.06
Nodes (31): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, One deploy, normalised from whatever shape the tool returned., RenderDeploy, Polls Render for platform-level failures and files them as issues., Snapshot for ``/api/render/ops/status`` and the dashboard. ``self_heal_ready``…, Map service ID → human name, degrading to the ID when unavailable. (+23 more)
...
Cohesion: 0.03
Nodes (63): Recommend skills relevant to the current task. Called from the tool-call loop…, SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., Request to provision a specialist for a company., Result of provisioning a specialist., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities. (+55 more)

### Community 65 - "Artifact"
Cohesion: 0.05
Nodes (48): AgentRole, Run a pre-gate or report phase through the correct agent. Enforces permission…, Execute a slice via the Coder agent (write-permitted)., Review a slice via the Reviewer agent (different model from Coder). This is the…, Run verification commands via the Verifier agent. The Verifier is execution-…, Return a copy of *routing* with the profile's model injected for its role., When coder == reviewer model, swarm should log a warning., tests/test_workflow_models.py — Unit tests for workflow/models.py. (+40 more)
...
Cohesion: 0.06
Nodes (62): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog(), _heal_stuck_tasks() (+54 more)
...
Cohesion: 0.06
Nodes (51): Available iff config resolves AND the SDK is importable. Never raises — a…, e2b_enabled(), E2BConfig, _env_falsy(), _env_truthy(), services/e2b_config.py — Single reader of E2B sandbox environment config.…, Return ``True`` when E2B sandboxing should be activated. **EXPERIMENTAL…, Resolve the E2B sandbox config from env, or ``None`` when unconfigured. This is… (+43 more)
...
Cohesion: 0.08
Nodes (19): requires_db, asyncio, Test Company Graph services., Test that storage service can be initialized., Test company CRUD operations - skipped as requires specific config., Test scanner service., Test that scanner service can be initialized., Test specialist service. (+11 more)
...
Cohesion: 0.05
Nodes (76): DirectChatDoctor, PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational() (+68 more)
...
Cohesion: 0.05
Nodes (45): Dynamic model router package. Public API:: from router import get_router,…, _build_builtin_model_map(), _default_model(), _default_reasoning_model(), ModelRouter, _nvidia_key_present(), Any, Dynamic model router. Central routing logic for all chat and agent requests.… (+37 more)

### Community 128 - "telegram_bot.py"
Cohesion: 0.07
Nodes (57): get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _answer_callback(), _api_headers(), _check_rate_limit() (+49 more)
...
Cohesion: 0.05
Nodes (85): activation_required(), ActivationResult, activate_instance(), ActivateRequest, ActivateResponse, activation_audit_log(), activation_status(), ActivationStatusResponse (+77 more)
...
### Community 172 - "test_provider_410_per_model.py"
Cohesion: 0.14
...
### Community 177 - "test_crispy_workflow.py"
Cohesion: 0.08
...
Cohesion: 0.02
Nodes (93): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+85 more)
...
### Community 194 - "AuditEvent"
Cohesion: 0.07
Nodes (21): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), AuditEvent, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., The stored object must not hold the secret at all., An unbounded audit list is a memory leak with a security justification., test_a_broken_sink_cannot_suppress_the_audit_or_raise() (+13 more)

### Community 195 - "resolve_active_brain"
Cohesion: 0.08
Nodes (47): BrainResolution, provider_api_key(), Return the live API key for *provider* (env-only — never persisted)., get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), _norm() (+39 more)
...
Cohesion: 0.05
Nodes (35): Raised when a runtime fails readiness validation before execution starts., RuntimePreflightError, runtimes/health.py — RuntimeHealthService. Periodically polls all registered…, runtimes/registry.py — RuntimeCapabilityRegistry. Maps task types and required…, Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry (+27 more)
...
Cohesion: 0.04
Nodes (59): ApprovalPolicy, BusinessSystem, Company, CompanyCreateRequest, CompanyGraph, CompanyGraphResponse, CompanyGraphSnapshot, CompanyUpdateRequest (+51 more)
...
Cohesion: 0.06
Nodes (31): _exponential_backoff_cooldown(), _notify_watchdog(), ProviderRouter, Any, Response, Try all models for one provider. Returns ProviderResult on success, None on…, Notify the brain watchdog of a provider call outcome (fire-and-forget).…, Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either… (+23 more)
...
### Community 231 - "Task"
Cohesion: 0.05
Nodes (62): Full task/issue document., Task, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty…, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, _coordinator(), asyncio, BaseException, Brain-availability preflight tests (graceful degradation). When NO LLM brain is… (+54 more)

### Community 232 - "WorkspaceManager"
Cohesion: 0.07
Nodes (45): test_git_ref_rejects_empty(), test_git_ref_rejects_flag_injection(), test_git_ref_rejects_shell_metacharacters(), test_git_ref_rejects_traversal(), test_git_ref_valid(), test_git_scheme_allows_ssh(), test_http_scheme_rejects_ssh(), test_https_public_host_allowed() (+37 more)
...
Cohesion: 0.04
Nodes (39): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, _best_cloud_primary_base(), InternalAgentAdapter, TaskResult, TaskSpec, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in… (+31 more)
...
Cohesion: 0.04
Nodes (59): autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap(), CompanyAgencyService, get_company_agency_service(), _is_runtime_available_sync(), _pick_available_runtime() (+51 more)
...
### Community 276 - "TestStreamableHTTPTransport"
Cohesion: 0.13
...
Cohesion: 0.07
Nodes (41): _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), merge_pr_route(), Public Doctor endpoint — no authentication required. Returns system-level…, Authenticated Doctor endpoint — full diagnostics. Returns all system-level… (+33 more)
...
Cohesion: 0.08
Nodes (28): _latest_metric_value(), _note_recurrence(), _parse_timestamp(), Any, datetime, services/render_ops.py — autonomous Render debugging + environment monitoring.…, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Pull the most recent numeric sample out of a ``get_metrics`` payload. Render… (+20 more)
...
Cohesion: 0.04
Nodes (44): _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), _build_provider_router(), _builtin_provider_records(), call_llm(), _chat_provider_policy(), _fallback_local_provider_record() (+36 more)
...
Cohesion: 0.06
Nodes (55): get_improvement_loop(), get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, get_trend_watcher(), agent/trend_watcher.py — Internet-connected AI trend intelligence. Fetches from…, set_trend_watcher(), build_ceo_router(), Any (+47 more)
...
### Community 317 - "PolicyDecision"
Cohesion: 0.08
Nodes (29): classify(), evaluate_call(), guard_tool_call(), GuardResult, _host_of(), _is_write_tool(), Any, Extract a hostname from a URL, or return a bare host unchanged. (+21 more)
...
### Community 345 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 356 - "open_phase_report"
Cohesion: 0.10
...
Cohesion: 0.12
Nodes (25): _admin_out(), AdminCommandBody, _anthropic_chat_payload(), _anthropic_text(), BrainPolicyUpdate, _provider_headers(), _provider_kind(), ProviderReorderBody (+17 more)
...
Cohesion: 0.06
Nodes (51): delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), list_seo_audits(), BaseModel, get, post (+43 more)
...
### Community 383 - "skill_bindings.py"
Cohesion: 0.09
...
Cohesion: 0.06
Nodes (31): _cached(), _check_storage_health(), cost_attribution_stats(), doctor_health(), _fast_count(), get_active_provider(), get_activity(), get_stats() (+23 more)
...
Cohesion: 0.08
Nodes (36): FastAPI dependency: require Power User or Admin role. Raises 403 otherwise., Return a FastAPI dependency that checks for a specific permission., require_permission(), require_power_user(), sync/ — Syncthing-style workspace synchronisation service., add_peer(), get_folder_index(), get_sync_file() (+28 more)
...
Cohesion: 0.04
Nodes (61): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), get_ring(), KeyRing (+53 more)
...
Cohesion: 0.05
Nodes (44): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), stop_orchestrator_supervisor(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper)., Canonical request to execute work through the golden path. This is the ONLY… (+36 more)
...
Cohesion: 0.14
Nodes (18): loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Condense a tool result down to what belongs in an audit row. A tool result can…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, _summarise_tool_result(), build_compaction_prompt(), build_execution_prompt(), build_planning_prompt() (+10 more)
...
Cohesion: 0.11
Nodes (12): _parse_iso(), Resolve *relative* inside source dir and reject traversal/symlink escapes., Create, open, and lifecycle-manage per-job isolated workspaces. All workspaces…, Resolve *relative* within *ws*.source and reject traversal/symlink escapes.…, Update last_heartbeat timestamp and persist., Remove expired workspaces that are cleanup_eligible and past cleanup_after.…, Remove and recreate the tmp directory for *ws*., Return counts of workspaces by status (scans disk, sync). Call ``await… (+4 more)
...
### Community 505 - "test_render_mcp.py"
Cohesion: 0.06
...
Cohesion: 0.03
Nodes (105): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, Enum, str, tasks/models.py — Pydantic models for the task/issue system., Comment or reply on a task., TaskComment (+97 more)

### Community 514 - "HTTPException"
Cohesion: 0.03
Nodes (142): auth_me(), auto_recommend_skills(), clear_cost_attribution(), clear_response_cache(), create_access_token(), create_github_pr(), create_provider(), delete_api_key() (+134 more)
...
### Community 537 - "reset_failover_manager"
Cohesion: 0.10
...
Cohesion: 0.04
Nodes (38): CerebrasProvider, Provider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Provider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NvidiaProvider, Provider (+30 more)

### Community 539 - "implement_agent.py"
Cohesion: 0.12
Nodes (19): build_tool_calling_router(), main(), _openai_tools_to_anthropic(), Any, Safely insert an entry under ## [Unreleased] without touching the rest of the…, Convert OpenAI function-calling tool schemas to Anthropic tool schemas., Run the implementation agent loop using Claude Opus via Anthropic SDK. Returns…, Return a router limited to providers that pass ``tools`` through intact. The… (+11 more)
...
### Community 544 - "test_unit7_catalog_propagation.py"
Cohesion: 0.08
...
Cohesion: 0.11
Nodes (18): _embedded(), _embedded_run(), _fb_plan(), _fb_run(), _freebuff_max_steps(), Generate a read-only plan (embedded or via proxy). Shape: {model, plan}., Execute a FreeBuff task (embedded or via proxy). Shape: {result: {...}}., Run FreeBuffAgent in-process against a fresh clone, committing + opening a PR.… (+10 more)
...
### Community 605 - "test_scanner_live.py"
Cohesion: 0.23
...
Cohesion: 0.03
Nodes (37): Find a repository by its URL., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, Add a repository to a company. Args: company_id: Company ID url: Repository URL…, Get a repository by ID., Initialize the service. Args: store: Optional CompanyGraphStore instance. If… (+29 more)
...
Cohesion: 0.10
Nodes (11): Any, Exception, Open a workspace for resumption. Only READY or PAUSED workspaces may be…, Raise :exc:`WorkspaceAccessDeniedError` if *requesting_session_id* doesn't own…, Acquire the exclusive async lock for *ws*. Raises :exc:`WorkspaceLockError` if…, Base class for all workspace errors., WorkspaceAccessDeniedError, WorkspaceError (+3 more)
...
### Community 668 - "TestMCPToolsListCache"
Cohesion: 0.19
...
### Community 669 - "test_issue_triage.py"
Cohesion: 0.17
...
### Community 756 - "audit"
Cohesion: 0.03
Nodes (61): compute_savings(), compute_time_series(), get_savings(), get_usage(), get_user_savings(), _period_start(), Any, BaseModel (+53 more)
...
### Community 758 - "._notify_approval_gate"
Cohesion: 0.22
...
### Community 759 - "OperationalIncident"
Cohesion: 0.13
...
Cohesion: 0.04
Nodes (40): Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs…, Determine domain and task type from the request text., Generate a structured execution plan. Uses the LLM if available; falls back to…, Select the best specialist(s) for the classified domain., Run doctor checks before execution., Resolve and bind skills, memory, and Company Graph context. (+32 more)
...
Cohesion: 0.02
Nodes (158): get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, admin_seed(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody (+150 more)
...
Cohesion: 0.08
Nodes (24): AnthropicProvider, Any, AsyncClient, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs…, Anthropic's extended-thinking constraints on ``budget_tokens``: at least 1024,…, Return a system-prompt instruction that enforces JSON output. Anthropic's… (+16 more)
...
### Community 790 - "run_freebuff_bot.py"
Cohesion: 0.60
...
### Community 829 - "ApprovalCheckpoint"
Cohesion: 0.14
...
### Community 832 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
### Community 835 - "operational_incidents.py"
Cohesion: 0.13
...
### Community 911 - "FeatureUnavailableError"
Cohesion: 0.12
...
### Community 916 - "test_agent_tools.py"
Cohesion: 0.19
...
### Community 947 - "LLMProvider"
Cohesion: 0.08
Nodes (16): LLMProvider, ABC, Any, AsyncClient, ProviderConfig, Every provider adapter implements exactly this., Perform one completion. Raises ``PermanentError``/``TransientError``., Yield normalised chunks for one streaming completion. (+8 more)
...
### Community 953 - "is_anthropic_base_url"
Cohesion: 0.12
...
### Community 963 - "Agent Autonomy Roadmap"
Cohesion: 0.25
...
Cohesion: 0.15
Nodes (11): codeql_count(), dependabot_count(), main(), Any, _repo_parts(), _request(), OrchestratorSupervisor, Any (+3 more)
...
Cohesion: 0.03
Nodes (128): int, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager., text_key() (+120 more)
...
### Community 1207 - "The full agent capability roster"
Cohesion: 0.33
...
### Community 1214 - "summarise_phases"
Cohesion: 0.15
...
### Community 1215 - "test_tokenin_provider.py"
Cohesion: 0.18
Nodes (8): get_provider_tier(), Return the tier (``free`` / ``paid`` / ``local``) for *provider*. Returns…, test_get_provider_tier_known(), test_get_provider_tier_unknown_returns_unknown(), tests/test_tokenin_provider.py — TokenIn free frontier gateway wiring.…, test_router_from_env_registers_tokenin(), test_router_omits_tokenin_without_key(), test_tokenin_in_literal_and_catalog()
...
### Community 1218 - "TestBackgroundAgentDispatch"
Cohesion: 0.24
...
### Community 1219 - "Who is this for?"
Cohesion: 0.40
...
### Community 1221 - "get_orchestrator_queue"
Cohesion: 0.27
...
### Community 1224 - "local_brain_provider_config"
Cohesion: 0.29
...
### Community 1228 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 1232 - "Privacy, security, and cost"
Cohesion: 0.50
...
### Community 1234 - "_get_current_user"
Cohesion: 0.25
...
### Community 1236 - "Continual Harness (`agent/harness_spec.py`)"
Cohesion: 0.25
...
### Community 1251 - "TaskAutomationService"
Cohesion: 0.40
Nodes (3): Task, Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService
...
- **Why does `_fixture()` connect `_fixture` to `TaskStore`, `ProviderConfig`, `test_llm_router_queue_cache.py`, `test_company_api.py`, `brain_config.py`, `test_daily_automation_2026_07_09.py`, `router_factory`, `test_langfuse_agency_wide.py`, `validate_policy_text`, `PolicyEngine`, `SelfHealingAgent`, `TaskWorkflowService`, `test_provider_state_durability.py`, `reset_failover_manager`, `Agency`, `PrimeAgentAdapter`, `CEOLedger`, `clear_cooldowns`, `WebsiteScanner`, `NotificationDispatcher`, `get_self_healing_agent`, `test_agent_tool_governance.py`, `test_monitor_lib.py`, `test_openclaw_endpoints.py`, `test_unit8_model_catalog.py`, `ImprovementLoop`, `_resolve_push_token`, `test_llm_router_strategies.py`, `test_ceo_router.py`, `test_failover_client_shared.py`, `test_process_quick_note_workflow.py`, `AgentJobManager`, `SQLiteStore`, `ChatHistoryStore`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `test_freebuff_bot.py`, `get_task_store`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `services/seo_audit.py`, `test_telegram_diag_endpoint.py`, `config.py`, `ResearchTask`, `TestClient`, `test_procedural_memory.py`, `e2e/test_browser.py`, `UserRole`, `parse_event_stream`, `test_knowledge_sync.py`, `test_startup_warmup.py`, `.on_task_complete`, `SeoFixRequest`, `KeyStore`, `direct_chat.py`, `claim`, `test_task_store_fails_loud_in_production.py`, `test_context_rulebook.py`, `model_router.py`, `ArtifactStore`, `run_task`, `test_issue_intake.py`, `.record`, `test_openclaw_gateway.py`, `._check_permissions`, `provider_max_rpm`, `test_doctor_service_token_check.py`, `BackgroundAgent`, `test_telegram_freebuff.py`, `TestHarnessAdapter`, `MCPClient`, `test_new_features_e2e.py`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `_Budget`, `test_provider_410_per_model.py`, `llm/router.py`, `test_doctor_coding_brain.py`, `test_e2b_data_flow.py`, `OllamaCircuitBreaker`, `analyze_page`, `TestRuntimeControl`, `model_discovery.py`, `ScheduledJob`, `test_features_api.py`, `test_video_transcript.py`, `TrendWatcher`, `FailoverResult`, `test_agile_api.py`, `test_app_settings.py`, `_Recorder`, `test_portfolio_intake.py`, `_FakeSandbox`, `test_trend_watcher.py`, `AgentSwarm`, `api_keys_for`, `TestClient`, `ProviderRouter`, `_clean_singletons`, `Task`, `ContextPruner`, `BrainWatchdog`, `WorkflowOrchestrator`, `reap_expired_companies`, `test_daily_2026_07_27.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `OutputFilter`, `test_platform_controls.py`, `capture_screens.py`, `test_scheduler_hydration_bounded.py`, `TestFeaturesAPI`, `_resolve_user_github_token`, `test_rate_limiter.py`, `TestAuthAndTaskOwnership`, `test_sam_voice.py`, `OrchestratorQueue`, `services/background.py`, `test_purge_backlog.py`, `operational_incidents.py`, `test_kimi_bridge_server.py`, `session_retro.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `test_control_plane_api.py`, `test_webui_provider_priority.py`, `test_claude_code_adapter.py`, `test_service_token.py`, `test_brain_patch_service_token.py`, `test_harness_spec.py`, `test_ping.py`, `test_telegram_service_webhook.py`, `test_runtimes_health_endpoint.py`, `test_dashboard_cache.py`, `test_regression.py`, `test_agency_fix.py`, `test_telegram_mutating_commands.py`, `test_llm_router_resilience.py`, `test_v4_api.py`, `ExecutionRequest`, `test_llm_router_disabled.py`, `allow_paid`, `client`, `WorkflowRun`, `TestMongoGate`, `test_voice_pipeline.py`, `_job_text`, `test_task_clarification.py`, `cost_tracker.py`, `TestNormalizeResponseFormat`, `test_task_service_failed_comment.py`, `test_ai_insights.py`, `test_sam_livekit.py`?**
  _High betweenness centrality (0.121) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `test_agent_free_brain.py`, `proxy.py`, `TestAgentLoopMCPIntegration`, `AutonomyTracker`, `Any`, `TaskSpec`, `LocalWorkspace`, `ReactScratchpad`, `test_daily_automation_2026_07_09.py`, `BrainFailoverExhausted`, `test_governance_sandbox.py`, `PolicyEngine`, `CEODispatcher`, `ContextManager`, `Agency`, `reset_failover_manager`, `MCPClient`, `test_agent_tool_governance.py`, `MultiAgentSwarm`, `_resolve_push_token`, `test_failover_client_shared.py`, `StuckDetector`, `loop.py`, `call_llm`, `WorkspaceTools`, `test_daily_automation_2026_07_11.py`, `test_governance_api.py`, `AgentJobManager`, `.execute`, `E2BSandboxSession`, `TestAgentRunnerSafety`, `test_empirical_verify.py`, `Specialist`, `test_agent_chat_integration.py`, `AgentPlan`, `UserMemoryStore`, `TokenBudget`, `TestAgentRunnerExecution`, `FreeBuffAgent`, `AdaptiveHalter`, `ContextPruner`, `direct_chat.py`, `InternalAgentAdapter`, `workflow_orchestrator.py`, `WorkflowOrchestrator`, `GitHubTools`, `AgentSessionStore`, `backend/server.py`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `test_all_providers_discovery.py`, `ProviderConfig`, `test_bedrock_provider.py`, `test_anthropic_router.py`, `kimi_bridge_provider_config`, `test_colibri_brain_shim.py`, `settings.py`, `implement_agent.py`, `clear_cooldowns`, `TestAnthropicPayloadStructuredOutput`, `TrafficDirector`, `test_provider_410_per_model.py`, `call_llm`, `TestRouterIntegration`, `is_anthropic_base_url`, `test_tokenin_provider.py`, `failover_client.py`, `test_chat_mode_regressions.py`, `system_instruction`, `direct_chat.py`, `test_bedrock_live.py`, `backend/server.py`?**
