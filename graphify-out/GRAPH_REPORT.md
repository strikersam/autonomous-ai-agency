...
## Corpus Check
- 1351 files · ~1,878,537 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 26133 nodes · 52643 edges · 1133 communities (1022 shown, 111 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 5721 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e9324aa6`
...
## Communities (1133 total, 111 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.01
Nodes (390): get_harness_adapter(), admin_seed(), _agent_timeout_fallback_response(), auth_me(), auto_recommend_skills(), _autonomy_bg_cycle(), brain_failover_status(), brain_providers() (+382 more)

### Community 1 - "FreeBuffAgent"
Cohesion: 0.11
Nodes (89): BackgroundTask, CommitAttribution, CommitTracker, Path, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata…, Create git commits enriched with agent-session attribution trailers. Usage::…, Return ``--trailer`` arguments ready to append to a ``git commit`` call., Stage *files* and create an attributed commit. Returns the commit SHA on… (+81 more)

### Community 2 - "AgentSessionStore"
Cohesion: 0.05
Nodes (137): Agency, CEO-coordinated multi-agent agency for continuous codebase management. The CEO…, AgentJobRequest, AgentJobSnapshot, BaseModel, Complete point-in-time view of a job, safe to serialise as API response., Convenience: the canonical text response, whether success or failure., Validated input for creating a new agent job. Passed from the API handler into… (+129 more)

### Community 3 - "ProviderConfig"
Cohesion: 0.02
Nodes (208): get_tracker(), Return the process-singleton RateLimitTracker., _acquire_provider_probe(), clear_cooldowns(), _dead_model_key(), _exponential_backoff_cooldown(), extract_openai_text(), get_dead_models() (+200 more)
...
### Community 5 - "ExecutionRequest"
Cohesion: 0.04
Nodes (56): Called by APScheduler when a cron fires. Dispatches to the orchestrator., _scheduler_on_fire(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, services/orchestrator_supervisor.py — Deterministic Supervisor Issue #522: A…, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.… (+48 more)

### Community 6 - "proxy.py"
Cohesion: 0.03
Nodes (158): Any, Return recent commits with agent attribution trailers parsed out., get_sam(), middleware, admin_control(), admin_create_key(), admin_create_user(), admin_delete_user() (+150 more)

### Community 7 - "NotificationDispatcher"
Cohesion: 0.06
Nodes (51): AgentRole, str, Lifecycle status of a sprint., SprintStatus, Initiative, InitiativeStatus, PortfolioIntelligence, Assembles a PortfolioManager from live signals with WSJF scoring. (+43 more)
...
### Community 9 - "get_company_access"
Cohesion: 0.04
Nodes (75): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), generate_onboarding_questions() (+67 more)

### Community 10 - "LLMRequest"
Cohesion: 0.04
Nodes (83): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, AnthropicProvider, Any, AsyncClient, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, Adapter for ``POST /v1/messages``., Accept both OpenAI and native Anthropic tool declarations. (+75 more)

### Community 11 - "TaskStore"
Cohesion: 0.05
Nodes (58): Any, Task, TaskStatus, Create a task. Deduplicates by source_id if set (Charter G3). If a task with…, Fetch a task by ID. If owner_id is set, enforces ownership., Return the task previously created for an external ``source_id`` (e.g.…, Normalise a timestamp value to a float Unix epoch. Handles: - float / int…, List tasks for a specific user with optional filters. ``include_system``… (+50 more)

### Community 12 - "llm/router.py"
Cohesion: 0.03
Nodes (107): _load(), main(), int, get_budget(), The process-wide budget tracker., get_cache(), The process-wide cache manager., openai_body_from_response() (+99 more)

### Community 13 - "workflow_orchestrator.py"
Cohesion: 0.06
Nodes (77): CoordinateRequestV2, CoordinateResponse, BaseModel, agent/coordinate.py — Multi-Agent Coordination models, helpers, and optional…, Extended coordinate request supporting both agents+tasks and legacy workers., SwarmSummary, AgentSpec, CoordinatorResult (+69 more)

### Community 14 - "ceo_dispatcher.py"
Cohesion: 0.05
Nodes (53): CEOResult, _complexity_rank(), _decompose_into_subtasks(), _harvest_changed_files(), _offload(), Any, Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a… (+45 more)

### Community 15 - "RuntimeCapabilityRegistry"
Cohesion: 0.05
Nodes (34): OpenHandsAdapter, Any, TaskResult, TaskSpec, Create a conversation in OpenHands and poll for completion., Adapter for OpenHands — TIER 2 / EXPERIMENTAL coding agent. NOTE: OpenHands…, Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can… (+26 more)

### Community 16 - "brain_config.py"
Cohesion: 0.03
Nodes (95): _brain_provider_status(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, BrainConfig, BrainConfigStore, default_brain_config(), get_brain_config(), invalidate_brain_config_cache(), provider_api_key() (+87 more)

### Community 17 - "TaskSpec"
Cohesion: 0.05
Nodes (69): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).… (+61 more)

### Community 18 - "services/seo_audit.py"
Cohesion: 0.08
...
Cohesion: 0.04
Nodes (54): CompanyGraphSnapshot, A point-in-time snapshot of a Company Graph for history and rollback., MongoDBStore, Any, Company, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model. (+46 more)
...
Cohesion: 0.02
Nodes (70): Find a website by its URL., A company website with detected systems and stack inference., Website, CompanyGraphStore, Delete a company and all its associated data., Create a new company in SQLite., Get a company by ID from SQLite., List companies with optional filtering. (+62 more)
...
Cohesion: 0.05
Nodes (17): FeatureMatrix, Central support matrix — single source of truth. Loads the canonical feature…, Render the matrix as a Markdown table for docs., tests/test_feature_matrix.py — Feature maturity / support matrix tests. Covers:…, Integration test: admin endpoint returns feature matrix JSON., TestAdminVisibility, TestConfigOverrides, TestEnforcement (+9 more)

### Community 28 - "webui/router.py"
Cohesion: 0.07
Nodes (57): _bootstrap(), fixture, Path, ProviderManager, WorkspaceManager, tests/test_webui_provider_priority.py — Priority + reorder + brain-policy…, The /policy/brain endpoint must return the resolved brain + the paid policy…, The /providers/role-tags endpoint surfaces brain/sub/fallback roles consistent… (+49 more)

### Community 29 - "test_llm_router_strategies.py"
Cohesion: 0.05
Nodes (81): count, HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, HealthTracker, Health and circuit-breaker state for every provider., Whether the breaker will admit a request right now. Transitions OPEN →… (+73 more)

### Community 30 - "SelfHealingAgent"
Cohesion: 0.04
Nodes (70): heal_signature(), HealingEvent, _now(), Any, agent/self_healing.py — Self-Healing Agent (closed-loop, Autonomy Charter G2)…, Translate external failure signals into improvement tasks and verify the fix…, Launch the background sweeper that resolves quiet verifying heals., Called when a CI workflow fails. (+62 more)

### Community 31 - "Specialist"
Cohesion: 0.04
Nodes (43): BusinessCategory, SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, CompanyGraphService (+35 more)

### Community 32 - "test_unit8_model_catalog.py"
Cohesion: 0.04
Nodes (71): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, all_provider_ids(), _provider_ids_from_literal(), Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, CatalogActiveBrain, CatalogMirror (+63 more)
...
### Community 34 - "TaskWorkflowService"
Cohesion: 0.05
...
### Community 37 - "fmtErr"
Cohesion: 0.05
Nodes (62): createQuickNote(), createTask(), fmtErr(), getActivity(), getCostAttribution(), getDecisionLog(), getDueSoonTasks(), getSavings() (+54 more)

### Community 38 - "probe_model_liveness"
Cohesion: 0.05
...
### Community 41 - "test_llm_router_queue_cache.py"
Cohesion: 0.04
Nodes (65): build_summariser(), chunk_document(), ContextManager, estimate_tokens(), FitResult, message_tokens(), prune(), Any (+57 more)
...
Cohesion: 0.04
Nodes (61): Backoff policy for retryable failures., RetryConfig, _digest(), get_ring(), KeyRing, KeyState, packages/llm/keys.py — multi-key rotation with per-key health. A provider may…, Per-provider round-robin key selection with cooldowns. (+53 more)
...
### Community 45 - "SpecialistService"
Cohesion: 0.04
...
Cohesion: 0.05
Nodes (76): E2BAdapter, Any, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime() (+68 more)
...
Cohesion: 0.08
Nodes (59): CEODispatcher, Real CEO delegation: wake runtimes, fan out, and merge results., A subtask's full history: what it is, and every attempt at it., SubtaskRecord, _goal(), asyncio, Tests for the CEO ledger, the supervised escalation loop, and the 24x7 sweeper.…, A storage outage must never take the agency down. (+51 more)

### Community 49 - "KeyPool"
Cohesion: 0.04
Nodes (39): _build_request(), Return ``(url, headers, is_anthropic_native)`` for *provider*. Anthropic's…, api_keys_for(), _digest(), KeyPool, _KeyState, _PoolState, Per-provider API key rotation — the one lever that adds capacity. Every other… (+31 more)

### Community 50 - "BrainWatchdog"
Cohesion: 0.06
Nodes (32): BrainWatchdog, _is_provider_actually_available(), Any, services/brain_watchdog.py — Brain health watchdog. Monitors the active brain…, Fail over to the next available provider., Persist the new provider in the brain config store (fire-and-forget). Uses the…, Send a Telegram notification about the failover., Reset the singleton (test helper). (+24 more)

### Community 51 - "GoalRecord"
Cohesion: 0.08
Nodes (29): _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, One piece of work the CEO has taken ownership of. ``state`` is the supervisor's…, Return ``(done, total)`` subtask counts. (+21 more)
...
### Community 55 - "CEOSupervisor"
Cohesion: 0.07
Nodes (27): backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., reset_ceo_dispatcher(), get_ceo_ledger() (+19 more)

### Community 56 - "Task"
Cohesion: 0.05
Nodes (56): Full task/issue document., Update the updated_at timestamp., Task, Best-effort Telegram heads-up that a task is parked awaiting approval. Inline…, _escape_md_v1(), Escape Telegram Markdown-v1 reserved chars in free-text fields. Markdown-v1…, asyncio, The reconciler must not re-queue tasks parked at the pre-execution gate.… (+48 more)
...
### Community 74 - "SeoCheckDefinition"
Cohesion: 0.12
...
### Community 84 - "test_bedrock_provider.py"
Cohesion: 0.05
...
Cohesion: 0.11
Nodes (23): PreflightReport, AgentJob, Any, WorkspaceEscapeError, WorkspaceIDError, FeatureMaturity, FeatureUnavailableError, Exception (+15 more)

### Community 88 - "failover_client.py"
Cohesion: 0.04
Nodes (71): _auto_disable(), _Budget, _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), FailoverResult, _is_billing_refusal(), _is_ollama() (+63 more)
...
Cohesion: 0.08
Nodes (49): Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost(), cmd_keylist() (+41 more)
...
### Community 104 - "test_provider_state_durability.py"
Cohesion: 0.06
Nodes (18): fake_mongo(), _FakeCollection, _FakeDb, isolated_state(), _live_mongo_url(), fixture, Operator provider state must survive a redeploy. The per-provider kill switch…, Tests must never mutate a shared operational store. (+10 more)

### Community 105 - "PortfolioManager"
Cohesion: 0.03
Nodes (41): CapacityAllocation, InitiativeProgress, PortfolioManager, PortfolioMetrics, Enum, Agentic Portfolio Management — initiative prioritisation, capacity allocation,…, Percentage of linked sprint points completed., Result of fitting initiatives into a fixed capacity by WSJF priority. (+33 more)
...
### Community 111 - "activation_api.py"
Cohesion: 0.09
...
Cohesion: 0.04
Nodes (88): BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), _norm(), _pick_from_records() (+80 more)
...
### Community 119 - "test_telegram_approval_gate.py"
Cohesion: 0.07
Nodes (28): Path, service_manager.py — Telegram & Notification Integration Extension Extends the…, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess)., Run the bot with stop-event awareness., # NOTE: do NOT re-escape here — services.daily_digest.format_digest_markdown, Manages the Telegram bot as a managed service alongside ollama/proxy/tunnel.… (+20 more)
...
### Community 122 - "test_e2b_adapter.py"
Cohesion: 0.06
...
### Community 134 - "test_slop_gate.py"
Cohesion: 0.07
...
### Community 135 - "get_task_store"
Cohesion: 0.05
Nodes (58): quick_notes_submit(), Submit a quick-note URL or instruction from the dashboard FAB., packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, Reset the singleton (for tests)., reset_failover_manager(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _heal_brain_failover() (+50 more)
...
### Community 140 - "_resolve_brain_provider"
Cohesion: 0.04
...
### Community 143 - "cache.py"
Cohesion: 0.06
Nodes (36): Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any (+28 more)
...
### Community 147 - "Usage"
Cohesion: 0.10
Nodes (23): AlertHandler, BudgetTracker, Counter, _Dimensions, _month(), Any, packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, Register a callback fired when a spend threshold is crossed. (+15 more)
...
### Community 152 - "_hostname_matches"
Cohesion: 0.22
...
### Community 156 - "brain_failover.py"
Cohesion: 0.09
...
### Community 161 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (79): _merge_changed_files(), Collect changed_files across all specialists into a single de-duped list., ClassifyOutput, get_ceo_fallback_stats(), PlanOutput, CLASSIFY phase: domain and task type determination., PLAN phase: structured execution plan., SELECT_SPECIALIST phase: which specialist(s) handle this work. (+71 more)

### Community 162 - "AgileManager"
Cohesion: 0.05
Nodes (46): AgileManager, Manages multiple agile sprints with velocity tracking., List all active sprints., Predict next sprint velocity from historical data., Number of managed sprints., add_initiative(), AllocationOut, BoardOut (+38 more)
...
### Community 168 - "resolve_component_model"
Cohesion: 0.06
...
### Community 171 - "KnowledgePage.js"
Cohesion: 0.18
...
### Community 181 - "TaskHarnessAdapter"
Cohesion: 0.07
...
Cohesion: 0.10
Nodes (36): IssueCategory, IssueSeverity, Enum, str, agent/improvement_loop.py — Continuous Improvement Engine Background scanner…, agent/trend_watcher.py — Internet-connected AI trend intelligence. Fetches from…, cluster_friction(), clusters_to_issues() (+28 more)
...
Cohesion: 0.11
Nodes (13): _Counter, _Gauge, _Histogram, _labels(), MetricsRegistry, Any, packages/llm/metrics.py — Prometheus metrics without the client library. The…, Prometheus text exposition (version 0.0.4). (+5 more)
...
### Community 203 - "._execute_step"
Cohesion: 0.05
Nodes (26): _nvidia_api_key(), Any, Path, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a…, Send chat messages and return the assistant's text output. If an…, Call-time resolver for an agent role model id. Delegates to…, Parse an LLM tool-call response, tolerating common malformations. Handles: 1.… (+18 more)
...
### Community 207 - "test_control_plane_api.py"
Cohesion: 0.03
Nodes (55): agent/scheduler.py — Scheduled Agent Jobs Cron-based job scheduler. Each job…, set_scheduler(), create_schedule(), delete_schedule(), get_schedule(), get_schedule_runs(), list_schedules(), BaseModel (+47 more)
...
### Community 213 - "TestSchedulerStore"
Cohesion: 0.07
...
### Community 217 - "MCPUnavailableError"
Cohesion: 0.12
...
### Community 223 - "test_microagents.py"
Cohesion: 0.15
...
### Community 235 - "AgentPlan"
Cohesion: 0.03
Nodes (73): Agent subsystem — planner / executor / verifier loop., AgentPhaseError, Exception, Raised when a named agent phase (planning, verification, etc.) fails., AgentEvent, AgentPlan, AgentSession, AgentSessionMessage (+65 more)
...
Cohesion: 0.10
Nodes (17): _now(), Any, Register a new job. Returns the created :class:`ScheduledJob`.…, Fire a job immediately (webhook / manual trigger)., Remove a job. Returns *True* if it existed., Update the display name of a job., Enable or disable a job without deleting it., Capture the FastAPI main event loop so APScheduler's background thread can… (+9 more)
...
### Community 255 - "validate_outbound_url"
Cohesion: 0.07
...
### Community 261 - "LLMRouter"
Cohesion: 0.06
Nodes (29): Attempt, text_key(), AgentPolicy, LLMConfig, Per-agent routing overrides (ADR-008 §"named profiles")., The fully-resolved routing configuration., Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys() (+21 more)
...
### Community 272 - "test_ceo_router.py"
Cohesion: 0.08
Nodes (43): build_ceo_router(), Any, APIRouter, _env_flag(), _env_int(), services/ceo_supervisor.py — the CEO's 24x7 babysitting loop. Delegation alone…, Read a bounded int, clamping out-of-range values rather than raising., Install (or clear) the singleton — used by startup wiring and tests. (+35 more)

### Community 273 - "test_failover_silent_exhaustion.py"
Cohesion: 0.10
...
### Community 279 - "seo_report_pdf.py"
Cohesion: 0.24
...
Cohesion: 0.08
Nodes (16): _get_settings(), Validated `reasoning_effort` for Ollama thinking models, or ``""``. Returns one…, Typed configuration loaded from environment variables., UNIT 8: when True, the catalog is mirrored to the DB + the ``GET…, When True, ship-code task types (portfolio_initiative / issue / quick_note) get…, When True (default), the agency's coding loop + Hermes prefer Cohere's ``north-…, Settings, The flag must default to ON (UNIT 8 — rollout complete, flag is the rollback… (+8 more)
...
### Community 290 - "test_workflow_models.py"
Cohesion: 0.13
...
Cohesion: 0.10
Nodes (25): _provider_env_value(), provider_max_parallel(), provider_max_rpm(), provider_max_tpm(), _provider_positive_float(), provider_weight(), Read a per-provider env var, tolerating dashes in the provider id. Provider ids…, Shared parse/validate for the numeric per-provider traffic budgets. Returns… (+17 more)

### Community 305 - "ProviderHealth"
Cohesion: 0.14
Nodes (8): _Outcome, ProviderHealth, Any, Claim a half-open probe slot. Returns False when none are free., Record a failed attempt and trip the breaker if thresholds are crossed.…, Take a provider out of rotation by hand (admin action)., Restore a provider immediately (admin action)., Rolling health for one provider.
...
### Community 307 - "DetectedSystem"
Cohesion: 0.04
Nodes (49): DetectedSystem, A business system detected on a company's website or in their stack., Get the most confident evidence description., OnboardingService, Workflow, Detect the Git provider from a repository URL. Args: repo_url: Repository URL…, Start the onboarding process for a company. Args: company_id: Company ID…, Schedule a fire-and-forget background task, keeping a strong reference.… (+41 more)
...
### Community 311 - "scan_repo_with_user_research.py"
Cohesion: 0.22
...
### Community 332 - "DashboardLayout.js"
Cohesion: 0.07
Nodes (31): deleteModel(), getRoutingPolicy(), listModels(), pullModel(), refreshRuntimeHealth(), runTaskOnRuntime(), startAllRuntimes(), startRuntime() (+23 more)
...
### Community 339 - "test_unit7_catalog_propagation.py"
Cohesion: 0.07
...
### Community 340 - "classify_direct_chat_intent"
Cohesion: 0.13
...
### Community 350 - "agency.py"
Cohesion: 0.03
Nodes (63): AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _fetch_github_quick_notes(), get_agency() (+55 more)

### Community 351 - "FeatureEntry"
Cohesion: 0.08
...
### Community 353 - "get_failover_manager"
Cohesion: 0.08
Nodes (29): openclaw_websocket(), websocket, WebSocket endpoint for iOS / mobile web UI pairing + command routing., get_failover_manager(), Return the singleton BrainFailoverManager., _cmd_chat(), _cmd_freebuff(), _cmd_list_files() (+21 more)
...
### Community 355 - "test_issue_triage.py"
Cohesion: 0.17
...
### Community 369 - "LLM Router — architecture"
Cohesion: 0.04
...
### Community 379 - "test_chat_mode_regressions.py"
Cohesion: 0.13
...
Cohesion: 0.05
Nodes (46): _get_provider_policy(), Read the durable provider policy from DB, falling back to a safe default.…, _best_cloud_primary_base(), TaskResult, TaskSpec, Execute a TaskSpec using the internal AgentRunner and convert the agent's…, Create an isolated execution context for a single task. Tries ``git worktree…, Clean up the worktree or temp copy created by ``_create_worktree``. (+38 more)
...
### Community 394 - "test_freebuff.py"
Cohesion: 0.10
...
### Community 397 - "RuntimeReadinessReport"
Cohesion: 0.10
...
### Community 400 - "TestBrainFailoverModelUpdates"
Cohesion: 0.12
...
### Community 402 - "RepowiseIntelligence"
Cohesion: 0.05
...
### Community 403 - "test_schedule_persistence.py"
Cohesion: 0.17
...
### Community 404 - "test_telegram_service_webhook.py"
Cohesion: 0.10
Nodes (17): Proactively push a Telegram approval-gate message with inline buttons. Sent…, Send a Telegram message with an inline keyboard to all configured chats., Best-effort secret/email/IP redaction for outbound Telegram/webhook messages., _redact_for_notification(), _FakeResponse, _make_task(), fixture, SimpleNamespace (+9 more)

### Community 405 - "run_trend_analysis"
Cohesion: 0.19
...
### Community 409 - "llm_providers.py"
Cohesion: 0.16
...
### Community 425 - "test_skill_executors_live.py"
Cohesion: 0.16
...
### Community 433 - "ContextCompressor"
Cohesion: 0.11
...
### Community 451 - "tasks/service.py"
Cohesion: 0.04
Nodes (68): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest (+60 more)
...
### Community 479 - "PersistentQueue"
Cohesion: 0.20
Nodes (6): PersistedRequest, PersistentQueue, Any, One queued request durable across a restart., Durable pending-work store backed by Redis, with a memory fallback.…, Everything accepted but not completed — the restart replay set.

### Community 480 - "WindowsServiceManager"
Cohesion: 0.18
...
### Community 486 - "DailyDigestAggregatorTests"
Cohesion: 0.19
...
### Community 493 - "test_task_pre_execution_gate.py"
Cohesion: 0.25
...
### Community 512 - "router/health.py"
Cohesion: 0.20
...
### Community 513 - "DockerAgentAdapter"
Cohesion: 0.17
...
### Community 514 - "AdminDigestRouterAuthTests"
Cohesion: 0.23
...
### Community 523 - "ProjectScaffolder"
Cohesion: 0.10
...
### Community 546 - "test_scanner_security.py"
Cohesion: 0.15
...
### Community 553 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 556 - "test_autonomy_pipeline_regressions.py"
Cohesion: 0.15
...
### Community 558 - "test_doctor_service_token_check.py"
Cohesion: 0.21
...
### Community 562 - "test_agent_chat_integration.py"
Cohesion: 0.23
...
### Community 565 - "asyncio"
Cohesion: 0.12
Nodes (8): requires_db, asyncio, Test that storage service can be initialized., Test company CRUD operations - skipped as requires specific config., Test that specialist service can be initialized., Test that onboarding service can be initialized., End-to-end against the real Mongo (CI service): the exact handler sequence…, get_company_graph must surface persisted detections, and delete_company must…
...
### Community 601 - "TestKillSwitchDurability"
Cohesion: 0.15
...
### Community 615 - "test_bootstrap_source_id_index.py"
Cohesion: 0.27
...
### Community 628 - "TestGetCurrentTimeTool"
Cohesion: 0.15
...
### Community 629 - "loop.py"
Cohesion: 0.22
Nodes (13): _check_extra_kwargs(), _enforce_signature(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, build_compaction_prompt(), build_execution_prompt() (+5 more)

### Community 630 - "TestRequireAdmin"
Cohesion: 0.29
Nodes (3): Return a FastAPI dependency that checks for a specific permission., require_permission(), TestRequireAdmin

### Community 631 - "plan_research"
Cohesion: 0.19
...
### Community 635 - "test_providers_live_e2e.py"
Cohesion: 0.27
...
### Community 642 - "test_workspace_ref_path_preflight.py"
Cohesion: 0.22
...
### Community 644 - "make_isolated_workspace"
Cohesion: 0.23
...
### Community 673 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 676 - "test_scanner_ssl_cert.py"
Cohesion: 0.25
...
Cohesion: 0.18
Nodes (13): _get_ceo_dispatcher(), Execute the plan via the selected specialist(s)., Return the shared CEODispatcher singleton (importable for monkeypatching)., GitHub token used to push branches / open PRs during EXECUTION (#506).…, _resolve_push_token(), _clean_env(), fixture, tests/test_orchestrator_push_token.py — #506 push/PR token resolution.… (+5 more)
...
### Community 679 - "test_agent_readiness_audit.py"
Cohesion: 0.18
...
### Community 680 - "test_empirical_verify.py"
Cohesion: 0.49
...
### Community 681 - "test_event_log.py"
Cohesion: 0.45
...
### Community 682 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 701 - "get_tool_registry"
Cohesion: 0.22
...
Cohesion: 0.15
Nodes (12): 1. Verify the router sees your providers, 2. Enable on one instance, 3. Watch for a few hours, 4. Roll out or roll back, Adding the config files, Gateway mode, LLM Router — migration guide, Migrating a caller to the router directly (+4 more)
...
### Community 715 - "test_permissions.py"
Cohesion: 0.15
...
### Community 718 - "TestMCPClientStructuredOutput"
Cohesion: 0.31
...
### Community 742 - "TestHarnessRegistry"
Cohesion: 0.17
Nodes (4): fixture, tests/test_harness_adapter.py — Tests for ECC cross-harness adapter., Harness session tracking and metrics., TestHarnessRegistry
...
### Community 744 - "test_ceo_cross_verify.py"
Cohesion: 0.29
...
### Community 746 - "test_shared_state.py"
Cohesion: 0.31
...
### Community 756 - "OrchestratorCheckpointStore"
Cohesion: 0.09
...
### Community 767 - "._get"
Cohesion: 0.22
Nodes (5): _ProviderUsage, Sliding-window usage counters for one provider., Count a request that is about to be sent to *provider_id*. Called immediately…, Record the outcome of a request started with ``record_start``. Must be called…, Count a request that was routed away from *provider_id* (diagnostics).
...
### Community 775 - "TestParseToolCalls"
Cohesion: 0.39
...
### Community 845 - ".discover_directory"
Cohesion: 0.29
...
### Community 852 - "._ensure_store"
Cohesion: 0.29
Nodes (4): Force-dedup and clean stale schedules from both the durable store and in-memory…, Delete EVERY schedule from the durable store and in-memory state. Operator…, #505: Remove a job from durable storage., get_scheduler_store()
...
### Community 867 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 869 - "Workspace Issues"
Cohesion: 0.29
...
### Community 870 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 872 - "security_fix_agent.py"
Cohesion: 0.57
...
### Community 916 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 930 - "webui/commands.py"
Cohesion: 0.47
...
### Community 950 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 951 - "Agent API Issues"
Cohesion: 0.40
...
### Community 952 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 957 - "scripts/agile_ceremonies.py"
Cohesion: 0.70
...
### Community 968 - "CircuitBreakerOpenError"
Cohesion: 0.50
...
### Community 984 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1000 - "test_skills_route_order.py"
Cohesion: 0.67
...
### Community 1033 - "test_direct_adapter_does_not_bypass"
Cohesion: 0.67
...
### Community 1034 - "_enable_filter"
Cohesion: 0.67
Nodes (3): _enable_filter(), fixture, Ensure filter is enabled for all tests.

## Knowledge Gaps
- **3335 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `aider_config.sh script`, `OPENAI_API_BASE` (+3330 more)
...
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `FreeBuffAgent`, `AgentSessionStore`, `_get_provider_policy`, `ExecutionRequest`, `proxy.py`, `NotificationDispatcher`, `AutonomyTracker`, `BackgroundAgent`, `workflow_orchestrator.py`, `ceo_dispatcher.py`, `TaskSpec`, `LocalWorkspace`, `direct_chat.py`, `test_agent_free_brain.py`, `test_ceo_dispatcher.py`, `failover_chat_completion`, `_resolve_push_token`, `test_empirical_verify.py`, `E2BSandboxSession`, `WorkflowRun`, `runtimes/api.py`, `test_ceo_supervision.py`, `test_daily_automation_2026_07_11.py`, `test_agent_chat_integration.py`, `KeyPool`, `ContextPruner`, `GitHubTools`, `WorkspaceTools`, `TestZeroAttemptDiagnostics`, `test_brain_failover_413.py`, `._execute_step`, `AdaptiveHalter`, `ContextManager`, `MCPUnavailableError`, `Workspace`, `MCPClient`, `ReactScratchpad`, `test_ceo_cross_verify.py`, `AgentPlan`, `StuckDetector`, `test_backend_server_features.py`, `_StubManager`, `TestGetCurrentTimeTool`, `loop.py`, `TokenBudget`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `TaskStore` connect `TaskStore` to `backend/server.py`, `test_trend_scoping.py`, `get_task_store`, `test_telegram_task_callback.py`, `test_task_source_id_race.py`, `services/background.py`, `test_portfolio_intake.py`, `TaskWorkflowService`, `test_phase6_workflow.py`, `test_task_brain_preflight.py`, `test_tasks_awaiting_approval_api.py`, `Task`, `TaskDispatcher`, `tasks/service.py`, `tasks/api.py`, `test_sqlite_store.py`, `test_issue_intake.py`, `AgentPlan`, `test_task_pre_execution_gate.py`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `WorkspaceTools` connect `WorkspaceTools` to `FreeBuffAgent`, `AgentSessionStore`, `MCPClient`, `AgentRunner`, `NotificationDispatcher`, `ToolRegistry`, `._execute_step`, `AgentPlan`, `webui/router.py`, `RepowiseIntelligence`, `loop.py`, `MCPUnavailableError`, `Workspace`, `validate_outbound_url`?**
