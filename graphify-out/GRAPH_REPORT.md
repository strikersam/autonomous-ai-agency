# Graph Report - autonomous-ai-agency  (2026-09-26)

## Corpus Check
- 1592 files · ~2,309,633 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 55 file(s) not represented in the graph (top: (none) 17, .bat 5, .css 5)

## Summary
- 33821 nodes · 68804 edges · 1429 communities (1222 shown, 207 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 6735 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5258834d`
...
- test_daily_automation_2026_08_16.py
- test_seo_audit.py
...
- test_send_webhook_posts_json_on_a_safe_target
...
- .is_self_repo_auto_commit_enabled
- .__init__
- .dream_count
- .memory_count
- .replay
- test_reasoning_effort_setting_validates_value
- .test_detects_throat_clearing
- .test_no_issues_on_clean_text
- .test_multiple_tell_types
- .test_detects_jargon

## God Nodes (most connected - your core abstractions)
1. `_fixture()` - 337 edges
2. `AgentRunner` - 235 edges
3. `Task` - 227 edges
4. `TaskStatus` - 143 edges
5. `TaskStore` - 142 edges
6. `ProviderRouter` - 140 edges
...
## Communities (1429 total, 207 thin omitted)

### Community 0 - "TaskSpec"
Cohesion: 0.03
Nodes (113): abc, 3b. Hermes — **our own** Hermes server (in-repo), UI-wired ✅  (size: M), kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., json_safe(), runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).… (+105 more)

### Community 1 - "typing"
Cohesion: 0.01
Nodes (347): Browser admin UI for login, service control, key management, and diagnostics., ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered) Runs the…, agent/background.py — Background Agent An always-on worker thread that…, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, Cached LLM Client wrapper. Drop-in wrapper around any LLM API call that…, get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-… (+339 more)

### Community 2 - "AgentRunner"
Cohesion: 0.01
Nodes (300): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, Adding a new tool, CLAUDE.md — agent/, Security surface, Skills worth invoking here, Testing, What this package does (+292 more)

### Community 3 - "Added"
Cohesion: 0.02
Nodes (249): get_agency(), set_agency(), Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., BudgetExceededError, Exception, Reset token counters for all sessions (caps preserved). Called at the start of…, Reset all budgets if the UTC calendar day has changed since last reset. Safe to… (+241 more)

### Community 4 - "brain_config.py"
Cohesion: 0.03
Nodes (108): patch_brain_policy_route(), Apply a partial brain config update. Hard constraint: every changed model id is…, Hard constraints (from the plan) — all met, New files, BrainConfig, BrainConfigPatch, BrainConfigStore, _build_base_url_env_from_yaml() (+100 more)

### Community 5 - "Changed"
Cohesion: 0.01
Nodes (245): Agency, AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _fetch_github_quick_notes() (+237 more)

### Community 6 - "test_refresh_agent_built_proof.py"
Cohesion: 0.12
...
### Community 7 - "LLMRouter"
Cohesion: 0.03
Nodes (95): Return True only when the tool is definitively read-only and non-destructive.…, Layer 3 — Model Router (`router/`), Added, Fixed, count, Added, Fixed, Bulkheads (+87 more)

### Community 8 - "backend/server.py"
Cohesion: 0.01
Nodes (443): get_sam(), set_skill_registry(), get_harness_adapter(), admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse (+435 more)

### Community 9 - "AgentScheduler"
Cohesion: 0.04
Nodes (54): gc, _age_seconds(), AgentScheduler, Any, Reconstruct a ScheduledJob from its as_dict() output., Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Fire a job immediately (webhook / manual trigger)., Remove a job. Returns *True* if it existed. (+46 more)

### Community 10 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.33
...
### Community 12 - "api.js"
Cohesion: 0.02
Nodes (59): Key files, addTaskComment(), approveGovernanceRequest(), approveTaskCheckpoint(), approveTaskExecution(), autoRecommendCompanySkills(), createMcpServer(), createQuickNote() (+51 more)

### Community 13 - "proxy.py"
Cohesion: 0.03
Nodes (165): AgentRunRequest, set_quick_note_queue(), fastapi_middleware_cors, middleware, admin_control(), admin_create_user(), admin_delete_user(), admin_login() (+157 more)

### Community 14 - "pytest"
Cohesion: 0.02
Nodes (96): fastapi_testclient, Reset the singleton (test helper)., reset_watchdog(), pytest, client(), _Cursor, TestClient, GET /api/activation/users must list registered users, not just toggled ones. It… (+88 more)

### Community 15 - "test_llm_router_queue_cache.py"
Cohesion: 0.03
Nodes (91): AlertHandler, BudgetTracker, Counter, _Dimensions, get_budget(), _month(), Any, packages/llm/budget.py — token and cost accounting with spend alerts. Tracks… (+83 more)

### Community 16 - "LLMRequest"
Cohesion: 0.03
Nodes (115): Added, Added, ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, OpenAICompatible, Any, AsyncClient, Perform one completion. Raises ``PermanentError``/``TransientError``. (+107 more)

### Community 17 - "Autonomy Charter — Telegram-Gated Self-Running Agency"
Cohesion: 0.08
Nodes (21): 1. Mission & operating principles, 2. Brain policy (free cloud LLMs), 3. The Gate Matrix (core artifact), 4. Telegram gate protocol, 5. The five autonomous loops, 7. Definition of "fully autonomous" — acceptance criteria, 8. Safety invariants (carried from `agent/CLAUDE.md`), 🟢 Autonomous — run, then notify-only (+13 more)

### Community 18 - "test_llm_router_strategies.py"
Cohesion: 0.06
Nodes (60): HealthConfig, Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds., RoutingConfig, HealthTracker, _Outcome, ProviderHealth, Any (+52 more)

### Community 19 - "test_operational_incidents.py"
Cohesion: 0.05
...
### Community 20 - "test_unit6_resolve_component_model.py"
Cohesion: 0.05
Nodes (37): tests/test_unit6_resolve_component_model.py — UNIT 6 regression tests. Verifies…, When the DB cache is fresh AND provider matches the active primary, the DB-…, When the DB primary differs from the requested provider, the catalog preset for…, When `provider` is None, the DB primary's saved model wins., With no DB cache and no provider, the env var is consulted., An unknown role returns the safe default rather than raising., An unknown provider (no catalog preset) falls through to env / safe default., The 'default' role is treated as 'executor'. (+29 more)

### Community 21 - "_fixture"
Cohesion: 0.01
Nodes (118): _clear_discovered_models(), _clear_response_cache(), _isolate_operator_provider_state(), MonkeyPatch, Path, Default all tests to legacy workflow mode (Phase 2 compatibility). Only patches…, Keep the suite out of the developer's real operator-state database.…, Drop the per-provider model listings between tests.… (+110 more)

### Community 22 - "Task"
Cohesion: 0.03
Nodes (80): queue_fix_task(), Queue an agent task to fix *alert*. Returns (task_id, newly_created)., Enum, str, Full task/issue document., Task, TaskPriority, Any (+72 more)

### Community 23 - "llm/router.py"
Cohesion: 0.03
Nodes (96): collections_abc, heapq, int, failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Router-backed implementation of ``packages.ai.failover_client``'s entry point.… (+88 more)

### Community 24 - "test_ceo_micromanager.py"
Cohesion: 0.04
Nodes (108): Configuration reference, Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, Run one sub-task, judge the result, and escalate it if it is slop. The loop is…, _runtime_id_for_role(), build_subtask_brief(), _coerce_subtasks() (+100 more)

### Community 25 - "user_research_skill.py"
Cohesion: 0.07
Nodes (36): analyze_quantitative(), _classify_sentiment(), _extract_keywords(), BaseModel, QualAnalysis, QualQuote, QualTheme, QuantAnalysis (+28 more)

### Community 26 - "test_governance_sandbox.py"
Cohesion: 0.03
Nodes (83): 1.11 Multi-Agent Governance (10 / 100 / 1000 agents), Failure behaviour, P1 — Close the remaining product gaps, Task 6 — Deeper website/repo scanning (more than "2 systems"), Task 7 — Knowledge base auto-reflect (ECC / Obsidian / graphify-style), Task 8 — Vector retrieval for the knowledge base (self-learn), Convenience: build and record an event in one call., record_event() (+75 more)

### Community 27 - "PolicyEngine"
Cohesion: 0.04
Nodes (87): fnmatch, _action_matches(), _as_list(), Decision, GroupPolicy, _host_matches(), Mode, _normalise_path() (+79 more)

### Community 28 - "test_governance_enforcement.py"
Cohesion: 0.04
Nodes (79): 5. What was implemented, Measurable improvements, 1.12 Cost Governance, BudgetTracker, GovernanceGate, Per-session consumption counters and their ceilings. Ceilings come from the…, Return the name of the first exhausted session-wide limit, or None. Only covers…, Return a reason string if *tool* has hit its per-tool session cap. Checks only… (+71 more)

### Community 29 - "build_llm_router"
Cohesion: 0.04
Nodes (58): get_cache(), The process-wide cache manager., gateway_enabled(), Whether the OpenAI-compatible passthrough routes accept traffic. Off by…, Whether callers should route through ``LLMRouter`` (ADR-008 §8). Defaults to…, router_enabled(), _available(), build_llm_router() (+50 more)

### Community 30 - "build_governance_router"
Cohesion: 0.06
Nodes (56): build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget(), get_metrics() (+48 more)

### Community 31 - "RenderOpsMonitor"
Cohesion: 0.05
Nodes (33): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, One deploy, normalised from whatever shape the tool returned., RenderDeploy, Polls Render for platform-level failures and files them as issues., RenderOpsMonitor, _client() (+25 more)

### Community 32 - "TaskStatus"
Cohesion: 0.03
Nodes (102): AgentDefinition, Any, field_validator, A user-defined agent configuration stored in the database., Set the global agent store instance (e.g., with MongoDB on startup)., Accept both ISO-8601 datetime strings (from DB) and float timestamps., Keep legacy backend fields and Control Plane UI fields aligned., set_agent_store() (+94 more)

### Community 33 - "RequestQueue"
Cohesion: 0.04
Nodes (34): Bulkhead, Deduplicator, Any, Semaphore, T, Collapse concurrent identical calls onto a single execution., Execute ``factory``, or await an identical call already running. Returns…, One job waiting for a concurrency slot, ordered by (priority, arrival). (+26 more)

### Community 34 - "cache.py"
Cohesion: 0.06
Nodes (25): Agency Core — Operational Knowledge (verified live, 2026-06-10/11), Architecture truths, Open backlog (epic #504), Pros of linking the GitHub repo (vs running unlinked), Runbooks, CacheManager, cosine_similarity(), _Entry (+17 more)

### Community 35 - "telegram_bot.py"
Cohesion: 0.04
Nodes (90): Workflow Orchestrator Approval Gate (🔴 Telegram gate), _admin_headers(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost(), cmd_freebuff(), cmd_keylist() (+82 more)

### Community 36 - "get_ceo_ledger"
Cohesion: 0.11
Nodes (24): build_ceo_router(), ceo_status(), get_goal(), list_goals(), redrive_goal(), run_sweep(), Any, APIRouter (+16 more)

### Community 37 - "._stub_curl"
Cohesion: 0.18
Nodes (3): The critical guard: when both tiers hit a Cloudflare wall, the fallback returns…, Tier 1 hits the Cloudflare wall; the headless browser clears the automatic JS…, fake_render()

### Community 38 - "test_llm_router_disabled.py"
Cohesion: 0.09
Nodes (36): auto_disable(), _billing_signals(), describe(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable., Persist a provider as disabled, through the store that already owns it. (+28 more)

### Community 39 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (74): _decompose_into_subtasks(), get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, reset_ceo_dispatcher(), get_ceo_fallback_stats(), Increment the named counter (silently ignores unknown reasons). (+66 more)

### Community 40 - "AnthropicProvider"
Cohesion: 0.04
Nodes (48): Added, Added, AnthropicProvider, Any, AsyncClient, Translate OpenAI-shaped messages into Anthropic's system/turn split., OpenAI carries a tool result as role="tool" with a tool_call_id; Anthropic…, OpenAI puts a tool call alongside the assistant's text; Anthropic needs… (+40 more)

### Community 41 - "company_api.py"
Cohesion: 0.01
Nodes (431): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+423 more)

### Community 42 - "ToolRegistry"
Cohesion: 0.05
Nodes (32): Any, Path, Register a tool definition., Decorator to register a function as an agent tool. Usage::…, Remove a tool from the registry. Returns True if removed., Look up a tool by name., Return all registered tools., Find tools that advertise a specific capability. (+24 more)

### Community 43 - "CEOLedger"
Cohesion: 0.08
...
### Community 44 - "test_control_plane_api.py"
Cohesion: 0.06
...
### Community 45 - "looks_like_secret_file"
Cohesion: 0.04
Nodes (94): onboarding_gate_enabled(), Async read of the gate default straight from the DB., loops_overview(), Full Loop Engineering fleet view for the UI: the catalogued loops plus the…, _get_cached_tasks(), _get_tasks_cache_lock(), Lock, Return cached task dicts; fetches + caches from TaskStore when stale. Uses… (+86 more)

### Community 46 - "failover_chat_completion"
Cohesion: 0.07
Nodes (78): BrainFailoverExhausted, failover_chat_completion(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, Run one chat completion across the brain-failover chain. Tries each healthy…, _free_tier(), _hit_ids(), _many_providers() (+70 more)

### Community 47 - "test_llm_router_resilience.py"
Cohesion: 0.04
Nodes (60): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), get_ring(), KeyRing (+52 more)

### Community 48 - "SeoAuditEngine"
Cohesion: 0.06
Nodes (36): BaseModel, field_validator, A single occurrence of a check firing on a specific URL., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoSiteFindings (+28 more)

### Community 49 - "check_model_catalog_consistency.py"
Cohesion: 0.22
Nodes (14): _check_cross_catalogue(), _check_prefer_models(), _check_presets(), _llm_declared(), _load(), main(), Any, Path (+6 more)

### Community 50 - "resolve_active_brain"
Cohesion: 0.04
Nodes (79): BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), _norm(), _pick_from_records(), _build() (+71 more)

### Community 51 - "CompanyGraphService"
Cohesion: 0.02
Nodes (52): BusinessCategory, requires_db, CompanyGraphService, SpecialistFamily, SystemType, Get a company by ID. Args: company_id: Company ID Returns: Company instance or…, Update a company. Args: company_id: Company ID **kwargs: Fields to update…, Delete a company and all its associated data. The store only clears the… (+44 more)

### Community 52 - "test_agent_runner.py"
Cohesion: 0.05
Nodes (53): _make_runner(), Path, Regression for #1422 item 3: when the model never returns a JSON object, the…, The agent LLM call must use the BrainConfig budget, not a hardcoded 16384.…, Reproduce the exact error from the screenshot: slices present, goal absent., Reproduce the production failure: a planner emits steps keyed by `name` with no…, End-to-end: model returns CRISPY-style slices, plan should still execute., spawn_subagent creates a child AgentRunner and returns a condensed result. (+45 more)

### Community 53 - "UserRole"
Cohesion: 0.11
Nodes (31): Finding A — `list_for_user` Mongo query diverges from the `_can_read` policy, Finding B — `/api/secrets` router is mounted with no authentication dependency, Minor, non-security, Part 1 — The audit, Suggested fixes (not applied here), The chain, What was checked and is sound, UserRole (+23 more)

### Community 54 - "runtimes/api.py"
Cohesion: 0.05
Nodes (64): _enrich_runtimes(), get_policy(), _load_rich_policy(), PolicyUpdateBody, Any, BaseModel, post, put (+56 more)

### Community 55 - "test_scanner_security.py"
Cohesion: 0.11
...
Cohesion: 0.06
Nodes (20): idle, Charts, Donut(), ExecutionTimeline(), Sparkline(), ErrorBoundary, useSafeData(), DashboardScreen() (+12 more)

### Community 57 - "ExecutionRequest"
Cohesion: 0.05
Nodes (41): Execute work through the 11-phase golden path., workflow_orchestrator_execute(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any, Emit an alert to the activity feed and log., Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor() (+33 more)

### Community 58 - "ProviderConfig"
Cohesion: 0.06
Nodes (43): is_commercial_provider(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Infer an access tier from *hostname*, longest matching suffix winning. Returns…, _tier_from_hostname(), _nvidia_provider_chain() (+35 more)

### Community 59 - "ProviderRouter"
Cohesion: 0.07
Nodes (27): ProviderRouter, Priority-ordered LLM provider fallback with health checks and retries., main(), probe_one(), _bedrock_api_response(), _bedrock_provider(), _mock_boto3(), Any (+19 more)

### Community 60 - "WorkflowOrchestrator"
Cohesion: 0.04
Nodes (44): Changed, Changed, Feature maturity — what's stable vs. beta, Actively wake every sleeping/circuit-open runtime. The default health service…, get_orchestrator_checkpoint_store(), Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Determine domain and task type from the request text. (+36 more)

### Community 61 - "test_render_mcp.py"
Cohesion: 0.04
Nodes (49): Render MCP — platform debugging and environment monitoring, Write safety, _as_list(), _coerce_payload(), Any, RuntimeError, packages/integrations/render_mcp.py — Render platform access over MCP. The…, Return tool output as Python data. MCP tool results arrive either as… (+41 more)

### Community 62 - "test_web_reach.py"
Cohesion: 0.04
Nodes (75): Ask the C-suite advisory layer a business question. This is the CEO's bridge…, Index ``root`` once in a daemon thread when the code graph is enabled. Without…, start_warm_up(), _domain_list_reason(), _domain_matches(), _egress_policy_reason(), _parse_domain_list(), Return True if *host* is *pattern* or a subdomain of it. (+67 more)
...
Cohesion: 0.04
Nodes (69): _build_default_base_url_from_yaml(), _build_display_names_from_yaml(), _build_key_env_from_yaml(), _build_tier_from_yaml(), get_provider_candidates(), get_provider_display_name(), get_provider_tier(), _load_models_yaml() (+61 more)

### Community 65 - "get_task_store"
Cohesion: 0.06
Nodes (58): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover() (+50 more)

### Community 66 - "FreeBuffAgent"
Cohesion: 0.04
Nodes (62): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is… (+54 more)

### Community 67 - "test_model_router.py"
Cohesion: 0.04
Nodes (89): Health check and availability filtering, classify_task(), _extract_recent_text(), Any, Concatenate plain text from the last *last_n* messages., Return the most likely task category for this request. Args: messages: OpenAI-…, _enabled(), get_available_models() (+81 more)

### Community 68 - "SeoAuditReport"
Cohesion: 0.13
Nodes (27): html, io, Complete result of one audit run., SeoAuditReport, Paragraph, reportlab_lib, reportlab_lib_pagesizes, reportlab_lib_styles (+19 more)

### Community 69 - "config.py"
Cohesion: 0.03
Nodes (90): AgentPolicy, _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir(), _env_key_names(), expand_env() (+82 more)

### Community 70 - "require_power_user"
Cohesion: 0.13
Nodes (29): FastAPI dependency: require Power User or Admin role. Raises 403 otherwise., require_power_user(), add_peer(), get_folder_index(), get_sync_file(), get_sync_service(), list_conflicts(), list_peers() (+21 more)

### Community 71 - "setup/api.py"
Cohesion: 0.07
...
### Community 72 - "V5App.jsx"
Cohesion: 0.05
Nodes (44): getActivity(), HubTabs(), Spinner(), activityToAlert(), AlertsBell(), priorityConfig, _relativeTime(), typeIcon (+36 more)
...
### Community 74 - "test_crispy_burn_in.py"
Cohesion: 0.04
Nodes (42): importlib, asyncio, Tests for app_settings — DB-persisted settings + onboarding-gate default. These…, is_user_onboarding_allowed falls back to the global default for users with no…, test_defaults_when_unset(), test_gate_default_controls_unlisted_user(), test_refresh_cache_warms_sync_readers(), test_set_and_get_persists() (+34 more)

### Community 75 - "AgentSessionStore"
Cohesion: 0.09
Nodes (23): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentSession, AgentSessionCreateRequest, AgentSessionMessage, BaseModel, A single entry in the session's append-only event log. Inspired by Anthropic's…, VerificationResult (+15 more)

### Community 76 - "test_startup_warmup.py"
Cohesion: 0.14
Nodes (9): _isolate_warmup_overflow(), Regression tests for the bounded startup warm-up and login bootstrap. Uvicorn…, Whatever triggers it, wiring must register the pair., It has to be cheap, or it cannot live outside the warm-up budget., ``_warmup_overflow`` is process-global — keep tests from sharing it., Guard against an index silently dropping out of the data table., test_bootstrap_index_declaration_covers_the_known_collections(), test_wire_feature_stores_registers_both_stores() (+1 more)

### Community 77 - "ResilientFetcher"
Cohesion: 0.15
...
### Community 78 - "PreflightReport"
Cohesion: 0.04
Nodes (78): PreflightIssue, PreflightReport, BaseModel, AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState, FailedJob (+70 more)

### Community 79 - "4. Troubleshooting"
Cohesion: 0.10
...
### Community 80 - "SelfHealingAgent"
Cohesion: 0.07
Nodes (32): Translate external failure signals into improvement tasks and verify the fix…, Launch the background sweeper that resolves quiet verifying heals., Signal that the fix for *signature* has been applied/merged. Transitions the…, Same as :meth:`mark_fix_landed` but keyed by event/issue id (the id the…, Resolve any verifying heal whose quiet window has elapsed. Returns the number…, E2: Return a corrective hint for each failure category., Build a *self-contained, actionable* escalation message. The previous message…, Best-effort: reflect a verified heal back into the ImprovementLoop state so the… (+24 more)

### Community 81 - "seo_portfolio_bridge.py"
Cohesion: 0.05
...
### Community 84 - "services/background.py"
Cohesion: 0.05
Nodes (72): _dispatch_async(), _run(), get_log_monitor(), LogMonitor, _note_recurrence(), Best-effort, synchronous: tell the healer this signature was seen again., Fire-and-forget: schedule a self-healing task from any thread., Attach to the root logger and forward errors to the self-healing agent. Usage::… (+64 more)

### Community 85 - "get_user_role"
Cohesion: 0.09
Nodes (18): get_audit_log(), get_user_role(), is_admin(), is_power_user_or_above(), mask_dict(), _mask(), mask_secret(), Any (+10 more)

### Community 86 - "test_ceo_supervision.py"
Cohesion: 0.06
Nodes (63): A subtask's full history: what it is, and every attempt at it., SubtaskRecord, _goal(), asyncio, Tests for the CEO ledger, the supervised escalation loop, and the 24x7 sweeper.…, A storage outage must never take the agency down., Two sweeping processes must not both re-drive the same goal.…, A sweeper that loses the claim reports in-flight, not redriven. (+55 more)
...
Cohesion: 0.04
Nodes (52): browserslist, development, production, dependencies, axios, fast-uri, livekit-client, lucide-react (+44 more)

### Community 89 - "test_provider_router.py"
Cohesion: 0.05
Nodes (50): _acquire_provider_probe(), _normalize_nvidia_base_url(), _openai_url(), Try to acquire a distributed probe lock for *provider_id*. Returns True if this…, Release the probe lock for *provider_id*., Normalize NVIDIA base URLs to avoid double /v1 when openai_compat_url appends…, _release_provider_probe(), _best_cloud_primary_base() (+42 more)

### Community 90 - "test_sqlite_store.py"
Cohesion: 0.05
Nodes (62): _push_down_where(), Build a SQL ``WHERE`` suffix from the subset of *query* conditions that map…, asyncio, tests/test_sqlite_store.py — Unit tests for the SQLite storage adapter. These…, The exact query shape backend/server.py's provider "Set default" uses: clear…, Unfiltered count uses the SELECT COUNT(*) fast path and must match the number…, estimated_document_count mirrors an unfiltered count_documents., db['tasks'] must work like db.tasks (motor exposes both). (+54 more)
...
### Community 92 - "test_all_features.py"
Cohesion: 0.03
...
### Community 93 - "TestOrchestratorQueue"
Cohesion: 0.13
...
### Community 95 - "PrimeAgentAdapter"
Cohesion: 0.04
...
### Community 97 - "ImprovementLoop"
Cohesion: 0.05
Nodes (59): DetectedIssue, get_improvement_loop(), ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity, _now(), Any (+51 more)

### Community 98 - "AuditLog"
Cohesion: 0.05
Nodes (38): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log. (+30 more)

### Community 99 - "test_shared_state.py"
Cohesion: 0.07
Nodes (24): claim(), cooldown_get(), cooldown_set(), incr_window(), Reset the singleton (for tests)., Try to acquire a named lock. Returns True if acquired, False if already held., Release a previously acquired lock., Put a key on cooldown for *ttl* seconds. (+16 more)

### Community 100 - "AgentJobResult"
Cohesion: 0.08
...
### Community 104 - "RepowiseIntelligence"
Cohesion: 0.04
...
### Community 106 - "workflow_orchestrator.py"
Cohesion: 0.07
...
### Community 107 - "WorkspaceManager"
Cohesion: 0.06
...
### Community 108 - "test_colibri_provider.py"
Cohesion: 0.13
...
### Community 109 - "TestClient"
Cohesion: 0.12
Nodes (23): _call(), _data(), _is_error(), Path, skipif, TestClient, Comprehensive MCP workspace git operation tests. Tests the full JSON-RPC path:…, Send a JSON-RPC 2.0 request to the mounted MCP server. (+15 more)

### Community 110 - "BrowserSession"
Cohesion: 0.05
...
Cohesion: 0.05
Nodes (59): _bullets(), generate_backlog_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive a retrospective from the task tracker when no sprint is active. DONE /…, The result of allocating portfolio capacity into a new sprint. (+51 more)

### Community 113 - "tasks/api.py"
Cohesion: 0.14
...
### Community 117 - "service_daemon.py"
Cohesion: 0.04
Nodes (65): fastapi_responses, fastapi_staticfiles, BaseModel, post, Autonomous AI Agency Launcher - One-button service start with web UI. Run this…, start_proxy(), start_tunnel(), StatusResponse (+57 more)

### Community 118 - "AgentJobManager"
Cohesion: 0.02
Nodes (84): AgentJob, AgentJobManager, heartbeat(), make_isolated_workspace(), _now(), Any, Path, Run a job using the provided runner and update the job's lifecycle, progress,… (+76 more)

### Community 119 - "AgileSprint"
Cohesion: 0.06
...
### Community 121 - "resolve_e2b_config"
Cohesion: 0.05
...
### Community 122 - "pr_approval_gate.py"
Cohesion: 0.14
...
### Community 125 - "WorkspaceTools"
Cohesion: 0.05
Nodes (42): Register the built-in agent tools that are always available., _register_builtin_tools(), Return a previously saved memory value, or an empty string if absent., Persist a key/value pair to the user's profile store., Delegate to RepowiseIntelligence for a natural-language codebase question., Delegate to RepowiseIntelligence for a semantic/text codebase search., Delegate to RepowiseIntelligence to list architectural decision nodes., Return a high-level repository map and hotspot summary. (+34 more)

### Community 126 - "Next Action"
Cohesion: 0.09
Nodes (22): #1565 nightly regression (2026-09-25, row 80), Current state (2026-09-24), Daily automation 2026-09-25 — DONE (row 80), Daily automation 2026-09-25 (row 81), Daily automation 2026-09-26 (row 82), Next Action, Next daily run (2026-09-26), Previous state (+14 more)
...
### Community 128 - "NIMConnectionPool"
Cohesion: 0.05
Nodes (27): Security, NIMConnectionPool, ProviderCircuit, Any, AsyncClient, Response, Attempt to move from OPEN to HALF_OPEN after recovery timeout., Check if a request can be made through this circuit. (+19 more)

### Community 129 - "_record_id"
Cohesion: 0.25
...
### Community 130 - "KnowledgeScreen.jsx"
Cohesion: 0.04
...
### Community 132 - "test_vision_routing.py"
Cohesion: 0.13
Nodes (10): best_vision_model(), has_image_content(), ModelCapability, Model capability registry. Defines the known local models, their strengths, and…, # NOTE: suspended under US export-control directive as of 2026-06-12., Return the name of the best registered vision-capable model, or None. Prefers…, Return True if any message contains an image_url content part., Tests for vision request routing and session ID propagation. Covers: -… (+2 more)
...
### Community 135 - "test_trend_scoping.py"
Cohesion: 0.10
...
### Community 136 - "TaskDispatcher"
Cohesion: 0.07
Nodes (36): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, _make_task(), asyncio, tests/test_dispatcher_iso8601.py — regression test for the TaskDispatcher auto-… (+28 more)
...
### Community 138 - "test_llm_router_e2e.py"
Cohesion: 0.09
...
### Community 142 - "clear_cooldowns"
Cohesion: 0.07
Nodes (35): clear_cooldowns(), _dead_model_key(), _is_model_dead(), is_provider_on_cooldown(), mark_provider_failed(), Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown., Clear all cooldown entries (useful for testing). Delegates to… (+27 more)

### Community 143 - "E2BSandboxSession"
Cohesion: 0.04
...
### Community 145 - "test_loop_registry.py"
Cohesion: 0.08
...
Cohesion: 0.07
Nodes (34): 1. Authentication & Authorization, API Key Authentication, JWT / Token Auth, Category 2 — API Key Naming Confusion, TD-005 [MEDIUM] — Production Keys Have `test-key-` Prefix, _check_rate_limit(), default_keys_path(), issue_new_api_key() (+26 more)

### Community 147 - "AdminIdentity"
Cohesion: 0.04
Nodes (32): Admin Authentication, Acceptance Checks, `admin_auth.py` checklist, `agent/tools.py` checklist, Escalation, Instructions, `key_store.py` checklist, `proxy.py` auth middleware checklist (+24 more)

### Community 148 - "ai_runner.py"
Cohesion: 0.08
...
### Community 149 - "ChatHistoryStore"
Cohesion: 0.07
...
### Community 150 - "test_issue_intake.py"
Cohesion: 0.08
...
### Community 152 - "ENGINEERING_STANDARDS.md — Patterns & Reference"
Cohesion: 0.08
...
### Community 154 - "CEODispatcher"
Cohesion: 0.07
Nodes (32): CEODispatcher, _runner_factory(), CEOResult, _complexity_rank(), Aggregated output from a multi-specialist execution., Build a single-task sub-task list (low-complexity fast path)., Real CEO delegation: wake runtimes, fan out, and merge results., Decompose the request, wake runtimes, run sub-tasks concurrently, merge.… (+24 more)
...
### Community 157 - "secrets_store.py"
Cohesion: 0.13
Nodes (25): create_secret(), delete_secret(), get_secret_metadata(), get_secrets_store(), _get_user(), list_secrets(), Any, BaseModel (+17 more)

### Community 158 - "KeyPool"
Cohesion: 0.08
...
### Community 159 - "OperationalIncident"
Cohesion: 0.09
Nodes (24): _format_incident(), OperationalIncident, _phase_section(), Any, Strip credentials from text before it is written into an issue body. Failure…, One failure mode that recurred often enough to warrant diagnosis., Live counters, for the diagnostics endpoint and tests., The in-process phase view — authoritative, and the reason this works with no… (+16 more)

### Community 160 - "Settings"
Cohesion: 0.06
Nodes (20): _get_settings(), Typed configuration loaded from environment variables., When True, the governance layer evaluates and audits agent actions. This is…, When True, approval-gated actions self-approve. Local dev only., ``RENDER_SERVICE_IDS`` split into a clean list (empty when unset)., When True, mutating Render MCP tools may be called. Default False., When True, agents may drive a real browser (agent/browser.py)., True when a Browserbase key is present — the low-RAM remote path. (+12 more)

### Community 161 - "test_response_cache.py"
Cohesion: 0.12
...
### Community 162 - "test_brain_config_api.py"
Cohesion: 0.07
Nodes (35): Architecture (per plan §3), Implementation — DB-persisted, UI-switchable Brain (PR #824 follow-up), Resolution precedence, Risks & mitigations (per plan §6), Rollout / verification (per plan §5), Tests, What this is NOT, ProbeResult (+27 more)

### Community 163 - "NotificationDispatcher"
Cohesion: 0.07
...
### Community 165 - "PortfolioManager"
Cohesion: 0.07
Nodes (18): PortfolioManager, Manages a portfolio of initiatives with WSJF prioritisation and roadmapping., Remove an initiative from the portfolio., Number of initiatives in the portfolio., Link an agile sprint to an initiative it helps deliver., Tests for agents/portfolio.py — Agentic Portfolio Management. Uses importlib to…, WSJF ranking is the heart of portfolio management., Greedy capacity allocation by WSJF. (+10 more)

### Community 166 - "seo_api.py"
Cohesion: 0.04
Nodes (87): _get_current_user_thunk(), _get_optional_user_thunk(), Request, build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit() (+79 more)

### Community 167 - "TrafficDirector"
Cohesion: 0.08
...
### Community 168 - "ContextWindowManager"
Cohesion: 0.08
...
### Community 169 - "test_hermes_in_process.py"
Cohesion: 0.06
...
### Community 170 - "tasks/models.py"
Cohesion: 0.04
Nodes (64): tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, CommentAddRequest, ExecutionApprovalRequest, ExecutionLogEntry, FollowUpRequest (+56 more)

### Community 171 - "._log_event"
Cohesion: 0.09
Nodes (19): _extract_slices_from_plan(), Any, Connection, Path, PhaseType, Append an event to the workflow event log., Return current WorkflowRun snapshot or None., Reject the plan — marks run as failed. (+11 more)

### Community 172 - "v4_api.py"
Cohesion: 0.03
Nodes (80): _load(), persona_for_family(), agents/persona_library.py — role personas for auto-provisioned specialists.…, System prompt for a provisioned specialist of ``family`` ('' if none). The…, _load_improvement_state(), Any, BaseModel, get (+72 more)

### Community 173 - "test_agent_tool_governance.py"
Cohesion: 0.09
...
### Community 174 - "WorkflowRun"
Cohesion: 0.11
Nodes (22): engine(), _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test() (+14 more)

### Community 175 - "test_mcp_registry.py"
Cohesion: 0.05
Nodes (30): Measure one server. Never raises — a probe failure is a status, not an error., Measure every declared server concurrently and return dashboard rows. Probes…, status_all(), _status_for(), _noop_async(), Any, asyncio, tests/test_mcp_registry.py — the single MCP catalogue and the screen it backs.… (+22 more)

### Community 176 - "_resolve_brain_provider"
Cohesion: 0.06
Nodes (27): Resolve the LLM endpoint for agent execution (module-level, #522 failover).…, _resolve_brain_provider(), _prio(), _mock_provider_records(), tests/test_orchestrator_failover.py — Provider failover, brain resolution,…, Non-Ollama providers without an API key are skipped., Ollama-type providers don't need an API key., When _list_configured_provider_records raises, falls back to OLLAMA_BASE. (+19 more)

### Community 177 - "test_schedule_growth_invariants.py"
Cohesion: 0.05
...
### Community 178 - "WorkflowEngine"
Cohesion: 0.09
Nodes (49): approve(), build(), cancel(), _engine(), get_agent_team(), get_artifact_content(), get_events(), get_run() (+41 more)

### Community 179 - "MemoryCategory"
Cohesion: 0.15
...
### Community 180 - "AutonomyTracker"
Cohesion: 0.05
Nodes (20): AutonomyCounter, AutonomySnapshot, AutonomyTracker, Any, Return a point-in-time snapshot of all KPIs., Reset all counters (test helper)., Reset the singleton (test helper)., Thread-safe counter for a single KPI metric. (+12 more)

### Community 181 - "test_verification_strategies.py"
Cohesion: 0.09
...
### Community 182 - "enforcement.py"
Cohesion: 0.06
Nodes (42): 4. Security review, Deliberate design decisions, stated for review, Findings addressed during implementation, Verification status — stated precisely, events_from_identity(), Extract the identity-derived audit fields from an AgentIdentity. Kept here…, BudgetExceeded, classify() (+34 more)

### Community 183 - "KnowledgeGraph"
Cohesion: 0.07
...
### Community 184 - "SetupWizardPage.js"
Cohesion: 0.08
...
### Community 185 - "test_sam_livekit.py"
Cohesion: 0.06
Nodes (26): jwt, auth_headers(), livekit_env(), no_livekit_env(), tests/test_sam_livekit.py — SAM realtime voice (LiveKit) integration. Covers: -…, SAM_LLM_* env vars must override the NVIDIA defaults (Hermes/proxy routing)., Under TESTING the in-process worker must never be eligible to start., start_in_process must be a safe no-op in the test environment (conftest sets… (+18 more)

### Community 186 - "api_keys_for"
Cohesion: 0.13
...
### Community 187 - "test_video_transcript.py"
Cohesion: 0.05
...
### Community 188 - "scan_repo_with_user_research.py"
Cohesion: 0.11
...
### Community 189 - "MCPClient"
Cohesion: 0.04
Nodes (38): MCPClient, Any, Thin async MCP client with open/close circuit breaker. Thread-safe only within…, Full URL of the JSON-RPC endpoint this client posts to., Build the request headers shared by ``_rpc`` and ``notify``. ``Accept`` lists…, Propagate the calling agent's identity across the process boundary. The MCP…, Attach the agent identity whose actions this client executes., Remember a server-issued ``Mcp-Session-Id`` for subsequent requests. (+30 more)

### Community 190 - "control_overrides.py"
Cohesion: 0.05
Nodes (68): _company_advisory_context(), Pull a compact company profile for the C-suite advisory, or None. Best-effort:…, default_executives(), get_executive_advisory(), Return the shared :class:`ExecutiveAdvisory`, built on first use., Return the built-in C-suite personas., _is_admin(), _is_ephemeral_user() (+60 more)

### Community 191 - "openclaw_gateway.py"
Cohesion: 0.12
Nodes (23): openclaw_websocket(), websocket, WebSocket endpoint for iOS / mobile web UI pairing + command routing., _cmd_chat(), _cmd_freebuff(), _cmd_list_files(), _cmd_read_file(), _cmd_status() (+15 more)

### Community 192 - "Implementation Prompt: Rich TaskBoard + Agile Sprint Integration"
Cohesion: 0.08
...
### Community 193 - "_build_anthropic_response"
Cohesion: 0.09
Nodes (27): _build_anthropic_response(), _emit_safely(), _finish_reason_to_stop_reason(), handle_anthropic_messages(), _messages_to_openai(), _openai_choice_to_anthropic_content(), _post_anthropic_with_fallback(), Any (+19 more)

### Community 194 - "Workspace"
Cohesion: 0.08
...
### Community 196 - "ManagedAgentDreams"
Cohesion: 0.22
...
### Community 199 - "test_daily_2026_06_04.py"
Cohesion: 0.07
Nodes (44): _content_block_to_text(), Convert a single Anthropic content block to a plain text string., Adding a model, Adding a task category, CLAUDE.md — router/, Environment variables, Testing, What this package does (+36 more)

### Community 200 - "SyntheticDataPipeline"
Cohesion: 0.07
...
### Community 202 - "test_features_api.py"
Cohesion: 0.05
Nodes (4): _auth_override(), client(), _fake_auth(), Integration tests for all new feature API routes in proxy.py.

### Community 203 - "WorkspaceManifest"
Cohesion: 0.08
Nodes (23): _derive_workspace_root(), WorkspaceStatusLiteral, Create an isolated workspace for a session and optional job. Creates the…, Retrieve the WorkspaceManifest for a given session and optional job. Looks up…, List all known workspaces, optionally filtered by status., Mark a workspace as active (in-use)., Pause a workspace (e.g. between agent steps)., Mark a workspace as completed. (+15 more)

### Community 204 - "mcp_registry.py"
Cohesion: 0.10
...
### Community 206 - "test_sam_voice.py"
Cohesion: 0.06
...
### Community 208 - "portfolio_api.py"
Cohesion: 0.08
...
### Community 210 - "nvidia_models.py"
Cohesion: 0.09
Nodes (20): Example: Access your home PC models from any machine. Install: pip install…, build_review_context(), _gh(), main(), Aggregate all review feedback for the PR into a single context string., _is_chat_model(), _rank_key(), rank_models() (+12 more)

### Community 211 - "analyze_qualitative"
Cohesion: 0.12
...
### Community 214 - "test_classify_dependabot_update.py"
Cohesion: 0.07
...
### Community 215 - "Fixed"
Cohesion: 0.03
Nodes (75): serve_spa(), Fixed, Fixed, Active Task Tracker, Bug Log, Current Sprint Tasks, Roadmap Items (from `docs/roadmap-killer-todos.md`), Fixed (+67 more)

### Community 216 - "test_repo_connection.py"
Cohesion: 0.11
Nodes (26): attach_repo_connection(), build_repo_connection(), detect_delivery_policy(), parse_repo_url(), provider_of(), Exception, Detect a repo's :class:`DeliveryPolicy` (GitHub-only). - Protected default…, Build a GitHub :class:`RepoConnection` from a repo URL. Raises… (+18 more)
...
### Community 218 - "test_feature_matrix.py"
Cohesion: 0.09
Nodes (8): FeatureUnavailableError, Exception, Load canonical features and apply per-feature then bulk env overrides., Apply a config override string like 'stable', 'beta', 'disabled', 'enabled',…, Raised when code attempts to use a feature that is disabled or unavailable., tests/test_feature_matrix.py — Feature maturity / support matrix tests. Covers:…, TestEnforcement, TestSingleton

### Community 219 - "Persistent Memory System"
Cohesion: 0.05
...
### Community 220 - "_payload"
Cohesion: 0.15
Nodes (5): _payload(), Adaptive-thinking Claude models 400 on temperature / legacy thinking. The…, TestAnthropicPayloadExtendedThinking, TestAnthropicPayloadModelGuards, TestAnthropicPayloadPromptCaching

### Community 221 - "_llm_catalog"
Cohesion: 0.08
...
### Community 223 - "TestAuthAndTaskOwnership"
Cohesion: 0.05
...
### Community 225 - "FailoverResult"
Cohesion: 0.05
Nodes (35): 1. `LLMRouter` is the only gateway, 2. Providers are data, not code, 3. Secrets stay in the environment, 4. Three independent failure scopes, 5. Bulkhead isolation, 6. Context is managed losslessly, 7. Configuration is six committed YAML files, 8. Backwards compatibility by shim, not by rewrite (+27 more)

### Community 226 - "TestSchedulerStore"
Cohesion: 0.09
...
### Community 227 - "audit"
Cohesion: 0.07
Nodes (26): audit(), Request, FastAPI dependency: require any authenticated user., Append an audit log entry. Never logs raw secrets — only secret IDs / masked…, require_authenticated(), Go deeper, Quick start, Routes (+18 more)

### Community 228 - "scrub"
Cohesion: 0.07
Nodes (22): _luhn_ok(), Any, True when ``digits`` (bare, no separators) satisfies the Luhn checksum., Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Recursively strip secrets from an arbitrary argument structure. Fails…, Return the most recent events, newest first, optionally filtered., Aggregate view for the dashboard and the metrics endpoint. ``would_block`` is…, _redact_pii() (+14 more)

### Community 229 - "_llm_catalog"
Cohesion: 0.07
Nodes (20): _cost_tracker_source(), _llm_catalog(), tests/test_daily_automation_2026_09_22.py — Daily automation tests…, Adding a new fallback must not displace the primary., moonshotai/kimi-k2-instruct must now be priced at $1.00/$3.00 per MTok., Price correction must not alter capability flags., Kimi K2 is known-dead on Groq self-serve (HTTP 404); must not be a candidate.…, gemini-omni-1.1-flash must be in the catalog but NOT in routing candidates. (+12 more)

### Community 230 - "Configuration Reference"
Cohesion: 0.09
Nodes (23): Agent governance — identity, policy, approvals, audit, sandboxes, Agent Models, Anthropic API Compatibility / Claude Code, Authentication and Keys, Configuration Reference, Continual Harness, Dashboard (React UI on :3000, API on :8001), Feature Flags (+15 more)

### Community 231 - "test_backend_server_features.py"
Cohesion: 0.05
...
### Community 232 - "Any"
Cohesion: 0.09
Nodes (23): Added, Added, _active_primary_provider(), is_north_mini_code_default(), True when the ``NORTH_MINI_CODE_DEFAULT`` flag is on (default ON). Reads the…, Best-effort read of the active brain's primary provider (or ``None``)., Resolve the model id to force for a code-execution run, or ``None``. Returns…, resolve_coding_model_preference() (+15 more)

### Community 233 - "Killer TODO Roadmap — local-llm-server"
Cohesion: 0.25
...
### Community 235 - "test_background_services.py"
Cohesion: 0.07
...
### Community 236 - "_process_task_callback"
Cohesion: 0.09
...
### Community 237 - "_fake_http_sequence"
Cohesion: 0.06
Nodes (19): _env_defaults(), _fake_http_sequence(), fake_urlopen(), _import_controller(), The diag output must surface binary/model errors clearly., Pins the v3 fix: after the multi-port preamble probe finds colibri serving a…, Yield a list of (status, body) tuples the daemon will see in order when it…, Pins the multi-port fix end-to-end: when ONLY colibri's :8081 is up serving… (+11 more)
...
### Community 239 - "test_telegram_webhook.py"
Cohesion: 0.06
...
### Community 243 - "test_memory_guard.py"
Cohesion: 0.12
...
### Community 244 - "fmtErr"
Cohesion: 0.07
Nodes (29): API, changeUserRole(), chatSend(), createApiKey(), deleteApiKey(), deleteCompany(), fmtErr(), getAgentChatJob() (+21 more)

### Community 245 - "system_instruction"
Cohesion: 0.16
...
### Community 246 - "Fixed"
Cohesion: 0.03
Nodes (73): Send chat messages and return the assistant's text output. If an…, Atomically delete a run-once row only while it is still unfired. Closes the…, FailureCategory, HealingEvent, _now(), Any, Called when a CI workflow fails., Called when a GitHub issue with a bug label is opened. (+65 more)

### Community 247 - "test_brain_availability_doctor.py"
Cohesion: 0.10
...
### Community 249 - "TestCatalogFable51"
Cohesion: 0.09
...
### Community 250 - "_FakeCommandResult"
Cohesion: 0.12
...
### Community 254 - "test_pr923_fixes.py"
Cohesion: 0.07
...
### Community 255 - "test_purge_backlog.py"
Cohesion: 0.07
...
### Community 259 - "test_governance_api.py"
Cohesion: 0.10
...
### Community 260 - "REWRITE_PLAN.md — Phased Migration Strategy"
Cohesion: 0.08
Nodes (25): Already completed (pre-migration fixes), Current Status, Inventory of suspected dead code, Migration Safety Checklist, Phase 3: Auth Consolidation (Week 5), Phase 4: Scheduler Redesign (Week 6), Phase 5: Storage Abstraction (Week 7), Phase 6: Frontend Cleanup (Week 8) (+17 more)

### Community 261 - "test_runtime_governance.py"
Cohesion: 0.09
...
### Community 263 - "FeatureMaturity"
Cohesion: 0.05
Nodes (41): 18. Feature Maturity & Support Matrix, check_feature(), get_feature(), list_features(), Any, get, post, features/api.py — Admin API for the feature support matrix. Exposes: GET… (+33 more)
...
### Community 265 - "AgentPlan"
Cohesion: 0.10
Nodes (15): AgentPlan, Agent: Planner (Architect), Failure Behaviour, Handoff, Output Format, Preferred Model, Responsibilities, Role (+7 more)

### Community 266 - "_status_snapshot"
Cohesion: 0.13
...
### Community 268 - "ProvidersScreen.jsx"
Cohesion: 0.06
Nodes (22): getBrainProviders(), getRenderHealth(), getRenderOpsStatus(), runRenderOpsScan(), setBrainProviderEnabled(), syncProviderToRender(), api, BTN (+14 more)

### Community 269 - "test_mcp_governance.py"
Cohesion: 0.10
...
### Community 274 - "test_nvidia_model_discovery.py"
Cohesion: 0.06
...
### Community 278 - "⚙️ Operations Manager Agent"
Cohesion: 0.06
...
### Community 279 - "_resolve_user_github_token"
Cohesion: 0.10
Nodes (26): Return the caller's GitHub token from EITHER place it can be stored. A token…, _resolve_user_github_token(), dis, _FakeCollection, _FakeDB, patch_db(), _apply(), asyncio (+18 more)

### Community 280 - "reset_store"
Cohesion: 0.09
Nodes (25): Reset the store singleton (used in tests). Also resets the motor client…, reset_store(), get_storage(), packages/storage/factory.py — storage backend factory. Returns the appropriate…, Return the active storage backend. During migration, this delegates to the…, Reset the storage singleton (for tests)., reset_storage(), tests/test_motor_event_loop_isolation.py — regression test for the flaky… (+17 more)

### Community 281 - "Platform Guide — the full tour"
Cohesion: 0.04
Nodes (47): 2026-06-16, 2026-06-25, 2026-06-26, 2026-07-04, 2026-07-05, 2026-07-09, Agile, portfolio & product, Architecture (+39 more)

### Community 282 - "context_rules.py"
Cohesion: 0.11
...
### Community 284 - "test_brain_failover.py"
Cohesion: 0.11
...
### Community 286 - "_execute_skill_impl"
Cohesion: 0.11
...
### Community 287 - "_routing_candidates"
Cohesion: 0.09
...
### Community 288 - "test_provider_enable_disable.py"
Cohesion: 0.07
...
### Community 290 - "ContextCompressor"
Cohesion: 0.11
...
### Community 292 - "test_multi_agent_swarm.py"
Cohesion: 0.18
Nodes (13): AgentConfig, build_agent_specs(), build_swarm(), Create a MultiAgentSwarm and agent specs from request configs., Agent definition for /agent/coordinate requests., Convert AgentConfig list to AgentSpec list for MultiAgentSwarm., test_build_agent_specs_empty_returns_default(), test_build_agent_specs_with_configs() (+5 more)

### Community 293 - "SecurityScanner"
Cohesion: 0.11
...
### Community 297 - "test_brain_patch_service_token.py"
Cohesion: 0.18
...
### Community 298 - "test_brain_priority_scanner.py"
Cohesion: 0.10
Nodes (22): Regression tests for: brain-skip-paid, provider-priority persistence, scanner…, Critical failover-safety test: if every free provider's base URL is excluded…, When the ONLY configured provider is a paid one (e.g. operator set…, When only Anthropic is configured AND allow_paid=False (default), the resolver…, The PUT /api/providers/{id} endpoint did not persist priority edits because the…, Priority must be an int (or None for unset) and within a sane range so a typo…, The PUT /api/providers/{id} handler does: for k, v in…, When AGENT_LLM_BASE_URL is set, the brain resolver must return that endpoint —… (+14 more)

### Community 299 - "test_provider_render_env.py"
Cohesion: 0.17
Nodes (10): provider_env_names(), packages/integrations/render_env.py — write a single Render env var over REST.…, Return ``(key_env, base_url_env)`` for a provider id, or ``None``. Sourced from…, _enable_render(), tests/test_provider_render_env.py — UI → Render env write for provider keys.…, test_endpoint_success_writes_key(), test_endpoint_unknown_provider_404(), test_provider_env_names_case_insensitive() (+2 more)

### Community 300 - "build_tool_calling_router"
Cohesion: 0.24
...
### Community 301 - "test_rate_limiter.py"
Cohesion: 0.16
Nodes (16): pace(), Proactively pace a request to *provider_id*. No-op (returns 0.0 immediately)…, Clear all token-bucket state (tests only). Does not touch the header tracker's…, reset(), Tests for packages/ai/rate_limiter.py — proactive x-ratelimit-* throttling…, Regression: float("inf") used to parse successfully and pass the rpm > 0 check,…, Regression: provider_max_rpm() already upper-cases internally when building the…, _reset_buckets() (+8 more)

### Community 302 - "test_kimi_bridge_server.py"
Cohesion: 0.07
...
### Community 303 - "test_e2b_data_flow.py"
Cohesion: 0.06
Nodes (34): fake_sandbox(), _FakeAsyncSandboxClass, _FakeCmdResult, _FakeCommands, _FakeFiles, _FakeSandbox, Any, asyncio (+26 more)

### Community 304 - "register_webui"
Cohesion: 0.04
Nodes (55): allow_paid_brain(), get_brain_preference(), is_anthropic_model(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, True when *model* names a paid Anthropic/Bedrock-Claude model. Covers native…, _FakeAsyncClient, _FakeResponse (+47 more)

### Community 305 - "test_microagents.py"
Cohesion: 0.15
...
### Community 306 - "tests/conftest.py"
Cohesion: 0.06
Nodes (45): create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., N5 dual-auth dependency: accept EITHER a user session OR a service token. Used…, _user_or_service_token(), 4. Architecture reference, Auth flows (+37 more)
...
### Community 308 - "fastapi"
Cohesion: 0.04
Nodes (91): BaseModel, backend/admin_local_brain_router.py — admin-session proxy for the local-brain…, ToggleBody, BaseModel, backend/connectors_api.py — the connector catalogue API (`/api/connectors/*`).…, Body for ``POST /api/connectors/webhook/send`` (rule 11 — no raw dict in)., WebhookSendRequest, fastapi (+83 more)

### Community 309 - "governance.py"
Cohesion: 0.09
...
### Community 311 - "test_telegram_diag_endpoint.py"
Cohesion: 0.09
...
### Community 312 - "WorkflowBuildRequest"
Cohesion: 0.09
Nodes (26): Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory, do_approve() (+18 more)

### Community 314 - "test_daily_automation_2026_09_21.py"
Cohesion: 0.10
...
### Community 315 - "test_workspace_isolation.py"
Cohesion: 0.16
...
### Community 318 - "ResourceWatchdog"
Cohesion: 0.10
...
### Community 320 - "Platform Engineer Agent"
Cohesion: 0.06
...
### Community 321 - "Marketing SEO Specialist"
Cohesion: 0.06
...
### Community 322 - "High-Agency Frontend Skill"
Cohesion: 0.06
Nodes (30): 10. FINAL PRE-FLIGHT CHECK, 1. ACTIVE BASELINE CONFIGURATION, 2. DEFAULT ARCHITECTURE & CONVENTIONS, 3. DESIGN ENGINEERING DIRECTIVES (Bias Correction), 4. CREATIVE PROACTIVITY (Anti-Slop Implementation), 5. PERFORMANCE GUARDRAILS, 6. TECHNICAL REFERENCE (Dial Definitions), 7. AI TELLS (Forbidden Patterns) (+22 more)

### Community 323 - "test_bootstrap_source_id_index.py"
Cohesion: 0.12
...
### Community 324 - "TestNormalizeResponseFormat"
Cohesion: 0.11
...
### Community 325 - "Quick-Note GitHub Issues Processing - Session Summary"
Cohesion: 0.06
...
### Community 326 - "TaskBoardScreen.jsx"
Cohesion: 0.07
...
Cohesion: 0.14
Nodes (7): Execute the ``webhook`` connector: POST *payload* as JSON to *url*. Fails soft…, send_webhook(), packages/integrations — clients for third-party platforms the agency runs on.…, tests/test_connector_registry.py — the connector catalogue + webhook action.…, test_send_webhook_does_not_follow_a_redirect(), test_send_webhook_rejects_loopback_before_any_request(), test_send_webhook_rejects_non_http_scheme()

### Community 329 - "model_router.py"
Cohesion: 0.04
Nodes (58): F. Router invariants (`router/`), F. Router invariants (`router/`), is_model_available(), Return True if *model* is in the Ollama tag list (or health checks off).…, Dynamic model router package. Public API:: from router import get_router,…, _build_builtin_model_map(), _default_model(), ModelRouter (+50 more)

### Community 330 - "DistributedRateLimiter"
Cohesion: 0.09
...
### Community 331 - "switch_brain.py"
Cohesion: 0.15
Nodes (30): detect_ollama_models(), dim(), fail(), get_auth_headers(), get_brain_config(), get_ngrok_tunnel_url(), header(), info() (+22 more)

### Community 332 - "CEOSupervisor"
Cohesion: 0.09
Nodes (18): CEOSupervisor, Any, What one sweep observed and did. Returned for tests and diagnostics., Sweeps the CEO ledger and drives open goals to closure., Sweep on the configured cadence until cancelled. A failing sweep is logged and…, Inter-sweep delay, behind a method so tests can shorten it. Patching…, True when this process already has a re-drive in flight for *goal_id*., Status for the health endpoint and dashboard. (+10 more)

### Community 333 - "analyze_page"
Cohesion: 0.15
...
### Community 334 - "TestClient"
Cohesion: 0.07
Nodes (24): proxy_client(), MonkeyPatch, TestClient, TestClient against proxy.py:app with a known API key seeded., API-key-based /api/auth/me on proxy.py (port 8000)., GET /api/auth/me with valid API key → 200 with derived profile., GET /api/auth/me with unknown key → 403., GET /api/auth/me with no header → 401. (+16 more)

### Community 335 - "test_skill_registry_boot_refresh.py"
Cohesion: 0.11
...
### Community 336 - "test_autonomy_gate.py"
Cohesion: 0.12
...
### Community 337 - "test_phase6_workflow.py"
Cohesion: 0.13
Nodes (25): Any, Create a pull request. Returns the PR object dict. If a PR already exists for…, Return True if the PR exists and is open or merged; False if 404., Create *branch_name* from *base_sha*. If the branch already exists, returns its…, safe_create_branch(), safe_create_pr(), verify_pr_exists(), _make_store() (+17 more)

### Community 338 - "AgileManager"
Cohesion: 0.07
...
### Community 340 - "Application Security Engineer"
Cohesion: 0.07
...
### Community 341 - "Fixed"
Cohesion: 0.08
Nodes (38): Response, Public SSRF-guarded GET (rule 14) for callers outside this module. Raises…, GET *url*, re-validating every redirect hop against ``unsafe_target_reason`` —…, Layer 2 — Chat Handlers (`chat_handlers.py`, 710 lines), Fixed, _apply_chat_defaults(), _emit_safely(), _extract_exact_output() (+30 more)

### Community 342 - "failover_client.py"
Cohesion: 0.06
Nodes (43): _describe_registry(), _disable_unless_key_serves_other_models(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion(), _looks_unknown_model(), _paid_reserve() (+35 more)

### Community 343 - "get_skill_bindings"
Cohesion: 0.08
Nodes (30): generate_onboarding_questions(), _get_fallback_questions(), Generate AI-tailored onboarding questions based on detected domain and…, Return hardcoded fallback questions when AI generation fails., (5): Tailored questions are hardcoded today, D. Wire AI-tailored questions to the frontend (`QuestionsStep`), What already exists (don't rebuild), submitOnboardingAnswers() (+22 more)

### Community 344 - "build_render_router"
Cohesion: 0.13
...
### Community 345 - "output_filter.py"
Cohesion: 0.07
...
### Community 346 - "RuntimeSummary"
Cohesion: 0.15
Nodes (12): compare(), main(), Any, Check an operator-supplied task file before anything executes., render(), _run_one(), RunRecord, RuntimeSummary (+4 more)

### Community 347 - "local_controller.py"
Cohesion: 0.09
Nodes (38): Fixed, Fixed, Fixed, Fixed, _bin_exists(), _choose_local_brain(), _default_agency_url(), _default_machine_id_file() (+30 more)

### Community 348 - "triage_orphaned_context_prs.py"
Cohesion: 0.11
...
### Community 349 - "test_scheduler_hydration_bounded.py"
Cohesion: 0.11
Nodes (17): _BrokenScheduler, _FakeStore, _FastScheduler, _HangingScheduler, asyncio, NoReturn, Regression test for the 2026-08-01 502/503 crash-loop.…, A fast hydration must not be deferred or warned about. (+9 more)

### Community 350 - "test_ceo_router.py"
Cohesion: 0.10
Nodes (32): _env_int(), Read a bounded int, clamping out-of-range values rather than raising., Install (or clear) the singleton — used by startup wiring and tests., Drop the singleton (test helper)., Bounds for the unattended supervision loop., reset_ceo_supervisor(), set_ceo_supervisor(), SupervisorConfig (+24 more)
...
### Community 352 - "_redact_for_notification"
Cohesion: 0.11
...
### Community 353 - "_register_code_graph_tools"
Cohesion: 0.14
...
### Community 354 - "test_telegram_mutating_commands.py"
Cohesion: 0.09
...
Cohesion: 0.15
Nodes (13): Rewrite the spec file, preserving any non-entry (hand-written) lines., One standing instruction plus the evidence that earned it., SpecEntry, write_entries(), Regressions for defects found in review of this module., verify=False is the internal path used for merging, and stays permissive., The join's newlines count against HARNESS_SPEC_MAX_CHARS too., A single global instance would serve one workspace's spec to all of them. (+5 more)

### Community 357 - "test_rag_context.py"
Cohesion: 0.12
...
### Community 360 - "Data Engineer Agent"
Cohesion: 0.07
...
### Community 363 - "Kept Rules — the 44 that survive the audit"
Cohesion: 0.07
Nodes (24): C1 — The bill of materials is wrong in both directions, C2 — Three different answers to "where do I read env vars?", C3 — Two different file-size limits, C4 — The frontend does not deploy to Vercel, C5 — The documented P0 escape hatch does not exist, C6 — `CLAUDE.md` §14.11 conflicts with §14.9, C7 — Two `§10` headings in `CLAUDE.md`, C8 — Duplicated rule sets that have already drifted (+16 more)

### Community 364 - "RepoConnection"
Cohesion: 0.11
Nodes (29): Phase 0 — `RepoConnection` plumbing + delivery-policy detection, Phase 1 — Plan-PR → Implementation  *(highest leverage; closes the live gap)*, Phase 2 — Review-comment resolution (Codex / CodeRabbit), Phase 3 — Quality gate + policy-conformant landing, Phase 4 — Monitor & regression guard, Phases, 6. Integration gaps to wire (follow-up implementation), Acceptance criteria (+21 more)

### Community 365 - "AppShell.jsx"
Cohesion: 0.07
Nodes (25): `activation_api.py`, Backend changes, `backend/company_api.py`, `db/sqlite_store.py`, Docs / changelog, Frontend changes, Goal, Implementation Plan — Onboarding-Gate Admin Setting + Ephemeral Companies (+17 more)

### Community 366 - "generate_context.py"
Cohesion: 0.12
...
### Community 367 - "SteeringInjector"
Cohesion: 0.11
...
### Community 368 - "test_claude_setup_audit.py"
Cohesion: 0.16
...
### Community 369 - "test_session_retro.py"
Cohesion: 0.13
...
### Community 370 - "test_internal_agent_did_work.py"
Cohesion: 0.12
...
### Community 371 - "_captured_request_headers"
Cohesion: 0.12
...
### Community 372 - "sam_livekit_worker.py"
Cohesion: 0.07
Nodes (40): detect_alert_intent(), fetch_alerts(), fix_alerts(), handle_alert_command(), _plural(), Any, agent/sam_actions.py — Deterministic actions SAM can take on the alerts feed.…, Queue fix tasks for the open alerts and return SAM's spoken report. (+32 more)

### Community 373 - "UX Researcher Agent Personality"
Cohesion: 0.07
...
### Community 374 - "DevOps Automator Agent Personality"
Cohesion: 0.07
...
### Community 375 - "Mobile App Builder Agent Personality"
Cohesion: 0.07
...
### Community 376 - "Developer Agent Personality"
Cohesion: 0.07
...
### Community 377 - "Analytics Reporter Agent Personality"
Cohesion: 0.07
...
### Community 378 - "Support Responder Agent Personality"
Cohesion: 0.07
...
### Community 379 - "test_iteration_6_features.py"
Cohesion: 0.14
Nodes (15): codeql_count(), dependabot_count(), main(), Any, OpenClaw security fix helper. Lightweight CLI used by CI to check/fix…, _repo_parts(), _request(), requests (+7 more)

### Community 380 - "Python Dependencies (`requirements.txt`)"
Cohesion: 0.07
...
### Community 382 - "Sampling Strategies Internals"
Cohesion: 0.09
...
### Community 383 - "ProviderConsole.jsx"
Cohesion: 0.08
...
### Community 384 - "test_webui_provider_priority.py"
Cohesion: 0.05
Nodes (60): test_git_ref_rejects_empty(), test_git_ref_rejects_flag_injection(), test_git_ref_rejects_shell_metacharacters(), test_git_ref_rejects_traversal(), test_git_ref_valid(), test_git_scheme_allows_ssh(), test_http_scheme_rejects_ssh(), test_https_public_host_allowed() (+52 more)

### Community 385 - "BrainWatchdog"
Cohesion: 0.11
...
### Community 390 - "run_regression"
Cohesion: 0.10
Nodes (16): main(), Full desktop regression suite., Full mobile regression suite (navigation + key page loads)., API Key CRUD: create, copy, list, delete., Schedules: create, list., GitHub integration: status, repos., Activation: status, users., Run the full regression suite for a given viewport. (+8 more)

### Community 391 - "_Recorder"
Cohesion: 0.09
...
### Community 392 - "SamAgent"
Cohesion: 0.12
Nodes (13): Any, SAM voice agent — the voice-controlled interface to the agency., Process a voice command and return SAM's spoken response. Args: text: The…, Call the NVIDIA NIM LLM (free tier) for SAM's response., Rule-based fallback when the LLM is unavailable., SamAgent, test_feed_failure_is_reported_not_raised(), test_fix_request_queues_runnable_tasks() (+5 more)

### Community 393 - "ProjectScaffolder"
Cohesion: 0.14
...
### Community 394 - "test_brain_default_model.py"
Cohesion: 0.12
...
### Community 395 - "AGENTS.md — Codebase Map & Operations Reference"
Cohesion: 0.15
...
### Community 396 - "UI Designer Agent Personality"
Cohesion: 0.07
...
### Community 397 - "Product Sprint Prioritizer Agent"
Cohesion: 0.07
...
### Community 398 - "Claude Code + Qwen Local Setup"
Cohesion: 0.07
...
### Community 399 - "Docker Agent Runtimes Setup"
Cohesion: 0.07
...
### Community 400 - "_emit_langfuse_http_sync"
Cohesion: 0.13
Nodes (14): _department_trace_tags(), _emit_langfuse_http_sync(), _emit_sdk(), Any, Avoid oversized payloads that make Langfuse reject the event., _truncate_for_langfuse(), emit_chat_observation must accept session_id without error., When session_id is provided, it should appear in the meta dict passed to emit… (+6 more)

### Community 401 - "CostAttributor"
Cohesion: 0.09
...
### Community 402 - "AgentStore"
Cohesion: 0.07
Nodes (33): AgentStore, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents., Return all agents in the system (admin use only)., main(), Register runtime agents in the store. (+25 more)

### Community 403 - "TestDiagCommand"
Cohesion: 0.11
...
### Community 405 - "._divergent"
Cohesion: 0.15
Nodes (8): _hardcoded_candidates(), These two are the ones this work reconciled, and each was reconciled because…, A shrinking divergence is progress, but the list must be trimmed to match or it…, ``PROVIDER_CANDIDATES`` as written in the module, before YAML overrides. Read…, The YAML wins at import. A fix applied only to the Python copy is a fix that…, Checked against the catalogue that wins, not the copy that loses., TestTheCatalogueIsWhatActuallyRuns, _yaml_candidates()

### Community 406 - "_first_paragraph"
Cohesion: 0.15
...
### Community 407 - "test_skill_registry.py"
Cohesion: 0.09
...
### Community 409 - "VoiceCommandInterface"
Cohesion: 0.14
...
### Community 410 - "Frontend Developer Agent Personality"
Cohesion: 0.08
...
### Community 411 - "Technical Debt Register — local-llm-server"
Cohesion: 0.10
...
### Community 412 - "test_brain_failover_413.py"
Cohesion: 0.10
...
### Community 413 - "Artifact"
Cohesion: 0.14
Nodes (16): AgentRole, Artifact, A durable, phase-produced document (markdown or JSON)., Return the first artifact matching *name*, or None., PhaseRunner, Path, PhaseType, Execute a single CRISPY phase and produce its artifact. The runner is… (+8 more)

### Community 414 - "CEO Micro-Management"
Cohesion: 0.13
...
### Community 415 - "skill_bindings.py"
Cohesion: 0.09
...
### Community 416 - "Part A — CodeRabbit review fixes for this PR (do first, small)"
Cohesion: 0.06
...
### Community 417 - "tasks/store.py"
Cohesion: 0.06
Nodes (37): Background dispatcher for task execution., _coerce_ts(), Coerce ISO-8601 datetime strings (from DB) to float timestamps., Any, tasks/store.py — MongoDB-backed task store. Falls back to in-memory storage…, Fail loud in production if no persistent storage is configured. Telegram…, List tasks for a specific user with optional filters. ``include_system``…, Return BLOCKED tasks that are candidates for auto-retry. (+29 more)
...
### Community 422 - "test_workflow_shell_vars_are_declared.py"
Cohesion: 0.10
...
### Community 424 - "github_tools.py"
Cohesion: 0.18
Nodes (24): get_repo(), _get_token(), _get_user(), init_workspace(), list_branches(), list_prs(), list_repos(), BaseModel (+16 more)

### Community 425 - "register_user_research_tools"
Cohesion: 0.12
Nodes (14): auto_register(), Any, field_validator, Register the four user-research tools with a ``ToolRegistry``. Each tool wraps…, Register the user research tools into the module-level singleton registry.…, register_user_research_tools(), user_research_plan_tool(), user_research_qual_tool() (+6 more)

### Community 426 - "test_memory.py"
Cohesion: 0.15
...
### Community 430 - "Project Shepherd Agent Personality"
Cohesion: 0.08
...
### Community 432 - "Plan: Onboarding Flow Bug Fixes + Wire AI-Tailored Questions"
Cohesion: 0.20
Nodes (10): (1) & partly (4): "Something went wrong" masks the real error everywhere, (2) & (3): Company creation flow / non-admin gate placement, Agent Prompt (paste this to start the implementation session), Context — bugs reported during manual QA, NEW — (1)'s dominant real-world cause: live scanner crashes on `master` (found via CI on this PR, 2026-06-14), Open question for @strikersam, Plan: Onboarding Flow Bug Fixes + Wire AI-Tailored Questions, Root-cause findings (already investigated — do not re-derive) (+2 more)

### Community 433 - "Tween"
Cohesion: 0.14
Nodes (26): _a(), _assertThisInitialized(), cb(), dc(), Fo(), ga(), gb(), hb() (+18 more)

### Community 434 - "Session Handoff — 2026-06-15"
Cohesion: 0.08
...
### Community 436 - "video_transcript.py"
Cohesion: 0.11
...
### Community 439 - "test_backend_runtime_bootstrap.py"
Cohesion: 0.09
Nodes (11): anyio, Web lifespan delegates to start_background_services when…, RUN_BACKGROUND_IN_WEB=false: lifespan starts but background services are NOT…, _StubRuntimeManager, _StubRuntimeRegistry, _StubTask, _StubTaskDispatcher, test_backend_lifespan_skips_bg_when_flag_false() (+3 more)
...
### Community 443 - "ClaudeCodeAdapter"
Cohesion: 0.19
...
### Community 444 - "AI Engineer Agent"
Cohesion: 0.08
...
### Community 445 - "Research Synthesist Agent Personality"
Cohesion: 0.08
...
### Community 446 - "Local AI Stack with Docker"
Cohesion: 0.08
...
### Community 447 - "LLM Router — troubleshooting"
Cohesion: 0.08
...
### Community 448 - "TASK 4 — End-to-end approval-gate test"
Cohesion: 0.08
...
### Community 451 - "test_schedule_backlog_drain.py"
Cohesion: 0.15
Nodes (19): _one_shot(), asyncio, Why the 2026-08-01 backlog never drained, despite a fix already existing.…, A timestamp we cannot parse must not authorise a delete., The pre-existing behaviour has to survive the new branch above it., The previous failure was invisible: four digits at INFO beside two zeroes., Adding it looks like a fix and is a worse bug. ``packages/storage/sqlite.py``…, Guard the drift that caused this: two files naming the same collection. (+11 more)

### Community 452 - "TemporalContextGraph"
Cohesion: 0.10
...
### Community 455 - "test_daily_automation_2026_09_24.py"
Cohesion: 0.06
Nodes (25): _brain_config_source(), _cost_tracker_source(), _get_cost_tracker_price(), _llm_catalog(), tests/test_daily_automation_2026_09_24.py — Daily automation tests…, claude-opus-5-5 must appear in the LLM catalog with correct flags., cost_tracker.py must contain correct pricing for both model IDs., Base and versioned IDs must have identical pricing. (+17 more)

### Community 456 - "test_executive_advisory.py"
Cohesion: 0.13
Nodes (22): ExecutiveAdvisory, Routes a business question to the right executives and synthesises them., _FakeMemory, asyncio, Tests for the intelligent C-suite advisory layer (agent/executive_advisory.py).…, Return an async llm that yields queued replies and records calls., _recording_llm(), test_advise_consults_selected_execs_and_synthesizes() (+14 more)

### Community 457 - "test_openclaw_endpoints.py"
Cohesion: 0.09
...
### Community 458 - "test_provider_policy.py"
Cohesion: 0.10
Nodes (23): asyncio, tests/test_provider_policy.py — Unit tests for the paid-provider kill switch.…, ProviderPolicyUpdate defaults allow_paid to False., ProviderPolicyUpdate accepts allow_paid=True., ProviderPolicyUpdate converts truthy strings to True (Pydantic v2 coercion).…, CI provider_policy.allow_paid() honors PROVIDER_POLICY_ALLOW_PAID env var., CI provider_policy.allow_paid() returns False when env var unset and API…, _get_provider_policy returns allow_paid=False when no document exists. (+15 more)

### Community 459 - "_FakePersistence"
Cohesion: 0.13
Nodes (16): _every_minute_one_shot(), _FakePersistence, An agency-directive-shaped row: cron="* * * * *", uniquely named., 2026-08-03: the 7-day fallback let a live crash loop regrow the backlog from a…, Must not delete a same-pattern row that hasn't had time to fire yet., A "0 9 * * *" run-once row legitimately waits up to ~24h for its one scheduled…, toggle(job_id, enabled=False) pauses a job without deleting it — the fast path…, _register_aps parses cron via ``.strip().split()``, so a cron string with… (+8 more)

### Community 460 - "heal_signature"
Cohesion: 0.08
...
### Community 462 - "open_phase_report"
Cohesion: 0.11
...
### Community 463 - "openclaw_str_e_fix.py"
Cohesion: 0.15
...
### Community 464 - "Deploy: FreeBuff Telegram bot (24×7)"
Cohesion: 0.09
...
### Community 465 - "Workspace Isolation Architecture"
Cohesion: 0.12
...
### Community 466 - "TestPoliciesGovernanceStableClaim"
Cohesion: 0.20
...
### Community 469 - "_test_e2e_telegram_approval"
Cohesion: 0.11
...
### Community 470 - "isolated_telegram_config"
Cohesion: 0.13
...
### Community 472 - "test_cerebras_catalogue.py"
Cohesion: 0.13
...
### Community 473 - "test_mostly_failed_steps.py"
Cohesion: 0.12
...
### Community 474 - "test_service_token.py"
Cohesion: 0.08
...
### Community 475 - "test_executive_advisory_api.py"
Cohesion: 0.10
Nodes (21): AdviceResult, ExecOpinion, One executive's answer to the question., The full advisory response: per-executive opinions plus the synthesis., Drop the singleton (test helper)., reset_executive_advisory(), _make_app(), FastAPI (+13 more)

### Community 476 - "FilterResult"
Cohesion: 0.15
...
### Community 477 - "AdaptivePermissions"
Cohesion: 0.19
...
### Community 480 - "Security Analysis — local-llm-server"
Cohesion: 0.12
...
### Community 481 - "Changelog"
Cohesion: 0.07
Nodes (35): [5.0.0], Changed, Changed, Changelog, Changelog, Fixed, Removed, [v4.1.0] (+27 more)

### Community 482 - "_pr"
Cohesion: 0.16
Nodes (7): _pr(), TestRunSweep, get_check_runs(), list_open_prs(), send_card(), list_open_prs(), TestSelectNotifiable

### Community 483 - "AgentsScreen.jsx"
Cohesion: 0.08
Nodes (27): 1. Editable companies, anytime (not a one-shot wizard), 2. Question-driven provisioning — no cosmetic questions, 4. Agents start pre-powered, Invariants, Phases, Tailored Onboarding, Editable Companies & Dynamic Roles, The gaps to close, UI-first — an API is not "done" (+19 more)

### Community 484 - "_Cursor"
Cohesion: 0.10
...
### Community 485 - "harness_routing.py"
Cohesion: 0.14
...
### Community 486 - "context_plan_gate.py"
Cohesion: 0.13
...
### Community 487 - "WindowsServiceManager"
Cohesion: 0.17
Nodes (7): Tunnel — Permanent Static URL (ngrok), _creationflags(), CompletedProcess, Path, Spawn a new proxy process on Linux/Mac using the current Python interpreter., ServiceState, WindowsServiceManager

### Community 488 - "test_task_source_id_race.py"
Cohesion: 0.14
...
### Community 489 - "TestLegacyRouterCacheTTL"
Cohesion: 0.13
...
### Community 490 - "test_anthropic_refusal_fallback.py"
Cohesion: 0.15
...
### Community 491 - "yaml"
Cohesion: 0.12
Nodes (14): EvalTask, load_tasks(), Path, The representative task catalogue for the routing evaluation. Tasks live in…, One representative task with its pre-defined acceptance criteria., Load and validate the task catalogue from ``tasks.yaml``., Fail loudly on a malformed catalogue rather than at scoring time., _validate() (+6 more)

### Community 492 - "LocalWorkspace"
Cohesion: 0.15
...
### Community 494 - "🧭 Product Manager Agent"
Cohesion: 0.10
...
### Community 495 - "test_admin_local_brain_router.py"
Cohesion: 0.15
...
### Community 496 - "V3 API Migration Plan — LLM Relay Platform"
Cohesion: 0.10
...
### Community 497 - "The fifteen strategies"
Cohesion: 0.10
...
### Community 499 - "models/seo_audit.py"
Cohesion: 0.10
...
### Community 500 - "test_provider_failover_integration.py"
Cohesion: 0.15
...
### Community 502 - "PriorityTaskQueue"
Cohesion: 0.14
...
### Community 504 - "test_openclaw_str_e_fix.py"
Cohesion: 0.10
...
### Community 505 - "test_provider_state_durability.py"
Cohesion: 0.09
...
### Community 506 - "test_agent_coordinate.py"
Cohesion: 0.18
Nodes (11): build_task_specs(), Convert TaskInput list to TaskSpec list for MultiAgentSwarm., Task definition for /agent/coordinate requests., TaskInput, Tests for agent/coordinate.py models, helpers, and v2 router., test_agent_config_defaults(), test_build_task_specs(), test_build_task_specs_with_type_alias() (+3 more)

### Community 507 - "test_unit5_ui_provider_surface.py"
Cohesion: 0.10
...
### Community 509 - "test_task_pre_execution_gate.py"
Cohesion: 0.24
...
### Community 510 - "register_admin_gui"
Cohesion: 0.14
...
### Community 512 - "_extract_tech_relevance"
Cohesion: 0.16
...
### Community 513 - "Skill: modularity-review"
Cohesion: 0.10
...
### Community 514 - "Design Audit"
Cohesion: 0.10
Nodes (19): Code Quality, Color and Surfaces, Component Patterns, Content, Design Audit, Fix Priority, How This Works, Iconography (+11 more)

### Community 515 - "Findings"
Cohesion: 0.10
...
### Community 522 - "Autonomous AI Agency"
Cohesion: 0.10
...
### Community 523 - "build_workflow.py"
Cohesion: 0.30
Nodes (19): _c(), _get(), _header(), main(), _make_headers(), _phase_icon(), _post(), _print_phases() (+11 more)

### Community 524 - "probe_model_liveness"
Cohesion: 0.04
Nodes (46): 1. Render (production backend + worker), 2. Cloudflare Worker (frontend), 3. Local development machines, 4. GitHub secrets, 5. MongoDB collections, Post-Merge Environment Cleanup Guide, Post-merge verification checklist, Rollback (+38 more)

### Community 525 - "test_integration_c4_c5_c6_d3.py"
Cohesion: 0.16
...
### Community 526 - "TestBrainFailoverModelUpdates"
Cohesion: 0.18
...
### Community 527 - "test_dashboard_cache.py"
Cohesion: 0.18
Nodes (12): _cached(), Single-flight TTL cache. Concurrent callers wait for the first producer., _CollWithEstimate, _CollWithoutEstimate, asyncio, tests/test_dashboard_cache.py — unit tests for the dashboard hot-path helpers…, test_cached_is_single_flight(), test_cached_recomputes_after_expiry() (+4 more)

### Community 528 - "test_portfolio_intelligence.py"
Cohesion: 0.05
...
### Community 529 - "test_quick_note_source_gate.py"
Cohesion: 0.12
Nodes (8): _load(), parametrize, Quick notes must be judged on their linked source, never on its URL slug. Quick…, scripts/context_plan_gate.py is the second line: process-quick-note reads the…, A reject on an issue with no URL is a plain reject, not needs-source., TestImplementSideGate, TestParse, TestSourceGate

### Community 530 - "test_research_coordinator.py"
Cohesion: 0.11
...
### Community 531 - "WorkflowPhase"
Cohesion: 0.18
Nodes (14): _append_transition(), BaseModel, Enum, str, agent/workflow.py — Persisted workflow state machine. Implements the Autonomous…, Advance task through the full phase sequence. Resumes from…, Execute a single phase and advance to the next., Ordered phases of a task's execution lifecycle. (+6 more)

### Community 532 - "_MemCollection"
Cohesion: 0.13
Nodes (6): _MemCollection, _MemDeleteResult, tests/test_scheduler_store.py — Tests for durable scheduler store (#505)., _MemCollection.count_documents returns doc count., _MemCollection.delete_many removes docs matching updated_at < cutoff., _MemDeleteResult supports explicit count for batch deletes.

### Community 533 - "test_tasks_cache_ttl_env.py"
Cohesion: 0.21
...
### Community 534 - "Native operations"
Cohesion: 0.12
Nodes (14): Maintainer verification, Native operations, Read-only reviewer interpretation, Role pins and spawn contract, Runtime routing evidence, Selective route declaration, preflight, and caching, Worker packet and parent acceptance, Exact mode contracts (+6 more)

### Community 535 - "test_fabric_patterns.py"
Cohesion: 0.06
Nodes (15): main(), out_path(), Path, Generate Langfuse and Telegram mockup screenshots for documentation., save_html_screenshot(), build_screens(), page(), Generate v4 UI screenshots for the README using HTML mockups + system… (+7 more)

### Community 536 - "MultiAgentSwarm"
Cohesion: 0.20
Nodes (11): AgentSpec, CoordinatorResult, MultiAgentSwarm, execute(), Any, Dependency-aware multi-agent task runner with capability routing. DEPRECATED:…, TaskSpec, test_multi_agent_swarm_blocks_missing_dependency() (+3 more)

### Community 537 - "HarnessEnrichment"
Cohesion: 0.14
...
### Community 540 - "Skill: fabric-patterns"
Cohesion: 0.11
...
### Community 541 - "Analysis & Synthesis Instructions"
Cohesion: 0.11
Nodes (18): 1. Define the Atmosphere, 2. Map the Color Palette, 3. Establish Typography Rules, 4. Define the Hero Section, 5. Describe Component Stylings, 6. Define Layout Principles, 7. Define Responsive Rules, 8. Encode Motion Philosophy (+10 more)

### Community 542 - "Performance Analysis — local-llm-server"
Cohesion: 0.11
...
### Community 543 - "Production Readiness Assessment — local-llm-server"
Cohesion: 0.11
...
### Community 547 - "Dynamic Model Routing"
Cohesion: 0.11
...
### Community 548 - "ControlsScreen.jsx"
Cohesion: 0.15
...
### Community 554 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 555 - "TestSelfHealingInfrastructureClassification"
Cohesion: 0.19
...
### Community 556 - "test_health_endpoints.py"
Cohesion: 0.15
...
### Community 558 - "trend_analysis.py"
Cohesion: 0.18
...
### Community 559 - "test_voice_pipeline.py"
Cohesion: 0.14
...
### Community 560 - "validate_session_id"
Cohesion: 0.16
...
### Community 563 - "MemoryMiddleware"
Cohesion: 0.17
...
### Community 565 - "agent/workspace.py"
Cohesion: 0.29
...
### Community 566 - "WorkspaceManager"
Cohesion: 0.12
...
### Community 567 - "memory_consolidation.py"
Cohesion: 0.36
...
### Community 568 - "Test Automation Engineer"
Cohesion: 0.11
...
### Community 569 - "Comprehensive Skill Index (By Category)"
Cohesion: 0.11
...
### Community 570 - "Agent Skill: Principal UI/UX Architect & Motion Choreographer (Awwwards-Tier)"
Cohesion: 0.11
Nodes (17): 1. Meta Information & Core Directive, 2. THE "ABSOLUTE ZERO" DIRECTIVE (STRICT ANTI-PATTERNS), 3. THE CREATIVE VARIANCE ENGINE, 4. HAPTIC MICRO-AESTHETICS (COMPONENT MASTERY), 5. MOTION CHOREOGRAPHY (FLUID DYNAMICS), 6. PERFORMANCE GUARDRAILS, 7. EXECUTION PROTOCOL, 8. PRE-OUTPUT CHECKLIST (+9 more)

### Community 571 - "test_tasks_reconciler_todo_requeue.py"
Cohesion: 0.22
...
### Community 572 - "Architecture Overview — local-llm-server"
Cohesion: 0.11
...
### Community 574 - "AdaptiveHalter"
Cohesion: 0.14
Nodes (9): AdaptiveHalter, Any, Return current halter state for logging / telemetry., Tracks step-level progress and signals when a run should halt early. The halter…, Ratio of applied steps to steps attempted (0.0–1.0). Returns 1.0 when no steps…, Record one step outcome; return a halt reason or None to continue. ``status``…, Verify loop.py imports and wires AdaptiveHalter correctly., AgentRunner must expose _adaptive_halter on construction. (+1 more)

### Community 575 - "Part A — Health Report"
Cohesion: 0.11
...
### Community 576 - "probe_catalogues.py"
Cohesion: 0.11
Nodes (30): Fixed, Status Key, Fixed, _auth_headers(), _chat_targets(), _dump_matching(), _kind(), list_models() (+22 more)

### Community 577 - "sync_readme_gallery.py"
Cohesion: 0.20
...
### Community 580 - "TestStopSlopChecker"
Cohesion: 0.12
Nodes (9): Should clean up extra spaces after removal, Should detect phrases case-insensitively, Should format report correctly, Issues should have helpful suggestions, Should detect weak emphasis adverbs, Should detect Wh-sentence starters, Test AI tell detection and removal, Should remove emphasis crutches (+1 more)

### Community 581 - "WorkflowScreen.jsx"
Cohesion: 0.14
Nodes (12): approveWorkflow(), buildWorkflow(), cancelWorkflow(), getWorkflowRun(), rejectWorkflow(), btn(), BuildForm(), PHASE_STATUS_COLOR (+4 more)

### Community 583 - "TestUpdateTask"
Cohesion: 0.18
...
### Community 584 - "ErrorInterceptorMiddleware"
Cohesion: 0.18
...
### Community 588 - "SessionRole"
Cohesion: 0.25
Nodes (6): Enum, str, Role within a cowork session., Current phase of a cowork session., SessionPhase, SessionRole

### Community 589 - "HarnessAdapter"
Cohesion: 0.12
...
### Community 591 - "Salesforce Architect"
Cohesion: 0.12
...
### Community 592 - "SKILL: Industrial Brutalism & Tactical Telemetry UI"
Cohesion: 0.12
...
### Community 593 - "Skill: data-quality-audit"
Cohesion: 0.12
...
### Community 594 - "What "Slop" Looks Like"
Cohesion: 0.12
...
### Community 595 - "BrainCard.jsx"
Cohesion: 0.19
Nodes (13): Files touched, Modified files, getBrainConfig(), patchBrainConfig(), testBrainModel(), BrainCard(), errText(), PROVIDER_LABEL_FALLBACK (+5 more)

### Community 596 - "local_brain_router.py"
Cohesion: 0.19
...
### Community 597 - "Section-by-Section Acceptance Criteria"
Cohesion: 0.12
...
### Community 600 - "redact_connection_url"
Cohesion: 0.12
Nodes (11): Security, Security, packages/security/redact.py — strip secrets out of strings before they reach a…, Strip embedded credentials from a connection URI before logging it. Covers both…, redact_connection_url(), Register agent runtimes into the local agent store. Usage: python…, Initialize the store with a specific backend. Args: backend: 'mongodb' or…, Regression test: production leaked a live MongoDB password in plaintext.… (+3 more)

### Community 601 - "ChatResponse"
Cohesion: 0.07
Nodes (16): CerebrasProvider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, OllamaProvider, Ollama provider adapter — local LLM inference., ChatResponse (+8 more)

### Community 602 - "agent_readiness_audit.py"
Cohesion: 0.21
...
### Community 606 - "test_task_brain_preflight.py"
Cohesion: 0.21
...
### Community 607 - "e2e/test_browser.py"
Cohesion: 0.19
...
### Community 608 - "test_self_heal.py"
Cohesion: 0.12
Nodes (15): tests/test_self_heal.py — tests for PR #937 self-healing mechanism. No…, packages/ai/self_heal.py must exist and define the heal function., backend/server.py scheduler_tick must call _self_heal_tick every tick., Admin endpoint POST /api/scheduler/self-heal must exist., 410 must mark the provider failed and fail over to the next one., 429 must record the failure so sustained rate-limiting trips failover., _DISPATCH_RETRY_LIMIT must be 5 (lowered from 10 in PR #937)., self_heal source must unblock tasks with runtime/brain in blocked_reason. (+7 more)

### Community 609 - "_migration_block"
Cohesion: 0.15
...
### Community 610 - "test_frontend_deployment_guards.py"
Cohesion: 0.18
...
### Community 612 - "TestRouterIntegration"
Cohesion: 0.21
...
### Community 614 - "control_registry.py"
Cohesion: 0.20
Nodes (14): coerce(), _coerce_choice(), _coerce_number(), _coerce_toggle(), controls_by_group(), Any, packages/config/control_registry.py — the platform-control API. The public…, The catalogue grouped for the dashboard, groups in display order. (+6 more)

### Community 615 - "hermes_prompt.py"
Cohesion: 0.19
...
Cohesion: 0.25
Nodes (10): add_pr_comment(), _find_existing_pr(), get_branch_sha(), get_default_branch(), _headers(), agent/safe_agency.py — Safe GitHub operations for the workflow engine. All…, Return the first open PR matching head → base, or raise if not found., Post a comment on a PR's issue thread. (+2 more)

### Community 617 - "Trajectory"
Cohesion: 0.15
...
### Community 621 - "AITellIssue"
Cohesion: 0.21
Nodes (7): AITellIssue, Find all AI tells in text, Find throat-clearing phrases, Find emphasis crutches (weak adverbs), Find meta-commentary (text referring to itself), Find Wh-sentence starters (weak prose starters), Find basic passive voice patterns (strict mode only)

### Community 622 - "Skill: repowise-intelligence"
Cohesion: 0.12
...
### Community 623 - "ARCHITECTURE.md — Target Architecture"
Cohesion: 0.12
...
### Community 625 - "test_social_login_oauth.py"
Cohesion: 0.24
...
### Community 626 - "github_source.py"
Cohesion: 0.20
...
### Community 627 - "Attention Mechanisms Internals"
Cohesion: 0.11
Nodes (18): ta(), x(), xa(), zb(), Attention Complexity, Attention Mechanisms Internals, Causal Masking, Flash Attention (+10 more)

### Community 628 - "CheckRun"
Cohesion: 0.16
Nodes (6): TestCheckRun, Trigger verification for all applied-but-unverified slices., Verify a single slice (no execute/review, just verify)., CheckRun, Any, Structured, execution-based verification result. Verifier agents produce ONLY…

### Community 629 - "Skill: repowise-intelligence"
Cohesion: 0.12
Nodes (15): 1. Graph Intelligence (Dependency Graph), 2. Git Intelligence, 3. Documentation Intelligence, 4. Decision Intelligence, Acceptance Checks, Directory Structure, Example Usage, Implementation Approach (+7 more)

### Community 630 - "The 10-Step Workflow"
Cohesion: 0.12
...
### Community 631 - "Contributing to local-llm-server"
Cohesion: 0.12
...
### Community 632 - "DirectChatSession"
Cohesion: 0.13
...
### Community 633 - "Agent Runtime Setup"
Cohesion: 0.12
...
### Community 634 - "test_p0_roadmap_a4_a5_b2.py"
Cohesion: 0.17
...
### Community 635 - "test_kimi_bridge_provider.py"
Cohesion: 0.20
...
### Community 637 - "fabric_cli.py"
Cohesion: 0.29
Nodes (15): cmd_apply(), cmd_list(), cmd_new(), cmd_save(), cmd_show(), cmd_stitch(), _ensure_patterns_dir(), main() (+7 more)

### Community 638 - "AgentPhaseError"
Cohesion: 0.14
Nodes (11): AgentPhaseError, Exception, Raised when a named agent phase (planning, verification, etc.) fails., MonkeyPatch, Regression: a planning timeout must not surface as an empty "planning: ".…, Regression: AGENT_SPEC_APPROVAL_REQUIRED=true must never let a run through un-…, Persistence stays best-effort when nothing is gating on it — a storage hiccup…, test_agent_runner_fails_closed_when_spec_approval_required_but_persist_broken() (+3 more)
...
### Community 640 - "test_setup_api.py"
Cohesion: 0.19
...
### Community 641 - ".on_task_complete"
Cohesion: 0.14
...
### Community 642 - "test_agency_workflows_carry_the_failover_chain.py"
Cohesion: 0.18
...
### Community 644 - "DecisionsStoreTests"
Cohesion: 0.12
...
### Community 645 - "test_dockerfile_ships_root_modules.py"
Cohesion: 0.17
...
### Community 646 - "test_force_cleanup_conditional_delete.py"
Cohesion: 0.18
Nodes (9): _orphan(), asyncio, _RaceLostPersistence, _RaceWonPersistence, tests/test_force_cleanup_conditional_delete.py Covers two changes to the…, Store whose conditional delete always loses the race (as if _fire() had already…, test_failed_delete_not_counted_on_fired_dedup_and_stuck_paths(), test_orphan_still_unfired_is_expired_via_conditional_delete() (+1 more)

### Community 648 - "test_migrate_local_brain_env.py"
Cohesion: 0.38
...
### Community 650 - "test_portfolio_api.py"
Cohesion: 0.16
...
### Community 651 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
### Community 655 - "test_ai_insights.py"
Cohesion: 0.18
...
### Community 657 - "Skill: Agentic Portfolio Management"
Cohesion: 0.13
...
### Community 658 - "Skill: agent-harness"
Cohesion: 0.13
...
### Community 659 - "Skill: checkpoint-strategy"
Cohesion: 0.13
...
### Community 661 - "Skill: local-ai-query"
Cohesion: 0.13
...
### Community 662 - "Skill: parallel-agents"
Cohesion: 0.13
...
### Community 663 - "Skill: parallel-worktrees"
Cohesion: 0.13
...
### Community 664 - "Design System: Taste Standard"
Cohesion: 0.13
...
### Community 666 - "test_new_features_e2e.py"
Cohesion: 0.26
...
### Community 667 - "1. Capability-by-capability"
Cohesion: 0.13
Nodes (14): 0. The finding that shapes everything else, 1.10 Least Privilege, 1.13 Compliance (SOC2 / ISO27001 / GDPR), 1.14 Local Development Experience, 1.2 Tool Governance, 1.3 Runtime Isolation, 1.4 Secret Management, 1.5 Policy Engine (+6 more)

### Community 668 - "OpenClaw — iOS Control of the Agency (Single-Service Free-Tier Deploy)"
Cohesion: 0.13
...
### Community 669 - "_scanner"
Cohesion: 0.16
Nodes (6): Last-resort fallback that asks builtwith.com what it already knows about a…, Replace curl_cffi's AsyncSession.get with a canned response., The whole point of the headless pass: tech markers that only appear in the JS-…, _scanner(), TestBuiltWithFallback, TestRenderedDomDetection

### Community 670 - "_override_user"
Cohesion: 0.25
...
### Community 673 - "_parse_reset_epoch"
Cohesion: 0.33
...
### Community 674 - "ADR-007: Storage backend duck-typing over formal ABC"
Cohesion: 0.40
Nodes (4): ADR-007: Storage backend duck-typing over formal ABC, Consequences, Context, Decision

### Community 675 - "CircuitBreakerOpenError"
Cohesion: 0.50
...
### Community 676 - "OrchestratorQueue"
Cohesion: 0.17
...
### Community 677 - "StreamingDeltaReconstructor"
Cohesion: 0.26
...
### Community 678 - "test_critical_flows.py"
Cohesion: 0.26
...
### Community 679 - "test_agile_api.py"
Cohesion: 0.13
...
### Community 680 - "TestBrainConfigUpdates"
Cohesion: 0.22
...
### Community 681 - "test_dockerfile_ships_config_dir.py"
Cohesion: 0.14
...
### Community 684 - "test_swarm_concurrency.py"
Cohesion: 0.15
...
### Community 685 - "test_task_service_failed_comment.py"
Cohesion: 0.21
...
### Community 694 - "DeterministicEngine"
Cohesion: 0.21
...
### Community 695 - "Project Manager Agent Personality"
Cohesion: 0.14
...
### Community 696 - "StopSlopChecker"
Cohesion: 0.14
Nodes (8): Initialize checker. Args: strict: If True, also report adverbs even if not in…, Remove most obvious AI tells from text, Detect and optionally remove AI tells from text, StopSlopChecker, Should report success on clean text, Strict mode should detect passive voice, Should detect meta-commentary, Should remove throat-clearing phrases

### Community 697 - "Process"
Cohesion: 0.14
...
### Community 698 - "Skill: lr-schedule-advisor"
Cohesion: 0.14
...
### Community 702 - "Checks Performed"
Cohesion: 0.14
...
### Community 703 - "Skill: training-stability-monitor"
Cohesion: 0.14
...
### Community 704 - "Skill: branch-cleanup"
Cohesion: 0.14
...
### Community 705 - "Skill: perplexity — Web Research via Perplexity API"
Cohesion: 0.14
...
### Community 707 - "WorkflowEngine"
Cohesion: 0.19
Nodes (12): Drives a Task through the execution state machine with persistence. Usage::…, Record a plan stub (concrete planning happens inside the agent loop)., WorkflowEngine, _make_task(), Create a minimal Task for testing., test_phase_judge_fail_on_error(), test_phase_judge_pass(), test_phase_summarize_done_on_success() (+4 more)

### Community 708 - "Quick-Note Issues Processing Summary"
Cohesion: 0.14
...
### Community 709 - "Separate hosted dashboard backend (`backend/server.py`)"
Cohesion: 0.14
...
### Community 710 - "467 Brutal Audit — File-by-File Status"
Cohesion: 0.05
...
### Community 711 - "LLM Router — configuration guide"
Cohesion: 0.14
...
### Community 712 - "Implementation Plan — DB-persisted, UI-switchable Brain (no redeploy)"
Cohesion: 0.14
...
### Community 713 - "Runbook: Auto-Resume After Cooldown / Interruption"
Cohesion: 0.14
...
### Community 714 - "TestNormalizeAnthropicOutputFormat"
Cohesion: 0.16
...
### Community 716 - "render_ops.py"
Cohesion: 0.06
Nodes (35): _latest_metric_value(), _note_recurrence(), _parse_timestamp(), Any, datetime, services/render_ops.py — autonomous Render debugging + environment monitoring.…, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Pull the most recent numeric sample out of a ``get_metrics`` payload. Render… (+27 more)

### Community 717 - "GitHubPolicyProbe"
Cohesion: 0.19
...
### Community 718 - "importlib_util"
Cohesion: 0.05
Nodes (18): importlib_util, _load_agent_runtime_module(), test_wrapper_exposes_hermes_task_endpoints(), fake_chat_with_ollama(), test_wrapper_exposes_opencode_run_endpoint(), test_wrapper_falls_back_to_installed_model(), tests/test_changelog_parity_guard.py — corruption guard for the changelog gate.…, A 7-equals line under a title (Markdown setext H1) must not false-positive. (+10 more)

### Community 719 - "test_agent_scripts_share_one_model_list.py"
Cohesion: 0.10
...
### Community 720 - "TestAnthropicToolListCaching"
Cohesion: 0.34
...
### Community 721 - "_StubManager"
Cohesion: 0.14
Nodes (3): _apply(), Minimal stand-in for BrainFailoverManager., _StubManager

### Community 722 - "test_gh_brain_failover.py"
Cohesion: 0.10
Nodes (12): gh_brain_failover(), Regression test for the auto-PR scripts' brain failover. On 2026-09-06 the…, Guards the actual regression: the crash-prone single-call site is gone., An empty string is falsy — this used to be returned as a live NVIDIA candidate…, The regression itself: a 402 (or any non-2xx) must not be fatal while another…, _Response, TestAutonomousAgentUsesTheSharedFailover, TestBrainCandidates (+4 more)

### Community 723 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 725 - "RateLimitTracker"
Cohesion: 0.19
Nodes (6): RateLimitTracker, Snapshot of all tracked provider quotas. Safe to call from any context., Reset all state (primarily for tests)., In-memory tracker for per-provider rate-limit state., TestClear, TestGetStats

### Community 726 - "test_daily_2026_07_27.py"
Cohesion: 0.07
Nodes (21): filter_safe_tools(), get_tool_annotations(), Typed representation of MCP tool annotations (spec 2025-11-05 §5.6.1). All…, Extract ``ToolAnnotations`` for a named tool from a ``list_tools()`` result.…, Return tools where ``readOnlyHint`` is True and ``destructiveHint`` is not…, ToolAnnotations, asyncio, Tests for the 2026-07-27 daily automation run. Features covered: 1. Self-… (+13 more)

### Community 727 - "RegistrySkill"
Cohesion: 0.16
...
### Community 729 - "CollaborationContext"
Cohesion: 0.21
...
### Community 730 - "Software Architect Agent"
Cohesion: 0.15
...
### Community 734 - "Skill: Brain Dump"
Cohesion: 0.15
...
### Community 737 - "Skill: duplicate-thread"
Cohesion: 0.15
...
### Community 738 - "Skill: Email Triage"
Cohesion: 0.15
...
### Community 741 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
...
### Community 746 - "admin_digest_router.py"
Cohesion: 0.19
...
### Community 747 - "admin_update_task_router.py"
Cohesion: 0.22
...
### Community 750 - "Skill: graphify — Knowledge Graph Token Optimization"
Cohesion: 0.15
Nodes (12): Acceptance Checks, Claude's query protocol (use this instead of Read tool for exploration):, Graph Artifacts — What to Commit, How to Use the Graph (Token Savings Protocol), Installation (one-time per machine), Instead of reading raw files:, Key commands:, Relationship to repowise-intelligence Skill (+4 more)

### Community 751 - "Skill: platform-setup — Autonomous Agency Bootstrap"
Cohesion: 0.15
...
### Community 752 - "Agency Core — Progress & Resume Log"
Cohesion: 0.13
...
### Community 753 - "Device compatibility and model picks"
Cohesion: 0.15
...
### Community 755 - "TestEstimateTokensForMessages"
Cohesion: 0.15
...
### Community 756 - "test_cost_attribution.py"
Cohesion: 0.09
Nodes (9): _build_cost_table(), _load_env_overrides(), Parse MODEL_COST_INPUT / MODEL_COST_OUTPUT env overrides. Format:…, Tests for packages/ai/cost_tracker.py — per-model cost attribution. Covers: -…, _reset(), TestClearStats, TestCostForTokens, TestEnvOverrides (+1 more)

### Community 757 - "._order_group"
Cohesion: 0.14
...
### Community 758 - "_is_bedrock_model_id"
Cohesion: 0.27
...
### Community 759 - "_get_provider_policy"
Cohesion: 0.19
...
### Community 760 - "_InMemoryBackend"
Cohesion: 0.18
...
### Community 761 - "test_providers_live_e2e.py"
Cohesion: 0.24
...
Cohesion: 0.22
Nodes (4): Local providers (ollama, lmstudio, vllm, localai) default to a localhost…, ``gh workflow run ... -f provider=ollama`` must still work: an operator…, TestDisabledProvidersAreNotFalselyReportedUnreachable, _boom()

### Community 763 - "TestModelCostTableUpdates"
Cohesion: 0.26
...
### Community 764 - "test_deploy_trigger_covers_image.py"
Cohesion: 0.21
...
### Community 765 - "TestRuntimeControl"
Cohesion: 0.15
...
### Community 766 - "test_nightly_e2e_helpers.py"
Cohesion: 0.07
Nodes (8): _ExpectResponse, _Loc, Exception, Nightly e2e helpers (#1565) — hermetic, no browser or backend. Two failures hid…, A SPA whose login round-trip takes longer than any fixed sleep. The URL only…, _Response, _SlowLoginPage, TestBrowserLogin

### Community 767 - "TestKillSwitchDurability"
Cohesion: 0.15
...
### Community 768 - "test_quick_note_engine.py"
Cohesion: 0.17
...
### Community 776 - "Skill: system-prompt-audit"
Cohesion: 0.17
...
### Community 777 - "Skill: task-alive-updates"
Cohesion: 0.17
...
### Community 780 - "test_procedural_memory.py"
Cohesion: 0.24
...
### Community 781 - "check_container_posture.py"
Cohesion: 0.26
...
### Community 782 - "Skill: agent-browser — Real Chrome Browser Automation"
Cohesion: 0.17
...
### Community 787 - "Quantization Internals"
Cohesion: 0.17
...
### Community 788 - "autonomous_fix.py"
Cohesion: 0.11
...
### Community 789 - "extract_failures"
Cohesion: 0.21
...
### Community 790 - "Added"
Cohesion: 0.07
Nodes (31): Any, Force the next ``list_tools()`` call to fetch a fresh tool list from the…, brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, Added, Added, Failure behaviour, Pre-call budget checks (+23 more)

### Community 791 - "_encrypt"
Cohesion: 0.18
...
### Community 792 - "test_daily_2026_07_24.py"
Cohesion: 0.20
...
### Community 793 - "_safe_resolve"
Cohesion: 0.13
Nodes (7): If a symlink inside the workspace points outside, resolve_path blocks it., TestPathSafety, Path, Safely resolve a path inside a workspace's source directory. Rejects traversal…, Read and validate a manifest from disk., Resolve *path* and verify it stays under *base_root*. Blocks symlink escape:…, _safe_resolve()

### Community 794 - "install-agents.sh"
Cohesion: 0.39
...
### Community 795 - "test_openclaw_gateway.py"
Cohesion: 0.14
...
### Community 796 - "Kimi Web-Bridge Service"
Cohesion: 0.17
...
### Community 797 - "is_internal"
Cohesion: 0.09
Nodes (18): get_status(), get, Serve the launcher UI., Get current service status., root(), _count_syllables(), estimate_pixel_width(), flesch_reading_ease() (+10 more)

### Community 798 - "TestDeprecationWarnings"
Cohesion: 0.22
Nodes (4): Parallel execution paths are blocked in orchestrator mode., AgentRunner.run() raises RuntimeError in orchestrator mode., Agency.run_cycle() is a *sanctioned internal caller*: under orchestrator mode…, TestDeprecationWarnings

### Community 799 - "_resolve_push_token"
Cohesion: 0.17
...
### Community 802 - "test_compose_and_coordinate_api.py"
Cohesion: 0.19
...
### Community 803 - "TestModelRegistryUpdates"
Cohesion: 0.13
...
### Community 804 - "._patch_cname"
Cohesion: 0.19
Nodes (7): 5. Input Validation, A repeated key in a dict literal is silent: the later value wins. Found on…, TestTheCostTableHasNoDuplicateKeys, BuiltWith-style off-site identification: a CNAME chain that points at a known…, TestDnsCdnDetection, fake_resolve(), target()

### Community 806 - ".test_set_github_token_sqlite_string_id_does_not_500"
Cohesion: 0.20
Nodes (3): MonkeyPatch, TestClient, TestGithubTokenSQLiteRegression

### Community 807 - "TestAuthAndTaskCreation"
Cohesion: 0.17
...
### Community 808 - "test_langfuse_agency_wide.py"
Cohesion: 0.10
...
### Community 809 - "provider_max_rpm"
Cohesion: 0.11
Nodes (21): provider_api_keys(), _provider_env_value(), provider_key_rotation_enabled(), provider_max_rpm(), _provider_positive_float(), provider_weight(), Read a per-provider env var, tolerating dashes in the provider id. Provider ids…, Shared parse/validate for the numeric per-provider traffic budgets. Returns… (+13 more)

### Community 810 - "_run_analyze"
Cohesion: 0.27
...
### Community 811 - "coordinator.py"
Cohesion: 0.17
Nodes (10): AgentCoordinator, Enum, str, agent/coordinator.py — Multi-Agent Coordinator One coordinator agent breaks a…, Describes a single worker's task and constraints., Run N worker AgentRunners in parallel under a single coordinator. Usage::…, TaskStatus, WorkerSpec (+2 more)
...
### Community 815 - "build_tech_db.py"
Cohesion: 0.35
...
### Community 820 - "_extractive_compress"
Cohesion: 0.18
...
### Community 821 - "AI Engineering Insights Skill"
Cohesion: 0.18
...
### Community 824 - "SRE (Site Reliability Engineer) Agent"
Cohesion: 0.18
...
### Community 826 - "Skill: pro-workflow"
Cohesion: 0.18
...
### Community 829 - "Skill: resource-panel"
Cohesion: 0.18
...
### Community 830 - "Skill: sandboxed-exec"
Cohesion: 0.18
...
### Community 831 - "test_serve_spa_prefixes.py"
Cohesion: 0.24
Nodes (9): Backend package for the Autonomous AI Agency dashboard., _prefixes(), Behavioral: GET to a path that has NO upstream handler but IS in the protected…, Regression tests for SPA catch-all prefix protection (backend/server.py). Bug…, SPA_PROTECTED_PREFIXES must be exposed at module scope (not inside an if-block)…, test_legitimate_spa_paths_are_not_blocked(), test_protected_paths_are_covered_by_prefix_tuple(), test_serve_spa_returns_non_html_for_protected_orphan_path() (+1 more)

### Community 832 - "Skill: dev-browser — Browser Automation via Sandboxed JS"
Cohesion: 0.18
...
### Community 833 - "ECC Harness Patterns Skill"
Cohesion: 0.18
...
### Community 836 - "Stop-Slop Quality Skill"
Cohesion: 0.18
...
### Community 838 - "test_harness_spec.py"
Cohesion: 0.20
...
### Community 839 - "Universality: case-coverage matrix"
Cohesion: 0.10
...
### Community 840 - "2. Critical Bugs & Exact Detection Signatures"
Cohesion: 0.18
...
### Community 841 - "Issue #467 — Section 1: Pulled State + PR Inventory"
Cohesion: 0.18
...
### Community 842 - "Deploy to Google Cloud Run"
Cohesion: 0.18
...
### Community 848 - "RuntimeManager"
Cohesion: 0.05
Nodes (33): Architecture, Deployment on Render, Environment variables, First-time setup, Graceful shutdown, Local development, Overview, Troubleshooting (+25 more)

### Community 849 - "test_bedrock_live.py"
Cohesion: 0.23
Nodes (11): Next daily run (2026-09-27), _NEEDS_CREDS, asyncio, ProviderRouter discovers Bedrock from env and completes a real chat call., Health check returns True when real credentials are loaded from env., Call Bedrock Converse API directly with boto3 — no proxy layer., Verify the configured model ID accepts a converse request without auth errors., test_bedrock_direct_boto3_ping() (+3 more)

### Community 850 - "Provider"
Cohesion: 0.07
Nodes (20): packages.ai — provider abstraction, model registry, and failover manager., ProviderManager, Any, packages/ai/manager.py — ProviderManager. Single entry point for all LLM calls.…, Coordinates provider selection, failover, and health., Return providers sorted by priority (lowest = highest priority)., Send a chat request with automatic failover. Retry policy: 1. If ``model`` is…, Check health of all configured providers. (+12 more)

### Community 851 - "migrate_local_brain_env.py"
Cohesion: 0.25
...
### Community 852 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 853 - "_request"
Cohesion: 0.15
Nodes (5): Listing is not proof. A retired id can still appear in a catalogue., `--json PATH` writes a machine-readable summary the scheduled drift-report step…, TestAFailedProbeIsNotASuccess, _request(), TestTheJsonSummary

### Community 854 - "test_conftest_hermetic_env.py"
Cohesion: 0.18
...
### Community 855 - "test_scanner_ssl_cert.py"
Cohesion: 0.19
...
### Community 857 - "TestMCPClientStructuredOutput"
Cohesion: 0.38
...
### Community 858 - "Autonomy Uplift — Living Roadmap & Detailed Implementation Specs"
Cohesion: 0.17
...
### Community 859 - "TestDecisionsBotLinks"
Cohesion: 0.25
...
### Community 860 - "test_empirical_verify.py"
Cohesion: 0.49
...
### Community 861 - "test_event_log.py"
Cohesion: 0.45
...
### Community 862 - "TestZeroAttemptDiagnostics"
Cohesion: 0.25
...
### Community 863 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 865 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.22
...
### Community 867 - "test_state_file_merge_drivers.py"
Cohesion: 0.47
...
### Community 870 - "financial_analyst.py"
Cohesion: 0.22
...
### Community 872 - "Protocol: Premium Utilitarian Minimalism UI Architect"
Cohesion: 0.20
...
### Community 873 - "The 5-Step Wrap-Up Ritual"
Cohesion: 0.20
...
### Community 874 - "Brag Plan: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 876 - "Hyperframes Composition Brief: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 877 - "_normalize_tool_choice"
Cohesion: 0.31
...
### Community 878 - "Skill: Agentic Agile"
Cohesion: 0.20
...
### Community 879 - "Skill: browserbase-ui-test — Adversarial UI Testing"
Cohesion: 0.20
...
### Community 882 - "Graphiti Temporal Context Skill"
Cohesion: 0.20
...
### Community 885 - "467 Golden Path — Locked Implementation Order"
Cohesion: 0.20
...
### Community 887 - "LLM Router — provider guide"
Cohesion: 0.20
...
### Community 888 - "One command (recommended)"
Cohesion: 0.20
...
### Community 889 - "TOP 6 — Highest-ROI Items (Validated by Opus Research Agent)"
Cohesion: 0.29
...
### Community 890 - "test_pytest_failure_parser.py"
Cohesion: 0.23
...
### Community 891 - "enrich_quick_note_issues.py"
Cohesion: 0.36
...
### Community 893 - "run_patched_colibri.py"
Cohesion: 0.27
...
### Community 895 - "TestTheWorkflowIsSafeAndReadOnly"
Cohesion: 0.20
...
### Community 896 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 897 - "test_dependabot_sweep_workflow.py"
Cohesion: 0.05
...
### Community 898 - "_make_anthropic_provider"
Cohesion: 0.26
Nodes (3): _make_anthropic_provider(), TestAuthHeadersExtendedThinking, TestAuthHeadersPromptCaching

### Community 899 - "test_model_catalog_guard.py"
Cohesion: 0.29
...
### Community 901 - "TestAnthropicWorkspaceIdCapture"
Cohesion: 0.33
...
### Community 903 - "TestAnthropicPayloadStructuredOutput"
Cohesion: 0.29
...
### Community 904 - "._dispatch"
Cohesion: 0.18
Nodes (5): Run the logic for ``phase`` and return the next phase. Handles both sync and…, Route to the best specialist agent by domain and capability., Verify execution results: PR exists, branch created, no error_message., Issue pass/fail verdict based on execution result and error state., Write a final summary comment and set the terminal status.

### Community 905 - "test_workflow_api_mount.py"
Cohesion: 0.20
...
### Community 908 - "get_registry"
Cohesion: 0.05
Nodes (31): get_registry(), Return model registry, extended with ROUTER_EXTRA_MODELS env entries.…, claude-opus-5 must be in the built-in alias map so routing resolves it., TestClaude5RegistryEntries, test_bedrock_haiku_4_5_in_registry(), test_bedrock_opus_48_in_registry(), test_bedrock_opus_4_6_v1_in_registry(), test_bedrock_opus_4_7_in_registry() (+23 more)

### Community 909 - "TestRevenuePortfolio"
Cohesion: 0.20
...
### Community 910 - ".mark_rate_limited"
Cohesion: 0.20
...
### Community 911 - "_extract_workflow_relevance"
Cohesion: 0.33
...
### Community 912 - "record_usage_endpoint"
Cohesion: 0.22
...
### Community 918 - "Skill: changelog-enforcer"
Cohesion: 0.22
Nodes (8): Acceptance Checks, Changelog Location, Entry Format, Examples, Hook Behaviour, Instructions, Skill: changelog-enforcer, When to Use

### Community 919 - "ContributorState"
Cohesion: 0.13
Nodes (10): ContributorState, State of a single contributor within a session., Branch, Components, Purpose, Quick Start, Session Roles, Skill: cowork-session (Claude Cowork) (+2 more)
...
### Community 922 - "KV Cache Internals"
Cohesion: 0.22
...
### Community 923 - "The 8-Step Golden Path"
Cohesion: 0.22
...
### Community 924 - "Competitor Analysis — Autonomous AI Agency"
Cohesion: 0.22
...
### Community 925 - "Issue #1356: quick-note:https://searchengineland.com/turn-seo-backlog-into-roadmap-485713"
Cohesion: 0.22
...
### Community 926 - "Render MCP — autonomous platform debugging and environment monitoring"
Cohesion: 0.20
Nodes (9): 1. Coding sessions — stdio, via `.mcp.json`, 2. The running agency — Streamable HTTP against a deployed sidecar, Enabling it, HTTP API, If the private address does not resolve, Render MCP — autonomous platform debugging and environment monitoring, The monitoring loop, Transport notes (+1 more)

### Community 927 - "Release Procedure"
Cohesion: 0.22
...
### Community 928 - "V2.0 Modernization — Runbook"
Cohesion: 0.22
...
### Community 930 - "TestTheProducersAndTheParserCannotDrift"
Cohesion: 0.25
...
### Community 931 - "test_scanner_e2e.py"
Cohesion: 0.24
Nodes (10): pytest_asyncio, asyncio, Test technology platform detection (e.g., Docker), Test open source documentation detection (e.g., React), Test modern SaaS stack, Test Developer tools stack, test_scanner_developer_tools(), test_scanner_modern_saas() (+2 more)

### Community 932 - "model_discovery.py"
Cohesion: 0.10
...
### Community 937 - "test_backend_requirements_cover_runtime_imports.py"
Cohesion: 0.25
...
### Community 938 - ".test_every_request_carries_a_real_user_agent"
Cohesion: 0.22
...
### Community 939 - "get_control"
Cohesion: 0.22
Nodes (9): effective_value(), The value *spec* currently resolves to, following override → env → default., get_control(), The spec for *key*, or ``None`` when it is not operator-controllable., The call-volume throttle is present, numeric, and defaults to the calmer free-…, A runtime an operator cannot pick from the dropdown is unreachable., test_agency_tick_minutes_throttle_exists(), test_effective_value_follows_override_then_env_then_default() (+1 more)

### Community 940 - "FetchResult"
Cohesion: 0.20
...
### Community 942 - "Removal Ledger — 308 normative statements cut, and why"
Cohesion: 0.20
...
### Community 944 - "TestChatFallbackAndApproval"
Cohesion: 0.22
...
### Community 947 - "test_process_quick_note_workflow.py"
Cohesion: 0.25
...
### Community 948 - "TestEveryFullSuiteJobHasMongo"
Cohesion: 0.22
...
### Community 949 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 952 - "Slice"
Cohesion: 0.14
Nodes (9): tests/test_workflow_models.py — Unit tests for workflow/models.py., TestApprovalGate, TestSlice, TestWorkflowRun, ApprovalGate, Hard approval gate between plan and execution. The workflow engine sets…, An independently deliverable unit of work within the execute phase. Each slice…, Return the Slice with *slice_id*, or None. (+1 more)

### Community 953 - "PR #634 Implementation Tracker"
Cohesion: 0.20
...
### Community 956 - "get_connector"
Cohesion: 0.22
Nodes (9): _connectors(), get_connector(), list_connectors(), Any, Return the connector catalogue. Read live so it can grow at runtime., Return the catalogue as plain dicts. Cheap; used by the API, UI, tests., Return the spec for *connector_id*, or None if it is not registered., test_catalogue_lists_the_webhook_connector() (+1 more)

### Community 957 - "_score_turns"
Cohesion: 0.36
...
### Community 959 - "hybrid_reasoning.py"
Cohesion: 0.36
...
### Community 960 - "AITellType"
Cohesion: 0.33
Nodes (5): AITellType, Enum, str, Categories of AI tells, Tests for quality checker (stop-slop inspired)

### Community 961 - "Skill: docs-sync"
Cohesion: 0.25
...
### Community 962 - "_start_in_web_bot_tasks"
Cohesion: 0.29
...
### Community 963 - "_parse_tool_calls_from_response"
Cohesion: 0.39
...
### Community 964 - "Agent: Judge (Release / QA Gate)"
Cohesion: 0.25
Nodes (7): Activation, Agent: Judge (Release / QA Gate), Enforcement, Output, Responsibilities, Role, Verdict Meanings

### Community 965 - "Skill: browserbase-browser — Real Browser Automation"
Cohesion: 0.25
...
### Community 970 - "GitHub Branch Protection Settings"
Cohesion: 0.25
...
### Community 977 - "Issue #362: Nvidia repo setup"
Cohesion: 0.25
...
### Community 978 - "Issue #364: quick-note:https://www.marktechpost.com/2026/06/01/meet-memory-os-a-6-layer-open-source-memory-stack-built-on-top-of-hermes-agent/"
Cohesion: 0.25
...
### Community 979 - "Issue #378: quick-note:https://www.marktechpost.com/2026/06/02/tinyfish-launches-bigset-an-open-source-multi-agent-system-that-builds-structured-live-datasets-from-plain-english-descriptions/"
Cohesion: 0.25
...
### Community 980 - "Issue #379: quick-note:https://searchengineland.com/schema-markup-optimize-agentic-web-479080"
Cohesion: 0.25
...
### Community 981 - "Issue #380: quick-note:https://cursor.com/blog/cloud-agent-lessons"
Cohesion: 0.25
...
### Community 982 - "Issue #381: quick-note:https://www.xda-developers.com/claude-code-with-opus-48-is-expensive-but-i-made-it-efficient-with-my-local-ai-workflow/"
Cohesion: 0.25
...
### Community 983 - "Issue #382: quick-note:https://claude.com/blog/how-coderabbit-used-claude-to-build-an-agent-orchestration-system"
Cohesion: 0.25
...
### Community 984 - "Issue #383: quick-note:https://www.marktechpost.com/2026/05/29/hexo-labs-open-sources-sia-a-self-improving-agent-that-updates-both-the-harness-and-the-model-weights/"
Cohesion: 0.25
...
### Community 985 - "Issue #416: feat: Self-hosted Codebuff (freebuff) on free NVIDIA models + Telegram bot phone control"
Cohesion: 0.25
...
### Community 986 - "Issue #485: [Trend Digest] Week of 2026-06-08"
Cohesion: 0.25
...
### Community 987 - "Issue #488: quick-note:https://github.com/cookiy-ai/user-research-skill"
Cohesion: 0.25
...
### Community 988 - "Issue #491: Implement whatever is necessary from https://github.com/BehiSecc/awesome-claude-skills"
Cohesion: 0.25
...
### Community 989 - "Issue #493: Use the https://github.com/mvanhorn/last30days-skill skill to get the trend updated"
Cohesion: 0.25
...
### Community 990 - "Issue #495: Read https://www.anthropic.com/news/claude-fable-5-mythos-5 and understand if mythos or fable can be added to the repo"
Cohesion: 0.25
...
### Community 992 - "Issue #581: Sprint tracker: pending work after brand rename + mobile-first pass"
Cohesion: 0.25
...
### Community 994 - "Issue #657: quick-note:https://github.com/earendil-works/pi"
Cohesion: 0.25
...
### Community 995 - "Issue #659: quick-note:https://github.com/nex-agi/Nex-N2"
Cohesion: 0.25
...
### Community 996 - "Issue #660: quick-note:https://github.com/getsentry/sentry-for-ai"
Cohesion: 0.25
...
### Community 997 - "Issue #661: quick-note:https://github.com/XiaomiMiMo/MiMo-Code"
Cohesion: 0.25
...
### Community 998 - "Issue #664: quick-note:https://github.com/Grominet95/jarvis-OS"
Cohesion: 0.25
...
### Community 999 - "Issue #666: quick-note:https://github.com/porokka/jarvis-os"
Cohesion: 0.25
...
### Community 1000 - "Issue #670: quick-note:https://github.com/perplexityai/bumblebee"
Cohesion: 0.25
...
### Community 1001 - "Issue #672: quick-note:https://github.com/Chachamaru127/claude-code-harness"
Cohesion: 0.25
...
### Community 1002 - "Issue #676: quick-note:https://github.com/WeiboAI/VibeThinker"
Cohesion: 0.25
...
### Community 1003 - "Issue #820: quick-note:https://github.com/cobusgreyling/loop-engineering"
Cohesion: 0.25
...
### Community 1004 - "TestWorkflowIntegration"
Cohesion: 0.13
Nodes (8): _NoopStore, Any, List recent runs. When ``owner_id`` is provided, only runs stamped with that…, No-op checkpoint store when the real one is unavailable., Tests for workflow orchestrator integration., Verify WorkflowOrchestrator correctly bypasses for internal callers., Verify InternalAgentAdapter provides agent execution for workflows., TestWorkflowIntegration

### Community 1005 - "SECTION A — Agent Efficiency (Hermes / AOS / MYT)"
Cohesion: 0.25
...
### Community 1006 - "test_agency.py"
Cohesion: 0.09
Nodes (46): AgentRole, _parse_ceo_directives(), Enum, str, set_improvement_loop(), Configuration, Deploying on Render, Installation (+38 more)
...
### Community 1009 - "resolve_hermes_base_url"
Cohesion: 0.32
...
### Community 1010 - "verify.sh"
Cohesion: 0.50
Nodes (6): fail(), pass(), verify.sh script, snapshot_files(), write_legacy_roles(), write_v050_roles()

### Community 1011 - "prompts/README.md"
Cohesion: 0.06
Nodes (25): Command: /plan, References, Usage, What It Does, When to Use, Command: /resume, References, Usage (+17 more)

### Community 1012 - "RunnerLock"
Cohesion: 0.32
...
### Community 1018 - "test_daily_automation_2026_09_25.py"
Cohesion: 0.10
...
### Community 1020 - "SessionMemory"
Cohesion: 0.20
Nodes (6): Any, Extract insight statements from a batch of memories., An individual memory snapshot from an agent session., Record a new session memory for this agent., Consolidate unconsolidated memories into a dream. Creates a dream when the…, SessionMemory

### Community 1021 - "HttpxFetcher"
Cohesion: 0.22
...
### Community 1023 - "Positional Encoding Internals"
Cohesion: 0.25
...
### Community 1025 - "test_provider_models_db_outage.py"
Cohesion: 0.25
...
### Community 1026 - "RuntimeHealth"
Cohesion: 0.04
Nodes (28): N2. Surface Hermes (and all runtimes) status in the Doctor/Runtimes UI ⬜  (size: S, risk: low), AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., Check whether Docker is available and report the adapter's runtime health.…, GooseAdapter, Any, Adapter for Goose — TIER 2 general-purpose local runtime. (+20 more)

### Community 1027 - "SECTION C — Direct Chat Improvements (CBF / HRM)"
Cohesion: 0.25
...
### Community 1030 - "TOOLS.md — Available Tools for AI Agents"
Cohesion: 0.25
...
### Community 1031 - "ToolCall"
Cohesion: 0.15
Nodes (11): Any, field_validator, One tool selection parsed from the executor model's JSON response., Reject names with no dispatch path, accept everything reachable., ToolCall, The defect that made the branches above dead code in the first place.…, Registering + advertising a tool must be enough; no third edit here.…, Widening validation must not turn it off. (+3 more)

### Community 1032 - "Runbook — Instance Activation"
Cohesion: 0.25
...
### Community 1035 - "Marketing Content Creator Agent"
Cohesion: 0.29
...
### Community 1036 - "Marketing Growth Hacker Agent"
Cohesion: 0.29
...
### Community 1040 - "TestHttpxFetcher"
Cohesion: 0.27
Nodes (5): TestHttpxFetcher, handler(), run(), run(), run()

### Community 1041 - "ModelRegistry"
Cohesion: 0.29
...
### Community 1054 - "claude-mem Plugin — Persistent Memory for All Sessions"
Cohesion: 0.29
...
### Community 1056 - "Platform Controls"
Cohesion: 0.29
...
### Community 1058 - "Cloudflare = the real working app"
Cohesion: 0.29
...
### Community 1059 - "record_step_failures"
Cohesion: 0.14
...
### Community 1063 - "Agent Transparency Report"
Cohesion: 0.15
...
### Community 1064 - "check_doc_images.py"
Cohesion: 0.48
...
### Community 1065 - "quickstart.sh"
Cohesion: 0.52
Nodes (6): die(), ensure_env(), ok(), say(), quickstart.sh script, warn()

### Community 1066 - "MCPToolResult"
Cohesion: 0.36
...
### Community 1067 - "_provider"
Cohesion: 0.23
Nodes (6): _provider(), SimpleNamespace, Rule 6: secrets are never logged, not even partially., Error paths print exception text — that must not carry the key., TestAuthFollowsTheProviderDeclaration, TestNoKeyEverReachesTheLog

### Community 1068 - "test_daily_2026_06_14.py"
Cohesion: 0.38
...
### Community 1073 - "TestItIsNotBuiltForOneVendor"
Cohesion: 0.25
...
### Community 1074 - "test_managed_agents.py"
Cohesion: 0.20
...
### Community 1075 - "TestReasonsAreActionable"
Cohesion: 0.29
...
### Community 1076 - "TestProvidersScreen"
Cohesion: 0.43
...
### Community 1077 - "Issue #1427: quick-note:https://github.com/bingreeky/JIT"
Cohesion: 0.22
...
### Community 1078 - "TestMongoService"
Cohesion: 0.29
...
### Community 1084 - "._extract_tokens"
Cohesion: 0.40
...
### Community 1090 - "Music Cues: happy-beats-business-moves-vol-1-by-ende-dot-app"
Cohesion: 0.33
...
### Community 1091 - "/fix-bug — Bug Fix Agent"
Cohesion: 0.33
Nodes (5): Escalation, /fix-bug — Bug Fix Agent, Process, Rules, Usage

### Community 1092 - "_FlakyPersistence"
Cohesion: 0.22
Nodes (4): _FlakyPersistence, Every removal path fails at the durable store., A store.remove() failure must not be reported as a successful expiry — the row…, test_a_failed_durable_delete_is_not_counted_as_expired()

### Community 1093 - "pre-commit"
Cohesion: 0.60
Nodes (5): pre-commit script, _error(), _head(), _info(), _warn()

### Community 1094 - "Skill: browserbase-fetch — Lightweight Web Fetch"
Cohesion: 0.33
...
### Community 1105 - "Pre-Mortem Analysis: Agency Core autonomy story (Cloudflare deployment)"
Cohesion: 0.33
...
### Community 1106 - "_nvidia_default"
Cohesion: 0.29
...
### Community 1110 - "test_runtime_control.py"
Cohesion: 0.33
Nodes (5): anyio, _StubManager, test_start_runtime_returns_remote_managed_when_runtime_is_reachable(), fake_run(), test_stop_runtime_returns_remote_managed_on_remote_only_docker_error()

### Community 1111 - "Event"
Cohesion: 0.33
...
### Community 1112 - ".test_thin_static_detection_still_escalates_to_render"
Cohesion: 0.25
Nodes (3): gucci.com / workers.dev regression: a bot-walled or JS-rendered site leaks a…, fake_detect(), _sys()

### Community 1113 - "steering_for_task"
Cohesion: 0.47
...
### Community 1114 - "asyncio"
Cohesion: 0.22
Nodes (6): asyncio, A deferred step that fails must not leak a reference or an exception., A failed bootstrap leaves the handler to its own DB error handling., test_deferred_step_failure_is_consumed_and_unregistered(), test_login_bootstrap_reports_failure_without_propagating(), test_warmup_step_returns_the_value_when_it_finishes_in_budget()

### Community 1117 - "setup-claude-code.sh script"
Cohesion: 0.60
Nodes (5): log_error(), log_info(), log_success(), print_header(), setup-claude-code.sh script

### Community 1119 - "test_activity_feed.py"
Cohesion: 0.33
...
### Community 1124 - "DirectChatDoctor"
Cohesion: 0.32
Nodes (5): DirectChatDoctor, Run doctor checks before execution; block task if critical issues found., asyncio, When git is missing and no GitHub token is present, the doctor should report…, test_missing_git_and_token()

### Community 1125 - "_ErrorCaptureHandler"
Cohesion: 0.25
...
### Community 1134 - "test_daily_automation_2026_08_16.py"
Cohesion: 0.25
Nodes (7): _apply_reasoning_budget(), _prune_chat_messages(), Map the non-standard ``reasoning_budget`` field to ``thinking_token_budget``.…, Run context pruner on the message list when pruning is enabled., Daily automation tests — 2026-08-16. Covers three improvements landed this…, Stub out modules that require full stack deps to allow unit-testing pure logic.…, _stub_heavy_imports()

### Community 1135 - "test_seo_audit.py"
Cohesion: 0.12
Nodes (11): Aggregated report row - Screaming Frog CSV compatible., SeoIssueReportRow, compute_pressure(), loss_share_from_pressure(), Map aggregate issue pressure to an at-risk revenue share via the diminishing-…, Return (per-row issue pressure, total pressure) for the revenue model.…, parametrize, tests/test_seo_audit.py - SEO / GEO / AIO audit engine tests (issue #533).… (+3 more)

### Community 1137 - ".update_status"
Cohesion: 0.33
...
### Community 1139 - ".daily_active_users"
Cohesion: 0.40
...
### Community 1154 - "test_watchdog_name_normalization.py"
Cohesion: 0.24
Nodes (9): _is_provider_actually_available(), Check if a provider is actually available (not just has a key). Unlike…, tests/test_watchdog_name_normalization.py — PR #963 Verifies the watchdog…, _is_provider_actually_available must return False for ollama without URL., _is_provider_actually_available must return True for ollama with URL., self_heal must normalize active_provider before checking failure counts., test_is_provider_actually_available_checks_ollama_url(), test_is_provider_actually_available_true_for_ollama_with_url() (+1 more)

### Community 1155 - "Documentation map"
Cohesion: 0.40
Nodes (5): Architecture and operations, Documentation map, Repo hygiene, Screenshots and README sync, Start here

### Community 1156 - "Feature Support Matrix"
Cohesion: 0.40
Nodes (5): Admin API, Config Overrides, Feature Support Matrix, Gating Behavior, Maturity Tiers

### Community 1157 - "_build_curl_cffi_fetcher"
Cohesion: 0.36
Nodes (6): _build_curl_cffi_fetcher(), get(), get_text(), head(), _sess(), Return a PageFetcher that uses curl_cffi for all requests.

### Community 1159 - "SamConversation"
Cohesion: 0.29
...
### Community 1165 - "test_generate_context_standing_instructions.py"
Cohesion: 0.32
...
### Community 1166 - "test_runtimes.py"
Cohesion: 0.03
Nodes (44): Architecture, Modules, F2 — MCP Server Exposing Proxy Capabilities [P1] [CBF / ECC], _find_agent_runtime_script(), _get_ollama_base(), Path, Return the Ollama base URL from environment or default., Find the docker/agent_runtime.py script relative to this file. (+36 more)

### Community 1167 - "build_llama_cpp.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 1168 - "download_glm52_weights.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 1169 - "download_glm52_weights.sh script"
Cohesion: 0.70
Nodes (4): download_glm52_weights.sh script, fail(), ok(), warn()

### Community 1170 - "setup_colibri.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 1171 - "setup_colibri.sh script"
Cohesion: 0.70
Nodes (4): setup_colibri.sh script, fail(), ok(), warn()

### Community 1172 - "status_colibri_server.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 1174 - "send_digest"
Cohesion: 0.40
Nodes (4): Send the digest text via NotificationDispatcher (Telegram)., send_digest(), patch, TestSendDigest

### Community 1175 - "TestMobileNavigation"
Cohesion: 0.40
Nodes (3): Mobile-specific: hamburger menu, responsive layout., Verify key pages load in mobile viewport., TestMobileNavigation

### Community 1177 - "updater.py"
Cohesion: 0.43
...
### Community 1179 - "_FakeClient"
Cohesion: 0.25
Nodes (3): _FakeClient, _FakeResp, Minimal async-context httpx.AsyncClient stand-in.

### Community 1181 - "Traffic Distribution Across Providers"
Cohesion: 0.10
...
### Community 1184 - "test_nuclear_cleanup_targets_the_schedule_collection"
Cohesion: 0.25
...
### Community 1186 - "TestGhIsNotReAuthenticated"
Cohesion: 0.40
...
### Community 1187 - "SECTION F — Developer Experience (CBF / ECC)"
Cohesion: 0.50
...
### Community 1188 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 1190 - "_is_blocked_host"
Cohesion: 0.25
Nodes (6): _is_blocked_host(), Render a page with a real headless browser (Playwright/Chromium) and return…, Cheap (no-DNS) SSRF check for headless-browser subrequests. A rendered page's…, _guard(), A URL with no hostname (file://, malformed) must be blocked, and…, TestSubrequestSsrfGuard

### Community 1191 - "test_weekly_trend_digest_workflow.py"
Cohesion: 0.50
...
### Community 1192 - "codebase-explorer.md"
Cohesion: 0.50
Nodes (3): Hard constraints, Method, Output

### Community 1193 - "docs-auditor.md"
Cohesion: 0.50
Nodes (3): Hard constraints, Output, What to check

### Community 1194 - "_infer_parameters_from_func"
Cohesion: 0.38
...
### Community 1198 - "Credential Rotation Runbook"
Cohesion: 0.50
Nodes (3): Credential Rotation Runbook, Guardrails already in place, What to rotate (owner action, ~10 minutes)

### Community 1199 - "Runbook: `make doctor`"
Cohesion: 0.50
Nodes (3): Roadmap, Runbook: `make doctor`, What it checks and why

### Community 1200 - "render"
Cohesion: 0.50
Nodes (3): RENDER_API_KEY, docker, render

### Community 1201 - "_register_web_reach_tools"
Cohesion: 0.29
Nodes (3): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), test_capability_registry_exposes_web_reach_tools()

### Community 1202 - "TestSupportMatrixDocsSync"
Cohesion: 0.29
...
### Community 1203 - "test-anthropic.js"
Cohesion: 0.50
Nodes (3): ref_anthropic_ai_sdk, { Anthropic }, client

### Community 1204 - "JCodeAdapter"
Cohesion: 0.07
...
### Community 1211 - "classify_domain"
Cohesion: 0.29
...
### Community 1212 - "CLAUDE.md — Operating Manual"
Cohesion: 0.29
Nodes (7): 2. Standing Instructions — agent discipline, 3. What this repo is, 5. Bill of materials, 6. Key commands, 7. Environment variables, 8. Where else to look, CLAUDE.md — Operating Manual

### Community 1218 - "Karpathy Guidelines Skill"
Cohesion: 0.29
...
### Community 1219 - "test_a_scheduled_diagnosis_is_held_until_it_completes"
Cohesion: 0.67
...
### Community 1221 - "test_skills_route_order.py"
Cohesion: 0.67
...
### Community 1222 - "github"
Cohesion: 0.50
Nodes (3): github, enabled, silent

### Community 1223 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1224 - "test_one_model_catalogue.py"
Cohesion: 0.29
Nodes (4): The platform-wide NVIDIA default must not be a retired model.…, Model ids belong in a catalogue, not in Python. Four files in this repo each…, ``config/llm/models.yaml`` drives capability filtering. An entry for a retired…, TestBothCataloguesAgreeOnWhatExists

### Community 1233 - "test_p0_roadmap_implementation.py"
Cohesion: 0.43
...
### Community 1239 - "test_the_reserve_is_bounded_when_read_from_the_environment"
Cohesion: 0.67
...
### Community 1276 - "brain_failover.py"
Cohesion: 0.10
...
### Community 1295 - "TestRenderSettings"
Cohesion: 0.29
Nodes (3): An enabled loop with no API key would fail every tick — so it stays dormant., Monitoring is the standing state, not an opt-in after an incident., TestRenderSettings

### Community 1298 - "orchestrator"
Cohesion: 0.33
...
### Community 1336 - "SECTION B — NVIDIA / Cloud Model Integration (Nemotron / NVD)"
Cohesion: 0.33
...
### Community 1337 - "SECTION D — Deployment & Infrastructure (CHM / NVD)"
Cohesion: 0.33
...
### Community 1339 - "apply_phase1_changes.py"
Cohesion: 0.33
...
### Community 1340 - "._remove_worktree"
Cohesion: 0.33
Nodes (5): Clean up the worktree or temp copy created by ``_create_worktree``., When the workspace is not a git repo, falls back to a temp copy., _remove_worktree is a no-op when worktree_path == workspace (fallback failed)., test_create_worktree_fallback_to_copy(), test_remove_worktree_noop_for_original()

### Community 1341 - "parse_pytest_failures.py"
Cohesion: 0.40
...
### Community 1342 - "run_freebuff_bot.py"
Cohesion: 0.47
...
### Community 1343 - "TestAgentRunnerExecution"
Cohesion: 0.33
...
### Community 1346 - "TestCEOAgencySystem"
Cohesion: 0.33
...
### Community 1349 - "test_company_api.py"
Cohesion: 0.12
...
### Community 1350 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 1352 - "TestScanBudget"
Cohesion: 0.33
Nodes (3): A scan must never hang past its wall-clock budget — a slow/blocked domain has…, TestScanBudget, _fast_impl()

### Community 1354 - "test_setup_detect_models_ssrf.py"
Cohesion: 0.29
...
### Community 1355 - "WorkspaceEscapeError"
Cohesion: 0.40
...
### Community 1357 - "distill"
Cohesion: 0.40
Nodes (5): _clean_heading(), distill(), _heading_key(), Drop control characters and leading emoji/symbols from a heading., Reduce a persona file to its role-defining sections, capped at a section…

### Community 1358 - "SECTION E — Autonomy & Self-Healing (AOS / MYT / ECC)"
Cohesion: 0.40
...
### Community 1359 - "_add_colibri_shim_changelog_entry.py"
Cohesion: 0.50
...
### Community 1363 - "Quick Reference — Minimal Configs"
Cohesion: 0.40
...
### Community 1364 - "Optional hardening (operator decisions)"
Cohesion: 0.40
Nodes (5): Compose secret scoping — shipped, opt-in, Container hardening overlay, Optional hardening (operator decisions), Protect the policy file from agents, Rootless Docker daemon

### Community 1366 - ".update_from_response"
Cohesion: 0.40
Nodes (3): _ProviderQuota, Response, Parse x-ratelimit-* headers and update per-provider quota state. Safe to call…

### Community 1367 - "Phase 1: Foundation (Weeks 1-2)"
Cohesion: 0.40
...
### Community 1368 - "Phase 2: Provider Abstraction (Weeks 3-4)"
Cohesion: 0.40
...
### Community 1369 - "TestHermesKeepAlive"
Cohesion: 0.40
...
### Community 1374 - "test_activity_logs.py"
Cohesion: 0.67
Nodes (3): clear_error_log_buffer(), _auth_headers(), test_activity_endpoint_includes_recent_error_logs()

### Community 1375 - "test_a_failed_filing_releases_the_cooldown_so_the_next_recurrence_retries"
Cohesion: 0.40
...
### Community 1378 - "_start_colibri_fn"
Cohesion: 0.50
...
### Community 1380 - "4. Agent Execution Performance"
Cohesion: 0.50
...
### Community 1382 - "_openai_tools_to_anthropic"
Cohesion: 0.50
...
### Community 1391 - "test_no_exception_detail_leaks.py"
Cohesion: 0.50
...
### Community 1392 - "test_feature_stores_are_wired_on_demand_not_eagerly"
Cohesion: 0.50
...
### Community 1394 - "test_catalogue_probe.py"
Cohesion: 0.33
Nodes (4): probe(), The provider catalogue probe: must not leak, must not lie, must not be…, The first real run died in 10 seconds on ModuleNotFoundError: httpx. The probe…, TestTheWorkflowInstallsWhatTheImportNeeds

### Community 1396 - "test_fixes_reliability.py"
Cohesion: 0.12
Nodes (11): AgentStep, tests/test_fixes_reliability.py — Regression tests for the batch of fixes.…, AgentRunner must expose a public plan() coroutine., DashboardHome.js must use Promise.allSettled() not Promise.all(). Promise.all()…, AgentRunner.run() must accept a metadata= keyword argument., plan() should delegate to _generate_plan() and return an AgentPlan. Uses async…, plan() must accept (and ignore) metadata= for forward-compat., InternalAgentAdapter.execute() must NOT pass provider_chain= to AgentRunner.… (+3 more)

### Community 1397 - "2. Ollama Connection Handling"
Cohesion: 0.67
...
### Community 1399 - "SECTION H — Vision / Multimodal (NVD)"
Cohesion: 0.67
...
### Community 1405 - "test_mint_token_rejects_missing_args"
Cohesion: 0.67
Nodes (3): parametrize, Empty key/secret/identity/room must raise ValueError., test_mint_token_rejects_missing_args()

### Community 1406 - "_StopLifespan"
Cohesion: 0.67
Nodes (3): RuntimeError, Sentinel: halt ``lifespan()`` once the startup ordering is observed., _StopLifespan

## Knowledge Gaps
- **3552 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3547 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14765 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **207 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AgentRunner` connect `AgentRunner` to `TaskSpec`, `typing`, `Added`, `test_governance_api.py`, `Changed`, `ToolCall`, `backend/server.py`, `AgentPlan`, `diagnostics.py`, `proxy.py`, `pytest`, `E2BSandboxSession`, `test_agent_loop_delegates_to_the_shared_client`, `ContextPruner`, `MultiAgentSwarm`, `test_ceo_micromanager.py`, `CEODispatcher`, `PolicyEngine`, `test_brain_failover_413.py`, `Traffic Distribution Across Providers`, `TestDeprecationWarnings`, `_resolve_push_token`, `Path`, `ContextManager`, `ToolRegistry`, `coordinator.py`, `Agent: Implementer (Executor)`, `failover_chat_completion`, `test_agent_tool_governance.py`, `register_webui`, `TestAgentLoopMCPIntegration`, `test_agent_runner.py`, `WorkflowOrchestrator`, `AdaptiveHalter`, `TestAgentRunnerExecution`, `FreeBuffAgent`, `AgentSessionStore`, `ReactScratchpad`, `PreflightReport`, `Section-by-Section Acceptance Criteria`, `Fixed`, `test_empirical_verify.py`, `TokenBudget`, `Changelog`, `Issue #656: Bugs`, `AgentJobRequest`, `StuckDetector`, `UserMemoryStore`, `workflow_orchestrator.py`, `LocalWorkspace`, `RepoConnection`, `SECTION A — Agent Efficiency (Hermes / AOS / MYT)`, `GitHubTools`, `Agency Core — Progress & Resume Log`, `test_fixes_reliability.py`, `Fixed`, `Issue #504: EPIC: Autonomy hardening — live-verified defects 2026-06-10`, `WorkspaceTools`, `AgentPhaseError`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `_fixture()` connect `_fixture` to `AgentRunner`, `Added`, `RuntimeHealth`, `Changed`, `test_refresh_agent_built_proof.py`, `UsageEvent`, `pytest`, `test_llm_router_queue_cache.py`, `test_quick_note_source_gate.py`, `test_llm_router_strategies.py`, `Task`, `PolicyEngine`, `build_llm_router`, `TaskStatus`, `test_llm_router_disabled.py`, `company_api.py`, `CEOLedger`, `test_control_plane_api.py`, `router_factory`, `failover_chat_completion`, `test_llm_router_resilience.py`, `routed`, `looks_like_secret_file`, `SeoAuditEngine`, `UserRole`, `ExecutionRequest`, `test_tasks_reconciler_todo_requeue.py`, `WorkflowOrchestrator`, `FreeBuffAgent`, `config.py`, `test_startup_warmup.py`, `PreflightReport`, `TestProviderRouter`, `SelfHealingAgent`, `services/background.py`, `test_provider_router.py`, `AgentSwarm`, `e2e/test_browser.py`, `PrimeAgentAdapter`, `ImprovementLoop`, `AuditLog`, `test_shared_state.py`, `test_task_brain_preflight.py`, `test_knowledge_sync.py`, `ArtifactStore`, `_step`, `test_context_rulebook.py`, `captured`, `test_trend_watcher.py`, `TestChatHistoryStore`, `test_llm_router_e2e.py`, `diagnostics.py`, `clear_cooldowns`, `E2BSandboxSession`, `KeyStore`, `ChatHistoryStore`, `test_issue_intake.py`, `test_spec_store.py`, `test_new_features_e2e.py`, `TestHarnessRegistry`, `KeyPool`, `OperationalIncident`, `controls_app`, `NotificationDispatcher`, `test_agile_api.py`, `test_hermes_in_process.py`, `_Recorder`, `tasks/models.py`, `v4_api.py`, `test_agent_tool_governance.py`, `WorkflowRun`, `test_task_service_failed_comment.py`, `_resolve_brain_provider`, `process_note`, `test_sam_livekit.py`, `api_keys_for`, `test_video_transcript.py`, `OllamaCircuitBreaker`, `test_daily_2026_06_04.py`, `467 Brutal Audit — File-by-File Status`, `TestNormalizeAnthropicOutputFormat`, `test_features_api.py`, `test_sam_voice.py`, `test_agent_scripts_share_one_model_list.py`, `TrendWatcher`, `test_gh_brain_failover.py`, `test_daily_2026_07_27.py`, `FakeRunner`, `TestAuthAndTaskOwnership`, `Any`, `_process_task_callback`, `Path`, `test_telegram_webhook.py`, `TestEstimateTokensForMessages`, `test_cost_attribution.py`, `Fixed`, `test_brain_availability_doctor.py`, `_client`, `test_v4_api.py`, `TestRuntimeControl`, `test_purge_backlog.py`, `LocalBrainStore`, `FeatureMaturity`, `test_persistent_memory.py`, `test_procedural_memory.py`, `OperationalIncidentTracker`, `test_nvidia_model_discovery.py`, `orchestrator`, `OutputFilter`, `_resolve_user_github_token`, `test_openclaw_gateway.py`, `_resolve_push_token`, `test_provider_enable_disable.py`, `TestModelRegistryUpdates`, `SecurityScanner`, `TestHarnessAdapter`, `TestAuthAndTaskCreation`, `test_langfuse_agency_wide.py`, `test_brain_patch_service_token.py`, `SeoFixRequest`, `test_rate_limiter.py`, `test_kimi_bridge_server.py`, `test_e2b_data_flow.py`, `test_spa_trailing_slash_redirect.py`, `WorkflowBuildRequest`, `_MemDB`, `test_bootstrap_source_id_index.py`, `TestNormalizeResponseFormat`, `test_harness_spec.py`, `test_setup_detect_models_ssrf.py`, `analyze_page`, `TestClient`, `test_skill_registry_boot_refresh.py`, `test_doctor_service_token_check.py`, `TestCountTokensEndpoint`, `test_ceo_router.py`, `OrchestratorCheckpointStore`, `_redact_for_notification`, `_register_code_graph_tools`, `_clean_phases`, `test_state_file_merge_drivers.py`, `_clean_director`, `test_session_retro.py`, `test_catalogue_probe.py`, `_inline_threads`, `TestTheWorkflowIsSafeAndReadOnly`, `test_webui_provider_priority.py`, `test_dependabot_sweep_workflow.py`, `parse_event_stream`, `TestMongoGate`, `AgentStore`, `ContextPruner`, `test_brain_failover_413.py`, `tasks/store.py`, `model_discovery.py`, `test_workflow_shell_vars_are_declared.py`, `TestSwarmRoleRouting`, `TestChatFallbackAndApproval`, `test_process_quick_note_workflow.py`, `SQLiteStore`, `ClaudeCodeAdapter`, `test_openclaw_endpoints.py`, `test_provider_policy.py`, `heal_signature`, `test_service_token.py`, `test_executive_advisory_api.py`, `Changelog`, `yaml`, `test_agency.py`, `test_provider_failover_integration.py`, `test_provider_state_durability.py`, `test_daily_automation_2026_09_25.py`, `test_task_pre_execution_gate.py`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `useSafeData()` connect `ref_react` to `KnowledgeScreen.jsx`, `AgentsScreen.jsx`, `AgentJobResult`, `TaskBoardScreen.jsx`, `ProvidersScreen.jsx`, `looks_like_secret_file`, `api.js`, `SchedulesScreen.jsx`, `fmtErr`, `Added`?**
...
- **Should `TaskSpec` be split into smaller, more focused modules?**
  _Cohesion score 0.033209186840471756 - nodes in this community are weakly interconnected._
- **Should `typing` be split into smaller, more focused modules?**
  _Cohesion score 0.01028666236519495 - nodes in this community are weakly interconnected._
- **Should `AgentRunner` be split into smaller, more focused modules?**
  _Cohesion score 0.010070921985815603 - nodes in this community are weakly interconnected._