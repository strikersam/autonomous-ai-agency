...
## Corpus Check
- 1432 files · ~2,065,182 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28951 nodes · 53836 edges · 1210 communities (1096 shown, 114 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 2660 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `bb3e06dd`
...
## Communities (1210 total, 114 thin omitted)
...
### Community 4 - "TaskExecutionCoordinator"
Cohesion: 0.05
...
### Community 5 - "gateway.py"
Cohesion: 0.07
Nodes (46): FailoverResult, A successful completion plus the accounting its callers need., failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Router-backed implementation of ``packages.ai.failover_client``'s entry point.…, Whether the compat bridges should delegate to ``LLMRouter``. Reads the flag on… (+38 more)

### Community 6 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (137): CEOResult, _decompose_into_subtasks(), _merge_changed_files(), Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, Collect changed_files across all specialists into a single de-duped list., reset_ceo_dispatcher() (+129 more)
...
Cohesion: 0.03
Nodes (103): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns…, E2BAdapter, Any, runtimes/adapters/e2b.py — E2B Firecracker micro-VM runtime adapter. Routes…, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``… (+95 more)

### Community 10 - "test_ceo_supervision.py"
Cohesion: 0.03
Nodes (115): CEODispatcher, _harvest_changed_files(), _offload(), Any, Semaphore, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask. (+107 more)
...
Cohesion: 0.03
Nodes (95): Attempt, int, text_key(), describe(), The operator-facing reason string for an unfixable status., packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys() (+87 more)

### Community 13 - "test_llm_router_queue_cache.py"
Cohesion: 0.02
Nodes (116): CacheManager, cosine_similarity(), _Entry, get_cache(), LRUCache, payload_key(), Any, T (+108 more)
...
Cohesion: 0.04
Nodes (79): _as_list(), Decision, GroupPolicy, Mode, PolicyEngine, Any, Enum, Path (+71 more)
...
Cohesion: 0.04
Nodes (105): _complexity_rank(), services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Split the request into briefed, tier-assigned specialist sub-tasks. Returns the…, _runtime_id_for_role(), _should_fan_out(), build_subtask_brief(), _coerce_subtasks(), decompose() (+97 more)
...
### Community 24 - "Agency"
Cohesion: 0.04
Nodes (55): Agency, AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _fetch_github_quick_notes() (+47 more)
...
### Community 27 - "TestRuntimeControl"
Cohesion: 0.04
...
### Community 30 - "Task"
Cohesion: 0.05
Nodes (57): Full task/issue document., Task, asyncio, A reject + human Retry starts a NEW approval cycle → fresh notification., A gated task parked while IN_PROGRESS must not be treated as stranded., test_gate_notifies_only_on_first_park(), test_gate_renotifies_after_reject_and_retry(), test_reconciler_skips_gate_parked_tasks() (+49 more)

### Community 31 - "backend/server.py"
Cohesion: 0.02
Nodes (211): admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody, BrainTestRequest (+203 more)
...
Cohesion: 0.07
Nodes (37): Resolve the model id for a component's role on a provider. Parameters…, resolve_component_model(), tests/test_unit6_resolve_component_model.py — UNIT 6 regression tests. Verifies…, When the DB cache is fresh AND provider matches the active primary, the DB-…, When the DB primary differs from the requested provider, the catalog preset for…, When `provider` is None, the DB primary's saved model wins., With no DB cache and no provider, the env var is consulted., An unknown role returns the safe default rather than raising. (+29 more)

### Community 34 - "analyze_quantitative"
Cohesion: 0.10
...
Cohesion: 0.03
Nodes (99): _dispatch_async(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), Any, LogRecord, agent/log_monitor.py — Application Log Monitor Captures ERROR/CRITICAL log… (+91 more)
...
Cohesion: 0.04
Nodes (104): AgentRole, _parse_ceo_directives(), str, DetectedIssue, get_improvement_loop(), ImprovementLoop, ImprovementLoopState, IssueCategory (+96 more)
...
Cohesion: 0.05
Nodes (54): get_ceo_dispatcher(), Return the shared CEODispatcher singleton., get_ceo_ledger(), Return the shared :class:`CEOLedger`, constructing it on first use., CEOSupervisor, _env_flag(), _env_int(), Any (+46 more)
...
Cohesion: 0.05
Nodes (43): allow_paid_brain(), is_anthropic_model(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, True when *model* names a paid Anthropic/Bedrock-Claude model. Covers native…, _opus_model(), Return an Opus model ID iff the operator explicitly opted into a paid brain.…, _FakeAsyncClient, _FakeResponse (+35 more)
...
Cohesion: 0.05
Nodes (74): BudgetTracker, classify(), GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., Return the ``(surface, action)`` a tool call really represents. The *action* is…, AgentIdentity, _coerce_text() (+66 more)
...
Cohesion: 0.02
Nodes (109): provision_specialist(), Provision a new specialist for a company. Creates and provisions a specialist…, BusinessSystem, Company, CompanyGraph, CompanyGraphSnapshot, SpecialistFamily, SystemType (+101 more)
...
Cohesion: 0.05
Nodes (63): The task store the background services should use, wiring it if needed.…, _task_store_for_background(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog() (+55 more)
...
Cohesion: 0.05
Nodes (53): Agent subsystem — planner / executor / verifier loop., _check_extra_kwargs(), _enforce_signature(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, # NOTE: "ollama_base" is kept for backwards compatibility; this runner only…, AgentEvent (+45 more)
...
### Community 90 - "brain_failover.py"
Cohesion: 0.09
Nodes (31): brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, describe_disabled_reason(), _disabled_from_mongo(), _disabled_from_sqlite(), _is_paid_allowed_db(), _kv_connect(), _kv_path() (+23 more)
...
Cohesion: 0.05
Nodes (73): PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob (+65 more)
...
Cohesion: 0.07
Nodes (56): Convenience: resolve all four role models for a component. Returns a dict with…, resolve_component_role_models(), Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, sanitize_paste_for_preview(), _admin_headers(), _answer_callback(), _api_headers(), _check_rate_limit() (+48 more)
...
Cohesion: 0.09
Nodes (21): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, Register the built-in agent tools that are always available., Register the interactive-browser capability (agent/browser.py). Unlike…, _register_browser_tools(), _register_builtin_tools() (+13 more)
...
### Community 149 - ".start_onboarding"
Cohesion: 0.07
...
Cohesion: 0.05
Nodes (53): _extract_mentioned_paths(), Pick the auto-PR model from the recommended free-cloud chain by key. Mirrors…, Extract plausible file paths from issue text., Read existing files for codebase context (max 8000 chars total)., _read_grounding_files(), _select_brain(), main(), _openai_tools_to_anthropic() (+45 more)
...
### Community 179 - "settings.py"
Cohesion: 0.04
Nodes (68): Feed LLM token/cost spend into the governance session budget. Kept separate…, Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _egress_policy_reason(), Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Write the audit row for a completed (or blocked) MCP tool call., Import the governance layer once, recording why if it is absent. (+60 more)
...
Cohesion: 0.02
Nodes (105): app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client(), MonkeyPatch (+97 more)
...
Cohesion: 0.04
Nodes (68): agents/__init__.py — CRISPY multi-agent coding system., AgentProfile, _catalog_defaults(), _catalog_provider(), _get_defaults(), load_all_profiles(), make_architect_profile(), make_coder_profile() (+60 more)
...
### Community 222 - "weekly_digest.py"
Cohesion: 0.13
...
Cohesion: 0.02
Nodes (168): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint() (+160 more)

### Community 232 - "runtimes/manager.py"
Cohesion: 0.05
Nodes (56): _best_cloud_primary_base(), InternalAgentAdapter, TaskResult, TaskSpec, Built-in agent loop — Nvidia NIM primary, Ollama fallback., Return runtime dependencies required by this adapter. Returns:…, Determine availability of the internal agent runtime. Checks cloud providers in…, Execute a TaskSpec using the internal AgentRunner and convert the agent's… (+48 more)
...
Cohesion: 0.03
Nodes (87): _brain_provider_status(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, BrainResolution, provider_api_key(), provider_base_url(), provider_key_present(), Return the OpenAI-compatible base URL for *provider* (env- and UI-aware)., Return the live API key for *provider* (env-only — never persisted). (+79 more)
...
Cohesion: 0.06
Nodes (51): get_tracker(), The process-wide health tracker., get_ring(), The process-wide key ring., get_metrics(), The process-wide metrics registry., _ok(), parametrize (+43 more)
...
Cohesion: 0.10
Nodes (38): get_failover_manager(), ProviderHealth, Return the singleton BrainFailoverManager., Reset the singleton (for tests)., Circuit-breaker state for a provider., reset_failover_manager(), _clean_env(), _make_manager() (+30 more)
...
Cohesion: 0.06
Nodes (45): _backend(), CEOLedger, _now(), services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, Durable store for :class:`GoalRecord`s. Every method is best-effort and never…, Persist *goal*, stamping ``updated_at``., Mongo server-selection timeout, read and clamped at call time. Parsing this at…, Return the most recently updated goals regardless of state. (+37 more)
...
### Community 320 - "test_issue_triage.py"
Cohesion: 0.16
...
### Community 322 - "has_image_content"
Cohesion: 0.12
...
### Community 324 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
### Community 333 - "test_agent_tools.py"
Cohesion: 0.19
...
### Community 368 - "test_task_service_failed_comment.py"
Cohesion: 0.18
...
### Community 379 - "_enforce_rate_limit"
Cohesion: 0.13
...
Cohesion: 0.03
Nodes (82): Backoff policy for retryable failures., RetryConfig, BreakerState, Enum, str, _digest(), KeyRing, KeyState (+74 more)
...
Cohesion: 0.11
Nodes (29): auto_disable(), _billing_signals(), disabled_provider_ids(), is_unfixable(), packages/llm/disabled.py — bridge to the durable provider on/off switch. The…, Provider ids currently switched off. Empty when the store is unreachable., Persist a provider as disabled, through the store that already owns it., The one list of "you have no money" phrases, borrowed not copied. Lazy because… (+21 more)
...
Cohesion: 0.05
Nodes (60): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion() (+52 more)
...
Cohesion: 0.09
Nodes (17): _job_text(), Guards on ``.github/workflows/dependabot-auto-merge.yml``. Three separate gates…, `unknown` must be handled like `major`, never waved through., The classifier is a repo script, so the job needs a checkout., --admin bypasses required checks. --auto waits for them., Flatten one job back to text so token/flag use can be asserted on it., Root cause 1: GH_PAT is empty on dependabot-triggered events., Root cause 3: a GITHUB_TOKEN merge commit leaves CI action_required. The sweep… (+9 more)
...
### Community 505 - "_is_ephemeral_user"
Cohesion: 0.27
...
Cohesion: 0.03
Nodes (66): Return orchestrator queue depth, active runs, and supervisor state (#522)., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_status(), get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any (+58 more)
...
Cohesion: 0.04
Nodes (25): Drop the cached Mongo client, backoff, and read caches (tests)., reset_kv_state(), one_configured_provider(), A registry with exactly two known providers and isolated state., fake_mongo(), _FakeCollection, _FakeDb, isolated_state() (+17 more)
...
### Community 607 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.22
...
### Community 650 - "TestDirectChatAgentExecution"
Cohesion: 0.33
...
### Community 695 - "TestModelRegistryUpdates"
Cohesion: 0.14
...
### Community 766 - "get"
Cohesion: 0.02
Nodes (93): auth_me(), auto_recommend_skills(), brain_failover_status(), _check_storage_health(), cost_attribution_stats(), discover_remote_skills(), doctor_health(), _get_backend_sam() (+85 more)
...
Cohesion: 0.06
Nodes (42): _dead_model_key(), _exponential_backoff_cooldown(), get_dead_models(), _get_director(), _is_model_dead(), _mark_model_dead(), _notify_watchdog(), provider_router.py — auto-generated module docstring (user-research skill scan). (+34 more)
...
- **Why does `_fixture()` connect `_fixture` to `ProviderConfig`, `TaskExecutionCoordinator`, `gateway.py`, `WorkflowRun`, `test_company_api.py`, `brain_config.py`, `test_llm_router_queue_cache.py`, `ExecutionRequest`, `validate_policy_text`, `PolicyEngine`, `SelfHealingAgent`, `test_colibri_brain_shim.py`, `TaskWorkflowService`, `test_provider_state_durability.py`, `Agency`, `PrimeAgentAdapter`, `TestRuntimeControl`, `Task`, `NotificationDispatcher`, `services/background.py`, `test_monitor_lib.py`, `test_openclaw_endpoints.py`, `test_unit8_model_catalog.py`, `ImprovementLoop`, `test_model_router.py`, `test_llm_router_strategies.py`, `test_ceo_router.py`, `test_failover_client_shared.py`, `_resolve_push_token`, `webui/router.py`, `test_sam_livekit.py`, `AgentJobManager`, `SQLiteStore`, `ChatHistoryStore`, `test_telegram_auto_approve.py`, `test_autonomy_status.py`, `get_task_store`, `test_local_brain_state.py`, `test_operational_incidents.py`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `SeoAuditEngine`, `test_telegram_diag_endpoint.py`, `config.py`, `ResearchTask`, `tests/conftest.py`, `TestClient`, `test_procedural_memory.py`, `OrchestratorCheckpointStore`, `e2e/test_browser.py`, `TokenBudget`, `UserRole`, `test_startup_warmup.py`, `SeoFixer`, `KeyStore`, `claim`, `test_context_rulebook.py`, `ArtifactStore`, `run_task`, `test_issue_intake.py`, `test_openclaw_gateway.py`, `TaskStore`, `BackgroundAgent`, `TestHarnessAdapter`, `MCPClient`, `test_new_features_e2e.py`, `FeatureMaturity`, `persist_plan_spec`, `TestEstimateTokensForMessages`, `_Budget`, `KeyPool`, `clear_cooldowns`, `test_task_clarification.py`, `UsageEvent`, `test_e2b_task_wiring.py`, `test_tasks_awaiting_approval_api.py`, `settings.py`, `OllamaCircuitBreaker`, `analyze_page`, `test_features_api.py`, `test_video_transcript.py`, `test_brain_failover_413.py`, `TrendWatcher`, `emit_chat_observation`, `test_agile_api.py`, `test_app_settings.py`, `test_portfolio_intake.py`, `test_trend_watcher.py`, `AgentSwarm`, `TestClient`, `_FakeSandbox`, `company_api.py`, `ContextPruner`, `test_e2b_data_flow.py`, `resolve_active_brain`, `test_agent_tool_governance.py`, `test_daily_2026_07_27.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `parse_event_stream`, `OutputFilter`, `test_platform_controls.py`, `capture_screens.py`, `test_scheduler_hydration_bounded.py`, `_clear_cache`, `_resolve_user_github_token`, `TestMongoGate`, `test_rate_limiter.py`, `test_brain_failover.py`, `test_sam_voice.py`, `OrchestratorQueue`, `test_kimi_bridge_server.py`, `test_purge_backlog.py`, `clear_override`, `_clean_director`, `test_brain_availability_doctor.py`, `sam`, `test_telegram_service_webhook.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `test_control_plane_api.py`, `isolated_telegram_config`, `test_task_service_failed_comment.py`, `test_webui_provider_priority.py`, `test_task_store_fails_loud_in_production.py`, `test_claude_code_adapter.py`, `test_brain_patch_service_token.py`, `test_service_token.py`, `test_harness_spec.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `test_dashboard_cache.py`, `test_regression.py`, `test_agency_fix.py`, `test_telegram_mutating_commands.py`, `test_llm_router_resilience.py`, `test_v4_api.py`, `test_llm_router_disabled.py`, `allow_paid`, `client`, `reap_expired_companies`, `SchedulerStore`, `ai/router.py`, `test_doctor_coding_brain.py`, `test_voice_pipeline.py`, `_job_text`, `cost_tracker.py`, `TestNormalizeResponseFormat`, `TestAgentRunnerSafety`?**
...
- **Why does `AgentRunner` connect `AgentRunner` to `test_daily_automation_2026_07_11.py`, `proxy.py`, `test_ceo_dispatcher.py`, `WorkflowRun`, `TaskSpec`, `test_ceo_supervision.py`, `ReactScratchpad`, `PolicyEngine`, `BackgroundAgent`, `test_ceo_micromanager.py`, `ContextManager`, `Agency`, `MCPClient`, `backend/server.py`, `MultiAgentSwarm`, `test_failover_client_shared.py`, `StuckDetector`, `test_backend_server_features.py`, `webui/router.py`, `WorkspaceTools`, `settings.py`, `test_governance_api.py`, `AgentJobManager`, `TestAgentLoopMCPIntegration`, `E2BSandboxSession`, `test_empirical_verify.py`, `test_brain_failover_413.py`, `loop.py`, `LocalWorkspace`, `TokenBudget`, `TestAgentRunnerExecution`, `runtimes/manager.py`, `TestAgentRunnerSafety`, `GitHubTools`, `UserMemoryStore`, `AdaptiveHalter`, `ContextPruner`, `direct_chat.py`, `test_agent_tool_governance.py`, `FreeBuffAgent`, `AgentSessionStore`?**
...
- **Should `MongoDBStore` be split into smaller, more focused modules?**
  _Cohesion score 0.023513957307060754 - nodes in this community are weakly interconnected._