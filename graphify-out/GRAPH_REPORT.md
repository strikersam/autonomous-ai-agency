# Graph Report - autonomous-ai-agency  (2026-07-28)

## Corpus Check
- 1311 files · ~1,826,479 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 24964 nodes · 50480 edges · 1129 communities (1007 shown, 122 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 6061 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0d532422`
...
## Communities (1129 total, 122 thin omitted)

### Community 0 - "TaskSpec"
Cohesion: 0.03
Nodes (129): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider).      Ret, AiderAdapter, Any, TaskResult, TaskSpec, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized).  Aider (https, Run aider non-interactively via `--message` flag. (+121 more)

### Community 1 - "backend/server.py"
Cohesion: 0.01
Nodes (348): admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), _append_agent_session_message(), auth_me(), auto_recommend_skills(), brain_failover_status(), _brain_provider_status() (+340 more)

### Community 2 - "ProviderConfig"
Cohesion: 0.02
Nodes (188): get_tracker(), Return the process-singleton RateLimitTracker., _acquire_provider_probe(), clear_cooldowns(), _dead_model_key(), _exponential_backoff_cooldown(), extract_openai_text(), get_dead_models() (+180 more)

### Community 3 - "proxy.py"
Cohesion: 0.03
Nodes (158): Any, Return recent commits with agent attribution trailers parsed out., get_sam(), get_skill_registry_safe(), Return the global SkillRegistry if set, else None.     Used by onboarding and ot, middleware, admin_control(), admin_create_key() (+150 more)

### Community 4 - "FeatureMatrix"
Cohesion: 0.02
Nodes (67): check_feature(), get_feature(), list_features(), Any, get, post, Return the full support matrix with summary., Return a single feature entry. (+59 more)
...
Cohesion: 0.02
Nodes (120): AgentRunner, _nvidia_api_key(), Any, Path, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it.          Asks the planner model to writ, Send chat messages and return the assistant's text output.          If an Anthro, Call-time resolver for an agent role model id.      Delegates to ``services.brai (+112 more)

### Community 7 - "UserMemoryStore"
Cohesion: 0.05
Nodes (129): Agency, CEO-coordinated multi-agent agency for continuous codebase management.      The, AgentJobError, AgentJobRequest, AgentJobSnapshot, BaseModel, agent/contract.py — Typed public contract for the agent job lifecycle.  Phase, Structured error payload attached to a failed job. (+121 more)
...
Cohesion: 0.09
Nodes (15): get_scheduler_store(), _MemCollection, _MemCursor, _MemDB, _MemDeleteResult, Any, services/scheduler_store.py — Durable scheduler persistence.  Issue #505: AgentS, Delete a persisted job. (+7 more)
...
Cohesion: 0.06
Nodes (128): AccountLifecycleResponse, _DoctorCheck, _DoctorReport, get_public_doctor_report(), OnboardingAnswersRequest, OnboardingProgressResponse, OnboardingQuestionsRequest, BaseModel (+120 more)

### Community 12 - "FreeBuffAgent"
Cohesion: 0.07
Nodes (106): BackgroundAgent, BackgroundTask, Any, Enqueue *task* for processing. Returns the task (with task_id set)., Convenience: create a task and submit it in one call., Real handler — dispatches through AgentRunner when available.          HARDENE, Always-on worker that drains a task queue on a daemon thread.      GATE: Golde, BrowserSession (+98 more)

### Community 13 - "AgentSessionStore"
Cohesion: 0.03
Nodes (85): Agent subsystem — planner / executor / verifier loop., AgentPhaseError, _check_extra_kwargs(), _enforce_signature(), Exception, loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic classe, Raised when a named agent phase (planning, verification, etc.) fails. (+77 more)

### Community 14 - "Specialist"
Cohesion: 0.03
Nodes (72): get_skill(), list_skills(), List all available skills with optional filtering.      Returns the skill catalo, Get a single skill by its ID., SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., A specialist agent that can be provisioned for company-specific tasks. (+64 more)

### Community 15 - "test_ceo_dispatcher.py"
Cohesion: 0.04
Nodes (85): get_ceo_dispatcher(), _merge_changed_files(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., Collect changed_files across all specialists into a single de-duped list., reset_ceo_dispatcher(), ClassifyOutput, get_ceo_fallback_stats() (+77 more)

### Community 16 - "seo_api.py"
Cohesion: 0.03
Nodes (111): account_lifecycle(), auto_recommend_skills(), cancel_onboarding(), create_company(), generate_onboarding_questions(), get_company(), get_company_access(), get_company_graph() (+103 more)
...
Cohesion: 0.09
Nodes (14): _ProviderUsage, Sliding-window usage counters for one provider., In-process traffic distribution and budget accounting for providers., Drop request/token events that have fallen out of the window., Count a request that is about to be sent to *provider_id*.          Called immed, Record the outcome of a request started with ``record_start``.          Must be, Count a request that was routed away from *provider_id* (diagnostics)., EWMA latency in ms; never-sampled providers sort first.          Returning -1.0 (+6 more)
...
Cohesion: 0.03
Nodes (101): _active_primary_provider(), BrainConfig, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), is_north_mini_code_default() (+93 more)
...
Cohesion: 0.04
Nodes (88): agent/workflow.py — Persisted workflow state machine.  Implements the Autonomo, Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system.  Provides a lightweight task/issue tracker, Enum, str, tasks/models.py — Pydantic models for the task/issue system., Comment or reply on a task. (+80 more)
...
Cohesion: 0.07
Nodes (33): HealingEvent, _now(), Any, Translate external failure signals into improvement tasks and verify the     fix, Launch the background sweeper that resolves quiet verifying heals., Called when a CI workflow fails., Called when a GitHub issue with a bug label is opened., Called from the v4 dashboard 'Report Bug' form and the LogMonitor.          ``si (+25 more)

### Community 26 - "skill_bindings.py"
Cohesion: 0.03
Nodes (94): Status of a user story within a sprint., StoryStatus, HarnessAdapter, HarnessSpec, Any, agents/harness_adapter.py — ECC Cross-Harness Adapter  Normalises API difference, Adapt harness-native requests to the local-llm-server internal format.      Each, Detect which harness sent this request from headers.          Check order: expli (+86 more)
...
Cohesion: 0.04
Nodes (50): _NoopStore, Any, Exception, Approve a run paused at the ApprovalGate.          The caller must re-invoke ``e, Inject additional instructions into an in-flight run (no state change)., Push a Telegram approval-gate notification when a run pauses (Charter G1)., G5: resolve how a run should land from the company's DeliveryPolicy.          Re, Persist first-merge consent once an operator approves a ``telegram_gate``. (+42 more)

### Community 29 - "Task"
Cohesion: 0.05
Nodes (60): ApprovalCheckpoint, Full task/issue document., Human approval gate in a task's execution., Task, PUBLIC_URL-anchored dashboard deep link for the task detail.          Returns em, Best-effort Telegram heads-up that a task is parked awaiting approval., asyncio, TestTaskModel (+52 more)
...
Cohesion: 0.04
Nodes (59): emit_agency_observation(), Record an agency platform operation in Langfuse.      Unlike ``emit_chat_observa, Any, packages/ai/self_heal.py — automatic brain self-healing.  When the active brain, Background tick that runs self_heal_brain_and_unblock_tasks().      Called from, One-shot self-healing pass.      1. Checks if the active brain provider is in a, self_heal_brain_and_unblock_tasks(), _self_heal_tick() (+51 more)

### Community 32 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (59): Initialize the service.         Args:             store: Optional CompanyGraphSt, CompanyGraphStore, Delete a company and all its associated data., Delete a company and all associated data from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON         blob, Count total companies in the store., Create a new website in SQLite. The full model is stored in ``data``         so (+51 more)

### Community 33 - "agency.py"
Cohesion: 0.05
Nodes (44): AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _fetch_github_quick_notes(), get_agency() (+36 more)
...
Cohesion: 0.06
Nodes (35): Hand the 'connect & verify the repo' work to the agency's own agents.      The t, _seed_connect_task(), _is_brain_connection_error(), Any, BaseException, Task, TaskResult, TaskSpec (+27 more)

### Community 36 - "KeyStore"
Cohesion: 0.06
Nodes (38): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), get_output_filter(), Get or create the singleton OutputFilter instance., _check_rate_limit(), default_keys_path() (+30 more)

### Community 37 - "InternalAgentAdapter"
Cohesion: 0.04
Nodes (93): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), ResumeRequest, AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState (+85 more)
...
Cohesion: 0.06
Nodes (68): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``.      Adapter, Attempt, One delegation of one subtask to one tier., A subtask's full history: what it is, and every attempt at it., SubtaskRecord, What one sweep observed and did. Returned for tests and diagnostics., SweepReport (+60 more)

### Community 41 - "Initiative"
Cohesion: 0.04
Nodes (47): AgentRole, str, Initiative, Create and register a new initiative, returning it., Add a pre-built Initiative (e.g. from the intelligence layer)., Look up an initiative by ID., A portfolio initiative (epic) prioritised via WSJF.      WSJF (Weighted Shortest, Aggregate cost of delay (CoD) used as the WSJF numerator. (+39 more)
...
Cohesion: 0.03
Nodes (113): E2BAdapter, Any, Declare ``E2B_API_KEY`` as a required env dependency.          The base ``prefli, Runtime adapter that executes tasks inside an E2B sandbox.      Activation: regi, e2b_status(), _enrich_runtimes(), get_decision_log(), get_policy() (+105 more)

### Community 49 - "E2BSandboxSession"
Cohesion: 0.05
Nodes (60): E2BSandboxSession, _inject_token(), Kill the sandbox. Idempotent; never raises (best-effort cleanup)., Seed the sandbox from a host worktree by packing tracked files.          Creates, Extract changed files from the sandbox back to the host worktree.          After, Best-effort scrub of ``token`` from ``text``., Return ``(authed_url, clean_url)`` for a GitHub repo URL., Wraps the E2B async SDK to look like an MCP client to ``AgentRunner``.      Life (+52 more)

### Community 50 - "workflow_orchestrator.py"
Cohesion: 0.05
Nodes (82): CoordinateRequestV2, CoordinateResponse, BaseModel, Extended coordinate request supporting both agents+tasks and legacy workers., SwarmSummary, AgentSpec, CoordinatorResult, MultiAgentSwarm (+74 more)
...
Cohesion: 0.04
Nodes (43): delete_company_endpoint(), delete, Delete a company and all associated data.      Access control is handled by get_, Scan a repository for technology stack and systems., scan_repo_endpoint(), BusinessCategory, CompanyGraphService, get_company_graph_service() (+35 more)

### Community 56 - "SeoAuditRequest"
Cohesion: 0.04
Nodes (64): BaseModel, field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts  Typed Pydantic models for, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Aggregated report row - Screaming Frog CSV compatible., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website. (+56 more)

### Community 57 - "resolve_e2b_config"
Cohesion: 0.05
Nodes (64): TaskResult, TaskSpec, runtimes/adapters/e2b.py — E2B Firecracker micro-VM runtime adapter.  Routes age, Execute a task inside a fresh E2B sandbox.          Flow:           1. Open an :, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.          ``run_, Available iff config resolves AND the SDK is importable.          Never raises —, e2b_enabled(), E2BConfig (+56 more)

### Community 58 - "PortfolioManager"
Cohesion: 0.04
Nodes (37): CapacityAllocation, InitiativeProgress, PortfolioManager, PortfolioMetrics, Enum, Agentic Portfolio Management — initiative prioritisation, capacity allocation, a, Percentage of linked sprint points completed., Result of fitting initiatives into a fixed capacity by WSJF priority. (+29 more)
...
Cohesion: 0.04
Nodes (69): _check_background_liveness(), _check_ci_parity(), _check_company_graph(), _check_disk(), _check_event_log_integrity(), _check_feature_matrix(), _check_github_readiness(), _check_ollama() (+61 more)
...
Cohesion: 0.05
Nodes (56): Semaphore, CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), _offload(), Any, services/ceo_dispatcher.py — Real CEO delegation layer.  The CEO splits a comple (+48 more)
...
### Community 77 - "self_healing.py"
Cohesion: 0.09
Nodes (25): FailureCategory, heal_signature(), HealState, Enum, str, agent/self_healing.py — Self-Healing Agent (closed-loop, Autonomy Charter G2)  T, Classified failure types for targeted self-healing (E2)., Lifecycle of a heal (Autonomy Charter G2 closed loop). (+17 more)
...
Cohesion: 0.08
Nodes (39): _classify_sentiment(), _extract_keywords(), plan_research(), Any, BaseModel, field_validator, QualAnalysis, QualQuote (+31 more)

### Community 86 - "KeyPool"
Cohesion: 0.05
Nodes (36): api_keys_for(), _digest(), get_pool(), KeyPool, _KeyState, _PoolState, Per-provider API key rotation — the one lever that adds capacity.  Every other r, Round-robin key selection with per-key rate-limit cooldowns. (+28 more)
...
Cohesion: 0.06
Nodes (37): Resolve the model id to force for a code-execution run, or ``None``.      Return, resolve_coding_model_preference(), _check_auth(), health(), Any, BaseModel, get, post (+29 more)

### Community 103 - "services/background.py"
Cohesion: 0.08
Nodes (46): get_improvement_loop(), get_log_monitor(), agent/sam.py — SAM Voice Agent (System Autonomy Manager)  SAM is the voice-contr, get_self_healing_agent(), set_self_healing_agent(), get_trend_watcher(), set_trend_watcher(), Real-time self-healing activity: log-monitor capture counters plus the     most (+38 more)
...
Cohesion: 0.12
Nodes (11): AgentJobResult, Any, field_validator, Typed result returned by a completed agent job.      The ``response`` field is, Accept a bare string (legacy runner output) or a full dict., Build a snapshot from an ``AgentJob`` dataclass instance., get_chat_agent_job(), model_validator (+3 more)

### Community 117 - "AutonomyTracker"
Cohesion: 0.06
Nodes (23): AutonomyCounter, AutonomySnapshot, AutonomyTracker, get_tracker(), Any, agent/kpi.py — Autonomy KPIs: evidence capture and metrics tracking.  Tracks k, Return a point-in-time snapshot of all KPIs., Reset all counters (test helper). (+15 more)
...
### Community 143 - "test_quick_note.py"
Cohesion: 0.08
...
### Community 145 - "TestNormalizeResponseFormat"
Cohesion: 0.10
...
Cohesion: 0.06
Nodes (39): _get_provider_policy(), get_provider_policy_route(), Read the durable provider policy from DB, falling back to a safe default.      R, Return the provider policy (paid-provider kill switch state)., Resolve the LLM endpoint for a named surface (task/chat/ceo/sdlc/…).      Honour, resolve_provider_for(), asyncio, tests/test_provider_policy.py — Unit tests for the paid-provider kill switch.  T (+31 more)
...
### Community 159 - "test_control_plane_api.py"
Cohesion: 0.07
...
Cohesion: 0.04
Nodes (58): auto_disable_provider(), _disabled_from_mongo(), _disabled_from_sqlite(), disabled_providers(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _mongo_db() (+50 more)
...
Cohesion: 0.06
Nodes (16): fake_mongo(), _FakeCollection, _FakeDb, isolated_state(), fixture, Operator provider state must survive a redeploy.  The per-provider kill switch a, Tests must never mutate a shared operational store., The storage layer's localhost default is a placeholder, not config.          Tre (+8 more)
...
### Community 167 - "webui/router.py"
Cohesion: 0.06
Nodes (55): _bootstrap(), fixture, Path, ProviderManager, WorkspaceManager, tests/test_webui_provider_priority.py — Priority + reorder + brain-policy surfac, The /policy/brain endpoint must return the resolved brain + the paid     policy, The /providers/role-tags endpoint surfaces brain/sub/fallback roles     consiste (+47 more)
...
### Community 170 - "HarnessRegistry"
Cohesion: 0.09
Nodes (19): get_harness_adapter(), harness_active(), harness_catalog(), harness_session_close(), harness_session_start(), Return the full ECC harness catalog with capabilities.      Public — no auth req, Return the currently active ECC harnesses with session metrics.      Authenticat, Register a new harness session (called by the orchestrator on execute). (+11 more)
...
### Community 177 - "session_retro.py"
Cohesion: 0.13
Nodes (30): cluster_friction(), clusters_to_issues(), collect_friction_events(), FrictionCluster, FrictionEvent, judge_cluster(), Any, services/session_retro.py — session retrospective mining.  Closes the gap betwee (+22 more)
...
Cohesion: 0.11
Nodes (19): _backend(), CEOLedger, GoalRecord, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure., One piece of work the CEO has taken ownership of.      ``state`` is the supervis, Return ``(done, total)`` subtask counts. (+11 more)
...
Cohesion: 0.07
Nodes (45): build_ceo_router(), Any, APIRouter, backend/ceo_router.py — observability and manual control for the CEO.  Surfaces, Reject non-admin callers for routes that spend provider budget.      Delegates t, _require_admin(), _env_flag(), _env_int() (+37 more)

### Community 193 - "tests/conftest.py"
Cohesion: 0.10
Nodes (29): Item, app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state(), non_admin_client() (+21 more)
...
Cohesion: 0.07
Nodes (29): export_seo_audit(), Export a stored audit.      - ``csv``       aggregated findings, Screaming Frog, _build_curl_cffi_fetcher(), _build_pdf(), main(), _parse_args(), Namespace, Path (+21 more)
...
Cohesion: 0.07
Nodes (32): AgentScheduler, _now(), Any, agent/scheduler.py — Scheduled Agent Jobs  Cron-based job scheduler.  Each job h, Register a new job.  Returns the created :class:`ScheduledJob`.          ``run_o, Fire a job immediately (webhook / manual trigger)., Remove a job. Returns *True* if it existed., Update the display name of a job. (+24 more)
...
Cohesion: 0.10
Nodes (35): brain_availability_summary(), Non-secret answer to "can the brain answer a request right now?".      Three cal, Drop the singleton (test helper)., reset_ceo_ledger(), _doctor(), _P, _patch_providers(), asyncio (+27 more)
...
### Community 259 - "agents/agile_ceremonies.py"
Cohesion: 0.08
...
Cohesion: 0.17
Nodes (7): ProviderCircuit, Attempt to move from OPEN to HALF_OPEN after recovery timeout., Check if a request can be made through this circuit., Per-provider circuit breaker state machine., TestProviderCircuit, parametrize, Counting every sub-500 as a success meant the breaker could never         open f
...
### Community 275 - "get_registry"
Cohesion: 0.04
Nodes (37): north_mini_code_model_for(), Return the North Mini Code model id served by *provider*, else ``None``.      ``, get_registry(), Return model registry, extended with ROUTER_EXTRA_MODELS env entries.      ROU, test_bedrock_haiku_4_5_in_registry(), test_bedrock_opus_48_in_registry(), test_bedrock_opus_4_6_v1_in_registry(), test_bedrock_opus_4_7_in_registry() (+29 more)
...
### Community 278 - "get_tool_registry"
Cohesion: 0.11
Nodes (14): get_tool_registry(), Return the module-level ToolRegistry singleton.      On first call, registers, get_enrichment(), HarnessEnrichment, invalidate_enrichment_cache(), Any, agent/harness_enrichment.py — Automatic Harness Enrichment for Agent Prompts  Ev, Build a compact catalog of available runtime skills.          Discovers from Ski (+6 more)
...
### Community 287 - "test_agency.py"
Cohesion: 0.14
Nodes (31): _parse_ceo_directives(), set_improvement_loop(), agency(), asyncio, fixture, Path, Tests for agent/agency.py, With no issues, CEO should report nominal. (+23 more)
...
Cohesion: 0.04
Nodes (69): autonomy_status(), Public autonomy readiness probe — no authentication required.      A live deploy, requires_db, Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap(), CompanyAgencyService, get_company_agency_service(), _is_runtime_available_sync() (+61 more)

### Community 303 - "BrainFailoverManager"
Cohesion: 0.08
Nodes (19): _disabled_ids(), Reset every circuit breaker when they have all tripped, and re-select.      With, The operator kill-switch set, or empty when it cannot be read., _recover_all_unhealthy(), BrainFailoverManager, Any, Permit one probe call without claiming the provider succeeded.          This is, Seconds until the soonest cooling provider is probeable again.          ``None`` (+11 more)
...
### Community 315 - "test_unit7_catalog_propagation.py"
Cohesion: 0.07
...
Cohesion: 0.08
Nodes (16): NIMConnectionPool, Any, AsyncClient, Response, Persistent httpx.AsyncClient pool with circuit breaker and retry logic.      Man, Get or create the shared httpx.AsyncClient., Context manager for a pooled client session., Close the connection pool. (+8 more)
...
Cohesion: 0.05
Nodes (48): agents/store.py — Persistent store for user-defined agent configurations.  Store, Return an agent by ID.          If *owner_id* is provided, enforces that the age, Delete an agent.  Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents., Return all agents in the system (admin use only)., Set the global agent store instance (e.g., with MongoDB on startup)., set_agent_store(), ensure_bootstrap() (+40 more)

### Community 336 - "_resolve_user_github_token"
Cohesion: 0.17
Nodes (18): Return the caller's GitHub token from EITHER place it can be stored.      A toke, _resolve_user_github_token(), _FakeCollection, _FakeDB, patch_db(), asyncio, fixture, The doctor must find a GitHub token wherever the connect flow stored it.  Produc (+10 more)
...
Cohesion: 0.09
Nodes (20): Agent Autonomy Roadmap, Design constraints honored, New environment variables, Proactive rate-limit pacing (free-tier reliability), The eight gaps and what closed them, Verification performed, What was already strong (verified, no changes needed), Why this document exists (+12 more)
...
### Community 349 - "IssueCategory"
Cohesion: 0.12
Nodes (25): IssueCategory, IssueSeverity, Enum, str, agent/improvement_loop.py — Continuous Improvement Engine  Background scanner th, agent/trend_watcher.py — Internet-connected AI trend intelligence.  Fetches fr, _match_family(), Any (+17 more)
...
Cohesion: 0.19
Nodes (12): _deep_merge(), Deep merge two dicts. Override values take precedence., CircuitBreakerOpenError, CircuitState, Enum, RuntimeError, Raised when a request is blocked by an open circuit breaker., A single instruction/response pair for fine-tuning. (+4 more)
...
### Community 392 - ".start_onboarding"
Cohesion: 0.08
...
Cohesion: 0.12
Nodes (15): tests/test_sam_voice.py — Integration tests for SAM voice agent.  Tests the SAM, Same session_id must return the same session., get_sam() must return the same instance., get_status() must return uptime and session count., Empty input must return a prompt to repeat., Whitespace-only input must be treated as empty., Fallback must acknowledge task requests., A new session must be created on first command. (+7 more)
...
Cohesion: 0.21
Nodes (12): _now(), agent/background.py — Background Agent  An always-on worker thread that proces, Tests for agent/background.py — Background Agent., _task(), test_as_dict(), test_create_and_submit(), test_get_missing_returns_none(), test_get_task() (+4 more)
...
### Community 412 - "telegram_service.py"
Cohesion: 0.13
...
Cohesion: 0.04
Nodes (56): Scan a website for technology stack and systems.          This performs a compre, scan_website_endpoint(), main(), Returns (url, ok, summary)., _scan_one(), _is_safe_url(), _looks_like_bot_challenge(), Any (+48 more)
...
Cohesion: 0.08
Nodes (16): get_synthetic_pipeline(), Any, Add a step result. Returns the sample if accepted, None if filtered out., Bulk-add samples from an agent session's step results.          Each step result, Return samples filtered by minimum reward score., Export samples in Alpaca JSONL format.          Returns the path to the exported, Export samples in ShareGPT JSONL format.          Returns the path to the export, Export all samples as a structured JSON array.          Returns the path to the (+8 more)
...
Cohesion: 0.04
Nodes (53): NotificationDispatcher, Any, Path, Start the Telegram bot. Returns True if started successfully., Signal the bot to stop and wait for graceful shutdown., Run the Telegram bot long-poll loop (inline, not subprocess)., Run the bot with stop-event awareness., Routes background task results to configured notification channels.      Current (+45 more)

### Community 476 - "test_onboarding_provisioning.py"
Cohesion: 0.16
...
### Community 477 - "scan_repo_with_user_research.py"
Cohesion: 0.22
...
Cohesion: 0.10
Nodes (23): cached_models(), discover_models(), _fresh_entry(), _models_url(), _parse_ids(), Any, Ask a provider which models the configured API key may actually use.  ``config/m, Return the cached list for *provider_id* without any network call.      The disp (+15 more)
...
### Community 508 - "test_tasks_awaiting_approval_api.py"
Cohesion: 0.31
Nodes (12): _client(), asyncio, fixture, Task, TestClient, GET /api/tasks/awaiting-approval — dashboard surface for the pre-execution gate., _seed(), task_store() (+4 more)
...
### Community 536 - "OrchestratorSupervisor"
Cohesion: 0.09
...
### Community 576 - "sam_livekit_worker.py"
Cohesion: 0.17
...
### Community 578 - "._execute_with_fallback"
Cohesion: 0.16
...
### Community 606 - "DailyDigestAggregatorTests"
Cohesion: 0.19
...
### Community 609 - "TestBrainConfigUpdates"
Cohesion: 0.14
...
### Community 667 - "test_task_pre_execution_gate.py"
Cohesion: 0.25
...
### Community 670 - "test_commit_tracker.py"
Cohesion: 0.23
...
Cohesion: 0.05
Nodes (33): cleanup_stale_jobs(), _is_stale(), Any, packages/scheduler/cleanup.py — schedule deduplication + stale removal.  Extract, Remove a job from the store. Returns True on success, False on failure.      Log, Check if a created_at timestamp is older than ttl_seconds.      Handles multiple, Remove stale run-once + stuck agency jobs from the durable store.      Args:, _safe_remove() (+25 more)
...
### Community 677 - ".discover_directory"
Cohesion: 0.29
...
### Community 701 - "test_task_self_repo_auto_commit.py"
Cohesion: 0.17
Nodes (14): _mock_gh(), parametrize, tests/test_task_self_repo_auto_commit.py — TaskExecutionCoordinator._build_spec, A task with company_id set must go through the existing E2B     company-repo wir, Task types not in the ship-code set (e.g. "custom_research") must NOT get     au, SELF_REPO_AUTO_COMMIT_ENABLED=false is the rollback lever — even with     valid, If no GitHub repo/token is configured, the task must stay report-only     rather, A resolution error (e.g. agent.agency import failure) must never crash     _buil (+6 more)
...
### Community 715 - "test_telegram_task_callback.py"
Cohesion: 0.19
...
### Community 722 - "analyze_qualitative"
Cohesion: 0.25
...
### Community 797 - "model_catalog.py"
Cohesion: 0.21
Nodes (11): get_catalog_models_route(), Return the mirrored model catalog (advisory-only).      Flag-gated by ``FREELLM_, get_catalog(), get_model_catalog_store(), is_catalog_enabled(), packages/ai/model_catalog.py — UNIT 8: free-LLM-API model catalog sync.  Advisor, Return the process-wide ModelCatalogStore singleton., Convenience wrapper used by the GET /api/catalog/models endpoint. (+3 more)
...
### Community 803 - "get_livekit_config"
Cohesion: 0.18
...
Cohesion: 0.04
Nodes (40): _auto_disable(), _Budget, _build_request(), _describe_registry(), FailoverResult, _is_billing_refusal(), _is_ollama(), _key_pool() (+32 more)

### Community 841 - "Coding Standards"
Cohesion: 0.22
...
### Community 846 - "test_agent_tools.py"
Cohesion: 0.43
...
### Community 888 - "start_in_process"
Cohesion: 0.25
...
### Community 911 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 939 - "_resolve_default_executor_model"
Cohesion: 0.50
...
### Community 950 - "test_direct_chat_doctor.py"
Cohesion: 0.67
Nodes (3): asyncio, When git is missing and no GitHub token is present, the doctor should report bot, test_missing_git_and_token()
...
### Community 992 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 1016 - "test_runtime_registration_scope.py"
Cohesion: 0.33
...
### Community 1031 - "_normalize_dockerfile"
Cohesion: 0.33
...
### Community 1035 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 1036 - "Agent API Issues"
Cohesion: 0.40
...
### Community 1037 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 1041 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1057 - "test_mint_token_rejects_missing_args"
Cohesion: 0.67
...
### Community 1125 - "test_docker_binary_missing.py"
Cohesion: 0.07
Nodes (10): Any, Any, Resolve the NVIDIA default model via the catalog (UNIT 7).      Was hardcoded to, _resolve_nvidia_default_model(), Any, Called once when the runtime is registered.  Override to start         a subproc, Return True if the runtime is available (not circuit-open)., True if the runtime is healthy enough for the router to select it. (+2 more)
...
## Knowledge Gaps
- **3228 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `aider_config.sh script`, `OPENAI_API_BASE` (+3223 more)
...
- **Why does `AgentRunner` connect `AgentRunner` to `TaskSpec`, `backend/server.py`, `proxy.py`, `test_empirical_verify.py`, `TestMCPServer`, `test_agent_free_brain.py`, `UserMemoryStore`, `Workspace`, `AdaptiveHalter`, `FreeBuffAgent`, `AgentSessionStore`, `test_ceo_dispatcher.py`, `LocalWorkspace`, `ExecutionRequest`, `WorkflowRun`, `InternalAgentAdapter`, `Initiative`, `test_failover_client_shared.py`, `ContextPruner`, `runtimes/api.py`, `workflow_orchestrator.py`, `_resolve_push_token`, `test_brain_failover_413.py`, `resolve_e2b_config`, `GitHubTools`, `diagnostics.py`, `CEODispatcher`, `TokenBudget`, `AgentJobManager`, `TestZeroAttemptDiagnostics`, `failover_client.py`, `ContextManager`, `MCPUnavailableError`, `MCPClient`, `ReactScratchpad`, `StuckDetector`, `AutonomyTracker`, `WorkspaceTools`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `SelfHealingAgent` connect `SelfHealingAgent` to `BackgroundServices`, `ContextWindowManager`, `services/background.py`, `ImprovementLoop`, `CostAttributor`, `test_integration_c4_c5_c6_d3.py`, `self_healing.py`, `PromptCacheManager`, `ChatHistoryStore`, `Harness`, `NotificationDispatcher`, `telegram_service.py`, `IssueCategory`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderConfig` to `backend/server.py`, `TestRouterIntegration`, `UserMemoryStore`, `TrafficDirector`, `HealthStatus`, `TestBrainFailoverBackoff`, `InternalAgentAdapter`, `test_colibri_provider.py`, `Initiative`, `resolve_active_brain`, `test_all_providers_discovery.py`, `failover_client.py`, `test_bedrock_provider.py`, `_P`, `test_anthropic_router.py`, `system_instruction`, `test_chat_mode_regressions.py`, `TestActiveStrategy`, `test_bedrock_live.py`?**
