...
## Corpus Check
- 1529 files · ~2,227,228 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 32423 nodes · 66727 edges · 1317 communities (1150 shown, 167 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 6582 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e32a9486`
...
- _clean_phases
- get_operational_incident_tracker
...
## Communities (1317 total, 167 thin omitted)

### Community 0 - "llm/router.py"
Cohesion: 0.03
Nodes (131): int, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager., text_key() (+123 more)

### Community 1 - "backend/server.py"
Cohesion: 0.02
Nodes (179): set_skill_registry(), admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody (+171 more)
...
Cohesion: 0.03
Nodes (170): Describes a single worker's task and constraints., WorkerSpec, set_quick_note_queue(), get_sam(), fastapi_middleware_cors, middleware, admin_control(), admin_create_key() (+162 more)

### Community 4 - "TaskSpec"
Cohesion: 0.01
Nodes (231): abc, 3b. Hermes — **our own** Hermes server (in-repo), UI-wired ✅  (size: M), N2. Surface Hermes (and all runtimes) status in the Doctor/Runtimes UI ⬜  (size: S, risk: low), kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider… (+223 more)

### Community 5 - "llm/context.py"
Cohesion: 0.07
...
Cohesion: 0.02
Nodes (169): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding() (+161 more)

### Community 7 - "TaskStatus"
Cohesion: 0.04
Nodes (91): Helpers that turn scheduler and playbook activity into real tasks., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, _coerce_ts(), Enum, str, tasks/models.py — Pydantic models for the task/issue system., Coerce ISO-8601 datetime strings (from DB) to float timestamps., Comment or reply on a task. (+83 more)
...
### Community 9 - "api.js"
Cohesion: 0.01
Nodes (46): approveGovernanceRequest(), autoRecommendCompanySkills(), createMcpServer(), createQuickNote(), createTask(), deleteGithubToken(), deleteMcpServer(), deleteModel() (+38 more)

### Community 10 - "BrainConfig"
Cohesion: 0.02
Nodes (120): Architecture (per plan §3), Files touched, Hard constraints (from the plan) — all met, Implementation — DB-persisted, UI-switchable Brain (PR #824 follow-up), New files, Resolution precedence, Risks & mitigations (per plan §6), Rollout / verification (per plan §5) (+112 more)

### Community 11 - "load_config"
Cohesion: 0.08
Nodes (42): _apply_key_env(), _build(), _coerce(), config_dir(), expand_env(), _fields_of(), _load_cache(), load_config() (+34 more)

### Community 12 - "LLMRequest"
Cohesion: 0.03
Nodes (101): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, packages_llm_providers, build_provider(), Instantiate the adapter for one provider config. Unknown kinds fall back to the…, LLMRequest, Cheap character-based token estimate (~4 chars/token). Deliberately dependency-…, A provider-neutral chat request. ``model`` is a *hint*: the router may… (+93 more)
...
### Community 14 - "sandbox.py"
Cohesion: 0.05
Nodes (38): Path to enforcement, Step 1 — Deploy and watch (1–2 weeks), Step 2 — Tune, Step 3 — Enforce one group at a time, Step 4 — Enable a real sandbox backend, detect_backend(), E2BBackend, LocalBackend (+30 more)

### Community 15 - "SelfHealingAgent"
Cohesion: 0.06
Nodes (45): HealingEvent, _now(), Any, Translate external failure signals into improvement tasks and verify the fix…, Launch the background sweeper that resolves quiet verifying heals., Called when a GitHub issue with a bug label is opened., Called from the v4 dashboard 'Report Bug' form and the LogMonitor.…, Record that an error with *signature* was just seen again. Called by the… (+37 more)

### Community 16 - "PolicyEngine"
Cohesion: 0.04
Nodes (78): fnmatch, _load(), Import the governance layer once, recording why if it is absent., _action_matches(), _as_list(), Decision, GroupPolicy, _host_matches() (+70 more)
...
Cohesion: 0.07
Nodes (44): build_docker_run_argv(), DockerBackend, load_profiles(), Path, Build the hardened ``docker run`` argv for *profile*. Split out from the…, Runs sandboxes as hardened Docker containers. The subprocess runner is…, Load sandbox profiles from YAML, merged over the built-ins. A file may add…, Creates, tracks, reaps, and audits sandboxes. Lifecycle guarantees: * Every… (+36 more)

### Community 19 - "test_model_router.py"
Cohesion: 0.05
Nodes (81): classify_task(), _extract_recent_text(), Any, Concatenate plain text from the last *last_n* messages., Return the most likely task category for this request. Args: messages: OpenAI-…, Reset the singleton and clear the cached model map (test helper)., reset_router(), clear_router() (+73 more)

### Community 20 - "_is_ephemeral_user"
Cohesion: 0.27
...
Cohesion: 0.05
Nodes (67): BudgetTracker, GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., new_session_id(), Build an :class:`AgentIdentity` from whatever the caller knows. Total by design…, Return the stable ``agent:<slug>`` id for a human agent name. Deterministic and…, Return a fresh, unguessable session id. Unguessable matters because the session… (+59 more)

### Community 23 - "test_e2b_sandbox.py"
Cohesion: 0.05
Nodes (52): _inject_token(), maybe_attach_e2b(), Best-effort scrub of ``token`` from ``text``., Open an E2B session and attach it as ``runner._mcp``. Single wiring line for…, Return ``(authed_url, clean_url)`` for a GitHub repo URL., _scrub_token(), fake_sandbox(), _FakeAsyncSandboxClass (+44 more)

### Community 24 - "Task"
Cohesion: 0.04
Nodes (61): Full task/issue document., Task, _gate_outward_facing_enabled(), _is_outward_facing(), Any, One-line human explanation of why this task needs human approval. Reads risk…, Best-effort label of who will run this task once approved. Returns…, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty… (+53 more)

### Community 25 - "CompanyGraphService"
Cohesion: 0.03
Nodes (43): BusinessCategory, Find a website by its URL., A company website with detected systems and stack inference., Website, CompanyGraphService, SpecialistFamily, SystemType, Get a company by ID. Args: company_id: Company ID Returns: Company instance or… (+35 more)

### Community 26 - "ModelConfig"
Cohesion: 0.08
Nodes (18): _merge_env_defaults(), ModelConfig, Capability and pricing metadata for one model id., Add providers that are configured in the environment but absent from YAML. This…, Give every provider's default model a registry entry if YAML omitted it., _seed_model_defaults(), ModelRegistry, Add or replace a model (used by runtime discovery). (+10 more)

### Community 27 - "test_scanner_headless.py"
Cohesion: 0.03
Nodes (29): 5. Input Validation, Tests for the scanner's headless-render fallback (JS-rendered / bot-protected…, The scan flow must invoke the render fallback when static detection is empty…, BuiltWith-style off-site identification: a CNAME chain that points at a known…, A scan must never hang past its wall-clock budget — a slow/blocked domain has…, Last-resort fallback that asks builtwith.com what it already knows about a…, Replace curl_cffi's AsyncSession.get with a canned response., The whole point of the headless pass: tech markers that only appear in the JS-… (+21 more)

### Community 28 - "_start_autonomy_loops"
Cohesion: 0.06
Nodes (53): _dispatch_async(), _run(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), LogRecord, Register the error capture handler on the root logger. (+45 more)

### Community 29 - "Agency"
Cohesion: 0.04
Nodes (54): Agency, AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _collect_recent_git_context(), _now_str(), Any (+46 more)

### Community 30 - "heal_signature"
Cohesion: 0.10
...
Cohesion: 0.06
Nodes (87): BrainFailoverExhausted, failover_chat_completion(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, Run one chat completion across the brain-failover chain. Tries each healthy…, _free_tier(), _hit_ids(), _many_providers() (+79 more)

### Community 32 - "WebsiteScanner"
Cohesion: 0.03
Nodes (74): pytest_asyncio, main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), _is_blocked_host(), _is_safe_url(), _looks_like_bot_challenge() (+66 more)
...
### Community 35 - "strategies.py"
Cohesion: 0.05
Nodes (86): count, Preferring local, Choosing one, provider_health, Costs are higher than expected, Latency got worse, HealthConfig, Strategy selection and degradation behaviour. (+78 more)

### Community 36 - "MongoDBStore"
Cohesion: 0.03
Nodes (106): bson, Step 5 — Note real gaps instead of pretending they're solved, What this maps to (real code, not a generic scaffold), ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraph, CompanyGraphResponse (+98 more)
...
Cohesion: 0.03
Nodes (112): contextvars, CEOResult, _complexity_rank(), Aggregated output from a multi-specialist execution., _should_fan_out(), BoundContext, ClassifyOutput, ExecutionResult (+104 more)
...
### Community 40 - "brain_config.py"
Cohesion: 0.02
Nodes (105): packages_ai, _active_primary_provider(), is_north_mini_code_default(), provider_api_key(), provider_api_keys(), _provider_env_value(), provider_key_present(), provider_key_rotation_enabled() (+97 more)

### Community 41 - "AgentRunner"
Cohesion: 0.03
Nodes (81): AgentRunner, _run(), _check_extra_kwargs(), _enforce_signature(), Any, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Append an event to the durable session log if a store is wired in. (+73 more)
...
Cohesion: 0.07
Nodes (21): PrimeAgentAdapter, Adapter for the Prime Agent / pi coding CLI., Fail preflight when the provider extension is configured but missing. Without…, tests/test_prime_agent_runtime.py — PrimeAgentAdapter unit tests. The event-…, plain `pi` rejects --cwd; the workspace is set on the subprocess instead., Regressions for defects found in review of this adapter., health_check() and readiness_check() must agree about the binary. The base…, Preflight rejects agency without the proxy extension configured. (+13 more)

### Community 44 - "test_unit8_model_catalog.py"
Cohesion: 0.03
Nodes (92): all_provider_ids(), _provider_ids_from_literal(), Resolve the model id for a component's role on a provider. Parameters…, Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, resolve_component_model(), CatalogActiveBrain, CatalogMirror (+84 more)

### Community 45 - "Added"
Cohesion: 0.04
Nodes (52): Return True only when the tool is definitively read-only and non-destructive.…, Remember a server-issued ``Mcp-Session-Id`` for subsequent requests., Added, Added, Render MCP — platform debugging and environment monitoring, 1. Coding sessions — stdio, via `.mcp.json`, 2. The running agency — Streamable HTTP against a deployed sidecar, Configuration (+44 more)

### Community 46 - "ref_react"
Cohesion: 0.06
Nodes (33): API, getActivity(), HubTabs(), Spinner(), ActivationGate(), activityToAlert(), AlertsBell(), priorityConfig (+25 more)

### Community 47 - "build_matrix"
Cohesion: 0.15
Nodes (16): Recommend skills relevant to the current task. Called from the tool-call loop…, build_matrix(), _families(), main(), Any, Map family -> sorted list of test files that mention it (quoted token)., Return one row per family, derived entirely from code., render_markdown() (+8 more)

### Community 48 - "secrets_store.py"
Cohesion: 0.07
Nodes (45): Finding A — `list_for_user` Mongo query diverges from the `_can_read` policy, Finding B — `/api/secrets` router is mounted with no authentication dependency, Minor, non-security, Part 1 — The audit, Suggested fixes (not applied here), The chain, What was checked and is sound, UserRole (+37 more)
...
### Community 50 - "PageFetcher"
Cohesion: 0.08
Nodes (24): get_status(), get, Serve the launcher UI., Get current service status., root(), A single occurrence of a check firing on a specific URL., SeoIssueInstance, is_internal() (+16 more)

### Community 51 - "runtimes/api.py"
Cohesion: 0.04
Nodes (79): B. Make runtime activation non-blocking (`runtimes/control.py`,, FastAPI dependency: require any authenticated user., require_authenticated(), _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes() (+71 more)

### Community 52 - "setup/api.py"
Cohesion: 0.06
Nodes (68): is_user_onboarding_allowed(), Return True if this user may run the onboarding wizard. Resolution order: 1. If…, motor_motor_asyncio, clear_wizard_state_cache(), complete_wizard(), _delete_wizard_state(), detect_configured_providers(), detect_hardware_for_wizard() (+60 more)
...
Cohesion: 0.06
Nodes (22): _digest(), KeyPool, _KeyState, _PoolState, Round-robin key selection with per-key rate-limit cooldowns., Cool a single key after a 429 from it. Honours the provider's own ``Retry-…, True when every key in the pool is resting. This is the signal that the…, Diagnostics. Contains no key material — digests only. (+14 more)

### Community 55 - "Changed"
Cohesion: 0.04
Nodes (91): _close_github_issue(), _fetch_github_quick_notes(), _gh_repo(), _gh_token(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, Real handler — dispatches through AgentRunner when available. HARDENED (PR…, Public wrapper around _generate_plan for callers that want plan-only. Returns… (+83 more)

### Community 56 - "WorkflowOrchestrator"
Cohesion: 0.04
Nodes (52): Changed, Changed, Feature maturity — what's stable vs. beta, Admin API, Config Overrides, Feature Matrix, Feature Support Matrix, Gating Behavior (+44 more)

### Community 57 - "SeoFixRequest"
Cohesion: 0.28
Nodes (8): Request to remediate auto-fixable findings in a local code repository., SeoFixRequest, Convenience wrapper used by the API layer and skill executor., run_fixes(), Path, tests/test_seo_fixer.py - repo-aware SEO auto-fixer tests (issue #533)., TestDryRun, TestScoping
...
Cohesion: 0.05
Nodes (23): MockTransport, _build_curl_cffi_fetcher(), get(), get_text(), head(), _sess(), Return a PageFetcher that uses curl_cffi for all requests., FetchResult (+15 more)

### Community 60 - "resolve_active_brain"
Cohesion: 0.07
Nodes (51): BrainResolution, get_brain_config(), Convenience wrapper used by the agent loop + brain resolver., get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), _norm(), _pick_from_records() (+43 more)
...
Cohesion: 0.03
Nodes (57): _bootstrap_within_budget(), _create_bootstrap_indexes(), ensure_bootstrap(), The task store the background services should use, wiring it if needed.…, Point the feature stores at the shared database connection. Deliberately…, Create every boot index concurrently rather than one round-trip at a time.…, Idempotent bootstrap for indexes + seeded admin/providers. FastAPI startup…, Auto-detect the best available Ollama model and update ollama-local in the DB.… (+49 more)

### Community 63 - "test_web_reach.py"
Cohesion: 0.05
Nodes (60): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), _domain_list_reason(), _domain_matches(), _parse_domain_list(), Return True if *host* is *pattern* or a subdomain of it., Split a comma-separated env var into a normalised domain list., Return why the env-var domain policy blocks *host*, or None. Checks… (+52 more)

### Community 64 - "facade.py"
Cohesion: 0.09
Nodes (32): create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., create_access_token(), create_refresh_token(), get_current_user(), get_optional_user() (+24 more)

### Community 65 - "BrainWatchdog"
Cohesion: 0.07
Nodes (23): 0. The goal (unchanged), 1. Shipped in the previous pass ✅ (recap, do not redo), 2. Pending ⬜ — detailed implementation specs, 3. Deferred 🔭, 4. Operating notes (unchanged, for implementers), N1. Activate the reliability spine — wire the watchdog, schedule the digest ⬜  (size: M, risk: low), N5. Mutating Telegram control (switch brain / merge PR from the phone) ✅  (size: M, risk: high — `risky-module-review`), Next-Pass Roadmap — Detailed Implementation Specs (+15 more)

### Community 66 - "test_webui_provider_priority.py"
Cohesion: 0.06
Nodes (48): AdminIdentity, test_admin_api_login_status_and_control(), _fake_user_auth(), Path, test_admin_can_create_anthropic_provider_via_webui_admin_api(), test_admin_can_create_provider_via_webui_admin_api(), test_ui_providers_and_workspaces_use_app_state(), _bootstrap() (+40 more)
...
Cohesion: 0.06
Nodes (55): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget() (+47 more)

### Community 69 - "test_ceo_cross_verify.py"
Cohesion: 0.20
...
Cohesion: 0.05
Nodes (39): Report whether the SAM realtime voice (LiveKit) transport is configured., sam_livekit_status_backend(), auth_headers(), livekit_env(), no_livekit_env(), _normalize_dockerfile(), parametrize, tests/test_sam_livekit.py — SAM realtime voice (LiveKit) integration. Covers: -… (+31 more)
...
Cohesion: 0.04
Nodes (52): Return orchestrator queue depth, active runs, and supervisor state (#522)., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_status(), get_orchestrator_queue(), get_orchestrator_supervisor(), OrchestratorSupervisor, Any (+44 more)
...
### Community 80 - "agent/workspace.py"
Cohesion: 0.29
...
### Community 81 - "test_research_coordinator.py"
Cohesion: 0.11
...
Cohesion: 0.05
Nodes (53): Parse an LLM tool-call response, tolerating common malformations. Handles: 1.…, Spawn a child AgentRunner for a delegated sub-task. When sub-agent configs are…, Return up to *limit* stored patterns relevant to *query*. Relevance is scored…, Fixed, 8. Backwards compatibility by shim, not by rewrite, Fixed, Compatibility, _describe_registry() (+45 more)

### Community 91 - "ai/router.py"
Cohesion: 0.04
Nodes (51): Added, Added, Record token usage for *model* (fire-and-forget, never raises). ``tag`` is a…, record_usage(), _acquire_provider_probe(), CommercialFallbackRequiredError, _exponential_backoff_cooldown(), _get_director() (+43 more)
...
### Community 97 - "OnboardingScreen.jsx"
Cohesion: 0.07
Nodes (23): Phase 4 — Onboarding fixes (#593, #619, PR #623) ⏳, consultExecutives(), createCompany(), delegateSeoFindings(), getSeoAudit(), listExecutives(), listSeoAudits(), EXECS (+15 more)

### Community 98 - "SeoFixer"
Cohesion: 0.15
Nodes (10): _humanize_filename(), BeautifulSoup, Path, Derive readable alt text from an image path: 'img/hero-banner_2.jpg' -> 'Hero…, Applies (or proposes) automatic SEO remediations inside a repo checkout., Scan the repo, fix every enabled auto-fixable problem found., SeoFixer, repl() (+2 more)

### Community 99 - "test_hermes_in_process.py"
Cohesion: 0.07
Nodes (26): _check_auth(), health(), Any, BaseModel, get, post, Execute a task synchronously via the InternalAgentAdapter (our brain). Response…, Body for POST /tasks — mirrors the payload HermesAdapter.execute sends. (+18 more)

### Community 100 - "ProviderConfig"
Cohesion: 0.06
Nodes (40): is_commercial_provider(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Regression tests for the agent loop's multi-provider failover wire format.…, The pieces the failover loop assembles for an Anthropic-native provider., TestAnthropicRequestShape (+32 more)

### Community 101 - "resolve_e2b_config"
Cohesion: 0.05
...
### Community 102 - "TaskWorkflowService"
Cohesion: 0.04
Nodes (59): AgentDefinition, AgentStore, Any, field_validator, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents. (+51 more)

### Community 103 - "UserInfo"
Cohesion: 0.07
Nodes (47): AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState, FailedJob, BaseModel, Enum, str (+39 more)
...
### Community 105 - "get_secrets_store"
Cohesion: 0.13
Nodes (22): _get_github_token_for_user(), Fetch GitHub token for user from secrets store or environment., create_secret(), delete_secret(), get_secret_metadata(), get_secrets_store(), _get_user(), list_secrets() (+14 more)
...
Cohesion: 0.05
Nodes (47): get_mcp_client(), Return the module-level MCPClient. Reads MCP_SERVER_BASE_URL at call time (not…, _internal_configured(), MCPServerSpec, _not_dialable(), _playwright_configured(), _playwright_spec(), _probe_http() (+39 more)
...
Cohesion: 0.05
Nodes (60): _bullets(), generate_backlog_retro(), generate_sprint_retro(), generate_standup(), plan_next_sprint(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive retro notes for ``sprint`` from its current metrics. Records… (+52 more)
...
Cohesion: 0.07
Nodes (24): Attempt, _backend(), CEOLedger, GoalRecord, _now(), Any, One delegation of one subtask to one tier., One piece of work the CEO has taken ownership of. ``state`` is the supervisor's… (+16 more)

### Community 116 - "Any"
Cohesion: 0.07
Nodes (21): _NoOpSpan, _NoOpTracer, otel_middleware_factory(), otel_status_error(), otel_status_ok(), Any, Exception, Lazy-initialised OpenTelemetry tracer provider. Only imports the OTEL SDK when… (+13 more)

### Community 117 - "test_integration_c4_c5_c6_d3.py"
Cohesion: 0.15
Nodes (17): get_current_trace_id(), get_tracer(), langfuse_metadata_with_trace(), Portable trace context that can be passed across async boundaries., Return an OpenTelemetry tracer for the given name., Shut down tracing (flush pending spans)., Extract trace context from an incoming HTTP request. Looks for W3C Trace…, Convert a TraceContext to W3C trace context HTTP headers. (+9 more)
...
### Community 122 - "register_admin_gui"
Cohesion: 0.14
...
### Community 125 - "model_router.py"
Cohesion: 0.09
Nodes (21): _build_builtin_model_map(), _default_model(), _nvidia_key_present(), Dynamic model router. Central routing logic for all chat and agent requests.…, Build the built-in alias table — Nvidia NIM models when key is set, local…, Resolve the default executor model via the catalog (UNIT 7). Was a hardcoded…, best_model_for(), best_vision_model() (+13 more)

### Community 126 - "CompanyGraphStore"
Cohesion: 0.03
Nodes (48): Added, C. Defer company persistence to a final "Confirm" step + relocate the email gate, Added, buildGraphElements(), Company, The core company entity - root of the Company Graph., cost_for_tokens(), Return the USD cost for (prompt_tokens, completion_tokens) on *model*. Returns… (+40 more)
...
Cohesion: 0.04
Nodes (53): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentPlan, AgentRunRequest, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names() (+45 more)

### Community 131 - "Command"
Cohesion: 0.07
Nodes (17): Command, CommandDispatcher, Parse and execute a slash command from raw text. Args: text: Raw command text,…, Return all enabled commands in a given category., Return all registered commands., Number of registered commands., A single slash command with metadata, handler, and lifecycle hooks., Execute the command, running pre/post hooks if configured. (+9 more)

### Community 132 - "register_webui"
Cohesion: 0.05
Nodes (51): 1. Render (production backend + worker), 2. Cloudflare Worker (frontend), 3. Local development machines, 4. GitHub secrets, 5. MongoDB collections, Post-Merge Environment Cleanup Guide, Post-merge verification checklist, Rollback (+43 more)

### Community 133 - "test_spec_store.py"
Cohesion: 0.07
Nodes (37): build_spec_router(), approve_spec(), _decide(), get_spec_artifact(), list_spec_artifacts(), reject_spec(), Any, APIRouter (+29 more)
...
### Community 136 - "test_agent_runner.py"
Cohesion: 0.04
...
### Community 137 - "._run_phase_with_timeout"
Cohesion: 0.17
...
Cohesion: 0.08
Nodes (20): autonomy_status(), Public autonomy readiness probe — no authentication required. A live deploy…, Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap(), ensure_self_company(), Idempotently register the platform as its own company and kick off the agency.…, self_bootstrap_enabled(), Tests for the self-onboarding bootstrap. The platform should register itself as… (+12 more)

### Community 148 - "RenderFinding"
Cohesion: 0.08
Nodes (24): _note_recurrence(), _parse_timestamp(), datetime, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Run one full scan and return every finding, filed or not. Never raises: a…, Scan, then file everything that is past its cooldown. The cooldown is claimed…, Map service ID → human name, degrading to the ID when unavailable., Flag a failed latest deploy, or one that has been building too long. (+16 more)

### Community 149 - "HarnessRegistry"
Cohesion: 0.17
...
### Community 150 - "_resolve_jwt_secret"
Cohesion: 0.38
...
### Community 151 - "test_kimi_bridge_server.py"
Cohesion: 0.07
...
### Community 156 - ".update_intelligence"
Cohesion: 0.08
...
### Community 160 - "test_self_heal_v2.py"
Cohesion: 0.05
Nodes (71): _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog(), _heal_stuck_tasks(), _heal_telegram(), _heal_timestamps(), _purge_days() (+63 more)
...
### Community 162 - "_step"
Cohesion: 0.07
Nodes (17): parametrize, `--missing-ok` is correct for an unplanned issue and wrong otherwise., `continue-on-error: true` means a crash must be its own state., Waiting is not reviewing., They have their own handler step; excluding them would strand them., Run the actual jq, rather than reading it. Reading the expression proves a…, An all-paperwork backlog is an idle tick, not a unit of work., Shell redirection pasted into Python, raising behind `|| true`. (+9 more)
...
### Community 164 - "AuditLog"
Cohesion: 0.04
Nodes (41): AuditEvent, AuditLog, _luhn_ok(), Any, True when ``digits`` (bare, no separators) satisfies the Luhn checksum., Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Recursively strip secrets from an arbitrary argument structure. Fails…, Redact secret-shaped substrings, then truncate. (+33 more)

### Community 165 - "FeatureMaturity"
Cohesion: 0.08
Nodes (21): __init__.py — Feature flag/matrix package., FeatureMaturity, get_feature_matrix(), Enum, str, Feature maturity classification., Return the global FeatureMatrix singleton., Reset the singleton (useful for testing). (+13 more)

### Community 166 - "telegram_bot.py"
Cohesion: 0.03
Nodes (137): Normalise a raw planner JSON response into a consistent plan dict. Handles: -…, Fixed, Current Sprint Tasks, Fixed, build_body(), _is_retired(), Render the tracking-issue body — retired model ids only., Activate a company's AI agency after onboarding completes. Steps: 1. Assign… (+129 more)

### Community 167 - "._log_event"
Cohesion: 0.08
...
### Community 168 - "parse_event_stream"
Cohesion: 0.10
...
### Community 169 - "LLMRouter"
Cohesion: 0.06
Nodes (36): Fixed, Fixed, payload_key(), Exact-match cache key over the fields that change the answer. Routing…, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, run() (+28 more)

### Community 170 - "useSafeData"
Cohesion: 0.06
Nodes (33): changeUserRole(), createApiKey(), createSchedule(), deleteApiKey(), pauseSchedule(), resumeSchedule(), setUserOnboarding(), triggerSchedule() (+25 more)
...
### Community 173 - "require_admin"
Cohesion: 0.06
Nodes (45): activate_instance(), ActivateRequest, ActivateResponse, activation_audit_log(), _append_audit(), AuditLogEntry, change_user_role(), get_onboarding_settings() (+37 more)
...
### Community 175 - "delegate_seo_findings"
Cohesion: 0.08
Nodes (31): build_seo_roadmap(), delegate_seo_findings(), plan_seo_sprint(), BaseModel, post, Options for turning an audit's delegation plan into real agent tasks., Create real agent tasks from the audit's delegation plan. Each work package…, Options for building a Now/Next/Later roadmap from SEO findings. (+23 more)

### Community 176 - "KeyStore"
Cohesion: 0.05
Nodes (46): Acceptance Checks, `admin_auth.py` checklist, `agent/tools.py` checklist, Escalation, Instructions, `key_store.py` checklist, `proxy.py` auth middleware checklist, Risky Modules in This Repo (+38 more)

### Community 177 - "AdminAuthManager"
Cohesion: 0.12
Nodes (6): Admin Authentication, AdminAuthManager, AdminSession, AdminSessionStore, _is_truthy(), WindowsCredentialAuthenticator

### Community 178 - "test_mcp_governance.py"
Cohesion: 0.07
Nodes (44): Replace the process-wide store. Tests only., reset_approval_store(), Replace the process-wide audit log. Tests only., reset_audit_log(), Replace the process-wide engine. Tests only., reset_policy_engine(), _enforce(), isolated_governance() (+36 more)

### Community 179 - "test_schedule_growth_invariants.py"
Cohesion: 0.05
...
### Community 184 - "seo_api.py"
Cohesion: 0.04
Nodes (62): export_seo_audit(), list_seo_audits(), get, SEO / GEO / AIO Audit API Router Endpoints for the world-class SEO audit engine…, Return the full SEO/GEO/AIO check catalog. Static metadata only, but gated…, List stored audits for this company (most recent first)., Export a stored audit. - ``csv`` aggregated findings, Screaming Frog…, seo_check_catalog() (+54 more)

### Community 185 - "looks_like_secret_file"
Cohesion: 0.07
Nodes (52): onboarding_gate_enabled(), Async read of the gate default straight from the DB., Removed, Removed, Removed, Removed, N3. Real CI-failure autofix — close the "Agency: cannot fix tests" loop (issue #398) ✅  (size: L, risk: medium), _extract_mentioned_paths() (+44 more)
...
Cohesion: 0.04
Nodes (35): getBrainProviders(), setBrainProviderEnabled(), syncProviderToRender(), OBSERVE_STATUS, ProviderHealthToggleCard(), AuditTable(), BACKEND_META, DECISION_COLOR (+27 more)
...
Cohesion: 0.12
Nodes (39): _governance_check(), Evaluate a runtime dispatch against policy; audit it either way. Returns…, _decision(), _engine(), Any, Governance on runtime dispatch — the last coverage gap.…, `route_and_execute` callers handle failure; they do not expect a new exception…, So callers can branch without string-matching the output. Regression: an… (+31 more)
...
Cohesion: 0.07
Nodes (27): PreflightIssue, PreflightReport, BaseModel, check_all(), clean_store(), _fake_user(), Tests for direct-chat evolution: intent routing, sticky context, humanized…, Regression: AgentRunner must not be called with removed kwargs. Previous… (+19 more)

### Community 202 - "WorkspaceTools"
Cohesion: 0.02
Nodes (86): Register the built-in agent tools that are always available., _register_builtin_tools(), Build the complete enrichment block (tools + skills). Returns empty string when…, Inject available tools + skills catalog into the system message.…, note_phase_end(), note_phase_start(), Record that *phase* began; returns the token that ends it. Called from…, Close the invocation identified by *token*. Never raises. (+78 more)
...
### Community 204 - "_build_anthropic_response"
Cohesion: 0.09
...
Cohesion: 0.12
Nodes (21): _mem_key(), _FakeMemory, asyncio, Tests for the intelligent C-suite advisory layer (agent/executive_advisory.py).…, Return an async llm that yields queued replies and records calls., _recording_llm(), test_advise_consults_selected_execs_and_synthesizes(), test_advise_is_fail_soft_when_a_voice_errors() (+13 more)

### Community 209 - "test_operational_incidents.py"
Cohesion: 0.02
Nodes (104): agent, _diagnose_and_file(), _file_incident(), _format_incident(), _iso_from_monotonic(), normalise(), _now(), open_phase_report() (+96 more)

### Community 210 - "_emit_langfuse_http_sync"
Cohesion: 0.13
...
Cohesion: 0.04
Nodes (31): Any, Reconstruct a ScheduledJob from its as_dict() output., ScheduledJob, _MemCollection, _MemCursor, _MemDB, _MemDeleteResult, Any (+23 more)

### Community 213 - "DetectedSystem"
Cohesion: 0.04
Nodes (59): P1 — Close the remaining product gaps, Task 6 — Deeper website/repo scanning (more than "2 systems"), Task 7 — Knowledge base auto-reflect (ECC / Obsidian / graphify-style), Task 8 — Vector retrieval for the knowledge base (self-learn), DetectedSystem, Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection. (+51 more)

### Community 214 - "test_unit7_catalog_propagation.py"
Cohesion: 0.07
Nodes (29): Resolve the NVIDIA default model via the catalog (UNIT 7). Was hardcoded to…, _resolve_nvidia_default_model(), _catalog_preset(), tests/test_unit7_catalog_propagation.py — UNIT 7 regression tests. Verifies…, ``_get_defaults()`` must consult the catalog first; the hardcoded…, ``_get_defaults()`` returns the catalog-derived defaults (not the hardcoded…, ``jcode.py`` must NOT have the stale hardcoded ``meta/llama-3.3-70b-instruct``…, ``opencode.py`` must NOT have the stale hardcoded model id inline. (+21 more)
...
### Community 217 - "PortfolioManager"
Cohesion: 0.06
...
### Community 218 - "Autonomy Platform — Remaining Implementation Plan"
Cohesion: 0.08
...
Cohesion: 0.03
Nodes (84): gc, AgentScheduler, _now(), Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Register a new job. Returns the created :class:`ScheduledJob`.…, Fire a job immediately (webhook / manual trigger)., Remove a job. Returns *True* if it existed., Return the running event loop, or ``None`` when called synchronously. Used so… (+76 more)

### Community 223 - "activation.py"
Cohesion: 0.07
...
Cohesion: 0.13
Nodes (21): cross_verify(), True if any path matches the repo's risky-module trigger list., Have an independent agent re-check a completed task's changed files. Returns…, touches_risky_module(), RunnerFactory, _agent_result(), Tests for agent/verification_strategies.py (cross_verify + race).…, Regression: _score_attempt used to branch on a top-level "status" key that… (+13 more)
...
### Community 231 - "v4_quick_notes_submit"
Cohesion: 0.11
...
### Community 232 - "analyze_quantitative"
Cohesion: 0.14
Nodes (10): analyze_quantitative(), QuantAnalysis, The output of the **Quant** capability., The output of the **Synthesize** capability. Combines one QualAnalysis + one…, Compute descriptive statistics for a numeric series. Args: source: Where the…, Combine qualitative + quantitative findings into a decision-ready brief. If…, ResearchBrief, synthesize_research() (+2 more)

### Community 233 - "Kept Rules — the 44 that survive the audit"
Cohesion: 0.05
...
### Community 234 - "sam_livekit_worker.py"
Cohesion: 0.16
Nodes (19): livekit_agents, LiveKitConfig, Resolved LiveKit + speech-provider configuration., _build_llm(), _build_stt(), _build_tts(), entrypoint(), main() (+11 more)

### Community 235 - "test_crispy_burn_in.py"
Cohesion: 0.03
Nodes (53): importlib, tests/test_agency_fix.py — N3 acceptance tests for scripts/agency_fix.py. The…, An edit that produces a syntactically-broken Python file must be rejected —…, An edit that truncates a real code file to a trivial body must be rejected —…, With no issue linked, decline is just an exit-code signal — no API call., When an issue is linked but no GH_PAT/GH_TOKEN is set, the decline fails loudly…, When the API call itself fails (network error), decline_cleanly returns False…, End-to-end N3 acceptance: a deliberately-broken fixture test is fixed by a… (+45 more)

### Community 236 - "WorkflowEngine"
Cohesion: 0.08
...
### Community 237 - "test_direct_chat_async.py"
Cohesion: 0.08
Nodes (22): make_isolated_workspace(), Path, Create an isolated workspace directory under *root*. This is the legacy path…, _workspace_component(), _fake_user(), _FakeChatResult, _FakeResponse, asyncio (+14 more)
...
Cohesion: 0.08
Nodes (41): Layer 2 — Chat Handlers (`chat_handlers.py`, 710 lines), _apply_chat_defaults(), _apply_reasoning_budget(), _emit_safely(), _extract_exact_output(), _filter_fragment(), _filter_openai_sse_line(), handle_ollama_native_chat() (+33 more)
...
### Community 246 - "TestNormalizeAnthropicOutputFormat"
Cohesion: 0.16
...
### Community 259 - "SetupWizardPage.js"
Cohesion: 0.05
...
Cohesion: 0.09
Nodes (24): Fixed, Fixed, DetailsStep(), _apply_update(), _Collection, _DeleteResult, _InsertResult, _match() (+16 more)
...
### Community 268 - "test_governance_api.py"
Cohesion: 0.07
...
### Community 269 - "LLMResponse"
Cohesion: 0.04
Nodes (59): Modules, Writing a custom adapter, packages/llm — the multi-provider LLM routing layer. Every LLM call in this…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error(), LLMProvider, OpenAICompatible, ABC (+51 more)

### Community 270 - "_is_dns_failure"
Cohesion: 0.14
...
### Community 271 - "reset_store"
Cohesion: 0.04
Nodes (52): Reset the store singleton (used in tests). Also resets the motor client…, reset_store(), Already completed (pre-migration fixes), Current Status, Inventory of suspected dead code, Migration Safety Checklist, Phase 1: Foundation (Weeks 1-2), Phase 2: Provider Abstraction (Weeks 3-4) (+44 more)
...
### Community 274 - ".session_id"
Cohesion: 0.06
Nodes (38): Changed, Changed, 1.1 Agent Identity, classify(), evaluate_call(), guard_tool_call(), GuardResult, _host_of() (+30 more)
...
### Community 282 - "test_rate_limiter.py"
Cohesion: 0.04
...
Cohesion: 0.07
Nodes (46): ProviderHealth, Enum, Reset the singleton (for tests)., Drop the cached Mongo client, backoff, and read caches (tests)., Circuit-breaker state for a provider., reset_failover_manager(), reset_kv_state(), _clean_env() (+38 more)
...
Cohesion: 0.05
Nodes (53): get_enrichment(), Return the enrichment instance for a workspace. Keyed by workspace root rather…, build_block(), _flag(), _int_env(), _known_entry_texts(), _one_line(), propose_entries() (+45 more)
...
Cohesion: 0.06
Nodes (49): Changing these rules, How the gate behaves, Quick-Note Context Rulebook, R10 — Use the repository's real identity **[gate]**, R11 — Name a real integration point **[gate]**, R12 — Mark epistemic status at the claim **[review]**, R1 — Ground the plan in the source before planning anything **[gate]**, R2 — Say what the artifact actually is **[gate]** (+41 more)
...
### Community 297 - "portfolio_api.py"
Cohesion: 0.08
...
### Community 302 - "AppShell.jsx"
Cohesion: 0.06
Nodes (24): `activation_api.py`, Backend changes, `backend/company_api.py`, `db/sqlite_store.py`, Docs / changelog, Frontend changes, Goal, Implementation Plan — Onboarding-Gate Admin Setting + Ephemeral Companies (+16 more)
...
### Community 307 - "SQLiteStore"
Cohesion: 0.03
Nodes (70): Create a new company in SQLite., Get a company by ID from SQLite., Update a company in SQLite., Delete a company and all associated data from SQLite., List companies from SQLite., Count total companies in SQLite., Reconstruct a Website from a SQLite row, preferring the full JSON blob (which…, Create a new website in SQLite. The full model is stored in ``data`` so scan… (+62 more)
...
### Community 310 - "Context: Agentic Agile + Portfolio Management"
Cohesion: 0.11
...
### Community 311 - "lifespan"
Cohesion: 0.04
Nodes (54): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled_cached(), Any (+46 more)
...
Cohesion: 0.04
Nodes (42): _agent_provider_failure_response(), _append_agent_session_message(), _build_auto_skill_guidance(), _run_agent_job(), _ensure_tasks_source_id_unique_index(), _build(), _is_index_options_conflict(), _mask_observations() (+34 more)
...
### Community 318 - "_extract_pytest_failure"
Cohesion: 0.42
...
Cohesion: 0.09
Nodes (38): Added, Fixed, Added, Fixed, _auth_headers(), _chat_targets(), _dump_matching(), _kind() (+30 more)

### Community 320 - "Fixed"
Cohesion: 0.05
Nodes (49): Fixed, A failed drive does not abandon the goal, The ledger, Fixed, _run_baseline_pytest(), _is_billing_refusal(), True when a 4xx body says the account is out of credit or quota. Providers…, Sleep if remaining quota for *provider_id* is critically low. Returns the… (+41 more)
...
### Community 331 - "require_power_user"
Cohesion: 0.13
...
Cohesion: 0.03
Nodes (112): _actor(), build_platform_controls_router(), _admin(), list_controls(), reset_control(), update_controls(), ControlUpdateBody, APIRouter (+104 more)
...
### Community 334 - "session_retro.py"
Cohesion: 0.13
...
Cohesion: 0.08
Nodes (41): Changed, Changed, AdminOnboardingPanel(), ActivationPanel(), _content_block_to_text(), Convert a single Anthropic content block to a plain text string., is_anthropic_model(), True when *model* names a paid Anthropic/Bedrock-Claude model. Covers native… (+33 more)
...
Cohesion: 0.05
Nodes (63): DirectChatDoctor, Return True if the PR exists and is open or merged; False if 404., verify_pr_exists(), _append_transition(), classify_domain(), BaseModel, Enum, str (+55 more)
...
### Community 341 - "_is_admin"
Cohesion: 0.05
Nodes (43): _is_admin(), Check if a user has admin role. Works for both social_auth users (role in…, clear_cost_attribution(), clear_response_cache(), get_brain_policy_route(), get_catalog_models_route(), patch_brain_policy_route(), patch (+35 more)

### Community 342 - "SeoAuditReport"
Cohesion: 0.10
Nodes (35): _expire_stale_pending_report(), get_seo_audit(), Fetch a complete stored audit report. Side-effect: an audit whose stub is still…, Auto-fail a pending SEO audit stub that is older than…, html, io, Complete result of one audit run., SeoAuditReport (+27 more)

### Community 343 - "Screens"
Cohesion: 0.11
...
Cohesion: 0.12
Nodes (18): _cached(), get_activity(), legacy_scheduler_list(), _produce_scheduler_jobs(), Single-flight TTL cache. Concurrent callers wait for the first producer., _clear_cache(), _CollWithEstimate, _CollWithoutEstimate (+10 more)

### Community 354 - "AnthropicProvider"
Cohesion: 0.11
...
### Community 355 - "test_phase5_doctor.py"
Cohesion: 0.11
...
### Community 356 - "_runner"
Cohesion: 0.08
Nodes (34): _drive(), governance_on(), _observations(), Any, MonkeyPatch, Path, Governance must not silently swallow the calls it permits., Golden Rule: routing through the seam changes nothing with the gate off. (+26 more)
...
Cohesion: 0.11
Nodes (10): CircuitState, Return True if the runtime is available (not circuit-open)., Return health snapshots for all known runtimes., Force an immediate health check of all runtimes and return results., Attempt to start a dead runtime subprocess before re-probing. Uses the local…, Reduce probe frequency for runtimes that have never come online., Async health polling service for all registered runtimes., Start the background polling loop with an immediate initial check. (+2 more)
...
### Community 361 - "CompanyAgencyService"
Cohesion: 0.10
Nodes (17): (4): Agent provisioning "loading forever" — blocking subprocess in async path, CompanyAgencyService, _is_runtime_available_sync(), _pick_available_runtime(), Any, SpecialistFamily, Orchestrates specialist activation, runtime startup, and 24x7 scheduling for a…, Return the best available runtime for a specialist family. Checks available… (+9 more)
...
### Community 365 - "ExecutiveAdvisory"
Cohesion: 0.13
Nodes (15): _compose_context(), ExecOpinion, ExecutiveAdvisory, _format_company_context(), Grounding, Any, Evidence gathered for a question before the executives answer., One executive's answer to the question. (+7 more)
...
### Community 369 - "test_llm_router_queue_cache.py"
Cohesion: 0.08
Nodes (30): CacheManager, cosine_similarity(), Any, Cosine similarity between two vectors, 0.0 when either is degenerate., Owns every cache layer and the policy for what may enter them., Whether a response to ``request`` may be stored. Deliberately conservative — a…, Nearest cached response above the similarity threshold. A linear scan over at…, CacheConfig (+22 more)
...
Cohesion: 0.06
Nodes (25): Unique identifier (e.g. 'nvidia', 'cerebras')., provider_id_of(), Any, Extract a provider id from a ProviderConfig dataclass or a plain dict., In-process traffic distribution and budget accounting for providers., Return *providers* reordered according to the active strategy. ``group_key``…, Reorder one interchangeable group of providers., Ascending sort by *score* with a random tie-break. The tie-break is the point:… (+17 more)
...
Cohesion: 0.10
Nodes (26): Run an audit from synchronous code, loop-safe. Used by the skill-bindings…, run_audit_sync(), _run(), _execute_skill_impl(), _get_portfolio_manager(), Any, Live Graphify executor — queries the codebase knowledge graph. Order of…, Live council reviewer — deterministic, rules-based multi-perspective review… (+18 more)

### Community 378 - "run_regression"
Cohesion: 0.09
...
### Community 380 - "pytest"
Cohesion: 0.01
Nodes (154): fastapi_testclient, jwt, pytest, Every workflow that runs an agent script must carry the whole brain chain.…, _load_agent_runtime_module(), test_wrapper_exposes_hermes_task_endpoints(), fake_chat_with_ollama(), test_wrapper_exposes_opencode_run_endpoint() (+146 more)
...
Cohesion: 0.12
Nodes (19): AdviceResult, The full advisory response: per-executive opinions plus the synthesis., Drop the singleton (test helper)., reset_executive_advisory(), _make_app(), FastAPI, tests/test_executive_advisory_api.py — auth and behaviour for…, Install an advisory whose LLM is a canned reply — no network. (+11 more)

### Community 383 - "build_tool_prompt"
Cohesion: 0.08
Nodes (32): build_compaction_prompt(), build_execution_prompt(), build_planning_prompt(), build_tool_prompt(), build_verification_prompt(), Any, prompts.py — Prompt builders for planner, executor, and verifier roles., Activation (+24 more)
...
### Community 386 - "test_user_research_skill.py"
Cohesion: 0.10
Nodes (16): _classify_sentiment(), _extract_keywords(), plan_research(), Any, field_validator, Compute the minimum sample size for a proportion estimate. Uses the standard…, Tokenise text into (word, frequency) pairs, dropping stopwords., Tiny rule-based sentiment classifier. No LLM, deterministic, fast. Real… (+8 more)

### Community 387 - "ApprovalStore"
Cohesion: 0.08
...
### Community 391 - "BrainFailoverManager"
Cohesion: 0.09
Nodes (19): Reset every circuit breaker when they have all tripped, and re-select. Without…, _recover_all_unhealthy(), BrainFailoverManager, ProviderInfo, Any, Return the next healthy provider, excluding any in the exclude set. Ordering:…, Record a successful call — resets the circuit breaker., Permit one probe call without claiming the provider succeeded. This is the… (+11 more)

### Community 392 - "repo"
Cohesion: 0.11
Nodes (19): Tool Parameters, Tool Parameters, 1. Create the Telegram bot, 1. Hit the diagnostic endpoint, 2. Deploy, 2. Single-poller guard (409 conflict), 3. Set environment variables (Render dashboard → the worker → Environment), 3. Webhook set → long-poll blocked (+11 more)
...
### Community 394 - "provider_max_rpm"
Cohesion: 0.29
...
### Community 395 - "model_discovery.py"
Cohesion: 0.11
Nodes (22): Return the discovered model list for *provider_id*, or ``[]`` if unknown., _served_models(), cached_models(), dead_models(), discover_models(), _fresh_entry(), _models_url(), _parse_ids() (+14 more)

### Community 396 - "extract_refusal"
Cohesion: 0.27
...
### Community 399 - "TestRenderMCPClient"
Cohesion: 0.10
...
### Community 400 - "executive_advisory_api.py"
Cohesion: 0.16
...
### Community 403 - "[Unreleased]"
Cohesion: 0.04
Nodes (59): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, Register the interactive-browser capability (agent/browser.py). Unlike…, _register_browser_tools(), _browse_page_tool(), Adding a new tool, CLAUDE.md — agent/, Security surface (+51 more)
...
### Community 405 - "LLM Router — troubleshooting"
Cohesion: 0.09
...
### Community 409 - "_status_snapshot"
Cohesion: 0.08
Nodes (43): ArgumentParser, build_parser(), cmd_autostart_install(), cmd_status(), cmd_supervise(), cmd_wait(), Namespace, Block until download completes + colibri answers /v1/models. (+35 more)
...
### Community 420 - "build_tool_calling_router"
Cohesion: 0.24
...
### Community 425 - "_test_e2e_telegram_approval"
Cohesion: 0.11
Nodes (20): _approve_execution_via_rest(), _delete_task(), _extract_admin_token(), _login_admin(), _looks_like_admin_token(), _open_dashboard(), _poll_task_execution_approved(), Client (+12 more)
...
Cohesion: 0.13
Nodes (22): _get_admin_email(), _get_admin_name(), _get_admin_secret(), login(), LoginRequest, LoginResponse, BaseModel, V3 API authentication endpoints (/api/auth/*). (+14 more)
...
### Community 432 - "Fixed"
Cohesion: 0.11
Nodes (17): FailureCategory, Called when a CI workflow fails., E2: Classify a failure from its description text. Order matters:…, E2: Return a corrective hint for each failure category., Classified failure types for targeted self-healing (E2)., ma(), Fixed, Session Log (+9 more)
...
### Community 434 - "RepoConnection"
Cohesion: 0.14
Nodes (25): Phase 0 — `RepoConnection` plumbing + delivery-policy detection, 6. Integration gaps to wire (follow-up implementation), Loop 3 — Agentic SDLC (the golden path), Acceptance criteria, Design, Objective, Part E — G5: `RepoConnection` + `DeliveryPolicy` (GitHub-only scope), Tech stack / touch points (+17 more)
...
### Community 440 - "self_heal_brain_and_unblock_tasks"
Cohesion: 0.08
Nodes (26): Called by Cloudflare Cron every minute. Protected by CRON_SECRET header., Manually trigger brain self-healing + unblock tasks. Admin-only endpoint. Runs…, scheduler_self_heal(), scheduler_tick(), Any, Background tick that runs self_heal_brain_and_unblock_tasks(). Called from the…, One-shot self-healing pass. 1. Checks if the active brain provider is in a…, self_heal_brain_and_unblock_tasks() (+18 more)

### Community 441 - "analyze"
Cohesion: 0.07
Nodes (23): Analysis, analyze(), _classify(), _failure_table(), main(), Return the exhaustion :class:`Analysis` for a run's captured *output*., The verdict plus the human-readable justification behind it., Classify *text* with the repo's own classifier. Imported lazily:… (+15 more)

### Community 442 - "ProviderHealth"
Cohesion: 0.13
Nodes (8): _Outcome, ProviderHealth, Any, Claim a half-open probe slot. Returns False when none are free., Hand a half-open probe permit back when nothing was actually tried. A candidate…, Record a failed attempt and trip the breaker if thresholds are crossed.…, Restore a provider immediately (admin action)., Rolling health for one provider.
...
### Community 444 - "TaskDispatcher"
Cohesion: 0.12
Nodes (16): Re-queue BLOCKED tasks that have cooled down and are ready for retry., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, _make_task(), asyncio, A BLOCKED task with a recent ISO 8601 updated_at should be skipped (cooldown…, Build a task with a specific updated_at value (bypassing the validator). (+8 more)
...
### Community 447 - "context_plan_gate.py"
Cohesion: 0.13
Nodes (19): evaluate(), evaluate_path(), main(), _parse_args(), PlanDecision, Namespace, Path, Did the planner actually retrieve the source it reasoned about? (+11 more)
...
### Community 456 - "DecisionsStore"
Cohesion: 0.30
Nodes (3): DecisionsStore, Any, Connection

### Community 457 - "_build_execution_request"
Cohesion: 0.27
Nodes (12): _build_execution_request(), Any, Build a minimal ``ExecutionRequest`` for plain-text → orchestrator.execute.…, admin_user(), _auto_approve(), non_admin_user(), Auto-approve policy for Telegram plain-text → orchestrator routing. Routine…, test_admin_execute_after_approval_gates() (+4 more)

### Community 458 - "E2BAdapter"
Cohesion: 0.04
Nodes (61): ctypes_util, E2BAdapter, Any, Declare ``E2B_API_KEY`` as a required env dependency. The base ``preflight``…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, Runtime adapter that executes tasks inside an E2B sandbox. Activation:…, services, _load_malloc_trim() (+53 more)
...
### Community 463 - "TestCapabilitiesAreNotClaimedWithoutEvidence"
Cohesion: 0.24
...
### Community 466 - "test_task_service_failed_comment.py"
Cohesion: 0.18
...
### Community 467 - "get_tool_registry"
Cohesion: 0.11
...
Cohesion: 0.15
Nodes (7): HarnessEnrichment, Any, Build a compact catalog of available runtime skills. Discovers from…, Standing instructions from the Continual Harness spec. Deliberately uncached:…, Inject enrichment blocks into a system prompt string. Appends blocks after the…, Auto-discovers skills and tools for agent prompt injection. Usage:: enrichment…, Build a compact, token-efficient catalog of available agent tools. Discovers…

### Community 470 - "Bulkhead"
Cohesion: 0.10
...
### Community 475 - "Added"
Cohesion: 0.10
Nodes (26): Ask the C-suite advisory layer a business question. This is the CEO's bridge…, Perform MCP handshake. Optional — tools/call works without it., _egress_policy_reason(), Return why governance policy blocks *host*, or None. Runs strictly *after* the…, Added, Fixed, Bug Log, Next Action (+18 more)
...
### Community 478 - "get_ceo_dispatcher"
Cohesion: 0.11
...
### Community 484 - "AgentJobSnapshot"
Cohesion: 0.22
...
### Community 494 - "run_freebuff_bot.py"
Cohesion: 0.47
...
### Community 499 - "is_model_available"
Cohesion: 0.12
...
### Community 500 - "Fixed"
Cohesion: 0.06
Nodes (42): True if browser automation is enabled and Playwright is importable., Atomically delete a run-once row only while it is still unfired. Closes the…, Fixed, Fixed, _fetch_models_json(), _is_chat_model(), live_model_ids(), _rank_key() (+34 more)
...
### Community 507 - "4. Troubleshooting"
Cohesion: 0.10
...
### Community 508 - "Agent Runtime Setup"
Cohesion: 0.12
...
### Community 509 - "Agent Governance Guide"
Cohesion: 0.03
Nodes (78): 1. Executive summary, 2. Architecture review, 3. Risk assessment, 4. Security review, 5. What was implemented, 6. Explicitly not implemented, 7. Remaining recommendations, 8. Future enhancements (+70 more)

### Community 510 - "TestTheGatewayRegistryOffersOnlyLiveModels"
Cohesion: 0.11
Nodes (14): _hardcoded_candidates(), parametrize, A frozen record of the divergence found on 2026-08-28. Reconciling these eleven…, These two are the ones this work reconciled, and each was reconciled because…, A shrinking divergence is progress, but the list must be trimmed to match or it…, A sixth model source, found on 2026-08-28 while verifying a fix.…, The platform default must survive ``require_tools`` filtering. Undeclared…, ``PROVIDER_CANDIDATES`` as written in the module, before YAML overrides. Read… (+6 more)
...
### Community 514 - "test_ai_insights.py"
Cohesion: 0.18
...
### Community 518 - "test_feature_matrix.py"
Cohesion: 0.12
...
Cohesion: 0.10
Nodes (23): UI-first — an API is not "done", Phase 2 — Per-surface assignment in the UI (the "one place"), createAgent(), createProvider(), deleteProvider(), getProviderPolicy(), provisionSpecialist(), updateCompany() (+15 more)

### Community 522 - "_redact_for_notification"
Cohesion: 0.11
...
### Community 528 - "autonomous_fix.py"
Cohesion: 0.17
Nodes (17): _decline(), _fetch_failure_context(), _list_target_prs(), main(), _post_comment(), _pr_head(), _prior_attempt_count(), process_pr() (+9 more)
...
### Community 534 - "OrchestratorQueue"
Cohesion: 0.17
...
### Community 541 - "generate_onboarding_questions"
Cohesion: 0.39
...
### Community 544 - "TestMCPToolsListCache"
Cohesion: 0.15
...
Cohesion: 0.03
Nodes (74): date, .github/scripts/provider_policy.py — Read the durable provider policy from the…, Reset the cached policy (test helper)., reset_cache(), livenim, Popen, build_prompt(), call_llm() (+66 more)
...
### Community 549 - "SeoAuditEngine"
Cohesion: 0.09
Nodes (19): field_validator, Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoSiteFindings, Crawls a site and produces a complete SeoAuditReport., Pick the fetch backend for this run (http / browser / auto)., Execute the full audit: crawl, page checks, site checks, scoring. (+11 more)

### Community 550 - "test_autonomy_status.py"
Cohesion: 0.11
...
### Community 552 - "AdvisoryMemory"
Cohesion: 0.20
Nodes (6): AdvisoryMemory, _keywords(), The C-suite's durable memory of past recommendations. A thin adapter over…, Distinct significant words (len ≥ 5), order-preserved, for LIKE recall., test_advisory_memory_is_fail_soft_on_broken_store(), test_advisory_memory_roundtrips_via_store()
...
Cohesion: 0.10
Nodes (9): Convenience: the canonical text response, whether success or failure., Path, Fixed, Fixed, MonkeyPatch, Path, Tests for scripts/fabric_cli.py and the fabric-patterns pattern engine., test_new_scaffolds_pattern() (+1 more)
...
### Community 558 - "RepowiseIntelligence"
Cohesion: 0.08
...
### Community 560 - "handle_workflow_ide_chat"
Cohesion: 0.15
...
### Community 561 - "error_interceptor.py"
Cohesion: 0.13
...
### Community 563 - "get_operational_incident_tracker"
Cohesion: 0.33
Nodes (6): Best-effort: hand an operational failure to the incident tracker. Import is…, _record_operational(), get_operational_incident_tracker(), Return the shared tracker, creating it on first use., Module-level entry point used by ``agent/log_monitor.py``., record_operational_error()
...
### Community 569 - "test_voice_pipeline.py"
Cohesion: 0.14
...
### Community 573 - "analyze_qualitative"
Cohesion: 0.22
...
### Community 574 - "os"
Cohesion: 0.01
Nodes (356): activation_api.py — FastAPI routes for instance activation & per-user…, Browser admin UI for login, service control, key management, and diagnostics., ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered) Runs the…, agent/background.py — Background Agent An always-on worker thread that…, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, Cached LLM Client wrapper. Drop-in wrapper around any LLM API call that…, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata… (+348 more)
...
### Community 579 - "TestCreateCompanyValidation"
Cohesion: 0.33
...
### Community 580 - "DeterministicEngine"
Cohesion: 0.21
...
### Community 583 - "KnowledgeScreen.jsx"
Cohesion: 0.14
...
### Community 585 - "sync_readme_gallery.py"
Cohesion: 0.20
...
### Community 593 - "looks_blocked"
Cohesion: 0.17
Nodes (6): BrowserFetcher, looks_blocked(), Browser backend: renders pages with a real Chromium via Playwright. This is the…, Heuristic: does this result look like a bot-block rather than content?, _result(), TestLooksBlocked
...
### Community 595 - "test_agent_api.py"
Cohesion: 0.16
Nodes (8): test_agent_run_returns_structured_failure(), test_agent_session_endpoints_require_and_return_state(), fake_run(), test_agent_session_run_redacts_internal_exception_details(), test_agent_session_run_reuses_bearer_key_for_same_origin_provider(), test_openai_chat_completions_exact_output_short_circuits(), fake_verify(), test_openai_chat_completions_exact_output_streams()
...
### Community 598 - "test_kimi_bridge_provider.py"
Cohesion: 0.20
...
### Community 600 - "sys"
Cohesion: 0.01
Nodes (141): detect_secrets(), main(), Recover CHANGELOG.md from a Git merge conflict in its [Unreleased] block. Pre-…, _extract_unreleased_body(), _insert(), main(), Insert the Maintenance changelog section at the end of the [Unreleased] block.…, Return (body_start, body_end_exclusive, body) for the [Unreleased] block. (+133 more)
...
### Community 602 - "test_task_pre_execution_gate.py"
Cohesion: 0.24
...
### Community 604 - "e2e/test_browser.py"
Cohesion: 0.21
...
### Community 606 - "GitHubPolicyProbe"
Cohesion: 0.19
...
### Community 612 - "TestAnthropicToolListCaching"
Cohesion: 0.34
...
### Community 620 - "test_social_login_oauth.py"
Cohesion: 0.24
...
### Community 624 - "TestHarnessAdapter"
Cohesion: 0.06
...
### Community 626 - "467 Brutal Audit — File-by-File Status"
Cohesion: 0.05
Nodes (37): Beta, Config Overrides, Disabled (demoted per issue #467 Section I), Enforcement, Experimental, Feature Maturity / Support Matrix, Maturity Tiers, Stable Core (+29 more)
...
### Community 635 - "test_setup_api.py"
Cohesion: 0.21
...
### Community 636 - "TestRenderDeclaresTheChain"
Cohesion: 0.18
...
### Community 638 - "verification_strategies.py"
Cohesion: 0.15
Nodes (13): Any, race(), agent/verification_strategies.py — opt-in parallel patterns for high-stakes…, Heuristic fallback score when the reward model is unavailable.…, Run *n* independent attempts at *instruction* concurrently; return the winner.…, _score_attempt(), _score_result(), get_reward_scorer() (+5 more)
...
### Community 641 - "ResearchOrchestrator"
Cohesion: 0.18
...
### Community 645 - "verify_service_token"
Cohesion: 0.14
...
### Community 660 - "test_new_features_e2e.py"
Cohesion: 0.26
...
### Community 661 - "TestEstimateTokensForMessages"
Cohesion: 0.15
...
### Community 663 - "Skill: user-research"
Cohesion: 0.18
...
### Community 664 - "Main proxy (`proxy.py`)"
Cohesion: 0.25
...
### Community 667 - "McpCard.jsx"
Cohesion: 0.14
Nodes (9): getRenderHealth(), getRenderOpsStatus(), runRenderOpsScan(), api, BTN, McpCard(), NOTE(), relTime() (+1 more)

### Community 668 - "yaml"
Cohesion: 0.04
Nodes (33): The representative task catalogue for the routing evaluation. Tasks live in…, _check_cross_catalogue(), _check_prefer_models(), _check_presets(), _llm_declared(), _load(), main(), Any (+25 more)
...
### Community 672 - "test_task_brain_preflight.py"
Cohesion: 0.21
...
Cohesion: 0.17
Nodes (15): _safe_allowlist(), validate_command(), AdminCommandBody, BrainPolicyUpdate, ProviderReorderBody, BaseModel, Body for POST /ui/api/route — preview which model auto-routing would pick., Drag-and-drop reorder payload — list of provider_ids in the desired top-to-… (+7 more)
...
### Community 676 - "test_agent_coordinate.py"
Cohesion: 0.23
...
### Community 677 - "test_p0_roadmap_c3_c4_c5.py"
Cohesion: 0.16
Nodes (10): get_context_window_manager(), Enum, How to truncate messages when over the context limit., Return the module-level ContextWindowManager singleton., Result of a context window truncation operation., TruncationResult, TruncationStrategy, Result of reconstructing a streamed response. (+2 more)

### Community 678 - "TestRuntimeControl"
Cohesion: 0.15
...
### Community 683 - "ApprovalCheckpoint"
Cohesion: 0.18
...
### Community 688 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
### Community 700 - "test_telegram_task_callback.py"
Cohesion: 0.17
...
### Community 704 - "Added"
Cohesion: 0.07
Nodes (30): Any, Force the next ``list_tools()`` call to fetch a fresh tool list from the…, Added, Added, A worked example, Adding capacity: multi-key rotation, Attribution, Configuration (+22 more)
...
### Community 707 - "_list_configured_provider_records"
Cohesion: 0.15
Nodes (14): _build_provider_router(), _builtin_provider_records(), _chat_provider_policy(), _fallback_local_provider_record(), _in_container(), _list_configured_provider_records(), _nvidia_nim_provider_record(), Return a minimal set of built-in provider records without touching MongoDB.… (+6 more)

### Community 708 - "LRUCache"
Cohesion: 0.15
...
### Community 709 - "generate_context.py"
Cohesion: 0.09
...
### Community 710 - "TestBrainConfigUpdates"
Cohesion: 0.22
...
### Community 712 - "flesch_reading_ease"
Cohesion: 0.22
...
### Community 713 - "test_swarm_concurrency.py"
Cohesion: 0.15
...
### Community 717 - "._chat_text"
Cohesion: 0.21
Nodes (10): Send chat messages and return the assistant's text output. If an…, _base_url(), emit_chat_observation(), _env_val(), get_langfuse_client(), _langfuse_enabled(), Record one generation in Langfuse (SDK first, then REST fallback). Args:…, Read env; strip whitespace and a single pair of surrounding quotes (common… (+2 more)

### Community 718 - "_timed_phase"
Cohesion: 0.18
...
### Community 720 - "TestAuthAndTaskCreation"
Cohesion: 0.17
...
### Community 723 - "LLM Router — configuration guide"
Cohesion: 0.14
...
### Community 737 - "Runbook: Auto-Resume After Cooldown / Interruption"
Cohesion: 0.14
...
### Community 738 - "SeoIssueReportRow"
Cohesion: 0.20
Nodes (8): Aggregated report row - Screaming Frog CSV compatible., SeoIssueReportRow, compute_pressure(), loss_share_from_pressure(), Map aggregate issue pressure to an at-risk revenue share via the diminishing-…, Group findings into agent-delegable work packages, one per category., Return (per-row issue pressure, total pressure) for the revenue model.…, TestRevenueModelHelpers
...
### Community 755 - "test_phase4_runtime_resilience.py"
Cohesion: 0.09
...
### Community 762 - "test_deploy_trigger_covers_image.py"
Cohesion: 0.21
...
### Community 765 - "chat_completions"
Cohesion: 0.20
...
### Community 777 - "knowledge_graph.py"
Cohesion: 0.18
...
### Community 785 - "admin_update_task_router.py"
Cohesion: 0.22
...
### Community 787 - "test_brain_config_api.py"
Cohesion: 0.09
...
### Community 788 - "_is_bedrock_model_id"
Cohesion: 0.27
...
### Community 793 - "._dispatch_tool"
Cohesion: 0.18
Nodes (11): Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Compose secret scoping — shipped, opt-in, Container hardening overlay, Known limitations at merge, Migration Notes, Optional hardening (operator decisions), Protect the policy file from agents, Rollback (+3 more)

### Community 794 - "ResearchAgent"
Cohesion: 0.29
...
### Community 796 - "TestDeprecationWarnings"
Cohesion: 0.17
...
### Community 797 - "TestAnthropicWorkspaceIdCapture"
Cohesion: 0.33
...
### Community 799 - "test_app_settings.py"
Cohesion: 0.21
...
### Community 806 - "TestTheSharedListFitsBothCallers"
Cohesion: 0.18
...
### Community 823 - "LOOP.md — The loops that run this agency"
Cohesion: 0.22
...
### Community 827 - "5. The five autonomous loops"
Cohesion: 0.40
...
### Community 828 - "TestGeminiCostTrackerEntries"
Cohesion: 0.26
...
### Community 835 - "allow_paid"
Cohesion: 0.03
Nodes (53): Example: Access your home PC models from any machine. Install: pip install…, 1. Mission & operating principles, 2. Brain policy (free cloud LLMs), 3. The Gate Matrix (core artifact), 7. Definition of "fully autonomous" — acceptance criteria, 8. Safety invariants (carried from `agent/CLAUDE.md`), 🟢 Autonomous — run, then notify-only, Autonomy Charter — Telegram-Gated Self-Running Agency (+45 more)

### Community 836 - "coordinate.py"
Cohesion: 0.24
...
### Community 838 - "test_spa_trailing_slash_redirect.py"
Cohesion: 0.06
Nodes (19): _NEEDS_CREDS, asyncio, ProviderRouter discovers Bedrock from env and completes a real chat call., Health check returns True when real credentials are loaded from env., Call Bedrock Converse API directly with boto3 — no proxy layer., Verify the configured model ID accepts a converse request without auth errors., test_bedrock_direct_boto3_ping(), test_bedrock_health_check_with_real_creds() (+11 more)

### Community 839 - "reset_tracker"
Cohesion: 0.24
...
### Community 840 - "._run_git_command"
Cohesion: 0.20
...
### Community 844 - "Plan: Onboarding Flow Bug Fixes + Wire AI-Tailored Questions"
Cohesion: 0.20
...
### Community 853 - "Part A — CodeRabbit review fixes for this PR (do first, small)"
Cohesion: 0.20
...
### Community 858 - "_push_down_where"
Cohesion: 0.18
...
### Community 862 - "TestMCPClientStructuredOutput"
Cohesion: 0.38
...
### Community 872 - "Competitor Analysis — Autonomous AI Agency"
Cohesion: 0.20
...
### Community 873 - "Separate hosted dashboard backend (`backend/server.py`)"
Cohesion: 0.20
...
### Community 875 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 881 - "_get_current_user"
Cohesion: 0.22
...
### Community 885 - "get_db"
Cohesion: 0.03
Nodes (101): auth_me(), auto_recommend_skills(), _check_storage_health(), cost_attribution_stats(), create_access_token(), discover_remote_skills(), doctor_health(), get_active_provider() (+93 more)
...
### Community 887 - "TestTheWorkflowIsSafeAndReadOnly"
Cohesion: 0.20
...
### Community 893 - "get_task_store"
Cohesion: 0.07
Nodes (40): get_agency(), set_agency(), _autonomy_bg_cycle(), autonomy_tick(), _deferred_startup_cleanup(), _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody (+32 more)
...
### Community 900 - "TestRevenuePortfolio"
Cohesion: 0.20
...
### Community 913 - "Universality: case-coverage matrix"
Cohesion: 0.08
Nodes (25): A. Connection & credentials, Autonomous SDLC Loop (Agency Core, repo-agnostic), B. Provider & host, C. Delivery / branch policy  *(detected — see DeliveryPolicy)*, D. CI / checks, Design principle: repo-agnostic, not GitHub-Actions-bound, Detect & respect each repo's delivery policy, E. Review automation & humans (+17 more)
...
### Community 922 - "TestPreCompiledPatterns"
Cohesion: 0.22
...
### Community 923 - "agile_sprints.py"
Cohesion: 0.31
Nodes (8): Enum, Agentic Agile — Sprint management with velocity tracking and burndown. Issue:…, Lifecycle status of a sprint., Status of a user story within a sprint., Qualitative health signal derived from sprint metrics., SprintHealth, SprintStatus, StoryStatus

### Community 924 - "record_usage_endpoint"
Cohesion: 0.22
...
### Community 925 - "LLM Router — architecture"
Cohesion: 0.22
...
### Community 930 - "list_specs"
Cohesion: 0.33
Nodes (4): list_specs(), Return the catalogue without probing. Cheap; used by tests and docs., Settings changes must be visible without reimporting the module., TestCatalogue

### Community 931 - "_latest_metric_value"
Cohesion: 0.25
Nodes (6): _latest_metric_value(), Any, Pull the most recent numeric sample out of a ``get_metrics`` payload. Render…, Snapshot for ``/api/render/ops/status`` and the dashboard. ``self_heal_ready``…, Extract (usage, limit) from a two-metric ``get_metrics`` payload. Render…, _split_memory()
...
### Community 934 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.22
...
### Community 946 - "test_backend_requirements_cover_runtime_imports.py"
Cohesion: 0.25
...
### Community 950 - "TestChatFallbackAndApproval"
Cohesion: 0.22
...
### Community 952 - "test_probe_base_url_override_hits_the_typed_url"
Cohesion: 0.22
...
### Community 959 - "Agent Autonomy Roadmap"
Cohesion: 0.25
...
### Community 963 - "test_delegate_propagates_runtime_errors"
Cohesion: 0.25
...
### Community 966 - "default_executives"
Cohesion: 0.29
...
### Community 983 - "hybrid_reasoning.py"
Cohesion: 0.36
...
### Community 997 - "TOP 6 — Highest-ROI Items (Validated by Opus Research Agent)"
Cohesion: 0.29
...
### Community 998 - "SECTION A — Agent Efficiency (Hermes / AOS / MYT)"
Cohesion: 0.25
...
### Community 1001 - "commercial_equivalent.py"
Cohesion: 0.50
...
### Community 1003 - "SECTION C — Direct Chat Improvements (CBF / HRM)"
Cohesion: 0.25
...
### Community 1006 - "TestItIsNotBuiltForOneVendor"
Cohesion: 0.25
...
### Community 1009 - "TestProviderRouter"
Cohesion: 0.25
...
### Community 1010 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 1012 - "TestAgentAutoAssignment"
Cohesion: 0.25
...
### Community 1018 - "test_provider_models_db_outage.py"
Cohesion: 0.25
...
### Community 1019 - "TOOLS.md — Available Tools for AI Agents"
Cohesion: 0.25
...
### Community 1022 - "SamConversation"
Cohesion: 0.29
...
### Community 1027 - "parse_react_response"
Cohesion: 0.47
...
### Community 1028 - "Agents"
Cohesion: 0.29
Nodes (5): Agents, Environment variables, `/freebuff <task>`, Running 24×7, Telegram phone control

### Community 1029 - "SIA.py"
Cohesion: 0.27
...
### Community 1032 - "Threat Model — Autonomous Agent Execution"
Cohesion: 0.29
...
### Community 1049 - "_is_trivial_message"
Cohesion: 0.29
...
### Community 1054 - "Workspace Issues"
Cohesion: 0.29
...
### Community 1055 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 1056 - "._call_reward_api"
Cohesion: 0.33
...
Cohesion: 0.09
Nodes (19): Adding a model, Adding a task category, CLAUDE.md — router/, Environment variables, Testing, What this package does, Dynamic model router package. Public API:: from router import get_router,…, ModelRouter (+11 more)

### Community 1062 - "_fake_http_sequence"
Cohesion: 0.16
Nodes (13): Fixed, Fixed, Fixed, Fixed, _choose_local_brain(), _probe_v1_models(), Probe ``ports`` in order; return the first responding listener. Returns…, Probe the local server's /v1/models. Returns (port_state, models, has_glm52,… (+5 more)

### Community 1063 - "TestNoHardcodedModels"
Cohesion: 0.33
...
### Community 1066 - "_fixture"
Cohesion: 0.02
Nodes (114): Drop the cached config (test hook)., reset(), Test hook — clear all key statistics., reset(), _clear_discovered_models(), _clear_response_cache(), _isolate_operator_provider_state(), MonkeyPatch (+106 more)
...
### Community 1068 - "WorkspaceEscapeError"
Cohesion: 0.40
...
### Community 1071 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1074 - "agents/commands.py"
Cohesion: 0.50
...
### Community 1076 - "_FakeCollection"
Cohesion: 0.25
Nodes (3): fake_mongo(), _FakeCollection, _FakeDb

### Community 1077 - "TestCli"
Cohesion: 0.57
...
### Community 1098 - "The Agent Roster"
Cohesion: 0.33
Nodes (6): 🔨 Implementer, ⚖️ Judge, 📋 Planner, 🔍 Reviewer, 🔭 Scout, The Agent Roster

### Community 1099 - "_decompose_into_subtasks"
Cohesion: 0.33
...
### Community 1102 - "SECTION B — NVIDIA / Cloud Model Integration (Nemotron / NVD)"
Cohesion: 0.33
...
### Community 1103 - "SECTION D — Deployment & Infrastructure (CHM / NVD)"
Cohesion: 0.33
...
### Community 1108 - "_openai_tools_to_anthropic"
Cohesion: 0.50
...
### Community 1109 - "_clean_director"
Cohesion: 0.50
...
### Community 1112 - "compare_runtimes.py"
Cohesion: 0.11
...
### Community 1129 - "TestDoctorEndpoint"
Cohesion: 0.50
...
### Community 1135 - "_make_sam_assistant"
Cohesion: 0.50
Nodes (4): _make_sam_assistant(), create_task(), __init__(), Build the SAM voice Agent with in-process agency tools. Defined inside a…

### Community 1136 - "BudgetExceeded"
Cohesion: 0.67
...
### Community 1151 - ".daily_active_users"
Cohesion: 0.40
...
### Community 1152 - "SECTION E — Autonomy & Self-Healing (AOS / MYT / ECC)"
Cohesion: 0.40
...
### Community 1153 - "SECTION F — Developer Experience (CBF / ECC)"
Cohesion: 0.29
...
### Community 1156 - "Agent API Issues"
Cohesion: 0.40
...
### Community 1157 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 1159 - "Documentation map"
Cohesion: 0.40
...
### Community 1172 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 1183 - "TestHermesKeepAlive"
Cohesion: 0.40
...
### Community 1185 - "test_key_pool.py"
Cohesion: 0.06
Nodes (25): _build_request(), _provider_keys(), Every configured key for *provider*, primary first. Returns an empty list when…, Return ``(url, headers, is_anthropic_native)`` for *provider*. Anthropic's…, api_keys_for(), Clear pool state (tests only)., All keys configured for *provider_id*, primary first. Thin delegate to…, reset() (+17 more)

### Community 1186 - "routed"
Cohesion: 0.33
...
### Community 1203 - "_inject_tool_results_as_messages"
Cohesion: 0.50
...
### Community 1208 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1209 - "Langfuse Issues"
Cohesion: 0.50
...
### Community 1213 - "Telegram Bot Issues"
Cohesion: 0.50
...
### Community 1215 - "pytest_collection_modifyitems"
Cohesion: 0.50
Nodes (4): Item, Config, pytest_collection_modifyitems(), pytest_configure()

### Community 1216 - "This repository is maintained by its own agents"
Cohesion: 0.50
...
### Community 1232 - "test_skills_route_order.py"
Cohesion: 0.67
...
### Community 1241 - "WebhookSendRequest"
Cohesion: 0.67
...
### Community 1262 - "test_the_reserve_is_bounded_when_read_from_the_environment"
Cohesion: 0.67
...
- **3066 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3061 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13968 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **167 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AgentRunner` connect `AgentRunner` to `ContextPruner`, `AgentPlan`, `UserMemoryStore`, `backend/server.py`, `proxy.py`, `TaskSpec`, `MultiAgentSwarm`, `GitHubTools`, `test_agent_runner.py`, `test_governance_api.py`, `PolicyEngine`, `Universality: case-coverage matrix`, `.session_id`, `[Unreleased]`, `AdaptiveHalter`, `coordinator.py`, `Path`, `._dispatch_tool`, `Agency Core — Progress & Resume Log`, `ReactScratchpad`, `TestDeprecationWarnings`, `failover_chat_completion`, `E2BSandboxSession`, `telegram_bot.py`, `test_ceo_dispatcher.py`, `TestAgentLoopMCPIntegration`, `test_fabric_patterns.py`, `ContextManager`, `build_matrix`, `Fixed`, `Added`, `RepoConnection`, `AgentJobRequest`, `Changed`, `Security Analysis — local-llm-server`, `WorkflowOrchestrator`, `test_backend_server_features.py`, `._generate_plan`, `os`, `Agent Autonomy Roadmap`, `Fixed`, `build_governance_router`, `StuckDetector`, `FreeBuffAgent`, `test_ceo_cross_verify.py`, `WorkspaceTools`, `E2BAdapter`, `._chat_text`, `test_daily_2026_06_04.py`, `test_empirical_verify.py`, `Section-by-Section Acceptance Criteria`, `test_ceo_supervision.py`, `LocalWorkspace`, `Issue #504: EPIC: Autonomy hardening — live-verified defects 2026-06-10`, `failover_client.py`, `get_ceo_dispatcher`, `TokenBudget`, `test_agent_free_brain.py`, `_runner`, `AgentSessionStore`, `SECTION A — Agent Efficiency (Hermes / AOS / MYT)`, `UserInfo`, `Competitor Analysis — Autonomous AI Agency`, `Issue #656: Bugs`, `WorkflowEngine`, `pytest`, `CompanyGraphStore`, `build_tool_prompt`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `ma()` connect `Fixed` to `la`, `de`, `gsap.min.js`?**
...
- **Should `llm/router.py` be split into smaller, more focused modules?**
  _Cohesion score 0.02617924528301887 - nodes in this community are weakly interconnected._
- **Should `backend/server.py` be split into smaller, more focused modules?**
  _Cohesion score 0.022278268179907523 - nodes in this community are weakly interconnected._
- **Should `TestAuthAndTaskOwnership` be split into smaller, more focused modules?**
