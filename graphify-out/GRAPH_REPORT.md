...
## Corpus Check
- 1363 files · ~1,910,770 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 26636 nodes · 53945 edges · 1142 communities (1032 shown, 110 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 6048 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `73ee8633`
...
## Communities (1142 total, 110 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.01
Nodes (465): get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, get_harness_adapter(), Warm the in-process cache from the DB. Best-effort; never raises. Reads…, Persist a setting and refresh the in-process cache., refresh_cache(), set_setting(), _store() (+457 more)

### Community 1 - "ai/router.py"
Cohesion: 0.05
Nodes (46): _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog() (+38 more)

### Community 2 - "company_api.py"
Cohesion: 0.04
Nodes (178): ephemeral_ttl_hours(), Async read of the ephemeral TTL (hours) straight from the DB., account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), _DoctorCheck (+170 more)

### Community 3 - "proxy.py"
Cohesion: 0.02
Nodes (297): Browser admin UI for login, service control, key management, and diagnostics., Update or append a KEY=value line in the .env file., register_admin_gui(), _save_env_var(), BackgroundAgent, BackgroundTask, Any, Enqueue *task* for processing. Returns the task (with task_id set). (+289 more)

### Community 4 - "ExecutionRequest"
Cohesion: 0.02
Nodes (128): Reset the singleton (test helper)., reset_ceo_dispatcher(), ExecutionRequest, get_ceo_fallback_stats(), PlanOutput, Any, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke… (+120 more)
...
Cohesion: 0.02
Nodes (109): delete_company_endpoint(), get_skill(), list_skills(), match_specialists(), delete, SystemType, Match specialists to a task. Returns the best specialists for the given task…, List all available skills with optional filtering. Returns the skill catalog… (+101 more)

### Community 8 - "AgentSessionStore"
Cohesion: 0.02
Nodes (241): Agency, CEO-coordinated multi-agent agency for continuous codebase management. The CEO…, AgentJobRequest, AgentJobSnapshot, BaseModel, Complete point-in-time view of a job, safe to serialise as API response., Convenience: the canonical text response, whether success or failure., Validated input for creating a new agent job. Passed from the API handler into… (+233 more)

### Community 9 - "llm/router.py"
Cohesion: 0.02
Nodes (124): _load(), main(), int, get_budget(), The process-wide budget tracker., get_cache(), The process-wide cache manager., text_key() (+116 more)

### Community 10 - "test_runtimes.py"
Cohesion: 0.04
Nodes (52): AiderAdapter, Any, Adapter for Aider — TIER 3 specialized git-aware code editor., GooseAdapter, Any, Adapter for Goose — TIER 2 general-purpose local runtime., HermesAdapter, Adapter for Hermes Agent — FIRST CLASS autonomous runtime. (+44 more)

### Community 11 - "test_ceo_supervision.py"
Cohesion: 0.03
Nodes (134): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), _harvest_changed_files(), _merge_changed_files(), _offload(), Any (+126 more)
...
Cohesion: 0.04
Nodes (67): chunk_document(), ContextManager, estimate_tokens(), FitResult, message_tokens(), prune(), Any, packages/llm/context.py — keeping conversations inside the context window. The… (+59 more)

### Community 16 - "Initiative"
Cohesion: 0.05
Nodes (16): Initiative, Create and register a new initiative, returning it., Add a pre-built Initiative (e.g. from the intelligence layer)., Look up an initiative by ID., A portfolio initiative (epic) prioritised via WSJF. WSJF (Weighted Shortest Job…, Aggregate cost of delay (CoD) used as the WSJF numerator., Weighted Shortest Job First score — higher schedules sooner., Associate an agile sprint that delivers part of this initiative. (+8 more)

### Community 17 - "TaskExecutionCoordinator"
Cohesion: 0.08
Nodes (20): _is_brain_connection_error(), Any, BaseException, Task, TaskResult, TaskSpec, TaskStatus, Best-effort label of who will run this task once approved. Returns… (+12 more)
...
### Community 20 - "get_workflow_orchestrator"
Cohesion: 0.04
Nodes (53): Return orchestrator queue depth, active runs, and supervisor state (#522)., Called by APScheduler when a cron fires. Dispatches to the orchestrator., _scheduler_on_fire(), workflow_orchestrator_status(), codeql_count(), dependabot_count(), main(), Any (+45 more)
...
Cohesion: 0.02
Nodes (119): AgentRole, str, AutonomyCounter, AutonomySnapshot, AutonomyTracker, get_tracker(), Any, agent/kpi.py — Autonomy KPIs: evidence capture and metrics tracking. Tracks key… (+111 more)
...
### Community 26 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (96): CompanyGraphSnapshot, KnowledgeItem, SpecialistFamily, Find a website by its URL., Find a repository by its URL., Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., Find knowledge items matching any of the given tags. (+88 more)
...
Cohesion: 0.03
Nodes (85): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, all_provider_ids(), provider_key_present(), Return every provider id recognised by the brain config system. Iterates the…, True when the env var for *provider*'s key is set (or it's Ollama)., CatalogActiveBrain, CatalogMirror (+77 more)

### Community 31 - "test_llm_router_resilience.py"
Cohesion: 0.02
Nodes (92): HealthConfig, Backoff policy for retryable failures., Circuit breaker + health tracking thresholds., RetryConfig, BreakerState, HealthTracker, _Outcome, ProviderHealth (+84 more)
...
Cohesion: 0.04
Nodes (72): BrainConfig, BrainConfigStore, default_brain_config(), provider_base_url(), _provider_env_value(), _provider_ids_from_literal(), provider_key_rotation_enabled(), BaseModel (+64 more)
...
Cohesion: 0.04
Nodes (37): _build_request(), Return ``(url, headers, is_anthropic_native)`` for *provider*. Anthropic's…, _digest(), KeyPool, _KeyState, _PoolState, Per-provider API key rotation — the one lever that adds capacity. Every other…, Round-robin key selection with per-key rate-limit cooldowns. (+29 more)

### Community 39 - "test_webui_provider_priority.py"
Cohesion: 0.19
...
Cohesion: 0.04
Nodes (70): _now(), agent/background.py — Background Agent An always-on worker thread that…, _check_background_liveness(), _check_ci_parity(), _check_company_graph(), _check_disk(), _check_event_log_integrity(), _check_feature_matrix() (+62 more)
...
Cohesion: 0.02
Nodes (133): AgentRunner, _check_extra_kwargs(), _enforce_signature(), _nvidia_api_key(), Any, Path, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Append an event to the durable session log if a store is wired in. (+125 more)
...
Cohesion: 0.05
Nodes (77): classify_task(), _extract_recent_text(), Any, Task classification from request context. Classifies an incoming request into a…, Concatenate plain text from the last *last_n* messages., Return the most likely task category for this request. Args: messages: OpenAI-…, Reset the singleton and clear the cached model map (test helper)., reset_router() (+69 more)
...
Cohesion: 0.04
Nodes (90): brain_availability_summary(), Non-secret answer to "can the brain answer a request right now?". Three callers…, get_ceo_dispatcher(), Return the shared CEODispatcher singleton., get_ceo_ledger(), Return the shared :class:`CEOLedger`, constructing it on first use., Drop the singleton (test helper)., reset_ceo_ledger() (+82 more)

### Community 51 - "._connect"
Cohesion: 0.11
...
### Community 53 - "SeoAuditRequest"
Cohesion: 0.05
Nodes (64): delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), list_seo_audits(), BaseModel, get, post (+56 more)
...
### Community 56 - "loop.py"
Cohesion: 0.02
Nodes (154): PreflightIssue, PreflightReport, Any, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), get_enrichment() (+146 more)
...
Cohesion: 0.04
Nodes (61): _auto_disable(), _Budget, _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), FailoverResult, _is_billing_refusal(), _is_ollama() (+53 more)
...
### Community 69 - "engine.py"
Cohesion: 0.07
Nodes (30): _fake_artifact(), _make_engine(), fixture, tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, TestPhaseSequence (+22 more)
...
### Community 76 - "OrchestratorQueue"
Cohesion: 0.07
...
Cohesion: 0.04
Nodes (62): packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, get_failover_manager(), Return the singleton BrainFailoverManager., Reset the singleton (for tests)., reset_failover_manager(), Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _heal_brain_failover() (+54 more)
...
### Community 105 - "SeoAuditEngine"
Cohesion: 0.09
Nodes (26): BaseModel, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Site-level facts discovered during the crawl., An agent-delegable remediation work package derived from the findings. Findings…, Lightweight listing entry for past audits., SeoAuditSummary (+18 more)
...
Cohesion: 0.07
Nodes (30): Aggregated report row - Screaming Frog CSV compatible., SeoIssueReportRow, analyze_page(), compute_pressure(), _count_syllables(), estimate_pixel_width(), flesch_reading_ease(), _host_key() (+22 more)
...
Cohesion: 0.05
Nodes (50): Status of a user story within a sprint., StoryStatus, EdgeType, Types of relationships between knowledge nodes., ConsolidationPhase, MemoryKind, Enum, str (+42 more)
...
Cohesion: 0.05
Nodes (64): quick_notes_submit(), Submit a quick-note URL or instruction from the dashboard FAB., Owns lifecycle transitions, comment semantics, and approval rules., TaskWorkflowService, Persistent task store backed by MongoDB. Uses the same motor client pattern as…, Set the global task store instance (e.g., during app startup with MongoDB)., set_task_store(), TaskStore (+56 more)
...
Cohesion: 0.05
Nodes (72): allow_paid_brain(), BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), is_anthropic_model() (+64 more)

### Community 134 - "agency.py"
Cohesion: 0.05
Nodes (42): AgencyCycleResult, AgentDirective, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context(), _fetch_github_quick_notes(), get_agency() (+34 more)
...
Cohesion: 0.03
Nodes (108): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, _coerce_ts(), ExecutionLogEntry, Any, Enum (+100 more)

### Community 148 - "cache.py"
Cohesion: 0.06
...
### Community 181 - "test_provider_policy.py"
Cohesion: 0.20
...
Cohesion: 0.07
Nodes (35): Resolve the model id for a component's role on a provider. Parameters…, resolve_component_model(), tests/test_unit6_resolve_component_model.py — UNIT 6 regression tests. Verifies…, When the DB cache is fresh AND provider matches the active primary, the DB-…, When the DB primary differs from the requested provider, the catalog preset for…, When `provider` is None, the DB primary's saved model wins., With no DB cache and no provider, the env var is consulted., An unknown role returns the safe default rather than raising. (+27 more)
...
Cohesion: 0.09
Nodes (15): get_scheduler_store(), _MemCollection, _MemCursor, _MemDeleteResult, Any, services/scheduler_store.py — Durable scheduler persistence. Issue #505:…, Delete a persisted job., Return the total number of persisted jobs. (+7 more)
...
### Community 198 - "test_brain_config_store.py"
Cohesion: 0.09
Nodes (27): invalidate_brain_config_cache(), Convenience wrapper used by the admin API endpoints., Clear the singleton's cache (used by tests + brain_policy invalidation)., Clear the in-process cache. Called by the admin API after a successful PATCH…, set_brain_config(), _isolated_store(), fixture, tests/test_brain_config_store.py — BrainConfigStore unit tests. Pins the… (+19 more)
...
### Community 225 - "test_render_mcp.py"
Cohesion: 0.09
...
### Community 226 - "MCPUnavailableError"
Cohesion: 0.15
...
Cohesion: 0.09
Nodes (18): _backend(), CEOLedger, _now(), Any, services/ceo_ledger.py — durable record of what the CEO is driving to closure.…, Return ``(done, total)`` subtask counts., Durable store for :class:`GoalRecord`s. Every method is best-effort and never…, Persist *goal*, stamping ``updated_at``. (+10 more)
...
### Community 234 - "SetupWizardPage.js"
Cohesion: 0.10
...
### Community 265 - "test_workflow_models.py"
Cohesion: 0.10
...
Cohesion: 0.08
Nodes (16): ScheduledJob, _MemDB, fixture, tests/test_scheduler_store.py — Tests for durable scheduler store (#505)., count() returns 0 for an empty store., count() reflects the number of saved jobs., count() decreases after delete., delete_stale() keeps jobs updated recently. (+8 more)
...
Cohesion: 0.07
Nodes (31): AgentRole, Return the AgentProfile for the agent driving *phase*., Run a pre-gate or report phase through the correct agent. Enforces permission…, Execute a slice via the Coder agent (write-permitted)., Review a slice via the Reviewer agent (different model from Coder). This is the…, Run verification commands via the Verifier agent. The Verifier is execution-…, Return a copy of *routing* with the profile's model injected for its role., Raise PermissionError if the profile lacks required permissions. (+23 more)
...
Cohesion: 0.14
Nodes (29): Complete result of one audit run., SeoAuditReport, Paragraph, _appendix_full_findings(), _appendix_worst_pages(), _appendix_wsjf_roadmap(), _cell(), _cover_page() (+21 more)
...
### Community 360 - "router_factory"
Cohesion: 0.15
Nodes (15): _big_request(), _ok(), fixture, A conversation comfortably past the metered provider's per-minute budget.…, A 131k window behind a 4k budget is a 4k ceiling., Routing around the low ceiling beats compressing to fit it., Production's case: every reachable provider has a low ceiling. Nothing can be…, No compression tax on requests that already fit. (+7 more)
...
### Community 375 - "._execute_with_fallback"
Cohesion: 0.16
...
### Community 376 - "OnboardingService"
Cohesion: 0.06
...
### Community 448 - "is_anthropic_base_url"
Cohesion: 0.12
...
### Community 454 - "test_orchestrator_merge_decision.py"
Cohesion: 0.22
...
### Community 464 - "resolve_role_model_sync"
Cohesion: 0.07
Nodes (32): _get_provider_policy(), Read the durable provider policy from DB, falling back to a safe default.…, Synchronous call-time resolver for an agent role model id. Precedence (highest…, resolve_role_model_sync(), Resolve the LLM endpoint for a named surface (task/chat/ceo/sdlc/…). Honours…, resolve_provider_for(), An explicit ``requested`` value always wins., test_resolve_role_model_precedence_requested_wins() (+24 more)
...
### Community 504 - "_is_bedrock_model_id"
Cohesion: 0.27
...
### Community 516 - "oauth.py"
Cohesion: 0.22
Nodes (8): _github_login_url(), google_exchange_code(), google_fetch_user(), _google_login_url(), _new_state(), social_auth.py — GitHub and Google OAuth social login (CANONICAL MODULE).…, Utility: return the Google OAuth authorize URL., Utility: return the GitHub OAuth authorize URL.
...
Cohesion: 0.14
Nodes (15): _build_cost_table(), cost_for_tokens(), get_cost_table(), get_stats(), _load_env_overrides(), Any, Per-model cost attribution for the LLM provider router. Maintains in-memory…, Return the USD cost for (prompt_tokens, completion_tokens) on *model*. Returns… (+7 more)
...
### Community 536 - "model_router.py"
Cohesion: 0.08
Nodes (30): _enabled(), get_available_models(), invalidate_cache(), is_model_available(), Ollama model availability check with TTL cache. Keeps a short-lived cache of…, Force the next call to re-probe Ollama (useful in tests)., Return True if *model* is in the Ollama tag list (or health checks off).…, Return the set of model names currently present in Ollama. Returns an empty set… (+22 more)
...
### Community 574 - "_FakeAsyncClient"
Cohesion: 0.20
...
### Community 582 - "TestHarnessAdapter"
Cohesion: 0.09
...
### Community 642 - "test_bootstrap_source_id_index.py"
Cohesion: 0.27
...
### Community 659 - "BackgroundServices"
Cohesion: 0.38
...
### Community 661 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 696 - "4. Current Architecture (As-Is)"
Cohesion: 0.29
...
Cohesion: 0.11
Nodes (13): fake_mongo(), _FakeDb, isolated_state(), _live_mongo_url(), fixture, Operator provider state must survive a redeploy. The per-provider kill switch…, Temp SQLite mirror + clean caches, so no test sees another's state., Return a reachable MONGO_URL, or None so the test skips. (+5 more)
...
### Community 763 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 768 - "1. What This Repo Does"
Cohesion: 0.40
...
### Community 777 - "reset_provider_cooldowns"
Cohesion: 0.40
...
### Community 794 - "10. Testing Constitution"
Cohesion: 0.50
...
### Community 857 - "5. AI Provider Architecture"
Cohesion: 0.50
...
### Community 859 - "probe_live_providers.py"
Cohesion: 0.67
...
### Community 864 - "10. CI/CD Standards"
Cohesion: 0.67
Nodes (3): 10. CI/CD Standards, Deployment, Pipeline (22 checks)

### Community 865 - "11. Rewrite Strategy"
Cohesion: 0.67
...
### Community 873 - "3. Repository Constitution"
Cohesion: 0.67
...
### Community 892 - "6. Agent Architecture"
Cohesion: 0.67
Nodes (3): 6. Agent Architecture, Agent lifecycle, Current state

### Community 893 - "7. Scheduler Architecture"
Cohesion: 0.67
...
### Community 895 - "8. Authentication Architecture"
Cohesion: 0.67
...
### Community 899 - "RuntimeManager"
Cohesion: 0.07
Nodes (17): RoutingDecision, Any, TaskResult, TaskSpec, Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it. (+9 more)
...
### Community 905 - "clean_store"
Cohesion: 0.67
Nodes (3): clean_store(), fixture, Reset the brain config store + point SQLITE_DB_PATH at a tmp path.

### Community 906 - "_isolate_warmup_overflow"
Cohesion: 0.67
Nodes (3): _isolate_warmup_overflow(), fixture, ``_warmup_overflow`` is process-global — keep tests from sharing it.
...
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `proxy.py`, `ExecutionRequest`, `resolve_active_brain`, `_StubManager`, `StuckDetector`, `AgentSessionStore`, `test_ceo_supervision.py`, `TaskSpec`, `PortfolioManager`, `.execute`, `AdaptiveHalter`, `E2BAdapter`, `KeyPool`, `diagnostics.py`, `failover_chat_completion`, `E2BSandboxSession`, `LocalWorkspace`, `loop.py`, `MCPClient`, `_FakeAsyncClient`, `test_empirical_verify.py`, `ContextPruner`, `ContextManager`, `GitHubTools`, `get_task_store`, `MultiAgentSwarm`, `TestZeroAttemptDiagnostics`, `MCPUnavailableError`, `TokenBudget`, `workflow_orchestrator.py`, `Workspace`, `WorkspaceTools`, `ReactScratchpad`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `TaskStore` connect `TaskStore` to `backend/server.py`, `test_trend_scoping.py`, `TaskExecutionCoordinator`, `tasks/service.py`, `test_phase4_runtime_resilience.py`, `test_phase6_workflow.py`, `Task`, `test_portfolio_intake.py`, `test_task_source_id_race.py`, `TaskDispatcher`, `loop.py`, `_ts_to_float`, `test_task_brain_preflight.py`, `test_tasks_awaiting_approval_api.py`, `test_issue_intake.py`, `get_task_store`, `tasks/api.py`, `test_sqlite_store.py`, `services/background.py`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `get_agent_bus()` connect `AgentMessageBus` to `loop.py`, `test_p0_roadmap_a4_a5_b2.py`, `AgentRunner`?**
