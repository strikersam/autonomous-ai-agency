# Graph Report - autonomous-ai-agency  (2026-09-24)

## Corpus Check
- 1538 files · ~2,242,237 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 32683 nodes · 67203 edges · 1305 communities (1133 shown, 172 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 6616 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4141efd9`
...
## Communities (1305 total, 172 thin omitted)

### Community 0 - "os"
Cohesion: 0.01
Nodes (316): Browser admin UI for login, service control, key management, and diagnostics., ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, agent/agency.py — Autonomous Agent Agency (CEO-driven, LLM-powered) Runs the…, agent/background.py — Background Agent An always-on worker thread that…, Cached LLM Client wrapper. Drop-in wrapper around any LLM API call that…, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata…, context_pruner.py — auto-generated module docstring (user-research skill scan)., agent/error_interceptor.py — HTTP Error Interceptor Middleware… (+308 more)

### Community 1 - "Specialist"
Cohesion: 0.03
Nodes (65): What already exists (don't rebuild), SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, build_matrix() (+57 more)

### Community 2 - "SeoAuditEngine"
Cohesion: 0.05
Nodes (41): Serve the launcher UI., root(), field_validator, A single occurrence of a check firing on a specific URL., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance (+33 more)

### Community 3 - "Changed"
Cohesion: 0.03
Nodes (133): _fetch_github_quick_notes(), _gh_repo(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, Real handler — dispatches through AgentRunner when available. HARDENED (PR…, Changed, Changed, A0. Fix live scanner crashes on real-world sites (`services/scanner.py`) — do first (+125 more)

### Community 4 - "MongoDBStore"
Cohesion: 0.04
...
Cohesion: 0.02
Nodes (115): cosine_similarity(), payload_key(), Exact-match cache key over the fields that change the answer. Routing…, Cosine similarity between two vectors, 0.0 when either is degenerate., build_summariser(), summarise(), chunk_document(), ContextManager (+107 more)

### Community 6 - "backend/server.py"
Cohesion: 0.02
Nodes (164): set_skill_registry(), admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, AuthorizeReposBody (+156 more)

### Community 7 - "get_feature_matrix"
Cohesion: 0.08
...
### Community 8 - "test_refresh_agent_built_proof.py"
Cohesion: 0.07
Nodes (9): _load_agent_runtime_module(), test_wrapper_exposes_hermes_task_endpoints(), fake_chat_with_ollama(), test_wrapper_exposes_opencode_run_endpoint(), test_wrapper_falls_back_to_installed_model(), doc_paths(), tests/test_refresh_agent_built_proof.py Root-cause fix for the "agent-built…, The real committed docs, run through the rewriter with their own current… (+1 more)

### Community 9 - "proxy.py"
Cohesion: 0.03
Nodes (166): Describes a single worker's task and constraints., WorkerSpec, set_quick_note_queue(), fastapi_middleware_cors, middleware, admin_control(), admin_create_user(), admin_delete_user() (+158 more)

### Community 10 - "api.js"
Cohesion: 0.01
Nodes (54): Key files, addTaskComment(), approveGovernanceRequest(), approveTaskCheckpoint(), approveTaskExecution(), autoRecommendCompanySkills(), createMcpServer(), createQuickNote() (+46 more)

### Community 11 - "brain_config.py"
Cohesion: 0.02
Nodes (160): _migrate_brain_to_safe_default(), Architecture (per plan §3), Files touched, Hard constraints (from the plan) — all met, Implementation — DB-persisted, UI-switchable Brain (PR #824 follow-up), New files, Resolution precedence, Rollout / verification (per plan §5) (+152 more)

### Community 12 - "LLMRouter"
Cohesion: 0.06
Nodes (35): Fixed, Fixed, Modules, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, run(), Any (+27 more)

### Community 13 - "LLMRequest"
Cohesion: 0.02
Nodes (150): Added, collections_abc, Added, Writing a custom adapter, ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic…, classify_error() (+142 more)

### Community 14 - "test_llm_router_resilience.py"
Cohesion: 0.02
Nodes (145): count, Preferring local, Choosing one, Latency got worse, HealthConfig, Backoff policy for retryable failures., Strategy selection and degradation behaviour., Circuit breaker + health tracking thresholds. (+137 more)

### Community 15 - "test_operational_incidents.py"
Cohesion: 0.07
Nodes (27): Find agent phases that started but never finished.…, summarise_phases(), filed(), Recurring operational failures must diagnose and file themselves. The gap these…, The reported scenario, end to end: four real timeouts, one incident., Two timeouts an hour apart are not a pattern., Even many *distinct* failure modes cannot produce unbounded incidents. The…, `record` runs inside a logging.Handler — it must swallow everything. (+19 more)

### Community 16 - "TaskSpec"
Cohesion: 0.01
Nodes (288): abc, 3b. Hermes — **our own** Hermes server (in-repo), UI-wired ✅  (size: M), N2. Surface Hermes (and all runtimes) status in the Doctor/Runtimes UI ⬜  (size: S, risk: low), Resolve the model id to force for a code-execution run, or ``None``. Returns…, resolve_coding_model_preference(), FastAPI dependency: require any authenticated user., require_authenticated(), kimi_bridge_runtime_config() (+280 more)

### Community 17 - "Added"
Cohesion: 0.03
Nodes (125): Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., _first_paragraph(), Return the first non-empty, non-heading line. Skips YAML frontmatter (--- ...…, BudgetExceededError, Exception, Reset token counters for all sessions (caps preserved). Called at the start of…, Reset all budgets if the UTC calendar day has changed since last reset. Safe to… (+117 more)

### Community 18 - "SQLiteStore"
Cohesion: 0.04
...
### Community 19 - "Agency"
Cohesion: 0.05
Nodes (63): Agency, AgencyCycleResult, AgentDirective, AgentRole, _build_quick_note_instruction(), _close_github_issue(), get_agency(), _gh_token() (+55 more)

### Community 20 - "test_agent_runner.py"
Cohesion: 0.04
...
Cohesion: 0.02
Nodes (110): _clear_discovered_models(), _clear_response_cache(), _isolate_operator_provider_state(), MonkeyPatch, Path, Default all tests to legacy workflow mode (Phase 2 compatibility). Only patches…, Keep the suite out of the developer's real operator-state database.…, Drop the per-provider model listings between tests.… (+102 more)

### Community 22 - "test_governance_sandbox.py"
Cohesion: 0.04
Nodes (74): build_docker_run_argv(), detect_backend(), DockerBackend, E2BBackend, load_profiles(), LocalBackend, Any, Path (+66 more)

### Community 23 - "failover_chat_completion"
Cohesion: 0.04
Nodes (103): Compatibility, What changes for callers, BrainFailoverExhausted, failover_chat_completion(), FailoverResult, _log_recovery(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the… (+95 more)
...
### Community 27 - "test_model_catalog.py"
Cohesion: 0.04
Nodes (76): _build_base_url_env_from_yaml(), _build_candidates_from_yaml(), _build_default_base_url_from_yaml(), _build_display_names_from_yaml(), _build_key_env_from_yaml(), _build_presets_from_yaml(), _build_tier_from_yaml(), get_provider_candidates() (+68 more)

### Community 28 - "Fixed"
Cohesion: 0.03
Nodes (66): Perform MCP handshake. Optional — tools/call works without it., Fixed, Fixed, Active Task Tracker, Bug Log, Current Sprint Tasks, Roadmap Items (from `docs/roadmap-killer-todos.md`), Fixed (+58 more)

### Community 29 - "E2BSandboxSession"
Cohesion: 0.04
Nodes (69): MCPUnavailableError, RuntimeError, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any, Create the sandbox. Raises :class:`MCPUnavailableError` on failure. (+61 more)

### Community 30 - "RuntimeCapabilityRegistry"
Cohesion: 0.07
Nodes (14): Return the single best adapter for *task_type*. If *preferred_runtime_id* is…, Maintains the catalogue of registered adapters and answers 'which runtimes can…, Return all adapters that can handle *task_type*, ordered by tier., RuntimeCapabilityRegistry, Admin-configurable routing policy. Defaults are conservative (local-first, no…, RoutingPolicy, task_harness (no _base_url) must not spawn agent_runtime.py., StubAdapter (+6 more)

### Community 31 - "PolicyEngine"
Cohesion: 0.04
Nodes (89): fnmatch, _action_matches(), _as_list(), Decision, GroupPolicy, _host_matches(), Mode, _normalise_path() (+81 more)
...
### Community 33 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (103): CEOResult, _complexity_rank(), _decompose_into_subtasks(), Reset the singleton (test helper)., Aggregated output from a multi-specialist execution., Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, Build a single-task sub-task list (low-complexity fast path)., reset_ceo_dispatcher() (+95 more)

### Community 34 - "failover_client.py"
Cohesion: 0.04
Nodes (64): FailureCategory, Called when a CI workflow fails., E2: Classify a failure from its description text. Order matters:…, Classified failure types for targeted self-healing (E2)., ma(), Fixed, Session Log, Fixed (+56 more)

### Community 35 - "enforcement.py"
Cohesion: 0.05
Nodes (48): 4. Security review, Deliberate design decisions, stated for review, Findings addressed during implementation, Verification status — stated precisely, _load(), Import the governance layer once, recording why if it is absent., events_from_identity(), Any (+40 more)

### Community 36 - "test_scanner_headless.py"
Cohesion: 0.02
Nodes (57): Fable 5 — Read-Only Audit & Skill-Distillation Notes, Part 0 — A caveat on how this task started, 10. Cloudflare Worker Security, 2. CORS Configuration, 3. Agent Filesystem Writes, 4. Secrets Management, 5. Input Validation, 6. Dependency Security (+49 more)

### Community 37 - "Task"
Cohesion: 0.04
Nodes (63): Full task/issue document., Task, _gate_outward_facing_enabled(), _is_outward_facing(), Any, One-line human explanation of why this task needs human approval. Reads risk…, Best-effort label of who will run this task once approved. Returns…, PUBLIC_URL-anchored dashboard deep link for the task detail. Returns empty… (+55 more)

### Community 38 - "test_runtime_governance.py"
Cohesion: 0.05
Nodes (59): F2 — MCP Server Exposing Proxy Capabilities [P1] [CBF / ECC], governance_enabled(), True when the governance layer should run at all. A single global off switch…, CircuitState, Return the last-known health for *runtime_id* (may be stale)., Return True if the runtime is available (not circuit-open)., Return health snapshots for all known runtimes., Force an immediate health check of all runtimes and return results. (+51 more)

### Community 39 - "test_web_reach.py"
Cohesion: 0.05
Nodes (62): Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_web_reach_tools(), _domain_list_reason(), _domain_matches(), get_web_reach(), _parse_domain_list(), Return True if *host* is *pattern* or a subdomain of it., Split a comma-separated env var into a normalised domain list. (+54 more)

### Community 40 - "test_unit8_model_catalog.py"
Cohesion: 0.04
Nodes (61): CatalogActiveBrain, CatalogMirror, CatalogProviderEntry, _include_router_providers(), ModelCatalogStore, Any, BaseModel, Whether to publish LLM-router providers in the catalog (default OFF). The… (+53 more)

### Community 41 - "WorkspaceTools"
Cohesion: 0.02
Nodes (90): CLAUDE.md — agent/, Skills worth invoking here, Testing, What this package does, Path, Return the first *lines* lines of a file. Just-in-time retrieval: the executor…, Return a lightweight index of files with line counts and sizes. This is the…, Delegate to RepowiseIntelligence for a natural-language codebase question. (+82 more)

### Community 42 - "[Unreleased]"
Cohesion: 0.05
Nodes (57): True if browser automation is enabled and Playwright is importable., Adding a new tool, Security surface, build_tool_prompt(), _load_script_module(), Any, ModuleType, Dynamically load a pure-stdlib helper module from .github/scripts/. Returns… (+49 more)

### Community 43 - "TaskStatus"
Cohesion: 0.05
Nodes (59): Submit a quick note instruction. Creates a Task with task_type='quick_note'., v4_quick_notes_submit(), Creates tasks from scheduler and playbook runs using one workflow., TaskAutomationService, str, TaskStatus, Re-run a task. Permissive across every non-active state so the dashboard Retry…, Add a follow-up instruction to a task and re-queue it for execution. Unlike… (+51 more)

### Community 44 - "test_kimi_bridge_provider.py"
Cohesion: 0.20
...
### Community 45 - "CEOSupervisor"
Cohesion: 0.05
Nodes (48): build_ceo_router(), ceo_status(), get_goal(), list_goals(), redrive_goal(), run_sweep(), Any, APIRouter (+40 more)

### Community 46 - "secrets_store.py"
Cohesion: 0.05
...
### Community 47 - "Added"
Cohesion: 0.05
Nodes (43): Return True only when the tool is definitively read-only and non-destructive.…, Remember a server-issued ``Mcp-Session-Id`` for subsequent requests., Added, Added, Render MCP — platform debugging and environment monitoring, Configuration, Write safety, When True, the Render ops loop runs. On by default. Also requires… (+35 more)

### Community 48 - "KeyStore"
Cohesion: 0.07
...
### Community 49 - "get_db"
Cohesion: 0.03
Nodes (109): auth_me(), auto_recommend_skills(), _builtin_provider_records(), _check_storage_health(), cost_table(), create_github_pr(), delete_api_key(), delete_github_token() (+101 more)

### Community 50 - "test_mcp_registry.py"
Cohesion: 0.05
...
### Community 51 - "RenderOpsMonitor"
Cohesion: 0.05
Nodes (37): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, One deploy, normalised from whatever shape the tool returned., RenderDeploy, Polls Render for platform-level failures and files them as issues., RenderOpsMonitor, _client() (+29 more)

### Community 52 - "test_governance_api.py"
Cohesion: 0.10
...
### Community 54 - "parse_event_stream"
Cohesion: 0.10
...
### Community 55 - "Added"
Cohesion: 0.02
Nodes (127): get_enrichment(), Return the enrichment instance for a workspace. Keyed by workspace root rather…, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Condense a tool result down to what belongs in an audit row. A tool result can…, Inject available tools + skills catalog into the system message.…, _summarise_tool_result(), Cooldown + hourly cap. Caller must hold the lock. Reserves the slot on success.…, Called from the v4 dashboard 'Report Bug' form and the LogMonitor.… (+119 more)

### Community 56 - "Settings"
Cohesion: 0.05
Nodes (28): _get_settings(), Typed configuration loaded from environment variables., When True, the governance layer evaluates and audits agent actions. This is…, When True, approval-gated actions self-approve. Local dev only., ``RENDER_SERVICE_IDS`` split into a clean list (empty when unset)., When True, mutating Render MCP tools may be called. Default False., When True, agents may drive a real browser (agent/browser.py)., True when a Browserbase key is present — the low-RAM remote path. (+20 more)

### Community 57 - "TaskStore"
Cohesion: 0.04
Nodes (85): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, _coerce_ts(), Enum, tasks/models.py — Pydantic models for the task/issue system., Coerce ISO-8601 datetime strings (from DB) to float timestamps., Comment or reply on a task. (+77 more)

### Community 58 - "test_unit7_catalog_propagation.py"
Cohesion: 0.05
Nodes (45): _catalog_defaults(), Resolve per-role defaults via the catalog. Returns None on import error., Checklist, Rollout notes, Summary, Test plan, UNIT 1 — Fix duplicate ceo_direct tasks ✅, UNIT 2 — Portfolio → task materializer (default ON) ✅ (+37 more)

### Community 59 - "test_docs_consistency.py"
Cohesion: 0.17
...
### Community 60 - "pytest"
Cohesion: 0.01
Nodes (249): detect_secrets(), main(), Recover CHANGELOG.md from a Git merge conflict in its [Unreleased] block. Pre-…, fastapi_testclient, _load(), main(), ModuleType, Scheduled agile ceremonies digest. Run by `.github/workflows/agile-… (+241 more)

### Community 61 - "validate_policy_text"
Cohesion: 0.05
Nodes (39): propose_policy(), validate_policy(), _audit_proposal(), _baseline_loosened(), _baseline_patterns(), diff_policy(), _GitHub, parse_policy_text() (+31 more)

### Community 62 - "AgentRunner"
Cohesion: 0.03
Nodes (92): Convenience: the canonical text response, whether success or failure., AgentRunner, _run(), _check_extra_kwargs(), _enforce_signature(), Any, Path, Raise TypeError if fn's signature drifts from the locked contract (Pydantic… (+84 more)

### Community 63 - "llm/router.py"
Cohesion: 0.02
Nodes (139): heapq, int, all_models(), best_model_for(), ModelInfo, models_by_provider(), packages/ai/registry.py — Model Registry. Centralized registry of all models…, Register the default free-tier models. (+131 more)

### Community 64 - "config.py"
Cohesion: 0.03
Nodes (76): CacheManager, _Entry, LRUCache, Any, T, Live (unexpired) entries — used by the semantic layer's scan., Owns every cache layer and the policy for what may enter them., Whether a response to ``request`` may be stored. Deliberately conservative — a… (+68 more)

### Community 65 - "test_bedrock_provider.py"
Cohesion: 0.05
...
### Community 66 - "DetectedSystem"
Cohesion: 0.05
Nodes (46): P1 — Close the remaining product gaps, Task 6 — Deeper website/repo scanning (more than "2 systems"), Task 7 — Knowledge base auto-reflect (ECC / Obsidian / graphify-style), Task 8 — Vector retrieval for the knowledge base (self-learn), DetectedSystem, Evidence, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection. (+38 more)

### Community 67 - "resolve_e2b_config"
Cohesion: 0.05
Nodes (53): Available iff config resolves AND the SDK is importable. Never raises — a…, e2b_enabled(), E2BConfig, _env_falsy(), _env_truthy(), services/e2b_config.py — Single reader of E2B sandbox environment config.…, Return ``True`` when E2B sandboxing should be activated. **EXPERIMENTAL…, Resolve the E2B sandbox config from env, or ``None`` when unconfigured. This is… (+45 more)

### Community 68 - "control.py"
Cohesion: 0.06
Nodes (48): (1) & partly (4): "Something went wrong" masks the real error everywhere, (2) & (3): Company creation flow / non-admin gate placement, (4): Agent provisioning "loading forever" — blocking subprocess in async path, Agent Prompt (paste this to start the implementation session), B. Make runtime activation non-blocking (`runtimes/control.py`,, Context — bugs reported during manual QA, E. General bug sweep, Implementation Plan (+40 more)

### Community 69 - "ExecutionRequest"
Cohesion: 0.05
Nodes (46): Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), get_orchestrator_supervisor(), stop_orchestrator_supervisor(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper). (+38 more)

### Community 70 - "test_governance_enforcement.py"
Cohesion: 0.05
Nodes (72): BudgetTracker, GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., _coerce_text(), new_session_id(), Any, packages/governance/identity.py — who is acting, and under what mandate. Docker… (+64 more)

### Community 71 - "Added"
Cohesion: 0.12
Nodes (21): Any, Force the next ``list_tools()`` call to fetch a fresh tool list from the…, brain_providers(), Every configured provider with its health AND its on/off state. Powers the…, Added, Added, Failure behaviour, BarChart() (+13 more)

### Community 72 - "SetupWizardPage.js"
Cohesion: 0.05
...
### Community 74 - "test_ceo_supervision.py"
Cohesion: 0.05
Nodes (72): CEODispatcher, _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, Real CEO delegation: wake runtimes, fan out, and merge results., Attempt, One delegation of one subtask to one tier., A subtask's full history: what it is, and every attempt at it., SubtaskRecord (+64 more)

### Community 75 - "AgentDefinition"
Cohesion: 0.05
...
### Community 76 - "test_schedule_backlog_drain.py"
Cohesion: 0.07
...
Cohesion: 0.05
Nodes (38): Changed, Changed, API, getActivity(), HubTabs(), Spinner(), ActivationGate(), activityToAlert() (+30 more)

### Community 78 - "agents/api.py"
Cohesion: 0.09
Nodes (36): _apply_activity_status(), create_agent(), delete_agent(), get_agent(), _get_user(), list_agents(), list_runtime_agents(), Any (+28 more)

### Community 79 - "WorkflowRun"
Cohesion: 0.08
Nodes (25): engine(), _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test(), _tracking_run_single() (+17 more)

### Community 80 - "detector.py"
Cohesion: 0.06
...
### Community 81 - "setup/api.py"
Cohesion: 0.06
Nodes (69): is_user_onboarding_allowed(), Return True if this user may run the onboarding wizard. Resolution order: 1. If…, motor_motor_asyncio, audit(), Append an audit log entry. Never logs raw secrets — only secret IDs / masked…, complete_wizard(), _delete_wizard_state(), detect_configured_providers() (+61 more)

### Community 82 - "AgentSwarm"
Cohesion: 0.06
Nodes (20): AgentSwarm, Any, Return the agent role responsible for *phase*., Return the AgentProfile for the agent driving *phase*., Return a JSON-serialisable summary of all agent profiles., Run a pre-gate or report phase through the correct agent. Enforces permission…, Execute a slice via the Coder agent (write-permitted)., Review a slice via the Reviewer agent (different model from Coder). This is the… (+12 more)

### Community 83 - "test_cost_aware_routing_eval.py"
Cohesion: 0.06
...
### Community 86 - "yaml"
Cohesion: 0.04
Nodes (35): _env_float(), Read a float env var, falling back to *default* on unset/garbage., merge_step(), Guards on ``.github/workflows/auto-merge.yml``. The job has no checkout step,…, --admin here would merge the major bumps dependabot-auto-merge holds back., test_dependabot_prs_are_left_to_their_own_workflow(), tests/test_daily_automation_2026_09_22.py — Daily automation tests…, _brain_config_source() (+27 more)

### Community 87 - "clear_cooldowns"
Cohesion: 0.05
Nodes (49): clear_cooldowns(), _dead_model_key(), _is_model_dead(), is_provider_on_cooldown(), mark_provider_failed(), Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown., Clear all cooldown entries (useful for testing). Delegates to… (+41 more)

### Community 88 - "resolve_active_brain"
Cohesion: 0.07
Nodes (54): 1. Render (production backend + worker), BrainResolution, provider_api_key(), Return the live API key for *provider* (env-only — never persisted)., get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache() (+46 more)

### Community 89 - "WebsiteScanner"
Cohesion: 0.03
Nodes (70): pytest_asyncio, main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), Autonomous AI Agency Services, OnboardingService, Detect the Git provider from a repository URL. Args: repo_url: Repository URL… (+62 more)

### Community 90 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (67): BusinessCategory, Find a website by its URL., Find a repository by its URL., A company website with detected systems and stack inference., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo (+59 more)

### Community 91 - "MCPClient"
Cohesion: 0.08
...
### Community 94 - "portfolio_intelligence.py"
Cohesion: 0.07
Nodes (40): generate_standup(), Build a :class:`StandupReport` from ``.claude/state/active-tasks.md``. Reads…, InitiativeStatus, _bug_scores(), _clean(), _default_repo(), _env_github_token(), estimate_job_size() (+32 more)
...
### Community 96 - "v3_auth.py"
Cohesion: 0.08
Nodes (41): _get_admin_email(), _get_admin_name(), _get_admin_secret(), login(), LoginRequest, LoginResponse, BaseModel, post (+33 more)

### Community 97 - "SelfHealingAgent"
Cohesion: 0.06
Nodes (40): HealingEvent, _now(), Any, Translate external failure signals into improvement tasks and verify the fix…, Launch the background sweeper that resolves quiet verifying heals., Called when a GitHub issue with a bug label is opened., Record that an error with *signature* was just seen again. Called by the…, Signal that the fix for *signature* has been applied/merged. Transitions the… (+32 more)

### Community 98 - "_build_payload_or_500"
Cohesion: 0.18
...
### Community 103 - "agent/models.py"
Cohesion: 0.06
Nodes (35): Agent subsystem — planner / executor / verifier loop., AgentEvent, AgentRunRequest, AgentSessionCreateRequest, AgentSessionMessage, AgentStep, _known_tool_names(), Any (+27 more)

### Community 104 - "TestClient"
Cohesion: 0.10
Nodes (29): bare_repo(), _call(), _data(), git_config_env(), _is_error(), mcp_workspace_root(), Path, skipif (+21 more)

### Community 105 - "test_rate_limiter.py"
Cohesion: 0.09
...
### Community 106 - "test_shared_state.py"
Cohesion: 0.04
Nodes (37): claim(), cooldown_clear(), cooldown_get(), cooldown_set(), _get_backend(), incr_window(), _InMemoryBackend, Shared-state abstraction — in-memory (default) and Redis backends. Provides… (+29 more)

### Community 107 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.05
...
### Community 109 - "seo_api.py"
Cohesion: 0.06
Nodes (47): build_seo_roadmap(), delegate_seo_findings(), _expire_stale_pending_report(), export_seo_audit(), get_seo_audit(), list_seo_audits(), plan_seo_sprint(), BaseModel (+39 more)

### Community 110 - "test_sam_voice.py"
Cohesion: 0.04
Nodes (55): get_sam(), Any, SAM voice agent — the voice-controlled interface to the agency., Process a voice command and return SAM's spoken response. Args: text: The…, Call the NVIDIA NIM LLM (free tier) for SAM's response., Rule-based fallback when the LLM is unavailable., A single voice conversation session with SAM., SamAgent (+47 more)

### Community 111 - "seo_portfolio_bridge.py"
Cohesion: 0.06
...
### Community 112 - "AgentPlan"
Cohesion: 0.04
Nodes (66): free_nvidia_models(), FreeBuffAgent, _nvidia_api_key(), Return the curated list of free NVIDIA NIM models FreeBuff may use., Codebuff-style coding agent pinned to free NVIDIA NIM models. FreeBuff is a…, List the free NVIDIA NIM models a user may pick (e.g. via Telegram)., True when *model* is in the curated free NVIDIA NIM set., Coerce *requested* to a free NVIDIA model. Returns *requested* when it is… (+58 more)

### Community 113 - "gsap.min.js"
Cohesion: 0.05
Nodes (28): Context(), Db(), Eb(), fb(), Hc(), ia(), Ic(), ie() (+20 more)

### Community 114 - "looks_like_secret_file"
Cohesion: 0.06
Nodes (75): onboarding_gate_enabled(), Async read of the gate default straight from the DB., seed_default_providers(), _get_cached_tasks(), _get_tasks_cache_lock(), Lock, Return cached task dicts; fetches + caches from TaskStore when stale. Uses…, Removed (+67 more)
...
### Community 117 - "test_procedural_memory.py"
Cohesion: 0.05
Nodes (24): get_procedural_memory(), _overlap_score(), ProceduralMemoryStore, ProceduralRecord, Any, Store a successful step pattern and return its record id. Duplicate step…, Return all stored records (for admin/debug)., Remove all in-memory records. Returns the count removed. (+16 more)

### Community 118 - "main"
Cohesion: 0.07
Nodes (44): append_checkpoint(), cmd_audit(), cmd_changelog_check(), cmd_logs(), cmd_manifest(), cmd_resume(), cmd_start(), cmd_status() (+36 more)

### Community 119 - "KnowledgeScreen.jsx"
Cohesion: 0.06
Nodes (26): consultExecutives(), createWikiPage(), delegateSeoFindings(), deleteSource(), getSeoAudit(), ingestSource(), listExecutives(), listSeoAudits() (+18 more)

### Community 120 - "Killer TODO Roadmap — local-llm-server"
Cohesion: 0.03
Nodes (56): ★1 — 3-Phase Context-Pruner Middleware [P0] [CBF], ★2 — Specialized Sub-Agents with Per-Role Cheap Models [P0] [CBF + HRM], ★3 — Reasoning Token Budget + Toggle [P0] [NVD], ★4 — Skill/Procedural Memory (agentskills.io compatible) [P1] [HRM], ★5 — Sandboxed Agent Execution (E2B / Docker micro-VM) [P1] [CHM] ✅ Delivered 2026-07-04, ★6 — Cost Analytics + FTS5 Shared Memory + Agent Constitution [P1] [AOS], ★7 — Adaptive Loop Halting (Early Exit on High Confidence) [P1] [MYT + HRM], A1 — Hermes ChatML Prompt Format for Tool Calling [P0] [HRM] (+48 more)
...
### Community 123 - "test_telegram_freebuff.py"
Cohesion: 0.07
...
### Community 125 - "BrowserSession"
Cohesion: 0.05
Nodes (30): BrowserAction, BrowserSession, _not_started(), PageState, Any, agent/browser.py — Browser Automation Controls a real browser via Playwright so…, Which backend a start() would use: 'browserbase', 'local', or 'stub'., Evaluate a JavaScript expression in the page context. (+22 more)
...
### Community 129 - "REWRITE_PLAN.md — Phased Migration Strategy"
Cohesion: 0.06
...
### Community 134 - "redact_connection_url"
Cohesion: 0.17
...
### Community 139 - "Provider"
Cohesion: 0.08
Nodes (20): NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, Ollama provider adapter — local LLM inference., packages.ai — provider abstraction, model registry, and failover manager., ProviderManager, Any, packages/ai/manager.py — ProviderManager. Single entry point for all LLM calls.…, Coordinates provider selection, failover, and health., Return providers sorted by priority (lowest = highest priority). (+12 more)

### Community 141 - "test_colibri_brain_shim.py"
Cohesion: 0.05
...
### Community 142 - "get_task_store"
Cohesion: 0.06
Nodes (60): quick_notes_submit(), _QuickNoteBody, Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover() (+52 more)

### Community 143 - "test_llm_router_e2e.py"
Cohesion: 0.09
...
### Community 145 - "local_brain_router.py"
Cohesion: 0.19
...
### Community 146 - "test_trend_watcher.py"
Cohesion: 0.05
...
Cohesion: 0.12
Nodes (13): Checkpoint, CheckpointStore, Any, Path, Persist a checkpoint to disk. Returns the file path., Return the latest checkpoint for a session, or None., Return all checkpoint files for a session, sorted by step index., Remove all checkpoints for a session. (+5 more)

### Community 149 - "AgentJobManager"
Cohesion: 0.06
Nodes (26): AgentJob, AgentJobManager, heartbeat(), _now(), Any, Run a job using the provided runner and update the job's lifecycle, progress,…, Serialize the AgentJob to a JSON-serializable dictionary for external clients.…, Runtime Integration (+18 more)

### Community 150 - "test_persistent_memory.py"
Cohesion: 0.06
...
### Community 152 - "PatternConsolidation"
Cohesion: 0.08
...
### Community 153 - "RenderFinding"
Cohesion: 0.05
Nodes (25): _latest_metric_value(), _parse_timestamp(), Any, datetime, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Pull the most recent numeric sample out of a ``get_metrics`` payload. Render…, Snapshot for ``/api/render/ops/status`` and the dashboard. ``self_heal_ready``…, Run one full scan and return every finding, filed or not. Never raises: a… (+17 more)
...
### Community 155 - "test_brain_availability_doctor.py"
Cohesion: 0.10
...
### Community 157 - "test_brain_failover.py"
Cohesion: 0.09
...
### Community 160 - "Fixed"
Cohesion: 0.05
Nodes (52): build_planning_prompt(), Fixed, CreateIssue, Fixed, Highest-priority configured provider, or ``None`` if none is set. Shares…, _select_brain(), brain_candidates(), call_brain_with_failover() (+44 more)
...
### Community 162 - "SpecEntry"
Cohesion: 0.11
Nodes (23): build_block(), Path, Absolute path of the harness spec for a workspace., Parse existing entries. Never raises — a broken file yields no entries. With…, Rewrite the spec file, preserving any non-entry (hand-written) lines., Compact prompt block of standing instructions, or '' when there are none.…, One standing instruction plus the evidence that earned it., read_entries() (+15 more)

### Community 163 - "QuickNoteQueue"
Cohesion: 0.09
Nodes (32): _fetch_text(), __init__(), _now(), process_note(), Any, Path, QuickNote, QuickNoteQueue (+24 more)

### Community 164 - "probe_catalogues.py"
Cohesion: 0.11
Nodes (33): Fixed, Fixed, _auth_headers(), _chat_targets(), _dump_matching(), _kind(), list_models(), _list_or_report() (+25 more)

### Community 165 - "test_spec_store.py"
Cohesion: 0.06
Nodes (39): build_spec_router(), approve_spec(), _decide(), get_spec_artifact(), list_spec_artifacts(), reject_spec(), Any, APIRouter (+31 more)

### Community 166 - "DashboardScreen.jsx"
Cohesion: 0.08
Nodes (9): Charts, Donut(), ExecutionTimeline(), Sparkline(), ErrorBoundary, DashboardScreen(), fmtTokens(), relTime() (+1 more)

### Community 167 - "WorkflowBuildRequest"
Cohesion: 0.06
Nodes (36): Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory, do_approve() (+28 more)

### Community 168 - "models/seo_audit.py"
Cohesion: 0.09
...
### Community 169 - "test_trend_scoping.py"
Cohesion: 0.08
Nodes (54): Issue title: the failure mode plus how hard it is recurring., _company_attr(), company_stack_tags(), extract_stack_tags(), fan_out_trend(), fan_out_trends(), is_code_change_trend(), map_trend_to_company_task() (+46 more)

### Community 170 - "portfolio_api.py"
Cohesion: 0.08
...
### Community 171 - "tasks/api.py"
Cohesion: 0.12
Nodes (60): BackgroundTasks, add_comment(), approve_checkpoint(), approve_execution(), clarify_task(), create_task(), _current_user(), delete_task() (+52 more)

### Community 172 - "diagnostics.py"
Cohesion: 0.07
...
Cohesion: 0.05
Nodes (32): CompanyAgencyService, get_company_agency_service(), _is_runtime_available_sync(), _pick_available_runtime(), Any, SpecialistFamily, services/company_agency.py — Company Agency Orchestration Service After…, Orchestrates specialist activation, runtime startup, and 24x7 scheduling for a… (+24 more)

### Community 175 - "AgentScheduler"
Cohesion: 0.03
Nodes (62): gc, AgentScheduler, _now(), Any, Reconstruct a ScheduledJob from its as_dict() output., Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Register a new job. Returns the created :class:`ScheduledJob`.…, Fire a job immediately (webhook / manual trigger). (+54 more)

### Community 176 - "_FakeCommandResult"
Cohesion: 0.20
Nodes (4): _FakeCommandResult, _FakeCommands, _FakeFiles, counting_run()

### Community 177 - "check_container_posture.py"
Cohesion: 0.26
...
### Community 179 - "test_e2b_task_wiring.py"
Cohesion: 0.08
...
### Community 180 - "activation.py"
Cohesion: 0.08
Nodes (42): activation_required(), ActivationResult, _b64url_decode(), _b64url_encode(), _decode_jwt_unverified(), _generate_token_for_owner(), get_or_create_instance_id(), instance_id() (+34 more)

### Community 181 - "TaskDispatcher"
Cohesion: 0.06
Nodes (25): Fixed, Fixed, Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, Record the human decision on a task's **pre-execution** approval gate. A…, _make_task(), asyncio (+17 more)

### Community 182 - "harness_routing.py"
Cohesion: 0.14
...
### Community 183 - "test_hermes_in_process.py"
Cohesion: 0.06
...
### Community 184 - "NotificationDispatcher"
Cohesion: 0.05
...
### Community 185 - "._log_event"
Cohesion: 0.08
Nodes (23): N4. Promote CRISPY from EXPERIMENTAL → stable after burn-in 🟡 (size: M, risk: medium — `risky-module-review`), Any, Connection, Path, PhaseType, Append an event to the workflow event log., Create a new WorkflowRun and begin pre-gate phase execution. The run is…, Return current WorkflowRun snapshot or None. (+15 more)

### Community 186 - "activation_api.py"
Cohesion: 0.09
...
### Community 188 - "PerformanceAnalytics"
Cohesion: 0.12
...
### Community 190 - "chat_handlers.py"
Cohesion: 0.11
...
### Community 191 - "sync/service.py"
Cohesion: 0.06
Nodes (48): Request, FastAPI dependency: require Power User or Admin role. Raises 403 otherwise., require_power_user(), sync/ — Syncthing-style workspace synchronisation service., add_peer(), get_folder_index(), get_sync_file(), get_sync_service() (+40 more)

### Community 192 - "_mock_provider_records"
Cohesion: 0.12
...
### Community 194 - "WorkflowEngine"
Cohesion: 0.08
Nodes (52): get_tracker(), Return the shared AutonomyTracker singleton (lazy-init)., tests/test_contracts_agency.py — Contract tests for agency core autonomy…, approve(), build(), cancel(), _engine(), get_agent_team() (+44 more)

### Community 195 - "PortfolioManager"
Cohesion: 0.07
...
### Community 196 - "test_catalogue_probe.py"
Cohesion: 0.03
Nodes (72): date, .github/scripts/provider_policy.py — Read the durable provider policy from the…, Reset the cached policy (test helper)., reset_cache(), caption_tracks(), extract_player_response(), fetch_transcript(), _get() (+64 more)

### Community 197 - "test_platform_controls.py"
Cohesion: 0.10
...
### Community 198 - "test_pr923_fixes.py"
Cohesion: 0.05
Nodes (38): _deferred_startup_cleanup(), cleanup_stale_jobs(), _is_stale(), nuclear_cleanup(), Any, Remove a job from the store. Returns True on success, False on failure. Logs…, Directly delete ALL stale jobs from the DB collection. More aggressive than…, Check if a created_at timestamp is older than ttl_seconds. Handles multiple… (+30 more)

### Community 199 - "OllamaCircuitBreaker"
Cohesion: 0.08
...
### Community 201 - "test_daily_2026_07_27.py"
Cohesion: 0.07
...
### Community 202 - "user_research_skill.py"
Cohesion: 0.08
...
Cohesion: 0.08
Nodes (12): Any, Path, Run a shell command inside the workspace via an explicit shell binary., Resolve rel against root, reject path traversal., Run a subprocess. Never uses shell=True., Manages a single isolated workspace directory., Canonical root path (follows macOS /var → /private/var symlinks)., Clone repo_url into this workspace. Injects token from env if available. (+4 more)
...
### Community 206 - "test_classify_dependabot_update.py"
Cohesion: 0.07
...
### Community 208 - "PromptCacheManager"
Cohesion: 0.06
...
### Community 209 - "test_repo_connection.py"
Cohesion: 0.04
Nodes (86): Completed Task Archive — June to August 2026, A. Connection & credentials, Autonomous SDLC Loop (Agency Core, repo-agnostic), B. Provider & host, C. Delivery / branch policy  *(detected — see DeliveryPolicy)*, Companies without a connected repo (URL-only onboarding), D. CI / checks, Design principle: repo-agnostic, not GitHub-Actions-bound (+78 more)

### Community 210 - "ApprovalCheckpoint"
Cohesion: 0.18
...
### Community 211 - "test_local_controller.py"
Cohesion: 0.06
...
### Community 213 - "Implementation Plan — DB-persisted, UI-switchable Brain (no redeploy)"
Cohesion: 0.14
...
Cohesion: 0.05
Nodes (54): create_access_token(), create_refresh_token(), get_current_user(), get_optional_user(), github_callback(), github_repo_access(), github_repo_callback(), google_callback() (+46 more)

### Community 215 - "register_webui"
Cohesion: 0.07
Nodes (41): allow_paid_brain(), True only when the operator explicitly opted into a paid (Anthropic) brain.…, test_allow_paid_brain_default_false(), test_allow_paid_brain_opt_in(), Any, Path, run_command(), _admin_out() (+33 more)

### Community 216 - "SyntheticDataPipeline"
Cohesion: 0.07
...
Cohesion: 0.07
Nodes (25): WorkspaceStatusLiteral, Retrieve the WorkspaceManifest for a given session and optional job. Looks up…, List all known workspaces, optionally filtered by status., Mark a workspace as active (in-use)., Pause a workspace (e.g. between agent steps)., Mark a workspace as completed., Mark a workspace as failed., Mark a workspace as cancelled (via cancelling -> cancelled). (+17 more)

### Community 220 - "PersistentMemoryStore"
Cohesion: 0.07
...
### Community 224 - "cmd_loops"
Cohesion: 0.22
Nodes (10): cmd_loops(), _grade_icon(), Loop Engineering fleet readiness + the costliest loops, from /api/loops., tests/test_telegram_observe.py Tests for the read-only "observe from Telegram"…, _run(), test_cmd_autonomy_degrades_gracefully(), test_cmd_autonomy_formats_brain_and_readiness(), fake_get() (+2 more)

### Community 225 - "emit_chat_observation"
Cohesion: 0.08
Nodes (38): observability_diag_public(), PUBLIC diagnostic endpoint for Langfuse — no auth required. Returns exactly…, CommercialEquivalent, estimate_commercial_equivalent_usd(), get_prices(), _load_from_env(), _parse_mapping(), Any (+30 more)
...
### Community 227 - "workflow_orchestrator.py"
Cohesion: 0.09
Nodes (36): contextvars, BoundContext, ExecutionResult, JudgeVerdict, MergeDecision, MonitorOutput, _orchestrator_bypass(), PersistOutput (+28 more)

### Community 228 - "test_agent_tool_governance.py"
Cohesion: 0.09
...
### Community 229 - "TaskBoardScreen.jsx"
Cohesion: 0.08
Nodes (22): clarifyTask(), escalateTask(), followUpTask(), HEALTH_COLORS, HEALTH_DOTS, LIFECYCLE_STAGES, priorityColors, relTime() (+14 more)

### Community 230 - "MetricsRegistry"
Cohesion: 0.09
...
### Community 236 - "NIMConnectionPool"
Cohesion: 0.07
Nodes (23): [5.0.0], Removed, Security, [5.0.0], Removed, Security, NIMConnectionPool, Any (+15 more)

### Community 237 - "LLM Router — architecture"
Cohesion: 0.08
Nodes (20): Bulkheads, Circuit breaker, Configuration, Context management, LLM Router — architecture, Request lifecycle, The three failure scopes, What problem it solves (+12 more)

### Community 238 - "WorkflowPhase"
Cohesion: 0.13
...
### Community 239 - "Persistent Memory System"
Cohesion: 0.05
...
### Community 243 - "ExecutiveAdvisory"
Cohesion: 0.10
Nodes (17): ExecOpinion, Executive, ExecutiveAdvisory, Grounding, Any, Evidence gathered for a question before the executives answer., One executive's answer to the question., Routes a business question to the right executives and synthesises them. (+9 more)

### Community 244 - "verify_service_token"
Cohesion: 0.14
...
### Community 246 - "467 Brutal Audit — File-by-File Status"
Cohesion: 0.05
...
### Community 247 - "ProvidersScreen.jsx"
Cohesion: 0.06
Nodes (25): getBrainProviders(), getLocalBrainState(), getRenderHealth(), getRenderOpsStatus(), postLocalBrainToggle(), runRenderOpsScan(), setBrainProviderEnabled(), syncProviderToRender() (+17 more)

### Community 248 - "test_control_plane_api.py"
Cohesion: 0.06
...
### Community 250 - "test_sam_livekit.py"
Cohesion: 0.04
Nodes (67): Report whether the SAM realtime voice (LiveKit) transport is configured., sam_livekit_status_backend(), 1. Get LiveKit credentials, 2. Configure the backend (Render env vars), 3. The SAM voice worker, 4. Talk to SAM, SAM Realtime Voice over LiveKit, Setup (+59 more)

### Community 251 - "LogMonitor"
Cohesion: 0.08
Nodes (35): _dispatch_async(), _run(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, LogRecord, Register the error capture handler on the root logger., Best-effort: hand an operational failure to the incident tracker. Import is… (+27 more)

### Community 252 - "AgentSessionStore"
Cohesion: 0.10
...
### Community 253 - "TestHarnessAdapter"
Cohesion: 0.06
...
Cohesion: 0.07
Nodes (37): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled_cached() (+29 more)

### Community 255 - "model_router.py"
Cohesion: 0.06
Nodes (34): Dynamic model router package. Public API:: from router import get_router,…, _build_builtin_model_map(), _default_model(), ModelRouter, _nvidia_key_present(), Any, Dynamic model router. Central routing logic for all chat and agent requests.…, Build the built-in alias table — Nvidia NIM models when key is set, local… (+26 more)

### Community 256 - "governance.py"
Cohesion: 0.10
...
### Community 257 - "analyze"
Cohesion: 0.14
Nodes (13): Analysis, analyze(), _classify(), _failure_table(), main(), Return the exhaustion :class:`Analysis` for a run's captured *output*., scripts/analyze_exhaustion.py Decide whether a run that ran out of retries has…, The verdict plus the human-readable justification behind it. (+5 more)

### Community 258 - "_llm_catalog"
Cohesion: 0.07
...
### Community 259 - "llm_providers.py"
Cohesion: 0.13
...
### Community 260 - "Kept Rules — the 44 that survive the audit"
Cohesion: 0.05
...
### Community 261 - "test_background_services.py"
Cohesion: 0.07
...
### Community 262 - "brain_failover.py"
Cohesion: 0.11
...
### Community 263 - "CEOLedger"
Cohesion: 0.08
Nodes (21): _backend(), CEOLedger, GoalRecord, _now(), Any, One piece of work the CEO has taken ownership of. ``state`` is the supervisor's…, Return ``(done, total)`` subtask counts., Durable store for :class:`GoalRecord`s. Every method is best-effort and never… (+13 more)

### Community 264 - "OrchestratorQueue"
Cohesion: 0.07
...
### Community 266 - "test_purge_backlog.py"
Cohesion: 0.08
...
### Community 267 - "test_telegram_webhook.py"
Cohesion: 0.06
...
### Community 268 - "monitor_colibri.py"
Cohesion: 0.08
...
### Community 270 - "probe_model_liveness"
Cohesion: 0.07
Nodes (37): _describe_http_status(), probe_model_liveness(), _probe_ollama(), _probe_openai_compat(), ProbeResult, BaseModel, Probe an OpenAI-compatible provider (Cerebras / Groq / NIM / Z.ai / Aerolink)., Probe local Ollama via GET /api/tags (cheap) then a 5-token completion. (+29 more)

### Community 271 - "TestCatalogFable51"
Cohesion: 0.09
...
### Community 273 - "local_controller.py"
Cohesion: 0.13
...
### Community 275 - "anthropic_compat.py"
Cohesion: 0.08
Nodes (33): _build_anthropic_response(), _content_block_to_text(), _emit_safely(), _finish_reason_to_stop_reason(), get_local_model(), handle_anthropic_messages(), _messages_to_openai(), _openai_choice_to_anthropic_content() (+25 more)

### Community 276 - "LLM Router — troubleshooting"
Cohesion: 0.08
...
### Community 280 - "Part A — CodeRabbit review fixes for this PR (do first, small)"
Cohesion: 0.07
...
### Community 284 - "keys.py"
Cohesion: 0.07
Nodes (26): _digest(), get_ring(), KeyRing, KeyState, packages/llm/keys.py — multi-key rotation with per-key health. A provider may…, Per-provider round-robin key selection with cooldowns., Return the next healthy key and its index. Returns ``(None, -1)`` when every…, Cool a key down and return the cooldown length in seconds. Consecutive limits… (+18 more)

### Community 285 - "test_provider_enable_disable.py"
Cohesion: 0.07
...
### Community 286 - "pr_approval_gate.py"
Cohesion: 0.14
...
### Community 288 - "agent_runtime.py"
Cohesion: 0.09
...
### Community 289 - "test_daily_2026_06_04.py"
Cohesion: 0.10
...
### Community 290 - "test_issue_intake.py"
Cohesion: 0.08
...
### Community 291 - "test_all_providers_discovery.py"
Cohesion: 0.17
...
### Community 294 - "_resolve_brain_provider"
Cohesion: 0.04
Nodes (54): ProviderUpdate, Decisions (locked with the owner), Design: one UI-controlled Provider Policy (single source of truth), Free NVIDIA brain + UI-controlled provider policy + no silent spend, Implementation plan + TO-DO (check off as you go), Open-PR / issue disposition (read + acted on), Phase 1 — Stop the bleeding + paid kill switch (do first, ship alone if needed), Phase 3 — Persistence hardening (issues #537, #524) (+46 more)

### Community 295 - "test_connector_registry.py"
Cohesion: 0.07
Nodes (20): Fixed, _connectors(), ConnectorSpec, get_connector(), list_connectors(), Any, Execute the ``webhook`` connector: POST *payload* as JSON to *url*. Fails soft…, A declared connector, independent of whether it is currently usable.… (+12 more)

### Community 296 - "Platform Guide — the full tour"
Cohesion: 0.04
Nodes (51): 1. Clone and install, 2026-06-16, 2026-06-25, 2026-06-26, 2026-07-04, 2026-07-05, 2026-07-09, 2. Configure (+43 more)

### Community 297 - "ImprovementLoop"
Cohesion: 0.08
Nodes (34): DetectedIssue, ImprovementLoop, ImprovementLoopState, IssueCategory, IssueSeverity, _now(), Any, Enum (+26 more)

### Community 298 - "platform_controls_router.py"
Cohesion: 0.15
...
### Community 299 - "scrub"
Cohesion: 0.07
Nodes (26): _luhn_ok(), True when ``digits`` (bare, no separators) satisfies the Luhn checksum., Redact SSNs and Luhn-valid payment-card numbers from a string. Runs after the…, Recursively strip secrets from an arbitrary argument structure. Fails…, Redact secret-shaped substrings, then truncate., _redact_pii(), _card(), scrub() (+18 more)
...
### Community 301 - "test_live_server.py"
Cohesion: 0.22
...
### Community 302 - "test_anthropic_router.py"
Cohesion: 0.10
Nodes (9): _make_anthropic_provider(), _payload(), Tests for Anthropic-specific router features. Covers: - Prompt caching…, Adaptive-thinking Claude models 400 on temperature / legacy thinking. The…, TestAnthropicPayloadExtendedThinking, TestAnthropicPayloadModelGuards, TestAnthropicPayloadPromptCaching, TestAuthHeadersExtendedThinking (+1 more)

### Community 303 - "test_v3_auth.py"
Cohesion: 0.17
...
### Community 304 - "_routing_candidates"
Cohesion: 0.09
...
### Community 305 - "ContextCompressor"
Cohesion: 0.11
...
### Community 307 - "Dynamic Model Routing"
Cohesion: 0.11
...
### Community 308 - "test_llm_router_disabled.py"
Cohesion: 0.09
...
### Community 309 - "test_chat_mode_regressions.py"
Cohesion: 0.11
...
### Community 310 - "test_kimi_bridge_server.py"
Cohesion: 0.07
...
### Community 312 - "test_e2b_data_flow.py"
Cohesion: 0.06
...
Cohesion: 0.13
Nodes (14): _FakeMemory, asyncio, Return an async llm that yields queued replies and records calls., _recording_llm(), test_advise_consults_selected_execs_and_synthesizes(), test_advise_is_fail_soft_when_a_voice_errors(), test_advise_single_exec_needs_no_synthesis(), test_advise_unknown_role_is_dropped() (+6 more)

### Community 316 - "test_microagents.py"
Cohesion: 0.15
...
### Community 317 - "get_failover_manager"
Cohesion: 0.07
...
### Community 318 - "test_task_source_id_race.py"
Cohesion: 0.14
...
### Community 319 - "Langfuse Observability Guide"
Cohesion: 0.06
...
### Community 320 - "v3_models.py"
Cohesion: 0.10
Nodes (40): _get_current_user, _get_bearer_token(), _get_current_user(), logout(), Depends, get, Extract and validate current user from token., Get current authenticated user. (+32 more)

### Community 321 - "control_overrides.py"
Cohesion: 0.14
...
### Community 322 - "extract_failures"
Cohesion: 0.21
...
### Community 323 - "WorkflowOrchestrator"
Cohesion: 0.04
Nodes (49): Return orchestrator queue depth, active runs, and supervisor state (#522)., workflow_orchestrator_status(), Changed, Changed, Nothing goes down quietly, The 24x7 agency — your agents never go idle, What runs automatically after onboarding, When something goes wrong, agents fix it — not you (+41 more)

### Community 324 - "test_workspace_isolation.py"
Cohesion: 0.16
...
### Community 325 - "RepowiseIntelligence"
Cohesion: 0.08
...
### Community 328 - "ResourceWatchdog"
Cohesion: 0.10
...
### Community 329 - "ApprovalStore"
Cohesion: 0.07
Nodes (25): ApprovalRequest, ApprovalStatus, ApprovalStore, Any, Enum, str, Bounded, in-process store of approval requests. Uses a :class:`threading.Lock`…, Register a pending approval and return it. Arguments are scrubbed on the way… (+17 more)

### Community 330 - "High-Agency Frontend Skill"
Cohesion: 0.06
Nodes (30): 10. FINAL PRE-FLIGHT CHECK, 1. ACTIVE BASELINE CONFIGURATION, 2. DEFAULT ARCHITECTURE & CONVENTIONS, 3. DESIGN ENGINEERING DIRECTIVES (Bias Correction), 4. CREATIVE PROACTIVITY (Anti-Slop Implementation), 5. PERFORMANCE GUARDRAILS, 6. TECHNICAL REFERENCE (Dial Definitions), 7. AI TELLS (Forbidden Patterns) (+22 more)

### Community 331 - "_resolve_user_github_token"
Cohesion: 0.10
Nodes (28): _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor(), Consolidated system health report: preflight checks + runtime health. Returns a…, Public Doctor endpoint — no authentication required. Returns system-level…, Authenticated Doctor endpoint — full diagnostics. Returns all system-level… (+20 more)

### Community 332 - "resolve_provider_for"
Cohesion: 0.06
Nodes (35): _get_provider_policy(), ProviderPolicyUpdate, Read the durable provider policy from DB, falling back to a safe default.…, Editable subset of the durable provider policy (paid-provider kill switch)., Resolve the LLM endpoint for a named surface (task/chat/ceo/sdlc/…). Honours…, _prio(), resolve_provider_for(), asyncio (+27 more)

### Community 333 - "TestNormalizeResponseFormat"
Cohesion: 0.11
...
### Community 334 - "Quick-Note GitHub Issues Processing - Session Summary"
Cohesion: 0.06
...
### Community 335 - "DistributedRateLimiter"
Cohesion: 0.09
...
### Community 342 - "compare_runtimes.py"
Cohesion: 0.16
...
### Community 343 - "triage_orphaned_context_prs.py"
Cohesion: 0.11
...
### Community 344 - "AnthropicProvider"
Cohesion: 0.07
Nodes (31): Added, Fixed, Added, Fixed, AnthropicProvider, Any, AsyncClient, Translate OpenAI-shaped messages into Anthropic's system/turn split. (+23 more)

### Community 345 - "test_agents.py"
Cohesion: 0.12
...
### Community 346 - "_redact_for_notification"
Cohesion: 0.11
...
### Community 351 - "test_rag_context.py"
Cohesion: 0.12
...
### Community 352 - "Added"
Cohesion: 0.08
Nodes (24): Ask the C-suite advisory layer a business question. This is the CEO's bridge…, _egress_policy_reason(), Return why governance policy blocks *host*, or None. Runs strictly *after* the…, Added, Current state (2026-09-24), Next Action, Next daily run (2026-09-25), Prior state (2026-09-23) (+16 more)

### Community 353 - "StuckDetector"
Cohesion: 0.14
...
### Community 355 - "SteeringInjector"
Cohesion: 0.11
...
### Community 356 - "test_claude_setup_audit.py"
Cohesion: 0.16
...
### Community 358 - "test_ceo_router.py"
Cohesion: 0.13
...
### Community 359 - "test_session_retro.py"
Cohesion: 0.13
...
### Community 360 - "test_internal_agent_did_work.py"
Cohesion: 0.12
...
### Community 361 - "_captured_request_headers"
Cohesion: 0.12
...
### Community 362 - "test_commit_tracker.py"
Cohesion: 0.18
...
### Community 367 - "test_iteration_7_features.py"
Cohesion: 0.21
Nodes (11): codeql_count(), dependabot_count(), main(), Any, OpenClaw security fix helper. Lightweight CLI used by CI to check/fix…, _repo_parts(), _request(), requests (+3 more)

### Community 368 - "Python Dependencies (`requirements.txt`)"
Cohesion: 0.07
...
### Community 369 - "build_render_router"
Cohesion: 0.13
...
### Community 370 - "_ensure_tasks_source_id_unique_index"
Cohesion: 0.10
...
### Community 371 - "RateLimitTracker"
Cohesion: 0.10
...
### Community 372 - "Sampling Strategies Internals"
Cohesion: 0.09
...
### Community 373 - "ProviderConsole.jsx"
Cohesion: 0.08
...
### Community 374 - "validate_outbound_url"
Cohesion: 0.13
...
### Community 379 - "test_daily_automation_2026_09_21.py"
Cohesion: 0.09
...
### Community 381 - "ProjectScaffolder"
Cohesion: 0.14
...
### Community 382 - "ScheduleStore"
Cohesion: 0.09
Nodes (21): _backend(), _json_default(), Any, Return all persisted schedule docs (for boot rehydration)., Persist (insert or update) a single schedule by job_id., Delete a persisted schedule., Fallback JSON encoder for schedule docs (datetimes, sets, etc.)., Durable schedule persistence. Backend is chosen from ``STORAGE_BACKEND``: *… (+13 more)

### Community 383 - "_require_admin"
Cohesion: 0.06
Nodes (31): clear_cost_attribution(), clear_response_cache(), get_brain_policy_route(), get_catalog_models_route(), patch_brain_policy_route(), patch, FastAPI dependency: resolve the caller and 403 unless they are admin., Persist the durable provider policy and return the new state. Also writes to… (+23 more)

### Community 384 - "Claude Code + Qwen Local Setup"
Cohesion: 0.07
...
### Community 385 - "Configuration Reference"
Cohesion: 0.06
Nodes (34): Any, Return recent commits with agent attribution trailers parsed out., Agent governance — identity, policy, approvals, audit, sandboxes, Agent Models, Anthropic API Compatibility / Claude Code, Authentication and Keys, Claude Code setup, Configuration Reference (+26 more)

### Community 386 - "Docker Agent Runtimes Setup"
Cohesion: 0.07
...
### Community 388 - "test_anthropic_refusal_fallback.py"
Cohesion: 0.19
...
### Community 391 - "VoiceCommandInterface"
Cohesion: 0.14
...
### Community 392 - "Performance Analysis — local-llm-server"
Cohesion: 0.08
...
### Community 393 - "Technical Debt Register — local-llm-server"
Cohesion: 0.08
...
### Community 397 - "test_daily_automation_2026_08_03.py"
Cohesion: 0.11
...
### Community 398 - "ensure_self_company"
Cohesion: 0.08
Nodes (22): Fire-and-forget self-bootstrap; never blocks or crashes startup., _schedule_self_bootstrap(), _create_company_directly(), ensure_self_company(), _find_self_company(), Return the existing self company (matched by domain), or None., Fallback: create the company directly via the graph service. Used when…, Idempotently register the platform as its own company and kick off the agency.… (+14 more)

### Community 399 - "test_workflow_shell_vars_are_declared.py"
Cohesion: 0.10
...
### Community 401 - "record_step_failures"
Cohesion: 0.14
...
### Community 402 - "service_daemon.py"
Cohesion: 0.06
...
### Community 405 - "test_phase6_workflow.py"
Cohesion: 0.15
...
### Community 410 - "CEO Micro-Management"
Cohesion: 0.13
...
### Community 411 - "Session Handoff — 2026-06-15"
Cohesion: 0.08
...
### Community 412 - "ProviderRouter"
Cohesion: 0.03
Nodes (117): Added, Added, _acquire_provider_probe(), _exponential_backoff_cooldown(), extract_openai_text(), _get_director(), is_commercial_provider(), _normalize_nvidia_base_url() (+109 more)

### Community 414 - "e2e/test_browser.py"
Cohesion: 0.21
...
### Community 415 - "test_activation_api.py"
Cohesion: 0.13
...
### Community 416 - "test_service_token.py"
Cohesion: 0.08
...
### Community 417 - "register_admin_gui"
Cohesion: 0.14
...
### Community 418 - "capability_registry.py"
Cohesion: 0.12
Nodes (11): browse_page(), Open *url* in a real browser, return its rendered text, and close. Fail-soft:…, get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, Register the built-in agent tools that are always available., Register the interactive-browser capability (agent/browser.py). Unlike…, _register_browser_tools(), _browse_page_tool() (+3 more)

### Community 419 - "TestAuthAndTaskOwnership"
Cohesion: 0.06
...
### Community 422 - "Local AI Stack with Docker"
Cohesion: 0.08
...
### Community 423 - "get_scheduler"
Cohesion: 0.09
Nodes (30): legacy_scheduler_delete(), legacy_scheduler_get(), legacy_scheduler_list(), legacy_scheduler_trigger(), _produce_scheduler_jobs(), Called by Cloudflare Cron every minute. Protected by CRON_SECRET header., Force-dedup and clean stale schedules from the durable store. Admin-only…, scheduler_force_cleanup() (+22 more)

### Community 424 - "Implementation Prompt: Rich TaskBoard + Agile Sprint Integration"
Cohesion: 0.08
...
### Community 425 - "TASK 4 — End-to-end approval-gate test"
Cohesion: 0.08
...
### Community 427 - "test_agent_free_brain.py"
Cohesion: 0.05
Nodes (33): livenim, is_anthropic_model(), True when *model* names a paid Anthropic/Bedrock-Claude model. Covers native…, Resolve the free NVIDIA NIM brain from env, or ``None`` if unconfigured.…, resolve_free_nvidia_brain(), _FakeAsyncClient, _FakeResponse, _free_env() (+25 more)
...
### Community 430 - "Sol Advisor Orchestration"
Cohesion: 0.09
...
### Community 431 - "PriorityTaskQueue"
Cohesion: 0.14
...
### Community 432 - "TemporalContextGraph"
Cohesion: 0.10
...
### Community 433 - ".update_intelligence"
Cohesion: 0.08
...
### Community 434 - "HarnessEnrichment"
Cohesion: 0.14
...
### Community 437 - "build_governance_router"
Cohesion: 0.16
Nodes (28): build_governance_router(), approve(), _decide(), deny(), destroy_sandbox(), get_audit(), get_budget(), get_metrics() (+20 more)

### Community 438 - "test_backend_server_features.py"
Cohesion: 0.06
Nodes (21): _append_agent_session_message(), _build_auto_skill_guidance(), _mask_observations(), Path, Truncate tool/observation content in older messages to prevent context bloat., Swap localhost → `ollama` when running inside a container. Inside a container,…, _resolve_ollama_url(), _run_agent_loop() (+13 more)

### Community 439 - "openclaw_str_e_fix.py"
Cohesion: 0.15
...
### Community 440 - "Deploy: FreeBuff Telegram bot (24×7)"
Cohesion: 0.09
...
### Community 441 - "Workspace Isolation Architecture"
Cohesion: 0.15
...
### Community 442 - "Telegram Bot Setup"
Cohesion: 0.08
...
### Community 446 - "TrafficDirector"
Cohesion: 0.05
Nodes (43): provider_max_parallel(), provider_max_tpm(), _provider_positive_float(), provider_weight(), Shared parse/validate for the numeric per-provider traffic budgets. Returns…, Return the operator-configured tokens/min cap for *provider*. Reads…, Return the operator-configured share weight for *provider*. Reads…, Return the operator-configured in-flight request cap for *provider*. Reads… (+35 more)

### Community 447 - "test_phase4_runtime_resilience.py"
Cohesion: 0.11
Nodes (24): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., _make_task(), asyncio, tests/test_phase4_runtime_resilience.py Phase 4: runtime resilience tests.…, Tasks in TODO or DONE status are ignored by the reconciler., IN_PROGRESS tasks with pending_agent_run=True are NOT touched (already queued)., TaskDispatcher calls reconcile once before the polling loop starts. (+16 more)

### Community 448 - "AgentMessageBus"
Cohesion: 0.12
...
### Community 449 - "isolated_telegram_config"
Cohesion: 0.13
...
### Community 451 - "test_mostly_failed_steps.py"
Cohesion: 0.12
...
### Community 453 - "SkillRegistry"
Cohesion: 0.12
Nodes (9): Any, Path, Central registry that indexes local + remote skills and provides context-aware…, Return ranked skill recommendations based on tech stack, active workflow types,…, Force-refresh remote skills, bypassing TTL. Returns count added., Update the GitHub token used for authenticated API calls., SkillRegistry, SkillRegistry method signature enforcement. (+1 more)
...
### Community 457 - "checkpoint.py"
Cohesion: 0.11
Nodes (27): checkpoint_agent_state(), _checkpointing_enabled(), cleanup_checkpoints(), _get_checkpoint_store(), Durable agent checkpointing for crash-recovery and resumption. Provides…, Return the process-wide singleton CheckpointStore., Check whether durable checkpointing is enabled via env var., Save the current agent state as a durable checkpoint. Called at key lifecycle… (+19 more)

### Community 458 - "autonomous_fix.py"
Cohesion: 0.11
...
### Community 459 - "argparse"
Cohesion: 0.04
Nodes (60): argparse, CLI for the cost-aware routing evaluation. python -m evals.cost_aware_routing…, Backward-compatibility shim — use scripts/generate_api_key.py instead. This…, _as_list(), _clean(), convert(), _default_source(), _has_pattern() (+52 more)

### Community 460 - "WindowsServiceManager"
Cohesion: 0.19
...
### Community 461 - "test_render_mcp.py"
Cohesion: 0.12
Nodes (15): True when a filed finding will actually be scheduled as a fix. Detection and…, _self_heal_ready(), _env(), tests/test_render_mcp.py — Render MCP integration. Covers: - MCPClient…, The Go binary hardcodes :10000 and ignores $PORT, so Render must be told., In HTTP mode the token comes from the caller's header, so nothing here is…, The mux serves /mcp only; a health check there opens an SSE stream that never…, pkg/oauth fails the process at boot on a partial OAuth config. (+7 more)

### Community 462 - "TestLegacyRouterCacheTTL"
Cohesion: 0.13
...
### Community 463 - "test_p0_roadmap_b1_c2_a3.py"
Cohesion: 0.10
Nodes (14): _infer_parameters_from_func(), Infer a basic JSON Schema from a function's signature., _inject_tool_results_as_messages(), Inject tool call results as follow-up messages for multi-turn execution. When…, BaseModel, Result of a single reward model scoring operation., RewardScore, Score must be between 0.0 and 1.0. (+6 more)

### Community 464 - "test_executive_advisory_api.py"
Cohesion: 0.13
Nodes (16): AdviceResult, The full advisory response: per-executive opinions plus the synthesis., _make_app(), FastAPI, tests/test_executive_advisory_api.py — auth and behaviour for…, Install an advisory whose LLM is a canned reply — no network., stub_advisory(), test_consult_maps_advisory_failure_to_502() (+8 more)

### Community 465 - "test_admin_local_brain_router.py"
Cohesion: 0.15
...
### Community 466 - "V3 API Migration Plan — LLM Relay Platform"
Cohesion: 0.10
...
### Community 467 - "The fifteen strategies"
Cohesion: 0.10
...
### Community 468 - "test_integration_c4_c5_c6_d3.py"
Cohesion: 0.09
Nodes (29): functools, Enum, How to truncate messages when over the context limit., TruncationStrategy, get_current_trace_id(), get_tracer(), langfuse_metadata_with_trace(), otel_status_error() (+21 more)

### Community 469 - "implement_agent.py"
Cohesion: 0.13
Nodes (19): build_tool_calling_router(), main(), _nvidia_candidates(), _openai_tools_to_anthropic(), Any, Safely insert an entry under ## [Unreleased] without touching the rest of the…, Convert OpenAI function-calling tool schemas to Anthropic tool schemas., Run the implementation agent loop using Claude Opus via Anthropic SDK. Returns… (+11 more)

### Community 470 - "test_north_mini_code.py"
Cohesion: 0.11
Nodes (9): north_mini_code_model_for(), Return the North Mini Code model id served by *provider*, else ``None``.…, tests/test_north_mini_code.py — North Mini Code 1.0 integration. Covers the…, The switch defaults ON so North is the default post-install., The agency/Hermes execution path defaults to North via the resolver., test_flag_default_is_on(), test_internal_agent_consults_coding_resolver(), test_north_mini_code_model_for_maps_only_serving_providers() (+1 more)

### Community 471 - "note_phase_start"
Cohesion: 0.10
...
### Community 472 - "system_instruction"
Cohesion: 0.06
Nodes (17): extract_refusal(), is_strict(), Any, Extract the ``refusal`` string from an OpenAI-format response body. Returns the…, Return True when the caller has requested strict schema enforcement. Strict…, Return a plain-English JSON instruction for a ``response_format`` dict. Returns…, system_instruction(), is_strict() detects strict: true inside json_schema. (+9 more)

### Community 473 - "test_ceo_cross_verify.py"
Cohesion: 0.20
...
### Community 474 - "memory_guard.py"
Cohesion: 0.12
Nodes (23): ctypes_util, _load_malloc_trim(), memory_guard_enabled(), memory_guard_loop(), services/memory_guard.py — keep RSS from creeping to OOM on small dynos. The…, Parse the sweep interval, flooring it and tolerating a bad value. 180s is…, True unless explicitly disabled. Default on: the whole point is that the…, Resolve glibc ``malloc_trim``. Returns None when unavailable (non-glibc). (+15 more)

### Community 475 - "OrchestratorCheckpointStore"
Cohesion: 0.10
Nodes (13): _NoopDB, OrchestratorCheckpointStore, Any, Restore in-flight runs at startup. Called during backend bootstrap. Returns a…, Fallback in-memory store when no DB is available., Persist orchestrator runs so they survive restarts., Persist a WorkflowRun snapshot., Load a persisted run snapshot. (+5 more)

### Community 476 - "RewardScorer"
Cohesion: 0.10
...
### Community 477 - "test_openclaw_str_e_fix.py"
Cohesion: 0.10
...
### Community 478 - "Context: Agentic Agile + Portfolio Management"
Cohesion: 0.11
Nodes (14): Story points added since the sprint started (scope creep). Returns 0 before the…, Record a retrospective observation (what went well / poorly)., Record a follow-up action item from the retrospective., Derive a qualitative health signal from the metrics. - COMPLETE: all points…, Agile improvements shipped alongside, Autonomous intelligence (`agents/portfolio_intelligence.py`), Capacity & roadmap, Context: Agentic Agile + Portfolio Management (+6 more)

### Community 479 - "test_failover_silent_exhaustion.py"
Cohesion: 0.06
...
### Community 480 - "get_trend_watcher"
Cohesion: 0.15
Nodes (16): get_trend_watcher(), set_trend_watcher(), Tests for trend_analysis.py — last30days-style window over TrendWatcher (issue…, TestRunTrendAnalysis, TestWindow, test_singleton_round_trip(), BaseModel, trend_analysis.py — last30days-style trend analysis (issue #493). Adapts the… (+8 more)

### Community 481 - "ChatResponse"
Cohesion: 0.10
Nodes (9): CerebrasProvider, Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., ChatResponse, RateLimit, Standard response from a provider chat call., Provider rate limit info. (+1 more)

### Community 482 - "ProviderManager"
Cohesion: 0.23
...
### Community 485 - "_extract_tech_relevance"
Cohesion: 0.16
...
### Community 486 - "Skill: modularity-review"
Cohesion: 0.10
...
### Community 487 - "Design Audit"
Cohesion: 0.10
Nodes (19): Code Quality, Color and Surfaces, Component Patterns, Content, Design Audit, Fix Priority, How This Works, Iconography (+11 more)

### Community 488 - "Findings"
Cohesion: 0.10
...
### Community 489 - "report_to_markdown"
Cohesion: 0.10
Nodes (18): main(), _parse_args(), Namespace, Run the full audit and write output files. Returns exit code., run_audit(), main(), Path, Run the audit engine against ``url`` with an injected HTTP fetcher. (+10 more)

### Community 490 - "Skill: modularity-review"
Cohesion: 0.10
Nodes (19): Acceptance Checks, Applying to This Repo, Further Reading, Modularity Findings Template, Part A: Reviewing Existing Code for Modularity Problems, Part B: Designing New Modular Boundaries, Skill: modularity-review, Step 1 — Map the dependency graph (+11 more)

### Community 491 - "crispy_client.py"
Cohesion: 0.14
Nodes (18): cmd_approve(), cmd_artifacts(), cmd_build(), cmd_events(), cmd_reject(), cmd_status(), cmd_watch(), _get() (+10 more)

### Community 492 - "4. Troubleshooting"
Cohesion: 0.10
...
### Community 493 - "_execute_skill_impl"
Cohesion: 0.11
...
### Community 494 - "control_registry.py"
Cohesion: 0.18
...
### Community 496 - "Autonomous AI Agency"
Cohesion: 0.10
...
### Community 500 - "TestBrainFailoverModelUpdates"
Cohesion: 0.18
...
### Community 501 - "test_dashboard_cache.py"
Cohesion: 0.15
...
### Community 503 - "test_research_coordinator.py"
Cohesion: 0.11
...
### Community 505 - "test_webui_provider_priority.py"
Cohesion: 0.38
Nodes (15): _bootstrap(), Path, tests/test_webui_provider_priority.py — Priority + reorder + brain-policy…, test_admin_reorder_endpoint_requires_auth(), test_admin_reorder_endpoint_validates_body(), test_admin_reorder_endpoint_writes_priorities(), test_create_persists_priority(), test_get_secret_includes_priority() (+7 more)

### Community 506 - "WorkspaceManager"
Cohesion: 0.15
...
### Community 508 - "Analysis & Synthesis Instructions"
Cohesion: 0.11
Nodes (18): 1. Define the Atmosphere, 2. Map the Color Palette, 3. Establish Typography Rules, 4. Define the Hero Section, 5. Describe Component Stylings, 6. Define Layout Principles, 7. Define Responsive Rules, 8. Encode Motion Philosophy (+10 more)

### Community 509 - "Production Readiness Assessment — local-llm-server"
Cohesion: 0.11
...
### Community 510 - "WorkflowScreen.jsx"
Cohesion: 0.13
...
### Community 512 - "Admin Dashboard Guide"
Cohesion: 0.11
...
### Community 513 - "ControlsScreen.jsx"
Cohesion: 0.15
...
### Community 517 - "Delegation Plan (agent-ready work packages)"
Cohesion: 0.11
...
### Community 523 - "test_brain_patch_service_token.py"
Cohesion: 0.18
...
### Community 524 - "TestSelfHealingInfrastructureClassification"
Cohesion: 0.19
...
### Community 525 - "Fixed"
Cohesion: 0.06
Nodes (32): Atomically delete a run-once row only while it is still unfired. Closes the…, _in_container(), Return True when running inside a container runtime. ``/.dockerenv`` alone…, Fixed, Fixed, _notify_watchdog(), Notify the brain watchdog of a provider call outcome (fire-and-forget).…, Record models found by a provider's ``/models`` endpoint. Discovered models… (+24 more)

### Community 526 - "operational_incidents.py"
Cohesion: 0.12
...
### Community 527 - "validate_session_id"
Cohesion: 0.16
...
### Community 528 - "test_issue_triage.py"
Cohesion: 0.14
...
### Community 529 - "SEO / GEO / AIO Audit Engine"
Cohesion: 0.12
...
### Community 531 - "analyze_qualitative"
Cohesion: 0.25
...
### Community 533 - "agents/agile_ceremonies.py"
Cohesion: 0.09
Nodes (25): _bullets(), generate_backlog_retro(), generate_sprint_retro(), Agentic Agile — autonomous ceremonies (standup, retro, sprint planning). Where…, Render a :class:`Retrospective` as a markdown section., Derive retro notes for ``sprint`` from its current metrics. Records…, Derive a retrospective from the task tracker when no sprint is active. DONE /…, Render the sprint plan as markdown. (+17 more)

### Community 534 - "AI Engineering Insights Skill"
Cohesion: 0.18
...
### Community 535 - "Comprehensive Skill Index (By Category)"
Cohesion: 0.11
...
### Community 536 - "Agent Skill: Principal UI/UX Architect & Motion Choreographer (Awwwards-Tier)"
Cohesion: 0.11
Nodes (17): 1. Meta Information & Core Directive, 2. THE "ABSOLUTE ZERO" DIRECTIVE (STRICT ANTI-PATTERNS), 3. THE CREATIVE VARIANCE ENGINE, 4. HAPTIC MICRO-AESTHETICS (COMPONENT MASTERY), 5. MOTION CHOREOGRAPHY (FLUID DYNAMICS), 6. PERFORMANCE GUARDRAILS, 7. EXECUTION PROTOCOL, 8. PRE-OUTPUT CHECKLIST (+9 more)

### Community 537 - "Architecture Overview — local-llm-server"
Cohesion: 0.11
...
### Community 539 - "SpecialistTask"
Cohesion: 0.13
Nodes (14): Any, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask., Record the goal and its sub-tasks in the ledger before any work runs. Writing…, Run sub-tasks through RuntimeManager so each one is actually routed to the…, Run sub-tasks via RuntimeManager with per-sub-task provider_preference.…, Dispatch one attempt of *st* to its runtime and normalise the result., Return ``(subtask_id, summary)`` for each completed dependency., Append this attempt to the sub-task's ledger history. (+6 more)

### Community 540 - "The rules"
Cohesion: 0.11
...
### Community 541 - "Part A — Health Report"
Cohesion: 0.11
...
### Community 542 - "_hydrate_scheduler_bounded"
Cohesion: 0.10
...
### Community 546 - "analyze_quantitative"
Cohesion: 0.17
Nodes (6): analyze_quantitative(), Compute descriptive statistics for a numeric series. Args: source: Where the…, Combine qualitative + quantitative findings into a decision-ready brief. If…, synthesize_research(), TestAnalyzeQuantitative, TestSynthesizeResearch

### Community 547 - "_telegram_sends_suppressed"
Cohesion: 0.10
Nodes (15): _send(), _send(), Any, True when outbound Telegram sends must be suppressed. Tests must never page a…, Callback for BackgroundAgent.on_task_complete. Dispatches task result…, Send notification to configured Telegram chat IDs., Send a Telegram message with an inline keyboard to all configured chats., POST task result to configured webhook URL. Both ``error`` and ``result`` are… (+7 more)

### Community 548 - "._cannot_list"
Cohesion: 0.16
...
### Community 549 - "test_phase5_doctor.py"
Cohesion: 0.11
...
### Community 551 - "test_verification_strategies.py"
Cohesion: 0.07
...
### Community 556 - "register_user_research_tools"
Cohesion: 0.15
...
### Community 557 - "is_model_available"
Cohesion: 0.06
Nodes (38): Architecture Audit — local-llm-server, Architecture Diagram, Component Map, Layer 10 — WebUI (`webui/`), Layer 11 — Infrastructure, Layer 1 — API Proxy (`proxy.py`, 1719 lines), Layer 3 — Model Router (`router/`), Layer 4 — Agent System (`agent/`) (+30 more)
...
### Community 560 - "DeterministicEngine"
Cohesion: 0.21
...
### Community 561 - "SKILL: Industrial Brutalism & Tactical Telemetry UI"
Cohesion: 0.12
...
### Community 562 - "Skill: data-quality-audit"
Cohesion: 0.12
...
### Community 563 - "What "Slop" Looks Like"
Cohesion: 0.12
...
### Community 566 - "memory_consolidation.py"
Cohesion: 0.36
...
### Community 567 - "Slice"
Cohesion: 0.15
Nodes (8): TestSlice, TestWorkflowRun, _extract_slices_from_plan(), Execute, review, and verify a single slice. Returns the updated Slice. Raises…, Extract slice definitions from a plan.md artifact. Looks for sections matching:…, An independently deliverable unit of work within the execute phase. Each slice…, Return the Slice with *slice_id*, or None., Slice

### Community 568 - "provider_max_rpm"
Cohesion: 0.29
...
### Community 572 - "test_daily_digest.py"
Cohesion: 0.06
...
### Community 573 - "OperationalIncidentTracker"
Cohesion: 0.11
...
### Community 574 - "test_p0_roadmap_b3_b4_b5.py"
Cohesion: 0.15
...
### Community 576 - "test_task_brain_preflight.py"
Cohesion: 0.21
...
### Community 577 - "_migration_block"
Cohesion: 0.15
...
### Community 578 - "DecisionsStoreTests"
Cohesion: 0.12
...
### Community 580 - "test_skill_registry.py"
Cohesion: 0.13
...
### Community 581 - "TestRouterIntegration"
Cohesion: 0.21
...
### Community 582 - "test_unit5_ui_provider_surface.py"
Cohesion: 0.10
...
### Community 584 - "webui/router.py"
Cohesion: 0.06
Nodes (51): fastapi_responses, chat_completions(), ChatCompletionRequest, _content_to_str(), _ContentPart, health(), lifespan(), list_models() (+43 more)

### Community 585 - "executive_advisory_api.py"
Cohesion: 0.15
Nodes (21): _company_advisory_context(), Pull a compact company profile for the C-suite advisory, or None. Best-effort:…, default_executives(), Return the built-in C-suite personas., AdvisoryConsultRequest, AdvisoryResponse, build_executive_advisory_router(), consult() (+13 more)

### Community 586 - "AgentJobSnapshot"
Cohesion: 0.28
...
### Community 587 - "ErrorInterceptorMiddleware"
Cohesion: 0.19
...
### Community 588 - "hermes_prompt.py"
Cohesion: 0.19
...
### Community 589 - "MemoryMiddleware"
Cohesion: 0.17
...
### Community 594 - "Skill: repowise-intelligence"
Cohesion: 0.12
...
### Community 595 - "ARCHITECTURE.md — Target Architecture"
Cohesion: 0.12
...
### Community 596 - "Fixed"
Cohesion: 0.18
Nodes (18): Fixed, Fixed, difflib, Fixed, Fixed, DetailsStep(), ProviderResult, _blocks() (+10 more)

### Community 597 - "Decision"
Cohesion: 0.11
...
### Community 598 - "test_social_login_oauth.py"
Cohesion: 0.24
...
### Community 599 - "Attention Mechanisms Internals"
Cohesion: 0.11
Nodes (18): ta(), x(), xa(), zb(), Attention Complexity, Attention Mechanisms Internals, Causal Masking, Flash Attention (+10 more)

### Community 600 - "Skill: repowise-intelligence"
Cohesion: 0.12
Nodes (15): 1. Graph Intelligence (Dependency Graph), 2. Git Intelligence, 3. Documentation Intelligence, 4. Decision Intelligence, Acceptance Checks, Directory Structure, Example Usage, Implementation Approach (+7 more)

### Community 601 - "The 10-Step Workflow"
Cohesion: 0.12
...
### Community 602 - "Contributing to local-llm-server"
Cohesion: 0.12
...
### Community 603 - "Agent Runtime Setup"
Cohesion: 0.14
...
### Community 605 - "TestMCPToolsListCache"
Cohesion: 0.15
...
### Community 608 - "test_setup_api.py"
Cohesion: 0.19
...
### Community 609 - "test_agency_workflows_carry_the_failover_chain.py"
Cohesion: 0.18
...
### Community 611 - "test_dockerfile_ships_root_modules.py"
Cohesion: 0.17
...
### Community 612 - "test_langfuse_agency_wide.py"
Cohesion: 0.17
Nodes (11): tests/test_langfuse_agency_wide.py — tests for PR #961 agency-wide Langfuse.…, langfuse_obs.py must define emit_agency_observation., emit_agency_observation must be a no-op when Langfuse is not configured., backend/server.py scheduler_tick must call emit_agency_observation., packages/ai/self_heal.py must call emit_agency_observation., emit_agency_observation must accept all documented parameters., test_emit_agency_observation_accepts_all_params(), test_emit_agency_observation_exists() (+3 more)
...
### Community 622 - "Skill: agent-harness"
Cohesion: 0.13
...
### Community 623 - "Skill: checkpoint-strategy"
Cohesion: 0.13
...
### Community 625 - "Skill: local-ai-query"
Cohesion: 0.13
...
### Community 626 - "Skill: parallel-agents"
Cohesion: 0.13
...
### Community 627 - "Skill: parallel-worktrees"
Cohesion: 0.13
...
### Community 631 - "Agency Core — Progress & Resume Log"
Cohesion: 0.13
...
### Community 632 - "OpenClaw — iOS Control of the Agency (Single-Service Free-Tier Deploy)"
Cohesion: 0.13
...
### Community 633 - "ChatScreen.jsx"
Cohesion: 0.07
Nodes (17): chatSend(), getAgentChatJob(), getSession(), listProviderModels(), listProviders(), listSessions(), AVAILABLE_AGENTS, CHAT_PHASES (+9 more)

### Community 634 - "test_brain_config_api.py"
Cohesion: 0.13
Nodes (13): Risks & mitigations (per plan §6), _catalogue_candidates(), _catalogue_role_preset(), _catalogue_safe_default(), tests/test_brain_config_api.py — admin endpoint contract tests. Pins the…, The role preset the catalogue defines, not a literal copy of it. Two tests here…, The rotation as the catalogue defines it, after the YAML override.…, Reiterates the security contract: no key value ever appears in the response. (+5 more)

### Community 635 - "HarnessRegistry"
Cohesion: 0.17
...
### Community 638 - "StreamingDeltaReconstructor"
Cohesion: 0.26
...
### Community 639 - "FeatureMaturity"
Cohesion: 0.08
Nodes (17): 18. Feature Maturity & Support Matrix, __init__.py — Feature flag/matrix package., FeatureMaturity, FeatureUnavailableError, Enum, Exception, str, features/matrix.py — Feature maturity tiers and support matrix. Single source… (+9 more)

### Community 640 - "TestBrainConfigUpdates"
Cohesion: 0.22
...
### Community 641 - "test_dockerfile_ships_config_dir.py"
Cohesion: 0.14
...
### Community 644 - "test_scanner_live.py"
Cohesion: 0.23
...
### Community 645 - "test_tasks_reconciler_todo_requeue.py"
Cohesion: 0.22
...
### Community 649 - "Agent Orchestration Design"
Cohesion: 0.14
...
### Community 650 - "HealthStatus"
Cohesion: 0.12
Nodes (6): NvidiaProvider, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., Check health of all configured providers., HealthStatus, Provider health check result., Check provider health.

### Community 651 - "Workspace"
Cohesion: 0.12
...
### Community 660 - "Skill: branch-cleanup"
Cohesion: 0.14
...
### Community 661 - "Skill: perplexity — Web Research via Perplexity API"
Cohesion: 0.14
...
### Community 663 - "test_openclaw_endpoints.py"
Cohesion: 0.12
...
### Community 665 - "TestBrainFailoverBackoff"
Cohesion: 0.23
...
### Community 666 - "DirectChatSession"
Cohesion: 0.15
...
### Community 667 - "Separate hosted dashboard backend (`backend/server.py`)"
Cohesion: 0.14
...
### Community 669 - "LLM Router — configuration guide"
Cohesion: 0.14
...
### Community 671 - "Runbook: Auto-Resume After Cooldown / Interruption"
Cohesion: 0.14
...
### Community 675 - "._get_conn"
Cohesion: 0.15
Nodes (7): Delete a session and all its messages. Returns True if deleted., Return total session and message counts., Update a message's content. Returns True if found and updated., Return the number of messages in a session., Run SQLite VACUUM to reclaim disk space., Return store statistics., Get or create the database connection (thread-unsafe; use from one thread).

### Community 676 - "What to clean up"
Cohesion: 0.20
...
### Community 677 - "_warmup_step"
Cohesion: 0.18
Nodes (13): _build_ceo_prompt(), _collect_recent_git_context(), _now_str(), Return recent commits and changed files for CEO situational awareness., _maybe_boot_purge(), _purge_summary_clean(), Await one warm-up step, deferring it to the background if it overruns.…, True when a purge summary contains no error markers. _purge_backlog_core… (+5 more)
...
### Community 679 - "_build_execution_request"
Cohesion: 0.24
...
### Community 680 - "Skill: user-research"
Cohesion: 0.14
...
### Community 682 - "TestAnthropicToolListCaching"
Cohesion: 0.34
...
### Community 683 - "useSafeData"
Cohesion: 0.06
Nodes (34): createAgent(), createSchedule(), pauseSchedule(), resumeSchedule(), triggerSchedule(), useSafeData(), AgentCard(), AgentsScreen() (+26 more)

### Community 684 - "_StubManager"
Cohesion: 0.14
Nodes (3): _apply(), Minimal stand-in for BrainFailoverManager., _StubManager

### Community 685 - "TestDisabledReasonRendering"
Cohesion: 0.14
...
### Community 687 - "agent/workspace.py"
Cohesion: 0.18
...
### Community 688 - "AGENTS.md — Codebase Map & Operations Reference"
Cohesion: 0.15
...
### Community 689 - "CollaborationContext"
Cohesion: 0.21
...
### Community 707 - "Skill: platform-setup — Autonomous Agency Bootstrap"
Cohesion: 0.15
...
### Community 708 - "Device compatibility and model picks"
Cohesion: 0.15
...
### Community 709 - "test_critical_flows.py"
Cohesion: 0.26
...
### Community 712 - "ApplyReviewAgent"
Cohesion: 0.22
...
### Community 714 - "TestEstimateTokensForMessages"
Cohesion: 0.15
...
### Community 715 - "knowledge_graph.py"
Cohesion: 0.18
...
### Community 716 - "_get_provider_policy"
Cohesion: 0.19
...
### Community 717 - "generate_onboarding_questions"
Cohesion: 0.39
Nodes (8): generate_onboarding_questions(), _get_fallback_questions(), Generate AI-tailored onboarding questions based on detected domain and…, Return hardcoded fallback questions when AI generation fails., (5): Tailored questions are hardcoded today, D. Wire AI-tailored questions to the frontend (`QuestionsStep`), submitOnboardingAnswers(), QuestionsStep()

### Community 718 - "cost_tracker.py"
Cohesion: 0.18
...
### Community 721 - "TestModelCostTableUpdates"
Cohesion: 0.26
...
### Community 722 - "test_deploy_trigger_covers_image.py"
Cohesion: 0.21
...
### Community 725 - "test_providers_live_e2e.py"
Cohesion: 0.24
...
### Community 733 - "Skill: system-prompt-audit"
Cohesion: 0.17
...
### Community 734 - "Skill: task-alive-updates"
Cohesion: 0.17
...
### Community 737 - "company_api.py"
Cohesion: 0.03
Nodes (152): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+144 more)

### Community 738 - "TestModelsEndpointAliases"
Cohesion: 0.31
...
### Community 739 - "Skill: agent-browser — Real Chrome Browser Automation"
Cohesion: 0.17
...
### Community 743 - "Quantization Internals"
Cohesion: 0.17
...
### Community 745 - "Traffic Distribution Across Providers"
Cohesion: 0.15
...
### Community 748 - "install-agents.sh"
Cohesion: 0.39
...
### Community 749 - "test_p0_roadmap_a4_a5_b2.py"
Cohesion: 0.15
...
### Community 750 - "Kimi Web-Bridge Service"
Cohesion: 0.17
...
### Community 751 - "_resolve_push_token"
Cohesion: 0.17
...
### Community 755 - "test_cerebras_catalogue.py"
Cohesion: 0.13
...
### Community 756 - "Company"
Cohesion: 0.06
Nodes (20): C. Defer company persistence to a final "Confirm" step + relocate the email gate, Company, Any, field_validator, The core company entity - root of the Company Graph., Coerce unrecognised system_type values to 'custom' so the model never crashes…, Get a company by ID. Args: company_id: Company ID Returns: Company instance or…, Update a company. Args: company_id: Company ID **kwargs: Fields to update… (+12 more)

### Community 757 - "test_compose_and_coordinate_api.py"
Cohesion: 0.19
...
### Community 758 - "scan_repo_with_user_research.py"
Cohesion: 0.22
...
### Community 759 - ".test_set_github_token_sqlite_string_id_does_not_500"
Cohesion: 0.20
Nodes (3): MonkeyPatch, TestClient, TestGithubTokenSQLiteRegression

### Community 760 - "_Response"
Cohesion: 0.10
Nodes (9): Parse a 429/503 ``Retry-After`` header → seconds, or None. Accepts either…, Translate a Bedrock Converse API response to OpenAI chat.completion format., TestAnthropicToOpenAICacheUsage, The regression itself: a 402 (or any non-2xx) must not be fatal while another…, _Response, TestCallBrainWithFailover, fake_post(), fake_post() (+1 more)

### Community 761 - "_run_analyze"
Cohesion: 0.27
...
### Community 769 - "_extractive_compress"
Cohesion: 0.18
...
### Community 773 - "Skill: pro-workflow"
Cohesion: 0.18
...
### Community 776 - "Skill: resource-panel"
Cohesion: 0.18
...
### Community 777 - "Skill: sandboxed-exec"
Cohesion: 0.18
...
### Community 778 - "test_serve_spa_prefixes.py"
Cohesion: 0.24
...
### Community 779 - "Skill: dev-browser — Browser Automation via Sandboxed JS"
Cohesion: 0.18
...
### Community 780 - "ECC Harness Patterns Skill"
Cohesion: 0.18
...
### Community 783 - "Stop-Slop Quality Skill"
Cohesion: 0.18
...
### Community 785 - "test_server_autonomy_and_index_fixes.py"
Cohesion: 0.20
...
### Community 786 - "2. Critical Bugs & Exact Detection Signatures"
Cohesion: 0.18
...
### Community 787 - "Issue #467 — Section 1: Pulled State + PR Inventory"
Cohesion: 0.18
...
### Community 788 - "Deploy to Google Cloud Run"
Cohesion: 0.18
...
### Community 790 - "ChatHistoryStore"
Cohesion: 0.22
Nodes (5): ChatHistoryStore, Persistent SQLite-backed chat history store. Messages are stored with…, Close the database connection., Create tables if they don't exist., TestChatHistoryIntegration

### Community 791 - "Runbook — Apply the Fast Free NVIDIA Brain to Render (TASK 2)"
Cohesion: 0.18
...
### Community 792 - "CI Troubleshooting Runbook"
Cohesion: 0.18
...
### Community 793 - "admin_update_task_router.py"
Cohesion: 0.22
...
### Community 794 - "Worker Service — Operations Runbook"
Cohesion: 0.18
...
### Community 797 - "TestRuntimeControl"
Cohesion: 0.15
...
### Community 799 - "OrchestratorSupervisor"
Cohesion: 0.26
...
### Community 801 - "LLM Router — provider guide"
Cohesion: 0.22
...
### Community 802 - "test_agent_readiness_audit.py"
Cohesion: 0.18
...
### Community 803 - "TestTheSharedListFitsBothCallers"
Cohesion: 0.18
...
### Community 807 - "TestDecisionsBotLinks"
Cohesion: 0.25
...
### Community 808 - "test_empirical_verify.py"
Cohesion: 0.49
...
### Community 809 - "test_event_log.py"
Cohesion: 0.45
...
### Community 810 - "TestZeroAttemptDiagnostics"
Cohesion: 0.25
...
### Community 811 - "test_google_provider_models.py"
Cohesion: 0.18
...
### Community 812 - "TestAuthAndTaskCreation"
Cohesion: 0.17
...
### Community 813 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.22
...
### Community 815 - "test_provider_state_durability.py"
Cohesion: 0.09
...
### Community 816 - "test_state_file_merge_drivers.py"
Cohesion: 0.47
...
### Community 820 - "financial_analyst.py"
Cohesion: 0.22
...
### Community 822 - "Protocol: Premium Utilitarian Minimalism UI Architect"
Cohesion: 0.20
...
### Community 823 - "The 5-Step Wrap-Up Ritual"
Cohesion: 0.20
...
### Community 824 - "Brag Plan: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 826 - "Hyperframes Composition Brief: Autonomous AI Agency (feature tour, v2)"
Cohesion: 0.20
...
### Community 827 - "_normalize_tool_choice"
Cohesion: 0.31
...
### Community 828 - "Skill: Agentic Agile"
Cohesion: 0.20
...
### Community 829 - "Skill: browserbase-ui-test — Adversarial UI Testing"
Cohesion: 0.20
...
### Community 830 - "Skill: financial-analyst (Agentic CFO)"
Cohesion: 0.20
...
### Community 831 - "Graphiti Temporal Context Skill"
Cohesion: 0.20
...
### Community 832 - "Skill: seo-audit-report"
Cohesion: 0.20
...
### Community 833 - "Agent Readiness Report"
Cohesion: 0.20
...
### Community 834 - "467 Golden Path — Locked Implementation Order"
Cohesion: 0.20
...
### Community 835 - "Competitor Analysis — Autonomous AI Agency"
Cohesion: 0.20
...
### Community 836 - "WorkspaceMetrics"
Cohesion: 0.17
Nodes (6): _derive_workspace_root(), Path, Simple counters for workspace operations., Read and validate a manifest from disk., Derive the canonical, isolated workspace root directory. Layout:…, WorkspaceMetrics

### Community 837 - "One command (recommended)"
Cohesion: 0.20
...
### Community 838 - "test_pytest_failure_parser.py"
Cohesion: 0.23
...
### Community 839 - "make_isolated_workspace"
Cohesion: 0.33
...
### Community 840 - "test_direct_chat_evolution.py"
Cohesion: 0.11
...
### Community 841 - "services/seo_audit.py"
Cohesion: 0.07
Nodes (28): bs4, csv, lxml, Static definition of a single audit check (catalog entry)., SeoCheckDefinition, _count_syllables(), estimate_pixel_width(), flesch_reading_ease() (+20 more)

### Community 842 - "MCPToolResult"
Cohesion: 0.29
...
### Community 844 - "run_patched_colibri.py"
Cohesion: 0.27
...
### Community 846 - "start_background_services"
Cohesion: 0.08
...
### Community 847 - "Added"
Cohesion: 0.24
Nodes (8): Added, Added, buildGraphElements(), get_cost_table(), Return the active cost table as a JSON-serialisable dict., _ProviderQuota, Parse x-ratelimit-* headers and update per-provider quota state. Safe to call…, Auto-provision specialists based on detected system types. Args: company_id:…

### Community 848 - "DeltaChunk"
Cohesion: 0.27
...
### Community 849 - "_push_down_where"
Cohesion: 0.18
...
### Community 850 - "test_backend_lifespan_skips_bg_when_flag_false"
Cohesion: 0.24
...
### Community 851 - "TestPolicyAuthoringUiStableClaim"
Cohesion: 0.29
...
### Community 854 - "TestExtendedThinkingRouting"
Cohesion: 0.20
...
### Community 855 - "test_dependabot_sweep_workflow.py"
Cohesion: 0.05
...
### Community 856 - "test_conftest_hermetic_env.py"
Cohesion: 0.18
...
### Community 857 - "test_model_catalog_guard.py"
Cohesion: 0.29
...
### Community 859 - "test_task_clarification.py"
Cohesion: 0.17
...
### Community 860 - "test_workflow_api_mount.py"
Cohesion: 0.20
...
### Community 861 - "TestAgentRunnerSafety"
Cohesion: 0.24
...
### Community 862 - "test_executive_advisory.py"
Cohesion: 0.12
Nodes (21): AdvisoryMemory, _compose_context(), _format_company_context(), get_executive_advisory(), _keywords(), _mem_key(), agent/executive_advisory.py — an intelligent C-suite business-advisory layer.…, The C-suite's durable memory of past recommendations. A thin adapter over… (+13 more)

### Community 864 - "Autonomy Charter — Telegram-Gated Self-Running Agency"
Cohesion: 0.06
...
### Community 865 - "_extract_workflow_relevance"
Cohesion: 0.33
...
### Community 866 - "TestClaudeOpus55CostTracker"
Cohesion: 0.25
...
### Community 870 - "Skill: learn-rule"
Cohesion: 0.22
...
### Community 871 - "Tailored Onboarding, Editable Companies & Dynamic Roles"
Cohesion: 0.12
Nodes (18): 1. Editable companies, anytime (not a one-shot wizard), 2. Question-driven provisioning — no cosmetic questions, 4. Agents start pre-powered, Invariants, Phases, Tailored Onboarding, Editable Companies & Dynamic Roles, The gaps to close, UI-first — an API is not "done" (+10 more)

### Community 872 - "Instructions"
Cohesion: 0.22
...
### Community 874 - "Skill: changelog-enforcer"
Cohesion: 0.22
Nodes (8): Acceptance Checks, Changelog Location, Entry Format, Examples, Hook Behaviour, Instructions, Skill: changelog-enforcer, When to Use

### Community 875 - "Workflow"
Cohesion: 0.22
...
### Community 876 - "Skill: cowork-session (Claude Cowork)"
Cohesion: 0.22
Nodes (8): Branch, Components, Purpose, Quick Start, Session Roles, Skill: cowork-session (Claude Cowork), Testing, When to Use

### Community 877 - "Instructions"
Cohesion: 0.14
Nodes (13): Acceptance Checks, `admin_auth.py` checklist, `agent/tools.py` checklist, Escalation, Instructions, `key_store.py` checklist, `proxy.py` auth middleware checklist, Risky Modules in This Repo (+5 more)

### Community 878 - "Skill: video-context — read a video without watching it"
Cohesion: 0.22
...
### Community 879 - "record_usage_endpoint"
Cohesion: 0.22
...
### Community 880 - "ADR 003: Multi-Agent Orchestration with Plan-Execute-Verify Loop"
Cohesion: 0.22
...
### Community 881 - "The 8-Step Golden Path"
Cohesion: 0.22
...
### Community 882 - "Issue #1356: quick-note:https://searchengineland.com/turn-seo-backlog-into-roadmap-485713"
Cohesion: 0.22
...
### Community 883 - "Release Procedure"
Cohesion: 0.22
...
### Community 884 - "V2.0 Modernization — Runbook"
Cohesion: 0.22
...
### Community 885 - "ENGINEERING_STANDARDS.md — Patterns & Reference"
Cohesion: 0.20
...
### Community 887 - "._run_git_command"
Cohesion: 0.20
...
### Community 888 - "OnboardingScreen.jsx"
Cohesion: 0.10
...
### Community 893 - ".append"
Cohesion: 0.24
Nodes (5): Connection, Append a message to the session. Returns the message's sequence number.…, Append multiple messages at once. Returns number of messages appended., Trim the oldest messages so total stays under *max_messages*. Returns the…, Remove oldest messages until session is within the limit.

### Community 894 - "_FakeAsyncClient"
Cohesion: 0.20
Nodes (3): _FakeAsyncClient, _FakeResponse, Returns 413 for every call to a Groq URL, success for anything else.

### Community 895 - "TestAgentRunnerPublicPlanMethod"
Cohesion: 0.20
...
### Community 896 - ".test_every_request_carries_a_real_user_agent"
Cohesion: 0.22
...
### Community 898 - "test_runtimes_health_endpoint.py"
Cohesion: 0.33
...
### Community 900 - "test_process_quick_note_workflow.py"
Cohesion: 0.25
...
### Community 901 - "TestEveryFullSuiteJobHasMongo"
Cohesion: 0.22
...
### Community 902 - "TestPaidPolicyDurability"
Cohesion: 0.22
...
### Community 904 - "_safe_resolve"
Cohesion: 0.14
Nodes (6): If a symlink inside the workspace points outside, resolve_path blocks it., TestPathSafety, parametrize, TestPathTraversalPrevention, Resolve *path* and verify it stays under *base_root*. Blocks symlink escape:…, _safe_resolve()

### Community 905 - "stt.py"
Cohesion: 0.36
...
### Community 906 - "classify_direct_chat_intent"
Cohesion: 0.43
...
### Community 907 - "coordinator.py"
Cohesion: 0.25
Nodes (8): Enum, str, agent/coordinator.py — Multi-Agent Coordinator One coordinator agent breaks a…, TaskStatus, emit_deprecation(), is_legacy_mode(), True when parallel execution paths are allowed (with warnings)., Log a deprecation warning when a parallel path is used.

### Community 909 - "_score_turns"
Cohesion: 0.36
...
### Community 911 - "hybrid_reasoning.py"
Cohesion: 0.36
...
### Community 912 - "quality_checker.py"
Cohesion: 0.32
...
### Community 913 - "research_coordinator.py"
Cohesion: 0.36
...
### Community 915 - "_start_in_web_bot_tasks"
Cohesion: 0.29
...
### Community 916 - "_parse_tool_calls_from_response"
Cohesion: 0.39
...
### Community 917 - "Agent: Judge (Release / QA Gate)"
Cohesion: 0.25
Nodes (7): Activation, Agent: Judge (Release / QA Gate), Enforcement, Output, Responsibilities, Role, Verdict Meanings

### Community 918 - "Skill: browserbase-browser — Real Browser Automation"
Cohesion: 0.25
...
### Community 922 - "GitHub Branch Protection Settings"
Cohesion: 0.25
...
### Community 929 - "Issue #362: Nvidia repo setup"
Cohesion: 0.25
...
### Community 930 - "Issue #364: quick-note:https://www.marktechpost.com/2026/06/01/meet-memory-os-a-6-layer-open-source-memory-stack-built-on-top-of-hermes-agent/"
Cohesion: 0.25
...
### Community 931 - "Issue #378: quick-note:https://www.marktechpost.com/2026/06/02/tinyfish-launches-bigset-an-open-source-multi-agent-system-that-builds-structured-live-datasets-from-plain-english-descriptions/"
Cohesion: 0.25
...
### Community 932 - "Issue #379: quick-note:https://searchengineland.com/schema-markup-optimize-agentic-web-479080"
Cohesion: 0.25
...
### Community 933 - "Issue #380: quick-note:https://cursor.com/blog/cloud-agent-lessons"
Cohesion: 0.25
...
### Community 934 - "Issue #381: quick-note:https://www.xda-developers.com/claude-code-with-opus-48-is-expensive-but-i-made-it-efficient-with-my-local-ai-workflow/"
Cohesion: 0.25
...
### Community 935 - "Issue #382: quick-note:https://claude.com/blog/how-coderabbit-used-claude-to-build-an-agent-orchestration-system"
Cohesion: 0.25
...
### Community 936 - "Issue #383: quick-note:https://www.marktechpost.com/2026/05/29/hexo-labs-open-sources-sia-a-self-improving-agent-that-updates-both-the-harness-and-the-model-weights/"
Cohesion: 0.25
...
### Community 937 - "Issue #416: feat: Self-hosted Codebuff (freebuff) on free NVIDIA models + Telegram bot phone control"
Cohesion: 0.25
...
### Community 938 - "Issue #485: [Trend Digest] Week of 2026-06-08"
Cohesion: 0.25
...
### Community 939 - "Issue #488: quick-note:https://github.com/cookiy-ai/user-research-skill"
Cohesion: 0.25
...
### Community 940 - "Issue #491: Implement whatever is necessary from https://github.com/BehiSecc/awesome-claude-skills"
Cohesion: 0.25
...
### Community 941 - "Issue #493: Use the https://github.com/mvanhorn/last30days-skill skill to get the trend updated"
Cohesion: 0.25
...
### Community 942 - "Issue #495: Read https://www.anthropic.com/news/claude-fable-5-mythos-5 and understand if mythos or fable can be added to the repo"
Cohesion: 0.25
...
### Community 943 - "validate_job_id"
Cohesion: 0.28
Nodes (4): TestJobIdValidation, Create an isolated workspace for a session and optional job. Creates the…, Validate and return a job ID, or raise InvalidJobIdError., validate_job_id()

### Community 944 - "Issue #581: Sprint tracker: pending work after brand rename + mobile-first pass"
Cohesion: 0.25
...
### Community 945 - "Issue #657: quick-note:https://github.com/earendil-works/pi"
Cohesion: 0.25
...
### Community 946 - "Issue #659: quick-note:https://github.com/nex-agi/Nex-N2"
Cohesion: 0.25
...
### Community 947 - "Issue #660: quick-note:https://github.com/getsentry/sentry-for-ai"
Cohesion: 0.25
...
### Community 948 - "Issue #661: quick-note:https://github.com/XiaomiMiMo/MiMo-Code"
Cohesion: 0.25
...
### Community 949 - "Issue #664: quick-note:https://github.com/Grominet95/jarvis-OS"
Cohesion: 0.25
...
### Community 950 - "Issue #666: quick-note:https://github.com/porokka/jarvis-os"
Cohesion: 0.25
...
### Community 951 - "Issue #670: quick-note:https://github.com/perplexityai/bumblebee"
Cohesion: 0.25
...
### Community 952 - "Issue #672: quick-note:https://github.com/Chachamaru127/claude-code-harness"
Cohesion: 0.25
...
### Community 953 - "Issue #676: quick-note:https://github.com/WeiboAI/VibeThinker"
Cohesion: 0.25
...
### Community 954 - "Issue #820: quick-note:https://github.com/cobusgreyling/loop-engineering"
Cohesion: 0.25
...
### Community 955 - "TestMCPClientStructuredOutput"
Cohesion: 0.38
...
### Community 956 - "gather_render_evidence"
Cohesion: 0.22
Nodes (8): gather_render_evidence(), Any, Live counters, for the diagnostics endpoint and tests., Pull recent Render logs and summarise them for the issue body. This is the…, Run *coro* without blocking the caller, from any thread. The caller is a…, _schedule(), No RENDER_API_KEY must still file the incident — the recurrence itself is the…, test_evidence_reports_unavailable_when_render_is_not_configured()

### Community 957 - "Runbook — Instance Activation"
Cohesion: 0.25
...
### Community 959 - "TestPreCompiledPatterns"
Cohesion: 0.22
...
### Community 961 - "PULL_REQUEST_TEMPLATE.md"
Cohesion: 0.25
Nodes (7): Changelog, Changes, Council Review (for larger PRs), Related, Risky Module Review, Summary, Testing

### Community 962 - "Issue #1427: quick-note:https://github.com/bingreeky/JIT"
Cohesion: 0.22
Nodes (8): Context Plan — Issue #1427: quick-note:https://github.com/bingreeky/JIT, Decision, How its four axes map onto what this repo already has, Issue #1427: quick-note:https://github.com/bingreeky/JIT, Recommendation to the maintainer, Source Grounding, What the source actually is, Why this was written by hand

### Community 963 - "verify.sh"
Cohesion: 0.50
Nodes (6): fail(), pass(), verify.sh script, snapshot_files(), write_legacy_roles(), write_v050_roles()

### Community 964 - "Prompt Library"
Cohesion: 0.25
Nodes (8): Agents, Commands, How This Library Is Maintained, Philosophy, Prompt Library, Skills, Transparency, What Is This?

### Community 965 - "_build_pdf"
Cohesion: 0.28
Nodes (7): _build_pdf(), findings_block(), hr(), S(), section(), Path, Render an executive-level PDF from the audit report dict.

### Community 967 - "KV Cache Internals"
Cohesion: 0.22
...
### Community 968 - "test_doctor_coding_brain.py"
Cohesion: 0.32
...
### Community 969 - "Render MCP — autonomous platform debugging and environment monitoring"
Cohesion: 0.22
...
### Community 971 - "test_local_brain_router_smoke.py"
Cohesion: 0.25
...
### Community 976 - "test_provider_models_db_outage.py"
Cohesion: 0.25
...
### Community 977 - "TestSupportMatrixDocsSync"
Cohesion: 0.29
...
### Community 978 - "Any"
Cohesion: 0.25
Nodes (5): Any, List sessions ordered by most recently updated., Return messages for a session, ordered by sequence number., Return messages as clean role/content dicts (no internal keys). This is the…, Export a full session with all messages as a JSON-serialisable dict.

### Community 982 - "dry_clone_repo"
Cohesion: 0.32
...
### Community 987 - "_load_local_metrics_since"
Cohesion: 0.43
...
### Community 998 - "ADR-006: Strangler Fig migration with backward-compat shims"
Cohesion: 0.29
Nodes (6): ADR-006: Strangler Fig migration with backward-compat shims, Consequences, Context, Decision, Examples, Migration path

### Community 999 - "test_backend_requirements_cover_runtime_imports.py"
Cohesion: 0.25
...
### Community 1000 - "claude-mem Plugin — Persistent Memory for All Sessions"
Cohesion: 0.29
...
### Community 1001 - "Platform Controls"
Cohesion: 0.29
...
### Community 1002 - "test_probe_base_url_override_hits_the_typed_url"
Cohesion: 0.22
...
### Community 1003 - "Cloudflare = the real working app"
Cohesion: 0.29
...
### Community 1004 - "Troubleshooting"
Cohesion: 0.04
...
### Community 1009 - "Agent Transparency Report"
Cohesion: 0.29
...
### Community 1010 - "check_doc_images.py"
Cohesion: 0.48
...
### Community 1011 - "quickstart.sh"
Cohesion: 0.52
Nodes (6): die(), ensure_env(), ok(), say(), quickstart.sh script, warn()

### Community 1012 - "TestChatFallbackAndApproval"
Cohesion: 0.22
...
### Community 1013 - "test_scanner_deps_parity.py"
Cohesion: 0.31
...
### Community 1016 - "TestRevenuePortfolio"
Cohesion: 0.22
...
### Community 1017 - "CapacityAllocation"
Cohesion: 0.25
...
### Community 1018 - "Positional Encoding Internals"
Cohesion: 0.25
...
### Community 1021 - "_build_claude_command"
Cohesion: 0.33
Nodes (6): _build_claude_command(), _find_claude_bin(), _inline_claude_fallback(), Build the claude CLI command for a session., Find the claude CLI binary., Python code to run when claude CLI is not available.

### Community 1022 - "apply_phase1_changes.py"
Cohesion: 0.33
...
### Community 1024 - "TestReasonsAreActionable"
Cohesion: 0.29
...
### Community 1025 - "TestProvidersScreen"
Cohesion: 0.43
...
### Community 1027 - "Prime Agent Runtime"
Cohesion: 0.25
...
### Community 1028 - "TestMongoService"
Cohesion: 0.29
...
### Community 1037 - "SIA.py"
Cohesion: 0.27
Nodes (4): SIA, torch, torch_nn, torch_utils_data

### Community 1038 - "openclaw_mobile_ui"
Cohesion: 0.33
...
### Community 1039 - "Music Cues: happy-beats-business-moves-vol-1-by-ende-dot-app"
Cohesion: 0.33
...
### Community 1054 - "test_job_failure_structures_runtime_preflight"
Cohesion: 0.29
...
### Community 1055 - "test_generate_context_standing_instructions.py"
Cohesion: 0.32
...
### Community 1056 - "Pre-Mortem Analysis: Agency Core autonomy story (Cloudflare deployment)"
Cohesion: 0.33
...
### Community 1057 - "TestProviderRouter"
Cohesion: 0.25
...
### Community 1059 - "Runtime & Onboarding Issues"
Cohesion: 0.33
...
### Community 1060 - "StatusPill.jsx"
Cohesion: 0.47
Nodes (5): C, STATUS_META, StatusPill(), TONE_BG(), TONE_BORDER()

### Community 1061 - "TestAgentAutoAssignment"
Cohesion: 0.25
...
### Community 1067 - "_resolve_jwt_secret"
Cohesion: 0.38
Nodes (4): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), The bug: an unset JWT_SECRET minted a fresh random key each process start,…, TestJwtSecretStability

### Community 1070 - "setup-claude-code.sh script"
Cohesion: 0.60
Nodes (5): log_error(), log_info(), log_success(), print_header(), setup-claude-code.sh script

### Community 1072 - "test_activity_feed.py"
Cohesion: 0.33
...
### Community 1078 - "Advisor Strategy — Local Proxy Handling"
Cohesion: 0.29
...
### Community 1081 - "TestNoHardcodedModels"
Cohesion: 0.33
Nodes (3): parametrize, The regression itself: a model id baked into this script., TestNoHardcodedModels

### Community 1084 - "_await_hermes_ready"
Cohesion: 0.29
Nodes (7): _await_hermes_ready(), Any, Await the refresh and log the outcome; swallow every failure., Block until uvicorn is genuinely accepting connections. Without this the…, Run the agency's own Hermes server inside this process, on loopback.…, _refresh_skills_quietly(), _start_hermes_in_process()

### Community 1088 - "ModelConfig"
Cohesion: 0.07
Nodes (20): ModelConfig, Capability and pricing metadata for one model id., ModelRegistry, Add or replace a model (used by runtime discovery)., Models satisfying every stated requirement, best first. Ordering is by…, The biggest-context model available — the context-overflow escape hatch., The lowest-cost model that still meets the requirements., The fastest model by declared speed tier. (+12 more)

### Community 1089 - "TestTheAgentRunsCurrentCode"
Cohesion: 0.33
...
### Community 1091 - "test_cost_attribution.py"
Cohesion: 0.29
Nodes (3): Tests for packages/ai/cost_tracker.py — per-model cost attribution. Covers: -…, TestClearStats, TestEnvOverrides
...
### Community 1107 - "DirectChatDoctor"
Cohesion: 0.47
...
### Community 1114 - "resolve_hermes_base_url"
Cohesion: 0.47
...
### Community 1115 - "steering_for_task"
Cohesion: 0.47
...
### Community 1116 - ".create_session"
Cohesion: 0.33
Nodes (3): Import a session from an export dict. Returns the new session_id., Drop oldest sessions if we exceed the maximum., Create a new session and return its ID.

### Community 1117 - "inspect-agent-runtime.sh"
Cohesion: 0.60
Nodes (3): fail(), inspect-agent-runtime.sh script, usage()

### Community 1118 - "Prompt Library Changelog"
Cohesion: 0.40
Nodes (4): Added, Format, Prompt Library Changelog, [Unreleased]

### Community 1119 - "Proof"
Cohesion: 0.40
...
### Community 1120 - "build_llama_cpp.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 1121 - "TestAgentRunnerExecution"
Cohesion: 0.33
...
### Community 1122 - "download_glm52_weights.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 1123 - "download_glm52_weights.sh script"
Cohesion: 0.70
Nodes (4): download_glm52_weights.sh script, fail(), ok(), warn()

### Community 1124 - "setup_colibri.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), Warn(), W()

### Community 1125 - "setup_colibri.sh script"
Cohesion: 0.70
Nodes (4): setup_colibri.sh script, fail(), ok(), warn()

### Community 1126 - "status_colibri_server.ps1"
Cohesion: 0.70
Nodes (4): Fail(), Ok(), W(), Warn()

### Community 1128 - "send_digest"
Cohesion: 0.40
Nodes (4): Send the digest text via NotificationDispatcher (Telegram)., send_digest(), patch, TestSendDigest

### Community 1129 - "TestDirectChatAgentExecution"
Cohesion: 0.33
...
### Community 1132 - "AuditLog"
Cohesion: 0.08
Nodes (24): AuditEvent, AuditLog, One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer…, Store and emit *event*. Never raises., Return the most recent events, newest first, optionally filtered., Aggregate view for the dashboard and the metrics endpoint. ``would_block`` is… (+16 more)

### Community 1136 - "TestGhIsNotReAuthenticated"
Cohesion: 0.40
...
### Community 1140 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 1153 - "test_rate_limiter_concurrency.py"
Cohesion: 0.60
...
### Community 1155 - "_fetch_pytest_failures.py"
Cohesion: 0.50
...
### Community 1156 - "render"
Cohesion: 0.50
Nodes (3): RENDER_API_KEY, docker, render

### Community 1157 - "nvidia_live_test.py"
Cohesion: 0.40
...
### Community 1158 - "test_repo_access_preflight_fails_when_git_ls_remote_fails"
Cohesion: 0.40
...
### Community 1159 - "test-anthropic.js"
Cohesion: 0.50
Nodes (3): ref_anthropic_ai_sdk, { Anthropic }, client

### Community 1160 - ".__init__"
Cohesion: 0.50
Nodes (3): Any, Resolve the default executor model via the catalog (UNIT 7). Was hardcoded to…, _resolve_default_executor_model()

### Community 1162 - "stop_colibri_server.ps1"
Cohesion: 0.83
Nodes (3): Fail(), Ok(), W()

### Community 1163 - "submit_simple_task"
Cohesion: 0.50
Nodes (4): Submit a task to the agent planner., Submit a simple task via the tasks API., submit_simple_task(), submit_task()

### Community 1164 - "client"
Cohesion: 0.15
Nodes (13): auth_headers(), client(), TestClient, TestClient for the backend FastAPI app (one per module for speed)., Login once and return auth headers for the entire module., admin_jwt(), _extract_admin_token(), _login_admin() (+5 more)

### Community 1165 - "gen_screenshots.py"
Cohesion: 0.47
Nodes (5): main(), out_path(), Path, Generate Langfuse and Telegram mockup screenshots for documentation., save_html_screenshot()

### Community 1167 - "parse_pytest_failures.py"
Cohesion: 0.40
...
### Community 1174 - "run_freebuff_bot.py"
Cohesion: 0.47
...
### Community 1175 - "WebhookSendRequest"
Cohesion: 0.67
Nodes (3): BaseModel, Body for ``POST /api/connectors/webhook/send`` (rule 11 — no raw dict in)., WebhookSendRequest

### Community 1224 - "test_a_failed_filing_releases_the_cooldown_so_the_next_recurrence_retries"
Cohesion: 0.40
...
### Community 1225 - "allow_paid"
Cohesion: 0.11
Nodes (18): Example: Access your home PC models from any machine. Install: pip install…, allow_paid(), _fetch_policy(), Fetch the provider policy from the backend API. Never raises., Return True if paid providers (Anthropic) are allowed by policy., _call_review_llm(), get_pr_diff(), get_pr_files() (+10 more)

### Community 1228 - ".daily_active_users"
Cohesion: 0.40
...
### Community 1282 - "TestDirectChatNonBlocking"
Cohesion: 0.40
...
### Community 1283 - "TestModelRoleSeparation"
Cohesion: 0.40
...
### Community 1286 - "model_discovery.py"
Cohesion: 0.07
Nodes (31): Return the discovered model list for *provider_id*, or ``[]`` if unknown., _served_models(), cached_models(), dead_models(), discover_models(), _fresh_entry(), _models_url(), _parse_ids() (+23 more)

### Community 1288 - "webhook_secret"
Cohesion: 0.50
...
### Community 1291 - "TestHermesKeepAlive"
Cohesion: 0.40
...
### Community 1297 - "test_probe_reports_dns_failure_end_to_end"
Cohesion: 0.50
...
### Community 1299 - "test_a_scheduled_diagnosis_is_held_until_it_completes"
Cohesion: 0.67
...
### Community 1301 - "get_status"
Cohesion: 0.67
Nodes (3): get_status(), get, Get current service status.

## Knowledge Gaps
- **3073 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3068 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14049 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **172 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AgentRunner` connect `AgentRunner` to `os`, `TestAgentLoopMCPIntegration`, `backend/server.py`, `TokenBudget`, `proxy.py`, `coordinator.py`, `ContextPruner`, `github_tools.py`, `Added`, `TaskSpec`, `UserMemoryStore`, `test_agent_runner.py`, `GitHubTools`, `failover_chat_completion`, `test_ceo_micromanager.py`, `Fixed`, `E2BSandboxSession`, `PolicyEngine`, `Fixed`, `failover_client.py`, `AdminScreen.jsx`, `test_verification_strategies.py`, `test_empirical_verify.py`, `WorkspaceTools`, `[Unreleased]`, `test_agent_free_brain.py`, `Path`, `test_governance_api.py`, `Section-by-Section Acceptance Criteria`, `test_backend_server_features.py`, `Added`, `ContextManager`, `AgentJobRequest`, `MultiAgentSwarm`, `pytest`, `Issue #1427: quick-note:https://github.com/bingreeky/JIT`, `Competitor Analysis — Autonomous AI Agency`, `WorkflowOrchestrator`, `WorkflowEngine`, `test_ceo_supervision.py`, `Added`, `test_repo_connection.py`, `AdaptiveHalter`, `test_ceo_cross_verify.py`, `TestAgentRunnerSafety`, `StuckDetector`, `TestAgentRunnerExecution`, `workflow_orchestrator.py`, `test_agent_tool_governance.py`, `agent/models.py`, `ReactScratchpad`, `_resolve_push_token`, `AgentPlan`, `direct_chat.py`, `Agency Core — Progress & Resume Log`, `Killer TODO Roadmap — local-llm-server`, `AgentSessionStore`, `TestAgentRunnerPublicPlanMethod`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `_fixture()` connect `_fixture` to `Specialist`, `SeoAuditEngine`, `Changed`, `test_llm_router_queue_cache.py`, `get_feature_matrix`, `test_refresh_agent_built_proof.py`, `test_brain_patch_service_token.py`, `Fixed`, `test_llm_router_resilience.py`, `test_operational_incidents.py`, `TaskSpec`, `Added`, `Agency`, `failover_chat_completion`, `E2BSandboxSession`, `PolicyEngine`, `TestProviderRouter`, `TestAgentAutoAssignment`, `test_phase5_doctor.py`, `test_ping.py`, `Task`, `test_unit8_model_catalog.py`, `TaskStatus`, `controls_app`, `secrets_store.py`, `KeyStore`, `test_model_router.py`, `parse_event_stream`, `Added`, `TaskStore`, `pytest`, `OperationalIncidentTracker`, `AgentRunner`, `llm/router.py`, `config.py`, `test_task_brain_preflight.py`, `DetectedSystem`, `resolve_e2b_config`, `test_governance_enforcement.py`, `AgentDefinition`, `WorkflowRun`, `AgentSwarm`, `UsageEvent`, `ArtifactStore`, `yaml`, `clear_cooldowns`, `orchestrator`, `SQLiteStore`, `test_startup_warmup.py`, `SelfHealingAgent`, `TestChatHistoryStore`, `TestClient`, `test_rate_limiter.py`, `test_shared_state.py`, `AuditLog`, `test_knowledge_sync.py`, `test_sam_voice.py`, `AgentPlan`, `test_procedural_memory.py`, `test_new_features_e2e.py`, `heal_signature`, `_step`, `_Recorder`, `test_tasks_reconciler_todo_requeue.py`, `test_context_rulebook.py`, `client`, `get_task_store`, `test_llm_router_e2e.py`, `ContextPruner`, `test_trend_watcher.py`, `test_persistent_memory.py`, `test_openclaw_endpoints.py`, `test_brain_availability_doctor.py`, `test_brain_failover.py`, `TestNormalizeAnthropicOutputFormat`, `test_spec_store.py`, `_build_execution_request`, `models/seo_audit.py`, `test_trend_scoping.py`, `test_hermes_in_process.py`, `NotificationDispatcher`, `Usage`, `_mock_provider_records`, `test_task_service_failed_comment.py`, `OllamaCircuitBreaker`, `test_video_transcript.py`, `test_daily_2026_07_27.py`, `TestEstimateTokensForMessages`, `allow_paid`, `cost_tracker.py`, `wiki_client`, `test_features_api.py`, `analyze_page`, `_process_task_callback`, `test_agent_tool_governance.py`, `SchedulerStore`, `_resolve_push_token`, `test_agile_api.py`, `test_app_settings.py`, `467 Brutal Audit — File-by-File Status`, `test_control_plane_api.py`, `LogMonitor`, `test_spa_trailing_slash_redirect.py`, `TestHarnessAdapter`, `model_router.py`, `_clean_phases`, `model_discovery.py`, `OrchestratorQueue`, `Path`, `test_telegram_webhook.py`, `ChatHistoryStore`, `trusted_citations`, `LocalBrainStore`, `keys.py`, `TestRuntimeControl`, `test_provider_enable_disable.py`, `test_issue_intake.py`, `OutputFilter`, `TestCountTokensEndpoint`, `ImprovementLoop`, `TestAuthAndTaskCreation`, `test_provider_state_durability.py`, `test_state_file_merge_drivers.py`, `SecurityScanner`, `test_llm_router_disabled.py`, `test_kimi_bridge_server.py`, `TestClient`, `test_e2b_data_flow.py`, `WorkflowOrchestrator`, `test_direct_chat_evolution.py`, `_resolve_user_github_token`, `TestNormalizeResponseFormat`, `start_background_services`, `test_skill_registry_boot_refresh.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_dependabot_sweep_workflow.py`, `TestMongoGate`, `test_task_clarification.py`, `_redact_for_notification`, `test_ceo_router.py`, `test_session_retro.py`, `test_regression.py`, `Configuration Reference`, `test_process_quick_note_workflow.py`, `test_workflow_shell_vars_are_declared.py`, `ProviderRouter`, `test_service_token.py`, `TestAuthAndTaskOwnership`, `test_agent_free_brain.py`, `test_key_pool.py`, `TrafficDirector`, `test_doctor_coding_brain.py`, `checkpoint.py`, `test_executive_advisory_api.py`, `captured`, `OrchestratorCheckpointStore`, `TestChatFallbackAndApproval`, `test_dashboard_cache.py`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `Specialist` connect `Specialist` to `company_api.py`, `MongoDBStore`, `SQLiteStore`, `Company`, `CompanyGraphStore`?**
...
- **Should `os` be split into smaller, more focused modules?**
  _Cohesion score 0.011475057852422645 - nodes in this community are weakly interconnected._
- **Should `Specialist` be split into smaller, more focused modules?**
  _Cohesion score 0.029120559114735003 - nodes in this community are weakly interconnected._
- **Should `SeoAuditEngine` be split into smaller, more focused modules?**
