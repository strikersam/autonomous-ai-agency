# Graph Report - local-llm-server  (2026-09-13)

## Corpus Check
- 1506 files · ~2,184,380 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30553 nodes · 57034 edges · 1254 communities (1100 shown, 106 thin omitted)
...
## Communities (1254 total, 106 thin omitted)

### Community 0 - "backend/server.py"
Cohesion: 0.01
Nodes (276): get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, get_harness_adapter(), admin_seed(), _agent_provider_failure_response(), _agent_timeout_fallback_response(), AgentToolCallEntry, ApiKeyCreate (+268 more)

### Community 1 - "TaskSpec"
Cohesion: 0.03
Nodes (110): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, Any, TaskResult, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Run aider non-interactively via `--message` flag., Adapter for Aider — TIER 3 specialized git-aware code editor. (+102 more)
...
Cohesion: 0.03
Nodes (143): AgentDefinition, AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+135 more)

### Community 5 - "Task"
Cohesion: 0.03
Nodes (85): Task, Full task/issue document., Task, _brain_is_configured(), _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing(), Any (+77 more)
...
Cohesion: 0.02
Nodes (108): Test hook — clear all usage accounting., reset(), CacheManager, cosine_similarity(), _Entry, LRUCache, payload_key(), Any (+100 more)
...
Cohesion: 0.02
Nodes (120): get_cache(), The process-wide cache manager., AgentPolicy, _apply_env_overrides(), _apply_key_env(), _build(), _coerce(), config_dir() (+112 more)

### Community 11 - "_fixture"
Cohesion: 0.01
Nodes (139): parse_event_stream(), Reduce a ``--mode json`` NDJSON stream to a :class:`ParsedRun`. Kept a module-…, app_client(), _clear_discovered_models(), _clear_response_cache(), client(), _isolate_brain_data_layer(), _isolate_operator_provider_state() (+131 more)

### Community 12 - "CompanyGraphService"
Cohesion: 0.03
Nodes (76): BusinessCategory, ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraph, CompanyGraphResponse, CompanyGraphSnapshot, CompanyResponse (+68 more)
...
Cohesion: 0.03
Nodes (121): get_ceo_dispatcher(), _merge_changed_files(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., Collect changed_files across all specialists into a single de-duped list., reset_ceo_dispatcher(), BoundContext, ClassifyOutput (+113 more)
...
Cohesion: 0.03
Nodes (145): backend/ceo_router.py — observability and manual control for the CEO. Surfaces…, Reject non-admin callers for routes that spend provider budget. Delegates to…, _require_admin(), account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company() (+137 more)
...
Cohesion: 0.04
Nodes (53): MergeDecision, _NoopStore, Phase, Any, Enum, Exception, Approve a run paused at the ApprovalGate. The caller must re-invoke…, Inject additional instructions into an in-flight run (no state change). Backs… (+45 more)

### Community 20 - "LLMRequest"
Cohesion: 0.04
Nodes (107): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, OpenAICompatible, Adapter for any endpoint speaking the OpenAI chat-completions shape. Adding a…, build_provider(), Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the…, resolve_kind() (+99 more)
...
### Community 24 - "test_ephemeral_reaper.py"
Cohesion: 0.10
...
Cohesion: 0.05
Nodes (45): Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, Ollama provider adapter — local LLM inference., packages.ai — provider abstraction, model registry, and failover manager., ProviderManager, Any (+37 more)
...
Cohesion: 0.03
Nodes (60): KnowledgeItem, Find knowledge items matching any of the given tags., Structured knowledge about the company, systems, or processes., Add a knowledge item to a company. Args: company_id: Company ID title:…, Get a knowledge item by ID., Search knowledge items., Get knowledge items by tags for a company. Args: company_id: Company ID tags:…, MongoDBStore (+52 more)
...
### Community 29 - "llm/router.py"
Cohesion: 0.02
Nodes (121): Attempt, int, text_key(), failover_chat_completion_via_router(), openai_body_from_response(), Any, packages/llm/compat.py — backwards-compatible bridges to the legacy call paths.…, Router-backed implementation of ``packages.ai.failover_client``'s entry point.… (+113 more)

### Community 30 - "direct_chat.py"
Cohesion: 0.04
Nodes (76): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), AcceptedJob, AgentJobEnvelope, CompletedJob, DirectChatState, FailedJob (+68 more)

### Community 31 - "E2BSandboxSession"
Cohesion: 0.04
Nodes (86): MCPUnavailableError, Raised when the MCP server is unreachable or the circuit is open., E2BSandboxSession, _inject_token(), maybe_attach_e2b(), Any, Create the sandbox. Raises :class:`MCPUnavailableError` on failure., Kill the sandbox. Idempotent; never raises (best-effort cleanup). (+78 more)
...
### Community 37 - "test_runtimes.py"
Cohesion: 0.04
Nodes (36): runtimes/health.py — RuntimeHealthService. Periodically polls all registered…, Any, runtimes/manager.py — RuntimeManager., Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking). (+28 more)

### Community 38 - "ceo_dispatcher.py"
Cohesion: 0.08
Nodes (27): CEOResult, _complexity_rank(), _offload(), Any, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is…, Build an agent.coordinator.TaskSpec from a CEO SpecialistTask., Aggregated output from a multi-specialist execution. (+19 more)
...
Cohesion: 0.05
Nodes (47): Execute work through the 11-phase golden path., Called by APScheduler when a cron fires. Dispatches to the orchestrator. This…, _scheduler_on_fire(), workflow_orchestrator_execute(), ExecutionRequest, get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper). (+39 more)
...
Cohesion: 0.02
Nodes (146): BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), is_north_mini_code_default() (+138 more)
...
Cohesion: 0.03
Nodes (92): get_catalog_models_route(), Return the mirrored model catalog (advisory-only). Flag-gated by…, Force a catalog mirror rebuild + persist (admin only). Rebuilds the catalog…, sync_catalog_route(), all_provider_ids(), _provider_ids_from_literal(), Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the… (+84 more)

### Community 50 - "PolicyEngine"
Cohesion: 0.03
Nodes (78): AgentIdentity, _coerce_text(), new_session_id(), Any, packages/governance/identity.py — who is acting, and under what mandate. Docker…, The acting subject of a governed action. Immutable by construction so a…, Return a copy carrying extra execution context. Used when context arrives after…, Serialise for audit rows and API responses. Every field here is non-secret by… (+70 more)

### Community 51 - "OnboardingService"
Cohesion: 0.04
Nodes (50): OnboardingProgress, Tracks the onboarding progress for a company., Result of a repository scan with detected stack and systems., Inferred technology stack from website/repo analysis., RepoScanResult, StackInference, OnboardingService, Workflow (+42 more)

### Community 52 - "_start_ceo_agency"
Cohesion: 0.31
...
Cohesion: 0.05
Nodes (52): is_commercial_provider(), _normalized_provider_type(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Infer an access tier from *hostname*, longest matching suffix winning. Returns…, _tier_from_hostname() (+44 more)

### Community 56 - "services/background.py"
Cohesion: 0.04
...
### Community 62 - "Surface"
Cohesion: 0.04
Nodes (71): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _egress_policy_reason(), Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Write the audit row for a completed (or blocked) MCP tool call., Import the governance layer once, recording why if it is absent., record() (+63 more)
...
Cohesion: 0.03
Nodes (73): Backoff policy for retryable failures., Drop the cached config (test hook)., reset(), RetryConfig, BreakerState, Enum, str, packages/llm/health.py — provider health tracking and circuit breaking. Two… (+65 more)
...
Cohesion: 0.03
Nodes (82): DetectedSystem, Evidence, Request to scan a website and infer its stack., Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., Request to scan a repository and infer its stack., A business system detected on a company's website or in their stack., Get the most confident evidence description. (+74 more)
...
Cohesion: 0.03
Nodes (45): Company, The core company entity - root of the Company Graph., requires_db, Get a company by ID. Args: company_id: Company ID Returns: Company instance or…, Update a company. Args: company_id: Company ID **kwargs: Fields to update…, List companies with optional filtering. Args: owner_id: Filter by owner ID…, Initialize the service. Args: store: Optional CompanyGraphStore instance. If…, CompanyGraphStore (+37 more)
...
Cohesion: 0.10
Nodes (38): _drive(), _enforce(), governance_on(), _observations(), Any, MonkeyPatch, Path, The executor loop must not have a side door around the governance gate.… (+30 more)
...
Cohesion: 0.07
Nodes (47): _load_script_module(), Any, ModuleType, agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Dynamically load a pure-stdlib helper module from .github/scripts/. Returns…, Zero-key internet access: pages, YouTube transcripts, search, RSS. Every public…, Read a web page as plain text, following the same fallback chain (direct ->…, GET *url*, re-validating every redirect hop against ``unsafe_target_reason`` —… (+39 more)

### Community 86 - "lifespan"
Cohesion: 0.05
Nodes (47): all_settings(), _as_bool(), _as_int(), ephemeral_ttl_hours(), ephemeral_ttl_hours_cached(), get_setting(), _maybe_schedule_refresh(), onboarding_gate_enabled() (+39 more)

### Community 87 - "test_self_heal_v2.py"
Cohesion: 0.04
Nodes (66): Reset the singleton (for tests)., reset_failover_manager(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog(), _heal_stuck_tasks(), _heal_task_duplicates() (+58 more)
...
Cohesion: 0.06
Nodes (47): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), FailoverResult, _is_billing_refusal(), _is_ollama(), _key_pool() (+39 more)
...
Cohesion: 0.03
Nodes (75): _brain_provider_status(), get_brain_policy_route(), Return per-provider metadata for the GET endpoint. Iterates every provider in…, Return the discovered model list for *provider_id*, or ``[]`` if unknown., Return the active brain config + per-provider key-present flags. The response…, _served_models(), provider_api_key(), provider_base_url() (+67 more)

### Community 101 - "telegram_bot.py"
Cohesion: 0.08
Nodes (47): _admin_headers(), _answer_callback(), _api_headers(), _check_rate_limit(), cmd_control(), cmd_cost(), cmd_keylist(), cmd_merge() (+39 more)
...
Cohesion: 0.06
Nodes (44): _check_extra_kwargs(), _enforce_signature(), _note_phase_end(), _note_phase_start(), loop.py — AgentRunner: plan → execute → verify loop with locked tool signatures., Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Call-time resolver for an agent role model id. Delegates to… (+36 more)
...
Cohesion: 0.08
Nodes (45): allow_paid_brain(), BrainResolution, get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), _norm(), _pick_from_records(), Any (+37 more)

### Community 114 - "ai/router.py"
Cohesion: 0.05
Nodes (56): clear_cooldowns(), _dead_model_key(), get_dead_models(), _is_model_dead(), is_provider_on_cooldown(), _mark_model_dead(), mark_provider_failed(), ProviderFallbackError (+48 more)
...
Cohesion: 0.07
Nodes (55): governance_enabled(), True when the governance layer should run at all. A single global off switch…, _blocked_result(), _governance_check(), _governance_identity(), Any, TaskResult, Build the failed TaskResult returned when policy denies a dispatch. A result… (+47 more)
...
### Community 136 - "get_task_store"
Cohesion: 0.04
Nodes (76): _autonomy_bg_cycle(), _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody, Shared purge implementation for the admin endpoint and the boot hook., Background CEO cycle + task dispatch. Runs fire-and-forget., Submit a quick-note URL or instruction from the dashboard FAB., apply_from_db() (+68 more)
...
Cohesion: 0.04
Nodes (78): _acquire_provider_probe(), CommercialFallbackRequiredError, _exponential_backoff_cooldown(), extract_openai_text(), _get_director(), _normalize_nvidia_base_url(), _notify_watchdog(), _ollama_reasoning_effort() (+70 more)

### Community 140 - "TestAdapterMetadata"
Cohesion: 0.04
Nodes (26): Resolve the base URL of the agency's own Hermes server. Precedence:…, resolve_hermes_base_url(), HermesAdapter, Any, AsyncClient, Adapter for Hermes Agent — FIRST CLASS autonomous runtime., OpenHandsAdapter, Any (+18 more)
...
Cohesion: 0.06
Nodes (14): get_harness_registry(), HarnessMetrics, HarnessRegistry, HarnessSessionRecord, _NoopDB, Any, BaseModel, services/harness_registry.py — Persistent Harness Registry Tracks which AI… (+6 more)
...
### Community 220 - "RuntimeHealth"
Cohesion: 0.04
Nodes (35): Check whether Docker is available and report the adapter's runtime health.…, GooseAdapter, Any, Adapter for Goose — TIER 2 general-purpose local runtime., JCodeAdapter, Any, Path, Write .jcode/mcp.json in the workspace, pointing at our proxy's MCP endpoint.… (+27 more)
...
### Community 230 - "skill_bindings.py"
Cohesion: 0.05
Nodes (40): Enum, SuperClaude Workflow Engine — Workflow, Task, and topological DAG execution.…, Return tasks whose dependencies are all satisfied., Number of tasks in the workflow., Number of completed tasks., Number of failed tasks., Executes workflows using topological ordering., Register a workflow with the engine. (+32 more)
...
### Community 272 - "get_skill_bindings"
Cohesion: 0.09
Nodes (25): get_skill_bindings(), Live council reviewer — deterministic, rules-based multi-perspective review…, Get the singleton SkillBindings instance., _run_council_review(), parametrize, browserbase / agent-browser / perplexity are MCP/CLI tools, NOT runtime skills.…, test_publicly_advertised_skill_is_registered(), test_skills_not_in_registry_are_not_claimed_as_registered_skills() (+17 more)
...
### Community 298 - "test_backend_server_features.py"
Cohesion: 0.06
Nodes (43): _anthropic_headers(), _anthropic_payload(), _anthropic_response_text(), _auth_headers(), chat_completion_text(), list_openai_models(), LlmProviderConfig, normalize_base_url() (+35 more)

### Community 299 - "facade.py"
Cohesion: 0.06
Nodes (46): _get_current_user_thunk(), _get_optional_user_thunk(), Request, create_refresh_token(), get_current_user(), get_optional_user(), Get user if authenticated, otherwise return None (for public endpoints)., N5 dual-auth dependency: accept EITHER a user session OR a service token. Used… (+38 more)
...
Cohesion: 0.08
Nodes (39): Replace the process-wide audit log. Tests only., reset_audit_log(), Replace the process-wide engine. Tests only., reset_policy_engine(), isolated_governance(), Give every test its own engine, audit log, and approval store., isolated_governance(), Give every test its own engine, audit log, and approval store. (+31 more)
...
### Community 341 - "OrchestratorSupervisor"
Cohesion: 0.10
...
### Community 356 - "test_nvidia_default_model.py"
Cohesion: 0.17
...
### Community 366 - "NvidiaProvider"
Cohesion: 0.07
Nodes (7): CerebrasProvider, NvidiaProvider, NVIDIA NIM — free LLM provider (meta/llama-3.3-70b-instruct)., OllamaProvider, RateLimit, Provider rate limit info., Return the provider's rate limits.

### Community 367 - "agents/api.py"
Cohesion: 0.14
...
### Community 384 - "TestPoliciesGovernanceStableClaim"
Cohesion: 0.25
...
Cohesion: 0.12
Nodes (24): openclaw_command(), openclaw_websocket(), websocket, WebSocket endpoint for iOS / mobile web UI pairing + command routing., HTTP command endpoint — same as the WebSocket command path but over POST. More…, get_failover_manager(), Return the singleton BrainFailoverManager., _cmd_chat() (+16 more)
...
### Community 404 - "HTTPException"
Cohesion: 0.02
Nodes (148): AgentStatusEntry, AgentStatusResponse, BrainTestRequest, _build_agent_status_snapshot(), clear_cost_attribution(), clear_response_cache(), create_github_pr(), create_mcp_server() (+140 more)
...
### Community 443 - "flesch_reading_ease"
Cohesion: 0.24
...
### Community 446 - "test_phase6_workflow.py"
Cohesion: 0.16
...
### Community 449 - "test_telegram_task_callback.py"
Cohesion: 0.12
...
### Community 476 - "weekly_digest.py"
Cohesion: 0.13
...
### Community 503 - "test_bootstrap_source_id_index.py"
Cohesion: 0.20
...
### Community 516 - "test_telegram_service_webhook.py"
Cohesion: 0.18
...
### Community 555 - "test_agent_free_brain.py"
Cohesion: 0.06
...
### Community 609 - "test_llm_router_tpm.py"
Cohesion: 0.20
...
### Community 628 - "test_setup_api.py"
Cohesion: 0.23
...
### Community 635 - "AgentJobSnapshot"
Cohesion: 0.23
...
### Community 667 - "test_workspace_ref_path_preflight.py"
Cohesion: 0.22
...
### Community 674 - ".on_task_complete"
Cohesion: 0.16
...
### Community 699 - "test_task_brain_preflight.py"
Cohesion: 0.31
...
### Community 710 - "test_ceo_cross_verify.py"
Cohesion: 0.29
...
### Community 750 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.22
...
### Community 755 - "_redact_for_notification"
Cohesion: 0.24
...
### Community 790 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 828 - "DirectChatDoctor"
Cohesion: 0.47
...
### Community 840 - "_decompose_into_subtasks"
Cohesion: 0.33
...
### Community 851 - "TestWorkflowIntegration"
Cohesion: 0.33
...
### Community 888 - "TestTelegramNotifications"
Cohesion: 0.40
...
### Community 891 - "verify_pr_exists"
Cohesion: 0.50
...
### Community 1116 - "test_v4_reliability.py"
Cohesion: 0.05
Nodes (30): PreflightReport, agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, Fail preflight when the provider extension is configured but missing. Without…, Any, Structured, actionable preflight validation issue., Preflight result returned before a runtime task starts., Return runtime-specific dependency declarations for preflight., Return a best-effort tool availability report for diagnostics. (+22 more)
...
- **3532 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3527 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 13478 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **106 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `TaskSpec`, `test_operational_incidents.py`, `TaskStatus`, `Task`, `test_brain_patch_service_token.py`, `test_telegram_service_webhook.py`, `test_llm_router_queue_cache.py`, `test_gh_brain_failover.py`, `config.py`, `test_llm_router_strategies.py`, `BrainWatchdog`, `test_ai_insights.py`, `test_model_router.py`, `AgentJobManager`, `WorkflowRun`, `SelfHealingAgent`, `TrafficDirector`, `test_ephemeral_reaper.py`, `test_company_api.py`, `Agency`, `test_telegram_webhook.py`, `direct_chat.py`, `E2BSandboxSession`, `SeoAuditEngine`, `ExecutionRequest`, `test_agent_free_brain.py`, `test_failover_client_shared.py`, `NotificationDispatcher`, `test_unit8_model_catalog.py`, `PolicyEngine`, `OnboardingService`, `_start_ceo_agency`, `PrimeAgentAdapter`, `ProviderConfig`, `services/background.py`, `test_ceo_router.py`, `test_openclaw_endpoints.py`, `test_sam_livekit.py`, `test_llm_router_resilience.py`, `validate_policy_text`, `ChatHistoryStore`, `SQLiteStore`, `resolve_e2b_config`, `TokenBudget`, `e2e/test_browser.py`, `telegram_inbound_handlers.py`, `test_agent_tool_governance.py`, `test_autonomy_status.py`, `test_self_heal_v2.py`, `test_local_brain_state.py`, `test_phase5_doctor.py`, `test_refresh_agent_built_proof.py`, `ResearchTask`, `test_tasks_reconciler_todo_requeue.py`, `test_procedural_memory.py`, `test_llm_router_tpm.py`, `test_llm_router_e2e.py`, `claim`, `KeyStore`, `OutputFilter`, `ai/router.py`, `TestClient`, `test_startup_warmup.py`, `KeyPool`, `UserRole`, `sam`, `ArtifactStore`, `BackgroundAgent`, `test_new_features_e2e.py`, `test_context_rulebook.py`, `get_task_store`, `test_trend_scoping.py`, `ProviderRouter`, `SeoFixer`, `_step`, `run_task`, `test_changelog_check_workflow.py`, `test_trend_watcher.py`, `test_openclaw_gateway.py`, `MCPClient`, `TestHarnessAdapter`, `.run`, `TestEstimateTokensForMessages`, `.on_task_complete`, `FeatureMaturity`, `_resolve_brain_provider`, `test_telegram_freebuff.py`, `test_e2b_data_flow.py`, `test_process_quick_note_workflow.py`, `TestSwarmRoleRouting`, `OllamaCircuitBreaker`, `_Budget`, `_job_text`, `test_e2b_task_wiring.py`, `WorkflowRun`, `test_task_brain_preflight.py`, `test_colibri_brain_shim.py`, `test_doctor_service_token_check.py`, `test_features_api.py`, `test_video_transcript.py`, `test_dashboard_cache.py`, `nvidia_models.py`, `TrendWatcher`, `AgentSwarm`, `RuntimeHealth`, `test_portfolio_intake.py`, `emit_chat_observation`, `WorkflowBuildRequest`, `brain_failover.py`, `test_agile_api.py`, `test_app_settings.py`, `TestClient`, `test_task_clarification.py`, `ContextPruner`, `test_control_plane_api.py`, `test_daily_2026_07_27.py`, `test_persistent_memory.py`, `_resolve_user_github_token`, `test_platform_controls.py`, `ScheduledJob`, `get_skill_bindings`, `test_kimi_bridge_server.py`, `test_sam_voice.py`, `TestAgentRunnerSafety`, `test_nvidia_model_discovery.py`, `test_mcp_governance.py`, `ImprovementLoop`, `test_brain_availability_doctor.py`, `analyze_page`, `rate_limiter.py`, `SecurityScanner`, `_resolve_push_token`, `test_catalogue_probe.py`, `OrchestratorQueue`, `TestMongoGate`, `test_purge_backlog.py`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `test_provider_enable_disable.py`, `test_webui_provider_priority.py`, `_undeclared`, `test_service_token.py`, `test_harness_spec.py`, `test_agency_fix.py`, `_Recorder`, `api_keys_for`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `model_discovery.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_telegram_diag_endpoint.py`, `test_knowledge_sync.py`, `test_ping.py`, `test_telegram_task_callback.py`, `test_freebuff_bot.py`, `test_provider_state_durability.py`, `ClaudeCodeAdapter`, `TestNormalizeResponseFormat`, `test_voice_pipeline.py`, `client`, `clear_override`, `cost_tracker.py`, `test_doctor_coding_brain.py`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `TaskSpec`, `proxy.py`, `BackgroundAgent`, `FreeBuffAgent`, `test_ceo_dispatcher.py`, `UserMemoryStore`, `AgentJobManager`, `ContextManager`, `WorkflowRun`, `ReactScratchpad`, `Agency`, `direct_chat.py`, `E2BSandboxSession`, `TestAgentRunnerSafety`, `test_empirical_verify.py`, `MCPClient`, `test_agent_chat_integration.py`, `.run`, `StuckDetector`, `ceo_dispatcher.py`, `MultiAgentSwarm`, `AgentSessionStore`, `TestAgentRunnerExecution`, `test_failover_client_shared.py`, `test_agent_free_brain.py`, `PolicyEngine`, `test_daily_automation_2026_07_11.py`, `WorkspaceTools`, `Surface`, `_resolve_push_token`, `test_ceo_cross_verify.py`, `test_ceo_supervision.py`, `LocalWorkspace`, `TokenBudget`, `test_agent_tool_governance.py`, `test_self_heal_v2.py`, `test_governance_api.py`, `InternalAgentAdapter`, `loop.py`, `GitHubTools`, `AdaptiveHalter`, `ContextPruner`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `backend/server.py`, `test_all_providers_discovery.py`, `implement_agent.py`, `test_bedrock_live.py`, `TrafficDirector`, `settings.py`, `direct_chat.py`, `kimi_bridge_provider_config`, `ProviderConfig`, `TestLegacyRouterCacheTTL`, `test_anthropic_refusal_fallback.py`, `test_colibri_brain_shim.py`, `TestRouterIntegration`, `test_bedrock_provider.py`, `_payload`, `TestLegacyRouterServerFallback`, `is_anthropic_base_url`, `failover_client.py`, `NvidiaProvider`, `TestAnthropicPayloadStructuredOutput`, `ai/router.py`?**
