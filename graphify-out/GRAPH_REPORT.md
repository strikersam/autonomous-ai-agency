...
## Corpus Check
- 1436 files · ~2,055,183 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 28479 nodes · 53186 edges · 1196 communities (1087 shown, 109 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 3663 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2598430a`
...
## Communities (1196 total, 109 thin omitted)

### Community 0 - "HTTPException"
Cohesion: 0.04
Nodes (111): _get_current_user_thunk(), _get_optional_user_thunk(), Request, admin_seed(), call_llm(), create_access_token(), create_api_key(), create_github_pr() (+103 more)

### Community 1 - "_fixture"
Cohesion: 0.02
Nodes (98): _accumulate_usage(), _assistant_messages(), _iter_events(), _message_text(), parse_event_stream(), ParsedRun, Any, Structured view of one ``--mode json`` event stream. (+90 more)
...
Cohesion: 0.07
Nodes (40): _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), is_commercial_provider(), _mark_model_dead(), _normalized_provider_type(), _notify_watchdog() (+32 more)
...
Cohesion: 0.03
Nodes (95): ApprovalPolicy, BusinessSystem, Company, CompanyCreateRequest, CompanyGraph, CompanyGraphResponse, CompanyGraphSnapshot, CompanyResponse (+87 more)

### Community 7 - "TaskStore"
Cohesion: 0.03
Nodes (107): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, ExecutionApprovalRequest, ExecutionLogEntry (+99 more)

### Community 8 - "brain_config.py"
Cohesion: 0.03
Nodes (107): BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store(), invalidate_brain_config_cache(), provider_base_url() (+99 more)
...
Cohesion: 0.03
Nodes (90): AgentDefinition, AgentStore, Any, field_validator, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised., Return all agents owned by *owner_id*, optionally including public agents. (+82 more)
...
### Community 25 - "Task"
Cohesion: 0.04
Nodes (50): _coerce_ts(), Any, field_validator, Full task/issue document., Update the updated_at timestamp., Coerce ISO-8601 datetime strings (from DB) to float timestamps., Dict-compatible accessor. Log entries are serialized to/from dicts in many…, Task (+42 more)
...
Cohesion: 0.04
Nodes (51): MongoDBStore, Any, Company, ObjectId, Prepare a Pydantic model for SQLite storage., Prepare a SQLite row for Pydantic model., List companies with optional filtering., MongoDB implementation of the Company Graph store. Uses Motor (async MongoDB… (+43 more)

### Community 28 - "ExecutionRequest"
Cohesion: 0.03
Nodes (125): get_orchestrator_checkpoint_store(), BoundContext, ClassifyOutput, ExecutionRequest, ExecutionResult, _get_ceo_dispatcher(), get_ceo_fallback_stats(), JudgeVerdict (+117 more)

### Community 29 - "PolicyEngine"
Cohesion: 0.04
Nodes (91): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, _load(), Import the governance layer once, recording why if it is absent., _action_matches(), _as_list(), Decision (+83 more)
...
Cohesion: 0.03
Nodes (110): CEODispatcher, CEOResult, _complexity_rank(), _decompose_into_subtasks(), get_ceo_dispatcher(), _merge_changed_files(), _offload(), Any (+102 more)
...
Cohesion: 0.06
Nodes (28): field_validator, A single occurrence of a check firing on a specific URL., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoSiteFindings, PageFindings (+20 more)

### Community 41 - "resolve_active_brain"
Cohesion: 0.03
Nodes (94): allow_paid_brain(), BrainResolution, get_active_brain_sync(), get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), _norm() (+86 more)

### Community 42 - "AgentRunner"
Cohesion: 0.04
Nodes (59): AgentRunner, Any, Path, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in., Summarise a long history and compact it. Asks the planner model to write a…, Send chat messages and return the assistant's text output. If an…, Call-time resolver for an agent role model id. Delegates to… (+51 more)
...
### Community 49 - "SpecialistService"
Cohesion: 0.03
Nodes (90): Evidence, OnboardingProgress, Result of a website scan with detected systems and stack inference., Evidence supporting a system detection., Tracks the onboarding progress for a company., Result of a repository scan with detected stack and systems., Inferred technology stack from website/repo analysis., RepoScanResult (+82 more)

### Community 50 - "CompanyGraphService"
Cohesion: 0.02
Nodes (55): BusinessCategory, SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist, requires_db (+47 more)
...
Cohesion: 0.02
Nodes (69): DetectedSystem, Find a repository by its URL., A business system detected on a company's website or in their stack., Get the most confident evidence description., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo (+61 more)
...
Cohesion: 0.05
Nodes (39): Called by APScheduler when a cron fires. Dispatches to the orchestrator., _scheduler_on_fire(), OrchestratorSupervisor, Deterministic supervisor for the orchestrator. Runs as a background coroutine.…, stop_orchestrator_supervisor(), get_workflow_orchestrator(), Return the shared WorkflowOrchestrator singleton., Reset the singleton (test helper). (+31 more)
...
### Community 88 - "test_render_mcp.py"
Cohesion: 0.04
...
### Community 91 - "report_to_markdown"
Cohesion: 0.10
...
Cohesion: 0.15
Nodes (48): ApprovalRequest, BackgroundTasks, add_comment(), approve_checkpoint(), approve_execution(), clarify_task(), create_task(), _current_user() (+40 more)
...
Cohesion: 0.06
Nodes (56): AgentPhaseError, Exception, Raised when a named agent phase (planning, verification, etc.) fails., _make_runner(), MonkeyPatch, Path, Regression: a planning timeout must not surface as an empty "planning: ".…, Reproduce the exact error from the screenshot: slices present, goal absent. (+48 more)
...
### Community 130 - "test_llm_router_tpm.py"
Cohesion: 0.06
...
Cohesion: 0.07
Nodes (27): Dynamic model router package. Public API:: from router import get_router,…, _build_builtin_model_map(), _default_model(), ModelRouter, _nvidia_key_present(), Any, Dynamic model router. Central routing logic for all chat and agent requests.…, Build the built-in alias table — Nvidia NIM models when key is set, local… (+19 more)
...
Cohesion: 0.04
Nodes (42): AgentJob, AgentJobManager, make_isolated_workspace(), _now(), Any, Path, Run a job using the provided runner and update the job's lifecycle, progress,…, Create an isolated workspace directory under *root*. This is the legacy path… (+34 more)
...
### Community 157 - "AuditLog"
Cohesion: 0.05
Nodes (40): AuditEvent, AuditLog, Any, packages/governance/audit.py — the evidence trail for every governed action.…, Redact secret-shaped substrings, then truncate., One governed action, fully described. Field order follows the…, One-line JSON, suitable for a SIEM shipper tailing the log., Bounded in-memory ring buffer plus a structured log stream. The ring buffer… (+32 more)
...
Cohesion: 0.05
Nodes (68): governance_enabled(), True when the governance layer should run at all. A single global off switch…, Return the last-known health for *runtime_id* (may be stale)., Return True if the runtime is available (not circuit-open)., Return health snapshots for all known runtimes., Force an immediate health check of all runtimes and return results., Attempt to start a dead runtime subprocess before re-probing. Uses the local…, Async health polling service for all registered runtimes. (+60 more)
...
Cohesion: 0.05
Nodes (56): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Agent subsystem — planner / executor / verifier loop., AgentEvent (+48 more)

### Community 188 - "ProviderConfig"
Cohesion: 0.06
Nodes (41): provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, Infer an access tier from *hostname*, longest matching suffix winning. Returns…, _tier_from_hostname(), local_brain_enabled(), local_brain_provider_config() (+33 more)
...
Cohesion: 0.08
Nodes (44): Reset the singleton (for tests)., reset_failover_manager(), _heal_brain_failover(), _heal_stuck_tasks(), _heal_task_duplicates(), _heal_telegram(), _heal_timestamps(), Any (+36 more)
...
Cohesion: 0.09
Nodes (32): delegate_seo_findings(), _expire_stale_pending_report(), get_seo_audit(), list_seo_audits(), BaseModel, get, post, SEO / GEO / AIO Audit API Router Endpoints for the world-class SEO audit engine… (+24 more)
...
Cohesion: 0.08
Nodes (33): DirectChatDoctor, get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, _DoctorCheck, _DoctorReport, get_doctor_diagnostics(), get_doctor_report(), get_public_doctor() (+25 more)
...
### Community 243 - "BrainFailoverManager"
Cohesion: 0.09
...
### Community 250 - "_StubRuntimeManager"
Cohesion: 0.11
Nodes (13): anyio, Scheduler's on_fire handler is set to TaskAutomation.handle_scheduled_job., Calling bg.stop() twice must not raise or double-stop., A Hermes that never reports ready must not be waited on past the budget.…, start_background_services() must start the RuntimeManager., _StubDispatcher, _StubRegistry, _StubRuntimeManager (+5 more)
...
### Community 294 - "TaskHarnessAdapter"
Cohesion: 0.09
...
### Community 321 - "BrainFailoverExhausted"
Cohesion: 0.10
...
Cohesion: 0.14
Nodes (15): clean_task(), _install(), _NullDispatcher, _NullRuntimeManager, asyncio, The configured remote skill repos must be fetched without a human trigger.…, Remote skills are optional; a rate limit must not surface as an error., The guard against re-creating the very bug this fixes. ``refresh_remote``… (+7 more)
...
### Community 326 - "analyze_quantitative"
Cohesion: 0.13
...
### Community 375 - "webui/router.py"
Cohesion: 0.04
Nodes (86): AdminIdentity, _fake_user_auth(), Path, test_admin_can_create_anthropic_provider_via_webui_admin_api(), test_admin_can_create_provider_via_webui_admin_api(), test_ui_providers_and_workspaces_use_app_state(), _bootstrap(), Path (+78 more)
...
### Community 415 - "PerformanceAnalytics"
Cohesion: 0.12
...
### Community 422 - "test_telegram_service_webhook.py"
Cohesion: 0.18
...
### Community 430 - "test_self_heal.py"
Cohesion: 0.11
...
### Community 462 - "workflow/models.py"
Cohesion: 0.05
...
Cohesion: 0.12
Nodes (20): apply_overrides(), clear_override(), Persist the full override set., Write *overrides* into ``os.environ`` and refresh dependent caches. Keys that…, Re-read every ``settings`` attribute from the updated environment. Re-runs…, Validate, persist, and apply *updates*. A value equal to the environment/code…, Drop the override for *key*, reverting it to the environment default., Read the stored overrides, propagating a storage failure. Read-modify-write… (+12 more)
...
### Community 582 - "get"
Cohesion: 0.02
Nodes (79): auth_me(), auto_recommend_skills(), _check_storage_health(), cost_attribution_stats(), discover_remote_skills(), doctor_health(), get_catalog_models_route(), get_public_kpis() (+71 more)
...
### Community 633 - "test_background_services.py"
Cohesion: 0.19
Nodes (10): Return True when the web process should also run background services., run_background_in_web(), Unit tests for services/background.py — start_background_services wiring.…, RUN_BACKGROUND_IN_WEB defaults to True., The constant itself must leave real margin under Render's 5s timeout., _StubTaskAutomation, test_hermes_startup_timeout_is_well_under_renders_health_check_window(), test_run_background_in_web_default() (+2 more)
...
### Community 637 - "test_setup_api.py"
Cohesion: 0.23
...
### Community 639 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 671 - "test_task_brain_preflight.py"
Cohesion: 0.31
...
### Community 769 - "_redact_for_notification"
Cohesion: 0.24
...
Cohesion: 0.02
Nodes (156): _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, _append_agent_session_message(), AuthorizeReposBody, BrainTestRequest (+148 more)

### Community 831 - "test_p0_roadmap_b1_c2_a3.py"
Cohesion: 0.11
Nodes (14): _infer_parameters_from_func(), Infer a basic JSON Schema from a function's signature., _inject_tool_results_as_messages(), _parse_tool_calls_from_response(), Parse OpenAI tool_calls from a model response. Handles: - Direct JSON…, Inject tool call results as follow-up messages for multi-turn execution. When…, BaseModel, Result of a single reward model scoring operation. (+6 more)
...
### Community 882 - "_register_builtin_tools"
Cohesion: 0.16
Nodes (16): Register the built-in agent tools that are always available., Register the Web Reach capability (agent/web_reach.py): zero-key internet…, _register_builtin_tools(), _register_web_reach_tools(), get_web_reach(), Module-level singleton, mirroring `agent.capability_registry.get_tool_registry`., extract_real_url(), fetch() (+8 more)
...
### Community 897 - "services/seo_audit.py"
Cohesion: 0.06
Nodes (28): Aggregated report row - Screaming Frog CSV compatible., SeoIssueReportRow, compute_pressure(), _count_syllables(), estimate_pixel_width(), flesch_reading_ease(), _host_key(), is_internal() (+20 more)
...
Cohesion: 0.07
Nodes (29): _fetch_github_quick_notes(), _gh_repo(), _gh_token(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, _autonomy_bg_cycle(), Background CEO cycle + task dispatch. Runs fire-and-forget., get_store() (+21 more)
...
### Community 945 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
Cohesion: 0.06
Nodes (46): extract_openai_text(), _normalize_nvidia_base_url(), _openai_url(), Normalize NVIDIA base URLs to avoid double /v1 when openai_compat_url appends…, Registry base URLs must produce routes the provider actually serves., brain_failover must not drift from the catalog's base URLs., TestFailoverRegistryBaseUrls, _clear_probe_locks() (+38 more)

### Community 1028 - "Variants"
Cohesion: 0.40
...
### Community 1073 - "_clean_director"
Cohesion: 0.50
...
- **Why does `_fixture()` connect `_fixture` to `llm/router.py`, `test_freebuff_bot.py`, `test_provider_router.py`, `TaskStore`, `brain_config.py`, `test_operational_incidents.py`, `routed`, `TaskWorkflowService`, `test_llm_router_strategies.py`, `SelfHealingAgent`, `test_openclaw_endpoints.py`, `KeyRing`, `test_orchestrator_merge_decision.py`, `config.py`, `Task`, `AgentProfile`, `PolicyEngine`, `Agency`, `get_self_healing_agent`, `E2BSandboxSession`, `test_failover_client_shared.py`, `test_unit8_model_catalog.py`, `SeoAuditEngine`, `resolve_active_brain`, `services/background.py`, `UserRole`, `test_agent_tool_governance.py`, `SQLiteStore`, `NotificationDispatcher`, `SpecialistService`, `test_telegram_auto_approve.py`, `test_model_router.py`, `test_autonomy_status.py`, `_clean_director`, `PrimeAgentAdapter`, `test_local_brain_state.py`, `test_telegram_diag_endpoint.py`, `ChatHistoryStore`, `get_workflow_orchestrator`, `TokenBudget`, `ResearchTask`, `types.py`, `TestClient`, `e2e/test_browser.py`, `get_scheduler`, `test_brain_failover_413.py`, `test_procedural_memory.py`, `claim`, `resolve_e2b_config`, `ArtifactStore`, `test_startup_warmup.py`, `test_colibri_brain_shim.py`, `KeyStore`, `TestHarnessAdapter`, `enforcement.py`, `BrainWatchdog`, `diagnostics.py`, `test_context_rulebook.py`, `run_task`, `test_openclaw_gateway.py`, `test_provider_state_durability.py`, `test_doctor_service_token_check.py`, `test_llm_router_tpm.py`, `test_issue_intake.py`, `test_kimi_bridge_server.py`, `test_telegram_freebuff.py`, `test_new_features_e2e.py`, `test_e2b_adapter.py`, `AgentJobManager`, `MCPClient`, `_Budget`, `persist_plan_spec`, `AuditLog`, `test_task_brain_preflight.py`, `clear_cooldowns`, `test_daily_2026_07_27.py`, `test_e2b_task_wiring.py`, `OllamaCircuitBreaker`, `AgentSwarm`, `test_ceo_router.py`, `SeoFixRequest`, `emit_chat_observation`, `FeatureMaturity`, `test_sam_livekit.py`, `AgentPlan`, `test_features_api.py`, `test_video_transcript.py`, `test_agile_api.py`, `test_app_settings.py`, `SchedulerStore`, `test_task_clarification.py`, `test_e2b_data_flow.py`, `test_self_heal_v2.py`, `test_brain_availability_doctor.py`, `TrendWatcher`, `test_llm_router_disabled.py`, `test_trend_watcher.py`, `test_brain_patch_service_token.py`, `test_portfolio_intake.py`, `TestEstimateTokensForMessages`, `test_agent_free_brain.py`, `TestClient`, `ContextPruner`, `DirectChatDoctor`, `OrchestratorQueue`, `test_sam_voice.py`, `test_persistent_memory.py`, `SecurityScanner`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `OutputFilter`, `test_platform_controls.py`, `TestMongoGate`, `test_mcp_governance.py`, `tests/conftest.py`, `session_retro.py`, `test_purge_backlog.py`, `_start_ceo_agency`, `TestSwarmRoleRouting`, `analyze_page`, `test_claude_code_adapter.py`, `test_crispy_burn_in.py`, `test_provider_enable_disable.py`, `test_skill_registry_boot_refresh.py`, `isolated_telegram_config`, `test_scheduler_hydration_bounded.py`, `reap_expired_companies`, `test_rate_limiter.py`, `test_phase5_doctor.py`, `api_keys_for`, `capture_screens.py`, `test_service_token.py`, `webui/router.py`, `test_harness_spec.py`, `test_runtimes_health_endpoint.py`, `services/seo_audit.py`, `test_task_store_fails_loud_in_production.py`, `test_regression.py`, `test_agency_fix.py`, `test_dashboard_cache.py`, `platform_controls_router.py`, `OrchestratorCheckpointStore`, `test_monitor_lib.py`, `test_v4_api.py`, `model_discovery.py`, `_resolve_push_token`, `test_provider_410_per_model.py`, `test_knowledge_sync.py`, `test_telegram_service_webhook.py`, `client`, `allow_paid`, `test_ping.py`, `UsageEvent`, `test_voice_pipeline.py`, `workflow/models.py`, `test_task_service_failed_comment.py`, `_is_admin`, `TestNormalizeResponseFormat`, `test_doctor_coding_brain.py`, `get_skill_bindings`, `test_company_api.py`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `LocalWorkspace`, `proxy.py`, `direct_chat.py`, `ReactScratchpad`, `ContextManager`, `TaskSpec`, `test_e2b_adapter.py`, `test_daily_automation_2026_07_11.py`, `test_governance_api.py`, `AgentJobManager`, `MCPClient`, `ExecutionRequest`, `PolicyEngine`, `Agency`, `E2BSandboxSession`, `StuckDetector`, `test_failover_client_shared.py`, `test_ceo_dispatcher.py`, `MultiAgentSwarm`, `test_agent_tool_governance.py`, `WorkspaceTools`, `AgentPlan`, `backend/server.py`, `BrainFailoverExhausted`, `TokenBudget`, `UserMemoryStore`, `test_autonomous_agency_e2e.py`, `test_brain_failover_413.py`, `GitHubTools`, `AgentSessionStore`, `InternalAgentAdapter`, `test_agent_free_brain.py`, `test_empirical_verify.py`, `ContextPruner`, `diagnostics.py`, `loop.py`, `TestAgentLoopMCPIntegration`, `AdaptiveHalter`, `test_agent_runner.py`, `FreeBuffAgent`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `TaskStore` connect `TaskStore` to `_fixture`, `test_issue_intake.py`, `workflow.py`, `brain_config.py`, `TaskWorkflowService`, `Task`, `Agency`, `test_task_brain_preflight.py`, `services/background.py`, `backend/server.py`, `get_task_store`, `_build_default_manager`, `test_self_heal_v2.py`, `test_startup_warmup.py`, `TaskDispatcher`, `test_portfolio_intake.py`, `_ts_to_float`, `tasks/api.py`, `test_sqlite_store.py`, `test_trend_scoping.py`?**
...
- **Should `HTTPException` be split into smaller, more focused modules?**
