...
the route: none for solo; Luna / Max for execution delegation, Terra / High for routine
delegation, fresh Sol / High for complex-task audit; and one selected implementer
plus fresh Sol reviewer for full. Cache each
...
2. Observe the selected role, model, and effort through public spawn/details metadata
   ...
   Max for execution delegate/full implementation, Terra / High for routine
   delegate/full implementation, and Sol / High for complex audit/full review.
...
- `delegate`: one selected Luna / Max execution implementer or Terra / High routine implementer executes the complete
...
Use this lane only when a declared delegate or full route selects it for execution
work. The installed role pins GPT-5.6 Luna at max reasoning. It must
...
or misclassified work may justify a declared Sol / High complex-task review escalation; do not force a retry
...
~~~text
ROLE
Act as Sol Advisor's Luna / Max execution worker. Execute the supplied
...
## Terra / High - routine delegate/full implementation lane

Use this lane only when a declared delegate or full route selects routine work,
including routine follow-up revealed by a first Luna result. The installed role pins GPT-5.6 Terra at high reasoning. A
...
~~~text
ROLE
Act as Sol Advisor's Terra / High routine worker. Resolve the supplied
specification within the settled architecture, preserve every stated interface and
constraint, and surface ambiguity instead of redesigning the architecture.
