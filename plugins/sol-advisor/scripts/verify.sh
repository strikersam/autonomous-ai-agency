...
grep -Fq 'delegate uses native GPT-5.6 Luna / Max for execution work' "$manifest" || fail "manifest omits Luna execution contract"
grep -Fq 'GPT-5.6 Terra / High for routine work' "$manifest" || fail "manifest omits Terra routine contract"
...
grep -Fqi 'first Luna result' "$contracts" || fail "contracts omit Luna escalation evidence"
grep -Fqi 'Sol / High complex-task review escalation' "$contracts" || fail "contracts omit Sol complex-task escalation"
