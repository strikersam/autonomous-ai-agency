...
This guard closes that gap. It fails on drift it can prove from the files
themselves. Run it in CI and before a commit that touches any model catalogue.
...
3. HARD — a genuine provider contradiction: an id both catalogues declare, where
   a provider the gateway *also models* serves it in ``config/models.yaml`` but
   is missing from the id's ``providers`` set in ``config/llm/models.yaml``. Now
   that one id can carry several providers (``providers: [nvidia, groq]``) the
   fix is to add the provider, so this is a hard failure. A provider that lives
   only in ``config/models.yaml`` (a legacy-only provider the gateway does not
   model, e.g. the ``aerolink`` Claude gateway) is not a contradiction and is
   skipped.

Exit codes: 0 clean, 1 a check failed, 2 a file is missing or unparseable.
...
def _check_cross_catalogue(brain_doc: dict[str, Any], declared: dict[str, set[str]]) -> list[str]:
    """HARD check 3: a genuine provider contradiction between the two catalogues.

    A contradiction is an id that both catalogues declare, where a provider the
    llm catalogue *also knows* serves it under config/models.yaml but is absent
    from the id's llm-catalogue provider set — that is real drift, and the fix is
    to add the provider to the id's ``providers`` list (now that one id can carry
    several).

    A provider that appears only in config/models.yaml (a legacy-only provider
    the gateway router does not model at all — e.g. the ``aerolink`` Claude
    gateway) is *not* a contradiction: the gateway simply does not serve it. Those
    are skipped rather than flagged, so this check can be a hard failure without
    false positives.
    """
    gateway_providers: set[str] = set()
    for provs in declared.values():
        gateway_providers |= provs
    errors: list[str] = []
    for pid, spec in (brain_doc.get("providers") or {}).items():
        if pid not in gateway_providers:
            continue  # legacy-only provider — the gateway does not model it
        for mid in (spec or {}).get("candidates") or []:
            ...
            if provs and pid not in provs:
                errors.append(
                    ...
                    f"'{pid}' to its `providers` list in config/llm/models.yaml"
                )
    return errors
...
def main() -> int:
    ...
    errors = (
        _check_prefer_models(routing, declared)
        + _check_presets(brain)
        + _check_cross_catalogue(brain, declared)
    )
    ...
    if errors:
        print(f"CATALOGUE DRIFT: {len(errors)} inconsistency(ies)", file=sys.stderr)
        return 1
    print(f"CATALOGUE OK: {len(declared)} declared ids, no drift")
    return 0
