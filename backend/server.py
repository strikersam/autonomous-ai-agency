...
async def refresh_token(request: Request):
    ...
    try:
        ...
        try:
            ...
            # ObjectId() rejects plain string IDs used by the SQLite backend.
            # Fall back to a string-key lookup (mirrors get_optional_user).
...
# On Render free tier, the OpenClaw Gateway runs as a background subprocess
# inside this same container (port 18789). The FastAPI app reverse-proxies
# /openclaw/* to localhost:18789 so the iOS app pairs against
# https://local-llm-server.onrender.com/openclaw.
# If the OpenClaw CLI is not installed, the in-process fallback handles
# /openclaw/health and /api/openclaw/qr.
...
async def openclaw_qr(user: dict = Depends(get_current_user)) -> dict:
    ...
    pairing_token = _os.environ.get("OPENCLAW_PAIRING_TOKEN", "")
    if not pairing_token:
        return {"error": "OPENCLAW_PAIRING_TOKEN not set", "hint": "Set it in the Render dashboard → Environment."}
    payload = f"openclaw://pair?gateway={gateway_url}&token={pairing_token}"
    return {
        "payload": payload,
        "gateway_url": gateway_url,
...
async def scheduler_tick(request: Request):
    ...
    try:
        ...
        # run-once jobs that already fired. This prevents the 1600+ schedule
        # multiplication that happens when run-once agency tasks fail (e.g.
        # NVIDIA 410 Gone) and their schedule rows persist in the DB.
        # force_cleanup is async + idempotent — safe to call every tick.
        try:
            cleanup = await scheduler.force_cleanup()
            if cleanup.get("deleted", 0) > 0 or cleanup.get("deduped", 0) > 0:
                log.info("scheduler_tick: cleanup deleted=%d deduped=%d total=%d",
                         cleanup["deleted"], cleanup["deduped"], cleanup["total"])
        except Exception as exc:
            log.debug("scheduler_tick: force_cleanup failed (non-fatal): %s", exc)
...
try:
    import backend.render_router as render_router_module  # noqa: E402
    app.include_router(render_router_module.build_render_router(get_current_user))
except Exception as _render_router_err:  # noqa: BLE001
    log.warning("Render router not mounted: %s", _render_router_err)
