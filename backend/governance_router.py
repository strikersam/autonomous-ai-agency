...
def build_governance_router(get_current_user: Callable[..., Any]) -> APIRouter:
    ...
    async def governance_status(user: dict = Depends(get_current_user)) -> dict:
        ...
        if last_error:
            result["last_policy_error"] = last_error
            result["last_policy_error_at"] = engine.last_error_at
        return result
    ...
    async def reload_policy(user: dict = Depends(get_current_user)) -> dict:
        ...
        if last_error:
            ...
            result["error_at"] = engine.last_error_at
            result["note"] = "file present but invalid — embedded default is active"
        elif not applied:
            result["note"] = "file missing — embedded default is active"
        else:
            result["note"] = None
        return result
